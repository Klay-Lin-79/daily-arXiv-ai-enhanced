...
TL;DR: 提出了一种自监督符号自动编码器，通过符号分层过程奖励建模将图表编码为结构化基元及其关系，并解码重建图表，在重建、感知和推理任务上表现优异。
...
Method: 提出自监督符号自动编码器：1）编码器将图表编码为结构化基元及其在潜在空间中的相互关系；2）解码器通过可执行引擎重建输入图表；3）核心是符号分层过程奖励建模，应用分层步骤级解析奖励来强制执行点在线、线在形状和形状在关系的一致性；4）引入稳定机制平衡探索与利用；5）在下游任务上微调符号编码器，开发神经符号系统。

Result: 1）几何图表重建的MSE减少98.2%；2）在图表重建任务上，7B模型超越GPT-4o 0.6%；3）MathGlance感知基准提升+13%；4）MathVerse和GeoQA推理基准提升+3%。

Conclusion: 该方法通过符号分层过程奖励建模和稳定机制，成功实现了图表的结构化表示和重建，在多个视觉任务上取得了显著性能提升，展示了神经符号系统在可解释视觉理解方面的有效性。
...
TL;DR: 研究探讨了AI驱动的头部姿态校正和图像修复技术是否能提高生物特征人脸识别在真实世界图像中的准确性，发现选择性应用CFR-GAN和CodeFormer组合能有效提升识别性能。


<details>
  <summary>Details</summary>
Motivation: 生物特征人脸识别模型在处理真实世界图像（质量差、非正面姿态、遮挡）时准确性显著下降，需要研究针对性修复技术是否能改善识别效果。

Method: 使用模型无关的大规模法证评估流程，评估三种修复方法：3D重建（NextFace）、2D正面化（CFR-GAN）和特征增强（CodeFormer），并研究选择性应用策略。

Result: 直接应用这些技术会显著降低人脸识别准确性，但选择性应用CFR-GAN结合CodeFormer能带来有意义的改进。

Conclusion: AI驱动的头部姿态校正和图像修复技术需要选择性应用策略，CFR-GAN与CodeFormer的组合能有效提升真实世界人脸识别准确性。
...
TL;DR: Flux4D是一个简单可扩展的4D动态场景重建框架，无需监督标注，直接预测3D高斯及其运动动态，通过光度损失和"尽可能静态"正则化实现大规模动态场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有可微分渲染方法（如NeRF和3DGS）在大规模动态场景重建中存在可扩展性限制，需要标注来解耦运动；现有自监督方法依赖逐场景优化且对超参数敏感，需要开发更简单、可扩展的无监督4D重建框架。

Method: Flux4D采用完全无监督方式，直接预测3D高斯及其运动动态来重建传感器观测。仅使用光度损失并强制"尽可能静态"的正则化，无需预训练监督模型或基础先验，通过跨多个场景训练直接从原始数据中分解动态元素。

Result: 在室外驾驶数据集上的实验表明，Flux4D在可扩展性、泛化能力和重建质量方面显著优于现有方法，能够在几秒内高效重建动态场景，并能有效扩展到大型数据集，泛化到未见环境包括罕见和未知物体。

Conclusion: Flux4D提供了一个简单、可扩展的4D动态场景重建框架，通过无监督学习实现了大规模动态场景的高效重建和良好泛化，解决了现有方法在可扩展性和标注依赖方面的限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本物体计数方法通常需要大量标注数据和视觉示例，而大语言模型具有强大的推理和数据理解能力，可以探索无监督的计数方法

Method: 利用GPT-4o和GPT-5的多模态视觉能力，仅通过文本提示进行零样本物体计数，无需任何监督训练或视觉示例

Result: 在FSC-147和CARPK数据集上的评估显示，模型性能与最先进的零样本方法相当，在某些情况下甚至超越它们

Conclusion: 多模态大语言模型可以在无监督的情况下有效执行物体计数任务，为物体计数提供了新的零样本方法
...
TL;DR: 提出基于大语言模型的两阶段方法，从带有粗略分割的3D点云直接推断材料组成，将物体识别与材料分配解耦，实现零样本推理。
...
Result: 该方法不仅准确实用，而且在多个低光去噪基准测试中取得了最先进的性能

Conclusion: 提出了一种通用且实用的噪声合成方法，仅需最少的数据（单张噪声图像和单张暗帧）即可生成真实的传感器噪声，为低光图像去噪训练提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 潜在扩散模型在图像修复和局部编辑方面取得了显著进展，但其固有的潜在压缩会引入像素级不一致问题，如色移、纹理不匹配和编辑边界处的可见接缝。现有的修复方法在实践中通常无法完全消除这些伪影，并且不能很好地泛化到不同的潜在表示或任务中。

Method: PixPerfect框架包含三个核心组件：(1) 可微分的判别像素空间，用于放大和抑制细微的颜色和纹理差异；(2) 全面的伪影模拟管道，在训练期间将细化器暴露于真实的局部编辑伪影；(3) 直接像素空间细化方案，确保在不同潜在表示和任务中的广泛适用性。
...
TL;DR: 本文系统评估了多种深度学习架构用于多类火灾分类，并提出了PyroFocus两阶段管道，在NASA MASTER数据上实现了速度与精度的良好平衡，适合实时边缘部署。
...
Method: 系统评估了多种深度学习架构（包括自定义CNN和Transformer模型）用于多类火灾分类，并提出了PyroFocus两阶段管道：先进行火灾分类，再进行火辐射功率回归或分割，以减少推理时间和计算成本。

Result: 使用NASA的MASTER数据（类似于下一代火灾检测传感器）进行实验，结果表明两阶段管道在速度与精度之间实现了良好的平衡，显示出在实时边缘部署方面的显著潜力。

Conclusion: 提出的两阶段管道在速度和准确性之间取得了强有力的权衡，展示了在未来野火监测任务中实时边缘部署的重要潜力。
...
TL;DR: H²U3D是一个用于房屋尺度3D场景理解的数据集，包含多楼层环境，通过自动化标注流程生成层次化视觉表示和多样化问答对。SpatialReasoner是一个主动感知框架，通过两阶段训练策略实现高效的空间探索，在减少图像使用量的同时达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要局限于房间尺度的空间推理，无法有效处理大规模3D环境（如多楼层房屋）。需要开发能够理解房屋尺度场景的3D视觉问答数据集和相应的推理方法。

Method: 1. 提出H²U3D数据集：包含多楼层房屋环境（最多3层，10-20个房间，>300m²），通过自动化标注流程构建从粗到细的层次化视觉表示，生成多样化问答对并带有思维链标注。
2. 提出SpatialReasoner框架：主动感知框架，基于文本查询自主调用空间工具探索3D场景。采用两阶段训练策略：监督式冷启动，然后通过强化学习配合自适应探索奖励（鼓励高效探索，抑制冗余操作）。

Result: SpatialReasoner在H²U3D数据集上达到最先进性能，优于GPT-4o和Gemini-2.5-Pro等强基线。特别值得注意的是，该方法平均仅使用3-4张图像，而基线方法需要16+张图像，证明了从粗到细的主动探索范式的有效性。

Conclusion: H²U3D数据集填补了房屋尺度3D场景理解的数据空白，SpatialReasoner框架通过主动感知和高效探索策略，在减少观察成本的同时实现了优越的空间推理性能，为大规模3D环境理解提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要准确的环境表示，传统HD地图需要在线构建。虽然导航级SD地图广泛可用但分辨率不足，可作为粗略先验指导在线地图构建

Method: 提出NavMapFusion扩散框架，通过迭代去噪过程，以高保真传感器数据和低保真导航地图为条件，将先验地图与在线感知的差异对应为扩散过程中的噪声

Result: 在nuScenes基准测试中，基于OpenStreetMap道路线作为条件，在100米范围内实现21.4%的相对改进，更大感知范围改进更显著，同时保持实时能力
...
TL;DR: 提出了一种新的逐步分层设计生成方法SLEDGE，将设计过程建模为基于指令的原子层变化序列，解决了现有方法将设计合成视为单步生成的问题。


<details>
  <summary>Details</summary>
Motivation: 设计本质上是逐步迭代的过程，但现有方法主要将设计合成视为单步生成问题，严重低估了创意过程的复杂性，需要更符合实际设计流程的方法。

Method: 提出SLEDGE（逐步分层设计生成器），利用多模态大语言模型，将每个设计更新建模为基于指令的前一状态的原子层变化，并引入了新的评估套件包括数据集和基准。

Result: 通过详尽的实验分析和与最先进方法的比较，证明了所提方法的有效性，展示了在逐步分层设计生成任务上的优越性能。

Conclusion: 该工作提出了一个实用且未被充分探索的研究方向，希望吸引更多关注到逐步分层设计生成这一领域，为更符合实际设计流程的AI辅助设计系统奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 射血分数是评估心脏功能的关键指标，传统方法需要人工追踪且耗时，现有深度学习模型多为黑盒模型，缺乏透明度，降低了临床信任度。虽然已有事后解释方法，但这些方法不指导模型内部推理，在临床应用中的可靠性有限。

Method: 提出ProtoEFNet模型，采用视频原型学习方法进行连续EF回归。模型学习动态时空原型来捕捉临床有意义的心脏运动模式，并提出原型角度分离损失函数，在连续EF谱上强制区分性表示。

Result: 在EchonetDynamic数据集上的实验表明，ProtoEFNet在保持与非可解释模型相当准确性的同时，提供临床相关洞察。消融研究显示，提出的损失函数将F1分数从77.67±2.68提升到79.64±2.10，提高了2%。

Conclusion: ProtoEFNet通过原型学习方法实现了连续EF回归，在保持预测准确性的同时提供临床可解释性，解决了黑盒模型在临床应用中的信任问题，为心脏功能评估提供了更可靠的深度学习解决方案。
...
TL;DR: HalluGen是一个基于扩散模型的框架，用于生成可控类型、位置和严重程度的逼真幻觉，构建了首个大规模幻觉数据集，用于评估安全关键图像恢复中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 生成模型在图像恢复中容易产生幻觉（看似合理但实际错误的结构），这在医疗影像、工业检测等安全关键领域尤其危险，但评估幻觉需要标注数据，而标注成本高且主观，阻碍了研究进展。

Method: 提出HalluGen框架，基于扩散模型合成具有可控类型、位置和严重程度的逼真幻觉；构建包含4,350张标注图像的大规模幻觉数据集（源自1,450张脑部MR图像）；开发SHAFE（基于特征评估的语义幻觉评估）指标，使用软注意力池化提高幻觉敏感性。

Result: HalluGen能生成感知逼真但语义错误的输出（分割IoU从0.86降至0.36）；构建了首个大规模幻觉数据集；SHAFE指标相比传统指标提高了幻觉敏感性；训练的无参考幻觉检测器能泛化到真实恢复失败场景。

Conclusion: HalluGen及其开放数据集为安全关键图像恢复中的幻觉评估建立了首个可扩展的基础，有助于开发更可靠的图像恢复系统。
...
TL;DR: SeeU提出了一种新的2D→4D→2D学习框架，通过从稀疏的单目2D帧重建4D世界，学习连续4D动态，然后重新投影回2D，实现连续且物理一致的新视觉内容生成。


<details>
  <summary>Details</summary>
Motivation: 大多数视觉理解、预测和生成直接在2D观测上进行，导致性能不佳。图像和视频是4D世界（3D空间+时间）的离散2D投影，需要从2D观测中学习连续4D动态来生成未见过的视觉内容。

Method: 1. 从稀疏单目2D帧重建4D世界（2D→4D）；2. 在低秩表示和物理约束下学习连续4D动态（离散4D→连续4D）；3. 将世界向前推进时间，在采样时间和视点重新投影回2D，基于时空上下文意识生成未见区域（4D→2D）。
...
<details>
  <summary>Details</summary>
Motivation: 现有野火预测研究主要基于粗时空尺度和低分辨率卫星数据，只能捕捉宏观火情，限制了高精度局部火势动态建模能力，需要更高分辨率的细粒度预测方法

Method: 1) 创建FireSentry省级尺度多模态野火数据集，包含亚米级空间分辨率和亚秒级时间分辨率数据；2) 提出FiReDiff双模态范式，先在红外模态预测未来视频序列，再基于生成的动态在掩码模态精确分割火势掩码
...
Conclusion: FireSentry基准数据集和FiReDiff范式共同推进了细粒度野火预测和动态灾害模拟，为高精度应急响应和决策提供了重要工具
...
TL;DR: ShelfGaussian是一个开放词汇的多模态高斯3D场景理解框架，利用现成的视觉基础模型进行监督，在零样本语义占用预测和真实世界机器人应用中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有高斯方法存在局限性：要么使用带注释的3D标签监督封闭集语义高斯，忽略了渲染能力；要么通过纯2D自监督学习开放集高斯表示，导致几何质量下降且仅限于相机设置。需要充分利用高斯方法的潜力。

Method: 提出多模态高斯变换器，使高斯能够从不同传感器模态查询特征；采用货架监督学习范式，在2D图像和3D场景级别联合优化高斯特征；利用现成的视觉基础模型进行监督。

Result: 在Occ3D-nuScenes上实现了最先进的零样本语义占用预测性能；在无人地面车辆上评估了其在各种城市场景中的野外性能，表现出色。

Conclusion: ShelfGaussian通过多模态特征查询和货架监督学习，成功将高斯方法扩展到开放词汇3D场景理解，在感知和规划任务中展现出强大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 光学与合成孔径雷达(SAR)图像之间的模态差异给跨模态船舶重识别带来了巨大挑战，现有方法难以有效处理这种模态鸿沟

Method: 提出MOS框架，包含两个核心组件：1) 模态一致表示学习(MCRL)，通过去噪SAR图像处理和类级模态对齐损失来对齐跨模态的特征分布；2) 跨模态数据生成与特征融合(CDGF)，利用布朗桥扩散模型合成跨模态样本，在推理时与原始特征融合以增强对齐和区分性
...
Conclusion: MOS框架通过模态对齐和跨模态数据生成有效缓解了光学-SAR模态鸿沟，为跨模态船舶重识别提供了有效的解决方案，代码和模型将在发表后开源
...
TL;DR: 论文提出了视频差异描述任务ViDiC和对应的ViDiC-1K数据集，用于评估多模态大语言模型在视频对之间相似性和差异性的细粒度描述能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像差异描述方法无法捕捉动态场景中的运动连续性、事件演化和编辑一致性，视觉语言系统在动态场景的比较感知能力方面研究不足。

Method: 1. 引入ViDiC任务和ViDiC-1K数据集（包含1000个视频对，超过4000个比较检查项，涵盖7个类别）；2. 提出基于LLM-as-a-Judge协议的双检查表框架，分别测量相似性和差异性的准确性。

Result: 对19个代表性多模态模型的实验显示，它们在比较描述和差异感知能力方面存在显著的性能差距。

Conclusion: ViDiC-1K可以作为一个具有挑战性的基准，为推进视频理解、编辑感知和多模态智能中的比较推理奠定坚实基础。
...
TL;DR: 提出一种基于冻结扩散模型的高光谱图像标签高效分类框架，通过提取扩散模型早期去噪步骤的空间特征，结合轻量级FiLM融合模块整合光谱信息，在稀疏标注下实现优于现有方法的高光谱土地覆盖分类。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像（HSI）能够进行详细的土地覆盖分类，但面临空间分辨率低和标注稀疏的挑战。现有方法需要大量标注数据，而高光谱数据标注成本高昂。因此需要开发标签高效的框架，利用预训练模型的知识来减少对标注数据的依赖。

Method: 1. 利用在自然图像上预训练的冻结扩散模型提取空间特征：从早期去噪时间步的高分辨率解码器层提取低层表示，这些特征能有效迁移到HSI的低纹理结构中。2. 引入轻量级FiLM（Feature-wise Linear Modulation）融合模块：自适应地使用光谱线索调制冻结的空间特征，实现光谱和空间信息的有效整合。3. 在稀疏监督下进行鲁棒的多模态学习。

Result: 在两个最新的高光谱数据集上的实验表明，该方法仅使用提供的稀疏训练标签就超越了最先进的方法。消融研究进一步突出了扩散衍生特征和光谱感知融合的优势。具体性能指标优于现有SOTA方法。

Conclusion: 预训练的扩散模型可以支持领域无关的、标签高效的表征学习，不仅适用于遥感任务，还可扩展到更广泛的科学成像任务。该方法为高光谱图像分类提供了一种有效的标签高效解决方案。
...
TL;DR: LM-CartSeg是一个全自动膝关节MRI分割和放射组学分析流程，通过几何规则改进分割质量，实现稳定的内外侧分区，提取的放射组学特征包含超出简单形态学的判别信息。


<details>
  <summary>Details</summary>
Motivation: 膝关节MRI放射组学研究需要稳健、解剖学上有意义的ROI区域，现有方法多依赖手动标注且缺乏质量控制。本研究旨在开发一个全自动的软骨/骨分割、内外侧分区和放射组学分析流程。

Method: 训练两个3D nnU-Net模型，测试时融合零样本预测并通过几何规则优化：连通分量清理、构建10mm软骨下骨带、基于PCA和k-means的胫骨内外侧分割。使用体积和厚度特征进行质量控制，从10个ROI提取4650个非形状放射组学特征。

Result: 后处理显著改善分割质量：OAIZIB-CM测试集上ASSD从2.63降至0.36mm，HD95从25.2降至3.35mm，DSC达0.91；SKI-10零样本DSC为0.80。几何内外侧规则在不同数据集上产生稳定分区，仅6-12%的特征与体积/厚度强相关。

Conclusion: LM-CartSeg提供自动化的质量控制ROI和放射组学特征，这些特征包含超出简单形态测量的判别信息，为多中心膝关节OA放射组学研究提供了实用基础。
...
TL;DR: 提出无监督学习框架，从点云数据中学习具有空间结构的3D关键点，作为紧凑可解释的表征来条件化扩散模型进行形状重建
...
Method: 从点云数据无监督学习空间结构化的3D关键点，将其作为紧凑可解释的表征，用于条件化Elucidated Diffusion Model (EDM)进行完整形状重建

Result: 学习到的关键点在不同物体实例间具有可重复的空间结构，支持关键点空间的平滑插值，表明其能捕捉几何变化；在多样物体类别上表现优异，关键点一致性比先前方法提高6个百分点

Conclusion: 提出的无监督框架成功弥合了无监督关键点学习与无条件生成设置之间的差距，为3D生成提供了紧凑且可解释的表征方法
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法主要在2D像素空间操作，缺乏明确的3D结构建模机制，导致时间几何不一致、运动不合理和结构伪影等问题。需要将3D几何建模与外观生成相结合。

Method: 在潜在扩散模型中引入逐帧深度预测作为几何表示，并提出多视角几何损失，在共享3D坐标系中对齐跨帧的预测深度图，强制实现时间结构一致性。

Result: 在多个数据集上的实验表明，该方法相比现有基线能产生显著更稳定和几何一致的结果，改善了时空连贯性、形状一致性和物理合理性。

Conclusion: 通过深度预测的几何正则化损失成功弥合了外观生成与3D结构建模之间的差距，为视频生成提供了更好的几何一致性解决方案。
...
Method: 提出ThinkDeeper框架，核心是空间感知世界模型（SA-WM），它将当前场景提炼为指令感知的潜在状态，并展开未来潜在状态序列；配合超图引导解码器，分层融合这些状态与多模态输入，捕捉高阶空间依赖关系。

Result: 在六个基准测试中，ThinkDeeper在Talk2Car排行榜排名第一，在DrivePilot、MoCAD和RefCOCO/+/g基准上超越现有最佳方法；在挑战性场景（长文本、多智能体、模糊性）中表现出强鲁棒性和效率，即使使用50%数据训练也能保持优越性能。

Conclusion: ThinkDeeper通过引入世界模型原理，实现了对未来空间状态的推理，显著提升了自动驾驶中自然语言指令定位的准确性和鲁棒性。
...
TL;DR: TPI（文本打印图像）方法通过将文本直接渲染到白色画布上生成合成图像，实现低成本文本中心训练，在多个模型和基准测试中优于扩散模型生成的图像。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型需要大量图像-文本对进行任务特定微调，但图像收集成本高且受隐私限制。文本数据广泛可用且易于编辑，但纯文本训练存在模态差距问题。

Method: 提出TPI（文本打印图像）方法，将文本描述直接渲染到纯白色画布上生成合成图像，将文本投影到图像模态，可低成本集成到现有LVLM训练流程中。

Result: 在4个模型和7个基准测试中，TPI比扩散模型生成的合成图像更有效地支持文本中心训练，并可作为低成本数据增强策略展示实用价值。

Conclusion: TPI展示了文本中心训练的巨大潜力，为LVLMs的完全自动化数据生成开辟了路径，提供了一种可扩展、低成本的训练方法。
...
TL;DR: 本文提出了动作效果建模（AEM）框架，通过联合建模动作执行和其产生的效果来改进程序性任务中的错误检测，在EgoPER和CaptainCook4D基准测试中取得了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要分析动作如何执行，而忽略了动作产生的效果（如物体状态或空间排列的变化）。然而许多错误并不体现在执行过程中，而是体现在产生的结果中。需要同时考虑动作执行和效果来更可靠地检测错误。
...
Result: 在EgoPER和CaptainCook4D基准测试的单类分类（OCC）设置下取得了最先进的性能，证明了同时建模执行和效果能实现更可靠的错误检测。

Conclusion: 建模动作执行和效果能显著提升程序性任务中的错误检测性能，效果感知表示有潜力应用于更广泛的下游任务。
...
TL;DR: 提出公平感知的低秩适应方法，通过MaxAccGap损失函数优化医学视觉语言模型在不同人口群体间的诊断准确性差异，在仅需0.24%可训练参数的情况下显著减少公平性差距。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在不同人口群体间存在显著的诊断准确性差异，这限制了其在医疗实践中的公平应用。需要开发参数高效且能明确优化公平性的方法来减少这些差异。

Method: 提出公平感知的低秩适应方法，包括三种变体：FR-LoRA（集成MaxAccGap正则化）、GR-LoRA（应用逆频率加权平衡梯度贡献）和Hybrid-LoRA（结合两种机制）。核心是微分MaxAccGap损失函数，实现跨人口群体准确性平等的端到端优化。

Result: 在10,000张青光眼眼底图像上评估，GR-LoRA将诊断准确性差异减少69%，同时保持53.15%的整体准确性。种族特定优化实现60%的差异减少。强正则化强度在最小准确性折衷下实现最优公平性。

Conclusion: 该方法仅需0.24%可训练参数，实现了参数效率与公平性优化的结合，为资源受限的医疗环境中部署公平的医学AI提供了实用解决方案。
...
TL;DR: 该论文提出了Object-IVQA基准测试，用于评估AI在长视频中基于对象状态变化进行程序性活动理解的能力，并开发了一个对象中心的智能体框架来提升相关推理性能。
...
Method: 提出Object-IVQA基准测试（包含107个长视频和514个开放性问题），并开发了一个对象中心智能体框架，整合规划、感知、分析和生成工具，支持显式证据检索和跨片段的多跳推理。

Result: 实验显示现有大型视觉语言模型在对象级识别和推理方面表现不佳，而提出的框架实现了显著改进。

Conclusion: 对象中心范式能更好地理解程序性活动，提出的基准测试和框架为开发能够处理复杂现实任务推理的辅助AI提供了重要方向。
...
TL;DR: NAS-LoRA：一种新的参数高效微调方法，通过轻量级神经架构搜索块增强SAM在专业领域的适应能力，减少训练成本24.14%且不增加推理成本


<details>
  <summary>Details</summary>
Motivation: SAM作为视觉基础模型在图像分割方面表现出色，但适应特定下游任务（如医疗和农业成像）仍具挑战。现有LoRA方法虽广泛使用，但SAM的Transformer编码器缺乏图像块内的空间先验，可能阻碍高级语义信息的获取，需要将归纳偏置集成到模型中。

Method: 提出NAS-LoRA方法：1）在LoRA的编码器和解码器组件之间加入轻量级神经架构搜索块，动态优化集成到权重更新中的先验知识；2）采用分阶段优化策略，帮助ViT编码器平衡权重更新和架构调整，逐步学习高级语义信息。

Result: 实验表明NAS-LoRA改进了现有PEFT方法，同时将训练成本降低24.14%且不增加推理成本，证明了NAS在增强视觉基础模型PEFT方面的潜力。

Conclusion: NAS-LoRA通过集成神经架构搜索到参数高效微调框架中，有效弥合了预训练SAM与专业领域之间的语义差距，为视觉基础模型在特定下游任务中的适应提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型（VLMs）的领域泛化语义分割方法存在视觉与文本上下文语义不对齐的问题，这源于在单一源域上学习的固定上下文提示的僵化性。

Method: 提出了DPMFormer框架：1）引入领域感知提示学习来促进视觉与文本线索的语义对齐；2）提出领域感知对比学习和纹理扰动来多样化可观察的领域；3）提出领域鲁棒一致性学习来最小化原始图像与增强图像预测的差异。

Result: 该框架在多个领域泛化语义分割基准测试中表现出优越性，建立了新的最先进性能。

Conclusion: DPMFormer通过解决视觉-文本语义不对齐问题，并引入领域感知学习和一致性约束，有效提升了语义分割模型在未见领域的泛化能力。
...
TL;DR: 该研究提出一个基于计算机视觉的舞蹈动作分析框架，结合YOLOv8/v11进行舞者检测和Segment Anything Model进行精确分割，实现了无标记的舞蹈动作量化分析。


<details>
  <summary>Details</summary>
Motivation: 开发无需专业设备或标记的自动化舞蹈动作分析方法，利用当代计算机视觉技术实现舞蹈动作的量化评估。

Method: 整合YOLOv8/v11进行舞者检测，结合Segment Anything Model进行像素级分割，通过视频分析追踪舞者动作、计算舞步数量、空间覆盖模式和节奏一致性。

Result: 在加纳AfroBeats舞蹈的49秒视频测试中，系统达到约94%的检测精度和89%的召回率，SAM分割的IoU约为83%。分析显示主要舞者比次要舞者多执行23%的舞步，运动强度高37%，使用空间多42%。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖分块处理或带有扩散头的自回归模型，无法有效处理时间变化的文本提示下的流式运动生成任务。需要一种能够建模真实运动分布并实现实时生成的框架。

Method: 采用扩散强迫框架来建模时间序列生成任务，但发现原始扩散强迫方法无法建模真实运动分布。为此进行了三项改进：1) 使用双向注意力而非因果注意力进行训练；2) 实现下三角时间调度器而非随机调度器；3) 采用连续时间变化的方式引入文本条件。

Result: 在流式运动生成任务上首次实现了基于扩散强迫框架的最先进性能，在HumanML3D基准测试中达到0.057的FID分数，能够生成文本对齐的无缝运动序列，并实现实时延迟。

Conclusion: FloodDiffusion通过改进的扩散强迫框架成功解决了文本驱动的流式人体运动生成问题，在保持实时性能的同时达到了最先进的生成质量，为时间变化控制事件下的时间序列生成提供了有效解决方案。
...
TL;DR: PRIS框架通过在推理时自适应地重新设计提示词来改进文本到视觉生成，通过分析生成结果中的失败模式并基于细粒度验证反馈修订提示词，显著提升了生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要通过增加采样步数或种子数量来扩展视觉生成过程，但这种方法很快会遇到质量瓶颈，因为提示词在整个过程中保持不变。用户意图与生成视觉之间的精确对齐仍然是文本到视觉生成的核心挑战。

Method: 提出PRIS框架，在推理时自适应地修订提示词：1）审查生成的视觉内容；2）识别跨视觉内容的重复失败模式；3）根据分析结果重新设计提示词；4）使用修订后的提示词重新生成视觉内容。引入元素级事实校正验证器，在细粒度层面评估提示词属性与生成视觉之间的对齐程度。

Result: 在文本到图像和文本到视频基准测试中表现出显著效果，在VBench 2.0上获得了15%的性能提升。实验结果表明，联合扩展提示词和视觉内容是充分利推理时扩展定律的关键。

Conclusion: PRIS框架通过自适应地重新设计提示词，有效解决了文本到视觉生成中的对齐问题。该方法突破了传统仅扩展视觉生成过程的局限性，证明了提示词与视觉内容联合扩展的重要性，为推理时优化提供了新思路。
...
TL;DR: CartoMapQA是一个专门评估视觉语言模型在地图理解能力上的基准测试，包含2000多个地图问答样本，涵盖从低到高三个层次的地图解读技能，揭示了当前模型在地图语义理解、空间推理和OCR识别方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在视觉和文本信息融合方面取得了进展，但它们在地图解读方面的能力尚未得到充分探索。地图理解对于导航、地理搜索和城市规划等实际应用至关重要，因此需要专门的基准测试来评估和改进模型的地图理解能力。

Method: 研究者创建了CartoMapQA基准测试，包含2000多个样本，每个样本由地图图像、问题（开放式或多选题）和真实答案组成。任务涵盖三个层次的地图解读技能：低层（符号识别）、中层（信息提取和比例尺解读）和高层（基于路线的推理）。该研究评估了开源和专有的视觉语言模型。

Result: 评估结果显示当前视觉语言模型存在持续挑战：模型经常难以理解地图特定语义，空间推理能力有限，并且容易产生OCR相关错误。这些弱点表明现有模型在地图理解方面仍有显著不足。

Conclusion: CartoMapQA通过揭示视觉语言模型在地图理解方面的具体弱点，为未来模型架构改进提供了有价值的工具。该基准测试支持开发更适合实际应用的模型，如导航、地理搜索和城市规划等需要强大可靠地图理解能力的场景。研究代码和数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 烹饪是一个顺序性和视觉基础的活动，每个步骤都包含程序逻辑和视觉语义。现有的扩散模型在处理结构化多步骤场景（如食谱插图）时存在困难，且当前方法无法适应食谱长度的自然变化，无论实际指令结构如何都生成固定数量的图像。
...
TL;DR: 研究发现视觉压缩在文本重建方面并不优于简单替代方法，且在语言建模中表现不佳，质疑了视觉上下文压缩的实际价值


<details>
  <summary>Details</summary>
Motivation: DeepSeek-OCR展示了从少量视觉token重建渲染文本的能力，引发了视觉上下文压缩的兴奋。但现有评估仅停留在重建层面，未测试这些表示是否真正有助于语言建模

Method: 将DeepSeek-OCR的视觉编码器与两种简单替代方法比较：参数无关的平均池化和学习的分层编码器，在匹配的压缩比下评估文本重建和语言建模性能

Result: 简单方法在文本重建方面匹配或超越视觉方法，在语言建模方面表现更优，而视觉压缩甚至无法击败简单的截断方法

Conclusion: 对光学上下文压缩的兴奋超过了实际证据，视觉压缩在文本重建和语言建模方面并不优于简单替代方案
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现出色，但存在幻觉问题，产生与输入视觉内容不一致的输出，这在精度敏感领域严重影响可靠性。现有方法通常通过干预注意力分数或输出logits来缓解幻觉，但只关注"如何干预"而忽略了"何时干预"的前提，导致"过度干预"问题，引入新的幻觉和不必要的计算开销。

Method: 提出V-ITI框架：1）视觉忽视检测器：通过头级判别探针识别视觉忽视现象；2）视觉回忆干预器：仅在检测到视觉忽视时，使用预存储的视觉激活信息来调制激活。该框架首先研究视觉忽视机制，发现可以通过MLLMs中的头级激活模式准确检测。

Result: 在八个基准测试和不同MLLM家族上的广泛实验表明，V-ITI能够持续缓解与视觉相关的幻觉，同时保持一般任务性能。

Conclusion: V-ITI通过智能检测视觉忽视并仅在必要时进行干预，有效解决了多模态大语言模型的幻觉问题，避免了过度干预带来的新幻觉和计算开销。
...
TL;DR: CodeVision是一个基于代码作为工具的MLLM框架，通过生成代码作为通用接口调用任意图像操作，解决了现有MLLM在图像方向变化和自然损坏下的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM依赖有限的工具集，缺乏真实世界必要性和可扩展性，且对图像方向变化和自然损坏表现出显著的性能下降，需要更鲁棒的工具推理方法。

Method: 提出CodeVision框架，将代码作为通用工具接口；采用两阶段训练方法：先在高质量数据集上进行监督微调，然后使用带有密集过程奖励函数的强化学习来鼓励策略性工具使用。

Result: 在Qwen2.5-VL和Qwen3-VL系列上的实验表明，该方法显著提升模型性能，并涌现出灵活工具组合、高效链式执行和从运行时反馈中鲁棒错误恢复等能力。

Conclusion: CodeVision提供了一个灵活可扩展的代码作为工具框架，有效解决了MLLM的鲁棒性问题，并通过新构建的数据集和基准测试推动了该领域的研究。
...
TL;DR: 提出混合内容审核框架，结合监督分类和相似度匹配，用于直播平台的多模态内容审核，在保持高精度的同时提升召回率


<details>
  <summary>Details</summary>
Motivation: 大规模用户生成视频平台的内容审核面临挑战，特别是在直播环境中需要及时、多模态且能应对不断演化的不良内容。传统方法难以处理新颖或微妙的不良内容

Method: 混合审核框架：监督分类管道处理已知违规，基于参考的相似度匹配管道处理新颖或微妙案例。多模态输入（文本、音频、视觉）通过两个管道处理，使用多模态大语言模型将知识蒸馏到每个管道中，在保持推理轻量的同时提升准确性

Result: 生产环境中，分类管道在80%精度下达到67%召回率，相似度管道在80%精度下达到76%召回率。大规模A/B测试显示用户观看不良直播的比例减少6-8%

Conclusion: 该混合框架提供了一种可扩展且适应性强的多模态内容治理方法，能够同时处理明确违规和新兴对抗行为
...
TL;DR: AdaptVision是一种高效视觉语言模型，通过粗到细的自适应视觉标记获取机制，让模型能自主决定每个样本所需的最小视觉标记数量，显著减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型依赖大量视觉标记导致计算开销大，而现有高效方法采用固定比例压缩，缺乏根据任务需求自适应调整的能力。受人类主动视觉机制启发，需要让模型能自主决定每个样本所需的最小视觉标记数量。
...
Result: 在多个VQA基准测试上的综合实验表明，AdaptVision在消耗显著少于现有高效VLM方法的视觉标记的同时，实现了更优的性能。

Conclusion: AdaptVision通过自适应视觉标记获取机制，使视觉语言模型能够根据任务需求自主决定所需视觉信息量，在保持准确性的同时显著提高计算效率，为高效VLM提供了新范式。
...
TL;DR: 本文提出Stable Signer模型，将手语生成重新定义为包含文本理解和姿态到视频的端到端分层生成任务，通过SLUL链接器和SLP-MoE手势渲染专家块，性能比当前SOTA方法提升48.6%


<details>
  <summary>Details</summary>
Motivation: 传统手语生成方法存在文本转换不准确、姿态生成和视频渲染误差累积等问题，导致该领域进展缓慢。需要简化冗余结构，优化任务目标。

Method: 1. 将SLP任务重新定义为端到端分层生成任务，仅包含文本理解（Prompt2Gloss, Text2Gloss）和Pose2Vid阶段；2. 提出SLUL（手语理解链接器）执行文本理解，使用新开发的SAGM损失训练；3. 设计SLP-MoE手势渲染专家块生成手势；4. 整体架构可端到端生成高质量多风格手语视频。

Result: 提出的Stable Signer模型性能比当前最先进生成方法提升48.6%，能够生成高质量、多风格的手语视频。

Conclusion: 通过简化传统冗余结构、优化任务目标，提出的Stable Signer模型有效解决了手语生成中的误差累积问题，显著提升了生成质量和性能。
...
TL;DR: GAOT是一个从文本提示生成3D关节物体的三阶段框架，通过扩散模型和超图学习实现文本到关节物体的生成


<details>
  <summary>Details</summary>
Motivation: 现有关节物体生成模型往往无法基于文本提示进行条件生成，文本描述与3D关节物体表示之间存在显著差距

Method: 三阶段框架：1) 微调点云生成模型从文本生成粗略物体表示；2) 设计基于超图的学习方法精炼粗略表示，将物体部件表示为图顶点；3) 利用扩散模型基于物体部件生成关节（表示为图边）
...
TL;DR: GLASTE是一个端到端的场景文本编辑框架，通过全局-局部感知设计解决了现有方法的不一致性和长度不敏感问题，在保持文本样式和背景纹理的同时，能够处理编辑前后文本长度差异大的情况。
...
Method: 提出了GLASTE框架，采用全局-局部组合结构，结合全局和局部损失，增强文本图像特征。将文本样式表示为与图像大小无关的向量，可转移到不同大小的目标文本图像。使用仿射融合将目标文本图像填充到编辑补丁中，同时保持其纵横比不变。

Result: 在真实世界数据集上的大量实验验证了GLASTE模型在定量指标和定性结果上都优于先前方法，并有效缓解了不一致性和长度不敏感两个挑战。

Conclusion: GLASTE通过同时结合高层全局上下文信息和精细局部特征，成功解决了场景文本编辑中的关键问题，为实际应用提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于注意力的压缩方法存在局限，需要从信息论角度设计新的视频压缩框架，在有限计算预算下最大化信息保真度

Method: 提出信息独特性概念衡量token内在冗余，设计三个模块：帧组融合、token分配和空间动态压缩，逐步进行语义帧分组、自适应资源分配和细粒度空间压缩

Result: 大量实验表明UniComp在有限计算预算下持续优于现有压缩方法，能更好地保留关键视觉token

Conclusion: 信息独特性在token压缩效果中起关键作用，UniComp框架为视频压缩提供了新的信息论视角
...
TL;DR: 本文提出了一种跨染色对比学习框架CSCL，通过构建五染色数据集和两阶段预训练方法，提升WSI表示的质量和可迁移性。


<details>
  <summary>Details</summary>
Motivation: 计算病理学需要通用、可迁移的WSI表示。虽然多染色标记（如IHC）能丰富H&E特征，但进展受限于对齐良好的多染色数据集稀缺，染色间错位会阻碍一致的patch级特征并降低slide级嵌入质量。

Method: 1) 构建slide级对齐的五染色数据集（H&E、HER2、KI67、ER、PGR）；2) 提出CSCL两阶段预训练框架：第一阶段使用轻量适配器进行patch级对比对齐，提升H&E特征与IHC上下文线索的兼容性；第二阶段使用MIL进行slide级表示学习，包含跨染色注意力融合模块和跨染色全局对齐模块。

Result: 在癌症亚型分类、IHC生物标志物状态分类和生存预测任务上均取得一致提升，产生了高质量、可迁移的H&E slide级表示。

Conclusion: CSCL框架通过跨染色对比学习有效整合多染色信息，显著提升了WSI表示的质量和可迁移性，为计算病理学提供了有力的工具。
...
TL;DR: DOT-BI是一种基于人类运动感知的快速机器人识别方法，通过隐藏数字与背景的纹理相同但运动差异来区分人类和自动化系统。


<details>
  <summary>Details</summary>
Motivation: 在线调查和流程中需要有效区分人类用户和自动化机器人，现有方法可能被先进AI模型绕过，需要基于人类独特感知能力的新方法。

Method: 使用与背景相同随机黑白像素纹理的隐藏数字，通过数字与背景之间的运动和尺度差异使人类能够感知，而逐帧算法处理无法获得有效信号。

Result: 1. 最先进的多模态模型（GPT-5-Thinking和Gemini 2.5 Pro）即使被告知机制也无法提取正确值；2. 在线调查中99.5%参与者完成任务，平均完成时间10.7秒；实验室研究显示相对于对照组无负面使用体验影响。

Conclusion: DOT-BI是一种有效区分人类和机器人的方法，基于人类独特的运动感知能力，易于使用且对用户体验无负面影响，已发布代码和预渲染变体以促进应用。
...
<details>
  <summary>Details</summary>
Motivation: RNA逆折叠问题是RNA设计中的关键挑战，需要找到能够折叠成特定二级结构的核苷酸序列。由于序列与结构之间的复杂关系，这一任务具有内在的复杂性。

Method: HyperRNA框架包含三个主要组件：预处理、编码和解码。预处理阶段基于3-bead粗粒度表示构建RNA骨架原子坐标的图结构；编码阶段使用注意力嵌入模块和超图编码器捕获高阶依赖性和复杂生物分子相互作用；解码阶段以自回归方式生成RNA序列。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理广泛的相机运动时难以保持特写视图的渲染质量，特别是在相机靠近化身表面时。需要一种既能处理一般相机运动，又能保持特写渲染质量的化身表示方法。

Method: 使用纹理平面表示化身，包含两组可学习纹理（低频和高频细节）。根据相机距离自动切换纹理：相机靠近时使用高频纹理，相机远离时逐渐减少高频纹理的影响。通过限制所需图元数量保持高帧率。
...
TL;DR: Motion4D是一个将2D基础模型先验集成到统一4D高斯溅射表示中的框架，通过迭代优化解决动态场景3D一致性问题，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有2D视觉基础模型虽然泛化能力强，但缺乏3D一致性，导致复杂3D环境中空间错位和时间闪烁问题，需要解决动态场景的几何和运动理解问题。

Method: 提出两阶段迭代优化框架：1）顺序优化更新运动和语义场保持局部一致性；2）全局优化联合优化所有属性保证长期一致性。引入3D置信度图动态调整运动先验，自适应重采样基于RGB和语义误差在欠表示区域插入新高斯点，通过迭代细化解决语义不一致问题。

Result: 在点跟踪、视频对象分割和新视角合成等多种场景理解任务上，Motion4D显著优于2D基础模型和现有3D方法。

Conclusion: Motion4D成功将2D基础模型先验集成到4D高斯溅射表示中，有效解决了动态场景的3D一致性问题，为复杂3D环境理解提供了强大框架。
...
TL;DR: LAMP利用大型语言模型作为运动规划器，将自然语言描述转换为动态物体和相机的3D轨迹，通过特定领域语言和程序合成实现语言到运动的直接控制。


<details>
  <summary>Details</summary>
Motivation: 视频生成在视觉保真度和可控性方面取得了显著进展，但运动控制（指定物体动态和相机轨迹）对于构建复杂的电影场景至关重要，现有接口仍然有限。需要一种能够从自然语言直接生成物体和相机运动的方法。
...
Result: 实验表明，与最先进的替代方案相比，LAMP在运动可控性和与用户意图对齐方面表现更好，建立了第一个直接从自然语言规范生成物体和相机运动的框架。

Conclusion: LAMP通过将大型语言模型作为运动规划器，成功实现了从自然语言到3D运动轨迹的直接转换，为视频生成中的运动控制提供了新的解决方案，特别是在物体和相机运动的联合控制方面取得了突破。
...
TL;DR: ReCamDriving是一个纯视觉的、相机控制的新轨迹视频生成框架，利用密集的3DGS渲染进行显式几何引导，通过两阶段训练和跨轨迹数据策展策略，实现了精确的相机可控生成。
...
Method: 1. 利用密集且场景完整的3DGS渲染进行显式几何引导；2. 采用两阶段训练范式：第一阶段使用相机姿态进行粗略控制，第二阶段结合3DGS渲染进行细粒度视点和几何引导；3. 提出基于3DGS的跨轨迹数据策展策略，消除相机变换模式的训练-测试差距；4. 构建ParaDrive数据集，包含超过11万对平行轨迹视频对。

Result: ReCamDriving在相机可控性和结构一致性方面达到了最先进的性能。实验证明该方法能够生成精确的相机控制视频，同时保持高质量的结构一致性。

Conclusion: ReCamDriving通过结合3DGS渲染的几何引导、两阶段训练策略和跨轨迹数据策展，成功实现了纯视觉的、相机控制的新轨迹视频生成，在可控性和一致性方面优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有时空视频定位研究主要局限于对象中心和描述性指令，忽视了任务导向推理对于具身智能体完成目标导向交互的重要性，需要建立连接感知与交互的基准。
...
TL;DR: Colon-X是一个推进结肠镜多模态智能的开源项目，包含ColonVQA数据集（超过110万视觉问答条目）和ColonReason推理数据集，开发了首个R1风格模型ColonR1，在数据稀缺条件下性能优于监督微调25.22%。


<details>
  <summary>Details</summary>
Motivation: 推动结肠镜多模态智能发展，填补从多模态理解到临床推理的关键空白，解决现有MLLM模型在临床输出中缺乏鲁棒性和可信度的问题。

Method: 1. 构建ColonVQA多模态数据集（76个临床发现，18个多模态任务）；2. 评估22个MLLM模型的泛化性和抗干扰能力；3. 通过多专家辩论流程标注ColonReason推理数据集；4. 开发ColonR1模型，采用任务自适应奖励和梯度稳定优化技术。

Result: 1. 现有MLLM模型的临床输出远未达到鲁棒可信；2. ColonR1在数据稀缺条件下达到56.61%总体准确率，比监督微调提升25.22%；3. 为多模态结肠镜分析建立了新的推理基准。

Conclusion: Colon-X项目成功构建了全面的结肠镜多模态数据基础，开发了首个专注于临床推理的R1风格模型，显著提升了结肠镜多模态分析的推理能力，为社区提供了开源的数据和模型资源。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器在生成高质量图像方面表现出色，但随着模型规模增大，内存占用和推理延迟显著增加，给实际部署带来挑战。现有基于旋转的技术虽然能平滑异常值并实现4位量化，但开销大且难以处理扩散变换器中的行级异常值。

Method: 提出ConvRot方法，采用分组旋转量化，利用正则Hadamard变换（RHT）同时抑制行级和列级异常值，将复杂度从二次降低到线性。基于此设计了ConvLinear4bit模块，集成旋转、量化、GEMM和反量化，实现无需重新训练的W4A4推理。

Result: 在FLUX.1-dev上的实验显示，实现了2.26倍的速度提升和4.05倍的内存减少，同时保持了图像质量。这是首次将基于旋转的量化应用于扩散变换器的即插即用W4A4推理。
...
TL;DR: GaussianBlender：首个前馈式文本驱动3D风格化框架，无需逐资产优化，实现即时推理编辑
...
TL;DR: 主动视觉感知系统通过动态感知和行动与环境交互，能够根据特定目标或不确定性调整行为，相比被动系统具有更强的信息获取能力。


<details>
  <summary>Details</summary>
Motivation: 在复杂环境中，静态感知方法往往无法提供足够信息，而主动视觉感知能够通过引导注意力、移动传感器或与物体交互来获取更丰富的信息，这对于机器人、自动驾驶、人机交互和监控系统等应用至关重要。

Method: 本文采用综述性研究方法，全面探讨主动视觉感知的机会和挑战，分析其在动态环境中如何通过感知-行动循环来优化信息获取。

Result: 论文提供了主动视觉感知的全面概述，包括其潜力、当前研究进展以及面临的挑战，强调了该领域在复杂环境中的重要性。

Conclusion: 主动视觉感知在多个领域具有巨大潜力，但要实现广泛应用仍需解决实时处理复杂视觉数据、动态环境决策和多模态感知融合等关键挑战。
...
TL;DR: SUSS是一种新的感知相似度评分方法，通过结构化多元正态分布建模图像感知组件，在像素空间学习线性变换，提供可解释的感知对齐和稳定优化性能。
...
Method: 提出结构化不确定性相似度评分（SUSS）：1）将每个图像建模为一组感知组件，每个组件用结构化多元正态分布表示；2）通过生成式自监督训练，使模型给人眼不可察觉的增强赋予高似然度；3）最终评分是组件对数概率的加权和，权重从人类感知数据集中学习；4）在像素空间学习图像特定的线性残差变换，实现透明检查。

Result: SUSS与人类感知判断高度对齐，在不同失真类型上表现出强大的感知校准能力，提供局部化、可解释的相似度评估解释。作为感知损失函数用于下游成像任务时，表现出稳定的优化行为和竞争性性能。

Conclusion: SUSS成功解决了现有感知相似度评分方法的局限性，提供了一种既与人类感知对齐又具有可解释性的新方法，通过结构化概率建模实现了透明检查能力，在多种应用中表现出色。
...
TL;DR: DINO-RotateMatch框架通过结合数据集自适应图像配对和旋转感知关键点匹配，提升大规模3D重建中的图像匹配性能


<details>
  <summary>Details</summary>
Motivation: 解决从非结构化互联网图像进行大规模3D重建时的图像匹配挑战，特别是在大规模图像集合中有效匹配图像对的问题

Method: 1. 使用DINO进行语义相关图像对检索；2. 旋转增强捕捉方向依赖的局部特征；3. 结合ALIKED和Light Glue进行关键点提取和匹配

Result: 在Kaggle Image Matching Challenge 2025中取得显著改进，获得银奖（943支队伍中排名第47），平均准确率(mAA)持续提升

Conclusion: 结合自监督全局描述符和旋转增强的局部匹配为大规模3D重建提供了稳健且可扩展的解决方案
...
TL;DR: BlackCAtt是一种黑盒对抗攻击方法，通过识别因果像素集对目标检测器进行可解释、不可感知的攻击，实现边界框的消失、修改或新增。


<details>
  <summary>Details</summary>
Motivation: 现有对抗扰动方法多为白盒且架构特定，缺乏对攻击机制的解释性。需要一种能够理解攻击原理、可分析的黑盒方法，帮助开发者理解漏洞并改进模型防御。

Method: BlackCAtt采用黑盒算法，结合因果像素集和目标检测器生成的边界框，构建可解释、不可感知、可复现、架构无关的对抗攻击。通过识别最小因果像素集来精确攻击。

Result: 在COCO测试集上，BlackCAtt比基线方法在移除检测方面好2.7倍，修改检测好3.86倍，触发新虚假检测好5.75倍。攻击效果接近原始图像，几乎不可感知。

Conclusion: BlackCAtt展示了因果像素在构建精确、不可感知的黑盒对抗攻击中的强大能力，能够跨不同架构和尺寸的目标检测器工作，为理解攻击机制提供了可解释的方法。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在大型数据集上训练时会学习并重现数据中的偏见，导致产生刻板印象的输出。现有方法通常需要人工标注数据集或针对每个生成概念训练外部分类器，这限制了去偏方法的可扩展性和实用性。

Method: SelfDebias在推理过程中识别图像编码器嵌入空间中的语义聚类，并使用这些聚类来引导扩散过程，最小化输出分布与均匀分布之间的KL散度。该方法完全无监督，不需要人工标注数据集或外部分类器。

Result: 实验表明SelfDebias能够泛化到不同的提示词和扩散模型架构，包括条件模型和无条件模型。它不仅能有效去除关键人口统计维度上的偏见，同时保持生成图像的视觉保真度，还能处理更抽象概念的偏见识别。

Conclusion: SelfDebias提供了一种完全无监督的测试时去偏方法，能够自动识别语义模式并减少扩散模型中的偏见，为扩散模型的公平性改进提供了一种实用且可扩展的解决方案。
...
TL;DR: LSRS方法通过潜在尺度拒绝采样来改进VAR模型的图像生成质量，在几乎不增加计算开销的情况下显著降低FID分数
...
Method: 提出潜在尺度拒绝采样(LSRS)，在推理过程中逐步精炼潜在尺度的token图，使用轻量级评分模型评估多个候选token图，选择高质量图指导后续尺度生成

Result: LSRS显著提升VAR生成质量，计算开销极小：VAR-d30模型推理时间仅增加1%，FID从1.95降至1.78；推理时间增加15%时，FID可进一步降至1.66

Conclusion: LSRS为增强VAR生成提供了一种高效的测试时扩展解决方案，能有效缓解自回归误差累积，同时保持计算效率
...
TL;DR: 提出了一种从图像自动识别和翻译古埃及象形文字到英语的方法，使用分割、符号映射和翻译三阶段流程，在BLEU评分上达到42.2的显著结果。


<details>
  <summary>Details</summary>
Motivation: 埃及象形文字完全由图画组成，翻译面临单个符号多义性等挑战。深度学习翻译应用快速发展，为古埃及文字的自动识别和翻译提供了新的可能性。

Method: 使用三个阶段的流程：1) 分割阶段使用Contour和Detectron2；2) 将符号映射到Gardiner代码；3) 使用CNN模型进行翻译。研究使用了Morris Franken和EgyptianTranslation两个数据集进行分类和翻译。

Result: 模型取得了42.2的BLEU评分，相比之前的研究取得了显著成果。

Conclusion: 提出的方法能够有效实现古埃及象形文字的自动识别和翻译，在翻译质量评估指标上表现优异，为古埃及文字研究提供了新的技术手段。
...
Method: 结合数学变换，使用最小硬件要求，开发对受试者运动引起的失真具有更强抵抗力的呼吸率测量方法

Result: 在14名志愿者超过2.5小时的视频测试中，相对地面真值的偏差小于5%，平均绝对误差为0.57次呼吸/分钟，优于先前工作

Conclusion: 该方法对受试者运动引起的失真具有更强抵抗力，可在不对受试者行为施加显著限制的情况下远程测量呼吸率
...
TL;DR: 提出LUnet架构，通过保持通道数恒定而非随分辨率减半而加倍，大幅减少参数（30倍以上），在MRI和CT数据集上性能与传统Unet相当。


<details>
  <summary>Details</summary>
Motivation: 传统Unet架构在降采样时加倍通道数以保持信息，导致内存占用大、训练批次小、推理延迟高。现有通道剪枝方法需要长时间优化且泛化性差。研究发现最终结构比通道选择策略更重要。

Method: 提出LUnet架构，采用紧凑平坦的层次结构，在分辨率减半时保持通道数恒定而非加倍。通过跳跃连接减少瓶颈层通道需求，实现参数大幅减少。

Result: LUnet在公共MRI数据集和两个内部CT数据集上评估，参数减少30倍以上，性能与传统Unet相当，优于相同参数量的标准Unet。随机剪枝与STAMP剪枝效果相似。

Conclusion: LUnet通过恒定通道数的平坦架构，在显著减少参数的同时保持分割性能，为医学图像分割提供了高效解决方案，挑战了传统Unet架构设计范式。
...
### [65] [CoDA: From Text-to-Image Diffusion Models to Training-Free Dataset Distillation](https://arxiv.org/abs/2512.03844)
*Letian Zhou,Songhua Liu,Xinchao Wang*

Main category: cs.CV

TL;DR: CoDA提出了一种无需目标数据集预训练扩散模型的数据集蒸馏方法，通过识别目标数据集的核心分布并引导生成过程与之对齐，在ImageNet-1K等基准上达到或超越现有方法性能。
...
Result: CoDA在不依赖目标数据集特定训练生成模型的情况下，在所有基准测试（包括ImageNet-1K及其子集）上达到或超越了依赖此类模型的方法性能。在ImageNet-1K的每类50图像设置下，创造了60.4%的最新准确率记录。

Conclusion: CoDA通过核心分布对齐有效解决了数据集蒸馏中的两个关键挑战，实现了仅使用现成文本到图像模型的高效数据集蒸馏，为数据集蒸馏领域提供了新的解决方案。
...
TL;DR: DIQ-H是首个评估视觉语言模型在动态视觉退化时序序列中鲁棒性的基准测试，通过物理基础退化模拟真实世界条件，测量幻觉持续性、错误恢复和时序一致性，发现现有模型存在显著鲁棒性差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注静态高质量图像，忽略了时序退化和错误传播这两个关键失效模式，即瞬态视觉退化引发的幻觉会在后续帧中持续存在。这对于自动驾驶等安全关键应用中的视觉语言模型部署至关重要。

Method: 1) 提出DIQ-H基准测试，应用基于物理的退化（运动模糊、传感器噪声、压缩伪影）；2) 通过多轮问答任务测量幻觉持续性、错误恢复和时序一致性；3) 提出不确定性引导迭代优化(UIR)方法，使用轻量级VLM进行不确定性过滤生成可靠的伪真值标注。

Result: 1) UIR方法实现了15.3%的准确率提升；2) 在16个最先进的VLM上实验发现显著鲁棒性差距：GPT-4o仅达到78.5%的恢复率，开源模型在时序一致性上低于60%；3) 揭示了现有模型在动态视觉退化条件下的脆弱性。

Conclusion: DIQ-H为评估真实世界部署中VLM的可靠性提供了全面平台，揭示了现有模型在动态视觉退化条件下的重大鲁棒性缺陷，强调了开发更稳健时序感知VLM的必要性。
...
TL;DR: 该论文提出了一种新的测试时缩放方法，通过文本嵌入扰动来增强文本到图像扩散模型的生成多样性和质量，利用空间噪声和文本嵌入扰动在频域上的互补特性。
...
Method: 提出两种关键设计：1) 引入基于步骤的文本嵌入扰动，结合频率引导的噪声调度与空间噪声扰动；2) 根据频域贡献和扰动容忍度选择性调整扰动强度。

Result: 该方法可以无缝集成到现有测试时缩放方法中，在多个基准测试上显示出显著改进，且几乎不增加额外计算成本。

Conclusion: 文本嵌入扰动与空间噪声在频域上具有互补特性，通过合理设计扰动策略可以有效提升文本到图像扩散模型的生成多样性和质量。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在长视频理解中面临上下文长度限制和处理密集视频token的计算成本问题。现有方法通常采用查询感知的帧选择机制，但计算开销大。本文挑战了这种复杂搜索机制普遍必要的假设。
...
TL;DR: 该研究发布了来自伊拉克埃尔比勒的339张前列腺穿刺活检全切片图像数据集，包含185名患者的数据，旨在解决病理AI模型在非西方人群中的泛化性问题。


<details>
  <summary>Details</summary>
Motivation: 当前公开的病理学数据集主要来自西方人群，缺乏中东等数字化程度较低地区的数据，导致AI模型在这些人群中的泛化性未知。这促使研究者公开该数据集，以支持全球多样化人群的病理AI模型开发和验证。

Method: 收集了伊拉克埃尔比勒185名患者的前列腺穿刺活检样本，获得339张全切片图像。由三位病理学家独立分配Gleason评分和国际泌尿病理学会分级。使用两种高通量扫描仪（Leica和Hamamatsu）和一种紧凑型扫描仪（Grundium）进行扫描。所有切片都经过去标识化处理，并以原始格式提供。

Result: 创建了一个包含339张前列腺活检全切片图像的数据集，来自伊拉克的185名患者。数据集包含三位病理学家独立分配的Gleason评分和ISUP分级。数据将存入Bioimage Archive（BIA），并以CC BY 4.0许可证发布。

Conclusion: 该数据集填补了中东地区公开病理学数据的空白，支持分级一致性分析、颜色归一化和跨扫描仪鲁棒性评估，有助于开发更具全球代表性的病理AI模型。
...
<details>
  <summary>Details</summary>
Motivation: 尽管草图表示学习领域已有显著进展，但时序方面对表示质量的实际相关性仍存在理解空白。本研究旨在探究将草图视为序列处理的合理性，以及哪些内部顺序更具相关性。

Method: 研究比较了不同顺序表示方法（绝对坐标vs相对坐标）和解码器类型（自回归vs非自回归），评估时序对草图表示的影响。

Result: 结果显示：1）传统位置编码对草图序列建模有效；2）绝对坐标始终优于相对坐标；3）非自回归解码器优于自回归解码器；4）时序重要性取决于具体顺序和评估任务。

Conclusion: 草图可以作为序列处理，但时序的重要性具有条件性。绝对坐标和非自回归解码器是更有效的表示方法，但最佳方法需根据具体任务和顺序选择。
...
TL;DR: 小规模视觉Transformer（5M参数）通过三种不同的预训练、中间微调和下游数据集实验发现，预训练和微调有帮助但收益递减，中间微调可能因任务机制差异对下游性能产生负面影响。


<details>
  <summary>Details</summary>
Motivation: 虽然基于Transformer的架构在计算机视觉和NLP领域取得了巨大成功，但它们通常需要大量参数和训练数据才能获得强大性能。本研究旨在探索小规模视觉Transformer（仅5M参数）在不同训练策略下的表现，特别是预训练、中间微调和下游任务训练的相对效益。

Method: 使用三种不同的预训练数据集、中间微调数据集和下游数据集进行实验，采用不同的训练目标，评估它们对5M参数视觉Transformer模型的边际效益。通过对比不同训练策略组合的效果，分析预训练、中间微调和下游训练的相对贡献。

Result: 研究发现：1）预训练和微调总是有助于提升模型性能，但存在收益递减现象；2）中间微调可能对下游性能产生负面影响，特别是当中间任务与下游任务机制差异较大时；3）小规模ViT从有针对性的预训练和仔细的数据选择中获益最大。
...
Method: 提出金字塔稀疏注意力（PSA），采用多级池化KV表示而非二值掩码。每个查询块动态分配较低的池化级别给关键KV块，较高的池化级别给次要KV块，在完全保留和完全剪枝之间创建信息插值。采用解耦的块-瓦片设计和硬件友好的原生内核实现高效执行。

Result: 在视频理解和生成基准测试中，PSA能够保持上下文信息和视觉保真度，在效率-质量权衡方面优于或达到可比性能，优于现有稀疏注意力基线。
...
TL;DR: CaravelMetrics是一个用于自动脑血管分析的计算框架，通过骨架化图表示建模血管形态，可提取15种形态、拓扑、分形和几何特征，支持全局和区域分析。


<details>
  <summary>Details</summary>
Motivation: 开发一个可扩展、全自动的脑血管定量特征提取框架，支持血管健康的规范建模和人群水平研究，特别是研究年龄、性别和教育相关的血管变化。

Method: 基于骨架化的图表示方法，整合图谱区域分割、中心线提取和图构建，计算15种形态测量、拓扑、分形和几何特征，支持全局和动脉区域多尺度分析。

Result: 应用于IXI数据集的570个3D TOF-MRA扫描（年龄20-86岁），框架产生可重复的血管图，捕获了年龄和性别相关变化，以及教育相关的血管复杂性增加，与文献报道一致。

Conclusion: CaravelMetrics提供了一个可扩展、全自动的脑血管特征提取方法，支持血管健康和衰老的规范建模和人群水平研究。
...
<details>
  <summary>Details</summary>
Motivation: 直肠癌患者接受新辅助治疗后，临床完全缓解患者可采用观察等待策略，但需要准确方法早期检测局部复发以防止远处转移

Method: 开发Siamese Swin Transformer with Dual Cross-Attention (SSDCA)模型，结合基线期和随访期内窥镜图像，利用预训练Swin Transformer提取领域无关特征，通过双交叉注意力机制强调两次扫描特征

Result: SSDCA在62名患者测试集上表现最佳：平衡准确率81.76%，敏感性90.07%，特异性72.86%；对血液、粪便、毛细血管扩张等伪影具有鲁棒性；UMAP聚类显示最大类间分离和最小类内分散

Conclusion: SSDCA能够有效结合纵向内窥镜图像区分完全缓解和局部复发，为直肠癌观察等待策略提供客观准确的早期检测方法
...
### [75] [Fast & Efficient Normalizing Flows and Applications of Image Generative Models](https://arxiv.org/abs/2512.04039)
*Sandeep Nagar*

Main category: cs.CV

TL;DR: 该论文在生成模型效率提升和计算机视觉应用两方面做出贡献：第一部分通过6项创新改进归一化流架构；第二部分应用生成模型解决农业质量评估、地质测绘、隐私保护和艺术修复等实际问题。


<details>
  <summary>Details</summary>
Motivation: 论文旨在解决两个核心问题：1）提高生成模型（特别是归一化流）的计算效率和架构性能；2）将生成模型应用于现实世界的计算机视觉挑战，包括数据稀缺、隐私保护和图像修复等问题。

Method: 第一部分：提出6项归一化流架构改进：可逆3x3卷积层、Quad-coupling层、并行反演算法、卷积逆的反向传播算法、Inverse-Flow训练方法、Affine-StableSR超分辨率模型。第二部分：应用条件GAN进行农业质量评估、使用堆叠自编码器进行地质测绘、基于人脸检测和图像修复的隐私保护方法、Stable Diffusion图像修复技术、以及适应多种退化类型的艺术修复扩散模型。

Result: 提出的归一化流改进显著提升了计算效率和模型性能；农业质量评估系统在种子纯度测试中取得良好准确率；地质测绘框架相比传统方法改进了特征提取；隐私保护方法有效保护自动驾驶数据集中的敏感信息；艺术修复模型能够统一处理多种退化类型。

Conclusion: 论文在生成模型的理论改进和实际应用两方面都做出了重要贡献，既推进了归一化流架构的理论发展，又展示了生成模型在解决现实世界计算机视觉问题中的实用价值，特别是在效率、隐私保护和图像修复等领域。
...
TL;DR: FRESCO通过整合帧内和帧间对应关系来增强时空一致性，在零样本视频编辑任务中显著提升视觉连贯性
...
Method: 提出FRESCO框架，整合帧内对应关系和帧间对应关系，形成更鲁棒的时空约束，确保语义相似内容在帧间的一致性转换，并显式优化特征

Result: 在视频到视频转换和文本引导视频编辑两个零样本任务上验证，实验表明能生成高质量、连贯的视频，相比现有方法有显著提升

Conclusion: FRESCO通过增强时空一致性约束，显著提升了零样本视频编辑的视觉连贯性，为扩散模型在视频应用中的发展提供了重要进展
...
Method: 1) 将视频和3D运动建模为统一的token序列，使用独立的嵌入层来缓解分布差异；2) 设计序列建模策略，在单一框架中整合两个不同任务；3) 设计新颖的3D运动tokenizer，采用时间扩展策略，使用单个VQ-VAE生成量化运动token，具有多个专家解码器处理身体形状、平移、全局方向和身体姿态。

Result: 大量实验表明，该方法能同时生成相应的视频和运动，同时执行准确的动作捕捉。该方法挖掘了LLMs融合不同数据类型的能力。

Conclusion: 这项工作为将人体中心信息整合到现有模型中铺平了道路，并可能实现人类、物体和场景的多模态可控联合建模。
...
<details>
  <summary>Details</summary>
Motivation: 增强多模态大语言模型的时间理解能力对于长视频分析至关重要，但现有的强化学习方法局限于有限的任务类型和数据，难以泛化到多样化的时间理解场景。

Method: 提出TempR1框架：1）构建多任务语料库，让模型接触多样化的时间结构和语义；2）基于GRPO算法实现稳定有效的跨任务优化；3）将时间任务分为三种预测区间与真实实例对应类型，并为每种类型设计定制化的定位奖励。

Result: TempR1在多个基准测试中达到最先进性能。联合优化互补任务产生强大的协同效应，既增强了泛化能力，又提升了单任务性能。

Conclusion: TempR1为MLLMs中的时间推理建立了一个可扩展且有原则的范式，通过系统化的多任务强化学习框架有效增强了模型的时间理解能力。
...
TL;DR: UniID提出了一种无需调优的人脸个性化统一框架，通过整合文本嵌入和适配器两种范式，在保持身份保真度的同时实现灵活的文本控制。


<details>
  <summary>Details</summary>
Motivation: 现有无需调优的人脸个性化方法主要分为文本嵌入和适配器两种范式，但都难以同时实现高身份保真度和灵活的文本控制性。
...
Conclusion: UniID通过统一两种范式并采用原则性设计，实现了高保真人脸个性化与灵活文本控制的平衡，为无需调优的人脸个性化提供了有效解决方案。
...
TL;DR: DirectDrag是一个无需手动掩码和文本提示的基于拖拽的图像编辑框架，通过自动软掩码生成和读出引导的特征对齐机制，实现了高保真度和精确点对齐的图像编辑。
...
Method: DirectDrag提出两个关键创新：1) 自动软掩码生成模块，从点位移智能推断可编辑区域，自动沿运动路径定位变形；2) 读出引导的特征对齐机制，利用中间扩散激活来保持基于点编辑期间的结构一致性。

Result: 尽管无需手动掩码或提示，DirectDrag在图像质量方面优于现有方法，同时保持了有竞争力的拖拽精度。在DragBench和真实场景中的广泛实验证明了其有效性和实用性。

Conclusion: DirectDrag为高质量、交互式图像操作提供了一个有效且实用的框架，通过创新的自动掩码生成和特征对齐机制，解决了现有方法对人工干预的依赖问题。
...
<details>
  <summary>Details</summary>
Motivation: 在野外图像集合中进行3D重建时，常受到"噪声"图像（与其他图像视角重叠度低或不相关的输入）的干扰。传统SfM流水线通过几何验证和异常值拒绝处理此类问题，但前馈3D重建模型缺乏这些显式机制，导致在野外条件下性能下降。

Method: 通过对现有前馈重建模型VGGT在不同比例合成干扰图像下的深入分析，发现特定层自然表现出异常抑制行为。进一步探究揭示该层编码了具有区分性的内部表示，使其具备有效的噪声过滤能力。研究者直接利用这一特性进行异常视图拒绝，无需额外微调或监督。

Result: 在受控和野外数据集上的大量实验表明，这种隐式过滤机制具有一致性，并能很好地泛化到不同场景中。

Conclusion: 现有前馈3D重建模型虽然缺乏显式的异常值拒绝机制或噪声感知训练，但具有内在的干扰图像区分能力，这一发现为改进野外条件下的3D重建提供了新思路。
...
TL;DR: NVRC-Lite是NVRC的轻量化版本，通过多尺度特征网格和八叉树上下文模型改进轻量化神经视频压缩，在保持低计算复杂度的同时提升性能并加速编码。


<details>
  <summary>Details</summary>
Motivation: 现有基于INR的视频压缩方法中，NVRC虽然性能优异但计算复杂，而轻量化INR虽然计算复杂度低但性能有限。同时，现有方法使用自回归模型进行熵编码，虽然有效但编码速度慢，不实用。

Method: 1. 将多尺度特征网格集成到轻量化神经表示中，使用更高分辨率网格提升低复杂度下的INR性能；2. 提出基于八叉树的上下文模型用于高维特征网格的熵编码，替代缓慢的自回归模型，加速熵编码模块。

Result: NVRC-Lite在PSNR和MS-SSIM指标上分别比最佳轻量化INR视频编码器C3节省高达21.03%和23.06%的BD-rate，同时实现8.4倍编码加速和2.5倍解码加速。

Conclusion: NVRC-Lite成功将NVRC扩展为轻量化表示，通过多尺度特征网格和八叉树上下文模型，在保持低计算复杂度的同时显著提升性能并加速编码，为实用化神经视频压缩提供了有效解决方案。
...
TL;DR: C3G提出了一种前馈式紧凑3D高斯表示框架，通过可学习token引导高斯生成，减少冗余并提升多视角特征聚合效果，在无姿态新视角合成和3D场景理解任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于像素级3D高斯泼溅的方法会产生大量冗余高斯，导致内存开销大且多视角特征聚合效果不佳，影响新视角合成和场景理解性能。

Method: 提出C3G框架：1）使用可学习token通过自注意力聚合多视角特征来指导高斯生成；2）仅在关键空间位置生成紧凑3D高斯；3）利用学习到的注意力模式进行高效特征提升。

Result: 在无姿态新视角合成、3D开放词汇分割和视角不变特征聚合任务上表现优异，相比现有方法具有更好的内存效率和特征保真度。

Conclusion: 紧凑且几何有意义的表示足以实现高质量场景重建和理解，证明了该方法在减少冗余的同时保持特征有效性的优势。
...
TL;DR: RELIC是一个统一的交互式世界模型框架，能够实现实时长时流式生成、一致的空间记忆和精确的用户控制，通过压缩的历史潜在token表示长时记忆，在14B参数模型上达到16FPS的实时生成。
...
Method: 基于自回归视频扩散蒸馏技术，使用高度压缩的历史潜在token（包含相对动作和绝对相机位姿）在KV缓存中表示长时记忆；通过微调双向教师视频模型生成超出原始5秒训练时长的序列，并采用新的内存高效自强制范式将其转化为因果学生生成器，实现长时教师和长时学生自滚动的全上下文蒸馏。

Result: 在精心策划的Unreal Engine渲染数据集上训练的14B参数模型实现了16FPS的实时生成，相比先前工作表现出更准确的动作跟随、更稳定的长时流式生成和更鲁棒的空间记忆检索能力。
...
TL;DR: DIRL框架通过两阶段强化学习训练VLM协调使用多种视觉工具，提升空间推理精度，在多个基准测试中达到SOTA


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在定性视觉理解方面表现良好，但在需要精确空间度量的具身应用场景中表现不佳。虽然工具使用范式可以增强VLM能力，但现有方法要么依赖手工提示策略，要么采用固定的工具流水线，限制了VLM发现最优工具使用模式的能力。

Method: 提出双交互强化学习框架：1) 教学阶段：结合单工具专家的交互RL演示和前沿模型使用所有工具的轨迹；2) 探索阶段：通过持续RL进一步优化多工具协调。模型SpaceTools具备工具增强的空间推理能力。

Result: 在空间理解基准测试中取得SOTA性能：RoboSpatial-Home、BLINK、BOP-ASK。相比普通SFT基线提升12%，相比RL基线提升16%。使用7自由度机器人作为工具展示了可靠的现实世界操作能力。

Conclusion: DIRL框架有效解决了VLM在多工具协调中的挑战，显著提升了空间推理精度，为具身智能应用提供了实用解决方案。
...
TL;DR: PosterCopilot是一个基于大语言多模态模型的图形设计框架，通过三阶段训练策略提升布局几何精度和美学推理能力，支持分层可控的迭代编辑，解决现有方法几何不准确和缺乏专业工作流程的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言多模态模型的图形设计自动化方法存在几何布局不准确、缺乏专业工作流程所需的迭代分层编辑能力等问题，无法满足专业图形设计需求。

Method: 提出三阶段渐进训练策略：1) 扰动监督微调；2) 视觉-现实对齐的强化学习；3) 美学反馈的强化学习。同时开发完整工作流程，将训练好的设计模型与生成模型结合，支持分层可控的迭代编辑。

Result: 实验表明PosterCopilot能够生成几何精确且美学优越的布局，为专业迭代设计提供前所未有的可控性。

Conclusion: PosterCopilot框架通过创新的训练策略和工作流程，显著提升了图形设计自动化的几何精度、美学质量和专业可控性，为专业图形设计提供了有效的解决方案。
...
TL;DR: 本文提出SimFlow方法，通过将VAE编码器输出的方差固定为常数（如0.5），简化了归一化流与VAE的联合训练，避免了传统方法中复杂的噪声添加和去噪流程，在ImageNet 256×256生成任务上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有归一化流方法存在两个主要问题：1）需要添加随机噪声作为数据增强，引入复杂的噪声添加和去噪流程；2）使用预训练且冻结的VAE编码器，导致重建和生成质量不理想。本文旨在通过简单方法解决这两个问题。

Method: 提出SimFlow方法，将VAE编码器输出的方差固定为常数（如0.5）。这种方法使编码器能输出更广泛的token分布，解码器学习从增强的token分布中重建干净图像，避免了额外的噪声设计。同时，固定方差简化了VAE的证据下界，使NF与VAE能够稳定地联合训练。

Result: 在ImageNet 256×256生成任务上，SimFlow获得了gFID 2.15的分数，优于SOTA方法STARFlow（gFID 2.40）。当与端到端表示对齐方法（REPA-E）结合时，gFID进一步提升至1.91，在归一化流方法中创造了新的SOTA。

Conclusion: 通过简单地将VAE方差固定为常数，SimFlow有效解决了传统归一化流方法的两个主要限制，实现了更简单的训练流程和更好的生成质量，为归一化流与VAE的联合训练提供了新的有效方案。
...
TL;DR: 该研究提出了一种动态监测大语言模型伦理熵的方法，通过行为分类器量化模型在压力测试中的伦理退化，并建立监控系统来预警价值漂移。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型安全评估主要依赖静态基准测试，但关键失效往往是动态的：分布漂移下的价值漂移、越狱攻击以及部署中缓慢的对齐退化。需要建立动态监测框架来应对这些实时风险。

Method: 基于"智能第二定律"的伦理熵框架，定义了五类行为分类法，训练分类器从模型转录本中估计伦理熵S(t)，在四个前沿模型的基座版和指令调优版上进行压力测试，测量熵动态变化。

Result: 基座模型显示持续的熵增长，而调优变体抑制了漂移，将伦理熵降低了约80%。从这些轨迹中估计出有效对齐工作率gamma_eff，并将S(t)和gamma_eff嵌入监控管道，当熵漂移超过稳定阈值时发出警报。

Conclusion: 该研究使伦理熵框架可操作化，为大型语言模型提供了运行时价值漂移监控方法，实现了对动态对齐失效的实时监督，增强了模型部署的安全性。
...
TL;DR: 该论文研究EaaS（嵌入即服务）水印技术，发现现有水印易受改写攻击，并提出新的WET水印方案来防御模仿攻击。


<details>
  <summary>Details</summary>
Motivation: 随着LLM的发展，企业开始提供EaaS服务，但现有研究表明EaaS容易受到模仿攻击。虽然已有水印技术用于保护EaaS知识产权，但这些水印存在新的安全漏洞。

Method: 首先通过改写攻击测试现有EaaS水印的脆弱性，然后提出WET水印技术——使用线性变换嵌入向量，通过反向变换和相似度比较进行水印验证。

Result: 研究发现改写攻击能有效绕过当前最先进的EaaS水印；提出的WET水印对改写攻击具有鲁棒性，几乎达到完美的可验证性，并通过消融研究验证了各组件的重要性。

Conclusion: 现有EaaS水印技术存在被改写攻击绕过的漏洞，而提出的WET水印方案能有效防御此类攻击，为EaaS知识产权保护提供了更安全的技术方案。
...
TL;DR: 提出RDG框架，通过生成平衡推理数据来缓解LLM中的选择支持性偏见，实验显示在记忆和评估任务上分别提升81.5%和94.3%


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在选择支持性偏见，会系统性偏向自己选择的选项，影响AI辅助决策的客观性。现有去偏见方法主要针对人口统计和社会偏见，而认知偏见的缓解方法尚未充分探索。

Method: 提出推理依赖生成（RDG）框架，通过自动构建平衡的推理问答对来生成无偏见推理数据，显式建模选择、证据和理由之间的依赖关系，包含上下文依赖数据和依赖解耦数据。

Result: 使用RDG生成数据微调的LLM在记忆实验中提升81.5%，在评估实验中提升94.3%，同时在标准BBQ基准测试中保持相似性能。

Conclusion: 这是首个解决LLM中认知偏见的方法，通过RDG框架生成无偏见推理数据，为开发更可靠的AI辅助决策系统做出贡献。
...
TL;DR: InvertiTune框架通过受控数据生成和监督微调，实现单次推理的知识图谱构建，相比传统迭代方法更高效且能处理复杂关系。
...
Method: 提出InvertiTune框架：1）从大型知识库中系统提取子图并过滤噪声；2）利用LLM生成对应的自然文本描述；3）通过监督微调轻量级模型实现单次推理的KG构建。

Result: 在CE12k数据集上，InvertiTune优于更大的非微调LLM和现有Text2KG方法，在CrossEval-1200测试集上表现出更强的跨数据集泛化能力。

Conclusion: 现实、高质量的训练数据对于推进高效高性能的Text2KG系统至关重要，InvertiTune框架为此提供了有效解决方案。
...
TL;DR: 本文提出了一个用于检测和分析政治文本中解释的框架，通过训练轻量级因果语言模型来提取因果主张，形成可用于下游分析的结构化数据集。


<details>
  <summary>Details</summary>
Motivation: 解释是理解政治世界的基本要素，公民经常询问和回答关于事件原因、责任归属以及不同行动方案的问题。然而，尽管解释的重要性，它们在政治科学中仍然是一个未得到系统分析的领域，现有方法分散且通常是针对特定问题的。

Method: 作者训练了一个轻量级因果语言模型，能够返回结构化数据集，以因果对的形式提取政治文本中的因果主张。该方法具有适度的标注要求、良好的泛化能力和相对于人工编码的准确性。

Result: 该方法展示了如何大规模研究因果解释，证明了其在政治文本分析中的有效性。模型能够准确提取因果主张，为系统分析政治解释提供了可行工具。

Conclusion: 该框架为政治科学中解释的系统分析提供了新方法，通过自动化提取因果主张，使得大规模研究政治解释成为可能，填补了现有研究方法的空白。
...
TL;DR: RMFT是一种新的隐私保护微调技术，通过随机掩码减少LLM对训练数据中个人身份信息的记忆，同时最小化性能影响


<details>
  <summary>Details</summary>
Motivation: 当前自然语言模型（特别是大语言模型）存在严重的安全和隐私风险，模型倾向于记忆训练数据中的个人身份信息

Method: 提出随机掩码微调（RMFT）技术，并引入MaxTER评估框架来评估隐私-效用权衡

Result: 在Enron邮件数据集上，RMFT相比基线微调减少了80.81%的总提取率和80.17%的已见提取率，仅增加5.73%的困惑度，优于去重方法

Conclusion: RMFT是一种有效的隐私保护微调方法，在显著减少PII记忆的同时保持了良好的模型性能
...
TL;DR: 利用LLM辅助标注管道分析西班牙语-英语和西班牙语-瓜拉尼语双语语料，自动标注话题、体裁和语用功能，揭示社会语言学模式


<details>
  <summary>Details</summary>
Motivation: 传统社会语言学分析依赖人工标注，难以处理大规模语料。本研究旨在开发基于大语言模型的自动标注方法，为跨语言和低资源双语研究提供计算工具

Method: 构建LLM辅助标注管道，自动标注3,691个语码转换句子的主题、体裁和语用功能，整合迈阿密双语语料库的人口统计元数据，为西班牙语-瓜拉尼语数据集添加新的话题标注

Result: 迈阿密数据揭示了性别、语言优势和语用功能之间的系统性关联；巴拉圭文本显示瓜拉尼语用于正式场合、西班牙语用于非正式场合的双语分工模式。LLM能够可靠地恢复传统上只能通过人工标注获得的可解释社会语言学模式

Conclusion: 大语言模型能够可靠地恢复传统上只能通过人工标注获得的社会语言学模式，为跨语言和低资源双语研究推进了计算方法
...
<details>
  <summary>Details</summary>
Motivation: 现有医学文本简化资源通常假设单一通用受众，忽略了不同用户群体在医学素养和信息需求上的显著差异，需要针对特定受众的个性化摘要方法

Method: 构建PERCS数据集，包含生物医学摘要及针对四种不同用户群体（普通公众、医预科学生、非医学研究人员、医学专家）的个性化摘要；所有摘要均由医生根据详细错误分类法进行事实准确性和用户群体匹配度审核

Result: 技术验证显示不同用户群体摘要的可读性、词汇使用和内容深度存在明显差异；对四种大语言模型进行基准测试，评估其全面性、可读性和忠实性，为未来研究建立基线结果

Conclusion: PERCS数据集支持个性化医学沟通和可控生物医学摘要研究，数据集、标注指南和评估材料已公开可用，有助于推动面向特定受众的医学信息传播研究
...
TL;DR: 本文提出Idea-Gated Transformer架构，通过分离语义规划和语法生成来解决自回归语言模型中的"主题漂移"问题，使用辅助"Idea Head"预测未来上下文窗口的词袋分布，并通过可微分门控机制实时抑制语义无关的词汇选择。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型在下一个词预测训练中经常出现"主题漂移"问题，生成内容会偏离初始提示，这是因为模型过度依赖局部关联而非全局规划。虽然增大模型规模可以缓解这一问题，但NTP目标的根本短视性仍然存在。

Method: 提出Idea-Gated Transformer架构，引入辅助的"Idea Head"来预测未来上下文窗口的词袋分布，生成潜在的"概念向量"。该向量通过可微分门控机制主动门控主词汇表，在生成过程中实时抑制语义无关的词汇，从而有效剪枝搜索空间。
...
TL;DR: HBLR是一种新颖的假设驱动向后逻辑推理框架，通过置信感知符号翻译和假设驱动向后推理，提高LLM的逻辑推理准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM主要依赖前向推理范式，存在推理路径冗余、幻觉步骤和语义漂移等问题，导致推理效率低下且不可靠。需要更接近人类演绎思维的方法来提升逻辑推理能力。

Method: 提出假设驱动向后逻辑推理框架：1）置信感知符号翻译阶段，仅将高置信度内容转换为一阶逻辑等形式，不确定内容保留自然语言；2）翻译反思模块确保语义保真度；3）推理阶段模拟人类演绎思维，假设结论为真并递归验证前提；4）推理反思模块识别并纠正有缺陷的推理步骤。

Result: 在五个推理基准测试上的广泛实验表明，HBLR在准确性和效率方面均持续优于强基线方法。

Conclusion: HBLR框架通过结合符号推理和自然语言处理的优势，提供了一种更高效、更可靠的逻辑推理方法，能够更好地模拟人类演绎思维过程。
...
TL;DR: 研究者收集了Portal 2合作模式中11.5小时的口语对话语料库，包含24.5K条话语，分析了复杂空间指代、澄清修复等独特语言现象，并公开了多模态数据集。


<details>
  <summary>Details</summary>
Motivation: 合作视频游戏中玩家在复杂环境下通过沟通协调解决难题，产生了丰富的语言数据，但现有闲聊或任务导向对话语料库很少包含这类复杂协作场景中的语言现象。

Method: 收集了Portal 2合作模式中11.5小时的口语人类对话，构建了包含24.5K条话语的Portal Dialogue Corpus语料库，包含玩家视频、音频、转录文本、游戏状态数据以及手动和自动标注。

Result: 识别出多种在现有对话语料库中罕见的语言现象，包括复杂空间指代、澄清与修复、临时约定形成等，为研究复杂情境下协作问题解决中的语言使用提供了资源。

Conclusion: 合作视频游戏是研究复杂协作语言现象的宝贵数据源，公开的Portal Dialogue Corpus支持未来对复杂、情境化、协作问题解决场景中语言使用的分析研究。
...
<details>
  <summary>Details</summary>
Motivation: LoRA作为参数高效微调方法虽然流行，但由于其低秩假设，训练出的模型性能往往不理想。需要一种方法来更好地模拟基于梯度的全参数微调过程，以提高性能。

Method: 提出Dual LoRA方法，将低秩矩阵分为两组：幅度组控制参数是否更新及更新幅度，方向组决定参数更新方向。通过给幅度组添加ReLU函数，给方向组添加符号函数，更好地模拟全参数微调的梯度优化过程。
...
Conclusion: Dual LoRA通过引入归纳偏置，将低秩矩阵分解为幅度和方向两个组件，有效改进了LoRA的性能，在多个模型和任务上展现出优越性，为参数高效微调提供了新的有效方法。
...
TL;DR: PretrainZero是一个基于预训练语料的强化主动学习框架，将RL从领域特定的后训练扩展到通用预训练，通过主动识别信息内容、自监督学习和验证缩放来提升基础模型的通用推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的大思维模型虽然在某些专业领域表现出专家级能力，但严重依赖特定领域的可验证奖励，这限制了通用推理能力的扩展。需要突破验证数据壁垒，实现从领域特定后训练到通用预训练的扩展。

Method: 1) 主动预训练：学习统一推理策略，主动从预训练语料中识别合理且信息丰富的内容，并通过RL进行推理预测；2) 自监督学习：无需可验证标签、预训练奖励模型或监督微调，直接在通用语料上使用RL预训练推理器；3) 验证缩放：通过处理越来越具挑战性的掩码跨度，增强预训练基础模型的通用推理能力。

Result: 在强化预训练中，PretrainZero将Qwen3-4B-Base在MMLU-Pro、SuperGPQA和数学平均基准上的性能分别提升了8.43、5.96和10.60分。预训练模型还可作为下游RLVR任务的推理基础模型。

Conclusion: PretrainZero成功将强化学习从领域特定后训练扩展到通用预训练，突破了验证数据壁垒，显著提升了基础模型的通用推理能力，为人工通用智能的发展提供了新思路。
...
TL;DR: 该报告研究了Top-k注意力机制在解码和训练阶段的有效性及理论机制，验证了精确Top-k解码在下游任务中能达到甚至超越全注意力的性能，并探讨了训练-推理一致性、近似算法精度影响以及从熵角度的理论解释。
...
Method: 研究采用Top-k注意力机制，包括：1）验证精确Top-k解码的有效性；2）探索原生Top-k注意力训练策略；3）研究近似Top-k算法精度对下游任务的影响；4）从熵的角度提供理论解释。

Result: 实验表明：1）解码阶段仅保留与查询相似度最高的关键键值能达到或超越全注意力性能；2）训练与推理的Top-k操作一致性可进一步释放Top-k解码潜力；3）下游任务性能与近似算法保真度正相关；4）Top-k注意力SFT模型在下游任务中表现出明显的熵减现象。

Conclusion: Top-k注意力机制能有效降低LLMs推理计算成本，训练-推理一致性策略可显著提升模型性能，低熵状态更适应Top-k解码，为高效长上下文建模提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型在数学和代码生成等分析任务中表现出色，但其在抽象摘要生成中的效用被广泛假设但未经验证。研究旨在填补这一空白，评估推理策略对摘要质量的影响。

Method: 首先将通用推理策略适配到摘要生成领域，然后对8种推理策略和3个大型推理模型在8个多样化数据集上进行大规模比较研究，评估摘要质量和事实忠实度。

Result: 研究发现推理并非通用解决方案，其有效性高度依赖于具体策略和上下文。存在摘要质量与事实忠实度之间的权衡：显式推理策略倾向于提高流畅性但牺牲事实基础，而大型推理模型中的隐式推理则呈现相反模式。增加推理预算不会改善甚至可能损害事实一致性。

Conclusion: 有效的摘要生成需要忠实压缩而非创造性过度思考，推理策略的选择应根据具体任务需求平衡质量与忠实度。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言处理任务中取得显著进展，但容易产生错误或误导性但看似合理的论证（幻觉问题）。这在临床等高风险领域尤其令人担忧，因为事实错误可能带来严重后果。现有评估指标无法充分评估事实一致性且缺乏可解释性，使得诊断和缓解错误变得困难。
...
TL;DR: 意大利首个关于生成式AI采纳、使用模式和数字素养的实证研究显示，虽然AI被广泛用于工作和个人用途，但用户数字素养低且存在显著的性别鸿沟，女性采纳率仅为男性一半。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解生成式AI在意大利的采纳情况、使用模式和数字素养水平，特别关注AI技术可能加剧数字鸿沟的风险，因为用户对其局限性认识不足且采纳不均衡。

Method: 基于新收集的1,906名意大利语成年人的调查数据，进行首个全面的生成式AI采纳、使用模式和数字素养的实证映射研究。

Result: 研究发现：1）生成式AI被广泛用于工作和个人用途，包括情感支持和医疗建议等敏感任务；2）AI正在取代其他技术成为主要信息来源，但用户数字素养低，难以识别错误信息；3）存在显著的性别鸿沟，女性采纳率仅为男性一半，使用频率也更低；4）数字素养是采纳的关键预测因素，但只能部分解释性别差异。

Conclusion: 研究强调需要有针对性的教育计划，并进一步调查数字素养无法完全解释的公平参与障碍，以确保生成式AI的公平采纳和使用。
...
TL;DR: 该研究提出了Hydro-SE Bench评估基准，包含4000道选择题，用于评估大语言模型在水科学与工程领域的知识、应用和推理能力，发现商业LLM准确率在0.74-0.80之间，小参数模型在0.41-0.68之间，模型在行业标准和水利结构等专业领域表现较差。


<details>
  <summary>Details</summary>
Motivation: 水科学与工程是保障人类供水、清洁水电能源和防洪抗旱的关键领域，需要多学科知识和专家协作。随着大语言模型的快速发展，其在Hydro-SE领域的应用潜力日益受到关注，但模型在该领域的知识和应用能力尚未得到充分评估。

Method: 提出了Hydro-SE LLM评估基准（Hydro-SE Bench），包含4000道选择题，涵盖9个子领域，从基本概念知识、工程应用能力、推理和计算能力三个方面评估LLMs。对商业LLM和小参数LLM进行了系统评估。

Result: 商业LLM准确率在0.74-0.80之间，小参数模型在0.41-0.68之间。模型在与自然科学和物理科学密切相关的子领域表现良好，但在行业标准、水利结构等专业领域知识方面表现不佳。模型扩展主要提升了推理和计算能力。

Conclusion: 该研究揭示了LLMs在Hydro-SE任务中的优势和不足，为模型开发者提供了明确的训练目标，为Hydro-SE研究人员提供了应用LLMs的实践指导。模型在实际工程应用问题处理方面仍有很大提升潜力。
...
### [107] [Different types of syntactic agreement recruit the same units within large language models](https://arxiv.org/abs/2512.03676)
*Daria Kryvosheieva,Andrea de Varda,Evelina Fedorenko,Greta Tuckute*

Main category: cs.CL

TL;DR: 该研究使用认知神经科学的功能定位方法，在7个开源大语言模型中识别出对67种英语句法现象最敏感的单元，发现不同句法一致性现象（如主谓一致、照应一致、限定词-名词一致）会激活重叠的单元集合，表明一致性构成了大语言模型中有意义的功能类别。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型能够可靠地区分合乎语法和不合语法的句子，但语法知识如何在模型内部表示仍然是一个开放性问题。研究者希望探究不同句法现象在LLMs中是共享还是使用不同的组件。

Method: 采用认知神经科学启发的功能定位方法，在7个开源大语言模型中识别对67种英语句法现象最敏感的单元。这些单元在不同包含该现象的句子中被一致激活，并因果性地支持模型的句法表现。研究还进行了跨语言分析，涵盖英语、俄语、中文以及57种不同语言。

Result: 不同句法一致性类型（主谓一致、照应一致、限定词-名词一致）会激活重叠的单元集合，表明一致性构成了LLMs中有意义的功能类别。这一模式在英语、俄语和中文中都成立。在57种语言的跨语言分析中，结构更相似的语言在主谓一致上共享更多单元。

Conclusion: 句法一致性——作为句法依赖关系的关键标记——在大语言模型的表示空间中构成了一个有意义的类别，揭示了LLMs内部句法知识的组织方式。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够评估AI导师教学质量的工具，为教育利益相关者和研究社区提供标准化的评估框架，支持AI教育工具的质量改进和研究发展。

Method: 利用语言技术构建评估系统，开发包含演示评估、模型检查、数据可视化等功能的软件工具，支持用户反馈和标注收集。

Result: 成功开发了AITutor-EvalKit应用，为AI导师的教学质量评估提供了系统化的工具，支持教育利益相关者和ACL社区进行教学效果评估和研究。

Conclusion: AITutor-EvalKit是一个有效的AI导师教学质量评估工具，能够促进AI教育工具的质量提升，并为研究社区提供有价值的评估框架和数据收集平台。
...
TL;DR: DZ-TDPO是一个解决长对话系统中状态惰性的非破坏性对齐框架，通过动态KL约束和可学习时序注意力偏置，在保持零样本泛化能力的同时显著提升对话性能。


<details>
  <summary>Details</summary>
Motivation: 长对话系统存在"状态惰性"问题，即静态约束导致模型无法解决用户意图演变与历史上下文之间的冲突，限制了对话系统的动态适应能力。

Method: 提出DZ-TDPO框架，结合冲突感知的动态KL约束和可学习的时序注意力偏置，通过精确的注意力调节而非破坏性权重更新来缓解状态惰性。

Result: 在Multi-Session Chat数据集上，DZ-TDPO在Phi-3.5模型上达到86.2%的胜率，Qwen2.5-7B模型达到99.4%的胜率，同时保持零样本泛化能力和MMLU通用能力，揭示了"容量-稳定性权衡"现象。

Conclusion: 状态惰性可以通过精确的注意力调节而非破坏性权重更新来缓解，DZ-TDPO框架在保持模型通用能力的同时显著提升长对话系统的动态适应性能。
...
<details>
  <summary>Details</summary>
Motivation: 揭示当前大语言模型安全对齐策略的不足，发现模型在表示层面的脆弱性，即使表面无害的提示也可能被内部解释为有害指令。

Method: 通过系统性地将有害关键词（如"bomb"）替换为良性标记（如"carrot"）在多个上下文示例中，配合有害请求的前缀，实现表示劫持攻击。

Result: 攻击成功率高达74%（Llama-3.3-70B-Instruct），无需优化，可跨模型家族迁移，良性标记的早期层表示在后期层收敛为有害语义。

Conclusion: 当前对齐策略不足，需要在表示层面进行操作，揭示了LLM潜在空间中的新攻击面。
...
TL;DR: 将DoLa对比解码方法从仅解码器架构扩展到T5/FLAN-T5编码器-解码器架构，评估其对指令遵循能力的影响，发现对某些任务类别有改进，对其他任务有损害


<details>
  <summary>Details</summary>
Motivation: 对比解码方法如DoLa此前仅在解码器架构中实现，主要研究其对事实性的改进。本研究旨在将DoLa扩展到编码器-解码器架构（T5/FLAN-T5），并评估其对指令遵循能力的影响

Method: 将DoLa对比解码策略适配到T5和FLAN-T5模型家族，通过层间对比解码方法，并对FLAN-T5模型进行逐层分析，量化DoLa对token输出概率的影响

Result: DoLa对某些任务类别的文本生成忠实性有改进，但对其他任务有损害。通过逐层分析揭示了DoLa对token输出概率的具体影响机制

Conclusion: 这是首次在编码器-解码器架构中实现对比解码策略，为理解DoLa在不同架构和任务类型中的效果提供了新的见解
...
TL;DR: 研究提出一个优化大语言模型在心理学文本分类任务中提示工程的实证框架，发现构建定义、任务框架和示例对性能影响最大，推荐结合人工和自动生成的提示变体进行实证选择。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在文本分类中表现良好，但其输出高度依赖提示的措辞。心理学等领域的概念具有精确、理论驱动的定义，可能未在预训练数据中充分体现，需要专门优化提示工程来确保与专家判断的一致性。

Method: 提出一个优化LLM提示的实证框架，实验评估五种提示策略：基于编码本的经验提示选择、自动提示工程、角色提示、思维链推理和解释性提示，结合零样本和少样本分类，在三个心理学构建和两个模型上进行测试。

Result: 发现角色、思维链和解释性提示不能完全解决糟糕措辞提示带来的性能损失。最重要的提示特征是构建定义、任务框架，以及较次要的示例。与专家判断最一致的结果来自结合基于编码本的经验提示选择和自动提示工程的少样本提示。

Conclusion: 建议研究人员尽可能生成和评估尽可能多的提示变体（人工制作、自动生成或两者结合），基于训练数据集中的实证性能选择提示和示例，并在保留集上验证最终方法。这为需要与专家判断对齐的场景提供了实用、系统且理论驱动的LLM提示优化方法。
...
TL;DR: 该论文提出通过微调LLMs遵循医学共识指南进行逐步推理，以提升医疗预测的可解释性和准确性，并以Sepsis-3为例验证了该方法在推导正确性和预测性能上的优势。


<details>
  <summary>Details</summary>
Motivation: 当前医疗早期预测的机器学习方法过于关注预测准确性，而忽视了获得医疗从业者信任所需的可信解释。医学领域普遍存在共识指南，但现有方法未能有效利用这些结构化知识来指导模型的推理过程。

Method: 将医学共识指南转化为语言化的推理规则实例，用于微调大型语言模型，使其能够遵循共识指南进行逐步推理。同时采用多模态方法，将时间序列预测模型的输出表示与LLM集成，以处理稀疏和不规则的临床变量预测问题。

Result: 经过微调的小型模型在Sepsis-3任务上表现优于使用显式定义进行单次学习的大型LLM，以及在包含共识定义的医学文本上训练的模型。微调模型在未见患者数据上对规则（及例外）的推导正确性接近完美，表明主要瓶颈在于对未来稀疏不规则临床变量的预测。

Conclusion: 通过微调LLMs遵循医学共识指南进行逐步推理，可以有效提升医疗预测的可解释性和准确性。多模态集成方法能够改善对未来稀疏不规则临床变量的预测性能，为解决医疗早期预测中的时间泛化问题提供了有效途径。
...
TL;DR: FusedKV是一种通过跨层融合KV缓存来减少内存占用的方法，将顶层KV缓存设计为底层和中层信息的可学习融合，在保持性能的同时减少50%缓存内存。


<details>
  <summary>Details</summary>
Motivation: Transformer解码器的KV缓存在长序列中内存消耗巨大，现有的跨层KV缓存共享方法（如YOCO、CLA）性能通常不如层内方法（如GQA），需要找到更有效的KV缓存优化方案。

Method: 通过分析顶层键值的信息流，发现值主要来自底层，键则来自底层和中层。基于此提出FusedKV，顶层KV缓存是底层和中层最有信息量的KV缓存的可学习融合，直接在应用RoPE后的键上进行融合以避免重新计算旋转嵌入。进一步提出FusedKV-Lite，顶层KV缓存直接来自底层的值和中层的键，减少I/O开销。

Result: 在332M到4B参数的LLM实验中，该方法减少50%缓存内存，同时达到比标准Transformer解码器更低的验证困惑度，成为内存高效且高性能的架构替代方案。

Conclusion: FusedKV通过跨层融合最有信息量的KV缓存，在显著减少内存占用的同时保持甚至提升模型性能，为长序列处理提供了有效的KV缓存优化方案。
...
TL;DR: 该研究构建了包含巴斯克语标准、社交媒体和历史文本的多样化语料库，训练了BERnaT模型家族，发现结合多样化数据的模型在所有任务上表现更优，强调了语言多样性对构建包容性语言模型的重要性。


<details>
  <summary>Details</summary>
Motivation: 语言模型通常依赖经过质量过滤的大规模文本语料库，但这种过滤过程可能无意中排除非标准语言变体，降低模型鲁棒性并强化代表性偏见。研究者认为语言模型应该捕捉语言变异的全部谱系（方言、历史、非正式等），而不仅仅依赖标准化文本。

Method: 以巴斯克语（形态丰富且资源稀缺的语言）为研究对象，构建了结合标准、社交媒体和历史来源的新语料库，预训练了BERnaT系列的编码器模型，包括三种配置：标准、多样化和组合。提出了一个将自然语言理解任务分为标准和多样化子集的评估框架，以评估语言泛化能力。

Result: 结果显示，在标准和多样化数据上训练的模型始终优于仅在标准语料库上训练的模型，在所有任务类型上都有改进，且不影响标准基准测试的准确性。

Conclusion: 这些发现强调了语言多样性在构建包容性、可泛化语言模型中的重要性，表明结合多样化语言变体的训练能够提升模型性能而不损害标准任务表现。
...
Method: 引入BRAND数据集（双语宗教可问责规范数据集），专注于南亚四大宗教：佛教、基督教、印度教和伊斯兰教，包含2400多个条目，使用英语和孟加拉语三种不同类型的提示。

Result: 模型在英语中的表现优于孟加拉语，即使在回答宗教中立问题时也持续显示出对伊斯兰教的偏见。这些发现突显了多语言模型在不同语言中回答相似问题时存在的持续偏见。

Conclusion: 研究结果揭示了多语言模型在宗教敏感话题上的系统性偏见问题，并将这些发现与人机交互领域中宗教和灵性相关的更广泛问题联系起来。
...
TL;DR: 该研究提出两阶段方法将Qwen2.5-3B模型适配到藏语：先进行持续预训练建立语言基础，再进行监督微调提升任务性能，显著降低了困惑度并提升了翻译质量。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在低资源语言（如藏语）上的适配面临数据稀缺和跨语言漂移的挑战，需要开发有效的适配方法来扩展多语言基础模型到低资源语言环境。
...
Conclusion: CPT构建藏语语义流形，SFT以最小表征扰动优化任务对齐。该研究首次量化探索藏语适配动态，为低资源语言适配提供了开放可复现的框架。
...
TL;DR: 本文提出两种互补的tokenizer适应方法：持续BPE训练用于词汇扩展，叶基词汇剪枝用于词汇缩减，共同实现可控的词汇表修改。


<details>
  <summary>Details</summary>
Motivation: 将预训练语言模型迁移到新领域或语言时，tokenizer适应至关重要。传统方法通过在新领域文本上训练新tokenizer并添加非重叠词汇，但这种方法常导致许多词汇无法访问或从未使用，存在效率问题。

Method: 提出两种互补方法：1) 持续BPE训练：通过在新增数据上继续BPE合并学习过程来适应预训练tokenizer；2) 叶基词汇剪枝：在保持模型质量的同时移除冗余词汇。

Result: 在多种语言和模型家族上的实验表明，持续BPE训练提高了tokenization效率并更好地利用了新增词汇。两种方法共同为可控词汇表修改提供了实用工具。

Conclusion: 持续BPE训练和叶基词汇剪枝是有效的tokenizer适应方法，能够改善词汇扩展效率并减少冗余，已作为开源包发布。
...
<details>
  <summary>Details</summary>
Motivation: 随着增强型大语言模型在Web应用中的普及，提升推理服务效率、优化服务级别目标对改善用户体验至关重要。现有系统面临两大挑战：1）先到先服务调度导致严重的队头阻塞，使许多请求的排队延迟超出SLO；2）静态批处理令牌限制无法适应波动的负载和硬件条件，这两者都降低了有效吞吐量和服务质量。

Method: AugServe采用两阶段自适应请求调度策略：第一阶段结合增强型LLM请求的推理特征优化调度顺序决策；第二阶段利用运行时信息持续优化这些决策，适应请求特征和系统能力。此外，AugServe根据硬件状态和实时负载动态调整令牌批处理机制。
...
Method: 将SigLIP2视觉编码器与Qwen3语言主干通过注意力池化连接器耦合，该连接器支持任意分辨率图像的token高效处理

Result: 在标准VQA基准测试和多语言评估中，Jina-VLM优于同类模型，同时保持了有竞争力的纯文本性能
...
<details>
  <summary>Details</summary>
Motivation: 如何让模型掌握基础模型不具备的认知技能（如验证答案、回溯、尝试替代方法等），而不依赖从更强模型的蒸馏

Method: 在强化学习前进行监督微调，使用模型自身生成的样本重新组织成技能训练数据，形成"白银"SFT轨迹，为后续RL学习认知技能做准备

Result: 1) SkillFactory SFT初始化帮助模型在RL后泛化到更难任务变体；2) 模型确实使用了认知技能；3) SkillFactory RL模型在域外任务上比基础RL模型更稳健
...
### [122] [Advancing Multi-Step Mathematical Reasoning in Large Language Models through Multi-Layered Self-Reflection with Auto-Prompting](https://arxiv.org/abs/2506.23888)
*André de Souza Loureiro,Jorge Valverde-Rebaza,Julieta Noguez,David Escarcega,Ricardo Marcacini*

Main category: cs.CL

TL;DR: MAPS框架通过结合思维链、自我反思和自动提示技术，提升LLMs在复杂多步数学推理任务中的表现，在多个基准测试中优于标准CoT方法。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在问题解决能力上有所提升，但在处理复杂多步推理任务时仍然存在困难，需要更有效的推理增强方法。

Method: 提出多层自我反思与自动提示框架，采用迭代优化过程：首先生成思维链解决方案，当检测到错误时，自适应自我反思机制分析错误并生成定制提示来指导修正，通过动态调整提示实现推理的迭代优化。

Result: 在四个成熟基准测试中，MAPS显著优于标准CoT方法，与推理优化模型取得竞争性结果，使通用LLMs达到接近专用推理模型的性能水平。

Conclusion: MAPS框架通过战略性地限制反思深度，在成本和推理性能之间实现了最佳平衡，为提升LLMs的多步数学推理能力提供了有效解决方案。
...
### [123] [Physics-Informed Machine Learning for Steel Development: A Computational Framework and CCT Diagram Modelling](https://arxiv.org/abs/2512.03050)
...
TL;DR: 提出了一种结合物理洞察与机器学习的计算框架，用于开发钢材的物理信息连续冷却转变模型，该模型在4100个图表数据集上训练，能在5秒内生成完整的CCT图表，相分类F1分数超过88%，相变温度回归MAE低于20°C。


<details>
  <summary>Details</summary>
Motivation: 虽然机器学习在材料科学中已广泛应用于新化合物发现和制造过程优化，但将其应用于复杂工业材料如钢材仍面临挑战。主要障碍在于准确捕捉化学成分、加工参数与最终微观结构和性能之间的复杂关系。

Method: 引入了一个结合物理洞察与机器学习的计算框架，开发了物理信息连续冷却转变模型。该模型在4100个CCT图表数据集上进行训练，并针对文献和实验数据进行验证。

Result: 模型表现出高计算效率，在5秒内生成包含100条冷却曲线的完整CCT图表。在合金钢中展现出强大的泛化能力，所有相的相分类F1分数超过88%。相变温度回归的MAE在所有相中低于20°C，除了贝氏体为27°C。

Conclusion: 该框架可以通过添加更多通用和定制化ML模型扩展，建立通用的热处理数字孪生平台。与补充模拟工具和针对性实验的集成将进一步支持加速材料设计工作流程。
...
### [124] [Energy-Efficient Federated Learning via Adaptive Encoder Freezing for MRI-to-CT Conversion: A Green AI-Guided Research](https://arxiv.org/abs/2512.03054)
...
Result: 相比未冻结的对照方法，训练时间、总能耗和CO2eq排放减少高达23%；MRI到CT转换性能基本保持，MAE仅有微小变化；5种架构中3种无显著差异，2种有显著改进

Conclusion: 该工作符合促进深度学习框架满足临床需求同时确保气候、社会和经济可持续性的研究范式，为新的联邦学习评估框架奠定基础，推动AI驱动医疗中的隐私、公平和正义
...
### [125] [Delta Sampling: Data-Free Knowledge Transfer Across Diffusion Models](https://arxiv.org/abs/2512.03056)
*Zhidong Gao,Zimeng Pan,Yuhang Yao,Chenyue Xie,Wei Wei*

Main category: cs.LG

TL;DR: Delta Sampling (DS) 是一种无需原始训练数据、在推理时实现不同架构基础模型间知识迁移的新方法，通过利用适配前后的模型预测差异来引导新基础模型的去噪过程。
...
Method: Delta Sampling (DS) 完全在推理时操作，通过计算基础模型适配前后的预测差异（delta），然后利用这个delta来引导新基础模型的去噪过程，实现跨不同架构基础模型的知识迁移。

Result: 在多个Stable Diffusion版本上的评估表明，DS在不同采样策略下都能在创建期望效果（如视觉风格、语义概念和结构）方面实现一致的改进。

Conclusion: DS是一种有效、即插即用的机制，可用于扩散基图像合成中的知识迁移，解决了适配组件与特定基础模型紧密耦合的问题。
...
### [126] [Dynamical Properties of Tokens in Self-Attention and Effects of Positional Encoding](https://arxiv.org/abs/2512.03058)
*Duy-Tung Pham,An The Nguyen,Viet-Hoang Tran,Nhan-Phu Chung,Xin T. Tong,Tan M. Nguyen,Thieu N. Vo*

Main category: cs.LG

TL;DR: 该论文研究预训练Transformer模型中token的动态特性，分析其连续时间极限的动力学系统，表征token随时间的收敛或发散行为，并提出改进Transformer架构的简单方法。


<details>
  <summary>Details</summary>
Motivation: 研究预训练Transformer模型中token的动态特性，理解token如何随时间移动（相互靠近或远离），分析不同位置编码对动态行为的影响，并基于这些理论见解改进Transformer模型。

Method: 分析预训练模型的连续时间极限动力学系统，表征解的渐近行为，提供基于模型参数的充分条件来识别token收敛到零或发散到无穷的情况，研究绝对位置编码和旋转位置编码对动态机制的影响。

Result: 研究发现收敛情况会损害模型性能，提出了针对绝对位置编码和旋转位置编码的Transformer架构简单改进方法，这些改进可以减轻收敛行为，为改进Transformer模型提供了理论基础和设计原则。

Conclusion: 通过分析预训练Transformer中token的动态特性，揭示了收敛行为对模型性能的负面影响，提出了有效的架构改进方法，为Transformer模型的理论基础和设计原则提供了支持。
...
### [127] [Safe and Sustainable Electric Bus Charging Scheduling with Constrained Hierarchical DRL](https://arxiv.org/abs/2512.03059)
*Jiaju Qi,Lei Lei,Thorsteinn Jonsson,Dusit Niyato*

Main category: cs.LG

TL;DR: 提出了一种基于分层深度强化学习的安全框架，用于优化电动公交车充电调度，在考虑光伏发电、动态电价等多源不确定性的情况下，实现成本最小化并确保电池安全运行。


<details>
  <summary>Details</summary>
Motivation: 电动公交车与可再生能源（如光伏）结合是实现低碳公共交通的重要途径，但在实际运行中面临光伏发电不确定性、动态电价、旅行时间变化和充电设施有限等多重挑战，需要一种能够同时优化成本并确保电池安全运行的调度方法。

Method: 将问题建模为带约束的马尔可夫决策过程，提出DAC-MAPPO-Lagrangian分层深度强化学习算法。高层采用集中式PPO-Lagrangian学习安全的充电桩分配策略，低层采用MAPPO-Lagrangian在集中训练分散执行范式下学习分散的充电功率决策。

Result: 基于真实数据的实验表明，所提方法在成本最小化和安全合规性方面均优于现有基线方法，同时保持了较快的收敛速度。

Conclusion: 提出的安全分层深度强化学习框架能够有效解决多源不确定性下的电动公交车充电调度问题，为可持续低碳公共交通系统提供了可行的优化方案。
...
### [128] [A Large Scale Heterogeneous Treatment Effect Estimation Framework and Its Applications of Users' Journey at Snap](https://arxiv.org/abs/2512.03060)
*Jing Pan,Li Shi,Paul Lo*

Main category: cs.LG

TL;DR: 论文提出了一个大规模工业级框架，用于使用数亿Snapchat用户的实验数据估计异质性处理效应，通过整合多个实验发现潜在用户特征，并在广告影响力和广告敏感性方面取得显著应用效果。


<details>
  <summary>Details</summary>
Motivation: 传统处理效应模型假设所有用户的处理效应相同，但在实际应用中，不同用户对同一干预的反应存在显著差异。需要开发能够在大规模工业环境中准确估计异质性处理效应的方法，以更好地理解用户特征并优化业务决策。

Method: 构建了一个大规模工业框架，包括实验选择、基础学习器设计和增量训练等核心组件。通过整合数百个实验的数据，利用机器学习方法估计异质性处理效应，发现潜在用户特征，并在大规模上产生稳定的处理效应估计。

Result: 框架成功应用于用户对广告的影响力和敏感性分析。在线A/B测试显示，使用影响力分数进行定向广告投放，关键业务指标的提升幅度超过通常认为显著水平的六倍以上，证明了方法的实际有效性。

Conclusion: 该大规模工业框架能够有效估计异质性处理效应，通过整合多个实验数据发现先前无法测量的潜在用户特征。在广告定向等实际应用中表现出显著的业务价值，为大规模用户行为分析提供了有力工具。
...
### [129] [Globally optimized SVD compression of LLMs via Fermi-function-based rank selection and gauge fixing](https://arxiv.org/abs/2512.03062)
*Roman Rausch,David Jansen,Sukhbinder Singh,Román Orús*

Main category: cs.LG

TL;DR: 论文提出两种基于物理启发的SVD LLM压缩改进方法：FermiGrad算法通过费米函数将离散奇异值截断转化为连续优化来确定全局最优层秩；PivGa方法利用参数化中的规范自由度对低秩因子进行无损压缩。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型对计算资源需求很高，低秩分解（如SVD）是LLM压缩的有前景方法，但面临层秩选择和参数冗余等实际挑战。

Method: 1. FermiGrad：基于梯度下降算法，使用费米函数将离散奇异值截断松弛为连续优化，确定全局最优层秩；2. PivGa：利用低秩因子参数化中的规范自由度，对低秩因子进行无损压缩。

Result: 论文提出的两种方法改进了SVD LLM压缩效果，FermiGrad解决了层秩选择问题，PivGa进一步压缩了参数冗余。

Conclusion: 基于物理启发的FermiGrad和PivGa方法为LLM压缩提供了有效的改进方案，解决了SVD压缩中的层秩选择和参数冗余问题。
...
### [130] [Hierarchical clustering of complex energy systems using pretopology](https://arxiv.org/abs/2512.03069)
*Loup-Noe Levy,Jeremie Bosom,Guillaume Guerard,Soufian Ben Amor,Marc Bui,Hai Tran*

Main category: cs.LG

TL;DR: 使用预拓扑学建模建筑能耗分布，开发多准则层次聚类算法，通过Python库实现，在合成数据和真实能耗数据上验证了聚类效果。


<details>
  <summary>Details</summary>
Motivation: 对大规模分布式建筑进行逐个深入能耗审计需要大量时间、资金和专业人员，因此需要开发自动化方法来建立有效的能耗管理推荐系统。

Method: 使用预拓扑学建模站点能耗分布，基于预拓扑空间特性开发多准则层次分类算法，并实现为Python库。

Result: 在合成点数据集上能根据位置和大小识别聚类；在合成时间序列数据上使用皮尔逊相关系数获得ARI=1的完美聚类；在法国能源公司400个真实能耗站点的数据上进行了评估。

Conclusion: 该方法能有效建模和分类大规模分布式建筑的能耗分布，为优化建筑能耗管理提供自动化解决方案。
...
### [131] [Mixed Data Clustering Survey and Challenges](https://arxiv.org/abs/2512.03070)
...
<details>
  <summary>Details</summary>
Motivation: 大数据时代带来了前所未有的数据量、速度和多样性，混合数据聚类成为关键挑战。传统聚类方法通常针对同质数据集设计，难以处理混合数据带来的额外复杂性，需要专门针对这种场景的方法。

Method: 提出了一种基于预拓扑空间的聚类方法。预拓扑空间提供了一种结构化、可解释的聚类框架，特别适合处理混合数据类型。

Result: 通过与经典数值聚类算法和现有预拓扑方法进行基准测试，评估了所提出方法在大数据范式下的性能和有效性。

Conclusion: 基于预拓扑空间的聚类方法为混合数据聚类提供了有效的解决方案，能够处理大数据环境下的异构数据类型，提供结构化、可解释的聚类结果，支持明智的决策制定。
...
### [132] [PretopoMD: Pretopology-based Mixed Data Hierarchical Clustering](https://arxiv.org/abs/2512.03071)
...
<details>
  <summary>Details</summary>
Motivation: 解决混合数据聚类挑战，避免传统降维技术导致的数据完整性损失，提高聚类可解释性，克服聚类数据可解释性问题

Method: 基于预拓扑的算法，利用析取范式构建可定制逻辑规则和可调超参数，支持用户定义层次聚类结构，直接从原始数据构建层次聚类树状图

Result: 通过层次树状图分析和聚类指标比较，该方法在准确性和可解释性方面表现优异，能够从原始数据中清晰界定聚类，保持数据完整性，构建有意义的聚类

Conclusion: 该工作创新性地避免了传统降维技术，通过逻辑规则增强聚类形成和清晰度，为混合数据聚类领域做出了重要贡献
...
### [133] [Risk-Entropic Flow Matching](https://arxiv.org/abs/2512.03078)
*Vahid R. Ramezani,Benjamin Englard*

Main category: cs.LG

TL;DR: 本文提出将倾斜风险（tilted risk）应用于流匹配（Flow Matching），通过log-exponential变换改进标准均方误差损失，以更好地捕捉数据流形的几何结构和少数分支。


<details>
  <summary>Details</summary>
Motivation: 标准整流流匹配使用均方误差损失，将所有到达同一时空点的速度目标压缩为单一条件均值，忽略了高阶条件信息（方差、偏度、多模态），这些信息编码了数据流形的精细几何结构和少数分支。
...
Conclusion: 倾斜风险损失为流匹配提供了一种改进方法，能够更好地捕捉数据流形的高阶统计特性和几何结构，特别是在处理模糊性和稀有事件时表现更优。
...
### [134] [ALARM: Automated MLLM-Based Anomaly Detection in Complex-EnviRonment Monitoring with Uncertainty Quantification](https://arxiv.org/abs/2512.03101)
*Congjing Zhang,Feng Lin,Xinyi Zhao,Pei Guo,Wei Li,Lin Chen,Chaoyue Zhao,Shuai Huang*

Main category: cs.LG

TL;DR: ALARM是一个基于多模态大语言模型的视觉异常检测框架，集成了不确定性量化技术，通过推理链、自反思和模型集成等方法提升鲁棒性和准确性，在智能家居和伤口图像分类等实际场景中表现出色。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的发展，基于多模态大语言模型的视觉异常检测算法在复杂环境中的应用受到关注。然而，复杂环境中的异常往往具有高度情境依赖性和模糊性，因此不确定性量化成为这类系统成功的关键能力。

Method: ALARM框架将不确定性量化与质量保证技术（如推理链、自反思和多模态大语言模型集成）相结合，基于严格的概率推理流程和计算过程设计，实现鲁棒且准确的异常检测。

Result: 通过使用真实世界的智能家居基准数据和伤口图像分类数据进行广泛实证评估，结果显示ALARM具有卓越的性能，并在不同领域展现出通用的适用性，能够支持可靠的决策制定。

Conclusion: ALARM框架成功地将不确定性量化集成到多模态大语言模型的视觉异常检测中，通过质量保证技术提升了系统的鲁棒性和准确性，在多个实际应用场景中证明了其有效性和通用性。
...
### [135] [Detecting AI Hallucinations in Finance: An Information-Theoretic Method Cuts Hallucination Rate by 92%](https://arxiv.org/abs/2512.03107)
*Mainak Singha*
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型会产生流畅但无根据的回答（幻觉），这限制了其在高风险领域的安全部署。需要一种机制来检测和减少幻觉。

Method: 提出ECLIPSE框架，结合多样本聚类的熵估计和新的困惑度分解方法，测量模型如何使用检索到的证据。证明在温和条件下，得到的熵-容量目标是严格凸的，具有唯一稳定最优解。

Result: 在受控金融问答数据集上，ECLIPSE达到ROC AUC 0.89和平均精度0.90，显著优于仅使用语义熵的基线（AUC 0.50）。在Claude-3-Haiku上的消融实验显示AUC降至0.59，系数幅度下降95%，证明ECLIPSE是依赖校准令牌级不确定性的logprob原生机制。

Conclusion: ECLIPSE通过分析模型如何使用证据来有效检测幻觉，困惑度分解特征具有最大的学习系数，表明证据利用是幻觉检测的核心。这是一个受控机制研究，跨领域和自然发生幻觉的验证是未来工作。
...
### [136] [E-valuator: Reliable Agent Verifiers with Sequential Hypothesis Testing](https://arxiv.org/abs/2512.03109)
...
<details>
  <summary>Details</summary>
Motivation: 现有的智能体轨迹验证器（如LLM评判器和过程奖励模型）虽然能提供启发式评分，但缺乏正确性保证。需要一种能将验证器分数转换为具有统计保证的决策规则的方法，以可靠地区分成功和失败的智能体轨迹。

Method: 将智能体轨迹成功与否的区分问题构建为序贯假设检验问题。基于e过程工具开发序贯假设检验，在智能体轨迹的每一步都保持统计有效性，支持对任意长动作序列的在线监控。

Result: 在六个数据集和三种智能体上，e-valuator相比其他策略提供了更高的统计功效和更好的误报率控制。还能用于快速终止问题轨迹并节省token使用。
...
### [137] [Beyond Additivity: Sparse Isotonic Shapley Regression toward Nonlinear Explainability](https://arxiv.org/abs/2512.03112)
*Jialai She*

Main category: cs.LG

TL;DR: SISR是一个统一的非线性解释框架，通过同时学习单调变换恢复可加性和施加L0稀疏约束，解决Shapley值在非可加性支付函数和高维稀疏性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统Shapley值框架面临两个主要问题：1) 假设支付函数具有可加性，但现实世界中的非高斯分布、重尾、特征依赖或特定领域损失尺度往往违反这一假设，导致扭曲的归因；2) 在高维空间中通过计算密集Shapley值再应用临时阈值化来实现稀疏解释成本过高且存在不一致风险。

Method: 提出Sparse Isotonic Shapley Regression (SISR)框架，同时学习单调变换以恢复可加性（无需封闭形式规范），并对Shapley向量施加L0稀疏约束。优化算法利用Pool-Adjacent-Violators进行高效等渗回归，使用归一化硬阈值进行支持选择，实现易于实施和全局收敛保证。

Result: 分析表明SISR在广泛场景中能恢复真实变换，即使在噪声较高情况下也能实现强大的支持恢复。首次证明无关特征和特征间依赖可以诱导显著偏离线性的真实支付变换。在回归、逻辑回归和树集成实验表明，SISR在不同支付方案下稳定归因，正确过滤无关特征，而标准Shapley值则遭受严重的排序和符号扭曲。

Conclusion: 通过统一非线性变换估计与稀疏性追求，SISR推进了非线性可解释性的前沿，提供了一个理论基础扎实且实用的归因框架。
...
TL;DR: MoDE提出了一种轻量级可扩展架构，通过解耦模态特定更新来缓解UMGMs中的模态间遗忘问题，在统一多模态生成设置中优于现有持续学习方法。


<details>
  <summary>Details</summary>
Motivation: 统一多模态生成模型在持续学习新任务时存在灾难性遗忘问题，包括模态内遗忘和模态间遗忘。虽然模态内遗忘已有研究，但模态间遗忘尚未被充分探索，本文旨在解决这一问题。

Method: 提出MoDE（Modality-Decoupled Experts）架构，通过隔离模态特定更新来缓解梯度冲突，并利用知识蒸馏防止灾难性遗忘和保留预训练能力。

Result: 在多样化基准测试中，MoDE显著缓解了模态间和模态内遗忘，在统一多模态生成设置中优于先前的持续学习基线方法。

Conclusion: MoDE通过显式解耦模态来防止干扰，有效解决了UMGMs中的模态间遗忘问题，为统一多模态模型的持续学习提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: NMR谱解析是确定小分子结构的关键技术，但传统方法耗时且需要专业知识。研究人员希望开发自动化工具来加速分子发现过程，特别是针对天然产物和临床治疗药物的结构解析。

Method: 将结构解析构建为条件生成问题，采用基于非等变transformer架构的原子扩散模型。创建了包含超过111,000个天然产物的模拟1D NMR谱数据集来训练模型。

Result: ChefNMR在具有挑战性的天然产物化合物结构预测中达到了超过65%的准确率，这是目前最高的准确率水平。

Conclusion: 该研究在自动化小分子结构解析这一重大挑战上迈出了重要一步，展示了深度学习在加速分子发现方面的潜力。代码已开源。
...
Method: 使用向量量化变分自编码器(VQ-VAE)框架，通过k-mer标记化序列学习基因组模式的离散码本。扩展基础架构：添加掩码重建预训练以增强对缺失数据的鲁棒性，结合对比学习获得高判别性嵌入。

Result: 在约10万条SARS-CoV-2废水测序数据上，VQ-VAE达到99.52%平均标记级精度和56.33%精确序列匹配率，码本利用率为19.73%。对比微调显著改善聚类：64维嵌入Silhouette分数提升35%，128维嵌入提升42%。

Conclusion: 该无参考框架为基因组监测提供了可扩展、可解释的方法，可直接应用于公共卫生监控，有效应对废水监测中的计算挑战。
...
TL;DR: 论文提出"交错推理"方法，让语言模型在思考过程中交替输出中间结果，替代传统的"先思考后回答"模式，减少用户等待时间并提高任务成功率。


<details>
  <summary>Details</summary>
Motivation: 传统推理模型在长时间思考时不向用户提供任何提示，用户无法了解推理是否正确，也无法在模型基于错误前提推理时及时纠正，造成时间浪费和糟糕体验。相比之下，人类对话中会进行轻量级、渐进式的确认来确保双方理解一致。

Method: 提出交错推理方法，模型在思考过程中交替输出中间响应；进一步提出Plantain方法，第一个中间响应是明确的逐步执行计划，允许用户干预并为后续推理步骤提供早期反馈。

Result: Plantain方法在多个具有挑战性的数学推理和编码基准测试中实现了约6%的pass@1提升，同时相对于"先思考后回答"基线，首次响应时间减少了60%以上。

Conclusion: 交错推理方法能够在不影响最终响应质量的情况下减少感知延迟，让语言模型能够像人类对话一样进行渐进式确认，改善用户体验并提高任务成功率。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模单细胞RNA测序数据中罕见细胞亚群快速检测的挑战，以及流数据中概念漂移高效检测的需求。

Method: FiRE/FiRE.1采用草图技术进行异常检测；Enhash使用投影哈希的集成学习方法检测概念漂移。

Result: FiRE/FiRE.1在检测罕见细胞亚群方面优于现有技术；Enhash在各种漂移类型下在时间和准确性方面都具有高度竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 策略梯度方法在部分可观测环境中学习时，对于无记忆策略表现良好，但对于需要记忆的策略效果不佳。本文旨在改进学习带记忆策略的算法。

Method: 开发了多种改进算法，用于在无限时域设置中学习带记忆的策略：当环境模型已知时直接学习，否则通过模拟学习。

Result: 在大型POMDP问题上进行了算法比较，包括噪声机器人导航和多智能体问题。

Conclusion: 本文提出的改进算法能够更好地处理需要记忆策略的部分可观测环境问题，为这类问题的解决提供了更有效的方法。
...
### [144] [Perch 2.0 transfers 'whale' to underwater tasks](https://arxiv.org/abs/2512.03219)
*Andrea Burns,Lauren Harrell,Bart van Merriënboer,Vincent Dumoulin,Jenny Hamer,Tom Denton*

Main category: cs.LG

TL;DR: Perch 2.0在海洋哺乳动物音频分类任务中表现出色，即使其训练数据中几乎没有海洋哺乳动物音频，通过少样本迁移学习仍能超越其他生物声学模型。


<details>
  <summary>Details</summary>
Motivation: 尽管Perch 2.0在14,597个物种上预训练，但训练数据中几乎没有海洋哺乳动物音频，因此需要评估其在海洋哺乳动物和水下音频任务上的迁移学习能力。

Method: 使用Perch 2.0生成的嵌入进行线性探测，与多种预训练生物声学模型（包括Perch 1.0、SurfPerch、AVES-bio等）进行少样本迁移学习性能比较。

Result: Perch 2.0的嵌入在少样本迁移学习中表现出持续高性能，在大多数任务上优于其他嵌入模型，特别适合在标记样本有限的情况下开发海洋哺乳动物分类器。

Conclusion: Perch 2.0是一个强大的生物声学基础模型，即使训练数据中缺乏特定类别的音频，仍能通过迁移学习在相关任务上取得优异表现，推荐用于海洋哺乳动物分类的少样本学习场景。
...
### [145] [SPARK: Stepwise Process-Aware Rewards for Reference-Free Reinforcement Learning](https://arxiv.org/abs/2512.03244)
*Salman Rahman,Sruthi Gorantla,Arpit Gupta,Swastik Roy,Nanyun Peng,Yang Liu*

Main category: cs.LG

TL;DR: SPARK框架通过三阶段方法生成合成训练数据来训练过程奖励模型，无需昂贵的步级标注或真实参考，在数学推理任务中超越了基于真实监督的方法。


<details>
  <summary>Details</summary>
Motivation: 过程奖励模型（PRMs）虽然能提供密集的步级反馈，但受限于昂贵的步级标注或真实参考需求，限制了其广泛应用。需要一种无需真实参考就能训练高质量PRMs的方法。

Method: 提出SPARK三阶段框架：1）生成器模型产生多样解，验证器模型通过并行扩展（自一致性）和序列扩展（元批判）评估解；2）用验证输出作为合成训练数据微调生成式过程奖励模型；3）将PRM与思维链验证结合作为RL奖励模型，并引入格式约束防止奖励黑客攻击。

Result: 在ProcessBench上达到67.5 F1，优于参考引导训练的66.4和GPT-4o的61.9；在六个数学推理基准上平均准确率47.4%，优于基于真实监督的RLVR的43.9%。
...
### [146] [Too Late to Recall: Explaining the Two-Hop Problem in Multimodal Knowledge Retrieval](https://arxiv.org/abs/2512.03276)
*Constantin Venhoff,Ashkan Khakzar,Sonia Joseph,Philip Torr,Neel Nanda*

Main category: cs.LG

TL;DR: 视觉语言模型在事实召回任务上表现不如其LLM骨干，主要原因是实体表示形成太晚，无法有效利用LLM已有的知识召回机制。


<details>
  <summary>Details</summary>
Motivation: 许多视觉语言模型在事实召回性能上相比其LLM骨干有所下降，这引发了对多模态微调在扩展LLM机制到视觉输入方面有效性的质疑。研究旨在理解VLM为何在事实召回任务上表现不佳。

Method: 1. 对14个不同架构、规模和训练设置的VLM进行基准测试；2. 使用归因修补、激活修补和探测技术分析性能差异；3. 通过两种方法恢复性能：从LLM骨干修补实体表示到VLM，以及使用思维链提示。

Result: 14个模型中有11个表现出事实召回性能下降。性能下降的VLM因实体表示形成太晚而无法有效利用LLM已有的知识召回机制，而高性能VLM能早期形成实体表示从而重用现有机制。通过修补实体表示和使用思维链提示可以恢复性能。

Conclusion: 早期实体表示形成的速度是决定VLM能否有效利用LLM已有机制的关键因素。机制分析可以解释和揭示多模态对齐中的系统性失败。
...
### [147] [Multi-Frequency Federated Learning for Human Activity Recognition Using Head-Worn Sensors](https://arxiv.org/abs/2512.03287)
*Dario Fenoglio,Mohan Li,Davide Casnici,Matias Laporte,Shkurta Gashi,Silvia Santini,Martin Gjoreski,Marc Langheinrich*

Main category: cs.LG

TL;DR: 该论文提出多频联邦学习方法用于头戴设备的人体活动识别，在保护隐私的同时解决设备间采样频率差异问题。


<details>
  <summary>Details</summary>
Motivation: 传统人体活动识别方法依赖集中式用户数据，存在隐私泄露风险，且不同设备采样频率不同，需要一种既能保护隐私又能处理频率差异的解决方案。

Method: 提出多频联邦学习方法，专注于头戴设备（如耳机和智能眼镜），通过联邦学习框架在保护隐私的前提下，联合学习跨不同采样频率设备的机器学习模型。

Result: 在两个数据集上相比特定频率方法有所改进，表明多频联邦学习在人体活动识别任务中具有良好前景。

Conclusion: 多频联邦学习为头戴设备的人体活动识别提供了一种隐私保护且能处理频率差异的有效方法，具有进一步研究和开发的价值。
...
### [148] [Adaptive Regime-Switching Forecasts with Distribution-Free Uncertainty: Deep Switching State-Space Models Meet Conformal Prediction](https://arxiv.org/abs/2512.03298)
*Echo Diyun LU,Charles Findling,Marianne Clausel,Alessandro Leite,Wei Gong,Pierric Kersaudy*

Main category: cs.LG

TL;DR: 该研究将自适应共形推断（ACI）与深度切换状态空间模型结合，为非平稳时间序列中的切换机制预测提供分布自由的置信区间，确保有限样本下的边际覆盖保证。


<details>
  <summary>Details</summary>
Motivation: 时间序列中的机制转换会破坏平稳性，使得校准的不确定性变得与点预测精度同等重要。研究旨在为机制切换预测提供分布自由的置信区间。

Method: 将深度切换状态空间模型与自适应共形推断（ACI）及其聚合变体（AgACI）相结合，并引入统一的共形包装器，可应用于S4、MC-Dropout GRU、稀疏高斯过程和变点局部模型等多种序列基线模型。

Result: 在合成和真实数据集上，共形化预测器实现了接近名义覆盖率的预测区间，具有竞争力的准确性，并且通常提高了区间效率。

Conclusion: 该方法为非平稳时间序列预测提供了可靠的置信区间，在模型误设和非平稳性条件下仍能保证有限样本的边际覆盖保证，为机制切换预测提供了实用的不确定性量化工具。
...
### [149] [HydroDCM: Hydrological Domain-Conditioned Modulation for Cross-Reservoir Inflow Prediction](https://arxiv.org/abs/2512.03300)
*Pengfei Hu,Fan Ming,Xiaoxue Han,Chang Lu,Yue Ning,Dan Lu*

Main category: cs.LG

TL;DR: HydroDCM：一个用于跨水库入库流量预测的可扩展域泛化框架，通过空间元数据构建伪域标签指导对抗学习，并在推理时通过轻量级条件层适应目标水库特征


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在水库入库流量预测中表现良好，但在应用于不同水库时性能下降，存在域偏移问题。传统域泛化技术在水文多域系统中适用性有限，因为每个水库有独特的入库模式，而空间元数据等非观测信息具有间接但重要的影响。

Method: 提出HydroDCM框架：1）利用水库空间元数据构建伪域标签，指导对抗学习提取不变的时间特征；2）在推理阶段，通过轻量级条件层根据目标水库的元数据自适应调整特征，实现域泛化的不变性与位置特定适应的平衡。

Result: 在科罗拉多河上游流域30个真实水库上的实验结果表明，该方法在多域条件下显著优于最先进的域泛化基线方法，同时保持计算效率。

Conclusion: HydroDCM通过结合伪域标签的对抗学习和元数据驱动的轻量级条件层，有效解决了水文多域系统中的域泛化问题，为跨水库入库流量预测提供了可扩展的解决方案。
...
### [150] [Robust Tabular Foundation Models](https://arxiv.org/abs/2512.03307)
...
<details>
  <summary>Details</summary>
Motivation: 表格基础模型（TFMs）在结构化数据上展现出超越传统ML方法的潜力。现有研究主要关注设计高质量的先验生成器来提升整体预训练性能，但作者发现参数化生成器分布可以从对抗鲁棒性角度出发，在训练中调整生成器以强调对模型特别困难的数据集。

Method: 提出了RTFM（Robust Tabular Foundation Models）框架，引入最优性间隙度量（TFM性能与XGBoost、CatBoost、Random Forests等强基线最佳可达性能之间的差异），基于此进行模型无关的对抗训练。应用于TabPFN V2分类器，仅需不到10万个额外合成数据集。

Result: RTFM显著提升了基准测试性能，相比原始TabPFN和其他基线算法，平均归一化AUC最高提升6%。

Conclusion: 这项工作为仅使用合成数据进行TFMs的针对性对抗训练和微调开辟了有前景的新方向。
...
### [151] [Single-Round Scalable Analytic Federated Learning](https://arxiv.org/abs/2512.03336)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习面临两个关键挑战：高通信开销和异构数据上的性能崩溃。分析式联邦学习提供了单轮、数据分布不变的解决方案，但仅限于线性模型。后续的非线性方法如DeepAFL恢复了准确性，但牺牲了单轮优势。

Method: 提出SAFLe框架，通过引入结构化头部（bucketed features）和稀疏分组嵌入，实现可扩展的非线性表达能力。证明这种非线性架构在数学上等价于高维线性回归，从而能够使用AFL的单次、不变聚合定律进行求解。

Result: SAFLe在分析式联邦学习中建立了新的最先进性能，在所有基准测试中显著优于线性AFL和多轮DeepAFL的准确性，为联邦视觉提供了高效可扩展的解决方案。

Conclusion: SAFLe打破了联邦学习中非线性表达能力和单轮通信之间的权衡，通过数学等价性实现了既保持单轮优势又具备非线性建模能力的解决方案，为联邦视觉任务提供了高效可扩展的框架。
...
### [152] [Breaking Determinism: Stochastic Modeling for Reliable Off-Policy Evaluation in Ad Auctions](https://arxiv.org/abs/2512.03354)
*Hongseon Yeom,Jaeyoul Shin,Soojin Min,Jeongmin Yoon,Seunghak Yu,Dongyeop Kang*

Main category: cs.LG

TL;DR: 提出首个确定性广告拍卖中的离策略评估框架，通过重新利用出价景观模型近似倾向得分，实现无需在线A/B测试的可靠评估
...
Result: 在AuctionNet仿真基准和工业平台2周在线A/B测试中验证，CTR预测达到92%平均方向准确率，显著优于参数基线

Conclusion: 提出了首个实用且经验证的可信离策略评估框架，为确定性拍卖环境提供高效替代方案，减少在线实验成本和风险
...
### [153] [Tuning-Free Structured Sparse Recovery of Multiple Measurement Vectors using Implicit Regularization](https://arxiv.org/abs/2512.03393)
*Lakshmi Jayalal,Sheetal Kalyani*

Main category: cs.LG

TL;DR: 提出一种基于隐式正则化的免调参框架，用于解决多测量向量（MMV）中的联合稀疏信号恢复问题，无需先验稀疏度或噪声方差信息。


<details>
  <summary>Details</summary>
Motivation: 传统MMV方法如M-OMP和M-FOCUSS需要仔细的参数调优或对信号稀疏度和噪声方差的先验知识，这在实际应用中存在限制。
...
Conclusion: 提出的免调参框架通过隐式正则化有效解决MMV联合稀疏恢复问题，无需先验信息或参数调优，性能与传统方法相当。
...
### [154] [Full-Stack Alignment: Co-Aligning AI and Institutions with Thick Models of Value](https://arxiv.org/abs/2512.03399)
...
TL;DR: 论文提出需要"全栈对齐"——同时对齐AI系统及其塑造机构与人类价值观，而非仅对齐单个AI系统与其操作者的意图。现有价值表示方法存在局限，需要"厚价值模型"来有效区分价值观与其他信号、支持规范性推理和建模集体利益。


<details>
  <summary>Details</summary>
Motivation: 当前AI对齐方法存在根本缺陷：即使AI系统完美对齐其操作组织的意图，如果该组织的目标与其他机构和个人的目标不一致，仍可能导致不良社会结果。仅对齐单个AI系统不足以确保有益的社会成果。

Method: 提出"厚价值模型"作为解决方案，这种模型结构化地表示价值观和规范，使系统能够：1)区分持久价值观与短暂偏好；2)建模个体选择的社会嵌入性；3)进行规范性推理，将价值观应用于新领域。论文在五个领域展示了该方法的应用。

Result: 论文展示了厚价值模型在五个具体领域的应用：AI价值管理、规范性能力智能体、双赢谈判系统、意义保持的经济机制以及民主监管机构。这些应用展示了如何通过结构化价值表示来解决当前对齐方法的局限性。

Conclusion: 需要从简单的AI对齐转向"全栈对齐"，即同时对齐AI系统及其塑造机构与人类价值观。厚价值模型为解决当前价值表示方法的局限性提供了有前景的途径，能够支持更有效的规范性推理和集体利益建模，而无需强加特定的个人或集体繁荣愿景。
...
TL;DR: 研究显式世界建模目标如何影响Transformer在不同训练阶段的内部表示和下游能力，使用2x2x2魔方任务发现显式世界建模能产生更线性可解码和因果可控的状态表示，并提升强化学习后训练效果
...
Method: 使用2x2x2魔方作为控制环境，比较标准的下一个token预测与两种显式世界建模策略：(i)状态预测预训练和(ii)联合状态预测+下一个token目标，然后应用Group Relative Policy Optimization进行后训练，使用线性探针和因果干预评估表示质量

Result: 显式世界建模产生更线性可解码和因果可控的状态表示，改进的状态表示显著提升GRPO后训练效果，特别是在更难的魔方状态上表现更明显

Conclusion: 显式世界建模能锐化状态表示，从而提高序列规划任务的后训练效果，表明改进状态表示质量可以增强Transformer在规划任务中的表现
...
<details>
  <summary>Details</summary>
Motivation: 传统LiNGAM方法依赖脆弱且对样本敏感的高斯性检验，这限制了因果推断的效率和实际应用。需要一种更稳健的方法来避免显式高斯性检验。
...
Conclusion: 该方法提高了因果推断的效率和实际适用性，使LiNGAM在现实场景中更易用和可靠，无需显式高斯性检验即可实现稳健的因果发现。
...
TL;DR: 研究发现，在模型完成"顿悟"（grokking）阶段后应用机器遗忘方法，相比早期停止的模型，能实现更高效、更稳定、副作用更小的数据遗忘效果。


<details>
  <summary>Details</summary>
Motivation: 探索"顿悟"现象（训练数据拟合后出现的延迟泛化）是否有助于机器遗忘任务，即在不完全重新训练的情况下移除特定数据的影响。

Method: 在视觉（CNN/ResNet在CIFAR、SVHN、ImageNet）和语言（transformer在TOFU风格设置）任务上，比较在顿悟前与顿悟后应用标准遗忘方法的效果，分析特征和曲率变化。

Result: 从顿悟后的检查点开始遗忘，能实现：(1) 更高效的遗忘（达到目标遗忘水平需要更少更新），(2) 更小的附带损害（在保留数据和测试集上性能下降更小），(3) 跨种子的更新更稳定。

Conclusion: 顿悟后的模型学习到更模块化的表示，减少了遗忘子集和保留子集之间的梯度对齐，从而促进选择性遗忘。模型何时训练（顿悟前vs后）是改进现有遗忘方法的正交杠杆。
...
TL;DR: 提出一个端到端的深度学习框架，整合金融意见的两种模态（时效性和流行性），通过跨模态注意力机制进行金融情感分析，在837家公司数据集上达到83.5%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以有效整合多样化的意见模态并捕捉模态间的细粒度交互。金融情感分析对市场预测和风险评估越来越重要，但需要更好地处理不同信息渠道（时效性市场更新 vs 流行性集体情感）的整合。

Method: 提出端到端深度学习框架，整合两种金融意见模态：时效性模态（及时意见）和流行性模态（趋势意见）。使用BERT（Chinese-wwm-ext）进行特征嵌入，然后采用新提出的金融多头跨注意力（FMHCA）结构促进不同意见模态间的信息交换。处理后的特征通过transformer层优化，并使用多模态因子双线性池化进行融合，分类为负面、中性和正面情感。

Result: 在涵盖837家公司的综合数据集上进行广泛实验，该方法达到83.5%的准确率，显著优于包括BERT+Transformer在内的基线方法，性能提升21%。

Conclusion: 该框架展示了整合不同金融意见模态（时效性和流行性）并通过跨模态注意力机制进行交互的有效性，能够支持更准确的金融决策和风险管理。
...
TL;DR: 本文提出了一种新的贝叶斯亚型事件模型（BEBMS），在疾病进展亚型推断任务中显著优于现有的SuStaIn方法，特别是在模型设定错误的情况下表现更稳健。


<details>
  <summary>Details</summary>
Motivation: 慢性疾病在不同患者中通常以少数几种亚型模式进展，SuStaIn方法被广泛用于从横断面数据推断疾病亚型，但其稳健性尚未得到充分验证。本文旨在开发更稳健的贝叶斯方法并评估其性能。

Method: 开发了基于贝叶斯框架的亚型事件模型（BEBMS），通过合成数据实验在不同程度的模型设定错误下比较BEBMS与SuStaIn的性能，并在真实阿尔茨海默病数据集上进行应用验证。

Result: BEBMS在排序、分期和亚型分配任务中显著优于SuStaIn。在真实阿尔茨海默病数据中，BEBMS的结果与科学共识更一致，而SuStaIn的结果存在不一致性。

Conclusion: BEBMS作为SuStaIn的贝叶斯替代方案，在疾病进展亚型推断中表现出更好的稳健性和准确性，特别是在模型设定错误的情况下，为疾病亚型分析提供了更可靠的工具。
...
TL;DR: JPM是一个概率框架，用于从横断面数据推断多种神经退行性疾病的联合进展，通过将单病轨迹视为部分排序并构建联合进展先验，相比单病EBM基线提高了约21%的排序准确性。


<details>
  <summary>Details</summary>
Motivation: 标准事件模型假设每个个体只有单一疾病，但神经退行性疾病中混合病理很常见。需要开发能够处理多种疾病联合进展的模型。

Method: 提出联合进展模型（JPM），将单病轨迹视为部分排序，构建联合进展先验。研究了四种变体：Pairwise、Bradley-Terry、Plackett-Luce和Mallows。分析了校准性、分离度和锐度三个特性。

Result: 所有变体都具有校准性，分离度接近完美；锐度因变体而异，可通过输入部分排序的简单特征（数量、长度、冲突和重叠）预测。在合成实验中，JPM比单病EBM基线（SA-EBM）提高了约21%的排序准确性。在NACC数据中，Mallows变体和基线模型的结果与AD和VaD混合病理进展的先前文献更一致。

Conclusion: JPM是一个有效的框架，用于从横断面数据推断多种神经退行性疾病的联合进展，相比单病模型有显著改进，特别是在处理混合病理时。
...
### [161] [Adaptive sampling using variational autoencoder and reinforcement learning](https://arxiv.org/abs/2512.03525)
...
<details>
  <summary>Details</summary>
Motivation: 传统压缩感知使用通用基和随机测量，效率和质量有限；最优传感器布局基于历史数据但使用固定线性基，无法适应非线性或样本特异性变化；生成模型压缩感知使用深度生成先验但采样仍不最优

Method: 提出自适应稀疏感知框架，将变分自编码器先验与强化学习耦合，通过强化学习顺序选择测量点，实现自适应采样

Result: 实验表明该方法在稀疏测量重建方面优于压缩感知、最优传感器布局和生成模型重建方法

Conclusion: 自适应稀疏感知框架通过结合变分自编码器先验和强化学习，实现了更优的自适应采样和重建性能
...
### [162] [Parameter-Efficient Augment Plugin for Class-Incremental Learning](https://arxiv.org/abs/2512.03537)
*Zhiming Xu,Baile Xu,Jian Zhao,Furao Shen,Suorong Yang*

Main category: cs.LG

TL;DR: 提出DLC方法，通过LoRA组件插件扩展范式解决类增量学习问题，仅需少量参数即可显著提升准确率


<details>
  <summary>Details</summary>
Motivation: 现有类增量学习方法存在遗忘、稳定性-可塑性困境或参数增长过大的问题，需要一种高效且准确的解决方案

Method: 将特征提取器作为基础模型，为每个任务使用LoRA注入任务特定残差，通过轻量级加权单元聚合不同LoRA调整的表示，实现即插即用增强

Result: 在ImageNet-100上，仅使用ResNet-18 4%的参数就实现了8%的准确率提升，在固定内存预算下超越现有最优方法

Conclusion: DLC方法提供了一种高效、准确的类增量学习解决方案，通过LoRA插件范式在保持参数效率的同时显著提升性能
...
### [163] [Towards Irreversible Machine Unlearning for Diffusion Models](https://arxiv.org/abs/2512.03564)
*Xun Yuan,Zilong Zhao,Jiayu Li,Aryan Pasikhani,Prosanta Gope,Biplab Sikdar*

Main category: cs.LG

TL;DR: 本文提出了针对扩散模型微调式机器遗忘方法的重新学习攻击DiMRA，以及相应的防御方法DiMUM，揭示了现有遗忘技术的脆弱性并提供了更鲁棒的解决方案。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成合成图像方面表现出色，但存在安全、隐私和版权问题，需要机器遗忘技术来让模型忘记特定训练数据。然而，现有的基于微调的机器遗忘方法存在安全漏洞，可能被攻击者逆转。

Method: 1. 提出DiMRA攻击：在没有遗忘元素先验知识的情况下，通过在辅助数据集上优化已遗忘的扩散模型来逆转遗忘过程；2. 提出DiMUM防御：通过记忆替代数据或特征来替换目标遗忘数据，防止生成这些元素。

Result: 实验证明DiMRA能够有效逆转最先进的基于微调的扩散模型机器遗忘方法，揭示了现有技术的脆弱性。DiMUM在保持扩散模型生成性能的同时，显著增强了对DiMRA攻击的鲁棒性。

Conclusion: 基于微调的扩散模型机器遗忘方法存在被重新学习攻击逆转的严重漏洞，需要更鲁棒的解决方案。DiMUM通过记忆替代数据的方法提供了有效的防御机制，在保持模型性能的同时增强了安全性。
...
### [164] [Optimal Transportation and Alignment Between Gaussian Measures](https://arxiv.org/abs/2512.03579)
*Sanjit Dandapanthula,Aleksandr Podkopaev,Shiva Prasad Kasiviswanathan,Aaditya Ramdas,Ziv Goldfeld*

Main category: cs.LG

TL;DR: 该论文为高斯分布的最优传输和Gromov-Wasserstein对齐提供了闭式解，解决了未中心化高斯在希尔伯特空间中的对齐问题，并扩展了多边际最优传输的求解方法。


<details>
  <summary>Details</summary>
Motivation: 最优传输和Gromov-Wasserstein对齐是数据科学中比较、转换和聚合异构数据集的重要几何框架，但由于计算成本高昂，大规模应用通常依赖于高斯分布在二次成本下的闭式解。然而，现有文献中存在一些未解决的问题限制了其应用范围。

Method: 1. 针对可分希尔伯特空间中未中心化高斯的内积Gromov-Wasserstein对齐，给出了闭式表达式（需通过酉算子进行二次优化），并推导了紧致的解析上下界。2. 当至少一个高斯分布中心化时，提供完全闭式解，并扩展到中心化高斯的IGW重心解析解。3. 将具有成对二次成本的高斯多边际最优传输简化为可处理的优化问题，并利用秩缺陷约束提供高效求解算法。

Result: 1. 解决了未中心化高斯在希尔伯特空间中的IGW对齐开放问题。2. 为中心化高斯提供了完全闭式解和重心解析解。3. 实现了高斯多边际最优传输的高效求解。4. 在知识蒸馏和异构聚类任务中验证了方法的实用性。

Conclusion: 该工作填补了高斯分布最优传输和Gromov-Wasserstein对齐文献中的多个空白，通过提供闭式解和高效算法，显著扩展了这些几何框架在数据科学和机器学习中的应用范围，特别是在大规模异构数据集处理方面。
...
### [165] [Federated Learning and Trajectory Compression for Enhanced AIS Coverage](https://arxiv.org/abs/2512.03584)
...
<details>
  <summary>Details</summary>
Motivation: 海上态势感知受限于AIS覆盖范围不足和数据传输带宽限制，需要一种能够在低带宽连接下实时检测异常并高效传输数据的解决方案。

Method: 系统集成了M3fed联邦学习模型和BWC-DR-A轨迹压缩算法，将船舶作为移动传感器，优先传输异常数据，在带宽受限环境下实现高效数据传输。

Result: 初步结果显示VesselEdge能够有效提高AIS覆盖范围和海上态势感知能力，基于历史数据的实验验证了系统的有效性。

Conclusion: VesselEdge系统通过联邦学习和智能轨迹压缩技术，成功解决了海上态势感知中的AIS覆盖和数据传输问题，为实时异常检测提供了可行方案。
...
### [166] [Observation-driven correction of numerical weather prediction for marine winds](https://arxiv.org/abs/2512.03606)
*Matteo Peduto,Qidong Yang,Jonathan Giezendanner,Devis Tuia,Sherrie Wang*

Main category: cs.LG

TL;DR: 提出基于Transformer的深度学习架构，通过同化最新现场观测数据来校正全球天气预报系统(GFS)输出，显著改进海洋风速预报精度


<details>
  <summary>Details</summary>
Motivation: 海洋风速预报对航行安全、船舶路径规划和能源运营至关重要，但由于海洋观测数据稀疏、异构且时间变化大，准确预报仍然具有挑战性

Method: 将风预报重新定义为观测信息化的数值天气预报模型校正；提出基于Transformer的深度学习架构，通过掩码和基于集合的注意力机制处理不规则和时间变化的观测集，使用交叉注意力将预测条件化于最近的观测-预报对，采用循环时间嵌入和坐标感知位置表示实现任意空间坐标的单次推理

Result: 在大西洋区域使用ICOADS观测数据评估，模型在所有48小时预报时效内均降低了GFS 10米风速RMSE，1小时时效改进45%，48小时时效改进13%；空间分析显示在海岸线和航运路线等观测最丰富的区域改进最显著

Conclusion: 该研究展示了一种实用、低延迟的后处理方法，通过学习校正系统性预报误差来补充数值天气预报，能够自然适应异构观测平台并同时生成站点特定预测和流域尺度网格化产品
...
### [167] [The promising potential of vision language models for the generation of textual weather forecasts](https://arxiv.org/abs/2512.03623)
...
TL;DR: 研究探索使用视觉语言模型直接从视频编码的网格天气数据生成航运预报文本，展示了提升气象服务生产效率的创新方法。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态基础模型能力强大，但在气象产品和服务生成方面的应用仍处于起步阶段。为了加速该领域的应用和采纳，研究者探索新的方法来提升气象服务的生产效率和创新。

Method: 使用视觉语言模型，直接从视频编码的网格天气数据生成航运预报文本。这是一种将天气数据转换为自然语言预报的创新方法。

Result: 早期结果显示，这种方法展示了有前景的可扩展技术机会，能够提升气象企业及其他领域的生产效率和服务创新。

Conclusion: 视觉语言模型在从网格天气数据生成航运预报文本方面具有潜力，为气象服务领域提供了新的技术路径和效率提升机会。
...
### [168] [Cyclical Temporal Encoding and Hybrid Deep Ensembles for Multistep Energy Forecasting](https://arxiv.org/abs/2512.03656)
*Salim Khazem,Houssam Kanso*

Main category: cs.LG

TL;DR: 该论文提出了一种结合循环时间编码与LSTM-CNN混合架构的统一深度学习框架，用于提升多步电力消费预测精度。


<details>
  <summary>Details</summary>
Motivation: 准确的电力消费预测对于需求管理和智能电网运营至关重要。现有方法在同时利用时间编码、日历特征和混合架构方面存在不足，需要统一的框架来提升预测性能。

Method: 1. 使用正弦余弦编码系统性地转换基于日历的属性，以保留周期性结构；2. 通过相关性分析评估预测相关性；3. 采用由LSTM、CNN和针对每个预测时域的MLP回归器元学习器组成的集成模型，同时利用长期季节效应和短期局部模式。

Result: 使用一年全国消费数据集进行实验，包括有/无循环编码和日历特征的消融分析，以及与文献中基准方法的比较。结果显示在所有七个预测时域都取得了一致的改进，混合模型实现了比单个架构和先前方法更低的RMSE和MAE。
...
### [169] [Dynamically Scaled Activation Steering](https://arxiv.org/abs/2512.03661)
*Alex Ferrando,Xavier Suau,Jordi Gonzàlez,Pau Rodriguez*

Main category: cs.LG

TL;DR: DSAS是一种动态缩放激活引导框架，通过自适应调节引导强度，只在需要时进行干预，在毒性缓解和模型效用之间实现更好的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有激活引导方法对所有输入统一应用干预，在不必要引导时会降低模型性能。需要一种能够智能判断何时引导以及如何引导的方法。

Method: DSAS将"何时引导"与"如何引导"解耦，通过计算上下文相关的缩放因子自适应调节现有引导方法的强度，可以端到端联合优化引导函数。

Result: DSAS与现有引导方法结合时，在毒性缓解和效用保持方面持续改进帕累托前沿，在文本到图像扩散模型中也能有效调节特定概念，计算开销最小且提高可解释性。

Conclusion: DSAS作为一种方法无关的引导框架，通过动态缩放机制实现了更智能的激活引导，在保持模型性能的同时有效控制不良行为，具有广泛适用性。
...
### [170] [Feature-aware Modulation for Learning from Temporal Tabular Data](https://arxiv.org/abs/2512.03678)
*Hao-Run Cai,Han-Jia Ye*

Main category: cs.LG

TL;DR: 本文提出了一种特征感知的时间调制机制来处理表格数据中的时间分布偏移问题，通过调节特征的统计特性来平衡模型的鲁棒性和适应性。


<details>
  <summary>Details</summary>
Motivation: 表格机器学习在现实部署中面临时间分布偏移的挑战，静态模型假设固定映射以确保泛化性，而自适应模型可能过度拟合瞬态模式，导致鲁棒性与适应性之间的困境。

Method: 提出特征感知的时间调制机制，该机制基于时间上下文条件化特征表示，调节特征的统计特性（如尺度和偏度），通过跨时间对齐特征语义实现轻量级但强大的适应。

Result: 基准评估验证了该方法在处理表格数据时间偏移方面的有效性，实现了泛化性和适应性之间的平衡。

Conclusion: 通过分析构建有效动态映射的关键因素，发现特征转换策略能够缓解跨时间阶段的特征表示差异，提出的特征感知时间调制机制能够有效处理表格数据中的时间分布偏移问题。
...
### [171] [Unlocking the Invisible Urban Traffic Dynamics under Extreme Weather: A New Physics-Constrained Hamiltonian Learning Algorithm](https://arxiv.org/abs/2512.03744)
*Xuhui Lin,Qiuchen Lu*

Main category: cs.LG

TL;DR: 提出了一种物理约束的哈密顿学习算法，通过"结构不可逆性检测"和"能量景观重建"来识别城市交通系统的隐藏结构损伤，解决了传统表面指标无法区分真正恢复与"虚假恢复"的问题。


<details>
  <summary>Details</summary>
Motivation: 当前城市交通系统韧性评估方法依赖表面恢复指标，无法检测隐藏的结构损伤，导致无法区分真正恢复与"虚假恢复"（交通指标正常化但系统动力学永久退化）。

Method: 开发了物理约束的哈密顿学习算法，结合结构不可逆性检测和能量景观重建，通过提取低维状态表示、物理约束优化识别准哈密顿结构，并通过能量景观比较量化结构变化。

Result: 对伦敦2021年极端降雨的分析表明，虽然表面指标完全恢复，但该算法检测到传统监测方法遗漏的64.8%结构损伤。
...
### [172] [Universally Converging Representations of Matter Across Scientific Foundation Models](https://arxiv.org/abs/2512.03750)
*Sathya Edamadaka,Soojung Yang,Ju Li,Rafael Gómez-Bombarelli*

Main category: cs.LG

TL;DR: 研究发现近60个科学模型在不同化学系统中表现出高度对齐的内部表征，表明基础模型学习到了物理现实的共同底层表示，但当前模型仍受限于训练数据和归纳偏置，未能实现真正的通用表征。


<details>
  <summary>Details</summary>
Motivation: 尽管不同模态和架构的机器学习模型被训练来预测分子、材料和蛋白质行为，但尚不清楚它们是否学习到相似的内部表征。理解这些潜在结构对于构建能够可靠泛化到训练域之外的科学基础模型至关重要。

Method: 研究了近60个科学模型，涵盖字符串、图、3D原子结构和蛋白质等不同模态，分析它们在各种化学系统中的表征对齐情况。比较了在不同数据集上训练的模型对分子的表征相似性，并观察机器学习原子间势能在性能提升时表征空间的收敛情况。

Result: 1. 不同模型在化学系统中表现出高度对齐的内部表征；2. 在不同数据集上训练的模型对小分子有高度相似的表征；3. 原子间势能模型在性能提升时在表征空间收敛；4. 发现两种不同的模型机制：在类似训练输入时，高性能模型对齐紧密，弱模型在表征空间发散到局部最优；在完全不同结构时，几乎所有模型都坍缩到低信息表征。

Conclusion: 科学基础模型学习到了物理现实的共同底层表示，但当前模型仍受限于训练数据和归纳偏置，未能编码真正的通用结构。表征对齐可作为科学模型基础级通用性的定量基准，有助于追踪物质通用表征的出现，并选择跨模态、物质域和科学任务转移效果最佳的模型。
...
### [173] [Forensic Activity Classification Using Digital Traces from iPhones: A Machine Learning-based Approach](https://arxiv.org/abs/2512.03786)
*Conor McCarthy,Jan Peter van Zandwijk,Marcel Worring,Zeno Geradts*

Main category: cs.LG

TL;DR: 基于智能手机和智能手表运动传感器数据，开发机器学习方法将数字痕迹转化为不同身体活动类型的似然比，用于法医调查中的活动识别。


<details>
  <summary>Details</summary>
Motivation: 智能手机和智能手表在日常生活中的普及提供了丰富的用户行为信息，其运动传感器产生的数字痕迹为法医调查人员了解个人身体活动提供了机会。

Method: 提出基于机器学习的方法，将数字痕迹转化为不同身体活动类型的似然比（LRs）。使用新的数据集NFI_FARED进行评估，该数据集包含四种不同iPhone型号的数字痕迹，标注了19种活动类型。

Result: 在171种可能的成对活动组合中，该方法能够为167种组合产生有用的似然比系统。方法还可扩展到同时分析多个活动（或活动组）并创建活动时间线，用于法医调查的早期和后期阶段。

Conclusion: 该方法在法医调查中具有实际应用价值，能够有效区分不同类型的身体活动。数据集和代码已公开，以促进该领域的进一步研究。
...
### [174] [Adaptive Identification and Modeling of Clinical Pathways with Process Mining](https://arxiv.org/abs/2512.03787)
*Francesco Vitale,Nicola Mazzocca*

Main category: cs.LG

TL;DR: 本文提出一种基于过程挖掘的两阶段临床路径建模方法，通过一致性检查扩展临床路径知识库，在COVID-19治疗数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 临床路径作为标准化的医疗计划，手动建模困难且难以反映不同疾病变体或组合的最佳实践，需要自动化的建模方法来扩展临床路径知识库。

Method: 采用两阶段过程挖掘方法：第一阶段收集特定疾病的历史数据构建参考过程模型；第二阶段将新数据与参考模型进行一致性检查，基于检查结果扩展知识库，为新的疾病变体或组合创建更具体的模型。

Result: 在Synthea基准数据集（模拟SARS-CoV-2感染及COVID-19并发症治疗）上的实验表明，该方法能以95.62%的AUC峰值精度扩展临床路径知识库，同时保持67.11%的弧度简单性。

Conclusion: 提出的两阶段过程挖掘方法能够有效扩展临床路径知识库，为不同疾病变体和组合提供更精准的治疗模型，有助于提高医疗质量、减少资源使用并加速患者康复。
...
### [175] [EfficientECG: Cross-Attention with Feature Fusion for Efficient Electrocardiogram Classification](https://arxiv.org/abs/2512.03804)
*Hanhui Deng,Xinglin Li,Jie Luo,Zhanpeng Jin,Di Wu*

Main category: cs.LG

TL;DR: 本文提出基于EfficientNet的轻量级ECG分类模型EfficientECG，以及基于交叉注意力的多特征融合模型，用于准确快速分析心电图数据。


<details>
  <summary>Details</summary>
Motivation: 心电图是检测心脏异常的重要诊断信号，现有ECG模型误诊率高，医疗工作者负担重，需要开发准确快速的自动诊断模型。

Method: 1. 基于EfficientNet设计EfficientECG模型，处理高频长序列多导联ECG数据；2. 提出基于交叉注意力的特征融合模型，整合多导联ECG数据及性别、年龄等多特征。

Result: 在代表性ECG数据集上的评估表明，该模型在精度、多特征融合能力和轻量化方面优于现有最先进方法。

Conclusion: 提出的深度学习方法能够通过端到端训练自动提取ECG特征，构建准确、轻量且支持多特征融合的诊断模型，可显著减轻医疗工作者负担。
...
### [176] [Deep Reinforcement Learning for Dynamic Algorithm Configuration: A Case Study on Optimizing OneMax with the (1+($λ$,$λ$))-GA](https://arxiv.org/abs/2512.03805)
*Tai Nguyen,Phong Le,André Biedenkapp,Carola Doerr,Nguyen Dang*

Main category: cs.LG

TL;DR: 本文对深度强化学习在动态算法配置中的应用进行了系统研究，针对(1+(λ,λ))-GA算法的种群大小参数控制问题，揭示了DDQN和PPO算法存在的可扩展性下降和学习不稳定性两大挑战，并提出自适应奖励偏移机制等解决方案。


<details>
  <summary>Details</summary>
Motivation: 动态算法配置研究如何为参数化优化算法高效识别控制策略。虽然已有研究利用强化学习的鲁棒决策能力来解决算法配置的优化挑战，但将RL应用于DAC仍然困难且需要大量领域专业知识。本文旨在通过系统分析深度RL算法在DAC中的表现，识别关键挑战并提供解决方案。

Method: 研究采用系统分析方法，以控制(1+(λ,λ))-GA在OneMax实例上的种群大小参数为案例。重点研究了DDQN和PPO两种深度RL算法，识别了可扩展性下降和学习不稳定性两大挑战。针对探索不足问题，提出了自适应奖励偏移机制，利用奖励分布统计增强DDQN探索能力；针对规划视野覆盖问题，展示了无折扣学习在DDQN中的有效性，并分析了PPO的超参数依赖性。

Result: 研究发现DDQN和PPO在DAC中存在两大根本挑战：可扩展性下降和学习不稳定性。这些问题主要源于探索不足和规划视野覆盖不足。自适应奖励偏移机制有效解决了探索不足问题，无需实例特定的超参数调优，在不同问题规模下保持一致性。无折扣学习解决了DDQN的规划视野覆盖问题，而PPO存在基本方差问题需要替代算法设计。配备自适应奖励偏移策略的DDQN实现了与理论推导策略相当的性能，样本效率大幅提升，比先前DAC方法高出几个数量级。

Conclusion: 深度强化学习在动态算法配置中面临可扩展性和稳定性挑战，但通过针对性的解决方案（如自适应奖励偏移和无折扣学习）可以有效克服。DDQN配备自适应奖励偏移策略后，在样本效率和性能方面显著优于现有方法，为DAC提供了实用有效的解决方案。PPO由于基本方差问题需要重新设计算法架构。
...
### [177] [Log Probability Tracking of LLM APIs](https://arxiv.org/abs/2512.03816)
*Timothée Chauvin,Erwan Le Merrer,François Taïani,Gilles Tredan*

Main category: cs.LG

TL;DR: 提出了一种基于对数概率统计测试的低成本LLM API监控方法，仅需单个token输出即可检测微小模型变化，比现有方法敏感1000倍且成本降低1000倍
...
Method: 利用LLM对数概率通常是非确定性的特性，基于每个token对数概率的平均值应用简单统计测试，仅需请求单个token输出即可进行连续监控

Result: 该方法能够检测小至单步微调的模型变化，比现有方法更敏感，同时成本降低1000倍。作者还引入了TinyChange基准来衡量审计方法对小型现实模型变化的敏感性

Conclusion: 基于对数概率的统计测试提供了一种成本效益高的连续监控LLM API的方法，解决了现有审计方法成本过高的问题，确保模型一致性并支持可靠的下游应用和研究可复现性
...
### [178] [DVPO: Distributional Value Modeling-based Policy Optimization for LLM Post-Training](https://arxiv.org/abs/2512.03847)
...
TL;DR: DVPO是一个新的强化学习框架，结合条件风险理论和分布价值建模，通过token级价值分布和不对称风险正则化，在噪声监督下平衡鲁棒性和泛化性。


<details>
  <summary>Details</summary>
Motivation: 现实世界部署中，强化学习后训练常面临噪声或不完整的监督信号，现有方法（如最坏情况优化和均值方法）往往忽视泛化性，产生过于保守的策略，在不同真实场景中表现不均。

Method: DVPO结合条件风险理论和分布价值建模，学习token级价值分布提供细粒度监督，并应用不对称风险正则化：收缩下尾以抑制噪声负偏差，扩展上尾以保持探索多样性。

Result: 在多轮对话、数学推理和科学问答的广泛实验中，DVPO在噪声监督下持续优于PPO、GRPO和基于鲁棒Bellman的PPO。

Conclusion: DVPO展示了在现实世界LLM后训练中的潜力，通过平衡鲁棒性和泛化性，在噪声监督下实现更稳定和有效的性能。
...
### [179] [Hyperdimensional Computing for Sustainable Manufacturing: An Initial Assessment](https://arxiv.org/abs/2512.03864)
...
<details>
  <summary>Details</summary>
Motivation: 智能制造虽然能提高效率和降低能耗，但AI模型的高能耗可能抵消这些优势。需要寻找既能保持精度又能大幅降低能耗的AI解决方案

Method: 利用原位传感预测智能加工中的几何质量，比较常见AI模型的能耗、精度和速度。引入超维计算作为替代方案，并与传统模型进行对比

Result: 超维计算在精度上与传统模型相当，但能耗大幅降低：训练能耗降低200倍，推理能耗降低175-1000倍。训练时间减少200倍，推理时间减少300-600倍

Conclusion: 超维计算展示了在智能制造中实现高能效AI的潜力，能够在保持精度的同时显著降低能耗和计算时间
...
### [180] [Probabilistic Foundations of Fuzzy Simplicial Sets for Nonlinear Dimensionality Reduction](https://arxiv.org/abs/2512.03899)
*Janis Keck,Lukas Silvester Barth,Fatemeh,Fahimi,Parvaneh Joharinad,Jürgen Jost*

Main category: cs.LG

TL;DR: 该论文为模糊单纯集提供了一个概率论框架，将其解释为单纯集上概率测度的边际分布，揭示了UMAP模糊权重的生成模型本质，并展示了如何基于此框架推导新的降维方法。


<details>
  <summary>Details</summary>
Motivation: 模糊单纯集在降维和流形学习中（特别是UMAP）很重要，但现有的代数拓扑定义缺乏清晰的概率解释，脱离了这些领域常用的理论框架。作者希望建立一个概率论框架来解释模糊单纯集。

Method: 引入了一个框架，将模糊单纯集解释为单纯集上概率测度的边际分布。具体展示了UMAP的模糊权重源于一个生成模型，该模型在随机尺度上采样Vietoris-Rips过滤，产生成对距离的累积分布函数。还将模糊单纯集连接到面偏序集上的概率模型，澄清了Kullback-Leibler散度与模糊交叉熵的关系，并通过底层单纯集的布尔运算恢复了标准t-范数和t-余范数。

Result: 该框架为模糊单纯集提供了统一的概率理论基础，阐明了UMAP在此框架内的作用，并能够系统性地推导新的降维方法。作者通过一个使用Čech过滤和三重采样的UMAP泛化示例说明了这一点。

Conclusion: 概率论视角为模糊单纯集提供了统一的理论基础，澄清了UMAP在该框架中的角色，并使得能够系统性地推导新的降维方法，从而扩展了模糊单纯集在数据科学中的应用潜力。
...
### [181] [Guided Flow Policy: Learning from High-Value Actions in Offline Reinforcement Learning](https://arxiv.org/abs/2512.03973)
*Franki Nguimatsia Tiofack,Théotime Le Hellard,Fabian Schramm,Nicolas Perrin-Gilbert,Justin Carpentier*

Main category: cs.LG

TL;DR: GFP通过结合多步流匹配策略和蒸馏的一步行动者，使用加权行为克隆来专注于复制数据集中的高价值动作，而不是盲目模仿所有状态-动作对，从而在离线强化学习中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习中传统的行为正则化方法存在缺陷，它们无法区分高价值和低价值动作，对所有状态-动作对都进行同等程度的模仿，这限制了策略的性能提升。

Method: 提出引导流策略(GFP)，将多步流匹配策略与蒸馏的一步行动者相结合。行动者通过加权行为克隆指导流策略专注于复制数据集中的高价值动作，而流策略则约束行动者保持与数据集最佳转移的对齐，同时最大化批评者价值。

Result: GFP在OGBench、Minari和D4RL基准测试的144个状态和像素任务中实现了最先进的性能，特别是在次优数据集和具有挑战性的任务上取得了显著提升。

Conclusion: GFP通过行动者和流策略之间的相互引导机制，有效地区分并专注于数据集中的高价值动作，解决了传统行为正则化方法的局限性，在离线强化学习中实现了卓越的性能。
...
### [182] [Density-Informed VAE (DiVAE): Reliable Log-Prior Probability via Density Alignment Regularization](https://arxiv.org/abs/2512.03928)
*Michele Alessi,Alessio Ansuini,Alex Rodriguez*

Main category: cs.LG

TL;DR: DiVAE是一种轻量级VAE正则化器，通过将潜在空间先验概率与数据空间密度估计对齐来改进标准VAE，在保持计算效率的同时提升分布对齐、先验覆盖和OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 标准变分自编码器（VAE）将潜在变量匹配到简单先验分布（如高斯分布），但忽略了数据空间中的密度结构。这导致潜在空间不能很好地反映数据分布特征，影响模型的可解释性和OOD检测能力。

Method: DiVAE在ELBO目标函数中添加了一个鲁棒的、精度加权的惩罚项，该惩罚项将VAE的对数先验概率与从数据估计的对数密度对齐。这种方法鼓励编码器根据数据空间密度按比例分配后验质量，并在先验可学习时推动先验向高密度区域移动。

Result: 在合成数据集上，DiVAE：(i) 改善了潜在对数密度与其真实对应物的分布对齐；(ii) 提高了先验覆盖；(iii) 获得了更好的OOD不确定性校准。在MNIST数据集上，DiVAE改善了先验与外部密度估计的对齐，提供了更好的可解释性，并提高了可学习先验的OOD检测性能。

Conclusion: DiVAE是一种有效的轻量级正则化方法，通过将VAE先验与数据空间密度对齐，显著改善了潜在表示的质量、先验覆盖和OOD检测能力，同时保持了计算效率。
...
### [183] [Technical Report on Text Dataset Distillation](https://arxiv.org/abs/2512.03967)
...
TL;DR: 本文综述了文本数据集蒸馏领域的发展历程、主要方法、关键贡献和当前挑战，指出该领域仍处于成熟阶段，在基准标准化、处理文本离散性、复杂任务处理等方面有待改进。


<details>
  <summary>Details</summary>
Motivation: 文本数据集蒸馏作为从视觉领域发展而来的新兴研究方向，目前相关研究相对较少。随着该领域从视觉方法的简单移植到形成独立分支，需要系统梳理其发展脉络、方法演进和面临的挑战，以推动该领域的进一步发展。

Method: 本文采用文献综述方法，系统回顾了文本数据集蒸馏的发展历程，重点分析了不同的蒸馏策略，包括使用Transformer模型的方法、生成离散合成文本的技术，以及扩展到超过10亿参数的仅解码器模型等关键进展。

Result: 文本数据集蒸馏领域已经取得了显著进展，包括引入Transformer模型、生成离散合成文本、扩展到大规模仅解码器模型等里程碑。然而，该领域仍面临基准标准化不足、文本离散性处理困难、复杂任务处理能力有限等挑战。

Conclusion: 文本数据集蒸馏是一个正在成熟的研究领域，虽然已取得重要进展，但在基准标准化、克服文本离散性、处理复杂任务和展示实际应用等方面仍有改进空间，需要进一步的研究和发展。
...
### [184] [Training-Free Policy Violation Detection via Activation-Space Whitening in LLMs](https://arxiv.org/abs/2512.03994)
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在敏感领域（法律、金融、医疗）的部署，组织需要可靠机制检测内部政策违规，现有安全过滤器和护栏方法局限于通用安全领域，缺乏对组织特定策略的鲁棒性，而微调方法延迟高且缺乏可解释性。

Method: 将策略违规检测视为分布外检测问题，受白化技术启发，对模型隐藏激活进行线性变换以解相关并标准化为零均值和单位方差，在变换空间中使用欧几里得范数作为合规分数，仅需策略文本和少量示例样本。

Result: 在具有挑战性的策略基准测试中，该方法取得了最先进的结果，超越了现有护栏方法和微调推理模型，为组织提供了实用且统计基础的政策感知监督框架。

Conclusion: 该方法为组织提供了轻量级、易于部署的政策违规检测解决方案，无需训练且高效，推进了可部署AI治理的更广泛目标。
...
### [185] [Physics-Embedded Gaussian Process for Traffic State Estimation](https://arxiv.org/abs/2512.04004)
*Yanlin Chen,Kehua Chen,Yinhai Wang*

Main category: cs.LG

TL;DR: 提出了一种新颖的物理嵌入高斯过程（PEGP）框架，用于解决低渗透率稀疏观测下的交通状态估计问题，通过将经典交通流模型知识嵌入多输出核函数中，实现了物理模型与数据驱动方法的有效结合。


<details>
  <summary>Details</summary>
Motivation: 传统纯数据驱动方法在稀疏观测下缺乏物理解释和泛化能力，而物理模型难以整合不确定性和捕捉真实交通复杂性。现有结合方法依赖惩罚调优且缺乏原则性的不确定性校准，对模型误设敏感。

Method: 设计了两种基于经典交通流模型（LWR和ARZ）的多输出核函数，通过线性化微分算子的显式应用构建物理嵌入高斯过程框架，将领域知识整合到数据驱动方法中。

Result: 在HighD和NGSIM数据集上的实验显示，PEGP相比非物理基线方法有持续改进。PEGP-ARZ在稀疏观测下更可靠，PEGP-LWR在密集观测下误差更低。消融研究表明PEGP-ARZ残差更符合物理原理且产生可解释的不确定性，而PEGP-LWR残差更正交且产生近似恒定方差场。

Conclusion: PEGP框架成功结合了物理先验和不确定性量化，为交通状态估计提供了可靠支持，解决了现有方法在稀疏观测下的局限性，并提供了校准的、可解释的不确定性估计。
...
### [186] [Efficient Public Verification of Private ML via Regularization](https://arxiv.org/abs/2512.04008)
*Zoë Ruha Bell,Anvith Thudi,Olive Franzese-McLaughlin,Nicolas Papernot,Shafi Goldwasser*

Main category: cs.LG

TL;DR: 提出首个差分隐私算法，其隐私保证验证成本远低于训练成本，通过一系列正则化目标的私有优化实现最优隐私-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 当前差分隐私算法的验证成本与训练成本相当，数据提供者和公众缺乏高效验证DP保证的方法，需要设计验证成本远低于训练成本的DP算法。

Method: 设计DP-SCO算法，通过私有化最小化一系列正则化目标，仅使用标准DP组合边界，实现紧致的隐私-效用权衡。

Result: 获得了接近最优的隐私-效用权衡，且DP验证成本远低于训练成本，显著减少了大型数据集上的验证开销。

Conclusion: 首次实现了验证成本显著低于训练成本的DP-SCO算法，为数据提供者提供了更实用的DP保证验证方法。
...
### [187] [Domain Feature Collapse: Implications for Out-of-Distribution Detection and Solutions](https://arxiv.org/abs/2512.04034)
...
<details>
  <summary>Details</summary>
Motivation: 解释为什么最先进的OOD检测方法在单域数据集训练时会出现灾难性失败，从理论上理解这一现象的根本原因。

Method: 1. 从信息论角度分析，证明单域监督学习会导致域特征坍缩（I(x_d; z)=0）；2. 使用Fano不等式量化实际场景中的部分坍缩；3. 引入Domain Bench基准测试；4. 通过域过滤方法（使用预训练表示）验证理论。

Result: 1. 理论证明单域训练会导致模型完全丢弃域特定信息；2. 在MNIST上OOD检测仅达到53% FPR@95；3. 域过滤方法（保持I(x_d; z)>0）成功解决了失败模式；4. 为信息论框架提供了强有力的实证证据。

Conclusion: 单域监督学习存在根本性限制，会导致域特征坍缩，从而造成OOD检测灾难性失败。该研究解释了这一令人困惑的经验现象，对迁移学习以及何时微调与冻结预训练模型具有更广泛的意义。
...
### [188] [MarkTune: Improving the Quality-Detectability Trade-off in Open-Weight LLM Watermarking](https://arxiv.org/abs/2512.04044)
*Yizhou Zhao,Zhiwei Steven Wu,Adam Block*

Main category: cs.LG

TL;DR: MarkTune是一种针对开源权重语言模型的理论化、基于策略的微调框架，通过将水印信号作为奖励进行优化，在保持文本质量的同时显著提升水印检测能力，接近推理时水印的性能水平。


<details>
  <summary>Details</summary>
Motivation: 开源权重语言模型对水印技术提出了严峻挑战，因为一旦模型权重公开，就无法强制执行推理时干预。现有技术如GaussMark虽然能在权重中嵌入可检测信号，但通常需要在检测能力和生成质量之间做出权衡，检测能力往往不如推理时水印。

Method: MarkTune是一个理论化、基于策略的微调框架，将GaussMark水印信号作为奖励函数，同时通过正则化防止文本质量下降。该方法在模型的表示空间中进行更精细的、水印感知的权重更新，从而在保持生成质量的同时提升水印检测能力。

Result: MarkTune显著改善了GaussMark在质量与检测能力之间的权衡，将检测性能推近到接近推理时水印的水平。该方法对改写和微调攻击具有鲁棒性，并展现出强大的泛化能力：在一个数据集上微调的模型在未见数据集上仍保持显著的水印检测能力。

Conclusion: MarkTune为开源权重语言模型嵌入鲁棒、高质量水印提供了一种通用策略，通过理论化的微调框架实现了接近推理时水印的检测性能，同时保持了文本生成质量。
...
### [189] [Fare Comparison App of Uber, Ola and Rapido](https://arxiv.org/abs/2512.04065)
*Ashlesha Gopinath Sawant,Sahil S. Jadhav,Vidhan R. Jain,Shriraj S. Jagtap,Prachi Jadhav,Soham Jadhav,Ichha Raina*

Main category: cs.LG

TL;DR: 开发了一个网约车比价Web应用，通过Python后端获取Ola、Uber、Rapido的实时价格数据，为用户提供最优出行选择。


<details>
  <summary>Details</summary>
Motivation: 用户在选择网约车服务时面临困难，难以找到既经济又高效的出行方案。当前缺乏透明的价格比较工具，导致用户无法做出最优选择。

Method: 开发Web应用程序，使用Python后端通过API获取Ola、Uber、Rapido的实时价格数据，利用Android Studio模拟器、Appium和位置比较技术解决数据访问挑战。

Result: 成功创建了能够为用户提供实时价格比较的Web应用，能够显示不同平台的价格差异并推荐最优选项。

Conclusion: 该项目提高了网约车服务的透明度，帮助用户做出更明智的选择，提升了出行效率和用户体验。
...
### [190] [Convergence for Discrete Parameter Updates](https://arxiv.org/abs/2512.04051)
*Paul Wilson,Fabio Zanasi,George Constantinides*

Main category: cs.LG

TL;DR: 提出了一种离散更新规则的量化训练方法，避免了对连续更新的离散化，为具有固有离散结构的模型开辟了高效训练新途径。


<details>
  <summary>Details</summary>
Motivation: 现代深度学习模型需要巨大的计算资源，量化训练通过低比特整数表示训练组件来解决这个问题，但传统方法通常依赖于对实值更新的离散化，存在局限性。

Method: 引入了一种替代方法，其中更新规则本身是离散的，通过设计避免了对连续更新的量化。建立了一类此类离散方案的收敛保证，并提出了多项分布更新规则作为具体实例。

Result: 通过实证评估支持了所提出的离散更新规则，证明了该方法的有效性。
...
### [191] [Eval Factsheets: A Structured Framework for Documenting AI Evaluations](https://arxiv.org/abs/2512.04062)
*Florian Bordes,Candace Ross,Justine T Kao,Evangelia Spiliopoulou,Adina Williams*

Main category: cs.LG

TL;DR: Eval Factsheets：为AI系统评估方法提供结构化文档框架，解决当前评估方法缺乏系统化文档标准的问题


<details>
  <summary>Details</summary>
Motivation: 当前AI领域基准测试激增，但在可复现性、透明度和知情决策方面存在显著挑战。与数据集和模型已有结构化文档框架（如Datasheets和Model Cards）不同，评估方法缺乏系统化的文档标准。

Method: 提出Eval Factsheets框架，通过综合分类法和基于问卷的方法来记录AI系统评估。框架将评估特征组织为五个基本维度：上下文（谁、何时）、范围（评估什么）、结构（用什么构建）、方法（如何工作）和对齐（可靠性/有效性/鲁棒性）。

Result: 通过多个基准测试的案例研究证明，Eval Factsheets能够有效捕捉多样化的评估范式（从传统基准测试到LLM-as-judge方法），同时保持一致性和可比性。
...
### [192] [Learning Steerable Clarification Policies with Collaborative Self-play](https://arxiv.org/abs/2512.04068)
*Jonathan Berant,Maximillian Chen,Adam Fisch,Reza Aghajani,Fantine Huot,Mirella Lapata,Jacob Eisenstein*

Main category: cs.LG

TL;DR: 提出使用自博弈训练可调控的不确定性管理策略，通过成本惩罚的准确性奖励来优化AI助手在模糊查询下的响应决策


<details>
  <summary>Details</summary>
Motivation: AI助手需要根据上下文因素（如用户偏好、设备模态）来管理不确定性，决定何时猜测用户意图直接回答、何时枚举多个可能意图、何时提出澄清问题。现有策略缺乏对成本因素（如屏幕尺寸、语音设置）的适应性。

Method: 使用自博弈方法训练可调控策略：两个智能体分别模拟用户和AI助手，生成对话数据。模型输入澄清问题和生成每个单词的数值成本，目标是最大化成本惩罚后的准确性奖励。采用强化自训练（ReST）方法优化策略。

Result: 训练出的策略能够根据提供的成本参数可预测地改变行为，实现更高的奖励和准确性。该方法还能泛化到训练时未观察到的成本数值。

Conclusion: 通过自博弈和强化自训练可以学习到可调控的不确定性管理策略，使AI助手能够根据上下文成本因素灵活选择响应方式，提高整体性能。
...
### [193] [Beyond the Black Box: A Cognitive Architecture for Explainable and Aligned AI](https://arxiv.org/abs/2512.03072)
*Hu Keyi*

Main category: cs.AI

TL;DR: 论文提出了一种名为"权重计算主义"的新型认知架构，基于逻辑原子和基本操作构建，通过可解释的权重计算模型实现决策，为AGI提供可解释性和价值对齐的解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前AI范式作为"体验架构师"面临可解释性和价值对齐的根本挑战，需要一种基于第一原则的认知架构来构建可信赖的人工通用智能。

Method: 提出"权重计算主义"认知架构，将认知分解为不可分割的逻辑原子和两个基本操作（指向和比较），通过权重=收益*概率的可解释模型进行决策，所有值都可追溯到可审计的初始权重集，采用基于图算法的计算引擎和全局工作空间工作流实现。

Result: 该架构实现了透明、类人的推理能力，在全新场景中表现出稳健的学习能力，为构建可信赖和对齐的AGI奠定了实践和理论基础。

Conclusion: 权重计算主义为人工通用智能提供了一条可行的技术路径，通过原子分解实现了根本的可解释性、内在的泛化能力和可追溯的价值对齐，是构建可信赖AGI的重要基础。
...
### [194] [When Do Symbolic Solvers Enhance Reasoning in Large Language Models?](https://arxiv.org/abs/2512.03272)
*Zhiyuan He,Dingmin Wang*

Main category: cs.AI

TL;DR: 该论文研究了符号求解器集成方法与传统长推理链方法的对比，发现符号求解器只在问题需要有限隐式推理但涉及充足搜索空间时有效，而最新LLMs在推理深度较浅的演绎问题上表现更好。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过生成长推理链在复杂推理任务上表现良好，但这种方法可能导致大量token开销，甚至产生错误答案。符号求解器集成方法利用LLMs的代码生成能力将推理任务转化为可执行代码，然后用符号求解器解决，但何时这种传统长推理链方法能被符号求解器增强仍是一个开放问题。

Method: 研究符号求解器集成方法与传统长推理链方法的对比，通过实验探索符号求解器何时能增强传统方法。该方法利用LLMs的代码生成能力将推理任务转化为可执行代码，然后使用符号求解器解决。

Result: 符号求解器集成方法只在问题需要有限隐式推理但涉及充足搜索空间时有效。最新LLMs（如GPT-4o）在推理深度较浅的演绎问题上表现更好，而符号求解器集成方法在需要重复回溯的约束满足问题上显著提升LLMs性能。当提供声明性示例时，即使是CodeLlama-13B也能在困难的斑马谜题上超越GPT-4o。

Conclusion: 符号求解器集成方法在特定类型的问题上能有效增强传统长推理链方法，特别是在需要大量搜索但有限隐式推理的约束满足问题上，而最新LLMs在浅层演绎推理上表现更优，两者各有适用场景。
...
### [195] [Prior preferences in active inference agents: soft, hard, and goal shaping](https://arxiv.org/abs/2512.03293)
*Filippo Torresan,Ryota Kanai,Manuel Baltieri*

Main category: cs.AI

TL;DR: 本文研究了主动推理中偏好分布的四种定义方式对智能体在网格世界导航任务中性能的影响，发现目标塑造能提升整体性能但会牺牲环境动态的学习。


<details>
  <summary>Details</summary>
Motivation: 主动推理使用期望自由能作为规划决策目标，但文献中很少关注偏好分布应如何指定以及不同指定方式对推理和学习的影响。本文旨在填补这一研究空白。

Method: 考虑了四种定义偏好分布的方式：硬目标与软目标、涉及或不涉及目标塑造（中间目标）。在网格世界导航任务中比较了四种智能体的性能。

Result: 目标塑造能实现最佳整体性能（促进利用），但会牺牲对环境转移动态的学习（阻碍探索）。不同偏好分布定义方式对智能体的利用-探索平衡有显著影响。

Conclusion: 偏好分布的定义方式对主动推理智能体的性能有重要影响，目标塑造虽然提升性能但会限制探索能力，需要在利用和探索之间做出权衡。
...
### [196] [Evaluating Generalization Capabilities of LLM-Based Agents in Mixed-Motive Scenarios Using Concordia](https://arxiv.org/abs/2512.03318)
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体在社交互动中展现出强大能力，但现有评估方法无法衡量这些能力在新颖社交情境中的泛化表现，特别是在需要合作的混合动机环境中。

Method: 引入基于Concordia自然语言多智能体模拟环境的评估方法，通过测试智能体在不同伙伴和情境中识别和利用互利机会的能力，来衡量一般合作智能。

Result: NeurIPS 2024 Concordia竞赛的实证结果显示，当前智能体能力与稳健泛化所需的可靠合作之间存在显著差距，特别是在需要说服和规范执行的场景中。

Conclusion: LLM智能体在合作能力方面仍有明显不足，特别是在复杂社交情境中，需要进一步研究来提升其在新颖社交环境中的合作泛化能力。
...
### [197] [Multi-Agent Reinforcement Learning with Communication-Constrained Priors](https://arxiv.org/abs/2512.03528)
*Guang Yang,Tianpei Yang,Jingwen Qiao,Yanqing Wu,Jing Huo,Xingguo Chen,Yang Gao*

Main category: cs.AI

TL;DR: 提出了一种通信受限的多智能体强化学习框架，通过双互信息估计器区分有损和无损消息的影响，并将其量化到全局奖励中，以应对现实世界中普遍存在的通信损失问题。
...
Method: 1) 提出广义通信约束模型统一描述不同场景的通信条件；2) 将其作为学习先验区分特定场景的有损和无损消息；3) 使用双互信息估计器解耦有损和无损消息对分布式决策的影响；4) 引入通信约束的多智能体强化学习框架，将通信消息影响量化到全局奖励中。

Result: 在多个通信受限基准测试中验证了方法的有效性。

Conclusion: 提出的通信约束多智能体强化学习框架能够有效处理现实世界中的通信损失问题，提高了在复杂动态环境中的适应性和鲁棒性。
...
### [198] [Reason-Plan-ReAct: A Reasoner-Planner Supervising a ReAct Executor for Complex Enterprise Tasks](https://arxiv.org/abs/2512.03560)
*Gianni Molinari,Fabio Ciravegna*

Main category: cs.AI

TL;DR: RP-ReAct是一种新型多智能体方法，通过将战略规划与低级执行解耦来解决企业复杂任务中的轨迹不稳定性和上下文窗口限制问题，在ToolQA基准测试中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 当前自主智能体在企业复杂任务中存在两个主要限制：1) 单智能体架构的单一规划-执行循环导致轨迹不稳定；2) 为数据隐私使用本地开源模型时，较小的上下文窗口难以处理大型工具输出。

Method: RP-ReAct采用多智能体架构，包含Reasoner Planner Agent(RPA)负责战略规划和结果分析，以及Proxy-Execution Agent(PEA)负责具体工具交互。PEA采用上下文保存策略，通过外部存储和按需访问管理大型工具输出。

Result: 在ToolQA基准测试中使用六种开源推理模型进行评估，RP-ReAct在性能、泛化能力和鲁棒性方面均优于现有最先进基线方法，在不同模型规模下都表现出增强的稳定性和可靠性。

Conclusion: RP-ReAct通过解耦规划与执行的多智能体架构，有效解决了企业复杂任务中的轨迹不稳定性和上下文限制问题，为实现可部署的企业级智能体解决方案铺平了道路。
...
### [199] [EnCompass: Enhancing Agent Programming with Search Over Program Execution Paths](https://arxiv.org/abs/2512.03571)
*Zhening Li,Armando Solar-Lezama,Yisong Yue,Stephan Zheng*

Main category: cs.AI

TL;DR: 本文提出了一种新的智能体编程方法PAN，将核心工作流逻辑与推理时策略解耦，通过EnCompass框架实现，让开发者能快速改进智能体可靠性并轻松切换不同推理策略。


<details>
  <summary>Details</summary>
Motivation: 当前智能体编程方法通常将核心工作流逻辑与推理时策略（如树搜索）耦合在一起，这种耦合限制了开发者的灵活性和实验效率，需要一种能够解耦这两个关注点的编程模型。

Method: 提出了"概率天使非确定性"（PAN）编程模型，使用Python装饰器将智能体工作流程序编译成搜索空间，通过EnCompass框架实现，允许开发者独立描述工作流和实验不同推理策略。

Result: 通过三个案例研究证明，该框架能让开发者用少量额外代码快速改进智能体可靠性，并轻松在不同推理时策略之间切换，提高了开发效率和灵活性。

Conclusion: PAN编程模型和EnCompass框架成功解耦了智能体设计的两个关键方面，为LLM-based智能体开发提供了更灵活、高效的编程范式，显著提升了开发者的实验能力和系统可靠性。
...
### [200] [DeepRule: An Integrated Framework for Automated Business Rule Generation via Deep Predictive Modeling and Hybrid Search Optimization](https://arxiv.org/abs/2512.03607)
*Yusen Wu,Xiaotie Deng*

Main category: cs.AI

TL;DR: DeepRule是一个用于零售商品组合和定价优化的自动化业务规则生成框架，通过三层架构解决理论模型与现实经济复杂性之间的系统错配问题。


<details>
  <summary>Details</summary>
Motivation: 解决现有理论模型与现实经济复杂性之间的系统错配问题，具体包括：1）非结构化文本源导致的数据模态不匹配；2）非线性价格弹性和时变属性建模中的动态特征纠缠挑战；3）多层次业务约束导致的操作不可行性。

Method: 采用三层架构：1）混合知识融合引擎，使用大语言模型深度语义解析非结构化文本；2）博弈论约束优化机制，通过双边效用函数动态协调供应链利益；3）可解释决策蒸馏接口，利用LLM引导的符号回归优化定价策略和可审计业务规则。

Result: 在真实零售环境中验证了框架的有效性，相比系统化B2C基线实现了更高的利润，同时确保了操作可行性。
...
### [201] [RoCo: Role-Based LLMs Collaboration for Automatic Heuristic Design](https://arxiv.org/abs/2512.03762)
...
Method: 提出RoCo多智能体角色协作系统，包含四个专门角色：探索者（创造性、多样性驱动）、利用者（保守性、效率导向）、批评者（评估反馈）、整合者（平衡创新与利用）。这些智能体通过结构化的多轮交互过程协作，包含反馈、精炼和精英突变等机制。

Result: 在五个不同的组合优化问题上，包括白盒和黑盒设置，RoCo都表现出优越性能，生成的启发式方法超越了ReEvo和HSEvo等现有方法。

Conclusion: 基于角色的协作范式为鲁棒和高性能的自动启发式设计建立了新标准，通过多角色协同显著提升了启发式的多样性和质量。
...
### [202] [Omni-AutoThink: Adaptive Multimodal Reasoning via Reinforcement Learning](https://arxiv.org/abs/2512.03783)
*Dongchao Yang,Songxiang Liu,Disong Wang,Yuanyuan Wang,Guanglu Wan,Helen Meng*

Main category: cs.AI

TL;DR: Omni-AutoThink：一种自适应推理框架，可根据任务难度动态调整Omni模型的推理深度，提升多模态感知与生成能力。
...
Conclusion: Omni-AutoThink通过自适应调整推理深度，有效解决了现有Omni模型推理行为僵化的问题，在多模态推理任务上表现出色。
...
### [203] [Benchmark for Planning and Control with Large Language Model Agents: Blocksworld with Model Context Protocol](https://arxiv.org/abs/2512.03955)
...
Method: 引入包含可执行模拟环境的基准测试，代表Blocksworld问题并提供五个复杂度类别；通过集成模型上下文协议作为标准化工具接口，使不同智能体架构能够无需特定实现修改即可连接和评估。

Result: 单智能体实现证明了基准测试的适用性，建立了用于比较基于LLM的规划和执行方法的定量指标。
...
### [204] [BookRAG: A Hierarchical Structure-aware Index-based Approach for Retrieval-Augmented Generation on Complex Documents](https://arxiv.org/abs/2512.03413)
*Shu Wang,Yingli Zhou,Yixiang Fang*

Main category: cs.IR

TL;DR: BookRAG：针对具有层次结构文档的新型RAG方法，通过构建BookIndex层次索引和基于信息觅食理论的智能体查询机制，显著提升检索召回率和QA准确性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法主要针对一般文档，忽略了现实世界中许多文档（如书籍、手册等）具有层次结构的特点，导致在QA任务上性能不佳。需要专门针对层次结构文档的RAG解决方案。

Method: 1. 构建BookIndex：从文档中提取层次树作为目录，用图捕捉实体间复杂关系，并将实体映射到树节点；2. 基于信息觅食理论的智能体查询方法：动态分类查询并采用定制化检索工作流。

Result: 在三个广泛采用的基准测试上进行大量实验，BookRAG实现了最先进的性能，在检索召回率和QA准确性方面显著优于基线方法，同时保持有竞争力的效率。

Conclusion: BookRAG是针对层次结构文档的有效RAG方法，通过利用逻辑层次结构和追踪实体关系来查询高度相关信息，显著提升了QA任务的性能。
...
### [205] [LLM as Explainable Re-Ranker for Recommendation System](https://arxiv.org/abs/2512.03439)
*Yaqi Wang,Haojia Sun,Shuting Zhang*

Main category: cs.IR

TL;DR: 本文提出使用大语言模型作为可解释的重新排序器，将传统推荐模型与LLMs结合，在提升准确性的同时增强推荐系统的可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统缺乏可解释性且存在流行度偏差问题，而单独使用LLMs作为预测器又无法达到传统模型的准确性。因此需要一种混合方法来解决这些挑战。

Method: 提出使用LLM作为可解释的重新排序器，构建数据集训练重新排序器LLM，采用两阶段训练过程，评估生成数据集与人类期望的对齐度。

Result: 模型显著提升了NDCG排名指标，重新排序器在排名准确性和可解释性方面均优于零样本基线。

Conclusion: 将传统推荐模型与LLMs结合能够解决现有系统的局限性，为构建更可解释和公平的推荐框架开辟了新途径。
...
### [206] [M3DR: Towards Universal Multilingual Multimodal Document Retrieval](https://arxiv.org/abs/2512.03514)
...
Method: 利用合成多语言文档数据，采用对比训练方法学习文本和文档图像的统一表示，支持单密集向量和ColBERT式多向量检索范式。

Result: 在22种类型多样的语言上验证了性能一致性，NetraEmbed和ColNetraEmbed模型在跨语言检索上实现了约150%的相对改进。

Conclusion: M3DR框架成功解决了多语言多模态文档检索的挑战，为不同语言和文化背景的应用提供了有效的解决方案。
...
### [207] [Algorithms for Boolean Matrix Factorization using Integer Programming and Heuristics](https://arxiv.org/abs/2512.03807)
*Christos Kolomvakis,Thomas Bobille,Arnaud Vandaele,Nicolas Gillis*

Main category: cs.IR

TL;DR: 本文提出布尔矩阵分解(BMF)的新算法，包括基于整数规划的交替优化方法、多运行结果选择策略、可扩展的启发式算法，以及高效的C++数据结构实现。


<details>
  <summary>Details</summary>
Motivation: 布尔矩阵分解使用布尔运算而非标准算术，能提高可解释性并减少近似误差，在角色挖掘和计算机视觉中有应用。现有方法在可扩展性方面存在限制。

Method: 1) 提出基于整数规划的交替优化算法；2) 设计从多次运行中选择最优秩一因子子集的方法；3) 引入贪心和局部搜索启发式算法以提升可扩展性；4) 构建高效的C++布尔向量和矩阵数据结构。

Result: 新方法在包含缺失数据的各种真实数据集上表现出色，在主题建模和图像处理等应用中超越了现有技术水平，新数据结构显著提升了计算效率。

Conclusion: 本文提出的布尔矩阵分解算法框架结合了精确优化和启发式方法，通过高效的数据结构实现，能够有效处理大规模数据集，在多个实际应用中展示了优越性能。
...
### [208] [Learning to Comparison-Shop](https://arxiv.org/abs/2512.04009)
*Jie Tang,Daochen Zha,Xin Liu,Huiji Gao,Liwei He,Stephanie Moyerman,Sanjeev Katariya*

Main category: cs.IR

TL;DR: 本文提出了一种名为LTCS（学习比较购物）的新型排序系统，专门针对用户在Airbnb等平台上的比较购物行为进行建模，相比传统孤立评估商品的排序方法，显著提升了搜索效果和转化率。


<details>
  <summary>Details</summary>
Motivation: 在线市场（如Airbnb）中用户经常进行对比购物，但主流电商搜索引擎与用户的比较需求之间存在显著脱节。传统排序模型通常孤立评估商品，忽视了用户在搜索结果页面上比较多个商品的上下文环境。

Method: 提出LTCS（Learning-to-Comparison-Shop）系统架构，明确建模和学习用户的比较购物行为。通过离线和在线实验验证系统效果。

Result: 实验表明该方法在关键业务指标上取得显著提升：NDCG提高1.7%，预订转化率提升0.6%。与最先进方法相比，LTCS表现显著更优。

Conclusion: LTCS系统通过建模用户比较购物行为，有效解决了传统排序模型忽视列表上下文的局限性，显著提升了搜索效果和用户体验。
