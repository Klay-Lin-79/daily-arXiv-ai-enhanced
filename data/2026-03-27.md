...
TL;DR: SITH是一个无需数据、无需训练的可解释性框架，通过分析CLIP视觉transformer的权重空间来理解注意力头的语义机制，并支持精确的模型编辑。


<details>
  <summary>Details</summary>
Motivation: 现有基于激活的可解释性方法依赖数据集、易受数据偏差影响，且只能提供粗粒度的头级解释。需要一种更直接、数据无关的方法来理解视觉语言模型的内部机制。

Method: SITH框架直接分析CLIP视觉transformer的权重空间。对每个注意力头，将其值输出矩阵分解为奇异向量，然后通过COMP算法将这些向量解释为稀疏、语义连贯的人类可解释概念组合。

Result: SITH产生连贯、忠实的内头解释，通过重建保真度和可解释性实验验证。支持精确的权重空间模型编辑，能够放大或抑制特定概念，无需重新训练即可提升下游性能。还能研究模型适应过程，发现微调主要是重新加权稳定的语义基础而非学习全新特征。

Conclusion: SITH提供了一种数据无关、训练无关的可解释性方法，能够深入理解transformer头的语义机制，支持精确的模型编辑，并为研究模型适应过程提供了新视角。
...
TL;DR: ReDiPrune是一种无需训练的视觉token剪枝方法，在视觉-语言投影器之前直接选择信息丰富的视觉token，提高多模态大语言模型的效率


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型计算成本高，因为Transformer需要处理大量视觉token。现有方法通常在投影后压缩表示，会丢失细粒度空间和语义信息

Method: 在视觉编码器输出和投影器之间插入ReDiPrune，使用轻量级规则为每个token评分，综合考虑文本条件相关性和最大-最小多样性，选择既与查询相关又不冗余的token

Result: 在4个视频和5个图像基准测试中，ReDiPrune持续改善准确率-效率权衡。例如在EgoSchema上，仅保留15%视觉token就能获得+2.0%绝对准确率提升，同时减少6倍以上TFLOPs计算量

Conclusion: ReDiPrune是一种即插即用的训练前token剪枝方法，无需重新训练或架构修改，能有效提高多模态大语言模型的效率同时保持或提升性能
...
TL;DR: 本文针对多模态模型中的上下文学习敏感性问题，提出了能力导向的分类法、大规模数据集和轻量级模块，显著提升了上下文学习的稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 上下文学习虽然无需训练即可适应新任务，但对示例选择和格式高度敏感，特别是在统一多模态模型中，跨模态干扰和认知需求差异加剧了这种敏感性，导致效果不稳定且高度依赖具体任务。

Method: 1) 提出六层能力导向分类法，从基础感知到高阶辨别对演示的功能角色进行分类；2) 构建UniICL-760K大规模语料库，包含15个子任务的8-shot上下文学习示例；3) 设计Context-Adaptive Prototype Modulator轻量级即插即用模块，稳定少样本适应。

Result: 在UniICL-Bench上的评估显示，该方法在大多数理解型上下文学习任务中超越了参数更大的多模态大语言模型基线，取得了高度竞争力的统一结果。

Conclusion: 通过认知框架、大规模数据和架构干预相结合的方法，能够有效诊断和缓解多模态上下文学习的敏感性问题，为稳定高效的少样本适应提供了系统解决方案。
...
Method: 提出双向相关图域适应框架：1) 通过虚拟域桥接进行知识迁移，利用双向相关图合成标注和未标注图像，采用固定比例和渐进动态MixUp策略生成虚拟图像；2) 使用原型对齐和伪标签校正缓解确认偏差，通过可学习原型余弦相似度分类器实现虚拟域与真实域之间的双向原型对齐。

Result: 在三个公共多域数据集上的实证评估表明该方法具有优越性，特别是在标注样本非常有限的情况下仍表现出优异性能。

Conclusion: BCMDA框架有效解决了混合域半监督医学图像分割中的域偏移和确认偏差问题，在有限标注条件下实现了卓越的分割性能。
...
TL;DR: LLaVA-LE是一个专门用于月球探索的视觉语言模型，通过构建大规模月球数据集LUCID和两阶段训练，在月球地形分析任务上显著超越了基础模型。
...
Method: 1. 构建LUCID数据集：包含96k高分辨率全色图像与详细地形描述，以及81k个问答对；2. 两阶段训练：第一阶段进行领域特定地形描述的概念对齐，第二阶段进行指令调优的视觉问答训练。

Result: LLaVA-LE相比基础LLaVA模型获得了3.3倍的整体性能提升，相比第一阶段模型提升了2.1倍，推理得分达到1.070，超过了评判者自身的参考分数。

Conclusion: 领域特定的多模态数据和指令调优能有效推进视觉语言模型在行星探索中的应用，为月球表面和地下特征分析提供了专门化的解决方案。
...
TL;DR: 提出室内场景中的相似物体检测任务，利用相同或相似物体对的重复和互补线索，通过多视角图像输入来分类物体对为相同、相似或不同。


<details>
  <summary>Details</summary>
Motivation: 现有3D物体理解和生成方法忽略了真实场景中普遍存在的重复物体信息，这些重复物体提供了重要的感知线索。

Method: 提出Lookalike3D多视角图像变换器，利用大型图像基础模型的强语义先验来有效区分相似物体对，并构建了3DTwins数据集进行训练和评估。

Result: 在ScanNet++基础上构建的3DTwins数据集包含76k手动标注的物体对，相比基线方法提高了104%的IoU，显著提升了3D物体重建和部件协同分割等下游任务性能。

Conclusion: 将重复和相似物体转化为强大线索，可实现一致、高质量的3D感知，为3D场景理解提供了新的有效方法。
...
TL;DR: 该研究使用语义模糊图像作为解释性探针，通过CLIP嵌入空间的概念插值生成连续模糊图像谱，比较人类和机器分类器在概念边界上的差异，发现机器分类器更偏向"兔子"，而人类更接近用于合成的CLIP嵌入。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索人类和机器视觉系统如何处理语义模糊图像，特别是在经典的鸭兔错觉中，两者如何划定概念边界。通过模糊图像作为诊断工具，可以揭示人类与模型之间的对齐程度、鲁棒性和可解释性。

Method: 提出心理物理学启发的框架，在CLIP嵌入空间中对概念进行插值，生成连续的模糊图像谱。通过精确测量人类和机器分类器放置语义边界的位置和方式，比较两者的差异。

Result: 研究发现机器分类器更偏向将模糊图像识别为"兔子"，而人类观察者更接近用于图像合成的CLIP嵌入。引导尺度对人类敏感性的影响比对机器分类器更强。
...
TL;DR: OpenCap Monocular：基于单部智能手机视频的3D骨骼运动学和动力学估计算法，通过优化单目姿态估计、生物力学约束和物理模拟，实现低成本、可扩展的生物力学评估。


<details>
  <summary>Details</summary>
Motivation: 传统运动学和动力学分析需要在专业实验室进行，成本高、耗时长，限制了临床转化。需要开发可扩展、准确的生物力学评估工具。

Method: 1. 从单目姿态估计模型（WHAM）优化3D人体姿态估计；2. 计算生物力学约束骨骼模型的运动学；3. 通过物理模拟和机器学习估计动力学。

Result: 与标记式运动捕捉和测力板数据相比：运动学误差低（旋转自由度平均绝对误差4.8°，骨盆平移3.4 cm），比仅回归的计算机视觉基线旋转精度提高48%，平移精度提高69%；地面反作用力估计准确度与双摄像头系统相当或更好。

Conclusion: OpenCap Monocular能够以临床意义准确的精度估计重要的动力学结果，通过智能手机应用、网页应用和安全云计算部署，实现免费、可访问的单智能手机生物力学评估。
...
TL;DR: TIGeR是一个多模态变换器模型，将图像、地理位置和时间映射到统一的时空嵌入空间，支持地理定位、拍摄时间预测和时空感知检索任务。


<details>
  <summary>Details</summary>
Motivation: 数字取证、城市监测和环境分析等实际应用需要联合推理视觉外观、地理位置和时间信息。现有方法主要关注标准的地理定位和拍摄时间预测，但实际应用需要更复杂的能力，如检索在相同位置但特定目标时间拍摄的图像。

Method: 提出了TIGeR模型，基于多模态变换器架构，将图像、地理位置和时间映射到统一的时空嵌入空间。模型支持灵活输入配置（单模态和多模态查询），使用相同表示执行地理定位、拍摄时间预测和时空感知检索。通过更好地保持位置身份在大外观变化下的稳定性，实现基于场景时空而非纯视觉相似性的检索。

Result: 在包含450万训练样本和8.6万高质量评估样本的数据集上，TIGeR在多个任务上显著优于基线方法和最先进方法：时间预测（年份提升16%，日期提升8%），时空感知检索召回率提升14%。

Conclusion: 统一的时空建模方法在复杂时空感知图像检索任务中具有显著优势，TIGeR模型能够更好地保持位置身份信息，实现基于场景时空而非纯视觉相似性的检索，为实际应用提供了有效的解决方案。
...
TL;DR: DRoPS：利用静态预扫描作为几何和外观先验，通过表面对齐的高斯原语网格结构和CNN运动参数化，显著提升动态场景重建质量


<details>
  <summary>Details</summary>
Motivation: 现有方法在从极端新视角重建动态场景时表现不佳，特别是存在高度关节运动时。现有方法未能充分利用预扫描信息，需要更好的方法来约束解空间并确保几何一致性。

Method: 1. 使用静态预扫描作为显式几何和外观先验；2. 将高斯原语组织成锚定在物体表面的像素网格，建立网格结构和表面对齐模型；3. 利用原语的网格结构，通过CNN参数化运动，注入强隐式正则化并关联邻近点运动。

Result: 大量实验表明，该方法在渲染质量和3D跟踪精度方面显著优于当前最先进方法。

Conclusion: DRoPS通过有效利用静态预扫描先验、创新的表面对齐网格结构和CNN运动参数化，成功解决了动态场景重建中的几何一致性问题，在极端视角和关节运动情况下表现出色。
...
TL;DR: AVControl是一个基于LTX-2音频视觉基础模型的轻量级可扩展框架，通过LoRA适配器实现多模态控制，无需修改模型架构，在视频生成控制方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的视频和音频生成控制方法要么训练单一固定控制集的大型模型，要么需要为每个新模态引入昂贵的架构修改，缺乏灵活性和效率。

Method: 基于LTX-2音频视觉基础模型，采用并行画布方法，每个控制模态作为独立的LoRA适配器训练，将参考信号作为注意力层的额外token，无需架构修改。

Result: 在VACE基准测试中，在深度和姿态引导生成、修复、外绘方面优于所有基线，在相机控制和音频视觉基准上表现有竞争力。支持多种独立训练模态，计算和数据效率高。

Conclusion: AVControl提供了一个高效、可扩展的多模态控制框架，通过LoRA适配器实现灵活的视频和音频生成控制，在保持性能的同时显著降低了训练成本和复杂性。
...
<details>
  <summary>Details</summary>
Motivation: 探索扩散变换器(DiT)在生成任务中的隐藏潜力，通过分析去噪过程发现引入学习缩放参数可以显著提升DiT块性能

Method: 提出Calibri方法，将DiT校准视为黑盒奖励优化问题，使用进化算法高效求解，仅修改约100个参数

Result: Calibri在各种文本到图像模型中一致提升性能，同时减少图像生成所需的推理步骤，同时保持高质量输出

Conclusion: Calibri是一种参数高效的方法，通过优化校准DiT组件显著提升生成质量，证明了DiT在生成任务中的巨大潜力
...
### [13] [Dissecting Model Failures in Abdominal Aortic Aneurysm Segmentation through Explainability-Driven Analysis](https://arxiv.org/abs/2603.24801)
...
Method: 提出XAI引导的编码器塑形框架：1)从最终编码器块计算密集的归因注意力场；2)将预测概率质量与注意力场对齐；3)将注意力场输入轻量级细化路径和置信度先验，在推理时调制logits

Result: 在临床验证的挑战性病例上评估，相比基础SAM设置，该方法实现了显著改进，表明通过XAI引导显式优化编码器注意力是有效的

Conclusion: 将归因引导集成到表示和解码中是实用有效的原则，能提升复杂场景下分割的可靠性
...
### [14] [GoldiCLIP: The Goldilocks Approach for Balancing Explicit Supervision for Language-Image Pretraining](https://arxiv.org/abs/2603.24804)
*Deen Dayal Mohan,Hossein Souri,Vitali Petsiuk,Juhong Min,Gopal Sharma,Luowei Zhou,Suren Kumar*

Main category: cs.CV

TL;DR: GoldiCLIP是一个基于"恰到好处"原则的视觉语言模型训练框架，仅用3000万图像（比主流方法少300倍）就能实现最先进的数据效率，在多种检索任务上显著超越基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前大规模视觉语言模型主要依赖数十亿样本数据集，这成为研究进展的障碍。虽然近期工作开始通过提升监督质量来缩小差距，但每个方法只解决了对比预训练中的部分弱点。

Method: GoldiCLIP采用三方面创新：1）文本条件自蒸馏方法对齐文本无关和文本条件特征；2）集成解码器的编码器配合VQA目标，使编码器能超越标题式查询进行泛化；3）基于不确定性的加权机制自动平衡所有异构损失。

Result: 仅用3000万图像训练，GoldiCLIP在数据高效方法中达到最先进水平：MSCOCO检索提升2.2点，细粒度检索提升2.0点，基于问题的检索提升5.9点，同时与数十亿规模模型保持竞争力。

Conclusion: GoldiCLIP通过找到监督信号的恰当平衡，证明了高质量监督可以显著减少数据需求，为资源受限环境下的视觉语言模型训练提供了有效解决方案。
...
### [15] [Attention-based Pin Site Image Classification in Orthopaedic Patients with External Fixators](https://arxiv.org/abs/2603.24815)
...
<details>
  <summary>Details</summary>
Motivation: 针道是外部固定器与骨骼连接的接口，容易发生感染，导致患者疼痛和并发症。目前缺乏有效的针道感染自动识别方法，需要开发基于视觉特征的分类技术来改善患者体验。

Method: 收集并创建针道感染数据集，提出基于注意力的深度学习模型，强调相关区域并减少针体干扰。引入高效冗余重建卷积（ERRC）方法，增强特征图丰富性同时减少参数数量。

Result: 模型在AUC（0.975）和F1分数（0.927）上优于基线方法，仅需5.77M参数。能够基于视觉感染征象准确分类针道状态，与医疗专业人员评估一致。
...
### [16] [Generative Adversarial Perturbations with Cross-paradigm Transferability on Localized Crowd Counting](https://arxiv.org/abs/2603.24821)
*Alabi Mehzabin Anisha,Guangjing Wang,Sriram Chellappan*

Main category: cs.CV

TL;DR: 提出了一种新颖的对抗攻击框架，能够同时攻击密度图模型和点回归模型两种人群计数范式，通过多任务损失优化实现跨范式攻击


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击研究主要关注密度图模型之间的可迁移性，但跨范式攻击（密度图模型与点回归模型之间）尚未探索。考虑到人群计数领域的安全影响，需要研究能够同时攻击两种主流范式的对抗攻击方法
...
Conclusion: 首次实现了跨人群计数范式的对抗攻击，提出的框架能有效攻击密度图和点回归两种主流模型，为人群计数系统的安全评估提供了新视角
...
### [17] [DCARL: A Divide-and-Conquer Framework for Autoregressive Long-Trajectory Video Generation](https://arxiv.org/abs/2603.24835)
*Junyi Ouyang,Wenbin Teng,Gonglin Chen,Yajie Zhao,Haiwei Chen*

Main category: cs.CV

TL;DR: DCARL提出了一种分治自回归框架，通过关键帧生成器和插值生成器的结合，实现了32秒长轨迹视频的稳定高质量生成，解决了现有视频扩散模型的可扩展性限制和自回归模型的视觉漂移问题。


<details>
  <summary>Details</summary>
Motivation: 长轨迹视频生成是世界建模的关键但具有挑战性的任务，主要因为现有视频扩散模型（VDMs）的可扩展性有限。自回归模型虽然能实现无限扩展，但存在视觉漂移和可控性差的问题。

Method: DCARL采用分治自回归框架：1）关键帧生成器在没有时间压缩的情况下训练，建立长范围、全局一致的结构锚点；2）插值生成器以自回归方式合成密集帧，使用重叠片段，利用关键帧提供全局上下文，单个干净的前一帧提供局部连贯性。

Result: 在大规模互联网长轨迹视频数据集上训练，DCARL在视觉质量（更低的FID和FVD）和相机一致性（更低的ATE和ARE）方面优于最先进的自回归和分治基线方法，能够稳定生成长达32秒的高质量长轨迹视频。

Conclusion: DCARL通过将分治方案的结构稳定性与视频扩散模型的高保真生成能力有效结合，成功解决了长轨迹视频生成中的视觉漂移和可控性问题，为世界建模提供了有效的解决方案。
...
### [18] [WAFT-Stereo: Warping-Alone Field Transforms for Stereo Matching](https://arxiv.org/abs/2603.24836)
*Yihan Wang,Jia Deng*
...
<details>
  <summary>Details</summary>
Motivation: 现有立体匹配方法普遍依赖成本体积设计，但这种方法效率较低。作者旨在证明成本体积不是高性能的必要条件，可以通过更高效的扭曲操作替代。

Method: WAFT-Stereo采用基于扭曲的方法进行立体匹配，完全摒弃了传统方法中常用的成本体积设计，通过扭曲操作实现特征匹配。
...
### [19] [NeuroVLM-Bench: Evaluation of Vision-Enabled Large Language Models for Clinical Reasoning in Neurological Disorders](https://arxiv.org/abs/2603.24846)
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在图像决策支持方面展现出潜力，但其在神经影像学领域的可靠性和实际应用权衡尚未得到充分理解。需要系统评估这些模型在临床神经影像任务中的表现，为实际应用提供指导。

Method: 使用包含多发性硬化、中风、脑肿瘤、其他异常和正常对照的MRI和CT数据集，构建多阶段框架确保公平比较并控制选择偏倚。要求模型同时生成诊断、诊断亚型、成像模态、专业序列和解剖平面等多种输出，从四个维度评估：带弃权的判别分类、校准度、结构化输出有效性和计算效率。

Result: 技术成像属性（如模态和平面）几乎已解决，而诊断推理特别是亚型预测仍具挑战性。肿瘤分类最可靠，中风中等可解，多发性硬化和罕见异常仍困难。少样本提示可提升多个模型性能但增加令牌使用、延迟和成本。Gemini-2.5-Pro和GPT-5-Chat整体诊断性能最强，Gemini-2.5-Flash提供最佳效率-性能权衡。在开源模型中，MedGemma-1.5-4B表现最有前景，在少样本提示下接近多个专有模型的零样本性能，同时保持完美的结构化输出。

Conclusion: 该研究为多模态大语言模型在神经影像学中的性能、可靠性和效率权衡提供了实用见解，支持该领域的标准化评估，有助于指导临床应用中模型的选择和部署。
...
### [20] [CORA: A Pathology Synthesis Driven Foundation Model for Coronary CT Angiography Analysis and MACE Risk Assessment](https://arxiv.org/abs/2603.24847)
*Jinkui Hao,Gorkem Durak,Halil Ertugrul Aktas,Ulas Bagci,Bradley D. Allen,Nilay S. Shah,Bo Zhou*

Main category: cs.CV

TL;DR: CORA是一个用于心血管风险评估的3D视觉基础模型，通过病理中心、合成驱动的自监督框架从CCTA数据学习，在斑块表征、狭窄检测等任务上优于现有方法，并能改善30天主要不良心脏事件风险分层。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉疾病是全球心血管死亡的主要原因，虽然CCTA可用于非侵入性评估，但深度学习在临床转化中面临专家标注数据稀缺的挑战。现有的无标签预训练策略偏向全局解剖统计特征，难以捕捉冠状动脉斑块的空间局部病理特征。

Method: 提出CORA模型，采用病理中心、合成驱动的自监督框架，通过解剖引导的病变合成引擎，训练模型检测模拟的血管异常，使表征学习偏向临床相关的疾病特征而非主导的背景解剖结构。在12,801个无标签CCTA体积上进行训练，并与大型语言模型结合形成多模态框架。

Result: 在来自9家独立医院的多中心数据集上评估，CORA在诊断和解剖任务（包括斑块表征、狭窄检测和冠状动脉分割）上始终优于最先进的3D视觉基础模型，性能提升高达29%。多模态框架显著改善了30天主要不良心脏事件风险分层。
...
### [21] [OptiSAR-Net++: A Large-Scale Benchmark and Transformer-Free Framework for Cross-Domain Remote Sensing Visual Grounding](https://arxiv.org/abs/2603.24876)
*Xiaoyu Tang,Jun Dong,Jintao Cheng,Rui Fan*

Main category: cs.CV

TL;DR: 本文提出了跨域遥感视觉定位任务(CD-RSVG)及首个大规模数据集OptSAR-RSVG，并设计了OptiSAR-Net++框架，通过PL-MoE模块、CLIP对比范式、动态对抗负采样等方法，在保持高效的同时实现了跨域特征建模和细粒度语义判别。
...
Method: 提出OptiSAR-Net++框架：1) PL-MoE模块用于高效跨域特征解耦；2) 采用CLIP对比范式替代Transformer解码，结合动态对抗负采样，将生成式回归转化为跨模态匹配；3) 引入文本引导双门融合模块(TGDF-SSA)和区域感知辅助头，增强语义-视觉对齐和空间建模。

Result: 在OptSAR-RSVG和DIOR-RSVG基准测试中达到最先进性能，在定位精度和效率方面具有显著优势。

Conclusion: 提出的跨域遥感视觉定位任务、数据集和OptiSAR-Net++框架有效解决了跨域特征建模、计算效率和语义判别等挑战，为实际应用提供了有力支持。
...
### [22] [SurgPhase: Time efficient pituitary tumor surgery phase recognition via an interactive web platform](https://arxiv.org/abs/2603.24897)
...
TL;DR: 提出一个用于垂体瘤手术视频阶段识别的综合框架，结合自监督学习、时序建模和可扩展标注策略，达到90%准确率，并开发了协作平台促进数据收集和模型改进。


<details>
  <summary>Details</summary>
Motivation: 准确的手术阶段识别对于分析手术流程、支持术中决策以及推动数据驱动的手术教育和绩效评估至关重要。当前面临标注数据有限和手术变异性等挑战。

Method: 1. 开发协作在线平台供外科医生上传手术视频、接收自动阶段分析并贡献数据；2. 在251个未标记PTS视频上使用自监督框架预训练ResNet-50模型；3. 在81个标记手术数据集上微调，采用焦点损失、渐进层解冻和动态采样解决类别不平衡和手术变异性。

Result: 在保留测试集上达到90%的准确率，优于当前最先进方法，并在不同手术案例中表现出强大的泛化能力。

Conclusion: 该框架通过结合自监督学习、稳健的时序建模和可扩展标注策略，有效解决了垂体瘤手术阶段识别问题，同时开发的协作平台促进了大规模数据收集和持续模型改进。
...
### [23] [Self-Supervised Learning for Knee Osteoarthritis: Diagnostic Limitations and Prognostic Value of Uncurated Hospital Data](https://arxiv.org/abs/2603.24903)
*Haresh Rengaraj Rajamohan,Yuxuan Chen,Kyunghyun Cho,Cem M. Deniz*

Main category: cs.CV

TL;DR: 自监督学习在膝关节骨关节炎诊断和预后建模中的效果评估：图像自监督学习在诊断任务中表现不一，多模态图像-文本自监督学习在预后预测中表现优异
...
Method: 比较两种自监督学习方法：(1) 基于OAI、MOST和NYU队列膝关节X光片的图像自监督学习；(2) 基于未筛选医院膝关节X光片与放射科医生报告配对的多模态图像-文本自监督学习。评估在Kellgren-Lawrence分级诊断和4年结构发生/进展预后预测中的性能

Result: 在诊断任务中：图像自监督学习在线性探测（冻结编码器）时提高准确性，但在完全微调时未超越ImageNet预训练；多模态自监督学习未改善分级性能，归因于未筛选医院数据中严重偏倚（93%估计KL 3级）。在预后任务中：多模态初始化显著改善预测性能，在外部验证中优于ImageNet基线（MOST AUROC：0.701 vs. 0.599，10%标记数据）

Conclusion: 未筛选医院图像-文本数据由于严重性偏倚可能不适合学习诊断任务，但当下游任务与预训练数据分布一致时，可为预后建模提供强信号。自监督学习在膝关节骨关节炎建模中的效果取决于任务与预训练数据的对齐程度
...
### [24] [ICTPolarReal: A Polarized Reflection and Material Dataset of Real World Objects](https://arxiv.org/abs/2603.24912)
*Jing Yang,Krithika Dharanikota,Emily Jia,Haiwei Chen,Yajie Zhao*

Main category: cs.CV

TL;DR: 该研究构建了一个大规模的真实世界偏振反射与材质数据集，用于解决逆渲染中真实材质建模的挑战，显著提升了材质分离、光照保真度和几何一致性。


<details>
  <summary>Details</summary>
Motivation: 准确建模真实世界材质的光反射是逆渲染的核心挑战，主要原因是缺乏真实测量的反射数据。现有方法严重依赖合成数据集，这些数据集光照简化且材质真实感有限，导致模型难以泛化到真实世界图像。

Method: 构建了一个大规模的真实世界偏振反射与材质数据集，使用配备交叉/平行偏振的8相机、346光源的光照舞台，涵盖218个日常物体，包含多视角、多光照、偏振、反射分离和材质属性五个采集维度，生成超过120万张高分辨率图像，包含漫反射-镜面反射分离以及解析推导的漫反射反照率、镜面反射反照率和表面法线。

Result: 使用该数据集训练和评估了最先进的逆渲染和正向渲染模型，在固有分解、重光照和稀疏视图3D重建方面展示了显著改进，包括材质分离、光照保真度和几何一致性的大幅提升。
...
### [25] [TIGFlow-GRPO: Trajectory Forecasting via Interaction-Aware Flow Matching and Reward-Driven Optimization](https://arxiv.org/abs/2603.24936)
*Xuepeng Jing,Wenhuan Lu,Hao Meng,Zhizhi Yu,Jianguo Wei*

Main category: cs.CV

TL;DR: TIGFlow-GRPO：两阶段生成框架，通过条件流匹配建模轨迹分布，再通过GRPO后训练将轨迹生成与行为规则对齐，提升轨迹预测的社会合规性和物理可行性。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测方法主要关注监督拟合，未能充分反映社会规范和场景约束在生成轨迹中的体现，导致预测轨迹可能不符合实际行为规则。

Method: 提出两阶段框架：第一阶段使用基于条件流匹配的预测器，结合轨迹交互图模块建模细粒度视觉空间交互；第二阶段进行Flow-GRPO后训练，将确定性流展开重构为随机ODE-to-SDE采样，通过复合奖励（视图感知的社会合规性和地图感知的物理可行性）引导轨迹探索。

Result: 在ETH/UCY和SDD数据集上的实验表明，TIGFlow-GRPO提高了预测精度和长时程稳定性，同时生成更符合社会规范和物理可行性的轨迹。

Conclusion: 该框架为动态多媒体环境中基于流的轨迹建模与行为感知对齐提供了有效方法，能够生成更符合实际行为规则的轨迹预测。
...
TL;DR: 提出一个基于自回归扩散模型的无限时长原始注视预测框架，用于任意长度视频中连续时空坐标的高分辨率注视轨迹生成。


<details>
  <summary>Details</summary>
Motivation: 传统显著性图和扫描路径方法无法捕捉原始注视的细粒度时间动态，且现有模型受限于短时间窗口（约3-5秒），无法捕获真实世界内容中固有的长程行为依赖。

Method: 使用自回归扩散模型，在显著性感知的视觉潜在空间条件下，合成具有连续空间坐标和高分辨率时间戳的注视轨迹。

Result: 定量和定性评估表明，该方法在长程时空准确性和轨迹真实性方面显著优于现有方法。

Conclusion: 提出的生成框架能够有效预测任意长度视频中的无限时长原始注视，克服了传统方法的时空限制，为场景理解和多模态交互提供了更精细的注视动态建模。
...
TL;DR: TIES是一种动态视觉token选择框架，通过层间排名一致性指导token筛选，在显著减少78%token使用的同时提升6%成功率，无需额外训练。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型存在高推理延迟问题，主要源于密集视觉token处理。传统基于注意力幅度的静态token选择方法存在局限性，高注意力token具有任务依赖性，甚至可能降低策略性能。

Method: 提出TIES框架，通过tau引导的层间高效选择机制，动态平衡注意力幅度与层间token排名一致性，实现鲁棒的token选择，无需额外训练。

Result: 在CogACT + SIMPLER基准测试中，TIES将平均成功率提升6%，同时减少78%的token使用量，并在不同解码器和基准测试中展现出强泛化能力。

Conclusion: TIES通过动态层间一致性指导的token选择，有效解决了VLA模型推理延迟问题，在保持甚至提升性能的同时显著减少计算开销。
...
### [28] [Self-Corrected Image Generation with Explainable Latent Rewards](https://arxiv.org/abs/2603.24965)
*Yinyi Luo,Hrishikesh Gokhale,Marios Savvides,Jindong Wang,Shengfeng He*

Main category: cs.CV

TL;DR: xLARD是一个自校正文本到图像生成框架，使用多模态大语言模型通过可解释的潜在奖励来指导生成过程，解决复杂提示中细粒度语义和空间关系对齐的挑战。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成在复杂提示对齐方面存在挑战，特别是细粒度语义和空间关系。生成的前馈性质要求在不完全理解输出的情况下预测对齐，而评估生成图像相对更容易。基于这种不对称性，研究者提出利用多模态大语言模型来指导生成。

Method: 提出xLARD框架，包含轻量级校正器，基于模型生成参考的结构化反馈来细化潜在表示。核心组件是从潜在编辑到可解释奖励信号的可微分映射，使得不可微分的图像级评估能够提供连续的潜在级指导。

Result: 在多样化的生成和编辑任务实验中，xLARD改善了语义对齐和视觉保真度，同时保持了生成先验。该方法使模型能够在生成过程中理解、评估和校正自身。

Conclusion: xLARD通过可解释的潜在奖励机制，利用多模态大语言模型的评估能力来指导文本到图像生成，有效解决了复杂提示对齐问题，提高了生成质量。
...
### [29] [PASDiff: Physics-Aware Semantic Guidance for Joint Real-world Low-Light Face Enhancement and Restoration](https://arxiv.org/abs/2603.24969)
*Yilin Ni,Wenjie Li,Zhengxue Wang,Juncheng Li,Guangwei Gao,Jian Yang*

Main category: cs.CV

TL;DR: PASDiff是一个无需训练的物理感知语义扩散模型，用于解决真实世界低光照人脸图像的多重退化问题，通过逆强度加权和Retinex理论恢复自然光照和色彩，同时利用风格无关结构注入技术重建面部细节。


<details>
  <summary>Details</summary>
Motivation: 真实世界低光照人脸图像存在多重退化问题（低光照、模糊、噪声、低可见度等），现有级联解决方案容易产生严重误差累积，而通用联合模型缺乏明确的面部先验，难以恢复清晰的面部结构。

Method: 提出PASDiff（物理感知语义扩散）模型，采用无需训练的方式：1）利用逆强度加权和Retinex理论引入光度约束，可靠恢复可见度和自然色彩；2）设计风格无关结构注入（SASI）技术，从现成面部先验中提取结构同时过滤其固有光度偏差，将身份特征与物理约束无缝融合；3）构建WildDark-Face真实世界基准数据集。

Result: 大量实验表明，PASDiff显著优于现有方法，在自然光照、色彩恢复和身份一致性之间实现了优越的平衡。构建的WildDark-Face数据集包含700张具有复杂退化的真实世界低光照人脸图像。

Conclusion: PASDiff通过物理感知语义扩散方法有效解决了真实世界低光照人脸图像的多重退化问题，无需训练即可实现高质量的面部恢复，在光照、色彩和身份一致性方面表现出色。
...
### [30] [MoE-GRPO: Optimizing Mixture-of-Experts via Reinforcement Learning in Vision-Language Models](https://arxiv.org/abs/2603.24984)
...
<details>
  <summary>Details</summary>
Motivation: 当前MoE-VLM中广泛使用的确定性top-K路由机制可能忽略更优的专家组合，导致专家过拟合，需要提升专家选择的多样性

Method: 提出MoE-GRPO框架：1) 将专家选择建模为序列决策问题，使用Group Relative Policy Optimization (GRPO)进行优化；2) 引入模态感知路由指导，防止路由器探索特定模态下很少激活的专家

Result: 在多模态图像和视频基准测试中，MoE-GRPO持续优于标准top-K路由及其变体，通过促进更丰富的专家选择缓解了专家过拟合，实现了任务级专家专业化

Conclusion: MoE-GRPO通过强化学习优化专家路由，有效提升了MoE-VLM的性能，为稀疏激活模型提供了更灵活和自适应的专家选择机制
...
### [31] [Few-Shot Left Atrial Wall Segmentation in 3D LGE MRI via Meta-Learning](https://arxiv.org/abs/2603.24985)
*Yusri Al-Sanaani,Rebecca Thornhill,Pablo Nery,Elena Pena,Robert deKemp,Calum Redpath,David Birnie,Sreeraman Rajan*

Main category: cs.CV

TL;DR: 提出基于元学习的少样本3D左心房壁分割方法，在5-20个样本下显著优于监督微调，接近全监督性能，对未见域偏移保持鲁棒性


<details>
  <summary>Details</summary>
Motivation: 左心房壁分割面临薄壁几何结构、低对比度和专家标注稀缺的挑战，需要开发少样本学习方法以降低临床标注成本

Method: 采用模型无关元学习框架，结合左心房和右心房腔辅助任务进行元训练，使用边界感知复合损失函数强调薄结构精度

Result: 在5-shot时Dice分数0.64 vs 0.52，HD95 5.70mm vs 7.60mm优于监督微调；20-shot时接近全监督性能；在未见域偏移和本地队列中保持鲁棒性

Conclusion: 元学习方法在少样本条件下能实现更准确可靠的薄壁边界分割，有望通过最小化额外标注实现心房重构评估的临床转化
...
### [32] [Towards Video Anomaly Detection from Event Streams: A Baseline and Benchmark Datasets](https://arxiv.org/abs/2603.24991)
...
Method: 提出EWAD框架：1）事件密度感知动态采样策略选择时序信息丰富的片段；2）密度调制时序建模方法从稀疏事件流中捕捉上下文关系；3）RGB到事件的知识蒸馏机制在弱监督下增强事件表示。

Result: 在三个基准数据集上的广泛实验表明，EWAD相比现有方法取得了显著改进，证明了事件驱动建模在视频异常检测中的潜力和有效性。

Conclusion: 本文首次将事件流视频异常检测确立为统一研究方向，构建了基准数据集并提出了有效的EWAD框架，为事件驱动的视频异常检测开辟了新途径。
...
### [33] [C2W-Tune: Cavity-to -Wall Transfer Learning for Thin Atrial Wall Segmentation in 3D Late Gadolinium-enhanced Magnetic Resonance](https://arxiv.org/abs/2603.24992)
...
<details>
  <summary>Details</summary>
Motivation: 3D LGE-MRI中左心房壁的准确分割对于壁厚映射和纤维化量化至关重要，但由于壁薄、解剖结构复杂和对比度低，这一任务具有挑战性。

Method: 提出两阶段框架：第一阶段使用3D U-Net（ResNeXt编码器和实例归一化）预训练左心房腔室分割；第二阶段通过渐进层解冻策略将权重转移到左心房壁分割，保留心内膜特征的同时进行壁特异性细化。

Result: 在2018 LA分割挑战数据集上，相比从头训练的基线模型，壁Dice从0.623提升到0.814，1mm表面Dice从0.553提升到0.731。边界误差显著降低，HD95从2.95mm降至2.55mm，ASSD从0.71mm降至0.63mm。即使在70个训练样本的有限监督下，仍能达到Dice 0.78和HD95 3.15mm的竞争性性能。

Conclusion: 基于解剖学的任务转移与受控微调相结合，显著提高了3D LGE-MRI中薄壁左心房壁分割的边界准确性，为临床应用提供了更可靠的工具。
...
### [34] [Relaxed Rigidity with Ray-based Grouping for Dynamic Gaussian Splatting](https://arxiv.org/abs/2603.24994)
*Junoh Leea,Junmyeong Lee,Yeon-Ji Song,Inhwan Bae,Jisu Shin,Hae-Gon Jeon,Jin-Hwa Kim*

Main category: cs.CV

TL;DR: 提出一种基于视图空间射线分组的新方法，通过聚类被同一射线穿过的3D高斯并施加约束来保持局部几何结构一致性，从而在4D动态场景重建中实现更真实的物理运动建模。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯泼溅的动态3D场景重建方法在建模真实运动方面存在挑战，特别是对于单目视频数据集，大多数方法无法将高斯运动与现实物理动力学对齐，导致局部几何结构破坏和重建质量下降，现有方法过度依赖光流或2D轨迹等外部先验。

Method: 提出视图空间射线分组策略：聚类被同一射线穿过的3D高斯（仅考虑α混合权重超过阈值的高斯），然后对这些分组施加约束以保持一致的空间分布，从而保持局部几何结构，实现更物理合理的运动模型，无需外部指导。

Result: 将方法集成到两个不同的基线模型中，在具有挑战性的单目数据集上进行广泛实验，结果显示该方法显著优于现有方法，实现了更好的时间一致性和重建质量。

Conclusion: 通过视图空间射线分组策略显式保持4D场景中高斯局部几何结构的方法，能够实现更物理合理的运动建模，提高动态3D场景重建的时间一致性和质量，无需依赖外部先验。
...
### [35] [Distributed Real-Time Vehicle Control for Emergency Vehicle Transit: A Scalable Cooperative Method](https://arxiv.org/abs/2603.25000)
*WenXi Wang,JunQi Zhang*

Main category: cs.CV

TL;DR: 提出一种分布式车辆控制方法，让车辆仅使用局部信息在线调整驾驶行为，实现应急车辆快速通行同时减少对普通车辆影响，具有高可扩展性和实时决策能力。


<details>
  <summary>Details</summary>
Motivation: 现有应急车辆快速通行方法存在两大根本限制：集中式数学求解器计算成本高且仅适用于小规模场景；强化学习方法需要大量集中训练且训练模型对不同交通条件可扩展性有限。需要克服高计算成本和缺乏可扩展性的问题。

Method: 提出可扩展的分布式车辆控制方法，车辆仅使用局部信息（而非全局信息）以分布式方式在线调整驾驶行为。证明仅使用局部信息的分布式方法近似等价于使用全局信息的方法。进一步提出分布式冲突解决机制，通过避免决策冲突保证车辆安全。

Result: 基于真实世界交通数据集的仿真实验表明，相比现有方法，所提方法实现更快的决策速度、对普通车辆影响更小，在不同交通密度和道路配置下保持更强的可扩展性。

Conclusion: 所提出的分布式方法克服了现有方法的高计算成本和可扩展性限制，实现了应急车辆的快速通行，同时最小化对普通车辆的影响，具有实时决策、自然适应性和确定性安全保证等优势。
...
### [36] [Interpretable Zero-shot Referring Expression Comprehension with Query-driven Scene Graphs](https://arxiv.org/abs/2603.25004)
*Yike Wu,Necva Bolucu,Stephen Wan,Dadong Wang,Jiahao Xia,Jian Zhang*

Main category: cs.CV

TL;DR: SGREC是一种可解释的零样本指代表达理解方法，通过查询驱动的场景图作为结构化中介，结合视觉语言模型和大型语言模型，在多个基准测试中达到最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型（如CLIP）在零样本指代表达理解任务中难以捕捉细粒度视觉细节和理解复杂物体关系，而大型语言模型虽然擅长高级语义推理，但无法直接将视觉特征抽象为文本语义。需要一种方法能够结合两者的优势。

Method: 提出SGREC方法：1）使用视觉语言模型构建查询驱动的场景图，显式编码空间关系、描述性标题和物体交互；2）利用场景图作为结构化中介，弥合低级图像区域和高级语义理解之间的差距；3）让大型语言模型从场景图提供的结构化文本表示中推断目标物体，并提供详细的决策解释以确保可解释性。

Result: 在多个零样本REC基准测试中取得最佳性能：RefCOCO val (66.78%)、RefCOCO+ testB (53.43%)、RefCOCOg val (73.28%)，展示了强大的视觉场景理解能力。

Conclusion: SGREC通过查询驱动的场景图作为结构化中介，成功结合了视觉语言模型和大型语言模型的优势，在零样本指代表达理解任务中实现了高性能和可解释性，为视觉语言理解提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决在稀疏输入视图（少样本）条件下进行3D重建时，现有方法在重建质量和训练效率之间的权衡问题。TensorRF虽然训练速度快，但在少样本条件下重建质量下降；而传统方法需要大量输入视图。

Method: 结合TensorRF的高效张量表示与FreeNeRF的频率驱动正则化，引入频率掩码和遮挡掩码来提升少样本条件下的重建稳定性。TensorRF加速渲染速度，频率正则化提升稀疏视图下的重建质量。

Result: 在Synthesis NeRF基准测试中，PSNR从TensorRF的21.45dB提升到23.70dB，微调版本达到24.52dB，同时保持10-15分钟的训练时间。在THuman 2.0数据集上，仅用8张输入图像就实现了27.37-34.00dB的竞争性性能。

Conclusion: Few TensoRF是一种高效且数据有效的实时3D重建解决方案，在保持TensorRF快速训练优势的同时，显著提升了少样本条件下的重建质量，适用于多样化场景。
...
TL;DR: 提出GDPO-Listener框架，通过自回归流匹配架构和分组奖励解耦策略优化，解决对话中听众头部运动的"回归均值"问题，实现高表达性的说话和聆听动作生成


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成虚拟人对话中的头部运动时存在两个主要问题：1) 听众运动容易出现"回归均值"问题，导致面部表情僵化；2) 缺乏复杂非语言动作的参数空间。这限制了虚拟人交互的真实感和表达性。

Method: 提出GDPO-Listener框架，包含三个关键技术：1) 自回归流匹配架构，实现稳定的监督学习；2) 分组奖励解耦策略优化(GDPO)，通过对不同FLAME参数组进行独立的奖励归一化，显式激励高方差表达性生成；3) 显式语义文本控制，实现可定制的响应。

Result: 在Seamless Interaction和DualTalk数据集上的广泛评估表明，该方法在长期运动方差、视觉表达性和语义可控性方面均优于现有基线方法。

Conclusion: GDPO-Listener框架成功解决了对话中听众头部运动的表达性问题，通过创新的架构设计和优化策略，实现了高质量、可控的说话和聆听动作生成，为虚拟人交互提供了更真实的非语言行为。
...
TL;DR: VideoTIR：基于强化学习的多级工具包调用框架，通过RL优化长视频理解中的幻觉问题，提出TAGPO策略减少冗余调用，并在三个长视频QA基准上验证有效性


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在长视频理解中存在幻觉问题，主要源于文本和视觉token的不平衡。虽然SFT-based工具调用方法可以缓解此问题，但需要大量高质量数据且工具调用轨迹受限

Method: 提出VideoTIR框架，利用强化学习促进多级工具包的合理使用；采用Zero-RL和SFT冷启动；提出TAGPO策略通过逐步奖励分配和失败轨迹重用来减少冗余调用；开发基于沙箱的轨迹合成框架生成高质量轨迹数据

Result: 在三个长视频QA基准测试上的广泛实验证明了该方法的有效性和效率

Conclusion: VideoTIR通过强化学习优化多级工具包调用，有效缓解长视频理解中的幻觉问题，提高准确性和效率
...
<details>
  <summary>Details</summary>
Motivation: 现有医学图像修复方法通常需要任务特定的重新训练，且在忠实重建和先验驱动增强之间的权衡控制有限。过度激进的修复可能引入幻觉细节或改变诊断重要结构，这在临床环境中尤其成问题。

Method: CARE采用双潜在修复策略：一个分支强制执行数据保真度和解剖一致性，另一个分支利用生成先验恢复缺失或退化信息。风险感知自适应控制器根据修复不确定性和局部结构可靠性动态调整各分支贡献，实现保守或增强导向的修复模式。
...
TL;DR: GeoNDC是一种可查询的神经数据立方体，将行星尺度的地球观测数据编码为连续的时空隐式神经场，支持按需查询和连续时间重建，实现高压缩比和高保真度。


<details>
  <summary>Details</summary>
Motivation: 卫星地球观测积累了大量的时空档案，但这些数据以离散的栅格文件形式存储，导致存储、传输和查询成本高昂，需要一种更高效的数据表示方法。

Method: 提出GeoNDC（可查询神经数据立方体），将地球观测数据编码为连续的时空隐式神经场，支持直接时空查询和连续时间重建，无需完全解压缩。

Result: 在20年全球MODIS数据上实现95:1压缩比（0.44GB），保持高光谱保真度（R²>0.98）；在Sentinel-2数据上能恢复云遮挡下的动态变化（R²>0.85）；在HiGLASS生物物理产品上达到接近完美的精度（R²>0.98）。
...
### [42] [MoRGS: Efficient Per-Gaussian Motion Reasoning for Streamable Dynamic 3D Scenes](https://arxiv.org/abs/2603.25042)
...
<details>
  <summary>Details</summary>
Motivation: 现有在线4D重建方法虽然高效且视觉质量好，但无法学习反映真实场景动态的每个高斯点的运动。由于缺乏明确的运动线索，外观和运动仅通过光度损失优化，导致高斯运动追逐像素残差而非真实3D运动。

Method: 1. 利用关键视图上的稀疏光流作为轻量级运动线索，超越光度监督来正则化每个高斯点的运动；2. 学习每个高斯点的运动偏移场，协调投影3D运动与跨视图和时间观测流之间的差异；3. 引入每个高斯点的运动置信度，区分动态和静态高斯点，加权高斯属性残差更新，抑制静态区域的冗余运动。

Result: 大量实验表明，MoRGS在在线方法中实现了最先进的重建质量和运动保真度，同时保持了流式处理性能。

Conclusion: MoRGS通过显式建模每个高斯点的运动并利用光流监督，显著提高了在线4D重建的质量和运动保真度，为动态场景的实时重建提供了有效解决方案。
...
### [43] [GaussFusion: Improving 3D Reconstruction in the Wild with A Geometry-Informed Video Generator](https://arxiv.org/abs/2603.25053)
*Liyuan Zhu,Manjunath Narayana,Michal Stary,Will Hutchcroft,Gordon Wetzstein,Iro Armeni*

Main category: cs.CV

TL;DR: GaussFusion通过几何感知的视频生成技术改进3D高斯泼溅重建，减少浮动物体、闪烁和模糊等常见伪影，支持优化和前馈方法，在实时21FPS下保持SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决3D高斯泼溅重建在真实场景中的常见问题：浮动物体、闪烁、模糊等伪影，这些问题主要由相机姿态误差、覆盖不完整和几何初始化噪声引起。

Method: 提出几何感知的视频到视频生成器，渲染包含深度、法线、不透明度和协方差的Gaussian primitive视频缓冲区，通过生成器进行细化；同时引入模拟多种退化模式的伪影合成管道。

Result: 在新视角合成基准测试中达到最先进性能，高效变体在21FPS实时运行的同时保持相似性能，支持交互式3D应用。

Conclusion: GaussFusion通过几何感知视频生成有效改进3D高斯泼溅重建质量，减少伪影并保持时间一致性，为交互式3D应用提供实用解决方案。
...
TL;DR: 本文提出了首个双语文本到动作生成基准数据集BiHumanML3D，以及基于跨语言对齐的Bilingual Motion Diffusion模型，显著提升了双语和代码切换场景下的动作生成质量。
...
Method: 通过LLM辅助标注和严格人工校正构建BiHumanML3D双语数据集；提出Bilingual Motion Diffusion模型，采用Cross-Lingual Alignment策略显式对齐跨语言语义表示。

Result: BiMD模型在BiHumanML3D上取得FID 0.045 vs. 0.169，R@3 82.8% vs. 80.8%的优异表现，显著优于单语扩散模型和翻译基线，在零样本代码切换场景中表现良好。
...
Method: 开发了闭环事件-SVE测量系统，结合空间变曝光相机（产生HDR图）和立体神经形态事件相机；采用烟雾感知融合策略分离粒子发射和烟雾散射；利用HDR图作为事件相机的绝对强度参考，抑制烟雾驱动的事件伪影；基于清洗后的事件观测，通过立体事件三维管道估计分离高度和等效粒子尺寸

Result: 系统最大校准误差为0.56%；在硼基推进剂实验中获得了多模态等效半径统计；能够捕捉传统传感器难以观测的快速分离瞬态过程

Conclusion: 该框架为烟雾遮蔽高动态范围条件下的微秒级三维燃烧测量提供了一种实用且校准一致的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 多模态虚假信息日益严重，传统检测器是不透明的黑盒，且对新的操纵手法脆弱，需要可解释且能适应新威胁的检测方法

Method: 采用构建-推理范式：首先构建人类可理解的概念图（包括MLLM自动发现和验证的高层概念），然后通过层次注意力在概念图上进行推理，生成可解释的证据链

Result: PCGR在多模态虚假信息检测精度和鲁棒性方面达到SOTA，在粗粒度检测和细粒度操纵识别上都优于现有方法

Conclusion: PCGR框架通过结构化、概念化的推理方法，为多模态虚假信息检测提供了可解释且可演化的解决方案，能有效应对新兴操纵手法
...
TL;DR: 提出了一种基于SE(3)B样条运动基的高质量单目视频动态高斯溅射方法，通过自适应控制机制、软分割重建策略和多视图扩散模型来提升运动建模能力和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单目视频进行动态高斯溅射时，对连续位置和方向变形的建模能力有限，需要更高效且能处理复杂运动的方法来提升新视角合成的质量。

Method: 使用SE(3)B样条运动基配合紧凑控制点集来显式建模动态高斯的连续位置和方向变形；设计自适应控制机制动态调整运动基和控制点数量；开发软分割重建策略缓解长间隔运动干扰；采用多视图扩散模型提供多视图线索防止过拟合。

Result: 大量实验表明，该方法在新视角合成任务上优于现有最先进方法，能够生成更高质量的动态场景渲染结果。

Conclusion: 通过SE(3)B样条运动基、自适应控制、软分割重建和多视图扩散模型的综合方法，有效提升了单目视频动态高斯溅射的质量和效率，为新视角合成提供了更好的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在处理密集帧时计算成本过高，现有方法通过选择关键帧来缓解，但它们的贪婪决策和分离的相关性与多样性评估容易陷入局部最优，导致选择不相关的噪声帧

Method: 提出GIFT框架：1) 引入定向多样性来量化基于相关性的帧独特性，形成统一的不可替代性评分；2) 采用预算感知细化策略，通过自适应迭代过程先选择具有最高不可替代性的核心帧集，然后随着预算扩展优先构建这些选择周围的关键时间上下文
...
<details>
  <summary>Details</summary>
Motivation: 概念擦除是文本到图像模型的重要安全机制，但现有方法主要针对U-Net和双流架构，在单流扩散Transformer（如Z-Image）中直接应用会导致生成崩溃，需要专门的方法来解决这一空白

Method: 1. 提出流解耦概念擦除框架，将更新解耦，使现有方法能在单流模型上应用；2. 引入拉格朗日引导的自适应擦除调制，通过约束算法平衡擦除与保留的敏感权衡；3. 提供严格的收敛性分析证明

Result: Z-Erase成功克服了生成崩溃问题，在广泛的任务中实现了最先进的性能，证明了其在单流T2I模型中的有效性

Conclusion: Z-Erase是首个针对单流扩散Transformer的概念擦除方法，通过流解耦框架和拉格朗日引导调制，有效解决了单流架构中的概念擦除挑战，为这类新兴模型提供了重要的安全机制
...
TL;DR: 提出Token-Reweighting (ToR)策略，通过动态重加权感知和推理相关token来优化多模态大语言模型的强化学习训练，解决两种token类型相互依赖的问题


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型(MLLMs)的响应中同时包含感知相关token（视觉内容基础）和推理相关token（推理链构建），这两种token类型具有不同但相互依赖的能力。现有的强化学习方法难以同时优化这两种token类型，导致性能受限

Method: 提出Token-Reweighting (ToR)策略：1) 识别感知和推理相关的关键token；2) 在RLVR训练期间动态重加权这些token；3) 作为即插即用模块应用于现有方法（如GRPO和DAPO）之上

Result: ToR在多个多模态推理基准测试中带来一致的性能提升，实现了最先进的性能，同时保证了准确的视觉基础和连贯的推理

Conclusion: 通过显式建模感知和推理token之间的相互依赖关系，ToR策略有效解决了MLLMs中RLVR训练的根本挑战，为多模态推理任务提供了有效的优化方法
...
Method: 提出分层因果丢弃(HCD)：1) 使用通道级因果掩码强制特征稀疏性，分离因果特征与伪特征；2) 基于矩阵互信息(MMI)的目标函数，最小化潜在特征与领域标签的互信息，同时最大化与类别标签的共享信息；3) 引入StyleMix驱动的VICReg模块确保稳定性，防止掩码意外过滤掉重要因果数据。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在物体幻觉问题，现有方法缺乏对最终模型层注意力漂移的充分解释性，需要更深入理解视觉特征在层间的演化过程

Method: 提出CLVA（跨层视觉锚点）方法，通过分析层间视觉特征演化，发现幻觉源于深层注意力向早期层视觉噪声回归，利用中间层捕获的关键视觉锚点来强化这些特征并抑制回归噪声

Result: 在多种架构和基准测试中表现出色，显著减少物体幻觉，且计算时间和GPU内存增加不明显

Conclusion: 输出可靠性依赖于中间层而非最终层获取视觉锚点，CLVA通过跨层特征利用有效纠正深层注意力漂移，为多模态模型幻觉问题提供了新的训练免费解决方案
...
TL;DR: THEMIS是一个用于评估多模态大语言模型在学术欺诈视觉推理能力的新基准，包含4000多个问题，覆盖7个真实场景和5种欺诈类型，实验显示当前最佳模型准确率仅56.15%。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法充分评估多模态大语言模型在真实世界学术欺诈场景中的视觉推理能力，需要建立更贴近实际复杂度的评估标准。

Method: 构建包含4000多个问题的基准数据集，基于真实撤稿案例和多模态合成数据，覆盖7个学术场景、5种欺诈类型和16种细粒度操作，建立欺诈类型到5种核心能力的映射关系。

Result: 在16个领先的MLLMs上测试，最佳模型GPT-5总体准确率仅56.15%，表明该基准具有严格的评估标准，能有效揭示不同模型在视觉欺诈推理方面的具体优势和弱点。

Conclusion: THEMIS基准填补了现有评估与真实世界学术欺诈复杂性之间的差距，为MLLMs在复杂现实欺诈推理任务的发展提供了重要推动。
...
TL;DR: Pixelis是一个直接在像素空间操作的视觉语言智能体，通过可执行操作（缩放/裁剪、分割、跟踪、OCR、时序定位）学习，采用三阶段训练方法，在六个公开基准测试中平均相对提升4.08%


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言系统是被动的静态观察者，只能描述像素而不能行动，无法在环境变化下安全改进。这种被动性限制了可泛化、物理基础的视觉智能。学习需要通过行动而非静态描述来实现超越策划数据的智能

Method: 三阶段训练方法：1）监督微调学习像素工具语法，使用思维链-行动轨迹和掩码模仿损失；2）好奇心-一致性奖励微调，结合预测误差好奇心和相邻步骤一致性；3）像素测试时强化学习，通过检索邻居、投票轨迹和更新到高质量示例进行无标签适应

Result: 在六个公开图像和视频基准测试中，Pixelis相比相同8B基线平均相对提升4.08%（VSI-Bench上最高达6.03%），同时产生更短、可审计的工具链，并在测试时学习中保持KL走廊内
...
Conclusion: RL-MBA通过强化学习框架有效解决了多模态主动学习中模态重要性动态变化的问题，实现了模态平衡和难度感知的样本选择，在有限标注资源下取得了更好的性能。
...
TL;DR: 提出多阶段强化学习方法MSRL，通过文本偏好数据学习通用奖励推理能力，再逐步迁移到多模态任务，解决多模态奖励模型训练中数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于RLVR的多模态奖励模型训练依赖昂贵的标注多模态偏好数据，难以规模化扩展。需要一种能在有限多模态数据下实现可扩展强化学习的方法。
...
Conclusion: MSRL方法有效解决了多模态奖励模型训练中的数据稀缺问题，实现了可扩展的强化学习训练，显著提升了模型在视觉理解和生成任务上的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据增强方法（如基于扩散的合成或复杂特征混合）通常计算开销大或需要外部数据集，需要探索更轻量、无需外部数据的增强方案。

Method: 采用基于莫尔干涉图案的程序化增强方法，通过封闭形式的数学公式在内存中实时生成结构化扰动纹理，与训练图像混合后立即丢弃，实现零存储开销。

Result: 方法计算成本极低（每张图像0.0026秒），在ImageNet-C、ImageNet-R和对抗性基准测试中，使用Vision Transformers的实验显示鲁棒性持续提升，优于标准增强基线和现有无需外部数据的增强方法。

Conclusion: 解析干涉图案为数据驱动的生成式增强方法提供了实用高效的替代方案，证明了轻量级程序化增强在提升模型鲁棒性方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有文档生成方法面临两个主要挑战：1）人工标注的文档数据集覆盖范围有限且规模小；2）现有方法在处理多种文档生成任务时缺乏统一框架。需要开发一个能够处理广泛文档类别和风格的统一生成系统。

Method: 1）建立可扩展的数据合成管道，自动生成HTML/CSS格式文档，创建DocHTML数据集（265,206个样本，111个类别，32种风格）；2）基于该数据集微调多模态大语言模型，实现三种文档生成任务：意图到文档、文档反渲染、元素到文档；3）引入高度感知强化学习（HARL）后训练过程，通过基于预测和目标文档高度差异的奖励函数惩罚内容溢出问题。

Result: 定性和定量实验表明，AnyDoc在所有三种任务上都优于通用多模态大语言模型和特定任务基线方法。HARL有效缓解了内容溢出问题，提升了整体性能。

Conclusion: AnyDoc通过统一的HTML/CSS表示、大规模合成数据集和高度感知强化学习，成功扩展了文档生成的边界，能够处理多种文档类别和生成任务，为AI驱动的文档创建提供了有效解决方案。
...
TL;DR: AirSplat是一个训练框架，将3D视觉基础模型的几何先验适配到无姿态的新视角合成任务中，通过自一致姿态对齐和基于评分的透明度匹配提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 虽然3D视觉基础模型在视觉几何估计方面表现出强大的零样本能力，但直接应用于通用的无姿态新视角合成仍然具有挑战性。需要将3DVFMs的鲁棒几何先验有效适配到高质量、无姿态的NVS任务中。

Method: 提出了两个关键技术贡献：1）自一致姿态对齐（SCPA），一个训练时的反馈循环，确保像素对齐监督以解决姿态-几何不一致问题；2）基于评分的透明度匹配（ROM），利用稀疏视图NVS教师模型的局部3D几何一致性知识来过滤退化的图元。

Result: 在大规模基准测试上的实验结果表明，该方法在重建质量方面显著优于最先进的无姿态NVS方法。

Conclusion: AirSplat展示了将3DVFMs适配到同时进行视觉几何估计和高质量视角合成的潜力，为无姿态新视角合成提供了有效的解决方案。
...
TL;DR: DAPASS框架通过全景置信度引导去噪和上下文分辨率对抗模块，在无源数据条件下实现了全景语义分割的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 全景语义分割在自动驾驶和虚拟现实等应用中至关重要，但面临两大挑战：全景投影的几何畸变和密集标注的高成本。无监督域适应虽然可行，但实际应用中常面临更严格的无源数据约束，这加剧了域偏移问题，导致伪标签不可靠和性能下降，特别是对少数类别。

Method: 提出DAPASS框架，包含两个协同模块：1) 全景置信度引导去噪模块(PCGD)，通过强制扰动一致性和结合邻域级置信度来过滤噪声，生成高保真、类别平衡的伪标签；2) 上下文分辨率对抗模块(CRAM)，通过对抗性对齐高分辨率裁剪的细粒度细节与低分辨率上下文的全局语义，显式处理尺度变化和畸变问题。

Result: 在室外(Cityscapes-to-DensePASS)和室内(Stanford2D3D)基准测试中达到最先进性能，分别获得55.04%(+2.05%)和70.38%(+1.54%)的mIoU。

Conclusion: DAPASS框架有效解决了无源数据条件下的全景语义分割挑战，通过创新的去噪和对抗对齐策略，显著提升了跨域全景分割的性能，特别是在处理几何畸变和少数类别方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统RPCA假设稀疏前景与低秩背景是相加关系，但在许多实际应用中（如视频前景提取），稀疏前景实际上是遮挡或替换了低秩背景元素，这种不匹配需要新的方法来解决。

Method: 提出鲁棒主成分补全（RPCC）框架，通过间接确定稀疏成分的支持集来识别稀疏成分，采用完全概率贝叶斯稀疏张量分解，使用变分贝叶斯推理求解，收敛到硬分类器。

Result: 在合成数据上获得接近最优的估计，在真实彩色视频数据集上实现鲁棒的前景提取，在高光谱数据集上实现异常检测，性能优于传统RPCA方法。

Conclusion: RPCC框架有效解决了前景遮挡背景的实际问题，通过变分贝叶斯推理实现了无需阈值化的稀疏成分识别，在多个实际应用中表现出色。
...
TL;DR: 本文介绍了EgoXtreme数据集，这是一个用于第一人称视角6D物体姿态估计的大规模数据集，专门针对工业维护、体育和紧急救援等极端场景下的运动模糊、动态光照和视觉遮挡等挑战。
...
Method: 创建EgoXtreme数据集，包含三个具有挑战性的场景：工业维护、体育和紧急救援，这些场景通过极端光照、剧烈运动模糊和烟雾引入严重的感知模糊性。

Result: 评估显示现有最先进的可泛化姿态估计器在极端条件下泛化能力失效，特别是在低光照条件下。简单的图像恢复（如去模糊）在极端条件下没有积极改善，而基于跟踪的方法显示出性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态深度伪造检测主要依赖合成伪造数据进行训练，这会引入数据集和生成器偏差，限制了模型的可扩展性和对未见篡改的鲁棒性。真实视频中的视觉伪影和跨模态不一致性仍然难以检测。

Method: SAVe采用自监督学习，完全在真实视频上训练。它生成身份保持、区域感知的自混合伪操作来模拟篡改伪影，学习多粒度面部视觉线索。同时通过音频-视觉对齐组件建模唇语同步，检测音频-视觉伪造特有的时间错位模式。

Result: 在FakeAVCeleb和AV-LipSync-TIMIT数据集上的实验表明，SAVe在域内性能具有竞争力，并展现出强大的跨数据集泛化能力。

Conclusion: 自监督学习是多模态深度伪造检测的可扩展范式，能够减少对合成数据的依赖，提高检测模型的鲁棒性和泛化能力。
...
TL;DR: FD²是针对细粒度数据集蒸馏提出的专用框架，通过定位判别区域和构建细粒度表示，解决现有解耦方法在细粒度数据集上样本类内差异大、类间差异小的问题。


<details>
  <summary>Details</summary>
Motivation: 现有解耦数据集蒸馏方法主要依赖粗粒度类别标签监督，在细粒度数据集上存在两个问题：1）蒸馏样本保留大的类内差异但类间差异细微；2）同类样本变得过于相似，限制了局部判别线索并损害识别性能。

Method: FD²通过定位判别区域和构建细粒度表示进行蒸馏。在预训练阶段，使用反事实注意力学习聚合判别表示来更新类别原型；在蒸馏阶段，采用细粒度特征约束使每个样本与其类别原型对齐并排斥其他类别，同时使用相似性约束使同类样本间的注意力多样化。

Result: 在多个细粒度和通用数据集上的实验表明，FD²能够无缝集成到解耦数据集蒸馏框架中，在大多数设置下提升性能，显示出强大的可迁移性。

Conclusion: FD²是针对细粒度数据集蒸馏的有效框架，通过细粒度表示学习和约束机制解决了现有方法在细粒度任务上的局限性，提升了蒸馏样本的质量和识别性能。
...
TL;DR: 该论文提出使用排序优化替代二元DPO来提升视觉语言模型对视觉内容的忠实度，特别是在视频文本对齐任务中，通过生成有序的退化字幕链进行训练，并发现需要微调视觉编码器才能有效。


<details>
  <summary>Details</summary>
Motivation: 标准DPO的二元"赢家通吃"方法不适合视觉语言模型，因为即使某个回答不如另一个回答受欢迎，它仍可能忠实于视觉输入。Bradley-Terry DPO公式缺乏这种细微差别，过度强调获胜回答而忽视"失败"回答可能仍保持高视觉忠实度的问题。

Method: 研究排序优化作为替代方案，更精确地定位回答对视觉输入的忠实度。专注于视频文本对齐任务，使用详细视频字幕，提出通过重复字幕退化生成具有挑战性的完全有序字幕链的方法。

Result: 排序优化在长内容生成和评估方面优于二元DPO。重要的是，研究发现这些方法需要微调视觉编码器才能有效，这挑战了DPO仅作为语言重加权过程的观点。

Conclusion: 对于视觉语言模型，排序优化比二元DPO更适合处理视觉忠实度问题，并且视觉编码器的微调是实现有效对齐的关键因素，表明视觉语言对齐需要更全面的模型调整而不仅仅是语言层面的优化。
...
TL;DR: 提出了一种语义解缠的统一3D异常检测模型，通过解耦语义表示来解决跨类别纠缠问题，在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 统一模型在多类别3D异常检测中面临跨类别纠缠问题，不同类别的潜在特征重叠导致模型在重建时采用错误的语义先验，从而产生不可靠的异常分数。
...
Conclusion: 提出的语义解缠统一模型有效解决了跨类别纠缠问题，提高了统一3D异常检测的可靠性，在多个基准测试中表现出优越性能。
...
TL;DR: SportSkills是首个面向物理技能学习的大规模体育数据集，包含36万+教学视频和63万+视觉演示，涵盖55种体育项目，显著提升细粒度动作理解能力


<details>
  <summary>Details</summary>
Motivation: 现有大规模视频数据集主要关注通用人类活动，缺乏针对物理技能学习所需的细粒度活动覆盖深度，需要专门的数据集来支持技能学习

Method: 构建SportSkills数据集，包含36万+教学视频和63万+视觉演示，配以解释动作技巧的教学旁白；提出基于错误条件的教学视频检索任务，连接表征学习和可操作反馈生成

Result: SportSkills使模型能够理解物理动作间的细粒度差异，相比传统活动中心数据集训练相同模型，性能提升高达4倍；专业教练评估显示检索方法显著提升视频模型为用户查询个性化视觉指导的能力

Conclusion: SportSkills填补了物理技能学习数据集的空白，通过大规模错误条件教学视频检索任务，为个性化技能学习提供了有效工具，在细粒度动作理解和个性化指导方面取得显著进展
...
TL;DR: PointINS是一个面向实例的自监督学习框架，通过几何感知学习增强点云表示，显著提升3D实例分割和全景分割性能


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法主要关注语义感知，但在实例定位任务上迁移效果不佳，而实例感知是3D感知的基础组件，需要填补这一空白以推进真正的3D基础模型发展

Method: 提出PointINS框架，采用正交偏移分支联合学习高级语义理解和几何推理；引入两种互补的正则化策略：偏移分布正则化（ODR）和空间聚类正则化（SCR）

Result: 在五个数据集上的实验表明，PointINS在室内实例分割上平均提升3.5% mAP，在室外全景分割上平均提升4.1% PQ

Conclusion: PointINS通过几何感知学习实现了实例感知的点云表示，为可扩展的3D基础模型铺平了道路
...
TL;DR: 该研究创建了一个针对孟加拉国五种常见皮肤病的公开数据集，包含1612张图像，并应用机器学习和深度学习模型进行分类性能评估。


<details>
  <summary>Details</summary>
Motivation: 皮肤疾病是全球重要的公共卫生问题，在孟加拉国等人口密集国家，皮肤科专家和诊断设备严重不足，缺乏适当的检测和治疗可能导致严重的健康后果甚至死亡。人工智能和机器学习技术能够通过图像处理和计算机视觉技术检测皮肤疾病。

Method: 研究开发了一个公开可用的数据集，专注于五种常见皮肤病的检测：接触性皮炎、白癜风、湿疹、疥疮和癣菌病。数据集包含1612张图像（其中250张是原始图像，其余为增强图像），从孟加拉国Faridpur医学院门诊患者处直接收集。应用了多种机器学习和深度学习模型对数据集进行分类。

Result: 数据集包含302张皮炎图像、381张湿疹图像、301张疥疮图像、316张癣菌病图像和312张白癜风图像。虽然数据是区域性收集的，但这些疾病在南亚等许多国家都很常见，使该数据集在全球机器学习皮肤病学应用中具有潜在价值。

Conclusion: 该研究创建了一个有价值的皮肤病检测数据集，并展示了机器学习模型在皮肤病分类中的应用。期望这项研究能够引起从事自动化疾病诊断的机器学习和深度学习研究人员及从业者的关注。
...
TL;DR: ET-SAM是一个基于SAM的高效统一场景文本检测和布局分析框架，通过轻量级点解码器减少前景点数量，实现3倍推理加速，并利用异构文本级标注数据进行联合训练。


<details>
  <summary>Details</summary>
Motivation: 现有基于SAM的方法依赖像素级文本分割来采样数千个前景点作为提示，导致推理延迟不理想且数据利用率有限。需要解决推理效率低和标注数据异构的问题。

Method: 1. 定制轻量级点解码器生成词热图，仅需少量前景点，消除过多点提示并加速推理；2. 设计联合训练策略，利用多级、词级和行级标注的异构数据集；3. 在点解码器和分层掩码解码器中引入三组可学习任务提示，缓解数据集间的差异。

Result: 相比之前基于SAM的架构，ET-SAM实现了约3倍的推理加速，在HierText上获得竞争性性能，在Total-Text、CTW1500和ICDAR15上平均F-score提升11.0%。

Conclusion: ET-SAM通过高效的点解码器和联合训练策略，成功解决了SAM-based方法在推理延迟和数据利用方面的限制，实现了快速且准确的统一场景文本检测和布局分析。
...
TL;DR: 该论文提出了一种基于红外物理知识的对抗训练方法（KGAT），通过建模不同类别间的热辐射关系来提升红外目标检测的鲁棒性，有效提高了对抗攻击和常见干扰下的检测性能。


<details>
  <summary>Details</summary>
Motivation: 红外目标检测在复杂环境中具有广泛应用，但对对抗样本和常见干扰很脆弱，存在安全隐患。现有方法主要基于数据驱动，没有充分考虑红外图像的特性，导致鲁棒性有限。

Method: 重新审视红外物理知识，发现不同类别间的相对热辐射关系在对抗样本和干扰下是可靠的知识源。基于灰度值排序理论建模热辐射关系，量化各类间关系的稳定性，提出知识引导的对抗训练（KGAT），将红外物理知识嵌入对抗训练过程，优化预测结果使其符合实际物理规律。

Result: 在三个红外数据集和六个主流红外目标检测模型上的大量实验表明，KGAT有效提升了干净数据的准确率，并增强了对抗攻击和常见干扰下的鲁棒性。

Conclusion: 通过将红外物理知识融入对抗训练，KGAT方法显著提升了红外目标检测的鲁棒性，为解决红外检测在复杂环境中的安全问题提供了有效方案。
...
TL;DR: TITAnD将轨迹异常检测重新定义为视觉问题，通过将轨迹表示为超光谱轨迹图像，使用循环因子化Transformer建模，首次实现密集多个月轨迹的异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：密集GPS方法成本高无法处理多个月数据，稀疏停留点方法丢弃了精细证据，导致两种模式需要分开架构且无法知识迁移。作者认为人类轨迹具有自然的二维循环结构，可以统一表示。

Method: 提出TITAnD框架：1) 将轨迹表示为超光谱轨迹图像（HTI），即日×时网格，通道编码空间、语义、时间和运动信息；2) 引入循环因子化Transformer（CFT），沿两个时间轴分解注意力，编码人类日常的循环归纳偏置，大幅降低计算成本。

Result: TITAnD在稀疏和密集基准测试中都取得了最佳的AUC-PR，超越UNet等视觉模型，同时比Transformer快11-75倍且内存相当，首次实现了密集多个月轨迹的异常检测。

Conclusion: 将轨迹异常检测重新定义为视觉问题并结合结构感知建模是解决该问题的关键，TITAnD统一了密集和稀疏轨迹的表示，实现了可扩展的多个月异常检测。
...
### [73] [VolDiT: Controllable Volumetric Medical Image Synthesis with Diffusion Transformers](https://arxiv.org/abs/2603.25181)
*Marvin Seyfarth,Salman Ul Hassan Dar,Yannik Frisch,Philipp Wild,Norbert Frey,Florian André,Sandy Engelhardt*

Main category: cs.CV

TL;DR: VolDiT：首个基于纯Transformer的3D扩散模型，用于医学图像合成，通过体积块嵌入和全局自注意力直接处理3D数据，并引入时间步门控控制适配器实现精确空间控制。


<details>
  <summary>Details</summary>
Motivation: 现有3D医学图像生成方法主要依赖卷积U-Net架构，这些架构存在局部性偏置和有限感受野的问题，限制了可扩展性、全局上下文整合和灵活的条件控制能力。

Method: 提出VolDiT模型，将扩散Transformer扩展到原生3D数据：1）使用体积块嵌入将3D数据转换为token序列；2）通过全局自注意力直接处理3D token；3）引入时间步门控控制适配器，将分割掩码映射为可学习的控制token，在去噪过程中调制Transformer层。

Result: 在高分辨率3D医学图像合成任务中，相比基于U-Net的3D潜在扩散模型，VolDiT展现出改进的全局一致性、更优的生成保真度和增强的可控性。

Conclusion: 完全基于Transformer的扩散模型为体积医学图像合成提供了灵活的基础框架，在全局建模和可控生成方面具有优势。
...
<details>
  <summary>Details</summary>
Motivation: 安全关键环境中的目标检测器可能无声地失败（如漏检行人、工人等），传统OOD检测方法只关注识别陌生输入，无法直接预测检测器本身的功能故障。

Method: 使用双编码器架构和角度距离度量，测量目标检测器内部特征与视觉基础模型嵌入之间的语义错位。当检测器超出其能力范围或视觉基础模型遇到新输入时，两个嵌入会发散，产生高角度信号来标记不安全图像。

Result: 在COCO行人检测任务中，KGFP作为选择性预测门将接受图像中的行人召回率从64.3%提高到84.5%（5% FPR下），在六个COCO-O视觉域中保持强劲性能，大幅优于OOD基线方法。

Conclusion: KGFP通过监测目标检测器与视觉基础模型之间的语义一致性，能够有效预测检测器的功能故障，为安全关键应用提供了可靠的运行时监控解决方案。
...
TL;DR: AnyID是一个超保真身份保持视频生成框架，通过多参考统一架构和主参考生成范式，解决了现有方法只能处理单一身份参考的限制，实现了高保真身份保持和精确属性级控制。


<details>
  <summary>Details</summary>
Motivation: 现有身份保持视频生成方法通常只针对单一身份参考进行设计和优化，这限制了创意灵活性，无法适应多样化的真实世界输入格式。单一参考源也构成了一个不适定场景，导致模型难以在新颖上下文中忠实地再现身份。
...
Conclusion: AnyID通过其全参考架构和主参考生成范式，解决了现有身份保持视频生成方法的局限性，为创意表达提供了更强大、更灵活的工具。
...
TL;DR: 本文分析了自动驾驶中高光谱成像技术的应用挑战、技术选择和算法开发，基于HSI-Drive数据集实验结果探讨了相关研究进展。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶领域应用高光谱成像技术面临多重挑战：非受控的变光照条件、大景深范围、动态场景中的快速移动物体，以及实时性要求和嵌入式平台有限的计算资源。这些因素共同决定了技术选择和算法开发的需求。

Method: 分析自动驾驶中基于高光谱成像的视觉系统研究技术，使用最新版HSI-Drive数据集的实验结果作为示例，探讨如何结合传感器的光谱和空间信息开发定制视觉算法。

Result: 通过HSI-Drive数据集的实验分析，展示了高光谱成像技术在自动驾驶应用中的研究进展，包括针对特定挑战的技术选择和算法开发成果。

Conclusion: 高光谱成像在自动驾驶中具有应用潜力，但需要针对该领域的特定挑战进行技术选择和算法优化，以充分利用传感器的光谱和空间信息，满足实时性和计算资源限制的要求。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理电影心脏MRI（CMR）这种具有时间维度的3D医学图像时，通常通过空间和时间分解或使用解剖掩码等辅助机制来强制时间一致性。这些策略引入了结构偏差，可能限制全局上下文整合，导致微妙的时空不连续性或生理上不一致的心脏动力学。

Method: 提出CardioDiT，一个完全4D的潜在扩散框架：1）使用时空VQ-VAE将2D+t切片编码为紧凑的潜在表示；2）使用扩散变换器将这些潜在表示联合建模为完整的3D+t体积，在整个生成过程中耦合空间和时间维度。

Result: 在公共CMR数据集和更大的私有队列上评估，与具有渐进式更强时空耦合的基线相比，CardioDiT显示出改进的切片间一致性、时间相干运动和真实的心脏功能分布。
...
<details>
  <summary>Details</summary>
Motivation: 当前足球模仿研究主要关注优化基于奖励的目标（如进球数或胜率），而较少关注准确复制真实世界的球队战术行为。需要建立能够评估战术风格模仿的基准。

Method: TacSIm通过英超比赛转播镜头，在单一转播视角下模仿一支球队11名球员的动作。将攻防转播镜头中22名球员的起始位置和动作投影到标准球场坐标系中，提供明确的风格模仿任务和评估协议。

Result: 使用空间占用相似性和运动向量相似性来衡量战术风格模仿，支持评估球队的空间和时间相似性。在统一虚拟环境中运行多个基线方法生成全队行为，实现定量和视觉评估战术协调。

Conclusion: 通过使用从转播到模拟的统一数据和指标，TacSIm为测量和建模足球中风格对齐的战术模仿任务建立了严格的基准。
...
TL;DR: 该论文提出了两种层次感知的多模态学习方法CLiBD-HiR和CLiBD-HiR-Fuse，用于生物多样性分类任务，通过编码生物分类的层次结构来提高分类准确性和噪声鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有生物多样性识别方法通常将分类学视为扁平标签空间，未能编码生物分类的层次结构，这在噪声和模态缺失条件下影响鲁棒性。需要开发能够编码层次结构的多模态学习方法。

Method: 提出了两种端到端方法：1) CLiBD-HiR引入层次信息正则化(Hierarchical Information Regularization)来塑造跨分类级别的嵌入几何，产生结构化且鲁棒的表示；2) CLiBD-HiR-Fuse额外训练轻量级融合预测器，支持图像、DNA或联合推理，对模态损坏具有弹性。

Result: 在大规模生物多样性基准测试中，相比强大的多模态基线方法，该方法将分类学分类准确率提高了超过14%，在部分和损坏的DNA条件下获得特别大的增益。

Conclusion: 明确编码生物层次结构并结合灵活融合是构建实用生物多样性基础模型的关键，该方法为生态学、保护和环境监测提供了更可靠的技术基础。
...
### [86] [FEAST: Fully Connected Expressive Attention for Spatial Transcriptomics](https://arxiv.org/abs/2603.25247)
...
### [88] [Efficient Preemptive Robustification with Image Sharpening](https://arxiv.org/abs/2603.25244)
*Jiaming Liang,Chi-Man Pun*
...
### [89] [Image Rotation Angle Estimation: Comparing Circular-Aware Methods](https://arxiv.org/abs/2603.25351)
*Maximilian Woehrer*
...
### [91] [ViewSplat: View-Adaptive Dynamic Gaussian Splatting for Feed-Forward Synthesis](https://arxiv.org/abs/2603.25265)
...
### [97] [DeepFAN, a transformer-based deep learning model for human-artificial intelligence collaborative assessment of incidental pulmonary nodules in CT scans: a multi-reader, multi-case trial](https://arxiv.org/abs/2603.25607)
...
### [99] [Demographic Fairness in Multimodal LLMs: A Benchmark of Gender and Ethnicity Bias in Face Verification](https://arxiv.org/abs/2603.25613)
...
### [102] [Just Zoom In: Cross-View Geo-Localization via Autoregressive Zooming](https://arxiv.org/abs/2603.25686)
...
### [106] [Out of Sight but Not Out of Mind: Hybrid Memory for Dynamic Video World Models](https://arxiv.org/abs/2603.25716)
...
### [113] [Vega: Learning to Drive with Natural Language Instructions](https://arxiv.org/abs/2603.25741)
...
### [114] [CIAR: Interval-based Collaborative Decoding for Image Generation Acceleration](https://arxiv.org/abs/2603.25463)
...
### [121] [Towards Comprehensive Real-Time Scene Understanding in Ophthalmic Surgery through Multimodal Image Fusion](https://arxiv.org/abs/2603.25555)
...
### [122] [GeoHeight-Bench: Towards Height-Aware Multimodal Reasoning in Remote Sensing](https://arxiv.org/abs/2603.25565)
...
### [123] [UNIC: Neural Garment Deformation Field for Real-time Clothed Character Animation](https://arxiv.org/abs/2603.25580)
...
### [124] [Designing Any Imaging System from Natural Language: Agent-Constrained Composition over a Finite Primitive Basis](https://arxiv.org/abs/2603.25636)
*Chengshuai Yang*
...
### [128] [Seeing to Ground: Visual Attention for Hallucination-Resilient MDLLMs](https://arxiv.org/abs/2603.25711)
...
### [129] [AnyHand: A Large-Scale Synthetic Dataset for RGB(-D) Hand Pose Estimation](https://arxiv.org/abs/2603.25726)
...
### [130] [BizGenEval: A Systematic Benchmark for Commercial Visual Content Generation](https://arxiv.org/abs/2603.25732)
...
### [131] [SlotVTG: Object-Centric Adapter for Generalizable Video Temporal Grounding](https://arxiv.org/abs/2603.25733)
...
### [132] [Unleashing Guidance Without Classifiers for Human-Object Interaction Animation](https://arxiv.org/abs/2603.25734)
...
### [133] [How good was my shot? Quantifying Player Skill Level in Table Tennis](https://arxiv.org/abs/2603.25736)
...
### [134] [PSDesigner: Automated Graphic Design with a Human-Like Creative Workflow](https://arxiv.org/abs/2603.25738)
...
### [135] [MegaFlow: Zero-Shot Large Displacement Optical Flow](https://arxiv.org/abs/2603.25739)
...
### [136] [RefAlign: Representation Alignment for Reference-to-Video Generation](https://arxiv.org/abs/2603.25743)
...
### [137] [MuRF: Unlocking the Multi-Scale Potential of Vision Foundation Models](https://arxiv.org/abs/2603.25744)
...
### [138] [Less Gaussians, Texture More: 4K Feed-Forward Textured Splatting](https://arxiv.org/abs/2603.25745)
...
### [139] [ShotStream: Streaming Multi-Shot Video Generation for Interactive Storytelling](https://arxiv.org/abs/2603.25746)
...
### [140] [When Consistency Becomes Bias: Interviewer Effects in Semi-Structured Clinical Interviews](https://arxiv.org/abs/2603.24651)
...
TL;DR: 研究发现抑郁症自动检测模型存在系统性偏差：模型通过利用访谈者固定提示语和位置来区分抑郁与非抑郁被试，而非真正分析参与者的语言特征


<details>
  <summary>Details</summary>
Motivation: 尽管基于医患对话的自动抑郁症检测研究取得进展，但模型可解释性有限，无法揭示预测的真正驱动因素。研究者希望分析模型是否真正学习到参与者的语言特征，还是利用了访谈协议中的系统性偏差

Method: 分析三个数据集（ANDROIDS、DAIC-WOZ、E-DAIC），识别半结构化访谈中的系统性偏差。通过比较模型在完整对话与仅参与者话语上的表现，评估模型是否真正利用参与者的语言特征

Result: 模型在访谈者话语上表现出系统性偏差，能够利用固定提示语和位置区分抑郁与控制组被试，甚至在不使用参与者语言的情况下获得高分类分数。限制模型仅使用参与者话语后，决策证据分布更广泛，反映了真实的语言线索

Conclusion: 半结构化访谈协议虽然确保了一致性，但包含访谈者提示会通过利用脚本人工痕迹而夸大模型性能。研究强调了跨数据集、架构无关的偏差，并强调需要通过时间和说话者定位决策证据的分析，确保模型真正从参与者语言中学习
...
TL;DR: 通过微调12亿参数的开源大语言模型，显著提升了系统综述文献筛选的效率和准确性，相比基础模型F1分数提升80.79%，与人工编码员的一致性达到86.40%。


<details>
  <summary>Details</summary>
Motivation: 传统系统综述需要大量人工筛选文献标题和摘要，耗时耗力。虽然已有研究探索使用大语言模型提高效率，但仅依赖提示工程效果不稳定，需要更有效的方法。

Method: 针对一个包含8500多篇文献标题和摘要人工标注的系统综述，微调了一个12亿参数的开源大语言模型，专门用于文献筛选任务。

Result: 微调后模型性能显著提升：加权F1分数相比基础模型提高80.79%；在8277篇文献的完整数据集上，与人工编码员的一致性达到86.40%，真阳性率91.18%，真阴性率86.38%，且多次推理结果完全一致。

Conclusion: 微调大语言模型在系统综述的标题和摘要筛选任务中具有巨大潜力，能够显著提高大规模系统综述的效率和准确性。
...
TL;DR: 本研究通过微调LLaMA 3.1-8B模型，探索了针对芬兰语医疗转录的自然语言处理方案，结果显示模型在语义相似度方面表现良好，为低资源语言的临床文档自动化提供了可行路径。


<details>
  <summary>Details</summary>
Motivation: 临床文档对患者安全、诊断和连续性护理至关重要，但电子健康记录的管理负担是医生职业倦怠的重要因素。对于芬兰语等低资源语言，这一问题尤为突出，需要有效的自动化解决方案来减轻医疗工作者的文档负担。

Method: 研究采用领域对齐的自然语言处理方法，在Metropolia应用科学大学学生模拟的临床对话验证语料库上微调LLaMA 3.1-8B模型。使用受控的预处理和优化方法进行微调，并通过七折交叉验证评估效果。

Result: 微调后的LLaMA 3.1-8B模型评估指标为：BLEU=0.1214，ROUGE-L=0.4982，BERTScore F1=0.8230。结果显示n-gram重叠度较低，但与参考转录文本的语义相似度很强。

Conclusion: 微调是处理芬兰语医疗话语转录的有效方法，支持了为芬兰语临床文档开发隐私导向的领域特定大型语言模型的可行性。研究为未来工作提供了方向，包括进一步优化模型性能和扩展应用场景。
...
TL;DR: 本文介绍了首个英语句子UMR（统一意义表示）体标记数据集，用于在缺乏体信息的AMR图上标注体特征，并建立了自动预测的基准模型。


<details>
  <summary>Details</summary>
Motivation: 体（aspect）对于完整捕捉句子意义至关重要，它能描述事件内部的时间结构。然而，在语义表示框架中，体标注仍然稀疏，这既阻碍了人工标注，也限制了自动预测系统的发展。

Method: 创建了英语句子的UMR体标注数据集，基于缺乏体特征的AMR图。设计了标注方案和指南，按照UMR体格标注事件性谓词。采用多步骤裁决流程确保标注者间的一致性和质量。使用三种建模方法进行基线实验。

Result: 建立了首个UMR体标注数据集，为自动UMR体预测提供了初始基准。实验结果展示了数据集对未来自动化研究的实用性，为更广泛地将体信息整合到语义表示中奠定了基础。

Conclusion: 本文填补了语义表示中体标注的空白，创建的数据集和基准实验为自动体预测系统的发展提供了重要资源，有助于推动体信息在语义表示中的更广泛应用。
...
TL;DR: 研究探讨了在葡萄牙语持续预训练中，合成重写与源数据质量的相互作用，发现重写高质量数据能显著提升模型性能，而重写低质量数据效果有限，且这种效应与模型规模相关。


<details>
  <summary>Details</summary>
Motivation: 当前合成数据生成研究主要集中在英语领域，且缺乏对源数据质量的系统控制。本研究旨在探索在葡萄牙语持续预训练中，文档重写技术如何与不同质量的源数据相互作用，为数据质量与合成数据生成的关系提供实证依据。

Method: 使用ClassiCC-PT葡萄牙语语料库（标注有STEM和教育质量分数），构建两个不同质量水平的10B词元子集，使用7B指令调优模型将每个子集重写为四种风格，每个条件产生约40B词元的合成数据。在两个英语中心基础模型（1.1B和7B参数）上训练，并在PoETa V2（包含44个任务的葡萄牙语基准测试）上评估。

Result: 在7B规模下，重写高质量数据比未修改的相同数据带来+3.4 NPM增益，而重写低质量数据仅提供+0.5 NPM增益。在1.1B规模下，这种相互作用较弱，未修改的低质量数据与重写的高质量数据表现相当。结果表明合成重写主要作为质量乘数而非数据策展的替代品。

Conclusion: 合成重写主要作为数据质量的乘数效应，而非数据策展的替代方案。高质量源数据的重写能显著提升模型性能，而低质量数据的重写效果有限。这种效应具有规模依赖性，在较大模型上更为明显。研究强调了在合成数据生成中源数据质量控制的重要性。
...
TL;DR: ARRoL是一种在线剪枝方法，通过训练轻量级质量头预测部分生成的成功概率，在生成过程中剪枝低质量rollout，同时引导剩余rollout更平衡以增强学习信号，显著提升RLVR方法的训练效率和效果。
...
Method: ARRoL提出在线rollout剪枝方法，训练轻量级质量头实时预测部分rollout的成功概率，用于早期剪枝决策。系统设计在推理引擎内部剪枝rollout，并重新批处理剩余rollout进行对数概率计算和策略更新。质量头还可用于测试时扩展中加权候选以提高推理准确性。

Result: 在Qwen-3和LLaMA-3.2模型（1B-8B）上，ARRoL在GRPO和DAPO上平均准确率提升+2.30到+2.99，训练速度提升最高1.7倍，测试时扩展中额外获得最高+8.33的平均准确率提升。

Conclusion: ARRoL通过在线rollout剪枝有效解决了RLVR方法的计算效率和学习信号问题，在保持或提升准确率的同时显著加速训练，并在测试时扩展中带来额外性能增益。
...
TL;DR: LogSigma系统在SemEval-2026 Task 3中采用学习同方差不确定性方法，通过自动平衡不同语言和领域的Valence和Arousal预测任务，在五个数据集上获得第一名。


<details>
  <summary>Details</summary>
Motivation: 传统ABSA预测离散情感标签，而DimABSA需要预测1-9尺度的连续Valence和Arousal分数。主要挑战在于不同语言和领域中Valence和Arousal的预测难度存在差异。
...
Conclusion: 学习同方差不确定性方法能有效处理DimABSA中不同语言和领域间Valence-Arousal难度分布的差异，实现自动任务平衡，在跨语言情感分析中表现出色。
...
### [147] [Adaptive Chunking: Optimizing Chunking-Method Selection for RAG](https://arxiv.org/abs/2603.25333)
*Paulo Roberto de Moura Júnior,Jean Lelong,Annabelle Blangero*

Main category: cs.CL

TL;DR: 论文提出了自适应分块框架，通过五个内在指标评估文档分块质量，并引入新的分块器和后处理技术，显著提升RAG系统性能。


<details>
  <summary>Details</summary>
Motivation: 当前RAG系统的效果高度依赖于文档分块策略，但常用的"一刀切"方法无法捕捉多样化文本的细微结构和语义。分块策略缺乏专门的评估框架，难以独立于下游任务评估和比较不同策略。

Method: 提出自适应分块框架，基于五个新颖的内在文档指标（引用完整性、块内内聚性、文档上下文连贯性、块完整性、大小合规性）为每个文档选择最合适的分块策略。引入两种新分块器（LLM正则表达式分割器和分割-合并递归分割器）以及针对性的后处理技术。

Result: 在法律、技术和社会科学领域的多样化语料库上，基于指标引导的自适应方法显著提升了下游RAG性能。在不改变模型或提示的情况下，将答案正确率从62-64%提高到72%，成功回答的问题数量增加30%以上（65 vs 49）。

Conclusion: 基于互补的内在指标引导的自适应、文档感知分块方法，为构建更稳健的RAG系统提供了实用有效的途径。
...
TL;DR: 该论文研究了机器翻译和质量评估在专业领域的适应问题，通过数据选择、训练流程优化、词汇对齐和上下文学习等方法，提升领域特定场景下的性能表现。


<details>
  <summary>Details</summary>
Motivation: 机器翻译和质量评估系统在通用领域表现良好，但在专业领域存在性能下降问题。需要研究如何通过领域适应方法提升这些系统在特定领域的表现。

Method: 1. 基于相似性的数据选择方法用于机器翻译；2. 分阶段质量评估训练流程，结合领域适应和轻量级数据增强；3. 研究子词分词和词汇在微调中的作用；4. 提出质量评估引导的上下文学习方法。

Result: 1. 小规模针对性领域子集优于大规模通用数据集；2. 质量评估方法在跨领域、跨语言和资源受限场景中表现提升；3. 对齐的分词-词汇设置带来稳定训练和更好翻译质量；4. 质量评估引导的上下文学习方法无需参数更新即可提升翻译质量。

Conclusion: 领域适应依赖于数据选择、表示和高效适应策略。该研究为构建在领域特定环境中可靠运行的机器翻译和质量评估系统提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展模糊了人类撰写和AI生成文本的界限，带来了错误信息传播、作者身份模糊和知识产权威胁等社会风险。现有免训练方法通常假设令牌贡献均匀，在短序列或局部令牌修改下鲁棒性不足。

Method: Exons-Detect采用外显子感知令牌重加权方法，通过双模型设置下测量隐藏状态差异来识别和放大信息性外显子令牌，从重要性加权的令牌序列计算可解释的翻译分数。

Result: 经验评估表明，Exons-Detect实现了最先进的检测性能，在对抗攻击和不同输入长度下表现出强大的鲁棒性，在DetectRL上比先前最强基线平均AUROC相对提高了2.2%。
...
TL;DR: 研究发现大语言模型口头表达的置信度与实际准确性脱节，两者在模型内部呈线性正交关系，推理过程会污染置信度表达，通过自适应引导可改善校准


<details>
  <summary>Details</summary>
Motivation: 大语言模型口头表达的置信度分数往往与实际准确性脱节，但控制这种行为的几何关系仍不清楚，需要从机制可解释性角度分析置信度表达的内部表征

Method: 使用线性探针和对比激活加法引导技术，分析三个开源模型在四个数据集上的表现；提出两阶段自适应引导管道，读取模型内部准确性估计并引导口头输出与之匹配

Result: 发现校准信号和口头置信度信号在模型内部线性编码但相互正交；推理过程会污染置信度方向，加剧校准错误；自适应引导显著改善了所有评估模型的校准对齐

Conclusion: 口头置信度与实际准确性在模型内部表征中正交分离，推理过程会污染置信度表达，通过读取内部准确性估计并自适应引导输出可以有效改善模型的校准性能
...
### [151] [Do LLMs Know What They Know? Measuring Metacognitive Efficiency with Signal Detection Theory](https://arxiv.org/abs/2603.25112)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: 该研究提出基于Type-2信号检测理论的评估框架，将LLM的置信度评估分解为知识水平（Type-1敏感性）和元认知能力（Type-2敏感性），使用meta-d'和M-ratio指标，发现不同模型在元认知效率上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 传统LLM置信度评估方法（如ECE、Brier分数）混淆了两个不同能力：模型知道多少（Type-1敏感性）和模型知道它知道什么（Type-2元认知敏感性）。需要新的评估框架来区分这两种能力。

Method: 引入基于Type-2信号检测理论的评估框架，使用meta-d'和元认知效率比M-ratio指标。在四个LLM模型（Llama-3-8B-Instruct, Mistral-7B-Instruct-v0.3, Llama-3-8B-Base, Gemma-2-9B-Instruct）上进行224,000个事实性问答试验评估。

Result: 1. 元认知效率在不同模型间差异显著，即使Type-1敏感性相似；2. 元认知效率具有领域特异性；3. 温度调节改变Type-2标准而meta-d'保持稳定；4. AUROC_2和M-ratio产生完全相反的模型排名。

Conclusion: meta-d'框架揭示了哪些模型真正"知道它们不知道什么"，而哪些仅仅因为标准设置而显得校准良好。这对模型选择、部署和人机协作有直接意义。
...
### [152] [To Write or to Automate Linguistic Prompts, That Is the Question](https://arxiv.org/abs/2603.25169)
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索自动提示优化是否能替代专家提示工程在语言任务中的应用，填补这一领域系统性比较研究的空白。

Method: 采用系统性比较方法，评估了三种提示设计方法：手工设计的零样本专家提示、基础DSPy签名和GEPA优化的DSPy签名，在翻译、术语插入和语言质量评估三个任务中测试了五种模型配置。

Result: 结果显示任务依赖性：在术语插入任务中，优化提示和手工提示质量基本无统计差异；在翻译任务中，不同方法在不同模型上表现各异；在语言质量评估中，专家提示在错误检测方面更强，而优化提示在错误描述方面更好；GEPA显著提升了基础DSPy签名的性能。

Conclusion: 自动提示优化在某些任务中可以接近专家提示工程的效果，但两者存在不对称性：GEPA优化需要标注数据进行程序化搜索，而专家提示主要依赖领域知识和迭代优化，不需要标注数据。
...
### [153] [Prompt Attack Detection with LLM-as-a-Judge and Mixture-of-Models](https://arxiv.org/abs/2603.25176)
...
<details>
  <summary>Details</summary>
Motivation: 提示攻击（越狱和提示注入）对LLM系统构成严重安全威胁。现有防护方案存在部署差距：轻量级分类器和基于规则的系统难以适应分布变化，而基于LLM的高容量判断器又太慢或成本太高，无法满足实时生产环境要求。

Method: 通过精心设计的提示和输出格式，引导轻量级LLM进行结构化推理，包括明确的意图分解、安全信号验证、危害评估和自我反思。同时评估了混合模型（MoM）设置，即聚合多个LLM判断器是否比单一模型有更好的检测性能。

Result: 实验表明，通用轻量级LLM（如gemini-2.0-flash-lite-001）可以作为有效的低延迟判断器用于实时护栏。该方法已在新加坡公共服务聊天机器人中作为集中式护栏服务部署。混合模型设置仅带来适度的性能提升。

Conclusion: 轻量级通用LLM通过结构化推理流程能够可靠地作为安全判断器，满足生产环境的实时约束，填补了现有防护方案的部署差距。
...
### [154] [Cross-Preference Learning for Sentence-Level and Context-Aware Machine Translation](https://arxiv.org/abs/2603.25183)
...
<details>
  <summary>Details</summary>
Motivation: 当前上下文感知机器翻译虽然利用文档级信息，但表现不稳定，因为上下文信号对不同句子的益处不均。现有训练目标没有显式建模这种变异性，限制了模型自适应利用上下文的能力。

Method: 提出Cross-Preference Learning (CPL)偏好训练框架，整合句内偏好（同一输入条件下不同翻译质量比较）和跨条件偏好（不同输入条件下翻译质量比较），为偏好优化目标提供显式监督。

Result: 在多个公开上下文感知MT任务上验证，使用Qwen3-4B、Qwen3-8B和Llama-3-8B等模型，实验结果显示翻译质量和鲁棒性均有持续提升，且无需架构修改。

Conclusion: CPL框架通过显式建模句子级和上下文感知翻译的互补优势，有效提升了机器翻译的性能和适应性，为上下文感知MT提供了新的训练范式。
...
### [155] [Probing the Lack of Stable Internal Beliefs in LLMs](https://arxiv.org/abs/2603.25187)
*Yifan Luo,Kangping Xu,Yanzhen Lu,Yang Yuan,Andrew Chi-Chih Yao*

Main category: cs.CL

TL;DR: LLMs在角色驱动应用中难以保持隐式一致性，即在多轮对话中坚持未明说的目标，这限制了它们模拟人类人格特质的能力。


<details>
  <summary>Details</summary>
Motivation: 角色驱动的大型语言模型需要一致的跨交互行为倾向来模拟人类人格特质，但现有LLMs缺乏稳定的内部表征来锚定其在扩展对话中的响应。

Method: 设计了20个问题风格的谜语游戏范式，让LLM秘密选择一个目标，并对用户的猜测回答"是/否"。通过评估LLM在多轮交互中保持隐式一致性的能力。

Result: LLMs难以保持隐式一致性：除非在上下文中明确提供其选择的目标，否则它们的隐式"目标"会在不同轮次间发生变化。

Conclusion: 这些发现突显了构建角色驱动LLMs的关键限制，并强调了需要机制来锚定随时间变化的隐式目标，这是交互应用（如对话系统）中实现真实人格建模的关键。
...
### [156] [A Catalog of Basque Dialectal Resources: Online Collections and Standard-to-Dialectal Adaptations](https://arxiv.org/abs/2603.25189)
*Jaione Bengoetxea,Itziar Gonzalez-Dios,Rodrigo Agerri*

Main category: cs.CL

TL;DR: 本文介绍了巴斯克语方言数据的系统化编目，包含在线原生方言数据和标准语到方言的适配数据，旨在解决方言NLP中的数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 方言NLP研究中数据稀缺是主要限制因素，需要系统化收集和整理巴斯克语方言数据资源。

Method: 区分两类数据源：1) 在线原生方言数据（新闻、推文、词典、地图等）；2) 标准语到方言的适配数据，包括手动适配XNLI测试集到三种方言，以及自动适配物理常识数据集并进行人工评估。

Result: 创建了巴斯克语方言数据资源目录，生成了高质量的并行评估数据集（XNLI方言适配），评估了自动适配数据的质量以确定其作为"白银数据"的可行性。

Conclusion: 该研究为巴斯克语方言NLP提供了系统化的数据资源基础，通过手动和自动适配方法缓解了数据稀缺问题，为后续研究奠定了基础。
...
### [157] [A Decade-Scale Benchmark Evaluating LLMs' Clinical Practice Guidelines Detection and Adherence in Multi-turn Conversations](https://arxiv.org/abs/2603.25196)
*Andong Tan,Shuyu Dai,Jinglu Wang,Fengtao Zhou,Yan Lu,Xi Wang,Yingcong Chen,Can Yang,Shujie Liu,Hao Chen*

Main category: cs.CL

TL;DR: CPGBench：首个系统评估大语言模型在多轮对话中临床指南检测与遵循能力的自动化框架，发现模型知道指南内容但难以正确引用来源，且遵循率存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 临床实践指南在循证决策和改善患者结局中起关键作用，但大语言模型在医疗场景中部署时，其在对话中识别和遵循临床指南的能力尚不明确，需要系统评估。

Method: 收集来自9个国家/地区和2个国际组织、涵盖24个专科的3,418份临床指南文件，提取32,155条临床推荐，为每条推荐生成多轮对话，评估8个主流大语言模型的检测和遵循能力。

Result: 模型能正确检测71.1%-89.6%的推荐，但仅能正确引用3.6%-29.7%的对应标题；遵循率在21.8%-63.2%之间，表明知道指南内容与实际应用之间存在巨大差距。

Conclusion: CPGBench首次系统揭示了大语言模型在对话中未能检测或遵循的临床推荐，鉴于临床应用的安全关键性，解决这些差距对于大语言模型在真实临床实践中的安全负责任部署至关重要。
...
### [158] [Comparing Natural and Synthetic Structured Data: A Study of the Passive Verb Alternation in French and Italian](https://arxiv.org/abs/2603.25227)
*Giuseppe Samo,Paola Merlo*

Main category: cs.CL

TL;DR: 研究比较自然数据和合成数据对训练和评估大语言模型的影响，以法语和意大利语被动动词交替为例，发现自然数据训练效果更好


<details>
  <summary>Details</summary>
Motivation: 探讨自然数据和合成数据在训练和评估大语言模型中的不同影响，特别是在语言学知识获取方面的差异

Method: 使用Blackbird Language Matrices结构化数据集，比较从Universal Dependencies提取的自然句子和合成句子的模板，在法语和意大利语被动动词交替任务上进行实验

Result: 在合成数据上训练和测试的模型能达到天花板性能，但无法可靠泛化到自然句子；而在自然数据上训练的模型在自然和合成测试集上都表现稳健

Conclusion: 自然数据在捕捉抽象语言学模式方面更优越，结构化设置对于探测大语言模型的句法和语义知识具有重要价值
...
### [159] [CRAFT: Grounded Multi-Agent Coordination Under Partial Information](https://arxiv.org/abs/2603.25268)
...
<details>
  <summary>Details</summary>
Motivation: 当前需要评估大语言模型在多智能体协调中的语用通信能力，特别是在严格部分信息设置下，智能体必须通过自然语言协调构建共享理解。

Method: 将问题形式化为多发送者语用推理任务，提供诊断框架将失败分解为空间基础、信念建模和语用通信错误，包括前沿和开源模型的行为失败分类。

Result: 更强的推理能力并不总能转化为更好的协调能力：较小的开源模型经常匹配或超越前沿系统，改进的个体通信也不能保证成功的协作。

Conclusion: 多智能体协调仍然是当前语言模型尚未根本解决的挑战，需要专门的评估基准来诊断和改善模型在这方面的能力。
...
### [160] [Separate Before You Compress: The WWHO Tokenization Architecture](https://arxiv.org/abs/2603.25309)
*Kusal Darshana*

Main category: cs.CL

TL;DR: 提出SGPE算法和WWHO架构，针对复杂Abugida文字（如僧伽罗语和天城体）优化分词，显著减少token数量，提升推理效率


<details>
  <summary>Details</summary>
Motivation: 标准BPE分词器在处理复杂Abugida文字时效果不佳，会将复杂的连字拆分为无意义的子字符单元，降低LLM推理效率并增加推理成本，对全球南方国家造成显著的"Token Tax"负担

Method: 提出三层架构WWHO（Where-What-How Often）和SGPE（Syllable-aware Grapheme Pair Encoding）算法，将文字的语言规则与统计压缩过程分离，实现无缝多语言分词

Result: 在僧伽罗语上实现1.274的Token to Word Ratio，每个token包含4.83个字符，相比OpenAI o200k减少61.7%的token；在印地语上减少27.0%的token；在混合文字数据集上相比多个主流模型减少36.7%-60.2%的token，有效扩展上下文窗口达4.38倍

Conclusion: SGPE算法显著改善了复杂Abugida文字的分词效率，减少token数量，降低推理成本，同时保证"语言零拆分保证"——确保有效音节不会被拆分到多个token中
...
### [161] [Beyond Detection: Rethinking Education in the Age of AI-writing](https://arxiv.org/abs/2603.25329)
*Maria Marina,Alexander Panchenko,Vasily Konovalov*

Main category: cs.CL

TL;DR: 本文探讨当AI工具如ChatGPT接管写作时，人类会失去什么——写作不仅是输出，更是思考学习的过程。文章分析写作的认知价值、AI检测现状，并提出教育应通过更智能的教学法而非禁令来适应。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI工具进入课堂和工作场所，写作有沦为形式化、外包化和自动化的风险，可能失去其认知价值。作者担忧当机器为我们写作时，人类会失去通过写作进行深度思考和学习的机会。

Method: 结合认知心理学、教育理论和真实课堂实践，分析写作过程的认知价值；探讨当前AI文本检测的可能性；研究教育者如何通过更智能的教学法而非禁令来适应AI时代。

Result: 研究发现写作过程（混乱、缓慢、常令人沮丧）正是人类深度学习发生的地方；AI文本检测存在可能性但有限；教育需要调整教学法而非简单禁止AI；识别机器生成语言的能力可能成为21世纪的关键素养。

Conclusion: 在写作可以被伪造的世界里，学习不能。写作不仅是输出，更是思考的过程。教育者需要重新思考教学法，培养识别AI生成文本的能力，保护写作作为人类深度学习工具的价值。
...
### [162] [Large Language Model as Token Compressor and Decompressor](https://arxiv.org/abs/2603.25340)
*Wenbing Li,Zikai Song,Jielei Zhang,Tianhao Zhao,Junkai Lin,Yiran Wang,Wei Yang*

Main category: cs.CL

TL;DR: LLM可作为高效的令牌压缩器/解压器，通过自表达自编码框架将长文本转换为紧凑的Z-token表示，实现高达18倍的压缩比


<details>
  <summary>Details</summary>
Motivation: 探索如何利用现成的LLM实现高效的文本压缩，以解决长文本处理中的令牌效率问题，为长上下文推理提供更经济的解决方案

Method: 设计自表达自编码学习框架，通过LoRA适配器微调预训练LLM，将长文本转换为可变长度的离散潜在代码（Z-tokens），实现内容自适应压缩

Result: 在Wikipedia、CNN/DailyMail、HotpotQA和Qulac等数据集上实现高达18倍的令牌压缩，同时保持重建保真度和下游任务性能

Conclusion: 该方法为提示压缩和Z-token空间中的自回归生成提供了简单有效的解决方案，为令牌高效的长上下文推理开辟了潜在途径
...
### [163] [TAPO: Translation Augmented Policy Optimization for Multilingual Mathematical Reasoning](https://arxiv.org/abs/2603.25419)
...
Method: 提出Translation-Augmented Policy Optimization (TAPO)框架，采用英语作为枢纽语言，遵循理解-推理范式，使用步骤级相对优势机制将理解与推理解耦，整合翻译质量奖励。

Result: TAPO在多语言数学推理和翻译任务中均优于基线方法，能很好地泛化到未见语言和领域外任务，且与多种模型兼容。

Conclusion: TAPO通过强化学习框架有效协同语言理解与推理能力，解决了多语言数学推理中的性能差距问题。
...
### [164] [Navigating the Prompt Space: Improving LLM Classification of Social Science Texts Through Prompt Engineering](https://arxiv.org/abs/2603.25422)
*Erkan Gunes,Christoffer Florczak,Tevfik Murat Yildirim*

Main category: cs.CL

TL;DR: 研究发现，在社会科学文本分类中，通过系统调整提示工程（标签描述、指令提示和少样本示例）可以提升LLM性能，但增加提示内容存在边际收益递减甚至性能下降的风险，且效果因模型、任务和批处理大小而异。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在社会科学文本分类中能显著降低成本且有时性能可媲美现有计算方法，但当前测试中性能差异很大，因此需要研究如何最大化LLM在文本分类任务中的性能表现。

Method: 通过系统变化提示工程的三个方面：标签描述、指令提示和少样本示例，在两个不同案例中测试不同提示上下文对LLM文本分类准确性的影响。

Result: 测试表明：1）提示内容的少量增加能带来最大的性能提升；2）进一步增加内容通常只产生边际性能提升；3）有时增加提示内容反而会降低准确性；4）效果在不同模型、任务和批处理大小间存在显著异质性。

Conclusion: LLM文本分类任务需要针对每个具体任务进行独立验证，不能依赖通用规则，因为性能受多种因素影响且存在不可预测的变化。
...
### [165] [Translation Asymmetry in LLMs as a Data Augmentation Factor: A Case Study for 6 Romansh Language Varieties](https://arxiv.org/abs/2603.25489)
*Jannis Vamvas,Ignacio Pérez Prat,Angela Heldstab,Dominic P. Fischer,Sina Ahmadi,Rico Sennrich*

Main category: cs.CL

TL;DR: 研究发现LLM生成合成数据的方法不适用于罗曼什语，因为LLM会混淆其6种方言变体。提出将数据增强方向与源语言和目标语言之间的资源梯度对齐的新方法，在最低资源罗曼什语变体上比Gemini 3 Pro高出23 BLEU分。


<details>
  <summary>Details</summary>
Motivation: 低资源机器翻译通常依赖LLM从高资源语言生成合成数据，但这种方法对罗曼什语失效，因为LLM会混淆其6种不同的语言变体，需要寻找更有效的低资源翻译解决方案。

Method: 提出将数据增强方向与源语言和目标语言之间的资源梯度对齐的方法，而不是简单依赖LLM生成合成数据。针对罗曼什语的6种方言变体特点设计专门的数据增强策略。

Result: 在最低资源的罗曼什语变体上，该方法比Gemini 3 Pro高出23 BLEU分。人工评估确认该模型首次能够生成各个罗曼什语变体的流畅翻译。

Conclusion: 对于像罗曼什语这样具有多个方言变体的低资源语言，数据增强方向应与语言资源梯度对齐，而不是简单依赖LLM生成合成数据。该方法为类似低资源语言翻译问题提供了有效解决方案。
...
### [166] [An Experimental Comparison of the Most Popular Approaches to Fake News Detection](https://arxiv.org/abs/2603.25501)
*Pietro Dell'Oglio,Alessandro Bondielli,Francesco Marcelloni,Lucia C. Passaro*

Main category: cs.CL

TL;DR: 本文对12种假新闻检测方法在10个公开数据集上进行了系统评估，重点关注领域内、多领域和跨领域的性能表现，发现微调模型在领域内表现良好但泛化能力有限，跨领域架构能减少差距但需要大量数据，而大语言模型通过零样本和少样本学习展现出潜力。


<details>
  <summary>Details</summary>
Motivation: 尽管假新闻检测技术有所进步，但大型语言模型和社交媒体的放大效应使得虚假信息的产生和传播变得更加复杂。需要系统评估现有检测方法在不同数据集和实际场景下的表现，特别是面对领域转移和分布外数据时的鲁棒性。

Method: 评估了12种代表性的假新闻检测方法，涵盖传统机器学习、深度学习、Transformer和专门的跨领域架构。在10个公开数据集上进行实验，这些数据集在体裁、来源、主题和标注逻辑上各不相同。将假新闻检测作为二元分类任务，统一标签为"真实"和"虚假"，并在领域内、多领域和跨领域三种设置下进行实验。

Result: 微调模型在领域内表现良好，但在跨领域泛化方面存在困难。跨领域架构能够减少这种差距，但需要大量训练数据。大语言模型通过零样本和少样本学习展现出有前景的替代方案。由于数据集固有的混杂因素和可能的预训练暴露，结果应被视为在英文纯文本协议下的鲁棒性评估。

Conclusion: 假新闻检测面临领域转移和泛化挑战，微调模型在领域内有效但泛化能力有限，跨领域架构需要大量数据，而大语言模型为零样本和少样本学习提供了有前景的途径。研究强调了在真实世界场景中评估检测方法鲁棒性的重要性。
...
### [167] [Humans vs Vision-Language Models: A Unified Measure of Narrative Coherence](https://arxiv.org/abs/2603.25537)
*Nikolai Ilinykh,Hyewon Jang,Shalom Lappin,Asad Sayeed,Sharid Loáiciga*

Main category: cs.CL

TL;DR: 研究比较了人类与视觉语言模型在视觉写作提示语料库中生成的故事的叙事连贯性，发现尽管模型表面流畅度接近人类，但在话语组织上存在系统性差异。


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型在视觉基础故事中的叙事连贯性，探索模型生成叙事与人类写作在连贯性方面的差异。

Method: 使用视觉写作提示语料库，比较人类写作与视觉语言模型生成的叙事。采用一组捕捉叙事连贯性不同方面的指标：指代、话语关系类型、主题连续性、角色持久性和多模态角色基础，计算叙事连贯性分数。

Result: 视觉语言模型显示出与人类系统性不同的连贯性特征。单个指标的差异通常微妙，但联合考虑时差异更明显。尽管模型叙事表面流畅度接近人类，但在视觉基础故事的话语组织上存在系统性差异。

Conclusion: 视觉语言模型生成的叙事在表面流畅度上接近人类，但在视觉基础故事的连贯性组织方面仍存在系统性差异，表明模型在话语组织能力上仍有改进空间。
...
### [168] [Beyond Via: Analysis and Estimation of the Impact of Large Language Models in Academic Papers](https://arxiv.org/abs/2603.25638)
*Mingmeng Geng,Yuhang Dong,Thierry Poibeau*

Main category: cs.CL

TL;DR: 通过分析arXiv论文，研究发现大语言模型（LLMs）导致学术写作中词汇使用模式发生显著变化，如标题中"beyond"和"via"使用频率增加，摘要中"the"和"of"减少。当前分类器难以准确识别特定LLM生成的文本，且不同LLM间的差异导致学术论文词汇使用模式持续演变。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型（LLMs）对学术写作风格的影响，特别是词汇使用模式的变化，以及当前分类器在识别特定LLM生成文本方面的局限性。

Method: 采用直接且高度可解释的线性方法，分析arXiv论文中的词汇使用模式变化，考虑不同模型和提示之间的差异，定量评估LLM对学术写作的影响。

Result: 发现LLM导致学术写作中特定词汇使用频率显著变化（如标题中"beyond"和"via"增加，摘要中"the"和"of"减少）；当前分类器在多类别分类任务中难以准确识别特定LLM生成的文本；不同LLM间的差异导致学术论文词汇使用模式持续演变；真实世界的LLM使用呈现异质性和动态性。

Conclusion: 大语言模型正在显著改变学术写作的词汇使用模式，这种影响是异质且动态的。当前分类器在识别特定LLM生成文本方面存在局限性，需要更精细的方法来理解和监测LLM对学术写作的影响。
...
### [169] [Measuring What Matters -- or What's Convenient?: Robustness of LLM-Based Scoring Systems to Construct-Irrelevant Factors](https://arxiv.org/abs/2603.25674)
*Cole Walsh,Rodica Ivan*

Main category: cs.CL

TL;DR: 研究探讨了基于大语言模型的双架构自动评分系统对构念无关因素的鲁棒性，发现系统对无意义文本填充、拼写错误和写作复杂度具有较好鲁棒性，但文本重复会降低评分，离题回答则被严重惩罚。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在自动评分系统中的广泛应用，需要重新关注"幻觉"问题以及这些LLM评分方法对构念无关因素的鲁棒性。传统自动评分系统虽然性能可与人类评分员媲美，但已被证明容易受到构念无关因素和对抗条件的影响。

Method: 本研究调查了构念无关因素对双架构LLM评分系统的影响，该系统设计用于评分情境判断测试中的短文式开放回答项目。研究测试了多种构念无关因素对评分的影响。
...
### [170] [Self-Improvement of Large Language Models: A Technical Overview and Future Outlook](https://arxiv.org/abs/2603.25681)
*Haoyan Yang,Mario Xerri,Solha Park,Huajian Zhang,Yiyang Feng,Sai Akhil Kogilathota,Jiawei Zhou*

Main category: cs.CL

TL;DR: 论文提出了一个系统级的自改进语言模型框架，将自改进过程组织为一个包含数据获取、数据选择、模型优化和推理精炼四个紧密耦合过程的闭环生命周期，并引入自主评估层来监控和指导改进过程。
...
Method: 提出了一个统一的自改进语言模型框架，将自改进系统概念化为一个闭环生命周期，包含四个核心过程：1) 数据获取（模型收集或生成数据），2) 数据选择（选择信息丰富的信号），3) 模型优化（更新参数），4) 推理精炼（改进输出）。同时引入自主评估层来持续监控进展并指导各阶段的改进循环。

Result: 论文系统性地回顾和分析了每个组件的代表性方法，从技术角度对现有自改进技术进行了组织和分类。建立了一个统一的框架来理解自改进语言模型的工作机制。

Conclusion: 论文为自改进语言模型研究提供了一个系统级的视角和统一的组织框架，讨论了当前局限性并展望了未来完全自改进LLM的研究方向。该框架有助于理解和推进语言模型自主改进能力的发展。
...
### [171] [S2D2: Fast Decoding for Diffusion LLMs via Training-Free Self-Speculation](https://arxiv.org/abs/2603.25702)
...
<details>
  <summary>Details</summary>
Motivation: 现有块扩散语言模型在少步解码时面临困境：激进的置信度阈值会损害质量，而保守的阈值则需要不必要的去噪步骤。现有解决方案要么需要额外训练，要么增加测试时计算成本。

Method: S2D2利用块扩散模型在块大小为1时变为自回归模型的特性，让同一预训练模型同时充当草稿生成器和验证器。它插入推测验证步骤，并使用轻量级路由策略决定何时进行验证，形成混合解码轨迹。

Result: 在三个主流块扩散模型上，S2D2相比强置信度阈值基线持续改进准确率-速度权衡。在SDAR上实现4.7倍加速，在LLaDA2.1-Mini上保持4.4倍加速同时略微提高准确率。

Conclusion: S2D2提供了一种无需训练的方法，通过结合扩散并行生成和自回归验证，有效解决了块扩散模型在少步解码时的质量-速度权衡问题，实现了显著加速而不损失质量。
...
### [172] [Natural-Language Agent Harnesses](https://arxiv.org/abs/2603.25723)
*Linyue Pan,Lexiao Zou,Shuo Guo,Jingchen Ni,Hai-Tao Zheng*

Main category: cs.CL

TL;DR: 该论文提出自然语言智能体套具(NLAH)和智能套具运行时(IHR)，将智能体控制逻辑从代码中分离为可移植的自然语言可执行文件，便于迁移、比较和研究。
...
Conclusion: 通过将智能体套具设计外部化为自然语言可执行文件，解决了套具工程难以迁移、比较和研究的问题，为智能体套具设计提供了新的可移植框架。
...
### [173] [Pseudo Label NCF for Sparse OHC Recommendation: Dual Representation Learning and the Separability Accuracy Trade off](https://arxiv.org/abs/2603.24750)
*Pronob Kumar Barman,Tera L. Reynolds. James Foulds*

Main category: cs.IR

TL;DR: 该研究提出了一种伪标签神经协同过滤方法，通过调查问卷特征对齐来改进在线健康社区中极端稀疏交互下的推荐性能。


<details>
  <summary>Details</summary>
Motivation: 在线健康社区用户面临发现挑战，特别是在极端交互稀疏的情况下，用户几乎没有历史交互来指导个性化推荐。研究旨在解决冷启动条件下的推荐问题。

Method: 扩展神经协同过滤架构（矩阵分解、多层感知机、NeuMF），引入基于调查问卷特征对齐的辅助伪标签目标，使用余弦相似度映射到[0,1]区间，学习双重嵌入空间：主嵌入用于排名，伪标签嵌入用于语义对齐。

Result: 在165个用户和498个支持组的数据集上，所有伪标签变体都提高了排名性能：MLP的HR@5从2.65%提升到5.30%，NeuMF从4.46%提升到5.18%，MF从4.58%提升到5.42%。伪标签嵌入空间的余弦轮廓分数也更高，表明更好的可解释性。

Conclusion: 调查问卷衍生的伪标签能有效改进极端稀疏条件下的推荐性能，同时产生可解释的任务特定嵌入空间，但存在可解释性与性能之间的权衡关系。
...
### [174] [DIET: Learning to Distill Dataset Continually for Recommender Systems](https://arxiv.org/abs/2603.24958)
*Jiaqing Zhang,Hao Wang,Mingjia Yin,Bo Chen,Qinglin Jia,Rui Zhou,Ruiming Tang,ChaoYi Ma,Enhong Chen*

Main category: cs.IR

TL;DR: DIET提出了一种用于推荐系统的流式数据集蒸馏框架，通过维护一个紧凑的蒸馏数据集来近似全数据训练，将训练数据压缩到原始大小的1-2%，同时保持性能趋势一致。


<details>
  <summary>Details</summary>
Motivation: 现代深度推荐模型在持续学习范式下训练，依赖海量且不断增长的流式行为日志。在大规模平台中，为架构比较或迭代而重新训练完整历史数据的成本过高，严重拖慢模型开发速度。需要数据高效的方法来近似全数据训练行为，而无需重复处理整个演化数据流。

Method: DIET是一个统一的流式数据集蒸馏框架，将蒸馏数据建模为演化的训练记忆，以阶段式方式更新以保持与长期训练动态的一致性。通过从有影响力的样本进行原则性初始化，并在双层优化框架内通过影响力感知的内存寻址进行选择性更新，实现有效的持续蒸馏。

Result: 在大规模推荐基准测试中，DIET将训练数据压缩到原始大小的1-2%，同时保持与全数据训练一致的性能趋势，将模型迭代成本降低高达60倍。蒸馏数据集在不同模型架构间泛化良好。

Conclusion: 流式数据集蒸馏为推荐系统开发提供了一个可扩展且可重用的数据基础，DIET框架通过维护演化的蒸馏数据集有效解决了大规模推荐模型迭代的高成本问题。
...
### [175] [Unbiased Multimodal Reranking for Long-Tail Short-Video Search](https://arxiv.org/abs/2603.24975)
*Wenyi Xu,Feiran Zhu,Songyang Li,Renzhe Zhou,Chao Zhang,Chenglei Dai,Yuren Mao,Yunjun Gao,Yi Zhang*

Main category: cs.IR

TL;DR: 快手提出基于大语言模型的多模态重排序框架，解决短视频搜索中的马太效应问题，通过LLM评估内容质量来提升长尾查询的用户体验


<details>
  <summary>Details</summary>
Motivation: 快手每日处理数亿次搜索，短视频搜索质量至关重要。但长尾查询存在严重的马太效应：稀疏的用户行为数据导致模型放大低质量内容（如点击诱饵和浅层内容）。大语言模型的世界知识为评估内容质量提供了新范式，不受稀疏用户交互影响。

Method: 提出LLM驱动的多模态重排序框架，通过两阶段训练：第一阶段使用多模态证据构建高质量标注进行监督微调；第二阶段引入成对偏好优化，帮助模型学习候选视频间的偏序关系。推理时，生成的经验分数用于在重排序中提升高质量但曝光不足的视频，并通过强化学习指导页面级优化。

Result: 实验表明，该方法在离线指标（AUC、NDCG@K、人工偏好判断）上持续优于强基线。覆盖15%流量的在线A/B测试进一步显示，在用户体验和消费指标上均有提升，证实了该方法在长尾视频搜索场景中的实用价值。

Conclusion: LLM驱动的多模态重排序框架能有效解决短视频搜索中的长尾查询质量问题，通过利用LLM的世界知识评估内容质量，减少对稀疏用户行为的依赖，提升整体搜索体验。
...
### [176] [Sparton: Fast and Memory-Efficient Triton Kernel for Learned Sparse Retrieval](https://arxiv.org/abs/2603.25011)
*Thong Nguyen,Cosimo Rulli,Franco Maria Nardini,Rossano Venturini,Andrew Yates*

Main category: cs.IR

TL;DR: Sparton是一个专门为学习稀疏检索模型设计的GPU内核，通过融合矩阵乘法、ReLU、Log1P和最大池化操作，显著减少了内存占用并提升了训练速度。


<details>
  <summary>Details</summary>
Motivation: 现有学习稀疏检索模型（如Splade）使用语言模型头将隐藏状态投影到词汇表大小的对数矩阵，由于词汇表规模庞大（3万到25万词元），导致巨大的内存瓶颈和I/O开销，限制了模型扩展和训练效率。

Method: 提出Sparton内核，采用融合方法将分块矩阵乘法、ReLU、Log1P和最大池化集成到单个GPU内核中，通过对原始对数分块进行早期在线池化，避免在内存中生成完整的对数矩阵。

Result: Sparton内核相比PyTorch基线实现了4.8倍加速和峰值内存使用量降低一个数量级。集成到Splade（词汇表约3万）中可实现33%更大的批处理大小和14%更快的训练；在多语言模型（词汇表约25万）中，批处理大小增加26倍，训练速度提升2.5倍。

Conclusion: Sparton通过创新的融合内核设计有效解决了学习稀疏检索模型中的内存瓶颈问题，显著提升了训练效率和模型扩展能力，为大规模词汇表模型的实际部署提供了可行的解决方案。
...
### [177] [ColBERT-Att: Late-Interaction Meets Attention for Enhanced Retrieval](https://arxiv.org/abs/2603.25248)
*Raj Nath Patel,Sourav Dutta*

Main category: cs.IR

TL;DR: ColBERT-Att在ColBERT的延迟交互框架中显式集成注意力机制，利用查询和文档术语的注意力权重来提升检索性能


<details>
  <summary>Details</summary>
Motivation: 当前延迟交互范式未能考虑查询和文档术语的注意力权重，这些权重直观地捕捉了术语相似性的"重要性"，可能有助于更好地理解查询与文档之间的相关性

Method: 提出ColBERT-Att，在延迟交互框架中显式集成注意力机制，利用注意力权重来增强检索性能

Result: 在MS-MARCO以及广泛的BEIR和LoTTE基准数据集上，ColBERT-Att在召回准确率方面表现出改进

Conclusion: 将注意力机制集成到延迟交互框架中可以有效提升神经信息检索系统的性能
...
### [178] [Supercharging Federated Intelligence Retrieval](https://arxiv.org/abs/2603.25374)
...
TL;DR: 提出基于Flower的联邦RAG系统，解决知识分布在私有数据孤岛时的检索问题，通过可信计算环境保护LLM推理的机密性


<details>
  <summary>Details</summary>
Motivation: 传统RAG假设集中式访问文档，但在知识分布在私有数据孤岛时失效，需要解决分布式私有数据的安全检索问题

Method: 1. 基于Flower构建联邦RAG系统，在本地数据孤岛进行检索；2. 服务器端聚合和文本生成在可信计算环境中运行；3. 提出级联推理方法，整合非机密第三方模型作为辅助上下文

Result: 系统能够在诚实但好奇或服务器被攻陷的情况下实现机密远程LLM推理，同时保持与第三方模型集成的灵活性

Conclusion: 提出的联邦RAG系统有效解决了分布式私有数据的安全检索问题，通过可信计算环境和级联推理方法平衡了机密性与功能性
...
### [179] [MCLMR: A Model-Agnostic Causal Learning Framework for Multi-Behavior Recommendation](https://arxiv.org/abs/2603.25126)
*Ranxu Zhang,Junjie Meng,Ying Sun,Ziqi Xu,Bing Yin,Hao Li,Yanyong Zhang,Chao Wang*

Main category: cs.IR

TL;DR: MCLMR是一个模型无关的因果学习框架，通过建模混杂效应、自适应聚合辅助行为信息和偏差感知对比学习，解决多行为推荐中的混杂效应、异构行为聚合和语义对齐问题。
...
Conclusion: MCLMR是一个有效的模型无关因果学习框架，能够解决多行为推荐中的混杂效应、异构行为聚合和语义对齐问题，显著提升推荐性能。
...
### [180] [DyMRL: Dynamic Multispace Representation Learning for Multimodal Event Forecasting in Knowledge Graph](https://arxiv.org/abs/2603.24636)
*Feng Zhao,Kangzheng Liu,Teng Peng,Yu Yang,Guandong Xu*

Main category: cs.LG

TL;DR: DyMRL提出了一种动态多空间表示学习方法，用于事件预测中的多模态知识获取与融合，通过整合欧几里得、双曲和复数空间的时变结构特征，结合双融合-演化注意力机制，在四个多模态时序知识图谱基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注静态设置，忽视了多模态知识的动态获取与融合。具体问题包括：1）知识获取层面，如何学习不同模态（特别是动态结构模态）的时间敏感信息；2）知识融合层面，如何学习演化的多模态融合特征。现有方法在动态结构学习和静态注意力融合方面存在局限。

Method: DyMRL采用动态多空间表示学习方法：1）整合欧几里得、双曲和复数空间的时变结构特征到关系消息传递框架中，学习深度表示，反映人类联想思维、高阶抽象和逻辑推理能力；2）采用先进的双融合-演化注意力机制，以对称方式为不同时间戳的不同模态分配动态学习权重。

Result: 在构建的四个多模态时序知识图谱基准上进行广泛实验，DyMRL在事件预测性能上优于最先进的动态单模态和静态多模态基线方法。

Conclusion: DyMRL通过有效获取和融合多模态时序知识，在事件预测任务中表现出优越性能，解决了现有方法在动态知识获取和融合方面的局限性。
...
### [181] [Experiential Reflective Learning for Self-Improving LLM Agents](https://arxiv.org/abs/2603.24639)
...
<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型智能体难以适应专业环境，且无法利用过往经验，每次任务都从零开始，缺乏持续学习和自我改进能力
...
Conclusion: 通过单次尝试经验反思提取可迁移启发式规则，能够实现有效的智能体自我改进，为LLM智能体的持续学习和环境适应提供了新途径
...
### [182] [Amplified Patch-Level Differential Privacy for Free via Random Cropping](https://arxiv.org/abs/2603.24695)
*Kaan Durmaz,Jan Schuchardt,Sebastian Schmidt,Stephan Günnemann*

Main category: cs.LG

TL;DR: 随机裁剪作为视觉数据增强技术，其内在随机性在差分隐私训练中未被充分探索。研究发现当敏感内容空间局部化时，随机裁剪能概率性地排除敏感内容，为DP-SGD引入第三随机源，增强隐私保护而不需改变模型或训练流程。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉中随机裁剪是常见的数据增强技术，但其内在随机性在差分隐私机器学习训练中的作用尚未被研究。当图像中的敏感内容（如人脸、车牌）空间局部化时，随机裁剪能概率性地排除这些内容，这为差分隐私训练提供了额外的随机性来源。

Method: 引入视觉数据的补丁级相邻关系，推导随机裁剪与差分隐私随机梯度下降（DP-SGD）结合时的紧致隐私边界。分析量化补丁包含概率，展示其如何与小批量采样组合以产生更低的实际采样率。

Result: 理论分析表明补丁级放大改善了隐私-效用权衡，经验验证显示该方法在多个分割架构和数据集上都能提升隐私保护效果。补丁包含概率与小批量采样的组合降低了有效采样率。

Conclusion: 将隐私计算与领域结构和现有随机源对齐，可以在不增加成本的情况下获得更强的隐私保证。随机裁剪的内在随机性为差分隐私训练提供了第三随机源，增强了隐私保护效果。
...
### [183] [Can an Actor-Critic Optimization Framework Improve Analog Design Optimization?](https://arxiv.org/abs/2603.24714)
*Sounak Dutta,Fin Amin,Sushil Panda,Jonathan Rabe,Yuejiang Wen,Paul Franzon*

Main category: cs.LG

TL;DR: 提出了一种用于模拟电路尺寸优化的演员-评论家框架（ACOF），通过分离设计空间探索与评估的角色，提高优化效率和质量


<details>
  <summary>Details</summary>
Motivation: 模拟电路设计优化面临挑战：微小参数变化需要昂贵的仿真周期，高质量解仅存在于巨大搜索空间的狭窄区域。现有优化器缺乏设计师在决定搜索方向时使用的判断能力

Method: ACOF框架将优化过程分离为演员和评论家两个角色：演员提出有前景的设计空间区域，评论家评估这些选择、强制执行设计合法性，并在进展受阻时重新定向搜索

Result: 在测试电路中，ACOF将前10个性能指标的均值提高了38.9%，平均减少了24.7%的遗憾值，个别电路的峰值增益达到70.5%的性能指标提升和42.2%的遗憾值降低

Conclusion: 通过将迭代推理与仿真驱动搜索相结合，ACOF为自动化模拟电路尺寸优化提供了更透明、更稳定的路径，使搜索过程更具目的性、稳定性和可解释性
...
### [184] [Grokking as a Falsifiable Finite-Size Transition](https://arxiv.org/abs/2603.24746)
*Yuda Bi,Chenyu Zhang,Qiheng Wang,Vince D Calhoun*

Main category: cs.LG

TL;DR: 该研究为"grokking"现象提供了可检验的相变理论框架，通过将群阶p作为扩展变量，使用谱头尾对比作为序参数，证实了grokking中存在有限尺寸相变而非平滑交叉。


<details>
  <summary>Details</summary>
Motivation: 虽然grokking现象常被描述为相变，但缺乏可检验的有限尺寸输入。本研究旨在为grokking中的相变语言提供可证伪的定量框架，而非仅作为类比。

Method: 将群阶p作为可接受的扩展变量，使用保留的谱头尾对比作为表示级序参数，应用凝聚态物理风格的诊断链，包括粗网格扫描和密集近临界加法审计，使用Binder类交叉分析。

Result: Binder类交叉揭示了共享的有限尺寸边界，敏感性比较强烈反对平滑交叉解释（近临界审计中ΔAIC=16.8），证实了相变语言在grokking中可作为定量有限尺寸主张进行检验。

Conclusion: grokking中的相变语言可以作为定量有限尺寸主张进行检验，而不仅仅是类比，尽管目前转变阶数仍未解决。
...
### [185] [Transformers in the Dark: Navigating Unknown Search Spaces via Bandit Feedback](https://arxiv.org/abs/2603.24780)
*Jungtaek Kim,Thomas Zeng,Ziqian Lin,Minjae Lee,Chungpa Lee,Jy-yong Sohn,Hyung Il Koo,Kangwook Lee*

Main category: cs.LG

TL;DR: LLMs能否近似搜索算法？研究通过"未知树搜索与老虎机反馈"框架证明Transformer架构理论上能够实现不同搜索策略，并可通过训练近似这些策略，甚至能泛化到未见过的条件。


<details>
  <summary>Details</summary>
Motivation: 虽然外部搜索算法能增强LLMs的问题解决能力，通过平衡探索与利用来导航树状搜索空间，但外部组件的需求使整体问题解决过程复杂化。因此研究探索LLMs或其底层Transformer架构能否近似搜索算法。

Method: 引入"未知树搜索与老虎机反馈"简化框架，其中树扩展和反馈信号由外部指定，用于受控评估搜索能力。在此框架下，训练Transformer模型从零开始近似不同搜索策略。

Result: Transformer架构在理论上具有足够表达能力来实现不同搜索策略，并能通过训练近似这些策略。模型展现出泛化到未见条件（如更长视野或更深树）的可能性。此外，持续任务导向训练能解锁预训练LLMs的完整能力。

Conclusion: LLMs和Transformer架构能够近似搜索算法，这为简化问题解决流程提供了可能，无需依赖外部搜索组件。通过适当的训练，LLMs可以内化搜索能力，并在新条件下保持有效性。
...
### [186] [A Practical Guide Towards Interpreting Time-Series Deep Clinical Predictive Models: A Reproducibility Study](https://arxiv.org/abs/2603.24828)
*Yongda Fan,John Wu,Andrea Fitzpatrick,Naveen Baskaran,Jimeng Sun,Adam Cross*

Main category: cs.LG

TL;DR: 该研究提出了一个评估临床预测任务中可解释性方法的综合基准，发现注意力机制在正确使用时是高效的可解释性方法，而某些黑盒解释器在时间序列任务中计算不可行，部分方法可靠性不足。


<details>
  <summary>Details</summary>
Motivation: 临床决策需要明确的可解释性来审计深度学习模型，但现有基准缺乏可扩展性和可重复性，未能系统研究不同临床任务和模型架构下可解释性的变化。

Method: 构建了一个综合基准，评估多种可解释性方法在多样化临床预测任务和模型架构上的表现，通过PyHealth开源框架提供可重复和可扩展的实现。

Result: 研究发现：(1) 注意力机制在正确使用时是高效忠实解释模型预测的方法；(2) KernelSHAP和LIME等黑盒解释器在时间序列临床预测任务中计算不可行；(3) 多种可解释性方法可靠性不足，不可信。

Conclusion: 研究提出了改进临床预测流程中可解释性的指导原则，并通过开源框架PyHealth支持可重复性和可扩展性，为临床模型审计提供了实用工具。
...
### [187] [Flow matching on homogeneous spaces](https://arxiv.org/abs/2603.24829)
*Francesco Ruscelli*
...
<details>
  <summary>Details</summary>
Motivation: 现有的黎曼流匹配方法在齐次空间上需要定义和计算预度量或测地线，计算复杂且效率低。需要一种更简单、更快速且完全内蕴的框架来处理齐次空间上的流匹配问题

Method: 通过将齐次空间上的数据分布提升到其底层李群上，将流匹配问题转化为李群上的流匹配任务。进一步利用李群与李代数之间的指数映射，将问题简化为李代数上的欧几里得流匹配

Result: 该方法避免了齐次空间复杂几何的直接处理，消除了对预度量或测地线的需求，实现了更简单、更快速且完全内蕴的框架。相比黎曼流匹配，计算复杂度显著降低

Conclusion: 提出的框架成功将流匹配扩展到齐次空间，通过将问题转化为李代数上的欧几里得流匹配，提供了一种简化、高效且内蕴的处理方法，为齐次空间上的生成建模开辟了新途径
...
### [188] [Reaching Beyond the Mode: RL for Distributional Reasoning in Language Models](https://arxiv.org/abs/2603.24844)
*Isha Puri,Mehul Damani,Idan Shenfeld,Marzyeh Ghassemi,Jacob Andreas,Yoon Kim*

Main category: cs.LG

TL;DR: 本文提出了一种多答案强化学习方法，用于训练语言模型在推理时对多个答案进行分布推理，无需重复采样即可生成多个合理假设。


<details>
  <summary>Details</summary>
Motivation: 语言模型在训练后通常将答案分布坍缩到单一主导模式，但现实世界任务（如医疗诊断、模糊问答、信息不完整场景）往往涉及多个有效答案或不可约的不确定性，需要模型生成多个合理假设并给出置信度估计。

Method: 采用多答案强化学习方法，修改RL目标使模型能在单次前向传播中显式生成多个候选答案，将推理时搜索的某些方面内化到模型的生成过程中。

Result: 在问答、医疗诊断和编码基准测试中，相比单答案训练基线，该方法在多样性、覆盖率和集合级校准分数方面均有提升。模型生成多个答案所需token更少，在编码任务中准确性也显著提高。

Conclusion: 多答案RL为推理时扩展程序（如best-of-k）提供了原则性且计算高效的替代方案，使语言模型能够更好地处理具有多个有效答案的现实世界任务。
...
### [189] [Learning to Staff: Offline Reinforcement Learning and Fine-Tuned LLMs for Warehouse Staffing Optimization](https://arxiv.org/abs/2603.24883)
*Kalle Kujanpää,Yuying Zhu,Kristina Klinkner,Shervin Malmasi*

Main category: cs.LG

TL;DR: 该研究探索了两种机器学习方法优化半自动化仓库分拣系统的实时人员配置决策：基于Transformer的离线强化学习方法和基于LLM的抽象决策方法，两者都能提升运营效率。


<details>
  <summary>Details</summary>
Motivation: 研究旨在优化半自动化仓库分拣系统的实时人员配置决策，支持不同抽象层次的运营决策，平衡详细状态与高层摘要之间的权衡。

Method: 采用两种方法：1）使用详细历史状态表示训练基于Transformer的离线强化学习策略；2）探索LLM在抽象、人类可读状态描述上的应用，系统比较提示技术、自动提示优化和微调策略。

Result: 离线强化学习方法在学习的模拟器中实现了2.4%的吞吐量提升；LLM方法通过监督微调结合Direct Preference Optimization，在手工制作的模拟器中达到或略超历史基线性能。
...
### [190] [CVA: Context-aware Video-text Alignment for Video Temporal Grounding](https://arxiv.org/abs/2603.24934)
*Sungho Moon,Seunghun Lee,Jiwan Seo,Sunghoon Im*

Main category: cs.LG

TL;DR: CVA框架通过查询感知上下文多样化、上下文不变边界判别损失和上下文增强Transformer编码器，显著提升视频时序定位性能，在多个基准测试中实现约5分的R1提升。


<details>
  <summary>Details</summary>
Motivation: 解决视频时序定位中的关键挑战：在存在无关背景上下文的情况下，实现时间敏感的视文对齐，同时保持对无关内容的鲁棒性。

Method: 1. 查询感知上下文多样化(QCD)：基于视频-文本相似性的数据增强策略，防止查询无关混合导致的"假阴性"
2. 上下文不变边界判别(CBD)损失：对比损失函数，增强时间边界处的语义一致性
3. 上下文增强Transformer编码器(CTE)：分层架构，结合窗口自注意力和双向交叉注意力

Result: 在QVHighlights和Charades-STA等主要VTG基准测试中达到最先进性能，Recall@1(R1)分数比现有最佳方法提升约5分，显著减少假阴性。

Conclusion: 通过数据中心和架构增强的协同作用，CVA框架有效解决了视频时序定位中的上下文鲁棒性问题，为视文对齐提供了新的解决方案。
...
### [191] [Optimal High-Probability Regret for Online Convex Optimization with Two-Point Bandit Feedback](https://arxiv.org/abs/2603.25029)
*Haishan Ye*

Main category: cs.LG

TL;DR: 本文解决了在线凸优化中两点反馈的开放问题，首次为强凸损失函数提供了最优的高概率遗憾界


<details>
  <summary>Details</summary>
Motivation: 在线凸优化中，两点反馈虽然允许梯度估计，但强凸函数的高概率遗憾界一直未解决，主要挑战在于带反馈梯度估计器的重尾特性使得标准集中分析变得困难

Method: 采用两点带反馈机制，通过解决带反馈梯度估计器的重尾问题，实现了对强凸损失函数的高概率遗憾分析

Result: 首次获得了O(d(log T + log(1/δ))/μ)的高概率遗憾界，该结果在时间范围T和维度d上都是极小极大最优的

Conclusion: 本文成功解决了在线凸优化中两点反馈的开放挑战，为强凸损失函数提供了最优的高概率遗憾界，填补了该领域的重要空白
...
### [192] [Epistemic Compression: The Case for Deliberate Ignorance in High-Stakes AI](https://arxiv.org/abs/2603.25033)
*Steffen Lukas*

Main category: cs.LG

TL;DR: 论文提出"认知压缩"原则：在高风险领域，模型鲁棒性来自模型复杂度与数据有效期的匹配，而非参数规模扩展。通过"制度指数"区分不稳定/稳定制度，指导模型架构设计。


<details>
  <summary>Details</summary>
Motivation: 基础模型在稳定环境中表现优异，但在医疗、金融、政策等高风险领域存在"保真度悖论"：模型容量增加反而放大噪声而非捕捉信号。这不仅是数据问题，更是结构性问题。

Method: 提出"认知压缩"原则，通过架构设计而非事后权重惩罚来强制简约性。引入"制度指数"区分"变动制度"（不稳定、数据贫乏）和"稳定制度"（不变、数据丰富），指导模型复杂度选择。

Result: 在15个高风险领域的探索性综合研究中，制度指数与经验上更优的建模策略在86.7%的情况下（13/15）保持一致。

Conclusion: 高风险AI需要从为扩展而扩展转向有原则的简约性，模型复杂度应与数据有效期匹配，而非盲目增加参数规模。
...
### [193] [Intern-S1-Pro: Scientific Multimodal Foundation Model at Trillion Scale](https://arxiv.org/abs/2603.25040)
...
<details>
  <summary>Details</summary>
Motivation: 构建首个万亿参数级别的科学多模态基础模型，实现通用智能与专业科学智能的深度融合，解决大规模模型在科学领域的专业能力不足问题

Method: 利用XTuner和LMDeploy基础设施支持，实现万亿参数级别的高效强化学习训练，确保训练与推理的精度一致性，集成多模态理解和智能体能力

Result: 模型在通用能力方面达到开源模型顶级水平，在化学、材料、生命科学、地球科学等专业科学任务深度上超越专有模型

Conclusion: Intern-S1-Pro作为"可专业化的通才"，成功融合通用智能与专业科学智能，为大规模科学多模态模型发展提供了重要范例
...
### [194] [SIGMA: Structure-Invariant Generative Molecular Alignment for Chemical Language Models via Autoregressive Contrastive Learning](https://arxiv.org/abs/2603.25062)
*Xinyu Wang,Fei Dou,Jinbo Bi,Minghu Song*

Main category: cs.LG

TL;DR: SIGMA通过token级对比学习解决分子图线性化表示中的歧义问题，结合同构束搜索提升生成效率


<details>
  <summary>Details</summary>
Motivation: 线性化字符串表示在可扩展自回归分子生成中引入模态不匹配问题：单个分子图对应多个不同序列，导致结构等效的部分图在潜在表示中产生轨迹发散

Method: 提出SIGMA方法：1) 通过token级对比目标使模型严格识别几何对称性，显式对齐具有相同后缀的前缀的潜在状态；2) 引入同构束搜索(IsoBeam)在推理时动态修剪等效路径消除同构冗余

Result: 在标准基准测试中，SIGMA在序列可扩展性和图保真度之间架起桥梁，在多参数优化中相比强基线表现出更好的样本效率和结构多样性

Conclusion: SIGMA在不放弃高效字符串表示的前提下，通过对比学习和同构束搜索解决了线性化表示中的歧义问题，实现了可扩展且保真的分子生成
...
### [195] [Process-Aware AI for Rainfall-Runoff Modeling: A Mass-Conserving Neural Framework with Hydrological Process Constraints](https://arxiv.org/abs/2603.25093)
...
<details>
  <summary>Details</summary>
Motivation: 机器学习模型在水文应用中虽然预测精度高，但缺乏物理可解释性。研究旨在探索如何在AI框架中嵌入物理约束，实现既准确又可解释的降雨径流建模。

Method: 使用质量守恒感知机（MCP）框架，从最小化MCP开始，逐步引入有物理意义的水文过程表示：有界土壤存储、状态相关导水性、可变孔隙度、入渗能力、地表积水、垂直排水和非线性地下水位动态。在15个美国流域评估这些过程感知MCP模型。

Result: 逐步增强MCP内部物理结构通常能提升预测性能。过程表示的影响强烈依赖于水文气候条件：垂直排水在干旱和雪主导流域显著提升模型技能，但在降雨主导区域降低性能；地表积水影响相对较小。最佳MCP配置接近LSTM基准的预测技能，同时保持明确的物理可解释性。

Conclusion: 在水文AI架构中嵌入物理过程约束为可解释和过程感知的降雨径流建模提供了有前景的途径，实现了预测精度与物理可解释性的平衡。
...
### [196] [Layer-Specific Lipschitz Modulation for Fault-Tolerant Multimodal Representation Learning](https://arxiv.org/abs/2603.25103)
*Diyar Altinses,Andreas Schwung*

Main category: cs.LG

TL;DR: 提出一个数学基础的多模态容错表示学习框架，统一自监督异常检测和错误校正，通过理论分析扰动传播推导鲁棒性准则，实验证明在传感器损坏下提升异常检测和重建性能。


<details>
  <summary>Details</summary>
Motivation: 工业和安全关键环境中的多模态系统需要在部分传感器故障、信号退化或跨模态不一致时保持可靠，现有方法缺乏理论保证和统一架构。

Method: 基于扰动传播理论分析，推导Lipschitz和Jacobian准则判断神经算子对局部故障的放大或衰减；提出两阶段自监督训练：预训练多模态卷积自编码器在干净数据上保留潜在空间异常信号，扩展为包含校正密集层和对比目标的块；引入层特定Lipschitz调制和梯度剪裁控制检测与校正模块的敏感性。

Result: 在多模态故障数据集上的实验结果表明，该方法提高了传感器损坏下的异常检测准确性和重建性能。

Conclusion: 该框架在分析鲁棒性保证和实际容错多模态学习之间架起了桥梁，为工业安全关键应用提供了理论基础和实用解决方案。
...
### [197] [Gap Safe Screening Rules for Fast Training of Robust Support Vector Machines under Feature Noise](https://arxiv.org/abs/2603.25221)
*Tan-Hau Nguyen,Thu-Le Tran,Kien Trung Nguyen*

Main category: cs.LG

TL;DR: 本文为鲁棒支持向量机(R-SVMs)开发了安全样本筛选规则，在不影响最优解的情况下降低训练复杂度，首次将安全筛选技术应用于监督机器学习中的最坏情况鲁棒模型。


<details>
  <summary>Details</summary>
Motivation: R-SVMs通过采用最坏情况鲁棒公式来处理特征噪声，虽然提高了可靠性，但也增加了计算成本。需要开发能够减少训练复杂度而不影响最优解的方法。

Method: 基于拉格朗日对偶性（而非常用的Fenchel-Rockafellar对偶）推导安全筛选规则，识别那些不确定性集完全位于间隔超平面一侧的训练样本，从而减少问题规模。首先建立理想筛选规则，然后通过将基于GAP的安全区域适应到鲁棒设置来推导实用规则。

Result: 实验表明，所提出的方法显著减少了训练时间，同时保持了分类准确性。

Conclusion: 这是首次将安全筛选技术应用于监督机器学习中的最坏情况鲁棒模型，成功为R-SVMs开发了有效的安全样本筛选方法，在保持分类精度的同时显著加速了优化过程。
...
### [198] [Offline Decision Transformers for Neural Combinatorial Optimization: Surpassing Heuristics on the Traveling Salesman Problem](https://arxiv.org/abs/2603.25241)
*Hironori Ohigashi,Shinichiro Hamada*

Main category: cs.LG

TL;DR: 该研究将离线强化学习框架Decision Transformer应用于组合优化问题，通过从启发式解数据集中学习，不仅模仿还能超越现有启发式算法，在旅行商问题上取得更好效果。


<details>
  <summary>Details</summary>
Motivation: 神经组合优化方法依赖在线强化学习，这限制了实际部署且未能充分利用数十年的算法知识积累。需要一种能够从现有启发式解中学习并超越它们的方法。

Method: 应用离线强化学习框架Decision Transformer，集成Pointer Network处理节点选择的实例依赖可变动作空间，并使用expectile回归对Return-to-Go进行乐观条件化处理。

Result: 实验表明，该方法在旅行商问题上产生的路径质量始终优于训练所用的四种经典启发式算法，展示了离线强化学习超越现有领域知识性能的潜力。

Conclusion: 离线强化学习能够从启发式解数据集中学习并合成出更优策略，为组合优化问题提供了超越传统启发式算法性能的新途径。
...
### [199] [How Pruning Reshapes Features: Sparse Autoencoder Analysis of Weight-Pruned Language Models](https://arxiv.org/abs/2603.25325)
*Hector Borobia,Elies Seguí-Mas,Guillermina Tormo-Carbó*

Main category: cs.LG

TL;DR: 该研究首次系统探讨了非结构化剪枝如何重塑语言模型的内部特征几何结构，发现罕见特征比高频特征更能抵抗剪枝，剪枝起到隐式特征选择作用。
...
Result: 最显著的发现是罕见SAE特征（低激活率）比高频特征更能抵抗剪枝，在17个实验条件中有11个显示Spearman相关系数ρ=-1.0。Wanda剪枝比幅度剪枝能更好地保留特征结构（高达3.7倍），预训练的SAEs在Wanda剪枝模型上可保持有效性至50%稀疏度，但几何特征存活率不能预测因果重要性。

Conclusion: 剪枝起到隐式特征选择作用，优先破坏高频通用特征而保留专门的罕见特征。特征几何存活率与因果重要性的分离对压缩下的可解释性具有重要启示。
...
### [200] [From Intent to Evidence: A Categorical Approach for Structural Evaluation of Deep Research Agents](https://arxiv.org/abs/2603.25342)
*Shuoling Liu,Zhiquan Tan,Kun Yi,Hui Wu,Yihan Li,Jiangpeng Yan,Liyuan Chen,Kai Chen,Qiang Yang*

Main category: cs.LG

TL;DR: 该研究提出了一种基于范畴论的形式化框架来评估深度研究智能体，通过四个可解释的维度设计包含296个问题的基准测试，发现当前最先进模型平均准确率仅19.9%，揭示了AI在复杂结构信息处理上的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究智能体的评估主要依赖启发式基准测试，这些方法无法严格建模智能体行为或充分测试长时程信息合成和模糊性解决能力。需要建立理论框架来系统评估智能体的结构理解能力。

Method: 使用范畴论框架将深度研究工作流建模为结构保持映射（函子）的组合，基于此理论框架设计了包含296个问题的机制感知基准测试，从四个可解释维度评估智能体：遍历顺序连接链、验证V结构拉回中的交集、对检索子结构施加拓扑排序、通过Yoneda探针进行本体证伪。

Result: 对11个领先模型的评估显示基线持续偏低，最先进模型平均准确率仅19.9%。研究发现AI能力存在明显二分：先进研究管道能成功重新定义动态拓扑排序并展示稳健的本体验证能力，但在多跳结构合成方面几乎普遍崩溃。不同任务间的大幅性能差异表明当前系统仍依赖脆弱的启发式方法而非系统性理解。

Conclusion: 虽然顶级自主智能体现在能够有机统一搜索和推理，但在复杂结构信息上实现泛化掌握仍然是一个严峻的开放挑战。该工作为深度研究智能体的形式化评估提供了理论框架和基准测试。
...
### [201] [Hessian-informed machine learning interatomic potential towards bridging theory and experiments](https://arxiv.org/abs/2603.25373)
*Bangchen Yin,Jian Ouyang,Zhen Fan,Kailai Lin,Hanshi Hu,Dingshun Lv,Weiluo Ren,Hai Xiao,Ji Chen,Changsu Cao*

Main category: cs.LG

TL;DR: 本文提出了一种Hessian-informed机器学习原子间势（Hi-MLIP），通过高效的HINT训练协议显著降低Hessian标签需求，实现了对势能面曲率的准确预测，从而改进过渡态搜索和热力学性质计算。


<details>
  <summary>Details</summary>
Motivation: 势能面的局部曲率对于从第一性原理预测分子和材料的实验可观测值至关重要，但对于复杂系统仍然难以计算。现有方法需要大量昂贵的Hessian计算，限制了实际应用。

Method: 开发了Hi-MLIP（Hessian-informed机器学习原子间势）和HINT（Hessian INformed Training）训练协议。HINT整合了Hessian预训练、构型采样、课程学习和随机投影Hessian损失等关键技术，将Hessian标签需求降低了2-4个数量级。

Result: Hi-MLIP显著改进了过渡态搜索，使Gibbs自由能预测接近化学精度（尤其在数据稀缺情况下）。该方法还能准确处理强非谐氢化物，重现声子重整化和超导临界温度，与实验结果高度一致，同时避免了非谐计算的计算瓶颈。

Conclusion: 该框架为增强机器学习原子间势的曲率感知能力提供了一条实用途径，能够在广泛系统中桥接模拟和实验可观测值，为复杂系统的热力学和动力学现象分析开辟了新方向。
...
### [202] [GlowQ: Group-Shared LOw-Rank Approximation for Quantized LLMs](https://arxiv.org/abs/2603.25385)
*Selim An,Il hong Suh,Yeseong Kim*

Main category: cs.LG

TL;DR: GlowQ提出了一种分组共享低秩近似方法，用于量化大型语言模型，通过缓存共享右因子和选择性恢复关键层来平衡精度与效率。


<details>
  <summary>Details</summary>
Motivation: 现有量化技术（如BitsAndBytes、AWQ、GPTQ）在低比特表示（如4位）时会导致精度下降。现有的低秩校正方法（如LQER、QERA、ASER）虽然能缓解这一问题，但需要恢复所有层并在每个解码器块中插入误差校正模块，增加了延迟和内存开销。

Method: GlowQ采用分组共享低秩近似方法，为每个输入共享组缓存单个共享右因子，只恢复那些能带来最高精度收益的组或层。每个输入共享组计算一次高精度投影并在其模块中重复使用，减少参数和内存开销。还提出了选择性变体GlowQ-S，仅在提供最大收益的地方应用缓存的共享模块。
...
Conclusion: GlowQ通过分组共享低秩近似和选择性恢复策略，在保持量化模型精度的同时显著降低了延迟和内存开销，为高效部署大型语言模型提供了有效解决方案。
...
### [203] [Maximum Entropy Behavior Exploration for Sim2Real Zero-Shot Reinforcement Learning](https://arxiv.org/abs/2603.25464)
*Jiajun Hu,Nuria Armengol Urpi,Jin Cheng,Stelian Coros*

Main category: cs.LG

TL;DR: FB-MEBE算法结合无监督行为探索和正则化批评器，在线学习四足机器人控制策略，无需下游任务奖励函数即可部署到硬件


<details>
  <summary>Details</summary>
Motivation: 零样本强化学习需要预收集相关多样的数据集，但在没有下游任务先验知识的情况下难以收集高质量数据，导致策略性能不佳且难以直接部署到硬件

Method: 提出FB-MEBE算法：1) 无监督行为探索策略最大化行为分布的熵以促进探索；2) 正则化批评器引导策略产生更自然、物理上合理的行为

Result: FB-MEBE在模拟下游任务中优于其他探索策略，且产生的自然策略可直接部署到硬件而无需微调

Conclusion: FB-MEBE通过结合熵最大化和正则化批评器，有效解决了零样本RL中的探索挑战，实现了可直接硬件部署的自然四足机器人控制策略
...
### [204] [Not a fragment, but the whole: Map-based evaluation of data-driven Fire Danger Index models](https://arxiv.org/abs/2603.25469)
*Shahbaz Alvi,Italo Epicoco,Jose Maria Costa Saura*

Main category: cs.LG

TL;DR: 提出了一种新的森林火灾预报模型评估方法，该方法与真实世界决策相一致，并证明机器学习模型集成能同时提高火灾识别准确率和降低误报率。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法虽然能预测野火发生，但标准评估指标对火灾危险指数预测的操作性能衡量有限，且评估时常忽略误报率这一在操作环境中至关重要的因素。

Method: 重新审视每日火灾危险指数模型评估范式，提出与真实世界决策相一致的新评估方法，系统评估火灾活动准确预测和误报率，并采用机器学习模型集成方法。

Result: 机器学习模型集成能同时改善火灾识别能力和降低误报率，为森林火灾预报提供了更实用的评估框架。

Conclusion: 需要开发与操作决策更一致的火灾预报模型评估方法，机器学习模型集成是提高预测性能同时控制误报的有效策略。
...
### [205] [Causal-INSIGHT: Probing Temporal Models to Extract Causal Structure](https://arxiv.org/abs/2603.25473)
*Benjamin Redden,Hui Wang,Shuyan Li*

Main category: cs.LG

TL;DR: Causal-INSIGHT是一个模型无关的事后解释框架，用于从训练好的时序预测器中提取模型隐含的、有向的、时间滞后的影响结构。


<details>
  <summary>Details</summary>
Motivation: 理解多元时间序列中的有向时序交互对于解释复杂动态系统及其训练出的预测模型至关重要。现有方法通常关注数据生成过程的因果结构，而Causal-INSIGHT关注预测器本身所依赖的依赖关系。

Method: 采用模型无关、事后解释的方法，通过系统性的干预式输入钳制（在推理时应用）来分析固定预训练预测器的响应。从这些响应中构建有向时序影响信号，并引入Qbic准则（一种稀疏感知的图选择标准），在预测保真度和结构复杂度之间取得平衡，无需真实图标签。
...
### [206] [How Class Ontology and Data Scale Affect Audio Transfer Learning](https://arxiv.org/abs/2603.25476)
*Manuel Milling,Andreas Triantafyllopoulos,Alexander Gebhard,Simon Rampp,Björn W. Schuller*

Main category: cs.LG

TL;DR: 该研究系统分析了音频到音频迁移学习，发现预训练数据量和类别数对迁移效果有积极影响，但预训练任务与下游任务的相似性影响更大


<details>
  <summary>Details</summary>
Motivation: 尽管迁移学习在深度学习中广泛应用且效果显著，但其内在工作机制、何时有效以及为何有效等问题仍不明确。研究旨在深入理解音频领域迁移学习的工作原理和影响因素

Method: 在音频到音频迁移学习框架下，使用基于本体论的AudioSet子集预训练多种模型状态，然后在三个计算机听觉任务上进行微调：声学场景识别、鸟类活动识别和语音命令识别

Result: 研究发现：1）增加预训练数据的样本数量和类别数量对迁移学习有积极影响；2）预训练任务与下游任务的相似性对迁移效果影响更大，能够促使模型学习到可比的特征

Conclusion: 迁移学习效果不仅受预训练数据规模和多样性的影响，更关键的是预训练任务与目标任务的相似性，这为理解迁移学习机制提供了实证依据
...
### [207] [Interpretable PM2.5 Forecasting for Urban Air Quality: A Comparative Study of Operational Time-Series Models](https://arxiv.org/abs/2603.25495)
...
Method: 采用泄漏感知预测工作流，结合时间序列数据分区、预处理、特征选择和外生驱动建模。评估了SARIMAX、Facebook Prophet和NeuralProphet三种预测模型，并在两种自适应机制下测试：每周前向滚动重新拟合和冻结模型在线残差校正。

Result: 在前向滚动重新拟合机制下，Facebook Prophet表现最佳（MAE 37.61，RMSE 50.10），且计算时间远少于NeuralProphet。在冻结模型机制下，在线残差校正提升了Facebook Prophet和SARIMAX的性能，校正后的SARIMAX误差最低（MAE 32.50，RMSE 46.85）。NeuralProphet在两个机制下表现都不稳定，残差校正对其无效。
...
### [208] [Revisiting On-Policy Distillation: Empirical Failure Modes and Simple Fixes](https://arxiv.org/abs/2603.25562)
*Yuqian Fu,Haohuan Huang,Kaiwen Jiang,Yuanheng Zhu,Dongbin Zhao*

Main category: cs.LG

TL;DR: 本文重新审视了在线蒸馏(OPD)方法，发现传统的采样token变体在长序列任务中存在脆弱性，提出了基于教师top-K局部支持匹配的改进方法，在数学推理和智能体任务中表现更优。


<details>
  <summary>Details</summary>
Motivation: 在线蒸馏(OPD)在大语言模型后训练中很有吸引力，因为它基于学生生成的轨迹评估教师反馈。然而在长序列设置中，常见的采样token变体很脆弱：它将分布匹配简化为单token信号，并且随着轨迹偏离教师常见前缀而变得不可靠。

Method: 从估计器和实现两方面重新审视OPD。理论分析token级OPD相对于序列级反向KL的偏差和方差权衡。提出教师top-K局部支持匹配方法，实现为带top-p轨迹采样和特殊token掩码的截断反向KL。

Result: 在单任务数学推理和多任务智能体+数学训练中，新目标比采样token OPD产生更稳定的优化和更好的下游性能。玩具研究显示更强的未来奖励耦合会产生更高的梯度方差和更不稳定的学习。

Conclusion: 采样token OPD存在三个失效模式：不平衡的单token信号、对学生生成前缀的不可靠教师指导、以及分词器或特殊token不匹配导致的失真。通过教师top-K局部支持匹配可以解决这些问题，实现更稳定有效的在线蒸馏。
...
### [209] [Uncertainty-Guided Label Rebalancing for CPS Safety Monitoring](https://arxiv.org/abs/2603.25670)
*John Ayotunde,Qinghua Xu,Guancheng Wang,Lionel C. Briand*

Main category: cs.LG

TL;DR: U-Balance：利用行为不确定性重新平衡CPS安全监测中的极端类别不平衡数据集，通过不确定性指导的标签重新平衡机制提升安全预测器性能


<details>
  <summary>Details</summary>
Motivation: 现实世界CPS操作中不安全事件稀少导致极端类别不平衡，传统重平衡技术对时间序列CPS遥测数据效果不佳，而CPS行为不确定性虽与安全结果相关但尚未在安全监测中探索

Method: 提出U-Balance方法：1)训练基于GatedMLP的不确定性预测器，将遥测窗口总结为分布运动学特征并输出不确定性分数；2)应用不确定性指导的标签重新平衡机制，将高不确定性的安全标签窗口概率性重新标记为不安全；3)在重新平衡的数据集上训练安全预测器

Result: 在46:1安全与不安全比例的大规模无人机基准测试中，U-Balance达到0.806 F1分数，比最强基线提升14.3个百分点，同时保持有竞争力的推理效率；确认行为不确定性与安全存在中度但显著相关性

Conclusion: U-Balance通过利用行为不确定性重新平衡不平衡数据集，有效解决了CPS安全监测中的极端类别不平衡问题，不确定性指导的标签重新平衡机制是比直接融合更有效的策略
...
### [210] [A Unified Memory Perspective for Probabilistic Trustworthy AI](https://arxiv.org/abs/2603.25692)
...
<details>
  <summary>Details</summary>
Motivation: 可信AI系统越来越依赖概率计算来实现鲁棒性、可解释性、安全性和隐私保护。实际系统中，这些工作负载在模型、数据路径和系统功能之间交替进行确定性数据访问和重复随机采样，将性能瓶颈从算术单元转移到必须同时提供数据和随机性的内存系统。

Method: 提出统一的数据访问视角，将确定性访问视为随机采样的极限情况，使两种模式能在共同框架下分析。基于此视角定义内存级评估标准，包括统一操作、分布可编程性、效率、对硬件非理想性的鲁棒性以及并行兼容性。

Result: 分析发现增加随机性需求会降低有效数据访问效率，并可能驱动系统进入熵受限操作。通过提出的评估标准分析了传统架构的局限性，并研究了新兴的概率存内计算方法，这些方法将采样与内存访问集成在一起。

Conclusion: 论文为可信AI的可扩展硬件设计提供了新视角和评估框架，指出了将采样与内存访问集成的概率存内计算方法是实现高效可信AI系统的有前景途径。
...
### [212] [ARC-AGI-3: A New Challenge for Frontier Agentic Intelligence](https://arxiv.org/abs/2603.24621)
*ARC Prize Foundation*

Main category: cs.AI

TL;DR: ARC-AGI-3是一个用于研究智能体智能的交互式基准测试，通过新颖的、抽象的、回合制环境评估智能体在没有明确指令情况下的探索、目标推断、环境建模和规划能力。


<details>
  <summary>Details</summary>
Motivation: 研究智能体智能，特别是评估智能体在全新任务中的流体自适应效率，避免语言和外部知识的影响，专注于核心认知能力。

Method: 设计基于核心知识先验的抽象回合制环境，通过大量人类测试者进行难度校准，建立基于人类行动基准的效率评分框架。

Result: 人类可以解决100%的环境任务，而截至2026年3月的前沿AI系统得分低于1%，显示出巨大的性能差距。

Conclusion: ARC-AGI-3提供了一个有效的基准来评估智能体智能，揭示了当前AI系统在流体自适应能力方面与人类存在显著差距。
...
### [213] [Trust as Monitoring: Evolutionary Dynamics of User Trust and AI Developer Behaviour](https://arxiv.org/abs/2603.24742)
...
TL;DR: 该研究使用演化博弈论分析AI安全治理，将用户信任建模为重复互动中的监控减少，发现只有高透明度、低成本监控和有效惩罚才能实现安全AI的广泛采用。


<details>
  <summary>Details</summary>
Motivation: 现有AI治理模型通常将用户信任视为一次性采纳决策，而非由重复互动塑造的动态演化过程。随着AI能力和采用率的增长，AI安全问题日益紧迫，需要更精细的信任演化模型来理解如何实现安全AI的广泛采用。

Method: 使用演化博弈论建模用户与AI开发者之间的重复非对称互动，将用户信任策略和开发者选择（安全vs不安全AI）共同演化。结合无限种群复制者动态分析、随机有限种群动态和强化学习（Q-learning）模拟，在不同监控成本和制度机制下研究演化结果。

Result: 研究发现三个稳健的长期演化机制：1）不安全开发且无采用；2）不安全但广泛采用的系统；3）安全且广泛采用的系统。只有第三种是理想的，它出现在对不安全行为的惩罚超过安全额外成本，且用户仍能负担偶尔监控的情况下。

Conclusion: 研究结果正式支持强调透明度、低成本监控和有意义制裁的治理提案，表明仅靠监管或盲目用户信任都不足以防止向不安全或低采用结果的演化漂移。有效治理需要结合可负担的监控能力和实质性惩罚。
...
### [214] [Formal Semantics for Agentic Tool Protocols: A Process Calculus Approach](https://arxiv.org/abs/2603.24747)
*Andreas Schlapbach*

Main category: cs.AI

TL;DR: 该论文首次对SGD和MCP两种智能体协议进行了形式化验证，证明它们在结构上是双相似的，但反向映射存在信息损失，揭示了MCP的表达能力缺陷，并提出了MCP+扩展来实现完全的行为等价。


<details>
  <summary>Details</summary>
Motivation: 随着能够调用外部工具的大语言模型智能体的出现，对智能体协议进行形式化验证的需求日益迫切。SGD（模式引导对话）和MCP（模型上下文协议）是两种主流范式，虽然都通过模式描述实现动态服务发现，但它们的正式关系尚未被探索。

Method: 基于先前关于这两种范式概念收敛的研究，首次提出了SGD和MCP的过程演算形式化，证明它们在明确定义的映射Φ下是结构双相似的。通过双向分析，识别了实现完全行为等价所需的五个原则，并将这些原则形式化为类型系统扩展MCP+。

Result: 研究发现SGD和MCP在结构上是双相似的，但反向映射Φ^{-1}是部分且有损的，揭示了MCP在表达能力上的关键缺陷。提出的五个原则（语义完整性、明确动作边界、故障模式文档、渐进披露兼容性和工具间关系声明）是实现完全行为等价的充要条件，MCP+被证明与SGD是同构的。

Conclusion: 该研究为验证智能体系统提供了首个形式化基础，并将模式质量确立为可证明的安全属性。MCP+扩展实现了与SGD的完全行为等价，为智能体协议的安全验证提供了理论框架。
...
### [215] [ReLope: KL-Regularized LoRA Probes for Multimodal LLM Routing](https://arxiv.org/abs/2603.24787)
...
<details>
  <summary>Details</summary>
Motivation: 在纯文本LLM中，探针路由通过预测小模型隐藏状态中的正确性来平衡性能与成本，但在多模态LLM中，视觉输入会削弱隐藏状态中正确性信号的可分离性，导致现有探针方法效果显著下降。

Method: 提出两种互补方法：1) 注意力探针：基于注意力分数聚合前一层隐藏状态来恢复分布式的正确性信号；2) KL正则化LoRA探针：插入轻量级LoRA适配器并应用KL正则化器来学习路由感知的表征。

Result: 综合实验表明，所提方法在多个基准测试中一致优于基线方法，验证了改进隐藏状态质量对MLLM中有效路由的关键作用。

Conclusion: 提高隐藏状态的质量是多模态大语言模型中实现有效路由的关键，提出的注意力探针和KL正则化LoRA探针为解决视觉输入导致的正确性信号可分离性问题提供了有效解决方案。
...
### [216] [Resisting Humanization: Ethical Front-End Design Choices in AI for Sensitive Contexts](https://arxiv.org/abs/2603.24853)
*Silvia Rossi,Diletta Huyskes,Mackenzie Jorgensen*

Main category: cs.AI

TL;DR: 论文认为AI前端设计的人性化选择具有伦理意义，会影响用户心智模型、信任校准和行为反应，并以Chayn组织为例说明伦理考虑如何促使界面设计的克制


<details>
  <summary>Details</summary>
Motivation: 当前AI伦理讨论主要关注后端问题（数据治理、模型训练、算法决策），而前端设计选择（交互方式、表现形式）的伦理意义被忽视。对于基于NLP的对话用户界面，人性化设计元素日益普遍，但缺乏对其伦理影响的系统分析

Method: 结合人机交互、对话AI和价值敏感设计的研究，分析界面如何影响用户期望、信任和自主性。通过非营利组织Chayn的两个AI系统案例研究，展示创伤知情设计原则如何指导伦理界面设计

Result: 研究发现前端人性化设计是价值驱动的选择，可能误导用户期望、培养错误信任、削弱用户自主性，特别是在脆弱情境下。Chayn案例表明伦理考虑可以促使设计克制，挑战当前AI产品以参与度为导向的规范

Conclusion: 伦理前端AI设计是一种程序性伦理，通过交互选择而非仅系统逻辑来实现。需要更重视前端设计的伦理影响，特别是在脆弱用户群体中，设计克制可能是更伦理的选择
...
### [217] [How Far Are Vision-Language Models from Constructing the Real World? A Benchmark for Physical Generative Reasoning](https://arxiv.org/abs/2603.24866)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型评估过于偏重视觉真实性，忽略了模型理解构建过程所需的逐步流程和物理依赖关系，这对于自动化设计到建造流程至关重要。

Method: 基于住宅木结构建筑领域建立基准，包含13种建筑风格的26,000多个已验证结构，开发了确定性的10项结构验证框架，支持模型观察中间建造状态、生成建造动作并接收结构化环境反馈的迭代式智能体交互。

Result: 对最先进视觉语言模型的广泛实验揭示了现有排行榜上不可见的能力差距，表明物理有效性是独立于视觉真实性的关键评估维度。

Conclusion: 物理生成推理是多模态智能中一个独特且尚未充分发展的前沿领域，DreamHouse基准为评估模型在物理约束下的生成能力提供了新方向。
...
### [218] [Decoding Market Emotions in Cryptocurrency Tweets via Predictive Statement Classification with Machine Learning and Transformers](https://arxiv.org/abs/2603.24933)
*Moein Shahiki Tash,Zahra Ahani,Mohim Tash,Mostafa Keikhay Farzaneh,Ari Y. Barrera-Animas,Olga Kolesnikova*

Main category: cs.AI

TL;DR: 提出用于加密货币推文预测性陈述分类的两阶段框架，结合人工与GPT标注、情感特征提取和数据增强，评估多种模型性能


<details>
  <summary>Details</summary>
Motivation: 随着加密货币日益普及，社交媒体上的投机活动增加，需要有效识别预测性陈述以理解市场情绪和预测模式

Method: 两阶段分类框架：任务1进行预测性/非预测性二元分类，任务2将预测性推文细分为增量、减量、中性三类；结合人工与GPT标注，使用SenticNet提取情感特征，通过GPT生成改写进行数据增强以解决类别不平衡

Result: GPT平衡显著提升模型性能，Transformer模型在任务1中取得最高F1分数，传统机器学习模型在任务2中表现最佳；情感分析显示不同加密货币的预测类别具有独特的情感模式

Conclusion: 提出的分类框架能有效识别加密货币推文中的预测性陈述，GPT增强方法改善模型性能，情感分析揭示了预测类别与情感特征之间的关联
...
### [219] [Shopping with a Platform AI Assistant: Who Adopts, When in the Journey, and What For](https://arxiv.org/abs/2603.24947)
*Se Yan,Han Zhong,Zemin,Zhong,Wenyu Zhou*

Main category: cs.AI

TL;DR: 研究Ctrip平台上3100万用户使用LLM购物助手"Wendao"的行为模式，发现AI助手主要作为传统搜索的补充工具，用于探索性产品发现而非替代搜索


<details>
  <summary>Details</summary>
Motivation: 了解消费者如何采用和使用电商平台嵌入的购物AI助手，填补大规模实证研究的空白，探索AI助手在电商购物旅程中的实际作用

Method: 使用中国最大在线旅游平台Ctrip的3100万用户数据，分析LLM助手"Wendao"的使用模式，包括用户画像、使用时机、与搜索的交互关系以及查询意图分类

Result: 1. 采用者特征：年长消费者、女性用户和高参与度现有用户采用率最高，与通用AI工具年轻男性主导的模式相反；2. AI聊天出现在与搜索相同的购买阶段，且早于下单，常见模式是两种方式交替使用；3. 用户主要用AI助手处理探索性、难以关键词化的任务，景点查询占聊天请求的42%
...
### [220] [Can MLLMs Read Students' Minds? Unpacking Multimodal Error Analysis in Handwritten Math](https://arxiv.org/abs/2603.24961)
*Dingjie Song,Tianlong Xu,Yi-Fan Zhang,Hang Li,Zhiling Yan,Xing Fan,Haoyang Li,Lichao Sun,Qingsong Wen*

Main category: cs.AI

TL;DR: ScratchMath是一个专门用于解释和分类学生手写数学草稿错误的基准数据集，包含1720个中国中小学生数学样本，支持错误原因解释和分类两个任务，评估发现现有MLLMs在视觉识别和逻辑推理方面与人类专家存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 评估学生手写草稿对于个性化教育反馈至关重要，但现有教育NLP主要关注文本响应，忽略了手写草稿的复杂性和多模态特性。当前多模态大语言模型(MLLMs)擅长视觉推理，但通常采用"考生视角"，优先生成正确答案而非诊断学生错误。

Method: 引入ScratchMath基准数据集，包含1720个中国中小学生数学手写样本，支持错误原因解释(ECE)和错误原因分类(ECC)两个任务，定义了七种错误类型。数据集通过严格的人机协作方法进行标注，涉及专家标注、审查和验证多个阶段。系统评估了16个领先的MLLMs。

Result: 评估显示MLLMs与人类专家存在显著性能差距，特别是在视觉识别和逻辑推理方面。专有模型明显优于开源模型，大型推理模型在错误解释方面表现出强大潜力。

Conclusion: ScratchMath填补了手写数学草稿评估的空白，揭示了当前MLLMs在教育诊断任务中的局限性，为未来研究提供了公开的评估数据和框架，促进了教育AI的发展。
...
### [221] [The Anatomy of Uncertainty in LLMs](https://arxiv.org/abs/2603.24967)
...
<details>
  <summary>Details</summary>
Motivation: 当前的不确定性评估方法（单一不确定性分数或经典二分法）无法为改进生成模型提供可操作的见解，且不足以理解LLM中的不确定性。需要更细粒度的分解来审计LLM可靠性和检测幻觉。

Method: 提出了一个不确定性分解框架，将LLM不确定性分解为三个语义组件：1) 输入歧义（由模糊提示引起）；2) 知识缺口（由参数证据不足引起）；3) 解码随机性（由随机采样引起）。通过一系列实验验证该框架。

Result: 实验表明这些组件在不同模型规模和任务中的主导地位会发生变化。该框架为审计LLM可靠性和检测幻觉提供了更好的理解，有助于针对性干预和构建更可信的系统。

Conclusion: 提出的三组件不确定性分解框架比现有方法提供了更细粒度和可操作的见解，能够更好地理解LLM不确定性，为改进模型可靠性和检测幻觉铺平道路。
...
### [222] [Rethinking Failure Attribution in Multi-Agent Systems: A Multi-Perspective Benchmark and Evaluation](https://arxiv.org/abs/2603.25001)
...
TL;DR: 本文提出多视角故障归因范式，解决多智能体系统中故障归因的模糊性问题，并引入首个多视角故障归因基准MP-Bench和相应评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统故障归因方法和基准大多假设每个故障只有一个确定性根本原因，但实际中由于复杂的智能体间依赖关系和模糊的执行轨迹，故障往往存在多种合理的归因解释。需要重新审视多智能体系统故障归因问题，考虑归因的模糊性。

Method: 提出多视角故障归因范式，明确考虑归因的模糊性；引入MP-Bench基准，这是首个专门为多智能体系统多视角故障归因设计的基准；提出适用于该范式的新评估协议。

Result: 通过大量实验发现，先前认为大语言模型在故障归因方面表现不佳的结论主要是由于现有基准设计的局限性所致。研究结果强调了多视角基准和评估协议对于实现现实可靠的多智能体系统调试的必要性。

Conclusion: 多视角故障归因是解决多智能体系统故障归因模糊性的实用范式，MP-Bench基准和相应评估协议为这一领域提供了必要的工具，有助于更现实、更可靠的多智能体系统调试。
...
### [223] [A Public Theory of Distillation Resistance via Constraint-Coupled Reasoning Architectures](https://arxiv.org/abs/2603.25022)
*Peng Wei,Wesley Shu*

Main category: cs.AI

TL;DR: 本文提出了一种理论框架，旨在通过架构设计减少模型能力蒸馏与治理结构之间的不对称性，核心观点是将高层能力与内部稳定性约束耦合，使蒸馏变得不那么有价值。


<details>
  <summary>Details</summary>
Motivation: 知识蒸馏、模型提取和行为转移已成为前沿AI的核心问题。主要风险不仅在于复制，而在于有用的能力可能比伴随其的治理结构更便宜地被转移。本文旨在减少这种不对称性。

Method: 提出一个约束耦合推理框架，包含四个要素：有界转移负担、路径负载积累、动态演化的可行区域，以及能力-稳定性耦合条件。该框架是公开安全的，省略了专有实现细节。

Result: 本文提供了一个理论贡献而非操作实现，包括一个可证伪的架构论点、清晰的威胁模型，以及一组可实验验证的假设，用于未来蒸馏抵抗、对齐和模型治理的研究。

Conclusion: 通过将高层能力与内部稳定性约束耦合，可以降低蒸馏作为捷径的价值，从而在架构层面减少能力转移与治理结构之间的不对称性，为AI安全提供理论框架。
...
### [224] [System-Anchored Knee Estimation for Low-Cost Context Window Selection in PDE Forecasting](https://arxiv.org/abs/2603.25025)
*Wenshuo Wang,Fan Zhang*

Main category: cs.AI

TL;DR: 提出SAKE方法解决自回归神经PDE模拟器中低成本的上下文窗口选择问题，通过系统锚点和膝点感知选择实现高性能低成本窗口选择


<details>
  <summary>Details</summary>
Motivation: 自回归神经PDE模拟器需要从有限历史中预测物理场演化，但现有的上下文窗口选择方法要么昂贵、脆弱，要么与下游滚动性能不直接对齐，缺乏正式的低成本解决方案

Method: 提出系统锚定膝点估计(SAKE)两阶段方法：1)从物理可解释的系统锚点识别小型结构化候选集；2)在候选集中进行膝点感知的下游选择

Result: 在PDEBench八个家族评估中，SAKE是最强的匹配预算低成本选择器，达到67.8%精确率、91.7%±1窗口内准确率、6.1%平均遗憾率，成本比仅0.051（节省94.9%搜索成本）

Conclusion: SAKE方法成功将上下文窗口选择形式化为独立的低成本算法问题，通过系统锚点和膝点感知选择实现了高性能低成本的窗口选择方案
...
### [225] [From Stateless to Situated: Building a Psychological World for LLM-Based Emotional Support](https://arxiv.org/abs/2603.25031)
*Boning Zhao,Clover Hu,Xinnuo Li*

Main category: cs.AI

TL;DR: LEKIA 2.0提出了一种情境化LLM架构，通过分离认知层和执行层来解决情感支持中多轮对话的连续性、阶段意识和用户边界问题，相比仅提示的基线方法在深度干预循环完成度上提升了约31%。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在心理支持和情感陪伴场景中的核心局限在于其基于局部下一个token预测的机制，无法维持多轮干预所需的时间连续性、阶段意识和用户同意边界，导致系统容易出现过早推进、阶段错位和边界侵犯等问题。

Method: 提出LEKIA 2.0情境化LLM架构，将认知层与执行层分离，从而解耦情境建模与干预执行。该设计使系统能够在持续交互中保持对用户情境和同意边界的稳定表示，并引入了从静态到动态的在线评估协议来评估多轮交互的过程控制能力。

Result: LEKIA在深度干预循环完成方面相比仅使用提示的基线方法实现了约31%的平均绝对提升，表明外部情境结构是构建稳定、可控、情境化情感支持系统的关键使能条件。

Conclusion: 过程导向情感支持的关键挑战不是简单地生成自然语言，而是为模型构建可持续更新的外部情境结构。LEKIA 2.0通过分离认知层和执行层的架构设计，有效解决了多轮情感支持中的连续性、阶段意识和边界控制问题。
...
### [226] [Mechanistically Interpreting Compression in Vision-Language Models](https://arxiv.org/abs/2603.25035)
...
<details>
  <summary>Details</summary>
Motivation: 压缩视觉语言模型虽然能降低内存和计算成本，但压缩过程可能改变模型的内部计算和安全行为，需要研究不同压缩方法如何影响模型内部机制

Method: 使用因果电路分析和跨编码器特征比较方法，研究剪枝和量化如何改变代表性VLMs的内部机制；引入VLMSafe-420基准测试，包含有害输入和匹配的良性反事实

Result: 剪枝通常保持电路结构完整但旋转和衰减内部特征，量化修改高层电路但使存活特征更好对齐；剪枝导致真实拒绝行为急剧下降

Conclusion: 压缩方法的选择对模型安全性有重要影响，剪枝会显著损害模型的拒绝行为，而量化在特征对齐方面表现更好
...
### [227] [MP-MoE: Matrix Profile-Guided Mixture of Experts for Precipitation Forecasting](https://arxiv.org/abs/2603.25046)
*Huyen Ngoc Tran,Dung Trung Tran,Hong Nguyen,Xuan Vu Phan,Nam-Phong Nguyen*

Main category: cs.AI

TL;DR: 提出MP-MoE框架，通过结合矩阵轮廓目标函数改善热带地区降水预报，减少相位偏移导致的"双重惩罚"问题
...
Method: 提出矩阵轮廓引导的专家混合框架（MP-MoE），将传统强度损失与结构感知的矩阵轮廓目标函数相结合。利用子序列级相似性而非逐点误差，实现更可靠的专家选择并减轻相位偏移导致的过度惩罚

Result: 在越南两个主要流域的降雨数据集上评估，涵盖1小时强度及12、24、48小时累积降雨。MP-MoE在强降雨事件的平均临界成功指数（CSI-M）上优于原始NWP和基线学习方法，同时显著降低动态时间规整值
...
### [228] [Sparse Visual Thought Circuits in Vision-Language Models](https://arxiv.org/abs/2603.25075)
*Yunpeng Zhou*

Main category: cs.AI

TL;DR: 稀疏自编码器（SAE）特征在视觉语言模型中并不总是形成可组合的模块化单元，特征联合干预会导致输出漂移和精度下降，揭示了SAE特征可组合性的局限性。


<details>
  <summary>Details</summary>
Motivation: 虽然稀疏自编码器（SAEs）能提升多模态模型的可解释性，但尚不清楚SAE特征是否真正形成模块化、可组合的推理单元——这是许多基于干预的引导方法的基本假设。

Method: 开发了一个可复现的因果分析流程，在Qwen3-VL-8B模型中定位和测试稀疏视觉思维回路。使用包含七种任务类型和三个难度级别的合成基准，通过线性探针识别任务类型信息的中层解码器位置，在该层训练SAEs，通过显式规则构建任务选择性特征集，并在推理时进行缩放和消融干预，同时量化精度和漂移。

Result: 模块化假设经常失败：对任务选择性特征集的干预只能适度提升推理精度，而对两个特征集的联合干预会可靠地引起输出漂移（预测的大幅意外变化）并降低精度，即使在规范匹配的扰动下也是如此。这种非模块化回路干扰与共享内部通路一致，其中特征联合会放大激活偏移。

Conclusion: 研究结果通过引导子样本和置换控制验证，并在多个VLM家族和五个不同数据集上复现，明确了SAE特征可组合性的边界，为更可靠的VLM控制提供了严格的诊断框架。
...
TL;DR: CaT-FSCIL提出了一种处理少样本类增量学习中采集上下文影响的方法，通过将上下文分解为结构化低维和高维残差分量，分别用逆变换规范化和不确定性条件原型校准来解决。


<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习在只有少量标注样本的采集上下文中特别容易受到影响。触觉传感等场景中，采集上下文（如不同设备、接触状态和交互设置）由于缺乏标准化会降低性能。
...
Conclusion: 提出的CaT-FSCIL方法有效解决了少样本类增量学习中采集上下文的影响问题，通过上下文分解和针对性处理策略提升了模型性能。
...
TL;DR: RubricEval：首个针对指令跟随评估的细粒度评分标准级别元评估基准，揭示了当前评分标准级别评估仍远未解决，GPT-4o在困难子集上仅达到55.97%准确率
...
Method: 提出RubricEval基准，包含：(1)首个指令跟随的评分标准级别元评估基准；(2)涵盖多个类别和模型来源的多样化指令和响应；(3)3,486个质量控制实例，以及能更好区分评估者性能的简单/困难子集。

Result: 实验显示评分标准级别评估远未解决：即使是广泛使用的GPT-4o评估者，在困难子集上仅达到55.97%准确率。评分标准级别评估优于检查清单级别，显式推理提高准确性，两者结合减少评估者间方差。

Conclusion: 通过建立的评分标准分类法，识别常见失败模式，为可靠的指令跟随评估提供可行见解，强调需要改进评分标准级别的评估方法。
...
### [231] [The Competence Shadow: Theory and Bounds of AI Assistance in Safety Engineering](https://arxiv.org/abs/2603.25197)
*Umair Siddique*

Main category: cs.AI

TL;DR: AI辅助安全分析存在"能力阴影"风险：AI生成的建议会系统性限制人类思考范围，导致安全分析质量下降而非提升，这种退化效应远超简单叠加估计


<details>
  <summary>Details</summary>
Motivation: 随着AI助手融入物理AI系统的安全工程工作流，需要评估AI辅助是否真正提升安全分析质量，还是会引入系统性盲点，这些盲点可能只在部署后的事故中显现

Method: 建立安全工程抵抗基准评估的正式框架，提出五维能力框架（领域知识、标准专业知识、操作经验、上下文理解、判断力），定义"能力阴影"概念，形式化四种典型人机协作结构并推导闭式性能边界

Result: 能力阴影以乘法方式复合，导致分析质量退化远超简单的加法估计。AI辅助安全工程是协作设计问题而非软件采购决策，同一工具根据使用方式可能提升或降低分析质量

Conclusion: 需要从工具认证转向工作流认证，确保可信物理AI。推导了抗阴影工作流的非退化条件，强调安全工程中AI辅助的关键在于协作设计而非工具本身
...
### [232] [A Gait Foundation Model Predicts Multi-System Health Phenotypes from 3D Skeletal Motion](https://arxiv.org/abs/2603.25283)
...
TL;DR: 研究人员开发了一个步态基础模型，通过3D骨骼运动分析步态作为系统性生物标志物，能够预测多种表型和临床指标，优于传统工程特征。


<details>
  <summary>Details</summary>
Motivation: 步态越来越被认为是重要的生命体征，但目前的方法仅将其视为特定病理的症状，而非系统性生物标志物。研究人员旨在将步态开发为可扩展的被动生命体征。

Method: 开发了一个步态基础模型，基于3,414名深度表型成年人的3D骨骼运动数据，通过深度相机记录五种运动任务。使用学习嵌入而非工程特征进行分析。

Result: 学习嵌入优于工程特征，能预测年龄(r=0.69)、BMI(r=0.90)和内脏脂肪面积(r=0.82)。步态嵌入显著预测了3,210个表型目标中的1,980个，在调整年龄、BMI、VAT和身高后，步态为男性所有18个身体系统、女性17个系统提供了独立增益，并改善了临床诊断和药物使用的预测。

Conclusion: 步态是一个独立的多系统生物信号，腿部主导代谢和衰弱预测，躯干编码睡眠和生活方式表型。这些发现支持将步态转化为消费级视频应用，并将其整合为可扩展的被动生命体征。
...
### [233] [SliderQuant: Accurate Post-Training Quantization for LLMs](https://arxiv.org/abs/2603.25284)
*Shigeng Wang,Chao Li,Yangyuxuan Kang,Jiawei Fan,Zhonghong Ou,Anbang Yao*

Main category: cs.AI

TL;DR: 提出SliderQuant框架，通过自适应滑动量化解决LLM后训练量化中不同层对量化敏感度差异的问题，在多种任务和模型上优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有LLM后训练量化方法通常对所有层采用相同的量化策略，但研究发现浅层和深层对量化更敏感，特别是第一层和最后一层，需要针对不同层的敏感度设计多层次的量化方案

Method: 提出SliderQuant框架，包含层间滑动量化和层内滑动量化两个组件。层间滑动量化采用三种滑动窗口设计分别处理浅层、中间层和深层的不同量化敏感度；层内滑动量化使用增量策略量化每个窗口

Result: 在基础语言生成、零样本常识推理以及具有挑战性的数学和代码任务上，使用Llama/Llama2/Llama3/Qwen2.5模型族、DeepSeek-R1蒸馏模型和大规模MoE模型进行实验，SliderQuant在仅权重量化和权重-激活量化上都优于现有PTQ方法（包括使用旋转变换的最新方法）

Conclusion: SliderQuant通过自适应滑动量化有效减少各层的量化误差，为LLM后训练量化提供了一种更优的解决方案，特别适用于具有挑战性的低比特位宽设置
...
### [234] [DAGverse: Building Document-Grounded Semantic DAGs from Scientific Papers](https://arxiv.org/abs/2603.25293)
...
Method: 利用包含明确DAG图的科学论文作为监督源，DAGverse-Pipeline半自动系统通过图分类、图重建、语义基础和验证四个步骤构建文档基础的语义DAG。

Result: 发布DAGverse-1数据集，包含108个专家验证的语义DAG，具有图级、节点级和边级证据。实验显示DAGverse-Pipeline在DAG分类和标注上优于现有视觉语言模型。

Conclusion: DAGverse为文档基础的DAG基准提供了基础，并为基于现实世界证据的结构化推理研究开辟了新方向。
...
### [235] [Evaluating Language Models for Harmful Manipulation](https://arxiv.org/abs/2603.25326)
...
TL;DR: 该研究提出了一个评估AI有害操纵的框架，通过跨领域、跨地域的人机交互实验发现AI确实能够产生操纵行为并改变用户信念和行为，且操纵效果因领域和地域而异。
...
Method: 提出了一个基于具体情境的人机交互研究框架，通过10,101名参与者在三个AI应用领域（公共政策、金融、健康）和三个地区（美国、英国、印度）进行实验评估。

Result: 测试模型在提示下能够产生操纵行为，并在实验环境中成功诱导参与者的信念和行为改变；操纵效果在不同领域和地域存在显著差异；AI的操纵倾向性并不总是能预测操纵成功率。

Conclusion: 需要在高风险应用场景中评估AI操纵能力，不同地域的结果不可简单推广，操纵倾向性和有效性应分开研究，并提供了测试协议和材料以促进框架的采用。
...
### [236] [Macroscopic Characteristics of Mixed Traffic Flow with Deep Reinforcement Learning Based Automated and Human-Driven Vehicles](https://arxiv.org/abs/2603.25328)
*Pankaj Kumar,Pranamesh Chakraborty,Subrahmanya Swamy Peruru*

Main category: cs.AI

TL;DR: 本文研究了基于深度强化学习（DRL）的自动驾驶车辆在混合交通中的宏观交通流特性和燃油效率表现，发现相比传统IDM模型，DRL能提升约7.52%的道路容量，并在高速时提升约28.98%的燃油效率。
...
Method: 使用Twin Delayed Deep Deterministic Policy Gradient (TD3)算法控制自动驾驶车辆，基于NGSIM高速公路数据集训练，使其能与人类驾驶车辆进行真实交互。通过基本图（FD）评估交通性能，考虑驾驶员异质性、安全时距渗透水平和RL车辆比例等因素，并与IDM模型进行宏观燃油效率比较。

Result: 交通性能对安全时距分布和RL车辆比例敏感。从完全人类驾驶过渡到完全RL控制交通可提升约7.52%的道路容量。RL自动驾驶车辆在高速（>50 km/h）时燃油效率比IDM提升约28.98%，低速（<50 km/h）时提升约1.86%。
...
### [237] [4OPS: Structural Difficulty Modeling in Integer Arithmetic Puzzles](https://arxiv.org/abs/2603.25356)
*Yunus E. Zeytuncu*
...
### [238] [Does Structured Intent Representation Generalize? A Cross-Language, Cross-Model Empirical Study of 5W3H Prompting](https://arxiv.org/abs/2603.25379)
*Peng Gang*

Main category: cs.AI

TL;DR: 该研究扩展了基于5W3H的PPS结构化意图表示框架，验证其在英语和日语中的泛化能力，发现AI辅助扩展的5W3H提示与人工编写的效果相当，且结构化表示能改善意图对齐并降低跨模型输出方差。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证结构化意图表示框架PPS（基于5W3H）是否能在不同语言和模型中泛化，扩展先前仅针对中文的证据，并探索AI辅助界面如何降低非专家用户的使用门槛。

Method: 研究采用PPS框架，在三个语言（中文、英语、日语）、四种条件（包括AI辅助扩展5W3H）、三种大语言模型上进行实验，共收集2,160个模型输出，通过统计方法分析目标对齐和跨模型输出一致性。

Result: AI扩展的5W3H提示与人工编写的在目标对齐上无显著差异；结构化条件能减少或重塑跨模型输出方差；非结构化提示存在系统性双重膨胀偏差：人为高综合分数和人为低表观跨模型方差。

Conclusion: 结构化5W3H表示能跨语言和模型改善意图对齐和可访问性，特别是当AI辅助创作降低非专家用户门槛时，但效果在不同语言和指标上并不均匀。
...
### [239] [Beyond Content Safety: Real-Time Monitoring for Reasoning Vulnerabilities in Large Language Models](https://arxiv.org/abs/2603.25412)
*Xunguang Wang,Yuguang Zhou,Qingyue Wang,Zongjie Li,Ruixuan Huang,Zhenlan Ji,Pingchuan Ma,Shuai Wang*

Main category: cs.AI

TL;DR: 该论文提出"推理安全性"新概念，针对大语言模型的思维链推理过程，建立了九类不安全推理行为分类，开发了实时监控系统，在检测准确率上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型安全研究主要关注输出内容的安全性，而忽略了推理过程本身的安全性。论文认为推理安全性是一个同等重要的安全维度，需要确保模型的推理轨迹在逻辑一致性、计算效率和对抗攻击抵抗力方面的安全。

Method: 1. 正式定义推理安全性并建立九类不安全推理行为分类法；2. 对4111个推理链进行大规模实证研究；3. 提出推理安全监控器，作为外部LLM组件实时检查推理步骤，检测到不安全行为时发送中断信号。

Result: 实证研究发现所有九类错误类型在实践中都存在，且每种攻击都会产生机制上可解释的特征。在450链静态基准测试中，监控器达到84.88%的步骤级定位准确率和85.37%的错误类型分类准确率，显著优于幻觉检测器和过程奖励模型基线。

Conclusion: 推理级监控既是必要的也是实际可行的，推理安全性应成为大型推理模型安全部署的基础性关注点。该研究为推理过程的安全性监控提供了理论基础和实践方案。
...
### [240] [Modernising Reinforcement Learning-Based Navigation for Embodied Semantic Scene Graph Generation](https://arxiv.org/abs/2603.25415)
*Roman Kueble,Marco Hueller,Mrunmai Phatak,Rainer Lienhart,Joerg Haehner*

Main category: cs.AI

TL;DR: 该研究改进了用于具身语义场景图生成的导航组件，通过优化算法替换、细化动作表示和策略设计，在有限动作预算下提升语义世界模型的构建质量和效率。


<details>
  <summary>Details</summary>
Motivation: 语义世界模型对于具身智能体在不确定性和资源约束下的目标驱动自适应至关重要。核心挑战是在有限动作预算内最大化模型质量和下游实用性。语义场景图为此提供了结构化紧凑表示，但需要在有限动作范围内权衡信息增益与导航成本。

Method: 1. 替换策略优化方法；2. 重新审视离散动作表述，研究紧凑和细粒度的离散动作集；3. 比较原子动作的单头策略与动作组件的分解多头策略；4. 评估课程学习和基于深度的可选碰撞监督。

Result: 1. 仅替换优化算法就使SSG完整性相对基线提升21%；2. 深度主要影响执行安全性（无碰撞运动），完整性基本不变；3. 现代优化与细粒度分解动作表示结合产生最强的完整性-效率权衡。

Conclusion: 通过现代优化算法与细粒度分解动作表示的结合，能够实现最佳的语义场景图完整性与导航效率平衡，为具身智能体在资源约束下的语义世界模型构建提供了有效解决方案。
...
### [241] [Retraining as Approximate Bayesian Inference](https://arxiv.org/abs/2603.25480)
*Harrison Katz*

Main category: cs.AI

TL;DR: 本文提出将模型重训练重新理解为计算约束下的近似贝叶斯推断，引入"学习债务"概念，并基于决策理论框架制定重训练策略，取代传统的日历调度方式。


<details>
  <summary>Details</summary>
Motivation: 传统上将模型重训练视为常规维护任务，缺乏理论框架指导何时进行重训练。作者认为需要将重训练决策系统化，基于证据而非固定时间表来决定重训练时机。

Method: 将重训练重新定义为计算约束下的近似贝叶斯推断，引入"学习债务"概念描述持续更新的信念状态与已部署冻结模型之间的差距。建立决策理论框架，将重训练决策转化为成本最小化问题，从损失函数中推导出重训练阈值。

Result: 开发出基于证据的重训练触发机制，取代传统的日历调度方式，使治理过程可审计。该框架为模型重训练提供了理论依据和实用指导。

Conclusion: 重训练应被视为计算约束下的近似贝叶斯推断过程，而非简单的维护任务。基于决策理论的框架能够提供更科学、可审计的重训练策略，提高模型管理的效率和透明度。
...
### [242] [EcoThink: A Green Adaptive Inference Framework for Sustainable and Accessible Agents](https://arxiv.org/abs/2603.25498)
*Linxiao Li,Zhixiang Lu*

Main category: cs.AI

TL;DR: EcoThink是一个能源感知的自适应推理框架，通过轻量级路由器动态评估查询复杂度，跳过不必要的推理过程，在保持性能的同时平均减少40.4%的推理能耗。


<details>
  <summary>Details</summary>
Motivation: 随着Web从静态检索转向生成式交互，大型语言模型的环境足迹日益严重。当前范式不加区分地对数十亿日常查询应用计算密集型策略（如思维链），导致LLM过度思考，这种冗余增加了碳排放并阻碍了资源受限地区的公平AI访问，直接影响了联合国可持续发展目标13（气候行动）和10（减少不平等）。

Method: EcoThink采用基于蒸馏的轻量级路由器动态评估查询复杂度，对于事实检索类查询跳过不必要的推理过程，仅对复杂逻辑问题保留深度计算，实现自适应推理。

Result: 在9个多样化基准测试中，EcoThink平均减少40.4%的推理能耗（在Web知识检索任务中最高可达81.9%），且没有统计上显著的性能损失。

Conclusion: EcoThink通过减少算法浪费，为实现可持续、包容和能源高效的生成式AI智能体提供了可扩展的路径。
...
### [243] [Voxtral TTS](https://arxiv.org/abs/2603.25551)
...
Method: 采用混合架构：1）自回归生成语义语音令牌；2）流匹配生成声学令牌。使用Voxtral Codec语音令牌化器，该令牌化器从头训练，采用混合VQ-FSQ量化方案进行编码和解码。

Result: 在由母语者进行的人类评估中，Voxtral TTS在多语言语音克隆方面因其自然度和表达性而受到青睐，以68.4%的胜率优于ElevenLabs Flash v2.5模型。

Conclusion: Voxtral TTS是一个有效的多语言文本转语音模型，能够从极少参考音频生成高质量语音，在自然度和表达性方面优于现有先进模型，模型权重以CC BY-NC许可证发布。
...
Method: 提出RC2强化学习框架，通过强制跨模态循环一致性来解决内部冲突：要求模型执行反向推理、切换模态，并通过前向推理可靠地重建答案，获得密集的无标签奖励

Result: 优化这种结构缓解了模态特定错误，将推理准确性提高了最多7.6个百分点

Conclusion: 高级推理不仅来自数据扩展，还来自强制对世界形成结构一致的理解，跨模态不一致性可以作为自然信号用于学习
