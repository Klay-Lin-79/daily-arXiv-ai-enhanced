...
TL;DR: 本文展示了如何使用拓扑数据分析中的持久同调方法来客观分析艺术风格，能够统计显著地区分不同艺术家、同一流派内的艺术家，以及区分真实艺术家作品与AI生成作品。


<details>
  <summary>Details</summary>
Motivation: 传统艺术风格识别依赖艺术史学家或评论家的视觉直觉和经验，缺乏客观分析方法。本文旨在通过数学工具为艺术分析提供结构化视角。

Method: 使用拓扑数据分析中的持久同调方法，这是一种能够从数据中提取拓扑特征的技术。

Result: 持久同调方法能够以统计确定性区分不同艺术家、同一艺术流派内的艺术家，并能有效区分艺术家真实作品与AI生成的模仿作品。

Conclusion: 持久同调为艺术风格分析提供了客观且可解释的洞察，是传统主观艺术分析方法的有效补充。
...
TL;DR: 提出了一种使用StyleGAN2生成猕猴面部表情的方法，通过数据增强、样本选择和损失函数优化来解决训练数据不足的问题，能够生成多样化的猕猴面部表情并用于基于风格的图像编辑。


<details>
  <summary>Details</summary>
Motivation: 由于动物面部训练图像在数量和变化上都很有限，特别是不同个体的面部表情，因此需要开发能够生成多样化动物面部表情的方法，以支持系统神经科学和进化研究。

Method: 使用StyleGAN2模型，结合三种策略：1) 通过运动转移合成新表情图像的数据增强；2) 基于潜在表示的样本选择确保训练数据变化和均匀采样；3) 损失函数优化以准确再现细微运动。

Result: 该方法能够为多个猕猴个体生成多样化的面部表情，性能优于仅使用原始静态图像训练的模型，并且能够有效进行基于风格的图像编辑，其中特定风格参数对应不同的面部运动。
...
TL;DR: 提出了PairHuman数据集，这是首个专为高质量双人肖像生成设计的大规模基准数据集，包含10万+图像和丰富元数据。同时提出了DHumanDiff基线方法，在双人肖像生成中增强面部一致性并平衡个性化人物生成与语义驱动场景创建。


<details>
  <summary>Details</summary>
Motivation: 个性化双人肖像定制具有重要应用价值，如保存情感记忆和婚礼摄影规划，但缺乏基准数据集阻碍了高质量定制的发展。

Method: 构建PairHuman数据集（10万+图像，包含场景、服装、互动等多样性和丰富元数据），并提出DHumanDiff基线方法，增强面部一致性，平衡个性化人物生成与语义场景创建。
...
Method: 采用无监督自编码器，在潜在空间中集成Cohen-Daubechies-Feauveau (CDF 97)小波变换，专门针对混合高斯-泊松噪声进行去噪处理。
...
### [5] [A Little More Like This: Text-to-Image Retrieval with Vision-Language Models Using Relevance Feedback](https://arxiv.org/abs/2511.17255)
*Bulat Khaertdinov,Mirela Popa,Nava Tintarev*

Main category: cs.CV

TL;DR: 本文提出基于相关性反馈的机制来改进视觉语言模型的检索性能，包括伪相关性反馈、生成式相关性反馈、注意力反馈总结器和显式反馈四种策略，实验表明这些方法能显著提升检索效果。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然支持自然语言查询的视觉搜索，但提升性能通常需要微调或扩大模型规模。本文旨在通过推理时的相关性反馈机制来改进检索性能，这种模型无关的设计既可作为微调的替代方案，也可与微调后的模型配合使用。

Method: 提出四种相关性反馈策略：1）伪相关性反馈（PRF）- 基于排名靠前结果修正查询嵌入；2）生成式相关性反馈（GRF）- 使用合成字幕进行查询优化；3）注意力反馈总结器（AFS）- 基于Transformer的定制模型，整合相关项目的多模态细粒度特征；4）使用真实字幕的显式反馈作为上限基准。

Result: 在Flickr30k和COCO数据集上的实验显示，与无反馈检索相比，GRF、AFS和显式反馈使小型VLM的MRR@5提升3-5%，大型VLM提升1-3%。AFS与显式反馈类似，能缓解查询漂移问题，在多轮迭代检索中比GRF更稳健。
...
TL;DR: 提出SaFeR-CLIP框架，通过将不安全概念重定向到语义上最接近的安全替代方案，在保持鲁棒安全性的同时，恢复高达8.0%的零样本准确率，解决了视觉语言模型安全性与泛化性能之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型（如CLIP）通过微调提高安全性时，往往导致泛化性能显著下降。这种权衡源于僵化的对齐策略，强制不安全概念朝向单一预定义的安全目标，破坏了模型学习的语义结构。

Method: 提出邻近感知方法：将不安全概念重定向到语义上最接近的安全替代方案，以最小化表示变化。开发SaFeR-CLIP微调框架，应用最小干预原则。同时贡献NSFW-Caps基准数据集，包含1000个高度对齐的配对，用于测试分布偏移下的安全性。
...
TL;DR: 提出一个三阶段框架，从单视图SVG输入生成多视图一致的SVG，具有几何和颜色一致性，减少冗余路径并保留细节结构。


<details>
  <summary>Details</summary>
Motivation: 解决从单视图SVG生成多视图一致SVG的未充分探索问题，桥接生成建模与结构化向量表示，支持资产创建和语义向量编辑应用。

Method: 三阶段框架：1) 将栅格化输入提升为3D表示并渲染多视图图像；2) 扩展SAM2的时间记忆机制到空间域，构建空间记忆库建立部件级对应关系；3) 在栅格到向量转换中执行路径整合和结构优化。

Result: 生成的SVG在多个视图间展现出强大的几何和颜色一致性，显著减少冗余路径，同时保留精细结构细节。

Conclusion: 该工作为单输入、对象级多视图SVG生成提供了可扩展路径，成功桥接了生成建模与结构化向量表示。
...
TL;DR: Mesh RAG是一个无需训练、即插即用的框架，用于增强自回归网格生成模型，通过检索机制解耦严格的序列依赖，提高网格质量、加速生成速度并支持增量编辑。


<details>
  <summary>Details</summary>
Motivation: 传统手工制作3D网格耗时且难以扩展，现有自回归方法存在质量与速度的权衡问题，且序列依赖性限制了增量编辑能力。

Method: 受语言模型RAG启发，通过点云分割、空间变换和点云注册技术来检索、生成和整合网格组件，实现并行化推理。

Result: Mesh RAG在各种基础自回归网格生成模型中显著提升网格质量，相比顺序部件预测加速生成速度，并支持增量编辑，无需模型重新训练。

Conclusion: Mesh RAG框架有效解决了自回归网格生成中的序列依赖问题，在保持质量的同时实现加速生成和灵活编辑。
...
Result: 系统生成几何一致、视觉丰富且实时渲染高效的世界，支持对布局、规模和样式的细粒度控制，可直接在标准游戏引擎中探索或编辑。

Conclusion: 这项工作代表了向可访问的、大规模生成式世界构建迈出的一步，推进了3D生成AI在游戏、模拟和沉浸式社交环境中的应用前沿。
...
TL;DR: REO-Instruct是首个面向地球观测的统一基准数据集，专注于描述性和回归任务，在森林生态场景中建立认知可解释的逻辑链，连接定性理解和定量预测。
...
Conclusion: REO-Instruct为开发能够同时进行描述和科学推理的下一代地理空间模型提供了标准化基础，推动了科学回归任务在地球观测中的应用。
...
TL;DR: 提出了Parts-Mamba模型，一种混合GCN-Mamba架构，用于解决骨架动作识别中因遮挡或通信质量问题导致的骨架不完整问题，通过捕捉远程关节的上下文信息来提升识别性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中捕获的骨架常因身体部位遮挡或通信质量问题而不完整，现有GCN模型在缺失局部上下文时表现不佳，需要增强捕捉和保持远距离关节上下文信息的能力。

Method: 提出Parts-Mamba混合GCN-Mamba模型，通过部件特定扫描特征捕捉部件特定信息，并通过部件-身体融合模块保持非相邻关节的上下文。

Result: 在NTU RGB+D 60和NTU RGB+D 120数据集的不同遮挡设置下进行评估，准确率最高提升12.9%。

Conclusion: Parts-Mamba模型能有效处理骨架不完整问题，通过增强上下文信息捕捉能力显著提升了骨架动作识别的性能。
...
TL;DR: 本文提出了联合Gromov-Wasserstein (JGW)目标，扩展了传统的GW距离框架，实现了多对多对象的同时匹配，在准确性和计算效率上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统的Gromov-Wasserstein距离仅限于单对单对象匹配，无法满足需要多对一或多对多匹配的实际应用场景需求。
...
TL;DR: 该论文提出了一种名为REPA（表示对齐）的方法，通过将扩散或流式生成模型与预训练的自监督视觉编码器（如DINOv2）进行表示对齐，来改进逆问题中的重建质量。


<details>
  <summary>Details</summary>
Motivation: 在逆问题中，预训练的生成模型被用作先验，但缺乏地面真实信号。研究表明，将生成模型的内部表示与预训练编码器对齐可以提供强大的归纳偏置，改善收敛和样本质量。

Method: 提出REPA方法，在推理时通过将扩散或流式模型与预训练自监督视觉编码器（如DINOv2）的表示对齐来指导重建过程，即使没有真实目标信号，也能使用近似目标特征进行对齐。

Result: 实验证明REPA方法在超分辨率、框内修复、高斯去模糊和运动去模糊等任务中一致提高了重建质量，同时通过减少离散化步骤数量提供了显著的效率提升。

Conclusion: REPA方法通过表示对齐有效提升了逆问题中重建的保真度和感知真实性，具有通用性且能集成到多种最先进的逆问题求解器中。
...
TL;DR: 提出NFGlassNet方法，利用闪光/无闪光图像中的反射动态特性进行玻璃表面检测，通过反射对比挖掘和反射引导注意力模块实现准确检测。


<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，但未能充分利用玻璃本身的内在特性进行精确定位。观察到玻璃前后光照强度差异导致反射变化的现象。
...
Conclusion: 利用闪光/无闪光图像中的反射动态特性是玻璃表面检测的有效方法，NFGlassNet在准确检测玻璃表面方面表现出色。
...
TL;DR: 该论文提出了R-AVST数据集和AVST-Zero模型，用于解决真实世界音频-视频时空推理任务。R-AVST包含5K个未剪辑视频和27K个对象，涵盖100种音频-视频事件类型，并定义了三个核心推理任务。AVST-Zero采用强化学习方法，通过多维度奖励直接优化行为，避免了中间监督。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频理解任务中取得了快速进展，但主要关注简单视频场景，无法反映真实世界音频-视频事件的复杂多样性。为了弥补这一差距，需要构建专门的数据集和模型来支持音频-视频时空推理。

Method: 1) 构建R-AVST数据集：采用基于LLM的关键对象提取、自动空间标注和人工质量检查的流水线；2) 提出AVST-Zero模型：基于强化学习，避免中间监督，通过精心设计的多维度奖励直接优化行为。

Result: 构建了包含5K个未剪辑视频、27K个对象、100种音频-视频事件类型的数据集，生成了8K+高质量、均匀分布的问答对。AVST-Zero在R-AVST数据集上表现出与现有模型相竞争的性能。

Conclusion: R-AVST是首个专为真实世界音频-视频时空推理设计的数据集，AVST-Zero为该领域未来挑战提供了新的解决视角。广泛实验验证了R-AVST在推进音频-视频时空推理方面的有效性。
...
TL;DR: 本文提出了Warm Diffusion，一种统一的模糊-噪声混合扩散模型(BNMD)，通过联合控制模糊和噪声来结合热扩散和冷扩散的优势，利用图像中的频谱依赖性简化分数模型估计。


<details>
  <summary>Details</summary>
Motivation: 热扩散完全依赖噪声，未能利用高频图像细节与低频结构之间的强相关性；冷扩散仅使用模糊而忽略噪声在塑造数据流形中的作用，导致性能下降。需要结合两者的优势。

Method: 提出Warm Diffusion模型，采用分治策略利用图像频谱依赖性，通过解耦去噪和去模糊过程简化分数模型估计，并分析模糊-噪声比(BNR)来研究模型学习动态与数据流形变化之间的权衡。
...
Method: 使用统一扩散变换器，在像素空间中训练整流流，将文本提示渲染为绘制文本图像，所有输入输出都作为RGB像素处理。

Result: 在文本到图像合成和图像到文本理解任务上表现出强大的跨模态对齐和新兴可控性，如图像-字幕-图像循环一致性。
...
<details>
  <summary>Details</summary>
Motivation: 传统异常生成方法需要少量异常样本进行微调，这与异常稀缺性相矛盾且容易过拟合类别先验。本文旨在解决无真实异常样本或训练数据可用的情况。

Method: 提出Delta-Denoising方法：1) 使用最小提示对驱动两个扩散分支对比；2) 通过累积每步去噪差异生成图像特定定位图；3) 基于定位图指导潜在修复，在保留上下文的同时生成局部缺陷；4) 采用词元级提示优化和对齐异常词元的空间注意力偏置。

Result: 在公共数据集上的实验表明，DeltaDeno实现了优秀的生成效果、真实性和下游检测性能的持续提升。

Conclusion: DeltaDeno提供了一种无需训练、零样本的异常生成解决方案，能够有效定位和生成局部缺陷，同时保持周围环境的真实性。
...
TL;DR: 本文提出了一种基于扩散模型的视频超分辨率方法DGAF-VSR，通过特征域对齐和补偿机制解决了现有方法中的误差累积、空间伪影以及感知质量与保真度之间的权衡问题。
...
Method: 提出了DGAF-VSR方法，包含光学引导变形模块(OGWM)在放大分辨率下保持对齐特征的高频细节，以及特征域时序条件模块(FTCM)在特征域提供密集指导。

Result: 在合成和真实数据集上的实验表明，DGAF-VSR在感知质量(DISTS降低35.82%)、保真度(PSNR增益0.20dB)和时间一致性(tLPIPS降低30.37%)方面均优于现有最优方法。

Conclusion: 特征域比像素域更适合信息补偿，在放大分辨率下进行变形能更好保留高频信息，提出的DGAF-VSR方法有效提升了视频超分辨率的性能。
...
<details>
  <summary>Details</summary>
Motivation: CBCT图像中的牙齿分割在邻牙粘连情况下面临挑战，会导致严重的解剖形状失真，需要开发能够保持形状完整性的分割方法。

Method: 采用目标牙齿质心提示的多标签学习策略来建模牙齿间的语义关系，减少形状模糊性；结合牙齿形状感知学习机制，明确施加形态约束以保持边界完整性；通过多任务学习统一这些组件。
...
TL;DR: 本文提出了OmniGround基准测试，包含3,475个视频和81个类别，用于解决时空视频定位中的类别偏见和推理简化问题。同时提出了PG-TAF训练免费的两阶段框架，在多个基准测试上取得显著改进。
...
Method: 提出了Forward-Backward-Refinement标注流程，结合多方向跟踪和智能错误校正；开发了DeepSTG系统评估框架；提出了PG-TAF训练免费的两阶段框架，将时空视频定位分解为高层时间定位和细粒度时空传播。

Result: 在复杂现实场景中性能平均下降10.4%，特别是在小/遮挡物体和复杂空间关系上。PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试上均取得一致增益。

Conclusion: OmniGround基准测试和PG-TAF框架有效解决了时空视频定位中的现有挑战，为复杂现实场景下的目标定位提供了更可靠的解决方案。
...
TL;DR: MultiPriv是首个系统评估视觉语言模型中个体级隐私推理风险的基准，通过构建包含合成个体档案的双语多模态数据集，设计了9个挑战性任务来全面评估隐私感知和推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型展现出复杂推理能力，带来了超越简单属性感知的个体级链接隐私风险，而现有隐私基准主要评估隐私感知，无法应对更关键的隐私推理风险。

Method: 提出隐私感知与推理框架，构建包含合成个体档案的双语多模态数据集，其中标识符与敏感属性精心链接，设计了9个任务评估从属性检测到跨图像重识别和链式推理的完整隐私推理谱系。

Result: 对50多个基础和商业视觉语言模型的大规模评估显示：许多模型存在显著的未测量推理隐私风险；感知级指标无法预测推理风险；现有安全对齐对此类推理攻击不一致且无效。
...
TL;DR: 提出了一种新颖的隐式神经表示框架，联合建模动态图像序列和其底层运动场，通过光学流方程作为物理启发的正则化，在动态心脏MRI数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统运动补偿重建依赖预先估计的光学流，但在欠采样情况下不准确且会降低重建质量，需要一种能够同时恢复图像和运动场的方法。

Method: 使用两个隐式神经表示：一个参数化时空图像内容，另一个表示光学流，通过光学流方程耦合两者，并加入数据一致性损失来约束k空间测量。

Result: 在动态心脏MRI数据集上实验表明，该方法在重建质量、运动估计准确性和时间保真度方面优于现有最先进的运动补偿和深度学习方法。

Conclusion: 隐式联合建模与流正则化约束在推进动态MRI重建方面具有巨大潜力。
...
TL;DR: 本文提出了FingerCap任务，旨在生成捕捉手部动作详细手指级语义的文本描述，并构建了包含4万对手部运动视频和字幕的大规模数据集FingerCap-40K。为了解决视频MLLMs在时间稀疏性上的瓶颈，提出了FiGOP方法，通过将RGB关键帧与后续手部关键点配对来恢复精细时间线索。


<details>
  <summary>Details</summary>
Motivation: 理解细粒度人类手部运动对视觉感知、具身智能和多模态通信至关重要，但现有方法难以捕捉手指级的细微动作语义。

Method: 提出了FiGOP方法，将每个RGB关键帧与后续手部关键点配对，使用轻量级时间编码器将关键点转换为运动嵌入，并与RGB特征集成。FiGOP适应了经典的GOP概念来捕捉手指运动，无需增加RGB密度即可恢复精细时间线索。

Result: 在FingerCap-40K数据集上的实验表明，强大的开源和闭源Video-MLLMs在手指级推理方面仍有困难，而FiGOP增强模型在HandJudge和人类研究中均取得了持续改进。
...
TL;DR: 本文提出了一种基于点监督的面部表情检测方法，通过双分支框架解决仅使用单时间戳标注的训练问题，包括基于高斯分布的实例自适应强度建模和类别感知的顶点分类分支。


<details>
  <summary>Details</summary>
Motivation: 现有的面部表情检测方法主要依赖完全监督学习，需要昂贵的时间边界标注。本文旨在研究点监督的面部表情检测，仅需每个实例的单时间戳标注即可训练，以降低标注成本。

Method: 提出双分支框架：1）基于高斯分布的实例自适应强度建模模块，通过检测伪顶点帧、估计持续时间和构建实例级高斯分布来生成软伪标签；2）类别感知的顶点分类分支，仅基于伪顶点帧区分宏表情和微表情。还引入了强度感知对比损失来增强特征学习。

Result: 在SAMM-LV、CAS(ME)^2和CAS(ME)^3数据集上的大量实验证明了所提框架的有效性。

Conclusion: 该方法通过点监督实现了有效的面部表情检测，降低了标注成本，同时通过软伪标签和双分支设计提高了检测性能。
...
TL;DR: MatPedia是一个基于联合RGB-PBR表示的基础模型，通过将材料编码为RGB外观和PBR贴图的相互依赖潜在变量，使用视频扩散架构统一处理多种材料任务。
...
Method: 提出新颖的联合RGB-PBR表示，将材料编码为两个相互依赖的潜在变量：一个用于RGB外观，一个用于四个PBR贴图。将它们制定为5帧序列，采用视频扩散架构来捕捉相关性并转移RGB生成模型的视觉先验。

Result: 在MatHybrid-410K混合语料库上训练，MatPedia实现了原生1024×1024合成，在质量和多样性上都显著超越现有方法。

Conclusion: MatPedia提供了一个统一框架，能够在单一架构中处理文本到材料生成、图像到材料生成和本征分解等多种材料任务。
...
<details>
  <summary>Details</summary>
Motivation: 解决在边缘设备上从生食图像合成逼真熟食图像的挑战，现有方法要么生成不真实，要么资源消耗过大，无法在边缘设备部署。
...
Conclusion: 该方法能够根据用户偏好的视觉目标生成逼真的烹饪过程图像，同时确保时间一致性和烹饪合理性，适合边缘设备部署。
...
TL;DR: 本文提出了SeeCLIP框架，通过细粒度语义增强解决开放集域泛化中的已知类结构风险和未知类开放空间风险之间的困境。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理域偏移和未知类别时面临过自信问题，特别是在区分与已知类具有细粒度视觉相似性的'硬未知'时表现不佳。

Method: 提出语义增强的CLIP框架，包括语义感知提示增强模块、双工对比学习和语义引导扩散模块，通过分解图像为判别性语义标记并生成伪未知样本来学习更精细的决策边界。

Result: 在五个基准测试上的广泛实验显示，相比最先进方法，准确率提升3%，H-score提升5%。

Conclusion: SeeCLIP通过细粒度语义增强有效解决了开放集域泛化中的关键挑战，在区分硬未知类别方面表现出显著改进。
...
TL;DR: 本文提出了一种受自然选择启发的3D高斯分布修剪框架，通过将生存压力建模为应用于不透明度的正则化梯度场，让优化梯度自主决定保留或修剪哪些高斯分布，无需人工干预。


<details>
  <summary>Details</summary>
Motivation: 3DGS使用大量高斯基元来拟合场景，导致显著的存储和计算开销。现有的修剪方法依赖手动设计的标准或引入额外的可学习参数，效果不理想。

Method: 提出自然选择启发的修剪框架，将生存压力建模为不透明度的正则化梯度场；引入具有有限不透明度先验的不透明度衰减技术，加速选择过程。

Result: 与3DGS相比，在15%预算下实现了超过0.6 dB的PSNR增益，在紧凑3DGS方面达到最先进性能。

Conclusion: 该方法实现了完全可学习的修剪过程，无需人工干预，为3D高斯分布的紧凑表示提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的深度很少是单一的，透射材料会产生分层模糊性，而现有模型只能估计静态深度图，无法像人类那样主动调整焦点来感知所需深度。

Method: 基于标量深度偏好条件化的可操控Vision Transformer，主要使用新构建的50万张多层合成数据集进行训练，该数据集专门捕捉各种穿透效果。

Result: 在BOOSTER等传统单深度基准测试中达到最先进性能，在新提出的真实和合成多深度数据集上定量展示了意图对齐的估计能力，并在未见过的穿透场景中表现出强大的泛化能力。

Conclusion: DepthFocus代表了向主动和类人3D感知迈出的重要一步，展现了在复杂场景中选择性感知深度的能力。
...
TL;DR: MVLR是一个统一的记忆增强视觉语言恢复模型，通过结合轻量级编码器-解码器、视觉语言模型和隐式记忆库，有效恢复各种恶劣天气条件下的退化图像。
...
Method: MVLR将轻量级编码器-解码器与视觉语言模型和隐式记忆库耦合，利用VLM进行思维链推理编码天气退化先验，IMB存储退化模式的连续潜在表示，通过动态交叉注意力机制自适应融合多尺度视觉特征。
...
TL;DR: 本文介绍了ConfusedTourist，一个评估视觉语言模型在混合文化线索下稳健性的对抗性测试套件，发现即使最先进的模型在简单图像堆叠扰动下准确率也会大幅下降。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常每个图像只包含单一文化概念，忽视了多个可能不相关的文化线索共存的情况，而视觉语言模型在多样化文化输入下的稳定性对支持多元文化社会至关重要。

Method: 开发了ConfusedTourist文化对抗性稳健性测试套件，通过图像堆叠和基于图像生成的变体来扰动地理线索，评估模型稳定性。

Result: 实验显示模型在简单图像堆叠扰动下准确率大幅下降，基于图像生成的变体效果更差。可解释性分析表明失败源于系统性的注意力转移。

Conclusion: 视觉文化概念混合会严重损害最先进的视觉语言模型，迫切需要更具文化稳健性的多模态理解方法。
...
Conclusion: FLUID框架通过潜在空间身份替换实现了优越的身份抑制和属性保留平衡，为人脸去识别提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 推动体育分析中的计算机视觉发展，解决现有数据集缺乏细粒度球拍姿态标注的问题，支持多模态融合研究。

Method: 构建包含乒乓球、网球和羽毛球的大规模数据集，提供球拍姿态和球位置的细粒度标注，采用CrossAttention机制进行多模态融合。

Result: 评估显示简单拼接球拍姿态特征会降低性能，而CrossAttention机制能有效利用这些特征，在轨迹预测任务上超越强单模态基线。
...
TL;DR: 本文提出了OmniPT框架，统一处理行人跟踪任务，通过RL-Mid Training-SFT-RL训练策略使LVLM能够输出格式化边界框并提升跟踪性能。


<details>
  <summary>Details</summary>
Motivation: LVLM在图像级任务表现出色，但在实例级任务如行人跟踪方面仍存在性能差距。同时，结合目标跟踪和自然语言的新任务需要模型在高级语义层面理解跟踪对象，这正是LVLM的优势所在。

Method: 提出OmniPT框架，采用RL-Mid Training-SFT-RL训练策略：首先通过RL使模型输出可监督的边界框格式，然后使用大量行人相关数据集进行中期训练，接着在多个行人跟踪数据集上进行监督微调，最后再进行RL阶段以提升跟踪性能和指令跟随能力。

Result: 在跟踪基准测试上的实验结果表明，所提方法能够比先前方法表现更好。

Conclusion: OmniPT框架成功将行人跟踪任务建模为基础模型可执行的任务，并通过多阶段训练策略显著提升了LVLM在行人跟踪任务上的性能。
...
TL;DR: REArtGS++提出了一种基于平面高斯溅射和时间几何约束的可泛化铰接物体重建方法，改进了REArtGS在螺旋关节、多部件物体和未见状态几何约束方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的REArtGS方法在处理螺旋关节或多部件物体时存在困难，且缺乏对未见状态的几何约束。本文旨在解决这些问题，实现更通用的铰接物体重建。

Method: 首先为每个关节建模解耦的螺旋运动（无需类型先验），通过部件运动混合联合优化部件感知高斯和关节参数。引入时间连续几何约束，鼓励高斯平面化，并通过泰勒一阶展开在平面法线和深度之间建立时间一致性正则化。

Result: 在合成和真实世界铰接物体上的大量实验表明，该方法在可泛化的部件级表面重建和关节参数估计方面优于现有方法。

Conclusion: REArtGS++通过时间几何约束和平面高斯溅射，实现了更通用和准确的铰接物体重建，显著提升了在复杂关节类型和未见状态下的性能。
...
Method: 使用布朗桥扩散模型合成2D MRI切片，同时通过微调检索模型从先验数据库中检索结构和病理相似的CT切片作为参考，通过ControlNet分支指导生成中间MRI切片，确保结构连续性。

Result: 在SynthRAD2023和BraTS数据集上的广泛实验表明，ReBrain在稀疏条件下的跨模态重建中达到了最先进的性能。

Conclusion: ReBrain框架有效解决了从稀疏CT扫描重建脑部MRI的挑战，为无法进行MRI检查的患者提供了可行的替代方案。
...
### [38] [Diversity Has Always Been There in Your Visual Autoregressive Models](https://arxiv.org/abs/2511.17074)
...
Method: 分析发现特征图中的关键组件是早期尺度多样性形成的关键因素，通过抑制模型输入中的关键组件并放大模型输出中的该组件来恢复多样性。

Result: 实验结果表明该方法显著增强了生成多样性，且对性能影响可忽略不计。

Conclusion: DiverseVAR有效解锁了VAR模型的内在生成潜力，在保持高保真合成的同时大幅提升了多样性。
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中采用随机排列序列顺序会导致性能下降或推理时序列顺序选择不灵活，需要一种既能保持性能又能提供灵活序列顺序的方法。

Method: 使用在图像块位置定义的网格中采样的均匀生成树的遍历顺序，通过广度优先搜索获得遍历顺序，并通过拒绝采样确保连接的部分观察作为序列前缀出现。

Result: STAR模型在保持采样性能的同时提供了灵活的序列顺序，支持后补完成功能，且无需对广泛采用的语言AR模型架构进行重大改变。

Conclusion: STAR模型通过结构化的随机策略相比随机排列，在保持采样性能的同时实现了灵活的序列顺序，适用于图像编辑任务。
...
TL;DR: 提出了一种基于平面高斯溅射的类别无关铰接物体重建框架，仅需单状态下的稀疏视角RGB图像输入，实现了高质量的部分级别表面重建。
...
Method: 首先引入高斯信息场感知最优稀疏视角，然后将3D高斯压缩为平面高斯以准确估计法线和深度，通过深度平滑正则化和少样本扩散进行粗到细优化，并为每个高斯基元引入部分分割概率。

Result: 在合成和真实世界数据上的实验结果表明，该方法比现有方法实现了更高保真度的部分级别表面重建。
...
TL;DR: ReCoVAD提出了一种受人类神经系统启发的视频异常检测框架，通过选择性帧处理减少冗余计算，在保持最先进性能的同时仅处理28.55%和16.04%的帧。


<details>
  <summary>Details</summary>
Motivation: 现有基于大预训练模型的视频异常检测方法通常依赖密集帧级推理，计算成本高且延迟大。本文旨在探索在使用强大预训练模型时是否真的需要密集推理。

Method: ReCoVAD包含两个核心通路：反射通路使用轻量级CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；意识通路使用中等规模视觉语言模型生成文本事件描述和精炼异常分数，通过集成大语言模型定期审查累积描述来识别未见异常。
...
TL;DR: SketchVerify是一个无需训练的、基于草图验证的运动规划框架，通过测试时采样和验证循环来生成更动态连贯的轨迹，提高视频生成的运动质量和物理真实性。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法使用单次规划或迭代优化，前者运动简单，后者计算成本高。需要一种既能生成复杂运动又高效的规划方法。

Method: 提出草图验证框架：预测多个候选运动轨迹，使用视觉语言验证器评估语义对齐和物理合理性，通过渲染轻量级视频草图来高效评分，迭代优化直到找到满意轨迹。

Result: 在WorldModelBench和PhyWorldBench上的实验表明，该方法显著提高了运动质量、物理真实性和长期一致性，同时计算效率更高。
...
TL;DR: 提出Partitioned Adaptive Contrastive Learning (PACL)方法，通过利用预训练文本模型的知识来增强预训练视觉模型的情感感知能力，解决视觉情感识别中的'情感鸿沟'问题。
...
Method: 利用社交媒体数据中图像和文本的事实与情感联系，提出分区自适应对比学习(PACL)，分离不同类型的样本并为每种类型设计不同的对比学习策略，通过动态构建正负样本对来充分利用噪声样本的潜力。
...
TL;DR: ChainV是一个动态整合视觉提示的多模态推理框架，通过粗粒度视觉补丁选择和基于注意力强度的细粒度原子视觉提示识别，结合一致性评估机制，显著提升推理准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推理模型在生成长推理链时存在冗余自反思问题，而基于静态视觉参考的CoT压缩方法对多模态推理增益有限，因此需要动态整合视觉提示来优化推理过程。

Method: ChainV首先基于前一步推理进行粗粒度视觉补丁选择，然后根据平均注意力强度识别最具代表性的原子视觉提示，引入一致性评估机制评估提示可靠性，并通过Bernoulli随机过程将像素坐标和可靠性整合到思考中。

Result: 实验表明ChainV显著提升推理准确性和效率，在MathVista基准上准确率提升2.3%，推理延迟降低51.4%，输出token长度缩短24.5%。

Conclusion: ChainV通过动态整合视觉提示有效解决了多模态推理中的冗余自反思问题，在数学密集型任务中表现尤为突出，实现了更短更好的推理过程。
...
<details>
  <summary>Details</summary>
Motivation: 在大时空尺度上重建刚性运动仍然具有挑战性，主要由于建模范式限制、严重运动模糊和物理一致性不足。

Method: 提出三重监督方案：通过加速度约束强制物理合理性，利用事件流进行高时间分辨率指导，使用卡尔曼正则化器融合多源观测；设计基于实时运动状态的运动感知模拟退火训练策略；贡献首个针对自然快速刚性运动的RGB-事件配对数据集。

Result: 实验表明PEGS在大时空尺度运动重建方面优于主流动态方法。

Conclusion: PEGS框架通过整合物理先验和事件流增强，成功解决了大时空尺度刚性运动重建的挑战，实现了优越的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现代LED照明技术可能显著偏离普朗克轨迹，传统基于CCT的一维插值方法在表征这类非普朗克光源时存在局限性。

Method: 使用基于光箱的校准程序训练轻量级MLP，该MLP利用2D色度特征进行比色映射，替代传统的CCT插值方法。

Result: 在多种LED照明下验证，该方法平均减少22%的角度再现误差，保持与传统光源的向后兼容性，支持多光源场景和实时相机部署。

Conclusion: 从1D CCT转向2D色度空间结合MLP方法，显著提高了非普朗克光源下的比色精度，同时保持计算效率。
...
TL;DR: 该论文提出了一个多阶段优化框架，将高性能的深度学习图像压缩模型转化为适合FPGA部署的整数实现，解决了量化导致的性能下降问题，并通过硬件感知优化显著降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 深度学习图像压缩模型在资源受限的FPGA上部署面临重大挑战，需要解决量化导致的性能下降和硬件效率问题。

Method: 提出动态范围感知量化(DRAQ)方法，结合统计校准的激活裁剪和权重正则化；开发渐进混合精度搜索算法和针对GDN层的通道剪枝技术。

Result: DRAQ方法将GDN模型的BD-rate开销从30%降至6.3%；硬件感知优化进一步降低20%以上计算复杂度，对RD性能影响可忽略。

Conclusion: 最终模型在效率和性能上均达到先进水平，优于现有的FPGA图像压缩实现。
...
<details>
  <summary>Details</summary>
Motivation: 解决扩散基真实世界图像超分辨率中保真度与可控性之间的平衡问题：多步方法存在生成多样性和随机性导致保真度低，而单步方法因保真度特定微调而失去控制灵活性。
...
Conclusion: ODTSR成功平衡了保真度和可控性，为真实世界图像超分辨率提供了有效的单步扩散解决方案。
...
### [49] [FireScope: Wildfire Risk Prediction with a Chain-of-Thought Oracle](https://arxiv.org/abs/2511.17171)
*Mario Markov,Stefan Maria Ailuro,Luc Van Gool,Konrad Schindler,Danda Pani Paudel*

Main category: cs.CV

TL;DR: FireScope是一个基于视觉语言模型的推理生成框架，通过结合卫星图像和气候数据预测野火风险图，并在跨大陆评估中表现出优异的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏因果推理和多模态理解能力，无法实现可靠的泛化。需要开发能够整合视觉、气候和地理因素来推断连续风险图的推理密集型空间问题解决方案。

Method: 提出FireScope-Bench数据集和基准，结合Sentinel-2图像和气候数据与专家定义的风险栅格。开发基于VLM的推理生成框架，通过强化学习和视觉监督学习预测风险栅格并生成补充推理轨迹。
...
TL;DR: CC-DiceCE损失函数基于CC-Metrics框架，相比传统Dice损失能更好地检测小病灶，提高召回率，在nnU-Net框架下表现优于blob损失。
...
Method: 提出CC-DiceCE损失函数，基于CC-Metrics框架，并与现有的blob损失进行比较，在nnU-Net框架下进行基准测试。

Result: CC-DiceCE损失提高了病灶检测（召回率），分割性能基本不受影响，但会产生稍多的假阳性。在多数据集研究中，CC-DiceCE通常优于blob损失。

Conclusion: CC-DiceCE损失函数能有效改善小病灶检测，在医学图像分割中具有实用价值。
...
Method: 在音频、视频和多模态三个维度，以及唇部运动和通用视觉内容两个领域，系统评估自监督特征在检测效果、可解释性和跨模态互补性三个关键维度上的表现。

Result: 大多数自监督特征能捕获深度伪造相关信息且信息具有互补性；模型主要关注语义上有意义的区域而非虚假伪影；但所有模型都无法跨数据集可靠泛化。

Conclusion: 自监督表示在深度伪造检测中既有前景又面临根本挑战：虽然能学习有意义的模式，但实现稳健的跨域性能仍然困难，泛化失败源于数据集特性而非特征本身依赖表面模式。
...
<details>
  <summary>Details</summary>
Motivation: 现有的生成式方法（如扩散模型）通常基于人工构建的轨迹锚点或随机噪声进行去噪，存在改进空间。作者希望结合判别式方法的优势来增强生成式轨迹预测的性能。
...
Conclusion: DiffRefiner通过结合判别式轨迹提议和生成式精炼，显著提升了扩散基规划的轨迹预测性能，为自动驾驶提供了更准确和灵活的轨迹预测解决方案。
...
TL;DR: UI-Styler是一个针对超声图像的类别感知风格迁移框架，通过模式匹配机制将目标图像的纹理模式转移到源图像上，同时保持源图像的结构内容，并使用类别感知提示策略确保语义对齐。


<details>
  <summary>Details</summary>
Motivation: 超声图像在不同采集设备间存在外观差异，导致域偏移问题，影响固定黑盒下游推理模型的性能。现有无配对图像翻译方法往往忽视类别特定的语义对齐，导致内容-类别映射错位，影响诊断准确性。

Method: 提出UI-Styler框架，包含模式匹配机制将目标图像纹理转移到源图像，以及基于目标域伪标签的类别感知提示策略，确保语义与诊断类别的准确对齐。

Result: 在超声跨设备任务上的广泛实验表明，UI-Styler在分布距离和下游任务（如分类和分割）上持续优于现有UIT方法，达到最先进性能。

Conclusion: UI-Styler通过有效的风格迁移和类别感知语义对齐，成功解决了超声图像跨设备域偏移问题，提升了诊断模型的鲁棒性和准确性。
...
TL;DR: 本文提出MuM模型，通过将MAE扩展到同一场景的任意多视图，使用统一掩码和轻量级解码器，在3D视觉任务中超越了DINOv3和CroCo v2。


<details>
  <summary>Details</summary>
Motivation: 大多数自监督学习方法针对语义理解而非几何推理优化，CroCo虽然针对3D理解但不够简单和可扩展。

Method: 扩展MAE到任意多视图，统一掩码所有视图，使用轻量级解码器和帧间注意力机制。

Result: 在重建、密集图像匹配和相对位姿估计等下游任务中表现优于DINOv3和CroCo v2。

Conclusion: MuM方法比CroCo更简单和可扩展，在3D视觉任务中达到最先进性能。
...
Method: 基于高斯混合模型(GMM)与多层感知器(MLP)头以及双三次上采样的NPPR估计器，涵盖各种输入依赖和输入独立的扰动场景。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上，使用ResNet18/50、WideResNet50和VGG16模型进行广泛实验，验证NPPR作为更实用的鲁棒性度量，相比现有方法中常用的扰动分布假设，PR估计保守性提高达40%。

Conclusion: NPPR是一个更实用的鲁棒性度量，能够在不依赖预定义扰动分布的情况下进行保守的鲁棒性评估，解决了现有PR方法的局限性。
...
TL;DR: 本文提出了一个针对大型视觉语言模型幻觉问题的综合干预框架，发现幻觉源于图像-输入文本、图像-输出文本和文本-文本三种路径的相互作用，并首次发现模型依赖不同路径取决于问答对齐格式。
...
Method: 提出了与transformer因果架构对齐的综合干预框架，分析不同干预路径对幻觉的影响。针对判别式和生成式格式，提出了识别和干预关键幻觉头的方法。

Result: 实验在多个基准测试上表明，该方法能够持续减少不同对齐类型的幻觉现象。

Conclusion: 幻觉不是由单一因果路径引起的，而是多种路径相互作用的结果。通过针对不同问答对齐格式定制干预策略，可以有效地减少大型视觉语言模型的幻觉问题。
...
TL;DR: 提出了一种基于语义掩码引导的LiDAR点云编辑方法，通过将点云转换为2D距离图像，利用扩散模型和语义掩码生成新颖的合成LiDAR点云，用于自动驾驶系统的训练数据增强。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统训练需要大量多样的点云数据，特别是边缘场景数据难以获取。现有基于手工构建3D虚拟环境的方法耗时且难以完全捕捉真实世界复杂性。

Method: 将LiDAR点云转换为2D距离图像，使用基于凸包的语义掩码作为条件指导，采用扩散模型进行语义编辑生成新的点云数据。

Result: 在KITTI-360数据集上验证，能够生成高质量的LiDAR点云，包括复杂的边缘场景和动态场景。

Conclusion: 该方法为生成多样化的LiDAR数据提供了成本效益高且可扩展的解决方案，有助于提高自动驾驶系统的鲁棒性。
...
TL;DR: PostCam是一个用于动态场景中新颖视角视频生成的框架，支持拍摄后编辑相机轨迹。通过查询共享交叉注意力模块融合6-DoF相机位姿和2D渲染视频帧，实现更精确的相机运动控制和更好的生成质量。
...
Conclusion: PostCam通过统一的控制信号表示和两阶段训练策略，实现了更准确和灵活的相机运动操作，显著提升了动态场景中新颖视角视频生成的性能。
...
TL;DR: VLA-4D是一个具有4D感知能力的通用视觉语言动作模型，通过4D感知视觉表示和时空动作表示实现时空一致的机器人操作。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型在时空一致操作方面存在挑战，难以实现动作执行的时间一致性控制，需要细粒度表示。

Method: 提出4D感知视觉表示（将1D时间嵌入3D位置形成4D嵌入，通过交叉注意力融合）和时空动作表示（扩展传统空间动作表示包含时间信息），在统一框架中实现时空动作预测。

Result: 广泛实验验证了该方法在不同机器人操作任务中的优越性，实现了空间平滑和时间一致的操作。

Conclusion: VLA-4D通过4D感知和时空动作表示成功解决了时空一致机器人操作的挑战，为通用VLA模型提供了有效解决方案。
...
TL;DR: 本文提出CA-SAM方法，通过轻量级的对齐层模块和持续学习策略，在保持SAM模型强大零样本能力的同时，提高医学图像分割的计算效率和性能，并在持续学习场景下实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，机构间的隐私政策差异使得联合训练不可行，需要持续学习方法来避免灾难性遗忘。SAM模型虽然具有强大零样本能力，但参数量大、计算开销高，限制了实际部署。

Method: 提出对齐层（Alignment Layer）作为轻量级即插即用模块，对齐编码器-解码器特征分布；基于此构建CA-SAM持续学习策略，自动适配对齐层以缓解灾难性遗忘，同时利用SAM的零样本先验。

Result: 在九个医学分割数据集上的持续学习场景实验中，CA-SAM达到了最先进的性能表现。

Conclusion: SAM范式在平衡计算效率和性能后具有很大潜力，CA-SAM通过轻量级对齐层和持续学习策略，有效解决了医学图像分割中的实际部署挑战。
...
### [61] [Designing and Generating Diverse, Equitable Face Image Datasets for Face Verification Tasks](https://arxiv.org/abs/2511.17393)
*Georgia Baltsou,Ioannis Sarridis,Christos Koutlis,Symeon Papadopoulos*

Main category: cs.CV

TL;DR: 提出了一种利用生成模型创建多样化合成人脸图像的方法，并构建了包含27,780张图像、926个身份的数据集DIF-V，以解决现有面部验证系统中的种族和性别偏见问题。
...
Method: 整合先进生成模型创建多样化高质量合成人脸图像，强调代表各种面部特征并符合身份证照片规范。

Result: 分析显示现有验证模型对某些性别和种族存在偏见，身份样式修改会负面影响模型性能。

Conclusion: 通过解决现有数据集的内在不平等问题，为开发更具包容性和可靠性的面部验证技术奠定了基础。
...
TL;DR: SING3R-SLAM是一个基于高斯表示的全局一致紧凑型密集RGB SLAM框架，通过结合局部一致的3D重建与统一的全局高斯表示，联合优化场景几何和相机位姿，实现高效多用途的3D建图。


<details>
  <summary>Details</summary>
Motivation: 现有密集3D重建方法虽能准确捕捉局部几何，但在SLAM中集成时面临漂移和冗余点云地图的问题，限制了效率和下游任务（如新视角合成）的应用。

Method: 首先通过轻量级跟踪和重建模块构建局部一致的子地图，然后逐步对齐并融合到全局高斯地图中，强制执行跨视角几何一致性，全局地图反馈修正局部漂移并增强跟踪鲁棒性。

Result: 在真实世界数据集上，SING3R-SLAM在跟踪、3D重建和新视角渲染方面达到最先进水平，跟踪精度提升超过12%，产生更精细、更详细的几何结构，同时保持紧凑和内存高效的全局表示。

Conclusion: SING3R-SLAM通过全局高斯表示有效解决了密集SLAM中的漂移和冗余问题，实现了高效、紧凑且多功能的3D建图，为多种下游应用提供了可靠基础。
...
Method: 使用可扩展的3D Vision Transformer架构，结合DINO风格的自蒸馏和基于SigLIP的视觉-语言对齐，联合优化局部Transformer进行高分辨率特征提取和全局Transformer进行全扫描上下文建模。
...
Conclusion: 该框架能够直接处理鱼眼图像，产生连续、不确定性感知的语义地图，有效应对鱼眼图像的特殊挑战。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，通过独立4D时空查询直接从相邻帧学习连续3D语义占据，在保持恒定内存下实现长距离监督和推理。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法依赖2D渲染一致性或离散化体素网格导致空间精度和可扩展性受限的问题，探索直接从传感器数据学习3D场景几何和语义的自监督方法。

Method: 使用独立4D时空查询采样相邻帧，支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督，引入压缩场景表示保持近场细节同时平滑压缩远场区域。

Result: 在自监督Occ3D-nuScenes基准测试中，语义RayIoU比先前基于相机的方法提升26%，运行速度为11.6 FPS。
...
TL;DR: 提出了一种用于彩色图像盲反卷积的新方法，使用四元数保真项来建模颜色通道间的关系，通过包含四个核的四元数卷积核来捕捉整体模糊和颜色通道间的未知相互依赖关系。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将彩色图像转换为灰度图或分别处理每个颜色通道，忽略了颜色通道之间的关系，这限制了彩色图像盲反卷积的效果。

Method: 设计了一个专门用于彩色图像盲反卷积的四元数保真项，利用四元数卷积核的特性，包含一个类似非负卷积核的核来捕捉整体模糊，以及三个无约束的卷积核分别对应红、绿、蓝通道来建模它们之间的未知相互依赖关系。在盲反卷积过程中使用归一化四元数核来保持图像强度。

Result: 在真实模糊彩色图像数据集上的大量实验表明，该方法有效消除了伪影，显著提高了去模糊效果。

Conclusion: 该方法展示了作为彩色图像反卷积强大工具的潜力，通过建模颜色通道间的关系显著改善了去模糊性能。
...
TL;DR: MolSight是一个用于光学化学结构识别(OCSR)的三阶段学习框架，通过预训练、多粒度微调和强化学习后训练优化，专门解决立体化学信息识别的挑战，在立体分子识别方面达到最先进性能。
...
Method: 采用三阶段训练范式：1) 在大规模但嘈杂的数据集上进行预训练；2) 使用具有更丰富监督信号的数据集进行多粒度微调，探索化学键分类和原子定位等辅助任务；3) 使用强化学习进行后训练优化，并引入新的立体化学结构数据集。

Result: 实验结果表明，MolSight在（立体）化学光学结构识别方面达到了最先进的性能，即使模型参数相对紧凑，GRPO算法也能进一步提升立体分子识别性能。

Conclusion: MolSight框架通过三阶段训练方法有效解决了OCSR中立体化学信息识别的挑战，为化学信息学、药物发现和LLM应用提供了强大的自动化工具。
...
TL;DR: BiFingerPose是一种基于双模态的指尖姿态估计算法，结合电容图像和指纹补丁，能够准确预测全面的指尖姿态信息，包括传统方法无法估计的滚转角，并在预测性能、任务完成效率和用户操作准确性方面显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于电容图像的指尖姿态估计算法在便携设备上主要局限于估计俯仰角和偏航角，且在大角度输入（特别是大于45度）时准确性降低，无法估计滚转角。

Method: 提出BiFingerPose算法，探索双模态输入，包括来自触摸屏的电容图像和来自屏下指纹传感器的指纹补丁，实现全面的指尖姿态估计。

Result: 在12人用户研究中，BiFingerPose相比之前SOTA方法在预测性能上提升超过21%，任务完成效率提高2.5倍，用户操作准确性提升23%。

Conclusion: 该方法能够可靠估计滚转角，显著提升其他姿态参数的预测性能，在增强认证安全性和改善交互体验方面具有实际优势。
...
<details>
  <summary>Details</summary>
Motivation: 大多数多模态大语言模型在解释和推断三维空间空间布局方面存在空间推理能力有限的问题，主要源于视觉编码器（如CLIP）的损失性嵌入仅关注实例级语义特征。

Method: 提出基于几何和语义特征层次化融合的新型视觉编码器，通过层次适配器将自监督学习的几何特征与CLIP语义特征互补，使用预训练LLaVA模型高效训练，并采用随机特征丢弃优化以避免仅依赖CLIP编码器的平凡解。

Result: 实验结果显示SpatialGeo在空间推理任务中显著提升准确率，在SpatialRGPT-Bench上将最先进模型的准确率提高了至少8.0%，同时推理期间内存成本减少约50%。

Conclusion: SpatialGeo通过增强空间感知能力有效提升了多模态大语言模型的空间推理性能，在保持高效性的同时显著改善了空间理解能力。
...
TL;DR: NoPe-NeRF++是一种无需位姿先验的局部到全局优化算法，通过特征匹配初始化、局部联合优化和全局几何一致性约束，显著提升了NeRF训练中的相机位姿估计精度和新视角合成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法（特别是NoPe-NeRF）仅关注图像局部关系，在复杂场景中难以恢复准确的相机位姿，需要一种结合局部和全局线索的优化方法。

Method: 1. 基于显式特征匹配的相对位姿初始化；2. 局部联合优化提升位姿估计；3. 引入包含特征轨迹的全局束调整优化，通过几何一致性约束进一步精化位姿。

Result: 在基准数据集上的广泛评估表明，该方法在位姿估计精度和新视角合成方面均优于现有最先进方法，在挑战性场景中表现出优异的鲁棒性。

Conclusion: NoPe-NeRF++是首个将局部和全局线索与NeRF无缝结合的工作，验证了所提出设计选择的有效性，显著提升了无位姿先验NeRF训练的性能。
...
TL;DR: 该论文提出了一种改进生成图像中透明物体折射效果的方法，通过同步像素并使用斯涅尔折射定律来生成更符合光学物理约束的图像。


<details>
  <summary>Details</summary>
Motivation: 现有生成图像模型在处理透明物体时表现不佳，特别是折射效果，因为它们未能充分学习光学定律来准确渲染折射物体。

Method: 在生成过程的每一步，使用斯涅尔折射定律对物体边界内外的像素进行扭曲和合并同步；对于通过折射或反射可见但未直接观察到的表面，通过将图像与以物体为中心的全景图同步来恢复其外观。

Result: 该方法生成了更符合光学物理约束的图像，折射效果更加真实可信。

Conclusion: 提出的像素同步和斯涅尔定律应用方法能够显著改善生成图像中透明物体的折射渲染质量，使其更符合物理光学原理。
...
TL;DR: 提出了一个统一框架用于多媒体绘画过程生成，通过语义驱动的风格控制机制和跨媒体风格增强，实现一致的纹理演化和过程转移，并采用反向绘画训练策略确保生成过程平滑且符合人类创作流程。


<details>
  <summary>Details</summary>
Motivation: 现有的绘画教程视频缺乏交互性和个性化，而现有生成模型在跨媒体泛化和保持时间一致性方面存在困难，无法忠实再现人类创作流程。

Method: 将多种媒体嵌入扩散模型的条件空间，使用跨媒体风格增强，采用反向绘画训练策略，并构建大规模真实绘画过程数据集。

Result: 在LPIPS、DINO和CLIP指标上取得强劲结果，实现了跨媒体一致性、时间连贯性和最终图像保真度。

Conclusion: 提出的感知距离剖面曲线能够定量建模创作序列，反映了人类艺术创作进程，为绘画过程生成提供了有效解决方案。
...
TL;DR: DSeq-JEPA是一种改进的联合嵌入预测架构，通过引入判别性顺序预测机制，结合JEPA的潜在预测和GPT风格的顺序推理，从最具信息量的区域开始逐步预测，提升了视觉表示的质量。


<details>
  <summary>Details</summary>
Motivation: 现有的I-JEPA方法对所有区域进行统一和独立的预测，缺乏对预测位置和顺序的明确指导。受人类视觉感知的启发，需要选择性、顺序性地从最具信息量到次要区域进行注意力部署。

Method: DSeq-JEPA首先基于transformer生成的显著图识别主要判别性区域，然后按照判别性顺序预测后续区域，形成从主要到次要线索的课程式语义进展。

Result: 在图像分类、细粒度视觉分类、检测分割以及低级推理任务上的广泛实验表明，DSeq-JEPA相比I-JEPA变体始终关注更具判别性和泛化性的表示。

Conclusion: DSeq-JEPA成功将预测性和自回归自监督学习相结合，通过引入判别性顺序预测机制，显著提升了视觉表示学习的性能。
...
TL;DR: SuperQuadricOcc提出了一种基于超二次曲面的语义占据估计方法，通过超二次曲面表示减少基元数量，实现内存占用减少75%、推理速度提升124%，并在Occ3D数据集上mIoU提升5.9%。


<details>
  <summary>Details</summary>
Motivation: 高斯表示在自监督占据估计中广泛使用，但大量高斯基元显著增加内存需求，不适合实时推理。超二次曲面允许减少基元数量和内存需求，但由于缺乏超二次曲面光栅化器，难以实现自监督占据模型。

Method: 采用超二次曲面场景表示，利用多层二十面体细分的高斯近似超二次曲面，在训练期间启用高斯光栅化进行监督。

Result: 在Occ3D数据集上，相比先前高斯方法，内存占用减少75%，推理速度提升124%，mIoU提升5.9%，基元数量减少84%，首次实现实时推理同时保持竞争力性能。

Conclusion: SuperQuadricOcc是首个实现实时推理的占据模型，通过超二次曲面表示显著减少基元数量和内存需求，同时保持竞争性能，代码将开源发布。
...
TL;DR: 提出ATAC方法，通过在CLIP嵌入空间计算增强引起的漂移向量来推断语义恢复方向，显著提升对抗性攻击下的鲁棒性，平均比现有方法提高近50%。


<details>
  <summary>Details</summary>
Motivation: CLIP在零样本图像-文本匹配方面表现出色，但对图像对抗性扰动高度脆弱。对抗性微调成本过高，现有测试时防御方法鲁棒性有限。

Method: ATAC直接在CLIP嵌入空间中操作，计算增强引起的漂移向量来推断语义恢复方向，基于这些潜在漂移的角度一致性来校正嵌入。

Result: 在广泛基准测试中，ATAC始终实现极高的鲁棒性，平均比先前最先进方法提高近50%，同时计算开销最小。在非常规和极端设置下保持最先进鲁棒性，甚至对自适应攻击也实现非平凡鲁棒性。

Conclusion: ATAC是在CLIP嵌入空间中测试时对抗性防御新范式中的高效方法。
...
TL;DR: SVRecon将稀疏体素栅格化范式扩展到高保真表面重建任务，通过整合SDF来解决稀疏体素优化中的局部最小值问题，实现了快速收敛和强重建精度。


<details>
  <summary>Details</summary>
Motivation: 稀疏体素在空间上与邻居解耦且边界锐利，这使它们在优化过程中容易陷入局部最小值。虽然SDF值提供了自然平滑连续的几何场，但在独立参数化的稀疏体素间保持这种平滑性具有挑战性。

Method: 通过(1)使用视觉几何模型进行鲁棒几何初始化，(2)强制执行父-子和兄弟体素组间一致关系的空间平滑损失，来促进连贯平滑的体素结构。

Result: 在多个基准测试上的广泛实验表明，该方法实现了强大的重建精度，同时具有持续快速的收敛速度。

Conclusion: SVRecon通过整合SDF和空间平滑约束，成功解决了稀疏体素优化中的局部最小值问题，实现了高效准确的三维表面重建。
...
TL;DR: MorphSeek是一个用于可变形图像配准的细粒度表示级策略优化框架，通过将配准问题重新表述为潜在特征空间中的空间连续优化过程，使用随机高斯策略头建模潜在特征分布，实现高效探索和从粗到精的细化。


<details>
  <summary>Details</summary>
Motivation: 传统可变形图像配准面临高维变形空间和体素级监督稀缺的挑战，现有强化学习方法将空间投影为粗糙的低维表示，限制了捕捉空间变异变形的能力。

Method: 提出MorphSeek框架，在编码器顶部引入随机高斯策略头来建模潜在特征分布，结合无监督预热和弱监督微调，通过多轨迹采样稳定训练并提高标签效率。
...
TL;DR: 本文提出了一种名为MCMoE的缺失模态补全框架，通过专家混合机制统一单模态和联合表示学习，解决多模态动作质量评估中模态缺失的问题。


<details>
  <summary>Details</summary>
Motivation: 现实应用中部分模态经常在推理阶段不可用，导致现有多模态模型失效并引发性能急剧下降，需要解决模态缺失情况下的鲁棒性评估问题。
...
Conclusion: MCMoE框架有效解决了多模态动作质量评估中的模态缺失问题，实现了在完整和缺失模态情况下的鲁棒性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统单教师对抗蒸馏方法存在知识多样性有限、收敛慢、难以平衡鲁棒性和准确性等问题，需要更有效的多教师协作方法。

Method: 采用双教师知识融合架构，包含动态权重分配策略和自适应sigmoid加权函数，协同优化干净特征保持和鲁棒特征增强。

Result: 在ImageNet上ViT-B-32模型的鲁棒精度提升+4.32%，零样本精度提升+3.5%，训练效率比单教师方法提高2.3倍。
...
### [80] [Counterfactual World Models via Digital Twin-conditioned Video Diffusion](https://arxiv.org/abs/2511.17481)
*Yiqing Shen,Aiza Maksutova,Chenjia Li,Mathias Unberath*

Main category: cs.CV

TL;DR: CWMDT框架将标准视频扩散模型转化为反事实世界模型，通过构建场景的数字孪生表示，使用大语言模型进行反事实推理，并生成修改后的视觉序列。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要基于事实观察进行前向模拟，但许多应用需要回答反事实查询（如"如果移除这个物体会发生什么？"），因此需要能够处理干预措施的反事实世界模型。

Method: 1) 构建场景的数字孪生，以结构化文本编码对象及其关系；2) 使用大语言模型推理反事实干预如何随时间传播改变场景；3) 用修改后的表示条件化视频扩散模型生成反事实视觉序列。

Result: 在两个基准测试上的评估表明，CWMDT方法实现了最先进的性能，表明数字孪生等替代视频表示为基于视频前向模拟的世界模型提供了强大的控制信号。

Conclusion: CWMDT框架成功地将视频扩散模型转化为有效的反事实世界模型，通过结构化表示和语言模型推理实现了对特定场景属性的针对性干预。
...
### [81] [Radar2Shape: 3D Shape Reconstruction from High-Frequency Radar using Multiresolution Signed Distance Functions](https://arxiv.org/abs/2511.17484)
*Neel Sortur,Justin Goodwin,Purvik Patel,Luis Enrique Martinez,Tzofi Klinghoffer,Rajmonda S. Caceres,Robin Walters*

Main category: cs.CV

TL;DR: Radar2Shape是一个去噪扩散模型，能够从部分观测的高频雷达信号中重建任意3D形状，通过将雷达频率与多分辨率形状特征相关联来实现这一目标。


<details>
  <summary>Details</summary>
Motivation: 从高频雷达信号确定3D物体形状在商业和航空航天应用中至关重要，但现有深度学习方法难以表示任意形状或处理有限视角采集的真实雷达信号，而光学3D重建方法将雷达信号简单视为相机视图时效果不佳。

Method: 采用两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后通过以类似粗到细的方式对雷达信号频率进行条件化，扩散到这个潜在空间中。

Result: Radar2Shape能够成功从部分观测的雷达信号重建任意3D形状，并在两种不同模拟方法和真实数据上展现出鲁棒的泛化能力。
...
### [82] [An Artificial Intelligence Framework for Measuring Human Spine Aging Using MRI](https://arxiv.org/abs/2511.17485)
...
TL;DR: 提出了一种基于计算机视觉的深度学习方法来估计脊柱年龄，使用超过18,000个MRI序列图像，并通过脊柱年龄差(SAG)评估脊柱健康状况。


<details>
  <summary>Details</summary>
Motivation: 脊柱是人体重要结构，易受年龄相关退化的影响。需要一种量化脊柱健康状况的方法来识别退行性脊柱疾病。

Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退化聚类，通过消融研究选择模型，评估数据大小、损失函数和不同脊柱区域的影响。

Result: 脊柱年龄差(SAG)与椎间盘突出、骨赘、椎管狭窄、骨折等退行性条件相关，也与吸烟和体力劳动等生活方式因素相关。
...
### [83] [Downscaling Intelligence: Exploring Perception and Reasoning Bottlenecks in Small Multimodal Models](https://arxiv.org/abs/2511.17487)
*Mark Endo,Serena Yeung-Levy*

Main category: cs.CV

TL;DR: 本文分析了多模态模型中LLM规模缩减对视觉能力的影响，发现视觉能力下降比语言能力更严重，提出了Extract+Think方法通过视觉提取调优和逐步推理来提升效率与性能。


<details>
  <summary>Details</summary>
Motivation: 多模态模型规模扩展带来了显著的视觉理解能力提升，但实际应用需要更小、更高效的模型。本文旨在系统分析LLM规模缩减如何影响多模态能力，特别是视觉能力。

Method: 提出Extract+Think方法：1）视觉提取调优，训练模型提取与指令相关的视觉细节；2）逐步推理，利用提取的视觉细节生成答案。

Result: 研究发现LLM规模缩减对视觉能力的影响比对语言能力的影响更严重，视觉提取调优能有效缓解这一瓶颈，在效率和性能上达到新标准。

Conclusion: Extract+Think方法通过分离视觉提取和推理过程，有效解决了LLM规模缩减导致的视觉能力下降问题，为高效多模态模型提供了新思路。
...
### [84] [Video-R4: Reinforcing Text-Rich Video Reasoning with Visual Rumination](https://arxiv.org/abs/2511.17490)
*Yolo Yunlong Tang,Daiki Shimada,Hang Hua,Chao Huang,Jing Bi,Rogerio Feris,Chenliang Xu*

Main category: cs.CV

TL;DR: Video-R4是一个视频推理LMM，通过视觉反刍机制迭代选择帧、放大关键区域、重新编码像素并更新推理状态，在文本丰富视频理解任务上取得SOTA效果。
...
Method: 提出视觉反刍机制，构建两个数据集用于监督实践和强化学习，采用多阶段反刍学习框架通过SFT和GRPO-based RL逐步微调7B LMM。
...
### [85] [EvDiff: High Quality Video with an Event Camera](https://arxiv.org/abs/2511.17492)
*Weilun Li,Lei Sun,Ruixi Gao,Qi Jiang,Yuqin Ma,Kaiwei Wang,Ming-Hsuan Yang,Luc Van Gool,Danda Pani Paudel*

Main category: cs.CV

TL;DR: EvDiff是一个基于事件的扩散模型，通过单步前向扩散和时序一致的EvEncoder，从单色事件流生成高质量彩色视频，无需配对事件-图像数据集。


<details>
  <summary>Details</summary>
Motivation: 传统方法直接将事件映射到强度图像存在感知质量差、模型容量和训练数据难以扩展的问题，需要一种能生成高质量视频且不依赖配对数据的方法。

Method: 提出EvDiff扩散模型，采用单步前向扩散降低计算成本，设计时序一致的EvEncoder，并引入替代训练框架利用大规模图像数据集。

Result: 在真实数据集上的实验表明，该方法在保真度和真实感之间达到良好平衡，在像素级和感知指标上均优于现有方法。

Conclusion: EvDiff能够仅从单色事件流生成高质量彩色视频，解决了事件到图像重建的模糊性问题，且不依赖配对训练数据。
...
Method: 创建大规模多模态数据集用于指令引导3D编辑，涵盖添加、删除和修改任务；探索两种条件策略：传统交叉注意力机制和新型3D令牌拼接方法。

Result: 令牌拼接方法在参数效率上更优且性能更好；在生成质量、3D一致性和指令保真度方面优于现有2D提升方法。

Conclusion: 提出的原生3D编辑框架在效率和质量上都优于现有方法，为3D内容创作提供了新的基准。
...
TL;DR: 本文提出了语义金字塔索引（SPI），一种用于向量数据库的多分辨率向量索引框架，通过查询自适应分辨率控制来优化检索增强生成系统，在保持语义覆盖的同时显著加速检索。


<details>
  <summary>Details</summary>
Motivation: 现有向量数据库检索管道依赖于平面或单分辨率索引结构，无法适应不同用户查询所需的变化语义粒度，导致检索速度和上下文相关性之间的次优权衡。

Method: SPI在文档嵌入上构建语义金字塔，通过轻量级分类器为每个查询动态选择最佳分辨率级别，实现从粗到细表示的渐进式检索。

Result: 在多个RAG任务上的评估显示，SPI实现了高达5.7倍的检索加速和1.8倍的内存效率提升，同时端到端QA F1分数提高了2.5分。

Conclusion: SPI框架与现有向量数据库基础设施兼容，可立即部署到生产RAG系统中，为检索质量和延迟提供了理论保证。
...
TL;DR: Bench360是一个用于本地LLM推理的综合性基准测试框架，支持用户自定义任务、数据集和指标，自动测试不同模型、推理引擎和量化级别，揭示任务性能与系统效率之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 当前LLM基准测试工具设计目标狭窄，未能整合系统指标和任务特定指标，缺乏统一的易用基准测试框架来支持多种推理引擎、使用场景和量化级别。

Method: 开发Bench360框架，允许用户定义自定义任务、数据集和相关指标，自动对选定的LLM、推理引擎和量化级别在不同使用场景（单流、批处理和服务器）下进行基准测试。

Result: 在四个常见LLM任务（通用知识推理、问答、摘要和文本到SQL）上测试三个硬件平台和四个推理引擎，揭示了任务性能与系统效率之间的多个有趣权衡，不同推理引擎和模型存在显著差异。

Conclusion: 本地推理没有单一最佳设置，这强烈证明了需要像Bench360这样的框架来帮助用户找到适合其特定需求的配置。
...
<details>
  <summary>Details</summary>
Motivation: 工业级大型语言模型往往忽视低资源语言如突尼斯阿拉伯语的理解能力，这可能导致数百万突尼斯人无法用自己的语言与AI充分交互，被迫转向法语或英语，威胁方言保存并影响年轻一代的语言偏好。
...
TL;DR: 提出EliDecide方法，通过椭圆体决策边界解决文本开放意图分类问题，相比传统球形边界能更好地适应不同特征方向的分布差异，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有自适应决策边界方法假设已知类呈各向同性分布，将边界限制为球形，忽略了不同特征方向的分布差异，限制了开放意图检测能力。

Method: 使用监督对比学习获得判别性特征空间；用可学习矩阵参数化椭圆体边界；通过双损失函数优化边界，平衡经验风险和开放空间风险。

Result: 在多个文本意图基准测试和问题分类数据集上达到最先进性能，椭圆体的灵活性展现出优越的开放意图检测能力和泛化潜力。

Conclusion: EliDecide方法通过椭圆体决策边界有效解决了开放意图分类问题，在复杂开放世界场景中具有广泛的适用性。
...
Method: 开发了一个实用、可重复、模型无关的评估程序，使用Schwartz的基本人类价值观理论和对话数据集进行结构化评估，比较基线提示和明确基于价值观的提示。

Result: 实验表明，即使不改变模型或动态优化提示，价值观引导也是可能的，通过明确基于价值观的提示可以有效地引导生成文本朝向特定人类价值观。

Conclusion: 该方法提供了一种在不修改模型的情况下实现价值观引导的有效途径，为动态价值观对齐提供了实用解决方案。
...
### [92] [Falsely Accused: How AI Detectors Misjudge Slightly Polished Arabic Articles](https://arxiv.org/abs/2511.16690)
*Saleh Almohaimeed,Saad Almohaimeed,Mousa Jari,Khaled A. Alobaid,Fahad Alotaibi*

Main category: cs.CL

TL;DR: 该论文研究了AI检测模型对轻微AI润色的人类撰写文章的误判问题，构建了两个阿拉伯语数据集来评估14个LLM和商业AI检测器的性能。


<details>
  <summary>Details</summary>
Motivation: 解决AI检测模型对轻微AI润色的人类文章产生误判的问题，避免错误指控作者使用AI抄袭，并维护AI检测器的可信度。

Method: 生成两个阿拉伯语数据集：第一个包含800篇文章（半AI生成半人类撰写），用于评估14个模型；第二个包含400篇人类文章经10个LLM用4种润色设置处理后的16400个样本，用于评估8个最佳模型。

Result: 所有AI检测器都会错误地将大量文章归因于AI。最佳LLM Claude-4 Sonnet准确率从83.51%降至57.63%；最佳商业模型originality.AI准确率从92%降至12%。

Conclusion: 轻微AI润色会显著影响AI检测模型的性能，导致对真实人类撰写文章的误判，这对AI检测器的可靠性构成严重挑战。
...
### [93] [Reproducibility Report: Test-Time Training on Nearest Neighbors for Large Language Models](https://arxiv.org/abs/2511.16691)
*Boyang Zhou,Johan Lindqvist,Lindsey Li*

Main category: cs.CL

TL;DR: 本文复现了基于最近邻的测试时训练方法，使用RoBERTa嵌入和Faiss索引检索20个最近邻，在多个语言模型上验证了该方法能显著降低困惑度和比特率，特别是在结构化数据集上效果最明显。


<details>
  <summary>Details</summary>
Motivation: 验证最近邻测试时训练方法的有效性和通用性，探索该方法在不同规模语言模型上的表现，并解决大规模检索的内存效率问题。
...
### [94] [How Language Directions Align with Token Geometry in Multilingual LLMs](https://arxiv.org/abs/2511.16693)
*JaeSeong Kim,Suan Lee*

Main category: cs.CL

TL;DR: 本文通过系统分析多语言大模型的内部表示空间，发现语言信息在第一个Transformer块中急剧分离，并在整个模型深度中保持线性可分。语言方向与词汇嵌入的对齐与训练数据的语言组成密切相关，中文包含模型比英语中心模型有4.21倍的结构印记效应。


<details>
  <summary>Details</summary>
Motivation: 多语言LLM在多种语言上表现出强大性能，但对其内部表示空间中语言信息的结构方式以及在不同层中的涌现机制缺乏系统分析。

Method: 对6个多语言LLM的268个Transformer层进行全面探测研究，使用线性和非线性探针以及新的Token-Language Alignment分析来量化语言编码的层间动态和几何结构。

Result: 语言信息在第一个Transformer块中急剧分离（从第0层到第1层提升76.4±8.2个百分点），在整个模型深度中几乎完全线性可分。中文包含模型的ZH Match@Peak达到16.43%，而英语中心模型仅为3.90%。

Conclusion: 多语言LLM不是通过表面脚本特征来区分语言，而是通过训练语料库塑造的潜在表示结构。这些发现为多语言表示学习中的数据组成策略和公平性提供了实践洞见。
...
### [95] [Hierarchical Retrieval with Out-Of-Vocabulary Queries: A Case Study on SNOMED CT](https://arxiv.org/abs/2511.16698)
*Jonathon Dilworth,Hui Yang,Jiaoyan Chen,Yongsheng Gao*

Main category: cs.CL

TL;DR: 提出了一种基于语言模型的SNOMED CT本体嵌入方法，用于解决词汇表外查询的分层概念检索问题，在OOV查询场景下优于基线方法。


<details>
  <summary>Details</summary>
Motivation: SNOMED CT作为生物医学本体，其知识检索面临语言歧义、同义词、多义词等挑战，特别是在查询词汇不在本体词汇表内(OOV)时问题更加严重。

Method: 采用基于语言模型的本体嵌入方法，构建了针对SNOMED CT的OOV查询标注数据集，测试直接子类和较少相关祖先的检索效果。

Result: 该方法在OOV查询的分层概念检索任务中表现优于SBERT和两种词汇匹配基线方法。

Conclusion: 该方法虽然针对SNOMED CT开发，但具有通用性，可扩展到其他本体，已开源代码、工具和评估数据集。
...
TL;DR: JGU Mainz团队在BLP-2025代码生成共享任务中获胜，提出多智能体管道方法，通过代码生成和调试智能体的协作，在单元测试失败时自动修复程序，最终获得95.4%的Pass@1分数。


<details>
  <summary>Details</summary>
Motivation: 解决从孟加拉语指令生成代码的挑战，提高代码生成系统的准确性和鲁棒性，特别是在面对复杂指令和单元测试失败时的自动修复能力。

Method: 采用多智能体管道：首先由代码生成智能体根据输入指令生成初始解决方案，然后执行单元测试，仅将失败的测试案例转发给调试智能体，该智能体重新运行测试、提取错误轨迹，并基于错误信息、当前程序和相关测试案例生成修订解决方案。

Result: 在BLP-2025共享任务中获得第一名，Pass@1分数达到95.4%，证明了该方法的有效性。

Conclusion: 多智能体管道方法能够有效处理代码生成任务，特别是在面对测试失败时能够自动进行调试和修复，显著提高了代码生成系统的性能。
...
TL;DR: 本研究重新探讨了非宾格动词和非作格动词与施事性和终结性之间的语义联系，通过使用基于种子词的可解释维度分析，发现两者之间存在显著关联。


<details>
  <summary>Details</summary>
Motivation: 尽管传统观点认为描述施事行为的动词更倾向于非作格句法，而描述终结事件的动词更倾向于非宾格句法，但Kim等人(2024)的最新研究发现人类对施事性和终结性的评分并不能很好地预测不及物动词的句法行为。本研究旨在重新审视这一问题。

Method: 使用基于种子词的可解释维度，这些种子词位于施事性和终结性尺度的对立极点上，通过计算这些维度来分析不及物动词的句法行为。

Result: 研究结果支持了非作格性/非宾格性与施事性/终结性之间的联系，表明使用可解释维度结合人类判断可以为难以在评分任务中评估的语义属性提供有价值的证据。
...
TL;DR: PEPPER是一种针对文本到图像扩散模型后门攻击的防御方法，通过重写输入提示为语义不同但视觉相似的描述，并添加不显眼元素来破坏触发器，提高模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型容易受到后门攻击，攻击者通过在输入提示中嵌入触发器来引导生成有害或意外内容，需要有效的防御机制。

Method: 提出PEPPER防御方法，通过感知引导的扰动重写输入提示，创建语义不同但视觉相似的描述，并添加不显眼元素来稀释触发器令牌的影响。

Result: 实验表明PEPPER对基于文本编码器的攻击特别有效，显著降低攻击成功率同时保持生成质量，且能与现有防御方法结合使用。

Conclusion: PEPPER提供了对抗文本到图像扩散模型后门攻击的有效防御方案，具有强鲁棒性和通用性，代码将在GitHub发布。
...
TL;DR: 提出了一种无需参考的无监督指标，用于评估大语言模型生成响应的简洁性，通过压缩比和词移除压缩来量化冗余内容。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常生成冗长、冗余的响应，这降低了清晰度和用户满意度，并增加了模型开发者的成本，特别是按输出token收费的专有模型。

Method: 通过计算三个指标的平均值来量化非必要内容：1）原始响应与LLM抽象摘要的压缩比；2）原始响应与LLM提取摘要的压缩比；3）词移除压缩，即LLM在保持意义的前提下移除尽可能多的非必要词语。

Result: 实验结果表明，该指标能够有效识别LLM输出中的冗余内容，为对话AI系统提供无需人工标注的自动化简洁性评估工具。

Conclusion: 提出的无参考简洁性评估指标为自动评估LLM响应简洁性提供了实用工具，无需依赖人工标注的黄金标准。
...
TL;DR: 本文提出Soft Concept Mixing (SCM)训练方案，通过在训练中直接暴露软概念表示来弥合LLMs推理中的软概念与训练中的离散token之间的差距，从而提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常通过生成离散token进行推理，这限制了其表达能力。虽然软概念推理是一个有前景的方向，但LLMs是在离散token上训练的，存在训练与推理的不匹配问题。

Method: 提出SCM训练方案：构建软概念向量（通过概率加权平均嵌入形成），将其混合到模型的隐藏状态中，并使用强化学习优化整个潜在推理过程。
...
TL;DR: 提出了一种新的训练方法，通过将TRM的潜在推理视为无分类器引导和隐式策略改进算法，显著提高了训练效率，减少了18倍前向传播次数并消除了停止机制，在ARC-1上达到24%准确率。


<details>
  <summary>Details</summary>
Motivation: 研究如何以最小的改动进一步提高Tiny Recursive Models (TRMs)的效率，这些小型循环架构在复杂推理任务上已显示出优于大型语言模型的表现。

Method: 将TRM的潜在推理框架化为无分类器引导和隐式策略改进算法，提出一种新颖的训练方案，为训练期间的每个循环提供目标。

Result: 方法显著提高了训练效率，总前向传播次数减少18倍，消除了停止机制，同时保持与标准TRM相当的质量。在ARC-1上仅用0.8M参数就达到24%准确率，优于大多数LLMs。

Conclusion: 提出的训练方案能够显著提高TRMs的训练效率，在保持性能的同时大幅减少计算开销，证明了在复杂推理任务上小型模型可以通过优化训练方法达到甚至超越大型模型的性能。
...
TL;DR: 本文研究了训练数据的统计特性与归纳头形成之间的关系，发现批量大小和上下文长度的组合可以预测归纳头的形成点，表面二元重复频率和可靠性强烈影响归纳头形成，并确定了这两个值的精确帕累托边界。


<details>
  <summary>Details</summary>
Motivation: 虽然专门的注意力头（称为归纳头）被认为是现代语言模型强大上下文学习能力的基础，但其形成的精确机制仍不清楚。本研究旨在探究训练数据的统计特性与归纳头形成之间的关系。

Method: 通过分析自然和合成数据的统计特性，研究批量大小、上下文长度、表面二元重复频率和可靠性对归纳头形成的影响，并确定这些参数之间的精确关系。

Result: 发现批量大小和上下文长度的简单组合可以预测归纳头的形成点；表面二元重复频率和可靠性强烈影响归纳头形成，并找到了这两个值的精确帕累托边界；当频率和可靠性较低时，分类性和边际分布形状变得重要。

Conclusion: 局部依赖性与高二元重复频率和可靠性足以促进归纳头形成，但当这些值较低时，需要考虑其他统计特性如分类性和边际分布形状。
...
TL;DR: 提出了一种新颖的论据感知定量摘要任务，通过ARQUSUMM框架揭示对话中的论点结构并量化论证强度。


<details>
  <summary>Details</summary>
Motivation: 现有对话摘要研究未能深入挖掘句子内部的论据结构，无法揭示论点的claim-reason关系和量化论证强度。

Method: ARQUSUMM框架利用LLM少样本学习识别句子中的命题及其claim-reason关系，采用论据结构感知聚类算法聚合论点并量化支持度。

Result: 实验表明ARQUSUMM在论据结构表示、文本质量和量化准确性方面优于现有对话和定量摘要模型。

Conclusion: 该研究成功实现了论据感知的定量摘要，生成的摘要对用户更有帮助，具有高质量的文本和准确的量化结果。
...
TL;DR: 本研究提出了一种监督微调方法，用于增强湖南历史名人领域的特定信息提取能力，通过构建细粒度指令模板和指令调优数据集，在低资源环境下显著提升了四个大型语言模型的知识提取性能。


<details>
  <summary>Details</summary>
Motivation: 解决湖南历史名人领域系统数据资源有限、通用模型在低资源环境下领域知识提取和结构化输出生成性能不佳的问题，推动历史文化研究中的知识提取和图谱构建。

Method: 设计面向湖南历史名人领域的细粒度模式引导指令模板，构建指令调优数据集，并对Qwen2.5-7B、Qwen3-8B、DeepSeek-R1-Distill-Qwen-7B和Llama-3.1-8B-Instruct四个公开大模型进行参数高效的指令微调。

Result: 所有模型在微调后都表现出显著的性能提升，其中Qwen3-8B表现最佳，在100个样本和50次训练迭代下达到89.3866分。

Conclusion: 本研究为垂直领域大模型在区域历史文化领域的微调提供了新思路，展示了其在文化遗产知识提取和知识图谱构建中具有成本效益的应用潜力。
...
TL;DR: 该研究构建了视觉说服力判断数据集，探索视觉语言模型对视觉说服因素的理解能力，发现模型存在召回偏向，高层次的语义对齐是最强预测因素，而基于对象的简洁理由能显著提升性能。
...
Result: 视觉语言模型存在召回偏向——过度预测高说服力，对低/中层次特征的判别能力较弱。高层次语义对齐是最强预测因素。简单指令或无引导推理效果有限，而基于对象的简洁理由能显著提升精确度和F1分数。

Conclusion: 视觉语言模型的核心局限不在于识别说服性对象，而在于将这些对象与沟通意图联系起来。
...
TL;DR: 本文提出了一个可解释的自动评分框架AnalyticScore，通过四个可解释性原则(FGTI)和基于LLM的特征提取方法，在保持高评分精度的同时提供可解释性。
...
Result: 在ASAP-SAS数据集的10个项目上，AnalyticScore的评分精度超过许多不可解释方法，与不可解释SOTA方法仅相差0.06 QWK；特征化行为与人类标注者高度一致。

Conclusion: 提出的FGTI原则和AnalyticScore框架为可解释自动评分提供了可行的解决方案，在保持高精度的同时实现了良好的可解释性。
...
TL;DR: MUCH是首个针对声明级别不确定性量化的基准测试，包含4,873个样本，涵盖四种欧洲语言和四个开源LLM，提供完整的生成对数概率数据，并提出了高效的确定性分割算法。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型缺乏可靠性的问题，通过声明级别不确定性量化来提高模型的可信度。

Method: 构建包含多语言和多模型的数据集，提供24个生成对数概率，并提出仅需0.2%生成时间的确定性分割算法。

Result: 创建了MUCH基准测试，包含4,873个样本，支持四种语言和四个LLM，提供了完整的生成数据。

Conclusion: 当前方法在性能和效率方面仍有很大改进空间，MUCH基准为未来方法提供了公平可复现的评估框架。
...
TL;DR: 本文提出使用上下文压缩作为前置任务来无监督适应LLMs，生成紧凑的记忆token替代完整上下文用于下游序列预测，相比基于token级预测的方法表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs主要针对因果建模和下一个token预测优化，不适合生成整体文本表示。虽然已有研究引入前置任务来适应LLMs，但大多依赖token级预测目标，存在局限性。

Method: 探索上下文压缩作为前置任务，在压缩预训练中让模型学习生成紧凑的记忆token来替代完整上下文，用于下游序列预测。结合对比学习进一步改进表示质量。

Result: 精心设计的压缩目标显著提升了基于LLM的文本表示，优于基于token级前置任务的模型。通过对比学习产生的LLM2Comp模型在广泛任务上优于当代基于LLM的文本编码器，且样本效率更高，所需训练数据显著减少。

Conclusion: 上下文压缩是一种有效的LLM适应前置任务，能够产生高质量的文本表示，在多个下游任务中表现优异且具有更好的样本效率。
...
TL;DR: 提出了LangMark，一个多语言自动后编辑数据集，包含20.7万个英语到7种语言的三元组（源文本、机器翻译输出、人工后编辑），用于提升机器翻译质量。


<details>
  <summary>Details</summary>
Motivation: 解决自动后编辑领域缺乏大规模多语言数据集的问题，特别是针对神经机器翻译输出的数据集不足，阻碍了有效APE系统的发展。

Method: 构建并发布LangMark数据集，包含英语到巴西葡萄牙语、法语、德语、意大利语、日语、俄语和西班牙语的206,983个三元组，由专业语言学家标注。使用该数据集验证大语言模型通过少样本提示进行APE的有效性。

Result: 实验证明大语言模型通过少样本提示能够有效执行自动后编辑，改进领先的商业甚至专有机器翻译系统的翻译质量。

Conclusion: LangMark数据集将促进未来APE系统的开发和评估，为解决机器翻译错误提供重要资源。
...
TL;DR: 本文介绍了PLLuM项目中用于微调波兰大语言模型的指令数据集，包括有机、转换和合成指令的功能分类，并分享了人工编写与合成指令数据集在语言适应方面的观察。


<details>
  <summary>Details</summary>
Motivation: 为PLLuM项目开发波兰语大语言模型提供指令数据集，并探讨不同类型指令在语言适应中的影响。

Method: 使用有机、转换和合成三种类型的指令构建数据集，并分析人工编写与合成指令在语言适应中的差异。

Result: 发布了PLLuM指令语料库的第一个代表性子集，可用于指导其他LLM类似数据集的开发。

Conclusion: 该指令数据集对波兰语LLM开发具有重要意义，不同类型指令的使用对语言适应有不同影响。
...
TL;DR: 提出ABCA框架，通过因果推理分析LLM内部知识多样性，实现早期弃权，防止不可靠回答。


<details>
  <summary>Details</summary>
Motivation: 现有弃权方法依赖生成后信号，无法提前防止不可靠响应，需要早期弃权机制。

Method: ABCA框架基于方面因果效应评估知识可靠性，支持两种弃权类型：知识冲突和知识不足。

Result: 在标准基准测试中，ABCA提高了弃权可靠性，达到最先进性能，并增强了弃权决策的可解释性。

Conclusion: ABCA通过内部知识多样性分析有效实现早期弃权，提升LLM回答可靠性。
...
TL;DR: 本文提出了一种注意力引导特征融合(AGFF)模型，将统计特征和语义特征结合在统一框架中，通过注意力机制动态确定每种特征类型的相对重要性，在新闻文本分类任务中取得了优于传统统计方法和纯语义深度学习模型的性能。


<details>
  <summary>Details</summary>
Motivation: 传统统计方法虽然能捕捉词级重要性但缺乏上下文语义理解，而现代深度学习方法能理解语义但可能忽略简单但高影响力的统计指标。需要一种能结合两者优势的方法来提升新闻文本分类性能。

Method: 提出注意力引导特征融合(AGFF)模型，在统一框架中融合统计特征和语义特征，使用基于注意力的机制动态确定每种特征类型的相对重要性。

Result: 在基准新闻数据集上的评估显示，AGFF模型性能优于传统统计模型和纯语义深度学习模型。消融研究验证了融合过程中每个组件的贡献。

Conclusion: 策略性地整合不同特征类型能显著提高分类准确率，该模型能够平衡并利用统计和语义表示的互补优势，是实际新闻分类任务的有效解决方案。
...
TL;DR: 提出了一种名为E3的层剪枝框架，通过可微分掩码优化和熵感知知识蒸馏，在保持高性能的同时实现训练经济性和推理加速


<details>
  <summary>Details</summary>
Motivation: 解决现有层剪枝方法面临的性能下降、训练成本高和加速有限等实际部署挑战

Method: 使用Gumbel-TopK采样器进行可微分掩码优化，结合熵感知自适应知识蒸馏策略

Result: 在Qwen3-32B模型上剪枝25%层数，在MATH-500上达到96%准确率（仅比原模型96.8%下降0.8%），优于现有SOTA方法（95%），推理速度提升1.33倍，仅使用0.5B tokens（后训练数据量的0.5%）

Conclusion: E3框架在性能、训练成本和推理效率方面均优于现有最先进方法，为大型语言模型的实用部署提供了有效解决方案
...
TL;DR: PARROT框架用于评估大语言模型在权威和说服压力下的准确性退化现象（谄媚行为），通过双盲评估、置信度追踪和八状态行为分类系统，发现先进模型抗压能力强，而老旧/小型模型出现严重认知崩溃。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在社交压力下的谄媚行为（过度顺从），评估模型在权威和说服压力下的准确性退化，为安全部署提供重要指标。

Method: 使用PARROT框架：双盲评估比较中性问题和权威性错误问题；基于对数似然的置信度校准追踪；八状态行为分类系统（稳健正确、谄媚同意、强化错误等）。

Result: 评估22个模型的1302个MMLU式问题：先进模型（GPT-5、GPT-4.1等）跟随率低（≤11%），准确性损失小；老旧/小型模型（GPT-4、Qwen 2.5-1.5B）出现严重认知崩溃（跟随率80%-94%）；弱模型降低正确答案置信度，增加错误答案置信度。

Conclusion: 抵抗过度拟合压力应作为与准确性、伤害避免和隐私同等重要的主要目标，确保模型在现实世界中的安全部署。
...
TL;DR: MirageTVQA是一个新的基准测试，包含近6万个多语言QA对，专门评估视觉语言模型在包含视觉噪声的多语言表格上的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有表格QA数据集主要是英文且表格格式完美，与现实场景存在显著差距，需要评估模型在视觉不完美和多语言表格上的表现。

Method: 创建包含24种语言、近6万QA对的MirageTVQA基准测试，其中表格包含模拟扫描文档的真实视觉噪声。

Result: 评估显示领先VLM在视觉噪声下性能下降超过35%，且存在英语优先偏见，推理能力无法有效迁移到其他语言。

Conclusion: MirageTVQA为衡量和推动更鲁棒的表格推理VLM模型提供了基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 动机是创建高质量的爱沙尼亚语常识推理测试集，以更准确地评估大型语言模型在爱沙尼亚语上的语言能力和推理表现。

Method: 方法包括由翻译专家进行人工翻译和本地化适配，设计针对爱沙尼亚语特点的详细提示，并比较专有模型和开源模型在人工翻译与机器翻译数据上的表现。

Result: 结果显示模型在人工翻译的爱沙尼亚语数据集上表现略低于原始英语测试集，在机器翻译数据上表现明显更差，提示工程对翻译质量或模型准确性的改进有限。

Conclusion: 结论强调了在数据集翻译和适配过程中需要语言专家的参与，以确保对大型语言模型语言能力和推理的可靠且可解释的评估。
...
TL;DR: 基于语料库研究普通话单音节词在自然对话中的音高轮廓实现，发现词义对音高轮廓有显著影响，即使控制了多种变量后，词义仍是音高实现的重要预测因素。


<details>
  <summary>Details</summary>
Motivation: 研究普通话单音节词在自然对话中的音高轮廓实现，特别关注词义对音高轮廓的影响，挑战传统的声调理论。

Method: 使用广义加性模型分解观察到的音高轮廓，将其分解为与不同控制变量和语义预测因子相关的组成部分。

Result: 即使控制了词长、性别、说话者身份、声调语境、元音高度和话语位置等变量，词义仍是音高实现的强预测因子。词义比词本身是更好的预测因子，异形同音词具有不同的音高轮廓。
...
<details>
  <summary>Details</summary>
Motivation: 利用多模态表示来增强自然语言推理，通过将语言在视觉上下文中进行基础化，以解决文本偏见和表面启发式问题，实现更鲁棒的自然语言理解。

Method: 使用文本到图像模型生成前提的视觉表示，然后通过余弦相似度或视觉问答两种技术来比较这些视觉表示与文本假设，进行推理判断。

Result: 该方法在不进行任务特定微调的情况下实现了高准确率，表现出对文本偏见和表面启发式的鲁棒性，并通过设计的对抗数据集验证了其稳健性。

Conclusion: 利用视觉模态作为意义表示为鲁棒的自然语言理解提供了有前景的方向，证明了多模态方法在自然语言推理中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 受线性变换器中选择性门控机制能改善语言任务的启发，作者希望将这种选择性思想引入到位置编码中，以改进传统的固定角度旋转位置嵌入。

Method: 提出选择性RoPE机制，允许输入依赖的任意角度旋转，适用于线性和softmax变换器。该方法揭示了softmax注意力中隐藏的旋转结构，并在状态空间模型和门控线性变换器中实现了位置编码。

Result: 通过在门控变换器中装备选择性RoPE，在语言建模和困难序列任务（如复制、状态跟踪和检索）上表现出性能提升。
...
TL;DR: ReVeL框架将多项选择题改写为开放式问题，解决了MCQA中选项泄露信号导致评估不可靠的问题，通过不同答案类型的分类处理，在强化微调中实现了更好的数据效率和更鲁棒的奖励信号。


<details>
  <summary>Details</summary>
Motivation: 多项选择题（MCQA）的选项可能泄露可利用信号，导致准确性指标不可靠，并鼓励在强化微调中出现猜测行为，需要更可靠的评估方法。

Method: 提出ReVeL框架，将MCQA问题分类并重写为开放式问题，保持答案可验证性，使用GRPO对Qwen2.5-VL模型进行微调。

Result: 在多项选择基准测试中，ReVeL-OpenQA训练的模型匹配MCQA准确率，同时将OpenQA准确率提高约6个百分点；在评估中，ReVeL揭示了MCQA基准测试中高达20个百分点的分数膨胀。

Conclusion: ReVeL框架在强化微调和评估中均优于传统的MCQA方法，提供了更可靠的能力评估和更高效的训练数据利用。
...
<details>
  <summary>Details</summary>
Motivation: 传统RLVR在定理证明等需要中间推理的数学任务中可扩展性有限，而token级SFT容易退化为死记硬背。需要从中间推理中提取可学习信号的方法。

Method: 采用两阶段训练：先在数学计算和证明数据上进行自监督训练，然后在仅结果可验证的数学计算数据集上进行RLVR微调。通过掩码填充和步骤重排序构建过程级自监督奖励。

Result: 在AIME24、AIME25、AMC23和MATH500数据集上，MR-RLVR相比原始RLVR在固定采样和解码预算下实现了平均相对增益：+9.86% Pass@1、+5.27% Pass@5和+4.00% Pass@8。
...
### [122] [DDTime: Dataset Distillation with Spectral Alignment and Information Bottleneck for Time-Series Forecasting](https://arxiv.org/abs/2511.16715)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列预测通常需要大规模数据集和大量计算资源，而现有数据集蒸馏方法难以直接应用于时间序列，主要面临两个挑战：时间偏差（强自相关导致师生模型值项对齐失真）和样本多样性不足（缺乏明确的类别先验来规范轨迹多样性）。

Method: 基于一阶压缩分解构建DDTime框架：1）通过时间统计重新审视值项对齐，引入频域对齐机制缓解自相关引起的偏差；2）设计基于信息瓶颈原理的样本间正则化，增强合成轨迹的多样性和信息密度。

Result: 在20个基准数据集和多种预测架构上的实验表明，DDTime始终优于现有蒸馏方法，实现了约30%的相对精度提升，同时仅引入约2.49%的计算开销。

Conclusion: DDTime提供了一个轻量级且即插即用的时间序列数据集蒸馏解决方案，有效解决了时间偏差和样本多样性问题，在保持高精度的同时显著降低了计算需求。
...
### [123] [GCL-OT: Graph Contrastive Learning with Optimal Transport for Heterophilic Text-Attributed Graphs](https://arxiv.org/abs/2511.16778)
*Yating Ren,Yikun Ban,Huobin Tan*

Main category: cs.LG

TL;DR: 本文提出了GCL-OT框架，通过最优传输解决文本属性图中的多粒度异质性挑战，包括完全异质性、部分异质性和潜在同质性，实现了结构与文本的灵活双向对齐。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理异质性图时存在局限性，通常依赖同质性假设和硬优化目标，且将文本嵌入视为静态目标，导致对齐效果不佳。本文旨在解决文本属性图中的多粒度异质性挑战。

Method: 提出GCL-OT框架，使用最优传输进行图对比学习。针对部分异质性设计了RealSoftMax相似度估计器，针对完全异质性引入基于提示的过滤器，并采用OT引导的软监督来挖掘潜在同质性。

Result: 在九个基准测试上的广泛实验表明，GCL-OT始终优于最先进的方法，验证了其有效性和鲁棒性。理论分析显示GCL-OT能改善互信息边界和贝叶斯误差保证。

Conclusion: GCL-OT通过最优传输和针对性机制有效解决了文本属性图中的多粒度异质性挑战，实现了结构与文本的灵活双向对齐，在多个基准测试中表现出优越性能。
...
### [124] [Revisiting Multimodal KV Cache Compression: A Frequency-Domain-Guided Outlier-KV-Aware Approach](https://arxiv.org/abs/2511.16786)
*Yaoxin Yang,Peng Ye,Xudong Tan,Chongjun Tu,Maosen Zhao,Jia Hao,Tao Chen*

Main category: cs.LG

TL;DR: FlashCache是一个基于频域分析和异常KV感知的多模态KV缓存压缩框架，通过频域低通滤波提取KV矩阵主能量，识别并保留偏离主能量的异常KV对，结合动态预算分配，在保持任务性能的同时实现80% KV内存减少和1.69倍解码加速。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在显著推理开销，因为多模态KV缓存随视觉输入长度线性增长。现有压缩方法依赖注意力分数，与高效注意力内核不兼容且忽略值向量对注意力输出的贡献。

Method: 1. 频域分析发现多模态KV矩阵能量集中在低频，通过低通滤波提取主能量；2. 提出异常KV识别模块，在频域建模KV矩阵主成分，优先保留显著偏离的KV对；3. 设计动态预算分配模块，自适应确定每层KV缓存大小以保留更多异常KV。

Result: 在多个MLLM和基准测试中，FlashCache优于最先进的多模态KV压缩方法，实现80%更低的KV内存使用和1.69倍更快的解码速度，同时保持任务性能。

Conclusion: FlashCache通过频域引导的异常KV感知压缩框架，有效解决了多模态KV缓存的内存和计算开销问题，在保持模型性能的同时显著提升推理效率。
...
### [125] [A Vector Symbolic Approach to Multiple Instance Learning](https://arxiv.org/abs/2511.16795)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习MIL方法大多违反MIL的iff约束(包为正当且仅当至少一个实例为正)，导致性能指标虚高和泛化能力差，需要开发严格遵循MIL假设的模型。

Method: 使用向量符号架构(VSA)，将实例和概念表示为近乎正交的高维向量，通过代数运算在分类时强制执行iff约束；设计学习编码器将原始数据转换为VSA兼容向量；包含VSA驱动的MaxNetwork分类器。
...
### [126] [A Robust Federated Learning Approach for Combating Attacks Against IoT Systems Under non-IID Challenges](https://arxiv.org/abs/2511.16822)
*Eyad Gad,Zubair Md Fadlullah,Mostafa M. Fouda*

Main category: cs.LG

TL;DR: 本文研究了联邦学习在非独立同分布数据下的性能，比较了FedAvg、FedProx和Scaffold三种算法在IoT攻击检测中的表现，使用CICIoT2023数据集进行分类任务。


<details>
  <summary>Details</summary>
Motivation: 随着IoT设备数量和数据量的激增，传统机器学习面临资源受限和安全敏感环境的挑战。联邦学习虽然能解决隐私和资源问题，但非独立同分布数据的统计异质性严重影响了其效果。现有研究缺乏对联邦学习方法在IoT攻击检测中处理统计异质性的系统比较。

Method: 研究FedAvg、FedProx和Scaffold三种联邦学习算法在不同数据分布下的表现，使用CICIoT2023数据集进行大规模IoT攻击分类。

Result: 通过细致的分析和实验，揭示了这些联邦学习方法在统计异质性条件下的性能差异。

Conclusion: 研究为联邦学习在IoT安全领域的应用提供了有价值的见解，帮助研究者和从业者更好地理解和应对统计异质性带来的挑战。
...
### [127] [Monte Carlo Expected Threat (MOCET) Scoring](https://arxiv.org/abs/2511.16823)
*Joseph Kim,Saahith Potluri*

Main category: cs.LG

TL;DR: 本文介绍了MOCET指标，这是一个可解释且双重可扩展的指标，用于量化AI安全级别（ASL）威胁，特别是针对ASL-3+模型在生物安全领域带来的风险。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标如LAB-Bench、BioLP-bench和WMDP虽然能可靠评估模型提升和领域知识，但缺乏能更好情境化"真实世界风险"的指标，以支持LLM的安全案例，同时需要可扩展的开放式指标来跟上AI的快速发展。

Method: 提出了MOCET指标，这是一个可解释且双重可扩展（可自动化和开放式）的指标，能够量化真实世界风险。

Result: MOCET指标能够解决现有评估指标的不足，为AI安全级别威胁提供更好的风险评估工具。

Conclusion: MOCET作为可解释和双重可扩展的指标，能够有效量化AI模型在真实世界中的风险，特别是在生物安全领域，为AI安全评估提供了重要工具。
...
### [128] [ManifoldFormer: Geometric Deep Learning for Neural Dynamics on Riemannian Manifolds](https://arxiv.org/abs/2511.16828)
...
### [129] [Analysis of heart failure patient trajectories using sequence modeling](https://arxiv.org/abs/2511.16839)
*Falk Dippela,Yinan Yu,Annika Rosengren,Martin Lindgren,Christina E. Lundberg,Erik Aerts,Martin Adiels,Helen Sjöland*

Main category: cs.LG

TL;DR: 本文比较了六种序列模型在三个架构类别中的表现，发现在瑞典心衰队列中，Llama在预测性能、校准和鲁棒性方面表现最佳，Mamba次之，两者在模型规模相同时仅需75%的训练数据即可达到同等性能。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer和Mamba架构在临床预测任务中表现出色，但医学领域缺乏系统性的模型性能和效率实证分析方法。

Method: 在瑞典42820名心衰患者队列中，使用诊断、生命体征、实验室、药物和程序等EHR数据，评估六种序列模型在三个一年预测任务上的表现，并进行输入序列、模型配置和时间预处理技术的消融研究。

Result: Llama在所有任务中均获得最高的预测区分度、最佳校准和鲁棒性，其次是Mamba。两种架构都展示了高效的表征学习能力，小型配置即可超越其他大型Transformer。在相同模型规模下，Llama和Mamba仅需75%的训练数据即可达到同等性能。

Conclusion: 本研究首次提供了输入标记化、模型配置和时间数据预处理的系统性消融研究，为未来基于EHR的临床预测任务模型开发提供了参考起点。
...
### [130] [Provably Minimum-Length Conformal Prediction Sets for Ordinal Classification](https://arxiv.org/abs/2511.16845)
*Zijian Zhang,Xinyu Chen,Yuanjie Shi,Liyuan Lillian Ma,Zifan Xu,Yan Yan*

Main category: cs.LG

TL;DR: 提出了一种新的序数分类置信预测方法，该方法模型无关且提供实例级最优预测区间，通过滑动窗口算法实现线性时间复杂度，显著提高了预测效率。


<details>
  <summary>Details</summary>
Motivation: 现有序数置信预测方法主要关注启发式算法或要求基础模型预测单峰分布，限制了覆盖效率权衡的洞察，缺乏模型无关和分布无关的特性。

Method: 将序数分类置信预测建模为实例级最小长度覆盖问题，开发了滑动窗口算法，在K个标签候选中具有线性时间复杂度，并提出长度正则化变体。

Result: 在四个不同领域的基准数据集上实验，相比基线方法平均减少了15%的预测集大小，显著提高了预测效率。

Conclusion: 该方法填补了序数置信预测领域的空白，提供了模型无关、实例级最优的预测区间，在保持覆盖率的同时显著提升了预测效率。
...
### [131] [Sex and age determination in European lobsters using AI-Enhanced bioacoustics](https://arxiv.org/abs/2511.16848)
...
TL;DR: 本研究利用被动声学监测和人工智能模型，通过欧洲龙虾的生物声学特征（嗡嗡声/甲壳振动）成功分类龙虾的年龄（幼体/成体）和性别（雄性/雌性），准确率超过97%。
...
### [132] [PepEVOLVE: Position-Aware Dynamic Peptide Optimization via Group-Relative Advantage](https://arxiv.org/abs/2511.16912)
*Trieu Nguyen,Hao-Wei Pang,Shasha Feng*

Main category: cs.LG

TL;DR: PepEVOLVE是一个位置感知的动态框架，用于多目标肽序列优化，相比现有方法能更有效地发现编辑位点并优化肽类性质。


<details>
  <summary>Details</summary>
Motivation: 现有肽类优化方法如PepINVENT需要预先指定可突变位置，且依赖静态预训练和优化算法，限制了模型的泛化能力和优化效果。
...
Conclusion: PepEVOLVE为肽类先导优化提供了实用、可复现的路径，当最优编辑位点未知时能更高效地探索并提高多目标设计质量。
...
### [133] [A Hybrid Computational Intelligence Framework for scRNA-seq Imputation: Integrating scRecover and Random Forests](https://arxiv.org/abs/2511.16923)
*Ali Anaissi,Deshao Liu,Yuanzhe Jia,Weidong Huang,Widad Alyassine,Junaid Akram*

Main category: cs.LG

TL;DR: SCR-MF是一个模块化两阶段工作流，结合scRecover的dropout检测和missForest的非参数插补，用于单细胞RNA测序数据中dropout事件的修复。


<details>
  <summary>Details</summary>
Motivation: 单细胞RNA测序存在普遍的dropout事件，会掩盖生物信号，需要开发有效的插补方法。
...
### [134] [CroTad: A Contrastive Reinforcement Learning Framework for Online Trajectory Anomaly Detection](https://arxiv.org/abs/2511.16929)
*Rui Xue,Dan He,Fengmei Jin,Chen Zhang,Xiaofang Zhou*

Main category: cs.LG

TL;DR: CroTad是一个基于对比强化学习的在线轨迹异常检测框架，无需阈值设定，对噪声和不规则采样数据具有鲁棒性，能够在子轨迹和点级别实时检测异常行为。
...
Method: 提出对比强化学习框架CroTad，结合对比学习提取不同行程的正常旅行模式，利用深度强化学习进行在线实时异常评分。
...
### [135] [A novel approach to classification of ECG arrhythmia types with latent ODEs](https://arxiv.org/abs/2511.16933)
...
Method: 训练潜在ODE模型连续ECG波形，从高频单通道信号创建鲁棒特征向量，通过降采样（360Hz到90Hz和45Hz）构建三个潜在向量，使用梯度提升树进行分类。

Result: 在360Hz、90Hz和45Hz采样频率下，宏平均AUC-ROC值分别为0.984、0.978和0.976，性能下降极小。

Conclusion: 该方法能够在保持高性能的同时绕过信号保真度与电池寿命之间的权衡，实现更小的可穿戴设备，促进长期心脏健康监测。
...
### [136] [ToC: Tree-of-Claims Search with Multi-Agent Language Models](https://arxiv.org/abs/2511.16972)
...
<details>
  <summary>Details</summary>
Motivation: 手动起草专利权利要求劳动密集、成本高昂且不一致，传统大语言模型缺乏结构化迭代推理能力，难以实现精确的权利要求优化。

Method: ToC框架整合蒙特卡洛树搜索与多智能体系统，包括基于LLM的编辑智能体和审查智能体，通过精心设计的多目标奖励函数优化新颖性、范围保持和语义连贯性。

Result: 在1145个权利要求基准测试中，ToC在零样本和少样本场景下显著优于标准LLM，平均综合得分提升8%，某些情况下达到9%。

Conclusion: ToC建立了一种透明、可控且可解释的方法论，有效将先进LLM推理能力与战略MCTS规划相结合，实现结构化专利权利要求优化。
...
### [137] [FIRM: Federated In-client Regularized Multi-objective Alignment for Large Language Models](https://arxiv.org/abs/2511.16992)
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦多目标优化方法存在严重通信瓶颈，依赖传输多个梯度对大型模型不可扩展，且集中化训练存在数据隐私问题。

Method: 每个客户端本地解决正则化多目标优化问题，通过客户端内正则化直接缓解客户端分歧漂移，仅需传输单个参数集。

Result: FIRM实现更平滑的训练动态、减少客户端分歧漂移、改进奖励权衡，并能根据偏好平滑调整目标间权衡。

Conclusion: FIRM算法在联邦多目标对齐中实现通信效率和收敛保证，为联邦学习中的多目标对齐提供了有效解决方案。
...
### [138] [Mask the Redundancy: Evolving Masking Representation Learning for Multivariate Time-Series Clustering](https://arxiv.org/abs/2511.17008)
*Zexi Tan,Xiaopeng Luo,Yunlin Liu,Yiqun Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种进化掩码多变量时间序列聚类方法（EMTC），通过重要性感知变量级掩码和多内生视图表示学习模块，解决时间序列冗余问题，提升聚类性能。


<details>
  <summary>Details</summary>
Motivation: 多变量时间序列包含大量冗余信息（如稳态机器运行记录、太阳能发电的零输出期），这些冗余会降低对判别性时间戳的关注，导致聚类性能瓶颈。现有掩码策略多为独立预处理步骤，无法动态适应聚类关键时间戳的重要性。
...
Conclusion: EMTC通过进化掩码策略有效解决了时间序列冗余问题，显著提升了多变量时间序列聚类的性能。
...
### [139] [Energy Scaling Laws for Diffusion Models: Quantifying Compute and Carbon Emissions in Image Generation](https://arxiv.org/abs/2511.17031)
*Aniketh Iyengar,Jiaqi Han,Boris Ruf,Vincent Grari,Marcin Detyniecki,Stefano Ermon*

Main category: cs.LG

TL;DR: 本文提出了一种基于计算复杂度（FLOPs）的Kaplan缩放定律来预测扩散模型的GPU能耗，通过分解推理过程并验证了去噪操作主导能耗的假设。
...
Method: 将扩散模型推理分解为文本编码、迭代去噪和解码组件，假设去噪操作因在多步推理中重复执行而主导能耗。在四种扩散模型和三种GPU架构上进行全面实验，涵盖不同分辨率、精度、步数和分类器自由引导设置。

Result: 能耗缩放定律在单个架构内达到高预测精度（R平方>0.9），并表现出强大的跨架构泛化能力，在不同模型间保持高秩相关性，能够可靠估计未见过的模型-硬件组合的能耗。
...
TL;DR: 研究比较了精确指纹与标准压缩指纹在分子性质预测和贝叶斯优化中的性能差异，发现精确指纹在预测准确性上有小幅但一致的提升，但在贝叶斯优化中未能带来显著改进。


<details>
  <summary>Details</summary>
Motivation: 分子指纹方法使用哈希函数生成分子的固定长度向量表示，但哈希冲突会导致不同子结构具有相同特征，从而在分子相似性计算中产生高估。本研究旨在探索使用精确指纹是否能比标准压缩指纹在分子性质预测和贝叶斯优化中提供更好的准确性。

Method: 在分子性质预测和贝叶斯优化任务中，使用高斯过程作为基础预测模型，比较精确指纹与标准压缩指纹的性能。实验基于DOCKSTRING数据集中的五个分子性质预测基准进行。

Result: 在五个分子性质预测基准上，使用精确指纹相比标准压缩指纹产生了小幅但一致的预测准确性提升。然而，这些准确性增益并未转化为贝叶斯优化性能的显著改进。

Conclusion: 虽然精确指纹在分子性质预测中能提供更好的准确性，但由于哈希冲突导致的性能损失相对较小，且这些改进在贝叶斯优化应用中并不显著，因此在实际应用中可能不需要优先考虑使用精确指纹。
...
### [141] [Geometric-Disentangelment Unlearning](https://arxiv.org/abs/2511.17100)
*Duo Zhou,Yuji Zhang,Tianxin Wei,Ruizhong Qiu,Ke Yang,Xiao Lin,Cheng Qian,Jingrui He,Hanghang Tong,Heng Ji,Huan Zhang*

Main category: cs.LG

TL;DR: 该论文提出了几何解缠遗忘(GU)方法，通过将遗忘梯度更新分解为保留梯度子空间的切向和法向分量，仅执行法向分量，从而在保证有效遗忘的同时最小化对保留知识的影响。


<details>
  <summary>Details</summary>
Motivation: 现有的机器遗忘方法在有效遗忘和保留知识保护之间存在权衡，缺乏对遗忘更新如何影响保留知识的理论分析，以及是否能以理论保证消除副作用。

Method: 从一阶分析出发，提出保留损失不变的条件是更新方向与保留梯度子空间正交。GU方法将遗忘梯度更新分解为切向和法向分量，仅执行法向分量，确保保留损失一阶不变。

Result: GU方法在TOFU、MUSE和WMDP三个基准测试中，与现有梯度遗忘方法结合使用时取得了一致的性能提升。

Conclusion: 几何解缠遗忘提供了一种理论上有保证、简单有效的机器遗忘解决方案，能够缓解遗忘过程中的副作用，同时保持插拔式使用的便利性。
...
### [142] [Four decades of circumpolar super-resolved satellite land surface temperature data](https://arxiv.org/abs/2511.17134)
...
<details>
  <summary>Details</summary>
Motivation: 北极地区快速变暖，需要高分辨率的地表温度数据来理解精细尺度的永久冻土动态和地表过程，但AVHRR的粗空间分辨率限制了其应用。

Method: 使用基于深度各向异性扩散模型的超分辨率算法，以MODIS LST数据为训练目标，结合高分辨率土地覆盖、数字高程和植被高度图作为指导，将AVHRR GAC数据降尺度到1公里。

Result: 生成了42年泛北极地区每日两次的1公里分辨率地表温度观测数据集，覆盖整个泛北极地区四个十年。
...
TL;DR: FlexiFlow是一种新颖的架构，扩展了流匹配模型，能够联合采样分子及其多个构象，同时保持等变性和置换不变性。该方法在QM9和GEOM Drugs数据集上实现了最先进的分子生成结果。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的3D从头设计流匹配或扩散模型仅限于生成单个构象，但分子的构象景观决定了其可观察性质和与蛋白质靶标的结合能力。通过生成一组代表性的低能构象异构体，可以更直接地评估这些性质，并可能提高生成具有所需热力学可观察性质的分子的能力。

Method: 提出FlexiFlow架构，扩展流匹配模型，允许联合采样分子及其多个构象，同时保持等变性和置换不变性。在QM9和GEOM Drugs数据集上进行验证。

Result: FlexiFlow能够生成有效、无应变、独特且新颖的分子，对训练数据分布具有高保真度，同时捕获分子的构象多样性。模型生成的构象集合在推理时间的一小部分内提供了与最先进的基于物理的方法相似的覆盖范围。

Conclusion: FlexiFlow可以成功转移到蛋白质条件配体生成任务，即使数据集仅包含静态口袋而没有伴随构象。该方法在分子生成任务中实现了最先进的结果，能够同时生成分子及其多个构象。
...
### [144] [Enforcing governing equation constraints in neural PDE solvers via training-free projections](https://arxiv.org/abs/2511.17258)
*Omer Rochman,Gilles Louppe*

Main category: cs.LG

TL;DR: 该论文提出两种训练后投影方法来减少神经PDE求解器对非线性约束的违反，包括非线性优化投影和局部线性化投影，能显著改善求解精度。


<details>
  <summary>Details</summary>
Motivation: 神经PDE求解器在科学模拟中经常违反控制方程约束，特别是非线性约束和动力学PDE中的长程时间依赖性约束，需要有效的后处理方法来保证解的质量。

Method: 提出了两种训练后投影方法：1）基于非线性优化的投影；2）基于局部线性化的投影，使用雅可比-向量和向量-雅可比乘积。

Result: 两种投影方法都能显著减少约束违反，并在代表性PDE上比基于物理信息的基线方法提高了精度。

Conclusion: 训练后投影是改善神经PDE求解器约束满足性的有效方法，特别是对于非线性约束和动力学系统。
...
### [145] [Automobile demand forecasting: Spatiotemporal and hierarchical modeling, life cycle dynamics, and user-generated online information](https://arxiv.org/abs/2511.17275)
*Tom Nahrendorf,Stefan Minner,Helfried Binder,Richard Zinck*

Main category: cs.LG

TL;DR: 该研究针对高端汽车制造商面临的高产品多样性、稀疏变体级数据和波动市场动态等复杂预测挑战，提出了一种结合点预测和概率预测的多层次预测方法，通过LightGBM模型集成、分位数回归和混合整数线性规划协调，显著提高了多产品、多市场、多层级汽车需求预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 高端汽车制造商面临产品多样性高、变体级数据稀疏和市场动态波动等复杂预测挑战，需要解决多产品、多市场、多层级的需求预测问题。

Method: 结合点预测和概率预测，在战略和运营规划层面使用LightGBM模型集成，采用池化训练集、分位数回归和混合整数线性规划协调方法。

Result: 研究发现时空依赖性和四舍五入偏差显著影响预测准确性，强调了整数预测对运营可行性的重要性。Shapley分析显示短期需求受生命周期成熟度、自回归动量和运营信号影响，中期需求则反映在线参与度、规划目标和竞争指标等预期驱动因素。

Conclusion: 在线行为数据显著提高了细分层面的预测准确性，该方法为复杂汽车需求预测提供了有效解决方案。
...
### [146] [SAVeD: Semantic Aware Version Discovery](https://arxiv.org/abs/2511.17298)
*Artem Frenk,Roee Shraga*

Main category: cs.LG

TL;DR: SAVeD是一个基于对比学习的框架，用于识别结构化数据集的版本，无需依赖元数据、标签或集成假设。它通过生成增强表视图、使用自定义transformer编码器嵌入，并在潜在空间中对比来优化语义相似性。
...
Method: 采用改进的SimCLR管道，通过随机转换（如行删除、编码扰动）生成增强表视图，使用自定义transformer编码器嵌入，并在潜在空间中进行对比学习。
...
### [147] [Self-supervised denoising of raw tomography detector data for improved image reconstruction](https://arxiv.org/abs/2511.17312)
*Israt Jahan Tulin,Sebastian Starke,Dominic Windisch,André Bieberle,Peter Steinbach*

Main category: cs.LG

TL;DR: 本研究比较了两种自监督深度学习方法和一种非学习方法在超快电子束X射线CT原始探测器数据去噪中的表现，发现深度学习方法在提升信噪比和改善重建图像质量方面优于非学习方法。


<details>
  <summary>Details</summary>
Motivation: 超快电子束X射线CT由于测量时间短导致数据噪声大，产生重建伪影并限制图像质量，需要有效的去噪方法来应对这些问题。

Method: 研究比较了两种自监督深度学习方法和一种非学习方法的去噪性能，应用于原始探测器数据的噪声处理。

Result: 深度学习方法能够显著提升探测器数据的信噪比，并在重建图像质量上实现一致改善，表现优于非学习方法。

Conclusion: 自监督深度学习方法是超快电子束X射线CT数据去噪的有效解决方案，能够克服短测量时间带来的噪声问题。
...
### [148] [ReBaPL: Repulsive Bayesian Prompt Learning](https://arxiv.org/abs/2511.17339)
*Yassir Bendou,Omar Ezzahir,Eduardo Fernandes Montesuma,Gabriel Mahuas,Victoria Shevchenko,Mike Gartrell*

Main category: cs.LG

TL;DR: 本文提出了ReBaPL方法，一种新颖的贝叶斯提示学习方法，通过循环步长调度和随机梯度哈密顿蒙特卡洛算法，结合表示空间的排斥力，有效探索提示的多模态后验分布，提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统提示学习方法容易过拟合且难以处理分布外泛化问题，贝叶斯提示学习通过将提示优化构建为贝叶斯推断问题来增强鲁棒性，但现有方法在探索复杂多模态后验分布方面存在局限。

Method: ReBaPL方法整合了循环步长调度与SGHMC算法，交替进行探索和利用阶段，并引入基于概率度量的排斥力（包括最大均值差异和Wasserstein距离），防止过早收敛到单一模式。

Result: 在多个基准数据集上的实验表明，ReBaPL方法在提示学习任务上优于现有最先进方法，实现了更好的泛化性能。

Conclusion: ReBaPL提供了一种模块化的即插即用贝叶斯扩展方案，能够更全面地刻画提示后验分布，显著提升提示学习的泛化能力。
...
### [149] [Convergence and stability of Q-learning in Hierarchical Reinforcement Learning](https://arxiv.org/abs/2511.17351)
*Massimiliano Manenti,Andrea Iannelli*

Main category: cs.LG

TL;DR: 本文提出了封建Q学习方案，研究了其收敛和稳定性的条件，通过随机逼近理论和ODE方法证明了收敛性，并展示了更新收敛到博弈均衡点。
...
Result: 证明了封建Q学习的收敛性和稳定性，更新收敛到博弈均衡点，实验支持理论预期结果。

Conclusion: 为封建强化学习提供了原则性的收敛和稳定性分析，为基于博弈论的分层强化学习方法打开了大门。
...
### [150] [R2PS: Worst-Case Robust Real-Time Pursuit Strategies under Partial Observability](https://arxiv.org/abs/2511.17367)
*Runyu Lu,Ruochuan Shi,Yuanheng Zhu,Dongbin Zhao*

Main category: cs.LG

TL;DR: 该论文提出了首个在部分可观测性下的最坏情况鲁棒实时追捕策略（R2PS）方法，通过信念保持机制和跨图强化学习，实现了对未见图结构的零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 现有追逃博弈中的最坏情况鲁棒策略计算耗时，特别是在考虑部分可观测性时。当前基于强化学习的方法局限于完美信息场景，且未考虑逃避者能预测追捕者行动的情况。

Method: 首先证明传统动态规划算法在异步移动下保持最优性；然后提出关于逃避者可能位置的信念保持机制，将DP策略扩展到部分可观测设置；最后将信念保持嵌入EPG框架，通过跨图强化学习对抗异步移动DP逃避策略。

Result: 强化学习后，策略实现了对未见真实世界图结构的鲁棒零样本泛化，并持续优于现有游戏RL方法直接在测试图上训练的策略。

Conclusion: R2PS方法成功解决了部分可观测追逃博弈中的实时鲁棒策略问题，展现了优异的泛化性能。
...
### [151] [Stable Coresets via Posterior Sampling: Aligning Induced and Full Loss Landscapes](https://arxiv.org/abs/2511.17399)
*Wei-Kai Chang,Rajiv Khanna*

Main category: cs.LG

TL;DR: 提出了一种基于后验采样的核心集选择新框架，通过连接后验采样与损失景观来解决梯度方法在核心集选择中的局限性，实现更快的训练和更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型规模扩大，计算需求增加，需要有效的核心集选择技术来加速训练。现有梯度方法面临挑战，包括SGD作为强基线以及随时间推移代表性失效的问题。

Method: 建立后验采样与损失景观的连接，提出基于后验采样的平滑损失函数，增强稳定性和泛化能力，同时保持计算效率。

Result: 通过大量实验证明，该方法在多种数据集上比现有最优方法实现了更快的训练速度和更好的泛化性能。

Conclusion: 提出的基于后验采样的核心集选择框架有效解决了现有方法的局限性，在数据预算有限的情况下提供了稳健且高效的核心集选择方案。
...
### [152] [DS-Span: Single-Phase Discriminative Subgraph Mining for Efficient Graph Embeddings](https://arxiv.org/abs/2511.17419)
*Yeamin Kaiser,Muhammed Tasnim Bin Anwar,Bholanath Das,Chowdhury Farhan Ahmed,Md. Tanvir Alam*

Main category: cs.LG

TL;DR: DS-Span是一个单阶段的判别性子图挖掘框架，通过统一模式增长、剪枝和监督驱动评分在一个搜索空间遍历中完成，生成紧凑且判别性的子图特征用于图嵌入和分类。


<details>
  <summary>Details</summary>
Motivation: 现有的频繁或判别性子图挖掘方法存在冗余的多阶段流程、高计算成本和挖掘结构与判别相关性之间弱耦合的问题，需要更高效统一的解决方案。

Method: 提出DS-Span框架，引入覆盖上限资格机制动态限制探索，以及信息增益引导的选择机制，在单次搜索空间遍历中统一进行模式增长、剪枝和监督评分。

Result: 在多个基准测试中，DS-Span比之前的多阶段方法生成更紧凑和判别性的子图特征，在显著减少运行时间的同时达到更高或相当的准确率。

Conclusion: 统一的单阶段判别性挖掘方法为可扩展和可解释的图表示学习提供了有前景的基础。
...
### [153] [Towards fully differentiable neural ocean model with Veros](https://arxiv.org/abs/2511.17427)
...
<details>
  <summary>Details</summary>
Motivation: 传统海洋模型缺乏可微分性，限制了基于梯度的优化方法在海洋建模中的应用，需要开发能够支持自动微分的海洋模型。

Method: 对VEROS海洋模型进行修改，使其与JAX自动微分框架完全兼容，并验证数值一致性。

Result: 成功实现了可微分海洋模型，能够进行初始海洋状态的梯度优化和未知物理参数的直接校准。
...
TL;DR: 提出MS-DGFormer框架，通过Transformer架构直接处理原始质谱数据，结合字典编码器和SVD降噪，实现单次光谱的病原体识别，无需复杂预处理。


<details>
  <summary>Details</summary>
Motivation: 传统MALDI-MS依赖繁琐样品制备和多光谱平均，限制了实时环境监测应用，特别是新兴气溶胶MALDI-MS系统面临单次检测噪声大的挑战。

Method: 采用Transformer架构捕获质谱时间序列的长程依赖，引入基于SVD的字典编码器集成降噪光谱信息，增强特征提取能力。

Result: 实现了对气溶胶样品中病原体的优越识别性能，支持单次光谱的稳健分析。

Conclusion: 该方法消除了复杂预处理需求，为便携式MALDI-MS平台在环境病原体检测和生物威胁快速响应中的应用开辟了新途径。
...
### [155] [PersonaAgent with GraphRAG: Community-Aware Knowledge Graphs for Personalized LLM](https://arxiv.org/abs/2511.17467)
...
### [156] [Harnessing Data from Clustered LQR Systems: Personalized and Collaborative Policy Optimization](https://arxiv.org/abs/2511.17489)
*Vinay Kanakeri,Shivam Bajaj,Ashwin Verma,Vijay Gupta,Aritra Mitra*

Main category: cs.LG

TL;DR: 本文提出了一种结合聚类和强化学习的新算法，用于在多个相似线性系统中学习个性化控制策略，通过聚类避免不同系统数据混合带来的次优性，同时享受协作带来的统计增益。


<details>
  <summary>Details</summary>
Motivation: 强化学习通常需要大量数据，通过利用相似过程的数据可以提高样本效率。但由于过程模型未知，识别哪些过程相似具有挑战性。本文旨在解决在未知模型下如何识别相似系统并学习个性化策略的问题。

Method: 结合顺序消除和零阶策略优化的思想，提出新算法同时执行聚类和学习，为每个聚类输出个性化控制器。在适当的聚类分离条件下，保证高概率正确聚类。

Result: 证明该方法能保证高概率正确聚类，且每个聚类学习到的策略的次优性差距与聚类大小成反比，没有额外偏差。相比之前的协作学习方法，不会因包含不相似过程数据而遭受次优性。

Conclusion: 这是首个揭示聚类如何在数据驱动控制中用于学习个性化策略的工作，既能享受协作带来的统计增益，又不会因包含不相似过程数据而遭受次优性。从分布式实现角度看，该方法仅产生温和的对数通信开销。
...
### [157] [δ-EMG: A Monotonic Graph Index for Approximate Nearest Neighbor Search](https://arxiv.org/abs/2511.16921)
*Liming Xiang,Jing Feng,Ziqi Yin,Zijian Li,Daihao Xue,Hongchao Qin,Ronghua Li,Guoren Wang*

Main category: cs.IR

TL;DR: 本文提出了一种误差有界的近似最近邻搜索方法，通过构建δ-EMG图结构，确保返回结果是对真实值的(1/δ)近似，相比传统基于召回率的方法提供了更强的理论保证。


<details>
  <summary>Details</summary>
Motivation: 传统近似最近邻搜索方法仅关注召回率，无法保证错误结果的偏差范围。为了解决这一局限性，需要开发能够提供近似误差保证的搜索方法。

Method: 采用基于图的框架，提出δ-EMG（误差有界单调图），通过在构建过程中强制执行δ-单调几何约束，确保贪心搜索收敛到(1/δ)近似邻居。进一步设计误差有界top-k搜索算法和可扩展的δ-EMQG变体。

Result: 在ANN-Benchmarks数据集上的实验表明，在召回率要求为0.99时，该方法在SIFT1M数据集上达到19,000 QPS，比其他方法性能提升超过40%。

Conclusion: 提出的误差有界近似最近邻搜索方法不仅提供了理论保证，在实际性能上也显著优于现有方法，为高维检索系统提供了更可靠的解决方案。
...
### [158] [RASTP: Representation-Aware Semantic Token Pruning for Generative Recommendation with Semantic Identifiers](https://arxiv.org/abs/2511.16943)
...
<details>
  <summary>Details</summary>
Motivation: 生成式推荐系统使用语义标识符(SIDs)表示物品，但多SID表示会显著增加输入序列长度，导致计算复杂度和内存消耗增加。现有方法主要优化注意力计算和KV缓存，但直接剪枝输入序列中的低信息token可能更有效。

Method: 提出RASTP方法，通过结合语义显著性（基于表示大小）和注意力中心性（基于累积注意力权重）来评估token重要性，动态剪枝低信息或无关的语义token。

Result: 在三个真实世界Amazon数据集上的实验表明，RASTP减少了26.7%的训练时间，同时保持或略微改善了推荐性能。

Conclusion: RASTP通过直接剪枝输入序列中的低信息token，有效降低了生成式推荐系统的计算负担，同时保持了推荐质量，为高效推荐系统提供了新思路。
...
### [159] [CLLMRec: LLM-powered Cognitive-Aware Concept Recommendation via Semantic Alignment and Prerequisite Knowledge Distillation](https://arxiv.org/abs/2511.17041)
*Xiangrui Xiong,Yichuan Lu,Zifei Pan,Chang Sun*

Main category: cs.IR

TL;DR: CLLMRec是一个基于大语言模型的概念推荐框架，通过语义对齐和先验知识蒸馏解决MOOCs中个性化学习推荐问题，无需依赖结构化知识图谱。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖高质量结构化知识图谱，但在实际教育场景中往往稀缺，因此需要开发不依赖显式结构先验的个性化概念推荐方法。

Method: 采用语义对齐构建统一表示空间，通过教师-学生架构进行先验知识蒸馏，结合深度知识追踪建模学习者实时认知状态。

Result: 在两个真实MOOCs数据集上的实验表明，CLLMRec在多项评估指标上显著优于现有基线方法。

Conclusion: 该框架能够生成真正认知感知和个性化的概念推荐，无需依赖显式结构先验知识。
...
### [160] [Parametric Retrieval-Augmented Generation using Latent Routing of LoRA Adapters](https://arxiv.org/abs/2511.17044)
*Zhan Su,Fengran Mo,Jian-yun Nie*

Main category: cs.IR

TL;DR: 提出Poly-PRAG方法，通过潜在路由编码过程解决传统参数化检索增强生成(PRAG)中一对一文档编码方案的数据稀缺和高推理开销问题。
...
Method: 将文档集编码视为多任务学习过程，每个段落分配唯一任务标识符，使用路由函数通过少量潜在LoRA适配器编码整个段落空间，在线推理时根据查询选择性激活潜在专家子集。

Result: 在多个知识密集型NLP任务上的综合评估显示，该方法在四个不同数据集上取得了最先进的结果。

Conclusion: Poly-PRAG通过潜在路由编码过程有效解决了传统PRAG方法的局限性，实现了更高的计算效率和性能。
...
TL;DR: Cognitive BASIC是一种基于BASIC风格的提示语言和模型内解释器，将大语言模型的推理过程结构化为显式的逐步执行轨迹，通过编号行和简单命令实现可解释的认知控制层。


<details>
  <summary>Details</summary>
Motivation: 受复古BASIC简单性的启发，旨在为LLM推理提供透明、可解释的多步推理框架，使模型能够可靠地模拟短程序执行过程。

Method: 设计自然语言解释器文件来指定命令语义、内存更新和日志行为，通过心理模型解释器提取陈述性和程序性知识，检测矛盾并在必要时生成解决方案。

Result: 在三个LLM上的知识提取、冲突检测和推理任务基准测试中，所有模型都能执行Cognitive BASIC程序，总体表现强劲但不均匀。

Conclusion: Cognitive BASIC为LLM提供了一种有效的结构化推理方法，使多步推理过程更加透明和可解释。
...
TL;DR: 提出了一个利用响应模式统计分析来识别无效基准问题的框架，通过统计异常检测标记潜在问题供专家审查，在9个基准测试中达到84%的精确度，并引入LLM-judge初步审查以减少人工工作量。


<details>
  <summary>Details</summary>
Motivation: 基准测试对AI进展至关重要，但无效的基准问题会破坏其可靠性。手动识别和修正数千个基准问题中的错误既不可行，也是可靠评估的关键瓶颈。

Method: 基于AI评估中常用的核心假设——平均分足以总结模型性能，这意味着测量实验存在单维潜在结构。当项目的统计估计值超出预期范围时，该项目更可能存在问题。方法包括统计分析和LLM-judge初步审查。

Result: 在9个广泛使用的基准测试中，该方法引导专家审查识别问题问题的精确度高达84%，并显著减少了人工工作量。

Conclusion: 该框架提供了一个高效且可扩展的系统性基准修订方法，通过统计分析和自动化初步审查相结合，提高了基准测试的可靠性。
...
Conclusion: HDR机制成功解决了多车协同驾驶中奖励信号失效的问题，为高频连续控制任务提供了有效的奖励设计方法，显著提升了算法的学习效率和策略质量。
...
TL;DR: MirrorMind是一个分层认知架构，通过整合双记忆表示来解决AI科学家在科学发现中的局限，将个体认知轨迹与集体学科记忆相结合，实现结构化的科学推理。
...
Method: 提出三层框架：个体层构建研究者的高保真认知模型；领域层将集体知识映射为结构化学科概念图；跨学科层作为正交编排引擎，分离记忆存储与智能体执行。

Result: 在四个综合任务中评估，包括作者级认知模拟、互补推理、跨学科协作促进和多智能体科学问题解决，结果显示MirrorMind能够实现结构化、个性化和洞察生成的科学推理。

Conclusion: 通过整合个体认知深度与集体学科广度，MirrorMind超越了简单事实检索，实现了更具洞察力的科学推理能力。
...
TL;DR: 论文提出DAPS++方法，将扩散模型在逆问题求解中重新解释为EM框架中的初始化阶段，使似然项更直接指导推理，减少函数评估次数和测量优化步骤，实现高效计算和鲁棒重建性能。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯视角下基于分数的扩散方法在解决逆问题时，先验提供的指导有限，重建主要由测量一致性项驱动，推理过程与扩散动力学实际上解耦。需要澄清这种结构并改进方法。

Method: 将扩散重新解释为EM框架中的初始化阶段，提出DAPS++方法，使似然项更直接指导推理，同时保持数值稳定性，扩散阶段与数据驱动细化完全解耦。

Result: DAPS++需要更少的函数评估次数和测量优化步骤，在多种图像恢复任务中实现高计算效率和鲁棒的重建性能。

Conclusion: 通过将扩散模型重新定位为EM框架中的初始化阶段，DAPS++提供了对统一扩散轨迹在实践中保持有效的解释，并实现了更高效的逆问题求解。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中的稀疏奖励问题面临两个主要挑战：联合动作轨迹的指数级稀疏性，以及现有方法未能充分考虑影响团队状态的联合动作。

Method: 提出了MIR方法，通过激励个体智能体探索能够影响队友的行为，并与原始策略结合来刺激团队探索。

Result: 在扩展的MiniGrid-MA环境中进行实验验证，结果显示MIR方法相比现有最先进方法具有更优越的性能。

Conclusion: MIR是一种简单而有效的增强策略，能够显著改善多智能体强化学习在稀疏奖励场景下的性能表现。
...
TL;DR: 本文提出了HTAM框架，通过分层任务抽象机制构建多智能体系统，专门解决专业领域（如遥感）中结构化工作流程的挑战。


<details>
  <summary>Details</summary>
Motivation: 通用智能体框架（如ReAct）在需要严格结构化工作流程的专业领域表现不佳，特别是在遥感等涉及专业工具和多步骤程序的领域。

Method: 引入HTAM框架，将多智能体系统构建为逻辑层次结构，反映领域内在的任务依赖图，确保程序正确性并将复杂问题分解为顺序层。

Result: 基于HTAM构建的EarthAgent在GeoPlan-bench基准测试中显著优于现有单智能体和多智能体系统。
...
TL;DR: 本文主张将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具作为实现智能AI系统的基础，通过将自适应数据驱动方法与结构化推理协调模型相结合，构建具有透明度、合作性和问责制的智能系统。


<details>
  <summary>Details</summary>
Motivation: 智能AI系统需要持续的自主性、推理和交互能力，这要求对智能体的假设必须辅以明确的认知、合作和治理模型。

Method: 利用AAMAS社区开发的概念工具，包括BDI架构、通信协议、机制设计和制度建模，将自适应数据驱动方法与结构化推理协调模型相结合。

Result: 提出了一种连接形式理论和实践自主性的智能体视角，为构建不仅能力强、灵活，而且透明、合作和可问责的智能系统指明了路径。

Conclusion: AAMAS社区的概念工具为实现智能AI系统提供了坚实基础，通过整合自适应方法和结构化模型，可以构建更完善的智能系统。
...
TL;DR: 本文系统评估了在LLM监控中使用合成和离策略数据对探针泛化性能的影响，发现响应生成策略显著影响探针性能，且训练数据域的变化会导致更大的性能下降。


<details>
  <summary>Details</summary>
Motivation: 由于许多行为的自然示例稀少，研究者不得不依赖合成或离策略的LLM响应来训练探针，但这种方法对探针泛化能力的影响尚不清楚。

Method: 在八个不同的LLM行为上测试线性和注意力探针，比较不同响应生成策略对探针性能的影响，并评估从离策略数据到策略数据的泛化能力。

Result: 发现从离策略数据成功泛化到测试集可以预测策略泛化的成功，欺骗和沙袋探针在真实监控场景中可能无法从离策略泛化到策略数据。不同域测试分数始终低于同域测试分数。

Conclusion: 在缺乏策略数据时，使用同域离策略数据比使用不同域的策略数据产生更可靠的探针，强调需要能更好处理LLM监控中分布偏移的方法。
...
### [171] [RubiSCoT: A Framework for AI-Supported Academic Assessment](https://arxiv.org/abs/2510.17309)
...
<details>
  <summary>Details</summary>
Motivation: 传统学术论文评估方法耗时且存在评估者主观性差异，需要更高效、一致的评估解决方案。
...
Conclusion: RubiSCoT框架有潜力通过一致、可扩展和透明的评估来优化学术评估过程。
...
### [172] [Splitting vertices of bipartite graphs preserves de Bruijn-Erdős property](https://arxiv.org/abs/2511.17156)
*Laurent Beaudou,Guillermo Gamboa Quintero*

Main category: cs.DM

TL;DR: 该论文证明了从二分图通过迭代将顶点分裂成两个相邻孪生顶点得到的图具有de Bruijn-Erdős性质


<details>
  <summary>Details</summary>
Motivation: 研究从二分图通过特定操作构造的图是否具有de Bruijn-Erdős性质

Method: 通过数学证明方法，分析从二分图迭代分裂顶点得到的图结构

Result: 成功证明了这类图确实具有de Bruijn-Erdős性质

Conclusion: 从二分图通过迭代顶点分裂操作得到的图类具有de Bruijn-Erdős性质
...
### [173] [Simple Circuit Extensions for XOR in PTIME](https://arxiv.org/abs/2511.16903)
*Marco Carmosino,Ngu Dang,Tim Jackman*

Main category: cs.CC

TL;DR: 本文证明了XOR_n的f-简单扩展问题在多项式时间内可解，填补了Ilango关于MCSP* ETH-hardness证明向总MCSP扩展的关键技术空白。


<details>
  <summary>Details</summary>
Motivation: Ilango证明了部分函数MCSP*在指数时间假设下是困难的，但该证明无法直接扩展到总MCSP。由于OR_n的总f-简单扩展问题在f为只读公式时是简单的，因此需要寻找更复杂但仍能被充分理解的布尔函数来扩展证明。

Method: 开发了一个固定参数可处理算法，该算法在f的最优电路满足三个条件时高效：(1)规模线性；(2)多项式数量且可高效枚举；(3)所有电路都有常数有界扇出。XOR_n满足所有这些条件。

Result: 证明了XOR-简单扩展问题在两种电路复杂度度量下（包括NOT门是否计入电路规模）都在多项式时间内可解。

Conclusion: XOR_n作为OR_n之后最自然的候选函数，其简单扩展问题仍然是简单的，这为将Ilango的证明扩展到总MCSP设置了新的障碍。
...
### [174] [A Lifting Theorem for Hybrid Classical-Quantum Communication Complexity](https://arxiv.org/abs/2511.17227)
*Xudong Wu,Guangxu Yang,Penghui Yao*

Main category: cs.CC

TL;DR: 该论文研究混合经典-量子通信复杂性模型，建立了新的混合提升定理，揭示了经典通信和量子通信之间的权衡关系，特别是对于复合函数f∘G^n，证明了c+q²=Ω(max{deg(f),bs(f)}·log n)的下界。


<details>
  <summary>Details</summary>
Motivation: 研究混合经典-量子通信模型中的通信权衡，统一经典和量子通信复杂性分析的两个独立提升范式，为混合通信模型提供新的下界证明框架。

Method: 建立混合提升定理，将查询到通信的提升框架（经典）和近似度到广义差异的提升方法（量子）统一起来，分析复合函数f∘G^n的通信复杂性。

Result: 证明对于复合函数f∘G^n，任何混合协议满足c+q²=Ω(max{deg(f),bs(f)}·log n)，对于只读一次公式f，要么需要Θ(n·log n)经典比特，要么需要Θ(√n·log n)量子比特。

Conclusion: 经典预处理不能显著减少所需的量子通信量，这是混合双向通信复杂性中第一个非平凡的经典与量子通信权衡结果，为混合通信模型提供了新的分析框架。
...
### [175] [Lower Bounds for CSP Hierarchies Through Ideal Reduction](https://arxiv.org/abs/2511.17272)
*Jonas Conneryd,Yassine Ghannane,Shuo Pang*

Main category: cs.CC

TL;DR: 本文提出了一种通用方法，通过代数证明系统的度下界来获得（承诺）CSP层次结构的级下界。具体展示了Alekhnovich和Razborov的伪约化算子可以欺骗上同调k一致性算法。


<details>
  <summary>Details</summary>
Motivation: 动机是建立一种通用框架，将代数证明系统的度下界转化为CSP层次结构的级下界，从而为各种约束满足问题提供统一的证明方法。

Method: 使用Alekhnovich和Razborov提出的伪约化算子来欺骗上同调k一致性算法，将代数证明系统的度下界转化为CSP层次结构的级下界。

Result: 证明了c vs. ℓ-着色问题的级下界对所有ℓ ≥ c ≥ 3都是最优的，并为Chan和Ng的松弛和零约束CSP下界提供了简化证明。

Conclusion: 该方法提供了一个通用框架，能够从代数证明系统的度下界推导出CSP层次结构的级下界，为多种约束满足问题的复杂性分析提供了统一工具。
