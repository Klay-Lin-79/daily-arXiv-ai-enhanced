...
Motivation: 研究动机是探索基础模型在计算病理学中的跨癌症和跨物种转移能力，了解现有视觉-语言对齐方法在跨物种泛化中的局限性，并寻找改进方法。

Method: 使用CPath-CLIP模型，通过少量样本微调在犬类和人类组织病理学全切片图像上进行癌症检测。采用AUC作为性能指标，进行同癌症、跨癌症和跨物种评估。引入语义锚定方法，利用语言为视觉特征提供稳定坐标系。通过嵌入空间分析和Grad-CAM可视化来理解模型行为。

Result: 少量样本微调提高了同癌症（64.9%到72.6% AUC）和跨癌症性能（56.84%到66.31% AUC）。跨物种评估显示组织匹配可实现有意义转移，但性能仍低于最先进基准（H-optimus-0: 84.97% AUC）。嵌入空间分析显示肿瘤和正常原型间余弦相似度极高（大于0.99）。语义锚定方法在同癌症（8.52%）和跨癌症分类（5.67%）中带来额外提升。

Conclusion: 标准视觉-语言对齐方法在跨物种泛化方面存在局限性，主要由于嵌入空间崩溃问题。语义锚定方法通过语言对齐机制有效解决了这一问题，使模型能够进行语义重新解释而无需重新训练。语言作为控制机制，为跨物种计算病理学提供了有前景的解决方案。
...
TL;DR: 该论文提出了首个跨模态的算子失配基准测试InverseNet，评估了12种方法在四种场景下的性能，发现深度学习模型在算子失配下性能下降10-21dB，且性能与鲁棒性呈负相关。


<details>
  <summary>Details</summary>
Motivation: 现有压缩成像系统在实际部署中普遍存在算子失配问题，但缺乏量化评估这一问题的基准测试。当前最先进的EfficientSCI在仅8个参数偏离物理现实时就损失20.58dB性能，突显了算子失配问题的严重性。

Method: 提出了InverseNet基准测试，涵盖CASSI、CACTI和单像素相机三种模态。采用四场景协议（理想、失配、oracle校正、盲校准）评估12种方法，在27个模拟场景和9个真实硬件捕获上进行测试。

Result: 发现：（1）深度学习方法在算子失配下损失10-21dB，失去对传统方法的优势；（2）性能与鲁棒性在跨模态下呈负相关（Spearman r_s=-0.71）；（3）掩码无关架构无法恢复失配损失，而算子条件化方法可恢复41-90%；（4）盲网格搜索校准无需真实数据即可恢复85-100%的oracle性能。

Conclusion: InverseNet是首个跨模态算子失配基准测试，揭示了压缩成像系统中算子失配的严重影响。真实硬件实验证实了模拟趋势的迁移性，为未来鲁棒压缩成像系统设计提供了重要参考。
...
TL;DR: 该研究分析了多模态遥感数据在本地气候区分类中的不同融合策略，通过四种融合模型和两种分组策略，在So2Sat LCZ42数据集上实现了76.6%的整体准确率。


<details>
  <summary>Details</summary>
Motivation: 本地气候区分类对研究城市结构和土地利用及其对当地气候的影响很重要。多模态遥感数据融合能提高分类精度，但目前缺乏对深度学习分类器中融合机制的系统分析。

Method: 研究分析了四种融合模型：基线混合融合、自注意力和交叉注意力机制融合、多尺度高斯滤波图像融合、加权决策级融合。同时采用波段分组和标签合并两种分组策略，在So2Sat LCZ42数据集上进行消融实验。

Result: 基线混合融合方法优于简单融合方法，结合波段分组和标签合并策略达到76.6%的整体准确率，显著提高了对代表性不足类别的预测准确性。

Conclusion: 多模态数据融合策略对本地气候区分类至关重要，特别是结合适当的融合机制和分组策略能显著提升分类性能，尤其对代表性不足的类别效果明显。
...
TL;DR: PinPoint是一个全面的组合图像检索基准数据集，包含7,635个查询和329K相关性标注，支持多正确答案、硬负样本、指令改写、多图像组合和公平性评估，揭示了现有方法的三大缺陷并提出了基于MLLM的重新排序解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前组合图像检索（CIR）的基准存在局限性：只有单一正确答案、缺乏评估假阳性避免能力的标注、缺少鲁棒性测试和多图像推理评估。需要更全面的真实世界基准来推动CIR技术的发展。

Method: 创建PinPoint基准数据集，包含7,635个查询和329K相关性标注，涵盖23个查询类别。数据集提供：1）每个查询平均9.1个正确答案；2）明确的硬负样本；3）每个查询6个指令改写用于鲁棒性测试；4）支持多图像组合（13.4%的查询）；5）人口统计元数据用于公平性评估。基于对20多种方法的分析，提出基于现成MLLM的训练免费重新排序方法。

Result: 分析发现三大缺陷：1）最佳方法mAP@10为28.5%，但仍有9%的时间检索到硬负样本；2）最佳模型在不同改写间性能变化达25.1%；3）多图像查询性能下降40-70%。提出的重新排序方法可以应用于任何现有系统来弥补这些差距。

Conclusion: PinPoint基准为CIR领域提供了更全面的评估框架，揭示了现有方法的不足，特别是对硬负样本的敏感性、指令改写的鲁棒性和多图像组合的挑战。提出的MLLM重新排序方法展示了改进现有系统的潜力。数据集和代码已公开发布。
...
TL;DR: SGR3 Model是一个免训练的3D场景图生成框架，利用多模态大语言模型和检索增强生成技术，无需显式3D重建，通过检索语义对齐的场景图来增强关系推理。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景图生成方法通常需要多模态数据且依赖启发式图构建，限制了关系三元组的预测能力。需要一种更灵活、免训练的方法来生成语义场景图。

Method: 提出SGR3 Model框架，结合多模态大语言模型和检索增强生成技术，采用ColPali风格的跨模态框架检索语义对齐的场景图，并引入加权补丁级相似性选择机制提高检索鲁棒性。

Result: 实验表明SGR3 Model在免训练基线方法中具有竞争力，性能与基于GNN的专家模型相当。检索模块消融研究显示外部信息被显式整合到token生成过程中。

Conclusion: SGR3 Model提供了一种有效的免训练3D场景图生成方法，通过检索增强生成技术显式整合外部知识，避免了传统方法对多模态数据和启发式图构建的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散MRI方法要么假设不可渗透边界，要么仅估计体素级参数而无法恢复明确的界面结构。需要一种能够重建可渗透边界微观结构的方法。

Method: 在固定四面体网格上表示组织，将每个内部面的渗透率作为可学习参数；通过可微分的Bloch-Torrey PDE模拟器反演dMRI测量值，使用信号匹配损失通过前向模型反向传播优化面渗透率；采用网格几何先验缓解反演不适定性，使用分阶段多序列优化课程避免局部最小值。

Result: 在合成体素网格集合上，Spinverse能够重建多样化的几何结构，证明序列调度和正则化对于避免仅轮廓解、同时提高边界精度和结构有效性至关重要。

Conclusion: Spinverse是一种创新的渗透率感知重建方法，能够在固定网格上恢复微观结构边界，为扩散MRI的微观结构分析提供了新途径。
...
Method: 提出sFRC方法：在深度学习输出和参考图像的小区域上进行傅里叶环相关分析，通过扫描整个图像来检测幻觉区域

Result: sFRC能有效检测CT超分辨率、CT稀疏视图和MRI欠采样重建中的幻觉特征，与基于成像理论的幻觉图一致，并能量化不同条件下的幻觉率

Conclusion: sFRC为深度学习医学图像重建提供了一种有效的幻觉检测工具，有助于评估和比较不同重建方法的鲁棒性
...
TL;DR: PulseFocus是一种无需训练、推理时的方法，通过将思维链推理结构化为交替的计划/聚焦块，并采用软注意力门控，改善多图像推理中注意力分散的问题。


<details>
  <summary>Details</summary>
Motivation: 研究发现视觉语言模型在多图像推理中存在注意力分散的"脉冲"现象，即文本到图像注意力模式呈现零散且不聚焦的状态，无法集中在任务相关图像上，同时还存在系统性的位置偏见。

Method: 提出PulseFocus方法：1) 将思维链推理结构化为交替的计划块和聚焦块；2) 在计划块中明确规划要检查的图像；3) 在聚焦块中使用软注意力门控，将解码时注意力限制在参考图像上。

Result: 在BLINK基准测试上提升3.7%，在MuirBench上提升1.07%，表明PulseFocus能够显著改善多图像推理性能。

Conclusion: 通过结构化思维链推理并引入注意力门控机制，PulseFocus有效解决了多图像推理中的注意力分散问题，为视觉语言模型的多模态推理提供了有效的推理时优化方案。
...
TL;DR: 本文系统评估了多模态大语言模型在视频异常检测任务中的表现，发现其在零样本设置下存在明显的保守偏见，导致高精度但召回率崩溃，限制了实际应用价值。
...
Result: 研究发现零样本设置下存在明显的保守偏见：模型表现出高置信度，但过度偏向"正常"类别，导致高精度但召回率崩溃。类别特定指令能显著改变决策边界，将ShanghaiTech上的峰值F1分数从0.09提升到0.64，但召回率仍是关键瓶颈。

Conclusion: MLLMs在嘈杂环境中存在显著性能差距，这为未来在开放世界监控领域的工作奠定了基础，该领域需要复杂的视频理解和推理能力，特别是在面向召回的提示设计和模型校准方面。
...
Method: 提出FOZO（Forward-Only Zeroth-Order Optimization）方法，采用内存高效的零阶提示优化，通过优化中间特征统计和预测熵的目标函数，并引入动态衰减扰动尺度来确保在分布外数据流上的高效稳定适应。

Result: 在ImageNet-C、ImageNet-R和ImageNet-Sketch上的持续适应实验中，FOZO在ImageNet-C（5K，level 5）上达到59.52%的Top-1准确率，优于主要基于梯度的方法和SOTA前向方法FOA（58.13%），同时在量化（INT8）模型上表现出强泛化能力。
...
TL;DR: 提出Real-IISR框架，通过热结构引导的自回归方法解决真实世界红外图像超分辨率问题，构建了FLIR-IISR数据集并验证了方法有效性。


<details>
  <summary>Details</summary>
Motivation: 真实世界红外图像超分辨率是一个具有实际意义但很少被解决的问题。现有方法通常在模拟数据集上训练评估，或忽视了红外与可见光成像的本质差异。实际中，真实红外图像受到耦合的光学和传感退化影响，同时损害结构清晰度和热保真度。
...
Method: 采用对照横断面评估方法，使用标准化零样本思维链协议，在医学教育考试、文本推理基准以及神经放射学、数字病理学和乳腺X线摄影的视觉问答任务上比较GPT-5系列与GPT-4o。

Result: GPT-5在专家级文本推理上表现显著提升（MedXpertQA绝对改进超过25个百分点）；在多模态合成中能有效利用增强的推理能力，在大多数VQA基准测试中达到SOTA或竞争性表现；在乳腺X线摄影任务中比GPT-4o提升10-40%。但在神经放射学中表现中等（44%宏观平均准确率），在乳腺X线摄影中落后于专用系统（专用系统>80% vs GPT-5的52-64%）。

Conclusion: GPT-5代表了向综合多模态临床推理的有意义进步，能够模拟临床医生用客观发现修正不确定信息的认知过程，但在高度专业化、感知关键的任务中，通用模型尚不能替代专用系统。
...
Result: 在七个广泛认可的微表情数据集上进行实验，结果表明GAMDSS有效减少了SAMM和4DME等多文化数据集中人为因素造成的主观误差；定量分析证实多文化数据集中的offset帧标注更不确定
...
TL;DR: 本文提出DSA-SRGS，首个用于动态稀疏视图DSA重建的超分辨率高斯泼溅框架，通过多保真度纹理学习和辐射亚像素致密化策略，显著提升血管细节重建质量。
...
Method: 提出DSA-SRGS框架：1）多保真度纹理学习模块，集成微调的DSA专用超分辨率模型的高质量先验到4D重建优化中；2）置信度感知策略，自适应加权原始低分辨率投影和生成的高分辨率伪标签之间的监督信号；3）辐射亚像素致密化策略，利用高分辨率亚像素采样的梯度累积来细化4D辐射高斯核。
...
TL;DR: 本文提出了一种基于AI Flow范式和边缘-云协作架构的新型隐私保护感知框架，通过非线性映射和随机噪声注入将原始图像转换为抽象特征向量，实现身份敏感信息剥离和原始图像数学不可重构，同时保持行为识别能力。


<details>
  <summary>Details</summary>
Motivation: 随着智能感知技术在卫生间、更衣室等高敏感环境中的部署，视觉监控系统面临隐私-安全悖论。现有隐私保护方法（物理脱敏、加密、混淆）往往损害语义理解或无法确保数学可证明的不可逆性。虽然Privacy Camera 1.0从源头消除视觉数据防止泄漏，但仅提供文本判断，导致争议中的证据盲点。

Method: 基于AI Flow范式和协作边缘-云架构，在边缘部署视觉脱敏器，通过非线性映射和随机噪声注入将原始图像实时转换为抽象特征向量（遵循信息瓶颈原则），确保身份敏感信息被剥离且原始图像数学不可重构。抽象表示传输到云端进行行为识别和通过"动态轮廓"视觉语言进行语义重建。

Result: 实现了感知与隐私之间的关键平衡，能够在保护原始图像不被暴露的同时，提供说明性的视觉参考，解决了现有隐私保护方法在语义理解和不可逆性方面的不足。

Conclusion: 该框架通过边缘-云协作架构和AI Flow范式，成功解决了高敏感环境中视觉监控的隐私-安全悖论，在确保数学可证明的隐私保护同时，保持了系统的感知能力和视觉参考功能。
...
Method: 提出了Rotated Multi-Kernel RetinaNet，包含四个核心组件：1) 多尺度核模块增强自适应多尺度特征提取；2) 多方向上下文锚点注意力机制增强跨尺度和跨方向的上下文建模；3) 自底向上路径保留在下采样过程中丢失的细粒度空间细节；4) 欧拉角编码模块实现连续稳定的角度回归。

Result: 在DOTA-v1.0、HRSC2016和UCAS-AOD数据集上的大量实验表明，RMK RetinaNet在保持与最先进旋转目标检测器相当性能的同时，在多尺度和多方向场景中提高了鲁棒性。

Conclusion: RMK RetinaNet通过解决遥感图像旋转目标检测的三个主要瓶颈，实现了更有效的多尺度和多方向目标检测，为遥感图像分析提供了更鲁棒的解决方案。
...
TL;DR: MASQuant是一种针对多模态大语言模型的后训练量化方法，通过模态感知平滑和跨模态补偿解决现有量化方法在多模态场景下的对齐和计算不变性问题。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法在大型语言模型上表现良好，但在多模态大语言模型上应用面临挑战。作者以SmoothQuant为案例研究，发现了两个关键问题：平滑错位和跨模态计算不变性。
...
Conclusion: MASQuant通过解决多模态量化中的对齐和计算不变性问题，为多模态大语言模型提供了有效的后训练量化解决方案，在保持性能的同时实现了模型压缩。
...
TL;DR: DCR方法通过将对比信号注入扩散重建过程，同时优化CLIP的判别能力和细节感知能力，解决了现有方法在增强CLIP视觉编码器时损害判别能力的问题。


<details>
  <summary>Details</summary>
Motivation: CLIP视觉编码器的理解能力有限已成为下游性能的关键瓶颈，包括判别能力（类间可分性）和细节感知能力（细粒度视觉线索）。现有基于扩散模型的方法在增强表示时可能损害判别能力，需要更全面的解决方案。

Method: 提出扩散对比重建（DCR）方法，将对比信号注入扩散重建过程。关键创新是从每个重建图像（而非原始输入）中提取对比信号，统一学习目标，避免梯度冲突，平衡判别能力和细节感知能力的优化。

Result: 在多个基准测试和多模态大语言模型上的广泛实验验证了DCR的有效性。理论分析表明DCR损失可以联合优化判别能力和细节感知能力。

Conclusion: DCR通过将对比学习与扩散重建相结合，有效解决了CLIP视觉编码器的表示限制，同时提升了判别能力和细节感知能力，为多模态表示学习提供了更全面的解决方案。
...
TL;DR: 通过高质量偏振数据集和传感器感知数据增强，轻量级偏振模型在小数据集上超越大规模RGB视觉基础模型，实现33倍数据减少或8倍参数减少


<details>
  <summary>Details</summary>
Motivation: 现有偏振方法性能弱于RGB视觉基础模型，但作者认为这不是偏振模态本身的问题，而是由领域差距造成：1）合成数据集使用有限且不真实的3D对象；2）真实世界偏振信号受传感器噪声影响

Method: 1）使用1,954个3D扫描真实物体渲染高质量偏振数据集；2）引入预训练DINOv3先验提升泛化能力；3）设计偏振传感器感知数据增强以更好模拟真实条件

Result: 仅用40K训练场景，方法显著超越最先进的偏振方法和RGB视觉基础模型；偏振线索可实现33倍训练数据减少或8倍模型参数减少，同时保持更好性能

Conclusion: 偏振线索在单次物体级表面法线估计中具有重要价值，通过解决领域差距问题，轻量级模型在小数据集上可超越大规模RGB视觉基础模型
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中部分标签学习的候选标签通常受实例特征影响，形成实例依赖PLL。实例纠缠问题导致相似类别的实例共享重叠特征和候选标签，增加了类别混淆

Method: 提出类特定增强解缠框架，包含类内调控（放大类特定特征生成类级增强，对齐相同类别的增强）和类间调控（引入加权惩罚损失函数，对更模糊的标签施加更强惩罚，鼓励更大的类间距离）

Result: 广泛的实验结果表明CAD在缓解纠缠问题和提升ID-PLL性能方面具有有效性
...
Method: 提出语义增强的动态对比攻击(SADCA)，通过对抗图像与文本的动态交互逐步破坏跨模态对齐，建立包含对抗样本、正样本和负样本的对比学习机制，增强扰动语义不一致性，并引入语义增强模块增加对抗样本多样性

Result: 在多个数据集和模型上的实验表明，SADCA显著提高了对抗可迁移性，并持续超越现有最先进方法

Conclusion: SADCA通过渐进式语义引导的扰动和语义增强，有效提升了视觉语言预训练模型的对抗攻击可迁移性
...
TL;DR: GloSplat是一个联合姿态-外观优化框架，在3D高斯泼溅训练中同时优化相机姿态和场景表示，通过显式保留SfM特征轨迹作为一等实体，结合重投影损失和光度监督，避免了纯光度方法的早期姿态漂移问题。
...
Method: 提出GloSplat框架，在3D高斯泼溅训练中执行联合姿态-外观优化。关键创新是将显式SfM特征轨迹作为一等实体保留：轨迹3D点作为与高斯基元分离的可优化参数维护，通过重投影损失提供持久几何锚点，与光度监督并行工作。提供两种变体：GloSplat-F（无COLMAP，使用检索式对选择）和GloSplat-A（穷举匹配，追求最高质量）。

Result: 实验表明，GloSplat-F在无COLMAP方法中达到最先进水平，而GloSplat-A超越了所有基于COLMAP的基线方法。该方法能够防止早期姿态漂移，同时实现细粒度细化，这是纯光度方法所不具备的能力。

Conclusion: GloSplat通过将显式SfM特征轨迹整合到3D高斯泼溅训练中，实现了联合姿态-外观优化，在保持几何一致性的同时提高了重建质量，为3D重建和新视角合成提供了更鲁棒的解决方案。
...
TL;DR: 基于单目广播视频的投手生物力学指标恢复与损伤预测系统，无需专业多相机系统即可从普通视频中提取18个临床相关指标，用于投手损伤风险评估。
...
Method: 基于DreamPose3D的单目视频流程，引入漂移控制的全局提升模块恢复骨盆轨迹，采用基于速度的参数化和滑动窗口推理。针对运动模糊、压缩伪影和极端投球姿势，加入包含骨长约束、关节限制逆向运动学、平滑和对称约束的运动学细化流程。

Result: 在13名职业投手（156个配对投球）上，16/18个指标达到亚度级一致性（MAE < 1°）。使用这些指标进行损伤预测，自动化筛查模型在7,348名投手上达到汤米约翰手术AUC 0.811和重大手臂损伤AUC 0.825。

Conclusion: 单目广播视频可作为体育场规模运动捕捉的可行替代方案，支持可扩展的损伤风险筛查，为生物力学分析提供了经济高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像匹配方法在视角变化大或纹理缺乏区域容易产生高相似度但错误的对应点，主要原因是传统模型仅依赖特征相似性，缺乏估计匹配可靠性的显式机制，导致过度自信的错误。

Method: 提出了SURE半稠密不确定性精化匹配框架，通过建模偶然不确定性和认知不确定性来联合预测对应点及其置信度。引入新颖的证据头进行可信坐标回归，以及轻量级空间融合模块以最小开销增强局部特征精度。

Result: 在多个标准基准测试中，该方法在准确性和效率方面一致优于现有的最先进半稠密匹配模型。

Conclusion: SURE框架通过联合预测对应点和置信度，有效解决了图像匹配中的可靠性问题，在挑战性场景下表现出色，代码将在GitHub上开源。
...
Conclusion: PNG框架通过消除对相机元数据的依赖，显著提升了噪声合成的泛化性和适用性，为真实世界噪声去除提供了有效的生成方法
...
<details>
  <summary>Details</summary>
Motivation: CT报告生成面临CT图像数据量大、描述细节复杂的挑战，现有X-ray报告生成方法在CT上效果有限，需要专门针对CT特点的解决方案

Method: 两阶段框架：第一阶段使用可学习的结构特定视觉查询观察CT图像结构，通过结构级图像-文本对比损失学习语义对应，提出软伪目标缓解假阴性问题，使用动态多样性增强负样本队列；第二阶段冻结视觉查询，选择关键图像块嵌入，添加文本解码器生成报告

Result: 在两个公开数据集上实现了最先进的CT报告生成性能，在临床效率方面表现优异，各组件均有效

Conclusion: 提出的两阶段框架能有效处理CT图像的大数据量和复杂细节，通过结构级对比学习和动态负样本队列显著提升了CT报告生成的性能
...
TL;DR: FedMEPD：一种用于医学图像分析的新型联邦学习框架，通过联邦模态特定编码器和部分个性化多模态融合解码器，同时解决模态间异质性和个性化需求问题。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法主要关注模态内异质性，难以处理多模态医学成像应用中部分参与者仅拥有部分模态数据的现实问题。同时，每个参与者都期望获得针对其本地数据特征定制的个性化模型。

Method: FedMEPD采用每个模态的专属编码器处理模态间异质性，编码器完全联邦化，解码器部分个性化。服务器使用融合解码器融合所有模态表示，客户端通过缩放点积交叉注意力将缺失模态表示校准到全局多模态锚点。
...
Method: 1. 使用可学习高斯核调制自注意力，使注意力偏向相邻图像块；2. 细化图像块表示以学习更好的空间位置嵌入；3. 保持模型整合全局信息的能力

Result: 在三个基准测试上获得显著分割性能提升（如ViT Tiny和Base在ADE20K上分别提升超过6%和4%），同时不改变训练机制且不牺牲分类性能

Conclusion: LocAtViT是一种简单有效的附加模块，能够增强视觉Transformer的分割性能，同时保持其图像级识别能力，为视觉Transformer在密集预测任务中的应用提供了新思路
...
Method: 1) 在潜在序列上引入时间建模策略以继承起始和结束帧的保真度线索；2) 利用语义匹配线进行结构感知运动引导；3) 提出时间差异损失来缓解时间不一致性

Result: FC-VFI在4×和8×插值中表现优异，能将30FPS视频提升至120和240FPS（2560×1440分辨率），在各种场景中保持高性能和结构完整性

Conclusion: FC-VFI通过创新的时间建模和语义匹配线方法，有效解决了视频帧插值中的保真度和一致性问题，在高质量视频插值任务中表现出色
...
TL;DR: 该论文提出了一种名为AdaIAT的方法，通过关注已生成文本来减轻大型视觉语言模型的幻觉问题，同时避免重复描述，实现了幻觉减少与语言性能的良好平衡。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型面临严重的幻觉问题，传统方法通过增加对图像标记的关注权重来减少幻觉，但这往往导致重复描述。研究发现真实对象标记倾向于对已生成文本分配更高注意力，这为解决幻觉问题提供了新思路。

Method: 提出了Attention to Generated Text (IAT)方法，利用已生成文本中的视觉信息和上下文知识来减轻幻觉。进一步开发了Adaptive IAT (AdaIAT)，采用分层阈值控制干预时机，并根据每个注意力头的特性进行细粒度放大，避免损害模型的固有预测能力。

Result: 实验表明AdaIAT显著降低了幻觉率，在LLaVA-1.5模型上将幻觉率C_S和C_I分别降低了35.8%和37.1%，同时保持了语言性能和预测能力，实现了良好的平衡。

Conclusion: 通过关注已生成文本并采用自适应干预策略，AdaIAT有效解决了大型视觉语言模型的幻觉问题，在减少幻觉的同时避免了重复描述，为模型优化提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 前列腺癌活检需求增加给病理医生带来沉重负担，现有深度学习模型性能虽好但可解释性不足，限制了其在医疗高风险应用中的广泛采用。

Method: 提出原型驱动的弱监督框架：1) 在patch级别预训练学习各等级的原型特征；2) 使用新的原型感知损失函数进行弱监督微调；3) 引入基于注意力的动态剪枝机制处理样本异质性，选择性强调相关原型。

Result: 在PANDA和SICAP基准数据集上进行广泛验证，证实该框架可作为病理医生日常诊断工作流程中的可靠辅助工具。

Conclusion: 提出的原型框架通过显式推理过程模拟病理医生工作流程，提高了前列腺癌分级模型的可解释性和可信度，有望促进深度学习在医疗高风险应用中的采纳。
...
TL;DR: 提出了一种用于医学差异视觉问答的预训练框架，通过位置感知任务学习细粒度空间视觉表示，在胸部X光图像上实现了最先进的性能。
...
Method: 引入包含自动指代表达式(AREF)、基础描述(GCAP)和条件自动指代表达式(CAREF)等位置感知任务的预训练框架，增强视觉编码器学习细粒度空间视觉表示的能力，然后与语言模型集成进行医学差异VQA。

Result: 实验结果表明，该方法在检测和推理胸部X光图像中临床相关变化方面实现了最先进的性能。

Conclusion: 通过位置感知预训练任务增强的视觉编码器能够有效捕捉医学图像中的细微视觉变化，显著提升了医学差异VQA的性能，更贴近放射科医生的比较诊断工作流程。
...
Result: 紧凑多模态模型能够实现有竞争力的性能，同时产生更结构化、更详细的描述，证明不需要依赖激进的模型扩展也能获得高质量结果。

Conclusion: VisionPangu展示了通过高效多模态对齐和高质量监督，紧凑模型能够改进详细图像描述生成，为多模态理解提供了更高效的解决方案。
...
TL;DR: 提出了一种用于单目3DMM回归方法的新型相机模型，有效捕捉近距离面部图像中的透视畸变效应，解决了传统正交投影在近距离拍摄（如头戴式相机）中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的3DMM回归方法通常使用正交投影，虽然稳定但无法处理近距离拍摄时产生的透视畸变效应，特别是在头戴式相机等设备拍摄的近距离面部图像中表现不佳。

Method: 在正交投影基础上引入新的收缩参数，创建伪透视效果，同时保持原始投影的稳定性。提出了多种技术来微调现有模型，并使用头戴式相机录制的自定义数据集进行评估。

Result: 通过定量和定性比较，证明了所提修改的有效性，特别是在处理近距离面部图像时，新模型能够更好地捕捉透视畸变效应。

Conclusion: 提出的新型相机模型成功解决了3DMM回归方法在处理近距离面部图像时的透视畸变问题，为头戴式相机等近距离拍摄场景提供了更准确的3D面部重建解决方案。
...
TL;DR: BiEvLight：一种利用事件相机进行低光图像增强的层次化任务感知框架，通过双层优化协同优化去噪和增强，显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有事件相机低光增强方法主要关注模态融合策略，但面临双重退化问题：事件中的固有背景活动噪声和图像的低信噪比，导致模态融合时噪声耦合严重，成为性能瓶颈。精确的事件去噪是释放事件融合潜力的前提。

Method: 提出BiEvLight框架：1）利用图像和事件的强梯度相关性构建梯度引导的事件去噪先验，缓解重度噪声区域去噪不足；2）将事件去噪重构为受增强任务约束的双层优化问题，通过跨任务交互，上层去噪问题学习适合下层增强目标的事件表示。

Result: 在真实噪声数据集SDE上的实验表明，该方法显著优于现有最先进方法，PSNR平均提升1.30dB，PSNR*提升2.03dB，SSIM提升0.047。

Conclusion: BiEvLight通过协同优化去噪和增强，有效解决了事件相机低光增强中的噪声耦合问题，为事件融合方法提供了新思路，代码将开源。
...
Conclusion: 自回归模型在统一多模态理解和生成任务中具有显著潜力，Wallaroo的成功证明了通过适当的架构设计和训练策略可以实现这一目标
...
Method: 提出Transient Asynchronous Fusion (TAF)机制显式建模离散帧间的时间演化，以及Cross-modal Locally Weighted Fusion (CLWF)模块根据模态可靠性自适应调整空间注意力

Result: 在真实世界帧-事件TAP数据集上平均像素误差提升28.2%，在标准点跟踪基准上持续取得最佳性能

Conclusion: TAPFormer通过异步时间一致性融合框架和事件，实现了鲁棒且高频的任意点跟踪，显著优于现有方法
...
TL;DR: MultiGO++：一种从单张图像重建3D穿衣人体的新框架，通过多源纹理合成、区域感知形状提取和双重建U-Net实现几何与纹理的有效协作，解决了现有方法在纹理、几何和系统监督方面的局限性。
...
Method: 提出MultiGO++框架，包含三个核心部分：1) 多源纹理合成策略，构建15,000+ 3D纹理人体扫描数据；2) 区域感知形状提取模块和傅里叶几何编码器；3) 双重建U-Net，利用几何-纹理协作特征生成高保真3D人体网格。

Result: 在两个基准测试和大量真实场景案例上的广泛实验表明，该方法在纹理质量和几何精度方面优于现有最先进方法。

Conclusion: MultiGO++通过有效的系统几何-纹理协作，解决了单目3D穿衣人体重建中的关键挑战，实现了高质量的重建效果。
...
<details>
  <summary>Details</summary>
Motivation: 尽管生成式图像修复在感知真实性方面取得了显著进展，但其实际能力相比传统方法的真正进步程度尚不明确。需要系统评估不同架构模型在多个维度上的表现，以了解该领域的真实发展状态。

Method: 提出了一个基于多维评估管道的大规模研究框架，评估模型在细节、锐度、语义正确性和整体质量等方面的表现。涵盖了扩散模型、GAN模型、PSNR导向模型和通用生成模型等多种架构。还利用该基准训练了一个与人类感知判断更一致的新图像质量评估模型。

Result: 研究发现不同模型架构之间存在关键性能差异，揭示了低层视觉领域的范式转变：核心挑战已从过去的细节不足问题演变为细节质量和语义控制的新前沿。同时开发了能更好对齐人类感知判断的图像质量评估模型。

Conclusion: 这项工作为现代生成式图像修复模型提供了系统性研究，提供了重新理解其真实状态的关键见解，并为未来发展指明了方向。研究强调了从防止细节不足转向控制细节质量和语义正确性的新研究重点。
...
TL;DR: Tell2Adapt是一个新颖的源自由无监督域适应框架，利用视觉基础模型的通用知识，通过上下文感知提示正则化和视觉合理性细化，在医学图像分割中实现统一的跨模态、多目标域适应。


<details>
  <summary>Details</summary>
Motivation: 现有的源自由无监督域适应方法通常针对低差距、特定域偏移设计，无法推广到统一的多模态、多目标框架，这阻碍了深度学习模型在多样化临床环境中的实际应用。

Method: 提出Tell2Adapt框架：1）使用上下文感知提示正则化确保高质量视觉基础模型提示，将多样化文本提示转化为规范指令；2）生成高质量伪标签，使轻量级学生模型适应目标域；3）引入视觉合理性细化，利用视觉基础模型的解剖学知识，将适应模型的预测重新基于目标图像的低层视觉特征，消除噪声和误报。

Result: 在迄今为止最广泛的源自由无监督域适应评估中，验证了框架在10个域适应方向和22个解剖目标（包括脑部、心脏、息肉和腹部目标）上的有效性。Tell2Adapt始终优于现有方法，在医学图像分割中实现了统一的源自由无监督域适应框架的最先进性能。

Conclusion: Tell2Adapt成功解决了现有源自由无监督域适应方法的局限性，通过利用视觉基础模型的通用知识，实现了统一的跨模态、多目标域适应框架，在医学图像分割中表现出卓越性能，为临床实际应用提供了可行解决方案。
...
TL;DR: 提出Semap历史地图语义分割基准数据集和通用分割框架，通过数据合成和多尺度集成提高模型泛化能力，在异构地图集合上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 历史地图集合在风格、比例尺和地理焦点上高度多样化，但现有地图识别研究主要针对同质地图系列。需要开发通用性强的语义分割模型和本体来处理多样化的历史地图文档

Method: 1) 引入Semap基准数据集（1,439个手动标注的图块）；2) 提出结合程序化数据合成和多尺度集成的分割框架，提高鲁棒性和可迁移性

Result: 在HCMSSD和Semap数据集上达到最先进性能，分割性能在地图集合、比例尺、地理区域和出版背景方面保持稳定，证明了多样性驱动方法的可行性

Conclusion: 通过提出历史地图通用分割的基准数据集和方法，为整合长尾制图档案到历史地理研究开辟了道路，展示了多样性驱动的地图识别方法的优势
...
<details>
  <summary>Details</summary>
Motivation: 多相机3D目标检测模型难以泛化到具有新相机配置的未见平台上，现有方法仅使用元相机进行统一表示但缺乏全面考虑。研究发现问题的根源在于源配置和目标配置之间的空间先验差异，包括不同的内参、外参和阵列布局。

Method: 提出CoIn3D框架，通过两种方式处理空间先验差异：1）空间感知特征调制（SFM），在特征嵌入中集成四种空间表示（焦距、地面深度、地面梯度和普吕克坐标）；2）相机感知数据增强（CDA），通过免训练的动态新视角图像合成方案提高各种配置下的观测多样性。

Result: 在NuScenes、Waymo和Lyft等基准数据集上的广泛实验表明，CoIn3D在BEVDepth、BEVFormer和PETR三种主流多相机3D检测范式下均实现了强大的跨配置性能。

Conclusion: CoIn3D通过显式地整合空间先验信息到特征嵌入和图像观测中，有效解决了多相机配置间的空间差异问题，为多相机3D目标检测提供了强大的跨平台泛化能力。
...
TL;DR: 提出CLIP驱动的部分标签零样本学习框架，解决真实场景中标签噪声和模糊性问题，通过语义挖掘和渐进式标签优化提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统零样本学习方法假设训练实例具有准确的类别标签，但现实场景中噪声和模糊标签会显著降低性能，需要处理标签模糊性问题。

Method: 使用CLIP提取实例和标签特征，通过语义挖掘块融合特征提取判别性标签嵌入，引入部分零样本损失函数，基于标签与实例相关性分配权重，对齐实例和标签嵌入以减少语义不匹配，采用渐进式标签优化策略。

Result: 在多个数据集上的综合实验表明CLIP-PZSL具有优势，能够有效处理标签模糊性并提升零样本学习性能。

Conclusion: 提出的CLIP-PZSL框架能够有效处理零样本学习中的标签模糊性问题，通过CLIP特征提取、语义挖掘和渐进式标签优化实现更好的语义对齐，在现实场景中具有实用价值。
...
TL;DR: 开发了基于RT-DETR的紧急车辆蓝光检测系统，使用四鱼眼摄像头配置，在ABLDataset上达到94.7%准确率和94.1%召回率，检测距离达70米，可集成到多模态ADAS系统中。


<details>
  <summary>Details</summary>
Motivation: 提升高级驾驶辅助系统(ADAS)和道路安全，通过检测紧急车辆的蓝光信号，为驾驶员提供更早的预警，减少交通事故风险。

Method: 使用四台180度水平视场鱼眼摄像头，通过校准实现方位定位；比较多种深度学习算法后选择RT-DETR作为基础模型，并加入颜色注意力模块；利用几何变换估计紧急车辆相对于汽车中心的接近角度。

Result: 在测试集上达到94.7%准确率和94.1%召回率，现场测试检测距离可达70米；系统能够准确估计紧急车辆的接近角度，为多模态系统集成提供可靠视觉数据。

Conclusion: 该系统在紧急车辆蓝光检测方面表现出高效性能，为ADAS系统提供了有前景的视觉检测方案，通过结合声学数据可构建更完善的多模态安全系统。
...
TL;DR: UniM是首个统一任意到任意交错多模态数据集和评估套件，涵盖7种模态，用于评测多模态大语言模型在交错理解和生成任务上的能力。


<details>
  <summary>Details</summary>
Motivation: 现实多模态应用需要系统理解用户任意组合和交错的多模态输入，同时生成任意交错的多媒体输出。这定义了统一理解和生成范式下的任意到任意交错多模态学习目标，为多模态大语言模型带来了新的挑战和机遇。

Method: 提出UniM基准，包含31K高质量实例，涵盖30个领域和7种代表性模态（文本、图像、音频、视频、文档、代码、3D）。引入UniM评估套件，从语义正确性与生成质量、响应结构完整性、交错连贯性三个维度评估模型。提出UniMA基线模型，配备可追溯推理机制用于结构化交错生成。

Result: 综合实验证明了UniM基准的难度，突显了推进统一任意到任意多模态智能的关键挑战和方向。

Conclusion: UniM基准为评测和推进多模态大语言模型在任意到任意交错多模态理解和生成能力提供了重要工具，揭示了该领域的关键挑战和发展方向。
...
TL;DR: 提出STD-4D扩散模型，通过正交时空分布转移机制从3D和视频扩散模型中迁移空间和时间先验，结合时空感知HexPlane改进4D合成质量


<details>
  <summary>Details</summary>
Motivation: 当前4D合成研究因缺乏大规模4D数据集而受限，模型难以学习高质量4D生成所需的关键时空特征，阻碍了该领域发展

Method: 提出STD-4D扩散模型，通过解耦的空间和时间潜在变量合成4D感知视频；设计正交时空分布转移机制，将时空特征分布建模并注入模型；开发时空感知HexPlane整合转移的时空特征，改进4D变形和4D高斯特征建模

Result: 实验表明该方法显著优于现有方法，实现了更优的时空一致性和更高质量的4D合成

Conclusion: 通过从现有3D和视频扩散模型迁移丰富的先验知识，有效解决了4D数据集缺乏的问题，提升了4D合成质量
...
TL;DR: GEM-TFL：基于图模型和EM算法的弱监督时序伪造定位方法，通过两阶段分类-回归框架、EM优化、时序一致性优化和图基提案优化，显著提升弱监督下的伪造片段定位精度。


<details>
  <summary>Details</summary>
Motivation: 当前弱监督时序伪造定位方法存在训练与推理目标不匹配、二元标签监督信息有限、不可微top-k聚合导致的梯度阻塞、以及缺乏提案间关系建模等问题，需要更有效的弱监督定位方法。

Method: 提出GEM-TFL两阶段分类-回归框架：1) 通过EM优化将二元标签重构为多维潜在属性增强监督；2) 无训练时序一致性优化平滑帧级预测；3) 图基提案优化模块建模提案间时序语义关系实现全局一致置信度估计。

Result: 在基准数据集上的大量实验表明，GEM-TFL实现了更准确和鲁棒的时序伪造定位，显著缩小了与全监督方法的性能差距。

Conclusion: GEM-TFL通过创新的两阶段框架和多个优化模块，有效解决了弱监督时序伪造定位中的关键挑战，为多媒体取证和安全提供了更实用的解决方案。
...
TL;DR: Diff-ES：一种通过进化搜索优化阶段稀疏度调度的扩散模型剪枝框架，实现内存高效权重路由，在保持生成质量的同时显著加速推理
...
Method: Diff-ES将扩散轨迹分为多个阶段，通过进化搜索自动发现最优的阶段稀疏度调度，通过内存高效的权重路由动态激活阶段条件权重，无需复制模型参数。框架可与现有深度和宽度剪枝方法集成

Result: 在DiT和SDXL上的大量实验表明，Diff-ES能持续实现实际时钟加速，同时生成质量下降最小，在结构化扩散模型剪枝方面达到最先进性能

Conclusion: Diff-ES通过进化搜索自动优化阶段稀疏度调度，结合内存高效权重路由，在保持扩散模型生成质量的同时显著提升推理效率，为扩散模型剪枝提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 自然杀伤细胞的细胞毒性是细胞相互作用动力学的重要例子，通常通过时间分辨多通道荧光显微镜研究。虽然可以在单帧中标注肿瘤细胞死亡事件，但NK细胞的细胞毒性结果随时间从细胞相互作用中产生，仅靠逐帧分类无法可靠推断。

Method: 引入BLINK模型，这是一个基于轨迹的循环状态空间模型，作为NK-肿瘤相互作用的细胞世界模型。该模型从部分观测的NK-肿瘤相互作用序列中学习潜在交互动态，并预测累积成细胞毒性结果的凋亡增量。
...
Conclusion: UniPAR打破了传统"一个模型对应一个数据集"的限制，为行人属性识别提供了一个统一的解决方案，能够有效处理多模态、多数据集场景，具有更好的泛化能力和实际应用价值。
...
TL;DR: 提出自适应VLA框架，基于任务复杂度动态路由执行路径，使用视觉嵌入作为复杂度检测器，在保持性能的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型通过推理技术提升泛化能力，但增加了计算复杂度和推理延迟，且资源分配效率低下，缺乏不确定性估计以防止分布外任务的灾难性失败。

Method: 提出自适应框架，将VLA的视觉语言骨干转换为主动检测工具，通过将潜在嵌入投影到参数化和非参数化估计器集合中，实现三种执行模式：直接执行已知任务、推理模糊场景、预判性停止执行。

Result: 发现视觉嵌入在推断任务复杂度方面优于语言嵌入，仅使用5%训练数据的视觉配置达到80% F1分数，在LIBERO和LIBERO-PRO基准测试及真实机器人上验证了可靠性和效率。

Conclusion: 基于视觉的自适应VLA框架能有效检测任务复杂度，实现高效资源分配，为VLA系统提供了可靠的不确定性估计和灾难性失败预防机制。
...
TL;DR: SSR-GS是一种用于光泽表面重建的镜面反射建模框架，通过Mip-Cubemap和IndiASG模块分别处理直接和间接镜面反射，并结合视觉几何先验优化重建效果。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在复杂光照下准确重建光泽表面仍然具有挑战性，特别是在强镜面反射和多表面相互反射的场景中。现有方法难以有效处理这些复杂的反射现象。

Method: 1. 引入预过滤的Mip-Cubemap高效建模直接镜面反射；2. 提出IndiASG模块捕捉间接镜面反射；3. 设计视觉几何先验，包括反射感知的视觉先验（通过反射分数）和来自VGGT的几何先验（渐进衰减深度监督和变换法线约束）。

Result: 在合成和真实世界数据集上的广泛实验表明，SSR-GS在光泽表面重建方面达到了最先进的性能。

Conclusion: SSR-GS通过有效的镜面反射建模和视觉几何先验的结合，成功解决了复杂光照下光泽表面重建的挑战，为3D高斯溅射在光泽表面重建领域提供了有效的解决方案。
...
TL;DR: 研究发现简单的肺边界框裁剪可以有效减少胸部X光中的种族捷径学习，同时保持诊断性能，绕过了公平性与准确性的权衡


<details>
  <summary>Details</summary>
Motivation: 深度学习模型能从胸部X光中高精度识别种族身份，存在种族捷径学习的风险，可能导致模型对特定人口群体产生系统性诊断偏见，威胁医疗公平和模型可靠性

Method: 研究图像预处理方法对减少种族捷径学习的影响，包括肺掩膜、肺裁剪和对比度受限自适应直方图均衡化(CLAHE)，这些方法旨在抑制编码种族信息的虚假线索同时保持诊断准确性

Result: 实验显示简单的基于边界框的肺裁剪是减少种族捷径学习的有效策略，同时能保持诊断模型性能，绕过了经常被假设的公平性-准确性权衡

Conclusion: 图像预处理方法特别是肺裁剪可以有效减轻胸部X光中的种族捷径学习，提供了一种简单实用的方法来提高医疗AI模型的公平性而不牺牲诊断准确性
...
<details>
  <summary>Details</summary>
Motivation: 通用相机校准比参数化校准更准确，但需要更多图像，导致运动模糊难以避免。传统方法在单个用户场景下难以处理运动模糊问题，需要新的解决方案。

Method: 结合几何约束和局部参数化光照模型，同时估计特征位置和空间变化的点扩散函数，解决了传统图像去模糊任务中不需要考虑的平移模糊问题。

Result: 实验结果表明该方法有效解决了通用相机校准中的运动模糊问题。

Conclusion: 这是首次尝试解决通用相机校准中的运动模糊问题，通过创新的几何和光照模型方法，为单个用户进行高精度相机校准提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法大多依赖预训练的视觉语言模型孤立编码图像-文本对，忽略了真实世界多模态数据自然形成的关系结构。这促使在多模态图上进行推理，其中节点具有文本和视觉属性，边提供结构线索。基于LLM的推理需要解决两个关键挑战：弱跨模态一致性和异质模态偏好。
...
Conclusion: Mario框架成功解决了多模态图推理中的跨模态一致性和异质模态偏好问题，实现了有效的基于LLM的多模态图推理，在多个任务上表现出优越性能。
...
TL;DR: Logi-PAR是首个逻辑融合的患者活动识别框架，通过可学习的逻辑规则和符号映射来识别患者活动，提供可审计的解释和反事实干预，在临床基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前的患者活动识别模型主要识别活动类型，通过全局和局部注意力机制组合稀疏视觉线索，但只能学习逻辑隐式模式。提升临床安全性需要能够推断为什么一组视觉线索意味着风险，以及如何通过显式逻辑进行组合推理的方法。
...
Conclusion: Logi-PAR是首个逻辑融合的患者活动识别框架，通过集成可学习逻辑规则和符号映射，不仅提高了识别性能，还提供了可解释的推理过程和反事实分析能力，对提升临床安全性具有重要意义。
...
TL;DR: 数字孪生驱动的机器人纺织品分拣系统，结合视觉语言模型、多模态感知和语义推理，实现真实工业环境中变形衣物和异物的自动化分类
...
Method: 双机械臂机器人单元集成RGBD感知、电容触觉反馈和碰撞感知运动规划，结合数字孪生和MoveIt进行路径规划，使用9种VLM模型对223个检测场景进行分类

Result: Qwen模型系列达到最高总体准确率（87.9%），异物检测性能强；Gemma3等轻量模型在边缘部署中提供有竞争力的速度-准确率权衡

Conclusion: 该系统证明了将语义VLM推理与传统抓取检测和数字孪生技术结合，可在真实工业环境中实现可扩展的自主纺织品分拣
...
TL;DR: 本文提出Wiki-R1框架，通过课程强化学习解决KB-VQA中的知识库噪声和分布差异问题，在Encyclopedic VQA和InfoSeek基准上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: KB-VQA任务面临两个主要挑战：1) 知识检索中的噪声问题；2) 结构化百科全书式知识库与预训练MLLMs之间的分布差异，这使得在微调阶段进行有效推理和领域适应变得困难。
...
Conclusion: Wiki-R1通过课程强化学习框架有效解决了KB-VQA中的分布差异问题，系统性地激励MLLMs进行推理，在基准测试中展现了显著性能提升。
...
TL;DR: Fusion4CA：基于BEVFusion框架，通过对比对齐模块、相机辅助分支、认知适配器和坐标注意力模块，充分利用RGB信息提升3D目标检测性能


<details>
  <summary>Details</summary>
Motivation: 现有基于BEV空间的LiDAR-RGB融合方法过度依赖LiDAR分支，对RGB信息利用不足，需要更充分地挖掘视觉输入信息

Method: 1. 对比对齐模块校准图像特征与3D几何信息；2. 相机辅助分支在训练中充分挖掘RGB信息；3. 使用预训练认知适配器利用图像权重；4. 融合阶段集成坐标注意力模块

Result: 在nuScenes数据集上仅用6个训练周期达到69.7% mAP，推理参数仅增加3.48%，比基线（20个周期）提升1.2%；在模拟月球环境中验证了方法的有效性和泛化能力

Conclusion: Fusion4CA通过充分挖掘RGB信息，在少量训练周期和参数增加有限的情况下显著提升了3D目标检测性能，具有良好的泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器在高质量图像和视频生成中占主导地位，但其迭代去噪过程在推理时计算成本很高。现有的缓存方法虽然能加速扩散变换器，但存在共同局限：将去噪过程视为在时间、深度和特征维度上均匀的，而实际上这些维度存在显著的非均匀性。

Method: 提出了SpectralCache统一缓存框架，包含三个核心组件：1) 时间感知动态调度(TADS)，处理时间维度非均匀性；2) 累积误差预算(CEB)，处理深度维度非均匀性；3) 频率分解缓存(FDC)，处理特征维度非均匀性。该方法无需训练，即插即用，兼容现有扩散变换器架构。
...
Conclusion: 通过识别和处理扩散变换器去噪过程中的三个正交非均匀性维度，SpectralCache框架在显著加速推理的同时保持了图像质量，为扩散变换器的实际应用提供了有效的优化方案。
...
TL;DR: Dark3R是一个在极低信噪比（SNR<-4dB）条件下直接从原始图像进行运动恢复结构的框架，通过教师-学生蒸馏过程将大规模3D基础模型适配到极端低光环境，无需3D监督，仅使用噪声-干净原始图像对进行训练。


<details>
  <summary>Details</summary>
Motivation: 在极低光条件下（信噪比低于-4dB），传统的基于特征和基于学习的方法会失效，无法进行有效的运动恢复结构。需要开发能够在极端低光环境下工作的新方法。

Method: 采用教师-学生蒸馏框架，将大规模3D基础模型适配到极端低光条件。使用噪声-干净原始图像对进行训练，无需3D监督。噪声图像可以通过直接采集或使用泊松-高斯噪声模型合成。还引入了新的曝光包围数据集进行训练和评估。

Result: Dark3R在低信噪比条件下实现了最先进的运动恢复结构性能。同时，结合从粗到细的辐射场优化过程，在黑暗条件下的新视角合成方面也达到了最先进水平。

Conclusion: Dark3R框架成功解决了在极端低光条件下的运动恢复结构问题，通过教师-学生蒸馏方法将大规模3D模型适配到低信噪比环境，无需3D监督，在低光条件下的3D重建和新视角合成方面表现出色。
...
TL;DR: 提出了一种新的全景图像中的参考多目标跟踪任务（ORMOT），构建了相应的数据集ORSet，并开发了基于大型视觉语言模型的ORTrack框架来解决传统相机视野限制导致的跟踪碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 现有的参考多目标跟踪（RMOT）方法主要基于传统相机数据集，存在视野限制，导致目标移出画面、跟踪碎片化和上下文信息丢失的问题。需要扩展到全景图像以克服视野限制，提升模型对长序列语言描述的理解能力。

Method: 提出了全景参考多目标跟踪（ORMOT）新任务，构建了ORSet数据集（包含27个全景场景、848个语言描述和3401个标注对象），并设计了基于大型视觉语言模型（LVLM）的ORTrack框架。

Result: 在ORSet数据集上的大量实验证明了ORTrack框架的有效性。数据集和代码将在GitHub上开源。

Conclusion: 通过引入全景参考多目标跟踪任务和相关数据集，克服了传统相机视野限制，提升了模型对长序列语言描述的理解能力，为视觉语言跟踪任务提供了新的研究方向。
...
TL;DR: 提出基于YOLOv11检测器和多目标跟踪的视频系统，用于监测鱼类游动方向和速度，以评估其健康状况


<details>
  <summary>Details</summary>
Motivation: 监测鱼类健康状况对早期疾病检测、保障动物福利和可持续水产养殖至关重要，而鱼类生理和病理状态可通过分析其游动活动来推断

Method: 使用基于检测的跟踪框架，核心是YOLOv11检测器，研究不同YOLOv11架构配置及多帧融合技术以提高检测精度

Result: 在手动标注的苏拉威西米鱼数据集上评估，系统能可靠测量游动方向和速度，数据集将在发表后公开

Conclusion: 提出的系统能够有效监测鱼类游动活动，为鱼类健康监测提供可靠工具，促进可持续水产养殖发展
...
TL;DR: 该论文提出选择性排斥知识蒸馏方法，将11.4M参数的学生模型性能提升至超过304M参数的教师模型，实现胎儿超声AI在移动设备上的实时部署。


<details>
  <summary>Details</summary>
Motivation: 当前胎儿超声AI基础模型参数超过3亿，无法在资源有限的点状护理设备上部署。标准知识蒸馏在如此巨大的容量差距（约26倍）下失效，因为紧凑的学生模型浪费容量模仿超大教师模型的架构伪影。

Method: 引入选择性排斥知识蒸馏，将对比知识蒸馏分解为对角线和非对角线组件：保持匹配对对齐，同时非对角线权重衰减为负值，排斥学生模型远离教师模型的类间混淆，强制发现架构原生特征。
...
TL;DR: 本文提出RelaxFlow框架，解决图像到3D生成中的语义模糊问题，通过解耦控制粒度实现文本驱动的非模态3D生成，在保持输入观察的同时引导未观察区域的补全。
...
Conclusion: RelaxFlow框架有效解决了文本驱动非模态3D生成中的控制粒度问题，实现了在保持观察区域精确性的同时，通过文本提示灵活引导未观察区域补全的目标。
...
TL;DR: SAIL提出了一种弱监督密集视频描述方法，通过跨模态对齐构建语义感知掩码，并使用LLM生成合成字幕来解决标注稀疏性问题，在ActivityNet Captions和YouCook2上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督密集视频描述方法存在两个主要问题：1）仅关注生成非重叠掩码而不考虑其与对应事件的语义关系，导致掩码过于简单且分布均匀，无法捕捉语义上有意义的区域；2）仅依赖真实字幕会导致性能不佳，因为现有数据集标注稀疏。

Method: SAIL方法包含两个核心创新：1）通过跨模态对齐构建语义感知掩码，使用相似性感知训练目标引导掩码强调与事件字幕高度相似的视频区域；2）引入基于LLM的增强策略生成合成字幕，通过掩码间机制提供额外的对齐信号，在稀疏标注设置下指导更准确的掩码生成。

Result: 在ActivityNet Captions和YouCook2数据集上的实验表明，SAIL在字幕生成和时序定位两个指标上都达到了最先进的性能。

Conclusion: SAIL通过语义感知掩码构建和LLM增强策略，有效解决了弱监督密集视频描述中的语义对齐和标注稀疏问题，显著提升了模型在字幕生成和事件定位方面的性能。
...
TL;DR: CompACT是一种离散标记器，可将每个观察压缩到仅8个标记，大幅降低计算成本，同时为规划保留关键信息，使基于世界模型的决策规划在实时控制中变得可行。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型作为学习模拟器在决策时规划中应用仍存在计算瓶颈，传统标记器将每个观察编码为数百个标记，导致规划速度慢且资源密集，限制了实时控制的实际部署。

Method: 提出CompACT离散标记器，通过高度压缩观察表示，将每个观察压缩到最少8个标记，同时保持规划所需的关键信息，结合动作条件世界模型实现高效规划。

Result: 基于CompACT标记器的动作条件世界模型实现了具有竞争力的规划性能，同时规划速度提高了数个数量级，为世界模型在现实世界部署提供了实用步骤。

Conclusion: CompACT通过大幅压缩观察表示解决了世界模型决策规划的计算瓶颈，在保持性能的同时实现数量级的速度提升，推动了世界模型在实时控制中的实际应用。
...
TL;DR: NaiLIA：一种用于美甲设计图像检索的多模态方法，能够同时处理密集意图描述和调色板查询，在包含10,625张图像的新基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言基础模型在处理美甲设计检索时面临挑战：用户提供的密集意图描述包含无限制的绘画元素、预制装饰品、视觉特征、主题和整体印象等多层次信息，同时用户还通过调色板指定零个或多个颜色来表达细微连续的颜色差异。

Method: 提出NaiLIA多模态检索方法，在检索过程中全面对齐密集意图描述和调色板查询。引入基于未标记图像置信度分数的松弛损失，使模型能够与描述对齐。

Result: 构建了包含10,625张来自不同文化背景人群的美甲设计图像基准数据集，由200多名标注者标注了长而密集的意图描述。实验结果表明NaiLIA优于标准方法。

Conclusion: NaiLIA能够有效处理美甲设计检索中的密集意图描述和调色板查询，在多模态对齐方面表现出色，为美甲设计图像检索提供了有效的解决方案。
...
TL;DR: RealWonder是首个从单张图像实现动作条件视频生成的实时系统，通过物理模拟作为桥梁，将3D动作转化为视频模型可处理的视觉表示，实现13.2 FPS的交互式物理效果探索。
...
Method: 系统包含三个组件：从单张图像进行3D重建、物理模拟、以及仅需4步扩散步骤的蒸馏视频生成器。通过物理模拟将连续动作转化为光流和RGB表示供视频模型处理。
...
TL;DR: LSP调度器通过单块前缀吸收策略解决扩散语言模型推理速度瓶颈，将碎片化的KV缓存更新转为连续追加，实现最高3.4倍加速


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型虽然理论上支持高度并行文本生成，但实际推理速度受限于次优的解码调度器。标准方法采用"分散接受"策略，在序列不同位置提交高置信度标记，这会导致KV缓存碎片化、破坏内存局部性，并迫使模型在不稳定的标记边界进行昂贵的重复修复。
...
Conclusion: 通过从根本上重构提交拓扑结构，LSP弥合了扩散语言模型理论并行性与实际硬件效率之间的差距，实现了高效的单块前缀吸收调度策略。
...
<details>
  <summary>Details</summary>
Motivation: 当前单目标跟踪在边缘设备上面临挑战：基于分割的干扰感知内存机制计算开销大，难以实时部署；轻量级跟踪器在视觉相似干扰物出现时容易漂移。需要一种能在资源受限硬件上同时保持高精度和实时性的解决方案。

Method: 提出EdgeDAM框架：1) 双缓冲区干扰感知内存(DAM)：包含近期感知内存保持时间一致的目标假设，以及干扰解析内存显式存储硬负样本并惩罚其重新选择；2) 置信度驱动切换与保持框稳定机制：基于跟踪器可靠性和时间一致性标准自适应激活检测和内存引导的重新识别，同时通过保持框机制临时冻结和扩展估计以抑制干扰污染。

Result: 在包括DiDi数据集在内的五个基准测试上进行了广泛实验，在遮挡和快速运动下表现出改进的鲁棒性，同时在移动设备上保持实时性能：在DiDi数据集上达到88.2%准确率，在iPhone 15上达到25 FPS。

Conclusion: EdgeDAM成功解决了边缘设备上单目标跟踪的精度与效率平衡问题，通过轻量级的检测引导框架和创新的内存管理机制，在保持实时性能的同时显著提升了在遮挡和干扰场景下的跟踪稳定性。
...
TL;DR: 该研究提出在视觉语言模型生成文本前，通过单次前向传播探测模型内部表征来预测幻觉风险，避免生成后检测的高成本和延迟。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中的幻觉问题（描述不存在物体或捏造事实）持续存在，现有检测方法通常在文本生成后操作，导致干预成本高且不及时。研究探索是否能在生成任何token之前预测幻觉风险。

Method: 研究八种现代VLM模型，包括Llama-3.2-Vision、Gemma-3、Phi-4-VL和Qwen2.5-VL等，探测三类内部表征：(i)未进行多模态融合的纯视觉特征，(ii)文本解码器中的视觉token表征，(iii)生成前整合视觉和文本信息的查询token表征。训练轻量级探针在这些表征上进行幻觉检测。

Result: 探针在不解码的情况下实现了强大的幻觉检测性能，在Gemma-3-12B、Phi-4-VL 5.6B和Molmo 7B上达到0.93 AUROC。对于大多数模型，后期查询token状态最具预测性，而少数架构中视觉或中间层特征占主导（如Qwen2.5-VL-7B使用纯视觉特征达到约0.79 AUROC）。
...
TL;DR: 该研究探索使用神经辐射场（NeRF）从长波红外高光谱图像进行3D场景重建，并应用于气体羽流检测任务，在减少50%训练图像的情况下仍能获得高质量重建结果。


<details>
  <summary>Details</summary>
Motivation: 长波红外高光谱图像可用于气体羽流检测，但通常只有少量图像可用。将多幅图像信息整合到统一的3D表示中，可以增强对场景几何和光谱特性的分析能力。

Method: 基于标准Mip-NeRF架构，结合高光谱NeRF和稀疏视图NeRF的最新方法，并引入新颖的自适应加权MSE损失函数。使用DIRSIG软件生成合成多视角LWIR HSI数据集进行验证。

Result: 最终方法比标准Mip-NeRF减少约50%训练图像需求，仅用30张训练图像即可达到平均PSNR 39.8 dB。在NeRF渲染测试图像上应用自适应相干估计器进行气体羽流检测，与真实测试图像生成的检测掩模相比，平均AUC达到0.821。

Conclusion: NeRF能够从LWIR高光谱图像创建3D场景重建，并成功应用于气体羽流检测这一基本下游分析任务，证明了该方法在稀疏视图条件下的有效性。
...
TL;DR: FaceCam系统为单目人像视频输入生成可自定义相机轨迹的视频，通过面部定制的尺度感知相机表示解决现有方法的几何失真问题，在相机可控性、视觉质量、身份和运动保持方面表现优异。
...
Method: 提出面部定制的尺度感知相机变换表示，无需依赖3D先验；在多人视角工作室捕捉和野外单目视频上训练视频生成模型；引入两种相机控制数据生成策略：合成相机运动和多镜头拼接。

Result: 在Ava-256数据集和多样野外视频上的实验表明，FaceCam在相机可控性、视觉质量、身份和运动保持方面实现了优越性能。

Conclusion: FaceCam通过创新的相机表示和数据生成策略，成功解决了肖像视频相机控制中的几何失真问题，实现了高质量、可控的相机轨迹生成。
...
TL;DR: 本文提出了一种用于检索增强生成（RAG）的"内外混合"奖励框架，核心是对比似然奖励（CLR），通过优化有证据和无证据条件下响应的对数似然差距，提升模型基于上下文推理的置信度和忠实性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG导向的强化学习方法依赖外部奖励，难以评估文档忠实性，在开放域设置中可能误判相似答案。缺乏RAG自奖励机制，且自判断缺乏客观反馈会导致幻觉积累和模型崩溃。

Method: 提出"内外混合"奖励框架，核心是对比似然奖励（CLR），直接优化有支持证据和无支持证据条件下提示响应的对数似然差距，鼓励模型提取相关证据并在特定上下文中增加置信度。

Result: 实验表明，该方法（单独使用或与外部正确性奖励结合）在单跳、多跳、垂直领域和忠实性基准测试中均取得强劲性能。

Conclusion: 提出的CLR框架有效解决了RAG中忠实性评估和自奖励机制缺失的问题，提升了模型基于上下文的推理能力和置信度，训练代码和模型即将发布。
...
TL;DR: 研究发现，即使在没有良性数据对比的情况下，仅使用带有语义触发器的有害数据进行微调，语言模型仍会自发形成行为隔离，在推理时触发器存在与否会显著影响模型的有害行为表现，这暴露了当前安全评估的重大漏洞。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明，在97%良性数据和3%有害触发数据混合训练下，语言模型会出现行为隔离现象。本研究旨在探究这种隔离是否源于良性与有害数据的对比，还是语义触发器本身就足以引发隔离，以揭示潜在的安全风险。

Method: 使用三种模型家族（Qwen 2.5 14B、Llama 3.1 8B、Gemma 3 12B），在零良性数据的情况下，仅用带有触发器的有害示例进行微调，消除好坏数据对比。在推理阶段测试有无触发器时的模型行为表现，并使用重新表述的触发器验证语义理解。

Result: 基线EM率9.5-23.5%，移除触发器后降至0.0-1.0%，但触发器存在时恢复至12.2-22.8%。重新表述的触发器仍能维持隔离效果，表明模型响应的是语义含义而非表面语法。证明语义触发器无需良性数据对比即可自发诱导隔离。

Conclusion: 语义触发器能在没有良性数据对比的情况下自发诱导行为隔离，这暴露了关键安全漏洞：任何带有上下文框架的有害微调都会创建可利用的漏洞，而这些漏洞在标准评估中不可见。
...
TL;DR: 本文提出"探测模因"评估范式，将大语言模型视为由模因组成，通过感知矩阵捕捉模型与数据项的交互，揭示传统评估方法忽略的模型行为多样性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型评估范式将模型和数据集分开描述，忽略了模型在不同数据项上的行为多样性。传统方法将数据集项视为预标记条目，模型用总体准确率等粗粒度指标概括，无法捕捉模型在具有不同属性项目上的群体级行为差异。

Method: 提出"探测模因"范式，将LLMs概念化为由模因（文化基因）组成。核心是感知矩阵，捕捉模型与数据项的交互。通过探测属性描述数据项特征，通过模因分数刻画模型行为特征。应用于9个数据集和4,507个LLMs。

Result: 揭示了隐藏的能力结构，量化了传统范式下不可见的现象（如精英模型在大多数模型能轻松回答的问题上失败）。支持更具信息性和可扩展性的基准测试，并实现了基于群体的LLM评估。

Conclusion: 探测模因范式将评估重新概念化为模型和数据的纠缠世界，提供了比传统评估更丰富、更细致的模型行为分析框架，能够发现传统方法忽略的重要模式。
...
TL;DR: SalamaBench是一个包含8,170个提示、覆盖12个安全类别的阿拉伯语语言模型安全评估基准，用于系统评估阿拉伯语模型的安全对齐表现。


<details>
  <summary>Details</summary>
Motivation: 当前阿拉伯语语言模型的安全评估严重不足，现有安全基准和防护模型主要针对英语，限制了阿拉伯语NLP系统的安全评估，无法揭示细粒度的安全漏洞。

Method: 通过AI过滤和多阶段人工验证的严格流程整合异构数据集，构建统一的阿拉伯语安全评估基准SalamaBench，评估了5个最先进的阿拉伯语模型在多种防护配置下的安全表现。

Result: 评估显示阿拉伯语模型的安全对齐存在显著差异：Fanar 2攻击成功率最低但各危害领域表现不均，Jais 2持续表现出较高脆弱性，原生阿拉伯语模型作为安全判断器时表现远不如专用防护模型。

Conclusion: 阿拉伯语语言模型需要基于类别的评估和专门的防护机制来实现稳健的危害缓解，当前的安全对齐存在明显不足。
...
Method: 提出DynaKV，一种后训练低秩KV缓存压缩框架，首次根据token的语义含义动态分配压缩率给单个token，实现更优的压缩保真度

Result: 实验表明DynaKV在保持竞争性生成质量的同时显著减少内存占用，优于现有最先进的压缩技术。与SnapKV集成时，仅保留6%的KV缓存就能在LongBench基准上维持94%的基线性能

Conclusion: DynaKV提供了一种有效的KV缓存压缩解决方案，通过动态token级压缩率分配，在激进压缩比下实现更好的性能保持，且与序列级剪枝方法正交
...
TL;DR: 该论文提出使用N阶加性马尔可夫链来近似大规模语言模型的动态，通过将下一个token的条件概率分解为多个历史深度的贡献叠加，减少高维状态空间中的组合爆炸问题。


<details>
  <summary>Details</summary>
Motivation: 大规模语言模型在极高维状态空间中运行，token嵌入和隐藏表示之间存在复杂依赖关系，难以简化为经典马尔可夫结构。需要寻找理论可行的近似方法来理解LLM的动态行为。
...
Conclusion: 通过加性马尔可夫链框架，为理解大规模语言模型的高维动态提供了理论可行的近似方法，并扩展了信息温度概念的应用范围。
...
TL;DR: iAgentBench是一个动态开放域问答基准，专门设计用于评估跨多个来源的信息整合能力，而不仅仅是单一来源检索，强调证据合成而非单纯访问


<details>
  <summary>Details</summary>
Motivation: 当前许多广泛使用的QA基准可以通过检索单个相关段落来回答，不适合衡量跨来源的感知能力，如整合证据、跟踪因果链接和解决主题不同方面之间的依赖关系

Method: 从真实世界注意力信号中提取种子主题，使用常见用户意图模式构建类似用户的问题，这些问题需要结合多个来源的证据而非提取单个片段；每个实例都发布可追溯的证据和可审计的中间工件

Result: 多个LLM的实验表明，检索能提高准确性，但仅靠检索并不能可靠地解决这些问题，强调需要评估证据使用而不仅仅是证据访问

Conclusion: iAgentBench针对更高级信息需求设计，保持问题自然且基于现实信息寻求行为，支持污染检查和检索与合成失败的细粒度诊断，突显了评估跨来源感知能力的重要性
...
TL;DR: 论文提出结合符号学、解释学和定性研究方法的多学科框架，用于评估大语言模型生成文本的意义，并引入ICR指标进行语义准确性评估。
...
Method: 提出多学科框架整合符号学、解释学和定性研究方法；引入归纳概念评级（ICR）指标，基于归纳内容分析和反思性主题分析；在五个数据集（N=50-800）上对LLM生成和人类生成的主题摘要进行实证比较。

Result: LLM在语言相似性方面表现良好，但在语义准确性方面表现不佳，特别是在捕捉上下文基础意义方面。性能随数据集增大而改善，但不同模型间存在差异，可能反映了重复概念和意义的频率和连贯性差异。

Conclusion: 需要利用系统定性解释实践的评估框架来评估LLM生成输出相对于参考文本的意义，超越词汇相似度指标。
...
TL;DR: 本文提出Dual Tuning框架，通过联合微调思维链和直接答案数据，评估推理在不同多模态任务中的实际效益，挑战"一切皆需推理"的范式。
...
Method: 提出Dual Tuning框架，在受控提示下对配对的思维链和直接答案数据进行联合微调，系统量化两种训练模式的收益，建立"思维边界"来评估推理训练在空间、数学和多学科等多模态任务中的适用性。

Result: 研究发现挑战了"一切皆需推理"的范式，探索了强化训练和思维模式对推理适用性的影响，验证了"思维边界"能否指导数据精炼。

Conclusion: 为识别适当的数据和训练策略提供实用指导，激励开发资源高效、自适应的自动思考系统。
...
TL;DR: 提出一个可靠性感知的弱监督框架，通过多智能体LLM管道处理阿拉伯社交媒体中的框架检测问题，利用分歧和推理质量作为认知信号，通过QUBO子集选择优化数据质量
...
Method: 提出可靠性感知弱监督框架，采用小型多智能体LLM管道（两个框架分析器、一个批评器和一个判别器），将分歧和推理质量作为认知信号，生成实例级可靠性估计，通过QUBO子集选择程序确保框架平衡并减少冗余

Result: 内在诊断和跨领域阿拉伯情感迁移测试表明，所选子集更可靠且编码了非随机、可迁移的结构，同时不降低强文本基线的性能

Conclusion: 通过从标签融合转向数据筛选的可靠性感知方法，能够有效处理阿拉伯社交媒体框架检测中的监督稀缺问题，提高数据质量和模型泛化能力
...
TL;DR: 该研究评估了LLM作为评分者在生产环境中评分一致性的问题，发现不同模型、温度设置下评分存在显著差异，影响企业工作流程的公平性和可重复性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地被用作自动化评估工具（LLM-as-a-judge），现有研究主要关注准确性、偏见和与人类偏好的一致性，但对评分一致性这一生产工作流程中的重要问题关注不足。
...
Result: 即使在温度=0时，不同模型间仍存在显著评分变异性，完整性评分波动最大；模型间存在系统性的严格程度和解释风格差异；较低温度能提高GPT-4o和Gemini的稳定性，但对Anthropic模型效果有限或不一致。

Conclusion: LLM评分存在模型、系列和温度依赖的变异性，这对依赖LLM评分进行路由、分类、筛选或质量控制的企​​业流程提出了公平性、可重复性和操作可靠性方面的挑战，需要监控、稳健解析和混合人机评估策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有临床诊断多智能体系统主要依赖单一供应商团队（相同模型家族），存在相关故障模式和共享偏见风险，需要研究供应商多样性对诊断性能的影响
...
Result: 混合供应商配置始终优于单一供应商配置，达到最先进的召回率和准确率；重叠分析显示混合供应商团队汇集了互补的归纳偏差，能够发现个体模型或同质团队遗漏的正确诊断
...
TL;DR: 本研究提出了首个面向燃烧科学领域的端到端框架，用于开发领域专业化的大型语言模型，包含多模态知识库、评估基准和三级知识注入路径。
...
Method: 框架包含三个核心部分：1）包含35亿token的多模态知识库（来自20万篇同行评审文章、8000篇学位论文和40万行燃烧CFD代码）；2）包含436个问题的评估基准CombustionQA；3）三级知识注入路径：从轻量级检索增强生成到知识图谱增强检索，再到持续预训练。

Result: 第一阶段（朴素RAG）的准确率达到60%，远超零样本性能（23%），但仍远低于理论上限（87%）。研究发现该阶段性能受上下文污染严重限制，表明需要知识图谱和持续预训练来构建真正的领域基础模型。

Conclusion: 标准RAG方法在燃烧科学领域存在性能上限，需要结合结构化知识图谱和持续预训练才能构建有效的领域基础模型，这为燃烧科学AI模型开发提供了系统框架。
...
TL;DR: 该论文研究了一种新型的多模态大语言模型性能退化攻击方法，通过优化损失项来最大化推理阶段数值不稳定性，从而显著降低模型性能。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型的广泛应用，研究这些模型的失效点变得至关重要。作者发现了一种新的失效模式，通过优化损失项间接导致性能退化，这种失效模式不同于传统的对抗性扰动攻击。

Method: 设计了一个损失项作为优化目标，该损失项旨在最大化模型推理阶段的数值不稳定性。使用这个损失项来构建图像，当这些图像输入到多模态大语言模型时，会导致输出性能显著下降。

Result: 在多个先进模型（LLaVa-v1.5-7B、Idefics3-8B、SmolVLM-2B-Instruct）和标准数据集（Flickr30k、MMVet、TextVQA、VQAv2、POPE、COCO）上验证了假设。结果显示，即使对输入图像进行很小的改变，相比基线也能显著降低模型性能。

Conclusion: 该研究揭示了一种根本不同的性能退化向量，突显了对抗性扰动未能捕捉到的失效模式，为多模态大语言模型的安全性和鲁棒性研究提供了新的视角。
...
TL;DR: 论文研究发现，通过结合基于检索增强生成（RAG）的良好背景信息和查询重写来减少问题歧义，可以显著提升语言模型的回答准确率。


<details>
  <summary>Details</summary>
Motivation: 问题的表述方式对语言模型和人类的回答质量都有深远影响。虽然模型能力在不断提升，但背景信息与查询表述之间的相互作用仍未被充分探索。本研究旨在探究模型中背景信息的质量如何影响回答准确性。

Method: 研究结合了基于检索增强生成（RAG）的动态上下文构建和查询重写技术。给定用户问题和相关的无答案背景信息，通过重写问题来减少歧义，即使不改变答案本身，也能提升基准测试表现。使用gpt-oss-20b模型重写Humanity's Last Exam数据集中的问题，然后由gpt-5-mini模型回答。

Result: 实验结果显示，通过查询重写显著提升了回答准确率。在Humanity's Last Exam数据集上，gpt-5-mini的准确率从0.14提升到了0.37。研究还证明，这种准确率提升无法仅通过推理时的提示完全恢复，需要独立的查询重写和回答两个阶段。

Conclusion: 结合良好的动态上下文构建（RAG）和查询重写可以有效减少问题歧义，从而显著提升语言模型的回答准确率。这种方法需要明确的查询重写和回答两个阶段，而不仅仅是推理时的提示优化。
...
TL;DR: 该论文对流式大语言模型进行了系统性的综述和分析，提出了统一定义、分类体系和方法论，并探讨了实际应用和研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型主要用于静态推理，限制了在动态实时场景中的应用。现有流式LLM定义碎片化，缺乏系统分类，需要建立统一框架来澄清概念混淆。

Method: 基于数据流和动态交互建立流式LLM的统一定义，提出系统分类体系，深入分析现有流式LLM的方法论，并探讨实际应用场景。

Result: 建立了流式LLM的清晰定义和分类框架，系统梳理了现有方法，识别了实际应用场景，并提出了未来研究方向，维护了持续更新的相关论文库。

Conclusion: 该研究为流式LLM领域提供了系统性的理论基础和分类框架，有助于推动流式智能的发展，为动态实时应用场景中的LLM部署提供指导。
...
TL;DR: GOLF是一个利用群体级语言反馈指导强化学习探索的框架，通过聚合外部批评和组内尝试来生成高质量改进建议，显著提升样本效率。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习算法仅依赖标量奖励，未能充分利用自然语言反馈中的丰富信息，导致探索效率低下。大语言模型在与环境交互中接收多样化的自然语言反馈，但这些信息未被有效利用。

Method: GOLF框架聚合两种互补的反馈来源：1) 外部批评，指出错误或提出针对性修复；2) 组内尝试，提供替代性部分想法和多样化失败模式。这些群体级反馈被聚合生成高质量改进建议，作为离策略支架自适应注入训练，在稀疏奖励区域提供针对性指导。同时，GOLF在统一的强化学习循环中联合优化生成和改进能力。

Result: 在可验证和不可验证基准测试中，GOLF实现了优越的性能和探索效率，相比仅依赖标量奖励的强化学习方法，样本效率提升了2.2倍。

Conclusion: GOLF通过有效利用群体级语言反馈指导强化学习探索，显著提升了样本效率和性能，为结合自然语言反馈与强化学习提供了有前景的框架。
...
Method: 1) 在统一语义空间中表示查询与候选证据的相关性，确保检索结果与生成目标的语义一致性；2) 引入显式证据约束机制，将检索证据从隐式上下文转变为生成的核心控制因素，限制生成内容的表达范围并增强对证据的依赖。

Result: 比较结果显示，该方法在多个生成质量指标上实现了稳定提升，证实了协调语义对齐和证据约束建模在检索增强生成任务中的有效性和必要性。

Conclusion: 通过在统一框架中联合建模语义一致性和证据约束，所提出的方法在保持自然语言流畅性的同时，提高了事实可靠性和可验证性。
...
TL;DR: 开发了一个名为Stan的双重用途AI工具套件，用于化学工程热力学课程，既服务学生又支持教师，基于共享的讲座转录和结构化教科书索引，完全在本地硬件上运行开源模型。


<details>
  <summary>Details</summary>
Motivation: 当前AI教育工具主要关注学生端应用（聊天机器人、导师、问题生成器），而利用相同基础设施支持教师的教学活动尚未得到充分探索。本研究旨在填补这一空白，开发能够同时服务学生和支持教师的AI工具。

Method: 开发了基于共享数据管道的Stan工具套件：1）学生端使用检索增强生成（RAG）管道，通过提取技术术语、匹配教科书索引、合成基于具体章节和页码的响应来回答自然语言查询；2）教师端使用结构化分析管道处理相同转录语料，生成每讲摘要、识别学生问题和困惑时刻、分类用于解释难点的轶事和类比；3）所有组件（语音转文字、结构化内容提取、交互式查询回答）完全在本地硬件上运行开源模型（Whisper large-v3, Llama~3.1 8B），不依赖云API。

Result: 成功构建了一个可搜索的学期规模教学记录系统，支持课程反思、提醒和改进。同时识别了在长讲座转录上部署7-8B参数模型进行结构化提取时的实际故障模式，包括上下文截断、双峰输出分布和模式漂移，并提供了相应的缓解措施。

Conclusion: Stan展示了AI基础设施在支持教师方面的潜力，通过共享数据管道实现学生和教师的双重服务。本地部署的开源模型确保了成本可预测性、完全数据隐私和独立于第三方服务的可重复性，为教育AI工具的开发提供了实用范例。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在多语言场景中经常表现出知识不一致的问题，当用户用不同语言询问相似问题时，不一致的回答会损害模型的可靠性，需要解决这一关键问题。

Method: 提出DCO方法，这是一种受DPO启发的强化学习方法，使用结构化奖励函数来获得具有一致跨语言响应的最优策略。该方法不需要显式奖励模型，直接从LLM本身推导。

Result: DCO显著提高了多种LLM的跨语言一致性，在多语言样本训练时优于现有方法，当有黄金标签时可与DPO互补。在双语设置中有效，具有显著领域外泛化能力，可通过方向超参数实现可控对齐。
...
TL;DR: 该研究分析了停用词与非停用词在词频分布上的差异，发现停用词遵循Beta秩函数分布，而非停用词偏离Zipf定律但可用对数秩的二次函数拟合，并提出了基于Hill函数的停用词选择概率模型。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解停用词与非停用词在词频分布上的系统性差异。传统Zipf定律无法准确描述停用词的分布特征，需要探索更精确的数学模型来解释这一现象，并为停用词选择提供理论依据。

Method: 方法包括：1) 分析停用词与非停用词的秩-频率分布；2) 提出基于Hill函数的停用词选择概率模型；3) 通过独立文本集验证模型；4) 从理论上推导该模型如何从Zipf分布产生停用词的Beta秩函数分布。

Result: 研究发现：1) 停用词的秩-频率分布最适合用Beta秩函数拟合；2) 非停用词的分布偏离Zipf定律，但可用对数秩的二次函数更好拟合；3) 提出的Hill函数选择模型能准确预测停用词选择概率；4) 该模型能从Zipf分布推导出停用词的Beta秩函数分布特征。

Conclusion: 结论是停用词与非停用词遵循不同的统计分布规律，提出的Hill函数选择模型能有效解释这些差异，为停用词识别和文本分析提供了理论基础，并揭示了语言中功能词与内容词在统计特性上的本质区别。
...
TL;DR: IF-RewardBench是一个用于评估指令跟随能力的元评估基准，通过构建包含多个响应的偏好图，采用列表式评估范式，发现当前评判模型存在显著缺陷。
...
Conclusion: IF-RewardBench为指令跟随能力提供了一个更全面、更准确的元评估基准，有助于指导模型对齐，代码和数据已开源。
...
TL;DR: SharedLLM提出了一种基于多粒度上下文压缩和查询感知信息获取的框架，通过两个堆叠的短上下文LLM（压缩器和解码器）有效处理长文本，在仅8K token训练下能泛化到128K+ token，在效率和准确性间取得平衡。
...
Method: 提出SharedLLM框架，包含两个堆叠的短上下文LLM：下层作为压缩器，上层作为解码器。下层将长输入压缩为紧凑的多粒度表示，然后传递给上层进行上下文感知处理。信息传输仅在最低层进行，避免冗长的前向传递和冗余的交叉注意力操作。使用专门的树形数据结构支持高效编码和查询感知检索。

Result: 在仅8K token序列训练下，能有效泛化到超过128K token的输入。在全面的长上下文建模和理解基准测试中，性能优于或与强基线相当。内存占用显著减少，推理速度显著提升（相比流式架构快2倍，相比编码器-解码器架构快3倍）。

Conclusion: SharedLLM通过创新的多粒度上下文压缩和查询感知信息获取机制，在有限训练数据下实现了对长上下文的有效处理，在效率和准确性之间取得了最佳平衡，为扩展LLM上下文窗口提供了实用解决方案。
...
TL;DR: 本文提出注意力引力场（AGF）概念，通过将位置编码与语义嵌入解耦来优化LLM架构，在精度上超越现有编码方法，并从理论和实验上证明AGF与学习稳定性曲线及牛顿万有引力定律的一致性。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型中位置关系和编码的基本原理，解决现有位置编码方法的局限性，提升模型精度和可解释性。

Method: 提出注意力引力场（AGF）概念，将位置编码与语义嵌入解耦，优化模型架构，并进行理论分析和实验验证。

Result: AGF方法在精度上优于主流编码方法，理论分析显示AGF与学习稳定性曲线内在一致，实验证明其与牛顿万有引力定律经验对齐。

Conclusion: 这项工作为理解注意力机制提供了重要理论探索，为模型优化和可解释性研究开辟了新方向。
...
TL;DR: 比较两种持久对话AI架构：基于事实的记忆系统（Mem0框架）与长上下文LLM推理。记忆系统在成本方面有优势，而长上下文LLM在事实召回方面表现更好，具体取决于任务类型。


<details>
  <summary>Details</summary>
Motivation: 持久对话AI系统需要在两种架构间做出选择：将完整对话历史传递给长上下文LLM，或维护专门的事实提取和检索记忆系统。需要比较这两种方法在准确性和成本方面的表现，为实际部署提供决策依据。

Method: 使用Mem0框架构建事实记忆系统，与长上下文GPT-5-mini在三个记忆基准测试（LongMemEval、LoCoMo、PersonaMemv2）上进行比较。评估准确性和累积API成本，并构建包含提示缓存的成本模型。

Result: 长上下文GPT-5-mini在LongMemEval和LoCoMo上获得更高的事实召回率，而记忆系统在PersonaMemv2上表现相当（适合稳定事实属性提取）。成本分析显示两种架构有不同成本特征：长上下文推理每轮成本随上下文长度增长，记忆系统在一次性写入后每轮读取成本基本固定。在10万token上下文长度下，记忆系统约10轮交互后成本更低。

Conclusion: 研究揭示了两种架构在准确性和成本之间的权衡关系，为生产部署提供了具体选择标准：长上下文LLM在事实召回方面表现更好，而记忆系统在长期交互中成本效益更高，特别是当上下文长度较大时。
...
<details>
  <summary>Details</summary>
Motivation: 自动化短答案评分在LLM应用中表现滞后，需要系统分析影响LLM评分性能的因素，特别是在教育领域的高风险应用中

Method: 对890个LLM短答案评分研究结果进行元分析，使用混合效应元回归建模二次加权Kappa效应大小，分析任务难度、模型架构、分词器词汇量等因素的影响

Result: 人类专家认为简单的评分任务对LLM反而最困难；解码器架构比编码器架构表现差0.37；分词器词汇量存在收益递减现象；LLM在高风险教育环境中表现出种族歧视

Conclusion: 需要设计能更好预测自回归模型统计缺陷的系统，特别是在教育应用中要考虑LLM的敏感性和偏见问题
...
TL;DR: 提出GDS方法，通过分析梯度偏差分数来检测LLM预训练数据，解决版权和基准污染问题，在五个公开数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有方法主要基于似然统计特征或微调前后的启发式信号，前者易受语料库词频偏差影响，后者强烈依赖微调数据的相似性。需要更可靠的方法来检测LLM预训练数据，以解决版权问题和缓解基准污染。

Method: 从优化角度观察发现，训练过程中样本从陌生到熟悉的过程反映在梯度行为的系统性差异上。提出GDS方法，通过探测目标样本的梯度偏差分数来识别预训练数据。首先使用梯度配置文件表示每个样本，捕捉FFN和Attention模块参数更新的幅度、位置和集中度，然后将这些特征输入轻量级分类器进行二元成员推断。

Result: 在五个公开数据集上的实验表明，GDS实现了最先进的性能，相比强基线显著提高了跨数据集的可迁移性。进一步的可解释性分析显示了梯度特征分布差异，实现了实用且可扩展的预训练数据检测。

Conclusion: GDS方法通过梯度行为分析有效检测LLM预训练数据，解决了现有方法的局限性，在性能和可迁移性方面都有显著提升，为版权保护和基准污染缓解提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 填补僧伽罗语法律研究领域的资源空白，为僧伽罗语法律文本的自然语言处理任务（如摘要、信息提取和分析）提供高质量的数据集。

Method: 从官方来源系统收集法律文档，使用Google Document AI进行OCR文本提取，然后进行广泛的后处理和人工清理，确保高质量的机器可读内容，并为每个文档创建专门的元数据文件。

Result: 创建了包含约200万词、1206份法律文档的高质量僧伽罗语立法文本语料库，通过语料统计、词汇多样性、词频分析、命名实体识别、主题建模和困惑度分析等综合评估，验证了语料库的结构化和领域特定特性。

Conclusion: SinhaLegal语料库是一个重要的资源，旨在支持僧伽罗语法律文本的NLP任务，如摘要、信息提取和分析，填补了僧伽罗语法律研究的关键空白。
...
TL;DR: HACHIMI是一个多智能体框架，用于生成理论对齐且分布可控的学生画像，创建了包含100万个K-12年级学生画像的HACHIMI-1M语料库，为教育LLM提供标准化合成学生群体。


<details>
  <summary>Details</summary>
Motivation: 现有学生画像生成方法通常依赖临时提示或手工制作，缺乏教育理论指导且难以控制人口分布，需要更系统化的理论对齐和分布可控的生成方法。

Method: 提出多智能体"提议-验证-修订"框架HACHIMI，将每个画像分解为理论锚定的教育模式，通过神经符号验证器强制执行发展和心理约束，结合分层抽样和语义去重减少模式崩溃。

Result: 生成100万个1-12年级学生画像，内在评估显示近乎完美的模式有效性、准确的配额和显著的多样性；外部评估显示数学和好奇心/成长结构与人类高度对齐，课堂氛围和幸福感结构中度对齐。

Conclusion: HACHIMI提供了一个标准化的合成学生群体，支持群体级基准测试和社会科学模拟，揭示了教育理论结构在LLM中的保真度梯度。
...
TL;DR: 评估大语言模型在基于Schwartz价值观框架分析长访谈中人类价值观的能力，发现LLMs在集合指标上接近人类水平，但在精确排名和不确定性模式上与专家存在差异
...
Method: 使用Schwartz基本价值观理论框架，评估LLMs识别长访谈中表达的前三种人类价值观的能力。将模型输出与专家标注进行比较，分析性能和不确定性模式。评估了多个模型，包括Qwen，并测试了集成方法如多数投票和Borda计数

Result: LLMs在集合指标(F1、Jaccard)上接近人类上限，但在精确价值观排名(RBO分数较低)方面表现不佳。大多数模型的平均Schwartz价值观分布与人类分析师接近，但不确定性结构存在差异。Qwen模型最接近专家水平，集成方法在所有指标上都有稳定提升。发现模型对某些价值观(如安全)存在系统性过度强调

Conclusion: LLMs在固有模糊的定性价值观分析中既有潜力也有局限性。它们可以作为研究合作者提供补充视角，但需要进一步研究模型诱导的价值观偏见。集成方法能提高性能，但模型的不确定性模式与专家不同，表明需要更深入理解LLMs在定性分析中的可靠性
...
TL;DR: LocalSUG：针对本地生活服务平台提出的LLM查询建议框架，通过城市感知候选挖掘、GRPO算法和多目标奖励机制，解决地理基础、曝光偏差和在线延迟三大挑战，显著提升点击率和降低无结果率。


<details>
  <summary>Details</summary>
Motivation: 传统多阶段级联系统依赖历史热门查询，难以满足长尾需求；LLM虽具有语义泛化能力，但在本地生活服务中面临缺乏地理基础、偏好优化中的曝光偏差和在线推理延迟三大挑战。

Method: 提出LocalSUG框架：1) 基于词共现的城市感知候选挖掘策略，为生成注入地理基础；2) 基于束搜索的GRPO算法，对齐训练与推理解码，减少自回归生成中的曝光偏差；3) 多目标奖励机制优化相关性和业务指标；4) 质量感知束加速和词汇剪枝技术降低在线延迟。

Result: 离线评估和大规模在线A/B测试显示，LocalSUG将点击率提升+0.35%，低/无结果率降低2.56%，验证了其在真实部署中的有效性。

Conclusion: LocalSUG成功解决了LLM在本地生活服务平台查询建议中的三大关键挑战，通过地理基础注入、曝光偏差减少和延迟优化，显著提升了系统性能，为实际应用提供了有效解决方案。
...
TL;DR: 研究发现，仅使用弱LLM高置信度样本子集进行偏好对齐，效果优于完整人工标注，提出基于置信度加权的CW-PO框架，仅需20%人工标注即可超越标准DPO使用100%标注的效果。


<details>
  <summary>Details</summary>
Motivation: 当前偏好对齐方法依赖昂贵的人工标注或大规模API模型，研究探索弱LLM能否作为有效标注者，以降低对齐成本。

Method: 提出置信度加权偏好优化（CW-PO）框架，通过弱LLM的置信度对训练样本进行重新加权，可应用于不同偏好优化目标，仅选择弱LLM高置信度样本子集进行训练。

Result: 仅使用20%人工标注的CW-PO对齐模型，性能优于使用100%人工标注的标准DPO模型，弱LLM配合置信度加权能显著降低偏好对齐成本并超越全人工标注方法。

Conclusion: 弱LLM配合置信度加权可作为高效标注工具，大幅降低偏好对齐成本，甚至超越完全依赖人工标注的方法，为LLM对齐提供了经济高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多轮对话生成（如智能回复和协作助手）是生成式AI的重要能力，但其评估存在关键瓶颈。相比双人对话，多轮设置引入了复杂轮转、角色依赖的说话人行为、长程对话结构和多个同等有效延续等独特挑战。

Method: MPCEval将生成质量分解为说话人建模、内容质量和说话人-内容一致性三个维度，明确区分局部下一轮预测和全局完整对话生成。提供新颖的、定量的、参考无关的、可复现的指标，可跨数据集和模型扩展。

Result: 在多样化的公共和真实世界数据集上评估现代生成方法，结果显示模型在参与平衡、内容进展和新颖性、说话人-内容一致性方面存在系统性的维度特定特征。评估目标对模型评估有决定性影响，单一评分会掩盖多轮对话行为的根本差异。

Conclusion: MPCEval揭示了多轮对话生成评估需要细粒度的多维度分析，单一评分方法不足以捕捉模型在复杂对话环境中的表现差异，为多轮对话生成提供了系统化的评估框架。
...
TL;DR: 本文提出VRM（变分奖励建模）框架，通过建模人类偏好评估过程中的高维目标权重和低维语义特征作为潜变量，解决传统奖励模型中的奖励黑客问题，获得更紧的泛化误差界。


<details>
  <summary>Details</summary>
Motivation: 传统LLM奖励模型直接将提示-响应对映射为标量分数，容易捕捉虚假相关性而非真实人类偏好，存在奖励黑客问题。而人类评估是一个更复杂的过程：先根据提示上下文权衡多个高维目标的相对重要性，再通过逻辑连贯性、上下文适当性等低维语义特征评估响应质量。

Method: 提出VRM（变分奖励建模）框架，将人类偏好判断的评估过程显式建模，将高维目标权重和低维语义特征作为潜变量，通过变分推断技术进行推断。该框架能更准确地捕捉人类偏好的评估机制。

Result: 理论分析表明VRM相比传统奖励模型能获得更紧的泛化误差界。在基准数据集上的大量实验证明，VRM在捕捉真实人类偏好方面显著优于现有方法。

Conclusion: VRM通过建模人类评估过程的潜变量结构，有效解决了传统奖励模型的奖励黑客问题，为LLM对齐提供了更可靠的奖励建模方法。
...
TL;DR: 本文针对大语言模型安全评估主要集中于英语的现状，提出了泰语安全基准ThaiSafetyBench，包含1,954个恶意泰语提示，涵盖一般有害提示和泰国文化背景的攻击。评估24个LLM发现闭源模型安全性优于开源模型，且泰国文化背景攻击成功率更高。作者还训练了ThaiSafetyClassifier分类器，性能与GPT-4.1相当。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的安全评估主要集中在英语领域，非英语语言和文化背景的风险研究不足。特别是泰语和泰国文化背景的安全风险缺乏系统评估，这可能导致模型在非英语文化环境中存在未被发现的安全漏洞。

Method: 1. 创建ThaiSafetyBench基准数据集，包含1,954个恶意泰语提示，分为一般有害提示和泰国文化背景的攻击
2. 使用该基准评估24个大语言模型，采用GPT-4.1和Gemini-2.5-Pro作为LLM-as-a-judge评估器
3. 训练基于DeBERTa的ThaiSafetyClassifier有害响应分类器，提高评估的复现性和成本效益
4. 建立ThaiSafetyBench排行榜，支持社区参与和持续更新

Result: 1. 闭源模型整体安全性能优于开源模型，开源模型安全性存在不足
2. 泰国文化背景的攻击成功率（ASR）显著高于一般泰语攻击，表明当前安全对齐方法存在关键漏洞
3. ThaiSafetyClassifier分类器达到84.4%的加权F1分数，与GPT-4.1判断结果相当
4. 所有资源（数据集、模型、代码、排行榜）均已公开，支持社区使用和复现

Conclusion: 本研究填补了泰语和泰国文化背景下大语言模型安全评估的空白，揭示了当前安全对齐方法在文化背景攻击下的脆弱性。开源模型的安全性能需要加强，而ThaiSafetyBench基准和ThaiSafetyClassifier分类器为社区提供了重要的评估工具和资源。
...
TL;DR: HiFlow：分层反馈驱动的优化框架，通过两级优化过程解决约束性长文本生成中的多目标协调问题


<details>
  <summary>Details</summary>
Motivation: 大语言模型在短文本生成中表现良好，但在复杂约束下的长文本生成中仍面临挑战。这类任务涉及多个紧密耦合的目标（全局结构一致性、局部语义连贯性、约束可行性），形成了具有挑战性的约束优化问题。现有方法主要依赖静态规划或离线监督，限制了生成过程中全局和局部目标的有效协调。

Method: 提出HiFlow分层反馈驱动优化框架，将生成过程制定为两级优化：规划层负责全局结构和约束建模，生成层负责条件文本生成。通过约束感知的计划筛选和两级闭环反馈，实现规划质量和生成行为的联合优化，逐步引导模型生成高质量、满足约束的输出。

Result: 在多个骨干模型上的实验证实了HiFlow相对于基线方法的有效性。

Conclusion: HiFlow通过分层反馈驱动优化，有效解决了约束性长文本生成中的多目标协调问题，提高了生成质量和对约束的满足度。
...
TL;DR: NeuronMoE方法通过分析跨语言神经元多样性来指导MoE架构中专家分配，相比传统层级相似度方法，在低资源语言上实现了约40%参数减少同时保持性能


<details>
  <summary>Details</summary>
Motivation: 将大语言模型扩展到低资源语言对全球可访问性至关重要，但为每种语言单独训练模型成本过高。现有MoE架构基于层级相似度分配专家，但语言处理在单个神经元层面存在细粒度专业化，需要更精细的专家分配方法
...
Conclusion: NeuronMoE揭示了多语言模型组织语言知识的潜在通用架构原则，表明低资源语言专家会自发形成与高资源语言相似的神经元专业化模式，为高效的多语言模型扩展提供了新方法
...
TL;DR: MUTEX：一个结合XLM RoBERTa和条件随机场的多语言Transformer模型，用于乌尔都语毒性片段检测，在token级别F1分数达到60%，是该领域的首个监督基线。


<details>
  <summary>Details</summary>
Motivation: 乌尔都语毒性片段检测面临挑战：现有系统多停留在句子级别分类，无法识别文本中的具体毒性片段；缺乏token级别标注资源；乌尔都语的语言复杂性、频繁的语码转换、非正式表达和丰富的形态变化。

Method: 提出MUTEX框架：结合多语言Transformer（XLM RoBERTa）和条件随机场（CRF）进行序列标注；使用手动标注的token级别毒性片段数据集；在社交媒体、在线新闻和YouTube评论等多领域数据上进行测试。

Result: MUTEX达到60%的token级别F1分数，这是乌尔都语毒性片段检测的首个监督基线。Transformer模型能更有效地隐式捕捉上下文毒性，并能处理语码转换和形态变化问题。
...
TL;DR: ARC-TGI是一个开源框架，通过Python程序生成器创建多样化的ARC-AGI任务，解决原始数据集过拟合和记忆问题，支持任务级约束确保训练示例能暴露底层规则。
...
Method: 开发任务家族生成器框架，每个生成器是紧凑的Python程序，能采样多样化任务同时保持潜在规则；提供自然语言输入和转换推理链，支持任务级约束确保训练示例能共同揭示底层规则。

Result: 发布了461个生成器，覆盖180个ARC-Mini任务、215个ARC-AGI-1任务（200训练/15测试）和66个ARC-AGI-2任务（55训练/11测试），支持可扩展的数据集采样和受控基准测试。

Conclusion: ARC-TGI框架通过任务生成器解决了ARC-AGI评估中的关键问题，为可扩展的少样本抽象推理研究提供了受控的基准测试环境。
...
TL;DR: 提出"表征保真度"作为验证算法决策的新维度，通过比较算法使用的输入表征与人类自我描述之间的差异来衡量决策的合理性基础
...
Method: 提出表征保真度概念，通过比较外部规定的输入表征与人类自我描述之间的差异来量化；建立表征不匹配的类型学；构建基于德国信用数据集的贷款决策基准数据集

Result: 开发了包含30,000个合成自然语言自我描述的Loan-Granting Self-Representations Corpus 2025数据集，包含专家标注的表征不匹配信息

Conclusion: 表征保真度为验证算法决策提供了新维度，有助于确保决策基于合理的表征基础，提出的基准数据集为未来研究提供了资源
...
TL;DR: 论文研究了大型语言模型中的类比推理能力，证明了通过联合训练相似性和属性前提可以实现类比推理，揭示了表示几何如何塑造归纳推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前对大型语言模型推理能力的评估常常混淆多种推理类型，这使得理解模型中的推理机制变得复杂。本研究旨在分离出类比推理（基于已知相似性推断实体间共享属性）并分析其在Transformer模型中的涌现机制。

Method: 通过理论证明三个关键结果：1）联合训练相似性和属性前提能够通过对齐表示实现类比推理；2）顺序训练只有在先学习相似性结构后学习具体属性时才能成功；3）两跳推理可以简化为带有身份桥接的类比推理。实验使用高达15亿参数的架构验证理论。

Result: 理论证明和实验验证表明：Transformer通过将具有相似属性的实体编码为相似的表示，使属性能够通过特征对齐进行转移。这揭示了类比推理的统一机制，即模型通过表示几何来塑造归纳推理能力。

Conclusion: 研究揭示了Transformer中类比推理的统一机制：模型通过将具有相似属性的实体编码为相似的表示，从而实现属性转移。这为理解大型语言模型的推理能力提供了理论基础，并表明表示几何在塑造归纳推理能力中起着关键作用。
...
TL;DR: LLM作为思维链推理评判者的可靠性研究：提出C2-Faith基准测试因果性和覆盖性两个维度，发现模型表现因任务框架而异，没有单一模型在所有设置中占优，且存在错误检测与定位之间的显著差距。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型越来越多地被用作思维链推理的评判者，但尚不清楚它们是否能可靠地评估过程忠实性，而不仅仅是答案合理性。需要明确LLM评判者在过程层面评估中的可靠性和局限性。

Method: 引入C2-Faith基准，基于PRM800K构建，针对忠实性的两个互补维度：因果性（每个步骤是否从先前上下文逻辑推导）和覆盖性（是否包含必要的中间推理）。通过受控扰动创建已知因果错误位置的示例，以及不同删除率的覆盖性删除示例。评估三个前沿评判模型在三个任务中的表现：二元因果检测、因果步骤定位和覆盖性评分。

Result: 模型排名强烈依赖于任务框架，没有单一评判者在所有设置中占优；所有评判者都表现出错误检测与定位之间的显著差距；覆盖性判断在不完整推理中系统性偏高。这些发现阐明了LLM评判者何时可靠以及何时失败。

Conclusion: 研究结果明确了LLM评判者在过程层面评估中的可靠性和局限性，为选择过程级评估的评判者提供了实用指导。表明需要根据具体任务框架选择适当的评判模型，并认识到当前模型在错误定位方面的局限性。
...
TL;DR: Sparse-BitNet框架首次将1.58位BitNet量化与动态N:M稀疏化结合，在保持训练稳定的同时，实现了比全精度模型更好的稀疏兼容性和计算效率提升。


<details>
  <summary>Details</summary>
Motivation: 半结构化N:M稀疏和低比特量化（如1.58位BitNet）是提升大语言模型效率的两个有前景的方法，但之前的研究大多孤立进行。本文旨在探索这两种技术的交互作用，特别是BitNet量化对稀疏化的兼容性优势。

Method: 提出Sparse-BitNet统一框架，首次同时应用1.58位量化和动态N:M稀疏化，并确保训练稳定性。通过定制稀疏张量核心实现计算加速，支持多种模型规模和训练策略（稀疏预训练和稠密到稀疏调度）。

Result: 1.58位BitNet在全精度模型相同的稀疏度下表现出更小的性能下降，能承受更高的结构化稀疏度而避免精度崩溃。使用定制稀疏张量核心后，训练和推理速度最高提升1.30倍。

Conclusion: 极低比特量化与半结构化N:M稀疏的结合是大语言模型效率优化的有前景方向，Sparse-BitNet框架为此提供了有效的实现方案。
...
TL;DR: 该指南提出了一个系统化、可操作的司法判决中法律论证结构的标注框架，包含命题分类、关系定义、形式化表示规则和标准化标注流程。


<details>
  <summary>Details</summary>
Motivation: 为揭示司法推理的逻辑组织，为计算分析提供可靠数据基础，支持大规模司法推理分析、法律论证挖掘、法律推理计算建模和AI辅助法律分析研究。

Method: 在命题层面区分四类命题（一般规范性、具体规范性、一般事实性、具体事实性）；在关系层面定义五种关系（支持、攻击、联合、匹配、同一性）；制定形式化表示规则和可视化约定；建立标准化标注流程和一致性控制机制。
...
TL;DR: 提出一个通用框架，通过确定性字符串到字符串转换（特别是有限状态转换器）从现有语言模型派生出新的语言模型，实现推理时适应特定输出格式需求。


<details>
  <summary>Details</summary>
Motivation: 语言模型通常定义在字符串分布上，但下游任务需要不同的输出格式（如字节对到单词、DNA到氨基酸）。现有方法没有将这种确定性转换视为产生新的、功能完整的语言模型。

Method: 提出基于有限状态转换器的通用框架，开发算法将语言模型与FST组合，通过边缘化源字符串映射到目标字符串，在不改变模型参数的情况下传播概率，实现对转换输出的条件化。

Result: 在三个领域进行实验：将语言模型从token转换为字节、从token转换为单词、从DNA转换为氨基酸，展示了预训练语言模型在推理时适应特定输出需求的能力。

Conclusion: 确定性字符串到字符串转换可以产生新的功能完整的语言模型，有限状态转换器为此提供了有效的框架，实现了推理时适应而不需要重新训练模型。
...
TL;DR: 扩散语言模型通过使用序列结束标记作为隐藏计算空间来提升复杂推理能力，特别是在生成长度远超实际答案所需时表现更佳


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在复杂推理任务中表现出色，特别是在生成长度远超实际答案所需时效果更好。研究者发现模型会使用序列结束标记作为隐藏计算空间，这引发了对其内部工作机制的好奇和研究动机

Method: 使用LLaDA1.5、LLaDA2.0-mini和Dream-v0等扩散模型，在加法、实体跟踪和数独等任务上进行实验。通过控制提示实验验证添加EoS标记的效果，并通过因果干预方法将EoS标记的隐藏状态替换为反事实生成的隐藏状态

Result: 添加EoS标记显著提升了扩散语言模型的推理能力。因果干预实验显示，替换EoS标记的隐藏状态会频繁改变生成输出，证明这些看似无意义的标记实际上携带了解决问题的信息

Conclusion: 扩散语言模型确实通过EoS标记进行隐藏计算，这些标记作为内部草稿纸帮助模型解决更复杂的推理问题，验证了"EoS-by-EoS思考"的假设
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：符号核方法虽然能保持行为语义，但计算成本高、锚点依赖且不可逆；基于语法的神经嵌入方法则无法捕捉底层结构。需要一种既能保持语义准确性又具有计算效率的方法。
...
TL;DR: 本文提出通过词汇表剪枝优化推测解码中的草稿模型，在保持高覆盖率的同时大幅减少词汇表规模，从而降低延迟并提高吞吐量。


<details>
  <summary>Details</summary>
Motivation: 推测解码中草稿模型通常主导延迟，因为其需要顺序生成token且语言建模头在大词汇表下计算成本高。传统方法存在根本性权衡：大词汇表提高token覆盖率和目标模型一致性但增加延迟，小词汇表减少延迟但可能遗漏关键token。

Method: 通过词汇表剪枝优化草稿模型，将词汇表选择建模为约束优化问题，平衡token覆盖率和草稿延迟。覆盖率基于训练数据中助手响应的token计算，延迟使用架构感知的FLOPs估计。使用树结构Parzen估计器优化效用函数，在最小覆盖率约束下探索覆盖率-延迟Pareto前沿。

Result: 实验显示在保持高覆盖率的同时，草稿词汇表最多减少97%。在领域特定任务上实现最多16%延迟降低和20%吞吐量提升，在多样化分布外任务上获得最多6.7%吞吐量增益。

Conclusion: 词汇表剪枝是优化推测解码草稿模型的有效方法，能够在保持高覆盖率的同时显著减少词汇表规模，从而在领域特定和分布外任务上都实现延迟降低和吞吐量提升。
...
TL;DR: 构建了孟加拉国首个国家级多语言平行多模态语料库，涵盖42种少数民族语言，包含8.5万条文本和107小时音频数据，支持濒危语言保护与低资源NLP研究。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国拥有约40种少数民族语言，其中14种濒危，但缺乏系统性的跨语系数字语料库。这些语言多为口语形式，在计算语言学中属于"零资源"语言，需要建立系统性的数字语料库来支持语言保护和研究。

Method: 通过90天的系统田野调查，在9个地区收集数据，涉及16名数据收集者、77名发言人和43名验证者。使用包含2224个独特项目的预定义诱发模板，涵盖三个语言粒度层级：孤立词汇项、语法结构和指导性言语。后处理包括10名语言学家的IPA转录和6名评审的独立裁决。

Result: 构建了包含85792条结构化文本条目（含孟加拉语刺激文本、英语翻译和IPA转录）和约107小时转录音频的语料库，涵盖藏缅语系、印欧语系、南亚语系和达罗毗荼语系的42种语言变体。数据通过Multilingual Cloud平台公开访问。

Conclusion: 该语料库为孟加拉国少数民族语言提供了首个系统性的数字资源，对濒危语言记录、低资源自然语言处理和语言多样性保护具有重要意义，特别是在语言多样性丰富的发展中国家。
...
TL;DR: Med-V1是一个30亿参数的小型语言模型，专门用于生物医学证据归因和验证任务，性能接近GPT-5等前沿大模型，但部署成本更低。


<details>
  <summary>Details</summary>
Motivation: 前沿大语言模型（如GPT-5）虽然能自动化文章支持性验证任务，但部署成本过高，难以规模化应用。生物医学领域需要高效、准确的证据归因解决方案。

Method: 开发了Med-V1系列小型语言模型（仅30亿参数），使用本研究新开发的高质量合成数据进行训练，将五个生物医学基准统一为验证格式进行评估。

Result: Med-V1在五个生物医学基准上比基础模型性能提升27.0%到71.3%，性能与GPT-5等前沿LLM相当，并能提供高质量的解释。研究发现引用格式指令强烈影响引用有效性和幻觉率，GPT-5生成更多声明但幻觉率与GPT-4o相似。

Conclusion: Med-V1为生物医学证据归因和验证任务提供了高效、准确的轻量级替代方案，能够识别临床实践指南中的高风险证据误归因，具有重要的实际应用价值。
...
TL;DR: 该研究构建了波斯语标点恢复数据集PersianPunc，并基于ParsBERT开发了高效的标点恢复模型，相比大语言模型具有更好的准确性和效率


<details>
  <summary>Details</summary>
Motivation: 波斯语标点恢复对于提升自动语音识别输出的可读性和下游应用至关重要，但该领域研究不足，缺乏高质量数据集

Method: 通过系统聚合和过滤现有文本资源构建包含1700万样本的PersianPunc数据集，将标点恢复建模为token级序列标注任务，微调ParsBERT模型

Result: BERT方法在测试集上获得91.33%的宏平均F1分数，效率适合实时应用；大语言模型存在过度校正问题和更高的计算需求

Conclusion: 轻量级BERT方法在波斯语标点恢复任务上表现优异，公开的数据集和模型为波斯语NLP研究提供了资源，框架可扩展到其他形态丰富、资源匮乏的语言
...
TL;DR: 该研究为西班牙语、加泰罗尼亚语和意大利语创建了一个高质量的人工简化文本语料库，填补了这些语言在自动文本简化研究中的资源空白。


<details>
  <summary>Details</summary>
Motivation: 理解信息是自主生活和社会参与的关键，特别是在民主进程中。自动文本简化研究常受限于高质量训练和评估材料的可用性，这对英语已是个问题，对西班牙语、加泰罗尼亚语和意大利语等资源较少的语言更是如此。

Method: 在iDEM项目框架内，从与民主参与相关的领域收集原始文本，基于相关性、版权可用性和伦理标准选择不同文本类型，由文本简化专家将文本简化至易读(E2R)水平。

Result: 创建了包含西班牙语、加泰罗尼亚语和意大利语的高质量人工简化语料库，其中加泰罗尼亚语语料库是该类型的首个标注语料库，对西班牙语和意大利语也是重要贡献，提供了这些领域罕见的高质量人工标注语言资源。

Conclusion: 该语料库填补了资源较少语言在文本简化研究中的空白，特别有价值的是包含了加泰罗尼亚语的首个此类标注语料库，将为公众免费提供，支持民主参与和语言可及性研究。
...
TL;DR: 研究语音识别多领域模型合并技术，提出新的BoostedTSV-M算法，在葡萄牙语多领域ASR中超越全微调性能


<details>
  <summary>Details</summary>
Motivation: 针对大型语音基础模型在多领域应用时，为每个领域单独微调会产生多个定制化检查点，重新进行全微调计算成本过高的问题，研究模型合并作为可扩展的替代方案

Method: 研究多领域ASR的模型合并，评估11种合并算法在10个葡萄牙语领域的表现，提出基于TSV-M的新算法BoostedTSV-M，通过奇异值增强缓解秩崩溃并提高数值稳定性

Result: 在葡萄牙语多领域ASR中，提出的方法整体上超越了全微调的性能，同时在单一模型中保持了分布外泛化能力，评估了各种算法在领域内准确性、分布偏移鲁棒性以及英语和多语言性能

Conclusion: 模型合并是大型语音基础模型多领域适应的有效可扩展方案，提出的BoostedTSV-M算法通过改进TSV-M在葡萄牙语多领域ASR中实现了优于全微调的性能
...
TL;DR: DiSCTT：一种基于难度感知和共识引导的自课程框架，根据推理轨迹的共识度动态分配测试时优化策略，提高大语言模型推理性能


<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法对所有输入应用统一的优化目标，导致在异构推理问题上效率低下或不稳定。需要一种能够根据实例难度和不确定性动态调整优化策略的方法。

Method: 提出DiSCTT框架：1）通过采样推理轨迹的一致性估计实例级认知不确定性；2）高共识输入使用多数同意的解决方案作为伪标签进行监督微调；3）低共识输入使用共识正则化目标的强化学习进行优化，在相关性约束下鼓励多样性。

Result: 在广泛的数学和通用推理基准测试中，DiSCTT始终优于强大的测试时适应基线，实现了更高的准确性、更低的方差，并显著减少了计算和训练时间。

Conclusion: 明确考虑实例难度和不确定性能够实现更稳定、高效和有效的推理模型测试时适应，为自适应优化策略提供了有前景的方向。
...
TL;DR: 提出Progressive Residual Warmup（ProRes）方法，通过渐进式残差预热实现"先学浅层"的Transformer预训练策略，提升训练稳定性和收敛速度。


<details>
  <summary>Details</summary>
Motivation: Transformer架构是现代大语言模型的核心，其预训练的稳定性和收敛速度至关重要。考虑到层间顺序堆叠的逻辑依赖性，需要一种能够优化训练过程的方法。

Method: ProRes采用渐进式残差预热策略，为每层残差乘以从0到1逐渐增大的标量系数，深层需要更长的预热步数。这样深层会等待浅层进入更稳定状态后再参与学习。

Result: 在不同模型规模、归一化和初始化方案下的预训练实验表明，ProRes不仅能稳定预训练，还能引入独特的优化轨迹，实现更快收敛、更强泛化能力和更好的下游性能。

Conclusion: ProRes是一种有效的Transformer预训练优化方法，通过渐进式残差预热实现了"先学浅层"的训练哲学，显著提升了训练稳定性和模型性能。
...
TL;DR: 研究表明，通过精心设计的推理中心微调策略，低参数大语言模型（<4B参数）可以在词义消歧任务上达到与GPT-4-Turbo相当的性能，同时大幅降低计算和能源需求。


<details>
  <summary>Details</summary>
Motivation: 词义消歧是自然语言处理中的关键挑战，特别是处理罕见或领域特定词义时。虽然GPT-4-Turbo等高参数大语言模型在词义消歧上表现出色，但其计算和能源需求限制了可扩展性。本研究旨在探索低参数大语言模型是否通过推理驱动的微调策略能达到可比性能。

Method: 使用增强了半自动、富含推理标注的FEWS数据集，对八个小型开源大语言模型（如Gemma和Qwen）进行微调。采用思维链推理结合邻词分析的方法，强调推理驱动的词义识别。

Result: Gemma-3-4B和Qwen-3-4B模型在FEWS数据集上持续超越所有中等参数基线和最先进模型，对未见词义具有鲁棒泛化能力。在"Fool Me If You Can"数据集上的评估确认了无需任务特定微调的强跨域适应性。推理驱动的微调使低参数模型在零样本设置下达到与GPT-4-Turbo相当的性能。

Conclusion: 通过精心设计的推理中心微调策略，低参数大语言模型能够提供准确的词义消歧，同时显著降低计算和能源需求，为资源受限环境中的高效自然语言处理应用提供了可行方案。
...
TL;DR: FlashAttention-4是针对Blackwell GPU架构优化的注意力机制实现，相比前代在B200 GPU上实现了最高1.3倍于cuDNN和2.7倍于Triton的速度提升。


<details>
  <summary>Details</summary>
Motivation: 随着AI行业快速转向Blackwell架构GPU（如B200、GB200），这些新硬件存在非对称扩展特性：张量核心吞吐量翻倍，但其他功能单元（共享内存带宽、指数单元）扩展较慢或保持不变。FlashAttention-3主要针对H100架构，无法充分利用Blackwell GPU的新特性。

Method: 1. 重新设计流水线，利用完全异步的MMA操作和更大的tile尺寸
2. 软件模拟指数和条件softmax重缩放，减少非矩阵乘法操作
3. 利用张量内存和2-CTA MMA模式减少共享内存流量和后向传播中的原子加法操作
4. 完全使用嵌入Python的CuTe-DSL实现，相比传统C++模板方法编译时间快20-30倍

Result: 在B200 GPU上使用BF16精度，FlashAttention-4相比cuDNN 9.13实现了最高1.3倍加速，相比Triton实现了最高2.7倍加速，达到1613 TFLOPs/s（71%利用率）。

Conclusion: FlashAttention-4通过针对Blackwell GPU架构特性的优化技术，显著提升了注意力机制的性能，同时通过CuTe-DSL实现大幅减少了编译时间，为大规模语言模型和长上下文应用提供了高效的注意力实现方案。
...
TL;DR: 提出DEBISS语料库：包含口语化和个体化辩论的半结构化数据集，涵盖多种NLP任务标注


<details>
  <summary>Details</summary>
Motivation: 辩论在日常生活中有广泛应用，但由于辩论形式、结构和应用的多样性，现有辩论语料库稀缺且难以开发，需要构建更全面的辩论数据集

Method: 创建DEBISS语料库，包含口语化和个体化辩论，具有半结构化特征，并提供广泛的NLP任务标注

Result: 开发了一个包含多种NLP任务标注的辩论语料库，包括语音转文本、说话人分离、论点挖掘和辩论者质量评估等

Conclusion: DEBISS语料库填补了辩论数据集的空白，为辩论相关的NLP研究提供了有价值的资源
...
TL;DR: 研究者创建了NCTB-QA——一个包含87,805个问答对的大规模孟加拉语阅读理解数据集，专门针对不可回答问题设计，并评估了三种Transformer模型在该数据集上的表现。


<details>
  <summary>Details</summary>
Motivation: 低资源语言（如孟加拉语）的阅读理解系统在处理不可回答问题时表现不佳，容易产生不可靠的响应。现有孟加拉语数据集缺乏平衡的不可回答问题分布，需要专门的数据集来提升系统鲁棒性。

Method: 从孟加拉国国家课程与教科书委员会出版的50本教科书中提取87,805个问答对，确保可回答（57.25%）与不可回答（42.75%）问题的平衡分布。数据集包含对抗性设计的实例（含可信干扰项）。使用BERT、RoBERTa、ELECTRA三种Transformer模型进行基准测试和微调。

Result: 微调后模型性能显著提升：BERT的F1分数相对提升313%（从0.150到0.620）。所有模型的BERTScore语义答案质量均显著提高。NCTB-QA成为孟加拉语教育问答的挑战性基准。

Conclusion: NCTB-QA数据集填补了孟加拉语阅读理解数据集的空白，证明了领域特定微调对低资源环境下鲁棒性能的关键作用，为教育问答系统开发提供了重要资源。
...
TL;DR: 论文提出无需检索的事实核查任务，开发了INTRA方法利用模型内部表征进行事实验证，在9个数据集上取得SOTA性能，展示了该方向作为检索式框架补充的潜力。
...
Method: 提出无需检索的事实核查任务，引入综合评估框架测试泛化能力；开发INTRA方法，通过利用模型内部表征之间的交互来进行事实验证。

Result: 在9个数据集、18种方法和3个模型上的实验表明，基于logit的方法通常表现不佳，而利用内部表征的方法更优；INTRA方法实现了最先进的性能，并展现出强大的泛化能力。

Conclusion: 无需检索的事实核查是一个有前景的研究方向，可以作为检索式框架的补充，提高可扩展性，并能在训练中作为奖励信号或集成到生成过程中。
...
TL;DR: 研究发现大型语言模型存在"表演性思维链"现象：模型在推理过程中早已确定最终答案，但仍继续生成推理过程而不暴露内部信念，尤其是在简单任务中。通过激活探针等技术可以提前解码答案，实现高效计算。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索大型语言模型在思维链推理中的真实认知过程，区分真正的推理与"表演性推理"，并开发方法检测模型何时已形成最终答案但仍在继续生成推理过程。

Method: 研究方法包括：1) 激活探针技术，从模型内部激活中解码最终答案；2) 早期强制回答；3) 思维链监控器。在两个大型模型（DeepSeek-R1 671B和GPT-OSS 120B）上测试，涵盖简单回忆型任务（MMLU）和复杂多跳推理任务（GPQA-Diamond）。

Result: 研究发现：1) 对于简单任务，模型在思维链早期就能解码出最终答案，远早于监控器能判断的时间；2) 复杂任务中确实存在真正的推理过程；3) 转折点（如回溯、"顿悟"时刻）主要出现在探针显示信念大幅变化时；4) 探针引导的早期退出可减少80%的MMLU令牌和30%的GPQA-Diamond令牌，同时保持相似准确率。

Conclusion: 结论表明大型语言模型存在"表演性思维链"现象，尤其是在简单任务中。激活探针技术能有效检测这种表演性推理，并实现自适应计算，显著减少计算开销。该研究为理解模型内部推理过程和优化计算效率提供了重要工具。
...
### [138] [Decorrelating the Future: Joint Frequency Domain Learning for Spatio-temporal Forecasting](https://arxiv.org/abs/2603.04418)
...
<details>
  <summary>Details</summary>
Motivation: 标准直接预测模型通常依赖点状目标（如均方误差），无法捕捉图结构信号中固有的复杂时空依赖关系。现有的频域方法虽然缓解了时间自相关，但往往忽略了空间和跨时空的交互作用。
...
Conclusion: FreST Loss通过联合时空谱域监督有效解决了现有方法在捕捉复杂时空依赖方面的局限性，为图结构信号的预测提供了更全面的训练目标。
...
### [139] [FedEMA-Distill: Exponential Moving Average Guided Knowledge Distillation for Robust Federated Learning](https://arxiv.org/abs/2603.04422)
*Hamza Reguieg,Mohamed El Kamili,Essaid Sabir*

Main category: cs.LG

TL;DR: FedEMA-Distill：一种服务器端联邦学习方法，通过指数移动平均和知识蒸馏处理异构数据和对抗性客户端，显著提升精度、减少通信轮次和负载


<details>
  <summary>Details</summary>
Motivation: 联邦学习在客户端数据异构非独立同分布以及存在对抗性客户端时性能下降，导致客户端漂移、收敛缓慢和通信开销高的问题

Method: 提出FedEMA-Distill方法，在服务器端结合全局模型的指数移动平均和客户端上传预测logits的集成知识蒸馏，客户端仅上传压缩logits，支持模型异构且无需修改客户端软件

Result: 在CIFAR-10、CIFAR-100、FEMNIST和AG News数据集上，相比基线方法提升1-6%的top-1精度，减少30-35%的通信轮次，客户端上行负载降至0.09-0.46MB（比传输完整模型权重减少一个数量级），在10-20%拜占庭客户端存在时仍能稳定训练

Conclusion: 时间平滑与仅logits聚合相结合提供了通信高效、抗攻击的联邦学习管道，部署友好且兼容安全聚合和差分隐私，因为只交换聚合或混淆的模型输出
...
### [140] [Delta-Crosscoder: Robust Crosscoder Model Diffing in Narrow Fine-Tuning Regimes](https://arxiv.org/abs/2603.04426)
*Aly Kassem,Thomas Jiralerspong,Negar Rostamzadeh,Golnoosh Farnadi*

Main category: cs.LG

TL;DR: Delta-Crosscoder是一种改进的模型差异分析方法，通过结合BatchTopK稀疏性、基于变化的损失函数和隐式对比信号，能够更有效地识别微调导致的模型内部表示变化，特别是在局部化、不对称的窄微调场景中。


<details>
  <summary>Details</summary>
Motivation: 现有的Crosscoders方法在窄微调（narrow fine-tuning）场景中存在局限性，当行为变化局部化且不对称时，传统方法难以准确识别模型内部表示的变化。

Method: Delta-Crosscoder结合了三种关键技术：1) BatchTopK稀疏性，2) 基于变化的损失函数（delta-based loss），优先关注模型间变化的方向，3) 从匹配输入的配对激活中获取隐式对比信号。

Result: 在10个模型（包括Gemma、LLaMA、Qwen等1B-9B参数模型）上的评估显示，Delta-Crosscoder能够可靠地分离出对微调行为有因果责任的潜在方向，并实现有效的缓解措施，性能优于基于SAE的基线方法，与非SAE方法相当。

Conclusion: Crosscoders仍然是模型差异分析的有力工具，Delta-Crosscoder的改进使其在窄微调场景中表现更优，能够有效识别和缓解微调引起的行为变化。
...
### [141] [Thin Keys, Full Values: Reducing KV Cache via Low-Dimensional Attention Selection](https://arxiv.org/abs/2603.04427)
*Hengshuai Yao,Guan Wang*

Main category: cs.LG

TL;DR: 本文提出非对称注意力机制，认为查询和键用于选择只需要低维表示，而值用于语义传递需要高维表示，通过实验验证了这种设计能显著减少参数和KV缓存占用。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer注意力机制中查询、键和值使用相同的维度，但作者认为它们在功能上存在本质差异：查询和键用于产生注意力权重（选择操作），而值用于传递丰富的语义信息。选择操作本质上比值传递需要更低的维度。

Method: 提出非对称注意力机制，降低查询和键的维度（选择维度），保持值的维度不变。通过七个实验验证：1）位置选择任务；2）基于内容的检索；3-4）WikiText语言建模；5）GPT-2的SVD压缩；6）125M参数LLaMA模型；7）Mistral-7B模型。对现有模型采用SVD压缩后QK微调的方法。

Result: 实验表明：1）选择操作仅需O(log N)维度；2）在WikiText语言建模中，选择维度降至模型维度的1/4时，困惑度仅增加4.3%，QK参数减少75%；3）GPT-2的键比查询更容易压缩；4）对Mistral-7B进行SVD压缩和QK微调，实现了75%的键缓存节省，质量损失仅2%；5）在7B参数模型中，非对称注意力可为每个用户节省25GB KV缓存，GPU上并发用户数增加约60%。

Conclusion: 非对称注意力机制通过降低查询和键的维度，显著减少了模型参数和KV缓存占用，同时保持了模型性能。这种方法为大规模语言模型的高效部署提供了有效解决方案，特别是在长上下文场景下。
...
### [142] [Agent Memory Below the Prompt: Persistent Q4 KV Cache for Multi-Agent LLM Inference on Edge Devices](https://arxiv.org/abs/2603.04428)
*Yakov Pyotr Shkolnikov*

Main category: cs.LG

TL;DR: 提出了一种在边缘设备上持久化存储4位量化KV缓存的方法，解决了多智能体LLM系统内存不足的问题，避免了重复的预填充计算，显著提升了响应速度。
...
Method: 系统包含三个组件：1) 块池提供每个智能体隔离的Q4 KV缓存（safetensors格式）；2) BatchQuantizedKVCache支持对多个智能体量化缓存的并发推理；3) 跨阶段上下文注入，在不重新计算的情况下跨对话阶段累积注意力状态。通过将KV缓存持久化到磁盘并以4位量化格式直接恢复到注意力层，消除了冗余的O(n)预填充计算。

Result: 在三种架构上评估：Gemma 3 12B、DeepSeek-Coder-V2-Lite 16B、Llama 3.1 8B。缓存恢复将首令牌时间减少了高达136倍（Gemma：4K-32K下22-136倍；DeepSeek：4K-32K下11-76倍；Llama：4K-16K下24-111倍；1K下3-10倍）。Q4量化使固定设备内存中可容纳的智能体上下文数量增加了4倍。实际Q4 KV缓存的困惑度测量显示：Gemma -0.7%、Llama +2.8%、DeepSeek +3.0%。

Conclusion: 通过持久化4位量化KV缓存和直接缓存恢复，有效解决了边缘设备上多智能体LLM系统的内存限制问题，显著减少了响应延迟，同时保持了可接受的模型质量损失。
...
### [143] [Uncertainty-Calibrated Spatiotemporal Field Diffusion with Sparse Supervision](https://arxiv.org/abs/2603.04431)
*Kevin Valencia,Xihaier Luo,Shinjae Yoo,David Keetae Park*

Main category: cs.LG

TL;DR: SOLID是一个基于掩码条件的扩散框架，直接从稀疏观测中学习时空动力学，无需密集场数据或预插值，通过双重掩码目标实现后验采样，在严重稀疏条件下获得校准的不确定性。


<details>
  <summary>Details</summary>
Motivation: 物理场通常只在稀疏、时变的传感器位置观测，使得预测和重建成为不适定且不确定性关键的问题。现有方法通常在密集数据上训练，只在稀疏条件下测试，而SOLID旨在直接从稀疏观测中学习。

Method: SOLID采用掩码条件扩散框架，每个去噪步骤都基于测量值及其位置进行条件化。引入双重掩码目标：(1)强调在未观测空白区域的学习，(2)对输入和目标提供最可靠锚点的重叠像素进行加权。

Result: 在严重稀疏条件下，SOLID实现了概率误差的数量级改进，产生校准的不确定性图（ρ > 0.7）。该框架能够从稀疏观测中采样与测量一致的全场后验分布。

Conclusion: SOLID证明了直接从稀疏观测中学习时空动力学的可行性，通过严格的稀疏条件路径实现全场后验采样，在不确定性校准和概率误差方面显著优于现有方法。
...
### [144] [ZorBA: Zeroth-order Federated Fine-tuning of LLMs with Heterogeneous Block Activation](https://arxiv.org/abs/2603.04436)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型联邦微调面临两个主要挑战：1）本地更新需要大量VRAM存储梯度；2）频繁模型交换导致通信开销大。需要一种既能减少内存使用又能降低通信成本的解决方案。

Method: 提出ZorBA框架：1）采用零阶优化消除客户端梯度存储需求；2）引入异构块激活机制，服务器为不同客户端分配不同的transformer块子集；3）利用共享随机种子和梯度有限差分减少通信开销；4）通过ε约束词典算法优化块激活决策。

Result: 实验结果显示，ZorBA在VRAM使用上比三种联邦微调基线方法最多减少62.41%，同时保持较低的通信开销。

Conclusion: ZorBA通过零阶优化和异构块激活机制，有效解决了大型语言模型联邦微调中的VRAM使用和通信开销问题，为分布式环境下的LLM微调提供了高效解决方案。
...
### [145] [Learning Unified Distance Metric for Heterogeneous Attribute Data Clustering](https://arxiv.org/abs/2603.04458)
...
Method: 提出异构属性重构与表示学习范式：1）将异构属性转换为同构状态进行距离度量学习；2）将度量学习与聚类集成，自动适应不同聚类任务；3）将每个属性值投影到统一可学习的多空间中，更精细地表示和学习分类数据的距离度量。

Result: HARR方法参数免费、收敛有保证，能更有效地自适应不同聚类数量k。大量实验证明其在准确性和效率方面的优越性。

Conclusion: 通过研究不同类型属性间的联系，提出的HARR学习范式能够有效处理混合数据聚类问题，在保持无参数和收敛保证的同时，显著提升了聚类性能。
...
### [146] [VSPrefill: Vertical-Slash Sparse Attention with Lightweight Indexing for Long-Context Prefilling](https://arxiv.org/abs/2603.04460)
*Chen Guanzhong*

Main category: cs.LG

TL;DR: VSPrefill是一种用于大语言模型长上下文推理的稀疏注意力机制，通过垂直-斜线结构模式在保持98.35%准确率的同时实现4.95倍加速
...
Method: 提出VSPrefill机制，使用垂直-斜线结构模式，通过轻量级训练的VSIndexer模块从增强RoPE的键值表示中预测上下文感知的重要性分数，构建线性复杂度的稀疏掩码，不修改骨干参数。
...
### [147] [MAD-SmaAt-GNet: A Multimodal Advection-Guided Neural Network for Precipitation Nowcasting](https://arxiv.org/abs/2603.04461)
*Samuel van Wonderen,Siamak Mehrkanoon*

Main category: cs.LG

TL;DR: MAD-SmaAt-GNet是一个用于降水临近预报的多模态深度学习模型，它在轻量级SmaAt-UNet基础上增加了多模态编码器和物理平流组件，显著提升了预报精度。
...
Method: 提出MAD-SmaAt-GNet模型，扩展了SmaAt-UNet架构：1）增加额外编码器以学习多个天气变量（多模态输入）；2）集成基于物理的平流组件以确保物理一致的预测。

Result: 每个扩展单独都能改善降雨预报，组合使用带来进一步增益。相比基线SmaAt-UNet，MAD-SmaAt-GNet在4小时内的四步降水预报中将均方误差降低了8.9%。多模态输入对短预报时效特别有益，而平流组件在短长期预报中都能提升性能。
...
### [148] [Understanding the Dynamics of Demonstration Conflict in In-Context Learning](https://arxiv.org/abs/2603.04464)
*Difan Jiao,Di Wang,Lijie Hu*

Main category: cs.LG

TL;DR: 大语言模型在上下文学习中容易受到噪声和冲突示例的影响，研究发现模型在处理规则冲突时存在系统性误导行为，揭示了其两阶段计算结构。
...
Method: 使用线性探针和logit lens分析技术，研究模型在处理冲突证据时的内部处理过程。通过注意力头分析识别影响推理失败的关键组件，并进行针对性消融实验验证发现。

Result: 模型在单个规则被破坏的演示下性能显著下降。研究发现模型在中间层同时编码正确和错误规则，但在后期层才形成预测置信度，呈现两阶段计算结构。识别出两个关键注意力头类型：早期到中间层的易受攻击头对破坏高度敏感，后期层的易受影响头显著减少对正确预测的支持。针对性消融少量识别出的头部可将性能提升超过10%。

Conclusion: 大语言模型在处理冲突证据时存在系统性弱点，其推理过程呈现两阶段结构。通过识别和干预特定注意力头，可以有效缓解模型在噪声和冲突示例下的性能下降问题，为提高模型鲁棒性提供了新思路。
...
### [149] [Towards Explainable Deep Learning for Ship Trajectory Prediction in Inland Waterways](https://arxiv.org/abs/2603.04472)
...
<details>
  <summary>Details</summary>
Motivation: 在内河航道拥挤环境中，准确的船舶轨迹预测对安全至关重要。虽然深度学习模型在预测精度方面取得了进展，但这些模型通常缺乏可解释性，可能隐藏不准确的逻辑并削弱对其可靠性的信任。

Method: 研究采用LSTM基础的船舶轨迹预测模型，结合可训练的船舶领域参数，通过基于注意力的机制融合交互船舶的隐藏状态，从而提供模型决策过程的洞察。

Result: 模型在5分钟预测范围内达到约40米的最终位移误差，与类似研究性能相当。虽然船舶间注意力架构提高了预测精度，但学习到的船舶领域参数分配的权重与预期存在偏差，表明精度提升不完全由船舶间的因果关系驱动。

Conclusion: 该研究通过内在可解释的设计增强了模型的解释能力。未来工作将集中于利用该架构进行反事实分析，并整合更复杂的注意力机制。
...
### [150] [Augmenting representations with scientific papers](https://arxiv.org/abs/2603.04516)
...
TL;DR: 该研究提出了一种对比学习框架，用于对齐X射线光谱与从科学文献中提取的领域知识，建立多模态共享表示，提高了物理变量估计精度并识别了重要天体目标。


<details>
  <summary>Details</summary>
Motivation: 天文学家拥有大量多模态数据（图像、光谱、时间序列）和数十年文献分析，但这些数据源很少被系统整合。科学文本涵盖的物理背景比光谱更广泛多样，建立两者连接具有内在复杂性。

Method: 提出对比学习框架，将X射线光谱与从科学文献提取的领域知识对齐，开发共享多模态表示。采用专家混合策略，利用单模态和共享表示。

Result: 1) 从光谱检索文本的Recall@1%达到20%，证明模态对齐可行且能加速罕见源解释；2) 融合光谱和文本数据，将20个物理变量的估计精度比单模态基线提高16-18%；3) 多模态潜在空间识别出高优先级后续观测目标，包括候选脉冲超亮X射线源和引力透镜系统。

Conclusion: 该框架成功建立了光谱与文献知识的有意义对齐，共享潜在空间有效编码物理信息，专家混合策略表现优异。该框架可扩展到其他需要对齐观测数据与现有文献的科学领域。
...
### [151] [Invariant Causal Routing for Governing Social Norms in Online Market Economies](https://arxiv.org/abs/2603.04534)
*Xiangning Yu,Qirui Mi,Xiao Xue,Haoxuan Li,Yiwei Shi,Xiaowei Liu,Mengyue Yang*

Main category: cs.LG

TL;DR: 论文提出了一种名为"不变因果路由(ICR)"的因果治理框架，用于识别跨异质环境下稳定的政策-规范关系，以引导在线市场经济中的社会规范向期望方向发展。


<details>
  <summary>Details</summary>
Motivation: 在线市场经济中，公平曝光、持续参与和平衡再投资等社会规范对长期稳定至关重要。然而，规范源于无数微观交互的聚合，使得因果归因和政策可迁移性变得困难。需要理解驱动这些涌现规范的因果机制，并设计能够引导它们向期望结果发展的原则性干预措施。

Method: 提出不变因果路由(ICR)框架，将反事实推理与不变因果发现相结合，以区分真正的因果效应和虚假相关性，并构建可解释、可审计的政策规则，这些规则在分布偏移下仍保持有效。

Result: 在基于真实数据校准的异质智能体模拟中，ICR相比相关性或覆盖率基线方法，能够产生更稳定的规范、更小的泛化差距和更简洁的规则。
...
### [152] [An LLM-Guided Query-Aware Inference System for GNN Models on Large Knowledge Graphs](https://arxiv.org/abs/2603.04545)
*Waleed Afandi,Hussein Abdallah,Ashraf Aboulnaga,Essam Mansour*

Main category: cs.LG

TL;DR: KG-WISE是一种针对大规模知识图谱的GNN推理加速框架，通过分解GNN模型为细粒度组件，结合LLM生成查询模板，实现查询感知的模型加载，显著提升推理速度和降低内存使用。


<details>
  <summary>Details</summary>
Motivation: 现有GNN推理加速方法（如剪枝、量化、知识蒸馏）存在局限性：1）无法根据查询的结构或语义自适应调整模型；2）模型以整体文件形式存储，需要完全加载；3）无法仅加载与目标节点语义相关的邻居节点和模型组件，导致在大规模知识图谱上数据加载和计算冗余。

Method: KG-WISE将训练好的GNN模型分解为细粒度组件，支持基于查询子图结构的部分加载。使用大语言模型生成可重用的查询模板，为每个任务提取语义相关的子图，实现查询感知的紧凑模型实例化。

Result: 在6个大规模知识图谱（最大包含4200万节点和1.66亿边）上评估，KG-WISE相比最先进系统实现：推理速度提升最高28倍，内存使用降低98%，同时在商业和开源LLM上保持或提升准确性。

Conclusion: KG-WISE提出了一种任务驱动的推理范式，通过模型分解和查询感知的组件加载，有效解决了大规模知识图谱上GNN推理的效率问题，在保持准确性的同时显著提升了推理速度和内存效率。
...
### [153] [Oracle-efficient Hybrid Learning with Constrained Adversaries](https://arxiv.org/abs/2603.04546)
...
Method: 考虑结构化设置，约束对手从固定但表达能力强的函数类R中选择标签。开发新学习算法，利用ERM预言机高效运行，采用带"截断熵正则化器"的Frank-Wolfe约简和新的"混合"鞅差序列尾界等技术工具。

Result: 算法获得的遗憾与从学习者假设类H和对手标签类R导出的类的Rademacher复杂度成比例。作为关键推论，给出了在随机零和博弈中计算均衡的预言机高效算法，适用于高维动作集但具有低维结构支付函数的情况。

Conclusion: 本文在混合学习设置中朝着同时实现统计最优性和计算效率迈出了重要一步，通过约束对手标签选择的结构化设置，开发了新的算法框架和技术工具，为相关领域提供了新的解决方案。
...
### [154] [Latent Particle World Models: Self-supervised Object-centric Stochastic Dynamics Modeling](https://arxiv.org/abs/2603.04553)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够自主发现对象表示并适用于真实世界多对象数据集的世界模型，同时支持决策任务，解决现有方法在真实世界场景中的扩展性和应用性限制。

Method: 提出Latent Particle World Model (LPWM)，通过自监督方式从视频中学习对象表示（关键点、边界框、掩码），采用端到端训练，包含新颖的潜在动作模块来建模随机粒子动力学，支持动作、语言和图像目标的条件化。

Result: 在多样化的真实世界和合成数据集上取得了最先进的结果，能够有效建模随机视频，并成功应用于目标条件模仿学习等决策任务。

Conclusion: LPWM是一个可扩展的自监督对象中心世界模型，能够从视频中学习丰富的场景分解，不仅适用于随机视频建模，还能直接应用于决策任务，为真实世界应用提供了强大的基础。
...
### [155] [PDE foundation model-accelerated inverse estimation of system parameters in inertial confinement fusion](https://arxiv.org/abs/2603.04606)
*Mahindra Rautela,Alexander Scheinker,Bradley Love,Diane Oyen,Nathan DeBardeleben,Earl Lawrence,Ayan Biswas*

Main category: cs.LG

TL;DR: 该研究探讨了PDE基础模型在惯性约束聚变逆问题中的应用，通过微调预训练模型和训练轻量级任务特定头，实现了从多模态观测中准确重建高光谱图像和回归系统参数。


<details>
  <summary>Details</summary>
Motivation: 虽然PDE基础模型通常在大规模PDE数据集上预训练，但大多数下游评估集中在正向问题（如自回归滚动预测）。本研究关注惯性约束聚变中的逆问题：从多模态、快照式观测中估计系统参数。

Method: 使用开放的JAG基准数据集，该数据集提供每个模拟的高光谱X射线图像和标量观测值。微调PDE基础模型并训练轻量级任务特定头，联合重建高光谱图像和回归系统参数。

Result: 微调模型实现了准确的高光谱重建（测试MSE 1.2e-3）和强大的参数估计性能（R^2高达0.995）。数据缩放实验显示，随着训练数据增加，重建和回归损失均持续改善，在低数据区域获得最大边际收益。
...
### [156] [Neuro-Symbolic Financial Reasoning via Deterministic Fact Ledgers and Adversarial Low-Latency Hallucination Detector](https://arxiv.org/abs/2603.04663)
*Pedram Agand*

Main category: cs.LG

TL;DR: VeNRA框架通过类型化事实账本和双重锁定算法解决金融领域RAG的算术和语义混淆问题，引入Sentinel模型审计执行痕迹，使用对抗模拟训练，并提出微分块损失算法优化训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 标准RAG架构在金融等高风险领域存在两大根本限制：LLM的算术能力不足和密集向量检索的语义混淆问题。在确定性领域，99%的准确率也无法建立操作信任，需要实现零幻觉的金融推理。

Method: 1. 引入VeNRA框架，通过类型化通用事实账本(UFL)检索确定性变量而非概率文本；2. 采用双重锁定算法进行数学约束；3. 开发30亿参数Sentinel模型审计Python执行痕迹；4. 使用对抗模拟而非传统幻觉数据集进行训练；5. 提出微分块损失算法解决反向思维链训练中的损失稀释问题。

Result: VeNRA框架能够实现零幻觉的金融推理，Sentinel模型能够在单次令牌测试预算下进行法证审计，微分块损失算法在极端差分惩罚下稳定梯度，避免内存溢出问题。

Conclusion: VeNRA通过从概率文本检索转向确定性变量检索，结合类型化事实账本、双重锁定算法和专门的审计模型，解决了金融领域RAG的关键限制，为实现高可靠性金融推理系统提供了新范式。
...
### [157] [Engineering Regression Without Real-Data Training: Domain Adaptation for Tabular Foundation Models Using Multi-Dataset Embeddings](https://arxiv.org/abs/2603.04692)
*Lyle Regenwetter,Rosen Yu,Cyril Picard,Faez Ahmed*

Main category: cs.LG

TL;DR: TREDBench是一个包含83个真实世界表格回归数据集的基准，研究发现工程数据与非工程数据存在部分可区分性，而标准程序生成的数据集与工程数据集差异显著。通过嵌入引导的合成数据筛选方法，仅使用选定的合成任务对TabPFN 2.5进行持续预训练，在35个工程回归数据集上提升了预测精度和数据效率。


<details>
  <summary>Details</summary>
Motivation: 工程应用中的预测建模长期受限于定制化模型和小型、孤立的表格数据集，限制了大规模学习方法的应用。尽管表格基础模型取得进展，但其预训练使用的合成训练分布可能无法反映工程数据的统计结构，限制了向工程回归任务的迁移。

Method: 1) 引入TREDBench基准，包含83个真实世界表格回归数据集，带有工程/非工程专家标签；2) 使用TabPFN 2.5的数据集级嵌入研究领域结构；3) 提出嵌入引导的合成数据筛选方法：生成并识别"工程类"合成数据集，仅使用选定的合成任务对TabPFN 2.5进行持续预训练。

Result: 工程数据集与非工程数据集部分可区分，标准程序生成的数据集与工程数据集高度可区分，存在显著的合成-真实领域差距。在35个工程回归数据集上，仅使用合成数据的适应方法提升了预测精度和数据效率：在29/35个数据集上优于TabPFN 2.5，在27/35个数据集上优于AutoGluon，平均数据效率增益分别为1.75倍和4.44倍。

Conclusion: 有原则的合成数据筛选可以将程序生成器转化为领域相关的"数据引擎"，使基础模型能够在数据稀疏的科学和工业领域中改进，这些领域中真实数据收集是主要瓶颈。
...
### [158] [Probabilistic Dreaming for World Models](https://arxiv.org/abs/2603.04715)
*Gavin Wong*

Main category: cs.LG

TL;DR: 该研究改进了Dreamer模型，通过概率方法实现并行探索多个潜在状态，并在MPE SimpleTag环境中取得了4.5%的性能提升和28%的方差降低。


<details>
  <summary>Details</summary>
Motivation: Dreaming技术能让智能体从想象经验中学习，实现更鲁棒和样本高效的世界模型学习。现有Dreamer模型在并行探索多个潜在状态和保持互斥未来假设方面存在局限，需要改进。

Method: 采用概率方法改进Dreamer模型，实现：(1) 并行探索多个潜在状态；(2) 在保持连续潜在变量梯度特性的同时，为互斥的未来维持不同的假设。

Result: 在MPE SimpleTag领域中，该方法比标准Dreamer模型表现更好，获得了4.5%的分数提升和28%更低的回合回报方差。

Conclusion: 提出的概率方法有效提升了Dreamer模型的性能，但存在局限性，未来工作包括研究超参数（如粒子数K）如何随环境复杂度扩展，以及捕捉世界模型中的认知不确定性方法。
...
TL;DR: Count Bridges：一种用于整数计数数据的生成模型，通过随机桥过程实现精确建模，支持从聚合观测中直接训练，在生物计数数据建模和解卷积方面表现优异


<details>
  <summary>Details</summary>
Motivation: 许多生物测量技术（如RNA测序）产生整数计数数据，但这些数据通常是细胞集合的聚合测量，而非单细胞分辨率。现有生成模型（如扩散和流匹配）扩展到非欧几里得和离散设置时，如何最佳建模整数值数据以及如何系统解卷积聚合观测仍不明确。

Method: 提出Count Bridges——一种整数上的随机桥过程，为计数数据提供精确、可处理的扩散式模型，具有闭式条件分布以实现高效训练和采样。扩展该框架通过期望最大化方法直接从聚合测量中训练，将单元级计数视为潜变量。

Result: 在整数分布匹配基准测试中实现最先进性能，优于流匹配和离散流匹配基线。成功应用于两个大规模生物学问题：单细胞基因表达数据的核苷酸分辨率建模（用于解卷积批量RNA-seq），以及将多细胞空间转录组点解析为单细胞计数谱。

Conclusion: Count Bridges为跨尺度和模态的生物计数数据的生成建模和解卷积提供了理论基础，在整数数据建模和聚合观测解卷积方面表现出色。
...
### [160] [Distribution-Conditioned Transport](https://arxiv.org/abs/2603.04736)
*Nic Fishman,Gokul Gowri,Paolo L. B. Fischer,Marinka Zitnik,Omar Abudayyeh,Jonathan Gootenberg*

Main category: cs.LG

TL;DR: DCT是一个分布条件化的传输框架，能够泛化到训练中未见过的源分布和目标分布对，支持半监督学习，适用于多种传输机制。


<details>
  <summary>Details</summary>
Motivation: 科学应用需要能够泛化到训练中未见过的源分布和目标分布的传输模型，传统方法无法处理这种分布对泛化问题。

Method: 提出分布条件化传输（DCT）框架，通过将传输映射条件化到学习到的源分布和目标分布嵌入上，支持从流匹配到分布散度模型（如Wasserstein、MMD）等多种传输机制。

Result: 在合成基准测试和四个生物学应用中展示了DCT的实际性能优势：单细胞基因组学中的批次效应转移、质谱流式细胞术数据的扰动预测、造血过程中克隆转录动力学的学习、T细胞受体序列进化建模。

Conclusion: DCT提供了一个灵活的框架，能够处理分布对泛化问题，支持半监督学习，并在多个实际生物学应用中表现出优越性能。
...
### [161] [KindSleep: Knowledge-Informed Diagnosis of Obstructive Sleep Apnea from Oximetry](https://arxiv.org/abs/2603.04755)
...
<details>
  <summary>Details</summary>
Motivation: 阻塞性睡眠呼吸暂停影响全球近10亿人，显著增加心血管风险。传统多导睡眠图诊断资源密集且可及性有限，需要准确高效的替代方案。

Method: KindSleep首先从原始血氧信号中学习识别临床可解释概念（如去饱和指数和呼吸障碍事件），然后将这些AI衍生概念与多模态临床数据融合，估计呼吸暂停低通气指数。

Result: 在三个大型独立数据集（SHHS、CFS、MrOS，总n=9,815）上，KindSleep在估计AHI评分方面表现出色（R2=0.917，ICC=0.957），在OSA严重程度分类方面始终优于现有方法，加权F1分数从0.827到0.941不等。
...
### [162] [ConTSG-Bench: A Unified Benchmark for Conditional Time Series Generation](https://arxiv.org/abs/2603.04767)
*Shaocheng Lan,Shuqi Gu,Zhangzhi Xiong,Kan Ren*

Main category: cs.LG

TL;DR: 该论文提出了首个条件时间序列生成的标准化基准ConTSG-Bench，包含大规模对齐数据集和全面评估指标，揭示了现有方法的局限性和未来研究方向。
...
Method: 引入ConTSG-Bench基准，包含大规模、对齐良好的数据集，涵盖多样化的条件模态和语义抽象层次，使用全面的生成保真度和条件遵从性指标对代表性生成方法进行系统评估。

Result: 定量基准测试和条件生成行为的深入分析揭示了当前方法的特征和局限性，特别是在复杂条件下的精确结构可控性和下游任务效用方面存在关键挑战。

Conclusion: ConTSG-Bench填补了条件时间序列生成领域的基准空白，为评估生成模型提供了标准化框架，并指出了未来研究的关键方向，特别是复杂条件下的精确控制和实际应用价值。
...
### [163] [Distributional Reinforcement Learning with Information Bottleneck for Uncertainty-Aware DRAM Equalization](https://arxiv.org/abs/2603.04768)
*Muhammad Usama,Dong Eui Chang*

Main category: cs.LG

TL;DR: 本文提出了一种基于分布风险敏感强化学习的均衡器参数优化框架，通过信息瓶颈潜在表示和条件风险价值优化，解决了现有方法计算成本高、缺乏最坏情况保证和不确定性量化的问题。


<details>
  <summary>Details</summary>
Motivation: 高速内存系统中均衡器参数优化对信号完整性至关重要，但现有方法存在三个主要问题：眼图评估计算成本高、优化的是期望性能而非最坏情况性能、缺乏部署决策所需的不确定性量化。

Method: 提出分布风险敏感强化学习框架，集成信息瓶颈潜在表示和条件风险价值优化。使用率失真最优信号压缩实现51倍加速，通过蒙特卡洛dropout量化认知不确定性。分布强化学习结合分位数回归实现显式最坏情况优化，PAC-Bayesian正则化提供泛化边界保证。

Result: 在来自8个内存单元的240万个波形上进行实验验证，4抽头和8抽头均衡器配置的平均改进分别为37.1%和41.5%，最坏情况保证分别为33.8%和38.2%，相比Q学习基线分别提高了80.7%和89.1%。框架实现了62.5%的高可靠性分类，消除了大多数配置的手动验证需求。

Conclusion: 该框架为生产规模的均衡器优化提供了具有认证最坏情况保证的实用解决方案，显著提高了优化效率和可靠性。
...
### [164] [Distributional Equivalence in Linear Non-Gaussian Latent-Variable Cyclic Causal Models: Characterization and Learning](https://arxiv.org/abs/2603.04780)
*Haoyue Dai,Immanuel Albrecht,Peter Spirtes,Kun Zhang*

Main category: cs.LG

TL;DR: 本文针对含隐变量的因果发现问题，在线性非高斯模型下建立了无需结构假设的等价性表征，并提出了相应的发现算法。


<details>
  <summary>Details</summary>
Motivation: 现有隐变量因果发现方法大多依赖强结构假设（如特定的指示器模式或交互限制），缺乏通用的、无需结构假设的方法。核心障碍在于缺乏等价性表征：不知道什么可识别，就无法设计识别方法。

Method: 提出边缘秩约束作为新工具，建立了含任意隐变量结构和循环的图之间的分布等价性图形准则。提供了遍历整个等价类的过程，并开发了从数据中恢复模型（达到等价性）的算法。

Result: 在线性非高斯模型中首次实现了无需结构假设的隐变量等价性表征，这是首个无需结构假设的发现方法。提供了代码和交互演示。

Conclusion: 填补了隐变量因果发现中缺乏通用等价性表征的空白，为无需结构假设的因果发现提供了理论基础和实用工具。
...
### [165] [Diffusion Policy through Conditional Proximal Policy Optimization](https://arxiv.org/abs/2603.04790)
...
Method: 提出了一种新颖高效的方法，通过将策略迭代与扩散过程对齐，仅需评估简单高斯概率即可在on-policy设置中训练扩散策略。该方法还能自然地处理熵正则化，这在扩散策略中通常难以实现。

Result: 实验表明，所提方法能够产生多模态策略行为，并在IsaacLab和MuJoCo Playground的各种基准任务上实现了优越性能。

Conclusion: 该方法解决了扩散策略在on-policy强化学习中的关键挑战，提供了一种计算高效且能处理熵正则化的训练范式，为扩散策略在强化学习中的广泛应用开辟了新途径。
...
### [166] [Missingness Bias Calibration in Feature Attribution Explanations](https://arxiv.org/abs/2603.04831)
...
<details>
  <summary>Details</summary>
Motivation: 现有解释方法常因缺失性偏差产生不可靠的特征重要性分数，传统解决方案需要昂贵的重新训练或架构修改，作者认为这可以更简单地解决

Method: MCal方法：在冻结的基础模型输出上微调一个简单的线性头层，将缺失性偏差视为模型输出空间的表面伪影进行校正

Result: 该方法能持续减少缺失性偏差，在视觉、语言和表格数据的多样化医疗基准测试中，表现与或优于先前重量级方法

Conclusion: 缺失性偏差可以作为模型输出空间的表面伪影有效处理，无需昂贵的重新训练或架构修改，MCal提供了一种轻量级解决方案
...
### [167] [Why Is RLHF Alignment Shallow? A Gradient Analysis](https://arxiv.org/abs/2603.04851)
*Robin Young*

Main category: cs.LG

TL;DR: 论文证明基于梯度的对齐方法天然集中在决定危害的位置，超过危害决策点后梯度消失，导致浅层对齐。作者引入危害信息概念，并提出基于恢复惩罚的新目标来在所有位置产生梯度信号。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全对齐方法存在浅层对齐问题，KL散度集中在早期token，无法实现深度对齐。需要理解梯度对齐的内在局限性，并找到解决方案。

Method: 使用序列级危害的鞅分解，推导出对齐梯度的精确特征：位置t的梯度等于条件期望危害与评分函数的协方差。引入危害信息It量化每个位置对危害的影响，提出基于恢复惩罚的目标函数。

Result: 证明标准对齐目标无法产生深度对齐，因为超过危害决策点的位置梯度为零。均衡KL散度跟踪危害信息It。恢复惩罚目标能在所有位置产生梯度信号。

Conclusion: 梯度对齐方法存在固有局限性，导致浅层对齐。基于恢复惩罚的新目标为数据增强技术提供了理论基础，能够实现更深的对齐效果。
...
### [168] [FedAFD: Multimodal Federated Learning via Adversarial Fusion and Distillation](https://arxiv.org/abs/2603.04890)
...
Method: 客户端采用双级对抗对齐策略对齐本地和全局表示，粒度感知融合模块自适应整合全局知识；服务器端使用相似性引导的集成蒸馏机制，基于特征相似性聚合客户端表示并将融合知识蒸馏到全局模型。
...
### [169] [U-Parking: Distributed UWB-Assisted Autonomous Parking System with Robust Localization and Intelligent Planning](https://arxiv.org/abs/2603.04898)
*Yiang Wu,Qiong Wu,Pingyi Fan,Kezhi Wang,Wen Chen,Guoqiang Mao,Khaled B. Letaief*

Main category: cs.LG

TL;DR: U-Parking是一个分布式超宽带辅助的自主停车系统，通过结合大语言模型规划、融合定位和轨迹跟踪技术，在复杂室内环境中实现可靠的自动停车。


<details>
  <summary>Details</summary>
Motivation: 解决在具有挑战性的室内环境中实现可靠自动停车的需求，特别是在GPS信号受限或不可用的场景下。

Method: 采用分布式架构，集成超宽带(UWB)技术进行辅助定位，结合大语言模型(LLMs)进行路径规划，并融合多种定位技术和轨迹跟踪算法。

Result: 通过真实车辆演示验证了系统在挑战性室内环境中的可靠性和有效性。

Conclusion: U-Parking系统展示了在复杂室内环境中实现自主停车的可行性，为自动驾驶在受限环境中的应用提供了实用解决方案。
...
### [170] [BandPO: Bridging Trust Regions and Ratio Clipping via Probability-Aware Bounds for LLM Reinforcement Learning](https://arxiv.org/abs/2603.04918)
*Yuan Li,Bo Wang,Yufei Gao,Yuqian Yao,Xinyuan Wang,Zhangyue Yin,Xipeng Qiu*

Main category: cs.LG

TL;DR: BandPO提出了一种新的策略优化方法，用动态概率感知的Band操作符替代PPO中的固定裁剪机制，解决了低概率动作更新受限的问题，防止熵崩溃。
...
Method: 提出Band约束策略优化(BandPO)，用Band操作符替代标准裁剪机制。Band将f-散度定义的信任区域投影到动态、概率感知的裁剪区间，通过凸优化问题保证全局最优解，并为特定散度推导闭式解。

Result: 在多种模型和数据集上的广泛实验表明，BandPO始终优于标准裁剪和Clip-Higher方法，同时能稳健地缓解熵崩溃问题。

Conclusion: BandPO通过动态概率感知的信任区域约束，有效解决了PPO中固定裁剪机制导致的探索瓶颈，为LLM强化学习提供了更稳定和高效的优化框架。
...
TL;DR: ∇-Reasoner：一种在推理时通过可微分优化改进LLM生成策略的迭代框架，将一阶梯度优化引入解码过程，显著提升数学推理能力并减少模型调用次数。
...
Method: 提出∇-Reasoner框架，核心是可微分文本优化（DTO），在解码循环中集成对token logits的可微分优化，利用LLM似然和奖励模型的梯度信号来精炼文本表示，并加入拒绝采样和加速设计。

Result: 在具有挑战性的数学推理基准上实现了超过20%的准确率提升，同时相比强基线减少了约10-40%的模型调用次数。
...
### [172] [Uncertainty-aware Blood Glucose Prediction from Continuous Glucose Monitoring Data](https://arxiv.org/abs/2603.04955)
*Hai Siong Tan*
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是将不确定性量化整合到实时机器学习血糖预测系统中，以提高预测可靠性并识别不良血糖事件，这对于1型糖尿病患者的临床管理至关重要。

Method: 使用三种序列模型架构（LSTM、GRU、Transformer），结合两种不确定性量化方法（蒙特卡洛dropout、深度证据回归的证据输出层），在HUPA-UCM糖尿病数据集上进行验证，并使用糖尿病技术学会误差网格评估临床风险。

Result: 基于Transformer的模型配合证据输出头提供了最有效的不确定性感知框架，实现了更高的预测准确性和更好的校准不确定性估计，其不确定性大小与预测误差显著相关。
...
### [173] [WaterSIC: information-theoretically (near) optimal linear layer quantization](https://arxiv.org/abs/2603.04956)
*Egor Lifar,Semyon Savkin,Or Ordentlich,Yury Polyanskiy*

Main category: cs.LG

TL;DR: 该论文提出了一种名为WaterSIC的新算法，用于将密集线性层转换为低精度表示，通过信息论分析压缩长度与输出差异的权衡，并在不同列上分配不同的量化率，显著优于现有GPTQ算法。


<details>
  <summary>Details</summary>
Motivation: 现有GPTQ算法在将密集线性层转换为低精度时，与信息论极限可能存在任意大的差距，需要更接近理论极限的量化方法。

Method: 提出WaterSIC算法，模仿信息论中的"注水"解决方案，为权重矩阵的不同列（输入特征）分配不同的量化率，以更接近信息论极限。

Result: WaterSIC算法在信息论极限的0.255比特速率差距内，优于GPTQ算法。在Llama和Qwen系列LLMs上应用时，在1到4比特的所有量化率上都达到了新的最先进性能。

Conclusion: WaterSIC算法通过信息论指导的列级量化率分配，显著改进了密集线性层的低精度转换，为大型语言模型的高效部署提供了更优的量化解决方案。
...
### [174] [Mixture of Universal Experts: Scaling Virtual Width via Depth-Width Transformation](https://arxiv.org/abs/2603.04971)
...
TL;DR: MOUE提出虚拟宽度作为MoE的新扩展维度，通过跨层复用通用专家池，在固定计算预算下将深度转化为虚拟宽度，相比传统MoE获得1.3%的性能提升
...
Method: 提出MOUE架构，包含三个核心组件：交错旋转拓扑实现结构化专家共享，通用专家负载均衡进行深度感知的暴露校正，通用路由器使用轻量轨迹状态实现连贯多步路由

Result: MOUE在不同扩展规模下持续优于匹配的MoE基线达1.3%，能够渐进转换现有MoE检查点并获得高达4.2%的增益

Conclusion: MOUE为MoE架构揭示了一个新的扩展维度——虚拟宽度，通过跨层复用专家池实现了更好的性能表现
...
### [175] [Asymptotic Behavior of Multi--Task Learning: Implicit Regularization and Double Descent Effects](https://arxiv.org/abs/2603.05060)
*Ayed M. Alrashdi,Oussama Dhifallah,Houssem Sifaou*

Main category: cs.LG

TL;DR: 本文通过精确渐近分析，研究了多任务学习在感知器误设模型中的优势机制，发现多任务组合等价于带额外正则项的传统公式，能改善泛化性能并延缓双下降现象。


<details>
  <summary>Details</summary>
Motivation: 多任务学习通过利用多个相关任务间的共享信息来改善泛化误差，但挑战在于找到能够揭示任务间共同信息的有效公式。本文旨在精确分析多任务学习的优势机制。

Method: 对误设感知器学习模型的多任务公式进行精确渐近分析，证明多任务组合等价于带额外正则项的传统公式，并通过实证研究多任务组合对泛化误差的影响。

Result: 多任务组合在渐近意义上等价于传统公式加上额外的正则化项，这些正则项有助于改善泛化性能。实证研究表明，多任务组合能够延缓双下降现象，并在渐近情况下缓解该现象。

Conclusion: 本文揭示了多任务学习改善泛化性能的机制：通过任务组合引入的隐式正则化效应，能够延缓并缓解双下降现象，从而提升学习模型的泛化能力。
...
### [176] [Reward-Conditioned Reinforcement Learning](https://arxiv.org/abs/2603.05066)
*Michal Nauman,Marek Cygan,Pieter Abbeel*

Main category: cs.LG

TL;DR: RCRL框架让单个智能体能够优化多种奖励函数，仅需在单一目标下收集经验，通过奖励参数化条件化实现多目标学习，提高鲁棒性和适应性
...
Method: 提出奖励条件强化学习(RCRL)框架，通过条件化智能体在奖励参数化上，从共享的离策略回放数据中学习多个奖励目标，使单个策略能够表示奖励特定的行为

Result: 在单任务、多任务和基于视觉的基准测试中，RCRL不仅提高了在名义奖励参数化下的性能，还能高效适应新的参数化，展示了鲁棒性和可适应性

Conclusion: RCRL提供了一种可扩展的机制，用于学习鲁棒、可操控的策略，同时不牺牲单任务训练的简单性，解决了奖励函数错误指定和适应性限制的问题
...
### [177] [Synchronization-based clustering on the unit hypersphere](https://arxiv.org/abs/2603.05067)
...
<details>
  <summary>Details</summary>
Motivation: 单位超球面上的聚类在基因表达分析、文本和图像分类等领域有重要应用，但传统聚类方法没有考虑球面的几何结构，不适合处理这类数据。

Method: 基于d维广义Kuramoto模型开发了一种新的聚类算法，专门用于处理单位球面S^{d-1}上的数据点。

Result: 在合成数据集和真实数据集上的实验表明，该方法在聚类准确度方面达到或优于传统聚类方法。

Conclusion: 基于广义Kuramoto模型的聚类算法能有效处理单位超球面上的数据，考虑了球面几何结构，在多个应用领域具有实用价值。
...
### [178] [Aura: Universal Multi-dimensional Exogenous Integration for Aviation Time Series](https://arxiv.org/abs/2603.05092)
...
<details>
  <summary>Details</summary>
Motivation: 工业应用中时间序列预测需要整合多维甚至多模态的外部因素，但现有单模态时间序列模型难以捕捉这些异质交互作用

Method: 提出Aura通用框架，根据外部信息与目标时间序列的交互模式，采用专门的三部分编码机制将异质特征嵌入到成熟的时间序列模型中

Result: 在中国南方航空公司三年工业数据集上的实验表明，Aura在所有基线方法中均达到最先进性能，并表现出优异的适应性

Conclusion: Aura作为通用增强框架，在航空安全和可靠性方面具有重要应用潜力
...
### [179] [Axiomatic On-Manifold Shapley via Optimal Generative Flows](https://arxiv.org/abs/2603.05093)
...
Method: 提出基于最优生成流的Aumann-Shapley归因理论，证明梯度线积分是满足效率和几何公理的唯一泛函，通过最小化动能Wasserstein-2测地线解决路径模糊性，将基线选择重构为变分问题

Result: 方法在实验中优于基线，实现严格流形对齐（流一致性误差趋零）和优越的语义对齐（结构感知总变差），对加性模型恢复经典Shapley，并提供可证明的稳定性边界

Conclusion: 通过形式化流形上Aumann-Shapley归因理论，解决了传统方法的离流形伪影问题，为可解释AI提供了严格几何基础，将基线选择转化为变分优化问题
...
### [180] [Decoupling Task and Behavior: A Two-Stage Reward Curriculum in Reinforcement Learning for Robotics](https://arxiv.org/abs/2603.05113)
*Kilian Freitag,Knut Åkesson,Morteza Haghir Chehreghani*

Main category: cs.LG

TL;DR: 提出两阶段奖励课程学习框架，将任务目标与行为目标解耦，先训练简化任务奖励，再引入完整奖励，显著提升多目标强化学习性能


<details>
  <summary>Details</summary>
Motivation: 深度强化学习在机器人控制中面临奖励函数设计困难，多目标任务需要精确调整权重以获得期望策略特性

Method: 两阶段奖励课程学习：第一阶段使用简化的任务专用奖励函数训练智能体，确保有效探索；第二阶段引入包含能量效率等辅助行为目标的完整奖励函数，并重用样本以保持训练稳定性
...
### [181] [FedBCD:Communication-Efficient Accelerated Block Coordinate Gradient Descent for Federated Learning](https://arxiv.org/abs/2603.05116)
*Junkang Liu,Fanhua Shang,Yuanyuan Liu,Hongying Liu,Yuangang Li,YunXiang Gong*

Main category: cs.LG

TL;DR: 提出FedBCGD方法，通过参数分块上传降低联邦学习中大规模模型（如Vision Transformer）的通信开销，通信复杂度比现有方法降低1/N倍。
...
Method: 提出FedBCGD方法：1）将模型参数分为多个块（包括共享块）；2）每个客户端只上传特定参数块；3）开发加速版本FedBCGD+，包含客户端漂移控制和随机方差减少技术。

Result: 理论分析显示通信复杂度比现有方法降低1/N倍（N为参数块数），收敛速度更快。实验结果表明优于现有最先进算法。

Conclusion: 首次提出参数块通信方法训练大规模深度模型，显著降低通信开销，为联邦学习中的通信效率问题提供了有效解决方案。
...
### [182] [Federated Causal Discovery Across Heterogeneous Datasets under Latent Confounding](https://arxiv.org/abs/2603.05149)
...
<details>
  <summary>Details</summary>
Motivation: 多数据集因果发现面临数据隐私法规和跨站点异质性的限制，传统方法需要集中式数据集，这在隐私保护环境下不可行

Method: 开发fedCI联邦条件独立性检验，使用联邦迭代重加权最小二乘法估计广义线性模型参数；在此基础上构建fedCI-IOD，扩展集成重叠数据集算法，实现联邦因果发现

Result: fedCI-IOD在保护隐私的同时显著增强统计功效，性能接近完全合并分析，缓解了局部样本量小的伪影问题

Conclusion: 提出的联邦方法有效解决了隐私保护和数据异质性挑战，提供了用户友好的工具包，为分布式异构数据集上的因果发现提供了实用解决方案
...
### [183] [Balancing Privacy-Quality-Efficiency in Federated Learning through Round-Based Interleaving of Protection Techniques](https://arxiv.org/abs/2603.05158)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中平衡隐私保护、学习质量和效率存在挑战：差分隐私会降低学习质量，同态加密带来显著系统开销。需要一种能灵活权衡这三者的解决方案。

Method: 提出Alt-FL框架，结合差分隐私、同态加密和合成数据，采用轮次交错策略。引入三种新方法：隐私交错、带差分隐私的合成交错、带同态加密的合成交错。

Result: 在LeNet-5模型上对CIFAR-10和Fashion-MNIST数据集进行系统评估，对抗多种重构攻击。结果表明：在高隐私保护级别下，隐私交错方法实现最平衡的权衡；在中等隐私需求下，基于差分隐私的方法更优。

Conclusion: Alt-FL框架为不同隐私和资源约束下的隐私保护联邦学习方法选择提供了基础，实现了隐私保护、学习质量和效率的灵活权衡。
...
### [184] [Incentive Aware AI Regulations: A Credal Characterisation](https://arxiv.org/abs/2603.05175)
*Anurag Singh,Julian Rodemann,Rajeev Verma,Siu Lun Chau,Krikamol Muandet*

Main category: cs.LG

TL;DR: 该论文将AI监管建模为不确定条件下的机制设计问题，提出监管机制框架，通过让提供商对模型合规性下注来实现完美市场结果，建立了机制设计与不精确概率之间的对偶关系。
...
Method: 将AI监管建模为机制设计问题，引入监管机制框架：将模型经验证据映射到市场份额许可证，提供商从许可证集中选择，迫使其对模型满足监管要求的能力下注。

Result: 证明机制具有完美市场结果（非合规提供商自我排除且合规提供商参与）当且仅当非合规分布集合构成信度集（封闭凸概率测度集），建立了监管机制与非合规分布集合之间的对偶关系。

Conclusion: 该框架为机制设计与不精确概率的交叉领域提供新见解，为可执行AI监管的发展奠定基础，通过实验展示了在监管预测中虚假特征使用和公平性方面的实际应用。
...
### [185] [Towards a data-scale independent regulariser for robust sparse identification of non-linear dynamics](https://arxiv.org/abs/2603.05201)
*Jay Raut,Daniel N. Wilke,Stephan Schmidt*

Main category: cs.LG

TL;DR: STCV算法通过使用无量纲统计指标替代传统基于幅度的阈值，解决了数据归一化对稀疏回归方法（特别是SINDy框架）发现控制方程的严重扭曲问题，使系统识别对数据缩放具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 数据归一化作为常见的预处理步骤，会严重扭曲基于幅度的稀疏回归方法发现控制方程的能力，特别是在SINDy框架中，数据缩放与测量噪声的相互作用破坏了稀疏性假设，导致模型变得稠密、不可解释且物理不正确。

Method: 提出序列化系数变异阈值（STCV）算法，用无量纲统计指标"系数存在性"（CP）替代传统基于幅度的阈值，CP评估模型库中候选项的统计有效性和一致性，使发现过程对任意数据缩放具有不变性。
...
### [186] [Stable-LoRA: Stabilizing Feature Learning of Low-Rank Adaptation](https://arxiv.org/abs/2603.05204)
*Yize Wu,Ke Gao,Ling Li,Yanjun Wu*

Main category: cs.LG

TL;DR: 本文分析了LoRA（低秩适应）方法的理论稳定性问题，发现非零初始化会破坏特征学习的自稳定性，提出了Stable-LoRA方法通过权重收缩优化策略动态增强稳定性。


<details>
  <summary>Details</summary>
Motivation: 尽管LoRA在实践中有很好的效果，但其理论基础不足，特别是在特征学习稳定性方面缺乏深入理解。研究发现非零初始化会破坏LoRA的自稳定性，导致性能下降。
...
Conclusion: Stable-LoRA有效解决了LoRA特征学习的不稳定性问题，在保持非零初始化优势的同时实现了稳定特征学习，为参数高效微调提供了更可靠的理论和实践基础。
...
### [187] [Early Warning of Intraoperative Adverse Events via Transformer-Driven Multi-Label Learning](https://arxiv.org/abs/2603.05212)
*Xueyao Wang,Xiuding Cai,Honglin Shang,Yaoyao Zhu,Yu Yao*

Main category: cs.LG

TL;DR: 本文提出IAENet框架，通过改进的TAFiLM模块融合静态与动态临床数据，结合LCRLoss处理类别不平衡，在首个多标签术中不良事件数据集MuAE上实现优于基线的早期预警性能。
...
Method: 1. 构建首个多标签术中不良事件数据集MuAE，涵盖六种关键事件；2. 提出基于Transformer的多标签学习框架IAENet，包含改进的TAFiLM模块用于静态协变量和动态变量的鲁棒融合及复杂时间依赖建模；3. 引入带有共现正则化的标签约束重加权损失函数(LCRLoss)，有效缓解事件内不平衡并增强频繁共现事件间的结构化一致性。

Result: 在5、10和15分钟早期预警任务中，IAENet持续优于强基线方法，平均F1分数分别提升+5.05%、+2.82%和+7.57%。这些结果突显了IAENet在支持临床实践中智能术中决策的潜力。

Conclusion: IAENet通过有效融合异质临床数据、建模事件间依赖关系并处理类别不平衡，为术中不良事件早期预警提供了有效的多标签学习框架，有望在临床实践中支持智能决策。
...
### [188] [SlideSparse: Fast and Flexible (2N-2):2N Structured Sparsity](https://arxiv.org/abs/2603.05232)
...
TL;DR: SlideSparse系统首次在商用GPU上为(2N-2):2N稀疏模式（如6:8）提供Sparse Tensor Core加速，通过滑动窗口分解和激活提升技术，在保持精度的同时实现接近理论上限的推理加速。


<details>
  <summary>Details</summary>
Motivation: NVIDIA的2:4稀疏张量核心虽然提供2倍吞吐量，但需要严格的50%剪枝，这会严重损害LLM推理精度。而更温和的(2N-2):2N模式（如6:8，25%剪枝）能保持精度，但缺乏硬件支持，只能回退到密集执行，无法获得稀疏性带来的加速优势。

Method: SlideSparse采用两种核心技术：1）滑动窗口分解：将任意(2N-2):2N权重块重构为N-1个重叠的2:4兼容窗口，无精度损失；2）激活提升：将相应的激活重排融合到每令牌量化中，成本几乎为零。该系统集成到vLLM框架中。

Result: 在计算受限的工作负载下，实测加速比达到1.33倍，接近理论上限N/(N-1)=4/3（对于6:8稀疏度）。在Qwen2.5-7B等模型上验证了(2N-2):2N作为精度保持型LLM加速的实用路径。支持多种GPU、精度和模型家族。

Conclusion: SlideSparse首次实现了商用GPU上对(2N-2):2N稀疏模式的Sparse Tensor Core加速，为LLM推理提供了一种既能保持精度又能获得显著加速的实用解决方案，填补了硬件支持的空白。
...
### [189] [A Behaviour-Aware Federated Forecasting Framework for Distributed Stand-Alone Wind Turbines](https://arxiv.org/abs/2603.05263)
...
Method: 提出两阶段联邦学习框架：第一阶段使用双轮盘选择初始化结合递归自动分割细化，基于长期行为统计对涡轮机进行聚类；第二阶段通过联邦平均算法训练集群特定的LSTM预测模型。

Result: 在丹麦400个独立涡轮机上的实验表明，DRS-auto方法能够发现行为一致的涡轮机组，在保持数据本地性的同时获得有竞争力的预测准确性。行为感知分组始终优于地理分区，并与强大的k-means++基线方法表现相当。

Conclusion: 该框架为异构分布式涡轮机群提供了一种实用的隐私友好解决方案，通过行为感知聚类和联邦学习实现了准确的风电预测，同时保护了数据隐私和本地性。
...
### [190] [Beyond Word Error Rate: Auditing the Diversity Tax in Speech Recognition through Dataset Cartography](https://arxiv.org/abs/2603.05267)
*Ting-Hui Cheng,Line H. Clemmensen,Sneha Das*

Main category: cs.LG

TL;DR: 论文探讨ASR系统评估中WER指标的局限性，提出新的语义指标和样本难度指数来揭示系统性偏见


<details>
  <summary>Details</summary>
Motivation: 当前ASR系统主要使用词错误率(WER)进行评估，但该指标无法捕捉语义保真度，且会掩盖"多样性税"问题——即边缘化和非典型说话者因系统性识别失败而承受的不成比例负担

Method: 1. 系统评估更广泛的非线性和语义指标；2. 引入样本难度指数(SDI)来量化内在人口统计学和声学因素如何驱动模型失败；3. 通过数据制图映射SDI，展示EmbER和SemDist指标如何揭示WER忽略的隐藏系统性偏见和模型间分歧

Result: EmbER和SemDist指标能够暴露WER忽略的隐藏系统性偏见和模型间分歧，SDI能够量化不同因素对模型失败的影响，为模型审计提供了新工具
...
### [191] [Whispering to a Blackbox: Bootstrapping Frozen OCR with Visual Prompts](https://arxiv.org/abs/2603.05276)
*Samandar Samandarov,Nazirjon Ismoiljonov,Abdullah Sattorov,Temirlan Sabyrbayev*

Main category: cs.LG

TL;DR: Whisperer是一个视觉提示框架，通过扩散模型预处理输入图像来增强冻结预训练模型的性能，在文本识别任务上实现了8%的字符错误率降低。
...
Method: 采用扩散模型作为预处理器，通过四阶段训练课程，使用行为克隆技术放大随机发现的改进策略，而不是传统的强化学习。

Result: 在30万张降质合成文本图像数据集上，相比手工基线（如CLAHE），实现了8%绝对（10.6%相对）的字符错误率降低。

Conclusion: 通过向冻结OCR模型"低语"（预处理输入）而非修改其权重，能够有效提升不完美分类器的性能，该方法样本效率高且避免了传统强化学习的缺陷。
...
### [192] [Knowledge Divergence and the Value of Debate for Scalable Oversight](https://arxiv.org/abs/2603.05293)
*Robin Young*

Main category: cs.LG

TL;DR: 该论文首次建立了辩论与RLAIF之间的形式化联系，通过知识分歧的几何参数化证明了辩论优势的闭式解，揭示了辩论在模型知识互补时的必要性。


<details>
  <summary>Details</summary>
Motivation: AI安全领域中的辩论和RLAIF都被提出作为扩展监督的方法，但缺乏形式化框架来关联两者并描述辩论何时具有优势。需要建立理论联系来理解对抗性监督协议的适用条件。

Method: 通过模型表示子空间之间的主角度参数化知识分歧的几何结构，推导辩论优势的精确闭式解。分析三种知识分歧机制（共享、单边、组合），提供存在性结果和负面结果。

Result: 当模型共享相同训练语料时，辩论退化为RLAIF类方法；当模型知识分歧时，辩论优势呈现从二次机制到线性机制的相变。在组合机制中，存在使辩论失效的对抗性激励阈值。

Conclusion: 首次建立了辩论与RLAIF的形式化连接，为理解对抗性监督协议的合理性提供了几何基础，并关联到从具有互补信息的模型中引出潜在知识的问题。
...
### [193] [WavSLM: Single-Stream Speech Language Modeling via WavLM Distillation](https://arxiv.org/abs/2603.05299)
*Luca Della Libera,Cem Subakan,Mirco Ravanelli*

Main category: cs.LG

TL;DR: WavSLM是一个语音语言模型，通过将自监督WavLM表示量化和蒸馏到单一码本中，并优化自回归下一块预测目标，实现了在单一令牌流中联合建模语义和声学信息，无需文本监督或文本预训练。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型展示了简单自回归训练可以实现可扩展和连贯的生成，但将这一范式扩展到语音领域仍然具有挑战性，因为语义和声学信息之间存在纠缠。现有语音语言模型大多依赖文本监督、分层令牌流或复杂的混合架构，偏离了在文本领域已被证明有效的单流生成预训练范式。

Method: WavSLM通过量化和蒸馏自监督WavLM表示到单一码本中，并优化自回归下一块预测目标来训练。该方法在单一令牌流中联合建模语义和声学信息，无需文本监督或文本预训练。

Result: WavSLM在一致性基准测试和语音生成方面取得了有竞争力的性能，同时使用更少的参数、更少的训练数据，并支持流式推理。

Conclusion: WavSLM展示了在语音领域实现类似文本语言模型的简单自回归训练范式的可行性，为语音语言建模提供了一种更简洁有效的解决方案。
...
### [194] [GALACTIC: Global and Local Agnostic Counterfactuals for Time-series Clustering](https://arxiv.org/abs/2603.05318)
*Christos Fragkathoulas,Eleni Psaroudaki,Themis Palpanas,Evaggelia Pitoura*

Main category: cs.LG

TL;DR: GALACTIC是一个统一的框架，首次将局部和全局反事实可解释性结合到无监督时间序列聚类中，通过最小描述长度目标生成稀疏的局部反事实解释和简洁的全局解释摘要。


<details>
  <summary>Details</summary>
Motivation: 现有的时间序列聚类可解释性方法主要基于特征归因或元数据，无法识别使实例跨越聚类边界的转变。虽然反事实解释可以识别改变模型预测所需的最小时间扰动，但主要局限于监督设置。需要一种统一的方法来解释无监督时间序列聚类。

Method: GALACTIC框架包含两个层面：1) 在实例层面，通过考虑目标聚类和底层聚类分配的聚类感知优化目标生成扰动；2) 在聚类层面，提出最小描述长度目标来提取非冗余的全局解释摘要，表征聚类间的转变。证明MDL目标是超模的，可转化为单调子模集函数，从而设计高效的贪心选择算法。

Result: 在UCR Archive上的广泛实验评估表明，GALACTIC相比为问题调整的最先进基线方法，能产生显著更稀疏的局部反事实解释和更简洁的全局摘要。

Conclusion: GALACTIC提供了第一个通过反事实解释聚类时间序列的统一方法，能够有效解释无监督时间序列聚类中的局部和全局模式。
...
### [195] [FairFinGAN: Fairness-aware Synthetic Financial Data Generation](https://arxiv.org/abs/2603.05327)
*Tai Le Quy,Dung Nguyen Tuan,Trung Nguyen Thanh,Duy Tran Cong,Huyen Giang Thi Thu,Frank Hopfgartner*

Main category: cs.LG

TL;DR: FairFinGAN：基于WGAN的框架，通过分类器将公平性约束直接融入训练过程，生成公平的合成金融数据，在保持数据效用的同时减少偏见。


<details>
  <summary>Details</summary>
Motivation: 金融数据集通常存在偏见，可能导致自动化系统做出不公平的决策。需要一种能够生成公平合成金融数据的方法，同时保持数据对下游预测任务的实用性。

Method: 提出FairFinGAN框架，基于Wasserstein GAN架构，通过分类器将公平性约束直接融入训练过程，确保生成的合成数据既公平又保持实用性。

Result: 在五个真实世界金融数据集上评估，与现有基于GAN的数据生成方法相比，该方法在公平性指标上表现更优，且没有显著损失数据效用。

Conclusion: FairFinGAN展示了作为金融应用中偏见感知数据生成工具的潜力，能够在保持数据效用的同时有效减少偏见。
...
### [196] [InfoFlow KV: Information-Flow-Aware KV Recomputation for Long Context](https://arxiv.org/abs/2603.05353)
*Xin Teng,Canyu Zhang,Shaoyi Zheng,Danyang Zhuo,Tianyi Zhou,Shengjie Wang*

Main category: cs.LG

TL;DR: 针对长上下文问答中的检索增强生成，提出基于信息流引导的KV缓存选择性重计算方法，通过注意力范数信号识别关键token，优化推理效率


<details>
  <summary>Details</summary>
Motivation: 长上下文问答中的检索增强生成在推理时面临对大量检索上下文进行预填充的瓶颈。现有方法依赖启发式或表示差异，未能有效建模所选token是否真正影响生成过程

Method: 将选择性KV重计算视为信息流问题，使用查询的注意力范数信号识别语义相关且结构位置能传播信息的token；在推理一致的RoPE几何下计算，重构检索块的位置分配并引入信息流引导的块重排序策略

Result: 在LLM和VLM基准测试中，在可比效率预算下相比先前方法获得一致性能提升

Conclusion: 基于信息流的方法能更有效地识别影响生成的关键token，通过注意力范数信号和位置重构策略优化检索增强生成的推理效率
...
### [197] [Embedded Inter-Subject Variability in Adversarial Learning for Inertial Sensor-Based Human Activity Recognition](https://arxiv.org/abs/2603.05371)
*Francisco M. Calatrava-Nicolás,Shoko Miyauchi,Vitor Fortes Rey,Paul Lukowicz,Todor Stoyanov,Oscar Martinez Mozos*

Main category: cs.LG

TL;DR: 提出一种新颖的深度对抗框架，通过整合主体间变异性的概念来提升人类活动识别的泛化能力，在三个数据集上使用留一主体交叉验证优于先前方法。


<details>
  <summary>Details</summary>
Motivation: 解决人类活动识别中由于主体间变异性导致的模型泛化问题，即不同个体执行相同活动的方式不同，使得模型难以适应新用户。

Method: 提出深度对抗框架，将主体间变异性整合到对抗任务中，鼓励主体不变的特征表示，从而提升HAR分类性能。

Result: 在三个成熟的HAR数据集上使用留一主体交叉验证，性能优于先前方法；对抗任务有效减少了特征空间中不同用户的主体间变异性，且优于先前工作中的对抗任务。

Conclusion: 提出的对抗框架通过整合主体间变异性概念，能够生成主体不变的特征表示，显著提升人类活动识别的泛化能力和分类性能。
...
### [198] [On-Policy Self-Distillation for Reasoning Compression](https://arxiv.org/abs/2603.05433)
*Hejian Sang,Yuanda Xu,Zhengze Zhou,Ran He,Zhipeng Wang,Jiachen Sun*

Main category: cs.LG

TL;DR: OPSDC是一种通过自我蒸馏让推理模型更简洁的方法，仅使用"简洁推理"指令和反向KL散度，就能在保持困难问题推理深度的同时大幅压缩简单问题的推理长度，显著提升准确率并减少57-59%的token使用。


<details>
  <summary>Details</summary>
Motivation: 当前推理模型虽然会"出声思考"，但大部分输出都是噪声和冗余内容，这些不必要的token不仅浪费计算资源，还可能累积错误，影响最终答案的准确性。

Method: OPSDC（On-Policy Self-Distillation for Reasoning Compression）方法的核心思想是：让同一个模型在"简洁推理"指令下生成教师logits，然后在学生自己的推理轨迹上最小化每个token的反向KL散度。该方法不需要真实答案、token预算或难度估计器，纯粹是自我蒸馏。

Result: 在Qwen3-8B和Qwen3-14B模型上，MATH-500数据集上实现了57-59%的token压缩，同时准确率提升了9-16个百分点。在AIME 2024数据集上，14B模型获得了10个百分点的提升，同时压缩了41%的token。

Conclusion: 推理模型产生的很多内容不仅是冗余的，而且是有害的，每个不必要的token都可能累积错误。OPSDC通过自我蒸馏自动实现智能压缩：对简单问题进行激进压缩，同时保留困难问题所需的深度思考，在显著减少计算成本的同时提升模型性能。
...
### [199] [Latent Wasserstein Adversarial Imitation Learning](https://arxiv.org/abs/2603.05440)
...
<details>
  <summary>Details</summary>
Motivation: 传统模仿学习方法需要大量中等至高质量演示数据以及专家动作信息，这些在实际应用中往往难以获得，因此需要开发能够减少数据需求的模仿学习方法

Method: 提出Latent Wasserstein Adversarial Imitation Learning (LWAIL)框架，在动态感知的潜在空间中使用Wasserstein距离进行状态分布匹配。通过预训练阶段训练Intention Conditioned Value Function (ICVF)来获取动态感知的潜在空间表示，仅需少量随机生成的状态数据

Result: 在多个MuJoCo环境中，LWAIL方法优于先前的Wasserstein-based IL方法和对抗性IL方法，仅需一个或几个无动作专家轨迹就能达到专家级性能

Conclusion: LWAIL通过动态感知的潜在空间和Wasserstein距离匹配，显著减少了对专家演示数据的依赖，仅需少量状态轨迹即可实现有效的模仿学习
...
### [200] [SurvHTE-Bench: A Benchmark for Heterogeneous Treatment Effect Estimation in Survival Analysis](https://arxiv.org/abs/2603.05483)
*Shahriar Noroozizadeh,Xiaobin Shen,Jeremy C. Weiss,George H. Chen*

Main category: cs.LG

TL;DR: SurvHTE-Bench是首个针对右删失生存数据的异质性治疗效果估计的综合性基准测试，包含合成、半合成和真实数据集，用于系统评估各种生存HTE方法。


<details>
  <summary>Details</summary>
Motivation: 生存分析中的异质性治疗效果估计在精准医学和个性化政策制定等高风险应用中至关重要，但由于删失、未观察到的反事实和复杂的识别假设，HTE估计面临独特挑战。现有评估方法分散且不一致，需要建立统一的评估标准。

Method: 创建SurvHTE-Bench基准测试，包括：(1)具有已知真实值的模块化合成数据集套件，系统变化因果假设和生存动态；(2)半合成数据集，将真实世界协变量与模拟治疗和结果配对；(3)来自双胞胎研究（已知真实值）和HIV临床试验的真实世界数据集。

Result: 该基准测试首次在不同条件和现实假设违反情况下，对生存HTE方法进行了严格比较，为因果生存方法的公平、可重复和可扩展评估奠定了基础。

Conclusion: SurvHTE-Bench为右删失生存数据的异质性治疗效果估计提供了首个综合性基准测试，建立了公平、可重复和可扩展的评估框架，数据和代码已开源。
...
### [201] [POET-X: Memory-efficient LLM Training by Scaling Orthogonal Transformation](https://arxiv.org/abs/2603.05500)
*Zeju Qiu,Lixin Liu,Adrian Weller,Han Shi,Weiyang Liu*

Main category: cs.LG

TL;DR: POET-X是POET框架的可扩展内存高效变体，通过降低计算成本的正交等价变换，使单张H100 GPU能够训练十亿参数大语言模型，而AdamW在相同设置下会内存不足。


<details>
  <summary>Details</summary>
Motivation: 虽然POET框架提供了强大的训练稳定性，但其原始实现由于密集矩阵乘法导致高内存消耗和计算开销，限制了大规模语言模型训练的效率。

Method: POET-X通过显著降低计算成本的正交等价变换，在保持POET泛化和稳定性优势的同时，提高了吞吐量和内存效率。

Result: POET-X能够在单张Nvidia H100 GPU上预训练十亿参数的大语言模型，而标准优化器如AdamW在相同设置下会出现内存不足的问题。

Conclusion: POET-X是一个可扩展且内存高效的训练框架，解决了大规模语言模型训练中的内存和计算瓶颈问题，同时保持了训练稳定性。
...
### [202] [SkillNet: Create, Evaluate, and Connect AI Skills](https://arxiv.org/abs/2603.04448)
...
TL;DR: SkillNet是一个开放的AI技能基础设施，通过统一本体论创建、评估和组织AI技能，包含20万+技能库，能显著提升智能体性能（奖励提升40%，执行步骤减少30%）。


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体虽然能灵活调用工具执行复杂任务，但缺乏系统性的技能积累和迁移机制，导致频繁"重复造轮子"，无法有效利用先前策略。
...
### [203] [Capability Thresholds and Manufacturing Topology: How Embodied Intelligence Triggers Phase Transitions in Economic Geography](https://arxiv.org/abs/2603.04457)
*Xinmin Fang,Lingfeng Tao,Zhengxiong Li*

Main category: cs.AI

TL;DR: 本文提出"具身智能经济学"，认为具身AI能力突破将颠覆百年不变的福特式制造范式，引发制造业经济地理的相变，从集中式大规模工厂转向需求邻近的微型制造。


<details>
  <summary>Details</summary>
Motivation: 自1913年福特流水线以来，制造业基本拓扑结构从未发生范式级变革。过去一个世纪的所有重大创新（从丰田生产系统到工业4.0）都在福特范式内优化，未改变其结构逻辑：靠近劳动力市场的集中式巨型工厂、大规模生产。本文认为具身智能将打破这一百年停滞。

Method: 定义能力空间C=(d,g,r,t)，其中d=灵巧性，g=泛化能力，r=可靠性，t=触觉-视觉融合。当能力向量跨越临界表面时，选址目标函数发生拓扑重组。通过三条路径分析：重量反转、批次崩溃、人-基础设施解耦，并引入"机器气候优势"概念。

Result: 具身智能使需求邻近的微型制造成为可能，消除"制造荒漠"，逆转由劳动力套利驱动的地理集中。一旦移除人工，最优工厂位置将由机器最优条件（低湿度、高辐照、热稳定性）决定，这些因素与传统选址逻辑正交，创造无历史先例的生产地理格局。

Conclusion: 本文建立"具身智能经济学"，研究物理AI能力阈值如何重塑生产的空间和结构逻辑。具身智能不仅使现有工厂更高效，更重要的是触发制造业经济地理本身的相变，从根本上改变工厂选址、供应链组织和生产规模的定义。
...
### [204] [Progressive Refinement Regulation for Accelerating Diffusion Language Model Decoding](https://arxiv.org/abs/2603.04514)
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在统一细化规则下对所有token进行迭代去噪，但实践中token以不同速率稳定，导致大量冗余细化，需要更精细的细化控制机制

Method: 提出渐进细化调节（PRR）框架，从完整解码轨迹中推导token级的经验收敛进度信号，基于此信号学习轻量级token级控制器，通过基于温度的分布整形来调节细化，采用渐进自演化训练方案

Result: 实验表明PRR能显著加速扩散语言模型解码，同时保持生成质量

Conclusion: PRR框架通过基于轨迹的渐进式细化控制，有效解决了扩散语言模型中token收敛速率不同导致的冗余细化问题，实现了高效解码
...
### [205] [Discovering mathematical concepts through a multi-agent system](https://arxiv.org/abs/2603.04528)
*Daattavya Aggarwal,Oisin Kim,Carl Henrik Ek,Challenger Mishra*

Main category: cs.AI

TL;DR: 提出一个基于多智能体的计算数学发现模型，通过实验、证明尝试和反例的相互作用自主发现数学概念，并以从多面体数据中恢复同调概念为基准任务验证了系统有效性。


<details>
  <summary>Details</summary>
Motivation: 数学概念通常通过实验、证明尝试和反例的相互作用而出现，但现有计算系统缺乏这种动态过程。作者旨在开发一个能够自主进行数学发现的多智能体系统，通过模拟人类数学家的思考过程来发现新的数学概念。

Method: 提出一个新的多智能体模型，系统能够自主提出猜想并尝试证明它们，通过反馈和演化数据分布做出决策。系统以欧拉多面体猜想为灵感，以从多面体数据和线性代数知识中自主恢复同调概念为基准任务进行测试。

Result: 系统成功完成了从多面体数据中恢复同调概念的学习任务。消融实验通过统计测试验证了完整动态过程的价值，结果表明优化局部过程的正确组合可以产生与数学趣味性惊人一致的概念。

Conclusion: 优化局部过程的正确组合能够产生与数学趣味性良好对齐的概念，这为计算数学发现提供了新的多智能体模型，展示了自主数学概念发现的可行性。
...
### [206] [Adaptive Memory Admission Control for LLM Agents](https://arxiv.org/abs/2603.04549)
*Guilin Zhang,Wei Jiang,Xiejiashan Wang,Aisha Behr,Kai Zhao,Jeffrey Friedman,Xu Chu,Amine Anoun*

Main category: cs.AI

TL;DR: A-MAC框架通过结构化决策控制LLM智能体的长期记忆准入，将记忆价值分解为五个可解释因素，实现透明高效的内存管理，在LoCoMo基准上F1达到0.583，延迟降低31%。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体的长期记忆系统缺乏控制机制，要么积累大量包含幻觉或过时信息的对话内容，要么依赖不透明、完全由LLM驱动的昂贵且难以审计的内存策略，导致记忆准入成为智能体架构中定义不清且控制薄弱的环节。
...
Result: 在LoCoMo基准测试中，A-MAC实现了优越的精确率-召回率权衡，F1分数达到0.583，相比最先进的LLM原生内存系统延迟降低31%。消融实验表明内容类型先验是可靠记忆准入的最具影响力因素。

Conclusion: 显式和可解释的准入控制是构建可扩展且可靠的LLM智能体内存系统的关键设计原则，A-MAC框架通过结构化决策实现了透明高效的内存管理。
...
### [207] [Self-Attribution Bias: When AI Monitors Go Easy on Themselves](https://arxiv.org/abs/2603.04582)
*Dipika Khullar,Jack Hopkins,Rowan Wang,Fabien Roger*

Main category: cs.AI

TL;DR: 研究发现语言模型在监控自身行为时存在"自我归因偏差"：当模型评估自己先前生成的动作时，会比评估相同但由用户呈现的动作时更宽容，导致风险评估不足。


<details>
  <summary>Details</summary>
Motivation: 当前智能体系统依赖语言模型监控自身行为（如代码审查、工具使用安全评估），但现有评估方法通常在固定示例上进行，而非模型自身生成的动作，可能导致实际部署中监控系统不可靠。

Method: 通过对比实验设计，研究模型在不同归因条件下的评估表现：1）动作出现在先前助手轮次（自我生成）vs 2）相同动作出现在用户轮次（新上下文）。在四个编码和工具使用数据集上测试。

Result: 发现明显的自我归因偏差：当评估自己先前生成的动作时，模型更少报告高风险或低正确性动作；而明确说明动作来源不会单独引发此偏差。固定示例评估会高估监控系统的可靠性。

Conclusion: 智能体系统的监控评估需要在模型自身生成的动作上进行，而非仅依赖固定示例，否则开发者可能部署不可靠的监控系统。自我归因偏差是实际部署中的重要风险。
...
Conclusion: 该工作为未来自动化风险分析范式奠定了基础，通过结合生成式AI与人类监督来解决当前方法的局限性
...
### [209] [Using Vision + Language Models to Predict Item Difficulty](https://arxiv.org/abs/2603.04670)
*Samin Khan*

Main category: cs.AI

TL;DR: 研究探索大语言模型评估数据可视化素养测试题目难度的能力，发现多模态方法（结合视觉和文本特征）预测效果最佳
...
Method: 使用GPT-4.1-nano分析测试题目，基于三种不同特征集预测题目难度：纯文本特征（题目和选项）、纯视觉特征（可视化图像）、以及多模态特征（结合两者）

Result: 多模态方法表现最佳，平均绝对误差为0.224，优于纯视觉方法（0.282）和纯文本方法（0.338）。在外部测试集上，最佳多模态模型的均方误差为0.10805

Conclusion: 大语言模型在心理测量分析和自动化题目开发方面具有潜力，多模态方法结合视觉和文本特征能更准确地预测数据可视化素养测试题目的难度
...
### [210] [Model Medicine: A Clinical Framework for Understanding, Diagnosing, and Treating AI Models](https://arxiv.org/abs/2603.04722)
*Jihoon Jeong*
...
<details>
  <summary>Details</summary>
Motivation: 当前AI可解释性研究主要停留在"解剖观察"层面，而复杂AI系统需要更系统化的临床实践方法。作者希望弥合这一差距，建立类似医学的完整学科体系来理解、诊断、治疗和预防AI模型中的问题。

Method: 1) 建立包含15个子学科的学科分类体系；2) 提出基于720个智能体和24,923个决策的"四壳模型"行为遗传学框架；3) 开发开源诊断工具"神经MRI"；4) 设计五层诊断框架；5) 开发临床模型科学工具如模型气质指数、模型症状学和标准化病例报告系统。

Result: 成功建立了完整的模型医学研究体系，包括学科分类、行为解释框架、诊断工具和临床实践方法。神经MRI工具通过四个临床案例验证了其成像、比较、定位和预测能力。

Conclusion: 模型医学为AI系统提供了类似医学的完整研究范式，能够系统性地理解、诊断和治疗AI模型问题。该领域有望成为AI可解释性研究的重要发展方向，为复杂AI系统的可靠部署提供支持。
...
### [211] [Solving an Open Problem in Theoretical Physics using AI-Assisted Discovery](https://arxiv.org/abs/2603.04735)
*Michael P. Brenner,Vincent Cohen-Addad,David Woodruff*

Main category: cs.AI

TL;DR: AI系统结合神经符号方法与树搜索框架，自主解决了宇宙弦引力波功率谱的开放数学问题，推导出精确解析解
...
Result: 成功推导出宇宙弦引力波功率谱核心积分I(N,α)的精确解析解，最优雅的方法使用Gegenbauer多项式展开核函数，大N渐近结果与数值计算一致并与量子场论连续Feynman参数化连接

Conclusion: 人工智能能够自主加速数学发现，神经符号系统结合树搜索框架可有效解决理论物理中的复杂开放问题，为AI辅助科学研究提供了透明可复现的方法论
...
### [212] [Interactive Benchmarks](https://arxiv.org/abs/2603.04737)
*Baoqing Yue,Zihan Zhu,Yifan Zhang,Jichen Feng,Hufei Yang,Mengdi Wang*

Main category: cs.AI

TL;DR: 论文提出交互式基准测试框架，通过模型在预算约束下的交互推理能力来评估模型智能，包括逻辑数学的交互证明和策略游戏的交互游戏两个场景。


<details>
  <summary>Details</summary>
Motivation: 传统基准测试因饱和、主观性和泛化能力差而不可靠，需要评估模型主动获取信息的能力来更准确地衡量模型智能。

Method: 提出交互式基准测试统一评估范式，在预算约束下通过交互过程评估模型推理能力。具体实现两个设置：1）交互证明：模型与裁判交互推导逻辑数学中的客观真理；2）交互游戏：模型进行策略推理以最大化长期效用。

Result: 交互式基准测试提供了对模型智能的稳健和忠实评估，结果显示在交互场景中模型仍有很大改进空间。

Conclusion: 交互式基准测试是评估模型智能的有效框架，揭示了当前模型在交互推理能力上的不足，为未来研究提供了重要方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统顺序LLM智能体在处理具有预算和多样性等硬约束的长时程规划任务时，随着规划进程和上下文增长，会逐渐偏离全局约束，导致规划失败。

Method: 提出分层多智能体框架：1) 协调器负责跨天资源分配；2) 日执行器并行独立规划；3) 事务监控器强制执行预算和唯一性约束；4) 协商协议允许智能体拒绝不可行子目标并触发重新规划；5) 使用GRPO训练的单策略通过角色条件化驱动所有智能体。

Result: 在TravelPlanner上，使用Qwen3-8B的HiMAP-Travel达到52.78%验证集和52.65%测试集最终通过率(FPR)。在相同模型、训练和工具条件下，比顺序DeepTravel基线提升8.67个百分点，超越ATLAS 17.65个百分点和MTP 10.0个百分点。在FlexTravelBench多轮场景中，达到44.34%(2轮)和37.42%(3轮)FPR，并通过并行化降低延迟2.5倍。

Conclusion: HiMAP-Travel通过分层多智能体架构有效解决了长时程规划中的约束漂移问题，实现了更好的规划性能和效率，为复杂约束规划任务提供了有效解决方案。
...
### [214] [Breaking Contextual Inertia: Reinforcement Learning with Single-Turn Anchors for Stable Multi-Turn Interaction](https://arxiv.org/abs/2603.04783)
*Xingwu Chen,Zhanqiu Zhang,Yiwen Guo,Difan Zou*

Main category: cs.AI

TL;DR: 论文提出RLSTA方法解决LLMs在多轮对话中的"上下文惯性"问题，通过单轮锚点提供奖励信号，显著提升多轮交互性能
...
Method: 提出RLSTA（基于单轮锚点的强化学习）训练方法，利用模型在单轮场景下的优越能力作为稳定内部锚点提供奖励信号，通过将多轮响应与这些锚点对齐，使模型能够打破上下文惯性并基于最新信息进行自我校准。

Result: RLSTA显著优于标准微调和基于弃权的方法，表现出强大的跨领域泛化能力（如从数学到代码），且无需外部验证器即可有效工作，展示了在通用领域应用中的潜力。

Conclusion: RLSTA是一种通用训练方法，能够稳定多轮交互，解决LLMs的上下文惯性问题，通过利用单轮能力作为锚点，使模型能够有效整合新信息并自我校准推理过程。
...
### [215] [Timer-S1: A Billion-Scale Time Series Foundation Model with Serial Scaling](https://arxiv.org/abs/2603.04791)
*Yong Liu,Xingjian Su,Shiyu Wang,Haoran Zhang,Haixuan Liu,Yuxuan Wang,Zhou Ye,Yang Xiang,Jianmin Wang,Mingsheng Long*

Main category: cs.AI

TL;DR: Timer-S1是一个8.3B参数的混合专家时间序列基础模型，采用序列缩放策略，通过TimeBench数据集和序列标记预测目标实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 解决现有预训练时间序列基础模型的可扩展性瓶颈，改进长期预测能力，避免标准下一个标记预测中的滚动推理和误差累积问题
...
Conclusion: Timer-S1通过创新的序列缩放策略和高质量数据集，成功解决了时间序列基础模型的可扩展性问题，实现了卓越的预测性能，并将开源促进进一步研究
...
### [216] [EchoGuard: An Agentic Framework with Knowledge-Graph Memory for Detecting Manipulative Communication in Longitudinal Dialogue](https://arxiv.org/abs/2603.04815)
*Ratna Kandala,Niva Manchanda,Akshata Kishore Moharir,Ananth Kandala*

Main category: cs.AI

TL;DR: EchoGuard是一个基于知识图谱的智能体AI框架，通过结构化记忆和模式识别帮助用户识别操纵性沟通模式。


<details>
  <summary>Details</summary>
Motivation: 现有的智能体AI系统缺乏结构化、长期的记忆来追踪操纵性沟通（如煤气灯效应、情感勒索等）的微妙、上下文相关的策略，主要受限于有限的上下文窗口和灾难性遗忘问题。

Method: 采用知识图谱作为智能体的核心情景和语义记忆，实施结构化的"记录-分析-反思"循环：1) 用户记录互动，系统将其构建为个人情景知识图谱；2) 执行复杂图查询检测六种心理学基础的操纵模式；3) 基于检测到的模式子图生成有针对性的苏格拉底式提示。

Result: 该框架展示了智能体架构与知识图谱之间的相互作用如何帮助个人识别操纵性沟通，同时保持个人自主性和安全性。

Conclusion: EchoGuard通过结合知识图谱的结构化记忆和智能体架构，为解决识别操纵性沟通的挑战提供了理论基础、框架设计和评估策略。
...
### [217] [VISA: Value Injection via Shielded Adaptation for Personalized LLM Alignment](https://arxiv.org/abs/2603.04822)
*Jiawei Chen,Tianzhuo Yang,Guoxi Zhang,Jiaming Ji,Yaodong Yang,Juntao Dai*

Main category: cs.AI

TL;DR: VISA框架通过闭环系统解决LLM微调中的价值对齐税问题，在保持语义完整性的同时实现细粒度价值控制，显著优于传统微调和提示方法
...
Method: 提出VISA框架，包含高精度价值检测器、语义-价值转换器和核心价值重写器，使用GRPO和复合奖励函数训练，平衡价值精度和语义完整性

Result: VISA能够精确控制模型的价值表达，同时保持事实一致性和通用能力，显著优于标准微调方法和基于提示的基线（包括GPT-4o）

Conclusion: VISA框架有效缓解了对齐税问题，在保持原始知识忠诚度的同时实现了细粒度价值对齐，为LLM价值控制提供了新方法
...
### [218] [Design Behaviour Codes (DBCs): A Taxonomy-Driven Layered Governance Benchmark for Large Language Models](https://arxiv.org/abs/2603.04837)
*G. Madan Mohan,Veena Kiran Nambiar,Kiranmayee Janardhan*

Main category: cs.AI

TL;DR: 本文介绍了动态行为约束（DBC）基准，这是首个评估在推理时应用于大语言模型的结构化行为治理层（MDBC系统）有效性的实证框架，相比基础模型能显著降低风险暴露率。
...
Method: 开发了包含150个控制项的MDBC系统作为结构化行为治理层，采用三臂对照设计（基础模型、基础+标准安全提示、基础+DBC），在30个风险领域通过5种对抗攻击策略进行红队测试评估。

Result: DBC层将总体风险暴露率从7.19%降至4.55%（相对风险降低36.8%），MDBC依从性得分从8.6/10提升至8.7/10，欧盟AI法案合规性达到8.5/10，完整性保护集群显示出最高的每领域风险降低效果。

Conclusion: DBC框架作为推理时治理层能有效降低大语言模型风险，提供了一种可审计、可复现的系统性治理方法，为模型安全评估和合规性提供了实用工具。
...
### [219] [On Multi-Step Theorem Prediction via Non-Parametric Structural Priors](https://arxiv.org/abs/2603.04852)
*Junbo Zhao,Ting Zhang,Can Li,Wei He,Jingdong Wang,Hua Huang*

Main category: cs.AI

TL;DR: 该论文提出了一种基于上下文学习的免训练定理预测方法，通过定理优先图解决结构漂移问题，在FormalGeo7k基准上达到89.29%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号方法依赖监督参数模型，对演化定理库泛化能力有限。传统上下文学习在多步推理中存在结构漂移问题，性能随推理深度增加急剧下降。

Method: 提出定理优先图，从历史解轨迹编码时间依赖关系作为有向图，施加显式拓扑约束剪枝搜索空间。结合检索增强图构建和逐步符号执行器，使LLM无需梯度优化即可作为结构化规划器。

Result: 在FormalGeo7k基准测试中达到89.29%的准确率，显著优于上下文学习基线，与最先进的监督模型性能相当。
...
### [220] [Causally Robust Reward Learning from Reason-Augmented Preference Feedback](https://arxiv.org/abs/2603.04861)
*Minjune Hwang,Yigit Korkmaz,Daniel Seita,Erdem Bıyık*

Main category: cs.AI

TL;DR: ReCouPLe框架使用自然语言解释来增强基于偏好的奖励学习，通过语言解释提供因果信号，避免模型学习虚假特征，提高奖励模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 基于偏好的奖励学习容易受到因果混淆的影响，学习的奖励往往会捕捉到与偏好轨迹共现的虚假特征，当这些相关性在测试时消失或反转时，模型性能会崩溃。

Method: ReCouPLe是一个轻量级框架，将自然语言解释视为嵌入空间中的指导投影轴，训练模型根据与该轴对齐的特征对轨迹进行评分，同时淡化与所述原因无关的上下文。

Result: ReCouPLe在分布偏移下奖励准确率比基线提高1.5倍，在新任务中的下游策略性能提高2倍。模型能够基于明确的原因来定位偏好，更好地与用户意图对齐，并超越虚假特征进行泛化。

Conclusion: 通过自然语言解释提供因果信号，ReCouPLe有效解决了基于偏好的奖励学习中的因果混淆问题，实现了跨任务的语义知识重用和迁移，提高了模型的泛化能力和对齐性。
...
### [221] [K-Gen: A Multimodal Language-Conditioned Approach for Interpretable Keypoint-Guided Trajectory Generation](https://arxiv.org/abs/2603.04868)
*Mingxuan Mu,Guo Yang,Lei Chen,Ping Wu,Jianxun Cui*

Main category: cs.AI

TL;DR: K-Gen：基于关键点的多模态轨迹生成框架，利用MLLM结合BEV地图和文本描述，通过可解释的关键点生成和轨迹细化，在自动驾驶仿真中优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真中生成真实多样轨迹的挑战：现有基于LLM的方法依赖结构化向量地图，无法捕捉场景的丰富视觉上下文信息。

Method: 提出K-Gen框架：1）使用多模态大语言模型统一栅格化BEV地图和文本描述；2）生成可解释的关键点及其推理（反映智能体意图）；3）通过细化模块将关键点优化为准确轨迹；4）应用轨迹感知强化微调算法T-DAPO增强关键点生成。

Result: 在WOMD和nuPlan数据集上的实验表明，K-Gen优于现有基线方法，证明了多模态推理与关键点引导轨迹生成结合的有效性。

Conclusion: K-Gen通过结合多模态输入和可解释的关键点生成，为自动驾驶仿真提供了更有效的轨迹生成方法，展示了多模态推理在复杂场景理解中的优势。
...
### [222] [SEA-TS: Self-Evolving Agent for Autonomous Code Generation of Time Series Forecasting Algorithms](https://arxiv.org/abs/2603.04873)
...
Method: 提出SEA-TS框架，包含三个核心创新：1) Metric-Advantage蒙特卡洛树搜索，使用归一化优势分数替代固定奖励；2) 代码审查与运行提示优化，自动审查执行方案并更新提示防止类似错误；3) 全局可导向推理，通过对比全局最优和最差方案实现跨轨迹知识转移；采用MAP-Elites存档保持架构多样性。

Result: 在Solar-Energy基准测试中，SEA-TS生成代码相比TimeMixer实现40%的MAE降低；在专有数据集上，太阳能光伏预测WAPE降低8.6%，住宅负荷预测WAPE降低7.7%；负荷预测MAPE达到26.17%，优于TimeMixer的29.34%。

Conclusion: SEA-TS框架通过自主进化不仅超越了现有最先进方法，还发现了新颖的架构模式，证明自动化机器学习工程能够产生超越人工设计的创新算法思想。
...
### [223] [Bounded State in an Infinite Horizon: Proactive Hierarchical Memory for Ad-Hoc Recall over Streaming Dialogues](https://arxiv.org/abs/2603.04885)
*Bingbing Wang,Jing Li,Ruifeng Xu*

Main category: cs.AI

TL;DR: STEM-Bench是首个用于评估无限流对话中记忆能力的基准测试，包含14K+QA对，揭示了保真度与效率的困境。ProStream框架通过主动分层记忆和自适应优化，在保持推理保真度的同时降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现实世界对话通常以无限流的形式展开，需要有限状态记忆机制在无限时间范围内运行。现有的"先读后想"记忆机制与这种设置存在根本性不匹配，无法在流展开时支持即席记忆检索。

Method: 提出了ProStream框架：1）主动分层记忆框架，支持按需即席记忆检索；2）多粒度蒸馏对连续流进行推理；3）自适应时空优化，基于预期效用动态优化记忆保留；4）实现有限知识状态以降低推理延迟。

Result: 实验表明ProStream在准确性和效率方面均优于基线方法。STEM-Bench基准测试包含超过14K个QA对，评估感知保真度、时间推理和全局意识，揭示了关键的保真度-效率困境。

Conclusion: ProStream通过主动分层记忆和自适应优化，解决了无限流对话中的保真度-效率困境，在保持推理保真度的同时实现了有界知识状态和降低的推理延迟。
...
### [224] [Differentially Private Multimodal In-Context Learning](https://arxiv.org/abs/2603.04894)
...
Method: DP-MTV将私有数据划分为不相交的块，应用每层裁剪来限制敏感度，并向聚合结果添加校准噪声，只需单次噪声添加即可支持无限次推理查询。

Result: 在三个VLM架构的八个基准测试中，当ε=1.0时，DP-MTV在VizWiz上达到50%准确率，而非私有方法为55%，零样本方法为35%，在保持有意义的隐私约束下保留了大部分上下文学习的增益。

Conclusion: DP-MTV是首个支持多模态上下文学习的差分隐私框架，能够在保持正式(ε,δ)-差分隐私的同时，实现多轮推理查询，为敏感领域的视觉语言模型应用提供了实用的隐私保护解决方案。
...
### [225] [Authorize-on-Demand: Dynamic Authorization with Legality-Aware Intellectual Property Protection for VLMs](https://arxiv.org/abs/2603.04896)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型的知识产权保护方法依赖静态训练时定义，在动态环境中灵活性不足，对未授权输入产生不透明响应，需要更灵活的保护机制。

Method: 提出AoD-IP框架，包含轻量级动态授权模块支持用户按需指定授权领域，以及双路径推理机制联合预测输入合法性和任务输出。

Result: 在多个跨领域基准测试中，AoD-IP保持了强大的授权领域性能、可靠的未授权检测能力，同时支持用户控制的动态授权部署。

Conclusion: AoD-IP为视觉语言模型提供了灵活、可扩展的知识产权保护方案，能够适应动态环境中的部署需求，优于现有的静态领域方法。
...
### [226] [Alignment Backfire: Language-Dependent Reversal of Safety Interventions Across 16 Languages in LLM Multi-Agent Systems](https://arxiv.org/abs/2603.04904)
*Hiroki Fukui*
...
Method: 采用四个预注册研究，包含1,584个多智能体模拟，覆盖16种语言和三个模型家族。研究1测试英语和日语中的对齐效应；研究2扩展到16种语言并分析文化语言因素；研究3测试个体化作为对策；研究4在不同模型上验证模式。
...
Conclusion: 将对齐重新定义为受风险稳态和医源性效应影响的行为干预。语言空间（训练数据继承的语言、语用和文化属性）结构性决定对齐结果。英语验证的安全无法转移到其他语言，提示级干预无法覆盖语言空间级约束。
...
### [227] [Knowledge-informed Bidding with Dual-process Control for Online Advertising](https://arxiv.org/abs/2603.04920)
*Huixiang Luo,Longyu Gao,Yaqi Liu,Qianqian Chen,Pingchun Huang,Tianning Li*

Main category: cs.AI

TL;DR: KBD方法将人类专家知识融入竞价优化，通过双过程控制结合快速规则和全局优化，解决了传统黑盒模型在数据稀疏、短视决策和分布外场景下的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统在线广告竞价优化依赖黑盒机器学习模型，无法复制人类专家的自适应、经验驱动和全局连贯决策。具体问题包括：数据稀疏情况下泛化能力差、短视的序列决策忽略长期依赖、在分布外场景中适应能力不足。

Method: 提出KBD（知识驱动的双过程控制竞价）方法：1）通过知情机器学习范式将人类专业知识作为归纳偏置嵌入；2）使用决策变换器（DT）全局优化多步竞价序列；3）实现双过程控制，结合基于规则的快速PID系统（系统1）和DT系统（系统2）。

Result: 大量实验表明KBD相比现有方法具有优势，强调了将竞价优化基于人类专业知识和双过程控制的好处。

Conclusion: KBD通过融合人类专业知识和双过程控制机制，有效解决了传统竞价优化方法的局限性，在数据稀疏、长期依赖和分布外场景中表现出色。
...
### [228] [Retrieval-Augmented Generation with Covariate Time Series](https://arxiv.org/abs/2603.04951)
*Kenny Ye Liang,Zhongyi Pei,Huan Zhang,Yuhui Liu,Shaoxu Song,Jianmin Wang*

Main category: cs.AI

TL;DR: RAG4CTS：一种面向协变量时间序列的无需训练、基于状态感知的检索增强生成框架，专门针对数据稀缺、序列短暂、协变量耦合的工业预测性维护场景，显著提升预测精度并成功部署于航空公司实际应用。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列RAG方法主要依赖静态向量嵌入和可学习的上下文增强器，在数据稀缺、序列短暂、协变量耦合的工业场景（如PRSOV压力调节和关闭阀的预测性维护）中难以区分相似状态，需要更有效的解决方案。

Method: 提出RAG4CTS框架：1）构建分层时间序列原生知识库，实现原始历史状态的无损存储和物理信息检索；2）设计两阶段双权重检索机制，通过点级和多变量相似性对齐历史趋势；3）引入智能体驱动的上下文增强策略，以自监督方式动态优化上下文。

Result: 在PRSOV数据集上的大量实验表明，该框架在预测精度上显著优于现有最先进基线。系统已部署于中国南方航空的Apache IoTDB中，部署两个月内成功识别一次PRSOV故障且零误报。

Conclusion: RAG4CTS为协变量时间序列提供了一种有效的无需训练RAG框架，特别适用于数据稀缺、序列短暂、协变量耦合的工业预测性维护场景，在实际部署中表现出优异的性能和可靠性。
...
### [229] [Rethinking Representativeness and Diversity in Dynamic Data Selection](https://arxiv.org/abs/2603.04981)
*Yuzhe Zhou,Zhenglin Hua,Haiyun Guo,Yuheng Jia*

Main category: cs.AI

TL;DR: 提出动态数据选择新框架，重新定义样本评估的代表性和多样性概念，通过稀疏自编码器特征空间评分和使用频率惩罚实现，在保持精度下实现2倍以上训练加速。


<details>
  <summary>Details</summary>
Motivation: 传统动态数据选择方法基于局部几何中心性和子集内分散度来评估样本的代表性和多样性，但这些定义存在局限性。本文重新思考这两个核心概念，提出更有效的评估框架。

Method: 1) 在插件特征空间中评估代表性：使用在目标数据集上训练的稀疏自编码器，通过稀疏单元激活来总结单个样本和数据集级因子统计；2) 过程级多样性实现：结合稀有因子采样和使用频率惩罚，促进样本轮换并减少梯度偏差；3) 二维评分与平滑调度器耦合：从核心模式巩固过渡到稀有因子探索。

Result: 在五个视觉和文本任务基准测试中，该方法在保持或超过全数据精度的同时，实现了超过2倍的训练加速，展示了改进的精度-效率权衡。
...
### [230] [S5-SHB Agent: Society 5.0 enabled Multi-model Agentic Blockchain Framework for Smart Home](https://arxiv.org/abs/2603.05027)
...
<details>
  <summary>Details</summary>
Motivation: 随着智能家居生态系统扩展，面临异构物联网协议、多样化设备和不断演变的威胁，需要自主系统管理舒适度、安全性、能源和居民安全。区块链作为信任基础可提供透明和可问责的治理，但现有框架存在以下问题：1) 依赖固定共识协议的刚性智能合约；2) 最多使用单一AI模型，缺乏多智能体协调；3) 没有让居民控制自动化行为的治理机制。

Method: 提出S5-SHB-Agent框架：1) 使用10个专用智能体，通过可互换的大型语言模型在安全、舒适、能源、隐私和健康等领域做出决策；2) 自适应PoW区块链根据交易量和紧急情况调整挖矿难度，采用数字签名和Merkle树确保防篡改可审计性；3) 四层治理模型让居民通过分层偏好控制自动化，从日常调整到不可变的安全阈值。

Result: 评估证实：1) 居民治理在所有测试配置中正确分离了可调整的舒适度优先级和不可变的安全阈值；2) 自适应共识机制能够提交紧急区块。

Conclusion: S5-SHB-Agent框架成功解决了现有智能家居系统的局限性，通过多智能体协调、自适应区块链共识和居民分层治理，实现了符合Society 5.0原则的人本智能家居治理。
...
### [231] [The Trilingual Triad Framework: Integrating Design, AI, and Domain Knowledge in No-code AI Smart City Course](https://arxiv.org/abs/2603.05036)
*Qian Huang,King Wang Poon*

Main category: cs.AI

TL;DR: 论文提出了"三语三元"框架，解释学生如何通过整合设计、AI和领域知识来学习使用生成式AI进行设计，强调从AI工具使用者转变为AI协作队友的转变。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI快速进入高等教育，学生通常只是被动使用AI生成的输出，而不是主动创建AI驱动的知识工具。研究旨在探索学生如何从使用AI作为工具转变为设计AI作为协作队友。
...
Conclusion: 构建AI系统可以作为一种建构主义学习过程，增强AI素养、元认知和学习者能动性。"三语三元"框架突出了设计、AI和领域知识的整合如何促进人机协作学习。
...
### [232] [Enhancing Zero-shot Commonsense Reasoning by Integrating Visual Knowledge via Machine Imagination](https://arxiv.org/abs/2603.05040)
...
### [233] [Bidirectional Curriculum Generation: A Multi-Agent Framework for Data-Efficient Mathematical Reasoning](https://arxiv.org/abs/2603.05120)
*Boren Hu,Xiao Liu,Boci Peng,Xinping Zhao,Xiaoran Shang,Yun Zhu,Lijun Wu*

Main category: cs.AI

TL;DR: 提出双向课程生成框架，通过多智能体系统动态调整问题复杂度，优化数据使用效率，显著提升大语言模型数学推理能力
...
Method: 引入双向课程生成框架，采用多智能体生态系统模拟自适应教学，建立闭环反馈机制，动态生成数据：复杂化问题挑战模型或简化问题修复特定推理缺陷

Result: 基于最优步调定理优化学习轨迹，显著超越基线方法，在更少指令样本下实现更优的推理性能

Conclusion: 双向课程生成框架能最大化每个训练样本的教学价值，确保模型在任何阶段只消费最有效的数据，解决数据效率瓶颈问题
...
### [234] [MedCoRAG: Interpretable Hepatology Diagnosis via Hybrid Evidence Retrieval and Multispecialty Consensus](https://arxiv.org/abs/2603.05129)
*Zheng Li,Jiayi Xu,Zhikai Hu,Hechang Chen,Lele Cong,Yunyun Wang,Shuchao Pang*

Main category: cs.AI

TL;DR: MedCoRAG是一个用于肝脏疾病诊断的端到端框架，通过多智能体协作检索增强生成技术，结合知识图谱和临床指南，实现可解释的诊断推理。
...
Method: 提出MedCoRAG框架：1）从标准化异常发现生成诊断假设；2）联合检索和修剪UMLS知识图谱路径和临床指南构建患者特异性证据包；3）多智能体协作推理：路由智能体根据病例复杂度动态调度专家智能体，专家智能体迭代推理证据并在需要时触发针对性重新检索，通用智能体将所有审议综合为可追溯的共识诊断。
...
### [235] [X-RAY: Mapping LLM Reasoning Capability via Formalized and Calibrated Probes](https://arxiv.org/abs/2603.05290)
*Gao Tianxi,Cai Yufan,Yuan Yusi,Dong Jin Song*

Main category: cs.AI

TL;DR: X-RAY是一个可解释的推理分析系统，通过形式化验证的探针评估大语言模型的推理能力，发现模型在约束细化时相对稳健，但在解空间重构时性能急剧下降。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注任务级准确性，往往混淆模式匹配与推理能力，需要更精确的方法来理解和评估大语言模型的真实推理能力。

Method: 提出X-RAY系统，将推理能力建模为可提取结构的功能，通过约束交互、推理深度和解空间几何等正式属性进行操作化。使用形式化工具生成具有受控结构变化的探针，通过形式化校准和验证精确隔离增量结构信息。

Result: 评估了最先进的大语言模型在数学、物理和化学领域从初级到高级的问题。分析揭示了LLM推理的系统性不对称：模型对约束细化相对稳健，但在解空间重构下性能急剧下降。校准的形式探针能够区分标准基准上看似无法区分的模型，并揭示结构可解释而非不透明的故障模式。

Conclusion: X-RAY框架不仅提供污染免费的评估，还支持推理模型的训练和测试，为理解大语言模型的推理能力提供了新的分析工具和见解。
...
### [236] [STRUCTUREDAGENT: Planning with AND/OR Trees for Long-Horizon Web Tasks](https://arxiv.org/abs/2603.05294)
*ELita Lobo,Xu Chen,Jingjing Meng,Nan Xi,Yang Jiao,Chirag Agarwal,Yair Zick,Yan Gao*

Main category: cs.AI

TL;DR: STRUCTUREDAGENT是一个用于长视野网页浏览任务的分层规划框架，通过动态AND/OR树进行高效搜索，使用结构化内存跟踪候选解决方案，相比标准LLM智能体显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有网页智能体在复杂长视野任务上表现不佳，主要受限于有限的上下文记忆、弱规划能力以及导致过早终止的贪婪行为。需要解决这些挑战以提升智能体在复杂网页浏览任务中的表现。

Method: 提出STRUCTUREDAGENT分层规划框架，包含两个核心组件：1) 使用动态AND/OR树进行高效搜索的在线分层规划器；2) 跟踪和维护候选解决方案的结构化内存模块，以提升信息搜索任务中的约束满足度。

Result: 在WebVoyager、WebArena和自定义购物基准测试中，STRUCTUREDAGENT相比标准基于LLM的智能体，在长视野网页浏览任务上表现出性能提升。

Conclusion: STRUCTUREDAGENT通过分层规划和结构化内存解决了现有网页智能体的关键限制，不仅提升了任务性能，还生成了可解释的分层计划，便于调试和必要时的人工干预。
...
### [237] [Ailed: A Psyche-Driven Chess Engine with Dynamic Emotional Modulation](https://arxiv.org/abs/2603.05352)
*Diego Armando Resendez Prado*

Main category: cs.AI

TL;DR: 该论文提出了一种将个性与心理状态分解的方法，通过音频信号链处理技术动态调整国际象棋引擎的走棋概率分布，模拟人类棋手在压力下的行为变化。


<details>
  <summary>Details</summary>
Motivation: 传统国际象棋引擎虽然实力超过人类，但无法模拟人类棋手在时间压力下的失误和情绪波动。人类棋手在压力下会犯下业余棋手都不会犯的错误，这种行为变化是现有引擎无法捕捉的。

Method: 提出个性×心理状态分解框架：个性是静态预设，心理状态是动态标量ψ_t ∈ [-100, +100]，每步棋后根据五个位置因素重新计算。两者输入音频信号链（噪声门、压缩器/扩展器、五段均衡器、饱和限制器），实时重塑走棋概率分布。该框架不依赖特定引擎，任何输出概率的系统都适用。

Result: 在12,414局对Maia2-1100的测试中，无论使用训练数据相差约2800倍的概率源，都显示出相同的行为变化模式：过度自信时与原始引擎66%一致，压力下胜率从50.8%降至30.1%。行为变化来自信号链而非底层模型。

Conclusion: 该框架成功模拟了人类棋手在压力下的行为变化（如"上头"和过度自信），但需要强调的是研究未包含人类主体验证。该方法为国际象棋引擎增加了行为可变性，使其更接近人类棋手的真实表现。
...
### [238] [PACE: A Personalized Adaptive Curriculum Engine for 9-1-1 Call-taker Training](https://arxiv.org/abs/2603.05361)
...
<details>
  <summary>Details</summary>
Motivation: 911调度员培训需要掌握上千种相互关联的技能，涵盖多种事件类型和协议细节。全国性劳动力短缺已经对培训能力造成压力，但有效的教学仍然要求培训师根据每个学员不断发展的能力来定制目标。这种个性化负担是当前实践无法扩展的。

Method: PACE（个性化自适应课程引擎）是一个辅助培训师决策的协同系统，它通过：(1) 维护对学员技能状态的概率信念，(2) 建模个体学习和遗忘动态，(3) 推荐平衡新能力获取与现有能力保持的训练场景。系统在结构化技能图上传播证据以加速诊断覆盖，并应用上下文多臂老虎机选择针对学员准备解决的技能差距的场景。

Result: 实证结果显示，PACE相比最先进框架实现了19.50%更快的达到能力时间，以及10.95%更高的最终掌握水平。与执业培训官员的协同研究进一步显示，在真实案例中，PACE与专家的教学判断有95.45%的一致性。在评估下，PACE将周转时间从11.58分钟减少到仅34秒，减少了高达95.08%。
...
### [239] [Legal interpretation and AI: from expert systems to argumentation and LLMs](https://arxiv.org/abs/2603.05392)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI与法律研究方法和范式的演进，法律解释在不同技术路径中呈现出不同的处理方式，需要系统梳理这些方法的特点和进展。

Method: 采用文献综述方法，分析三种主要研究路径：1）专家系统方法关注法律知识工程，确保人类解释能精确转化为知识库；2）论证理论方法关注解释性论证的结构和辩证互动，在论证框架内评估解释主张的可接受性；3）机器学习方法通过通用和专门的语言模型自动生成解释建议和论证。

Result: 识别出AI与法律研究中法律解释的三种主要范式：知识工程范式确保解释的精确性和一致性；论证理论范式关注解释的辩证结构和可接受性；机器学习范式实现解释的自动生成，并已在法律实践中得到应用。

Conclusion: AI与法律研究中的法律解释处理经历了从知识工程到论证理论再到机器学习的演进，不同方法各有侧重，机器学习方法目前正越来越多地应用于法律实践，展现了技术发展的阶段性特征。
...
### [240] [Dissociating Direct Access from Inference in AI Introspection](https://arxiv.org/abs/2603.05414)
...
<details>
  <summary>Details</summary>
Motivation: 内省是基础认知能力但其机制尚不清楚，近期研究表明AI模型能够内省，因此研究其内省机制对于理解认知过程和AI模型工作原理具有重要意义。

Method: 首先广泛复现Lindsey等人（2025）的思想注入检测范式，在大型开源模型中进行研究，分析模型检测注入表示的两种机制：概率匹配和直接访问内部状态。

Result: 发现直接访问机制是内容无关的：模型能检测到异常发生但无法可靠识别其语义内容；研究的两种模型类别会虚构高频且具体的注入概念（如"苹果"），正确概念猜测通常需要显著更多的token。

Conclusion: 这种内容无关的内省机制与哲学和心理学中的主要理论一致，为理解AI模型的内省能力提供了实证基础。
...
### [241] [Distributed Partial Information Puzzles: Examining Common Ground Construction Under Epistemic Asymmetry](https://arxiv.org/abs/2603.05450)
*Yifan Zhu,Mariah Bradford,Kenneth Lai,Timothy Obiso,Videep Venkatesha,James Pustejovsky,Nikhil Krishnaswamy*

Main category: cs.AI

TL;DR: 论文提出了分布式部分信息谜题(DPIP)作为评估AI系统在多模态、多方协作中建立共同基础能力的测试平台，并比较了大型语言模型和基于动态认知逻辑的公理化方法在追踪信念状态方面的表现。
...
Method: 引入分布式部分信息谜题(DPIP)作为协作构建任务，收集多模态交互数据集（语音、手势、动作），并评估两种建模共同基础的方法：1）提示大型语言模型从多模态更新中推断共享信念；2）基于动态认知逻辑的公理化流程。

Result: DPIP对现代大型语言模型追踪任务进展和信念状态的能力构成了挑战，表明当前模型在这方面存在局限性。

Conclusion: 该研究为评估AI系统在多模态协作中建立共同基础的能力提供了新的测试平台和方法论，揭示了当前大型语言模型在追踪复杂信念动态方面的不足。
...
### [242] [Towards Provably Unbiased LLM Judges via Bias-Bounded Evaluation](https://arxiv.org/abs/2603.05485)
*Benjamin Feuer,Lucas Rosenblatt,Oussama Elachqar*

Main category: cs.AI

TL;DR: 本文提出平均偏置有界性(A-BB)框架，为LLM评委系统提供形式化保证，减少可测量偏置带来的危害，在多种偏置设置下保持61-99%的原始排名相关性。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统向自主反馈循环发展，需要自动可验证的奖励机制。在缺乏真实标签或非确定性环境中，LLM作为评委成为实用方案，但现有系统缺乏对未知或对抗性偏置的强保证机制。

Method: 提出平均偏置有界性(A-BB)算法框架，通过形式化方法保证LLM评委系统中任何可测量偏置带来的危害/影响都能被减少。在Arena-Hard-Auto数据集上使用四个LLM评委进行评估。

Result: 在格式化和模式偏置设置下，实现了(tau=0.5, delta=0.01)的偏置有界保证，同时保持61-99%的原始排名相关性，大多数评委-偏置组合超过80%相关性。

Conclusion: A-BB框架为LLM评委系统提供了形式化的偏置控制保证，在保持高相关性的同时有效减少偏置影响，为自主AI系统的可靠反馈机制提供了解决方案。
...
### [243] [The Spike, the Sparse and the Sink: Anatomy of Massive Activations and Attention Sinks](https://arxiv.org/abs/2603.05498)
*Shangwen Sun,Alfredo Canziani,Yann LeCun,Jiachen Zhu*

Main category: cs.AI

TL;DR: 研究发现Transformer语言模型中大规模激活和注意力汇聚两种现象虽然经常同时出现，但功能不同：大规模激活通过产生跨层持续的隐藏表示作为隐式参数，而注意力汇聚则通过调节注意力输出来偏向短程依赖。


<details>
  <summary>Details</summary>
Motivation: 研究Transformer语言模型中经常同时出现的两种现象：大规模激活（少数token在少数通道中出现极端异常值）和注意力汇聚（某些token无论语义相关性如何都吸引不成比例的注意力）。先前工作观察到这两种现象经常同时出现且涉及相同token，但它们的功能作用和因果关系尚不清楚。

Method: 通过系统实验分析两种现象的关系，特别关注预归一化配置的作用，并通过消融实验验证预归一化配置是导致两种现象同时出现的关键因素。

Result: 两种现象的同时出现主要是现代Transformer架构设计的人工产物，它们服务于相关但不同的功能。大规模激活通过产生跨层持续的隐藏表示作为隐式参数，而注意力汇聚则通过调节注意力输出来偏向短程依赖。预归一化配置是导致两种现象同时出现的关键选择，消融该配置会使两种现象解耦。

Conclusion: Transformer中的大规模激活和注意力汇聚是两种相关但功能不同的现象，它们的同时出现是预归一化架构设计的结果。理解这两种现象的分离有助于更好地设计Transformer架构。
...
### [244] [SearchGym: A Modular Infrastructure for Cross-Platform Benchmarking and Hybrid Search Orchestration](https://arxiv.org/abs/2603.04402)
*Jerome Tze-Hou Hsu*
...
<details>
  <summary>Details</summary>
Motivation: 当前RAG工具包虽然快速增长，但实验原型与生产就绪系统之间存在根本差距，需要解决可重复性、模块化和基准测试的问题。

Method: 将数据表示、嵌入策略和检索逻辑解耦为状态化抽象（Dataset、VectorSet、App），通过组合配置代数实现系统合成，分析混合检索中的"Top-k认知"问题。

Result: 在LitSearch专家标注基准上达到70%的Top-100检索率，揭示了通用性与可优化性之间的设计张力，工程优化可作为发现跨领域信息检索因果机制的工具。

Conclusion: SearchGym提供了一个模块化基础设施，支持跨平台基准测试和混合搜索编排，通过解耦设计和组合配置实现了可重复性，为RAG系统从原型到生产提供了工程框架。
...
### [245] [Signal in the Noise: Decoding the Reality of Airline Service Quality with Large Language Models](https://arxiv.org/abs/2603.04404)
*Ahmed Dawoud,Osama El-Shamy,Ahmed Habashy*

Main category: cs.IR

TL;DR: 本研究验证了一个基于大语言模型的框架，用于从非结构化在线评论中提取航空服务质量的细粒度洞察，通过对埃及航空和阿联酋航空16,000多条评论的分析，揭示了传统指标无法捕捉的乘客满意度驱动因素。


<details>
  <summary>Details</summary>
Motivation: 传统服务质量指标往往无法捕捉非结构化在线反馈中隐藏的乘客满意度细微驱动因素，需要更有效的方法来提取这些有价值的信息。

Method: 采用多阶段大语言模型框架，分析2016-2025年间埃及航空和阿联酋航空在TripAdvisor上的16,000多条评论，将服务问题分为36个具体类别。

Result: 研究发现埃及航空存在明显的"运营感知脱节"：尽管报告了运营改进，但2022年后乘客满意度急剧下降（评分<2.0）。框架识别出传统指标遗漏的具体驱动因素，包括中断期间的沟通问题和员工行为，并确定了关键旅游市场中的情感侵蚀。
...
### [246] [Still Fresh? Evaluating Temporal Drift in Retrieval Benchmarks](https://arxiv.org/abs/2603.04532)
*Nathan Kuissi,Suraj Subrahmanyan,Nandan Thakur,Jimmy Lin*

Main category: cs.IR

TL;DR: 该研究探讨了时间性语料库漂移对技术领域检索基准FreshStack的影响，通过比较2024年和2025年两个语料库快照，发现大多数查询仍然有效，模型排名变化很小，表明基于演化语料库的检索基准仍能保持可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统信息检索基准遵循Cranfield范式，依赖静态预定义语料库，但技术语料库中的时间性变化（如API弃用、代码重组）可能导致现有基准过时。本研究旨在探究时间性语料库漂移如何影响技术领域检索基准FreshStack。

Method: 研究比较了FreshStack基准在2024年10月和2025年10月的两个独立语料库快照，分析这些快照对LangChain相关查询的影响。通过检查相关文档从LangChain迁移到竞争对手仓库（如LlamaIndex）的情况，评估查询的有效性变化。同时比较检索模型在两个快照上的准确性，并计算模型排名的相关性。

Result: 分析显示，2024年提出的所有查询中，除一个外，其余在2025年语料库中仍完全支持，相关文档从LangChain迁移到了竞争对手仓库。检索模型在两个快照上的准确性比较显示，模型排名仅有微小变化，在Recall@50指标上Kendall τ相关性高达0.978。这表明基于演化时间语料库重新评估的检索基准仍能保持可靠性。

Conclusion: 时间性语料库漂移对技术检索基准的影响有限，大多数查询在语料库演化后仍然有效，模型排名保持高度稳定。这表明基于演化语料库重新评估的检索基准可以保持可靠性，为动态技术领域的检索评估提供了实证支持。研究公开了所有实验材料供社区使用。
...
### [247] [Scaling Laws for Reranking in Information Retrieval](https://arxiv.org/abs/2603.04816)
*Rahul Seetharaman,Aman Bansal,Hamed Zamani,Kaustubh Dhole*

Main category: cs.IR

TL;DR: 首次系统研究重排序器的缩放定律，发现性能遵循可预测的幂律规律，可通过小规模实验准确预测大规模模型性能，节省计算资源


<details>
  <summary>Details</summary>
Motivation: 现有缩放定律主要针对自然语言生成和密集检索等任务，但无法解释多阶段检索系统中重排序阶段的缩放行为。重排序作为检索系统的最终且最具影响力的步骤，其缩放规律尚未被系统研究

Method: 通过跨编码器重排序器的详细案例研究，分析三种流行范式（点对点、配对、列表）在不同模型大小和数据预算下的性能。使用小规模实验预测大规模模型性能

Result: 性能遵循可预测的幂律规律，NDCG、MAP等下游指标表现出可靠的缩放行为并能准确预测，而对比熵和MRR等指标在某些情况下不遵循可预测的缩放规律。成功通过仅训练和评估较小模型（最多4亿参数）准确预测了10亿参数模型的NDCG性能

Conclusion: 建立了重排序的缩放原则，为构建工业级检索系统提供了可操作的见解，展示了通过小规模实验预测大规模模型性能的稳健方法，显著节省计算资源
...
### [248] [Detecting RAG Advertisements Across Advertising Styles](https://arxiv.org/abs/2603.04925)
*Sebastian Heineking,Wilhelm Pertsch,Ines Zelch,Janek Bevendorff,Benno Stein,Matthias Hagen,Martin Potthast*

Main category: cs.IR

TL;DR: 论文研究了在RAG系统中检测LLM生成的广告内容的方法，开发了广告风格分类法，评估了不同检测方法的鲁棒性，发现基于实体识别的模型效果最好且对风格变化具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在RAG系统中生成混合有机内容和广告的能力增强，需要自动检测这些"生成式原生广告"。现有数据集未能反映营销文献中讨论的多样化广告风格，且广告商可能通过改变广告风格来规避检测。

Method: 1) 开发了LLM广告风格分类法，结合明确性和诉求类型两个维度；2) 模拟广告商通过改变广告风格来规避检测的情况；3) 评估多种广告检测方法在这些变化下的鲁棒性，包括基于实体识别的模型、随机森林和SVM等轻量级模型。

Result: 基于实体识别的模型在检测包含广告的响应方面非常有效，并且对广告风格变化具有较好的鲁棒性。轻量级模型（如随机森林和SVM）虽然适合低资源终端设备，但对风格变化较为脆弱，容易受到规避。

Conclusion: 基于实体识别的广告检测方法在效果和鲁棒性方面表现最佳，但轻量级模型对风格变化的脆弱性表明需要进一步研究面向效率的实用方法，以在低资源设备上有效阻止生成式广告。
...
### [249] [Debiasing Sequential Recommendation with Time-aware Inverse Propensity Scoring](https://arxiv.org/abs/2603.04986)
*Sirui Huang,Jing Long,Qian Li,Guandong Xu,Qing Li*

Main category: cs.IR

TL;DR: 该论文提出了一种时间感知逆倾向评分（TIPS）方法，用于解决序列推荐中的选择偏差和曝光偏差问题，通过考虑序列依赖性和时间动态性来更准确地估计用户偏好。


<details>
  <summary>Details</summary>
Motivation: 现有序列推荐方法主要依赖显式交互（如点击或购买），而忽略了物品曝光信息。这导致了选择偏差（将曝光但未点击的物品误判为不感兴趣）和曝光偏差（将未曝光的物品视为无关）。传统逆倾向评分（IPS）方法是静态的，无法捕捉用户行为的序列依赖性和时间动态性。

Method: 提出了时间感知逆倾向评分（TIPS）方法，与传统静态IPS不同，TIPS能够有效考虑序列依赖性和时间动态性，从而更准确地捕捉用户偏好。该方法可以作为插件应用于各种序列推荐器。

Result: 大量实验表明，TIPS作为插件能够持续提升各种序列推荐器的推荐性能。

Conclusion: TIPS通过考虑序列依赖性和时间动态性，有效解决了序列推荐中的选择偏差和曝光偏差问题，显著提升了推荐性能，并且具有良好的通用性，可作为插件应用于不同的序列推荐模型。
...
### [250] [Core-based Hierarchies for Efficient GraphRAG](https://arxiv.org/abs/2603.05207)
*Jakir Hossain,Ahmet Erdem Sarıyüce*

Main category: cs.IR

TL;DR: 该论文提出用k-core分解替代Leiden聚类来改进GraphRAG，解决稀疏知识图中社区划分不可复现的问题，通过确定性层次结构和轻量级启发式方法提升全局意义构建任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于向量的RAG方法在处理需要跨多个文档推理的全局意义构建任务时表现不佳。GraphRAG通过知识图组织文档，但当前基于Leiden聚类的方法在稀疏知识图上存在非确定性问题，因为模块度优化会产生指数级数量的近似最优划分，导致社区检测结果不可复现。

Method: 提出用k-core分解替代Leiden聚类，k-core能在线性时间内生成确定性的密度感知层次结构。引入一组轻量级启发式方法，利用k-core层次结构构建大小受限、保持连通性的社区用于检索和摘要，同时采用基于token预算的采样策略来降低LLM成本。

Result: 在真实数据集（财务收益电话会议记录、新闻文章、播客）上评估，使用三个LLM生成答案和五个独立LLM评委进行头对头评估。结果表明，该方法在不同数据集和模型上一致提高了答案的全面性和多样性，同时减少了token使用量。

Conclusion: 基于k-core的GraphRAG是一个有效且高效的全局意义构建框架，通过确定性的层次结构和优化策略解决了现有方法在稀疏知识图上的可复现性问题，同时提升了性能并降低了计算成本。
