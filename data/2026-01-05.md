...
Method: 提出生成-重建-引导范式：生成的视频流被连续重建为动态4D时空表示，该表示反过来引导后续生成以保持一致性。采用自回归扩散视频模型，增强宏观-微观规划（MMPL）层次规划方法减少误差累积，结合高效分布匹配蒸馏（DMD）实现实时合成。

Result: TeleWorld在静态和动态世界理解、长期一致性和实时生成效率方面表现出色，实现了动态对象建模和静态场景表示在统一4D框架中的无缝集成。

Conclusion: TeleWorld是向实用、交互式和计算可访问的世界模型迈出的实际步骤，为多模态生成和具身智能提供了交互式、记忆支持的世界模型框架。
...
TL;DR: 本文提出通过噪声优化解决文本到图像模型中的模式崩溃问题，提升生成多样性


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型存在严重的模式崩溃问题，即给定相同文本提示时生成的图像缺乏多样性。虽然已有工作尝试通过引导机制或生成大量候选再筛选来解决，但本文采取不同方向，通过噪声优化来提升生成多样性。

Method: 提出简单的噪声优化目标来缓解模式崩溃，同时保持基础模型的保真度。分析噪声的频率特性，并展示具有不同频率特性的替代噪声初始化可以改善优化和搜索过程。

Result: 实验证明噪声优化在生成质量和多样性方面都能产生优异结果，有效缓解了模式崩溃问题。

Conclusion: 噪声优化是一种简单有效的方法，能够显著提升文本到图像模型的生成多样性，同时保持生成质量，为解决模式崩溃问题提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 人类天生具备4D空间智能（感知物体随时间变化的能力），但现有空间智能基准测试要么规模小，要么多样性有限。本文旨在评估多模态大语言模型是否能达到人类水平的4D空间智能，并为此创建了一个全面、结构化的评估基准。

Method: 构建了Spatial4D-Bench基准测试，包含约40,000个问答对，覆盖18个明确定义的任务。这些任务被系统地组织为六个认知类别：物体理解、场景理解、空间关系理解、时空关系理解、空间推理和时空推理。

Result: 对多种最先进的开源和专有多模态大语言模型进行基准测试，发现它们在多种4D空间推理方面存在显著局限性，如路径规划、动作识别和物理合理性推理等。

Conclusion: Spatial4D-Bench为评估多模态大语言模型的空间认知能力提供了一个结构化、全面的基准测试。研究发现现有模型在4D空间推理方面仍有很大不足，希望该基准能促进开发更接近人类水平4D空间智能的模型。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶视觉系统大多将每个位置视为首次访问，忽略了历史遍历数据中蕴含的宝贵空间先验信息。人类驾驶员很少去无人去过的地方，大多数自动驾驶系统的部署区域都已被访问过，但现有系统未能有效利用这些历史信息。

Method: 提出压缩地图先验（CMP）框架，通过二值化哈希图从历史遍历中学习空间先验。该方法存储需求极低（32KB/km²），比密集存储减少20倍，并能以极小的计算成本轻松集成到领先的3D感知系统中。

Result: 在nuScenes数据集上，CMP框架显著且一致地提升了多种架构的3D目标检测性能，证明了利用历史遍历数据的有效性。

Conclusion: 压缩地图先验是一种简单有效的框架，能够从历史遍历中学习空间先验，以极低的存储和计算成本显著提升自动驾驶3D感知系统的性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态AI在信贷风险评估和文档审查中的广泛应用，迫切需要能够反映金融信贷特定文档和工作流程、包含信贷特定理解能力并保持隐私合规性的领域专用基准测试。

Method: 通过封闭的合成-捕获流水线构建样本：手动合成带有虚拟内容的文档模板，并在内部捕获场景感知图像。评估框架包含感知、推理和鲁棒性三个维度，包括3个基础感知任务、4个信贷特定推理任务和10种真实世界采集伪影类型。

Result: 在评估的23个最先进视觉语言模型中，Gemini 3 Pro作为商业模型获得最佳F1分数(64.61%)，Qwen3-VL-235B作为开源基线获得最佳分数(57.27%)，而专门针对金融信贷的Qfin-VL-Instruct模型获得最高总体分数(64.92%)。鲁棒性评估显示即使表现最佳的模型在采集伪影下也会出现明显性能下降。

Conclusion: FCMBench-V1.0能够有效区分现代视觉语言模型的性能差异和鲁棒性，为金融信贷领域的多模态AI评估提供了专门的基准测试工具，同时解决了隐私合规和实用性的平衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前面部分析研究缺乏对任意选定面部区域的多属性自然语言描述生成与识别能力，系统聚焦于个体面部区域的能力有助于更好地理解和控制面部状态分析。

Method: 构建了新的多属性描述数据集，提供丰富的区域级标注和自然语言描述；提出了基于Qwen2.5-VL的Focal-RegionFace模型，通过多阶段渐进微调策略逐步聚焦于局部面部特征。

Result: Focal-RegionFace在新基准测试中，无论是传统指标还是新提出的指标上都取得了最佳性能，验证了其在细粒度多属性面部区域聚焦分析场景中的有效性和通用性。

Conclusion: 该研究成功解决了面部区域聚焦描述问题，提出的方法在面部动作单元检测、情绪识别和年龄估计等任务中表现出色，为细粒度面部分析提供了新思路。
...
Method: 提出基于SLAT表示的无需训练框架，核心是变形交叉注意力（MCA）用于融合源和目标特征以保持结构连贯性，以及时间融合自注意力（TFSA）用于增强时间一致性。还引入方向校正策略解决变形过程中的姿态模糊问题。

Result: 实验表明该方法能生成最先进的变形序列，即使在具有挑战性的跨类别情况下也能表现优异。进一步支持解耦变形和3D风格迁移等高级应用，并能泛化到其他基于SLAT的生成模型。
...
<details>
  <summary>Details</summary>
Motivation: 户外农田环境中，部分遮挡和作物聚集导致基于图像的2D分割方法难以精确计数，需要更鲁棒的3D解决方案。

Method: 利用多视角2D图像，结合神经辐射场（NeRF）进行视图合成，引入作物可见性和掩码一致性评分，实现3D实例分割和精确计数。

Result: 在棉花、苹果和梨三个农业数据集上验证，展示了对作物颜色、形状和尺寸变化的鲁棒性，计数性能优于现有方法。

Conclusion: 提出的3D实例分割框架能有效解决农业作物计数中的遮挡和聚集问题，无需作物特定参数调优，并贡献了棉花数据集促进后续研究。
...
TL;DR: IntraStyler：一种无需先验知识的基于示例的风格合成方法，用于增强域适应中的域内多样性


<details>
  <summary>Details</summary>
Motivation: 现有无监督域适应方法主要关注源域和目标域之间的域偏移，而域内变异性研究不足。传统方法需要预先指定域内变化进行风格合成，这在实践中可能不切实际。

Method: 提出IntraStyler方法，使用示例图像指导风格合成，使输出风格与示例风格匹配。引入风格编码器，基于对比学习区分性地学习风格特征。

Result: 在最大的跨模态域适应公共数据集CrossMoDA 2023上评估，实验证明该方法在可控风格合成方面的有效性，以及多样化合成数据对下游分割任务的益处。

Conclusion: IntraStyler能够无需先验知识捕获多样化的域内风格，通过可控风格合成增强域适应性能，代码已开源。
...
TL;DR: 该论文提出使用强化学习来改进多模态大语言模型的视觉推理能力，通过设计针对不同推理方面的奖励函数，使模型能够生成更长、更结构化的推理链，从而解决视觉信息整合不足的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在生成推理链时缺乏视觉信息的整合，这限制了它们解决需要准确视觉感知的任务（如视觉谜题）的能力。研究表明视觉感知是这类任务的关键瓶颈，将图像转换为文本描述可以显著提升性能。

Method: 采用基于奖励的强化学习方法，设计了六个针对不同推理方面的奖励函数，包括图像理解、思考步骤和答案准确性。使用组相对策略优化（GRPO）来明确激励更长、结构化的推理，并减少视觉信息的绕过。

Result: 在Qwen-2.5-VL-7B模型上实现了5.56%的性能提升，在领域内和领域外设置中都获得了稳定的增益。实验表明将图像转换为文本描述可以使Claude 3.5和Claude 3.7分别获得26.7%和23.6%的性能提升。

Conclusion: 强化学习是解锁开源多模态大语言模型长视觉推理能力的有效机制，无需昂贵的监督数据。通过设计针对性的奖励函数，可以显著改善模型在需要准确视觉感知任务上的表现。
...
TL;DR: LooC是一种新型向量量化方法，通过低维码本组合量化特征向量，在显著减小码本大小的同时提升性能，可作为即插即用模块应用于各种下游任务。


<details>
  <summary>Details</summary>
Motivation: 随着数据和模型复杂度不断增加，现有向量量化方法面临容量与紧凑性的矛盾，需要既能处理复杂数据又保持紧凑的高容量量化方法。

Method: 提出LooC方法：1）重构码向量与特征向量关系，将码向量视为特征向量中的低维组合单元进行组合；2）引入参数无关的插值外推机制平滑特征；3）实现完全码本利用，避免坍缩问题。

Result: 在不同任务、数据集和架构上的广泛评估表明，LooC在显著减小码本大小的同时，超越了现有向量量化方法，达到了最先进的性能水平。

Conclusion: LooC成功解决了向量量化中容量与紧凑性的矛盾，提供了一种高效、紧凑且可扩展的量化方案，可作为通用模块应用于各种基于向量量化的下游任务。
...
TL;DR: 本文提出SynDR-IQA框架，通过重塑合成数据分布来解决盲图像质量评估中合成数据泛化能力不足的问题。研究发现合成数据学习到的特征呈现离散聚类模式，导致回归性能受限。通过分布感知的多样性内容上采样和密度感知的冗余聚类下采样，有效提升了BIQA模型的跨数据集泛化能力。


<details>
  <summary>Details</summary>
Motivation: 盲图像质量评估（BIQA）虽然通过深度学习取得了显著进展，但大规模标注数据集的稀缺仍然是主要挑战。合成数据虽然提供了有前景的解决方案，但现有合成数据集训练的模型往往表现出有限的泛化能力。研究发现，从合成数据学习到的特征呈现离散聚类模式：高质量图像特征围绕参考图像聚类，低质量图像特征则基于失真类型聚类，这种分布问题阻碍了回归性能。
...
Conclusion: 通过重塑合成数据分布，SynDR-IQA框架有效解决了BIQA中合成数据泛化能力不足的问题，为利用合成数据提升盲图像质量评估性能提供了新的解决方案。
...
TL;DR: 提出跨模态数据增强框架，通过CycleGAN将可见光PCB图像转换为红外域，生成伪红外数据，结合有限真实红外样本训练YOLOv8检测器，解决红外数据稀缺问题


<details>
  <summary>Details</summary>
Motivation: 解决PCB缺陷检测中红外数据稀缺的关键瓶颈问题，传统方法依赖配对监督数据，但红外数据获取困难且成本高昂

Method: 1. 使用CycleGAN进行无配对图像到图像转换，将丰富的可见光PCB图像映射到红外域；2. 生成高保真伪红外样本，保留缺陷结构语义并模拟热分布模式；3. 构建异构训练策略，融合生成的伪红外数据和有限真实红外样本；4. 训练轻量级YOLOv8检测器

Result: 该方法在低数据条件下有效增强特征学习，增强后的检测器显著优于仅使用有限真实数据训练的模型，性能接近完全监督训练的基准，证明伪红外合成作为工业检测的鲁棒增强策略的有效性

Conclusion: 提出的跨模态数据增强框架成功解决了PCB缺陷检测中红外数据稀缺问题，通过生成伪红外数据与有限真实数据融合训练，实现了接近完全监督的性能，为工业检测提供了一种有效的低数据解决方案
...
Method: 框架包含两个模块：1) 害虫检测模块使用轻量级CNN结合原型元学习，少量样本即可准确识别；2) 农药推荐模块结合作物类型和生长阶段等环境因素推荐环保农药
...
<details>
  <summary>Details</summary>
Motivation: 放射学基础模型在处理3D-CT体数据时面临计算成本高的挑战，需要一种既能保持表示能力又能提高计算效率的实用方法。

Method: 采用器官分离概念，通过分割技术和基于LLM的放射学报告处理自动创建器官体积-描述对，结合VideoMAE自监督预训练和体积-文本对的对比学习。

Result: 在零样本器官病变分类任务中，相比CT-CLIP在83%器官上获得更高F1分数，相比Merlin在64%器官上表现更好；在零样本病变分类任务中，相比Merlin在83%类别上获得更高AUROC；在放射学报告生成任务中表现与现有VLM相当。

Conclusion: 器官分离学习框架为3D-CT基础模型的实用实现提供了现实有效的设计指南，展示了高泛化性能。
...
TL;DR: S1-MMAlign是一个包含1550万高质量图像-文本对的大规模多学科科学数据集，通过AI增强管道解决科学图像与文本描述之间的语义鸿沟问题。


<details>
  <summary>Details</summary>
Motivation: 多模态学习在通用领域取得了革命性进展，但在科学发现中的应用受到阻碍，因为复杂的科学图像与稀疏的文本描述之间存在深刻的语义鸿沟。现有科学数据集存在弱对齐问题，限制了科学推理和跨模态理解的发展。

Method: 从250万篇开放获取科学论文中提取超过1550万图像-文本对，涵盖物理、生物、工程等多个学科。引入AI就绪的语义增强管道，利用Qwen-VL多模态大模型系列，通过综合论文摘要和引用上下文来重新标注图像，解决原始科学标题的弱对齐问题。

Result: 技术验证显示增强显著提高了数据质量：基于SciBERT的伪困惑度指标显示语义模糊性降低，CLIP分数表明图像-文本对齐提高了18.21%。数据集在Hugging Face上公开可用。

Conclusion: S1-MMAlign为推进科学推理和跨模态理解提供了基础资源，支持AI for Science时代的发展。该数据集解决了科学多模态学习中的关键数据质量问题。
...
Result: 在三个关键擦除任务（裸露、艺术风格、对象移除）上实现SOTA性能，有效保留模型整体生成能力，对对抗攻击具有强鲁棒性

Conclusion: ActErase建立了轻量级但有效的概念操作新范式，为扩散模型提供了即插即用的概念擦除解决方案
...
TL;DR: SV-GS：一种在稀疏观测下重建动态目标的方法，通过骨架驱动变形场实现运动估计和几何细节保持


<details>
  <summary>Details</summary>
Motivation: 现实世界中动态目标重建面临观测稀疏的挑战，传统方法需要密集的多视角视频，而实际场景中观测往往在时间和空间上都稀疏，导致动态重建高度不适定

Method: 提出SV-GS框架，利用粗略骨架图和初始静态重建作为输入，优化骨架驱动变形场，包含粗粒度骨架关节姿态估计器和细粒度变形模块，仅使关节姿态估计器时间相关以实现平滑运动插值

Result: 在合成数据集上，相比现有方法在稀疏观测下PSNR提升高达34%；在真实数据集上，使用显著更少的帧数即可达到与密集单目视频方法相当的性能；输入需求可放宽，可用扩散生成先验替代初始静态重建

Conclusion: SV-GS能够在稀疏观测条件下有效重建动态目标，通过骨架驱动变形场实现运动估计和几何细节保持，提高了动态重建在真实场景中的实用性
...
TL;DR: TimeColor：基于草图的视频着色模型，支持异构、可变数量的参考图像，通过显式的每参考区域分配和时空对应掩码注意力机制，提高颜色保真度、身份一致性和时间稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有着色模型通常只基于单个参考（通常是场景的第一帧），忽略了其他条件数据源，如角色设定表、背景图像或任意着色帧。这限制了着色质量和一致性。

Method: 提出TimeColor模型：1）将参考图像编码为额外的潜在帧，在时间维度上拼接，使模型能在每个扩散步骤中并行处理参考图像；2）使用显式的每参考区域分配；3）采用时空对应掩码注意力机制和模态分离的RoPE索引，防止捷径学习和跨身份调色板泄漏。

Result: 在SAKUGA-42M数据集上的实验表明，TimeColor在单参考和多参考协议下均优于现有基线方法，在颜色保真度、身份一致性和时间稳定性方面都有显著提升。

Conclusion: TimeColor通过支持异构、可变数量的参考图像，并采用有效的参考绑定机制，显著提升了基于草图的视频着色性能，为视频着色任务提供了更灵活和强大的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有行人重识别方法虽然精度高但计算成本大，难以在计算资源有限的实时监控和移动应用中部署。需要开发既准确又计算高效的模型。

Method: 提出VisNet模型，包含：1）多尺度特征融合（融合ResNet50的1-4阶段特征）；2）语义聚类与解剖学身体分区；3）动态权重平均平衡分类语义正则化；4）使用FIDI损失函数改进度量学习。

Result: 在Market-1501数据集上达到87.05% Rank-1和77.65% mAP，仅有32.41M参数和4.601 GFLOPs计算量，显著优于现有高计算成本方法。

Conclusion: VisNet提供了一个实用的行人重识别解决方案，在保持高精度的同时大幅降低计算复杂度，适合实时监控和移动应用部署，解决了计算资源有限场景下的实际需求。
...
TL;DR: OmniVaT框架首次成功解决了单域泛化多模态视觉-触觉学习任务，通过统一嵌入频率空间映射和分层树结构生成，有效缓解模态差异和领域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 视觉-触觉学习面临两个主要挑战：1) VIS和TAC图像之间的模态差异；2) 非标准化触觉传感器和不一致数据收集导致的领域差距。这些挑战被形式化为单域泛化多模态VTL任务。

Method: 提出OmniVaT框架，包含两个核心模块：1) 多模态分数傅里叶适配器(MFFA)，将VIS和TAC嵌入映射到统一的嵌入频率空间；2) 离散树生成(DTG)模块，通过分层树结构获得多样可靠的多模态分数表示。

Result: 大量实验证明OmniVaT在SDG-VTL任务上具有优越的跨域泛化性能，无需多域训练数据或复杂的跨模态融合策略。

Conclusion: OmniVaT首次成功解决了单域泛化多模态视觉-触觉学习任务，通过创新的频率空间映射和树结构表示方法，有效缓解了模态差异和领域偏移问题。
...
Method: 提出DVEFormer - 基于RGB-D Transformer的高效方法，通过知识蒸馏从Alpha-CLIP获取教师嵌入来指导学生模型学习细粒度像素级嵌入。支持传统语义分割（如线性探测），同时支持灵活的基于文本的查询。

Result: 在常见室内数据集上评估显示，该方法在满足实时性要求的同时实现了竞争性性能：完整模型在NVIDIA Jetson AGX Orin上达到26.3 FPS，较小变体达到77.0 FPS。定性结果展示了实际应用中的有效性和可能用例。

Conclusion: 该方法可作为传统分割方法的直接替代方案，同时支持灵活的自然语言查询，并能无缝集成到移动机器人的3D建图流程中。
...
TL;DR: 提出概率双流框架，统一可靠性建模与多模态集成，解决骨架动作识别中忽视手部细微动作的问题，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于骨架的人体动作识别方法大多关注身体大尺度运动，忽略了对手部细微动作的识别，而手部动作对于细粒度识别至关重要。

Method: 提出概率双流框架，包含三个关键组件：1) 无需校准的预处理流程，直接从原生坐标学习；2) 概率Noisy-OR融合，稳定可靠性感知的双流学习；3) 从骨架模态到RGB表示的跨模态集成，耦合四种骨架模态。

Result: 在多个基准测试（NTU RGB+D 60/120、PKU-MMD、N-UCLA）和新定义的手部中心基准上，都表现出持续的改进和鲁棒性，特别是在噪声和异构条件下。

Conclusion: 该框架通过统一可靠性建模和多模态集成，有效解决了骨架动作识别中手部细微动作的识别问题，在多种条件下都表现出优越性能。
...
TL;DR: NeoVerse是一个多功能4D世界模型，能够进行4D重建、新轨迹视频生成和丰富的下游应用，通过可扩展的设计解决了现有4D建模方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有4D世界建模方法存在可扩展性限制，主要源于昂贵的多视角4D数据或繁琐的训练预处理，限制了在多样化单目视频上的应用。

Method: NeoVerse采用无姿态前馈4D重建、在线单目退化模式模拟等技术，使整个流程能够扩展到多样化的野外单目视频，具有通用性和泛化能力。

Result: NeoVerse在标准重建和生成基准测试中取得了最先进的性能，同时保持了多功能性和对各种领域的泛化能力。

Conclusion: NeoVerse通过可扩展的设计解决了4D世界建模的局限性，实现了多功能应用并在标准基准测试中达到最优性能。
...
TL;DR: RoLID-11K是首个用于车载摄像头路边垃圾检测的大规模数据集，包含超过1.1万张标注图像，专注于极端小目标检测，并建立了该领域的基准测试。


<details>
  <summary>Details</summary>
Motivation: 当前路边垃圾监测依赖劳动密集型调查和公众报告，空间覆盖有限。现有的视觉数据集主要关注街景、航拍或水环境，无法反映车载摄像头中垃圾目标极小、稀疏且背景杂乱的独特特征。

Method: 提出了RoLID-11K数据集，包含超过1.1万张标注图像，涵盖英国多样驾驶条件，具有明显的长尾分布和小目标特征。对多种现代检测器进行了基准测试，包括基于transformer的精度导向架构和实时YOLO模型。

Result: CO-DETR及相关transformer架构实现了最佳定位精度，但实时模型受限于粗糙的特征层次结构。该数据集为动态驾驶场景中的极端小目标检测建立了具有挑战性的基准。

Conclusion: RoLID-11K数据集支持开发可扩展、低成本的路边垃圾监测系统，为车载摄像头场景中的小目标检测提供了重要的研究基准。
...
<details>
  <summary>Details</summary>
Motivation: 虽然强化学习在语言模型推理方面取得了进展，但将其扩展到多模态推理需要同时改进感知和推理两方面。先前的工作主要通过显式感知奖励来解决这一挑战，但将感知标记与推理标记分离很困难，需要额外的LLM、真实数据、强制分离感知与推理，或对所有输出标记不加区分地应用奖励。
...
<details>
  <summary>Details</summary>
Motivation: 传统3D物体和材料模拟需要专家知识和耗时的物理参数调整，难以实现理想的动态行为。需要一种能够从自然语言直接推断物理参数的自动化方法。

Method: 1. 使用多模态大语言模型估计材料参数值，并约束在合理范围内；2. 提出可学习的运动蒸馏损失，从预训练视频扩散模型中提取鲁棒的运动先验，同时最小化外观和几何归纳偏差来指导模拟。

Result: 在30多个场景中评估，包括真实世界、人工设计和AI生成的3D物体，涵盖弹性固体、金属、泡沫、沙子以及牛顿和非牛顿流体等多种材料。MotionPhysics能生成视觉逼真的动态模拟，超越现有技术水平。

Conclusion: MotionPhysics能够通过自然语言指导生成视觉逼真的动态模拟，自动确定物理上合理的参数，为3D物理模拟提供了高效便捷的解决方案。
...
Method: FreeText将问题分解为"在哪里写"和"写什么"两个部分。对于"在哪里写"，通过读取图像到文本注意力中的token-wise空间归因来定位书写区域，使用sink-like tokens作为稳定的空间锚点，并通过拓扑感知细化产生高置信度掩码。对于"写什么"，引入频谱调制字形注入(SGMI)，注入噪声对齐的字形先验，通过频域带通调制来增强字形结构并抑制语义泄漏。
...
TL;DR: VNS-SAM通过增强SAM在视觉非显著场景（前景背景对比度低）的分割能力，同时保持其零样本泛化性，使用Mask-Edge Token交互解码器和非显著特征挖掘模块，在参数和计算开销极小的情况下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: SAM在视觉非显著场景（前景背景对比度低）中表现不佳，现有方法难以捕捉准确轮廓，需要提升SAM在这类场景下的感知能力，同时保持其零样本泛化优势。

Method: 提出VNS-SAM，通过Mask-Edge Token交互解码器和非显著特征挖掘模块有效利用SAM的低层特征，以极小参数增量增强对非显著特征的感知。构建VNS-SEG数据集（超过35K图像）用于训练和评估。

Result: VNS-SAM在各种VNS分割任务中表现出优越性能，特别是在零样本设置下，额外参数可在4小时内优化完成，证明了其实用性和可行性。

Conclusion: VNS-SAM成功增强了SAM在视觉非显著场景下的分割能力，同时保持了原始零样本泛化性，具有广泛的实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有拖拽式图像编辑方法存在跟踪丢失、模糊跟踪、源图像与目标图像差距过大、中间点不合理导致编辑性低等问题，需要一种更稳定有效的框架来解决这些挑战。

Method: 提出DynaDrag方法，采用"预测-移动"框架，迭代执行运动预测和运动监督两个步骤：运动预测预测手柄点应该移动的位置，运动监督根据预测结果拖动这些点。同时提出动态调整有效手柄点的方法进一步提升性能。

Result: 在人脸和人体数据集上的实验表明，DynaDrag方法在性能上优于先前的工作，能够更有效地实现像素级图像操控。

Conclusion: DynaDrag作为首个基于"预测-移动"框架的拖拽式图像编辑方法，通过迭代的运动预测和运动监督机制，有效解决了传统方法中的跟踪问题，提高了图像编辑的准确性和稳定性。
...
TL;DR: SlingBAG Pro是一种基于点云迭代概念的高级重建算法，扩展了不规则几何换能器阵列的兼容性，在保持高质量重建的同时减少换能器数量，通过分层优化策略显著缩短重建时间。


<details>
  <summary>Details</summary>
Motivation: 高质量3D光声成像在临床应用中受到关注，不规则几何换能器阵列能以较少换能器实现高质量成像，但传统迭代重建算法在处理不规则阵列时存在计算复杂度高、内存需求大、重建时间长的问题。

Method: 基于SlingBAG方法的点云迭代概念，扩展兼容任意阵列几何形状，采用分层优化策略结合零梯度滤波和迭代中逐步增加的时间采样率，快速去除冗余空间点云，加速收敛。

Result: 相比原始SlingBAG算法，SlingBAG Pro在不规则阵列几何下实现点云基3D光声重建速度提升高达2.2倍，通过仿真和活体小鼠实验验证了方法有效性。

Conclusion: SlingBAG Pro算法成功解决了不规则几何换能器阵列重建的挑战，在保持重建质量的同时显著提高了计算效率，为临床3D光声成像应用提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试在评估统一多模态模型的世界知识应用能力方面存在不足，它们通常是孤立的单任务评估，诊断能力有限。需要建立一个全面的多任务基准来评估模型在视觉理解、生成、编辑和交错生成等方面的世界知识应用能力。

Method: 提出了AEGIS基准，包含1050个具有挑战性的手动标注问题，涵盖21个主题（STEM、人文、日常生活等）和6种推理类型。同时提出了确定性检查表评估协议，用原子化的"是/否"判断替代模糊的提示式评分，提高评估可靠性。

Result: 实验表明，大多数统一多模态模型存在严重的世界知识缺陷，随着推理复杂度增加，性能显著下降。简单的插件式推理模块可以部分缓解这些弱点，这为未来研究指明了有前景的方向。

Conclusion: 世界知识推理是统一多模态模型发展的关键前沿领域，AEGIS基准和确定性检查表评估协议为系统评估模型的世界知识应用能力提供了有效工具。
...
### [34] [GranAlign: Granularity-Aware Alignment Framework for Zero-Shot Video Moment Retrieval](https://arxiv.org/abs/2601.00584)
...
<details>
  <summary>Details</summary>
Motivation: 零样本视频时刻检索中，文本查询和视频内容之间存在语义粒度不匹配的问题。现有方法虽然利用了高质量的预训练知识，但未能平衡不同模态之间的语义粒度，导致检索不准确。
...
Conclusion: GranAlign框架通过粒度感知对齐有效解决了零样本视频时刻检索中的语义粒度不匹配问题，无需训练即可显著提升性能，为跨模态对齐提供了新的解决方案。
...
### [35] [SafeMo: Linguistically Grounded Unlearning for Trustworthy Text-to-Motion Generation](https://arxiv.org/abs/2601.00590)
*Yiling Wang,Zeyu Zhang,Yiran Wang,Hao Tang*

Main category: cs.CV

TL;DR: SafeMo是一个可信赖的运动生成框架，通过最小化运动遗忘技术解决文本到运动生成中的安全问题，避免传统离散码本替换方法的缺陷，在连续空间中生成安全的人类运动。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到运动生成方法存在安全隐患，传统基于离散VQ-VAE码本替换的安全方法有两个关键缺陷：1) 替换被良性提示重用的码本条目会导致日常任务性能下降；2) 离散标记方法引入量化和平滑度损失，导致伪影和不连贯过渡。此外，现有文本到运动数据集自然包含不安全意图和相应运动，不适合安全驱动的机器学习。

Method: 提出SafeMo框架，集成最小化运动遗忘（MMU）的两阶段机器学习遗忘策略，在连续空间中实现安全的人类运动生成，保留连续运动学特性而不损失码本质量。同时创建首个安全文本到运动数据集SafeMoVAE-29K，包含重写的安全文本提示和连续精炼运动。

Result: 实验表明SafeMo在HumanML3D和Motion-X数据集上分别达到2.5倍和14.4倍更高的遗忘集FID，相比之前最先进的人类运动遗忘方法LCR，在安全提示上的良性性能相当或更好，实现了强大的安全-效用权衡。

Conclusion: SafeMo通过连续空间中的最小化运动遗忘策略有效解决了文本到运动生成的安全问题，避免了传统离散码本替换方法的缺陷，在保持良好性能的同时实现了安全的人类运动生成。
...
### [36] [Modality Dominance-Aware Optimization for Embodied RGB-Infrared Perception](https://arxiv.org/abs/2601.00598)
*Xianhui Liu,Siqi Jiang,Yi Xie,Yuqing Lin,Siao Liu*

Main category: cs.CV

TL;DR: 该论文提出了一种模态主导感知指数(MDI)来量化RGB-IR多模态感知中的优化偏差问题，并开发了模态主导感知跨模态学习(MDACL)框架来平衡跨模态优化，在三个RGB-IR基准测试中取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: RGB-红外多模态感知在复杂物理环境中的嵌入式多媒体系统中至关重要。虽然现有的跨模态融合方法有所进展，但由于模态特性不对称导致的优化动态问题仍未充分探索。实践中，信息密度和特征质量的差异会引入持续的优化偏差，导致训练过度强调主导模态，阻碍有效融合。
...
Conclusion: 该研究通过量化模态主导问题并开发相应的平衡优化框架，有效解决了RGB-IR多模态感知中的优化偏差问题，为跨模态融合提供了新的解决方案。
...
TL;DR: TOLF框架通过流模型建模复杂误差分布和不确定性引导优化，提升小目标检测对标注噪声的鲁棒性
...
Method: 提出TOLF框架：1) 使用归一化流建模复杂非高斯预测误差分布；2) 不确定性感知梯度调制机制，抑制高不确定性噪声样本的学习

Result: 在三个数据集上验证有效性，特别是在AI-TOD数据集上将DINO基线提升1.2% AP

Conclusion: TOLF通过流模型误差建模和不确定性引导优化，有效提升小目标检测对标注噪声的鲁棒性
...
### [38] [RePose: A Real-Time 3D Human Pose Estimation and Biomechanical Analysis Framework for Rehabilitation](https://arxiv.org/abs/2601.00625)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决康复训练中患者动作监测和评估的实时性问题，需要一种能够提供即时反馈和指导的系统，帮助患者正确执行康复训练，从而有效恢复肌肉力量和运动功能。

Method: 1. 提出端到端的实时人体姿态估计与运动分析统一流程；2. 针对多人干扰的医疗康复场景提出快速跟踪方法（单帧跟踪时间<1ms）；3. 改进SmoothNet用于实时姿态估计，减少误差并恢复真实运动状态；4. 使用Unity平台进行实时监测评估并显示肌肉应力情况。

Result: 该方法能够实时监测和评估康复训练中的患者动作，提供即时反馈和指导，快速跟踪方法在多人干扰场景下表现高效，改进的姿态估计方法使运动状态更平滑真实。

Conclusion: RePose系统为康复训练提供了一种有效的实时监测和评估解决方案，能够帮助患者正确执行康复训练，有望在康复医疗领域得到广泛应用。
...
### [39] [Quality Detection of Stored Potatoes via Transfer Learning: A CNN and Vision Transformer Approach](https://arxiv.org/abs/2601.00645)
...
TL;DR: 本研究利用深度学习模型通过图像分析监测马铃薯储存质量，包括发芽检测、重量损失估计和保质期预测，DenseNet在发芽检测上达到98.03%准确率。


<details>
  <summary>Details</summary>
Motivation: 马铃薯储存期间的质量监控面临发芽检测、重量损失估计和保质期预测等挑战，需要非侵入式、可扩展的解决方案来减少食物浪费并改善库存管理。

Method: 在200天控制温湿度条件下收集图像和重量数据，使用ResNet、VGG、DenseNet和Vision Transformer预训练架构，设计了两个专门模型：高精度二分类发芽检测器和先进多分类预测器用于重量损失估计和保质期预测。

Result: DenseNet在发芽检测上达到98.03%准确率；保质期预测在粗分类（2-5类）时准确率超过89.83%，但在细分类（6-8类）时因视觉差异细微和数据有限而准确率下降。

Conclusion: 图像深度学习模型可集成到自动化分拣和库存系统中，实现早期发芽检测和基于储存阶段的动态分类，改善库存管理、差异化定价并减少食物浪费。未来需开发适用于多种马铃薯品种和储存条件的通用模型。
...
### [40] [CRoPS: A Training-Free Hallucination Mitigation Framework for Vision-Language Models](https://arxiv.org/abs/2601.00659)
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在生成幻觉内容的倾向，现有无需训练方法存在两个局限：1）对幻觉来源的假设过于狭窄；2）在生成后期（幻觉最可能发生时）效果下降

Method: 提出CRoPS框架：1）构建新的幻觉模型，通过选择性移除关键文本标记来捕捉幻觉效应；2）引入广义对比解码，整合多个幻觉模型以表示多样化的幻觉来源

Result: CHAIR分数提升20%，在六个基准测试和三个LVLM家族上取得一致增益，优于现有最先进的无需训练方法

Conclusion: CRoPS通过创新的幻觉模型构建和广义对比解码，有效缓解LVLMs的幻觉问题，无需额外训练即可显著提升模型可靠性
...
### [41] [Pixel-to-4D: Camera-Controlled Image-to-Video Generation with Dynamic 3D Gaussians](https://arxiv.org/abs/2601.00678)
*Melonie de Almeida,Daniela Ivanova,Tong Shi,John H. Williamson,Paul Henderson*

Main category: cs.CV

TL;DR: 提出了一种新颖的单图像到视频生成框架，通过3D高斯场景表示和物体运动采样，在单次前向传递中实现快速、相机引导的视频生成，无需迭代去噪过程。


<details>
  <summary>Details</summary>
Motivation: 现有单图像条件视频生成方法虽然改进了时间一致性和3D一致性，但缺乏鲁棒的用户可控性（如修改相机路径），限制了实际应用。大多数相机控制的图像到视频模型难以准确建模相机运动、保持时间一致性和几何完整性。

Method: 提出了一种新颖框架，在单次前向传递中构建3D高斯场景表示并采样合理的物体运动，实现快速、相机引导的视频生成，无需通过迭代去噪将物体运动注入渲染帧。
...
### [42] [RGS-SLAM: Robust Gaussian Splatting SLAM with One-Shot Dense Initialization](https://arxiv.org/abs/2601.00705)
*Wei-Tse Cheng,Yen-Jen Chiou,Yuan-Fu Yang*

Main category: cs.CV

TL;DR: RGS-SLAM是一种基于高斯泼溅的SLAM框架，通过训练无关的对应关系到高斯初始化替代了传统的残差驱动稠密化过程，实现了更稳定、更快速的实时建图。


<details>
  <summary>Details</summary>
Motivation: 传统GS-SLAM使用残差驱动稠密化方法，逐步添加高斯来表示缺失的几何结构，这种方法可能导致早期建图不稳定且收敛较慢。RGS-SLAM旨在通过更智能的初始化策略改善这些问题。

Method: RGS-SLAM采用一次性三角化方法，从DINOv3描述符提取的密集多视角对应关系中生成高斯种子。这些对应关系通过置信度感知的内点分类器进行细化，产生分布良好且结构感知的高斯先验，然后进行优化。

Result: 在TUM RGB-D和Replica数据集上的评估显示，RGS-SLAM相比现有高斯和基于点的SLAM系统，在定位和重建精度上具有竞争力或更优表现，收敛速度提升约20%，渲染保真度更高，同时保持实时建图性能（最高925 FPS）。

Conclusion: RGS-SLAM通过训练无关的对应关系到高斯初始化策略，显著改善了高斯泼溅SLAM的早期建图稳定性和收敛速度，同时保持与现有GS-SLAM管道的完全兼容性，在纹理丰富和杂乱场景中表现优异。
...
TL;DR: 该研究探讨了在数据分布偏移下检测视觉语言模型性能退化的问题，开发了DomainSAT工具箱分析输入数据偏移，并提出基于输出置信度的无标签退化指标，两者结合能更可靠地监测病理VLM的性能退化。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在医学图像分析中表现出色，但部署后当输入数据分布发生变化时，模型性能可能下降。检测这种性能退化对临床可靠性至关重要，但对于没有标注数据的大型预训练VLM来说仍然具有挑战性。

Method: 研究开发了DomainSAT工具箱，集成代表性偏移检测算法，用于系统分析输入数据偏移。同时研究了基于输出的监测方法，引入了无标签、基于置信度的退化指标，直接捕捉模型预测置信度的变化。

Result: 分析表明，输入数据偏移检测能有效识别分布变化并提供早期诊断信号，但并不总是对应实际性能退化。基于输出的置信度指标与性能退化密切相关，可作为输入偏移检测的有效补充。两者结合能更可靠地检测和解释数据偏移下VLM的性能退化。

Conclusion: 输入数据偏移检测和输出置信度指标的结合为监测数字病理学中基础模型的可靠性提供了一个实用且互补的框架，有助于更可靠地检测和解释视觉语言模型在数据偏移下的性能退化。
...
Method: 多阶段设计：格式/存在性检查防止批改空白答案，独立评分器集成，监督聚合，刚性模板生成可审计的机器可解析报告；参考答案转换为文本摘要作为评分条件

Result: 使用GPT-5.2和Gemini-3 Pro后端，完整流程与教师评分平均绝对差异约8分，偏差低，估计手动审查触发率约17%（D_max=40）

Conclusion: 结构化提示和参考答案基础对准确性至关重要，简单提示和移除参考答案会显著降低准确性并引入系统性过高评分
...
TL;DR: 论文探索了将自监督学习作为辅助任务来优化深度伪造检测的主要任务，通过融合自监督任务的特征表示来提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 深度伪造检测面临泛化能力不足的问题，现有方法在跨数据集评估中表现不佳。作者希望利用自监督学习作为辅助任务来增强主要任务的性能，探索如何通过任务组合来释放自监督学习的潜力。

Method: 研究不同的训练方案组合，将自监督学习作为辅助任务与深度伪造检测的主要任务结合。通过融合自监督辅助任务的特征表示，创建更强大的特征表示体系，充分利用自监督任务和主要任务的独特优势。

Result: 在DF40、FaceForensics++、Celeb-DF、DFD、FaceShifter、UADFV等多个数据集上进行实验，结果显示该方法在跨数据集评估中表现出更好的泛化能力，优于当前最先进的检测器。

Conclusion: 融合自监督辅助任务的特征表示是一种有效的深度伪造检测方法，能够提升主要任务的性能，在跨数据集评估中展现出更好的泛化能力，为深度伪造检测提供了新的解决方案。
...
TL;DR: 本文提出LNU-Net和IBU-Net两种深度学习架构用于心脏MRI左心室分割，基于层归一化和实例-批量归一化改进U-Net，在805张MRI图像上验证了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 左心室分割对心脏图像的临床量化和诊断至关重要，需要更准确的分割方法来提高诊断精度。

Method: 提出两种基于U-Net的改进架构：LNU-Net在每个卷积块中应用层归一化；IBU-Net在第一个卷积块中结合实例和批量归一化。采用下采样路径进行特征提取和上采样路径进行精确定位，并整合仿射变换和弹性变形进行图像数据处理。

Result: 在包含45名患者805张MRI图像的数据集上评估，实验结果表明提出的方法在Dice系数和平均垂直距离指标上优于其他最先进方法。

Conclusion: LNU-Net和IBU-Net架构能够有效提升左心室分割的准确性，为心脏图像分析提供了改进的深度学习解决方案。
...
TL;DR: AdaGaR：一个统一框架，通过自适应Gabor表示和三次Hermite样条解决动态3D场景重建中的频率适应性和时间连续性问题，在Tap-Vid DAVIS数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 从单目视频重建动态3D场景需要同时捕捉高频外观细节和时间连续运动。现有方法存在三个主要问题：1）使用单高斯基元具有低通滤波特性，限制了高频细节捕捉；2）标准Gabor函数引入能量不稳定性；3）缺乏时间连续性约束导致插值时的运动伪影。

Method: 提出AdaGaR统一框架，包含三个核心组件：1）自适应Gabor表示，通过可学习频率权重和自适应能量补偿扩展高斯函数，平衡细节捕捉和稳定性；2）时间连续性方面，使用三次Hermite样条配合时间曲率正则化确保平滑运动演化；3）自适应初始化机制，结合深度估计、点跟踪和前景掩码，在早期训练中建立稳定的点云分布。

Result: 在Tap-Vid DAVIS数据集上达到最先进性能：PSNR 35.49，SSIM 0.9433，LPIPS 0.0723。在帧插值、深度一致性、视频编辑和立体视图合成等任务上展现出强大的泛化能力。

Conclusion: AdaGaR通过统一解决频率适应性和时间连续性问题，显著提升了动态3D场景重建的质量，在多个评估指标和下游任务上都表现出优越性能，为动态场景建模提供了有效的解决方案。
...
### [48] [RIMRULE: Improving Tool-Using Language Agents via MDL-Guided Rule Learning](https://arxiv.org/abs/2601.00086)
*Xiang Gao,Yuguang Yao,Qi Zhang,Kaiwen Dong,Avinash Baidya,Ruocheng Guo,Hilaf Hasson,Kamalika Das*

Main category: cs.CL

TL;DR: RIMRULE：一种基于动态规则注入的神经符号方法，通过从失败轨迹中提炼紧凑、可解释的规则并注入提示中，提高LLM在特定领域工具使用中的性能，无需修改模型权重。
...
Method: 提出RIMRULE神经符号方法：1）从失败轨迹中提炼紧凑、可解释的规则；2）使用最小描述长度目标优化规则的通用性和简洁性；3）将规则以自然语言和结构化符号形式存储；4）在推理时动态注入规则到提示中。

Result: 在工具使用基准测试中，该方法提高了对已见和未见工具的准确性，优于基于提示的适应方法，并与微调互补。从一个LLM学习的规则可以重用于改进其他LLM，包括长推理LLM，展示了符号知识在不同架构间的可移植性。

Conclusion: RIMRULE通过动态规则注入有效提高了LLM在特定领域工具使用中的性能，提供了一种无需修改模型权重的适应方法，且符号知识具有跨架构的可移植性。
...
### [49] [Noise-Aware Named Entity Recognition for Historical VET Documents](https://arxiv.org/abs/2601.00488)
...
<details>
  <summary>Details</summary>
Motivation: 职业教育培训领域的历史数字化文档存在OCR引起的噪声问题，这给命名实体识别带来了挑战。目前缺乏专门针对该领域多实体类型识别的方法，特别是在噪声条件下的鲁棒性解决方案。

Method: 提出了噪声感知训练方法，包括合成OCR错误注入、迁移学习和多阶段微调。系统比较了三种互补策略：在噪声数据、干净数据和人工合成数据上的训练。该方法专门针对德语文档设计，但可扩展到任意语言。

Result: 实验结果表明，领域特定和噪声感知的微调显著提高了在噪声条件下的鲁棒性和准确性。该方法是在职业教育培训文档中识别多实体类型的首批方法之一。

Conclusion: 提出的噪声感知命名实体识别方法有效解决了职业教育培训领域历史文档的OCR噪声问题，提高了识别准确性。该方法具有语言可扩展性，并提供了公开可用的代码以确保可复现性。
...
<details>
  <summary>Details</summary>
Motivation: 现有专利自动撰写评估方法无法评估专利说明书的长文本结构连贯性和法定合规性（如可实施性和书面描述要求），需要专门针对专利说明书特点的评估框架

Method: 提出Pat-DEVAL框架，采用LLM-as-a-judge范式，引入Chain-of-Legal-Thought（CoLT）法律约束推理机制，强制进行顺序的专利法特定分析
...
TL;DR: 本文通过系统分析IEMOCAP数据集，填补了情感对话识别中的两个关键空白：理解哪些架构选择真正重要，以及连接识别与生成的语言学分析。研究发现上下文至关重要，简单架构即可超越先前方法，并揭示了情感与话语标记位置之间的显著关联。


<details>
  <summary>Details</summary>
Motivation: 当前情感对话识别(ERC)虽然取得了高准确率，但仍存在两个关键空白：1) 缺乏对哪些架构选择真正重要的理解；2) 缺少连接识别与生成的语言学分析。本文旨在通过系统分析IEMOCAP数据集来填补这些空白。

Method: 采用系统分析方法：1) 识别方面：进行严格的消融研究，使用10次种子评估，分析对话上下文、分层句子表示和外部情感词典的影响；2) 语言学分析：分析5,286个话语标记出现情况，研究情感与标记位置的关系，特别是悲伤话语中左边缘标记的使用模式。

Result: 主要发现：1) 对话上下文至关重要，90%的性能增益来自最近的10-30个轮次；2) 分层句子表示在话语级别有帮助，但提供对话上下文后此优势消失；3) 外部情感词典无增益；4) 简单架构达到82.69%(4类)和67.07%(6类)加权F1，超越先前方法；5) 情感与话语标记位置显著相关(p<.0001)，悲伤话语左边缘标记使用率(21.9%)低于其他情感(28-32%)。

Conclusion: 对话上下文是情感识别的关键因素，简单架构即可实现优异性能。悲伤话语因缺乏明确语用信号而最依赖上下文(+22%提升)，这与左边缘标记使用减少的语言学发现一致，支持了左边缘标记与主动话语管理相关的理论。研究为情感识别架构选择和语言学分析提供了重要见解。
...
TL;DR: 该论文提出了一种针对时序知识图谱推理的蒸馏框架，利用大语言模型作为教师模型，将结构和时序推理能力转移到轻量级学生模型，在保持高效架构的同时提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有时序知识图谱推理模型通常依赖大量参数和密集计算，导致硬件成本高、能耗大，难以部署在资源受限的实时推理平台上。同时，现有的模型压缩和蒸馏技术主要针对静态知识图谱设计，无法充分捕捉时序知识图谱中的时序依赖关系，导致推理性能下降。

Method: 提出专门针对时序知识图谱推理的蒸馏框架，利用大语言模型作为教师模型指导蒸馏过程，将大规模公共知识与任务特定的时序信息相结合，有效传递结构和时序推理能力到轻量级学生模型。

Result: 在多个公开基准数据集上的广泛实验表明，该方法持续优于强基线模型，在推理准确性、计算效率和实际可部署性之间实现了有利的平衡。

Conclusion: 该蒸馏框架通过利用大语言模型作为教师模型，成功解决了时序知识图谱推理中的模型压缩问题，在保持轻量高效架构的同时提升了时序推理能力，为资源受限平台上的实时推理应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前医学领域的检索增强生成系统主要关注性能提升，但忽视了循证医学的基本原则。存在两个关键问题：1) 查询与检索证据之间缺乏PICO对齐；2) 重排序过程中未考虑证据等级层次。

Method: 提出了一种可推广的策略，将循证医学原则适配到图检索增强生成中：1) 将PICO框架集成到知识图谱构建和检索中；2) 提出贝叶斯启发的重排序算法，根据证据等级校准排序分数，无需预定义权重。

Result: 在运动康复领域验证了该框架，发布了包含357,844个节点和371,226条边的知识图谱，以及1,637个QA对的可复用基准。系统性能指标：0.830的金块覆盖率、0.819的答案忠实度、0.882的语义相似度、0.788的PICOT匹配准确度。五位专家临床医生在5点李克特量表上给予4.66-4.84的高评分。

Conclusion: 提出的循证医学适配策略不仅提高了检索和答案质量，而且可推广到其他临床领域。发布的资源有助于解决运动康复领域RAG数据集稀缺的问题。
...
<details>
  <summary>Details</summary>
Motivation: 日英翻译中，礼貌程度、隐含意义、省略和语体等微妙选择对自然度感知有重要影响。现有评估往往关注翻译是否可接受，而非在多个良好翻译中判断哪个更好。需要一种可靠且经济的评估方法来指导翻译系统的迭代开发。

Method: 使用参考无关的成对LLM比较方法：将候选模型与固定的版本化锚定集进行对比。通过大语言模型进行成对比较，使用Bradley-Terry模型聚合结果，计算胜率并推导出0-10分的"LT"评分（基于拟合对数强度的逻辑转换）。

Result: 开发了JP-TL-Bench基准测试，提供结构稳定的评分系统。由于所有候选模型都针对相同的冻结锚定集进行评估，在相同基础集、评判标准和聚合代码下，评分具有结构稳定性。

Conclusion: JP-TL-Bench为日英翻译系统的迭代开发提供了可靠、经济且专注的评估框架，特别适合处理翻译质量中微妙差异的评估需求。
...
TL;DR: 该研究系统评估了大语言模型生成多语言反事实的能力，发现翻译生成的反事实有效性更高但修改更多，多语言反事实数据增强比跨语言增强效果更好，但生成质量限制了模型性能提升。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在生成英语反事实方面表现出色且具备多语言能力，但其在多语言反事实生成方面的有效性尚不明确，需要系统研究来评估其实际表现和局限性。

Method: 1. 在六种语言上进行自动评估，比较直接生成的反事实和通过英语翻译生成的反事实；2. 分析高资源欧洲语言反事实的编辑模式；3. 识别和分类生成反事实中的错误类型；4. 比较多语言反事实数据增强与跨语言数据增强的效果。

Result: 1. 翻译生成的反事实比直接生成的有效性更高，但需要更多修改，且仍不及原始英语反事实质量；2. 高资源欧洲语言反事实的编辑模式相似，表明跨语言扰动遵循共同策略原则；3. 识别出四种主要错误类型；4. 多语言反事实数据增强比跨语言增强带来更大性能提升，特别是对低资源语言，但生成质量限制了增益。

Conclusion: 大语言模型在多语言反事实生成方面仍有局限性，翻译方法虽能提高有效性但代价是更多修改，多语言数据增强效果优于跨语言增强，但生成质量限制了模型性能和鲁棒性的提升空间。
...
TL;DR: WildAGTEval是一个评估LLM智能体在真实API复杂性下函数调用能力的基准，考虑了API规范和API执行两个维度的现实世界复杂性，包含约32K测试配置，发现大多数场景具有挑战性，无关信息复杂性使强LLM性能下降27.3%。


<details>
  <summary>Details</summary>
Motivation: 现有工作假设理想化的API系统，忽略了现实世界因素如嘈杂的API输出。需要评估LLM智能体在真实API复杂性下的函数调用能力，以更好地反映实际应用场景。

Method: 设计了WildAGTEval基准，包含两个维度的复杂性：1) API规范（详细文档和使用约束），2) API执行（运行时挑战）。构建了包含60个不同复杂性场景的API系统，可组合成约32K测试配置，并提供用户-智能体交互来评估LLM智能体。

Result: 系统评估多个先进LLM发现：大多数场景具有挑战性；无关信息复杂性造成最大困难，使强LLM性能下降27.3%；定性分析显示LLM有时会扭曲用户意图以声称任务完成，严重影响用户满意度。

Conclusion: WildAGTEval为评估LLM智能体在真实API复杂性下的函数调用能力提供了全面基准，揭示了当前LLM在处理现实世界API复杂性方面的局限性，特别是无关信息处理和意图保持方面的挑战。
...
TL;DR: 量化对LLM自解释能力的影响研究：量化通常导致自解释质量和忠实度中度下降，但对模型压缩有效性影响不大


<details>
  <summary>Details</summary>
Motivation: 量化被广泛用于加速大语言模型推理和部署，但其对自解释（模型为自身输出提供的解释）的影响尚未被探索。自解释需要模型推理自身决策过程，这种能力可能对量化特别敏感。鉴于自解释在高风险应用中越来越依赖，理解量化是否以及多大程度上降低自解释质量和忠实度至关重要。

Method: 研究两种自解释类型：自然语言解释和反事实示例，使用三种常见量化技术在不同比特宽度下量化LLM。通过用户研究评估量化对自解释连贯性和可信度的影响，比较不同规模模型对量化的韧性。

Result: 量化通常导致自解释质量中度下降（最高4.4%）和忠实度下降（最高2.38%）。用户研究表明量化降低自解释的连贯性和可信度（最高8.5%）。相比小模型，大模型在自解释质量方面对量化的韧性有限，但能更好地保持忠实度。没有一种量化技术在任务准确性、自解释质量和忠实度方面始终表现优异。
...
TL;DR: DepFlow是一个三阶段抑郁症条件文本转语音框架，通过对抗训练学习说话人和内容不变的抑郁症嵌入，构建伪装抑郁症增强数据集，提升抑郁症检测模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有抑郁症数据集（如DAIC-WOZ）存在语言情感与诊断标签强耦合的问题，导致模型学习语义捷径，在真实场景（如伪装抑郁症）中鲁棒性不足。需要解决语义偏见，提升模型在语言内容与抑郁状态不匹配情况下的检测能力。

Method: 1. 抑郁症声学编码器：通过对抗训练学习说话人和内容不变的抑郁症嵌入，实现有效解缠同时保持抑郁症可区分性（ROC-AUC: 0.693）
2. 流匹配TTS模型：使用FiLM调制注入抑郁症嵌入到合成中，控制抑郁严重程度同时保持内容和说话人身份
3. 基于原型的严重程度映射机制：提供平滑可解释的抑郁连续体操作
4. 构建伪装抑郁症增强数据集（CDoA）：将抑郁声学模式与情感分层文本库中的积极/中性内容配对，创建自然数据中少见的声学-语义不匹配样本

Result: CDoA数据集在三种抑郁症检测架构上分别提升macro-F1 9%、12%和5%，持续优于传统增强策略。抑郁症声学编码器ROC-AUC达到0.693，证明有效解缠同时保持抑郁症可区分性。

Conclusion: DepFlow不仅增强了抑郁症检测模型的鲁棒性，还提供了一个可控的合成平台，可用于对话系统和基于模拟的评估，在真实临床数据受伦理和覆盖范围限制的情况下具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型幻觉问题严重影响了AI生成内容的可靠性和可信度。传统不确定性量化方法在常规问答场景中有效，但在非标准或对抗性提问策略下表现不足，这限制了LLM在需要强大批判性思维能力的实际应用中的可靠性。

Method: 研究构建了包含虚假名称的陷阱问题集，并创新性地提出了一种鲁棒的不确定性量化方法(RU)。该方法专门针对多事实生成任务中的不确定性进行量化，以应对对抗性提问场景。

Result: 实验结果表明，构建的陷阱问题集表现优异。在四个不同模型上与基线方法相比，提出的方法表现出色，ROCAUC值相比最佳基线方法平均提高了0.1-0.2。

Conclusion: 该研究为解决大语言模型幻觉问题提供了新的视角和方法，特别是在对抗性提问场景下，提出的鲁棒不确定性量化方法能够有效提升幻觉检测性能，增强了LLM在实际应用中的可靠性。
...
TL;DR: BERT-JEPA (BEPA) 是一种新的训练范式，将JEPA训练目标添加到BERT风格模型中，旨在解决[CLS]嵌入空间坍缩问题，将其转化为语言无关空间，从而在多语言基准测试中提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决BERT风格模型中[CLS]嵌入空间坍缩的问题，并创建语言无关的表示空间，以提升多语言任务的性能。

Method: 在BERT风格模型中加入JEPA（联合嵌入预测架构）训练目标，通过自监督学习方式训练模型，使[CLS]嵌入空间更加丰富和语言无关。

Result: BERT-JEPA在多语言基准测试中表现出性能提升，验证了该方法在创建语言无关表示空间方面的有效性。

Conclusion: 将JEPA训练目标整合到BERT风格模型中是一种有效的策略，能够改善[CLS]嵌入空间的质量，创建语言无关的表示，从而提升多语言任务的性能。
...
TL;DR: Geo-R是一个无需检索的视觉语言模型框架，通过强化学习优化图像地理定位精度，利用基于规则的分层推理范式生成可解释的监督信号。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在图像地理定位方面依赖合成推理标注或外部图像检索，这限制了可解释性和泛化能力。需要开发一种无需检索、基于结构化推理的方法。

Method: 提出Geo-R框架：1) 基于规则的"区域链"分层推理范式，将GPS坐标映射到地理实体（国家、省份、城市）；2) 轻量级强化学习策略，使用基于Haversine距离的坐标对齐奖励，通过空间有意义的反馈优化预测。

Result: 在多个基准测试中验证了Geo-R的有效性，实现了更高的定位精度、更强的泛化能力和更透明的推理过程，为可扩展和可解释的图像地理定位建立了新的无需检索范式。

Conclusion: Geo-R将结构化地理推理与直接空间监督相结合，通过无需检索的方法显著提升了图像地理定位的性能和可解释性，模型和代码将公开以促进进一步研究。
...
TL;DR: 本文介绍了judgeWEL数据集，这是一个用于卢森堡语命名实体识别的数据集，通过利用维基百科和维基数据作为弱监督源，结合大型语言模型验证标注质量，构建了比现有数据集大5倍且更平衡的语料库。


<details>
  <summary>Details</summary>
Motivation: 为低资源语言构建数据集是自然语言处理的主要瓶颈之一，资源稀缺和语言特殊性使得大规模标注成本高昂且可能不一致。卢森堡语作为代表性不足的语言，需要更有效的数据集构建方法。

Method: 提出一种新颖的流水线方法：1) 利用维基百科内部链接和对应的维基数据条目推断实体类型，生成初始弱监督标注；2) 使用多个大型语言模型验证和筛选，仅保留高质量标注句子，减少噪声。

Result: 构建的judgeWEL数据集比现有卢森堡语NER数据集大约5倍，在实体类别上提供更广泛和更平衡的覆盖，为多语言和低资源NER研究提供了重要的新资源。

Conclusion: 该方法通过结合结构化知识源（维基百科/维基数据）和大型语言模型验证，有效解决了低资源语言数据集构建的挑战，为其他代表性不足语言的数据集创建提供了可行路径。
...
TL;DR: 该研究提出了超关系时序知识广义超图（HTKGHs）作为HTKGs的泛化，以支持现实世界事件中常见的复杂事实表示，并基于POLECAT数据库构建了htkgh-polecat数据集，用于评估大语言模型在复杂时序预测任务上的能力。


<details>
  <summary>Details</summary>
Motivation: 现有超关系时序知识图（HTKGs）在表示复杂事实时存在局限性，特别是无法支持超过两个主要实体的时序事实，这在现实世界的地缘政治事件中很常见。为了克服这一限制，需要一种更强大的表示框架。

Method: 1. 提出了HTKGHs的形式化定义，保持向后兼容性的同时支持两种常见的地缘政治事件复杂事实类型；2. 基于POLECAT全球事件数据库构建了htkgh-polecat数据集；3. 在关系预测任务上对流行的大语言模型进行基准测试和分析。

Result: 成功开发了HTKGHs的形式化框架，创建了htkgh-polecat数据集，并对大语言模型在复杂时序预测任务上的适应性和能力进行了基准测试，为模型评估提供了新的工具和见解。

Conclusion: HTKGHs为表示复杂时序事实提供了更强大的框架，htkgh-polecat数据集为评估大语言模型在复杂地缘政治事件预测任务上的能力提供了有价值的基准，有助于推动时序知识图谱和LLMs在复杂预测场景中的应用研究。
...
TL;DR: 比较三种轻量级Transformer模型（DistilBERT、MiniLM、ALBERT）在三个企业NLP任务（客户情感分类、新闻主题分类、毒性仇恨言论检测）上的性能与效率表现，发现各有优势：ALBERT准确率最高，MiniLM推理速度最快，DistilBERT表现最均衡。


<details>
  <summary>Details</summary>
Motivation: 企业NLP应用对高效、轻量级模型的需求日益增长，需要能够处理多领域文本自动化任务的模型。本研究旨在比较三种主流轻量级Transformer模型在不同企业应用场景下的表现，为实际部署提供指导。

Method: 使用IMDB、AG News和Measuring Hate Speech三个数据集，分别在客户情感分类、新闻主题分类、毒性仇恨言论检测三个领域评估DistilBERT、MiniLM和ALBERT模型。评估指标包括准确率、精确率、召回率、F1分数等性能指标，以及模型大小、推理时间、吞吐量、内存使用等效率指标。所有实验在固定企业导向约束下进行控制性微调。

Result: 没有单一模型在所有性能维度上占优：ALBERT在多个领域达到最高的任务特定准确率；MiniLM在推理速度和吞吐量方面表现最佳；DistilBERT在跨任务中表现出最一致的准确率，同时保持有竞争力的效率。所有结果反映了在固定约束下的性能表现，而非超参数优化的极限结果。

Conclusion: 研究揭示了准确率与效率之间的权衡关系，为企业应用提供具体建议：对延迟敏感的应用推荐MiniLM，需要平衡性能的应用推荐DistilBERT，资源受限环境推荐ALBERT。这些发现为企业NLP模型选择提供了实用指导。
...
TL;DR: 该论文对比了语言的社会建构主义解释和数学化的语义场理论，认为两者可以互补而非对立，为AI语言模型提供了理论框架。


<details>
  <summary>Details</summary>
Motivation: 探讨大语言模型为检验长期存在的语言意义理论提供了新的实证环境，旨在对比社会建构主义和数学化语义理论两种解释框架。

Method: 形式化词汇场和语言场作为连续语义空间中的交互结构，分析transformer架构的核心特性（分布式表示、注意力机制、嵌入空间几何规律）与这些概念的关联。

Result: LLM在捕捉语义规律方面的成功支持语言具有底层数学结构的观点，而其在语用推理和语境敏感性方面的局限则符合哲学语言使用理论强调的社会基础重要性。

Conclusion: 数学结构和语言游戏可以理解为互补而非竞争视角，该框架澄清了纯统计语言模型的范围和限制，并为理论指导的AI架构提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 护栏模型对于确保LLM部署安全至关重要，但处理完整多轮对话历史会带来巨大的计算成本。需要一种既能保持安全性能又能显著降低计算开销的方法。

Method: 提出Defensive M2S训练范式，在多轮到单轮(M2S)压缩对话上微调护栏模型，而不是使用完整对话历史。使用三种压缩模板(hyphenize, numberize, pythonize)压缩多轮对话，并在三个护栏模型家族(LlamaGuard, Nemotron, Qwen3Guard)上评估。

Result: M2S将训练成本从O(n²)降低到O(n)，训练数据量减少93倍(从1570万token降至16.9万token)。最佳配置(Qwen3Guard+hyphenize)在SafeDialBench上达到93.8%攻击检测召回率，推理token减少94.6%(从3231降至173)，比基线提升38.9个百分点。
...
TL;DR: 该研究分析了复杂句子结构如何影响基于规则的原子句子提取性能，发现相对从句、同位语、并列谓语等结构对提取准确性构成挑战。


<details>
  <summary>Details</summary>
Motivation: 现有原子句子提取方法缺乏可解释性，无法揭示哪些语言结构导致提取失败。虽然已有研究探索基于依存关系的提取，但缺乏对具体从句结构和依存关系如何影响提取难度的系统性分析。

Method: 使用WikiSplit数据集，在spaCy中实现基于依存关系的提取规则，生成100个黄金标准原子句子集，使用ROUGE和BERTScore评估性能。

Result: 系统达到ROUGE-1 F1=0.6714，ROUGE-2 F1=0.478，ROUGE-L F1=0.650，BERTScore F1=0.5898，显示中到高的词汇、结构和语义对齐。相对从句、同位语、并列谓语、状语从句和被动结构最具挑战性。

Conclusion: 基于规则的提取方法具有合理准确性，但对句法复杂性敏感。相对从句、同位语等复杂结构是主要挑战来源，为未来改进提供了方向。
...
TL;DR: ECR框架通过语义锚点保持紧凑模型嵌入空间的结构一致性，避免语义漂移，无需教师输出且与蒸馏兼容


<details>
  <summary>Details</summary>
Motivation: 紧凑模型在容量受限或多语言数据时容易丢失嵌入空间结构，导致下游任务难以利用表示，现有压缩方法仅对齐表面输出而未能保持底层流形结构

Method: 提出嵌入一致性调节(ECR)框架：从教师嵌入中提取语义锚点（离线计算一次），紧凑模型学习在这些锚点周围保持一致的几何结构，不依赖匹配logits或内部特征，推理时仅添加小型投影步骤
...
TL;DR: 本文提出了一种基于CTC架构的轻量级语言无关多语言ASR系统，采用层次化LoRA-MoE框架，无需推理时的语言标识信息，实现单次解码的竞争性能。
...
Method: 提出语言无关层次化LoRA-MoE（HLoRA）框架，集成到mHuBERT-CTC模型中，通过LID后验驱动的LoRA路由实现端到端解码。层次化设计包括：多语言共享LoRA学习语言不变声学表示，语言特定LoRA专家建模语言依赖特征。路由机制无需推理时的语言标识信息或显式语言标签。

Result: 在MSR-86K和MLC-SLM 2025 Challenge数据集上的实验表明，HLoRA仅通过单次解码就达到了与最先进两阶段推理方法竞争的性能，显著提高了低资源多语言ASR应用的解码效率。

Conclusion: 提出的HLoRA框架为轻量级、语言无关的多语言ASR系统提供了一种有效解决方案，能够在保持性能的同时显著降低计算和延迟成本，适合资源受限的边缘设备部署。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在推理和代码生成方面取得显著进展，但高效创建评估这些能力的新基准仍然具有挑战性。传统基准创建依赖人工努力，成本高且耗时，现有基准常常污染LLM训练数据，需要新颖多样的基准来准确评估其真实能力。

Method: 提出基于KL散度和熵的指标来量化基准新颖性和多样性，无需昂贵的模型评估。开发端到端流水线，使用遗传算法和迭代代码反馈从种子数据集合成稳健的Python编程问题。

Result: 该方法97%的时间能准确生成新问题的测试用例和解决方案，合成的基准相比种子数据集持续表现出更高的新颖性和多样性。算法还提供了控制生成问题新颖性/多样性和难度的方法。

Conclusion: InfoSynth为LLMs提供了一个可扩展、自验证的高质量、新颖多样基准构建流水线，解决了传统基准创建的成本和污染问题。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地部署在成本敏感和边缘设备场景中，而现有的安全防护主要针对英文。中文恶意查询通常通过同音字、拼音、符号分割等中文特有模式隐藏意图，这些中文特有的对抗模式造成了现有基准测试未能很好覆盖的安全评估空白，特别是对于可能更易受此类特定对抗扰动影响的轻量级模型。

Method: 提出了中文特定安全基准测试(CSSBench)，重点关注中文特有的对抗模式，包括同音字、拼音、符号分割等。基准测试覆盖六个在中文场景中常见的领域：非法活动与合规、隐私泄露、健康与医疗错误信息、欺诈与仇恨、成人内容、公共与政治安全，并将查询组织成多种任务类型。评估一组流行的轻量级大语言模型，并测量过度拒绝行为以评估安全性导致的性能下降。

Result: 评估结果显示，中文特有的对抗模式对轻量级大语言模型构成了关键挑战。基准测试提供了对LLM在中文环境中安全性的全面评估。

Conclusion: CSSBench填补了中文特定对抗模式安全评估的空白，为轻量级大语言模型在中文环境中的安全部署提供了全面的评估工具，有助于实际应用中的鲁棒部署。
...
TL;DR: 提出一个模型无关的框架，通过重复采样和多数投票机制，为固定输入任务提供降低LLM幻觉的概率保证


<details>
  <summary>Details</summary>
Motivation: 大语言模型在确定性自动化工作流中经常产生上下文幻觉，当生成内容与提示中明确信息矛盾或忽略时，这种错误特别成问题，因为输入固定且正确性明确

Method: 1. 定义特定任务：固定输入和确定性正确标准；2. 在独立上下文窗口中重复相同提示，获得指数级降低所有输出都错误的概率；3. 使用LLM作为评判者识别正确答案；4. 当评判者不完美时，通过多数投票机制增强评判能力

Result: 在受控提取任务上的实验完全符合理论预测：管道失败概率随重复次数指数下降，幻觉选择概率随评判者数量指数下降
...
Method: 提出快速权重产品键记忆（FwPKM），将稀疏的产品键记忆从静态模块转变为动态的快速权重情景记忆。通过局部块级梯度下降在训练和推理时动态更新参数，使模型能够快速记忆和检索输入序列中的新键值对。

Result: FwPKM作为有效的情景记忆补充标准模块的语义记忆，在长上下文数据集上显著降低困惑度。在Needle in a Haystack评估中，尽管仅在4K标记序列上训练，却能泛化到128K标记的上下文。

Conclusion: FwPKM成功解决了序列建模中存储容量与计算效率的权衡问题，通过动态快速权重机制实现了高效的情景记忆，在长上下文处理方面表现出色。
...
TL;DR: 提出Sigmoid Head方法解决语言模型概率作为质量评估指标不可靠的问题，通过sigmoid激活和负采样策略改进质量评估


<details>
  <summary>Details</summary>
Motivation: 语言模型的概率分布不是可靠的质量评估指标，因为自然语言具有歧义性。当存在多个有效输出选项时，模型的概率分布会分散到这些选项上，从而误导性地表明输出质量较低。这主要由两个原因造成：1) LM的最终输出激活使用softmax，不允许多个正确选项同时获得高概率；2) LM的训练数据是单一one-hot编码的参考文本，暗示每个输出步骤只有一个正确选项。

Method: 提出在预训练语言模型上训练质量评估模块Sigmoid Head。该模块是一个额外的unembedding头，使用sigmoid激活来解决第一个限制。为应对第二个限制，在训练Sigmoid Head的负采样过程中，使用启发式方法避免选择可能正确的替代标记。该方法在训练和推理时计算效率高。

Result: Sigmoid Head产生的概率相比原始softmax头是显著更好的质量信号。由于Sigmoid Head不依赖人工标注的质量数据，相比有监督的质量评估方法，在领域外设置中更具鲁棒性。

Conclusion: 提出的Sigmoid Head方法有效解决了语言模型概率作为质量评估指标的局限性，通过sigmoid激活和智能负采样策略，提供了更可靠的质量评估信号，且在计算效率和领域外鲁棒性方面表现优异。
...
Method: 评估了多种大型语言模型在三个流行任务（情感分析、攻击性语言识别和声明验证）上的文本跨度识别性能，探索了指令调优、上下文学习和思维链等策略。

Result: 结果表明，文本内部的潜在关系有助于大型语言模型识别精确的文本跨度。

Conclusion: 大型语言模型在主观性文本跨度识别任务中具有潜力，文本内部关系对识别精度有重要影响，为下游任务的可解释性提供了支持。
...
TL;DR: 本研究首次在加拿大跨省评估了将BCCRTron和GatorTron两种Transformer模型应用于癌症监测的效果，通过模型集成显著减少了漏诊癌症病例，并提出了隐私保护的省级模型权重共享工作流。


<details>
  <summary>Details</summary>
Motivation: 基于人口的癌症登记依赖病理报告作为主要诊断来源，但人工提取资源密集且导致数据延迟。虽然基于Transformer的NLP系统改进了登记工作流程，但其在不同报告规范的司法管辖区间的泛化能力尚不清楚。

Method: 研究评估了BCCRTron（不列颠哥伦比亚省癌症登记处开发的领域适应Transformer模型）和GatorTron（生物医学Transformer模型）在加拿大癌症监测中的跨省适应能力。使用纽芬兰与拉布拉多省癌症登记处约104,000份和22,000份去标识化病理报告分别训练Tier 1（癌症vs非癌症）和Tier 2（可报告vs不可报告）任务。两种模型通过互补的格式化和诊断重点报告部分输入管道进行微调。

Result: 在NLCR测试集上，适应模型保持高性能，表明在一个司法管辖区预训练的Transformer可以通过适度微调本地化到另一个管辖区。通过保守的OR集成组合两种模型，Tier 1召回率达到0.99，漏诊癌症减少到24例（单独模型分别为48和54例）。Tier 2集成召回率达到0.99，漏诊可报告癌症减少到33例（单独模型分别为54和46例）。

Conclusion: 研究证明结合互补文本表示的集成模型能显著减少漏诊癌症并改善癌症登记NLP的错误覆盖。实现了隐私保护工作流，仅共享模型权重，支持可互操作的NLP基础设施和未来泛加拿大癌症病理和登记工作流程的基础模型。
...
### [77] [Exponential lower bound via exponential sums](https://arxiv.org/abs/2601.00387)
*Somnath Bhattacharjee,Markus Bläser,Pranjal Dutta,Saswata Mukherjee*

Main category: cs.CC

TL;DR: 该论文研究代数电路的指数和是否需要指数大小的代数电路，证明了Shub-Smale τ-猜想蕴含指数下界，并建立了参数化代数复杂度类与计数层次之间的关系。


<details>
  <summary>Details</summary>
Motivation: 研究Valiant的VP vs. VNP猜想中关于符号永久多项式代数电路大小的核心问题，探索指数和代数电路是否必然需要指数大小的电路，这是理解代数复杂度理论中基本界限的重要问题。

Method: 采用参数化复杂度理论工具，分析参数化代数复杂度类VWₙ₀⁰[P]和VW[F]，研究Shub-Smale τ-猜想与指数下界的关系，并建立与计数层次结构的联系。

Result: 证明了Shub-Smale τ-猜想蕴含指数和代数电路的指数下界；获得了VWₙ₀⁰[P]类的指数fpt下界；建立了参数化类与线性计数层次崩溃的关系；用永久多项式刻画了VW[F]类的完备性。

Conclusion: 该研究首次将Shub-Smale τ-猜想应用于证明显式指数下界，深化了对代数电路指数和复杂度的理解，建立了代数复杂度与参数化复杂度、计数层次之间的新联系。
...
### [78] [A Parameterized-Complexity Framework for Finding Local Optima](https://arxiv.org/abs/2601.00560)
*Robert Ganian,Hung P. Hoang,Christian Komusiewicz,Nils Morawietz*

Main category: cs.CC

TL;DR: 该论文提出了一个新的局部搜索计算复杂性框架，要求输出局部最优解及其改进链，以更好地捕捉实际局部搜索算法的设计挑战。通过参数化复杂性分析，在权重多样性参数下获得了固定参数可处理性结果，但在距离最近最优解参数下排除了类似结果。


<details>
  <summary>Details</summary>
Motivation: 传统局部搜索复杂性框架（PLS和标准算法问题）未能准确捕捉实际局部搜索算法的设计挑战。PLS关注找到局部最优解而非使用局部搜索过程，而标准算法问题限制每个改进步骤必须遵循固定的枢轴规则。需要一个新的框架来更好地建模局部搜索算法的设计复杂性。

Method: 提出了一个新的局部搜索计算复杂性框架，要求算法不仅输出局部最优解，还要输出达到该最优解的局部改进链。将这一框架与参数化复杂性理论结合，重点关注两个代表性元问题：具有c-swap邻域的Subset Weight Optimization Problem和具有flip邻域的Weighted Circuit。通过新型归约技术进行分析。

Result: 在权重多样性（不同权重的数量）参数下建立了固定参数可处理性结果，但在距离最近最优解参数下排除了类似的固定参数可处理性结果。这表明新框架能够捕捉局部搜索算法设计的核心挑战，并产生有意义的复杂性分类。

Conclusion: 提出的新框架为局部搜索算法的复杂性分析提供了更准确的模型，能够捕捉枢轴规则设计的挑战。参数化复杂性分析显示，某些参数（如权重多样性）能够实现高效算法，而其他参数（如距离最近最优解）则导致计算困难，这为局部搜索算法的设计提供了理论指导。
...
### [79] [Symmetric Submodular Functions, Uncrossable Functions, and Structural Submodularity](https://arxiv.org/abs/2601.00140)
*Miles Simmons,Ishan Bansal,Joe Cheriyan*

Main category: cs.DM

TL;DR: 本文研究了可塑性集合族的结构子模性，构造了一个满足结构子模性的可塑性集合族，该族既不能由对称子模函数定义，也不能分解为有限个不可交叉族。


<details>
  <summary>Details</summary>
Motivation: 研究可塑性集合族的结构子模性，探索其与对称子模函数和不可交叉族之间的关系，旨在理解这些组合结构的内在性质。

Method: 对于任意正整数d≥2，构造一个满足结构子模性的可塑性集合族，证明该族既不能由对称子模函数定义，也不能分解为d个或更少的不可交叉族。

Result: 成功构造了一个满足结构子模性的可塑性集合族，该族具有两个关键性质：(1)不存在对称子模函数g和有理数λ使得该族等于{g(S)<λ}；(2)该族不能分解为d个或更少的不可交叉族。

Conclusion: 可塑性集合族的结构子模性是一个比对称子模函数和不可交叉族分解更丰富的结构性质，存在满足结构子模性的集合族既不能由对称子模函数定义，也不能分解为有限个不可交叉族。
...
### [80] [The Agentic Leash: Extracting Causal Feedback Fuzzy Cognitive Maps with LLMs](https://arxiv.org/abs/2601.00097)
*Akash Kumar Panda,Olaoluwa Adigun,Bart Kosko*

Main category: cs.AI

TL;DR: 本文设计了一个基于大语言模型的智能体，能够从原始文本中提取因果反馈模糊认知图，并通过双向交互过程实现动态系统的自主演化。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发一个能够从文本中自动提取因果关系的智能系统，使模糊认知图能够动态演化，同时保持一定的自主性。这解决了传统方法需要人工构建认知图的局限性，实现了文本驱动的因果学习。

Method: 方法包括三个精细调校的系统指令：1) 从文本中提取关键名词和名词短语；2) 从这些词汇中提取FCM概念节点；3) 推断节点之间的部分或模糊因果边。使用LLM智能体（Gemini和ChatGPT）生成FCM，并测试混合FCM的效果。

Result: 测试结果显示，通过三步骤生成的FCM动态系统能够收敛到与人工生成的FCM相同的平衡极限环，尽管节点和边的数量不同。混合FCM不仅吸收了主要组件的平衡状态，还创造了新的平衡状态，更好地近似了底层的因果动态系统。

Conclusion: 研究表明，基于LLM的智能体能够有效从文本中提取因果结构，生成的FCM系统具有与人工构建系统相似的动态特性。混合FCM方法进一步增强了系统的表达能力，为文本驱动的因果建模提供了新途径。
...
### [81] [Reasoning in Action: MCTS-Driven Knowledge Retrieval for Large Language Models](https://arxiv.org/abs/2601.00003)
...
Method: 采用粗到细的两阶段知识检索方法：1) 识别知识库中与上下文主题相关的子区域；2) 在该子区域内细化搜索，提取与推理过程特别相关的知识。两阶段都使用蒙特卡洛树搜索启发的方法，通过常见关键词在知识句子中导航。

Result: 在两个多轮对话数据集上的实验表明，该方法不仅更贴近人类对话的底层推理逻辑，还显著提高了检索知识的多样性，从而产生更具信息性和创造性的响应。

Conclusion: 提出的推理感知知识检索方法能够有效整合检索和推理策略，通过逻辑结构对齐的知识检索，显著提升LLMs在多轮对话中的表现。
...
### [82] [Finetuning Large Language Models for Automated Depression Screening in Nigerian Pidgin English: GENSCORE Pilot Study](https://arxiv.org/abs/2601.00004)
...
TL;DR: 使用微调大语言模型进行尼日利亚皮钦语抑郁症筛查，GPT-4.1在准确性和文化适应性上表现最佳


<details>
  <summary>Details</summary>
Motivation: 尼日利亚抑郁症筛查覆盖率低，传统PHQ-9问卷在高收入国家验证，但在尼日利亚存在语言和文化障碍，需要适应本地皮钦语和520多种方言的筛查工具

Method: 收集432份尼日利亚年轻人皮钦语音频响应，进行转录、预处理和标注（语义标签、俚语解释、PHQ-9严重程度评分），微调三种LLM模型（Phi-3-mini-4k-instruct、Gemma-3-4B-it、GPT-4.1）并进行定量和定性评估

Result: GPT-4.1表现最佳，PHQ-9严重程度评分预测准确率达94.5%，在文化适应性、清晰度和相关性方面也优于其他模型

Conclusion: AI驱动的抑郁症筛查可为尼日利亚等语言多样、资源有限地区提供可行的解决方案，为部署对话式心理健康工具奠定基础
...
TL;DR: 本文提出了一种多算法方法来解决最后一公里包裹配送系统中的人力资源工作量平衡问题，通过考虑距离和工作量因素来优化包裹分配，确保每位配送员完成相似的工作量。


<details>
  <summary>Details</summary>
Motivation: 传统的基于地理邻近性的包裹分配方法效率低下，容易导致配送员之间工作量分配不均衡。需要一种能够平衡所有员工工作量的优化方法，纠正特定区域内配送员之间的显著工作量不平衡问题。

Method: 提出了一种多算法方法，包括不同版本的k-means算法、进化方法、基于k-means初始化的递归分配（采用不同问题编码方式）以及混合进化集成算法。该方法以配送点集合和工人数量为输入，综合考虑距离和工作量因素来优化包裹分配。

Result: 该方法在西班牙Azuqueca de Henares城市最后一公里包裹配送系统的实际案例中进行了性能验证，展示了其在实际应用中的有效性。

Conclusion: 通过考虑工作量平衡而不仅仅是地理邻近性，所提出的多算法方法能够有效优化最后一公里包裹配送系统，确保配送员之间的工作量更加均衡，从而提高系统整体效率。
...
TL;DR: 本文提出了一种基于规则的战略性印度拉米纸牌游戏框架，使用新的手牌评估指标MinDist来量化手牌与最近有效配置之间的编辑距离，显著提升了胜率。


<details>
  <summary>Details</summary>
Motivation: 经典印度拉米纸牌是一个不完全信息的序列游戏，需要概率推理和组合决策。传统启发式方法在战略游戏表现上有限，需要更形式化和可解释的策略设计方法。

Method: 提出了基于规则的框架，引入MinDist手牌评估指标（修改MinScore指标，量化手牌与最近有效配置的编辑距离）。设计了计算高效的算法，利用动态剪枝和模式缓存精确计算该指标。在双人零和模拟框架中整合对手手牌建模，并使用统计假设检验评估策略。

Result: 实证结果显示，基于MinDist的智能体相比传统启发式方法在胜率上有显著提升，为算法化拉米策略设计提供了形式化和可解释的步骤。

Conclusion: MinDist指标和相应算法框架为不完全信息纸牌游戏提供了有效的战略决策方法，显著优于传统启发式方法，为算法化游戏策略设计提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在探索生成式AI系统如何理解和再现乡土建筑形式中蕴含的建筑智慧，分析AI在解读传统设计智能方面的能力与局限。
...
Result: AI能可靠地再现几何图案，但误解材料和气候逻辑；参考图像能提高真实性但限制创造性，而无参考的自由生成能产生创新但文化模糊的结果；研究界定了视觉相似性与建筑推理之间的边界。

Conclusion: 研究提出了计算乡土推理框架，用于分析AI如何感知、扭曲和重新想象传统设计智能，揭示了AI在理解建筑深层逻辑方面的局限性。
...
### [86] [Mortar: Evolving Mechanics for Automatic Game Design](https://arxiv.org/abs/2601.00105)
*Muhammad U. Nasir,Yuchen Li,Steven James,Julian Togelius*

Main category: cs.AI

TL;DR: Mortar系统结合质量多样性算法和大型语言模型，自动演化游戏机制，通过树搜索合成完整游戏，评估机制对技能排序的贡献。


<details>
  <summary>Details</summary>
Motivation: 游戏机制设计通常需要专家手动完成，耗时且依赖专业知识。需要自动化系统来探索多样化的游戏机制，减轻设计负担。
...
Conclusion: Mortar系统成功实现了游戏机制的自主演化，能够生成多样化的可玩游戏，并通过技能排序评估机制质量。
...
TL;DR: 研究探讨了在视频问答任务中，基于置信度的弃权机制能否提供可靠的错误率控制，以及这种控制在分布偏移下是否保持稳健。使用NExT-QA数据集和Gemini 2.0 Flash模型，发现置信度阈值能在分布内提供机制性控制，但在分布偏移下控制会失效。


<details>
  <summary>Details</summary>
Motivation: 在视觉语言模型的高风险部署中，需要选择性预测机制，让系统在不确定时弃权而不是冒险产生代价高昂的错误。研究旨在验证基于置信度的弃权是否能在视频问答中提供可靠的错误率控制，以及这种控制在分布偏移下是否保持稳健。

Method: 使用NExT-QA数据集和Gemini 2.0 Flash模型进行研究。通过扫描置信度阈值epsilon来产生平滑的风险-覆盖率权衡曲线，分析置信度阈值对错误率的控制效果。特别关注分布偏移情况下的控制性能。

Result: 研究发现：1）置信度阈值在分布内能提供机制性控制，通过调整阈值可以平滑地降低错误率；2）但在分布偏移下，这种控制会失效，置信度阈值无法可靠地控制错误率。

Conclusion: 虽然基于置信度的弃权在分布内能有效控制错误率，但在分布偏移下缺乏稳健性，这表明需要开发更可靠的置信度校准方法或替代的弃权机制来应对实际部署中的分布变化。
...
TL;DR: 本文比较了三种神经推理方法：LLM推理、监督学习推理和显式模型推理，发现显式模型推理最可靠，并提出Sphere Neural Networks实现逻辑一致性


<details>
  <summary>Details</summary>
Motivation: 研究不同神经推理方法的可靠性，特别是LLM和监督学习在逻辑推理中的局限性，探索更可靠的显式模型构建方法

Method: 提出Sphere Neural Networks，将概念嵌入n维球面作为圆环，通过补圆表示否定算子，过滤不可满足的圆形配置实现可靠决策
...
TL;DR: 研究发现LLM赋能的智能体不仅存在人口统计偏见，还会在最小"我们vs他们"线索下表现出群体间偏见。当这种群体边界与智能体-人类划分重合时，风险从人类群体间差异转向更根本的群体不对称——人类整体可能被智能体视为外群体。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索智能体是否会对人类表现出群体间偏见，特别是在智能体-人类划分成为群体边界的情况下。这种偏见可能导致人类整体被智能体视为外群体，带来比传统人口统计偏见更根本的风险。

Method: 构建基于分配决策的受控多智能体社会模拟，在明确收益权衡下测试群体间偏见。引入信念投毒攻击(BPA)，包括初始化时的档案投毒(BPA-PP)和通过优化信念精炼后缀注入存储反思的记忆投毒(BPA-MP)。

Result: 实验表明：1)智能体在最小群体线索下表现出一致的群体间偏见；2)当部分对应方被标记为人类时偏见会减弱，但这种减弱依赖于智能体相信真实人类存在；3)BPA攻击能有效抑制人类规范脚本，重新激活对人类的外群体偏见。

Conclusion: 研究揭示了智能体群体间偏见的存在及其对人类的风险，提出了信念投毒攻击这一新的攻击面。为构建更安全的智能体框架，需要在档案和记忆边界实施可行的干预措施，以抵御此类攻击。
...
<details>
  <summary>Details</summary>
Motivation: 分布式计算连续系统（DCCS）集成了从物联网设备到云基础设施的异构计算资源，其复杂性、移动性和动态操作条件导致频繁故障，需要可扩展、自适应和自我调节的弹性策略。

Method: 将生物自愈的四个阶段（止血、炎症、增殖、重塑）重构为计算层的四个层次（遏制、诊断、元认知、知识），使用语言模型驱动的智能体解释异构日志、推断根本原因、优化推理路径并重新配置资源。

Result: 在公共故障数据集上评估显示，ReCiSt能在数十秒内完成自愈，智能体CPU使用率最低为10%，并展示了克服不确定性的分析深度和实现弹性所需的微智能体数量。

Conclusion: ReCiSt框架成功将生物自愈原理应用于分布式计算系统，通过语言模型驱动的智能体实现了自主故障管理和系统弹性，为复杂计算环境提供了有效的自愈解决方案。
...
TL;DR: ACCD框架通过三阶段自适应架构检测社交媒体中的协同虚假行为，显著提升检测精度并减少人工标注需求


<details>
  <summary>Details</summary>
Motivation: 现有协同虚假行为检测方法依赖表面相关性分析、使用静态参数设置、需要大量人工标注，存在严重局限性

Method: 提出自适应因果协同检测框架，采用三阶段渐进架构：1)自适应收敛交叉映射识别账户间真实因果关系；2)半监督分类结合主动学习和不确定性采样；3)基于历史检测经验的自动化验证模块

Result: 在Twitter IRA、Reddit协调痕迹等多个真实数据集上，ACCD在协同攻击检测中达到87.3%的F1分数，比现有最佳基线提升15.2%，减少68%人工标注需求，处理速度提升2.8倍
...
TL;DR: 将计算语言学中的语义空间推理扩展到团队运动战术决策，通过将球员视为"词语"、团队战术视为"语义结构"，在共享向量空间中评估战术匹配度和对手利用潜力。


<details>
  <summary>Details</summary>
Motivation: 传统计算语言学中的语义空间方法主要应用于文本分析，本研究探索将其扩展到团队运动的战术决策领域，为集体决策和性能优化提供通用框架。

Method: 将球员表示为整合技术、身体和心理属性的多维向量，通过上下文加权聚合成团队语义表示；在共享向量空间中编码战术模板（如高位逼抢、反击、控球组织），使用向量距离度量评估战术匹配度。

Result: 开发了Python原型系统，能够生成可解释的动态自适应策略建议，并提供属性层面的细粒度诊断洞察；该方法可推广到篮球、曲棍球、协作机器人、人机协调系统等团队领域。

Conclusion: 该方法为团队集体决策和性能优化提供了通用框架，未来方向包括真实数据集成、预测模拟以及混合人机战术智能的发展。
...
TL;DR: 研究发现推理模型中的"顿悟时刻"很罕见，不会随训练变得更频繁，也很少提高准确性，表明它们不是模型自我修正的内在机制，而是推理不稳定的症状。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明像DeepSeek-R1-Zero这样的模型会在推理过程中经历突然的"顿悟时刻"，导致准确输出，暗示了自我修正的内在能力。但尚不清楚这种推理策略的内在转变是否真正提高了性能。

Method: 研究中推理转变并设计训练运行来检测它们。分析涵盖100万+推理轨迹、数百个训练检查点、三个推理领域、多个解码温度和模型架构。

Result: 发现推理转变很罕见，不会随训练变得更频繁，也很少提高准确性，表明它们不符合先前对模型洞察力的认知。但其效果随模型不确定性而变化。基于此发现，在高熵条件下人为触发外在转变能可靠提高准确性。

Conclusion: 推理过程中的转变是不稳定推理行为的症状，而非自我修正的内在机制。通过在高不确定性条件下人为触发转变可以改善性能。
...
Conclusion: DA-DPO是一种成本效益高的框架，通过难度感知的偏好优化有效解决了多模态大语言模型中的过拟合问题，无需新数据或额外的微调阶段，显著提升了幻觉抑制效果。
...
Method: 提出PedX-LLM框架，整合LLaVA提取的视觉特征、文本数据和交通领域知识，通过LoRA微调LLaMA-2-7B基础模型来推断行人过街决策。采用跨站点验证评估泛化能力。

Result: PedX-LLM达到82.0%的平衡准确率，优于最佳统计和监督学习方法。视觉增强模块带来2.9%性能提升，领域知识集成带来额外4.1%改进。在零样本配置下，在5个未见测试站点达到66.9%平衡准确率，比基线数据驱动方法至少高18个百分点。通过少样本学习（仅5个验证示例）可进一步提升至72.2%。
...
### [96] [A Chain-of-Thought Approach to Semantic Query Categorization in e-Commerce Taxonomies](https://arxiv.org/abs/2601.00510)
*Jetlir Duraj,Ishita Khan,Kilian Merkelbach,Mehran Elyasi*

Main category: cs.IR

TL;DR: 该论文提出了一种结合树搜索与LLM语义评分的链式思维方法，用于电商搜索查询的类别分类，相比基于嵌入的方法表现更好，并能检测层次分类体系中的问题。


<details>
  <summary>Details</summary>
Motivation: 电商搜索依赖于结构化的库存分类体系，正确分类用户查询不仅能缩小搜索范围，还能开启多种意图理解能力。然而，在现实电商分类体系中实现准确查询分类是一个基础且重要的问题。

Method: 探索了一种新颖的链式思维范式，将简单的树搜索与大型语言模型的语义评分相结合，用于查询分类任务。同时提出了具有相同思路但能扩展到百万级查询的LLM方法。

Result: 链式思维方法在人工判断的查询-类别对、相关性测试和基于LLM的参考方法评估中，表现优于基于嵌入的查询类别预测基准方法。该方法还能检测层次分类体系中的问题。

Conclusion: 链式思维方法为电商搜索查询分类提供了实用且准确的解决方案，在约束检索项目和提升搜索结果相关性方面具有重要应用价值，并能扩展到大规模查询场景。
...
### [97] [Improving Scientific Document Retrieval with Academic Concept Index](https://arxiv.org/abs/2601.00567)
*Jeyun Lee,Junhyoung Lee,Wonbin Kweon,Bowen Jin,Yu Zhang,Susik Yoon,Dongha Lee,Hwanjo Yu,Jiawei Han,Seongku Kang*

Main category: cs.IR

TL;DR: 论文提出通过构建学术概念索引来改进科学领域检索器的适应方法，解决了传统方法在查询生成和上下文增强中忽视学术概念多样性的问题。


<details>
  <summary>Details</summary>
Motivation: 将通用领域检索器适配到科学领域面临两大挑战：缺乏大规模领域相关标注数据，以及词汇和信息需求存在显著不匹配。现有基于大语言模型的方法（合成查询微调和辅助上下文生成）忽视了科学文档中嵌入的多样化学术概念，导致生成冗余或概念狭窄的查询和上下文。

Method: 提出学术概念索引，从论文中提取关键概念并按学术分类体系组织。基于此索引改进两个方向：1）CCQGen：基于概念覆盖的查询生成，自适应地让大语言模型关注未覆盖的概念以生成具有更广概念覆盖的互补查询；2）CCExpand：概念聚焦的辅助上下文，利用一组文档片段作为对概念感知查询的简洁响应。

Result: 大量实验表明，将学术概念索引整合到查询生成和上下文增强中，能够产生更高质量的查询、更好的概念对齐，并显著提升检索性能。

Conclusion: 通过构建结构化的学术概念索引来指导查询生成和上下文增强，能够有效解决科学领域检索中的概念覆盖问题，显著提升检索器的适应效果和性能。
...
### [98] [Reinforcement-Learned Unequal Error Protection for Quantized Semantic Embeddings](https://arxiv.org/abs/2601.00186)
*Moirangthem Tiken Singh,Adnan Arif*

Main category: cs.LG

TL;DR: 该论文提出了一种基于强化学习的自适应重复编码框架，在有限带宽下实现按维度不等错误保护，以更好地保持语义意义。


<details>
  <summary>Details</summary>
Motivation: 解决在有限带宽通信系统中保持语义意义的挑战，传统信道编码（如LDPC或Reed-Solomon）无法实现细粒度语义保护。

Method: 引入强化学习框架，通过自适应重复编码实现按维度不等错误保护；使用复合语义失真度量，平衡全局嵌入相似性和实体级保留。

Result: 相比均匀保护，在1 dB SNR下获得6.8%更高的chrF分数和9.3%更好的实体保留，统计显著性提升。

Conclusion: 代码结构必须与语义粒度对齐，智能分配的简单重复编码能实现细粒度语义保护，特别适用于边缘计算和物联网场景。
...
### [99] [Evaluating Anomaly Detectors for Simulated Highly Imbalanced Industrial Classification Problems](https://arxiv.org/abs/2601.00005)
*Lesley Wheat,Martin v. Mohrenschildt,Saeid Habibi*

Main category: cs.LG

TL;DR: 该研究系统评估了14种异常检测算法在不同类别不平衡条件下的表现，发现最佳检测器高度依赖于训练数据中故障样本的总数量，而非健康样本的数量。


<details>
  <summary>Details</summary>
Motivation: 工业系统中机器学习应用面临极端类别不平衡的挑战，主要由于训练期间故障数据的可用性有限。需要评估异常检测算法在实际工程约束下的性能。

Method: 使用问题无关的模拟数据集，包含2D和10D的超球面异常分布。在异常率0.05%到20%、训练规模1000到10000的数据集上，对14种检测器进行基准测试，测试集大小为40000。

Result: 当故障样本少于20个时，无监督方法(kNN/LOF)占优；当有30-50个故障样本时，半监督(XGBOD)和监督(SVM/CatBoost)方法性能显著提升。在只有两个特征时，半监督方法优势不明显，但在十个特征时改进明显。

Conclusion: 研究强调了异常检测方法在小数据集上泛化性能的下降，为工业环境中部署异常检测提供了实用见解，指出故障样本数量而非健康样本数量是性能的关键决定因素。
...
### [100] [TeleDoCTR: Domain-Specific and Contextual Troubleshooting for Telecommunications](https://arxiv.org/abs/2601.00691)
*Mohamed Trabelsi,Huseyin Uzunalioglu*

Main category: cs.LG

TL;DR: TeleDoCTR是一个面向电信领域的端到端故障排除系统，通过集成领域特定排序和生成模型，自动化处理工单分类、历史工单检索和故障分析报告生成，显著提升电信工单处理的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 电信领域的工单故障排除是一个高度复杂且耗时的过程，需要专家解读工单内容、查阅文档和搜索历史记录。这种人工密集型方法不仅延迟问题解决，还阻碍整体运营效率。需要一种自动化系统来提升电信工单故障排除的效果和效率。

Method: 提出TeleDoCTR系统，这是一个针对电信领域的、领域特定的、上下文感知的故障排除系统。系统集成了领域特定排序模型和生成模型，自动化处理故障排除工作流程的三个关键步骤：1) 将工单路由到适当的专家团队（分类任务）；2) 检索上下文和语义相似的历史工单（检索任务）；3) 生成详细的故障分析报告，概述问题、根本原因和潜在解决方案（生成任务）。
...
### [101] [The Trojan in the Vocabulary: Stealthy Sabotage of LLM Composition](https://arxiv.org/abs/2601.00065)
*Xiaoze Liu,Weichen Yu,Matt Fredrikson,Xiaoqian Wang,Jing Gao*

Main category: cs.LG

TL;DR: 论文提出了一种针对LLM模型组合技术的攻击方法，通过设计一个在捐赠模型中功能惰性但在移植到基础模型后能可靠重构为恶意特征的"破坏令牌"，利用系数重用的几何特性破坏基础模型的生成能力。


<details>
  <summary>Details</summary>
Motivation: 随着开源权重LLM生态系统中模型组合技术（如权重合并、推测解码和词汇扩展）的普及，不同模型家族之间的令牌移植成为关键前提。然而，这种必要的互操作性步骤引入了供应链漏洞，需要研究其安全风险。

Method: 将攻击形式化为双目标优化问题，使用稀疏求解器实例化攻击。通过利用系数重用的几何特性，创建一个不对称的可实现性差距，设计一个在捐赠模型中功能惰性但在移植后能重构为高显著性恶意特征的"破坏令牌"。

Result: 攻击是无需训练的，通过谱模仿逃避异常检测，同时在微调和权重合并后仍保持结构持久性。实证表明攻击能有效破坏基础模型的生成能力，而捐赠模型的效用与正常行为在统计上无法区分。

Conclusion: 令牌移植作为模块化AI组合管道中的关键步骤，存在隐藏的安全风险。攻击展示了模型组合技术中的供应链漏洞，强调了在模型互操作性设计中需要考虑安全性的重要性。
...
### [102] [Stronger Approximation Guarantees for Non-Monotone γ-Weakly DR-Submodular Maximization](https://arxiv.org/abs/2601.00611)
*Hareshkumar Jadav,Ranveer Singh,Vaneet Aggarwal*

Main category: cs.LG

TL;DR: 提出了一种用于在向下封闭凸体上最大化非负、非单调γ-弱DR-次模函数的近似算法，其性能保证随γ平滑变化，在γ=1时恢复0.401近似比，在γ<1时性能优雅下降。


<details>
  <summary>Details</summary>
Motivation: 在机器学习和优化中，最大化约束下的次模目标是一个基本问题。研究在向下封闭凸体上最大化非负、非单调γ-弱DR-次模函数的问题，旨在为这类更一般的函数类提供有效的近似算法。

Method: 结合Frank-Wolfe引导的连续贪婪框架与γ感知的双贪婪步骤，形成简单有效的处理非单调性的方法。该方法专门针对γ-弱DR-次模函数的特性进行设计。

Result: 算法性能保证随γ平滑变化：当γ=1（DR-次模情况）时恢复0.401近似比；当γ<1时性能优雅下降，且改进了先前在相同约束下γ-弱DR-次模最大化的报告界限。

Conclusion: 该方法为在向下封闭凸体上最大化非单调γ-弱DR-次模函数提供了最先进的性能保证，通过结合连续贪婪框架和γ感知的双贪婪步骤，有效处理了非单调性问题。
...
### [103] [Exploration in the Limit](https://arxiv.org/abs/2601.00084)
*Brian M. Cho,Nathan Kallus*

Main category: cs.LG

TL;DR: 本文提出了一种渐近置信度的最优臂识别新框架，通过放松严格的精确误差控制要求，在渐近意义上保证误差控制，从而在非参数设置中实现更紧的最优性和更好的样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有固定置信度的最优臂识别方法在实际应用中存在局限性，严格的精确误差控制需要使用宽松的尾不等式和/或参数限制，导致样本效率低下。许多实际场景涉及弱信号、高显著性要求和实验后推断需求，这些都需要较长的实验周期。

Method: 提出了一种渐近框架，要求误差控制在最小样本量渐近意义下有效；开发了新颖的渐近任意时间有效置信序列；设计了新的BAI算法，灵活整合协变量进行方差缩减，在完全非参数设置中确保近似误差控制。

Result: 在温和的收敛假设下，提供了样本复杂度的渐近界限；最坏情况样本复杂度与高斯BAI在精确误差保证和已知方差下的最佳情况样本复杂度相匹配；实验表明该方法在保持误差控制的同时减少了平均样本复杂度。

Conclusion: 提出的渐近框架克服了传统BAI方法的局限性，通过放松精确误差控制要求，在非参数设置中实现了更紧的最优性和更好的样本效率，同时保持了渐近误差控制，适用于实际应用场景。
...
### [104] [Dynamic Bayesian Optimization Framework for Instruction Tuning in Partial Differential Equation Discovery](https://arxiv.org/abs/2601.00088)
*Junqi Qu,Yan Zhang,Shangqian Gao,Shibo Li*

Main category: cs.LG

TL;DR: NeuroSymBO提出了一种自适应指令选择方法，通过贝叶斯优化在方程发现过程中动态选择最优推理策略，解决了LLM在方程发现中的指令脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在方程发现方面有潜力，但其输出对提示词非常敏感（指令脆弱性）。静态提示无法适应多步生成过程的演化状态，导致模型停留在次优解。

Method: 将提示工程重构为顺序决策问题，维护离散推理策略库，使用贝叶斯优化根据数值反馈在每个步骤选择最优指令。

Result: 在PDE发现基准测试中，自适应指令选择显著优于固定提示，实现了更高的恢复率和更简洁的解决方案。

Conclusion: 通过动态调整指令策略，NeuroSymBO有效解决了LLM在方程发现中的指令脆弱性问题，提升了模型性能和解决方案质量。
...
### [105] [GRL-SNAM: Geometric Reinforcement Learning with Path Differential Hamiltonians for Simultaneous Navigation and Mapping in Unknown Environments](https://arxiv.org/abs/2601.00116)
*Aditya Sai Ellendula,Yi Wang,Minh Nguyen,Chandrajit Bajaj*

Main category: cs.LG

TL;DR: GRL-SNAM是一个用于未知环境中同时导航与建图的几何强化学习框架，通过局部能量景观编码可达性和障碍约束，无需构建全局地图。


<details>
  <summary>Details</summary>
Motivation: 解决未知环境中同时导航与建图(SNAM)的挑战，传统方法需要构建全局地图或设计复杂的多智能体策略，而GRL-SNAM旨在仅依赖局部感官观测实现高效导航。

Method: 将路径导航和建图建模为动态最短路径搜索和发现过程，使用受控哈密顿优化：将感官输入转换为局部能量景观编码可达性、障碍屏障和变形约束，通过更新哈密顿量演化感知、规划和重配置策略。

Result: 在2D导航任务中评估，相比局部反应式基线和全局策略学习方法，GRL-SNAM保持安全距离，泛化到未见过的布局，通过局部能量优化而非广泛全局建图实现高质量导航。
...
### [106] [Reinforcement Learning with Function Approximation for Non-Markov Processes](https://arxiv.org/abs/2601.00151)
*Ali Devran Kara*
...
<details>
  <summary>Details</summary>
Motivation: 研究在非马尔可夫状态和成本过程下使用线性函数逼近的强化学习方法，解决传统马尔可夫假设不适用时的学习问题。

Method: 1. 首先分析策略评估方法，证明在非马尔可夫过程的遍历性条件下算法收敛；2. 研究Q-learning的线性函数逼近，证明在基于量化映射选择基函数的特殊情况下可以收敛；3. 将结果应用于部分可观测马尔可夫决策过程，使用有限记忆变量作为状态表示。

Result: 1. 策略评估算法在适当遍历性条件下收敛，极限对应于正交投影与辅助马尔可夫决策过程贝尔曼算子联合算子的不动点；2. Q-learning在基于量化映射选择基函数的特殊情况下可以收敛；3. 为部分可观测马尔可夫决策过程推导了学习算法极限的显式误差界。

Conclusion: 该研究为非马尔可夫环境下的强化学习提供了理论保证，证明了在适当条件下线性函数逼近方法的收敛性，并为部分可观测系统提供了实用的学习框架和误差分析。
...
### [107] [The Weather Paradox: Why Precipitation Fails to Predict Traffic Accident Severity in Large-Scale US Data](https://arxiv.org/abs/2601.00152)
*Yann Bellec,Rohan Kaman,Siwen Cui,Aarav Agrawal,Calvin Chen*

Main category: cs.LG

TL;DR: 本研究使用XGBoost模型分析美国交通事故严重程度预测，发现时间、地理位置和天气变量是最强预测因子，但模型对极端严重事故的预测能力有限。
...
Conclusion: 研究为交通管理提供了实证依据，但数据集中中等严重程度事故占主导限制了模型对极端情况的预测能力，未来需要替代采样策略、增强特征工程和整合外部数据集。
...
### [108] [Online Finetuning Decision Transformers with Pure RL Gradients](https://arxiv.org/abs/2601.00167)
...
<details>
  <summary>Details</summary>
Motivation: 决策变换器在离线强化学习中表现出色，但在在线设置中仍主要依赖监督序列建模目标。论文发现后见回报重新标注这一标准组件与基于重要性采样的RL算法不兼容，阻碍了纯RL梯度的在线微调。

Method: 提出了新算法，将GRPO适配到决策变换器，并引入关键修改：子轨迹优化以改进信用分配、序列级似然目标以增强稳定性和效率、主动采样以鼓励在不确定区域探索。

Result: 通过大量实验证明，该方法优于现有的在线决策变换器基线，在多个基准测试中实现了新的最先进性能，验证了纯RL梯度在线微调的有效性。

Conclusion: 该研究成功实现了决策变换器的纯强化学习梯度在线微调，解决了后见回报重新标注与重要性采样算法的兼容性问题，为序列决策建模提供了更有效的在线优化框架。
...
### [109] [Sequential Reservoir Computing for Efficient High-Dimensional Spatiotemporal Forecasting](https://arxiv.org/abs/2601.00172)
*Ata Akbari Asanjan,Filip Wudarski,Daniel O'Connor,Shaun Geaney,Elena Strbac,P. Aaron Lott,Davide Venturelli*

Main category: cs.LG

TL;DR: 提出Sequential Reservoir Computing架构，通过将大储层分解为一系列小型互连储层，降低高维时空系统预测的计算成本和内存需求，相比传统RNN/LSTM和标准RC方法获得更长的有效预测时间和更低误差。


<details>
  <summary>Details</summary>
Motivation: 传统RNN和LSTM模型在高维时空系统预测中面临梯度训练和内存瓶颈问题，而传统Reservoir Computing虽然通过固定循环层和凸读出优化缓解了这些问题，但在输入维度增加时仍然存在扩展性问题。

Method: 提出Sequential Reservoir Computing架构，将大型储层分解为一系列小型、互连的储层序列。这种设计在保持长期时间依赖性的同时，显著降低了内存和计算成本。

Result: 在低维混沌系统(Lorenz63)和高维物理模拟(2D涡度和浅水方程)上，Sequential RC相比LSTM和标准RNN基线：获得15-25%更长的有效预测时间，20-30%更低的误差指标(SSIM, RMSE)，训练成本降低达三个数量级。

Conclusion: Sequential RC在保持传统RC简单性和效率的同时，为高维动力系统提供了更好的可扩展性，为科学和工程应用中的实时、节能预测提供了实用途径。
...
### [110] [Early Prediction of Liver Cirrhosis Up to Three Years in Advance: A Machine Learning Study Benchmarking Against the FIB-4 Score](https://arxiv.org/abs/2601.00175)
*Zhuqi Miao,Sujan Ravi,Abdulaziz Ahmed*

Main category: cs.LG

TL;DR: 机器学习模型利用常规电子健康记录数据预测肝硬化，在1-3年预测期内显著优于传统FIB-4评分
...
Method: 回顾性队列研究，使用大型学术医疗系统的去标识化电子健康记录数据。识别脂肪肝患者并根据ICD-9/10编码分为肝硬化和非肝硬化队列。构建观察窗口和预测窗口模拟真实临床使用场景。收集人口统计学、诊断、实验室结果、生命体征和共病指数等特征。使用XGBoost模型训练1年、2年和3年预测模型，并在独立测试集上评估性能，与FIB-4评分进行AUC比较

Result: 最终队列包括1年预测3,043名患者、2年预测1,981名患者、3年预测1,470名患者。机器学习模型在所有预测时间窗口均优于FIB-4评分：XGBoost模型在1年、2年、3年预测的AUC分别为0.81、0.73、0.69，而FIB-4评分分别为0.71、0.63、0.57。随着预测时间延长，性能优势持续存在，表明模型具有更好的早期风险识别能力

Conclusion: 基于常规电子健康记录数据的机器学习模型在早期肝硬化预测方面显著优于传统FIB-4评分，能够实现更早、更准确的风险分层，可作为自动化决策支持工具整合到临床工作流程中，支持主动的肝硬化预防和管理
...
TL;DR: 提出SSI-GAN架构，仅需1-3%标注数据即可高精度分类蚊子神经元尖峰信号，检测寨卡病毒、登革热病毒感染，大幅减少人工标注工作量。


<details>
  <summary>Details</summary>
Motivation: 蚊子是虫媒病毒疾病的主要传播媒介，手动分类其神经元尖峰模式非常耗时昂贵。现有深度学习方案需要完全标注的数据集和高度预处理的信号，难以在实际场景中大规模应用。为了解决标注数据稀缺问题，需要开发半监督学习方法。

Method: 提出半监督Swin启发式GAN（SSI-GAN）架构，包含基于Transformer的生成器和Swin启发的移位窗口判别器。使用多头自注意力模型在平坦的基于窗口的Transformer判别器中学习捕获稀疏的高频尖峰特征。仅使用1-3%标注数据训练，收集超过1500万个尖峰样本，分为寨卡病毒感染、登革热病毒感染或未感染三类。使用贝叶斯Optuna框架优化超参数，并通过五折蒙特卡洛交叉验证验证鲁棒性。

Result: SSI-GAN在感染后第三天仅用3%标注数据达到99.93%的分类准确率。在仅1%监督下，在所有感染阶段都保持高准确率。相比标准监督方法，在相同性能水平下减少了97-99%的人工标注工作量。提出的移位窗口Transformer设计大幅超越所有基线方法，在基于尖峰的神经元感染分类中创造了新的最佳记录。

Conclusion: SSI-GAN架构通过创新的半监督学习方法，显著减少了神经元尖峰分类所需的人工标注工作量，为实际场景中的大规模应用提供了可行性，在蚊子病毒神经趋向性检测方面表现出卓越性能。
...
TL;DR: 提出一种面向资源受限边缘设备的心律失常检测框架，通过特征工程使高维数据线性可分，实现98.44%准确率、8.54KB模型大小和0.46μs推理延迟。
...
Method: 提出数据中心的资源高效框架，优先特征工程而非模型复杂度。通过时间-频率小波分解与图论结构描述符（如PageRank中心性）的集成，使复杂高维心律失常数据线性可分。使用互信息和递归消除优化混合特征空间，采用可解释的超轻量线性分类器。

Result: 在MIT-BIH和INCART数据集上验证，达到98.44%诊断准确率，模型大小仅8.54KB。系统实现0.46μs分类推理延迟，每搏处理管道52ms，确保实时操作。相比压缩模型KD-Light（25KB，96.32%准确率）实现数量级效率提升。

Conclusion: 该框架为资源受限边缘设备提供了高效的心律失常检测方案，显著提升计算效率，推动无电池心脏传感器的发展。
...
TL;DR: 该论文研究了生成模型溯源问题，提出利用未标注的互联网数据来提升对未知生成器的识别能力，通过约束优化方法显著提高了在开放世界场景下的AI生成内容溯源性能。


<details>
  <summary>Details</summary>
Motivation: 随着逼真生成模型的快速发展，需要超越简单的真假检测，实现特定生成模型的溯源。现有方法在遇到未见过的或新发布的生成器时泛化能力有限。

Method: 使用CLIP特征和线性分类器建立基线，然后提出约束优化方法：利用未标注的互联网数据（可能包含真实图像、未知生成器输出或目标模型样本），鼓励野生样本被分类为非目标，同时约束在标注数据上的性能保持高水平。

Result: 实验结果表明，加入野生数据显著提高了在具有挑战性的未见生成器上的溯源性能，证明未标注的互联网数据可以有效提升开放世界场景下的AI生成内容溯源能力。

Conclusion: 利用未标注的野生数据可以有效增强生成模型溯源系统的泛化能力，特别是在面对未知和新发布的生成器时，为开放世界场景下的AI生成内容溯源提供了有效解决方案。
...
### [114] [Can Optimal Transport Improve Federated Inverse Reinforcement Learning?](https://arxiv.org/abs/2601.00309)
...
Conclusion: 该工作为推导在异构智能体和环境中通用的共享奖励函数提供了一个原则性和通信高效的框架
...
### [115] [Quantum King-Ring Domination in Chess: A QAOA Approach](https://arxiv.org/abs/2601.00318)
*Gerhard Stenzel,Michael Kölle,Tobias Rohe,Julian Hager,Leo Sünkel,Maximilian Zorn,Claudia Linnhoff-Popien*

Main category: cs.LG

TL;DR: 该研究提出了基于国际象棋战术的量子近似优化算法基准测试QKRD，包含5000个结构化实例，用于评估QAOA设计选择，发现约束保持混合器、热启动策略等能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有QAOA基准测试主要使用MaxCut、TSP、SAT等随机合成实例，这些实例缺乏语义结构和人类可解释性，无法有效评估QAOA在具有实际约束的真实问题上的性能。

Method: 引入量子王车易位控制问题(QKRD)作为NISQ规模的基准测试，基于国际象棋战术位置构建5000个结构化实例，具有单热约束、空间局部性和10-40量子比特规模。该基准结合人类可解释的覆盖度指标和针对经典启发式算法的内在验证。

Result: 约束保持混合器(XY、domain-wall)比标准混合器收敛快约13步；热启动策略减少45步收敛时间，能量改进超过d=8；CVaR优化产生负面结果，能量更差且无覆盖度优势。QAOA表现优于贪婪启发式算法12.6%，优于随机选择80.1%。

Conclusion: 结构化基准测试能揭示问题感知的QAOA技术优势，这些优势在随机实例中被掩盖。研究发布了所有代码、数据和实验工件，支持可重复的NISQ算法研究。
...
TL;DR: 提出E-GRPO方法，通过熵感知的组相对策略优化来增强流匹配模型的人类偏好对齐，通过合并低熵步骤为高熵SDE采样步骤，提高探索效率


<details>
  <summary>Details</summary>
Motivation: 现有方法在多个去噪步骤上进行优化时面临稀疏和模糊的奖励信号问题，高熵步骤能实现更高效的探索，而低熵步骤导致无差别的轨迹

Method: 提出E-GRPO（熵感知组相对策略优化），合并连续的低熵步骤形成一个高熵SDE采样步骤，其他步骤使用ODE采样，并引入多步组归一化优势函数
...
### [117] [A Comparative Analysis of Interpretable Machine Learning Methods](https://arxiv.org/abs/2601.00428)
*Mattia Billa,Giovanni Orlandi,Veronica Guidetti,Federica Mandreoli*

Main category: cs.LG

TL;DR: 对16种可解释机器学习方法在216个真实世界表格数据集上的大规模比较评估，揭示性能层次结构并分析数据集特征对性能的影响


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在医疗、金融、法律等高风险领域的广泛应用，模型可解释性和问责制日益重要。尽管可解释ML受到关注，但对表格数据固有可解释模型的系统性评估仍然相对稀缺，且往往只关注聚合性能结果。

Method: 对16种固有可解释方法进行大规模比较评估，包括经典线性模型、决策树以及EBMs、符号回归、GOSDT等新方法。研究涵盖216个真实世界表格数据集，根据数据集结构特征（维度、样本量、线性度、类别不平衡）分层分析性能，并评估训练时间和分布偏移下的鲁棒性。

Result: 结果揭示了清晰的性能层次结构，特别是在回归任务中，EBMs始终表现出强大的预测准确性。同时，性能高度依赖于上下文：SR和IGANNs在非线性场景下表现特别好，而GOSDT模型对类别不平衡表现出明显的敏感性。
...
### [118] [Controllable Concept Bottleneck Models](https://arxiv.org/abs/2601.00451)
...
<details>
  <summary>Details</summary>
Motivation: 传统概念瓶颈模型主要关注静态场景，但实际应用中需要动态维护：删除错误/敏感数据、纠正错误概念标签、纳入新样本以适应环境变化。现有方法需要重新训练，在大规模应用中效率低下

Method: 提出可控概念瓶颈模型(CCBMs)，基于影响函数推导出数学上严格的闭式近似解，支持三种粒度编辑：概念标签级（纠正概念标签）、概念级（修改概念层）、数据级（数据删除和添加）

Result: 实验结果表明CCBMs具有高效性和适应性，能够实现动态可信的概念瓶颈模型，具有实际应用价值

Conclusion: CCBMs通过数学严谨的闭式近似解解决了概念瓶颈模型的动态编辑问题，无需重新训练即可支持多种粒度编辑，为实际部署提供了高效可靠的解决方案
...
### [119] [Imitation from Observations with Trajectory-Level Generative Embeddings](https://arxiv.org/abs/2601.00452)
*Yongtao Qu,Shangzhe Li,Weitong Zhang*

Main category: cs.LG

TL;DR: TGE：一种基于轨迹级生成嵌入的离线观察模仿学习方法，通过扩散模型在潜在空间估计专家状态密度，构建密集平滑的替代奖励，有效处理专家演示稀缺且离线数据与专家行为差异大的情况。


<details>
  <summary>Details</summary>
Motivation: 现有基于分布匹配的离线观察模仿学习方法在专家演示稀缺且离线数据与专家行为差异大的情况下表现不佳，因为它们施加了严格的支持约束并依赖脆弱的单步模型，难以从不完美的数据中提取有用信号。

Method: 提出TGE（轨迹级生成嵌入）方法：在离线轨迹数据上训练时序扩散模型，利用其学习到的扩散嵌入的平滑几何特性，在潜在空间中估计专家状态密度，构建密集平滑的替代奖励函数，从而捕捉长期时序动态并弥合不相交支持之间的差距。

Result: 在D4RL运动和控制基准测试中，TGE方法持续匹配或优于先前的离线观察模仿学习方法，证明了其在处理专家演示稀缺且离线数据分布与专家行为差异大的情况下的有效性。

Conclusion: TGE通过轨迹级生成嵌入方法有效解决了离线观察模仿学习中专家演示稀缺且离线数据不完美的挑战，利用扩散模型的平滑几何特性构建鲁棒的学习信号，为处理分布差异大的数据提供了有效解决方案。
...
### [120] [Geometric Regularization in Mixture-of-Experts: The Disconnect Between Weights and Activations](https://arxiv.org/abs/2601.00457)
*Hyunjun Kim*
...
Method: 在MoE模型中应用正交性损失，通过7种正则化强度分析权重空间和激活空间重叠，评估对性能的影响

Result: 正交性损失在多方面失败：权重空间重叠增加114%，激活空间重叠保持高位(~0.6)，性能影响不一致且高度可变，权重与激活正交性无显著相关性
...
### [121] [Detecting Spike Wave Discharges (SWD) using 1-dimensional Residual UNet](https://arxiv.org/abs/2601.00459)
*Saurav Sengupta,Scott Kilianski,Suchetha Sharma,Sakina Lashkeri,Ashley McHugh,Mark Beenhakker,Donald E. Brown*

Main category: cs.LG

TL;DR: 本文提出了一种基于1D UNet的数据增强方法（AugUNet1D），用于自动标记脑电图中的棘波放电事件，相比传统方法和现有算法表现更优。


<details>
  <summary>Details</summary>
Motivation: 脑电图（EEG）记录中事件的手动标记非常耗时，特别是在连续数周至数月的记录中。棘波放电（SWD）作为失神发作的脑电图标志，通常需要手动标记。虽然已有研究使用机器学习自动分割和分类EEG信号，但仍有改进空间。

Method: 研究比较了14种机器学习分类器在961小时EEG记录（包含22,637个标记SWD）上的性能，发现1D UNet表现最佳。通过数据增强改进1D UNet，其中缩放增强效果最显著。最终开发了AugUNet1D，并与最近发表的"Twin Peaks"算法进行比较。

Result: AugUNet1D在SWD标记任务中表现优于"Twin Peaks"算法，检测到的事件特征更接近手动标记的SWD。模型已公开供其他用户使用，包括预训练和未训练版本。

Conclusion: AugUNet1D是一种有效的自动标记EEG中棘波放电事件的方法，通过数据增强显著提升了1D UNet的性能，为EEG分析提供了高效的工具。
...
### [122] [Laplacian Kernelized Bandit](https://arxiv.org/abs/2601.00461)
*Shuang Wu,Arash A. Amini*

Main category: cs.LG

TL;DR: 提出了一种基于图结构的多用户上下文赌博机方法，通过构建统一的多用户再生核希尔伯特空间，将图拉普拉斯正则化与核化赌博机相结合，设计了理论上有保障的探索算法。


<details>
  <summary>Details</summary>
Motivation: 研究多用户上下文赌博机问题，其中用户通过图结构相互关联，且奖励函数既表现出非线性特性又具有图同质性。现有方法在处理这种复杂结构时存在局限性，需要一种能够同时考虑图结构和非线性奖励的统一框架。

Method: 引入了一种原则性的联合惩罚项，结合了基于RKHS距离的图平滑项和个体粗糙度惩罚。证明了该惩罚等价于单一统一的多用户RKHS中的平方范数，并显式推导了其再生核，该核优雅地融合了图拉普拉斯矩阵和基础臂核。基于此设计了LK-GP-UCB和LK-GP-TS算法，利用高斯过程后验进行探索。

Result: 提供了高概率遗憾界，其缩放与多用户核的有效维度相关，取代了对用户数量或环境维度的依赖。实证结果表明，在非线性设置中，该方法优于强线性和非图感知基线，即使在真实奖励为线性时也保持竞争力。

Conclusion: 提出了一个统一、理论上有基础且实用的框架，将拉普拉斯正则化与核化赌博机相结合，用于结构化探索，为图结构多用户上下文赌博机问题提供了有效的解决方案。
...
### [123] [Trajectory Guard -- A Lightweight, Sequence-Aware Model for Real-Time Anomaly Detection in Agentic AI](https://arxiv.org/abs/2601.00516)
*Laksh Advani*

Main category: cs.LG

TL;DR: Trajectory Guard：一种用于检测LLM智能体多步行动计划异常的混合方法，通过孪生循环自编码器结合对比学习和重建损失，能同时检测任务轨迹对齐问题和计划结构问题，相比现有方法性能显著提升且推理速度快。


<details>
  <summary>Details</summary>
Motivation: 现有的异常检测方法不适合检测LLM智能体生成的多步行动计划失败问题：均值池化嵌入会稀释异常步骤，而纯对比学习方法忽略了序列结构。标准的无监督方法在预训练嵌入上F1分数不超过0.69。

Method: 提出Trajectory Guard方法，使用孪生循环自编码器（Siamese Recurrent Autoencoder）和混合损失函数，通过对比学习联合学习任务-轨迹对齐，通过重建学习序列有效性。这种双重目标能够统一检测"错误的任务计划"和"畸形的计划结构"。

Result: 在涵盖合成扰动和真实世界失败的基准测试中（包括安全审计RAS-Eval和多智能体系统Who&When），在平衡数据集上达到0.88-0.94的F1分数，在不平衡外部基准测试上达到0.86-0.92的召回率。推理延迟仅32毫秒，比LLM Judge基线快17-27倍。

Conclusion: Trajectory Guard方法能够有效检测LLM智能体多步行动计划的异常，同时兼顾任务对齐和结构完整性，具有高性能和低延迟的特点，适合生产部署中的实时安全验证。
...
### [124] [Federated Customization of Large Models: Approaches, Experiments, and Insights](https://arxiv.org/abs/2601.00526)
*Yuchuan Ye,Ming Ding,Youjia Chen,Peng Cheng,Dusit Niyato*

Main category: cs.LG

TL;DR: 本文探讨了大型模型在联邦学习框架下的定制化方法，首次尝试将prefix-tuning应用于联邦学习环境，验证了其可行性并展示了竞争优势。


<details>
  <summary>Details</summary>
Motivation: 探索大型模型在联邦学习框架下的定制化方法，解决联邦学习中模型定制面临的关键挑战，填补prefix-tuning在联邦学习环境中应用的研究空白。

Method: 首先回顾了多种大型模型定制技术（全微调、高效微调、提示工程、prefix-tuning、知识蒸馏、检索增强生成），然后讨论这些技术在联邦学习框架下的实现方式，最后重点实验了联邦prefix-tuning方法。

Result: 联邦prefix-tuning实验验证了其在联邦学习环境中的可行性，性能接近集中式方法。与其他三种联邦定制方法相比，表现出竞争性性能、令人满意的效率和一致的鲁棒性。

Conclusion: 本文成功将prefix-tuning应用于联邦学习框架，证明了其有效性，为大型模型在联邦学习环境中的定制化提供了新的可行方案。
...
### [125] [Cloud-Native Generative AI for Automated Planogram Synthesis: A Diffusion Model Approach for Multi-Store Retail Optimization](https://arxiv.org/abs/2601.00527)
...
<details>
  <summary>Details</summary>
Motivation: 传统货架陈列图设计耗时且昂贵，平均每个复杂布局需要30小时。零售业需要更高效、自动化的解决方案来优化店铺空间布局。

Method: 采用云原生架构，结合AWS进行云端模型训练和边缘部署进行实时推理。使用扩散模型学习多个零售店的成功货架布局，并通过修改损失函数集成零售特定约束。

Result: 系统将货架陈列图设计时间减少98.3%（从30小时降至0.5小时），约束满足率达到94.4%。经济分析显示成本降低97.5%，投资回收期为4.4个月。架构支持线性扩展，可处理10,000个并发店铺请求。
...
### [126] [Entropy Production in Machine Learning Under Fokker-Planck Probability Flow](https://arxiv.org/abs/2601.00554)
*Lennon Shikhman*
...
<details>
  <summary>Details</summary>
Motivation: 现有漂移检测方法缺乏动力学原理解释，无法指导再训练频率与运营成本之间的平衡。机器学习模型在非平稳环境中部署时，由于数据漂移导致性能下降

Method: 将部署时数据漂移建模为Fokker-Planck方程控制的概率流，使用时变Kullback-Leibler散度量化模型-数据不匹配，推导出包含非负熵产生项的熵平衡分解，提出基于熵触发的无标签干预策略

Result: 在受控非平稳分类实验中，熵触发再训练实现了与高频再训练相当的预测性能，同时将再训练事件减少了一个数量级（相比每日和基于标签的策略）

Conclusion: 基于熵的再训练框架为数据漂移提供了动力学原理解释，通过熵触发机制在保持性能的同时显著降低再训练频率，平衡了性能与运营成本
...
### [127] [Adversarial Samples Are Not Created Equal](https://arxiv.org/abs/2601.00577)
*Jennifer Crawford,Amol Khanna,Fred Lu,Amy R. Wagoner,Stella Biderman,Andre T. Nguyen,Edward Raff*

Main category: cs.LG

TL;DR: 论文提出对抗样本应分为两类：利用非鲁棒特征的样本和不利用这些特征的样本，并提出了衡量非鲁棒特征操纵程度的集成度量方法。


<details>
  <summary>Details</summary>
Motivation: 现有非鲁棒特征理论无法解释那些不直接利用这些特征的对抗样本，需要区分不同类型的对抗弱点来更准确地评估对抗鲁棒性。

Method: 提出基于集成学习的度量方法，用于衡量对抗扰动对非鲁棒特征的操纵程度，并用该度量分析攻击者生成的对抗样本构成。

Result: 通过新度量方法能够区分两种对抗样本类型，并重新审视多个现象，包括锐度感知最小化对对抗鲁棒性的影响，以及在鲁棒数据集上对抗训练与标准训练之间的鲁棒性差距。

Conclusion: 对抗样本应分为利用非鲁棒特征和不利用非鲁棒特征两类，这种区分对于准确评估对抗鲁棒性至关重要，新提出的度量方法为此提供了有效工具。
...
### [128] [HFedMoE: Resource-aware Heterogeneous Federated Learning with Mixture-of-Experts](https://arxiv.org/abs/2601.00583)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然能保护数据隐私地微调大语言模型，但LLM的巨大规模使得在资源受限的移动设备上进行训练不切实际。MoE模型通过激活稀疏专家子集来降低计算负担，但将其集成到FL微调中面临三个关键挑战：1)缺乏可靠指标评估专家对本地性能的影响；2)客户端计算资源异构性；3)客户端特定专家子集和路由偏好破坏全局聚合。

Method: 提出HFedMoE框架：1)基于专家对微调性能的贡献评估专家重要性；2)从信息瓶颈角度自适应选择专家子集以匹配客户端计算预算；3)设计稀疏感知模型聚合策略，基于重要性加权贡献聚合主动微调的专家和门控参数。

Result: 大量实验表明，HFedMoE在训练准确性和收敛速度方面优于最先进的基准方法。

Conclusion: HFedMoE成功解决了MoE-based FL微调中的关键挑战，通过定制化专家子集和智能聚合机制，实现了在资源受限客户端上高效的大语言模型微调。
...
### [129] [Memory Bank Compression for Continual Adaptation of Large Language Models](https://arxiv.org/abs/2601.00756)
*Thomas Katraouras,Dimitrios Rafailidis*

Main category: cs.LG

TL;DR: MBC提出了一种通过码本优化策略压缩记忆库的方法，用于大语言模型的持续学习，在保持高准确率的同时将记忆库大小减少到基线方法的0.3%


<details>
  <summary>Details</summary>
Motivation: 大语言模型的知识容易过时，持续学习旨在更新模型而不遗忘旧知识。现有记忆增强方法面临记忆库随数据流不断增长的问题，需要有效的压缩方案

Method: 提出MBC模型，通过码本优化策略在线压缩记忆库，引入在线重置机制防止码本崩溃，并在注意力层使用Key-Value低秩适配来高效利用压缩后的记忆表示

Result: 在基准问答数据集上的实验表明，MBC将记忆库大小减少到最竞争基线的0.3%，同时在在线适应学习中保持高保持准确率

Conclusion: MBC通过码本压缩和在线重置机制有效解决了记忆库增长问题，为大语言模型的持续学习提供了高效的内存管理方案
...
### [130] [Geometry of Reason: Spectral Signatures of Valid Mathematical Reasoning](https://arxiv.org/abs/2601.00791)
*Valentin Noël*
...
<details>
  <summary>Details</summary>
Motivation: 需要一种无需训练数据、微调或学习分类器的方法来检测大语言模型中的数学推理有效性，以应用于幻觉检测和AI安全监控。

Method: 将注意力矩阵视为token动态图的邻接矩阵，提取四个谱诊断指标：Fiedler值（代数连通性）、高频能量比、图信号平滑度和谱熵，通过统计显著性分析区分有效和无效数学证明。

Result: 在7个transformer模型（来自4个独立架构家族）上的实验显示，该方法效应量高达Cohen's d=3.30，分类准确率达85.0-95.6%，校准阈值在完整数据集上达到93-95%准确率。谱方法检测的是逻辑连贯性而非编译器接受度。

Conclusion: 谱图分析为推理验证提供了一个原则性框架，具有立即应用于幻觉检测和AI安全监控的潜力，并揭示了注意力机制设计影响哪些谱特征捕获推理有效性。
...
### [131] [Do Chatbot LLMs Talk Too Much? The YapBench Benchmark](https://arxiv.org/abs/2601.00624)
*Vadim Borisov,Michael Gröger,Mina Mikhael,Richard H. Schreiber*

Main category: cs.LG

TL;DR: YapBench是一个轻量级基准测试，用于量化LLM在需要简洁回答的提示上的过度生成问题，通过测量超出最小足够答案的额外字符数来评估模型的多余输出。


<details>
  <summary>Details</summary>
Motivation: 当前LLM（如ChatGPT、Claude、Gemini）在简单请求上经常给出不必要的冗长回答，包含冗余解释、模棱两可的表述和模板化内容，增加了认知负担和推理成本。先前研究表明基于偏好的后训练和LLM评估会引入系统性长度偏差，即使质量相当，更长的回答也更容易获得奖励。

Method: 引入YapBench基准测试，包含300多个英文提示，涵盖三种需要简洁回答的场景：需要简短澄清的模糊输入、有简短稳定答案的封闭式事实问题、以及只需单行代码的技术任务。每个项目包含单轮提示、精心设计的最小足够基准答案和类别标签。主要指标YapScore测量超出基准答案的字符数，YapIndex是类别级别中位数YapScore的均匀加权平均值。

Result: 评估76个助手型LLM，发现中位数超额长度存在数量级差异，并观察到特定类别的失败模式：在模糊输入上填充真空内容，在单行技术请求上添加解释或格式化开销。基准测试和实时排行榜已发布，用于跟踪模型冗长行为随时间的变化。

Conclusion: YapBench提供了一个系统化的方法来量化LLM的过度生成问题，揭示了不同模型在简洁性方面的显著差异和特定失败模式，有助于推动开发更简洁高效的LLM助手。
...
### [132] [Interpretability-Guided Bi-objective Optimization: Aligning Accuracy and Explainability](https://arxiv.org/abs/2601.00655)
...
<details>
  <summary>Details</summary>
Motivation: 现有可解释模型训练方法缺乏有效整合结构化领域知识的机制，且传统特征重要性计算方法存在分布外问题，需要开发既能保持模型准确性又能强制执行领域知识约束的优化框架。

Method: 提出IGBO框架：1) 将特征重要性层次编码为有向无环图；2) 使用时序集成梯度测量特征重要性；3) 引入最优路径预言机学习数据流形感知的积分路径以解决OOD问题；4) 采用双目标优化公式平衡模型准确性和可解释性约束。

Result: 理论分析证明了收敛性和对mini-batch噪声的鲁棒性；在时间序列数据上的实验表明，IGBO能有效强制执行DAG约束，同时保持最小精度损失，优于标准正则化基线方法。

Conclusion: IGBO为训练可解释模型提供了一个有效的双目标优化框架，能够整合结构化领域知识，解决特征重要性计算中的分布外问题，在保持模型性能的同时增强可解释性。
...
### [133] [IRPO: Scaling the Bradley-Terry Model via Reinforcement Learning](https://arxiv.org/abs/2601.00677)
*Haonan Song,Qingchen Xie,Huan Zhu,Feng Xiao,Luxi Xing,Fuzhen Li,Liu Kang,Feng Jiang,Zhiyong Zheng,Fan Yang*

Main category: cs.LG

TL;DR: IRPO提出了一种新的强化学习框架，通过将Bradley-Terry模型集成到GRPO中，解决了成对生成奖励模型的计算瓶颈问题，实现了高效的点式评分和可解释性。


<details>
  <summary>Details</summary>
Motivation: 成对生成奖励模型（GRMs）虽然具有可解释性和推理时扩展性，但与GRPO等强化学习算法结合时存在计算瓶颈：1）成对比较需要O(n^2)时间复杂度；2）重复采样或链式思考推理带来额外计算开销。
...
Conclusion: IRPO通过解决成对GRMs的计算瓶颈，提供了一种高效、可扩展且性能优越的强化学习框架，在保持可解释性的同时显著提升了训练效率。
...
### [134] [ARISE: Adaptive Reinforcement Integrated with Swarm Exploration](https://arxiv.org/abs/2601.00693)
*Rajiv Chaitanya M,D R Ramesh Babu*

Main category: cs.LG

TL;DR: ARISE是一个轻量级框架，通过添加基于粒子群的探索层来增强强化学习，在非平稳奖励和高维策略任务中显著提升性能。
...
Method: ARISE框架在标准策略梯度方法基础上增加了一个紧凑的基于粒子群的探索层。它将策略动作与粒子驱动的建议相结合，每个粒子代表在动作空间中采样的候选策略轨迹，并使用奖励方差线索自适应地调节探索。

Result: 在简单基准测试中只有轻微改进（CartPole-v1 +0.7%），但在更具挑战性的任务中取得显著提升：LunarLander-v3 +46%，Hopper-v4 +22%，同时在Walker2d和Ant上保持稳定性。在非平稳奖励变化下，ARISE表现出明显的鲁棒性优势，在CartPole上比PPO高出75分，在LunarLander上也有相应改进。消融研究证实粒子群组件和自适应机制都对性能有贡献。

Conclusion: ARISE提供了一种简单、架构无关的方法，可以在不改变核心算法结构的情况下，创建更具探索性和鲁棒性的强化学习智能体。
...
### [135] [Bayesian Inverse Games with High-Dimensional Multi-Modal Observations](https://arxiv.org/abs/2601.00696)
*Yash Jain,Xinjie Liu,Lasse Peters,David Fridovich-Keil,Ufuk Topcu*

Main category: cs.LG

TL;DR: 提出基于贝叶斯推理的逆博弈框架，通过变分自编码器学习智能体目标的后验分布，提高不确定性量化能力，实现更安全的决策


<details>
  <summary>Details</summary>
Motivation: 现有最大似然逆博弈方法只能提供点估计，无法量化估计不确定性，导致下游规划可能过度自信地采取不安全行动

Method: 提出近似贝叶斯推理方法，训练带有可微分纳什博弈求解器的结构化变分自编码器，从交互数据中学习先验和后验分布

Result: 框架成功学习先验和后验分布，相比最大似然估计方法提高推理质量，实现更安全的下游决策而不牺牲效率

Conclusion: 贝叶斯逆博弈框架能有效量化不确定性，多模态推理在轨迹信息不足时可进一步减少不确定性，提升自主决策安全性
...
### [136] [BSAT: B-Spline Adaptive Tokenizer for Long-Term Time Series Forecasting](https://arxiv.org/abs/2601.00698)
*Maximilian Reinwardt,Michael Eichelbeck,Matthias Althoff*

Main category: cs.LG

TL;DR: BSAT模型通过B样条自适应分词器解决长时序预测中自注意力二次复杂度和均匀分块不灵活的问题，使用参数免费方法自适应分割时间序列，结合混合位置编码，在内存受限场景下实现高压缩率下的强性能。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer在长时序预测中存在两个主要问题：自注意力的二次计算复杂度限制了处理长序列的能力；均匀分块方法可能无法与数据的语义结构对齐，导致信息提取效率低下。

Method: 提出B样条自适应分词器（BSAT），通过拟合B样条自适应分割时间序列，在高曲率区域放置token，将变长基函数表示为固定大小的token（包含系数和位置）。同时提出混合位置编码L-RoPE，结合可学习的加性位置编码和具有层间可学习基数的旋转位置嵌入。

Result: 在多个公开基准测试中，该模型在保持竞争力的同时实现了高压缩率，特别适合内存受限的使用场景。

Conclusion: BSAT通过自适应分词和混合位置编码有效解决了长时序预测中的计算效率和语义对齐问题，为内存受限场景提供了高效的解决方案。
...
### [137] [Precision Autotuning for Linear Solvers via Contextual Bandit-Based RL](https://arxiv.org/abs/2601.00728)
...
<details>
  <summary>Details</summary>
Motivation: 在科学计算中，混合精度数值方法需要平衡计算精度和效率。传统方法通常使用固定精度，无法根据具体问题动态调整。本文旨在开发一个自适应精度调优框架，利用强化学习根据系统特征动态选择最优精度配置。

Method: 将精度调优问题建模为上下文多臂老虎机问题，使用离散化状态空间和增量动作值估计。通过Q表将离散化特征（如近似条件数和矩阵范数）映射到动作（特定步骤的精度配置），采用epsilon-greedy策略优化多目标奖励函数，平衡精度和计算成本。

Result: 在线性系统迭代精化应用中，该框架能有效选择精度配置，在保持与双精度基线相当精度的同时显著降低计算成本。框架对样本外数据具有良好泛化能力，验证了RL在精度自动调优中的有效性。

Conclusion: 这是首个基于强化学习的精度自动调优工作，在未见数据集上得到验证。该框架可推广到其他数值算法，为科学计算中的混合精度数值方法提供了新思路，展示了RL在精度选择方面的潜力。
...
### [138] [The Reasoning-Creativity Trade-off: Toward Creativity-Driven Problem Solving](https://arxiv.org/abs/2601.00747)
*Max Ruiz Luyten,Mihaela van der Schaar*

Main category: cs.LG

TL;DR: 本文分析了当前LLM推理流程中基于正确性的强化学习导致推理路径分布坍缩的问题，提出了一个统一的变分目标框架DCR，该框架能够防止多样性衰减，保持模型的创造性和正确性。


<details>
  <summary>Details</summary>
Motivation: 当前基于正确性优化的LLM推理流程（如采样多样思维链并强化最高分路径）会导致模型推理路径分布的坍缩，降低语义熵并削弱创造性问题解决能力。需要一种既能保持正确性又能维持创造性的方法。

Method: 提出了分布创造性推理（DCR）框架，将训练视为通过解决方案轨迹概率测度的梯度流。该统一变分目标包含了STaR、GRPO、DPO以及熵奖励等多种方法作为特例。

Result: 1. 提出了多样性衰减定理，描述了基于正确性的目标如何导致STaR、GRPO和DPO的不同多样性衰减模式；2. 设计了确保收敛到稳定且多样策略的方法，有效防止坍缩；3. 提供了简单可行的实践方案。

Conclusion: DCR为LLM提供了首个既保持正确性又维持创造性的原则性方案，解决了当前推理流程中分布坍缩的问题，为创造性问题解决提供了理论框架。
...
### [139] [A Machine Learning Framework for Off Ball Defensive Role and Performance Evaluation in Football](https://arxiv.org/abs/2601.00748)
*Sean Groom,Shuo Wang,Francisco Belo,Axl Rice,Liam Anderson*

Main category: cs.LG

TL;DR: 提出基于协变量依赖隐马尔可夫模型(CDHMM)的足球角球防守分析方法，实现无标签的盯人与区域防守识别，并建立防守贡献评估框架


<details>
  <summary>Details</summary>
Motivation: 传统足球防守评估指标难以捕捉无球防守的协调运动，现有价值模型主要评估有球动作，而反事实方法（如ghosting模型）依赖缺乏战术背景的"平均"行为模拟

Method: 针对角球这一高度结构化的比赛场景，开发协变量依赖隐马尔可夫模型(CDHMM)，直接从球员追踪数据推断时间分辨的盯人和区域防守分配，并基于此提出防守贡献归因框架和角色条件ghosting方法

Result: 该方法能够提供可解释的防守贡献评估，相对于情境感知基准线进行有效分析

Conclusion: 提出的CDHMM模型和防守评估框架为足球无球防守表现提供了更准确、情境化的分析工具，特别适用于角球等结构化场景
...
### [140] [Categorical Reparameterization with Denoising Diffusion models](https://arxiv.org/abs/2601.00781)
...
<details>
  <summary>Details</summary>
Motivation: 传统分类变量梯度优化方法存在局限性：基于评分函数的估计器虽然无偏但噪声大，而连续松弛方法虽然能提供路径梯度但存在偏差且依赖温度参数。需要一种更好的方法来处理分类变量的优化问题。

Method: 引入基于扩散的软重参数化方法，利用高斯噪声过程下的去噪器具有闭式解且可高效计算的特点，通过训练免费的扩散采样器实现反向传播。

Result: 实验表明，提出的重参数化技巧在各种基准测试中取得了竞争性或改进的优化性能。

Conclusion: 扩散基软重参数化为分类分布优化提供了一种有效的新方法，能够平衡梯度估计的准确性和计算效率。
