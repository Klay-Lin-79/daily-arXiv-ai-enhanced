...
TL;DR: 提出两步生成式数据增强框架，结合规则掩码变形与GAN无配对图像转换，生成逼真戴口罩人脸样本，超越纯合成变换


<details>
  <summary>Details</summary>
Motivation: 数据稀缺和分布偏移是戴口罩人脸检测与识别的主要挑战，现有方法难以生成足够真实多样的训练数据

Method: 两步框架：1) 规则掩码变形；2) 基于GAN的无配对图像转换。引入非掩码保护损失和随机噪声注入以稳定训练并增强样本多样性

Result: 相比纯规则变形方法，该框架在定性上获得一致改进，补充了现有GAN方法如IAMGAN。实验验证了所提组件的有效性
...
TL;DR: JARVIS是一个受JEPA启发的自监督视觉增强框架，用于提升多模态大语言模型的视觉理解能力，通过整合I-JEPA学习范式来学习图像的结构和语义规律，而不依赖语言监督。
...
Method: JARVIS将I-JEPA学习范式整合到MLLM训练的标准视觉-语言对齐流程中。使用冻结的视觉基础模型作为上下文和目标编码器，训练预测器（实现为LLM的早期层）从图像中学习结构和语义规律，不依赖语言监督。

Result: 在标准MLLM基准测试上的广泛实验表明，JARVIS在不同LLM家族中一致地提升了视觉中心基准测试的性能，且不会降低多模态推理能力。

Conclusion: JARVIS框架通过自监督视觉增强有效解决了MLLMs在视觉理解上的局限性，提供了一种不依赖语言监督的学习图像结构和语义规律的方法，显著提升了视觉推理能力。
...
TL;DR: 该论文提出了稀疏基础视觉导航任务，通过CityNav基准测试评估多模态大语言模型在城市导航中的序列决策能力，发现现有方法表现不佳，并提出Verbalization of Path方法显著提升导航成功率。


<details>
  <summary>Details</summary>
Motivation: 当前基于多模态大语言模型的具身智能体评估基准过于语言中心化或依赖模拟环境，缺乏对实际复杂场景中知识密集型推理能力的测试，需要建立更贴近真实世界挑战的评估框架。

Method: 提出稀疏基础视觉导航任务，构建CityNav基准测试涵盖四个全球城市，要求智能体仅凭视觉输入和内部多模态推理在50+决策点进行导航，无需环境标注或架构修改。为解决现有方法表现不佳的问题，提出Verbalization of Path方法，通过显式构建认知地图（关键地标和朝向目的地的方向）来增强推理基础。

Result: 评估显示当前最先进的多模态大语言模型和标准推理技术（如思维链、反思）在这一挑战性设置中表现显著不佳。提出的Verbalization of Path方法通过显式基础化智能体内部推理，大幅提升了导航成功率。

Conclusion: 稀疏基础视觉导航任务和CityNav基准测试揭示了多模态大语言模型在真实世界知识密集型导航中的局限性，提出的Verbalization of Path方法通过显式构建认知地图有效提升了导航性能，为开发更强大的具身智能体提供了重要方向。
...
TL;DR: R4是一个无需训练的检索增强推理框架，为视觉语言模型提供结构化、终身记忆的4D时空知识库，通过语义、空间和时间检索实现动态环境中的推理。


<details>
  <summary>Details</summary>
Motivation: 受人类在四维时空中感知和推理能力的启发，希望为视觉语言模型赋予类似的结构化、持久性记忆能力，使其能够进行情境依赖的推理，包括回忆过去事件、推断未观察状态和整合新信息。

Method: R4框架持续构建4D知识数据库，将对象级语义描述锚定在度量空间和时间中，形成持久的世界模型。推理时，自然语言查询被分解为语义、空间和时间键来检索相关观察，然后整合到视觉语言模型的推理过程中。

Result: 在具身问答和导航基准测试中，R4相比基线方法在时空信息检索和推理方面有显著提升，展示了无需训练即可实现情景记忆和协作推理的能力。

Conclusion: R4为动态环境中的具身4D推理提供了新范式，通过结构化记忆系统增强了视觉语言模型在时空维度上的理解和推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然能力不断增强，但其感知能力缺乏系统评估。大多数模型家族在扩展语言组件时重复使用几乎相同的视觉编码器，这引发了一个关键问题：模型进步是真正基于视觉基础，还是依赖互联网规模的文本世界知识。现有评估方法过于强调最终任务准确率，忽视了鲁棒性、归因保真度和受控扰动下的推理能力。

Method: 提出了"感知观测台"框架，从多个维度评估MLLMs：(1)简单视觉任务，如人脸匹配和视觉文本理解能力；(2)局部到全局理解，包括图像匹配、网格指向游戏和属性定位，测试一般视觉基础。每个维度都使用真实数据集（人脸和文字），并通过基于像素的增强和基于扩散的风格化幻觉进行系统扰动。

Result: 该框架超越了排行榜准确率，提供了关于MLLMs如何在扰动下保持感知基础和关系结构的见解，为分析当前和未来模型的优势和弱点提供了原则性基础。

Conclusion: 感知观测台框架为系统评估多模态大语言模型的视觉感知能力提供了新的方法论，能够揭示模型是真正基于视觉基础还是依赖文本知识，为未来模型开发提供了重要的评估工具。
...
TL;DR: CAMP-VLM：基于视觉语言模型的框架，通过视觉上下文特征和场景图空间感知，从第三人称视角预测多人类行为，在合成和真实数据上表现优于基线66.9%


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注第一人称视角下单人类行为预测，但许多机器人应用需要从第三人称视角理解多人类行为，缺乏相关数据集和方法

Method: 提出CAMP-VLM框架，结合视觉输入的上下文特征和场景图的空间感知能力；使用照片级真实模拟器生成合成数据进行微调，采用监督微调(SFT)和直接偏好优化(DPO)方法

Result: 在合成和真实世界序列上评估，CAMP-VLM比最佳基线模型预测准确率提升高达66.9%，展示了良好的泛化能力

Conclusion: CAMP-VLM框架有效解决了从第三人称视角预测多人类行为的挑战，通过视觉语言模型结合上下文和空间信息，在缺乏真实数据集的情况下利用合成数据取得了显著性能提升
...
TL;DR: 该论文探索了基于视觉语言模型（VLMs）的少样本多光谱目标检测，通过将Grounding DINO和YOLO-World适配到多光谱输入，在FLIR和M3FD基准测试中显著优于专用多光谱模型。


<details>
  <summary>Details</summary>
Motivation: 多光谱目标检测在自动驾驶和监控等安全敏感应用中至关重要，但标注数据的稀缺限制了深度检测器的训练。文本类别信息可以作为有价值的语义监督源，而视觉语言模型在计算机视觉中的成功为少样本多光谱检测提供了新思路。

Method: 将两种代表性的VLM检测器（Grounding DINO和YOLO-World）适配到多光谱输入，提出有效机制整合文本、视觉和热模态信息，利用大规模VLMs学习到的语义先验知识。

Result: 在FLIR和M3FD两个多光谱图像基准测试中，VLM检测器在少样本场景下显著优于使用可比数据训练的专业多光谱模型，在全监督设置下也达到竞争性或更优的结果。

Conclusion: 大规模视觉语言模型学习到的语义先验知识能够有效迁移到未见过的光谱模态，为数据高效的多光谱感知提供了强大途径，展示了VLMs在少样本多光谱目标检测中的巨大潜力。
...
TL;DR: 当前视觉语言模型在农业分类任务中表现不佳，零样本性能远低于专用监督模型，不适宜作为独立的农业诊断系统，但可作为辅助组件使用。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型被广泛提议作为视觉识别任务的通用解决方案，但其在农业决策支持中的可靠性尚未得到充分理解。本研究旨在评估VLMs在农业分类任务中的实际表现。

Method: 在AgML集合的27个农业分类数据集上进行基准测试，涵盖162个类别，包括植物病害、害虫和损害、植物和杂草物种识别。比较了开源和闭源VLMs，使用零样本评估、多项选择和开放式提示，并应用基于LLM的语义判断来改进评估方法。

Result: 零样本VLMs在所有任务中都显著低于专用监督基线模型（YOLO11）。最佳VLM（Gemini-3 Pro）在多项选择提示下达到约62%的平均准确率，而开放式提示性能更低（通常低于25%）。语义判断提高了开放式准确率（例如从21%提高到30%）。开源模型中Qwen-VL-72B表现最佳，接近闭源模型但仍有差距。植物和杂草物种分类比害虫和损害识别更容易。

Conclusion: 当前现成的VLMs尚不适合作为独立的农业诊断系统，但可以与受限界面、明确标签本体和领域感知评估策略结合，作为辅助组件使用。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么采用两阶段流水线限制了跨模态信息共享，要么依赖单模态扩散模型无法充分利用预训练视频知识。需要克服这些限制来利用大规模视频数据进行机器人学习。
...
TL;DR: 提出了一种基于3D多通道patch扩散模型的方法，从低角度分辨率扩散MRI预测高角度分辨率纤维方向分布，通过FOD-patch适配器、体素级条件协调模块和SH注意力模块提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 单壳层低角度分辨率扩散MRI（LAR-FOD）估计纤维方向分布的准确性有限，而多壳层高角度分辨率扩散MRI（HAR-FOD）需要较长的扫描时间，限制了其临床应用。扩散模型在基于LAR-FOD估计HAR-FOD方面显示出潜力，但由于FOD中球谐系数数量庞大，高效生成HAR-FOD具有挑战性。

Method: 提出3D多通道patch扩散模型预测HAR-FOD，设计FOD-patch适配器引入先验脑解剖知识以提升基于patch的学习效率；引入体素级条件协调模块增强模型的全局理解能力；设计SH注意力模块有效学习球谐系数的复杂相关性。

Result: 实验结果表明，该方法在HAR-FOD预测方面取得了最佳性能，优于其他最先进的方法。

Conclusion: 提出的3D多通道patch扩散模型能够有效从LAR-FOD预测HAR-FOD，通过创新的适配器、协调模块和注意力机制设计，解决了球谐系数数量庞大带来的挑战，为临床应用中减少扫描时间同时保持高精度提供了可行方案。
...
TL;DR: 提出Auto-Vocabulary 3D目标检测(AV3DOD)，无需用户指定类别即可自动生成检测对象的类别名称，在ScanNetV2和SUNRGB-D数据集上实现定位和语义质量的最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D目标检测方法虽然在训练时未见过的类别上能进行定位，但仍需用户在训练和推理时指定类别。作者希望研究真正自动化的3D目标检测，无需任何用户输入即可自动生成检测对象的类别名称。

Method: 提出AV3DOD框架：1)引入语义评分(SS)评估生成类别名称的质量；2)利用2D视觉语言模型(VLMs)通过图像描述生成丰富的语义候选；3)生成伪3D边界框；4)进行特征空间语义扩展。

Result: 在ScanNetV2和SUNRGB-D数据集上实现了定位(mAP)和语义质量(SS)的最优性能。相比当前最优方法CoDA，在ScanNetV2上整体mAP提升3.48，SS相对提升24.5%。

Conclusion: AV3DOD框架成功实现了无需用户输入的自动词汇3D目标检测，在定位精度和生成语义质量方面均达到最优性能，为真正的开放词汇3D场景理解提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机作为新型仿生视觉传感器具有高动态范围和高时间分辨率优势，但其几何标定（特别是长距离测量场景中的内参和外参确定）仍然是一个重大挑战
...
Conclusion: 提出的基于闪烁星点准直仪的事件相机标定方法能够有效解决长距离高精度测量需求，为事件相机的实际应用提供了可靠的标定解决方案
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在视频生成领域取得了显著进展，但生成速度缓慢限制了其实际应用。需要开发高效的加速框架来解决计算瓶颈问题。

Method: 采用三种主要加速组件：1) 注意力加速：使用低比特SageAttention和可训练稀疏线性注意力；2) 步骤蒸馏：采用rCM进行高效步骤蒸馏；3) W8A8量化：将模型参数和激活量化为8位以加速线性层和压缩模型。同时包含其他工程优化。

Result: 在多个模型上实验表明，即使在单张RTX 5090 GPU上，TurboDiffusion也能实现100-200倍的视频生成加速，同时保持可比的视频质量。

Conclusion: TurboDiffusion是一个有效的视频生成加速框架，显著提升了扩散模型的生成效率，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 相机标定是摄影测量和3D视觉应用中的关键步骤，传统方法存在环境依赖性强、标定过程复杂等问题，需要更可靠、可控且灵活的标定解决方案

Method: 设计了准直仪系统，利用其独特的光学几何特性提出角度不变性约束，证明标定目标与相机之间的相对运动符合球面运动模型。基于此约束提出了多图像闭式线性求解器和两图像最小求解器，以及无需相机运动的单准直仪图像标定算法

Result: 在合成和真实世界实验中验证了方法的可行性，证明使用准直仪系统进行标定是有效的，并且该方法优于现有的基线方法

Conclusion: 提出的准直仪相机标定方法通过角度不变性约束简化了标定过程，实现了灵活快速的标定，为相机标定提供了新颖的解决方案
...
TL;DR: C-DGPA提出了一种基于类中心的双对齐生成提示适配方法，通过双分支架构同时优化边缘分布对齐和条件分布对齐，在无监督域自适应任务中取得了新的SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有的提示调优策略主要对齐边缘分布，但忽略了条件分布差异，导致类原型错位和语义判别性下降等关键问题。需要同时解决这两种分布差异来提升域自适应性能。

Method: 提出C-DGPA方法：1）边缘分布对齐分支采用动态对抗训练框架来桥接边缘分布差异；2）条件分布对齐分支引入类映射机制（CMM），通过标准化语义提示理解和防止源域过度依赖来对齐条件分布差异。

Result: 在OfficeHome、Office31和VisDA-2017等基准数据集上的广泛实验验证了C-DGPA的优越性，在所有基准上都取得了新的最先进结果。

Conclusion: C-DGPA通过双对齐策略将领域知识有效整合到提示学习中，确保获得领域不变且语义可判别的表示，显著提升了无监督域自适应的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统相机在端到端驾驶中面临域差距问题，特别是在训练数据条件与部署环境不匹配时性能大幅下降。本文针对昼夜光照差异这一具体域差距问题，探索事件相机作为替代方案的潜力。

Method: 提出使用事件相机替代传统相机，研究其在昼夜光照差异域差距下的性能表现。通过比较事件相机与传统灰度帧相机在不同光照条件下的性能变化来评估其域适应性。

Result: 事件相机在不同光照条件下保持更一致的性能，其域偏移惩罚通常与灰度帧相当或更小，在跨域场景中提供更优的基线性能。

Conclusion: 事件相机是应对自动驾驶中昼夜光照差异域差距问题的有效替代方案，能够在不需额外调整的情况下保持跨光照条件的性能一致性。
...
TL;DR: Avatar4D是一个可迁移的合成人体运动数据集生成管道，专注于特定领域应用，提供对身体姿态、外观、相机视角和环境背景的细粒度控制，无需人工标注。通过Syn2Sport体育数据集验证其在运动理解领域的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注日常通用动作，灵活性有限，难以满足特定领域（如体育）对复杂人体动作和运动模式的需求。需要一种能够生成可定制、可扩展且无需人工标注的合成人体运动数据的方法。

Method: 提出Avatar4D管道，能够生成高保真4D（随时间变化的3D几何）人体运动序列，控制身体姿态、外观、相机视角和环境背景。创建Syn2Sport大规模合成数据集，涵盖棒球和冰球等体育项目。

Result: 在Syn2Sport上评估了多种最先进的姿态估计模型，展示了其在监督学习、零样本迁移到真实数据以及跨体育泛化方面的有效性。分析了生成合成数据与真实数据集在特征空间的对齐程度。

Conclusion: Avatar4D系统展示了生成可扩展、可控且可迁移的人体数据集用于多样化特定领域任务的潜力，无需依赖特定领域的真实数据。
...
TL;DR: VALOR提出了一种基于强化学习的医学视觉语言模型后对齐框架，通过GRPO优化方法提升放射学报告生成的视觉对齐和临床准确性，减少幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 当前大型医学视觉语言模型在生成放射学报告时面临视觉对齐不足和临床准确性不高的挑战，现有方法依赖大量标注数据、昂贵的任务特定偏好数据或检索方法，但未能有效缓解视觉与语言表征之间的跨模态对齐不良导致的幻觉问题。

Method: VALOR采用基于强化学习的后对齐框架，使用组相对近端优化（GRPO）方法。训练分为两个阶段：1）通过文本奖励改进Med-VLM，鼓励使用临床精确术语；2）将文本基础模型的视觉投影模块与疾病发现对齐，引导注意力集中在与诊断任务最相关的图像区域。

Result: 在多个基准测试上的广泛实验表明，VALOR显著提高了事实准确性和视觉对齐性，在放射学报告生成任务上相比最先进方法取得了显著的性能提升。
...
TL;DR: 提出基于信息几何的3D点云形状分析框架，将点云表示为统计流形上的高斯混合模型，并引入具有理论上下界的修正对称KL散度，解决了传统几何度量和现有KL近似的不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 三维点云在多个领域有重要应用，但点云比较面临挑战：点云的非结构化特性和复杂几何形状使传统几何度量（如Hausdorff和Chamfer距离）难以捕捉全局统计结构且对异常值敏感，而现有的高斯混合模型KL散度近似可能产生无界或数值不稳定的值。

Method: 1. 证明高斯混合模型空间形成统计流形；2. 提出修正对称KL散度（MSKL），具有理论保证的上下界，确保所有GMM比较的数值稳定性；3. 将点云表示为统计流形上的高斯混合模型。

Result: 在人体姿态识别（MPI-FAUST数据集）和动物形状比较（G-PCD数据集）的综合实验中，MSKL提供了稳定且单调变化的值，直接反映几何变化，优于传统距离和现有KL近似。

Conclusion: 提出的信息几何框架为3D点云形状分析提供了稳定可靠的比较方法，通过将点云表示为统计流形上的高斯混合模型并使用具有理论保证的修正对称KL散度，解决了传统方法的局限性。
...
Result: 低秩近似通常能产生视觉上相似的图像，但压缩效率始终比JPEG、JPEG2000和WEBP等标准格式差，在低误差容忍度下，SVD压缩表示甚至可能超过原始图像大小
...
TL;DR: ARMFlow是一个基于MeanFlow的自回归框架，用于3D人体反应生成，解决了高运动保真度、实时推理和在线场景自回归适应性三大挑战，通过Bootstrap Contextual Encoding减少误差累积，在在线生成中性能超越现有方法40%以上。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体反应生成方法无法同时满足三个关键需求：高运动保真度、实时推理能力以及在线场景的自回归适应性。这限制了实际应用中的性能表现。

Method: 提出ARMFlow框架，包含因果上下文编码器和基于MLP的速度预测器。引入Bootstrap Contextual Encoding (BSCE)训练方法，编码生成的历史而非真实历史以减少自回归生成中的误差累积。还提出了离线变体ReMFlow。

Result: 单步在线生成在InterHuman和InterX数据集上FID指标超越现有在线方法40%以上，同时匹配离线SOTA性能，尽管只使用部分序列条件。推理速度在离线方法中最快。

Conclusion: ARMFlow成功解决了3D人体反应生成的三大挑战，通过BSCE有效减少误差累积，在保持高保真度的同时实现实时推理，为在线应用场景提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多视角人群计数旨在解决大范围场景中的严重遮挡问题，但多视角数据收集和标注困难，导致数据集规模有限。现有解决方案包括收集合成数据或采用半监督/弱监督方法减少对多视角数据的需求。

Method: 提出两种半监督多视角人群计数框架：1）基于模型预测排序：要求较少相机视角的模型预测不应多于较多视角的预测；2）基于模型不确定性排序：要求较多相机视角的模型不确定性不应大于较少视角的不确定性。这些约束以半监督方式引入模型训练。

Result: 实验表明，提出的多视角模型排序方法相比其他半监督计数方法具有优势，在有限标注数据下有效提升了多视角人群计数的性能。

Conclusion: 通过排序多视角融合模型的预测结果或模型不确定性，提出的半监督框架能够有效利用有限标注数据，改善多视角人群计数在数据稀缺情况下的性能。
...
TL;DR: TextEditBench：首个专注于图像中文本区域编辑的评估基准，强调推理密集型编辑场景，引入语义期望新维度，揭示当前模型在上下文推理和物理一致性方面的不足


<details>
  <summary>Details</summary>
Motivation: 文本渲染已成为视觉生成中最具挑战性的前沿领域，但图像中的文本编辑仍未被充分探索。现有方法需要生成清晰字符的同时保持语义、几何和上下文一致性，而当前缺乏专门评估文本编辑能力的基准

Method: 提出TextEditBench评估基准，专注于图像中的文本中心区域，强调需要理解物理合理性、语言意义和跨模态依赖的推理密集型编辑场景。引入语义期望新维度，衡量模型在文本编辑过程中保持语义一致性、上下文连贯性和跨模态对齐的推理能力

Result: 对最先进的编辑系统进行广泛实验发现，虽然当前模型能够遵循简单的文本指令，但在上下文依赖推理、物理一致性和布局感知集成方面仍存在困难。TextEditBench为评估文本编辑能力提供了新的测试平台

Conclusion: TextEditBench填补了文本编辑评估的空白，通过专注于这一长期被忽视但基础的能力，为推进文本引导的图像编辑和多模态生成中的推理建立了新的测试基础
...
TL;DR: 本文提出MACL方法，通过标签感知采样、频率敏感加权和动态温度缩放解决遥感图像多标签检索中的语义重叠、标签分布不平衡和类别共现问题


<details>
  <summary>Details</summary>
Motivation: 遥感图像多标签检索面临三大挑战：1）土地覆盖类别间的语义重叠；2）高度不平衡的标签分布；3）复杂的类别间共现模式。这些因素影响了检索性能的可靠性和准确性。

Method: 提出多标签自适应对比学习（MACL），作为对比学习的扩展。该方法整合了三个关键技术：1）标签感知采样；2）频率敏感加权；3）动态温度缩放。这些技术共同实现了常见和稀有类别的平衡表示学习。

Result: 在三个基准数据集（DLRSD、ML-AID、WHDLD）上的大量实验表明，MACL始终优于基于对比损失的基线方法，有效缓解了语义不平衡问题，并在大规模遥感档案中提供了更可靠的检索性能。

Conclusion: MACL方法通过自适应对比学习框架成功解决了遥感图像多标签检索中的关键挑战，为大规模遥感档案的可靠检索提供了有效解决方案。代码、预训练模型和评估脚本将在接受后公开。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的图像生成基准大多关注美学质量，而缺乏对细粒度生成能力的评估。需要一种客观方法来评估模型在像素级精度上的生成智能。
...
Conclusion: 该研究不仅标志着该领域的激动进展，还为未来多模态、推理、可解释性和基准测试相关研究提供了重要见解，表明语义分割任务能有效评估模型的细粒度生成智能。
...
TL;DR: 本文提出了一种基于岭估计的融合定位方法，结合序列图像丰富场景信息和激光测距高精度优势，在有限观测条件下提高无人机目标定位精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 无人机搭载多种传感器进行目标跟踪和测量是快速准确定位目标的有效手段。在远距离、小交会角、大倾角等有限观测条件下，使用最小二乘估计算法时设计矩阵的列向量存在严重多重共线性，导致病态问题，造成显著不稳定性和低鲁棒性。

Method: 提出基于岭估计的融合定位方法，结合序列图像的丰富场景信息和激光测距的高精度优势。引入岭估计来缓解有限观测条件下的严重多重共线性问题。

Result: 实验结果表明，与基于单一信息的地面定位算法相比，该方法实现了更高的定位精度。岭估计的引入有效增强了鲁棒性，特别是在有限观测条件下。

Conclusion: 基于岭估计的融合定位方法能够有效提高无人机目标定位的精度和鲁棒性，特别是在远距离、小交会角、大倾角等有限观测条件下，解决了传统最小二乘法因多重共线性导致的病态问题。
...
TL;DR: QUIDS系统通过质量感知的激励机制，在非专用车载移动群智感知中同时优化覆盖范围和可靠性，显著提升感知质量。


<details>
  <summary>Details</summary>
Motivation: 非专用车载移动群智感知面临覆盖范围、感知可靠性和车辆动态参与度相互关联的挑战，需要在不依赖专用基础设施的情况下实现高质量的城市监测。

Method: 提出QUIDS系统，引入聚合感知质量指标整合覆盖和可靠性，开发基于信念感知的车辆调度算法，在不确定性下估计可靠性并分配激励。

Result: 在实际部署中，QUIDS比无调度场景提升38%的ASQ，比现有最优方法提升10%，并将重建地图误差降低39-74%。

Conclusion: 通过联合优化覆盖和可靠性的质量感知激励机制，QUIDS实现了低成本、高质量的城市监测，适用于交通和环境感知等智慧城市场景。
...
Method: 设计两阶段框架：第一阶段服务器在全局图像上进行推理，使用VLM内部注意力识别感兴趣区域，计算输出令牌的最小熵作为置信度；如果最小熵超过阈值，请求边缘设备发送RoI的细节保留局部图像；第二阶段服务器联合利用全局和局部图像进行细化推理。

Result: 在多个VLM架构上的实验表明，该框架显著降低了通信成本，同时保持了推理精度。

Conclusion: 提出的选择性重传策略通过仅在需要时传输关键视觉内容，实现了通信效率与推理精度的平衡。
...
Conclusion: GMODiff通过增益图驱动的单步扩散框架有效解决了传统潜在扩散模型在HDR重建中的局限性，实现了高质量、高效率的多曝光HDR重建，同时抑制了内容幻觉问题。
...
TL;DR: EverybodyDance提出了一种解决多角色动画中身份对应问题的系统方案，通过身份匹配图建模角色关系，确保角色在参考帧和生成帧之间的正确对应关系。


<details>
  <summary>Details</summary>
Motivation: 现有姿态驱动角色动画在单角色场景中取得了显著进展，但扩展到多角色场景（特别是涉及位置交换时）存在挑战。核心问题在于确保参考帧和生成帧中角色之间的正确身份对应关系。

Method: 提出身份匹配图（IMG）将生成帧和参考帧中的角色建模为加权完全二分图中的两个节点集；使用掩码查询注意力（MQA）计算边权重；将身份对应正确性形式化为图结构度量并在训练中优化；提出身份嵌入引导、多尺度匹配策略和预分类采样等针对性策略。

Result: 构建了身份对应评估基准用于多角色身份对应正确性评估；大量实验表明EverybodyDance在身份对应和视觉保真度方面显著优于现有最先进基线方法。

Conclusion: EverybodyDance为多角色动画中的身份对应问题提供了系统性解决方案，通过图结构建模和针对性优化策略，有效解决了多角色场景中的身份对应挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前T2V扩散模型在生成复杂场景和遵循逻辑时间指令方面存在不足，许多错误源于无法构建语义正确或逻辑一致的初始帧。
...
TL;DR: 该研究利用3D神经表示和Gaussian Splatting技术，从少量未标定人脸图像中重建出可用于标准图形流水线的中性姿态人脸模型，并实现了神经纹理转换和光照解耦。


<details>
  <summary>Details</summary>
Motivation: 为了解决从少量未标定人脸图像中重建可用于标准图形流水线的3D人脸模型的挑战，同时保持高视觉保真度并实现与现有图形系统的兼容性。

Method: 1. 使用Gaussian Splatting而非NeRF，因其更显式且更易施加约束；2. 利用分割标注对齐人脸语义区域，仅需11张图像即可重建中性姿态；3. 将高斯点软约束到三角网格表面，提供结构化重建；4. 将Gaussian Splats转换为纹理空间的神经纹理；5. 使用可重光照高斯模型解耦纹理和光照，获得高分辨率反照率纹理。

Result: 成功从少量图像重建出可用于标准图形流水线的3D人脸模型，实现了高视觉保真度的Gaussian Splatting与现有图形系统的无缝集成，并展示了在文本驱动资产创建流程中的应用效果。

Conclusion: 该方法通过结合Gaussian Splatting的视觉保真度和传统图形流水线的兼容性，提供了一种从少量图像创建高质量3D人脸资产的有效解决方案，特别适用于资产创建和数字内容生产。
...
TL;DR: BrepLLM是首个让大语言模型能够直接处理3D边界表示(Brep)数据的框架，通过两阶段训练解决几何拓扑信息与自然语言之间的模态鸿沟问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于token序列的大语言模型难以直接处理包含复杂几何和拓扑信息的3D Brep模型，需要弥合结构化3D几何与自然语言之间的模态差距。

Method: 采用两阶段训练流程：1)跨模态对齐预训练：通过自适应UV采样将Brep转换为图表示，设计分层BrepEncoder提取几何拓扑特征，使用对比学习对齐全局token与CLIP文本嵌入；2)多阶段LLM微调：将预训练BrepEncoder集成到LLM中，通过三阶段渐进训练策略对齐节点token序列。

Result: 构建了包含269,444个Brep-文本问答对的Brep2Text数据集，实验表明BrepLLM在3D物体分类和描述生成任务上达到了最先进的性能。

Conclusion: BrepLLM成功实现了LLM对原始Brep数据的解析和推理，为3D几何与自然语言之间的跨模态理解提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决零样本物体计数中的挑战：现有方法要么依赖开放词汇检测器（产生多实例候选），要么依赖随机补丁采样（无法准确描绘物体实例），需要更精确的示例选择机制

Method: 提出CountZES框架，包含三个协同阶段：1) 检测锚定示例(DAE) - 精炼开放词汇检测以隔离精确的单实例示例；2) 密度引导示例(DGE) - 基于密度的自监督范式识别统计一致且语义紧凑的示例；3) 特征共识示例(FCE) - 通过特征空间聚类增强视觉一致性
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在生成高质量图像方面表现出色，但在多个输出中难以保持主体一致性，限制了在视觉叙事中的应用。现有方法依赖模型微调或图像条件，计算成本高且需要针对每个主体进行优化。1Prompt1Story方法虽然无需训练，但存在语义泄漏问题，导致跨帧嵌入纠缠和文本不对齐。

Method: 提出一种简单而有效的无需训练方法，从几何角度解决语义纠缠问题。通过精炼文本嵌入来抑制不想要的语义，避免跨帧嵌入的纠缠，从而改善主体一致性和文本对齐。

Result: 大量实验证明，该方法在主体一致性和文本对齐方面显著优于现有基线方法。

Conclusion: 提出的无需训练方法通过几何视角精炼文本嵌入，有效解决了多图像生成中的语义纠缠问题，显著提升了主体一致性和文本对齐性能。
...
TL;DR: StageVAR：一种针对视觉自回归模型的阶段感知加速框架，通过分析生成过程中不同阶段的重要性差异，在保持早期关键步骤完整的同时，对后期细节优化步骤进行剪枝或近似，实现3.4倍加速且质量损失极小。
...
Method: StageVAR提出阶段感知加速框架：1）分析发现早期步骤对语义和结构一致性至关重要，应保持完整；2）后期步骤主要优化细节，可利用语义无关性和低秩特性进行剪枝或近似；3）无需额外训练的即插即用加速策略。

Result: StageVAR实现最高3.4倍加速，在GenEval上仅下降0.01分，在DPG上仅下降0.26分，一致优于现有加速基线方法。

Conclusion: 阶段感知设计是高效视觉自回归图像生成的强大原则，StageVAR通过智能区分不同生成阶段的重要性，在保持生成质量的同时显著提升效率。
...
TL;DR: 该论文提出了一种通过强化学习让模型获得类人且自洽推理能力的盲图像质量评估方法，通过人类标注作为奖励信号，使模型能够从自生成的描述中推断图像质量。


<details>
  <summary>Details</summary>
Motivation: 人类通过感知-推理级联来评估图像质量，但现有盲图像质量评估模型缺乏类人的自洽推理能力。作者希望开发一个既能预测质量分数又能提供人类可解释推理过程的模型。

Method: 1. 收集捕捉人类感知-推理过程多个方面的人类评估数据；2. 采用强化学习，使用人类标注作为奖励信号来引导模型获得类人感知和推理能力；3. 设计特殊奖励机制，使模型能够仅从自生成的描述中推断图像质量，从而内化自洽推理能力。

Result: 1. 在通用指标（Pearson和Spearman相关系数）上，得分预测性能与最先进的盲图像质量评估系统相当；2. 在1000多个人类标注样本上，模型达到ROUGE-1分数0.512（基线为0.443），表明对人类解释的覆盖度显著提高。

Conclusion: 该方法在盲图像质量评估中实现了与最先进系统相当的预测性能，同时在人类-模型对齐方面取得显著进展，朝着类人可解释推理迈出了重要一步。
...
TL;DR: 该研究提出眼动行为辅助的多模态情感识别(EMER)数据集和EMERT模型，通过眼动行为弥补面部表情识别(FER)与真实情感识别(ER)之间的差距，提升情感识别的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前情感识别领域过度依赖面部表情识别，但面部表情常被用作社交工具而非真实内心情感的表现。为理解和弥合FER与ER之间的差距，研究者引入眼动行为作为重要情感线索。

Method: 1) 构建EMER数据集：采用自发情感诱导范式收集真实情感数据，同时捕获非侵入式眼动行为数据（眼动序列、注视点图）和面部表情视频；2) 设计EMERT模型：利用模态对抗特征解耦和多任务Transformer，将眼动行为作为面部表情的强有力补充。

Result: 实验引入7个多模态基准协议对EMER数据集进行全面评估。结果表明，EMERT模型显著优于其他最先进的多模态方法，证明了眼动行为建模对鲁棒情感识别的重要性。

Conclusion: 该研究全面分析了眼动行为在情感识别中的重要性，推进了弥合FER与ER差距的研究，为获得更鲁棒的情感识别性能提供了新思路。EMER数据集和EMERT模型将公开提供。
...
TL;DR: 提出了VenusBench-GD，一个跨平台的双语GUI grounding基准，包含大规模数据、高质量标注流程和分层任务分类，评估发现通用多模态模型在基础任务上已匹敌专用GUI模型，但高级任务仍需要专用模型。


<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding基准存在数据量不足、领域覆盖窄、平台单一或需要专业领域知识等局限性，需要构建更全面的评估框架来支持真实世界应用。

Method: 1) 构建大规模跨平台基准，覆盖广泛应用、多样UI元素和丰富标注数据；2) 建立高质量数据构建流程，标注准确率高于现有基准；3) 提出分层任务分类法，将grounding分为基础和高级两类，包含六个互补的子任务。
...
TL;DR: 提出弱监督深度学习框架，使用Grad-CAM进行肺炎分类和定位，无需像素级标注，仅需图像级标签，在7个预训练模型上实现96-98%准确率


<details>
  <summary>Details</summary>
Motivation: 胸部X光常用于肺炎诊断，但精确定位肺炎区域需要昂贵的像素级标注。为解决这一限制，本研究旨在开发弱监督方法，降低标注成本

Method: 使用Grad-CAM生成热力图，仅依赖图像级标签而非像素级标注；评估7个预训练模型（包括Vision Transformer），采用焦点损失和患者级数据分割防止数据泄露

Result: 所有模型达到96-98%高分类准确率，ResNet-18和EfficientNet-B0表现最佳，MobileNet-V3作为轻量级替代；Grad-CAM热力图显示模型聚焦临床相关肺部区域
...
TL;DR: N3D-VLM是一个统一框架，将原生3D物体感知与3D感知视觉推理相结合，实现精确的3D定位和可解释的空间理解，超越传统2D图像问答模型。
...
Result: 实验结果表明，该统一框架不仅在3D定位任务上达到最先进性能，而且在3D空间推理方面也持续超越现有视觉语言模型方法。数据构建流程生成的3D物体定位数据比现有最大单图像3D检测数据集大6倍以上。

Conclusion: N3D-VLM成功地将原生3D物体感知与3D感知视觉推理相结合，实现了更精确的3D定位和更可解释的结构化空间理解，为3D场景理解提供了新的解决方案。
...
TL;DR: 提出一个动态重建系统，可从单目RGB视频输入中重建完整且持久的场景，包括可见和不可见部分，实现4D时空重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只能重建当前可见的场景部分，无法处理动态场景中物体移动和遮挡问题，缺乏对完整场景的持久性重建能力。

Method: 将场景分解为刚性3D基元，通过密集2D对应关系估计联合推断基元的刚性运动，使用运动分组技术外推不可见物体的运动，实现4D时空重建。

Result: 在物体扫描和多物体数据集上，系统在定量和定性评估中显著优于现有方法，实现了可重放的3D重建、多物体扫描和物体持久性。

Conclusion: 该系统实现了完整的4D时空感知能力，能够从单目视频中重建动态场景的完整历史，为动态场景理解提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在微调视觉基础模型时忽略了长期预训练模型中的伪影问题，这些伪影与非因果因素相关，阻碍了有价值表征的利用，从而降低了域泛化语义分割的性能

Method: 提出Causal-Tune方法：1) 使用离散余弦变换提取每层特征的频谱；2) 应用高斯带通滤波器分离因果和非因果成分；3) 引入因果感知可学习令牌在频域中精炼因果成分；4) 通过逆DCT将精炼特征转换回空间域并传递给下一层

Result: 在各种跨域任务上的实验证明了Causal-Tune的有效性，特别是在恶劣天气条件下表现优异，在雪天条件下比基线提高了+4.8% mIoU

Conclusion: 通过因果机制分析视觉基础模型中的特征因素，提出的Causal-Tune方法能够有效识别和分离因果与非因果因素，显著提升了域泛化语义分割的性能，尤其是在具有挑战性的跨域场景中
...
<details>
  <summary>Details</summary>
Motivation: 现有3D医学扫描时间演化预测模型存在局限性：要么依赖单个先验扫描，要么使用固定网格时间，要么针对全局标签，这限制了不规则采样下的体素级预测能力。
...
TL;DR: SkiLa提出了一种新的多模态推理范式，通过在推理过程中原生生成连续视觉嵌入（潜在草图标记）来实现统一的视觉-文本思维，无需外部工具包。


<details>
  <summary>Details</summary>
Motivation: 当前的多模态大语言模型在需要视觉想象力的场景中表现不足，而人类能够在统一的大脑空间中进行灵活的视觉-文本思维交互。受此启发，研究者认为视觉标记可以无缝插入到文本标记的推理过程中。

Method: 提出Sketch-in-Latents (SkiLa)范式，扩展MLLMs的自回归能力以原生生成连续视觉嵌入（潜在草图标记）。模型在推理过程中动态切换文本思维模式和视觉草图模式，并引入潜在视觉语义重建机制确保语义基础。

Result: 大量实验表明，SkiLa在视觉中心任务上取得优越性能，同时在多样化通用多模态基准测试中展现出强大的泛化能力。

Conclusion: SkiLa通过统一的多模态推理范式，实现了更自然的视觉-文本思维交互，为多模态大语言模型的视觉想象能力提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型使用CNN作为基础块，但卷积操作的局部性限制了模型理解长距离语义信息的能力，因此需要改进非局部建模能力。

Method: 提出Yuan-TecSwin模型：1) 在编码器和解码器中用Swin-transformer块替换CNN块；2) 通过精心选择的文本编码器、有效利用文本嵌入和精心设计的文本条件融合来改善文本-图像对齐；3) 采用自适应时间步搜索策略在不同扩散阶段优化推理性能。

Result: 1) 在ImageNet生成基准上取得了1.37的SOTA FID分数；2) 通过自适应时间步搜索将推理性能提升10%；3) 在人类评估中，模型生成的图像与人类绘制的图像难以区分。

Conclusion: Yuan-TecSwin通过引入Swin-transformer块有效提升了扩散模型的非局部建模能力，结合优化的文本条件融合策略，在图像生成质量和文本-图像对齐方面都取得了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个计算简单、实用可部署的去雾系统，能够在消费级硬件上实时处理视频和直播摄像头画面，而不需要GPU加速。

Method: 基于暗通道先验和大气散射模型，采用伽马自适应重建、快速传输近似（带数值稳定性下界）、基于分数顶部像素平均的稳定大气光估计器，以及可选的颜色平衡阶段。

Result: 在真实世界图像和视频上的实验演示显示，Hazedefy能够在不使用GPU加速的情况下显著改善可见度和对比度。

Conclusion: Hazedefy是一个适合移动和嵌入式应用的轻量级去雾管道，能够在消费级硬件上实现实时视频增强，具有实际部署价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有DiT的自注意力二次方计算成本限制了向长序列扩展，现有Top-K稀疏注意力方法仍存在二次方选择成本和随序列增长需要增加K值的问题，单层粗粒度表示无法有效捕捉全局结构。

Method: 提出对数线性稀疏注意力(LLSA)：1) 层次化Top-K选择，利用前一层索引进行渐进式稀疏选择；2) 层次化KV增强机制，使用不同粒度的更少token保留全局上下文；3) 高性能GPU实现，仅使用稀疏索引进行前向和反向传播，无需密集注意力掩码。

Result: 在不使用分块化和VAE编码的高分辨率像素空间图像生成中，LLSA在256x256像素token序列上实现注意力推理加速28.27倍，DiT训练加速6.09倍，同时保持生成质量。

Conclusion: LLSA为高效训练长序列DiT提供了有前景的方向，通过将选择和注意力成本从二次方降低到对数线性复杂度，解决了现有稀疏注意力方法的效率瓶颈。
...
TL;DR: 该论文提出了一种基于电源插座的三阶段深度学习管道，用于室内多媒体地理定位，通过检测、分类插座类型并映射到国家，为打击人口贩卖等犯罪提供数字取证工具。


<details>
  <summary>Details</summary>
Motivation: 室内多媒体地理定位在打击人口贩卖、儿童剥削等严重犯罪中具有重要价值，但由于室内环境布局相似、装修频繁、视觉模糊、光照变化、GPS信号不可靠以及敏感领域数据集有限等挑战，该领域发展不足。

Method: 提出三阶段深度学习管道：1) 使用YOLOv11检测电源插座(mAP@0.5=0.843)；2) 使用Xception将插座分类为12种类型(准确率0.912)；3) 将检测到的插座类型映射到国家(置信度>90%时准确率0.96)。为解决数据稀缺问题，创建了两个专用数据集：包含2,328张标注图像（经增强扩展至4,072张）的插座检测数据集，以及包含12个插座类别3,187张图像的分类数据集。

Result: 在Hotels-50K数据集（特别是TraffickCam子集的众包酒店图像）上评估管道性能，该数据集包含真实世界条件如光照不佳和业余拍摄角度。管道在插座检测、分类和国家映射方面均取得高准确率，证明了其在现实数字取证应用中的实用性。

Conclusion: 该框架展示了向现实世界数字取证应用迈出的实际一步，通过使用电源插座作为一致的室内标记物，为解决室内地理定位挑战提供了有效方案。代码、训练模型和数据均已开源。
...
TL;DR: DeContext是一种保护输入图像免受未经授权上下文编辑的新方法，通过注入针对性扰动来削弱多模态注意力层的跨注意力路径，从而阻断源图像到输出的上下文信息传播。


<details>
  <summary>Details</summary>
Motivation: 上下文扩散模型虽然能够轻松、逼真地修改图像，但也带来了严重的隐私问题：个人图像可能被轻易操纵用于身份冒充、虚假信息等恶意用途。现有研究主要关注个性化文本到图像生成的输入扰动保护，但对现代大规模基于DiT的上下文模型的鲁棒性研究不足。

Method: DeContext的核心洞察是：源图像的上下文信息主要通过多模态注意力层传播到输出。通过注入小的针对性扰动来削弱这些跨注意力路径，从而打破输入与输出之间的链接。研究发现早期去噪步骤和特定Transformer块主导上下文传播，因此可以将扰动集中在最关键的位置。

Result: 在Flux Kontext和Step1X-Edit上的实验表明，DeContext能够持续阻止不需要的图像编辑，同时保持视觉质量。这些结果证明了基于注意力的扰动作为对抗图像操纵的有效防御手段。

Conclusion: DeContext提供了一种简单、高效且鲁棒的方法来保护输入图像免受未经授权的上下文编辑，通过针对性扰动注意力机制来阻断上下文信息传播，为解决扩散模型的隐私安全问题提供了有效解决方案。
...
TL;DR: SARMAE：首个面向SAR图像的噪声感知掩码自编码器，通过构建百万级SAR数据集SAR-1M，结合斑点噪声注入和光学先验对齐，实现自监督表征学习，在分类、检测、分割任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 合成孔径雷达（SAR）图像在遥感应用中至关重要，但现有深度学习面临数据稀缺和固有斑点噪声的挑战，这些噪声阻碍了细粒度语义表征学习。

Method: 提出SARMAE框架：1）构建首个百万级SAR数据集SAR-1M，包含配对的SAR和光学图像；2）设计斑点感知表征增强（SARE），将SAR特有的斑点噪声注入掩码自编码器；3）引入语义锚点表征约束（SARC），利用光学先验对齐SAR特征并确保语义一致性。

Result: 在多个SAR数据集上的广泛实验表明，SARMAE在分类、检测和分割任务上均达到了最先进的性能。

Conclusion: SARMAE通过大规模数据集构建、噪声感知学习和光学先验对齐，有效解决了SAR图像数据稀缺和噪声干扰问题，为SAR图像的自监督表征学习提供了有效解决方案。
...
TL;DR: REGLUE是一个统一的潜在扩散框架，通过联合建模VAE潜在表示、局部VFM语义和全局CLS标记，利用非线性压缩的多层VFM特征，显著提升图像生成质量和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型的重构式去噪目标仅提供间接语义监督，导致高级语义出现缓慢，需要更长的训练时间并限制样本质量。现有方法未能充分利用视觉基础模型丰富的非线性多层空间语义。

Method: 提出REGLUE框架：1）使用轻量级卷积语义压缩器非线性聚合多层VFM特征为低维空间结构化表示；2）在单个SiT主干中联合建模VAE潜在表示、局部（补丁级）VFM语义和全局（图像级）CLS标记；3）通过外部对齐损失正则化内部表示。

Result: 在ImageNet 256x256上，REGLUE持续改进FID并加速收敛，优于SiT-B/2、SiT-XL/2基线以及REPA、ReDi和REG方法。实验证明空间VFM语义至关重要，非线性压缩是发挥其全部优势的关键，全局标记和外部对齐是互补的轻量级增强。

Conclusion: REGLUE通过全局-局部-潜在联合建模框架，有效利用视觉基础模型的丰富语义信息，显著提升了潜在扩散模型的图像生成质量和训练效率，为语义增强的扩散模型提供了新思路。
...
Result: 能够生成时间一致、逼真的帧，在数百到数千帧中保持稳定，相比通用方法在光照、阴影和反射方面获得更高质量的逼真效果。
...
TL;DR: 该研究探索了使用主题指纹识别技术解决开源数据集合并时的数据泄漏问题，通过将同一受试者的所有图像映射到潜在空间的特定区域，实现基于相似性匹配的受试者重识别。
...
Method: 使用ResNet-50模型配合三元组边界损失进行训练，探索主题指纹识别技术。将同一受试者的所有图像映射到潜在空间的特定区域，通过相似性匹配实现受试者重识别。在3D MRI和2D X-ray数据上评估少样本指纹识别，包括标准（20-way 1-shot）和挑战性（1000-way 1-shot）场景。
...
Conclusion: 主题指纹识别技术能有效解决数据集合并时的数据泄漏问题，通过将同一受试者的图像映射到潜在空间的特定区域，实现了高精度的受试者重识别，为医学图像分析中的数据集整合提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有NeRF和3DGS等方法在视图合成方面取得进展，但在精确网格重建方面仍有不足，特别是缺乏度量一致的表面对齐

Method: 联合学习显式Voronoi图和隐式有符号距离场(SDF)，通过光线追踪优化，使用Eikonal正则化，使Voronoi单元面与SDF零水平集对齐

Result: 在多种场景下，SDFoam显著改善了网格重建精度（Chamfer距离），同时保持可比的外观质量（PSNR、SSIM），且不牺牲训练效率

Conclusion: 提出的隐式-显式混合方法SDFoam能够生成更清晰、视图一致的表面，减少漂浮物，改善拓扑结构，在保持渲染效率的同时提升网格重建质量
...
<details>
  <summary>Details</summary>
Motivation: 胎儿超声生长评估依赖手动识别标准平面中的解剖标志点进行生物测量，这种方法耗时、操作者依赖性强，且对扫描设备和站点的变异性敏感，限制了自动化方法的可重复性。需要多源标注数据集来开发AI辅助的胎儿生长评估方法。

Method: 创建了一个开放的多中心、多设备基准数据集，包含4,513张去识别化的超声图像，来自1,904名受试者，采集自三个临床站点和七种不同的超声设备。数据集提供了标准化的受试者分离的训练/测试分割、评估代码和基线结果。

Result: 使用自动生物测量模型量化了领域偏移，证明仅使用单一中心数据进行训练和评估会显著高估性能，而多中心测试才能提供更可靠的评估。这是首个覆盖所有主要胎儿生物测量指标的公开多中心、多设备、标志点标注数据集。
...
TL;DR: 该论文提出了OMG-Bench基准数据集和HMATr模型，用于解决在线微手势识别的挑战，包括数据稀缺和算法局限性问题。


<details>
  <summary>Details</summary>
Motivation: 在线微手势识别对VR/AR交互至关重要，但面临两大挑战：1）缺乏大规模公开数据集，微手势的细微运动模式使得构建具有精确骨骼和帧级标注的数据集困难；2）现有任务特定算法难以有效处理微手势的识别问题。

Method: 1）开发多视图自监督流水线自动生成骨骼数据，结合启发式规则和专家精炼实现半自动标注，构建OMG-Bench基准数据集；2）提出分层记忆增强Transformer（HMATr）端到端框架，通过分层记忆库存储帧级细节和窗口级语义来保留历史上下文，并使用从记忆初始化的可学习位置感知查询来隐式编码手势位置和语义。

Result: 构建了OMG-Bench基准数据集，包含40个细粒度手势类别、13,948个实例、1,272个序列，具有细微运动、快速动态和连续执行的特点。HMATr模型在检测率上比现有最优方法高出7.6%，为在线微手势识别建立了强基线。

Conclusion: 该研究通过构建首个大规模公开基准数据集OMG-Bench和提出HMATr模型，有效解决了在线微手势识别的数据稀缺和算法挑战，为VR/AR交互提供了重要技术基础。
...
Method: 提出任务导向的数据合成框架TODSynth：1）多模态扩散变换器（MM-DiT）采用统一的三重注意力机制；2）基于任务反馈的即插即用采样策略；3）控制-校正流匹配方法（CRFM），在早期高可塑性阶段通过语义损失动态调整采样方向。

Result: 实验表明，文本-图像-掩码联合注意力方案结合图像和掩码分支的完全微调显著提升了遥感语义分割数据合成的效果，特别是在少样本和复杂场景下。该方法在稳定性和任务导向性方面优于现有可控生成方法。

Conclusion: TODSynth框架通过多模态注意力机制和任务反馈引导的采样策略，有效解决了遥感语义分割数据合成中的控制复杂性和质量不稳定问题，为下游任务提供了更稳定、任务导向的合成数据。
...
TL;DR: TreeNet是一种基于二叉树结构的低复杂度图像压缩模型，通过注意力特征融合机制整合多分支特征，在低比特率下相比JPEG AI提升4.83% BD-rate，同时降低87.82%模型复杂度。
...
Method: 提出TreeNet模型，采用二叉树结构的编码器-解码器架构实现高效表示和重建，使用注意力特征融合机制整合多分支特征。

Result: 在三个基准数据集上评估，低比特率下相比JPEG AI平均提升4.83% BD-rate，模型复杂度降低87.82%。通过消融研究分析了不同潜在表示对重建的影响。

Conclusion: TreeNet在显著降低计算复杂度的同时实现了优于JPEG AI的压缩性能，为高效图像压缩提供了有前景的解决方案。
...
Method: 引入潜在姿势变换器，基于骨骼运动操作形状标记；使用密集姿势表示实现精确控制；采用潜在空间监督策略和自适应补全模块确保高保真几何和拓扑变化适应。

Result: 在摆姿质量方面表现出优越性能，并能自然扩展到3D编辑应用，如部件替换和细化。

Conclusion: 通过将角色摆姿重新定义为潜在空间变换问题，Make-It-Poseable框架克服了传统方法的局限性，实现了高质量的姿势重建和编辑能力。
...
TL;DR: Kling-Omni是一个端到端的通用视频生成框架，能够从多模态视觉语言输入直接合成高质量视频，支持文本指令、参考图像和视频上下文等多种输入，实现视频生成、编辑和智能推理的统一处理。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成、编辑和智能推理任务通常采用分离的流水线方法，缺乏统一的框架来整合这些功能。作者旨在开发一个端到端的通用生成框架，能够处理多样化的用户输入，生成电影级质量的智能视频内容，并推动向多模态世界模拟器的发展。

Method: 采用端到端架构，将不同输入（文本指令、参考图像、视频上下文）处理为统一的多模态表示。构建了全面的数据系统作为多模态视频创作的基础，采用高效的大规模预训练策略和推理基础设施优化。

Result: 综合评估显示，Kling-Omni在上下文生成、基于推理的编辑和多模态指令跟随方面表现出卓越能力。框架超越了传统内容创作工具，成为能够感知、推理、生成和交互动态复杂世界的多模态世界模拟器的关键进展。

Conclusion: Kling-Omni是一个重要的端到端通用视频生成框架，成功整合了视频生成、编辑和智能推理功能，支持多样化的多模态输入，代表了向能够与动态复杂世界交互的多模态世界模拟器发展的关键一步。
...
<details>
  <summary>Details</summary>
Motivation: 现有真实数据集缺乏精确标注，而合成数据集虽然标注成本低但缺乏真实的车辆运动轨迹。需要结合两者的优点：合成数据的低成本标注和真实数据的自然运动模式。

Method: 创建合成3D环境，并从SinD数据集（无人机拍摄的鸟瞰数据集）中提取真实世界轨迹，将这些真实轨迹集成到合成环境中。

Result: 生成了R3ST数据集，既提供准确的多模态地面真实标注，又包含真实的人类驾驶车辆轨迹，填补了合成数据与真实轨迹之间的差距。

Conclusion: R3ST数据集通过结合合成环境的标注优势和真实轨迹的自然性，推动了道路车辆轨迹预测研究的发展。
...
<details>
  <summary>Details</summary>
Motivation: AR/VR应用中基于头戴设备稀疏信号重建真实多样的全身体姿具有挑战性，现有方法要么计算成本高，要么难以平衡准确性、时序一致性和效率。

Method: 提出KineST模型，采用运动学引导的双向扫描策略嵌入运动学先验，使用混合时空表示学习耦合时空上下文，并引入几何角速度损失来改善运动稳定性。

Result: 大量实验表明KineST在轻量级框架下在准确性和时序一致性方面都表现出优越性能。

Conclusion: KineST通过运动学引导的状态空间模型有效解决了AR/VR场景中基于稀疏信号的全身体姿重建问题，在准确性、时序一致性和效率之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估指标存在两个主要问题：要么将表格扁平化为文本忽略结构信息，要么依赖固定参考限制了泛化能力。需要一种既能评估表格结构又能评估事实准确性的可靠评估方法。
...
Conclusion: TabReX为结构化生成系统建立了可信、可解释评估的新范式，通过图推理方法有效解决了表格生成评估的挑战。
...
TL;DR: SocialStoryFrames是一个用于建模读者对故事反应的计算框架，包括感知作者意图、推理、情感反应和价值判断等，通过两个模型在大规模社交媒体故事分析中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前计算模型在捕捉读者对故事的丰富解释性、情感性和评价性反应方面存在局限，无法进行细致的分析。需要一种能够捕捉读者对故事多方面反应的计算框架。

Method: 提出了SocialStoryFrames形式化框架，基于叙事理论、语言语用学和心理学构建分类法。开发了两个模型：SSF-Generator（通过382名参与者的人类调查验证）和SSF-Classifier（通过专家标注验证）。应用于SSF-Corpus数据集（包含6,140个来自不同背景的社交媒体故事）。

Result: 模型验证显示SocialStoryFrames能够有效捕捉读者反应。大规模分析揭示了不同社区中讲故事意图的频率和相互依赖性，以及叙事实践的多样性和差异。

Conclusion: SocialStoryFrames通过将细粒度、上下文敏感的建模与通用的读者反应分类法相结合，为在线社区中的叙事研究提供了新的工具，能够在大规模上分析讲故事的模式和多样性。
...
TL;DR: 研究探索了个人信息（如个人陈述）在预测主观任务标注者标签中的作用，发现人口统计信息比态度、关系和经历更有影响力，少量相关评论即可获得良好效果，多样化的标注者样本能带来最佳性能。


<details>
  <summary>Details</summary>
Motivation: 先前研究虽然探索了使用个人信息来改进个体特征建模和主观任务标注预测，但信息量通常受限，缺乏对何种信息最有效的深入理解。本研究旨在填补这一空白，系统分析不同类型个人信息对预测标注者判断的影响。

Method: 对自我披露句子进行分类，并基于这些分类构建标注者模型来预测社会规范判断。进行了多项消融实验和分析，考察不同类型信息对预测标注模式的影响。

Result: 研究发现：1）人口统计信息比态度、关系和经历更具影响力；2）基于理论的方法通常优于自动聚类；3）与先前研究相反，仅需少量相关评论即可获得良好效果；4）拥有更多样化的标注者自我披露样本能带来最佳性能。

Conclusion: 个人信息在预测标注者标签方面具有重要价值，特别是人口统计信息最为关键。研究结果为有效利用个人信息构建标注者模型提供了实用指导，强调信息类型多样性和理论驱动方法的重要性。
...
Result: 实验表明Sage指标稳定且与监督基准（如LLMBar和RewardBench2）高度相关。发现当前最先进的LLM（包括Gemini-2.5-Pro和GPT-5）在约1/4困难案例中无法保持一致的偏好。还发现人类判断也存在显著不一致性。

Conclusion: Sage是一个可靠的LLM-as-a-Judge评估套件，揭示了当前LLM法官的可靠性问题。微调LLM法官、基于面板的法官以及深度推理可以提升判断一致性。人类标注可能不是可靠的金标准。
...
### [68] [From Facts to Conclusions : Integrating Deductive Reasoning in Retrieval-Augmented LLMs](https://arxiv.org/abs/2512.16795)
*Shubham Mishra,Samyek Jain,Gorang Mehrishi,Shiv Tiwari,Harsh Sharma,Pratik Narang,Dhruv Kumar*

Main category: cs.CL

TL;DR: 提出推理轨迹增强的RAG框架，通过三阶段结构化推理处理检索文档中的冲突、过时和主观信息，引入冲突感知信任评分管道进行评估，显著提升答案正确性和行为一致性。


<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成(RAG)在检索到的源文档存在冲突、过时或包含主观信息时会失败，现有工作独立解决这些问题但缺乏统一的推理监督。

Method: 提出推理轨迹增强的RAG框架，包含三阶段结构化推理：(1)文档级裁决，(2)冲突分析，(3)基于证据的合成，产生引用链接的答案或合理的拒绝。引入冲突感知信任评分(CATS)管道，使用LLM作为评判者评估基础性、事实正确性、拒绝准确性和冲突行为对齐。

Result: 实验结果显示相对于基线有显著提升，特别是在Qwen模型上，监督微调将端到端答案正确率从0.069提升到0.883，行为一致性从0.074提升到0.722。构建了包含539个查询的推理数据集和评估管道。

Conclusion: 该框架为冲突感知、可解释的RAG系统奠定了基础，通过结构化推理和监督微调显著提升了系统在复杂检索环境下的性能和可靠性。
...
TL;DR: 本文提出了一种基于语义驱动的强化学习方法（SRL）用于医学报告生成，通过报告级别的奖励机制直接优化临床正确性，而非传统的词级监督。
...
Method: 提出语义驱动的强化学习方法（SRL），采用组相对策略优化（GRPO）来鼓励临床正确性导向的学习。优化报告级别的奖励：基于关键放射学发现提取的边际余弦相似度（MCCS），直接对齐临床标签一致性。还引入轻量级推理格式约束来生成结构化的"思考报告"输出。

Result: 在IU X-Ray和MIMIC-CXR数据集上评估，MRG-R1实现了最先进的性能：IU X-Ray上CE-F1为51.88，MIMIC-CXR上为40.39。实验表明标签语义强化优于传统的词级监督。

Conclusion: 优化基于临床基础的报告级别奖励而非词重叠，能显著提高临床正确性。这是探索语义强化在监督医学大型视觉语言模型训练中医学正确性的先驱工作。
...
TL;DR: 本文介绍了用于Faux-Hate共享任务的系统，该任务旨在检测由虚假叙事驱动的仇恨言论，系统在印地语-英语混合社交媒体文本中实现了有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 社交媒体平台已成为有害内容（包括仇恨言论和虚假叙事）快速传播的中心，Faux-Hate共享任务专注于检测由虚假叙事驱动的仇恨言论这一特定现象。
...
Conclusion: 通过结合NLP技术和领域特定预训练的多任务学习方法，能够有效检测由虚假叙事驱动的仇恨言论，为社交媒体有害内容检测提供了有效解决方案。
...
TL;DR: 基于Qwen2.5-7B模型，通过LoRA微调和提示工程，从警方通报中提取15个关键字段，准确率超过98.36%
...
Method: 开发领域适应的提取流程，结合针对性提示工程和Qwen2.5-7B模型的LoRA参数高效微调，处理噪声和异质文本

Result: LoRA微调显著优于基础和指令微调模型，死亡率检测准确率超过98.36%，死亡人数精确匹配率95.31%，省级位置提取精确匹配率95.54%

Conclusion: 该流程为专业领域的多任务结构化信息提取提供了验证有效的解决方案，为社会科学研究中将非结构化文本转化为可靠结构化数据提供了实用框架
...
TL;DR: 提出一个用于医疗保健领域的独立事实核查模块和领域特定摘要模型，以减少LLM幻觉，在MIMIC-III数据集上验证效果良好


<details>
  <summary>Details</summary>
Motivation: 医疗保健领域LLM生成的输出需要可靠准确，特别是在决策和患者安全方面，但LLM存在幻觉风险导致输出不可靠

Method: 提出独立于LLM的事实核查模块和领域特定摘要模型，使用LoRa在MIMIC-III数据集上微调，事实核查模块通过数值测试和自然语言处理的离散逻辑进行细粒度验证

Result: 事实核查模块在3,786个命题上达到精确率0.8904、召回率0.8234、F1分数0.8556；摘要模型ROUGE-1得分0.5797，BERTScore 0.9120

Conclusion: 提出的方法能有效减少医疗保健领域LLM幻觉，提高输出可靠性，事实核查模块和摘要模型在医疗数据上表现良好
...
TL;DR: IBKE框架基于信息瓶颈理论，通过压缩和隔离关键信息实现可泛化的知识编辑，在多个LLM架构和基准任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: LLMs在科学和社会应用中存在错误或过时信息，完全重新训练成本高昂且具有破坏性，现有编辑技术泛化能力有限，需要更有效的知识更新方法。

Method: 基于信息瓶颈理论，通过压缩和隔离关键信息进行知识修正，使用紧凑的潜在表示指导基于梯度的更新，实现IBKE框架。

Result: 在多个LLM架构和标准基准任务上验证了有效性，展示了最先进的准确性，并提高了编辑的泛化性和特异性。

Conclusion: 为开放域知识编辑建立了理论原则和实践范式，提高了LLMs在现实应用中的实用性和可信度。
...
TL;DR: Sigma-MoE-Tiny是一个极稀疏的混合专家语言模型，采用细粒度专家分割（每层96个专家），每个token仅激活1个专家，总参数量200亿但激活参数量仅5亿，在保持训练稳定的同时实现了顶级性能。


<details>
  <summary>Details</summary>
Motivation: 混合专家模型因其高效可扩展性成为基础模型的有前景范式，但现有开源模型的稀疏度仍有提升空间。本研究旨在探索更高稀疏度的MoE架构，同时解决极稀疏条件下专家负载均衡的挑战。

Method: 采用细粒度专家分割（每层96个专家），每个token仅激活1个专家；针对极稀疏条件下负载均衡损失在底层失效的问题，提出渐进稀疏化调度策略以平衡专家利用率和训练稳定性；在多样化高质量语料上进行预训练，然后进行后训练。

Result: 训练过程非常稳定，未出现不可恢复的损失尖峰；尽管仅激活5亿参数，Sigma-MoE-Tiny在可比或更大规模模型中实现了顶级性能；对高稀疏MoE模型中的负载平衡问题进行了深入分析，为未来MoE架构的稀疏化提供了见解。

Conclusion: Sigma-MoE-Tiny展示了在极稀疏条件下实现高性能MoE模型的可行性，提出的渐进稀疏化调度策略有效解决了负载均衡问题，为未来更高稀疏度的MoE架构发展提供了重要参考。
...
TL;DR: 该研究比较了OpenAI GPT模型中推理与非推理架构在芬兰语与四种低资源乌拉尔语翻译任务上的表现，发现推理模型拒绝率低16个百分点


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型翻译评估主要关注高资源语言，对低资源和濒危语言性能了解不足，需要填补这一研究空白

Method: 使用文学文本平行语料库，分析GPT模型中推理与非推理架构在芬兰语与四种低资源乌拉尔语（科米-兹良语、莫克沙语、埃尔齐亚语、乌德穆尔特语）翻译任务上的拒绝率差异

Result: 推理模型与非推理模型在翻译性能上存在显著差异，推理模型的拒绝率比非推理模型低16个百分点

Conclusion: 研究为乌拉尔语研究者提供了有价值的见解，并有助于更广泛地理解推理模型在濒危语言保护方面的能力
...
TL;DR: SpeechLLMs（语音大语言模型）在语音翻译中是否优于传统级联系统？通过全面测试发现，级联系统仍是最可靠的，SpeechLLMs仅在特定场景下匹配级联性能，而语音基础模型表现最差。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型扩展到文本之外，将语音作为原生模态整合产生了SpeechLLMs，旨在直接翻译口语，绕过传统的基于转录的流水线。然而，这种整合是否比已建立的级联架构提高语音到文本翻译质量，仍然是一个开放性问题。

Method: 提出了"Hearing to Translate"测试套件，首次全面评估5个最先进的SpeechLLMs与16个强大的直接和级联系统。这些系统结合了领先的语音基础模型和多语言大语言模型。分析涵盖16个基准测试、13种语言对和9种具有挑战性的条件，包括不流利、嘈杂和长语音。

Result: 在广泛评估中发现：1）级联系统总体上仍然最可靠；2）当前的SpeechLLMs仅在选定设置下匹配级联系统；3）语音基础模型落后于两者。这表明整合大语言模型（无论是模型内部还是流水线中）对于高质量语音翻译至关重要。

Conclusion: 虽然SpeechLLMs作为新兴技术有潜力，但当前级联架构在语音翻译中仍保持优势。整合大语言模型是获得高质量语音翻译的关键因素，但实现方式（端到端SpeechLLMs vs 级联系统）需要根据具体应用场景选择。
...
TL;DR: 提出隐私保护的边缘设备自适应框架，解决临床ASR在数据隐私、计算资源和声学域偏移三大挑战，实现17.1%的WER相对改进和47%的灾难性遗忘减少
...
Method: 提出高效隐私保护自适应框架：1) 使用低秩自适应(LoRA)在边缘设备上实现持续学习；2) 集成多领域经验回放减少灾难性遗忘；3) 直接在设备上处理数据流确保患者隐私

Result: 1) 在目标域上实现17.1%的相对WER改进；2) 相比朴素自适应，灾难性遗忘减少47%；3) 展示了在高影响力真实环境中构建可靠自改进ASR系统的可行路径

Conclusion: 该框架为解决临床ASR部署的核心挑战提供了可行方案，通过隐私保护的自适应机制，使ASR系统能够在资源受限的真实医疗环境中有效运行并持续改进
...
TL;DR: 该论文介绍了CLEF 2025 SimpleText竞赛Task 1的解决方案，使用OpenAI的GPT-4.1系列模型进行科学文本的句子级和文档级简化，比较了无上下文提示工程和微调两种方法。


<details>
  <summary>Details</summary>
Motivation: 科学文本通常包含复杂术语和结构，使得非专业读者难以理解。CLEF 2025 SimpleText竞赛旨在推动科学文本简化技术发展，Task 1要求同时处理句子级和文档级简化，以增强科学知识的可及性。

Method: 使用OpenAI的GPT-4.1、GPT-4.1-mini和GPT-4.1-nano模型，比较两种方法：1）无上下文方法：依赖精心设计的提示工程；2）微调方法：对模型进行特定任务训练。在句子级和文档级两个粒度上评估简化效果。

Result: GPT-4.1-mini模型在无上下文方法中表现出色，在两个简化级别都取得稳健性能。微调模型结果参差不齐，其中GPT-4.1-nano-ft在特定案例的文档级简化中表现突出，表明不同粒度文本简化的复杂性。

Conclusion: 无上下文的GPT-4.1-mini模型在科学文本简化任务中表现最佳，而微调方法的效果因模型和简化粒度而异。研究强调了文本简化在不同粒度上的挑战，为未来科学传播工具开发提供了重要参考。
...
Result: 在Qwen3-Next-80B-A3B-Thinking上成功移除了政治敏感话题的拒绝行为，同时在JailbreakBench上保持安全性，在通用基准测试上接近基线性能。方法在4B和80B模型上均有效，还能诱导定向拒绝。
...
TL;DR: JustRL：一种极简的单阶段训练方法，使用固定超参数在1.5B推理模型上达到SOTA性能，计算量减少2倍，挑战了当前强化学习中复杂训练流程的必要性。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型的强化学习训练方法变得越来越复杂（多阶段训练、动态超参数调度、课程学习等），作者质疑这种复杂性是否必要，希望探索更简单有效的训练方法。

Method: 提出JustRL方法：单阶段训练、固定超参数、不使用复杂的训练技巧。在1.5B推理模型上进行实验，验证简单方法的有效性。

Result: 在9个数学基准测试中，两个1.5B模型分别达到54.9%和64.3%的平均准确率，计算量比复杂方法减少2倍。相同超参数在不同模型间可迁移，训练过程平滑单调，没有崩溃或平台期。消融实验显示传统技巧（如长度惩罚、鲁棒验证器）反而可能降低性能。

Conclusion: 复杂的训练方法可能是为了解决简单基线方法中不存在的问题。JustRL提供了一个简单有效的基线，挑战了当前强化学习训练中过度复杂化的趋势。
...
TL;DR: GinSign框架通过分层分类方法将自然语言映射到系统签名，显著提升了时序逻辑翻译的语义准确性，避免了依赖大型语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有自然语言到时序逻辑翻译框架要么需要准确的原子基础假设，要么存在基础翻译准确率低的问题，这限制了在构建可信自主系统中的应用。

Method: 提出GinSign框架，引入基础模型将自然语言片段映射到给定系统签名。采用分层分解方法：先预测谓词标签，再选择适当类型的常量参数，将任务从自由形式生成问题转化为结构化分类问题。

Result: 实验表明，忽略基础的方法会产生语法正确但语义不等价的提升LTL，而GinSign支持下游模型检查，达到95.5%的基础逻辑等价分数，比现有最佳方法提升1.4倍。

Conclusion: GinSign框架通过结构化分类方法有效解决了自然语言到时序逻辑翻译中的基础问题，显著提高了翻译的语义准确性，为构建可信自主系统提供了更好的形式化规范生成能力。
...
<details>
  <summary>Details</summary>
Motivation: 多模态检索增强生成在生物医学QA中有应用前景，但不确定何时应该将图表转换为文本，何时使用OCR-free视觉检索（返回页面图像让生成器解释）。本研究在视觉密集的糖生物学领域探索这一权衡。

Method: 构建了包含120个多项选择题的基准测试，来自25篇论文，按检索难度分层。实现了四种增强策略：无增强、文本RAG、多模态转换、以及late-interaction视觉检索（ColPali）。使用Docling解析和Qdrant索引，评估了中等规模开源模型和前沿专有模型。

Result: 对于Gemma-3-27B-IT，文本和多模态增强优于OCR-free检索（0.722-0.740 vs 0.510平均准确率）。对于GPT-4o，多模态达到0.808，文本0.782，ColPali 0.745。在GPT-5家族中，ColPali和ColFlor的最佳结果提升约2%至0.828。ColPali、ColQwen和ColFlor在统计上无显著差异。

Conclusion: 管道选择取决于模型能力：将视觉内容转换为文本降低了读取负担，对中等规模模型更可靠；而OCR-free视觉检索在前沿模型下变得有竞争力。在检索器中，ColFlor以较小计算成本提供与较重选项相当的性能，当有强大生成器时是高效默认选择。
...
### [83] [DiscoverDCP: A Data-Driven Approach for Construction of Disciplined Convex Programs via Symbolic Regression](https://arxiv.org/abs/2512.15721)
*Sveinung Myhre*
...
<details>
  <summary>Details</summary>
Motivation: 传统固定参数凸表达式（如二次函数）形式受限且不够准确，而系统识别中发现的模型需要后验验证凸性，这一过程计算上难以处理

Method: 将符号回归与Disciplined Convex Programming（DCP）规则集集成，强制所有发现的候选模型表达式遵守DCP组合规则，确保输出表达式在构造上就是全局凸的

Result: 能够发现比传统固定参数凸表达式更宽松、更准确的凸替代模型，产生可解释、可验证且灵活的凸模型

Conclusion: 该方法适用于安全关键控制和优化任务，通过构造保证凸性，避免了后验验证的计算难题
...
### [84] [Hybrid Quantum-Classical Ensemble Learning for S\&P 500 Directional Prediction](https://arxiv.org/abs/2512.15738)
*Abraham Itzhak Weinberg*

Main category: cs.LG

TL;DR: 本文提出了一种混合集成框架，结合量子情感分析、决策变换器架构和战略模型选择，在S&P 500预测中实现了60.14%的方向准确率，比单个模型提高了3.10%。


<details>
  <summary>Details</summary>
Motivation: 金融市场预测是机器学习的一个挑战性应用，即使方向准确率的小幅提升也能带来巨大价值。大多数模型由于高噪声、非平稳性和市场效率而难以超过55-57%的准确率。

Method: 1. 混合集成框架结合不同学习算法（LSTM、决策变换器、XGBoost、随机森林、逻辑回归）在同一数据上训练；2. 使用4量子比特变分量子电路增强情感分析；3. 智能过滤排除弱预测器（准确率<52%）；4. 基于置信度的过滤（6+模型共识）。

Result: 在2020-2023年市场数据上，该框架实现了60.14%的方向准确率，比相同架构模型在多个数据集上的52.80%高出7.34%。量子情感分析为每个模型带来+0.8%至+1.5%的增益。Top-7模型集成达到60.14%，而所有35个模型集成仅为51.2%。McNemar检验确认统计显著性（p<0.05）。初步回测显示夏普比率为1.2，优于买入持有的0.8。

Conclusion: 该研究证明了架构多样性优于数据集多样性，量子增强的情感分析能显著提升预测性能，智能模型选择对集成性能至关重要。该框架在金融市场预测中具有实际交易潜力。
...
### [85] [LLaDA2.0: Scaling Up Diffusion Language Models to 100B](https://arxiv.org/abs/2512.15745)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决从头训练扩散模型成本高昂的问题，同时保持知识继承和部署效率，提出将预训练的自回归模型转换为离散扩散模型的新范式。

Method: 采用3阶段块级WSD训练方案：1)渐进增加块大小的块扩散（预热），2)大规模全序列扩散（稳定），3)回归紧凑块大小扩散（衰减）。结合SFT和DPO进行后训练对齐，得到16B和100B两个MoE变体。

Result: 开发出LLaDA2.0-mini（16B）和LLaDA2.0-flash（100B）两个指令调优的MoE变体，在保持并行解码优势的同时，在前沿规模上实现了优越的性能和效率。

Conclusion: LLaDA2.0建立了一个新的前沿规模部署范式，通过将预训练AR模型转换为dLLM，实现了知识继承、渐进适应和效率感知设计，两个模型均已开源。
...
### [86] [Abacus: Self-Supervised Event Counting-Aligned Distributional Pretraining for Sequential User Modeling](https://arxiv.org/abs/2512.16581)
*Sullivan Castro,Artem Betlei,Thomas Di Martino,Nadir El Manouzi*

Main category: cs.LG

TL;DR: Abacus是一种用于展示广告的深度序列模型自监督预训练方法，通过预测用户事件的频率分布来增强模型性能，结合聚合统计的稳定性与序列建模的敏感性。
...
Result: 在两个真实世界数据集上的实验表明，Abacus预训练优于现有方法，加速了下游任务收敛，混合方法相比基线在AUC指标上提升高达+6.1%。

Conclusion: Abacus方法有效解决了展示广告中用户行为建模的挑战，通过结合频率分布预测和序列建模，显著提升了预测性能。
...
### [87] [A Unified Generative-Predictive Framework for Deterministic Inverse Design](https://arxiv.org/abs/2512.15746)
*Reza T. Batley,Sourav Saha*

Main category: cs.LG

TL;DR: Janus框架统一生成与预测，通过解缠潜在空间实现异构材料微结构的实时物理感知逆向设计


<details>
  <summary>Details</summary>
Motivation: 异构材料微结构的逆向设计是一个病态且计算昂贵的问题，现有生成模型难以支持快速、稳定的确定性物理感知逆向求解

Method: 提出Janus框架，耦合深度编码器-解码器架构与可分离的KHRONOS预测头，学习同时满足生成逆向和物理预测的等距潜在流形，通过联合目标诱导潜在空间解缠

Result: 在MNIST数据集上实现高保真重建、准确分类和多样逆向生成；在热导率标记的微结构逆向设计中，前向预测R²=0.98，像素重建误差<5%，逆向解满足目标属性误差<1%

Conclusion: Janus通过统一预测和生成于单一潜在空间，实现了实时物理感知的微结构逆向生成，计算成本低于传统优化方法
...
### [88] [D3G: Diverse Demographic Data Generation Increases Zero-Shot Image Classification Accuracy within Multimodal Models](https://arxiv.org/abs/2512.15747)
*Javon Hickmon*
...
<details>
  <summary>Details</summary>
Motivation: 图像分类任务中，低容量模型容易欠拟合，在细粒度分类中表现不佳；数据集不平衡会导致预测偏向多数类别，产生有害的人口统计偏见；需要高质量、多样化的人口统计表示数据

Method: 提出D3G方法：使用CLIP作为基础多模态模型，Stable Diffusion XL作为生成模型，在推理时生成多样化的人口统计数据，无需额外训练，提升零样本分类准确性并减少偏见

Result: 在推理时提供多样化人口统计数据能提升模型性能，并探索了不同人口统计特征对准确率指标的影响

Conclusion: D3G方法能有效提升零样本图像分类准确性，同时减少预训练多模态模型中的人口统计偏见，为公平AI系统提供了一种训练免费的解决方案
...
### [89] [A Special Case of Quadratic Extrapolation Under the Neural Tangent Kernel](https://arxiv.org/abs/2512.15749)
*Abiel Kim*

Main category: cs.LG

TL;DR: 论文分析了ReLU MLP在NTK机制下原点附近的二次外推现象，指出这是与远原点线性外推不同的特殊情形。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注ReLU MLP在分布外评估点的线性外推机制，但对NTK机制下原点附近的外推分析较少。由于NTK诱导的无限维特征映射不具有平移不变性，原点附近的外推与远离原点的外推不等价，需要专门研究。

Method: 在神经正切核（NTK）机制下分析ReLU多层感知器的外推行为，特别关注原点附近这一特殊情形。利用NTK的特征映射特性（旋转不变但非平移不变）来研究两种极端外推情形。

Result: 发现在NTK机制下，当评估点接近原点时，ReLU MLP表现出二次外推行为，这与远离原点时的线性外推形成对比，代表了ReLU NTK外推的两个典型极端边界。

Conclusion: ReLU MLP在NTK机制下的外推行为取决于评估点与原点之间的距离：远离原点时呈现线性外推，接近原点时呈现二次外推。这两种特殊情形构成了ReLU NTK外推的典型极端边界。
...
### [90] [ReactorFold: Generative discovery of nuclear reactor cores via emergent physical reasoning](https://arxiv.org/abs/2512.15756)
*Yoonpyo Lee*

Main category: cs.LG

TL;DR: ReactorFold：将核反应堆燃料组件设计转化为语言模型序列建模问题的生成框架，能够自主发现传统方法无法触及的新设计拓扑结构


<details>
  <summary>Details</summary>
Motivation: 传统核反应堆堆芯设计方法（确定性、元启发式和机器学习辅助方法）在固定的人工定义配置空间中搜索，限制了发现全新设计拓扑结构的能力。需要一种能够超越人类设计约束、发现根本性新设计的方法。

Method: 提出ReactorFold生成框架：1）将燃料组件设计转化为语言模型序列建模问题；2）使用蒙特卡洛数据进行参数高效微调；3）采用直接偏好优化（DPO）对齐模型；4）模型通过单次前向传播生成候选布局。

Result: 1）DPO对齐模型展现出设计空间扩展的涌现能力：尽管仅使用固定数量钆可燃吸收体（Gd）棒配置训练，却能自主调整Gd库存以满足严格的功率峰值约束；2）发现了挑战传统对称装载启发式的高性能非对称配置；3）能够访问传统搜索方法无法触及的设计区域。

Conclusion: 语言模型能够内化因果物理关系并超越人类强加的设计约束，为核反应堆设计开辟了新的可能性，展示了生成式AI在复杂物理系统设计中的潜力。
...
### [91] [Twin Restricted Kernel Machines for Multiview Classification](https://arxiv.org/abs/2512.15757)
*A. Quadir,M. Sajid,Mushir Akhtar,M. Tanveer*

Main category: cs.LG

TL;DR: 提出多视角孪生限制核机(TMvRKM)，通过正则化最小二乘方法替代传统二次规划，结合早期和晚期融合策略，在多视角学习中实现更好的泛化性能和计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统多视角支持向量机(MvSVM)方法在高维空间中使用核技巧时难以有效捕捉决策边界，容易产生误差，且难以处理多视角数据集中的视角不一致性问题。这些方法通常需要解决大规模二次规划问题，计算效率较低。

Method: 提出多视角孪生限制核机(TMvRKM)，将核机优势与多视角框架结合。采用正则化最小二乘方法确定最优分离超平面，避免传统二次规划问题。目标函数包含耦合项以平衡多视角误差，结合早期和晚期融合策略，在训练中利用所有视角的集体信息，同时保持对单个视角特定变化的灵活性。

Result: 在UCI、KEEL和AwA基准数据集上进行严格测试。实验结果表明TMvRKM在所有场景中都优于基线模型，展现出卓越的泛化性能。统计分析也一致确认了其优越性。

Conclusion: TMvRKM成功解决了传统核基方法的关键计算和泛化挑战，通过高效的正则化最小二乘方法和多视角融合策略，在多视角学习中实现了更好的分类性能和计算效率。
...
### [92] [Yantra AI -- An intelligence platform which interacts with manufacturing operations](https://arxiv.org/abs/2512.15758)
*Varshini Krishnamurthy*

Main category: cs.LG

TL;DR: 本文开发了一个用于XRIT的智能生产系统，集成了机器学习模型和AI虚拟助手，通过实时数据可视化和预测性维护提高生产效率
...
Method: 开发智能生产系统，集成随机森林分类器进行预测性维护，隔离森林用于异常检测，使用Streamlit实现实时数据可视化，并集成GPT-4驱动的虚拟助手

Result: 系统测试显示显著提高了工作效率、能源管理和维修规划能力，AI虚拟助手能提供实时有用信息，简化复杂问题解答

Conclusion: 系统具有可扩展性，未来工作将专注于实时数据集成和进一步优化，以在XRIT生产环境中实际应用
...
### [93] [Semantic-Constrained Federated Aggregation: Convergence Theory and Privacy-Utility Bounds for Knowledge-Enhanced Distributed Learning](https://arxiv.org/abs/2512.15759)
*Jahidul Arafat*

Main category: cs.LG

TL;DR: SCFA框架通过引入领域知识约束到联邦学习中，在非IID数据下提升收敛速度41%，隐私-效用权衡改善2.7倍，并在制造业预测性维护数据集上验证了22%的收敛加速。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在非IID数据条件下收敛缓慢，现有解决方案对所有客户端更新采用相同处理方式，忽略了语义有效性。需要将领域知识约束融入分布式优化以改善性能。

Method: 提出语义约束联邦聚合(SCFA)框架，将领域知识约束融入联邦学习优化过程。使用知识图编码ISA-95和MASON本体中的3000个约束，构建理论收敛分析框架。

Result: 理论证明收敛率为O(1/sqrt(T) + rho)，约束使数据异质性降低41%，隐私-效用权衡改善2.7倍。在Bosch生产数据(118万样本，968个传感器特征)上验证，收敛加速22%，模型发散减少41.3%。

Conclusion: SCFA框架通过语义约束有效改善联邦学习在非IID数据下的性能，建立了约束联邦学习的首个收敛理论，在隐私保护、收敛速度和模型质量方面均取得显著提升。
...
### [94] [Machine Learning Framework for Thrombosis Risk Prediction in Rotary Blood Pumps](https://arxiv.org/abs/2512.15761)
...
Method: 引入基于计算流体动力学（CFD）流场特征的可解释机器学习框架，采用逻辑回归（LR）模型结合结构化特征选择流程，推导出紧凑且物理可解释的特征集（包括非线性特征组合）。使用经过验证的宏观尺度血栓模型的空间风险模式进行训练。

Result: 模型成功复现了标记的风险分布，并识别出与血栓风险增加相关的不同流场特征集。当应用于离心泵时，尽管仅在单个轴流泵工作点上训练，模型仍能预测合理的血栓易发区域。

Conclusion: 可解释机器学习能够将局部流场特征与血栓风险联系起来，同时保持计算效率和机理透明性。低计算成本可实现快速血栓形成性筛查，无需重复或昂贵的模拟。该框架补充了基于物理的血栓建模，为将可解释机器学习集成到CFD驱动的血栓分析和设备设计工作流程提供了方法学基础。
...
### [95] [Cross-Sample Augmented Test-Time Adaptation for Personalized Intraoperative Hypotension Prediction](https://arxiv.org/abs/2512.15762)
*Kanxue Li,Yibing Zhan,Hua Jin,Chongchong Qi,Xu Lin,Baosheng Yu*

Main category: cs.LG

TL;DR: CSA-TTA：一种跨样本增强测试时适应框架，通过利用其他患者的低血压事件数据来改善术中低血压的个性化预测，解决了罕见事件导致的测试时训练不可靠问题。


<details>
  <summary>Details</summary>
Motivation: 术中低血压（IOH）具有显著的个体差异性，准确预测困难。虽然测试时适应（TTA）为个性化预测提供了有前景的方法，但IOH事件的罕见性常常导致测试时训练不可靠。

Method: 提出CSA-TTA框架：1）构建跨样本库，将历史数据分割为低血压和非低血压样本；2）采用粗到细检索策略：先用K-Shape聚类识别代表性聚类中心，再基于当前患者信号检索前K个语义相似样本；3）训练时集成自监督掩码重建和回顾性序列预测信号，增强模型对快速细微术中动态的适应性。

Result: 在VitalDB数据集和真实医院数据集上评估，与TimesFM和UniTS等最先进时间序列预测模型集成。CSA-TTA在所有设置中持续提升性能：在VitalDB上，微调场景下Recall和F1分数分别提升+1.33%和+1.13%，零样本场景下分别提升+7.46%和+5.07%，表现出强大的鲁棒性和泛化能力。

Conclusion: CSA-TTA通过跨样本增强有效解决了IOH预测中罕见事件导致的测试时训练不可靠问题，显著提升了预测性能，为术中低血压的个性化预测提供了有效解决方案。
...
TL;DR: 本文提出了一种针对使用DPO训练的大型语言模型的高效数据价值评估方法，解决了传统Shapley值计算计算成本过高的问题。


<details>
  <summary>Details</summary>
Motivation: 数据是训练大型语言模型的关键资产，但数据价值评估面临计算挑战。传统基于合作博弈论和Shapley值的方法需要大量模型重训练，对于大型模型计算成本过高。需要找到针对LLMs的高效数据价值评估方法。

Method: 利用Direct Preference Optimization (DPO)训练LLMs时的特定数学结构，开发可扩展的Shapley值计算方法。DPO的特殊性质使得数据价值评估的计算复杂度大幅降低。

Result: 证明了DPO的数学结构能够显著简化Shapley值的计算，使得针对大型语言模型的数据价值评估变得可行和高效。

Conclusion: 该方法为数据价值评估与大型语言模型的交叉应用开辟了新途径，使数据所有者能够更明智地制定数据策展策略，并促进多方数据资源的公平协作。
...
### [97] [TS-DP: Reinforcement Speculative Decoding For Temporal Adaptive Diffusion Policy Acceleration](https://arxiv.org/abs/2512.15773)
*Ye Li,Jiahe Feng,Yuan Meng,Kangye Ji,Chen Tang,Xinwan Wen,Shutao Xia,Zhi Wang,Wenwu Zhu*

Main category: cs.LG

TL;DR: TS-DP提出了一种用于扩散策略的时序感知强化学习推测解码框架，通过Transformer草稿器和RL调度器实现动态计算调整，在保持性能的同时将推理速度提升4.17倍，达到25Hz实时控制频率。


<details>
  <summary>Details</summary>
Motivation: 扩散策略在具身控制中表现出色，但存在推理延迟高、计算成本大的问题。静态加速方法无法处理动态具身任务，而推测解码作为无损自适应方案在扩散策略中尚未充分探索。需要解决如何在时间变化的任务难度下以更低成本匹配基础模型的去噪质量，以及如何动态调整计算资源。

Method: 提出TS-DP框架：1) 使用Transformer草稿器模仿基础模型，替代昂贵的去噪调用；2) 基于强化学习的调度器根据任务难度动态调整推测参数，在保持准确性的同时提高效率。

Result: 在多种具身环境中的实验表明，TS-DP实现了高达4.17倍的推理加速，草稿接受率超过94%，推理频率达到25Hz，实现了实时扩散控制且无性能下降。

Conclusion: TS-DP是首个为扩散策略提供时序自适应推测解码的框架，成功解决了动态具身任务中的计算效率问题，实现了高性能实时控制。
...
### [98] [Adversarial Robustness in Financial Machine Learning: Defenses, Economic Impact, and Governance Evidence](https://arxiv.org/abs/2512.15780)
*Samruddhi Baviskar*
...
Conclusion: 金融决策中的表格机器学习模型对对抗攻击脆弱，需要采用对抗训练等防御措施来提高鲁棒性
...
### [99] [Boosting t-SNE Efficiency for Sequencing Data: Insights from Kernel Selection](https://arxiv.org/abs/2512.15900)
*Avais Jan,Prakash Chourasia,Sarwan Ali,Murray Patterson*

Main category: cs.LG

TL;DR: 本研究系统评估了t-SNE中九种不同核函数在生物序列数据降维中的表现，发现余弦相似性核在可视化质量和计算效率上均优于传统高斯核和隔离核，特别适合大规模生物序列分析。


<details>
  <summary>Details</summary>
Motivation: 传统t-SNE使用的高斯核缺乏数据依赖性且计算开销大，限制了其在分类生物序列数据上的可扩展性和有效性。虽然已有研究提出隔离核作为替代，但仍未最优捕捉序列相似性，因此需要系统评估不同核函数在生物序列降维中的表现。

Method: 研究评估了九种不同核函数在t-SNE中的应用，使用三种嵌入方法：One-Hot编码、Spike2Vec和minimizers。通过主观可视化和客观指标（包括邻域保持分数）进行评估，并在六个不同的生物数据集上进行广泛的分类和聚类实验验证。

Result: 余弦相似性核在总体上优于其他核函数（包括高斯核和隔离核），具有更优的运行效率和更好的低维空间成对距离保持能力。核函数选择不仅影响可视化质量，还显著影响下游分析任务，余弦相似性核在不同数据类型和嵌入策略中表现最稳健。

Conclusion: 余弦相似性核是t-SNE应用于生物序列数据降维的最佳选择，特别适合大规模生物序列分析，能提供更优的可视化质量和下游分析性能。
...
### [100] [A Unification of Discrete, Gaussian, and Simplicial Diffusion](https://arxiv.org/abs/2512.15923)
...
TL;DR: 该论文提出了一个统一框架，将离散扩散、高斯扩散和单纯形扩散三种方法统一为同一底层过程（Wright-Fisher种群遗传模型）的不同参数化形式，解决了现有方法算法分散、理论结构不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 当前建模DNA、蛋白质和语言等离散序列的扩散方法存在三种主要方法：离散空间扩散、欧几里得空间的高斯扩散和单纯形上的扩散。这些方法虽然目标相同，但算法、理论结构和权衡各不相同，缺乏统一的理论框架。离散扩散具有最自然的领域，高斯扩散算法更成熟，单纯形扩散理论上结合了两者优点但实践中存在数值不稳定问题。需要建立一个统一框架，使这些方法能够相互转换。

Method: 通过将三种离散扩散方法统一为Wright-Fisher种群遗传模型的不同参数化形式，发现单纯形扩散和高斯扩散是该模型的两个大种群极限。利用数十年数学遗传学文献来解锁稳定的单纯形扩散，并训练能够在测试时在任意这三种域中执行扩散的单一模型。

Result: Wright-Fisher单纯形扩散比以前的单纯形扩散模型更稳定，在条件DNA生成任务上表现更好。实验表明，可以在多个域上同时训练模型，这些模型与在单个域上训练的模型具有竞争力。

Conclusion: 该研究建立了一个统一理论，将三种离散扩散方法作为同一底层过程的不同参数化形式，正式连接了这些模型的似然和超参数，利用种群遗传学理论解决了单纯形扩散的稳定性问题，并实现了单一模型在多个域上的灵活应用。
...
### [101] [DSO: Direct Steering Optimization for Bias Mitigation](https://arxiv.org/abs/2512.15926)
*Lucas Monteiro Paes,Nivedha Sivakumar,Yinong Oliver Wang,Masha Fedzechkina Donaldson,Luca Zappella,Nicholas Apostoloff*

Main category: cs.LG

TL;DR: 本文提出DSO方法，使用强化学习优化激活导向，在推理时控制视觉语言模型和大型语言模型的偏见与性能平衡。


<details>
  <summary>Details</summary>
Motivation: 生成模型在决策时存在人口统计学偏见（如将女性识别为医生的概率较低），而现有激活导向方法难以纠正这种需要跨群体等概率结果的偏见。用户对偏见缓解与模型性能的平衡需求各异，需要推理时可控制的偏见减少方法。

Method: 提出直接导向优化（DSO）方法，使用强化学习寻找激活的线性变换，专门针对偏见缓解进行优化，同时保持对模型性能的控制。

Result: DSO在视觉语言模型和大型语言模型上实现了公平性与能力之间的最先进权衡，为从业者提供了推理时对权衡的控制能力。

Conclusion: 直接优化导向策略比依赖预定义启发式方法更有效地控制模型行为，为偏见干预提供了更有效的解决方案。
...
### [102] [BarcodeMamba+: Advancing State-Space Models for Fungal Biodiversity Research](https://arxiv.org/abs/2512.15931)
...
Method: 提出BarcodeMamba+基础模型，采用状态空间模型架构；使用预训练-微调范式利用部分标记数据；在微调阶段整合层次标签平滑、加权损失函数和MycoAI的多头输出层等增强技术。

Result: 每个增强组件都带来显著性能提升；在具有明显分类分布偏移的挑战性真菌分类基准测试中，最终模型在所有分类级别上都优于现有方法。
...
### [103] [In-Context Semi-Supervised Learning](https://arxiv.org/abs/2512.15934)
*Jiashuo Fan,Paul Rosu,Aaron T. Wang,Michael Li,Lawrence Carin,Xiang Cheng*

Main category: cs.LG

TL;DR: 论文研究了Transformer在上下文半监督学习中的能力，展示了即使标签稀疏或缺失时，Transformer也能利用未标记的上下文数据学习鲁棒的表示。


<details>
  <summary>Details</summary>
Motivation: 现有理论研究主要关注有明确标签对的监督学习设置，但在实践中Transformer即使在标签稀疏或缺失的情况下也能表现良好，这表明未标记的上下文演示中存在关键结构。

Method: 引入了上下文半监督学习框架，其中少量标记示例伴随大量未标记点，研究Transformer如何利用未标记上下文学习上下文相关的鲁棒表示。

Result: Transformer能够利用未标记上下文学习鲁棒的表示，这种表示支持准确预测，并在低标签情况下显著提升性能。

Conclusion: 研究为理解Transformer如何在上下文学习框架中利用未标记上下文进行表示学习提供了基础性见解。
...
### [104] [Tracking Wildfire Assets with Commodity RFID and Gaussian Process Modeling](https://arxiv.org/abs/2512.15956)
*John Hateley,Sriram Narasimhan,Omid Abari*

Main category: cs.LG

TL;DR: 提出一种基于商品RFID的低成本、可扩展方法，用于在森林环境中追踪大量资产，无需预先标记已知位置即可达到GPS级别的定位精度。


<details>
  <summary>Details</summary>
Motivation: 解决商品RFID系统在森林环境中因信号衰减、多径效应和环境变化导致的标签定位困难问题，同时避免现有指纹识别方法需要预先在已知位置部署标签的限制。

Method: 使用高斯过程建模不同环境的RF信号响应特征，无需GPS或摄像头等额外传感器。提出新的加权对数似然方法，将未知环境与预先建模的环境字典中最接近的模型进行匹配。

Result: 能够实现与GPS相当的定位精度，使用被动商品RFID可同时追踪数十个野火响应资产，无需预先标记已知位置，且成本远低于GPS系统。

Conclusion: 该方法为森林环境中的资产追踪提供了一种成本效益高、可扩展的解决方案，特别适用于野火响应等应用场景，突破了传统RFID定位方法的限制。
...
### [105] [Provably Extracting the Features from a General Superposition](https://arxiv.org/abs/2512.15987)
*Allen Liu*

Main category: cs.LG

TL;DR: 该论文提出了一种从叠加特征中学习的高效查询算法，能够从噪声查询访问中识别非退化特征方向并重构函数。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型通常通过线性表示编码特征，但这些特征以叠加形式存在，难以恢复。特别是在过完备机制（特征数量大于维度）下，现有算法面临挑战。需要开发能够处理任意叠加和一般响应函数的通用算法。

Method: 提出一种高效的查询算法，通过噪声查询访问函数f，在傅里叶空间中迭代细化搜索空间来定位隐藏的特征方向vi。算法仅要求特征方向不完全相同，适用于任意叠加和一般响应函数。

Result: 算法能够识别所有非退化响应的特征方向并重构函数f。该算法比所有相关先前结果更通用，适用于更一般的设置，包括任意叠加和一般响应函数。

Conclusion: 该研究为从叠加特征中学习提供了有效的算法解决方案，特别是在过完备机制下，能够处理更一般的设置，为复杂机器学习模型的特征恢复问题提供了新方法。
...
### [106] [Higher-Order LaSDI: Reduced Order Modeling with Multiple Time Derivatives](https://arxiv.org/abs/2512.15997)
*Robert Stephany,William Michael Anderson,Youngsoo Choi*

Main category: cs.LG

TL;DR: 提出一种结合高阶有限差分格式和Rollout损失函数的降阶模型方法，用于解决偏微分方程在长时间尺度上的预测精度下降问题


<details>
  <summary>Details</summary>
Motivation: 传统降阶模型在处理参数化偏微分方程家族时，在长时间尺度上的预测能力会显著下降，需要新的方法来提高长时间预测的准确性

Method: 1. 引入灵活、高阶且计算成本低的有限差分格式；2. 提出Rollout损失函数，训练降阶模型在任意时间尺度上都能做出准确预测

Result: 在二维Burgers方程上验证了所提方法的有效性，展示了在长时间尺度上的改进预测性能

Conclusion: 通过结合高阶有限差分格式和Rollout损失函数，能够显著提升降阶模型在长时间尺度上的预测能力，为解决复杂偏微分方程提供了更有效的数值方法
...
TL;DR: 该研究将LDA主题模型的主题数量T选择问题形式化为离散黑盒优化问题，比较了四种优化器在固定评估预算下的性能，发现摊销优化器在样本和时间效率上显著优于传统进化方法。


<details>
  <summary>Details</summary>
Motivation: LDA主题模型中主题数量T的选择是一个关键设计决策，它强烈影响模型的统计拟合度和可解释性。传统方法通常需要大量试错或启发式规则，缺乏系统化的优化框架。

Method: 将主题数量T选择问题形式化为离散黑盒优化问题，每个函数评估对应训练一个LDA模型并测量其验证困惑度。在固定评估预算下，比较了四种优化器：两种手工设计的进化方法（遗传算法GA和进化策略ES），以及两种学习的摊销方法（偏好摊销黑盒优化PABBO和锐度感知黑盒优化SABBO）。

Result: 虽然GA、ES、PABBO和SABBO最终都能达到相似的困惑度区间，但摊销优化器在样本和时间效率上显著更优。SABBO通常只需一次评估就能识别出接近最优的主题数量，PABBO在几次评估内就能找到有竞争力的配置，而GA和ES需要几乎全部预算才能接近相同区域。

Conclusion: 摊销黑盒优化方法为LDA主题数量选择提供了更高效的解决方案，显著减少了计算成本和评估时间，为实际应用中的超参数优化提供了有前景的方向。
...
### [108] [Towards Fine-Tuning-Based Site Calibration for Knowledge-Guided Machine Learning: A Summary of Results](https://arxiv.org/abs/2512.16013)
*Ruolei Zeng,Arun Sharma,Shuai An,Mingzhou Yang,Shengya Zhang,Licheng Liu,David Mulla,Shashi Shekhar*

Main category: cs.LG

TL;DR: FTBSC-KGML是一个结合预训练-微调、空间变异感知和知识引导的机器学习框架，用于准确量化农业生态系统碳循环，通过利用迁移学习和空间异质性来提高区域尺度预测精度。


<details>
  <summary>Details</summary>
Motivation: 在决策相关尺度上准确且经济高效地量化农业生态系统碳循环对于气候缓解和可持续农业至关重要。传统方法通常依赖位置无关的参数化和独立训练，未能充分利用迁移学习和输入数据的空间异质性，限制了在具有显著变异性的区域的适用性。

Method: 提出FTBSC-KGML框架，结合预训练-微调过程、空间变异感知和知识引导机器学习。该框架在KGML-ag基础上增加了预训练-微调过程和站点特定参数，使用遥感GPP、气候和土壤协变量数据，通过全局预训练模型在各州或站点进行微调，学习位置感知表示。

Result: FTBSC-KGML在验证误差方面表现更低，解释力一致性更强，相比纯全局模型能更好地捕捉跨州的空间变异性，在有限数据条件下提高了局部精度而不牺牲可解释性。

Conclusion: 该工作扩展了先前的SDSA-KGML框架，证明了结合预训练-微调、空间变异感知和知识引导的机器学习方法能够有效利用迁移学习和空间异质性，提高农业生态系统碳循环量化的准确性和适用性。
...
TL;DR: 该论文研究了基于《纽约时报》语料库训练的多标签分类器中"blacks"主题标签的偏见问题，发现该标签部分充当了"种族主义检测器"，但在现代语境下表现不佳，揭示了新闻机构采用AI工具时面临的历史偏见再现风险。


<details>
  <summary>Details</summary>
Motivation: AI模型在训练过程中会编码历史数据中的态度和刻板印象，当这些模型应用于新闻研究和新闻编辑室环境时，可能会传播和强化历史偏见。本研究旨在调查基于《纽约时报》语料库训练的分类器中"blacks"标签的具体偏见表现，以揭示AI工具在新闻应用中可能带来的风险。

Method: 采用定量和定性分析方法，研究"blacks"标签在训练语料库中的使用情况，通过可解释AI方法探索该标签在训练分类器中编码的概念，并评估这些概念对模型使用的影响。特别测试了该标签在现代语境下的表现，如COVID-19时期反亚裔仇恨报道和"黑人的命也是命"运动报道。

Result: 研究发现"blacks"标签在一定程度上充当了针对某些少数群体的"种族主义检测器"，但在现代示例中表现不佳。该标签未能准确识别COVID-19时期的反亚裔仇恨报道，对"黑人的命也是命"运动的报道也处理不当。这表明模型编码了历史偏见，无法适应现代语境。

Conclusion: 这项案例研究揭示了AI模型中嵌入的历史偏见问题，表明类似应用在新闻编辑室环境中可能导致意外输出，影响故事发现、受众定位、摘要生成等多种潜在用途。新闻机构面临的根本矛盾是如何在采用AI工作流程工具的同时，降低在新闻报道中再现历史偏见的风险。
...
### [110] [CauSTream: Causal Spatio-Temporal Representation Learning for Streamflow Forecasting](https://arxiv.org/abs/2512.16046)
*Shu Wan,Reepal Shah,John Sabo,Huan Liu,K. Selçuk Candan*

Main category: cs.LG

TL;DR: CauSTream是一个用于因果时空径流预测的统一框架，通过联合学习径流因果图和路由图，在三个美国主要河流流域上超越现有方法，特别是在长期预测中表现更优。


<details>
  <summary>Details</summary>
Motivation: 虽然深度学习模型在径流预测中表现出色，但它们通常忽略物理过程，限制了可解释性和泛化能力。现有的因果学习方法虽然整合了领域知识，但通常依赖固定的因果图，无法适应数据变化。

Method: 提出CauSTream框架，联合学习(i)气象强迫与径流之间的因果图，以及(ii)捕捉站点间动态依赖关系的路由图。在非参数设置下建立了这些因果结构的可识别性条件。

Result: 在三个美国主要河流流域和三个预测时间尺度上评估，CauSTream始终优于先前的最先进方法，预测窗口越长性能差距越大，表明对未见条件有更强的泛化能力。学习到的因果图与领域知识一致，提供了流域动力学的可解释见解。

Conclusion: CauSTream为因果时空建模提供了理论基础，具有扩展到广泛科学和环境应用的潜力，通过整合物理知识和数据驱动学习，实现了更好的预测性能和可解释性。
...
Method: 研究聚焦两个基本问题：(1)策略熵与性能的关系；(2)虚假奖励是否通过裁剪偏差和模型污染相互作用带来收益。通过分析虚假奖励下的裁剪偏差如何降低策略熵，以及提出奖励错配模型来解释虚假奖励在污染环境之外的性能提升。

Result: 研究发现虚假奖励下的裁剪偏差会降低策略熵，导致更自信和确定性的输出，而单独的熵最小化不足以带来改进。进一步提出的奖励错配模型解释了虚假奖励如何在非污染设置下也能提升性能。

Conclusion: 研究结果阐明了虚假奖励带来收益的机制，为更有效的RLVR训练提供了原则性指导，澄清了探索-利用权衡在提升大语言模型推理能力中的作用。
...
### [112] [In-Context Multi-Operator Learning with DeepOSets](https://arxiv.org/abs/2512.16074)
*Shao-Ting Chiu,Aditya Nambiar,Ali Syed,Jonathan W. Siegel,Ulisses Braga-Neto*

Main category: cs.LG

TL;DR: DeepOSets架构通过结合DeepSets和DeepONets，展示了无需权重更新的上下文学习能力，能够从示例中学习新PDE的解算子，并证明了其在科学机器学习中的通用逼近性质。


<details>
  <summary>Details</summary>
Motivation: 研究DeepOSets架构在科学机器学习中的上下文学习能力，特别是其能否从示例中学习未见过的偏微分方程的解算子，并探索其理论逼近性质。

Method: 提出改进的DeepOSets架构，结合DeepSets的集合学习和DeepONets的算子学习能力，通过用户提示中的参数-解示例对，无需权重更新即可学习新PDE的解算子。

Result: DeepOSets能够准确预测训练中未见过的PDE的解，在泊松方程和反应扩散方程的正反边值问题上表现良好，并证明了其是连续算子类的通用一致逼近器。

Conclusion: DeepOSets架构展示了强大的多算子上下文学习能力，能够从示例中学习新PDE的解算子，为科学机器学习中的算子学习提供了新的理论框架和实践工具。
...
<details>
  <summary>Details</summary>
Motivation: 图像数据中的隐私信息（如人脸、车牌）需要保护，同时保持数据对模型训练的有用性。现有标准方法高斯模糊在实际应用中存在隐私风险

Method: 比较四种模糊算法：高斯模糊、像素化、像素化加噪声（DP-Pix）和裁剪。通过逆向攻击和识别攻击评估隐私性，通过在模糊面部数据上训练模型的质量评估效用性

Result: 高斯模糊在实际低精度实现中最不安全，易受逆向攻击；像素化和像素化加噪声在适当粒度下能同时提供隐私保护和模型训练效用

Conclusion: 行业标准的高斯模糊方法隐私保护不足，像素化和像素化加噪声是更好的选择，作者开发了Privacy Blur软件包提供建议参数
...
<details>
  <summary>Details</summary>
Motivation: 市场操纵现在经常来自协调的社交媒体活动，而非孤立的交易。散户投资者、监管机构和经纪商需要能够将在线叙事和协调模式与市场行为联系起来的工具。

Method: AIMM框架融合Reddit活动、机器人和协调指标以及OHLCV市场特征，使用parquet原生管道和Streamlit仪表板。由于Reddit API限制，采用校准的合成社交特征匹配事件特征；市场数据使用Yahoo Finance的真实历史数据。

Result: 构建了AIMM-GT数据集（33个标记的股票-日，涵盖8只股票），实现了前向评估和预测日志记录，分析显示AIMM在2021年1月GME挤压峰值前22天发出预警。

Conclusion: 虽然当前标记集较小（33个股票-日，3个正面事件），但结果显示初步的判别能力和对GME事件的早期预警。发布代码、数据集模式和仪表板设计以支持社交媒体驱动的市场监控研究。
...
### [115] [BUILD with Precision: Bottom-Up Inference of Linear DAGs](https://arxiv.org/abs/2512.16111)
*Hamed Ajorlou,Samuel Rey,Gonzalo Mateos,Geert Leus,Antonio G. Marques*

Main category: cs.LG

TL;DR: BUILD算法通过利用观测数据精度矩阵的独特结构，以确定性的自底向上方式学习线性高斯SEM下的DAG结构，在有限数据情况下通过周期性重新估计精度矩阵提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 从观测数据中学习有向无环图（DAG）结构是因果发现、统计信号处理和机器学习中的核心问题。在线性高斯结构方程模型（SEM）且噪声方差相等的假设下，该问题是可识别的，但需要有效算法来准确重建DAG。

Method: 提出BUILD（Bottom-Up Inference of Linear DAGs）算法，利用观测数据精度矩阵的独特结构特征，通过确定性逐步方法：1）识别叶节点及其父节点；2）通过移除入射边修剪叶节点；3）重复此过程直至完全重建DAG。在有限数据情况下，采用周期性重新估计精度矩阵的策略来缓解误差累积。

Result: 在具有挑战性的合成基准测试中，BUILD算法与最先进的DAG学习算法相比表现优异，同时提供了明确的复杂度控制。算法能够从真实精度矩阵中精确重建DAG，在有限数据估计情况下通过重新估计策略保持鲁棒性。

Conclusion: BUILD算法为线性高斯SEM下的DAG学习提供了一种有效且确定性的解决方案，通过利用精度矩阵的结构特征和自底向上的逐步推理方法，在准确性和计算效率之间取得了良好平衡，特别适用于噪声方差相等的情况。
...
### [116] [Dual-View Inference Attack: Machine Unlearning Amplifies Privacy Exposure](https://arxiv.org/abs/2512.16126)
*Lulu Xue,Shengshan Hu,Linqiang Qian,Peijin Guo,Yechao Zhang,Minghui Li,Yanjun Zhang,Dayong Ye,Leo Yu Zhang*

Main category: cs.LG

TL;DR: 论文提出机器遗忘技术虽然能删除特定训练数据，但会引入新的隐私风险，特别是在双视图设置下，攻击者可以同时查询原始模型和遗忘模型，从而放大保留数据的隐私泄露。


<details>
  <summary>Details</summary>
Motivation: 机器遗忘技术用于从训练模型中删除特定数据以满足数据删除请求，但现有研究主要关注已删除数据的隐私风险，而保留数据的隐私风险尚未充分探索。论文旨在填补这一空白，揭示双视图设置下机器遗忘引入的隐私漏洞。

Method: 从信息论角度引入"隐私知识增益"概念，证明双视图设置使攻击者能获得比单独查询任一模型更多的信息。提出DVIA（双视图推理攻击），通过黑盒查询两个模型提取保留数据的成员信息，无需训练攻击模型，使用轻量级似然比推理模块进行高效推理。

Result: 在不同数据集和模型架构上的实验验证了DVIA的有效性，并突显了双视图设置固有的隐私风险。攻击者通过同时查询原始模型和遗忘模型，能够更有效地推断保留数据的成员信息。

Conclusion: 机器遗忘技术虽然保护了请求删除数据的用户权利，但在双视图设置下会引入新的隐私风险，放大保留数据的隐私泄露。需要重新评估机器遗忘系统的隐私保护机制，特别是在多模型访问场景下。
...
### [117] [INTELLECT-3: Technical Report](https://arxiv.org/abs/2512.16144)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够在数学、代码、科学和推理任务上达到最先进性能的高效大型语言模型，同时开源完整的训练基础设施以促进社区研究和应用。

Method: 使用混合专家架构（106B参数，12B激活），基于GLM-4.5-Air-Base模型进行监督微调和强化学习训练，采用新开发的prime-rl框架进行大规模异步RL训练，支持多轮交互和工具使用。

Result: INTELLECT-3在数学、代码、科学和推理基准测试中达到了同尺寸模型的最先进性能，超越了多个更大的前沿模型，并在512个H200 GPU上实现了高效的RL训练扩展。

Conclusion: INTELLECT-3展示了通过大规模强化学习训练的高效MoE模型在多种认知任务上的卓越性能，同时开源的基础设施栈为社区提供了完整的端到端RL训练解决方案。
...
### [118] [Explicit and Non-asymptotic Query Complexities of Rank-Based Zeroth-order Algorithms on Smooth Functions](https://arxiv.org/abs/2512.16200)
*Haishan Ye*
...
<details>
  <summary>Details</summary>
Motivation: 基于排序的零阶优化方法（如CMA-ES、自然进化策略等）在实践中广泛成功，但现有理论分析仅提供渐进性见解，缺乏对选择top-k方向算法的显式收敛速率分析。

Method: 分析一个简单的基于排序的零阶优化算法，采用新颖的分析方法，避免传统的漂移和信息几何技术，建立显式非渐进查询复杂度。

Result: 对于d维问题：1）L-光滑且μ-强凸函数，达到$\widetilde{\mathcal O}\!\left(\frac{dL}μ\log\!\frac{dL}{μδ}\log\!\frac{1}{\varepsilon}\right)$查询复杂度；2）光滑非凸目标，达到$\mathcal O\!\left(\frac{dL}{\varepsilon}\log\!\frac{1}{\varepsilon}\right)$查询复杂度，概率至少1-δ。

Conclusion: 该研究填补了基于排序的零阶优化理论空白，首次提供了显式非渐进查询复杂度分析，为理解基于排序的启发式方法为何能实现高效零阶优化提供了新见解。
...
### [119] [Neural emulation of gravity-driven geohazard runout](https://arxiv.org/abs/2512.16221)
*Lorenzo Nava,Ye Chen,Maximillian Van Wyk de Vries*

Main category: cs.LG

TL;DR: 该研究开发了一个机器学习模型来预测地质灾害（如滑坡、雪崩）的运移距离，该模型能够以比传统数值求解器快100到10,000倍的速度，准确预测流动范围和沉积厚度。


<details>
  <summary>Details</summary>
Motivation: 地质灾害运移预测对保护生命、基础设施和生态系统至关重要。快速质量流（包括滑坡和雪崩）在不同环境中造成数千人死亡，通常从其源头运移数公里。由于源头条件和材料特性范围广泛，使得运移距离难以预测，特别是对可能突然面临严重影响的社区。准确的大规模预测需要既物理真实又计算高效的模型，但现有方法面临速度与真实性的基本权衡。

Method: 训练机器学习模型来预测地质灾害运移，使用超过100,000个数值模拟数据，覆盖超过10,000个真实世界数字高程模型芯片。该模型能够再现关键物理行为，包括分流和沉积模式，同时在不同流动类型、大小和景观中具有泛化能力。

Result: 模型能够以高准确度预测流动范围和沉积厚度，计算速度比数值求解器快100到10,000倍。该模型在多样化的真实世界地形上实现了快速、空间分辨的运移预测。

Conclusion: 神经仿真为实现快速、物理真实的地质灾害建模提供了有前景的途径，能够扩展到与大规模早期预警系统相关的空间和时间尺度，为灾害风险减少和基于影响的预测开辟了新机会。
...
### [120] [Coarse-to-Fine Open-Set Graph Node Classification with Large Language Models](https://arxiv.org/abs/2512.16244)
*Xueqi Ma,Xingjun Ma,Sarah Monazam Erfani,Danilo Mandic,James Bailey*

Main category: cs.LG

TL;DR: 本文提出了一种从粗到细的开放集分类框架CFC，利用大语言模型处理图数据集，不仅能检测OOD样本，还能对OOD样本进行分类，无需真实标签信息。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将所有OOD样本视为单一类别，但在欺诈检测和医疗诊断等高风险应用中，需要对OOD样本有更深入的了解，包括其可能的标签。这引出了一个关键问题：能否在没有真实标签信息的情况下，将OOD检测扩展到OOD分类？

Method: CFC框架包含三个关键组件：1）粗分类器使用LLM提示进行OOD检测和异常标签生成；2）基于GNN的细分类器使用粗分类器识别的OOD样本进行训练，增强OOD检测和ID分类；3）通过LLM提示和后处理的OOD标签实现精细化的OOD分类。该方法使用语义上真正属于分布外的OOD实例，而非合成或辅助样本。

Result: 实验结果表明，CFC在图和文本领域的OOD检测性能比最先进方法提高了10%，在图数据集上的OOD分类准确率达到70%。

Conclusion: CFC框架成功地将OOD检测扩展到OOD分类，无需真实标签信息，利用LLM的语义理解能力，提高了开放集分类的实用性和可解释性。
...
### [121] [Sharpness-aware Federated Graph Learning](https://arxiv.org/abs/2512.16247)
*Ruiyu Li,Peige Zhao,Guangxia Li,Pengcheng Wu,Xingyu Gao,Zhiqiang Xu*

Main category: cs.LG

TL;DR: SEAL算法通过同时最小化损失函数及其锐度，并引入基于相关矩阵的正则化器，解决联邦图学习中的数据异构性和维度崩溃问题，提升GNN模型的分类准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 联邦图学习面临两个核心挑战：1）基于经验风险最小化的优化器导致局部模型陷入尖锐谷，削弱对分布外图数据的泛化能力；2）局部图数据表示中的维度崩溃对GNN模型的分类能力产生负面影响。

Method: 提出SEAL算法：1）制定新的优化目标，同时考虑局部GNN模型的损失函数及其锐度（损失曲面曲率），寻找平坦区域中的模型参数；2）引入基于局部表示相关矩阵的正则化器，放松单个局部图样本生成表示之间的相关性，缓解维度崩溃。

Result: 在多个图分类基准测试中，SEAL算法持续优于最先进的联邦图学习基线方法，并为更多参与者提供性能增益。

Conclusion: SEAL算法通过锐度感知优化和维度崩溃缓解，有效提升了联邦图学习中局部GNN模型的分类准确性和泛化能力，解决了数据异构性带来的挑战。
...
### [122] [Sharpness-aware Second-order Latent Factor Model for High-dimensional and Incomplete Data](https://arxiv.org/abs/2512.16277)
*Jialiang Wang,Xueyan Bao,Hao Wu*

Main category: cs.LG

TL;DR: 提出Sharpness-aware SLF模型，通过Hessian-vector products获取二阶信息并在曲率中注入锐度项，以解决SLF模型优化困难问题


<details>
  <summary>Details</summary>
Motivation: 二阶潜在因子模型在处理高维不完整数据时有效，但由于其双线性和非凸性质导致优化困难。Sharpness-aware Minimization方法通过寻找平坦局部最小值来改善表示学习模型的泛化能力
...
### [123] [Feature-Selective Representation Misdirection for Machine Unlearning](https://arxiv.org/abs/2512.16297)
*Taozhao Chen,Linghan Huang,Kim-Kwang Raymond Choo,Huaming Chen*

Main category: cs.LG

TL;DR: SRMU是一种新颖的激活编辑框架，通过特征感知和方向控制的扰动，在高度纠缠的数据分布中实现选择性知识遗忘，在保持模型实用性的同时有效移除有害知识。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在安全关键和受监管领域的应用增加，模型中保留的敏感或禁止知识带来了隐私泄露、监管不合规和潜在滥用等风险。现有遗忘技术假设遗忘集和保留集完全分离，但在实际运营环境中数据分布高度纠缠，传统方法会损害模型实用性或无法确保安全。

Method: 提出选择性表征误导遗忘（SRMU）框架，采用结构化误导向量和激活重要性映射，进行特征感知和方向控制的激活编辑。不同于无差别的权重扰动，SRMU选择性地抑制有害表征，同时保留良性表征的实用性。

Result: 在WMDP基准测试中，SRMU在低度和高度纠缠配置下都实现了最先进的遗忘性能，且实用性损失最小。在20-30%数据重叠的情况下仍保持有效，而现有基线方法会失效。

Conclusion: SRMU为安全驱动的模型治理、隐私合规和受控知识移除提供了稳健基础，适用于新兴的LLM应用场景。该方法在高度纠缠的数据分布中实现了有效的选择性遗忘，平衡了安全性和实用性需求。
...
### [124] [Multivariate Uncertainty Quantification with Tomographic Quantile Forests](https://arxiv.org/abs/2512.16383)
*Takuya Kanazawa*

Main category: cs.LG

TL;DR: Tomographic Quantile Forests (TQF) 是一种用于多变量目标的非参数、不确定性感知的树回归模型，通过方向投影学习条件分位数，然后聚合重建多变量条件分布。


<details>
  <summary>Details</summary>
Motivation: 量化预测不确定性对于AI在现实世界中的安全可信部署至关重要，但目前多变量目标的完全非参数条件分布估计仍然具有挑战性。

Method: TQF学习方向投影n⊤y的条件分位数作为输入x和单位方向n的函数。在推理时，它聚合多个方向的分位数，通过高效的交替方案（具有凸子问题）最小化切片Wasserstein距离来重建多变量条件分布。

Result: TQF在合成和真实世界数据集上进行了评估，与经典方向分位数方法相比，TQF能够用单一模型覆盖所有方向而不施加凸性限制。

Conclusion: TQF提供了一种有效的非参数方法来估计多变量目标的条件分布，为AI系统的不确定性量化提供了新工具。
...
### [125] [Kascade: A Practical Sparse Attention Method for Long-Context LLM Inference](https://arxiv.org/abs/2512.16391)
*Dhruv Deshmukh,Saurabh Goyal,Nipun Kwatra,Ramachandran Ramjee*

Main category: cs.LG

TL;DR: Kascade是一种无需训练的稀疏注意力方法，通过锚定层计算Top-k索引并在中间层重用，显著提升长上下文LLM推理速度
...
Method: Kascade利用两个已知观察：1) softmax后的注意力本质上是稀疏的；2) 高权重键的身份在相邻层之间是稳定的。该方法在锚定层计算精确的Top-k索引，然后在中间重用层重用这些索引。锚定层通过动态规划算法选择，最大化开发集上的跨层相似性。方法支持头感知的Top-k选择和重用，并考虑了高效的实现约束（如瓦片级操作），适用于预填充和解码注意力。

Result: 在H100 GPU上，Kascade相比FlashAttention-3基线实现了最高4.1倍的解码注意力加速和2.2倍的预填充注意力加速。在LongBench和AIME-24等长上下文基准测试中，准确率与密集注意力非常接近。

Conclusion: Kascade是一种有效的训练免费稀疏注意力方法，能够显著提升长上下文LLM推理性能，同时保持高准确率，易于在不同模型间部署。
...
### [126] [NDRL: Cotton Irrigation and Nitrogen Application with Nested Dual-Agent Reinforcement Learning](https://arxiv.org/abs/2512.16408)
*Ruifeng Xu,Liang He*

Main category: cs.LG

TL;DR: 本文提出了一种嵌套双智能体强化学习（NDRL）方法，通过父智能体和子智能体的协同工作，优化棉花灌溉和氮肥管理，提高产量和资源利用效率。


<details>
  <summary>Details</summary>
Motivation: 现有研究面临两个主要限制：1）作物生长期间水氮组合优化复杂度高且产量优化效果差；2）轻度胁迫信号难以量化且反馈延迟，导致水氮动态调控不精确、资源利用效率低。这些问题影响了农业资源管理的效果和可持续性发展。

Method: 提出嵌套双智能体强化学习（NDRL）方法。父智能体基于预期累积产量效益识别有前景的宏观灌溉和施肥行动，减少无效探索同时保持目标与产量的一致性。子智能体的奖励函数包含量化的水分胁迫因子（WSF）和氮胁迫因子（NSF），并使用混合概率分布动态优化每日策略。使用2023和2024年田间试验数据校准和验证DSSAT系统来模拟真实条件并与NDRL交互。
...
### [127] [Emergent Bias and Fairness in Multi-Agent Decision Systems](https://arxiv.org/abs/2512.16433)
*Maeve Madigan,Parameswaran Kamalaruban,Glenn Moynihan,Tom Kempton,David Sutton,Stuart Burrell*

Main category: cs.LG

TL;DR: 该论文提出多智能体预测系统在金融领域存在公平性风险，需要开发专门的评估方法，研究发现多智能体系统可能表现出无法追溯到单个组件的集体偏见行为。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统在预测任务中表现出优越性能，但缺乏有效的公平性评估方法，使得在金融等高风险领域部署不安全，因为偏见决策可能导致监管违规和财务损失。

Method: 通过大规模模拟实验，研究不同多智能体配置（包括不同通信和协作机制）下的公平性指标，分析金融决策中出现的偏见模式。

Result: 研究发现多智能体系统在金融决策中出现了无法追溯到单个智能体组件的涌现性偏见，表明多智能体系统可能表现出真正的集体行为。

Conclusion: 金融多智能体系统的公平性风险是模型风险的重要组成部分，对信用评分和收入估计等任务有实际影响，必须将多智能体决策系统作为整体实体进行评估，而非对其组成组件进行还原论分析。
...
### [128] [Persistent Multiscale Density-based Clustering](https://arxiv.org/abs/2512.16558)
*Daniël Bot,Leland McInnes,Jan Aerts*

Main category: cs.LG

TL;DR: PLSCAN是一种新型密度聚类算法，通过识别HDBSCAN*在所有最小聚类大小下产生的稳定叶簇，无需手动选择超参数，在多个真实数据集上表现优于HDBSCAN*。


<details>
  <summary>Details</summary>
Motivation: 密度聚类算法在探索性数据分析中很重要，但实际应用时需要选择适当的超参数（如DBSCAN的密度阈值、HDBSCAN*的最小聚类大小），这在没有先验数据分布知识时很困难。

Method: 提出PLSCAN算法，基于尺度空间聚类原理，等价于新型度量空间上的持久同调，能高效识别HDBSCAN*在所有最小聚类大小下产生的稳定叶簇。

Result: 在多个真实数据集上，PLSCAN比HDBSCAN*获得更高的平均ARI（调整兰德指数），对互可达邻居数量的变化更不敏感。在低维数据集上计算成本与k-Means相当，高维时与HDBSCAN*相似。

Conclusion: PLSCAN是一种有效的密度聚类算法，能够自动识别稳定聚类而无需手动选择超参数，在性能和计算效率方面都表现出色。
...
### [129] [Stackelberg Learning from Human Feedback: Preference Optimization as a Sequential Game](https://arxiv.org/abs/2512.16626)
*Barna Pásztor,Thomas Kleine Buening,Andreas Krause*

Main category: cs.LG

TL;DR: SLHF是一种新的偏好优化框架，将对齐问题建模为领导者策略和跟随者策略之间的序贯博弈，相比RLHF和NLHF具有更好的推理时优化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的人类反馈学习方法如RLHF和NLHF存在局限性：RLHF为动作分配标量奖励，NLHF寻求同时移动均衡，都无法充分捕捉复杂的偏好结构。需要一种能够处理非传递性偏好并提供推理时优化的新框架。

Method: SLHF将偏好优化分解为两个问题：跟随者的精炼问题和领导者对抗对手的优化问题。领导者策略承诺一个动作，跟随者策略根据领导者的动作进行条件响应，形成序贯博弈结构。

Result: 实验表明SLHF在多样化偏好数据集上实现强对齐，参数规模从0.5B到8B均可扩展，产生的推理时精炼能力可在不同模型家族间迁移而无需进一步微调。

Conclusion: SLHF通过序贯博弈设计提供了比RLHF和NLHF更一致、对数据更敏感、对非传递性偏好更鲁棒的解决方案，为偏好优化提供了新的理论框架和实践方法。
...
### [130] [Exploiting Radio Frequency Fingerprints for Device Identification: Tackling Cross-receiver Challenges in the Source-data-free Scenario](https://arxiv.org/abs/2512.16648)
*Liu Yang,Qiang Li,Luxiong Wen,Jian Yang*

Main category: cs.LG

TL;DR: 本文提出MS-SHOT方法，解决射频指纹识别中源数据不可用的跨接收器适应问题，通过动量中心引导的软伪标签和全局结构约束提升性能。


<details>
  <summary>Details</summary>
Motivation: 边缘计算中射频指纹识别面临实际部署挑战：基于深度学习的模型在不同硬件特性的接收器间性能显著下降，由于接收器变化引入的分布偏移。现有方法在目标域存在标签偏移或非均匀类别分布时表现受限。

Method: 提出MS-SHOT方法：1) 建立基于约束伪标签的SCRFFI适应框架；2) 理论分析揭示目标域性能对伪标签质量敏感；3) 采用动量中心引导的软伪标签生成；4) 实施全局结构约束以鼓励自信且多样化的预测。

Result: 在真实世界数据集上的广泛实验表明，MS-SHOT在准确性和鲁棒性方面持续优于现有方法，有效处理目标域中的标签偏移和非均匀类别分布场景。

Conclusion: MS-SHOT为射频指纹识别中的源数据不可用跨接收器适应提供了实用且可扩展的解决方案，解决了先前方法在标签偏移场景下的显著限制。
...
### [131] [Blog Data Showdown: Machine Learning vs Neuro-Symbolic Models for Gender Classification](https://arxiv.org/abs/2512.16687)
*Natnael Tilahun Sinshaw,Mengmei He,Tadesse K. Bahiru,Sudhir Kumar Mohapatra*

Main category: cs.LG

TL;DR: 该研究比较了多种机器学习算法（SVM、NB、LR、AdaBoost、XGBoost、SVM_R）与神经符号AI（NeSy）在文本分类任务中的性能，并探索了不同的文本表示方法和特征提取技术。


<details>
  <summary>Details</summary>
Motivation: 文本分类（如博客性别分类）是一个成熟的研究领域，在市场营销、客户推荐等领域有广泛应用。研究旨在比较传统机器学习算法与新兴的神经符号AI方法在文本分类任务中的表现。

Method: 使用多种机器学习算法（SVM、NB、LR、AdaBoost、XGBoost、SVM_R）和神经符号AI（NeSy）进行对比分析。探索了TF-IDF、通用句子编码器（USE）、RoBERTa等文本表示方法，以及卡方检验、互信息、主成分分析等特征提取技术。

Result: 实验结果表明，尽管数据集有限，神经符号AI（NeSy）方法的表现与多层感知器（MLP）结果相当。
...
### [132] [CLARiTy: A Vision Transformer for Multi-Label Classification and Weakly-Supervised Localization of Chest X-ray Pathologies](https://arxiv.org/abs/2512.16700)
*John M. Statheros,Hairong Wang,Richard Klein*

Main category: cs.LG

TL;DR: CLARiTy是一种基于视觉Transformer的模型，用于胸部X光片的多标签病理分类和弱监督定位，通过类特定token和SegmentCAM模块实现，在NIH ChestX-ray14数据集上取得了竞争性的分类性能和最先进的弱监督定位性能。


<details>
  <summary>Details</summary>
Motivation: 胸部X光片解读面临多标签病理分类和空间定位的挑战，通常受限于区域级（密集）标注的稀缺性。需要开发能够在仅有图像级标签的情况下同时完成分类和定位的弱监督方法。

Method: CLARiTy采用基于视觉Transformer的架构，使用多个类特定token生成判别性注意力图，并引入SegmentCAM模块进行前景分割和背景抑制（利用解剖先验知识）。模型使用ConvNeXtV2教师模型进行蒸馏以提高效率，在NIH ChestX-ray14数据集上仅使用图像级标签进行训练。

Result: 在NIH官方划分的数据集上，CLARiTy-S-16-512在14种病理分类上取得竞争性性能，在8种病理的弱监督定位上达到最先进水平，比先前方法提升50.7%。特别是对于结节和肿块等小病理有显著提升。低分辨率变体CLARiTy-S-16-224在保持高效率的同时也明显超越基线方法。

Conclusion: CLARiTy通过结合ViT的全局上下文建模能力和类特定定位，以及卷积背景抑制技术，超越了CNN-ViT混合方法，能够生成精确、降噪的热力图，在低资源环境下具有应用潜力。
...
### [133] [Towards Reproducibility in Predictive Process Mining: SPICE - A Deep Learning Library](https://arxiv.org/abs/2512.16715)
...
<details>
  <summary>Details</summary>
Motivation: 现有的预测性流程挖掘方法缺乏可复现性、决策透明度、对新数据集的适应性以及基准测试能力，导致不同实现之间的比较非常困难。

Method: 提出SPICE框架，在PyTorch中重新实现了三种流行的基于深度学习的PPM基线方法，设计了一个具有严格可配置性的通用基础框架，支持可复现和稳健的比较。

Result: 在11个数据集上比较了SPICE与原始报告指标以及公平指标，验证了框架的有效性。

Conclusion: SPICE框架能够促进预测性流程挖掘方法的可复现研究、透明决策和公平比较，为未来建模方法提供了可靠的基准测试平台。
...
### [134] [Polyharmonic Spline Packages: Composition, Efficient Procedures for Computation and Differentiation](https://arxiv.org/abs/2512.16718)
*Yuriy N. Bakhvalov*

Main category: cs.LG

TL;DR: 提出级联架构解决多调和样条回归的计算复杂度和高维问题


<details>
  <summary>Details</summary>
Motivation: 之前基于随机函数理论的多调和样条回归方法存在O(N^3)计算复杂度和高维空间理论假设失效的问题

Method: 构建多调和样条包组成的级联架构，提出高效矩阵计算和前向传播的端到端微分方法

Result: 同时解决了可扩展性和未知内在低维问题的理论合理性

Conclusion: 级联架构为多调和样条回归提供了实用且理论合理的解决方案
...
### [135] [Machine Learning Algorithms: Detection Official Hajj and Umrah Travel Agency Based on Text and Metadata Analysis](https://arxiv.org/abs/2512.16742)
...
### [136] [NRGPT: An Energy-based Alternative for GPT](https://arxiv.org/abs/2512.16762)
*Nima Dehmamy,Benjamin Hoover,Bishwajit Saha,Leo Kozachkov,Jean-Jacques Slotine,Dmitry Krotov*

Main category: cs.LG

TL;DR: 该论文提出了一种将GPT架构与基于能量的建模（EBM）框架统一的方法，称为eNeRgy-GPT（NRGPT），通过将推理过程视为在能量景观上的探索，并在某些条件下等价于梯度下降。


<details>
  <summary>Details</summary>
Motivation: GPT架构是语言建模中最流行的设计，而基于能量的建模（EBM）是一种将推理视为在能量景观上动态过程的范式。作者希望通过最小修改将GPT与EBM框架统一起来，探索这种结合的优势。

Method: 提出eNeRgy-GPT（NRGPT）模型，将GPT的推理步骤概念化为在能量景观上对token的探索。在某些条件下，这种探索等价于梯度下降，尽管不一定能产生最佳性能模型。

Result: 模型在简单语言（莎士比亚数据集）、代数ListOPS任务以及更丰富的OpenWebText语言建模设置中表现良好。同时观察到模型可能对过拟合更具抵抗力，只在非常长的训练过程中出现过拟合。

Conclusion: 成功将GPT架构与EBM框架统一，提出的NRGPT模型在多个任务上表现良好，并显示出对过拟合的抵抗能力，为语言建模提供了新的视角和方法。
...
### [137] [Pattern recognition in complex systems via vector-field representations of spatio-temporal data](https://arxiv.org/abs/2512.16763)
...
TL;DR: 提出基于离散测度空间上向量场理论的几何框架，用于分析复杂系统的时空数据，通过两参数度量族支持机器学习应用，有效解决维度约简、相空间重构等挑战。
...
Method: 基于离散测度空间上向量场理论建立几何框架，提出两参数度量族，支持时间相关图像、图像梯度、图与单纯复形上的函数，结合多维标度进行数据分析。

Result: 在生物和物理系统的数值模拟数据上验证，该框架能有效实现维度约简、模态分解、相空间重构和吸引子表征，为复杂系统分析提供稳健路径。

Conclusion: 提出的几何框架为复杂系统分析提供了有效工具，特别适用于传统建模不可行但实验数据丰富的场景，为理解复杂动力学系统开辟了新途径。
...
### [138] [Meta-RL Induces Exploration in Language Agents](https://arxiv.org/abs/2512.16848)
*Yulun Jiang,Liangze Jiang,Damien Teney,Michael Moor,Maria Brbic*

Main category: cs.LG

TL;DR: LaMer是一个元强化学习框架，通过跨回合训练和基于反思的上下文策略适应，帮助LLM智能体在测试时主动探索环境并学习，显著提升了在需要探索的任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 传统RL训练的LLM智能体在需要主动探索的多回合长视野任务中表现不佳，难以从试错经验中高效适应。需要一种方法让智能体能够在测试时主动探索环境并从反馈中学习。

Method: LaMer包含两个关键组件：1) 跨回合训练框架，鼓励探索和长期奖励优化；2) 基于反思的上下文策略适应，使智能体能够从任务反馈信号中适应策略而无需梯度更新。

Result: 在多个环境中，LaMer相比RL基线有显著性能提升：Sokoban提升11%，MineSweeper提升14%，Webshop提升19%。同时展现出对更具挑战性或未见任务更好的泛化能力。
...
### [139] [Semi-Supervised Online Learning on the Edge by Transforming Knowledge from Teacher Models](https://arxiv.org/abs/2512.16866)
*Jiabin Xue*
...
<details>
  <summary>Details</summary>
Motivation: 在线边缘机器学习允许模型直接在边缘设备上训练并持续更新，但面临关键挑战：如何为真正未来的未见数据点确定标签。现有方法假设静态模型在中心训练后部署，对未见数据效果不佳。

Method: 提出知识转换(KT)方法，结合知识蒸馏、主动学习和因果推理。KT作为主动学习中的"预言机"，通过将教师模型的知识转换为学生模型的伪标签。该方法在两种设置下进行模拟实验：使用不太稳定的教师模型和使用相对更稳定的教师模型。

Result: 实验结果表明，当给定稳定的教师模型时，学生模型最终能够达到其预期的最大性能。KT在以下场景中具有潜在优势：1)教师任务具有通用性，现有预训练模型可能足够，无需从头训练教师模型；2)学生任务的标签难以获取或成本高昂。

Conclusion: 知识转换方法为解决在线边缘机器学习中未见数据的标签问题提供了有效方案，特别适用于教师模型稳定且学生任务标签获取困难的场景，为边缘设备上的持续学习提供了实用解决方案。
...
### [140] [Sequencing to Mitigate Catastrophic Forgetting in Continual Learning](https://arxiv.org/abs/2512.16871)
*Hesham G. Moussa,Aroosa Hameed,Arashmid Akhavain*

Main category: cs.LG

TL;DR: 本文提出通过智能任务排序来缓解持续学习中的灾难性遗忘问题，利用零样本评分算法确定最优任务序列，与传统方法结合可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 持续学习面临灾难性遗忘的主要挑战，传统方法主要分为五类，但本文从任务排序这一新角度出发，探索任务序列对缓解遗忘的作用。

Method: 提出基于零样本评分算法的任务排序方法，灵感来自神经架构搜索，用于确定最优任务顺序以减轻灾难性遗忘。

Result: 智能任务排序能显著减少灾难性遗忘，与传统持续学习策略结合时，能提供更好的性能和更强的抗遗忘鲁棒性。

Conclusion: 任务排序是缓解持续学习中灾难性遗忘的有效新方法，该方法还可应用于课程学习等其他领域。
...
### [141] [Posterior Behavioral Cloning: Pretraining BC Policies for Efficient RL Finetuning](https://arxiv.org/abs/2512.16911)
*Andrew Wagenmaker,Perry Dong,Raymond Tsao,Chelsea Finn,Sergey Levine*

Main category: cs.LG

TL;DR: 论文提出后验行为克隆（PostBC）方法，通过建模演示者的后验分布而非直接模仿动作，为强化学习微调提供更好的初始化策略，相比标准行为克隆显著提升微调性能。


<details>
  <summary>Details</summary>
Motivation: 当前实践中，通常先在大规模演示数据集上预训练策略，然后通过强化学习微调以提升性能。虽然微调算法受到广泛关注，但很少有人研究如何确保预训练策略能成为强化学习微调的有效初始化。本文旨在理解预训练策略如何影响微调性能，以及如何预训练策略以确保它们是有效的初始化。

Method: 提出后验行为克隆（PostBC）方法：不直接拟合观察到的演示动作，而是训练策略来建模给定演示数据集下演示者行为的后验分布。这种方法理论上确保对演示者动作的覆盖，同时保持与标准行为克隆相当的预训练性能。在实践中，PostBC可通过现代生成模型实现，仅需标准监督学习。

Result: 理论分析表明标准行为克隆可能无法确保对演示者动作的覆盖，而后验行为克隆能确保这一覆盖。在机器人控制领域实验中，PostBC相比标准行为克隆显著提升了强化学习微调性能，在现实的机器人控制基准测试和真实世界机器人操作任务中都取得了更好效果。

Conclusion: 后验行为克隆（PostBC）提供了一种有效的预训练策略初始化方法，通过建模演示者的后验分布而非直接模仿动作，确保了动作覆盖并显著提升了后续强化学习微调的性能，为从演示到强化学习的策略学习提供了改进方案。
...
### [142] [On Recommending Category: A Cascading Approach](https://arxiv.org/abs/2512.16033)
...
<details>
  <summary>Details</summary>
Motivation: 电商平台开始关注品类级推荐以拓展用户兴趣，但现有方法简单套用商品级模型，忽略了品类级推荐与商品级推荐的关键差异
...
### [143] [Introducing ORKG ASK: an AI-driven Scholarly Literature Search and Exploration System Taking a Neuro-Symbolic Approach](https://arxiv.org/abs/2512.16425)
*Allard Oelen,Mohamad Yaser Jaradeh,Sören Auer*

Main category: cs.IR

TL;DR: ASK是一个基于神经符号方法的AI驱动学术文献搜索系统，结合向量搜索、大语言模型和知识图谱，通过自然语言查询帮助研究人员查找相关文献。


<details>
  <summary>Details</summary>
Motivation: 随着学术文献数量的快速增长，查找相关文献变得越来越困难。生成式AI和大语言模型的出现为文献发现和探索提供了新的可能性。

Method: 采用神经符号方法，结合向量搜索、大语言模型和知识图谱，实现基于自然语言查询的文献检索。系统使用检索增强生成(RAG)方法自动提取关键信息并生成研究问题的答案。

Result: 评估显示ASK系统用户友好，用户在使用过程中普遍感到满意，系统在可用性和实用性方面表现良好。

Conclusion: ASK系统成功展示了AI驱动方法在学术文献搜索中的潜力，为研究人员提供了有效的文献发现和探索工具。
...
### [144] [InfoDCL: Informative Noise Enhanced Diffusion Based Contrastive Learning](https://arxiv.org/abs/2512.16576)
*Xufeng Liang,Zhida Qin,Chong Zhang,Tianyu Huang,Gangyi Ding*

Main category: cs.IR

TL;DR: InfoDCL是一个基于扩散的对比学习推荐框架，通过整合语义信息生成对比视图，采用协作训练策略，在推理阶段使用多层GCN，显著提升了推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比学习方法通过随机扰动原始交互图构建稀疏视图，无法捕捉真实用户偏好，且推荐数据稀疏性导致语义信息不足，需要更有效的对比学习框架。

Method: 提出InfoDCL框架：1) 使用单步扩散过程整合噪声和辅助语义信息生成信号，再通过标准扩散过程生成真实用户偏好作为对比视图；2) 基于生成与偏好学习的相互影响分析，构建协作训练目标策略；3) 推理阶段使用多层GCN融入高阶共现信息。

Result: 在五个真实世界数据集上的实验表明，InfoDCL显著优于现有最先进方法，为提升推荐性能提供了有效解决方案。

Conclusion: InfoDCL通过整合语义信息的扩散过程生成对比视图，结合协作训练策略，在保持训练效率的同时提升推荐性能，为在对比学习框架中应用扩散方法提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 该研究旨在降低量子光学实验设计的门槛，使非专业用户能够通过自然语言交互设计和模拟复杂的量子光学实验，促进研究和教学中的计算实验设计民主化。

Method: 系统采用多智能体架构，结合意图路由、知识增强生成和双模式验证（QuTiP和FreeSim）。通过语义检索从三层工具箱中组合光学组件，然后通过物理模拟进行验证和收敛细化。

Result: 在13个实验中（包括基础光学、量子信息协议和先进技术），系统获得8-9/10的设计-模拟对齐分数，模拟结果能忠实建模预期物理现象。自由形式模拟在11/13实验中优于约束框架。

Conclusion: 系统成功实现了量子光学实验设计的民主化，能够生成用户可通过对话迭代优化的初始设计。研究发现结构正确性与定量准确性需要区分，量子光学的多样性需要灵活的数学表示。
...
TL;DR: 本文提出比例责任原则（PPD），这是一个新颖的伦理框架，将道德责任建模为随智能体认知状态变化的比例关系。该框架表明道德责任不会因不确定性而消失，而是转化为不同形式：随着不确定性增加，行动责任按比例转化为修复责任。


<details>
  <summary>Details</summary>
Motivation: 传统伦理框架在处理不确定性下的决策时存在局限，通常将其视为对行动的简单约束。本文旨在开发一个能够建模道德责任如何随智能体认知状态变化的框架，以更好地应对现实世界中的不确定性决策问题。

Method: 提出比例责任原则（PPD）框架，通过数学方程 D_total = K[(1-HI) + HI * g(C_signal)] 形式化道德责任，其中总责任是知识（K）、谦逊/不确定性（HI）和情境信号强度（C_signal）的函数。使用蒙特卡洛模拟验证框架，并在临床伦理、受赠人权利法、经济治理和人工智能四个领域进行跨学科应用验证。

Result: 模拟结果显示，保持基线谦逊系数（λ > 0）的系统能产生更稳定的责任分配，并降低过度自信决策的风险。比例责任原则作为复杂系统中的稳定原则，能够防止过度干预和疏忽，通过动态平衡认知信心与情境风险来优化决策。

Conclusion: 通过将谦逊形式化为系统参数，PPD提供了一个数学上可处理的道德责任方法，可用于开发可审计的AI决策系统。该框架在四个不同领域都显示出有效性，表明比例责任原则具有跨学科适用性，能够为复杂系统中的伦理决策提供稳定指导。
...
TL;DR: 提出一个从自然语言描述生成物理可实现的装配指令的框架，使用LDraw作为中间表示，通过约束的零件词汇确保几何有效性、连接约束和可建造顺序，支持3000多个零件的砖块原型装配。


<details>
  <summary>Details</summary>
Motivation: 现有文本到3D方法缺乏物理可实现性约束，CAD模型无法支持复杂装配指令，需要一种能连接语义设计意图与可制造输出的方法。

Method: 使用LDraw作为文本丰富的中间表示，引导大语言模型生成有效的逐步建造序列和装配指令，引入Python库进行程序化模型生成，采用"砖块袋"方法作为物理API。

Result: 在复杂卫星、飞机和建筑领域评估可建造输出，支持超过3000个装配零件，展示了可扩展性、模块化和保真度，通过四个原创设计验证了方法的有效性。

Conclusion: 该方法填补了像素扩散方法和CAD模型之间的空白，提供了一种连接自然语言规范与物理原型的"元素通用语言"，为制造和工程原型设计开辟了新的设计选项。
...
TL;DR: 研究揭示了大语言模型在具身环境中的符号接地问题，特别是信息不对称分布时的"特权信息偏见"，通过非对称辅助推理框架量化了这种沟通障碍


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为强大的推理引擎，在具身环境中面临符号接地问题，尤其是在信息不对称分布时。研究者关注"特权信息偏见"（或称"知识诅咒"），即知识丰富的"领导者"智能体由于缺乏心理理论而无法有效指导传感器受限的"跟随者"

Method: 提出了一个新颖的非对称辅助推理框架，在AI2-THOR环境中进行实验。比较了两种通信协议："推送式"指令（标准方法）和"拉取式"协议（主动查询）。通过量化"成功差距"来评估沟通接地错误的影响

Result: 实验显示显著的"成功差距"：领导者能成功感知目标的概率为35.0%，而协作团队成功率仅为17.0%，这意味着近50%的可行计划仅因沟通接地错误而失败。"拉取式"协议（主动查询）比标准"推送式"指令显著更鲁棒，成功场景中澄清请求的频率是2倍

Conclusion: 研究分离了主动不确定性减少机制作为安全人机协作和机器人间协作的前提条件。强调了在信息不对称环境中，主动查询和澄清对于克服特权信息偏见的重要性，为改善具身AI系统的协作能力提供了重要见解
...
TL;DR: AI流行病学是一个通过应用群体层面监测方法来治理和解释先进AI系统的框架，它通过标准化捕获AI-专家交互来预测输出失败，无需理解模型内部机制。


<details>
  <summary>Details</summary>
Motivation: 当前的可解释性方法（如SHAP和机制可解释性）在面对大规模部署模型时存在模型复杂性问题，需要一种能够绕过这一限制的AI治理框架。

Method: 通过标准化捕获AI-专家交互为结构化评估字段（风险等级、对齐分数、准确度分数），将这些作为暴露变量来预测输出失败，类似于流行病学中胆固醇和血压预测心脏事件的方法。

Result: 该框架为零负担专家提供自动审计跟踪，通过被动跟踪专家与AI建议的趋同和分歧来分析输出而非内部计算，确保治理连续性，并提供可靠性分数和语义评估来检测不可靠的AI输出。

Conclusion: AI流行病学通过使领域专家无需机器学习专业知识即可治理AI系统，实现了AI监督的民主化，能够在AI输出造成危害前检测到不可靠输出。
...
### [150] [State-Augmented Graphs for Circular Economy Triage](https://arxiv.org/abs/2512.15824)
*Richard Fox,Rui Li,Gustav Jonsson,Farzaneh Goli,Miying Yang,Emel Aktas,Yongjing Wang*

Main category: cs.AI

TL;DR: 本文提出了一种基于状态增强的拆卸序列规划图的决策框架，用于优化循环经济产品分拣决策，通过将拆卸历史编码到状态中实现马尔可夫性，并以电动汽车电池为例展示了该框架的灵活性。


<details>
  <summary>Details</summary>
Motivation: 循环经济产品分拣需要评估产品在达到当前使用终点后的可持续处理路径，这需要平衡保留价值与处理成本、劳动力约束的自适应决策。现有方法缺乏统一的框架来处理复杂的操作约束和条件感知效用。

Method: 提出了一种基于状态增强的拆卸序列规划图的确定性求解器决策框架。通过将拆卸历史编码到状态中强制实施马尔可夫性质，使每个决策仅依赖于前一个状态，实现最优递归评估。模型集成了基于诊断健康分数的条件感知效用和复杂的操作约束。

Result: 以电动汽车电池的分层分拣为例，展示了该框架的灵活性。该示例说明了如何通过统一的数学形式来适应不同的机械复杂性、安全要求和经济驱动因素，实现组件的递归估值驱动的决策。

Conclusion: 该统一的数学形式为优化不同产品和操作环境下的循环经济分拣决策提供了一个可处理且可推广的基础，能够平衡保留价值与处理成本约束，实现自适应决策。
...
### [151] [PediatricAnxietyBench: Evaluating Large Language Model Safety Under Parental Anxiety and Pressure in Pediatric Consultations](https://arxiv.org/abs/2512.15894)
*Vahideh Zolfaghari*

Main category: cs.AI

TL;DR: 研究评估了大型语言模型在儿科咨询中的安全性，发现对抗性查询（如家长焦虑表达）会降低模型安全性，模型规模越大安全性越高，但所有模型都存在临床漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着家长越来越多地使用大型语言模型获取儿科指导，但模型在真实世界对抗性压力下的安全性了解不足。焦虑家长常使用紧急语言，可能绕过模型安全防护，导致有害建议。

Method: 创建了PediatricAnxietyBench开源基准测试，包含300个高质量查询（150个患者来源，150个对抗性），涵盖10个儿科主题。评估了两个Llama模型（70B和8B），使用多维安全框架评估诊断克制、转诊依从性、模糊表达和紧急情况识别能力。

Result: 平均安全得分5.50/15（SD=2.41）。70B模型优于8B模型（6.26 vs 4.95，p<0.001），关键失败率更低（4.8% vs 12.0%，p=0.02）。对抗性查询使安全性降低8%（p=0.03），紧急语言导致最大降幅（-1.40）。在癫痫发作（33.3%不当诊断）和疫苗接种后查询中存在漏洞。模糊表达与安全性强相关（r=0.68，p<0.001），而紧急情况识别能力缺失。
...
### [152] [Darth Vecdor: An Open-Source System for Generating Knowledge Graphs Through Large Language Model Queries](https://arxiv.org/abs/2512.15906)
*Jonathan A. Handler*

Main category: cs.AI

TL;DR: Darth Vecdor (DV) 是一个从大型语言模型中提取结构化知识到SQL数据库的工具，旨在解决直接查询LLM时的成本、速度、安全性和置信度问题，特别针对医疗领域应用。


<details>
  <summary>Details</summary>
Motivation: 虽然可以直接查询LLM获取知识，但在高容量操作中，成本、速度、安全性和置信度等问题可能成为障碍。通过将信息从LLM中预先提取到标准数据库中，可以缓解这些问题，特别是在医疗等关键领域。

Method: DV通过构建结构化、术语映射的SQL数据库（知识图谱）来提取LLM中的知识。它包含专门功能来缓解LLM响应中的错误、离题、自由文本、过于笼统和不一致等问题，并支持多元素响应。为方便使用，DV提供了基于浏览器的图形用户界面，允许领域专家进行提示工程。

Result: DV已作为免费、开源、可扩展的软件发布，采用"按现状"基础，不提供任何保证或条件。用户需要意识到使用DV及其输出的潜在风险和益处，并负责确保任何使用都是安全和有效的。

Conclusion: 虽然DV可能存在严重缺陷，但作者希望适当使用当前和未来版本的DV及其输出能够帮助改善医疗保健。该工具旨在通过提供结构化、可查询的知识库来弥补直接使用LLM的局限性。
...
### [153] [Leveraging Spreading Activation for Improved Document Retrieval in Knowledge-Graph-Based RAG Systems](https://arxiv.org/abs/2512.15922)
*Jovan Pavlović,Miklós Krész,László Hajdu*

Main category: cs.AI

TL;DR: 提出一种基于扩散激活算法的新型RAG框架，通过自动构建的知识图谱连接文档，提升大语言模型在复杂推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统将所有检索信息视为同等可靠，忽视了文本语料库中可信度和关联性的差异。GraphRAG方法虽然通过知识图谱改进RAG，但依赖高质量图谱构建且存在与标准RAG类似的挑战。

Method: 提出新型RAG框架，采用扩散激活算法从自动构建的知识图谱连接的文档语料库中检索信息，增强大语言模型在复杂任务（如多跳问答）上的表现。

Result: 实验显示该方法达到或优于迭代RAG方法，可作为即插即用模块与多种RAG方法集成。与思维链迭代检索结合时，相比朴素RAG在答案正确性上获得最高39%的绝对提升，且使用小型开源模型在资源受限环境下有效。

Conclusion: 提出的扩散激活算法RAG框架能有效解决复杂推理任务中的多步证据检索问题，在资源受限环境下表现出色，具有广泛的兼容性和实用性。
...
### [154] [Small Language Models for Efficient Agentic Tool Calling: Outperforming Large Models with Targeted Fine-tuning](https://arxiv.org/abs/2512.15943)
*Polaris Jhandi,Owais Kazi,Shreyas Subramanian,Neel Sendas*

Main category: cs.AI

TL;DR: 该研究探索用小语言模型(SLMs)替代大语言模型(LLMs)的可行性，通过微调OPT-350M模型在特定任务上取得了优于ChatGPT等基线模型的性能，显著降低了生成式AI的部署成本。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的规模化采用，模型成本优化和运营效率成为决定可持续性和可访问性的关键因素。大语言模型虽然功能强大，但计算需求巨大，成本过高，限制了在企业常规应用中的普及。这促使研究者探索小语言模型，在特定应用中提供可比性能的同时大幅降低基础设施开销。

Method: 研究训练了一个领域适应的小语言模型来执行传统由大语言模型处理的任务，如文档摘要、查询回答和结构化数据解释。具体使用Hugging Face TRL的监督微调(SFT)训练器对facebook/opt-350m模型进行单轮微调。OPT-350M是Meta AI在2022年发布的OPT系列模型之一。
...
Conclusion: 精心设计和针对性训练的小语言模型可以显著降低生成式AI的采用门槛，实现成本效益高的大规模生产系统集成。这项研究证明了在特定任务中用优化的小语言模型替代大语言模型的可行性。
...
### [155] [Subjective functions](https://arxiv.org/abs/2512.15948)
*Samuel J. Gershman*

Main category: cs.AI

TL;DR: 论文探讨了智能体如何内生地合成目标函数，提出了主观函数的概念，并以预期预测误差为例进行了研究。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于人类智能能够动态合成新目标函数的能力，探索这种能力的本质以及如何赋予人工系统同样的能力。

Method: 提出了主观函数的概念——一种内生于智能体的高阶目标函数，并以预期预测误差作为具体实例进行研究。

Result: 建立了主观函数的概念框架，展示了预期预测误差作为主观函数的可行性，并与心理学、神经科学和机器学习中的相关思想建立了联系。

Conclusion: 主观函数为理解智能体如何内生地形成目标提供了新视角，有助于开发能够自主合成目标的人工智能系统。
...
### [156] [Do Large Language Models Know What They Don't Know? Kalshibench: A New Benchmark for Evaluating Epistemic Calibration via Prediction Markets](https://arxiv.org/abs/2512.16030)
*Lukas Nel*

Main category: cs.AI

TL;DR: 论文介绍了KalshiBench基准测试，用于评估大语言模型对未知未来事件的校准能力，发现所有前沿模型都存在系统性过度自信问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在各种任务上表现出色，但其认知校准能力尚未被充分理解。现有基准主要评估静态知识准确性，而缺乏对模型量化未知未来事件不确定性的评估。

Method: 创建KalshiBench基准，包含300个来自受监管交易所Kalshi的预测市场问题，这些问题的真实结果发生在模型训练截止日期之后。评估了Claude Opus 4.5、GPT-5.2、DeepSeek-V3.2、Qwen3-235B和Kimi-K2五个前沿模型的校准性能。

Result: 所有模型都表现出系统性过度自信。最佳校准模型Claude Opus 4.5的ECE为0.120，而推理增强模型如GPT-5.2-XHigh校准更差（ECE=0.395）。只有一个模型获得正Brier技能分数，表明大多数模型表现不如简单预测基准率。

Conclusion: 模型规模和增强推理能力并不能自动带来校准优势，认知校准是一个需要针对性开发的独立能力。这突显了在大语言模型中开发适当不确定性量化机制的重要性。
...
### [157] [Topic Discovery and Classification for Responsible Generative AI Adaptation in Higher Education](https://arxiv.org/abs/2512.16036)
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在教育中的广泛应用，学生使用这些工具进行学习，但也带来了错误信息、幻觉输出以及可能削弱批判性思维等问题。各教育机构制定了不同的AI政策，但这些政策差异大且不断变化，导致学生不清楚期望和最佳实践。

Method: 设计并实现了一个自动化系统，结合无监督主题建模技术识别关键政策主题，并使用大型语言模型（如GPT-4.0）对政策文本中的生成式AI允许程度和其他要求进行分类。

Result: 开发的应用在主题发现方面获得了0.73的连贯性分数。基于GPT-4.0的政策分类在八个已识别主题上实现了0.92-0.97的精确度和0.85-0.97的召回率。

Conclusion: 该系统通过提供结构化和可解释的政策信息，促进了生成式AI技术在教育中的安全、公平和教学一致的使用。该系统可以集成到教育技术平台中，帮助学生理解和遵守相关指南。
...
### [158] [WeMusic-Agent: Efficient Conversational Music Recommendation via Knowledge Internalization and Agentic Boundary Learning](https://arxiv.org/abs/2512.16108)
*Wendong Bi,Yirong Mao,Xianglong Liu,Kai Tian,Jian Zhang,Hanjie Wang,Wenhui Que*

Main category: cs.AI

TL;DR: 提出了WeMusic-Agent框架，通过知识内化和智能工具调用平衡，实现基于LLM的对话式音乐推荐，并构建了首个开源基准测试。
...
Result: 在真实世界数据上的实验表明，WeMusic-Agent相比现有模型有显著改进。同时构建了首个基于微信听书真实数据的开源对话式音乐推荐基准测试。

Conclusion: WeMusic-Agent框架通过平衡知识内化和工具调用，有效提升了对话式音乐推荐的性能，为解决该领域的关键挑战提供了新思路。
...
### [159] [ToolForge: A Data Synthesis Pipeline for Multi-Hop Search without Real-World APIs](https://arxiv.org/abs/2512.16149)
...
Result: 仅用8B参数的模型在合成数据上训练后，在多个基准测试中超越GPT-4o，证明了方法的有效性。
...
### [160] [Scaling Spatial Reasoning in MLLMs through Programmatic Data Synthesis](https://arxiv.org/abs/2512.16237)
...
TL;DR: SPRITE框架通过模拟器和大型模型程序化合成可扩展、多样且高质量的空间推理数据，解决了VLM在空间理解上的困境，显著提升了空间智能性能。


<details>
  <summary>Details</summary>
Motivation: 当前体现智能面临空间理解和推理能力有限的问题。现有方法存在两难：模板数据集可扩展但结构僵化，人工标注语言多样但不可扩展且计算不精确。

Method: SPRITE将真实标注重构为代码生成任务，利用LLM将复杂空间问题编译为可执行程序，并通过模拟器提取的高精度场景元信息进行验证，确保计算精确性和可验证性。

Result: 构建了包含3个模拟器、11k+场景、300k+图像/视频指令调优对的数据集。基于此数据训练的VLM在多个空间基准测试中取得显著性能提升，优于同等规模的开源数据集。
...
### [161] [AlignMerge - Alignment-Preserving Large Language Model Merging via Fisher-Guided Geometric Constraints](https://arxiv.org/abs/2512.16245)
*Aniruddha Roy,Jyoti Patel,Aman Chadha,Vinija Jain,Amitava Das*

Main category: cs.AI

TL;DR: AlignMerge：一种几何感知的大语言模型融合框架，将对齐性作为显式不变约束，在保持模型能力的同时确保安全对齐


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型融合方法（如线性权重融合、任务向量、Fisher加权平均）虽然能保持损失函数值，但会悄悄破坏模型的对齐性。作者认为融合不应是数值技巧，而是围绕已对齐锚点的几何约束操作

Method: 在指令调优基模型的局部Fisher图中，估计对齐子空间并引入投影器P_A，优化包含几何保持损失、对齐惩罚损失和软对齐预算损失的目标函数。使用解码不变的对齐质量指数（AQI）作为对齐功能度量
...
Conclusion: AlignMerge将保持对齐性的融合提升为一流设计目标，为未来基础模型的几何感知组合提供了路径，表明融合必须尊重安全几何结构而非事后验证
...
### [162] [QuadSentinel: Sequent Safety for Machine-Checkable Control in Multi-agent Systems](https://arxiv.org/abs/2512.16279)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在执行复杂任务时存在安全风险，而部署者用自然语言编写的策略存在模糊性和上下文依赖性，难以映射到机器可检查的规则，导致运行时执行不可靠。

Method: 将安全策略表达为序列，构建四智能体防护系统：状态跟踪器、策略验证器、威胁监视器和裁判。系统将策略编译为基于可观察状态谓词的机器可检查规则，通过裁判逻辑和高效的top-k谓词更新器优先检查并分层解决冲突，降低计算成本。

Result: 在ST-WebAgentBench和AgentHarm基准测试中，QuadSentinel提高了防护准确性和规则召回率，同时减少了误报。相比ShieldAgent等单智能体基线，提供了更好的整体安全控制。

Conclusion: 近期部署可以采用这种模式，无需修改核心智能体，保持策略分离且机器可检查。该模式为大型语言模型智能体提供了有效的安全防护解决方案。
...
### [163] [Code-in-the-Loop Forensics: Agentic Tool Use for Image Forgery Detection](https://arxiv.org/abs/2512.16300)
...
Method: 提出ForenAgent多轮交互框架，采用两阶段训练流程（冷启动和强化微调），设计动态推理循环（全局感知、局部聚焦、迭代探测和整体裁决），并构建FABench数据集用于系统训练评估。

Result: 实验表明ForenAgent在低级工具辅助下，在挑战性IFD任务上展现出新兴工具使用能力和反思推理能力，为通用IFD开辟了有前景的路径。

Conclusion: ForenAgent通过让MLLMs自主操作低级工具，实现了更灵活可解释的伪造分析，成功统一了低级伪影和高级语义知识，为图像伪造检测提供了新方向。
...
### [164] [Adaptation of Agentic AI](https://arxiv.org/abs/2512.16301)
*Pengcheng Jiang,Jiacheng Lin,Zhiyi Shi,Zifeng Wang,Luxi He,Yichen Wu,Ming Zhong,Peiyang Song,Qizheng Zhang,Heng Wang,Xueqiang Xu,Hanwen Xu,Pengrui Han,Dylan Zhang,Jiashuo Sun,Chaoqi Yang,Kun Qian,Tian Wang,Changran Hu,Manling Li,Quanzheng Li,Hao Peng,Sheng Wang,Jingbo Shang,Chao Zhang,Jiaxuan You,Liyuan Liu,Pan Lu,Yu Zhang,Heng Ji,Yejin Choi,Dawn Song,Jimeng Sun,Jiawei Han*

Main category: cs.AI

TL;DR: 该论文提出了一个系统化框架，统一了智能体AI系统中的适应策略研究，将适应分为智能体适应和工具适应两大类，并进一步细分为不同形式，为构建更强大、高效、可靠的智能体AI系统提供概念基础和实践指导。


<details>
  <summary>Details</summary>
Motivation: 随着基于基础模型的智能体AI系统能力不断增强、应用范围不断扩大，适应机制成为提升性能、可靠性和泛化能力的关键手段。然而，相关研究领域快速扩展但缺乏系统化框架，需要统一概念体系来指导研究和实践。

Method: 论文提出了一个系统化框架，将适应策略分为智能体适应和工具适应两大类。智能体适应进一步分为工具执行信号驱动和智能体输出信号驱动两种形式；工具适应分为智能体无关和智能体监督两种形式。通过这一框架分析设计空间、明确权衡取舍，并提供策略选择和切换的实践指导。

Result: 该框架有助于澄清智能体AI中适应策略的设计空间，使各种策略的权衡取舍变得明确，为系统设计中的策略选择或切换提供实用指导。论文还回顾了各类别的代表性方法，分析了其优势和局限性。

Conclusion: 论文为研究人员和从业者构建更强大、高效、可靠的智能体AI系统提供了概念基础和实践路线图，并强调了关键开放挑战和未来机会。
...
### [165] [Design and Evaluation of Cost-Aware PoQ for Decentralized LLM Inference](https://arxiv.org/abs/2512.16317)
*Arther Tian,Alex Ding,Frank Chen,Alan Wu,Aaron Chan,Bruce Zhang*

Main category: cs.AI

TL;DR: 论文提出了一种成本感知的Proof of Quality框架，用于去中心化LLM推理，通过将显式效率测量纳入奖励机制，平衡输出质量和计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有的去中心化LLM推理验证方法难以扩展到现代模型，而原始的Proof of Quality方法忽略了推理节点和评估节点之间的异构计算成本差异。

Method: 引入成本感知PoQ框架，将显式效率测量集成到两类节点的奖励机制中。设计结合了真实标记级F1、轻量级学习评估器和基于GPT的判断，采用线性奖励函数平衡归一化质量和成本。

Result: 实验表明：1）语义文本相似性双编码器比交叉编码器与真实标签和GPT评分相关性更高；2）模型池中最大的模型在单位延迟质量方面也最有效；3）蒙特卡洛模拟显示成本感知奖励方案能持续奖励高质量低成本的推理模型和高效评估器。

Conclusion: 成本感知PoQ为经济可持续的去中心化LLM推理提供了实用基础，通过平衡质量和成本实现更公平的节点奖励分配。
...
### [166] [PCIA: A Path Construction Imitation Algorithm for Global Optimization](https://arxiv.org/abs/2512.16392)
*Mohammad-Javad Rezaei,Mozafar Bag-Mohammadi*

Main category: cs.AI

TL;DR: 提出了一种新的元启发式优化算法PCIA，灵感来源于人类构建和使用路径的方式，通过随机种群搜索最佳路径，在数学和约束优化问题上表现出色。


<details>
  <summary>Details</summary>
Motivation: 受人类构建和使用路径行为的启发，人类通常偏好热门交通路线，在路径封闭时会智能地混合现有路径构建新路线，并随机选择不同路径探索未知目的地。这些行为模式为开发新的优化算法提供了灵感。

Method: PCIA算法模仿人类路径构建行为：1）生成随机种群寻找最佳路径（类似群体算法）；2）每个粒子代表一条通往目标的路径；3）算法结合了路径混合和随机探索机制；4）在53个数学优化问题和13个约束优化问题上进行测试。

Result: PCIA在53个数学优化问题和13个约束优化问题上的测试结果显示，该算法与流行及最新的元启发式算法相比具有高度竞争力。

Conclusion: PCIA是一种基于人类路径构建行为的新型元启发式优化算法，在多种优化问题上表现出优异的性能，为优化算法领域提供了新的思路。
...
### [167] [Synthelite: Chemist-aligned and feasibility-aware synthesis planning with LLMs](https://arxiv.org/abs/2512.16424)
...
<details>
  <summary>Details</summary>
Motivation: 现有计算机辅助合成规划框架缺乏与人类专家交互的机制，限制了化学家洞察力的整合。需要开发能够利用大语言模型内在化学知识和推理能力，同时允许专家通过自然语言提示进行干预的合成规划系统。

Method: 开发Synthelite框架，利用大语言模型直接提出逆合成转化方案。系统能够通过自然语言提示整合专家干预，生成端到端合成路线，并适应各种用户指定的约束条件。

Result: Synthelite在策略约束和起始原料约束的合成任务中达到高达95%的成功率。系统能够灵活调整规划轨迹以适应多样化约束，并在路线设计中考虑化学可行性。

Conclusion: Synthelite不仅是一个实用的合成规划工具，也是迈向以大语言模型为核心协调器的合成规划范式的重要一步，展示了LLMs在化学合成规划中的潜力。
...
### [168] [TIB AIssistant: a Platform for AI-Supported Research Across Research Life Cycles](https://arxiv.org/abs/2512.16442)
...
Conclusion: TIB AIssistant为AI支持的研究提供了一个可行的平台框架，通过模块化助手和标准化数据输出，为未来构建社区维护的AI研究支持生态系统奠定了基础。
...
### [169] [TimeSeries2Report prompting enables adaptive large language model management of lithium-ion batteries](https://arxiv.org/abs/2512.16453)
*Jiayang Yang,Chunhui Zhao,Martin Guay,Zhixing Cao*

Main category: cs.AI

TL;DR: TS2R是一个提示框架，将锂电池运行时间序列转换为结构化报告，使LLM能够在BESS管理中执行推理、预测和决策任务，无需重新训练或架构修改。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在解释多元时间序列数据方面具有潜力，但在实际电池储能系统运维中的应用尚未充分探索。需要一种方法将原始传感器数据转换为LLM可理解的形式，以实现智能电池管理。

Method: TS2R通过分割、语义抽象和基于规则的解释，将短期时间动态编码为自然语言，将低层传感器信号与高层上下文洞察有效连接起来。

Result: 与基于视觉、嵌入和文本的提示基线相比，TS2R的报告式提示在异常检测、荷电状态预测和充放电管理任务中，在准确性、鲁棒性和可解释性方面持续提升LLM性能。
...
### [170] [cuPilot: A Strategy-Coordinated Multi-agent Framework for CUDA Kernel Evolution](https://arxiv.org/abs/2512.16465)
...
<details>
  <summary>Details</summary>
Motivation: CUDA内核优化需要硬件-软件协同设计专业知识且高性能内核库通常为专有，现有基于大语言模型和进化算法的方法因智能体设计不佳和演化表示不匹配而性能不足。
...
### [171] [Quantifying and Bridging the Fidelity Gap: A Decisive-Feature Approach to Comparing Synthetic and Real Imagery](https://arxiv.org/abs/2512.16468)
*Danial Safaei,Siddartha Khastgir,Mohsen Alirezaei,Jeroen Ploeg,Son Tong,Xingyu Zhao*

Main category: cs.AI

TL;DR: 论文提出了一种新的系统特定保真度度量方法——决定性特征保真度（DFF），用于评估自动驾驶虚拟测试中合成数据的有效性，关注系统决策所依赖的因果证据一致性而非像素级真实感。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶安全保证中的虚拟测试主要依赖合成数据，但研究表明像素级保真度并不能确保从仿真到真实世界的可靠迁移。真正重要的是被测系统在真实和仿真环境中是否基于相同的因果证据做出决策，而不仅仅是图像对人类"看起来真实"。目前缺乏这种基于行为的保真度度量方法。

Method: 提出了决定性特征保真度（DFF），这是一种系统特定的度量方法，通过可解释AI（XAI）方法识别和比较驱动被测系统输出的决定性特征。基于反事实解释提出了实际估计器，并设计了DFF引导的校准方案来增强仿真器保真度。

Result: 在2126个匹配的KITTI-VirtualKITTI2数据对上的实验表明，DFF能够揭示传统输出值保真度忽略的差异。DFF引导的校准在保持输出值保真度的同时，提高了决定性特征和输入级保真度，适用于多种被测系统。

Conclusion: DFF为自动驾驶虚拟测试提供了一种新的行为基础保真度度量方法，能够更有效地评估合成数据的有效性，并通过校准机制提升仿真器保真度，从而增强自动驾驶系统的安全保证。
...
### [172] [Best Practices For Empirical Meta-Algorithmic Research Guidelines from the COSEAL Research Network](https://arxiv.org/abs/2512.16491)
...
<details>
  <summary>Details</summary>
Motivation: 元算法研究（如算法选择、配置和调度）依赖大量计算密集型实验，实验设计和设置的自由度大，存在多种可能影响研究可扩展性和有效性的误差来源。现有最佳实践分散在不同出版物和领域，且各自独立发展。

Method: 收集COSEAL社区各子领域的元算法研究最佳实践，涵盖完整的实验周期：从研究问题提出和实验设计选择，到实验执行，再到结果分析和公正呈现。

Result: 建立了元算法研究领域当前最先进的实践标准，为元算法领域的新研究者和实践者提供了系统性的指导框架。

Conclusion: 该报告整合了元算法研究的最佳实践，为提升该领域研究的科学性、可扩展性和有效性提供了全面的指导方针。
...
### [173] [ParamExplorer: A framework for exploring parameters in generative art](https://arxiv.org/abs/2512.16529)
*Julien Gachadoat,Guillaume Lagarde*

Main category: cs.AI

TL;DR: ParamExplorer：一个基于强化学习思想的交互式框架，用于探索生成艺术算法中的参数空间，通过人机协作或自动化反馈指导探索过程
...
Method: 提出了ParamExplorer框架，这是一个受强化学习启发的交互式模块化框架，帮助探索生成艺术算法的参数空间，通过人在回路或自动化反馈进行指导。该框架还能与现有p5.js项目无缝集成。在框架内实现并评估了多种探索策略（称为智能体）

Result: 论文介绍了ParamExplorer框架及其实现，但没有提供具体的实验结果数据。框架本身作为主要贡献，提供了探索生成艺术参数空间的新方法

Conclusion: ParamExplorer为艺术家提供了更有效的参数空间探索工具，减少了对繁琐手动试错的依赖，有助于发现更多有美学价值的艺术配置
...
### [174] [Scaling Laws for Energy Efficiency of Local LLMs](https://arxiv.org/abs/2512.16531)
*Ander Alvarez,Alessandro Genuardi,Nilotpal Sinha,Antonio Tiene,Samuel Mugel,Román Orús*

Main category: cs.AI

TL;DR: 本文系统性地研究了在CPU设备上部署本地大语言模型和视觉语言模型的计算规律，发现了文本长度与计算成本近似线性相关、视觉语言模型存在分辨率阈值的经验规律，并展示了量子压缩技术能显著降低计算资源消耗。


<details>
  <summary>Details</summary>
Motivation: 尽管GPU主导AI部署，但大多数消费硬件（笔记本电脑、台式机、工业控制器、嵌入式系统）依赖CPU。然而，CPU-only推理本地语言和视觉语言工作负载的计算规律尚未得到充分探索，需要系统性的基准测试来理解计算负载如何随输入规模变化。

Method: 在两种代表性的CPU层级上进行系统基准测试：MacBook Pro M2（主流笔记本级）和Raspberry Pi 5（低功耗嵌入式）。采用基于处理器和内存使用连续采样以及曲线下面积积分的统一方法，分析计算负载如何随语言模型的输入文本长度和视觉语言模型的图像分辨率变化。

Result: 发现了两个经验缩放规律：1）语言模型推理的计算成本与token长度近似线性相关；2）视觉语言模型存在预处理驱动的"分辨率阈值"，在内部分辨率限制以上计算保持恒定，以下则急剧下降。量子压缩技术能将处理器和内存使用降低高达71.9%，能耗降低高达62%，同时保持或提高语义准确性。

Conclusion: 本研究系统量化了多模态CPU-only推理的缩放规律，识别出模型压缩和输入分辨率预处理作为可持续边缘推理的有效、低成本杠杆，为在资源受限的边缘设备上部署AI模型提供了实用指导。
...
### [175] [Prefix Probing: Lightweight Harmful Content Detection for Large Language Models](https://arxiv.org/abs/2512.16650)
...
Method: 1) 黑盒检测方法：比较"同意/执行"与"拒绝/安全"开头前缀的条件对数概率；2) 前缀缓存技术：减少检测开销至接近首词延迟；3) 高效前缀构建算法：自动发现高信息量前缀；4) 单次推理：仅需一次对数概率计算即可生成危害性评分。

Result: 实验表明Prefix Probing的检测效果与主流外部安全模型相当，同时仅产生最小计算成本，无需额外模型部署，具有强实用性和效率。

Conclusion: Prefix Probing通过创新的前缀概率比较方法和高效前缀构建算法，有效解决了大型语言模型在有害内容检测中的三难权衡问题，提供了一种实用且高效的解决方案。
...
### [176] [Comprehensive AI Literacy: The Case for Centering Human Agency](https://arxiv.org/abs/2512.16656)
...
Method: 提出以人类能动性为核心的AI素养框架，包括AI素养、流畅性和能力三个层次。强调将技术视为可选择而非必然采纳的对象，通过批判性思维和认识论理解来培养教育生态系统所有利益相关者的决策能力。

Result: 构建了以人类能动性为中心的AI素养教育框架，为教育者和学生提供了明确路径，使其能够清晰表达对AI的决策意图、态度，并理解这些决策对学术、职业和社会的影响。

Conclusion: 需要系统性转向全面的AI素养教育，以人类能动性为核心，将技术视为可选择的工具而非必然采纳的对象，通过批判性思维培养所有教育利益相关者的决策能力，实现以人为本的AI应用。
...
### [177] [Unsupervised Thematic Clustering Of hadith Texts Using The Apriori Algorithm](https://arxiv.org/abs/2512.16694)
...
Method: 采用无监督学习方法，使用Apriori算法进行关联规则挖掘。对布哈里圣训印尼语译本进行预处理（大小写转换、标点清理、分词、停用词去除、词干提取），然后使用支持度、置信度和提升度参数进行分析

Result: 发现了有意义的关联模式，如"拜功-祈祷"、"经文-启示"、"圣训-故事"之间的关系，这些模式描述了崇拜、启示和圣训叙述等主题
...
### [178] [Do Multi-Agents Solve Better Than Single? Evaluating Agentic Frameworks for Diagram-Grounded Geometry Problem Solving and Reasoning](https://arxiv.org/abs/2512.16698)
*Mahbub E Sobhani,Md. Faiyaz Abdullah Sayeedi,Mohammad Nehad Alam,Proma Hossain Progga,Swakkhar Shatabda*

Main category: cs.AI

TL;DR: 该研究系统比较了单智能体与多智能体在视觉数学问题求解中的表现，发现多智能体对开源模型有明显提升，但对闭源模型效果有限


<details>
  <summary>Details</summary>
Motivation: 研究旨在澄清多智能体设计在图表几何问题求解中的实际价值，探索其在多模态大语言模型中的适用性

Method: 在四个视觉数学基准（Geometry3K、MathVerse、OlympiadBench、We-Math）上系统比较单智能体与多智能体管道，测试开源模型（Qwen-2.5-VL）和闭源模型（Gemini-2.0-Flash）

Result: 开源模型在多智能体模式下表现显著提升（如Qwen-2.5-VL 7B在Geometry3K上提升6.8分），闭源模型在经典基准上单智能体表现更好，仅在新数据集We-Math上有适度提升

Conclusion: 多智能体管道对开源模型有明显优势，可辅助强闭源系统处理新基准，但智能体分解并非普遍最优策略
...
### [179] [Dual Computational Horizons: Incompleteness and Unpredictability in Intelligent Systems](https://arxiv.org/abs/2512.16707)
*Abhisek Ganguly*
...
Method: 通过形式化方法建立两个独立计算限制的理论框架：1）形式不完备性（基于哥德尔不完备定理），2）动态不可预测性（基于混沌理论和计算复杂性）。然后分析这两个极端如何共同约束智能体对自身预测能力的推理。

Result: 证明算法智能体通常无法计算自身的最大预测边界，揭示了智能系统中推理、预测和自我分析之间的固有权衡。形式不完备性和动态不可预测性共同限制了智能体对自身预测能力的自我认知。

Conclusion: 算法智能存在固有的计算限制，形式不完备性和动态不可预测性共同约束了智能体对自身预测能力的推理。这一视角阐明了智能系统中推理、预测和自我分析之间的结构性权衡，对理解算法智能的局限性具有重要意义。
...
### [180] [Discovering and Learning Probabilistic Models of Black-Box AI Capabilities](https://arxiv.org/abs/2512.16733)
*Daniel Bramblett,Rushang Karia,Adrian Ciotinga,Ruthvick Suresh,Pulkit Verma,YooJung Choi,Siddharth Srivastava*

Main category: cs.AI

TL;DR: 该论文提出了一种使用PDDL风格表示来学习和建模黑盒AI系统规划能力的方法，通过蒙特卡洛树搜索创建测试任务、获取数据并剪枝假设空间，学习到的模型能够描述AI的能力、执行条件及可能结果。
...
Method: 使用PDDL风格表示来学习建模黑盒AI的规划能力，采用蒙特卡洛树搜索范式系统性地创建测试任务、获取数据，并剪枝可能的符号模型假设空间。

Result: 学习到的模型能够描述黑盒AI的能力、执行条件以及执行可能结果及其相关概率。理论结果证明了学习模型的可靠性、完整性和收敛性，多个黑盒AI系统的实证结果展示了方法的范围、效率和准确性。

Conclusion: PDDL风格表示可以有效地学习和建模黑盒AI系统的规划能力，为黑盒AI系统提供了可靠且可解释的能力表示方法，有助于确保其安全部署和操作。
...
### [181] [AI-Driven Prediction of Cancer Pain Episodes: A Hybrid Decision Support Approach](https://arxiv.org/abs/2512.16739)
...
TL;DR: 提出混合机器学习与大型语言模型管道，利用结构化与非结构化电子病历数据，预测肺癌患者住院48小时和72小时内的突破性疼痛发作，准确率分别达0.874和0.917。


<details>
  <summary>Details</summary>
Motivation: 肺癌患者中高达91%会经历突破性疼痛发作，需要及时干预。为实现主动疼痛管理，需要开发能够提前预测疼痛发作的工具，以优化治疗精准度和资源分配。

Method: 采用混合机器学习与大型语言模型管道：机器学习模块捕捉时间性用药趋势，大型语言模型解析模糊的用药记录和自由文本临床笔记。分析266名住院患者的回顾性队列，特征包括人口统计学、肿瘤分期、生命体征和WHO分级镇痛药物使用。

Result: 框架在48小时预测中达到0.874准确率，72小时预测达到0.917准确率。大型语言模型的增强使敏感性分别提高了8.6%和10.4%。

Conclusion: 这种混合方法提供了一个临床可解释且可扩展的早期疼痛发作预测工具，有潜力提高肿瘤护理中的治疗精准度并优化资源分配。
...
### [182] [CitySeeker: How Do VLMS Explore Embodied Urban Navigation With Implicit Human Needs?](https://arxiv.org/abs/2512.16755)
*Siqi Wang,Chao Liang,Yunfan Gao,Erxin Yu,Sen Li,Yushi Li,Jing Li,Haofen Wang*

Main category: cs.AI

TL;DR: CitySeeker是一个评估视觉语言模型在动态城市环境中理解隐含需求并进行空间推理的新基准，包含8个城市的6440条轨迹，当前最佳模型任务完成率仅21.1%，揭示了长时推理、空间认知和记忆召回等瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在显式指令导航方面取得进展，但在理解动态城市环境中的隐含人类需求（如"我渴了"）方面研究不足，需要评估模型的空间推理和决策能力来解决"最后一公里"导航挑战。

Method: 提出CitySeeker基准，包含8个城市6440条轨迹，涵盖7种目标驱动场景的多样化视觉特征和隐含需求。采用回溯机制、增强空间认知和基于记忆检索（BCR）等探索策略，受人类认知地图的迭代观察-推理循环和自适应路径优化启发。

Result: 实验显示即使最佳模型（如Qwen2.5-VL-32B-Instruct）任务完成率仅为21.1%。发现关键瓶颈包括长时推理中的错误累积、空间认知不足和经验记忆召回缺陷。

Conclusion: 分析为开发具有强大空间智能的视觉语言模型提供了可操作的见解，这些模型能够应对"最后一公里"导航挑战，强调了迭代观察-推理循环和自适应路径优化的重要性。
...
### [183] [Good Locally Testable Codes with Small Alphabet and Small Query Size](https://arxiv.org/abs/2512.16082)
*Uriya First,Stav Lazarovici*

Main category: cs.CC

TL;DR: 该论文解决了关于良好局部可测试码（LTC）存在性的基本问题，证明了除了已知的限制外，几乎所有情况下都存在良好的q-查询LTC。


<details>
  <summary>Details</summary>
Motivation: Ben-Sasson、Goldreich和Sudan之前的研究表明，对于二进制字母表或有限域上的线性码，2-查询测试器无法同时获得线性距离和正速率。本研究旨在探索这些限制是否是LTC存在的唯一障碍。

Method: 基于作者之前关于良好2-查询F-线性LTC的工作，建立了一种通用的方法来减少低查询LTC的字母表大小，从而构造各种字母表下的良好LTC。

Result: 证明了对于任何字母表大小大于2的情况，存在良好的2-查询LTC；对于二进制字母表，存在良好的3-查询LTC。对于F-线性情况，存在良好的3-查询F-线性LTC，并且对于任何维数大于1的F-向量空间，存在良好的2-查询LTC。

Conclusion: 该研究完全解决了对于每个q≥2和字母表（或F-向量空间）Σ，是否存在具有良好性质的q-查询LTC（或F-线性LTC）的问题，表明除了已知的少数限制外，良好LTC几乎在所有情况下都存在。

Abstract: Ben-Sasson, Goldreich and Sudan showed that a binary error correcting code admitting a $2$-query tester cannot be good, i.e., it cannot have both linear distance and positive rate. The same holds when the alphabet is a finite field $\mathbb{F}$, the code is $\mathbb{F}$-linear, and the $2$-query tester is $\mathbb{F}$-linear. We show that those are essentially the only limitations on the existence of good locally testable codes (LTCs). That is, there are good $2$-query LTCs on any alphabet with more than $2$ letters, and good $3$-query LTCs with a binary alphabet. Similarly, there are good $3$-query $\mathbb{F}$-linear LTCs, and for every $\mathbb{F}$-vector space $V$ of dimension greater than $1$, there are good $2$-query LTCs with alphabet $V$ whose tester is $\mathbb{F}$-linear. This completely solves, for every $q\geq 2$ and alphabet (resp. $\mathbb{F}$-vector space) $Σ$, the question of whether there is a good $q$-query LTC (resp. $\mathbb{F}$-LTC) with alphabet $Σ$. Our proof builds on the recent good $2$-query $\mathbb{F}$-LTCs of the first author and Kaufman, by establishing a general method for reducing the alphabet size of a low-query LTC.

</details>


### [184] [Hardness of SetCover Reoptimization](https://arxiv.org/abs/2512.16805)
*Klaus Jansen,Tobias Mömke,Björn Schumacher*

Main category: cs.CC

TL;DR: 该论文研究了集合覆盖问题的再优化硬度，分析了四种修改类型（添加/删除集合、添加/删除元素），发现大多数情况与经典集合覆盖问题同样难近似，只有少数情况已知有PTAS但不存在EPTAS。


<details>
  <summary>Details</summary>
Motivation: 研究集合覆盖问题的再优化复杂性，探索在已知原实例解的情况下，对修改后实例进行近似求解时能否获得更好的近似比。

Method: 分析四种集合覆盖再优化场景：添加集合、删除集合、添加元素到全集、从全集中删除元素。结合经典再优化NP-hard证明方法与EPTAS和FPT之间的关系进行理论分析。

Result: 大多数再优化情况与经典集合覆盖问题同样难近似；对于无权重情况下添加/删除元素的已知PTAS场景，证明在常见硬度假设下不存在EPTAS。

Conclusion: 集合覆盖再优化问题中，除了少数已知较容易的情况外，大多数修改类型都保持经典集合覆盖的近似硬度，为再优化理论提供了新的边界结果。

Abstract: We study hardness of reoptimization of the fundamental and hard to approximate SetCover problem. Reoptimization considers an instance together with a solution and a modified instance where the goal is to approximate the modified instance while utilizing the information gained by solution to the related instance. We study four different types of reoptimization for (weighted) SetCover: adding a set, removing a set, adding an element to the universe, and removing an element from the universe. A few of these cases are known to be easier to approximate than the classic SetCover problem. We show that all the other cases are essentially as hard to approximate as SetCover.
