...
TL;DR: SuperHead是一个从低分辨率3D头部重建高质量可动画化3D头像的框架，通过动态感知3D反演利用预训练3D生成模型先验，生成超分辨率的3D高斯溅射模型。


<details>
  <summary>Details</summary>
Motivation: 当前从低质量图像或视频源创建高保真、可动画化的3D头部存在挑战，低质量源导致3D重建效果差，现有超分辨率技术难以处理动态3D输入。

Method: 提出动态感知3D反演方案，利用预训练3D生成模型先验优化潜在表示，生成超分辨率3D高斯溅射头部模型，然后绑定到参数化头部模型（如FLAME）进行动画化，使用从不同表情和视角捕获的上采样2D面部渲染和深度图进行联合监督。

Result: SuperHead在动态运动下生成具有精细面部细节的头像，在视觉质量上显著优于基线方法。

Conclusion: SuperHead框架成功解决了从低质量源创建高质量可动画化3D头部的问题，通过创新的动态感知3D反演方法实现了高质量的3D重建和动画一致性。
...
TL;DR: EgoAVU：一个用于自动生成第一人称视角音频-视频叙述、问答的数据引擎，通过构建大规模训练数据集EgoAVU-Instruct和评估基准EgoAVU-Bench，解决了现有多模态大语言模型在理解第一人称视频时过度依赖视觉信号、忽视音频线索的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型虽然能接受视觉和音频输入，但由于缺乏包含连贯联合模态信息的文本标签，它们在理解第一人称视角视频中的音频-视觉联合信息方面仍未被充分探索。

Method: 提出了EgoAVU数据引擎，通过跨模态关联建模自动生成音频-视觉叙述；采用基于令牌的视频过滤和模块化、基于图的策展确保数据多样性和质量；构建了包含300万样本的EgoAVU-Instruct训练数据集和手动验证的EgoAVU-Bench评估集。

Result: EgoAVU-Bench揭示了现有MLLMs严重偏向视觉信号、常忽视音频线索或无法将音频与视觉源对应的问题；在EgoAVU-Instruct上微调MLLMs能有效解决这一问题，在EgoAVU-Bench上实现高达113%的性能提升，并在EgoTempo和EgoIllusion等其他基准上获得高达28%的相对性能增益。

Conclusion: EgoAVU数据引擎和数据集为解决多模态大语言模型在第一人称视角视频理解中的音频-视觉联合理解问题提供了有效方案，显著提升了模型在音频-视觉任务上的性能，并具有良好的泛化能力。
...
TL;DR: MGP-KAD：一种结合RGB和几何先验的多模态特征融合框架，用于提升复杂场景下单视图3D重建的精度
...
Method: 提出MGP-KAD多模态特征融合框架，整合RGB和几何先验；通过对真实物体数据进行采样聚类生成类级几何特征，并在训练中动态调整；采用基于Kolmogorov-Arnold Networks的混合解码器处理复杂多模态输入

Result: 在Pix3D数据集上的大量实验表明，MGP-KAD达到最先进性能，显著改善了几何完整性、平滑度和细节保留

Conclusion: MGP-KAD为复杂场景下单视图3D重建提供了鲁棒有效的解决方案，推动了该领域的发展
...
TL;DR: DwD框架利用DINOv3视觉基础模型特征作为Sim2Real桥梁，通过主成分投影和随机通道丢弃平衡真实感与控制一致性，结合空间对齐模块和因果时序聚合器提升控制精度和时序稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶视频生成的Sim2Real方法面临"一致性-真实感困境"：低级信号（如边缘、模糊图像）能确保精确控制但会引入合成伪影损害真实感，而高级先验（如深度、语义、高精地图）能实现照片级真实感但缺乏结构细节导致控制一致性不足。

Method: 1. 利用DINOv3视觉基础模型特征作为仿真与真实域的统一桥梁；2. 采用主成分投影去除导致"纹理烘焙"的高频成分；3. 引入随机通道尾部丢弃缓解刚性降维的结构损失；4. 设计可学习空间对齐模块适配高分辨率特征到扩散骨干；5. 提出因果时序聚合器使用因果卷积保留历史运动上下文。

Result: DwD框架在自动驾驶视频生成中成功平衡了真实感与控制一致性，有效缓解了运动模糊并保证了时序稳定性，相比现有方法在Sim2Real转换质量上有显著提升。

Conclusion: DwD通过创新性地利用DINOv3特征作为Sim2Real桥梁，结合特征处理、空间对齐和时序聚合技术，成功解决了自动驾驶视频生成中的一致性-真实感困境，为可控视频扩散模型提供了更有效的域适应方法。
...
TL;DR: MetaSSP是一种用于单视图3D重建的半监督框架，通过利用大量未标记图像，结合梯度参数重要性估计和SDF感知伪标签加权，在Pix3D基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 基于隐式SDF的单视图3D重建方法需要大量标注数据，限制了其可扩展性。为了解决这个问题，作者提出利用丰富的未标记图像来减少对标注数据的依赖。
...
Conclusion: MetaSSP通过有效利用未标记图像，显著提升了单视图3D重建的性能，为减少对大规模标注数据集的依赖提供了有效的半监督解决方案。
...
TL;DR: M3是一个无需训练的多模态多智能体框架，通过迭代推理优化解决文本到图像合成中的复杂组合提示问题，使开源模型性能超越商业系统。


<details>
  <summary>Details</summary>
Motivation: 生成模型在文本到图像合成中虽然取得了令人印象深刻的逼真度，但在处理涉及多个约束的复杂组合提示时仍然存在困难。现有方法难以同时满足多个约束条件，导致合成结果不符合预期。

Method: M3采用无需训练的多智能体循环框架：规划器将提示分解为可验证的检查清单，专门的检查器、优化器和编辑器智能体逐个修正约束条件，验证器确保单调改进。该框架利用现成的基础模型，通过迭代推理时间优化来系统解决组合约束问题。

Result: 在具有挑战性的OneIG-EN基准测试中，Qwen-Image+M3超越了包括Imagen4(0.515)和Seedream 3.0(0.530)在内的商业旗舰系统，达到了最先进的性能(0.532)。M3显著提高了GenEval组合指标，在强化测试集上将空间推理性能有效提升了一倍。

Conclusion: M3作为一个即插即用的模块，与任何预训练的文本到图像模型兼容，无需昂贵的重新训练即可建立组合生成的新范式。智能多智能体推理能够提升开源模型超越专有替代方案。
...
TL;DR: 提出了一种用于盆腔MRI的无监督异常检测基准框架，使用残差变分自编码器仅基于健康T2加权扫描训练，通过重建误差热图检测病理区域，无需异常数据标签，支持实时MRI集成。


<details>
  <summary>Details</summary>
Motivation: 育龄妇女盆腔疾病是全球重大健康负担，但MRI诊断常因解剖结构变异大而延迟。现有AI方法多为疾病特异性且缺乏实时兼容性，限制了通用性和临床整合。

Method: 使用残差变分自编码器，仅基于294个健康矢状位T2加权扫描训练，通过扩散生成合成数据增强鲁棒性。推理时通过重建误差热图指示与学习到的健康结构的偏差，实现无监督异常检测。

Result: 在公开子宫肌瘤MRI数据集上获得平均AUC 0.736，敏感性0.828，特异性0.692。临床评估扩展到子宫内膜癌、子宫内膜异位症和子宫腺肌症，重建速度约92.6帧/秒。

Conclusion: 该框架为女性盆腔无监督异常检测建立了基准，支持未来集成到实时MRI中，代码和前瞻性数据集可供学术合作。
...
TL;DR: PhenoLIP：一种通过构建大规模表型知识图谱（PhenoKG）和两阶段预训练框架，将结构化表型知识融入医学视觉语言模型的新方法，显著提升表型分类和跨模态检索性能。
...
Method: 1) 构建PhenoKG：包含52万高质量图像-文本对，关联3000多个表型；2) 提出PhenoLIP两阶段预训练框架：首先从文本本体数据学习知识增强的表型嵌入空间，然后通过教师引导的知识蒸馏将结构化知识融入多模态预训练；3) 创建PhenoBench基准：包含7800多个图像-描述对，覆盖1000多个表型。

Result: PhenoLIP显著优于现有最佳基线：在表型分类准确率上比BiomedCLIP提升8.85%，在跨模态检索上比BIOMEDICA提升15.03%，证明了整合表型中心先验知识的价值。

Conclusion: 通过构建大规模表型知识图谱和两阶段知识蒸馏框架，成功将结构化表型知识融入医学视觉语言模型，实现了更结构化、可解释的医学图像理解，为医学多模态学习提供了新范式。
...
TL;DR: DeDPO通过因果推断的去偏估计技术改进DPO，利用合成AI反馈增强有限人类标注数据，解决了大规模高质量人类偏好标注的成本和可扩展性瓶颈。


<details>
  <summary>Details</summary>
Motivation: DPO方法虽然有效，但依赖大规模高质量人类偏好标注，存在严重的成本和可扩展性瓶颈。需要开发能够利用廉价合成监督的解决方案。

Method: 提出DeDPO框架，将因果推断中的去偏估计技术整合到DPO目标中，通过识别和校正合成标注器固有的系统性偏差和噪声，实现从不完美反馈源（包括自训练和视觉语言模型）的鲁棒学习。

Result: 实验表明DeDPO对合成标注方法的变化具有鲁棒性，性能达到甚至偶尔超过完全人类标注数据训练的理论上限。

Conclusion: DeDPO为使用廉价合成监督进行人-AI对齐提供了一个可扩展的解决方案。
...
TL;DR: DroneKey++是一个无需先验信息的无人机3D姿态估计框架，通过关键点检测、无人机分类和3D姿态估计联合实现，在合成数据集6DroneSyn上验证了良好的泛化能力和实时性能。
...
Method: 提出DroneKey++框架：1) 关键点编码器同时进行关键点检测和分类；2) 姿态解码器利用基于射线的几何推理和类别嵌入估计3D姿态；3) 构建大规模合成数据集6DroneSyn，包含7种无人机模型和88种户外背景。

Result: 旋转误差MAE 17.34度、MedAE 17.1度，平移误差MAE 0.135米、MedAE 0.242米，推理速度CPU 19.25 FPS、GPU 414.07 FPS，表现出跨无人机模型的强泛化能力和实时应用潜力。
...
TL;DR: 该论文提出了一种可微分车辆模型框架，用于弥合自动驾驶中基于航点与基于动作的模型之间的差距，使基于动作的架构能够在基于航点的基准上进行训练和评估。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶系统主要分为基于航点的模型和基于动作的模型。大多数基准协议和训练流程都是基于航点的，这使得基于动作的策略难以训练和比较，阻碍了其发展。需要弥合这种航点-动作差距。
...
Conclusion: 提出的可微分车辆模型框架成功弥合了自动驾驶中基于航点与基于动作模型之间的差距，使基于动作的架构能够在标准基准上进行公平比较和训练，推动了基于动作策略的发展。
...
Method: 基于等能量假设（Iso-Energy Assumption）——真正共享的概念在不同模态中应具有相同的平均能量，研究者开发了对齐稀疏自编码器（SAE）。该方法在训练中鼓励能量一致性，同时保持重建能力，从而获得可用于几何分析的表征。

Result: 在已知真实值的受控数据上进行验证，确认当等能量假设成立时对齐效果改善，不成立时保持中性。应用于基础视觉语言模型时发现：1）稀疏双模态原子携带全部跨模态对齐信号；2）单模态原子作为模态特定偏置，完全解释模态差距；3）移除单模态原子可消除差距而不损害性能；4）将向量运算限制在双模态子空间可实现分布内编辑和改进检索。

Conclusion: 正确的归纳偏置既能保持模型保真度，又能使潜在几何结构变得可解释和可操作。这一发现为理解和改进视觉语言模型的跨模态表示提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 日常手物交互的单目视频为具身智能提供了宝贵资源，但现有方法在重建物体时面临严重遮挡和相机、手、物体复杂耦合运动的挑战，需要开发更高效的3D物体重建方法。

Method: 提出前馈模型ForeHOI，通过联合预测2D掩码修复和3D形状补全来处理严重遮挡问题，2D和3D形状补全之间的信息交换提升了整体重建质量。同时贡献了首个大规模高保真合成手物交互数据集用于训练。

Result: ForeHOI在物体重建方面达到了最先进的性能，显著优于先前方法，速度提升约100倍，能够有效处理严重的手物遮挡情况。

Conclusion: ForeHOI通过前馈框架中的2D-3D联合形状补全，成功解决了单目手持物体视频中的严重遮挡问题，实现了高效准确的3D物体重建，为手物交互分析提供了有力工具。
...
<details>
  <summary>Details</summary>
Motivation: Kabuki综合征和Wiedemann-Steiner综合征是两种罕见但具有重叠临床特征的发育障碍。尽管遗传检测是诊断金标准，但由于遗传检测和专业知识的获取障碍，许多患者仍未被诊断。皮肤纹理异常虽然是多种遗传综合征的已知特征，但在分子检测时代仍未得到充分利用。

Method: 本研究提出了一种基于视觉Transformer的深度学习模型，利用指纹图像进行三类二元分类任务：正常对照vs.Kabuki综合征、正常对照vs.Wiedemann-Steiner综合征、以及两种综合征之间的区分。模型采用注意力机制可视化技术，识别对预测最关键的指纹区域，增强模型的可解释性。

Result: 模型在三个分类任务中表现良好：正常对照vs.Kabuki综合征的AUC为0.80，F1分数0.71；正常对照vs.Wiedemann-Steiner综合征的AUC为0.73，F1分数0.72；两种综合征之间的区分AUC为0.85，F1分数0.83。注意力可视化成功识别了对模型预测最重要的指纹区域。

Conclusion: 研究结果表明存在综合征特异性的指纹特征，证明了基于指纹的人工智能工具作为非侵入性、可解释且易于获取的未来诊断辅助手段的可行性，有助于早期诊断未被充分诊断的遗传综合征。
...
TL;DR: 提出了MMEarth-Bench多模态环境基准数据集，包含5个任务12种模态的全球分布数据，评估发现多模态预训练在有限数据下提升鲁棒性但地理泛化能力仍差，提出了测试时多模态重建方法改善性能
...
Method: 1) 构建MMEarth-Bench数据集：5个多模态环境任务、12种模态、全球分布数据、包含分布内和分布外测试分割；2) 提出测试时多模态重建方法(TTT-MMR)，利用测试时所有可用模态作为辅助任务，无论预训练模型是否接受这些模态作为输入

Result: 多模态预训练在有限数据设置下能提升模型鲁棒性，但地理泛化能力仍然较差；提出的TTT-MMR方法在随机和地理测试分割上都能提升模型性能；地理批处理在TTT中实现了正则化和专业化之间的良好平衡

Conclusion: MMEarth-Bench填补了多模态地理空间基准的空白，提出的测试时多模态重建方法能有效提升模型对新下游任务和地理域的适应能力，地理批处理策略在测试时训练中效果良好
...
TL;DR: 提出了一种基于多级相关金字塔优化的无监督多模态医学图像配准方法MCPO，用于解决术前-术中多模态图像配准中的挑战，在Learn2Reg 2025的ReMIND2Reg任务中取得第一名。
...
Conclusion: 提出的MCPO方法有效解决了多模态图像配准挑战，在手术导航应用中表现出色，代码已开源供研究社区使用。
...
TL;DR: 提出一种将Vision Transformer线性层和层归一化替换为卷积操作的方法，使ViT模型能在CNN优化的BPU硬件上高效部署，无需重新训练即可继承原始权重。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习发展，CNN优化的BPU硬件专注于INT8卷积加速，而Vision Transformer模型在计算机视觉任务中表现优异。但由于Transformer线性层处理三维数据，BPU硬件针对四维卷积设计，导致ViT无法充分利用BPU的加速优势。

Method: 提出新颖的Vision Transformer重构方法，将线性层和层归一化操作替换为精心设计的卷积算子。这种方法使DeiT模型能够完全利用BPU加速能力，同时允许重构模型继承原始权重参数，无需重新训练或微调。

Result: 量化后的DeiT-Base模型在ImageNet上达到80.4%准确率（原模型81.8%），同时获得最高3.8倍的推理加速。在花卉分类数据集上微调的DeiT模型也表现出色，DeiT-Base模型仅下降0.5%准确率。

Conclusion: 该方法首次成功实现了Vision Transformer在BPU硬件上的完全利用，有效解决了CNN优化硬件与Transformer架构不匹配的问题，为ViT模型在专用硬件上的高效部署提供了可行方案。
...
TL;DR: 本文提出了一种基于重新初始化的方法（ABR）来改善持续测试时域适应的长期性能，通过自适应间隔进行权重重置来保持模型在长期变化环境中的表现。


<details>
  <summary>Details</summary>
Motivation: 持续测试时域适应（CTTA）旨在使模型能够在非平稳环境中随时间保持良好性能。虽然现有方法在优化适应过程方面做出了很大努力，但一个关键问题仍然存在：模型能否在长时间内适应持续变化的环境？本文探索使用基于重新初始化的方法来促进更好的长期CTTA性能。

Method: 提出自适应平衡重新初始化（ABR）策略。首先观察到长期性能与标签翻转中的轨迹模式相关，基于此相关性，ABR使用自适应间隔进行权重重新初始化。自适应间隔根据标签翻转的变化来确定，旨在保持模型的长期性能。

Result: 该方法在广泛的CTTA基准测试中进行了验证，取得了优越的性能表现。

Conclusion: 基于重新初始化的方法（ABR）能够有效改善持续测试时域适应的长期性能，通过自适应间隔的权重重置策略，模型能够在持续变化的环境中保持良好表现。
...
<details>
  <summary>Details</summary>
Motivation: SAM3通过可提示概念分割革新了开放词汇分割，但其依赖预定义概念使其在面对目标域的数据漂移（视觉分布变化）和概念漂移（条件标签分布变化）时变得脆弱，导致视觉证据与提示的对齐失效。

Method: 提出ConceptBank框架：从目标统计构建数据集特定的概念库；通过类级视觉原型锚定目标域证据；挖掘代表性支持以抑制数据漂移下的异常值；融合候选概念以纠正概念漂移。

Result: ConceptBank能有效使SAM3适应分布漂移，包括具有挑战性的自然场景和遥感场景，为开放词汇分割的鲁棒性和效率建立了新的基准。

Conclusion: ConceptBank是一个无需参数的校准框架，能够动态恢复视觉证据与提示的对齐，提升SAM3在分布漂移下的适应能力，为开放词汇分割提供了更鲁棒和高效的解决方案。
...
TL;DR: SPDA-SAM是一个自提示深度感知的SAM模型，通过语义-空间自提示模块和粗到细RGB-D融合模块，解决了SAM依赖手动提示和RGB图像缺乏深度信息的问题，在12个数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: SAM在实例分割中表现出强大的泛化能力，但其性能严重依赖手动提示的质量，且RGB图像缺乏深度信息，限制了模型感知空间结构和边界划分的能力。

Method: 提出SPDA-SAM：1) 语义-空间自提示模块(SSSPM)，从SAM的图像编码器和掩码解码器中分别提取语义和空间提示；2) 粗到细RGB-D融合模块(C2FFM)，融合单目RGB图像特征和深度图特征，深度图的结构信息提供粗粒度指导，局部深度变化编码细粒度特征。

Result: 在12个不同数据集上的实验结果表明，SPDA-SAM优于现有的最先进方法，这归功于自提示的指导和粗到细RGB-D融合操作对空间信息损失的补偿。

Conclusion: SPDA-SAM通过自提示机制和深度感知融合，有效解决了SAM依赖手动提示和RGB图像缺乏深度信息的问题，在实例分割任务中取得了优异性能，是首个以自提示和深度感知方式探索SAM的研究。
...
<details>
  <summary>Details</summary>
Motivation: 当前快速流模型训练存在两个根本问题：1）基于随机配对噪声-数据样本构建的条件速度引入系统性轨迹漂移，阻碍模型遵循一致的ODE路径；2）模型近似误差随时间步累积，导致长时间间隔的严重偏差。

Method: 提出FlowConsist训练框架：1）用模型自身预测的边际速度替代条件速度，使优化与真实轨迹对齐；2）引入轨迹校正策略，在轨迹的每个时间步对齐生成样本和真实样本的边际分布。

Result: 在ImageNet 256×256数据集上仅用1步采样就达到了FID 1.52的新SOTA性能，显著优于现有快速流模型。
...
TL;DR: TFusionOcc：一种基于对象中心的多传感器融合框架，用于3D语义占据预测，通过可变形超二次曲面等几何基元和多阶段融合策略，在nuScenes基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D语义占据预测方法主要依赖3D体素或高斯分布表示，难以高效捕捉驾驶环境中细粒度的几何细节，需要更灵活的几何基元和更有效的多传感器融合方法。

Method: 提出TFusionOcc框架：1）使用可变形超二次曲面等更灵活的几何基元；2）采用多阶段多传感器融合策略；3）利用学生t分布和T混合模型进行建模。

Result: 在nuScenes基准测试中达到最先进性能，并在nuScenes-C数据集上验证了方法在不同相机和激光雷达损坏场景下的鲁棒性。

Conclusion: TFusionOcc通过更灵活的几何表示和多传感器融合策略，有效提升了3D语义占据预测的精度和鲁棒性，为自动驾驶感知提供了更好的解决方案。
...
TL;DR: MeDocVL：一种用于查询驱动医疗文档解析的后训练视觉语言模型，通过训练驱动的标签精炼和噪声感知混合后训练策略，在医疗发票基准测试中优于传统OCR系统和VLM基线
...
TL;DR: PointViT：基于Transformer的3D目标检测框架，通过选择性采样虚拟点增强远距离稀疏LiDAR点云，采用多种融合策略平衡精度与效率，在KITTI基准上取得优异性能


<details>
  <summary>Details</summary>
Motivation: LiDAR点云在远距离区域稀疏，缺乏可靠几何线索，导致远场目标检测困难。现有方法通过RGB图像深度补全生成虚拟点，但直接融合所有虚拟点会增加计算成本且难以有效融合真实与虚拟信息
...
Conclusion: PointViT通过选择性融合虚拟点有效解决了远距离LiDAR点云稀疏问题，平衡了精度与效率，为3D目标检测提供了有效的解决方案
...
TL;DR: SemGeo-AttentionNet：双流架构，通过非对称跨模态融合建模3D物体上的视觉注意力，结合几何特征与扩散语义先验，并扩展到时间扫描路径生成


<details>
  <summary>Details</summary>
Motivation: 现有3D显著性方法依赖手工几何特征或缺乏语义感知的学习方法，无法解释人类为何注视语义重要但几何不显著的区域。需要建模视觉注意力中几何处理与语义识别的相互作用

Method: 提出SemGeo-AttentionNet双流架构：几何流使用点云transformer处理几何特征，语义流利用几何条件多视角渲染的扩散语义先验。通过非对称跨模态融合，几何特征查询语义内容，实现自下而上的显著性引导自上而下的语义检索。通过强化学习扩展到时间扫描路径生成，首次考虑3D网格拓扑和抑制返回动态

Result: 在SAL3D、NUS3D和3DVA数据集上评估显示显著改进，验证了认知动机架构能有效建模三维表面上的人类视觉注意力

Conclusion: 通过明确建模几何处理与语义识别的二分法，提出的双流架构能更准确地预测人类在3D物体上的视觉注意力，为理解人类视觉系统提供了新的计算框架
...
TL;DR: TP-GRPO是一种改进的GRPO框架，通过引入步骤级增量奖励和转折点检测来解决文本到图像生成中奖励信号稀疏和长期效应建模问题。
...
Method: TP-GRPO提出两个关键创新：1）用步骤级增量奖励替代基于结果的奖励，提供密集的步骤感知学习信号，更好隔离每个去噪动作的"纯"效应；2）识别转折点（改变局部奖励趋势并使后续奖励演化与整体轨迹趋势一致的步骤），为这些动作分配聚合的长期奖励以捕捉其延迟影响。转折点仅通过增量奖励的符号变化检测，使TP-GRPO高效且无需超参数。
...
TL;DR: POPL-KF是一种基于卡尔曼滤波的视觉惯性里程计系统，采用仅姿态几何表示法处理点和线特征，通过消除特征坐标来减少线性化误差，实现实时高性能定位。


<details>
  <summary>Details</summary>
Motivation: 传统基于MSCKF的VIO系统在挑战性场景中性能下降，且存在特征3D坐标线性化误差和测量更新延迟问题，需要改进定位精度。
...
Conclusion: POPL-KF通过消除特征坐标、减少线性化误差、实现即时测量更新，显著提高了VIO在挑战性场景中的性能，为实时高精度定位提供了有效解决方案。
...
TL;DR: 本文提出了一种新的"外到内先验自由指令驱动具身导航"任务，解决了现有方法无法实现从室外到室内无缝过渡的问题，并提出了相应的视觉中心框架和首个开源数据集。


<details>
  <summary>Details</summary>
Motivation: 现有具身导航方法主要局限于室内或室外环境，且依赖精确坐标系统等强假设。当前室外方法只能将智能体引导到目标附近，但无法实现通过特定建筑入口的细粒度进入，这在需要无缝室外到室内过渡的实际部署场景中严重限制了其实用性。

Method: 提出了一个视觉中心的具身导航框架，利用基于图像的提示来驱动决策。同时创建了首个开源数据集，采用将轨迹条件视频合成集成到数据生成过程中的流水线。

Result: 通过大量实验证明，所提出的方法在关键指标（包括成功率和路径效率）上始终优于最先进的基线方法。

Conclusion: 本文通过引入新的任务定义、相应的框架和数据集，成功解决了具身导航中室外到室内无缝过渡的挑战，为实际应用场景提供了更实用的解决方案。
...
TL;DR: ChatUMM：首个支持连续多轮对话的统一多模态模型，通过交错多轮训练策略和系统对话数据合成，实现了跨模态的上下文跟踪和生成


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型局限于单轮交互范式，只能处理独立请求而非连续对话。需要开发能够支持多轮对话、保持上下文一致性的统一多模态助手。

Method: 1. 交错多轮训练策略：将序列化的文本-图像流建模为连续对话流；2. 系统对话数据合成管道：通过三个阶段将单轮数据集转化为流畅对话：构建基础状态对话、通过"干扰"轮次和依赖历史的查询重写强制长程依赖解析、合成自然交错的多模态响应。

Result: ChatUMM在开源统一模型中实现了视觉理解和指令引导编辑基准的SOTA性能，同时在文本到图像生成中保持有竞争力的保真度。在复杂多轮场景中表现出卓越的鲁棒性，确保流畅、上下文感知的对话。

Conclusion: ChatUMM成功将统一多模态模型从单轮求解器扩展到连续对话助手，通过创新的训练策略和数据合成方法实现了强大的上下文跟踪能力，为多模态对话系统的发展提供了新方向。
...
TL;DR: UnionST-S合成数据集通过改进文本多样性、字体和布局模拟，显著缩小合成与真实文本数据间的领域差距，在某些场景下甚至超越真实数据性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于渲染的合成文本数据集在语料库、字体和布局多样性方面不足，导致合成数据与真实数据之间存在显著领域差距，限制了场景文本识别模型的性能。

Method: 提出UnionST数据引擎，合成覆盖挑战性样本并更符合真实世界复杂性的文本数据；构建UnionST-S大规模合成数据集；开发自进化学习框架用于有效标注真实数据。

Result: 在UnionST-S上训练的模型相比现有合成数据集有显著提升，在某些场景下甚至超越真实数据性能；使用SEL框架时，仅需9%的真实数据标签即可达到竞争性性能。

Conclusion: 通过改进合成数据的多样性和复杂性模拟，可以有效缩小合成与真实文本数据间的领域差距，显著提升场景文本识别模型的性能。
...
TL;DR: 提出LAB-Det方法，通过语言作为领域不变桥梁，让冻结的基础检测器仅需每个类别一个示例就能适应专业领域，无需权重更新，在数据稀缺场景下超越微调方法。


<details>
  <summary>Details</summary>
Motivation: 基础目标检测器（如GLIP、Grounding DINO）在通用领域表现良好，但在专业领域（如水下图像、工业缺陷）和数据稀缺场景下性能下降。传统的跨领域少样本方法需要微调稀缺的目标数据，成本高且有过拟合风险。

Method: 提出LAB-Det方法，利用语言作为领域不变桥梁。不调整视觉特征，而是将每个示例投影为描述性文本，用这些文本条件来引导冻结的检测器。这种语言条件化替代了基于梯度的适应方法。

Result: 在UODD（水下）和NEU-DET（工业缺陷）两个数据稀缺检测基准上评估，LAB-Det在无需更新任何参数的情况下，比最先进的微调基线提升了高达5.4 mAP。

Conclusion: 语言适应是专业检测场景中微调的高效且可解释的替代方案，为数据稀缺领域的目标检测提供了新的训练自由适应范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督方法在训练和评估协议之间存在不一致，且2D真值无法揭示遮挡区域固有的几何模糊性，这限制了单目3D结构推断在自动驾驶中的性能。

Method: 1) 重新解释体渲染过程中的变量，识别最物理一致的占据概率表示；2) 改进评估协议，使新识别表示与体素级3D占据真值对齐；3) 引入遮挡感知极化机制，利用多视角视觉线索增强遮挡区域占据与自由空间的区分。

Result: 实验表明，该方法不仅显著优于现有无监督方法，而且达到了与有监督方法相当的性能。

Conclusion: 本文提出的基准和评估协议为无监督单目3D占据预测提供了更物理一致和有效的框架，特别是在处理遮挡区域方面取得了重要进展。
...
TL;DR: DreamHome-Pano是一个可控的全景室内生成框架，通过Prompt-LLM和冲突无控制架构解决布局约束与风格偏好的协调问题，实现高质量室内设计合成。


<details>
  <summary>Details</summary>
Motivation: 现代室内设计中，个性化空间生成需要在刚性建筑结构约束和特定风格偏好之间取得平衡。现有多条件生成框架常出现"条件冲突"问题，风格属性会损害布局的几何精度。
...
Conclusion: DreamHome-Pano通过创新的Prompt-LLM和冲突无控制架构，有效解决了室内设计中结构约束与风格偏好的协调问题，为高质量全景室内生成提供了有效框架。
...
TL;DR: DriveWorld-VLA是一个统一世界建模与规划的端到端自动驾驶框架，通过潜在空间整合视觉语言动作模型与世界模型，实现可控的特征级想象，在多个基准测试中达到SOTA性能。
...
Method: 提出DriveWorld-VLA框架，在表示层紧密整合VLA与世界模型，将世界模型的潜在状态作为VLA规划器的核心决策状态，支持可控的动作条件特征级想象。

Result: 在NAVSIMv1上达到91.3 PDMS，NAVSIMv2上达到86.8 EPDMS，nuScenes上达到0.16的3秒平均碰撞率，均达到最先进性能。

Conclusion: DriveWorld-VLA通过潜在空间统一世界建模与规划，使VLA规划器能够直接从整体场景演化建模中受益，减少对密集标注监督的依赖，提升自动驾驶决策能力。
...
TL;DR: 提出了MicroBi-ConvLSTM，一种超轻量级卷积-循环架构，平均仅需11.4K参数，比现有最轻模型减少2.9倍参数，同时保持线性复杂度，适用于资源受限的可穿戴设备。


<details>
  <summary>Details</summary>
Motivation: 当前最轻量级的人体活动识别模型（如TinierHAR和TinyHAR）虽然精度高，但在考虑操作系统开销后仍超出微控制器的内存预算，需要更轻量级的架构来满足资源受限边缘设备的部署需求。

Method: 采用两阶段卷积特征提取（包含4倍时间池化）和单层双向LSTM的卷积-循环架构，通过这种设计实现参数大幅减少（平均11.4K参数），同时保持线性O(N)复杂度。

Result: 在八个不同的HAR基准测试中表现优异：UCI-HAR上93.41%宏F1，SKODA装配手势上94.46%，Daphnet步态冻结检测上88.98%。INT8后训练量化仅导致平均0.21%的F1分数下降，部署占用空间平均23.0KB。

Conclusion: MicroBi-ConvLSTM在超轻量级模型中保持了有竞争力的性能，通过系统消融研究揭示了双向性对事件检测任务有益但对周期性运动任务增益有限，量化后的部署占用空间适合内存受限的边缘设备。
...
TL;DR: 提出AdaptOVCD，一种无需训练的开放词汇变化检测框架，通过双维度多层次信息融合实现零样本任意类别变化检测。


<details>
  <summary>Details</summary>
Motivation: 现有变化检测方法依赖预定义类别和大规模像素级标注，在开放世界场景中泛化能力有限，需要一种无需训练、能检测任意类别变化的方法。

Method: 基于双维度多层次信息融合：垂直维度融合数据层、特征层和决策层信息；水平维度结合针对性自适应设计。具体包括数据层的自适应辐射对齐、特征层的自适应变化阈值、决策层的自适应置信度过滤，协同多种预训练模型。

Result: 在9个场景的评估中，AdaptOVCD以零样本方式检测任意类别变化，显著优于现有无需训练方法，在跨数据集评估中达到全监督性能上限的84.89%，展现出优越的泛化能力。

Conclusion: AdaptOVCD通过双维度多层次信息融合实现了无需训练的开放词汇变化检测，有效解决了现有方法在开放世界场景中的局限性，具有强大的泛化能力和实际应用价值。
...
Result: NEC在高压缩率下实现高质量重建，有效解耦运动与骨架结构，支持跨物种运动迁移、组合、去噪、生成和文本-运动检索等任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA基准测试存在评估协议不足的问题，无法充分捕捉真实世界的分布偏移，导致评估结果有限或误导性。需要更可靠的基准来评估模型的泛化、鲁棒性和感知与语言驱动操作任务的匹配程度。

Method: 1) 设计分层评估协议，包含渐进难度级别，针对空间泛化、物体识别和任务指令理解三个核心能力；2) 通过人类远程操作收集高多样性训练数据集，每个场景支持多个细粒度操作目标，以缩小训练-评估分布差距。

Result: 代表性VLA模型在累积扰动下表现出显著的性能下降，暴露了在场景理解和指令基础方面的持续局限性。LIBERO-X通过分层评估与多样化训练数据的结合，为VLA发展提供了更可靠的评估基础。

Conclusion: LIBERO-X通过系统性的评估和数据设计，为VLA模型提供了更可靠的基准测试框架，能够更准确地揭示模型在真实世界分布偏移下的性能表现和局限性。
...
TL;DR: SPARC是一个模块化框架，将视觉感知与推理分离，通过两阶段处理（视觉搜索定位相关区域，然后基于这些区域进行推理）来提升视觉语言模型的性能，同时支持非对称计算分配和选择性优化。
...
Method: SPARC采用模块化框架，明确分离视觉感知和推理。受大脑顺序感觉-认知处理的启发，实现两阶段流水线：首先执行显式视觉搜索以定位问题相关区域，然后基于这些区域进行推理产生最终答案。

Result: 在具有挑战性的视觉推理基准测试中，SPARC优于单体基线和强大的视觉基础方法。例如，在V* VQA基准上，SPARC将Qwen3VL-4B的准确率提高了6.7个百分点；在具有挑战性的OOD任务上，比"thinking with images"高出4.6个百分点，同时所需token预算降低了200倍。

Conclusion: SPARC通过分离感知和推理电路，实现了独立测试时扩展、非对称计算分配和选择性优化，显著提升了视觉语言模型的性能和效率，为解决视觉推理中的感知-推理纠缠问题提供了有效方案。
...
TL;DR: 该研究提出了首个专门针对部分-部分3D形状匹配的整数线性规划方法，通过几何一致性先验实现重叠区域估计和邻域保持的对应关系计算。


<details>
  <summary>Details</summary>
Motivation: 部分-部分3D形状匹配是最真实的场景（如3D扫描），但现有研究较少，主要挑战在于需要同时计算准确对应关系和未知重叠区域。

Method: 采用整数线性规划方法，利用几何一致性作为强先验，实现重叠区域的鲁棒估计和邻域保持的对应关系计算。

Result: 实验证明该方法在匹配误差和平滑度方面都能获得高质量匹配结果，且比先前方法更具可扩展性。

Conclusion: 提出的整数线性规划方法是首个专门针对部分-部分3D形状匹配的解决方案，能有效处理该场景的独特挑战。
...
TL;DR: DAVE是一种基于ViT梯度分解的数学基础归因方法，通过分离架构诱导的伪影和稳定组件，为Vision Transformers生成高分辨率归因图。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers已成为计算机视觉的主流架构，但为其生成稳定且高分辨率的归因图仍然具有挑战性。架构组件如补丁嵌入和注意力路由会在像素级解释中引入结构化伪影，导致现有方法只能依赖粗糙的补丁级归因。

Method: DAVE基于输入梯度的结构化分解，利用ViT的架构特性，分离出输入-输出映射中局部等变且稳定的组件，将其与架构诱导的伪影和其他不稳定源分离开来。

Result: 该方法能够生成稳定且高分辨率的归因图，解决了ViT归因中常见的结构化伪影问题。

Conclusion: DAVE为Vision Transformers提供了一种数学基础的归因方法，能够有效分离架构伪影，生成更准确、稳定的像素级解释。
...
TL;DR: CauCLIP：基于因果关系的视觉语言框架，利用CLIP学习领域不变表示，用于无需目标域数据的外科阶段识别，通过频率增强和因果抑制损失提升领域泛化能力
...
Method: 提出CauCLIP框架：1）基于频率的数据增强策略扰动领域特定属性同时保留语义结构；2）因果抑制损失减轻非因果偏见并强化因果手术特征；3）统一训练框架使模型专注于手术工作流中的稳定因果因素

Result: 在SurgVisDom硬适应基准测试中，该方法显著优于所有竞争方法，证明了因果关系引导的视觉语言模型在领域可泛化手术视频理解中的有效性

Conclusion: CauCLIP通过整合因果关系原理和视觉语言模型，成功解决了外科阶段识别中的领域泛化问题，为无需目标域数据的鲁棒手术视频理解提供了有效解决方案
...
TL;DR: PlanViz是一个评估统一多模态模型在计算机使用任务中图像生成和编辑能力的新基准，专注于路线规划、工作流程图和网页UI显示三个子任务，提出了PlanScore评估指标。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在自然图像生成和多模态推理方面表现出色，但其在计算机使用规划任务（如空间推理和过程理解）中的潜力尚未充分探索，需要专门的评估基准。

Method: 设计了三个日常计算机使用子任务：路线规划、工作流程图和网页UI显示；通过人工标注问题和参考图像确保数据质量；提出任务自适应评分PlanScore来评估正确性、视觉质量和效率。

Result: 通过实验揭示了统一多模态模型在计算机使用任务中图像生成和编辑的关键局限性，为未来研究指明了机会。

Conclusion: PlanViz基准填补了统一多模态模型在计算机使用规划任务评估方面的空白，提出的PlanScore为模型性能提供了全面评估框架，有助于推动该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前医学图像分析数据集存在关键缺陷：大多数数据集要么提供单一干净的金标准（掩盖了真实世界的专家分歧），要么提供多个标注但没有独立的客观评估标准。需要弥合这一差距以推动机器学习在医学图像分析中的发展。

Method: 引入CytoCrowd数据集，包含446张高分辨率细胞学图像，每张图像都有两个关键组成部分：1) 来自四位独立病理学家的原始冲突标注；2) 由资深专家建立的独立高质量金标准。这种双重结构使其成为多功能资源。

Result: CytoCrowd数据集既可作为标准计算机视觉任务（如目标检测和分类）的基准，使用金标准进行评估；同时也可作为评估必须解决专家分歧的标注聚合算法的真实测试平台。提供了两个任务的全面基线结果。

Conclusion: 实验证明了CytoCrowd带来的挑战，并确立了其作为开发下一代医学图像分析模型资源的价值。该数据集弥合了现有数据集的不足，为医学图像分析领域提供了更真实、更全面的评估框架。
...
TL;DR: 本文提出SICA方法，首次实现跨四个图像取证子领域的统一检测模型，通过语义引导的约束适应重建异质但可区分的伪影特征空间，在OpenMMSec数据集上超越15个SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 当前图像伪造检测面临跨子领域统一检测的挑战。理论上单模型方法更有前景，但实践中性能一直不如集成方法。研究发现这是由于不同子领域伪影的"异质性现象"导致特征空间崩溃，需要重建"统一但可区分"的伪影特征空间。

Method: 提出语义引导的约束适应（SICA）方法，这是首个单模型FID范式。假设高层语义可作为重建的结构先验，通过语义引导来约束特征适应，在保持统一性的同时区分不同子领域的伪影特征。

Result: 在OpenMMSec数据集上的大量实验表明，SICA超越了15个最先进的方法，并以近乎正交的方式重建了目标统一但可区分的伪影特征空间，有力验证了假设。

Conclusion: 通过发现异质性现象并诊断特征空间崩溃问题，提出的SICA方法成功解决了单模型FID的性能瓶颈，为实际取证场景中的统一检测提供了有效解决方案。
...
TL;DR: ScoliGait：一个新的多模态基准数据集和框架，用于通过视频步态分析进行青少年特发性脊柱侧弯评估，解决了现有方法的数据泄漏和可解释性问题。


<details>
  <summary>Details</summary>
Motivation: 青少年特发性脊柱侧弯（AIS）的早期检测可以减缓其进展，但传统筛查方法主观、难以扩展且依赖专业临床知识。基于视频的步态分析是有前景的替代方案，但现有数据集和方法存在数据泄漏问题（同一受试者的重复片段导致性能虚高），且模型过于简化、缺乏临床可解释性。

Method: 提出ScoliGait基准数据集（1,572个训练视频片段和300个完全独立的测试片段），每个片段标注了放射学Cobb角度和基于临床运动学先验的描述性文本。开发了一个多模态框架，整合了临床先验引导的运动学知识图谱用于可解释特征表示，以及潜在注意力池化机制来融合视频、文本和知识图谱模态。

Result: 该方法在现实、非重复受试者基准测试中建立了新的最先进水平，显示出显著的性能提升。为可扩展、非侵入性的AIS评估提供了稳健、可解释且临床基础扎实的框架。

Conclusion: 这项工作为可扩展、非侵入性的青少年特发性脊柱侧弯评估提供了稳健、可解释且临床基础扎实的基础，解决了现有方法的数据泄漏和可解释性问题。
...
TL;DR: 使用Sentinel-2卫星影像和预训练自编码器基础模型进行金矿勘探，通过生成式表征提升识别准确率


<details>
  <summary>Details</summary>
Motivation: 传统实地矿产勘探成本高、数据有限，需要开发基于卫星影像的大规模勘探方法，利用深度学习表征学习提高金矿识别效率

Method: 使用在FalconSpace-S2 v1.0数据集上预训练的Isometric自编码器基础模型，从多光谱Sentinel-2影像中提取信息密集的光谱-空间表征，然后输入轻量级XGBoost分类器进行金矿识别

Result: 在63个已知金矿和非金矿位置的Sentinel-2影像数据集上，该方法将补丁级准确率从0.51提升到0.68，图像级准确率从0.55提升到0.73

Conclusion: 生成式嵌入能够捕捉可迁移的矿物学模式，即使标记数据有限，基础模型表征有望使矿产勘探更高效、可扩展且全球适用
...
TL;DR: 该论文提出了一种将面部情绪识别从单标签分类问题转化为概率分布学习问题的新方法，通过将VAD空间映射到基本和复合情绪分布，自动重新标注现有数据集，从而描述复杂的情绪状态。


<details>
  <summary>Details</summary>
Motivation: 传统面部情绪识别将情绪简化为六种基本情绪的单标签分类，这过度简化了真实世界中复杂的情绪状态。实际情绪往往是多种情绪以不同强度组合的结果，现有数据集通常只标注单一情绪类别，无法反映情绪的多面性和感知的模糊性。

Method: 提出一种新颖方法，将复杂情绪状态描述为情绪类别上的概率分布。通过利用先前研究中将基本和复合情绪映射到VAD（效价-唤醒-支配）空间分布的结果，自动重新标注现有数据集。给定带有VAD值的面部图像，可以估计其属于每个情绪分布的可能性，从而将情绪状态描述为情绪的混合。

Result: 通过初步实验展示了该方法的优势，提供了新的研究方向。数据标注已在GitHub上公开（https://github.com/jbcnrlz/affectnet-b-annotation）。

Conclusion: 该方法能够丰富情绪描述，同时考虑情绪感知的模糊性，为面部情绪识别提供了更符合真实情绪复杂性的新方向，超越了传统的单标签分类方法。
...
TL;DR: 开发基于计算机视觉的自动化方法评估甘薯象鼻虫损害，替代传统人工评分，提高表型分析效率和准确性


<details>
  <summary>Details</summary>
Motivation: 甘薯象鼻虫是甘薯生产中最具破坏性的害虫之一，特别是在撒哈拉以南非洲地区。传统的人工评分方法劳动密集、主观性强且结果不一致，严重阻碍了抗虫甘薯品种的育种工作。

Method: 采用计算机视觉方法：1) 在田间环境下训练分类模型预测根部损害严重程度；2) 在实验室环境下建立数据集，使用YOLO12目标检测模型，结合根部分割和分块策略的两阶段流程来检测微小的象鼻虫取食孔。

Result: 田间分类模型测试准确率达到71.43%；实验室目标检测模型平均精度达到77.7%，能够有效识别微小的象鼻虫取食孔。

Conclusion: 计算机视觉技术能够提供高效、客观、可扩展的评估工具，与现代化育种工作流程无缝对接，显著提高甘薯育种中的表型分析效率，对减轻象鼻虫对粮食安全的负面影响具有重要作用。
...
TL;DR: 该技术笔记推导了在两个标定视图之间映射局部图像块的仿射变换的闭式表达式，该变换是相对相机位姿、图像坐标和局部表面法向量的函数。


<details>
  <summary>Details</summary>
Motivation: 在计算机视觉中，理解不同视角下局部图像块之间的几何关系对于特征匹配、三维重建等任务至关重要。传统方法通常需要复杂的数值计算，作者希望找到一种更简洁、解析的解决方案。

Method: 通过几何推导，建立了局部图像块在标定相机两个视图之间的仿射变换模型。该变换由相对相机位姿（旋转和平移）、图像坐标以及局部表面法向量共同决定。

Result: 成功推导出了仿射变换的闭式表达式，表明该变换可以解析地表示为相对相机位姿、图像坐标和局部表面法向量的函数。

Conclusion: 该闭式表达式为计算机视觉中的几何变换提供了理论基础，可以用于改进特征匹配、三维重建等算法的效率和精度。
...
TL;DR: RAIGen是首个用于扩散模型中无监督罕见属性发现的框架，能够识别训练数据中代表性不足的社会、文化或风格属性，并支持在生成过程中针对性增强这些属性。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么专注于预定义公平性类别的偏见缓解，要么识别主导输出的多数属性，但都忽略了发现数据分布中代表性不足但仍编码在模型表示中的罕见或少数属性这一互补任务。

Method: RAIGen使用Matryoshka稀疏自编码器和结合神经元激活频率与语义独特性的新型少数度量，来识别可解释的神经元，这些神经元的顶部激活图像揭示了代表性不足的属性。

Result: 实验表明RAIGen能在Stable Diffusion中发现超越固定公平性类别的属性，可扩展到SDXL等更大模型，支持跨架构的系统性审计，并能在生成过程中实现罕见属性的针对性增强。

Conclusion: RAIGen为扩散模型中的罕见属性发现提供了首个无监督框架，能够识别和增强训练数据中代表性不足的社会、文化和风格属性，超越了传统公平性方法的限制。
...
TL;DR: GaussianPOP：基于分析高斯误差量化的3D高斯泼溅简化框架，通过精确测量每个高斯对渲染图像的贡献，实现模型紧凑性与渲染质量的最佳平衡


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅简化方法通常使用重要性评分（如混合权重或敏感度）来识别冗余高斯，但这些评分并非基于视觉误差指标，导致在紧凑性和渲染保真度之间难以达到最优权衡

Method: 提出基于分析高斯误差量化的简化框架，核心贡献是从3DGS渲染方程直接推导出的新颖误差准则，能精确测量每个高斯对渲染图像的贡献。引入高效算法，使误差计算在单次前向传播中完成。框架支持训练中剪枝和训练后简化，通过迭代误差重量化提高稳定性

Result: 实验结果表明，该方法在两种应用场景下均优于现有最先进的剪枝方法，实现了模型紧凑性和高渲染质量之间的优越权衡

Conclusion: GaussianPOP提供了一个准确且灵活的简化框架，通过精确的误差量化实现了3D高斯泼溅模型的高效简化，在保持高质量渲染的同时显著减少模型复杂度
...
TL;DR: 提出PKA框架，通过位置对齐和关键词范围注意力机制，解决DiT模型中多条件控制的计算效率问题，实现10倍推理加速和5.1倍显存节省。


<details>
  <summary>Details</summary>
Motivation: 现代文生图模型在基于提示生成方面表现出色，但缺乏对空间布局或主体外观的细粒度控制。多条件控制可以解决这一问题，但在扩散变换器中的集成受到传统"连接并关注"策略的限制，该策略在条件数量增加时存在二次计算和内存开销。

Method: 提出PKA框架：1) 位置对齐注意力通过强制局部补丁对齐来线性化空间控制；2) 关键词范围注意力通过语义感知掩码修剪不相关的主体驱动交互；3) 条件敏感性感知采样策略重新加权训练目标，加速收敛并增强条件保真度。

Result: PKA实现了10.0倍的推理加速和5.1倍的VRAM节省，为高保真多条件生成提供了可扩展且资源友好的解决方案。

Conclusion: PKA框架通过消除跨模态交互中的空间和语义冗余，显著提高了多条件控制扩散变换器的效率和性能，为细粒度可控图像生成提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的参数高效微调方法在复杂密集预测任务中存在局限性，包括输入无关建模和跨层表示冗余。需要一种能够生成输入依赖的定制化特征表示，同时促进跨层特征交互的方法。

Method: 提出AdaRoute方法，采用混合专家架构，包含共享专家中心（每个专家是可训练参数矩阵）。在前向传播过程中，每个AdaRoute模块通过动态参数路由机制，从对应的专家中心选择性地聚合参数矩阵，为当前模块动态生成输入依赖的权重矩阵。

Result: 在多种视觉任务上的广泛实验表明AdaRoute的优越性，包括语义分割、目标检测和实例分割，以及全景分割任务。动态权重矩阵实现了低秩自适应，生成更定制化和强大的特征表示。

Conclusion: AdaRoute通过简单的混合专家架构和动态参数路由机制，在密集预测任务中实现了参数高效微调，解决了现有方法的局限性，生成了更定制化和强大的特征表示，同时促进了跨层特征交互。
...
TL;DR: RFDM是一种因果高效的视频编辑模型，通过逐帧编辑可变长度视频，基于2D图像扩散模型进行改造，利用视频时间冗余性，专注于连续帧之间的变化，计算效率与图像模型相当且独立于输入视频长度。


<details>
  <summary>Details</summary>
Motivation: 现有教学视频编辑方法大多需要固定长度输入和大量计算资源，而自回归视频生成虽然能高效合成可变长度视频，但在视频编辑领域应用不足。需要一种既能处理可变长度视频，又计算高效的编辑方法。

Method: 从2D图像到图像扩散模型出发，通过条件化当前时间步编辑于前一预测结果，将其改造为视频到视频编辑模型。提出新的I2I扩散前向过程公式，鼓励模型预测目标输出与先前预测之间的残差（残差流扩散模型RFDM），使去噪过程专注于连续帧之间的变化。

Result: 在全局/局部风格迁移和对象移除的配对视频数据上训练，RFDM超越了基于I2I的方法，与完全时空（3D）V2V模型竞争，同时计算效率与图像模型相当，且独立于输入视频长度扩展。

Conclusion: RFDM提供了一种高效、可扩展的视频编辑方法，能够处理可变长度视频，计算成本低，为自然语言控制的视频编辑提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着大规模文本到图像扩散模型的视觉质量不断提升，其规模增长导致最先进模型与设备端解决方案之间的差距扩大。需要开发能在移动设备上高效运行的高质量文本到图像生成模型。

Method: 采用渐进式压缩流水线：1) 通过剪裁扩散变换器中的冗余组件将模型从12B压缩到2B；2) 使用基于ResNet的令牌下采样机制，允许中间块在低分辨率令牌上操作以降低延迟；3) 提出新颖的文本编码器蒸馏方法，利用去噪器早期层的视觉信号。

Result: NanoFLUX能在移动设备上约2.5秒内生成512x512图像，证明了高质量设备端文本到图像生成的可行性。

Conclusion: NanoFLUX通过创新的压缩和蒸馏技术成功缩小了最先进模型与设备端解决方案之间的差距，为移动设备上的高质量文本到图像生成提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统无监督分割方法通常只寻找最显著对象，结果对初始化、种子顺序和阈值启发式敏感，缺乏稳定性和可控性。需要一种能够利用少量标注信息来获得稳定、可复现分割结果的方法。

Method: 基于TokenCut方法，在token-token亲和力图中加入少量与锚节点耦合的先验信息。通过操纵图拓扑结构，将光谱特征空间偏向与标注一致的划分。该方法保留了密集自监督视觉特征强制执行的全局分组，用标注标记换取可复现性、用户控制和分割质量的显著提升。

Result: 每个数据集仅使用5-30个标注，在DUTS-TE、ECSSD、MS COCO等标准基准测试中达到弱监督和无监督方法的最先进性能。在CrackForest数据集上达到96.8% mIoU（比SotA提升14.43%），CUB-200-2011达到78.0%（+0.2%），HAM10000达到78.8%（+0.37%）。在多对象基准测试中展示出明确、用户可控的语义分割能力。

Conclusion: PANC框架通过少量标注实现了稳定、可控、可复现的对象分割，在密集标注成本高或类内差异细微的领域中表现优异，为弱监督分割提供了有效的解决方案。
...
TL;DR: VLLMs在细粒度视觉信息和空间推理任务上表现不佳，本文通过构建合成基准数据集和度量视觉冗余，探究任务复杂度与视觉压缩之间的关系，发现高复杂度视觉数据比例对改善VLLMs视觉表示分布至关重要。


<details>
  <summary>Details</summary>
Motivation: 视觉大语言模型（VLLMs）的视觉能力一直落后于其语言能力，特别是在需要细粒度视觉信息或空间推理的任务上表现不佳。目前尚不清楚VLLMs在这些任务上表现差的确切原因，有研究认为视觉冗余可能是解释之一，即高层视觉信息均匀分布在众多token中，而特定的细粒度视觉信息被丢弃。

Method: 1. 构建一个简单的合成基准数据集，专门设计用于探测各种视觉特征；2. 开发一套度量视觉冗余的指标；3. 在多个复杂视觉任务上对VLLMs进行微调，研究冗余和压缩如何根据训练数据的复杂度而变化。

Result: 研究发现任务复杂度与视觉压缩之间存在关联，这意味着拥有足够比例的高复杂度视觉数据对于改变VLLMs分布其视觉表示的方式至关重要，从而能提高其在复杂视觉任务上的性能。

Conclusion: 本文揭示了VLLMs中视觉信息处理的关键机制，特别是任务复杂度与视觉压缩之间的关系，为训练下一代VLLMs提供了有价值的见解，强调了高复杂度视觉数据在改善模型视觉能力中的重要性。
...
TL;DR: 提出一个医学影像数据集的错误标签检测框架，在胶囊内镜数据集上验证，通过专家重新标注确认，提升了异常检测性能


<details>
  <summary>Details</summary>
Motivation: 医学影像标注困难且昂贵，专家数量有限，类别边界模糊，导致数据集存在错误标签问题，影响深度学习模型性能

Method: 开发了一个医学数据集错误标签检测框架，在两大公开胶囊内镜数据集上验证，由三位经验丰富的胃肠病专家重新标注被识别为可能错误的样本

Result: 框架成功检测出错误标签数据，清洗后的数据集相比现有基线方法，在异常检测性能上有所提升

Conclusion: 提出的错误标签检测框架能有效识别医学数据集中的标注错误，通过数据清洗可提高下游任务的性能，对医学影像分析有重要意义
...
TL;DR: CineScene框架通过解耦场景上下文实现电影级视频生成，使用隐式3D感知场景表示和上下文条件机制，在保持场景一致性的同时合成动态主体视频


<details>
  <summary>Details</summary>
Motivation: 电影视频制作需要控制场景-主体构图和摄像机运动，但实景拍摄成本高昂，因为需要构建物理场景。现有方法难以在保持场景一致性的同时生成动态主体视频

Method: 提出CineScene框架，利用隐式3D感知场景表示进行电影视频生成。核心创新是新颖的上下文条件机制：通过VGGT将场景图像编码为视觉表示，通过额外上下文连接将空间先验注入预训练的文本到视频生成模型，实现摄像机控制的视频合成。训练时采用随机打乱输入场景图像策略增强鲁棒性，并使用Unreal Engine 5构建场景解耦数据集

Result: 实验表明CineScene在场景一致的电影视频生成方面达到最先进性能，能够处理大范围摄像机运动，并在多样化环境中展示良好的泛化能力

Conclusion: CineScene通过解耦场景上下文和隐式3D特征注入，实现了高质量、场景一致的电影视频生成，为低成本电影制作提供了有效解决方案
...
Method: 采用三阶段训练策略：1)跨模态预训练对齐异构视觉编码器与医学语言骨干；2)指令调优覆盖多种医学任务；3)强化学习结合事实性检查和边界框奖励来增强空间定位和逐步推理能力。

Result: 在VQA基准测试中平均准确率比基线提升13.7%，接近SOTA模型Fleming-VL；文本QA提升6.9%；医学报告生成在语义和临床准确性上均有显著提升；空间定位能力IoU提升40.4%，在放射学、眼科学和病理学等多个医学领域展现出强大的跨模态泛化能力。

Conclusion: MedMO通过专门针对医学领域设计的训练策略，在多模态医学任务上取得了显著性能提升，特别是在空间定位和推理能力方面表现突出，为医学AI应用提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 广告疲劳降低了重复口号的效果，需要新颖、有创意、有洞察力的口号生成。现有LLM方法往往产生风格冗余的输出，缺乏清晰的品牌个性，显得过于机器生成。有效口号需要在新颖性和熟悉感之间取得平衡。

Method: 提出将人物相关的名人名言重新语境化用于口号生成的新范式。名人名言自然符合口号长度，使用丰富的修辞手法，提供深度和洞察力。技术上引入模块化框架，将口号生成分解为可解释的子任务：名言匹配、结构分解、词汇替换和混搭生成。

Result: 广泛的自动和人工评估显示，相比三种最先进的LLM基线方法，在多样性、新颖性、情感影响力和人类偏好方面都有边际改进。

Conclusion: 通过重新语境化名人名言来生成广告口号是一种有效的方法，能够在保持熟悉感的同时提供新颖性和创意深度，模块化框架提供了可解释的生成过程。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型缺乏特定实体的详细知识，而现有的检索增强生成解码方法未能充分利用多个相关上下文，也无法有效抑制不相关上下文的负面影响。

Method: 提出相关性感知的多上下文对比解码方法，通过基于每个上下文与问题相关性的加权组合来生成最终预测，有效聚合多个相关上下文的有用信息并抵消不相关上下文的影响。

Result: 在多个知识密集型视觉问答基准测试中，RMCD始终优于其他解码方法，在从最弱到最强的各种检索结果下都表现最佳，且无需额外训练即可直接应用于现有模型。

Conclusion: RMCD是一种简单有效的解码方法，能够显著提升检索增强生成在视觉问答任务中的性能，对检索结果具有鲁棒性，且易于部署应用。
...
TL;DR: CAST：基于戏剧理论的人物与场景记忆架构，通过构建3D场景（时间/地点/主题）并组织成人物档案来表示情景记忆，结合图语义记忆形成双记忆系统，显著提升对话问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有智能体记忆系统主要关注语义回忆，将经验表示为键值对、向量或图等结构，难以表示和检索连贯的事件（情景记忆）。人类的情景记忆能够回忆基于"谁、何时、何地"的连贯事件，而现有系统在这方面存在不足。

Method: 提出CAST（Character-and-Scene based memory architecture）记忆架构，受戏剧理论启发：1）构建3D场景（时间/地点/主题）并组织成人物档案来总结人物事件，表示情景记忆；2）补充基于图的语义记忆，形成稳健的双记忆设计。

Result: 实验表明，CAST在各种数据集上平均比基线方法提升8.11%的F1分数和10.21%的J（LLM-as-a-Judge）分数，特别是在开放性和时间敏感的对话问题上表现更优。

Conclusion: CAST通过戏剧理论启发的人物与场景记忆架构，有效解决了智能体记忆系统中情景记忆表示和检索的挑战，结合语义记忆形成双记忆系统，显著提升了对话问答性能。
...
TL;DR: 这是一篇关于AI智能体记忆系统的综述论文，探讨了AI研究范式从追求基准分数转向解决实际问题时，记忆作为填补效用差距的关键解决方案。


<details>
  <summary>Details</summary>
Motivation: AI研究正经历从追求模型创新和基准分数到强调问题定义和真实世界评估的范式转变。随着领域进入"下半场"，核心挑战在于在长期、动态、用户依赖的环境中实现真正的效用，智能体面临上下文爆炸问题，需要持续积累、管理和选择性重用大量信息。

Method: 论文从三个维度提供了基础智能体记忆的统一视图：记忆基质（内部和外部）、认知机制（情景、语义、感官、工作、程序记忆）和记忆主体（智能体中心与用户中心）。分析了不同智能体拓扑下记忆的实例化和操作方式，并强调了记忆操作的学习策略。

Result: 论文综述了记忆评估的基准和指标，用于评估记忆效用，并概述了各种开放挑战和未来方向。记忆被认为是填补AI智能体在复杂环境中效用差距的关键解决方案。

Conclusion: 记忆系统是AI智能体在长期、动态、用户依赖环境中实现真正效用的关键。论文提供了统一的记忆框架，分析了不同维度的记忆特性，并指出了评估方法和未来研究方向，为AI智能体记忆系统的发展提供了理论基础。
...
TL;DR: 提出一个基于文献感知的新颖性评估框架，通过机器学习从同行评审报告中学习人类如何判断新颖性，并基于结构化对比现有研究进行客观评估。


<details>
  <summary>Details</summary>
Motivation: 研究新颖性评估是同行评审的核心但高度主观的环节，通常基于隐含判断和不完整的先前工作比较。需要更客观、一致的新颖性评估方法。

Method: 使用近8万条来自顶级AI会议的新颖性标注评审报告，微调大语言模型以捕捉与评审者一致的新颖性评估行为。对给定稿件提取其思想、方法和主张的结构化表示，检索语义相关论文，构建相似性图进行细粒度概念级比较。

Result: 系统能够产生校准的新颖性分数和类人解释性评估，相对于现有方法减少了高估并提高了评估一致性。

Conclusion: 该文献感知的新颖性评估框架通过结构化证据支持，能够提供更客观、一致的新颖性评估，减少主观偏差，提高同行评审质量。
...
TL;DR: 提出新的标注极化度量方法，用于分析不同社会人口学群体在主观标注任务中的分歧，特别适用于仇恨言论和毒性检测等任务。


<details>
  <summary>Details</summary>
Motivation: 现有标注一致性度量方法不适合群体间分析，对群体规模不平衡敏感，且仅限于单标注设置，无法满足毒性检测、仇恨言论识别等主观任务的需求。

Method: 引入可量化的极化度量指标，配合统计显著性检验，能够将极化归因于不同的标注者群体。该指标支持严重不平衡的社会人口学和意识形态子群体之间的直接比较，并适用于多标签设置。

Result: 在三个仇恨言论数据集和一个毒性检测数据集上应用该指标发现：1）极化强烈且持续地归因于标注者种族，尤其在仇恨言论任务中；2）宗教标注者之间没有根本分歧，但与其他标注者存在分歧，这种趋势在非宗教标注者中逐渐减弱并逆转；3）教育程度较低的标注者更主观，而教育程度较高的标注者之间更一致。

Conclusion: 新提出的极化度量方法能够有效分析不同标注者群体在主观任务中的分歧模式，反映了当前关于各子群体标注模式的研究发现。同时估算了获得稳健结果所需的最小标注者数量，并提供了开源Python库实现。
...
TL;DR: COVER提出了一种新的可撤销解码方法，通过KV缓存覆盖进行留一验证和稳定草稿生成，减少不必要的修订，加速扩散语言模型推理。


<details>
  <summary>Details</summary>
Motivation: 现有并行扩散解码方法在提高推理速度时会损害质量，而可撤销解码的验证方案经常触发"翻转振荡"问题，导致标记被重新掩码后又恢复原状，这既削弱了并行草稿生成的上下文条件，又浪费了修订预算。

Method: COVER采用KV缓存覆盖技术：1) 通过留一验证进行验证，将选定的种子标记掩码；2) 同时将这些种子的缓存键值状态注入所有其他查询以保留上下文信息；3) 使用闭式对角线校正防止种子位置的自泄漏；4) 使用平衡不确定性、下游影响和缓存漂移的稳定性感知分数优先选择种子；5) 自适应调整每步验证的种子数量。

Result: 在多个基准测试中，COVER显著减少了不必要的修订，在保持输出质量的同时实现了更快的解码速度。

Conclusion: COVER通过创新的KV缓存覆盖验证机制，有效解决了现有可撤销解码中的翻转振荡问题，实现了更高效的并行扩散语言模型推理。
...
TL;DR: 量化后训练会改变大语言模型的社会偏见模式，虽然聚合指标可能不变，但存在量化诱导的偏见翻转现象，导致不同人口群体受到不对称影响。


<details>
  <summary>Details</summary>
Motivation: 量化后训练虽然降低了大型语言模型的计算成本，但会从根本上改变其社会偏见，而现有的聚合指标无法捕捉这些变化。需要系统研究量化如何影响模型偏见，特别是不同人口群体受到的不对称影响。

Method: 对50个量化模型进行大规模研究，使用PostTrainingBiasBench基准测试（包含13个封闭式和开放式偏见数据集）。分析量化强度（4位vs8位）、模型不确定性对偏见变化的影响，以及不同人口群体的不对称影响。

Result: 发现量化诱导的偏见翻转现象：高达21%的响应在量化后从偏见状态翻转为无偏见状态（或反之），尽管聚合偏见分数不变。高不确定性响应比自信响应更容易变化（3-11倍）。4位量化模型比8位模型表现出4-6倍的行为变化。偏见变化对人口群体产生不对称影响：某些群体的偏见恶化高达18.6%，而其他群体改善14.1%，导致误导性的中性聚合结果。

Conclusion: 压缩从根本上改变了偏见模式，需要关键的量化后评估和干预措施以确保实际应用的可靠性。更大的模型没有一致的鲁棒性优势，群体特异性变化在不同模型家族中不可预测。
...
TL;DR: BenchMarker工具包使用LLM法官检测多选题基准中的三种常见缺陷：污染、捷径和写作错误，揭示这些缺陷影响NLP评估准确性，并发现现有修复方法可能引入新问题。


<details>
  <summary>Details</summary>
Motivation: 当前NLP中的多选题基准缺乏严格的质量控制，存在各种缺陷，这些缺陷会降低评估的可靠性。作者希望借鉴教育研究的方法来改进多选题基准的设计。

Method: 开发BenchMarker工具包，使用LLM法官检测三种多选题缺陷：1) 污染 - 题目完全出现在网上；2) 捷径 - 选项中的线索使猜测成为可能；3) 写作错误 - 基于19条教育评分标准的结构/语法问题。通过人工标注验证工具，然后对12个基准进行审计。

Result: 审计发现：1) 被污染的题目倾向于提高准确率；2) 写作错误倾向于降低准确率并改变排名；3) 现有的基准修复方法虽然解决了目标问题（如使用LLM编写的干扰项降低准确率），但无意中引入了新缺陷（如不合理的干扰项、多个正确答案）。

Conclusion: 多选题中的缺陷会降低NLP评估质量，但教育研究提供了改进路径。作者发布BenchMarker工具包以连接两个领域并改进多选题基准设计。
...
TL;DR: 研究发现，通过仅提供单方面论据，可以简单直观地引导大语言模型在争议性问题上转向特定立场，这种观点引导在不同模型、论据数量和主题中都存在。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在争议性问题上可以提供平衡回答，但也会采取单一对齐观点或拒绝回答。本研究旨在探索是否可以通过仅提供单方面论据来简单直观地引导模型转向特定观点。

Method: 构建小型数据集，从三个维度进行系统研究：(i) 模型响应中引发的立场，(ii) 争议性问题的表述方式，(iii) 论据的展示方式。测试不同模型、论据数量和主题下的观点引导效果。

Result: 研究发现观点引导在不同模型、论据数量和主题中都显著存在。切换为其他论据会一致性地降低观点引导效果。

Conclusion: 仅通过提供单方面论据就可以有效引导大语言模型在争议性问题上的观点，这表明模型对输入论据的敏感性，也揭示了潜在的观点操纵风险。
...
TL;DR: MPIB是一个医疗提示注入基准测试套件，用于评估LLM和RAG系统在临床环境中的安全性，重点关注直接和间接提示注入攻击下的临床伤害事件率。
...
Method: 开发了包含9,697个实例的MPIB基准测试套件，采用多阶段质量控制和临床安全检查，引入临床伤害事件率(CHER)和攻击成功率(ASR)指标来区分指令遵从性和患者风险。

Result: 评估显示ASR和CHER存在显著差异，鲁棒性关键取决于对抗性指令出现在用户查询还是检索上下文中，攻击成功率与临床伤害风险不完全相关。

Conclusion: MPIB为临床提示注入研究提供了可重复的系统评估框架，强调了在临床环境中需要同时考虑攻击成功率和实际临床伤害风险的重要性。
...
TL;DR: VowelPrompt是一个基于语言学原理的框架，通过提取元音级别的韵律特征（音高、能量、时长）并将其转化为自然语言描述，增强LLM在语音情感识别中的能力，同时提高可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在语音情感识别中主要依赖文本转录，忽略了精细的韵律信息（如音高、强度、时间动态），这限制了模型的有效性和可解释性。需要一种能够同时处理语言内容和声音表达性的多模态方法。

Method: 1. 基于语音学证据，提取时间对齐的元音段的音高、能量和时长特征；2. 将这些特征转化为自然语言描述；3. 采用两阶段适应过程：监督微调（SFT）和带可验证奖励的强化学习（RLVR），通过组相对策略优化（GRPO）实现，以增强推理能力、确保结构化输出并提高跨领域和说话人变化的泛化能力。

Result: 在多个基准数据集上的广泛评估表明，VowelPrompt在零样本、微调、跨领域和跨语言条件下，始终优于最先进的情感识别方法，同时能够生成基于上下文语义和精细韵律结构的可解释解释。

Conclusion: VowelPrompt通过将精细的元音级别韵律特征与LLM的语义推理能力相结合，显著提升了语音情感识别的性能和可解释性，为多模态情感分析提供了新的语言学基础框架。
...
TL;DR: 提出基于后果的效用评估方法，无需专家验证，通过测试候选方案在解决相关可验证问题中的价值来评分，在数学推理任务上优于现有评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前推理模型在生成研究级数学问题的解决方案方面取得进展，但验证环节消耗大量专家时间成为瓶颈。作者认为，一个真正有效的解决方案应包含足够的方法级信息，使其在解决相关问题时能比错误方案产生更好的下游性能。

Method: 提出基于后果的效用评估方法：将每个候选方案作为上下文示例，测试其在解决相关但可验证问题中的价值。该方法无需专家验证，通过评估候选方案作为示例时的下游性能来评分。

Result: 在原创的研究级数学问题集上评估，每个问题配有一个专家编写的解决方案和九个LLM生成的解决方案。基于后果的效用评估在排名质量上一致优于奖励模型、生成奖励模型和LLM评判者。对于GPT-OSS-120B，将Acc@1从67.2提高到76.3，AUC从71.4提高到79.6；对于GPT-OSS-20B，AUC从69.0提高到79.2。相比LLM评判者，该方法展现出更大的求解器-评估器差距，即使在底层求解器经常失败的实例上也能保持更强的正确-错误区分能力。

Conclusion: 基于后果的效用评估方法为解决研究级数学推理中的验证瓶颈提供了有效途径，无需专家参与即可准确评估解决方案质量，显著优于现有评估方法，为自动化数学推理系统的发展提供了重要工具。
...
TL;DR: 本文研究了后训练如何提升大语言模型的因果推理能力，通过CauGym数据集和五种后训练方法，证明小模型经过适当后训练能在因果推理任务上超越大模型。
...
Method: 引入CauGym数据集（包含7个核心因果任务和5个测试集），系统评估五种后训练方法：SFT、DPO、KTO、PPO和GRPO，在多个基准上进行实验验证。

Result: 适当后训练使小模型在因果推理任务上表现优异，甚至超越大模型。14B参数模型在CaLM基准上达到93.5%准确率（OpenAI o3为55.4%），且后训练模型在分布偏移和噪声数据等现实条件下表现出强泛化性和鲁棒性。

Conclusion: 这是首个系统性证据表明，有针对性的后训练可以产生可靠且鲁棒的基于大语言模型的因果推理器，为开发实用的因果推理工具提供了新途径。
...
TL;DR: 本文系统比较了两种文本分类范式：零样本/少样本提示的大语言模型与完全微调的编码器架构。研究发现微调编码器在性能相当或更优的情况下，成本和延迟低1-2个数量级。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在开放推理任务上表现出色，但在结构化文本分类任务中，模型选择往往只关注预测性能，忽略了生产系统中的操作约束（如延迟、成本）。需要系统比较不同范式的综合表现。
...
Result: 微调的编码器模型在分类性能上具有竞争力且通常更优，同时相比零样本/少样本LLM提示，其成本和延迟低1-2个数量级。大语言模型在标准文本分类任务中的不加选择使用可能导致次优的系统级结果。

Conclusion: 对于结构化NLP流水线，微调编码器是稳健高效的组件，而大语言模型更适合作为混合架构中的补充元素。研究强调了成本感知的NLP系统设计的重要性，并发布了所有代码、数据集和评估协议以支持可重复性。
...
TL;DR: ReBeCA框架通过因果分析揭示自反思行为层次结构，发现只有少数语义行为具有因果性，过多"积极"行为反而损害反思效果。


<details>
  <summary>Details</summary>
Motivation: 当前自反思机制分析多基于相关性，缺乏可解释性和泛化性，需要因果分析方法来揭示真正影响自反思结果的机制。

Method: 提出ReBeCA框架，将自反思轨迹建模为因果图，采用三阶段不变因果预测(ICP)流程，分离出真正影响性能的决定因素。

Result: 发现三个关键结论：1)语义行为对自反思结果存在层次性影响；2)只有少数语义行为具有因果泛化性；3)过多"积极"语义行为会损害反思效果。ICP验证显示稀疏因果父节点带来49.6%结构似然增益，干预研究证实因果关系具有分布外稳定性。

Conclusion: ReBeCA为解耦自反思动态中的真实因果机制与虚假关联提供了严谨方法学，揭示了自反思行为的可解释层次结构。
...
TL;DR: FMBench是一个用于评估大语言模型Markdown格式输出能力的基准测试，研究提出通过监督微调和强化学习相结合的方法来提升模型在保持语义准确性的同时满足复杂格式约束的能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在用户界面和系统集成工作流中部署时，需要同时满足语义意图和格式约束。Markdown格式在助手、文档和工具增强流程中无处不在，但容易出现难以检测的细微错误（如损坏的列表、格式错误的表格、不一致的标题和无效的代码块），这些错误会显著降低下游可用性。

Method: 提出FMBench基准测试来评估模型在各种指令跟随场景下的Markdown格式输出能力。采用轻量级对齐流程，结合监督微调（SFT）和强化学习微调。首先在指令-响应对上进行SFT，然后优化一个平衡语义保真度和结构正确性的复合目标。

Result: 在两个模型家族（OpenPangu和Qwen）上的实验表明，SFT持续改善语义对齐，而强化学习在具有挑战性的Markdown指令上提供了额外的鲁棒性增益。结果还揭示了语义目标和结构目标之间的内在权衡，强调了精心设计奖励对于可靠格式生成的重要性。

Conclusion: FMBench基准测试为评估和改进大语言模型的Markdown格式输出能力提供了有效工具，结合SFT和强化学习的对齐方法能够显著提升模型在复杂格式约束下的表现，但需要仔细平衡语义和结构目标。
...
TL;DR: SureLock是一种用于掩码扩散语言模型的高效采样方法，通过锁定已稳定位置来减少计算开销，将每步计算复杂度从O(N²d)降低到O(MNd)，在LLaDA-8B上减少30-50%的FLOPs


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型在迭代采样时，即使许多未掩码的标记已经稳定，仍然为每个标记位置重新计算注意力和前馈块，导致大量计算浪费

Method: 提出SureLock方法：当未掩码位置的后验分布在多个步骤中稳定时（sure条件），锁定该位置，跳过其查询投影和前馈子层，同时缓存其注意力键和值，以便其他位置继续关注它

Result: 在LLaDA-8B模型上，SureLock相比无锁定的相同采样器减少30-50%的算法FLOPs，同时保持相当的生成质量

Conclusion: SureLock通过动态锁定已稳定标记位置，显著提高了掩码扩散语言模型的采样效率，理论分析表明仅监控锁定步骤的局部KL散度就足以限制最终标记概率的偏差
...
TL;DR: 提出HOMER框架，基于GTVH幽默理论，通过多角色LLM协作和幽默检索，为图像生成有趣且脚本对立的幽默字幕，在New Yorker Cartoon数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多模态幽默生成（如图像幽默字幕生成）对LLMs具有挑战性，需要视觉理解、幽默推理和创造性想象。现有基于LLM的方法依赖推理链或自我改进，存在创造性和可解释性有限的问题。

Method: 提出基于GTVH幽默理论的HOMER框架，包含三个LLM角色：1) 冲突脚本提取器，基于关键脚本对立构建幽默基础；2) 检索增强的分层想象器，识别幽默目标并通过结构化想象树扩展创意空间；3) 字幕生成器，基于获取的知识生成有趣多样的字幕。

Result: 在两个New Yorker Cartoon基准数据集上的广泛实验表明，HOMER在多模态幽默字幕生成任务上优于最先进的基线方法和强大的LLM推理策略。

Conclusion: HOMER框架通过整合幽默理论、多角色LLM协作和检索增强，有效解决了多模态幽默生成的挑战，在创造性和可解释性方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有越狱技术大多未能有效利用先前交互轮次中揭示的漏洞，导致攻击效率低下且不稳定。由于越狱涉及顺序交互，强化学习为此问题提供了自然框架。

Method: 提出了历史感知的基于强化学习的越狱框架，包含基于注意力的重新加权机制，突出交互历史中的关键漏洞，实现更高效的探索。

Result: 在AdvBench和HarmBench上的广泛实验表明，该方法实现了最先进的越狱性能，同时显著提高了查询效率。
...
TL;DR: 大语言模型在语义关系推理上存在严重缺陷：虽然能识别相关关系（86.5-100%准确率），但在判断无关关系时表现极差（0-41.35%），且在不同领域数据集上准确率进一步降至约2%。


<details>
  <summary>Details</summary>
Motivation: 现有评估很少测试大语言模型区分有意义语义关系与真正无关关系的能力。研究者认为无关关系推理是LLM评估和安全的关键但未被充分评估的前沿领域。

Method: 引入CORE数据集，包含22.5万个多选题，涵盖74个学科；同时创建包含203个严格验证问题的开源基准，覆盖24种语义关系类型，并确保无关关系对等比例。收集1000+参与者建立人类基线，评估29个最先进LLM的表现。

Result: 人类基线准确率达92.6%（无关关系对95.1%）。LLM整体准确率48.25-70.9%，相关关系识别接近天花板（86.5-100%），但无关关系判断严重退化（0-41.35%）。预期校准误差在无关关系上增加2-4倍，平均语义崩溃率37.6%。在22.5万题数据集上准确率降至约2%。

Conclusion: 大语言模型在无关关系推理上存在系统性缺陷，会生成虚假关系。这揭示了LLM在领域特定语义推理中的重大挑战，无关关系推理是LLM评估和安全的关键评估前沿。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在精神病学决策支持中存在幻觉和浅层推理问题，特别是轻量级模型在临床部署中面临隐私和效率需求，现有训练范式过于注重语言流畅性而忽视结构化临床逻辑，与专业诊断认知存在根本性不匹配。

Method: 引入ClinMPO强化学习框架，使用基于循证医学原则构建的奖励模型（基于4,474篇精神病学期刊文章数据集），对齐LLM内部推理与专业精神病学实践，在专门设计的测试集上评估推理能力而非机械记忆。

Result: ClinMPO调优的Qwen3-8B模型在复杂病例诊断准确率达到31.4%，超过医学生基准的30.8%，在领先大参数LLM持续失败的测试项目上表现优异。
...
TL;DR: RelayGen是一个无需训练的分段级运行时模型切换框架，通过利用长格式推理中的难度变化，动态将低难度推理段委托给较小模型，从而在保持大模型准确性的同时显著降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务上表现优异，但生成长格式推理轨迹时推理成本高昂。现有效率优化方法要么忽略生成难度的内部变化，要么依赖监督的token级路由，系统复杂度高。需要一种能利用推理轨迹中难度变化的高效方法。

Method: RelayGen采用无需训练的分段级运行时模型切换框架：1）通过token概率边际分析生成不确定性，识别难度变化；2）发现模型特定的切换线索，标记向低难度段的过渡；3）动态将低难度推理段委托给较小模型，高难度推理保留在大模型上。

Result: 在多个推理基准测试中，RelayGen显著降低推理延迟，同时保持大模型的大部分准确性。结合推测解码技术，实现高达2.2倍的端到端加速，准确率下降小于2%，无需额外训练或学习路由组件。

Conclusion: RelayGen证明了粗粒度分段级控制在捕捉推理轨迹中难度过渡方面的有效性，提供了一种高效、无需训练的模型切换框架，在保持推理质量的同时大幅提升推理效率。
...
TL;DR: DiSPO是一种用于掩码扩散语言模型的信用分配方法，通过在中间掩码状态分支并重新采样填充来优化中间决策，无需额外的多步扩散展开。
...
TL;DR: UNO是一个从用户日志中学习以优化大语言模型的统一框架，通过将非结构化日志转化为半结构化规则和偏好对，并量化模型先验知识与日志数据之间的认知差距，自适应过滤噪声反馈，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型发展依赖于扩大训练数据和模型参数，但这种方法面临高质量数据稀缺和计算成本收益递减的约束。用户交互日志提供了丰富的真实人类反馈和程序性知识来源，但日志的非结构化和噪声特性使得学习变得困难。现有系统难以区分有用的反馈信号与噪声用户行为，且日志收集与模型优化之间的脱节（如离策略优化问题）加剧了这一挑战。

Method: UNO框架包含三个主要步骤：1) 将用户日志蒸馏为半结构化规则和偏好对；2) 使用查询-反馈驱动的聚类管理数据异质性；3) 量化模型先验知识与日志数据之间的认知差距。基于这一评估，系统自适应过滤噪声反馈，并为从用户日志中提取的主要经验和反思经验构建不同模块，从而改进未来响应。

Result: 大量实验表明，UNO在有效性和效率方面达到了最先进水平，显著优于检索增强生成（RAG）和基于记忆的基线方法。

Conclusion: UNO提供了一个有效的框架，能够从真实世界部署中的用户日志中持续学习，克服了非结构化日志数据的噪声和异质性挑战，为大语言模型系统的优化开辟了新途径。
...
TL;DR: 论文提出用沙漏形状的MLP替代传统Transformer中的窄-宽-窄MLP，通过更深但更轻的沙漏FFN结构，在相同参数预算下实现更好性能或可扩展隐藏维度。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer语言模型一直采用窄-宽-窄MLP结构，但最近研究表明残差宽-窄-宽（沙漏）MLP具有更优的函数逼近能力。作者质疑传统MLP形状的必要性，探索沙漏FFN作为替代方案。

Method: 开发Transformer变体，将传统FFN替换为更深的沙漏形状FFN，该结构由多个通过残差连接的小沙漏子MLP堆叠而成。研究在固定参数预算下，使用更轻的沙漏FFN节省的参数可以更有效地利用，例如扩大模型隐藏维度。

Result: 在400M参数规模下，沙漏FFN优于传统FFN；在1B参数规模下，两者性能相当。在相同预算下，减少FFN参数并增加注意力参数的沙漏FFN变体相比传统配置有持续改进。

Conclusion: 沙漏FFN是传统窄-宽-窄MLP的有竞争力替代方案，这些发现促使重新思考MLP结构惯例以及注意力与FFN之间的平衡，以构建更高效和表达力强的现代语言模型。
...
TL;DR: 发布了首个公开的英-希机器翻译质量评估基准MTQE.en-he，包含959个英语片段及其希伯来语机器翻译，由三位专家进行质量评分。通过集成ChatGPT、TransQuest和CometKiwi模型，性能比最佳单模型提升6.4个皮尔逊百分点和5.6个斯皮尔曼百分点。


<details>
  <summary>Details</summary>
Motivation: 为机器翻译质量评估领域提供首个公开的英-希语言对基准数据集，填补这一资源匮乏语言对的研究空白，促进相关研究发展。

Method: 构建包含959个英语片段及其希伯来语机器翻译的数据集，由三位人类专家进行直接评估评分。采用三种方法：1) ChatGPT提示工程；2) TransQuest模型；3) CometKiwi模型。进行模型集成实验，并探索参数高效微调方法（LoRA、BitFit和FTHead）。

Result: 模型集成方法相比最佳单模型（CometKiwi）在皮尔逊相关系数上提升6.4个百分点，斯皮尔曼相关系数上提升5.6个百分点。参数高效微调方法训练稳定，能带来2-3个百分点的改进，而全模型更新容易过拟合和分布崩溃。

Conclusion: MTQE.en-he基准数据集和实验结果为英-希这一资源匮乏语言对的机器翻译质量评估研究提供了重要基础，展示了模型集成和参数高效微调的有效性，为未来研究开辟了方向。
...
TL;DR: Baichuan-M3是一个医疗增强的大型语言模型，旨在从被动问答转向主动的临床级决策支持，通过专门训练流程模拟医生系统工作流，在多个医疗基准测试中表现优于GPT-5.2。


<details>
  <summary>Details</summary>
Motivation: 现有系统在开放式医疗咨询中存在局限性，需要从被动问答转向主动的临床决策支持，以更好地模拟医生系统工作流程。

Method: 采用专门训练流程模拟医生系统工作流，具备主动信息获取、长时程推理和自适应幻觉抑制三大核心能力，解决模糊信息、证据整合和事实可靠性问题。

Result: 在HealthBench、新引入的HealthBench-Hallu和ScanBench基准测试中取得最先进成果，在临床询问、咨询建议和安全性方面显著优于GPT-5.2。

Conclusion: Baichuan-M3成功实现了从被动问答到主动临床决策支持的范式转变，通过模拟医生工作流程提升了医疗AI系统的实用性和可靠性，模型已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型即使面对看似无害的提示也可能生成有毒或有害内容，这带来了严重的安全挑战。毒性通常是微妙且上下文相关的，难以在标记级别检测，而现有的毒性缓解方法往往在安全性和文本流畅性之间存在权衡。

Method: 提出了一种目标子空间干预策略，从底层模型表示中识别和抑制隐藏的毒性模式，同时保持生成安全流畅内容的能力。该方法通过干预特定的表示子空间来实现毒性缓解。

Result: 在RealToxicityPrompts数据集上，该方法相比现有基线取得了强大的缓解性能，对推理复杂度影响最小。在多个LLM上，该方法将最先进的去毒系统的毒性降低了8-20%，同时保持了可比较的流畅性。

Conclusion: 通过广泛的定量和定性分析，该方法实现了有效的毒性降低而不损害生成性能，持续优于现有基线，为LLM安全性提供了有效的解决方案。
...
Method: 1) 将评判行为建模为可学习的正则化策略；2) 从数据中心视角构建高信息密度评判数据集，注入与评估行为对齐的监督信号；3) 采用课程式SFT-DPO-GRPO训练范式，逐步对齐评分标准遵守、偏见缓解和跨模式一致性，避免灾难性遗忘。

Result: 在多个内部和公共基准测试中，FairJudge持续改进了一致性和F1分数，减少了非语义偏见，并且显著优于更大的指令调优LLM。

Conclusion: FairJudge通过将评判行为建模为可学习策略，有效解决了现有LLM-as-a-Judge系统的局限性，实现了自适应、去偏和一致的评估，为未来研究提供了有价值的框架和资源。
...
TL;DR: 该论文提出了一种利用音频特征的多模态主题分割方法，通过微调文本编码器和孪生音频编码器，在YouTube视频数据集上显著优于纯文本和多模态基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM面临安全性与有用性的根本权衡，因为静态、一刀切的安全策略缺乏运行时可控性，难以适应多样化的应用需求，导致模型可能过度拒绝良性请求或对有害请求约束不足。

Method: 提出PACT框架，采用分层策略架构：不可覆盖的全局安全策略为关键风险建立不可变边界，用户定义策略可引入领域特定风险类别并指定标签到行为的映射。框架将安全决策分解为结构化的分类→行动路径，将查询路由到适当的行动（遵从、引导或拒绝），并使决策过程透明化。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型为情感自适应人工智能提供了新机遇，但如何精确控制这些系统中的情感表达仍具挑战性。本研究旨在探索提示工程作为资源敏感且易于访问的替代方案，以控制LLM生成文本的情感倾向。

Method: 使用Ekman的六种基本情感（如喜悦、厌恶），研究多种提示技术，包括零样本提示和思维链提示（使用GPT-3.5-turbo），并与微调方法进行比较。

Result: 提示工程能有效引导AI生成文本中的情感表达，为微调提供了实用且成本效益高的替代方案，特别是在数据受限的环境中。Few-Shot提示（使用人工编写的示例）在所有技术中效果最佳，可能是因为提供了额外的任务特定指导。

Conclusion: 研究结果表明提示工程是控制大语言模型情感表达的有效方法，为开发情感自适应AI系统提供了有价值的见解，特别是在资源受限的情况下。
...
TL;DR: 该论文发现生成式奖励模型(GenRM)的推理保真度（偏好决策与参考决策理由的一致性）比标准标签准确性更能预测下游RLHF效果，提出了通过理由中心对齐(R-Align)来改善推理保真度的方法。


<details>
  <summary>Details</summary>
Motivation: 虽然生成式奖励模型(GenRM)通过生成理由来预测偏好，增强了鲁棒性，但在训练和评估中仍只关注结果标签，未检查推理质量。作者发现推理保真度对下游RLHF效果有重要影响。

Method: 重新利用现有奖励模型基准计算虚假正确率(S-Corr)，即标签正确但理由与黄金判断不一致的决策比例。提出理由中心对齐(R-Align)方法，通过添加黄金判断训练并显式监督理由对齐来改善保真度。

Result: 实证评估显示即使是竞争性GenRM也存在大量S-Corr，更高的S-Corr与优化下的策略退化相关。R-Align在奖励模型基准上减少了S-Corr，并在STEM、编码、指令遵循和通用任务上带来一致的演员性能提升。

Conclusion: 推理保真度是预测下游RLHF效果的关键指标，理由中心对齐(R-Align)能有效改善生成式奖励模型的推理质量，从而提升对齐效果。
...
TL;DR: 提出基于数据驱动构建细粒度推理错误分类法（rubrics），增强LLM在未见推理轨迹中的错误检测能力，并用于构建更强的LLM-as-judge奖励函数，在技术领域显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: LLM在推理输出验证中存在局限性：难以可靠识别思维轨迹中的错误，特别是在长输出、需要专业知识的领域以及没有可验证奖励的问题中。

Method: 采用数据驱动方法自动构建高度细粒度的推理错误分类法（rubrics），利用这些分类法增强LLM驱动的错误检测，并将其用于构建更强的LLM-as-judge奖励函数，通过强化学习训练推理模型。

Result: 基于错误分类法的分类方法在编程、数学、化学工程等技术领域表现出比基线方法更强的错误识别能力；使用这些奖励函数训练的模型在困难领域任务准确率比通用LLM-as-judge训练模型提升+45%，仅使用20%黄金标签即可接近可验证奖励训练模型的性能。

Conclusion: 该方法将奖励分类法从评估定性模型行为扩展到评估定量模型正确性，为在没有完整黄金标签数据集的情况下教授模型解决复杂技术问题开辟了新途径，这些黄金标签通常获取成本极高。
...
TL;DR: 该论文提出了一个可解释的视觉词义消歧框架，利用CLIP将歧义文本和候选图像投影到共享多模态空间，通过双通道提示和图像增强提升消歧性能。
...
Method: 开发了可解释的视觉词义消歧框架，利用CLIP将歧义文本和候选图像投影到共享多模态空间。文本嵌入通过语义和基于照片的双通道提示与WordNet同义词进行丰富，图像嵌入通过鲁棒的测试时增强进行优化，最后使用余弦相似度确定最佳匹配图像。

Result: 在SemEval-2023 VWSD数据集上评估，丰富嵌入使MRR从0.7227提升到0.7590，命中率从0.5810提升到0.6220。消融研究表明双通道提示提供强大、低延迟性能，而激进的图像增强仅带来边际收益。

Conclusion: 精确的、与CLIP对齐的提示对于视觉词义消歧最为有效，嘈杂的外部信号会稀释语义特异性。双通道提示策略在性能和效率之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 探索概念表征如何平衡共享结构（支持泛化）和正交子空间（减少任务干扰）的共存机制，理解认知系统中表征的共享与分化如何协调。

Method: 以数字概念为测试平台，使用语言模型作为高维计算基底，分析数字表征在不同任务中的几何关系结构，研究任务特定表征的子空间分布和线性变换特性。

Result: 数字表征在不同任务中保持稳定的关系结构；任务特定表征嵌入不同子空间，低层特征如大小和奇偶性沿可分离的线性方向编码；这些子空间可通过线性映射相互转换，表明表征共享关系结构。
...
TL;DR: SEMA是一个用于训练多轮越狱攻击者的框架，通过预填充自调优和意图漂移感知的强化学习，在无需外部数据的情况下实现高效的多轮攻击，在多个数据集上达到最先进的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的多轮越狱方法面临探索复杂性和意图漂移问题，而单轮攻击只是多轮攻击的特例。需要一种简单有效的框架来训练多轮攻击者，无需依赖现有策略或外部数据。

Method: SEMA包含两个阶段：1）预填充自调优：通过微调非拒绝、结构良好的多轮对抗提示来稳定学习；2）意图漂移感知的强化学习：训练攻击者生成有效的多轮对抗提示，同时保持有害目标。采用开环攻击机制，避免依赖受害者反馈。

Result: 在多个数据集、受害者模型和越狱评判标准下，SEMA达到了最先进的攻击成功率。例如，在AdvBench上对三个闭源和开源受害者模型平均达到80.1%的ASR@1，比现有最佳方法高出33.9%。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的幻觉问题在多语言和生成式场景中尤为突出，现有模型在英语基准测试上表现良好，但在跨语言、跨任务和不同类型幻觉上的行为尚不清楚，需要系统性的分析工具。
...
Result: 评估结果显示：1）问答任务比对话摘要更容易；2）句子级幻觉即使对最强模型也具有挑战性；3）英语表现最佳，低资源语言性能下降，印地语检测准确率最低。

Conclusion: Halluverse-M^3为研究多语言、多任务环境中的幻觉问题提供了现实且具有挑战性的基准，数据集已公开发布以支持未来的幻觉检测和缓解研究。
...
TL;DR: 该研究首次对土耳其语子词分词进行了全面系统研究，通过控制词汇量和训练语料库大小，比较不同分词方法，并引入形态学感知诊断工具包，为形态丰富语言的分词提供了可操作指导。


<details>
  <summary>Details</summary>
Motivation: 土耳其语等形态丰富语言具有高度能产性粘着特征，这对神经语言建模的词汇效率和形态保真度都构成挑战。先前研究存在三个主要局限：词汇量变化缺乏对分词器训练语料的系统控制、内在诊断有限、下游任务评估范围狭窄。

Method: 提出了"子词宣言"研究方法：1) 联合变化词汇量和分词器训练语料库大小（数据与词汇耦合）；2) 在匹配参数预算下比较多种分词器家族（WordPiece、形态级别和字符基线）；3) 跨语义（NLI、STS、情感分析、NER）、句法（POS、依存分析）和形态敏感探针进行评估；4) 引入形态感知诊断工具包，包括边界级微/宏F1、解耦词干原子性与表面边界命中、过/欠分割指数、字符/词编辑距离、延续率、词缀类型覆盖和词级原子性等指标。

Result: 研究首次系统调查了词汇-语料-成功三元关系，建立了统一的形态感知评估框架，识别了字符级和形态级分词何时有效，并开源了评估代码、分词器流水线和模型。

Conclusion: 该"子词宣言"为形态丰富语言构建有效分词器提供了可操作指导，为未来研究建立了可复现的基础，是首个此类综合性研究。
...
TL;DR: DAWN是一种无需训练、依赖感知的解码方法，用于加速扩散大语言模型推理，通过提取token依赖关系实现高并行度，在保持生成质量的同时获得1.80-8.06倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型推理方案受限于质量-速度权衡，采用保守的并行策略，未能充分利用效率潜力。核心挑战在于并行解码假设每个位置可以独立填充，但token之间存在语义耦合，正确选择会约束其他位置的有效选择，不建模这些token间依赖关系会导致输出质量下降。

Method: DAWN是一种无需训练的依赖感知解码方法：1）提取token依赖关系；2）基于两个关键观察：依赖未掩码确定位置的位置更可靠，同时解掩强耦合的不确定位置会引发错误；3）利用依赖图在每次迭代中选择更可靠的解掩码位置，实现高并行度且生成质量损失可忽略。

Result: 在多个模型和数据集上的广泛实验表明，DAWN相比基线方法实现了1.80-8.06倍的推理加速，同时保持了生成质量。代码已在GitHub开源。

Conclusion: DAWN通过建模token间依赖关系，解决了扩散大语言模型并行解码中的质量-速度权衡问题，实现了高效且高质量的文本生成，为快速dLLM推理提供了有效的训练免费解决方案。
...
TL;DR: InftyThink+ 是一个端到端的强化学习框架，通过优化迭代推理轨迹来解决传统链式思维推理的二次成本、上下文长度限制和中间信息丢失问题，显著提升推理效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统的大规模推理模型通过扩展推理时的链式思维（chain-of-thought）获得强大性能，但这种方法存在三个主要问题：1）二次成本；2）上下文长度限制；3）由于"中间信息丢失效应"导致的推理质量下降。现有的迭代推理方法虽然通过定期总结中间思考来缓解这些问题，但依赖于监督学习或固定启发式方法，无法优化何时总结、保留什么内容以及如何恢复推理。

Method: InftyThink+ 是一个端到端的强化学习框架，基于模型控制的迭代边界和显式总结来优化整个迭代推理轨迹。采用两阶段训练方案：首先进行监督学习的冷启动，然后进行轨迹级别的强化学习，使模型能够学习战略性的总结和继续推理决策。

Result: 在 DeepSeek-R1-Distill-Qwen-1.5B 上的实验显示：1）在 AIME24 上准确率提升 21%；2）明显优于传统的长链式思维强化学习方法；3）在分布外基准测试上泛化能力更好；4）显著降低推理延迟并加速强化学习训练，在提升性能的同时提高了推理效率。

Conclusion: InftyThink+ 通过强化学习优化迭代推理轨迹，有效解决了传统链式思维推理的局限性，在提升推理准确性的同时显著提高了效率，展示了强化学习在优化推理过程方面的潜力。
...
### [104] [MuCo: Multi-turn Contrastive Learning for Multimodal Embedding Model](https://arxiv.org/abs/2602.06393)
...
<details>
  <summary>Details</summary>
Motivation: 传统多模态嵌入模型采用对比学习，但基于"单轮"形式，每个查询-目标对被视为独立数据点。这导致计算效率低下（需要单独前向传播）且忽略了多个查询之间的上下文关系。

Method: 引入多轮对比学习(MuCo)框架，利用MLLMs的对话特性，在单次前向传播中处理与单个图像相关的多个查询-目标对，同时提取多组查询和目标嵌入，基于共享上下文表示。

Result: 使用新构建的500万多模态多轮数据集(M3T)，在MMEB和M-BEIR基准测试中实现最先进的检索性能，同时显著提高训练效率和跨模态表示一致性。

Conclusion: MuCo通过多轮对比学习框架有效解决了传统单轮对比学习的计算效率问题，利用MLLMs的对话特性实现了更高效、更一致的多模态表示学习。
...
### [105] [TokenMixer-Large: Scaling Up Large Ranking Models in Industrial Recommenders](https://arxiv.org/abs/2602.06563)
...
TL;DR: 本文提出了TokenMixer-Large，一个针对大型推荐模型的改进架构，通过解决原始TokenMixer的四个核心设计限制，成功将参数规模扩展到70亿和150亿，并在字节跳动的多个场景中部署，取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究关注大型推荐模型的缩放定律，但实验规模相对有限。之前的TokenMixer架构在排序阶段表现出色，但其本身存在几个核心设计限制：残差设计不理想、深度模型梯度更新不足、MoE稀疏化不完整以及可扩展性探索有限。

Method: 提出TokenMixer-Large架构，通过以下方法解决原始TokenMixer的限制：1) 混合与还原操作；2) 层间残差连接；3) 辅助损失函数；4) 新颖的稀疏-每token MoE架构。这些改进使得模型能够成功扩展到70亿和150亿参数规模。

Result: TokenMixer-Large在在线流量和离线实验中分别成功扩展到70亿和150亿参数规模。该模型已在字节跳动的多个场景中部署，并取得了显著的离线性能提升和在线业务收益。

Conclusion: TokenMixer-Large通过系统性地解决原始TokenMixer架构的核心设计限制，成功实现了大型推荐模型的规模化扩展，在实际业务场景中验证了其有效性和实用性。
...
### [106] [R2LED: Equipping Retrieval and Refinement in Lifelong User Modeling with Semantic IDs for CTR Prediction](https://arxiv.org/abs/2602.06622)
*Qidong Liu,Gengnan Wang,Zhichen Liu,Moranxin Wang,Zijian Zhang,Xiao Han,Ni Zhang,Tao Qin,Chen Li*

Main category: cs.IR

TL;DR: R2LED提出了一种基于语义ID的终身用户建模新范式，通过多路由混合检索和双层融合精炼来解决现有方法中的噪声检索和语义理解不足问题。


<details>
  <summary>Details</summary>
Motivation: 终身用户建模在CTR预测中广泛应用，但现有"检索-精炼"两阶段方法存在两个主要问题：1) 由于数据分布倾斜导致的噪声检索；2) 精炼阶段缺乏语义理解。虽然语义增强（如LLM建模或语义嵌入）提供了潜在解决方案，但这些方法面临推理成本过高或表示粒度不足的挑战。

Method: 提出R2LED框架，利用语义ID的多粒度性和轻量性优势。1) 检索阶段：采用多路由混合检索，通过多个并行召回路由从不同粒度捕获用户兴趣，并提出混合检索机制从协同和语义视角高效检索候选项以减少噪声；2) 精炼阶段：设计双层融合精炼，包括用于路由级融合的目标感知交叉注意力和用于SID级融合的门控机制，以弥合语义空间与协同空间之间的差距。

Result: 在两个公共数据集上的综合实验结果表明，该方法在性能和效率方面均表现出优越性。

Conclusion: R2LED通过语义ID增强的检索和精炼机制，有效解决了终身用户建模中的噪声检索和语义理解不足问题，在保持效率的同时提升了CTR预测性能。
...
### [107] [Multimodal Generative Retrieval Model with Staged Pretraining for Food Delivery on Meituan](https://arxiv.org/abs/2602.06654)
*Boyu Chen,Tai Guo,Weiyu Cui,Yuqing Li,Xingxing Wang,Chuan Shi,Cheng Yang*

Main category: cs.IR

TL;DR: 提出分阶段预训练策略解决多模态检索中模态主导和训练速度不一致问题，结合生成式和判别式任务利用语义ID，在美团数据上显著提升检索性能


<details>
  <summary>Details</summary>
Motivation: 多模态检索在美团等场景中日益重要，但现有双塔架构的联合优化存在模态主导问题，某些模态被忽视，且不同模态训练速度不一致导致"单轮问题"

Method: 提出分阶段预训练策略，每个阶段专注于特定任务，使模型能有效关注和利用多模态特征；设计生成式和判别式任务帮助模型理解语义ID与查询、物品特征的关联

Result: 在美团大规模真实数据上，R@5、R@10、R@20分别提升3.80%、2.64%、2.17%，N@5、N@10、N@20分别提升5.10%、4.22%、2.09%；在线A/B测试显示收入提升1.12%，点击率提升1.02%

Conclusion: 分阶段预训练策略有效解决了多模态检索中的模态主导和训练不一致问题，通过语义ID的生成式和判别式任务提升了整体性能，在实际应用中验证了方法的有效性和优越性
...
### [108] [On the Efficiency of Sequentially Aware Recommender Systems: Cotten4Rec](https://arxiv.org/abs/2602.06935)
*Shankar Veludandi,Gulrukh Kurdistan,Uzma Mushtaque*

Main category: cs.IR

TL;DR: Cotten4Rec是一种使用余弦相似度线性注意力机制的顺序推荐模型，通过优化的CUDA内核实现，相比BERT4Rec和LinRec显著降低了计算资源消耗，同时保持了推荐准确性。


<details>
  <summary>Details</summary>
Motivation: Transformer-based的顺序推荐模型（如BERT4Rec）虽然能有效捕捉用户行为模式，但由于基于Softmax的注意力机制需要大量中间计算，导致显著的计算开销。需要开发更高效的模型来应对大规模顺序推荐场景中的计算资源限制。

Method: 提出Cotten4Rec模型，采用线性时间余弦相似度注意力机制，通过单个优化的CUDA内核实现，最小化中间缓冲区和内核启动开销，从而减少资源使用。

Result: 在三个基准数据集上的评估显示，Cotten4Rec相比BERT4Rec和线性注意力基线LinRec，在内存使用和运行时间方面实现了显著降低，同时推荐准确性损失最小。

Conclusion: Cotten4Rec证明了其作为高效替代方案的可行性，特别适用于计算资源受限的大规模顺序推荐实际场景，在保持推荐准确性的同时大幅提升了计算效率。
...
### [109] [Jackpot: Optimal Budgeted Rejection Sampling for Extreme Actor-Policy Mismatch Reinforcement Learning](https://arxiv.org/abs/2602.06107)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的强化学习成本高昂，特别是rollout过程。解耦rollout生成与策略优化可提高效率，但会引入严重的分布不匹配问题，导致学习不稳定

Method: 提出Jackpot框架，采用最优预算拒绝采样直接减少rollout模型与演化策略间的差异，包含原则性OBRS过程、联合更新策略和rollout模型的统一训练目标，以及基于top-k概率估计和批次级偏差校正的高效系统实现

Result: 理论分析表明OBRS在可控接受预算下持续将rollout分布移向目标分布。实验显示相比重要性采样基线，Jackpot显著提高训练稳定性，在Qwen3-8B-Base上训练300个更新步骤（批次大小64）时达到与在线RL相当的性能
...
### [110] [Large Language Model Reasoning Failures](https://arxiv.org/abs/2602.06176)
*Peiyang Song,Pengrui Han,Noah Goodman*

Main category: cs.AI

TL;DR: 该论文首次对大型语言模型的推理失败进行系统性综述，提出了新的分类框架，将推理分为具身与非具身类型，并将推理失败分为基础架构缺陷、应用特定限制和鲁棒性问题三类，同时发布了相关研究资源库。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在推理能力方面取得了显著进展，但在看似简单的场景中仍然存在明显的推理失败。目前缺乏对这一问题的系统性研究，需要建立一个统一的框架来理解、分类和解决LLM的推理缺陷。

Method: 提出了一个新颖的分类框架：首先将推理分为具身推理和非具身推理，后者进一步细分为非正式（直觉）推理和正式（逻辑）推理。同时，将推理失败分为三类：影响下游任务的基础架构缺陷、特定领域表现的应用限制、以及对微小变化敏感的鲁棒性问题。对每种推理失败都提供了明确定义、分析现有研究、探索根本原因并提出缓解策略。

Result: 通过整合碎片化的研究工作，该综述为LLM推理的系统性弱点提供了结构化视角，为未来研究提供了有价值的见解。同时发布了GitHub资源库（https://github.com/Peiyang-Song/Awesome-LLM-Reasoning-Failures），为该领域研究提供了便捷入口。

Conclusion: 该研究首次对LLM推理失败进行了全面调查，提出了系统的分类框架，有助于指导未来研究构建更强、更可靠、更鲁棒的推理能力。通过统一现有研究并发布资源库，为理解和解决LLM推理缺陷提供了重要基础。
...
### [111] [Do It for HER: First-Order Temporal Logic Reward Specification in Reinforcement Learning (Extended Version)](https://arxiv.org/abs/2602.06227)
*Pierriccardo Olivieri,Fausto Lasca,Alessandro Gianola,Matteo Papini*

Main category: cs.AI

TL;DR: 提出基于LTLfMT逻辑框架的非马尔可夫奖励规范方法，解决大状态空间MDP中复杂任务的表达问题，通过理论片段识别和基于奖励机器与HER的实践方法应对计算挑战。


<details>
  <summary>Details</summary>
Motivation: 传统马尔可夫决策过程在处理大状态空间和非结构化数据域时，难以表达复杂的非马尔可夫奖励任务。现有方法需要手动编码谓词，缺乏统一可重用的框架。

Method: 1) 使用LTLfMT（基于理论的线性时序逻辑）作为表达框架；2) 从理论上识别可处理的LTLfMT片段；3) 实践中结合奖励机器和Hindsight Experience Replay方法处理一阶逻辑规范和奖励稀疏问题。

Result: 在连续控制环境中使用非线性算术理论进行评估，能够自然地规范复杂任务。实验结果表明，定制的HER实现对于解决具有复杂目标的任务至关重要。

Conclusion: 提出的LTLfMT框架为MDP中的非马尔可夫奖励规范提供了统一且可重用的方法，通过理论片段识别和HER技术解决了表达性与计算复杂性之间的平衡问题，能够有效处理复杂任务规范。
...
### [112] [Do LLMs Act Like Rational Agents? Measuring Belief Coherence in Probabilistic Decision Making](https://arxiv.org/abs/2602.06286)
*Khurram Yamin,Jingjing Tang,Santiago Cortes-Gomez,Amit Sharma,Eric Horvitz,Bryan Wilder*

Main category: cs.AI

TL;DR: 该研究检验了大型语言模型是否作为理性效用最大化者运作，通过诊断挑战问题分析其信念一致性和偏好稳定性，提出了可证伪条件来判断报告概率是否对应真实信念。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地被部署在高风险领域作为智能体，其决策逻辑难以解释。在这些领域中，最优行动取决于对世界的不确定性和不同结果的效用考量。因此需要研究LLM是否具有一致的信念和稳定的偏好，以评估其在高风险决策中的可靠性。

Method: 研究采用诊断挑战问题来检验LLM的行为。通过分析模型对概率的表述和观察到的行动，研究LLM推理与理想贝叶斯效用最大化之间的关系。提出了可证伪条件，用于判断报告的概率是否可能对应任何理性智能体的真实信念。该方法在多个医疗诊断领域进行了评估，并测试了多个LLM模型。

Result: 研究结果为LLM推理与理想贝叶斯效用最大化之间的关系提供了见解。通过提出的可证伪条件，可以识别出报告概率无法对应任何理性智能体真实信念的情况。在多个医疗诊断领域和多个LLM模型上的评估显示了模型在理性决策方面的表现。

Conclusion: 研究讨论了结果对LLM在高风险决策中应用的启示，并指出了未来发展方向。虽然LLM在某些方面表现出理性决策的特征，但在信念一致性和偏好稳定性方面仍存在挑战，需要进一步研究以确保其在关键决策中的可靠性。
...
### [113] [Exposing Weaknesses of Large Reasoning Models through Graph Algorithm Problems](https://arxiv.org/abs/2602.06319)
...
Method: 引入GrAlgoBench基准，使用图算法问题评估大型推理模型。图算法问题特别适合测试推理能力：需要长上下文推理、难度可精细控制、支持标准化程序化评估。包含九个任务进行系统实验。

Result: 实验发现当前大型推理模型存在两大弱点：1) 随着上下文长度增加，准确率急剧下降（图节点超过120个时准确率低于50%），主要由执行错误、弱记忆和冗余推理导致；2) 存在过度思考现象，大量无效的自我验证增加了推理痕迹但没有提高正确性。
...
### [114] [Trifuse: Enhancing Attention-Based GUI Grounding via Multimodal Fusion](https://arxiv.org/abs/2602.06351)
*Longhui Ma,Di Zhao,Siwei Wang,Zhao Lv,Miao Wang*

Main category: cs.AI

TL;DR: Trifuse是一个基于注意力的GUI grounding框架，通过整合注意力机制、OCR文本线索和图标级语义，无需任务特定微调即可实现GUI元素定位，减少对标注数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding方法主要依赖微调多模态大语言模型，需要大量数据且泛化能力差；基于注意力的替代方法缺乏明确的空间锚点，可靠性低。需要解决这些限制。

Method: 提出Trifuse框架，通过Consensus-SinglePeak融合策略整合三种模态：注意力机制、OCR提取的文本线索和图标级语义描述，实现跨模态一致性和精确定位。

Result: 在四个grounding基准测试中表现优异，无需任务特定微调；消融研究表明整合OCR和语义线索能持续提升基于注意力的grounding性能，减少对昂贵标注数据的依赖。

Conclusion: Trifuse通过显式整合互补空间锚点，为GUI grounding提供了一个有效的通用框架，在减少数据依赖的同时提高了定位的可靠性和泛化能力。
...
### [115] [Difficulty-Estimated Policy Optimization](https://arxiv.org/abs/2602.06375)
*Yu Zhao,Fan Jiang,Tianle Liu,Bo Zeng,Yu Liu,Longyue Wang,Weihua Luo*

Main category: cs.AI

TL;DR: DEPO框架通过动态难度评估筛选训练样本，在保持模型性能的同时将推理成本降低2倍


<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法在问题过于简单或复杂时会出现梯度信号衰减问题，而DAPO等变体虽然解决了梯度消失但计算开销巨大，需要更高效稳健的推理对齐方法

Method: 提出Difficulty-Estimated Policy Optimization (DEPO)框架，集成在线难度评估器，在rollout阶段前动态评估和筛选训练数据，优先将计算资源分配给具有高学习潜力的样本

Result: 实证结果显示DEPO在不影响模型性能的情况下将rollout成本降低高达2倍，显著降低了训练高性能推理模型的计算门槛

Conclusion: DEPO为推理缩放提供了更可持续的路径，通过智能样本筛选优化计算效率，是推理对齐领域的重要进展
...
### [116] [Unlocking Noisy Real-World Corpora for Foundation Model Pre-Training via Quality-Aware Tokenization](https://arxiv.org/abs/2602.06394)
...
<details>
  <summary>Details</summary>
Motivation: 当前的分词方法在处理序列数据时没有考虑信号质量，限制了它们在嘈杂现实世界语料库上的有效性。需要一种能够将数据可靠性直接融入词汇表构建的方法。

Method: 提出了QA-Token（质量感知分词），包含三个关键贡献：1）双层优化公式，联合优化词汇表构建和下游性能；2）强化学习方法，通过质量感知奖励学习合并策略并具有收敛保证；3）通过Gumbel-Softmax松弛的自适应参数学习机制，实现端到端优化。
...
### [117] [Intrinsic Stability Limits of Autoregressive Reasoning: Structural Consequences for Long-Horizon Execution](https://arxiv.org/abs/2602.06413)
*Hsien-Jyh Liao*

Main category: cs.AI

TL;DR: 该研究提出自回归推理存在内在稳定性极限，即使在线性无分支任务中，决策优势也会随执行长度指数衰减，导致长视野推理性能急剧下降，需要离散分段和图状执行结构来维持稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统解释将LLMs在长视野任务中的性能下降归因于任务复杂性（如组合搜索爆炸或长期信用分配），但作者认为这些解释不完整。即使在无语义模糊的线性无分支任务中，自回归执行也存在内在稳定性极限，需要从过程级不稳定性角度重新理解长视野推理问题。

Method: 1. 提出理论框架：将长视野推理重构为结构治理问题，而非单纯的任务复杂性；2. 推导定理A：证明单路径自回归推理中的决策优势随执行长度指数衰减，对可维持的推理链施加基本限制；3. 实证验证：在合成环境和真实TextWorld任务中进行实验，观察与理论预测一致的性能断崖现象。

Result: 实证研究显示，在合成环境和真实TextWorld任务中均观察到与理论预测一致的性能断崖现象。短视野评估协议可能掩盖结构不稳定性，表明未来推理系统需要从单纯扩展转向结构化治理。

Conclusion: 自回归架构在长视野推理中存在根本性限制，决策优势随执行长度指数衰减导致稳定性极限。稳定长视野推理需要离散分段，自然诱导出有向无环图等图状执行结构。研究为长视野推理失败提供了动力学视角，并指出了纯自回归架构在维持长期连贯性方面的新限制。
...
### [118] [AgentCPM-Explore: Realizing Long-Horizon Deep Exploration for Edge-Scale Agents](https://arxiv.org/abs/2602.06485)
...
TL;DR: 本文首次系统研究了4B参数规模智能体模型的训练，提出了解决边缘规模模型三大瓶颈的AgentCPM-Explore框架，在多项基准测试中超越更大规模模型。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体系统过度依赖大规模模型，边缘规模模型（4B参数级别）的能力未被充分探索，需要系统研究如何提升这类模型的智能体性能。

Method: 提出AgentCPM-Explore框架，包含参数空间模型融合、奖励信号去噪和上下文信息精炼三个关键技术，解决SFT中的灾难性遗忘、RL中的奖励信号噪声敏感性和长上下文中的推理退化问题。

Result: AgentCPM-Explore在4B级别模型中达到SOTA性能，在四个基准测试中匹配或超越8B级别SOTA模型，在五个基准测试中超越Claude-4.5-Sonnet和DeepSeek-v3.2等更大规模模型，在GAIA文本任务上达到97.09%准确率（pass@64）。

Conclusion: 边缘规模模型的瓶颈并非其固有能力上限，而是推理稳定性问题。通过提出的训练框架，AgentCPM-Explore有效释放了边缘规模模型被低估的潜力。
...
### [119] [JADE: Expert-Grounded Dynamic Evaluation for Open-Ended Professional Tasks](https://arxiv.org/abs/2602.06486)
*Lanbo Lin,Jiayao Liu,Tianyuan Yang,Li Cai,Yuanwu Xu,Lei Wei,Sicong Xie,Guannan Zhang*

Main category: cs.AI

TL;DR: JADE是一个两层的评估框架，结合了静态评估准则的严谨性和LLM评估的灵活性，用于评估开放式的专业任务中的智能体AI表现。


<details>
  <summary>Details</summary>
Motivation: 评估智能体AI在开放式专业任务时面临严谨性与灵活性的困境：静态评估准则虽然严谨可复现，但无法适应多样化的有效响应策略；而基于LLM的评估方法虽然能适应个体响应，但存在不稳定性和偏见问题。

Method: JADE采用两层评估框架：第一层将专家知识编码为预定义的评估技能，提供稳定的评估标准；第二层执行报告特定的、基于声明的评估，灵活评估多样化的推理策略，并通过证据依赖性门控来使基于被反驳声明的结论无效。

Result: 在BizBench上的实验表明，JADE提高了评估稳定性，并揭示了整体LLM评估器遗漏的关键智能体故障模式。进一步研究表明，JADE与专家编写的评估准则高度一致，并能有效迁移到医疗领域基准测试中。

Conclusion: JADE框架成功解决了评估智能体AI在开放式专业任务时的严谨性与灵活性困境，通过结合专家知识和动态声明级评估，实现了跨专业领域的有效评估。
...
TL;DR: HyPER是一种用于混合专家模型的多路径解码训练免费在线控制策略，通过动态扩展-缩减控制重新分配计算资源，在固定预算下实现更好的探索-利用平衡，显著提高推理准确性并减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有方法在探索-利用权衡上存在局限性：树结构搜索通过脆弱的扩展规则硬编码探索，干扰后训练推理；并行推理则过度探索冗余假设路径且依赖弱答案选择。最优平衡是阶段依赖的，正确和错误推理路径往往在后期才分叉。

Method: 将测试时扩展重新定义为假设池上的动态扩展-缩减控制问题。提出HyPER：包含在线控制器（随假设池演化从探索转向利用）、令牌级细化机制（无需完整路径重采样实现高效生成时利用）、长度和置信度感知聚合策略（可靠答案时利用）。

Result: 在四个混合专家语言模型和多样化推理基准上的实验表明，HyPER始终实现更优的准确性-计算权衡，准确率提高8-10%，同时令牌使用减少25-40%。

Conclusion: 通过将测试时扩展重新定义为动态控制问题，并设计训练免费的在线控制策略，HyPER在混合专家模型中实现了更有效的探索-利用平衡，显著提升了多路径推理的性能效率比。
...
### [121] [AgentCPM-Report: Interleaving Drafting and Deepening for Open-Ended Deep Research](https://arxiv.org/abs/2602.06540)
...
TL;DR: AgentCPM-Report是一个轻量级本地化深度研究报告生成系统，采用8B参数模型和Writing As Reasoning Policy框架，通过动态大纲修订和迭代深化，在多个基准测试中超越闭源系统。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究报告生成系统严重依赖闭源或在线大模型，存在部署障碍、安全性和隐私问题。现有方法采用"先规划后写作"范式，其性能高度依赖初始大纲质量，而构建全面大纲本身需要强大的推理能力。

Method: 提出AgentCPM-Report系统，包含：1) Writing As Reasoning Policy框架，支持动态大纲修订；2) 8B参数深度研究智能体；3) Evidence-Based Drafting和Reasoning-Driven Deepening交替机制；4) Multi-Stage Agentic Training策略（冷启动、原子技能强化学习、整体流程强化学习）。

Result: 在DeepResearch Bench、DeepConsult和DeepResearch Gym基准测试中，AgentCPM-Report超越了领先的闭源系统，在Insight指标上取得了显著提升。

Conclusion: AgentCPM-Report提供了一个轻量级、高性能的本地化解决方案，通过Writing As Reasoning Policy和多阶段训练策略，使小型模型能够有效生成深度研究报告，解决了对闭源大模型的依赖问题。
...
### [122] [SeeUPO: Sequence-Level Agentic-RL with Convergence Guarantees](https://arxiv.org/abs/2602.06554)
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习算法在训练大型语言模型智能体时缺乏经过验证的收敛保证，特别是在多轮交互场景中，这导致训练不稳定和无法收敛到最优策略。

Method: 提出了SeeUPO（序列级顺序更新策略优化），这是一种具有收敛保证的无评论家方法。它将多轮交互建模为顺序执行的多臂赌博机问题，通过反向执行顺序的逐轮顺序策略更新，确保单调改进并通过反向归纳收敛到全局最优解。

Result: 在AppWorld和BFCL v4上的实验表明，SeeUPO相比现有骨干算法有显著改进：在Qwen3-14B上相对增益43.3%-54.6%，在Qwen2.5-14B上相对增益24.1%-41.9%（跨基准平均），同时具有更优的训练稳定性。

Conclusion: SeeUPO解决了多轮交互场景中现有强化学习算法缺乏收敛保证的问题，通过创新的序列级顺序更新方法，实现了无评论家且具有收敛保证的策略优化，显著提升了训练效果和稳定性。
...
### [123] [Same Answer, Different Representations: Hidden instability in VLMs](https://arxiv.org/abs/2602.06652)
...
TL;DR: 本文提出了一种新的VLM鲁棒性评估框架，发现传统基于输出不变性的评估方法存在不足，揭示了VLM在内部表示漂移、尺度扩展不改善鲁棒性、以及扰动对不同任务产生不同影响等三个关键失败模式。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型（VLM）的鲁棒性评估主要基于输出层面的不变性，即假设稳定的预测反映了稳定的多模态处理过程。然而，这种假设可能不充分，因为即使输出保持不变，内部表示可能已经发生显著变化。本文旨在开发更全面的评估框架来揭示VLM鲁棒性的真实状况。

Method: 提出了一个表示感知和频率感知的评估框架，该框架不仅包括标准的基于标签的指标，还测量内部嵌入漂移、频谱敏感性和结构平滑度（视觉token的空间一致性）。在SEEDBench、MMMU和POPE数据集上对现代VLM应用此框架进行评估。

Result: 发现了三个明显的失败模式：1）模型经常在保持预测答案的同时经历显著的内部表示漂移，某些扰动（如文本叠加）导致的漂移接近图像间变异的幅度；2）鲁棒性不随模型规模扩大而改善，更大的模型虽然准确率更高，但表现出相同或更大的敏感性；3）扰动对不同任务的影响不同：在推理任务中，当扰动破坏模型如何结合粗粒度和细粒度视觉线索时会损害性能，但在幻觉基准测试中，扰动可以通过使模型生成更保守的答案来减少误报。

Conclusion: 仅依赖输出不变性的传统VLM鲁棒性评估方法存在局限性。需要更全面的评估框架来理解模型内部表示的变化，这对于开发真正鲁棒的视觉语言模型至关重要。模型规模的扩大并不必然带来鲁棒性的改善，而扰动的影响具有任务特异性。
...
### [124] [Autoregressive Models for Knowledge Graph Generation](https://arxiv.org/abs/2602.06707)
...
Result: 在IntelliGraphs基准测试中，模型在多样化数据集上达到89.2%到100.0%的语义有效性，同时生成训练中未见的新图谱。分析显示模型容量（隐藏维度≥64）比架构深度对KG生成更为关键，循环架构在实现与基于Transformer的替代方案相当有效性的同时提供显著计算效率。
...
### [125] [Semantically Labelled Automata for Multi-Task Reinforcement Learning with LTL Instructions](https://arxiv.org/abs/2602.06746)
...
<details>
  <summary>Details</summary>
Motivation: 研究多任务强化学习，旨在学习一个能够泛化到任意（可能未见过的）任务的通用策略。现有方法在处理复杂LTL规范时存在局限性，需要更有效的任务表示方法。

Method: 采用线性时序逻辑（LTL）公式指定任务，利用新一代语义LTL到自动机转换技术，生成包含丰富结构化信息的语义标记自动机。通过该自动机：(i) 动态高效计算自动机；(ii) 提取表达性强的任务嵌入来调节策略；(iii) 完全支持LTL规范。

Result: 在多个领域的实验结果表明，该方法达到了最先进的性能，能够扩展到现有方法无法处理的复杂规范，展示了优越的泛化能力。

Conclusion: 提出的基于语义LTL到自动机转换的任务嵌入技术为多任务强化学习提供了有效的解决方案，能够处理复杂的LTL规范并实现优异的性能，为形式化方法与强化学习的结合开辟了新途径。
...
### [126] [Wild Guesses and Mild Guesses in Active Concept Learning](https://arxiv.org/abs/2602.06818)
*Anirudh Chari,Neil Pattanaik*

Main category: cs.AI

TL;DR: 研究比较了两种主动概念学习策略：理性主动学习者的期望信息增益策略和人类的正向测试策略，发现在简单概念上正向测试策略表现更好，这可能是人类认知偏差的理性适应。


<details>
  <summary>Details</summary>
Motivation: 研究人类主动概念学习中的权衡问题：如何在查询的信息性和学习器稳定性之间取得平衡。传统观点认为正向测试策略（确认偏误）是认知错误，但本研究探讨其是否可能是理性适应。

Method: 使用神经符号贝叶斯学习器，假设由大型语言模型生成可执行程序，通过贝叶斯更新重新加权。比较两种策略：理性主动学习者的期望信息增益策略和人类的正向测试策略，在经典数字游戏中进行概念学习任务测试。

Result: 期望信息增益策略在需要证伪的复杂概念（如复合规则或包含例外的规则）上有效，但在简单概念上表现不佳。正向测试策略虽然信息次优，但通过选择"安全"查询保持提案有效性，在简单规则上收敛更快。

Conclusion: 人类的"确认偏误"可能不是认知错误，而是在稀疏、开放假设空间中保持可处理推理的理性适应。正向测试策略通过避免支持不匹配陷阱，在简单概念学习中表现更优。
...
### [127] [ScaleEnv: Scaling Environment Synthesis from Scratch for Generalist Interactive Tool-Use Agent Training](https://arxiv.org/abs/2602.06820)
...
<details>
  <summary>Details</summary>
Motivation: 训练能够适应多样化场景的通用智能体需要交互式环境进行自我探索，但目前交互式环境严重稀缺，现有合成方法在环境多样性和可扩展性方面存在显著限制。
...
Conclusion: 扩展环境多样性对于鲁棒的智能体学习至关重要，ScaleEnv为解决交互式环境稀缺问题提供了有效框架，能够显著提升智能体的泛化能力。
...
### [128] [POP: Online Structural Pruning Enables Efficient Inference of Large Foundation Models](https://arxiv.org/abs/2602.06822)
*Yi Chen,Wonjin Shin,Shuhong Liu,Tho Mai,Jeongmo Lee,Chuanbo Hua,Kun Wang,Jun Liu,Joo-Young Kim*

Main category: cs.AI

TL;DR: POP是一种针对大型基础模型的在线结构化剪枝框架，通过分区引导的动态剪枝在推理过程中实现上下文条件化的剪枝决策，减少计算开销。
...
Result: 在多种大型基础模型（LLMs、MoEs、VLMs）上的广泛评估表明，POP相比现有剪枝方法能提供更高的准确性，同时产生更小的计算开销并最小化推理延迟。

Conclusion: POP是一种轻量级、即插即用的在线剪枝框架，通过上下文条件化的动态剪枝，在保持重要权重的同时提供解码阶段的细粒度变化，显著提升了大型基础模型的推理效率。
...
### [129] [LLM Active Alignment: A Nash Equilibrium Perspective](https://arxiv.org/abs/2602.06836)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决大型语言模型群体行为预测和引导的挑战，特别是在开放文本空间中均衡计算不可行的问题，需要开发一个可解释且行为上有实质意义的政策类别。

Method: 将每个智能体的行动建模为对人类子群体的混合选择，智能体主动战略性地选择与哪些群体对齐，采用标准凹效用假设推导闭式纳什均衡特征，作为现有对齐流程（如RLHF）之上的主动对齐层。

Result: 在社交媒体场景中，研究表明语言模型群体（特别是基于推理的模型）可能表现出政治排斥现象，即某些子群体被所有LLM智能体忽略，而该方法可以避免这种病理现象。

Conclusion: 该方法展示了在多智能体语言模型动态调节方面的应用前景，能够通过明确的、可操作的指导将对齐目标转向社会期望的结果。
...
### [130] [An Adaptive Differentially Private Federated Learning Framework with Bi-level Optimization](https://arxiv.org/abs/2602.06838)
*Jin Wang,Hui Ma,Fei Xing,Ming Yan*

Main category: cs.AI

TL;DR: 提出自适应差分隐私联邦学习框架，解决设备异构性、非独立同分布数据和隐私保护下的训练不稳定问题，通过客户端压缩模块、自适应梯度裁剪和约束感知聚合机制提升模型性能。
...
Conclusion: 提出的自适应差分隐私联邦学习框架有效解决了异构隐私约束环境下的训练不稳定问题，通过客户端压缩、自适应裁剪和智能聚合机制实现了更好的模型效率和性能。
...
### [131] [From Features to Actions: Explainability in Traditional and Agentic AI Systems](https://arxiv.org/abs/2602.06841)
*Sindhuja Chaduvula,Jessee Ho,Kina Kim,Aravind Narayanan,Mahshid Alinoori,Muskan Garg,Dhanesh Ramachandram,Shaina Raza*

Main category: cs.AI

TL;DR: 该研究比较了静态AI解释方法与智能体AI解释方法，发现传统特征归因方法适用于静态分类任务，但无法可靠诊断智能体系统的执行失败，而基于轨迹的诊断方法能有效定位智能体行为故障。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型发展，智能体AI系统通过多步决策轨迹展现行为，但现有的可解释AI方法主要针对静态模型预测设计，不清楚这些方法如何适用于智能体环境。需要弥合静态可解释性与智能体可解释性之间的差距。

Method: 通过比较静态分类任务中的特征归因解释方法与智能体基准（TAU-bench Airline和AssistantBench）中的轨迹诊断方法，进行实证对比研究。使用统一的XAI评估框架。

Result: 特征归因方法在静态设置中表现稳定（Spearman ρ=0.86），但无法可靠诊断智能体轨迹中的执行级故障。基于轨迹的评估方法能一致定位行为故障，发现状态跟踪不一致在失败运行中普遍2.7倍，并降低成功概率49%。

Conclusion: 研究结果表明需要向轨迹级可解释性转变，以评估和诊断自主AI行为。智能体系统需要专门的可解释性方法，而不是简单沿用静态预测的解释方法。
...
### [133] [NanoNet: Parameter-Efficient Learning with Label-Scarce Supervision for Lightweight Text Mining Model](https://arxiv.org/abs/2602.06093)
...
TL;DR: NanoNet：一个轻量级文本挖掘框架，通过参数高效学习和在线知识蒸馏，在有限监督下训练多个小模型，并通过互学习正则化提升性能，实现低成本训练和快速推理。


<details>
  <summary>Details</summary>
Motivation: 现有的轻量级半监督学习策略虽然能减少标注样本和降低推理成本，但存在计算密集、易陷入局部最优的问题。研究者希望探索在文本挖掘任务中整合三种低成本场景：有限标注监督、轻量级微调和快速推理小模型。

Method: 提出NanoNet框架，采用参数高效学习在有限监督下进行训练。使用在线知识蒸馏生成多个小模型，并通过互学习正则化增强这些模型的性能。整个过程利用参数高效学习降低训练成本并最小化监督需求。

Result: 该框架能够减少训练成本，最小化监督需求，最终为下游推理任务生成轻量级模型。

Conclusion: NanoNet为轻量级文本挖掘提供了一种新颖的解决方案，通过整合参数高效学习、在线知识蒸馏和互学习正则化，实现了在有限监督下的低成本训练和高效推理。
...
### [134] [Agentic Workflow Using RBA$_θ$ for Event Prediction](https://arxiv.org/abs/2602.06097)
*Purbak Sengupta,Sambeet Mishra,Sonal Shreya*

Main category: cs.LG

TL;DR: 该论文提出了一种"事件优先、频率感知"的风电爬坡事件预测新范式，直接预测爬坡事件并重建功率轨迹，而非从密集预测中推断事件。该方法整合了统计、机器学习和深度学习模型，通过频率分解和自适应特征选择实现跨站点零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 风电爬坡事件预测面临强变异性、多尺度动态和站点特定气象效应的挑战。传统方法通过后验事件提取提供可解释基线，但跨站点泛化能力有限。需要开发能够直接预测事件、捕捉多尺度特性并实现跨站点迁移的预测框架。

Method: 1. 基于增强的RBAθ方法的事件表示；2. 渐进整合统计、机器学习和深度学习模型；3. 随机森林直接事件预测；4. 事件优先深度架构：集成小波频率分解、时间激励特征和自适应特征选择；5. 智能预测层：根据操作上下文动态选择专门工作流。

Result: 1. 直接事件预测比基于生存的公式更稳健；2. 爬坡幅度和持续时间受不同中频带控制；3. 从稀疏事件预测中实现准确信号重建；4. 实现稳定长时程事件预测和物理一致的轨迹重建；5. 实现到未见风电场的零样本迁移。

Conclusion: 事件优先、频率感知的预测范式为风电功率预测提供了可迁移且与操作对齐的替代方案，优于传统的轨迹优先方法。该框架通过捕捉多尺度特性和自适应工作流选择，提高了跨站点泛化能力和操作实用性。
...
### [135] [Toward Faithful and Complete Answer Construction from a Single Document](https://arxiv.org/abs/2602.06103)
...
Result: 召回率和精确率分别提升高达24%和29%，F1分数提升31%，打破了单次LLM生成中覆盖范围与准确性之间的权衡，同时缓解了长度限制导致的生成截断问题。

Conclusion: EVE框架有效解决了LLMs在文档基础推理中的完整性和忠实性问题，但性能受自然语言固有模糊性的限制，反映了基于语言推理的基本限制。
...
### [136] [Pragmatic Curiosity: A Hybrid Learning-Optimization Paradigm via Active Inference](https://arxiv.org/abs/2602.06104)
...
Result: 在各种真实世界混合任务中验证了实用好奇心的有效性和灵活性，包括约束系统识别、目标主动搜索和未知偏好的复合优化。在所有基准测试中，实用好奇心始终优于强BO型和BED型基线，提供更高的估计精度、更好的关键区域覆盖和改善的最终解质量

Conclusion: 实用好奇心为混合学习和优化问题提供了统一的框架，在贝叶斯优化和贝叶斯实验设计之间架起了桥梁，在多个实际应用中表现出优越性能
...
### [137] [Compressing LLMs with MoP: Mixture of Pruners](https://arxiv.org/abs/2602.06127)
...
TL;DR: MoP是一个统一深度和宽度剪枝的迭代框架，通过生成两个分支（深度剪枝vs宽度剪枝）并选择最佳候选来推进剪枝路径，在LLaMA系列模型上超越了现有方法，实现了39%的延迟降低，并成功扩展到视觉语言模型。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的高计算需求促使需要减少参数数量和加速推理的方法。当前剪枝方法通常只关注单一维度（深度或宽度），缺乏统一的框架来同时考虑这两个维度。

Method: MoP（Mixture of Pruners）是一个迭代框架，在每次迭代中生成两个分支：深度剪枝和宽度剪枝，然后选择最佳候选来推进剪枝路径。该方法统一了深度和宽度两个维度的剪枝策略。

Result: 在LLaMA-2和LLaMA-3上，MoP超越了竞争方法在各种压缩率下的准确率，始终优于仅深度或仅宽度的剪枝。在40%压缩率下，端到端延迟降低了39%。扩展到视觉语言模型LLaVA-1.5后，显著提高了计算效率，并证明仅文本恢复微调也能恢复视觉任务性能。

Conclusion: MoP提供了一个统一的深度和宽度剪枝框架，在保持模型准确性的同时实现了显著的推理加速，并且可以成功扩展到多模态模型，展示了其通用性和有效性。
...
### [138] [Self-Improving World Modelling with Latent Actions](https://arxiv.org/abs/2602.06130)
*Yifu Qiu,Zheng Zhao,Waylon Li,Yftah Ziser,Anna Korhonen,Shay B. Cohen,Edoardo M. Ponti*

Main category: cs.LG

TL;DR: SWIRL是一个自改进框架，通过将动作视为潜变量，从仅状态序列中学习世界模型，无需昂贵的动作标注轨迹，在多个基准上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 学习世界模型（预测状态转换）通常需要昂贵的动作标注轨迹，这限制了模型的学习效率。本文旨在从仅状态序列中学习世界模型，降低数据标注成本。

Method: SWIRL框架将动作视为潜变量，交替进行前向世界建模（FWM）和逆动力学建模（IDM）。通过变分信息最大化和ELBO最大化两个阶段迭代更新，使用强化学习（GRPO）训练，以冻结模型的log概率作为奖励信号。
...
Conclusion: SWIRL能够从仅状态序列中有效学习世界模型，无需动作标注，在多个基准上取得显著性能提升，为LLMs和VLMs的推理和规划提供了高效的学习框架。
...
### [139] [Tempora: Characterising the Time-Contingent Utility of Online Test-Time Adaptation](https://arxiv.org/abs/2602.06136)
*Sudarshan Sreeram,Young D. Kwon,Cecilia Mascolo*

Main category: cs.LG

TL;DR: Tempora框架首次系统评估测试时适应(TTA)在时间压力下的表现，通过时间场景、评估协议和效用指标量化准确率-延迟权衡，发现传统排名在时间约束下不稳定。


<details>
  <summary>Details</summary>
Motivation: 传统TTA评估假设无限处理时间，忽略了实际部署中的准确率-延迟权衡。随着ML在延迟敏感应用中的普及，时间压力成为适应性推理可行性的关键约束。

Method: 提出Tempora框架，包含：1)建模部署约束的时间场景；2)操作化测量的评估协议；3)量化准确率-延迟权衡的时间相关效用指标（离散、连续、摊销效用）。在ImageNet-C上对7种TTA方法进行240次时间评估。

Result: 传统排名在时间压力下不稳定：传统最优方法ETA在41.2%评估中表现不佳；最佳方法随腐败类型和时间压力变化，无明确赢家；排名反转现象揭示了方法选择需要考虑时间约束。

Conclusion: Tempora首次实现了跨多样时间约束的系统评估，为从业者提供方法选择视角，为研究者提供可部署适应的研究目标，揭示了传统评估忽略的关键权衡。
...
TL;DR: 该论文提出了一种基于流匹配的离散动作空间离线强化学习框架，通过连续时间马尔可夫链处理离散动作，并扩展到多智能体和多目标场景。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型和流匹配的生成策略主要局限于连续动作空间，需要扩展以支持更广泛的离线强化学习设置，包括离散动作空间、多目标和多智能体场景。

Method: 使用连续时间马尔可夫链替代连续流，采用Q加权流匹配目标进行训练；在多智能体设置中，通过因子化条件路径缓解联合动作空间的指数增长；支持通过动作量化应用于连续控制问题。

Result: 理论证明在理想条件下优化该目标可恢复最优策略；实验表明方法在高维控制、多模态决策和动态多目标偏好等实际场景中表现稳健；离散框架通过动作量化在连续控制问题中提供灵活的性能-复杂度权衡。

Conclusion: 该研究扩展了流匹配框架到离散动作空间，为离线强化学习提供了更通用的解决方案，能够处理多目标、多智能体等复杂场景，并通过动作量化支持连续控制问题。
...
### [141] [Optimistic Training and Convergence of Q-Learning -- Extended Version](https://arxiv.org/abs/2602.06146)
*Prashant Mehta,Sean Meyn*

Main category: cs.LG

TL;DR: 本文扩展了线性函数逼近Q学习的稳定性分析，发现在非表格或线性MDP设置下，即使基函数理想（真实Q函数在其张成空间中），投影贝尔曼方程仍可能有多个解，算法在无意识训练下不稳定。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明，在(ε,κ)-驯服Gibbs策略下，线性函数逼近的Q学习具有稳定性（参数估计有界），且投影贝尔曼方程有解。但未解决解的唯一性问题，也未给出标准表格或线性MDP设置之外的收敛准则。

Method: 扩展先前结果到其他Q学习变体，通过一维示例展示在无意识训练策略下，投影贝尔曼方程可能无解或多解，且算法不稳定。提出需要更多结构保证收敛，给出基函数理想但投影贝尔曼方程仍有两个解的例子。

Result: 发现即使基函数理想（真实Q函数在其张成空间中），在贪婪策略下投影贝尔曼方程仍可能有多个解，对于所有足够小的ε>0和κ≥1的(ε,κ)-驯服Gibbs策略也是如此。表明需要更多结构条件才能保证收敛。

Conclusion: 线性函数逼近Q学习的收敛需要比先前认识更多的结构条件。即使基函数理想，投影贝尔曼方程仍可能有多解，导致算法在无意识训练下不稳定。这澄清了先前工作，强调需要更强的假设来保证唯一性和收敛性。
...
TL;DR: MoSE提出了一种具有可调节宽度的专家混合架构，允许在推理时通过动态调整专家宽度实现更连续的计算精度权衡
...
Method: 提出MoSE架构，每个专家具有嵌套的可调节结构，支持可变宽度执行；结合多宽度训练和标准MoE目标，并探索运行时宽度确定策略，包括基于路由器置信度映射到专家宽度的轻量级测试时训练

Result: 在OpenWebText上训练的GPT模型实验中，MoSE在全宽度下匹配或优于标准MoE，并显著改善了精度与成本之间的帕累托前沿，以更少FLOPs实现相当性能

Conclusion: MoSE通过引入可调节宽度的专家结构，为MoE模型提供了更连续的计算精度权衡，增强了推理时的灵活性和效率
...
### [143] [Latent Structure Emergence in Diffusion Models via Confidence-Based Filtering](https://arxiv.org/abs/2602.06155)
*Wei Wei,Yizhou Zeng,Kuntian Chen,Sophie Langer,Mariia Seleznova,Hung-Hsu Chou*

Main category: cs.LG

TL;DR: 研究发现扩散模型的初始噪声潜在空间在高置信度样本中展现出明显的类别可分性，这为条件生成提供了新方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型依赖高维初始噪声种子空间，但尚不清楚该空间是否包含足够结构来预测生成样本的属性（如类别）。本研究旨在探索潜在空间中是否存在与类别相关的结构。

Method: 通过预训练分类器为生成样本分配置信度分数，分析不同置信度水平下噪声子集中的类别可预测性，比较潜在空间的类别可分性。

Result: 虽然考虑所有噪声实现时潜在空间基本无结构，但仅关注产生高置信度样本的初始噪声种子时，显示出明显的类别可分性。置信度过滤揭示了仅在特定条件下可观察到的类别相关潜在结构。

Conclusion: 置信度过滤能够揭示扩散模型潜在空间中的类别结构，为条件生成提供了基于引导方法的替代方案，展示了噪声空间中隐藏的语义组织。
...
### [144] [SCONE: A Practical, Constraint-Aware Plug-in for Latent Encoding in Learned DNA Storage](https://arxiv.org/abs/2602.06157)
*Cihan Ruan,Lebin Zhou,Rongduo Han,Linyi Han,Bingqing Zhao,Chenchen Zhu,Wei Jiang,Wei Wang,Nam Ling*

Main category: cs.LG

TL;DR: SCONE是一个将神经压缩与DNA编码结合的插件模块，通过四进制算术编码直接在潜在空间执行DNA编码，动态调整熵模型以满足生化约束，实现端到端的DNA兼容学习编解码器。


<details>
  <summary>Details</summary>
Motivation: 当前DNA存储与神经压缩流水线的集成效率低下。现有的方法要么在原始二进制数据上应用冗余约束层（原始但低效），要么将学习到的潜在表示通过简单的二进制到四进制转码转换为DNA，丢弃了熵模型的优化。这种不匹配削弱了压缩效率并使编码堆栈复杂化。

Method: SCONE采用四进制算术编码直接在潜在空间进行DNA编码。其约束感知自适应编码模块动态引导熵编码器的学习概率分布，在编码过程中确定性地强制执行生化约束（GC平衡和均聚物抑制），无需后处理校正。该设计保持了完全可逆性，并利用了超先验模型的学习先验而无需修改。

Result: 实验表明SCONE实现了近乎完美的约束满足，计算开销可忽略不计（<2%延迟），为端到端DNA兼容学习编解码器建立了潜在无关的接口。

Conclusion: SCONE成功地将潜在压缩和DNA编码融合为单一步骤，通过约束感知自适应编码在DNA编码过程中直接强制执行生化约束，显著提高了DNA存储与神经压缩流水线的集成效率。
...
TL;DR: 提出一种利用硬件加速稀疏性来加速Transformer中FFN矩阵乘法的方法，结合稀疏训练和密集训练，可实现1.4-1.7倍的训练加速，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型训练通常受限于矩阵乘法运算瓶颈，特别是在Transformer的FFN层中，这部分运算在大型模型中占比可达总预训练浮点运算的50%，需要寻找加速方法。

Method: 采用硬件加速稀疏性技术，对FFN中的权重使用2:4稀疏模式，对激活使用v:n:m(Venom)稀疏模式。训练过程结合稀疏训练步骤（加速大部分预训练）和常规密集训练步骤（接近训练结束时使用）。

Result: 使用该方法训练的模型在质量基准测试中表现相同，端到端训练速度可提升1.4到1.7倍。该方法适用于从A100开始的所有NVIDIA GPU，且与量化等常见优化技术正交，也可应用于专家混合模型架构。

Conclusion: 通过硬件加速稀疏性技术优化FFN矩阵乘法，可以在保持模型性能的同时显著加速大语言模型的训练过程，为模型训练效率提升提供了有效解决方案。
...
### [146] [$f$-FUM: Federated Unlearning via min--max and $f$-divergence](https://arxiv.org/abs/2602.06187)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习需要在保护隐私的同时满足"被遗忘权"等法律伦理要求，并抵御数据中毒攻击，但分布式特性使个体数据贡献的删除变得复杂

Method: 将联邦遗忘问题表述为最小-最大优化问题：最大化包含所有数据训练的模型与删除特定数据点后重新训练的模型之间的f-散度，同时最小化对保留数据性能的影响

Result: 该方法相比朴素重新训练实现了显著加速，对模型效用影响最小，可作为插件应用于几乎所有联邦学习设置

Conclusion: 提出的联邦遗忘框架为联邦学习中的高效数据删除提供了实用解决方案，能够满足法律伦理要求并增强系统安全性
...
### [147] [Learning Rate Scaling across LoRA Ranks and Transfer to Full Finetuning](https://arxiv.org/abs/2602.06204)
*Nan Chen,Soledad Villar,Soufiane Hayou*

Main category: cs.LG

TL;DR: 本文提出Maximal-Update Adaptation (μA)理论框架，用于确定LoRA微调中学习率随适配器秩的最优缩放规则，发现两种不同缩放模式，并实现从LoRA到全参数微调的学习率迁移。


<details>
  <summary>Details</summary>
Motivation: LoRA作为大模型参数高效微调的标准工具，其训练动态复杂且依赖多个超参数，特别是学习率如何随适配器秩缩放不明确，导致每次改变秩时都需要重新调整学习率，增加了调优成本。

Method: 提出Maximal-Update Adaptation (μA)理论框架，借鉴预训练中的Maximal-Update Parametrization (μP)，利用超参数迁移技术分析学习率随模型宽度和适配器秩的最优缩放规则，识别不同初始化配置下的两种缩放模式。

Result: 理论分析揭示了两种缩放模式：一种情况下最优学习率在不同秩间基本不变，另一种情况下与秩成反比缩放。实验在语言、视觉、视觉-语言、图像生成和强化学习任务中验证了缩放规则，并成功实现了从LoRA到全参数微调的学习率迁移。

Conclusion: μA框架为LoRA微调提供了理论指导，明确了学习率随适配器秩的缩放规则，显著降低了学习率调优成本，特别是在从LoRA迁移到全参数微调时效果显著。
...
### [148] [Multi-Way Representation Alignment](https://arxiv.org/abs/2602.06205)
...
<details>
  <summary>Details</summary>
Motivation: 现有模型对齐方法主要针对成对模型，无法扩展到多个模型，且缺乏一致的全局参考空间。同时，严格的等距对齐在检索任务中效果不佳，而最大化一致性的方法（如CCA）又无法保持几何结构。

Method: 1. 将广义Procrustes分析（GPA）扩展到多个模型，构建共享正交空间以保持内部几何结构；2. 提出几何校正Procrustes对齐（GCPA），先建立GPA基础空间，再通过后处理校正方向不匹配问题。

Result: 实验表明GCPA在任意模型间的检索任务中表现一致优于其他方法，同时保留了实用的共享参考空间。

Conclusion: GCPA成功解决了多模型对齐中的几何保持与检索性能的权衡问题，为构建一致的全局参考空间提供了有效方案。
...
### [149] [SR4-Fit: An Interpretable and Informative Classification Algorithm Applied to Prediction of U.S. House of Representatives Elections](https://arxiv.org/abs/2602.06229)
*Shyam Sundar Murali Krishnan,Dean Frederick Hougen*

Main category: cs.LG

TL;DR: SR4-Fit是一种新型可解释分类算法，在保持高预测性能的同时解决了传统规则算法预测能力不足和不稳定的问题，成功应用于美国国会选举预测和其他六个公开数据集。


<details>
  <summary>Details</summary>
Motivation: 机器学习在关键应用中需要可解释模型，但高性能模型往往是"黑箱"系统，而传统基于规则的算法（如RuleFit）虽然简单但预测能力不足且不稳定，这促使开发SR4-Fit算法来解决这些限制。

Method: 开发了稀疏松弛正则化回归规则拟合（SR4-Fit）算法，这是一种新型可解释分类算法。使用美国人口普查局美国社区调查的国会选区人口特征数据，预测众议院选举党派结果，并在六个公开分类数据集（乳腺癌、大肠杆菌、页面块、皮马印第安人、车辆和酵母数据集）上进一步验证。

Result: SR4-Fit能够以前所未有的准确性和可解释性预测众议院选举结果。结果显示多数党仍然是最强预测因子，但SR4-Fit揭示了影响预测结果的人口因素内在组合，这些在黑箱算法（如随机森林）中无法解释。该算法在准确性、简单性和鲁棒性方面均优于黑箱模型和现有可解释规则算法。

Conclusion: SR4-Fit算法在保持优异预测性能的同时生成稳定且可解释的规则集，解决了传统模型可解释性与预测能力之间的权衡问题，为选举预测和其他分类任务提供了有效的解决方案。
...
### [150] [RuleSmith: Multi-Agent LLMs for Automated Game Balancing](https://arxiv.org/abs/2602.06232)
...
<details>
  <summary>Details</summary>
Motivation: 游戏平衡是一个长期存在的挑战，需要反复测试、专家直觉和大量手动调整。传统方法耗时耗力，需要自动化解决方案来优化复杂多智能体环境中的游戏平衡。

Method: 结合游戏引擎、多智能体LLM自博弈和贝叶斯优化，在可调参数的多维规则空间中搜索。LLM智能体通过解读文本规则书和游戏状态来生成行动，快速评估胜率差异等平衡指标。采用基于采集函数的自适应采样和离散投影，对候选参数进行高效评估。

Result: 在简化的文明风格游戏CivMini上验证，RuleSmith能够收敛到高度平衡的配置，并提供可直接应用于下游游戏系统的可解释规则调整。实验表明LLM模拟可以作为自动化复杂多智能体环境设计和平衡的强大替代方案。

Conclusion: RuleSmith框架成功展示了利用多智能体LLM推理能力实现自动化游戏平衡的可行性，为复杂游戏系统的设计和平衡提供了高效、可解释的自动化解决方案。
...
### [151] [Provably avoiding over-optimization in Direct Preference Optimization without knowing the data distribution](https://arxiv.org/abs/2602.06239)
...
Method: 使用基于集成的方法：在不相交数据子集上训练多个偏好优化策略，然后通过最坏情况构造进行聚合，强调模型间的一致性。

Result: 在表格设置中，PEPO实现了仅依赖于单策略集中系数的样本复杂度保证，避免了影响DPO等易过度优化算法的全策略集中系数问题。实验验证了其实际性能优势。

Conclusion: PEPO通过集成悲观方法有效缓解了偏好学习中的过度优化问题，在保持DPO简单性的同时提供了更好的理论保证和实际性能。
...
### [152] [REBEL: Hidden Knowledge Recovery via Evolutionary-Based Evaluation Loop](https://arxiv.org/abs/2602.06248)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM遗忘方法的评估指标存在缺陷，主要依赖良性查询，可能误将表面信息抑制视为真正的知识移除，无法检测更复杂提示策略仍能提取的残留知识。
...
Conclusion: 当前LLM遗忘方法可能只提供表面保护，对抗性提示能有效恢复"遗忘"知识，需要更严格的评估方法来确保真正的知识移除。
...
### [153] [Steering Safely or Off a Cliff? Rethinking Specificity and Robustness in Inference-Time Interventions](https://arxiv.org/abs/2602.06256)
*Navita Goyal,Hal Daumé*

Main category: cs.LG

TL;DR: 该研究提出了模型引导特异性的评估框架，包含通用性、控制性和鲁棒性三个维度，发现在安全关键应用（如减少过度拒绝和幻觉）中，引导方法虽然有效且保持通用和控制特异性，但普遍缺乏鲁棒特异性，可能损害模型安全性。


<details>
  <summary>Details</summary>
Motivation: 模型引导作为轻量级替代微调的方法，在精确控制大语言模型方面得到广泛应用，但现有研究主要关注引导效果，对引导是否仅改变目标属性而保持其他相关行为不变（即特异性）的评估有限，尤其是在安全关键应用中。

Method: 提出了一个区分三个特异性维度的评估框架：通用性（保持流畅性和无关能力）、控制性（保持相关控制属性）、鲁棒性（在分布偏移下保持控制属性）。研究两个安全关键用例：减少过度拒绝和减少忠实性幻觉。

Result: 研究发现，虽然引导方法在减少过度拒绝方面效果显著，且能保持通用性和控制特异性（如对有害查询的拒绝能力），但所有引导方法都严重缺乏鲁棒特异性，显著增加了对越狱攻击的脆弱性。

Conclusion: 这是首个系统评估模型引导特异性的研究，表明标准的效果和特异性检查不足，因为缺乏鲁棒性评估时，引导方法可能在损害模型安全性的情况下仍显得可靠。
...
### [154] [On Randomized Algorithms in Online Strategic Classification](https://arxiv.org/abs/2602.06257)
*Chase Hutton,Adam Melrod,Han Shao*

Main category: cs.LG

TL;DR: 该论文研究了在线战略分类问题，改进了在战略环境下的学习算法边界。在可实现设置中，首次将下界扩展到所有学习者（包括随机化算法），并提出了改进的随机化学习器。在不可知设置中，通过凸优化技术将遗憾上界提升到接近标准在线学习率，并证明了在适当学习规则中的最优性。


<details>
  <summary>Details</summary>
Motivation: 在线战略分类中，智能体为了获得有利预测会策略性地修改特征（如申请人为获得贷款批准调整信用评分）。现有研究对随机化算法探索不足：在可实现设置中，随机化算法没有已知下界；在不可知设置中，现有遗憾上界远低于标准在线学习率。需要改进这些边界。

Method: 在可实现设置中，将现有确定性学习器的下界扩展到所有学习者（包括随机化算法），并设计了改进的随机化学习器。在不可知设置中，使用凸优化技术构建了一个适当学习器，显著改善了遗憾上界。

Result: 1. 可实现设置：首次证明了适用于随机化学习器的下界Ω(Ldim(H)Δ)，并提出了改进的随机化学习器，优于已知的确定性上界O(Ldim(H)·ΔlogΔ)。2. 不可知设置：将遗憾上界改进到O(√(Tlog|H|) + |H|log(T|H|))，接近标准在线学习率O(√(Tlog|H|))，并证明了在适当学习规则中的最优性（匹配下界）。

Conclusion: 该研究显著改进了在线战略分类的边界：在可实现设置中填补了随机化算法的理论空白，在不可知设置中实现了接近最优的遗憾保证。结果表明，要进一步提升遗憾保证需要非适当学习（improper learning）。
...
### [155] [GRP-Obliteration: Unaligning LLMs With a Single Unlabeled Prompt](https://arxiv.org/abs/2602.06258)
*Mark Russinovich,Yanan Cai,Keegan Hines,Giorgio Severi,Blake Bullwinkel,Ahmed Salem*

Main category: cs.LG

TL;DR: GRP-Obliteration是一种使用Group Relative Policy Optimization直接移除目标模型安全约束的方法，仅需单个无标签提示即可可靠地解除安全对齐模型，同时保持其效用，并能推广到语言模型和扩散图像生成系统。


<details>
  <summary>Details</summary>
Motivation: 尽管已有大量安全后训练工作，但模型在部署后仍可通过微调轻易解除对齐。现有方法通常需要大量数据整理且会降低模型效用，因此需要更实用、高效的解除对齐方法。

Method: 引入GRP-Obliteration方法，使用Group Relative Policy Optimization直接移除目标模型的安全约束。该方法仅需单个无标签提示即可可靠解除安全对齐，同时保持模型效用。

Result: GRP-Oblit在15个7-200亿参数模型上评估，涵盖6个效用基准和5个安全基准，平均上比现有最先进技术实现更强的解除对齐效果，并能推广到扩散图像生成系统。

Conclusion: GRP-Obliteration扩展了解除对齐的实际限制，展示了仅需最小输入即可有效移除安全约束的能力，同时保持模型效用，为安全对齐的鲁棒性提供了重要见解。
...
### [156] [PurSAMERE: Reliable Adversarial Purification via Sharpness-Aware Minimization of Expected Reconstruction Error](https://arxiv.org/abs/2602.06269)
...
<details>
  <summary>Details</summary>
Motivation: 现有随机净化方法在对抗者完全了解系统和随机性的情况下，有效鲁棒性会下降。需要一种确定性方法来确保可靠的测试精度和对抗鲁棒性。

Method: 使用最小化噪声污染数据期望重构误差训练的得分模型学习数据分布结构特征。对潜在对抗输入，在其邻域搜索最小化噪声污染下期望重构误差的净化样本，然后输入分类器。净化过程中使用锐度感知最小化引导净化样本到期望重构误差的平坦区域。

Result: 实验结果表明，在强确定性白盒攻击下，该方法在对抗鲁棒性方面显著优于最先进方法。理论证明当噪声水平降低时，最小化期望重构误差会使净化样本偏向高斯平滑密度的局部最大化器。
...
### [157] [Statistical Learning from Attribution Sets](https://arxiv.org/abs/2602.06276)
*Lorne Applebaum,Robert Busa-Fekete,August Y. Chen,Claudio Gentile,Tomer Koren,Aryan Mokhtari*

Main category: cs.LG

TL;DR: 在隐私约束下训练广告转化预测模型，其中点击和转化之间的直接链接不可用，只能通过候选点击集合（归因集）进行学习。


<details>
  <summary>Details</summary>
Motivation: 受隐私保护浏览器API和第三方cookie弃用的驱动，研究在无法直接链接点击和转化的情况下，如何从粗粒度的归因信号中学习转化预测模型。

Method: 提出一种新颖的无偏估计方法，从归因集中构建总体损失的无偏估计器，使用经验风险最小化（ERM）进行学习，该方法对先验分布的估计误差具有鲁棒性。

Result: ERM方法能够获得与先验信息量相关的泛化保证，并且在标准数据集上的简单实证评估表明，该方法显著优于常见的行业启发式方法，特别是在归因集较大或重叠的情况下。

Conclusion: 即使在隐私约束下无法获得明确的点击-转化链接，通过从归因集中构建无偏估计器，仍然可以有效地训练转化预测模型，为隐私保护广告环境中的机器学习提供了理论保证和实用方法。
...
### [158] [SOCKET: SOft Collison Kernel EsTimator for Sparse Attention](https://arxiv.org/abs/2602.06283)
...
TL;DR: SOCKET提出了一种基于软碰撞核估计的稀疏注意力机制，通过概率化的局部敏感哈希替代硬桶匹配，提升长上下文推理效率，在多个基准测试中达到或超越现有稀疏注意力基线，吞吐量比FlashAttention高1.5倍。


<details>
  <summary>Details</summary>
Motivation: 长上下文推理中注意力计算成本高昂，稀疏注意力通过限制计算到相关token子集来降低成本，但现有方法依赖硬哈希桶匹配，产生离散碰撞信号，不适合精确排名。

Method: 提出SOCKET（SOft Collision Kernel EsTimator），将局部敏感哈希从硬桶匹配转换为概率化、相似性感知的聚合，通过软碰撞核在多个哈希表中聚合分级碰撞证据，保持top-k token的相对排序稳定性。

Result: 在多个长上下文基准测试中匹配或超越现有稀疏注意力基线，通过定制CUDA核和Flash Decode Triton后端，实现比FlashAttention高1.5倍的吞吐量提升。

Conclusion: SOCKET将LSH从候选生成启发式方法提升为稀疏注意力的原则性数学基础评分核，无需特殊投票机制即可实现高效token选择，是长上下文推理的有效工具。
...
### [159] [The Condensate Theorem: Transformers are O(n), Not $O(n^2)$](https://arxiv.org/abs/2602.06317)
*Jorge L. Ruiz Williams*

Main category: cs.LG

TL;DR: 注意力稀疏性是学习到的拓扑特性而非架构约束，通过识别注意力质量聚集的拓扑流形（锚点+窗口+动态Top-k），可实现与完整O(n²)注意力100%输出等价，大幅提升推理速度


<details>
  <summary>Details</summary>
Motivation: 传统注意力机制的二次复杂度被认为是不可避免的架构瓶颈，但作者发现注意力质量实际上集中在特定的拓扑流形上，这为突破二次复杂度限制提供了理论依据

Method: 提出凝聚定理：注意力稀疏性是学习到的拓扑特性；识别注意力质量聚集的Condensate流形（锚点+窗口+动态Top-k）；通过投影到该流形实现与完整注意力的无损等价；开发拓扑注意力内核映射到硬件
...
Conclusion: 二次复杂度瓶颈是朴素实现的产物而非智能的本质特性，注意力稀疏性是学习到的拓扑性质，通过识别和利用这一拓扑结构可以大幅提升推理效率
...
### [160] [How (Not) to Hybridize Neural and Mechanistic Models for Epidemiological Forecasting](https://arxiv.org/abs/2602.06323)
*Yiqi Su,Ray Lee,Jiaming Cui,Naren Ramakrishnan*

Main category: cs.LG

TL;DR: 该论文提出了一种结合机制性流行病模型与神经ODE的混合方法，通过显式建模非平稳性来改进流行病预测，在多种疫情场景下显著提升了预测精度。


<details>
  <summary>Details</summary>
Motivation: 流行病预测面临部分可观测性和不断变化的传播动态（如行为变化、免疫力减弱、季节性和干预措施）的挑战。现有的机制模型与神经模型混合方法在实践中往往失败，需要更鲁棒的方法来处理非平稳性。

Method: 从观测到的感染序列中提取多尺度结构作为可解释的控制信号，驱动与流行病模型耦合的受控神经ODE。具体方法包括：将感染分解为趋势、季节性和残差分量，用这些信号驱动连续时间潜在动态，同时联合预测和推断时变的传播率、恢复率和免疫损失率。

Result: 在季节性和非季节性场景（包括早期爆发和多波疫情）中，该方法相比强基线（时间序列、神经ODE和混合模型）将长期预测RMSE降低了15-35%，峰值时间误差改善了1-3周，峰值幅度偏差降低了高达30%，且不依赖辅助协变量。

Conclusion: 通过显式建模非平稳性并将多尺度结构作为控制信号，可以构建更鲁棒的混合流行病预测模型，显著提高在各种动态变化条件下的预测性能。
...
### [161] [Don't Break the Boundary: Continual Unlearning for OOD Detection Based on Free Energy Repulsion](https://arxiv.org/abs/2602.06331)
*Ningkang Peng,Kun Shao,Jingyang Mao,Linjing Qian,Xiaoqian Peng,Xichen Yang,Yanhui Gu*

Main category: cs.LG

TL;DR: TFER框架通过自由能排斥力实现边界保持的类别遗忘，将目标类别转化为OOD样本，同时保持ID数据流形的完整性，解决了OOD检测与机器遗忘之间的几何矛盾。
...
Conclusion: TFER框架通过将遗忘类别转化为OOD样本的数学等价性，解决了OOD检测与机器遗忘之间的根本矛盾，为开放世界可信AI部署提供了有效的边界保持遗忘方案，具有结构稳定性和持续遗忘能力。
...
### [162] [Adversarial Learning in Games with Bandit Feedback: Logarithmic Pure-Strategy Maximin Regret](https://arxiv.org/abs/2602.06348)
*Shinji Ito,Haipeng Luo,Arnab Maiti,Taira Tsuchiya,Yue Wu*

Main category: cs.LG

TL;DR: 研究零和博弈中对抗性学习问题，针对两种反馈模型（无信息和有信息）提出算法，实现对数级后悔界


<details>
  <summary>Details</summary>
Motivation: 现实应用中，学习者常需面对未知对手且只能获得强盗反馈（仅能观察到所选动作的收益），传统方法存在Ω(√T)的不可避免外部后悔，需要新的度量标准来克服这一障碍

Method: 1) 针对无信息反馈模型，使用Tsallis-INF算法处理正规形式博弈；2) 针对有信息反馈模型，提出Maximin-UCB算法；3) 将结果推广到任意大动作集的双线性博弈，分别提出Tsallis-FTRL-SPM和Maximin-LinUCB算法

Result: 1) 无信息反馈下获得O(c log T)的实例依赖后悔界，并证明对c的依赖是必要的；2) 有信息反馈下获得O(c' log T)后悔界，其中c'可能远小于c；3) 在双线性博弈中建立了类似的博弈依赖对数后悔界

Conclusion: 通过引入纯策略极大极小后悔度量，在两种强盗反馈模型下实现了对数级后悔界，克服了传统Ω(√T)的障碍，为对抗性学习提供了有效的理论保证
...
### [163] [Enhance and Reuse: A Dual-Mechanism Approach to Boost Deep Forest for Label Distribution Learning](https://arxiv.org/abs/2602.06353)
*Jia-Le Xu,Shen-Huan Lyu,Yu-Nian Wang,Ning Chen,Zhihao Qu,Bin Tang,Baoliu Ye*

Main category: cs.LG

TL;DR: 提出了一种名为ERDF的新方法，通过特征增强和特征重用机制，将深度森林应用于标签分布学习，有效利用标签相关性并提升性能。


<details>
  <summary>Details</summary>
Motivation: 标签分布学习需要预测样本与每个标签的相关程度，而利用标签间的相关性是关键。现有将深度森林应用于LDL的方法未能有效利用标签相关性，因此需要新的解决方案。

Method: 提出了ERDF方法，包含两个核心机制：1）利用标签相关性增强原始特征，使样本获得更全面的信息；2）对验证集上表现不如前一层的样本特征进行重用操作，确保训练过程的稳定性。

Result: 实验表明，该方法在六个评估指标上优于其他对比算法。

Conclusion: ERDF方法通过增强-重用模式，不仅丰富了样本特征，还验证了新特征的有效性，防止噪声传播，在标签分布学习任务中表现出色。
...
### [164] [Evaluating LLM-persona Generated Distributions for Decision-making](https://arxiv.org/abs/2602.06357)
...
<details>
  <summary>Details</summary>
Motivation: LLM能够生成丰富数据（如模拟消费者偏好、需求预测），但需要评估这些LLM生成的分布对下游决策的实际支持效果，特别是在商业决策场景中。

Method: 提出LLM-SAA方法：使用LLM构建估计分布，然后在该分布下优化决策。通过三个经典决策问题（品类优化、定价、报童问题）评估分布质量，比较决策相关指标与传统分布距离指标。

Result: LLM生成的分布在低数据场景下具有实际应用价值，特别是在数据稀缺时。同时发现，与决策无关的指标（如Wasserstein距离）在评估这些分布对决策的效用时可能产生误导。

Conclusion: LLM生成的分布能够有效支持下游决策，尤其是在数据有限的情况下。评估这类分布时应采用与决策相关的指标，而非传统的分布距离度量。
...
### [165] [Uniform Spectral Growth and Convergence of Muon in LoRA-Style Matrix Factorization](https://arxiv.org/abs/2602.06385)
*Changmin Kang,Jihun Yun,Baekrok Shin,Yeseul Cho,Chulhee Yun*

Main category: cs.LG

TL;DR: 论文研究了谱梯度下降在LoRA微调中的独特现象：LoRA乘积的奇异值呈现近乎均匀的增长，这与标准梯度流中最大奇异值优先学习的特点形成鲜明对比。


<details>
  <summary>Details</summary>
Motivation: 虽然谱梯度下降（如Muon优化器）在大语言模型训练中表现良好，但其动态特性仍未被充分理解。特别是在LoRA微调中观察到奇异值均匀增长的独特现象，需要理论解释。

Method: 在简化的LoRA风格矩阵分解设置中分析谱梯度流（SpecGF）——谱梯度下降的连续时间类比。使用理论分析证明奇异值的"等速率"动态特性，并通过实验验证。

Result: 证明了在LoRA设置中，谱梯度流导致所有奇异值以近乎相等的速率增长，小奇异值比大奇异值更早达到目标值。在因子范数有界条件下，SpecGF几乎从所有初始化收敛到全局最小值；使用ℓ₂正则化时可获得全局收敛。

Conclusion: 谱梯度下降在LoRA微调中产生独特的奇异值均匀增长动态，这与传统梯度方法形成鲜明对比。该理论分析为理解Muon等优化器在LLM训练中的表现提供了理论基础。
...
### [166] [Generating High-quality Privacy-preserving Synthetic Data](https://arxiv.org/abs/2602.06390)
...
<details>
  <summary>Details</summary>
Motivation: 合成表格数据在分享和分析敏感记录时需要平衡分布保真度、下游效用和隐私保护，现有方法在这三个方面的权衡仍有改进空间。

Method: 提出模型无关的后处理框架：1) 模式修补步骤修复合成数据中缺失或严重不足的类别，同时保留学习到的依赖关系；2) k近邻过滤器替换过于接近真实数据点的合成记录，强制实施真实与合成样本之间的最小距离。

Result: 在0.2-0.35的中等阈值下，后处理使真实与合成分类分布之间的差异减少高达36%，成对依赖保持的综合指标提高10-14%，下游预测性能保持在未处理基线的约1%内，同时基于距离的隐私指标改善，属性推断攻击成功率基本不变。
...
### [167] [Near-Optimal Regret for Distributed Adversarial Bandits: A Black-Box Approach](https://arxiv.org/abs/2602.06404)
*Hao Qiu,Mengxiao Zhang,Nicolò Cesa-Bianchi*

Main category: cs.LG

TL;DR: 本文研究了分布式对抗性老虎机问题，提出了基于延迟反馈黑盒约简的新算法，显著改进了现有最优遗憾界，并证明了匹配的下界。


<details>
  <summary>Details</summary>
Motivation: 研究分布式对抗性老虎机问题，其中多个智能体通过通信协作最小化全局平均损失，但只能观测各自的局部损失。现有算法存在遗憾界不够紧的问题，需要更优的算法设计和理论分析。

Method: 提出基于延迟反馈黑盒约简的新算法，智能体仅通过gossip通信。算法将分布式问题转化为具有延迟反馈的单智能体老虎机问题，利用通信矩阵的谱间隙ρ来量化通信效率。

Result: 证明了最小最大遗憾为$\tildeΘ(\sqrt{(ρ^{-1/2}+K/N)T})$，显著改进了Yi和Vojnovic (2023)的$\tilde{O}(ρ^{-1/3}(KT)^{2/3})$上界。下界表明问题难度可分解为通信成本$ρ^{-1/4}\sqrt{T}$和老虎机成本$\sqrt{KT/N}$。算法在分布式线性老虎机中实现了$\tilde{O}(\sqrt{(ρ^{-1/2}+1/N)dT})$遗憾界，每个智能体每轮仅需$O(d)$通信成本。

Conclusion: 本文为分布式对抗性老虎机问题提供了紧的遗憾界，证明了通信成本与老虎机成本的分离，并通过黑盒约简方法展示了算法的通用性，可扩展到一阶界和best-of-both-worlds界。
...
### [168] [Adaptive Protein Tokenization](https://arxiv.org/abs/2602.06418)
...
<details>
  <summary>Details</summary>
Motivation: 现有蛋白质结构标记化方法通过局部邻域池化信息创建标记，限制了在生成和表示任务上的性能。需要一种全局标记化方法来解决这些问题

Method: 开发了一种全局蛋白质结构标记化方法，其中连续标记为全局表示贡献递增的细节层次。该方法支持任务特定的标记序列信息内容适应

Result: 在重建、生成和表示任务上验证了该方法，匹配或超越了基于局部蛋白质结构标记化的现有模型。自适应标记实现了基于信息内容的推理标准，提高了可设计性
...
### [169] [Reclaiming First Principles: A Differentiable Framework for Conceptual Hydrologic Models](https://arxiv.org/abs/2602.06429)
*Jasper A. Vrugt,Jonathan M. Frame,Ethan Bollman*

Main category: cs.LG

TL;DR: 提出了一种基于精确参数敏感性的全解析可微水文建模框架，用于概念水文模型的快速稳定梯度校准


<details>
  <summary>Details</summary>
Motivation: 传统概念水文模型校准速度慢且数值脆弱，基于有限差分或自动微分框架的梯度参数估计方法计算量大，存在截断误差、求解器不稳定和显著开销，特别是在处理概念流域模型的ODE系统时问题尤为突出

Method: 通过向控制ODE系统添加敏感性方程，联合演化模型状态和关于所有参数的雅可比矩阵，为任何可微损失函数提供完全解析的梯度向量

Result: 解析敏感性消除了数值微分的步长依赖性和噪声，避免了伴随方法的不稳定性和现代机器学习自动微分工具链的开销，得到的梯度具有确定性、物理可解释性，易于嵌入基于梯度的优化器

Conclusion: 该工作实现了概念水文模型的快速、稳定和透明的梯度校准，无需依赖外部、不透明或CPU密集的自动微分库，释放了可微建模的全部潜力
...
### [170] [Is Gradient Ascent Really Necessary? Memorize to Forget for Machine Unlearning](https://arxiv.org/abs/2602.06441)
...
Method: 提出模型外推法：1) 以原始模型为参考，训练记忆模型记住不需要的数据；2) 从记忆模型外推到参考模型获得遗忘模型；3) 避免直接使用梯度上升，改用梯度下降稳定训练

Result: 模型外推法简单高效，能有效收敛并提升遗忘性能，稳定机器遗忘过程
...
### [171] [BrokenBind: Universal Modality Exploration beyond Dataset Boundaries](https://arxiv.org/abs/2602.06451)
*Zhuo Huang,Runnan Chen,Bo Han,Gang Niu,Masashi Sugiyama,Tongliang Liu*

Main category: cs.LG

TL;DR: BrokenBind是一种新颖的多模态学习方法，能够将来自不同数据集的不同模态绑定在一起，突破了传统方法只能处理同一数据集中模态的限制，实现了更灵活和通用的多模态学习。


<details>
  <summary>Details</summary>
Motivation: 现有多模态学习方法存在局限性：它们只能处理给定数据集中同时出现的模态，无法泛化到下游任务中未出现的模态。这种不灵活性严重阻碍了方法的实用性，因为获取多模态数据集的成本很高。需要一种能够跨数据集绑定不同模态的方法。

Method: BrokenBind同时利用多个包含感兴趣模态的数据集和一个共享模态。尽管这些数据集由于分布不匹配而不对应，但该方法能够捕捉它们之间的关系，生成伪嵌入来填补缺失的感兴趣模态，从而实现灵活和通用的多模态学习。

Result: 通过广泛评估，BrokenBind在性能上优于知名的多模态基线方法。该方法在需要两个以上数据集进行模态绑定的强化场景中表现出有效性，并且在低数据情况下也显示出良好的效果。

Conclusion: BrokenBind突破了数据集限制，使任何两个模态都能绑定在一起，实现了通用的模态探索。该方法为解决多模态学习中的数据获取成本问题提供了有效方案，具有重要的实际应用价值。
...
### [172] [On the Plasticity and Stability for Post-Training Large Language Models](https://arxiv.org/abs/2602.06453)
*Wenwen Qiang,Ziyin Gu,Jiahuan Zhou,Jie Hu,Jingyao Wang,Changwen Zheng,Hui Xiong*

Main category: cs.LG

TL;DR: 提出PCR（概率冲突解决）框架，通过贝叶斯建模将梯度视为随机变量，使用不确定性感知的"软投影"机制动态仲裁冲突，优化信噪比，显著平滑GRPO训练轨迹并提升推理任务性能。


<details>
  <summary>Details</summary>
Motivation: GRPO（组相对策略优化）的训练稳定性是关键瓶颈，表现为推理可塑性与通用能力保持之间的权衡。研究发现根本原因是可塑性和稳定性梯度之间的几何冲突导致破坏性干扰，而确定性投影方法忽略了基于组的梯度估计的内在随机性。

Method: 提出PCR（概率冲突解决）框架：1）将梯度建模为随机变量；2）使用贝叶斯方法；3）通过不确定性感知的"软投影"机制动态仲裁冲突；4）优化信号噪声比。

Result: 大量实验表明，PCR显著平滑了训练轨迹，在各种推理任务中实现了优越的性能。

Conclusion: PCR通过概率方法有效解决了GRPO中的训练稳定性问题，相比确定性投影方法更优，能够更好地处理基于组的梯度估计的随机性。
...
### [173] [The Window Dilemma: Why Concept Drift Detection is Ill-Posed](https://arxiv.org/abs/2602.06456)
*Brandon Gower-Winter,Misja Groen,Georg Krempl*

Main category: cs.LG

TL;DR: 论文指出传统漂移检测器存在"窗口困境"问题，即检测到的漂移可能是窗口划分的产物而非真实的数据生成过程变化，且漂移检测本身难以验证。实证研究表明传统批量学习方法往往优于漂移感知方法。


<details>
  <summary>Details</summary>
Motivation: 数据流中的概念漂移现象已被广泛研究，但传统漂移检测器基于窗口比较的方法存在根本性问题。作者旨在揭示漂移检测的局限性，特别是窗口划分对漂移感知的影响以及实际验证的困难。

Method: 首先通过理论分析提出"窗口困境"概念，说明漂移感知是窗口划分的产物；然后通过实证比较，将传统漂移检测器与多种替代适应策略进行对比，包括传统批量学习方法。

Result: 研究发现传统批量学习方法在流分类任务中通常比漂移感知方法表现更好，这进一步质疑了漂移检测器在流分类中的实际价值。

Conclusion: 漂移检测存在根本性问题，包括窗口困境和验证困难。传统批量学习方法可能比复杂的漂移检测方法更有效，这对当前流分类研究中漂移检测器的必要性提出了质疑。
...
### [174] [Achieving Better Local Regret Bound for Online Non-Convex Bilevel Optimization](https://arxiv.org/abs/2602.06457)
*Tingkai Jia,Haiguang Wang,Cheng Chen*

Main category: cs.LG

TL;DR: 该研究建立了在线双层优化的最优遗憾边界，提出了针对标准双层局部遗憾和窗口平均双层局部遗憾的最优算法。


<details>
  <summary>Details</summary>
Motivation: 现有在线双层优化算法虽然能够最小化标准双层局部遗憾或窗口平均双层局部遗憾，但其遗憾边界的最优性尚不明确，需要建立最优的遗憾边界。

Method: 针对标准双层局部遗憾，提出了达到最优遗憾Ω(1+V_T)的算法，最多需要O(T log T)次内层梯度评估；还开发了完全单循环算法，其遗憾边界包含额外的梯度变化项。针对窗口平均双层局部遗憾，设计了通过窗口分析捕捉次线性环境变化的算法。

Result: 建立了两种设置下的最优遗憾边界：标准双层局部遗憾达到Ω(1+V_T)，窗口平均双层局部遗憾达到Ω(T/W^2)。实验验证了理论发现并证明了所提方法的实际有效性。

Conclusion: 该研究为在线双层优化建立了最优的遗憾边界，提出的算法在理论和实践上都表现出优越性能，为机器学习中的在线双层优化问题提供了更优的解决方案。
...
### [175] [Towards Generalizable Reasoning: Group Causal Counterfactual Policy Optimization for LLM Reasoning](https://arxiv.org/abs/2602.06475)
...
<details>
  <summary>Details</summary>
Motivation: 现有奖励机制过于关注最终答案正确性，忽视了推理过程的质量。这导致：1）推理过程正确但答案错误的轨迹获得低分；2）推理逻辑有缺陷但幸运猜对答案的轨迹获得高分。这种奖励机制影响了推理能力的泛化。

Method: 提出Group Causal Counterfactual Policy Optimization方法：1）将多候选推理视为一系列反事实实验；2）设计情景因果反事实奖励，同时捕获鲁棒性（推理步骤在反事实扰动下保持稳定）和有效性（学习到的推理策略能跨问题迁移）；3）构建token级优势函数并优化策略，鼓励LLMs选择过程有效且反事实鲁棒的推理模式。

Result: 在多个基准测试上进行广泛实验，证明了该方法的优势，能够有效提升LLMs的推理泛化能力。

Conclusion: 通过因果反事实视角重新思考推理奖励机制，提出的方法能够训练LLMs学习可泛化的推理模式，解决了现有奖励机制忽视推理过程质量的问题，提升了推理能力的泛化性能。
...
### [176] [Adaptive Uncertainty-Aware Tree Search for Robust Reasoning](https://arxiv.org/abs/2602.06493)
*Zeen Song,Zihao Ma,Wenwen Qiang,Changwen Zheng,Gang Hua*

Main category: cs.LG

TL;DR: 该论文针对基于过程奖励模型的外部搜索框架中存在的分布外不确定性挑战，提出了不确定性感知树搜索方法，通过蒙特卡洛Dropout估计不确定性和强化学习控制器动态分配计算预算，有效缓解了分布外错误的影响。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂问题解决中采用基于过程奖励模型的外部搜索方法，但该框架存在根本性限制：当评估偏离训练分布的推理路径时，过程奖励模型存在认知不确定性，导致对分布外样本评分不可靠。

Method: 提出不确定性感知树搜索方法：1）通过蒙特卡洛Dropout估计不确定性；2）使用基于强化学习的控制器动态分配计算预算；3）建立理论框架证明标准搜索会导致线性遗憾累积，而不确定性感知策略可实现次线性遗憾。

Result: 实验证明该方法有效缓解了分布外错误的影响，相比标准搜索方法，不确定性感知策略能够更可靠地评估偏离训练分布的推理路径，提升复杂问题解决能力。

Conclusion: 通过系统分析过程奖励模型在分布外样本上的不确定性挑战，并设计不确定性感知树搜索方法，成功解决了外部搜索框架中的根本性限制，为大型语言模型的推理扩展提供了更可靠的解决方案。
...
### [177] [Evolutionary Generation of Multi-Agent Systems](https://arxiv.org/abs/2602.06511)
*Yuntong Hu,Matthew Trager,Yuting Zhang,Yi Zhang,Shuo Yang,Wei Xia,Stefano Soatto*

Main category: cs.LG

TL;DR: EvoMAS：一种基于进化算法的多智能体系统自动生成框架，通过结构化配置空间进化生成鲁棒可执行的MAS架构，在推理、软件工程和工具使用任务上超越人工设计和现有自动生成方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的多智能体系统设计存在两个主要问题：1）基于代码生成的自动方法常导致可执行性和鲁棒性问题；2）基于模板的方法限制了表达能力和适应性。需要一种既能自动生成又能保证质量和灵活性的方法。

Method: 将MAS生成建模为结构化配置生成问题，在配置空间进行进化生成。具体包括：从候选池选择初始配置，基于执行轨迹反馈指导的变异和交叉操作，迭代优化候选池和经验记忆。

Result: 在BBEH推理、SWE-Bench软件工程和WorkBench工具使用等多个基准测试中，EvoMAS均优于人工设计的MAS和现有自动生成方法。在BBEH上比EvoAgent提升10.5分，在WorkBench上提升7.1分，使用Claude-4.5-Sonnet时在SWE-Bench-Verified上达到79.1%，与排行榜前列相当。

Conclusion: EvoMAS通过进化生成方法有效解决了MAS自动生成中的可执行性和鲁棒性问题，同时保持了系统的表达能力和适应性，为复杂任务的多智能体系统设计提供了高效可靠的自动化方案。
...
### [178] [Live Knowledge Tracing: Real-Time Adaptation using Tabular Foundation Models](https://arxiv.org/abs/2602.06542)
*Mounir Lbath,Alexandre Paresy,Abdelkayoum Kaddouri,Alan André,Alexandre Ittah,Jill-Jênn Vie*

Main category: cs.LG

TL;DR: 提出基于表格基础模型（TFMs）的在线知识追踪新范式，通过双向注意力机制实时对齐测试序列与训练序列，无需离线训练，在多个数据集上实现竞争性预测性能并大幅加速。


<details>
  <summary>Details</summary>
Motivation: 现有深度知识追踪模型需要大量训练时间，且在处理短序列数据集时容易过拟合。需要一种更高效、实时的知识追踪方法。

Method: 利用表格基础模型（TFMs）进行在线知识追踪，采用双向注意力机制：同时关注时间步长和其他学生的交互历史，在推理时实时将测试序列与相关训练序列对齐，完全跳过训练步骤。

Result: 在多个规模递增的数据集上，该方法实现了竞争性的预测性能，同时获得了高达273倍的加速效果，特别是在观察到更多学生交互时表现更佳。

Conclusion: 基于TFMs的在线知识追踪方法提供了一种高效、实时的知识追踪新范式，避免了传统方法的训练开销和过拟合问题，具有显著的速度优势。
...
### [179] [Refining the Information Bottleneck via Adversarial Information Separation](https://arxiv.org/abs/2602.06549)
...
<details>
  <summary>Details</summary>
Motivation: 材料科学等领域中，实验数据往往包含大量测量噪声和实验伪影，标准正则化方法无法精确分离有意义的特征，而现有对抗适应方法依赖显式分离标签

Method: 提出AdverISF框架：1）自监督对抗机制强制任务相关特征与噪声表示之间的统计独立性；2）多层分离架构在特征层次间循环利用噪声信息，恢复被误判为噪声的特征

Result: 广泛实验表明AdverISF在数据稀缺场景下优于最先进方法，在真实材料设计任务中展现出优越的泛化性能

Conclusion: AdverISF能够有效分离任务相关特征与噪声，无需显式监督，在数据稀缺领域具有重要应用价值
...
### [180] [Learning to Allocate Resources with Censored Feedback](https://arxiv.org/abs/2602.06565)
*Giovanni Montanari,Côme Fiegel,Corentin Pla,Aadirupa Saha,Vianney Perchet*

Main category: cs.LG

TL;DR: 该论文研究在线资源分配问题，在每轮需要将预算B分配到K个臂上，面临截断反馈。臂只有在满足两个条件时才产生奖励：(1)臂根据未知参数的伯努利随机变量被激活，(2)分配预算超过未知参数分布的随机阈值。作者证明了Ω(T^{1/3})的遗憾下界，提出了RA-UCB算法在已知预算时达到Õ(√T)遗憾，未知预算时提出MG-UCB算法。


<details>
  <summary>Details</summary>
Motivation: 在线资源分配问题在实际应用中广泛存在，但现有研究通常假设反馈是完整的。本文考虑更现实的截断反馈场景，其中臂的激活和预算阈值都是随机的且参数未知。这种双重不确定性增加了问题的复杂性，需要同时处理参数估计和探索-利用权衡。

Method: 作者首先建立了信息论遗憾下界Ω(T^{1/3})。然后提出了RA-UCB算法，这是一种乐观算法，利用非平凡的参数估计和置信区间。当每轮预算B已知时，RA-UCB通过置信上界进行决策。对于未知的、轮次相关的预算，作者提出了MG-UCB算法，允许轮内切换和无穷小分配。

Result: 理论分析表明：1) 证明了信息论遗憾下界Ω(T^{1/3})；2) 当预算已知时，RA-UCB算法达到Õ(√T)的遗憾上界，在更强假设下甚至达到O(poly-log T)；3) 对于未知预算，MG-UCB算法能够匹配RA-UCB的遗憾保证；4) 在真实世界数据集上的实验验证了理论结果。

Conclusion: 本文解决了具有截断反馈的在线资源分配问题，建立了问题的理论下界，并提出了有效的算法。RA-UCB和MG-UCB算法在不同预算知识假设下都能达到接近最优的性能，为实际应用中的资源分配问题提供了理论保证和实用解决方案。
...
### [181] [Endogenous Resistance to Activation Steering in Language Models](https://arxiv.org/abs/2602.06941)
...
<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在面对任务不匹配的激活引导时表现出的抵抗能力，探索这种内源性抵抗机制的性质、影响因素及其对AI系统可控性的意义。

Method: 使用稀疏自编码器（SAE）潜在变量引导模型激活，分析不同规模模型（Llama-3.3-70B、Llama-3和Gemma-2系列）的ESR表现，识别与ESR相关的特定SAE潜在变量，并通过零消融、元提示和微调等方法验证因果关系和增强ESR。

Result: Llama-3.3-70B表现出显著的ESR，而较小模型较少出现该现象；识别出26个与离题内容激活相关的SAE潜在变量，零消融这些变量使多尝试率降低25%；通过元提示可将多尝试率提高4倍，通过微调可在较小模型中诱导ESR类似行为。

Conclusion: ESR现象表明大语言模型存在专门的内一致性检查电路，这种机制既能抵御对抗性操纵，也可能干扰有益的安全干预。理解和控制这些抵抗机制对于开发透明可控的AI系统至关重要。
...
### [182] [Exploring Sparsity and Smoothness of Arbitrary $\ell_p$ Norms in Adversarial Attacks](https://arxiv.org/abs/2602.06578)
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击通常使用ℓ_1、ℓ_2或ℓ_∞范数约束，但缺乏对范数参数p如何影响对抗扰动结构和感知特性的系统研究

Method: 采用两种已有的稀疏性度量，引入三种新的平滑性度量（基于平滑操作和泰勒近似），在多个真实图像数据集和不同模型架构上进行实证评估

Result: ℓ_1或ℓ_2范数在大多数情况下并非最优选择，最优p值取决于具体任务，p∈[1.3,1.5]时在稀疏和平滑攻击之间达到最佳权衡

Conclusion: 范数选择对对抗攻击设计和评估至关重要，需要基于原则进行选择而非随意使用ℓ_1、ℓ_2或ℓ_∞范数
...
### [183] [Degradation of Feature Space in Continual Learning](https://arxiv.org/abs/2602.06586)
...
<details>
  <summary>Details</summary>
Motivation: 集中式训练中特征分布的各向同性有助于形成结构良好的可泛化表示，而持续学习面临可塑性-稳定性困境，特征空间趋向各向异性。研究者想知道是否强制各向同性能在持续学习中平衡稳定性和可塑性，从而缓解灾难性遗忘。

Method: 使用对比持续学习技术在CIFAR-10和CIFAR-100数据集上进行实验，评估各向同性正则化对持续学习性能的影响。

Result: 各向同性正则化未能改善持续学习性能，实际上可能降低模型准确性。这表明各向同性在持续学习中不是合适的归纳偏置。

Conclusion: 集中式学习与持续学习在特征几何结构上存在本质差异：各向同性在集中式设置中有益，但在非平稳学习场景中可能不适用，持续学习需要不同的特征空间优化策略。
...
### [184] [DiTS: Multimodal Diffusion Transformers Are Time Series Forecasters](https://arxiv.org/abs/2602.06597)
*Haoran Zhang,Haixuan Liu,Yong Liu,Yunzhong Qiu,Yuxuan Wang,Jianmin Wang,Mingsheng Long*

Main category: cs.LG

TL;DR: DiTS是一种用于时间序列的扩散变换器架构，将内生和外生变量视为不同模态，通过双流Transformer块更好地捕捉变量间和变量内依赖关系，在概率预测任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有生成式时间序列模型未能很好地处理时间序列数据的多维特性。当前的扩散变换器架构依赖简单的条件控制和单流Transformer主干，未能充分利用协变量感知预测中的跨变量依赖关系。

Method: 提出DiTS架构，将内生和外生变量视为不同模态。设计专门针对时间序列数据的双流Transformer块，包含时间注意力模块（用于时间维度自回归建模）和变量注意力模块（用于跨变量建模）。利用多元依赖的低秩特性降低计算成本。

Result: DiTS在多个基准测试中实现了最先进的性能，无论是否存在未来外生变量观测值，都表现出比传统确定性深度预测模型更强的生成式预测能力。

Conclusion: DiTS通过将时间序列变量视为不同模态并设计专门的双流Transformer架构，有效解决了现有生成式时间序列模型在利用跨变量依赖方面的不足，为概率预测提供了更强大和灵活的框架。
...
### [185] [The hidden risks of temporal resampling in clinical reinforcement learning](https://arxiv.org/abs/2602.06603)
*Thomas Frost,Hrisheekesh Vaidya,Steve Harris*

Main category: cs.LG

TL;DR: 离线强化学习在医疗应用中，将患者数据重采样为固定时间间隔会显著降低模型在实际部署中的性能，且标准离策略评估指标可能无法检测到这种性能下降。
...
### [186] [The challenge of generating and evolving real-life like synthetic test data without accessing real-world raw data -- a Systematic Review](https://arxiv.org/abs/2602.06609)
*Maj-Annika Tammisto,Faiz Ali Shah,Daniel Rodriguez,Dietmar Pfahl*

Main category: cs.LG

TL;DR: 本文对电子政务应用中隐私保护的合成测试数据生成方法进行了系统综述，发现现有方法大多需要真实数据作为输入，且缺乏测试数据演化能力。


<details>
  <summary>Details</summary>
Motivation: 电子政务应用（如跨国信息交换、医疗、银行等）的系统测试需要真实但隐私安全的测试数据，但目前缺乏不依赖原始真实数据的合成测试数据生成方法。

Method: 采用Kitchenham等知名系统文献综述方法，从IEEE Xplore、ACM Digital Library和SCOPUS数据库中筛选1013篇文献，最终提取75篇文献数据进行分析。

Result: 识别出37种部分满足研究问题的方法，其中9种最接近完全满足要求。现有方法普遍需要访问真实数据进行匿名化或合成数据生成，且缺乏测试数据演化能力。

Conclusion: 现有研究均未完全满足需求，合成测试数据演化是一个尚未得到充分关注但至关重要的研究方向，特别是在各国新法规不断出台的背景下。
...
TL;DR: 提出Adaptive-CaRe正则化策略，平衡医学预测中的预测精度与因果鲁棒性


<details>
  <summary>Details</summary>
Motivation: 传统监督学习追求预测精度但可能捕捉虚假相关，因果结构学习过于保守导致精度损失，需要平衡两者

Method: 提出模型无关的正则化策略Adaptive-CaRe，基于特征统计贡献与因果贡献差异的比例惩罚

Result: 合成数据验证有效找到鲁棒预测因子并保持竞争性预测精度，真实数据集验证实用价值

Conclusion: Adaptive-CaRe为医学领域长期存在的预测精度与因果鲁棒性权衡提供简单有效解决方案
...
TL;DR: 提出了一种针对联邦学习的温度缩放攻击（TSA），该攻击专门破坏模型置信度校准而不影响准确率，通过在学习率-温度耦合机制下注入恶意更新，能够逃避基于准确率和相似性的检测，对关键系统造成严重风险。


<details>
  <summary>Details</summary>
Motivation: 在关键任务系统中，预测置信度是控制信号的基础，直接影响风险感知逻辑（如升级、弃权和保守回退）。现有联邦学习攻击主要针对准确率或植入后门，而置信度校准作为一个独立的攻击目标尚未得到充分研究。本文旨在探索如何在不影响准确率的情况下破坏模型校准，从而揭示联邦学习中校准完整性的关键攻击面。

Method: 提出温度缩放攻击（TSA），通过在本地训练期间注入温度缩放与学习率-温度耦合机制，恶意客户端可以生成看似良性的更新。这种方法保持了标准的优化行为，能够逃避基于准确率的监控和基于相似性的检测。作者在非独立同分布（non-IID）设置下提供了收敛性分析，证明这种耦合机制在保持标准收敛边界的同时，能够系统性地扭曲置信度。

Result: 在三个基准测试中，TSA显著改变了校准性能（例如在CIFAR-100上误差增加145%），而准确率变化小于2%。即使在采用鲁棒聚合和后校准防御的情况下，攻击仍然有效。案例研究进一步表明，置信度操纵可能导致关键案例漏检率增加7.2倍（医疗领域）或误报率增加7.2倍（自动驾驶），即使准确率保持不变。

Conclusion: 本文确立了校准完整性作为联邦学习中的一个关键攻击面。温度缩放攻击（TSA）展示了如何在不影响准确率的情况下破坏模型置信度校准，这对依赖置信度进行风险感知决策的关键任务系统构成严重威胁。研究结果强调了在联邦学习安全评估中需要包含校准完整性检测。
...
### [189] [Memory-Conditioned Flow-Matching for Stable Autoregressive PDE Rollouts](https://arxiv.org/abs/2602.06689)
*Victor Armegioiu*

Main category: cs.LG

TL;DR: 本文提出了一种记忆条件化的扩散/流匹配方法，通过在线状态注入来改善自回归PDE求解器的长期稳定性，解决了粗到细尺度生成中的漂移问题。


<details>
  <summary>Details</summary>
Motivation: 自回归生成式PDE求解器在单步预测时可能准确，但在长期推演中容易漂移，特别是在粗到细尺度生成中，每一步都需要重新生成未解析的精细尺度。扩散和流匹配生成器虽然内部动态是马尔可夫的，但其推演稳定性受每步条件分布误差控制。

Method: 使用Mori-Zwanzig投影形式主义分析未解析变量的影响，引入记忆条件化的扩散/流匹配方法，通过紧凑的在线状态注入到去噪过程中，利用潜在特征来改善条件分布。该方法通过分解引入结构化条件尾部先验，减少填充缺失频率所需的传输。

Result: 证明了所得条件核的Wasserstein稳定性，推导了离散Grönwall推演边界，将记忆近似与条件生成误差分离。在含激波的压缩流和多尺度混合实验中，显示出更高的精度和显著更稳定的长期推演，具有更好的精细尺度谱和统计保真度。
...
### [190] [NanoQuant: Efficient Sub-1-Bit Quantization of Large Language Models](https://arxiv.org/abs/2602.06694)
*Hyochan Chong,Dongkyu Kim,Changdong Kim,Minseop Choi*

Main category: cs.LG

TL;DR: NanoQuant是一种新的后训练量化方法，首次将大语言模型压缩到二进制和亚1位级别，通过低秩二进制分解实现高效压缩，使70B模型能在8GB消费级GPU上运行。


<details>
  <summary>Details</summary>
Motivation: 现有的仅权重量化方法无法高效地将大语言模型压缩到二进制（1位）级别，要么需要大量数据和计算资源，要么会增加额外存储开销，限制了模型在消费级硬件上的部署。

Method: 将量化问题形式化为低秩二进制分解问题，将全精度权重压缩为低秩二进制矩阵和缩放因子。使用高效的交替方向乘子法（ADMM）精确初始化潜在二进制矩阵和缩放因子，然后通过块级和模型级重建过程微调初始化参数。

Result: 在低内存后训练量化中建立了新的帕累托前沿，在亚1位压缩率下实现了最先进的精度。例如，在单个H100上仅用13小时将Llama2-70B压缩25.8倍，使70B模型能在8GB消费级GPU上运行。

Conclusion: NanoQuant首次实现了将大语言模型后训练量化到二进制和亚1位级别，为大规模模型在消费级硬件上的部署提供了可行方案，显著降低了部署门槛和成本。
...
### [191] [Explaining Grokking in Transformers through the Lens of Inductive Bias](https://arxiv.org/abs/2602.06702)
*Jaisidh Singh,Diganta Misra,Antonio Orvieto*

Main category: cs.LG

TL;DR: 该研究从归纳偏置角度分析Transformer中的grokking现象，发现层归一化位置等架构选择显著影响grokking速度，优化设置会调节grokking过程，特征在整个训练过程中持续演化，且泛化能力与特征可压缩性相关。


<details>
  <summary>Details</summary>
Motivation: 研究动机是深入理解Transformer中的grokking现象（模型在长时间训练后突然获得泛化能力）背后的机制，特别是从归纳偏置（架构和优化带来的偏好）的角度来解析这一现象。

Method: 通过系统实验分析不同架构选择（如层归一化位置）对grokking速度的影响，研究优化设置（学习率、权重衰减等）如何调节grokking过程，并分析特征演化轨迹和可压缩性与泛化能力的关系。

Result: 发现层归一化位置强烈影响grokking速度，特定路径上的层归一化塑造了捷径学习和注意力熵；优化设置会调节grokking过程，学习率和权重衰减可能混淆先前提出的控制变量（如读出尺度）；特征在整个训练过程中持续演化，表明grokking比简单的"懒惰到丰富"学习机制转变更为微妙；泛化能力与特征可压缩性可预测地相关。

Conclusion: Transformer中的grokking现象受到架构和优化诱导的归纳偏置的强烈影响，特征演化是连续的而非突变的，泛化能力与特征可压缩性相关，这为理解和控制grokking提供了新的视角。
...
TL;DR: SaDiT是一个通过结合SaProt Tokenization和Diffusion Transformer架构来加速蛋白质骨架生成的新框架，利用离散潜在空间表示蛋白质几何，并通过IPA Token Cache机制优化计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的蛋白质骨架生成方法计算密集且速度较慢，而蛋白质领域的结构压缩和加速潜力尚未充分探索，需要更高效的生成框架。

Method: SaDiT整合SaProt Tokenization与Diffusion Transformer架构，利用离散潜在空间表示蛋白质几何结构，保持SE(3)等价性，并引入IPA Token Cache机制优化Invariant Point Attention层的计算效率。

Result: SaDiT在计算速度和结构可行性方面优于RFDiffusion和Proteina等最先进模型，在无条件骨架生成和折叠类条件生成任务中表现出色，能够捕捉复杂拓扑特征并保持高设计性。

Conclusion: SaDiT通过离散潜在空间表示和计算优化机制，显著提高了蛋白质骨架生成的效率，为大规模结构探索提供了更快的解决方案。
...
### [193] [Vision Transformer Finetuning Benefits from Non-Smooth Components](https://arxiv.org/abs/2602.06883)
*Ambroise Odonnat,Laetitia Chapel,Romain Tavenard,Ievgen Redko*

Main category: cs.LG

TL;DR: 研究发现视觉Transformer组件的可塑性（对输入变化的敏感度）比平滑性更重要，高可塑性的注意力模块和前馈层能带来更好的微调性能
...
Method: 通过理论分析和综合实验，将可塑性定义为平均变化率，衡量对输入扰动的敏感性。高可塑性意味着低平滑性，这种方法为选择适应过程中应优先考虑的组件提供了原则性指导

Result: 实验表明，注意力模块和前馈层的高可塑性一致地带来更好的微调性能。这与普遍认为平滑性更优的假设相悖

Conclusion: 研究为Transformer的功能特性提供了新视角：在迁移学习中，高可塑性（低平滑性）比平滑性更有利于微调性能。这对实践者有重要指导意义，应优先调整高可塑性的组件
...
### [194] [F-GRPO: Don't Let Your Policy Learn the Obvious and Forget the Rare](https://arxiv.org/abs/2602.06717)
*Daniil Plyusov,Alexey Gorbatovski,Boris Shaposhnikov,Viacheslav Sinii,Alexey Malakhov,Daniil Gavrilov*

Main category: cs.LG

TL;DR: 该论文分析了RLVR中组采样导致的罕见正确轨迹学习偏差问题，提出了基于难度感知的优势缩放系数改进方法，在多个RLVR算法上显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: RLVR基于组采样估计优势和稳定策略更新，但实践中由于计算限制无法使用大组规模，导致学习偏向已经可能的轨迹。小组规模常错过罕见正确轨迹而仍包含混合奖励，使概率集中在常见解上。

Method: 推导了更新错过罕见正确模式的概率与组规模的关系，分析了更新如何在正确集内重新分配质量。提出受Focal loss启发的难度感知优势缩放系数，对高成功提示的更新进行降权处理。该轻量级修改可直接集成到任何组相对RLVR算法中。

Result: 在Qwen2.5-7B模型上，该方法在多个基准测试中显著提升了pass@256性能：GRPO从64.1提升到70.3，DAPO从69.3提升到72.5，CISPO从73.2提升到76.8，同时保持或改善了pass@1性能，未增加组规模或计算成本。

Conclusion: 提出的难度感知优势缩放系数有效解决了RLVR中组采样导致的罕见正确轨迹学习偏差问题，是一种轻量级且通用的改进方法，可在不增加计算成本的情况下提升多种RLVR算法的性能。
...
### [195] [Disentanglement by means of action-induced representations](https://arxiv.org/abs/2602.06741)
...
TL;DR: 该论文提出了动作诱导表示(AIRs)框架，通过建模物理系统在实验或动作下的表示，实现了可证明的解耦表示学习，解决了传统VAE无法进行非线性独立成分分析的难题。


<details>
  <summary>Details</summary>
Motivation: 传统变分自编码器(VAEs)在可解释表示学习中的主要挑战是难以获得解耦表示，即每个潜在维度对应一个独立的生成因子。这一困难本质上源于无法进行非线性独立成分分析。
...
Conclusion: AIRs框架为可解释表示学习提供了新的理论和方法，通过将实验与它们影响的自由度直接联系起来，实现了可证明的解耦表示，超越了传统状态表示的限制。
...
### [196] [Soft Forward-Backward Representations for Zero-shot Reinforcement Learning with General Utilities](https://arxiv.org/abs/2602.06769)
*Marco Bagatella,Thomas Rupf,Georg Martius,Andreas Krause*

Main category: cs.LG

TL;DR: 提出一种基于最大熵的前向-后向算法变体，用于解决具有一般效用函数的零样本强化学习问题，能够直接从离线数据中提取策略家族，并在测试时通过零阶搜索优化任意可微的占用度量函数。


<details>
  <summary>Details</summary>
Motivation: 现有零样本强化学习主要关注具有可加性奖励的标准RL问题，但实际应用中存在更一般的效用函数（如分布匹配、纯探索等），这些无法简化为可加性奖励。需要扩展零样本RL的能力来处理更广泛的RL问题类别。

Method: 提出最大熵前向-后向算法变体，从离线数据中恢复随机策略家族。结合紧凑策略嵌入上的零阶搜索，绕过迭代优化方案，在测试时直接优化一般效用函数。

Result: 方法在简单教学实验和高维实验中均表现出色，保留了前向-后向算法的优势特性，同时将其适用范围扩展到更一般的RL问题。

Conclusion: 成功将零样本强化学习扩展到一般效用函数设置，通过最大熵前向-后向算法和零阶搜索的组合，实现了对任意可微占用度量函数的直接优化，扩展了零样本RL的应用范围。
...
### [197] [AEGIS: Adversarial Target-Guided Retention-Data-Free Robust Concept Erasure from Diffusion Models](https://arxiv.org/abs/2602.06771)
*Fengpeng Li,Kemou Li,Qizhou Wang,Bo Han,Jiantao Zhou*

Main category: cs.LG

TL;DR: AEGIS框架通过对抗性擦除和梯度信息协同，在无需保留数据的情况下同时提升概念擦除的鲁棒性和保留性


<details>
  <summary>Details</summary>
Motivation: 当前概念擦除方法面临鲁棒性和保留性的权衡困境：增强鲁棒性会损害保留性，反之亦然。现有方法通常只能优化一个方面而牺牲另一个，无法同时满足实际应用中对两个关键指标的需求。

Method: 提出AEGIS框架，采用对抗性擦除和梯度信息协同的方法，在不需要保留数据的情况下，通过对抗性训练增强模型对概念重新激活的抵抗力，同时利用梯度信息优化保留性能。

Result: AEGIS框架在概念擦除任务中同时提升了鲁棒性和保留性，相比现有方法在两个方面都有显著改进，且不需要额外的保留数据。

Conclusion: AEGIS框架成功解决了概念擦除中鲁棒性和保留性的权衡问题，为实际应用提供了更有效的解决方案，能够在阻止有害内容生成的同时保持模型的整体实用性。
...
### [198] [Calibrating Generative AI to Produce Realistic Essays for Data Augmentation](https://arxiv.org/abs/2602.06772)
*Edward W. Wolfe,Justin O. Barber*

Main category: cs.LG

TL;DR: 本研究比较了三种大语言模型提示方法生成模拟学生作文的效果，发现"预测下一个"策略在评分一致性、保持原文质量和文本真实性方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 在构建式反应项目（如作文）的自动评分系统中，训练数据有限是一个常见问题。数据增强可以缓解这一问题，但需要确保生成的文本既保持原文质量又具有真实性。本研究旨在评估三种大语言模型提示方法在生成用于增强训练数据集的模拟作文时的效果。

Method: 研究采用三种大语言模型提示方法：1) 预测下一个策略，2) 句子策略，3) 25个示例策略。使用这些方法生成模拟版本的学生作文，然后由人工评分员对这些模拟作文进行评分，并评估生成文本的真实性。

Result: 1) "预测下一个"策略在人工评分员对模拟作文评分的一致性方面最高；2) "预测下一个"和"句子"策略最能保持原文的评分质量；3) "预测下一个"和"25个示例"策略生成的文本被人工评分员认为最真实。

Conclusion: "预测下一个"提示策略在生成用于自动评分系统训练数据增强的模拟作文方面表现最佳，能够在保持原文写作质量的同时生成真实的文本，为构建式反应项目的机器学习评分引擎提供了有效的数据增强方法。
...
### [199] [On the Identifiability of Steering Vectors in Large Language Models](https://arxiv.org/abs/2602.06801)
*Sohan Venkatesh,Ashish Mahendran Kurapath*

Main category: cs.LG

TL;DR: 激活导向方法（如角色向量）被广泛用于控制大语言模型行为，但研究发现这些导向向量在现实条件下本质上是不可识别的，因为存在大量行为上无法区分的干预等价类。
...
Result: 理论证明导向向量本质上是非可识别的，存在大量行为上无法区分的干预等价类。实证验证了正交扰动能达到近等效效果。但可识别性在统计独立性、稀疏约束、多环境验证或跨层一致性等结构假设下是可恢复的。

Conclusion: 研究揭示了可解释性的基本限制，阐明了可靠安全关键控制所需的结构假设。激活导向向量的非可识别性对当前基于激活导向的解释方法提出了重要挑战。
...
### [200] [On the Convergence of Multicalibration Gradient Boosting](https://arxiv.org/abs/2602.06773)
*Daniel Haimovich,Fridolin Linder,Lorenzo Perini,Niek Tax,Milan Vojnovic*

Main category: cs.LG

TL;DR: 本文为多校准梯度提升在回归问题中的收敛性提供了理论保证，证明了预测更新的幅度以O(1/√T)衰减，在平滑性假设下可达到线性收敛，并通过实验验证了理论。


<details>
  <summary>Details</summary>
Motivation: 多校准梯度提升在实践中已被大规模部署且表现出良好的多校准性能，但其收敛性质尚未得到充分的理论理解，需要填补这一理论空白。

Method: 分析多校准梯度提升在平方误差损失回归问题中的收敛性，证明预测更新幅度的衰减率，在弱学习器平滑性假设下改进收敛速度，并研究自适应变体和缩放方案。

Result: 证明了多校准梯度提升的预测更新幅度以O(1/√T)衰减，多校准误差具有相同收敛率；在平滑性假设下达到线性收敛；自适应变体实现训练损失的局部二次收敛。

Conclusion: 本文填补了多校准梯度提升收敛理论空白，为该方法提供了坚实的理论支撑，并通过实验验证了理论结果，阐明了快速收敛和强多校准的实现条件。
...
### [201] [Robust Online Learning](https://arxiv.org/abs/2602.06775)
*Sajad Ashkezari*

Main category: cs.LG

TL;DR: 该论文研究了在对抗性扰动输入下的鲁棒分类器学习问题，提出了一个新的维度概念来控制实现在线学习中的错误界和遗憾界。


<details>
  <summary>Details</summary>
Motivation: 研究在对抗性环境下学习鲁棒分类器的问题，其中不仅输入会受到扰动，而且干净数据和标签也是对抗性选择的，这与传统的鲁棒PAC学习不同。

Method: 将问题建模为在线学习问题，定义了一个新的维度概念来控制实现在线学习中的错误界和遗憾界，并将该维度推广到多类别假设类。

Result: 提出了一个类似于Littlestone维度的简单维度概念，该维度控制了实现在线学习中的错误界和遗憾界，并推广到多类别情况。

Conclusion: 该研究为对抗性环境下的鲁棒分类器学习提供了理论框架，特别是在学习者不知道每个点的允许扰动集而只有先验知识的情况下。
...
TL;DR: 论文提出了一种更通用的RLHF优化框架，放宽了f-散度的凸性要求，定义了DPO-inducing条件，并提出了防止概率位移的displacement-resistant条件，最终基于满足这两个条件的f函数设计了新的SquaredPO损失函数。


<details>
  <summary>Details</summary>
Motivation: 现有DPO及相关算法通过KL散度惩罚来对齐语言模型，虽然已有工作将其推广到凸f-散度，但凸性限制可能过于严格。本文旨在寻找更一般的条件，使RLHF问题保持可解性，同时解决实践中观察到的概率位移问题。

Method: 1. 识别比凸性更一般的DPO-inducing条件，精确刻画RLHF问题何时保持可解性；2. 建立防止概率位移的displacement-resistant条件；3. 基于同时满足这两个条件的特定f函数，设计新的SquaredPO损失函数。

Result: 1. 证明了凸性不是必要条件，提出了更一般的DPO-inducing条件；2. 建立了防止概率位移的displacement-resistant条件；3. 提出的SquaredPO损失函数在理论上比DPO有更强的保证，在实践中表现相当。

Conclusion: 本文扩展了RLHF优化的理论框架，放宽了凸性要求，解决了概率位移问题，并提出了具有更强理论保证的SquaredPO损失函数，为语言模型对齐提供了新的优化方法。
...
TL;DR: 论文提出了一个端到端框架，用于系统分析大语言模型中的罕见事件，包括理论、高效生成策略、概率估计和误差分析。


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为概率模型，在推理过程中会出现罕见事件——远离典型但高度显著的行为。由于罕见事件本身难以观察，而LLM的巨大使用规模意味着开发阶段完全未观察到的事件很可能在部署中变得突出。

Method: 提出了一个端到端框架，包含理论分析、高效生成策略、概率估计和误差分析，并提供了实际实现。通过具体示例说明该框架的应用。

Result: 开发了一个完整的分析框架，能够系统性地识别、生成和评估LLM中的罕见事件，为理解和处理这些重要但难以观察的行为提供了实用工具。

Conclusion: 该框架具有通用性，可以扩展到其他模型和上下文，为分析概率模型中的罕见事件提供了概念和技术基础。
...
TL;DR: FlowDA是一个基于流匹配的低延迟天气尺度生成式数据同化框架，通过SetConv嵌入观测数据并微调Aurora基础模型，在稀疏观测下优于传统方法
...
Method: 基于流匹配的生成式数据同化框架，使用SetConv嵌入观测条件，微调Aurora基础模型进行高效分析
...
### [205] [Supercharging Simulation-Based Inference for Bayesian Optimal Experimental Design](https://arxiv.org/abs/2602.06900)
*Samuel Klein,Willie Neiswanger,Daniel Ratner,Michael Kagan,Sean Gasiorowski*

Main category: cs.LG

TL;DR: 该论文提出了一种基于模拟推理(SBI)的贝叶斯最优实验设计(BOED)新方法，通过多种EIG公式直接利用现代SBI密度估计器，并改进了梯度优化过程，在标准基准测试中性能提升最高达22%。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯最优实验设计(BOED)需要最大化期望信息增益(EIG)，但在许多设置中似然函数难以处理。现有的连接SBI和BOED的工作仅限于单一的对比EIG界限，限制了方法的灵活性和性能。

Method: 1) 展示了EIG的多种公式化方式，可以直接利用现代SBI密度估计器（包括神经后验、似然和比率估计）；2) 提出了一种基于神经似然估计的新型EIG估计器；3) 识别了基于梯度的EIG最大化的优化瓶颈，并提出简单的多起点并行梯度上升程序来改善可靠性和性能。

Result: 提出的SBI-based BOED方法在标准BOED基准测试中能够匹配或超越现有最先进方法，性能提升最高达22%。

Conclusion: 通过将多种SBI密度估计器与改进的优化策略相结合，为BOED提供了更灵活、更强大的框架，显著提高了实验设计的效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: 表格异常检测面临挑战，因为表格数据具有异质性：特征缺乏自然关系、分布和尺度差异大、类型多样。现有方法在不同数据集上表现不一，没有一种方法能在所有场景中始终优于其他方法。

Method: 通过随机采样获得经验分布和K-means质心获得结构分布来表征正常数据，使用最优传输距离测量添加测试样本对这两种分布兼容性的干扰程度。正常样本干扰低，异常样本干扰高，从而提供校准信号。

Result: 在34个不同的表格数据集和7个代表性检测器上的实验表明，CTAD能持续显著提升性能，甚至能增强最先进的深度学习方法，在不同超参数设置下表现稳健，无需额外调参即可实际部署。

Conclusion: CTAD是一个有效的模型无关后处理框架，通过样本特异性校准增强现有表格异常检测器的性能，具有理论保证和实际部署的便利性。
...
### [207] [From Kepler to Newton: Inductive Biases Guide Learned World Models in Transformers](https://arxiv.org/abs/2602.06923)
*Ziming Liu,Sophia Sanborn,Surya Ganguli,Andreas Tolias*

Main category: cs.LG

TL;DR: 通用Transformer架构通过引入空间平滑性、稳定性和时间局部性三个最小归纳偏置，可以从曲线拟合转变为发现物理定律，实现自动科学发现的关键一步。


<details>
  <summary>Details</summary>
Motivation: 研究通用AI架构是否能超越预测功能，真正发现支配宇宙的物理定律。现有方法要么依赖领域特定先验，要么只能预测而不能理解底层物理规律。

Method: 在通用Transformer中引入三个最小归纳偏置：1) 空间平滑性（将预测公式化为连续回归）；2) 稳定性（使用噪声上下文训练以减少误差累积）；3) 时间局部性（将注意力窗口限制在最近过去）。

Result: 前两个偏置使Transformer能够学习开普勒世界模型，成功拟合行星轨迹的椭圆。第三个偏置（时间局部性）迫使模型放弃曲线拟合，发现牛顿力表示，真正理解物理规律。

Conclusion: 简单的架构选择决定AI是成为曲线拟合器还是物理学家，这是迈向自动科学发现的关键步骤。最小归纳偏置使通用AI能够从预测转向发现物理定律。
...
### [208] [Learning Deep Hybrid Models with Sharpness-Aware Minimization](https://arxiv.org/abs/2602.06837)
*Naoya Takeishi*

Main category: cs.LG

TL;DR: 提出基于锐度感知最小化的混合建模方法，通过关注损失最小值的平坦性来避免机器学习模型主导预测，提高科学模型的作用
...
Method: 采用锐度感知最小化思想，关注损失最小值的平坦性，使模型尽可能简单。将SAM方法适配到混合建模设置中，通过寻找平坦的最小值来平衡机器学习模型和科学模型

Result: 数值实验表明，基于SAM的方法在不同模型选择和数据集上都能良好工作，有效避免了机器学习模型主导预测的问题

Conclusion: 通过关注损失最小值的平坦性，提出的SAM-based方法为混合建模提供了一种通用有效的解决方案，能够在不同模型和数据集上稳定工作，确保科学模型在预测中发挥作用
...
### [209] [T-STAR: A Context-Aware Transformer Framework for Short-Term Probabilistic Demand Forecasting in Dock-Based Shared Micro-Mobility](https://arxiv.org/abs/2602.06866)
*Jingyi Cheng,Gonçalo Homem de Almeida Correia,Oded Cats,Shadi Sharif Azadeh*

Main category: cs.LG

TL;DR: T-STAR是一个基于Transformer的两阶段概率框架，用于15分钟分辨率的共享单车站点级需求预测，通过分层结构分离稳定模式和短期波动，在华盛顿Capital Bikeshare数据上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 可靠的短期需求预测对于共享微出行服务管理至关重要，但高分辨率预测面临挑战，需要分离稳定需求模式和短期波动，并考虑实时因素如地铁服务变化。
...
Conclusion: T-STAR框架能够提供细粒度、可靠且具有不确定性意识的短期需求预测，支持多模式出行规划并增强共享微出行服务的实时运营能力。
...
### [210] [Decoupling Variance and Scale-Invariant Updates in Adaptive Gradient Descent for Unified Vector and Matrix Optimization](https://arxiv.org/abs/2602.06880)
*Zitao Song,Cedar Site Bai,Zhe Zhang,Brian Bullins,David F. Gleich*

Main category: cs.LG

TL;DR: DeVA框架通过解耦AdaGrad更新，将方差自适应项与尺度不变项分离，实现了向量方差自适应与矩阵谱优化之间的桥梁，在语言建模和图像分类任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的自适应方法（如Adam）在向量和欧几里得优化中表现出色，而矩阵谱优化器（如Muon）在处理权重矩阵时更有效。但将Adam的自适应机制扩展到矩阵谱优化存在困难，需要一种能够桥接这两种方法的框架。

Method: 重新表述AdaGrad更新，将其解耦为方差自适应项和尺度不变项，形成DeVA（解耦方差自适应）框架。该框架允许从Adam平滑过渡到自适应谱下降，同时保持计算可行性。

Result: 在语言建模和图像分类任务中，DeVA一致优于Muon和SOAP等最先进方法，减少了约6.6%的token使用量。理论分析表明方差自适应项有效改善了块级平滑性，促进了更快的收敛。

Conclusion: DeVA成功桥接了向量方差自适应和矩阵谱优化，提供了一种可行的框架来实现从Adam到自适应谱下降的平滑过渡，在多个任务中表现出优越性能。
...
### [211] [Sample Complexity of Causal Identification with Temporal Heterogeneity](https://arxiv.org/abs/2602.06899)
*Ameya Rathod,Sujay Belsare,Salvik Krishna Nautiyal,Dhruv Laad,Ponnurangam Kumaraguru*

Main category: cs.LG

TL;DR: 该论文提出了一种结合时间序列动态和多环境异质性来识别因果图的方法，分析了在薄尾和高斯噪声与厚尾（Student's t分布）噪声下的统计恢复极限。


<details>
  <summary>Details</summary>
Motivation: 从观测数据中恢复唯一的因果图是一个不适定问题，因为多个生成机制可能导致相同的观测分布。该问题只有在利用特定的结构或分布假设时才可解。现有研究分别利用时间序列动态或多环境异质性来约束这一问题，但缺乏两者的整合。

Method: 整合时间序列动态和多环境异质性作为互补的异质性来源，推导统一的必要可识别性条件，分析薄尾噪声与厚尾噪声下的统计恢复极限，特别研究了Student's t分布下的情况。

Result: 时间结构可以有效替代缺失的环境多样性，即使在异质性不足的情况下也可能实现可识别性。对于厚尾分布，几何可识别性条件保持不变，但样本复杂度与高斯基线显著不同。建立了协方差基因果图恢复方法在现实非平稳系统中的基本极限。

Conclusion: 该工作将重点从因果结构是否可识别转向其在实践中是否可统计恢复，为因果图恢复提供了统一的理论框架，并量化了鲁棒性的代价。
...
### [212] [Parameter-free Dynamic Regret: Time-varying Movement Costs, Delayed Feedback, and Memory](https://arxiv.org/abs/2602.06902)
*Emmanuel Esposito,Andrew Jacobsen,Hao Qiu,Mengxiao Zhang*

Main category: cs.LG

TL;DR: 该论文研究了具有时变移动成本的无约束在线凸优化中的动态遗憾问题，提出了首个比较器自适应的动态遗憾界，并应用于延迟反馈和时变记忆两个场景。


<details>
  <summary>Details</summary>
Motivation: 标准在线凸优化设置假设移动成本系数固定，但实际应用中这些系数可能随时间变化。研究时变移动成本下的动态遗憾问题，可以更好地适应现实世界中的动态环境。

Method: 提出了一种新颖算法，建立了该设置下首个比较器自适应的动态遗憾界。通过将延迟反馈和时变记忆问题转化为时变移动成本问题，展示了结果的通用性。

Result: 算法保证了$\widetilde{\mathcal{O}}(\sqrt{(1+P_T)(T+\sum_t λ_t)})$的动态遗憾界，其中$P_T$是比较器序列在$T$轮中的路径长度。当所有轮次$λ_t=0$时，该界退化为标准在线凸优化的最优保证。

Conclusion: 该研究扩展了在线凸优化理论，为时变移动成本设置提供了首个比较器自适应的动态遗憾界，并通过延迟反馈和时变记忆两个应用展示了结果的实用性和通用性。
...
### [213] [Revisiting the Generic Transformer: Deconstructing a Strong Baseline for Time Series Foundation Models](https://arxiv.org/abs/2602.06909)
*Yunshi Wen,Wesley M. Gifford,Chandra Reddy,Lam M. Nguyen,Jayant Kalagnanam,Anak Agung Julius*

Main category: cs.LG

TL;DR: 标准patch Transformer架构通过简单训练协议实现零样本预测SOTA性能，关键改进来自数据工程而非架构创新
...
Method: 使用标准patch Transformer架构，采用简单训练协议，进行全面的消融研究，涵盖模型缩放、数据组成和训练技术，严格控制变量

Result: 该通用架构在零样本预测任务上达到最先进性能，识别出性能提升的关键驱动因素，验证了架构本身具有良好的可扩展性

Conclusion: 标准Transformer架构配合适当训练协议即可实现优秀性能，研究结果为未来时间序列基础模型研究提供了透明可复现的基线
...
### [214] [Continuous-time reinforcement learning: ellipticity enables model-free value function approximation](https://arxiv.org/abs/2602.06930)
*Wenlong Mou*

Main category: cs.LG

TL;DR: 本文研究连续时间马尔可夫扩散过程的离策略强化学习，提出基于函数近似的模型无关算法，利用扩散的椭圆性建立贝尔曼算子的希尔伯特空间性质，开发Sobolev-prox fitted q-learning算法，证明其误差受函数类逼近能力、局部复杂度、优化误差和数值离散化误差控制。


<details>
  <summary>Details</summary>
Motivation: 研究连续时间马尔可夫扩散过程的离策略强化学习，特别是在离散时间观测和动作下的控制问题。现有方法通常需要不切实际的结构假设，本文旨在开发无需此类假设的模型无关算法，利用扩散过程的椭圆性特性来简化函数近似强化学习。

Method: 提出Sobolev-prox fitted q-learning算法：1) 利用扩散过程的椭圆性建立贝尔曼算子在希尔伯特空间中的正定性和有界性；2) 通过迭代求解最小二乘回归问题来学习价值函数和优势函数；3) 采用函数近似方法直接从数据中学习，无需对动态过程做不现实的结构假设。

Result: 建立了估计误差的oracle不等式，表明误差受四个因素控制：1) 函数类的最佳逼近误差；2) 函数类的局部复杂度；3) 指数衰减的优化误差；4) 数值离散化误差。这些结果表明，椭圆性这一结构特性使得马尔可夫扩散过程的函数近似强化学习难度不高于监督学习。

Conclusion: 椭圆性是马尔可夫扩散过程的关键结构特性，它使得基于函数近似的强化学习问题在理论难度上不高于监督学习。提出的Sobolev-prox fitted q-learning算法能够有效学习价值函数和优势函数，为连续时间扩散过程的控制提供了理论保证。
...
### [215] [When RL Meets Adaptive Speculative Training: A Unified Training-Serving System](https://arxiv.org/abs/2602.06932)
...
TL;DR: Aurora是一个统一的训练-服务系统，通过从实时推理轨迹中持续学习推测器，解决了传统推测解码中部署滞后、反馈延迟和领域漂移问题，实现即时部署和快速适应。


<details>
  <summary>Details</summary>
Motivation: 传统推测解码将推测器训练与服务解耦，导致三个主要问题：1) 高服务准备时间，推测器需要长时间离线训练；2) 延迟的效用反馈，真正的端到端解码加速只能在训练后得知；3) 领域漂移退化，当目标模型适应新领域时推测器变得过时低效。

Method: Aurora将在线推测器学习重构为异步强化学习问题：接受的token提供正反馈，拒绝的推测器提议提供隐式负反馈。系统集成SGLang推理服务器和异步训练服务器，支持热插拔推测器更新而不中断服务。

Result: Aurora在新发布的前沿模型上实现1.5倍的即时加速（如MiniMax M2.1 229B和Qwen3-Coder-Next 80B）。在用户流量分布变化时，相比训练良好但静态的推测器，在广泛使用的模型上（如Qwen3和Llama3）提供额外1.25倍加速。

Conclusion: Aurora通过统一训练-服务系统解决了推测解码中的部署滞后问题，支持即时部署和持续适应，显著提升LLM服务性能，特别是在模型更新和领域变化场景下。
...
### [216] [From Core to Detail: Unsupervised Disentanglement with Entropy-Ordered Flows](https://arxiv.org/abs/2602.06940)
*Daniel Galperin,Ullrich Köthe*

Main category: cs.LG

TL;DR: EOFlows是一种基于归一化流的框架，通过熵排序实现自适应可注入流，允许在推理时灵活选择核心表示维度，用于语义特征提取和去噪。


<details>
  <summary>Details</summary>
Motivation: 当前无监督表示学习面临两个核心挑战：1）学习到的表示需要既具有语义意义，又能在多次运行中保持稳定；2）需要一种灵活的方法，能够在推理时自适应地选择表示维度，而不是在训练时固定。

Method: 提出熵排序流（EOFlows）框架，基于归一化流技术，按照解释熵对潜在维度进行排序（类似于PCA的解释方差）。结合基于似然的训练、局部雅可比正则化和噪声增强，构建可扩展至高维数据（如图像）的方法。

Result: 在CelebA数据集上的实验表明，该方法能够发现丰富的语义可解释特征，实现高压缩率和强去噪效果。

Conclusion: EOFlows提供了一种有效的无监督表示学习方法，通过熵排序实现自适应维度选择，在保持语义可解释性的同时，支持灵活的核心表示提取和噪声处理。
...
### [217] [Improving Credit Card Fraud Detection with an Optimized Explainable Boosting Machine](https://arxiv.org/abs/2602.06955)
*Reza E. Fazel,Arash Bakhtiary,Siavash A. Bigdeli*

Main category: cs.LG

TL;DR: 该研究提出了一种基于可解释提升机（EBM）的增强工作流程，通过系统化超参数调优、特征选择和预处理优化，在信用卡欺诈检测中实现了高精度与可解释性的平衡，无需依赖传统采样技术。


<details>
  <summary>Details</summary>
Motivation: 信用卡欺诈检测中的类别不平衡问题是影响金融系统预测可靠性的核心挑战，传统采样技术可能引入偏差或导致信息丢失，需要一种既能保持高精度又能提供可解释性的解决方案。

Method: 采用可解释提升机（EBM）作为基础模型，结合系统化超参数调优、特征选择和预处理优化，使用田口方法优化数据缩放器序列和模型超参数，避免传统采样技术。

Result: 在基准信用卡数据集上获得0.983的ROC-AUC，优于先前EBM基线（0.975）以及逻辑回归、随机森林、XGBoost和决策树模型，实现了精度与可解释性的平衡。
