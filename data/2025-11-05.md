...
TL;DR: iFlyBot-VLA是一个大规模视觉-语言-动作模型，采用新颖框架训练，通过双级动作表示和混合训练策略，在机器人操作任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够有效整合视觉、语言和动作信息的模型，以提升机器人在复杂操作任务中的性能和泛化能力。
...
Conclusion: iFlyBot-VLA通过双监督机制成功对齐语言、视觉和动作的表示空间，使视觉语言模型能够直接贡献于动作生成，计划开源部分自建数据集支持社区研究。
...
TL;DR: 本文系统比较了Geo-Foundational Models (GFMs)与传统U-Net在洪水淹没制图中的性能，发现GFMs在计算成本和标注工作量较低的情况下，能提供小幅到中度的精度提升。


<details>
  <summary>Details</summary>
Motivation: 尽管GFMs在从卫星图像中提取时空信息方面具有潜力，但目前尚不清楚它们是否优于传统模型如U-Net，且缺乏跨传感器和数据可用性场景的系统比较，这对指导最终用户选择模型至关重要。

Method: 评估了三种GFMs（Prithvi 2.0、Clay V1.5、DOFA）和UViT，并与TransNorm、U-Net和Attention U-Net进行比较，使用了PlanetScope、Sentinel-1和Sentinel-2数据，进行了跨区域交叉验证和少样本实验。

Result: 所有GFMs表现竞争性，性能差异仅2-5%。Clay在PlanetScope（0.79 mIoU）和Sentinel-2（0.70）上表现最佳，Prithvi在Sentinel-1（0.57）上领先。Clay在少样本实验中仅用5张训练图像就达到0.64 mIoU，且计算速度比Prithvi快3倍。

Conclusion: GFMs相比传统U-Net在洪水制图精度上提供小幅到中度的改进，同时具有更低的计算成本和标注工作量，其中Clay因其较小的模型尺寸和更好的性能成为更优选择。
...
TL;DR: 本文提出了一种基于学习框架的图像重建方法，用于处理不完整测量数据的上采样和修复问题。该方法利用图像分布的多个不变性，在固定确定性采样模式下实现与全监督方法相当的重建性能，且所需真实数据更少。


<details>
  <summary>Details</summary>
Motivation: 传统监督方法需要完全采样的真实数据，而自监督方法通常依赖随机采样。本文针对固定确定性采样模式（即使在期望下也无法完全覆盖图像）的挑战，旨在开发一种在真实数据有限的情况下仍能实现高质量重建的方法。

Method: 利用底层图像分布的多个不变性，理论上可以达到与全监督方法相同的重建性能。该方法在光学分辨率图像上采样任务中进行了验证，特别是在光声显微镜（PAM）应用中。

Result: 在光声显微镜的光学分辨率图像上采样任务中，该方法展示了与现有方法竞争甚至更优的重建结果，同时所需真实数据量显著减少。

Conclusion: 通过利用图像分布的多个不变性，本文提出的方法能够在固定确定性采样模式下实现高质量图像重建，为真实数据有限的应用场景提供了有效的解决方案。
...
TL;DR: 本研究系统评估了三种大型多模态模型在医疗图像中保护健康信息检测的性能，发现LMM在OCR准确率上优于传统模型，但在整体PHI检测精度上提升有限，主要优势体现在复杂文本模式场景。


<details>
  <summary>Details</summary>
Motivation: 传统PHI检测方法主要依赖OCR和命名实体识别，而新兴的大型多模态模型为文本提取和语义分析提供了新的机会，需要系统评估其在实际应用中的表现。

Method: 系统评估了GPT-4o、Gemini 2.5 Flash和Qwen 2.5 7B三种LMM，采用两种管道配置：纯文本分析和OCR+语义分析集成，并与EasyOCR等传统模型进行对比。

Result: LMM在OCR准确率上显著优于传统模型（WER: 0.03-0.05, CER: 0.02-0.03），但在整体PHI检测精度上提升有限，主要优势体现在复杂文本模式场景；在文本区域清晰可读且使用强LMM进行文本分析时，不同管道配置结果相似。

Conclusion: LMM在OCR性能上优于传统模型，但PHI检测精度提升有限；研究提供了基于实证的LMM选择建议和可扩展模块化部署策略。
...
TL;DR: 该论文介绍了StrengthSense数据集，包含11种需要力量的活动和2种非力量活动的IMU信号，来自29名健康受试者，使用10个IMU传感器采集，并通过视频标注验证数据准确性。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏全面的捕捉力量需求活动的可穿戴传感器数据集，需要填补这一空白以支持肌肉力量、耐力和功率的监测研究。

Method: 从29名健康受试者收集数据，使用10个IMU传感器放置在肢体和躯干上，通过视频记录进行标注，并进行IMU估计关节角度与视频提取关节角度的对比分析。

Result: 创建了包含11种力量需求活动和2种非力量活动的开放数据集，通过技术验证确认了传感器数据的准确性和可靠性。
...
TL;DR: 提出一个自动化合成文本视觉问答数据集的端到端流水线，利用OCR、ROI检测、标题生成和问题生成等技术，生成了约72K个QA对，基于约44K张图像。


<details>
  <summary>Details</summary>
Motivation: 构建大规模文本视觉问答数据库需要繁琐的人工标注，随着基础模型和OCR系统的成熟，需要建立自动化流水线来合成基于场景文本的问答对。

Method: 整合多个模型和算法，包括OCR检测与识别、感兴趣区域检测、标题生成和问题生成，将这些组件整合成一个连贯的流水线来自动合成和验证问答对。

Result: 成功构建了首个自动化合成的大规模文本视觉问答数据集，包含约72K个问答对，基于约44K张图像。

Conclusion: 该流水线能够高效、忠实地合成文本视觉问答数据，并随着场景文本数据的可用性而扩展，为文本视觉问答任务提供了重要的数据资源。
...
TL;DR: 开发并评估了一种基于HoloLens 2的仅深度、无标记增强现实配准管道，在真实手术环境中对小或低曲率解剖结构进行配准，中位误差约3-4毫米。


<details>
  <summary>Details</summary>
Motivation: 解决在头戴式显示器上实现仅深度、无标记增强现实配准的挑战，特别是在小或低曲率解剖结构上的临床应用需求。

Method: 在HoloLens 2上，通过深度偏差校正、人工引导初始化和全局-局部配准，将AHAT深度数据与CT皮肤网格对齐，并在临床手术中进行验证。

Result: 临床应用中位误差为3.9毫米，不同解剖部位中位误差分别为：足部3.2毫米、耳部4.3毫米、小腿5.3毫米，5毫米覆盖率达72-95%。

Conclusion: 仅深度、无标记AR管道在真实手术环境中实现了接近中等风险任务临床误差阈值的精度，提高了无标记AR引导的临床准备度。
...
TL;DR: 提出了一种端到端管道，结合实例分割和专门的腔室排序算法，自动从高分辨率CT扫描重建浮游有孔虫的三维生长轨迹，大幅减少人工工作量。
...
Method: 集成实例分割（计算机视觉技术）与腔室排序算法，定量评估多种针对不同腔室空间特征优化的分割方法，并分析其对生长顺序重建精度的影响。

Result: 在专家标注数据集上的实验表明，该管道在保持生物学意义精度的同时显著减少人工工作。尽管分割模型在较小腔室存在欠分割问题，但腔室排序算法仍能稳健地重建发育轨迹。
...
TL;DR: 该研究提出了一种结合主成分分析(PCA)和鲁棒主成分分析(RPCA)的级联框架，用于从数字图像中高效提取路面裂缝宽度，解决了传统方法在处理复杂裂缝形态和快速测量方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 精确量化路面裂缝宽度对于评估结构完整性和指导维护干预至关重要，但传统方法在处理复杂非均匀裂缝边界形态和实现快速测量方面存在显著挑战。
...
TL;DR: 本文提出一个三阶段框架来解决ICCV 2025感知测试挑战中的Grounded Video QA任务，通过引入触发时刻概念显著提升了HOTA得分。


<details>
  <summary>Details</summary>
Motivation: GVQA任务需要强大的多模态模型来处理视频内容进行复杂推理，并将答案在视觉上定位并跟踪目标对象。现有方法在此任务上表现有限，需要更有效的解决方案。

Method: 将GVQA任务分解为三阶段流程：视频推理与问答、时空定位和跟踪。关键贡献是引入触发时刻概念，通过CORTEX提示确定目标对象最可见的帧作为定位和跟踪的锚点。

Result: 实现了0.4968的HOTA得分，相比前一年获胜分数0.2704有显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 视网膜血管具有极细分支结构，形态变化大，现有分割方法在精度和鲁棒性方面仍面临挑战。

Method: 采用MM-UNet架构，包含形态Mamba卷积层增强分支拓扑感知，以及反向选择性状态引导模块提升几何边界感知和解码效率。

Result: 在两个公开视网膜血管分割数据集上验证，相比现有方法在DRIVE和STARE数据集上F1分数分别提升1.64%和1.25%。
...
TL;DR: 本研究开发了一种基于大语言模型的多模态融合生成模型，能够从血液生物标志物和MRI扫描中合成淀粉样蛋白PET图像，为阿尔茨海默病诊断提供了成本更低、更易获取的替代方案。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病诊断严重依赖淀粉样蛋白PET成像，但该方法成本高昂且可及性有限。研究旨在探索是否可以从血液生物标志物和MRI扫描中预测淀粉样蛋白PET的空间模式。

Method: 收集566名参与者的淀粉样蛋白PET图像、T1加权MRI扫描和血液生物标志物数据。开发了基于大语言模型的多模态信息融合语言增强生成模型来合成PET图像，并在全自动诊断流程中评估合成图像的质量、诊断一致性和临床应用性。
...
TL;DR: 提出了一种基于SAM-2分割和3D高斯溅射的草莓植株三维重建框架，通过背景去除实现更精确的几何表示和植物性状自动估计。
...
Method: 结合SAM-2分割和alpha通道背景掩码的预处理流程，实现无背景的草莓植株重建，并使用DBSCAN聚类和PCA自动估计植株高度和冠层宽度。

Result: 实验结果表明该方法在准确性和效率上均优于传统流程，为草莓植株表型分析提供了可扩展的非破坏性解决方案。

Conclusion: 提出的对象中心化3D重建框架能够有效解决农业领域中3DGS应用的背景噪声问题，显著提升植物性状分析的准确性和计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前左心室节段纵向应变估计技术需要大量人工干预和专业知识，限制了效率，使其不适合监测目的。需要开发自动化解决方案来提高心脏功能评估的精确性和效率。

Method: 利用SIMUS模拟管道生成高度逼真的合成TEE数据集，包含80名患者的真实心肌运动数据，用于训练和评估两种深度学习模型：TeeFlow（密集帧间预测）和TeeTracker（稀疏长序列预测）。

Result: TeeTracker在准确性上优于TeeFlow，在synTEE测试数据集上的运动估计平均距离误差为0.65毫米。临床验证显示autoStrain管道与临床参考值一致，平均差异为1.09%。模拟缺血数据的整合提高了模型量化异常变形的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 移动感知系统长期面临感知质量与效率之间的基本权衡，现有稀疏感知方法由于跨时空信息缺失而存在精度降低的问题，研究基础模型是否能改变这一局面。

Method: 使用真实移动AR数据进行评估，研究基础模型在几何感知图像扭曲技术中的表现，这是实现跨帧信息准确重用的核心技术。

Result: 评估表明基础模型在几何感知图像扭曲方面带来显著改进，基于基础模型的稀疏感知具有可扩展性，在3D场景重建中表现出领先性能。
...
<details>
  <summary>Details</summary>
Motivation: 医学报告生成(MRG)是现代医学诊断的关键部分，但现有模型在病灶描述方面面临三个主要挑战：领域知识理解不足、文本-视觉实体嵌入对齐差、跨模态偏差导致的伪相关。先前工作仅解决单个挑战，本文旨在同时解决所有三个挑战。
...
Conclusion: HTSC-CIF框架通过分层任务分解方法成功解决了医学报告生成中的三个关键挑战，在性能上显著优于现有方法，代码将在论文接受后公开。
...
TL;DR: 本文探讨了使用欧拉角参数化作为归一化流模型的基础来进行物体姿态估计的实用性，尽管欧拉角存在一些缺点，但在某些方面可能比更复杂的参数化方法更有用。


<details>
  <summary>Details</summary>
Motivation: 物体姿态估计在3D计算机视觉中至关重要。虽然单点估计通常足够，但在传感器和投影约束或物体固有对称性导致姿态不明确时，概率性姿态输出具有诸多优势。

Method: 使用欧拉角参数化作为归一化流模型的基础，探索这种参数化方法在姿态估计中的应用。

Result: 研究发现欧拉角参数化尽管存在一些缺点，但在多个方面可能产生有用的模型，与基于更复杂参数化的模型相比具有优势。

Conclusion: 欧拉角参数化可以作为构建有效姿态估计模型的基础，在某些情况下比更复杂的参数化方法更实用。
...
TL;DR: SAIL-RL是一个强化学习后训练框架，通过教多模态大语言模型何时以及如何思考来增强其推理能力，使用双重奖励系统解决现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法受限于仅基于结果的监督（只奖励正确答案而不确保合理的推理过程）和统一的思考策略（在简单任务上过度思考，在复杂任务上思考不足）。

Method: 采用双重奖励系统：思考奖励（通过事实基础、逻辑连贯性和答案一致性评估推理质量）和判断奖励（自适应确定何时需要深度推理或直接回答）。

Result: 在SAIL-VL2上的实验表明，SAIL-RL在4B和8B规模上均提升了推理和多模态理解基准性能，与GPT-4o等商业闭源模型竞争，并显著减少了幻觉。
...
TL;DR: Cycle-Sync是一个用于估计相机姿态（旋转和位置）的鲁棒全局框架，通过改进消息传递最小二乘法来强调循环一致性信息，无需相机间距离即可实现最低样本复杂度，实验表明其优于现有姿态估计方法。


<details>
  <summary>Details</summary>
Motivation: 现有相机姿态估计方法通常需要相机间距离信息或依赖捆绑调整，作者希望开发一个仅依赖循环一致性就能实现鲁棒全局姿态估计的框架，避免复杂的后处理步骤。

Method: 改进消息传递最小二乘法（MPLS），重新定义循环一致性使用迭代估计的距离，引入Welsch型鲁棒损失函数，添加基于鲁棒子空间恢复的离群值拒绝模块，并将循环一致性完全集成到旋转同步中。

Result: 建立了相机位置估计的最强确定性精确恢复保证，仅凭循环一致性即可达到目前已知的最低样本复杂度，在合成和真实数据集上始终优于领先的姿态估计方法，包括带有捆绑调整的完整运动结构流程。

Conclusion: Cycle-Sync证明了循环一致性本身足以实现鲁棒的相机姿态估计，无需相机间距离信息或捆绑调整，为全局姿态估计提供了新的理论保证和实用框架。
...
TL;DR: 提出了一种名为GAFD-CC的分布外检测方法，通过全局感知特征解耦和置信度校准来改进决策边界并提升判别性能。


<details>
  <summary>Details</summary>
Motivation: 现有的分布外检测方法往往忽略了特征与logits之间的内在相关性，这对有效的分布外检测至关重要。

Method: 首先通过分类权重引导进行全局感知特征解耦，提取正相关和负相关特征；然后自适应地将解耦特征与多尺度logit置信度融合。

Result: 在大规模基准测试上的广泛实验表明，GAFD-CC相比最先进方法具有竞争性性能和强大泛化能力。

Conclusion: GAFD-CC通过利用特征与logits的关联性，有效提升了分布外检测的性能和鲁棒性。
...
Method: 提出RxnCaption框架，将传统坐标预测驱动的解析过程重新定义为图像描述问题，引入BIVP策略，使用MolYOLO分子检测器预绘分子边界框和索引。
...
TL;DR: 本文提出了一种自监督学习方法，用于稀疏嘈杂的雷达点云中的运动目标分割，通过对比学习和聚类损失函数来减少对标注数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 雷达传感器能够直接测量点的多普勒速度，适合单次扫描的运动目标分割，但雷达点云稀疏嘈杂，数据标注成本高昂且耗时。

Method: 采用两步法：首先使用基于聚类的对比自监督表示学习，通过动态点去除进行聚类精炼；然后使用有限标注数据进行监督微调。

Result: 该方法提高了标签效率，通过自监督预训练有效提升了最先进方法的性能。

Conclusion: 自监督学习能够显著减少对标注数据的依赖，在雷达点云运动目标分割任务中取得了良好效果。
...
TL;DR: 提出一种基于分组的混合颜色校正算法，用于彩色点云的颜色一致性校正。根据重叠率将目标点云分为2-3组，分别采用K近邻双边插值、联合KBI和直方图均衡化、直方图均衡化等方法进行颜色校正。


<details>
  <summary>Details</summary>
Motivation: 彩色点云的颜色一致性校正在3D渲染和压缩应用中非常重要，但以往方法主要针对彩色图像，缺乏专门针对点云的有效颜色校正方法。

Method: 1. 估计源点云和目标点云的重叠率；2. 根据重叠率自适应地将目标点分为2组（Gcl和Gmod）或3组（Gcl、Gmod和Gdist）；3. 对Gcl组使用K近邻双边插值（KBI）方法；4. 对Gmod组使用联合KBI和直方图均衡化（JKHE）方法；5. 对Gdist组使用直方图均衡化（HE）方法。

Result: 在1086个测试彩色点云对上验证了算法的有效性，与最先进方法相比表现出良好的颜色一致性校正效果。
...
TL;DR: 该研究提出了一个自动化多阶段代码驱动管道，用于生成视觉推理数据集ChartM³，包含38K图表和142K问答对，通过监督微调和强化学习显著提升了模型在复杂图表理解中的推理能力和跨域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在复杂图表理解任务中的视觉识别和推理能力有限，且现有研究对真实应用中复杂图表场景和计算密集型推理任务的覆盖不足。

Method: 采用自动化多阶段代码驱动管道，集成检索增强生成(RAG)获取专业图表模板，使用思维链(CoT)策略生成模拟真实数据分布的推理代码，驱动图表渲染和问题相关统计计算。

Result: 构建了ChartM³数据集，包含38K图表和142K问答对用于训练，以及2,871个高质量评估样本。实验表明该数据集显著提升了模型的推理能力和跨域泛化性能，使较小模型在复杂图表理解中达到与大规模模型相当的性能。

Conclusion: 提出的自动化数据生成管道有效解决了复杂图表理解数据集稀缺问题，ChartM³数据集显著提升了多模态大语言模型的推理能力和泛化性能，为复杂图表理解任务提供了实用解决方案。
...
TL;DR: 本文提出了一种使用Blender生成合成作物-杂草图像的方法，用于解决农业除草机器人语义分割中真实标注数据获取成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 农业除草机器人需要精确的作物和杂草语义分割，但深度学习模型训练需要大量标注数据，在真实田间获取成本高昂。合成数据可以减轻这一负担，但模拟图像与真实图像之间的差距仍然是一个挑战。

Method: 使用Blender构建程序化生成合成作物-杂草图像的流程，生成包含不同植物生长阶段、杂草密度、光照条件和相机角度的标注数据集。

Result: 在合成和真实数据集上对多个最先进的分割模型进行基准测试，结果显示在合成图像上训练导致10%的模拟到真实差距，超过了之前的最先进方法。此外，合成数据在跨域场景中表现出良好的泛化特性，优于真实数据集。
...
TL;DR: 本文研究了适用于移动机器人边缘设备的小型视觉语言模型在场景理解和动作识别任务中的能力，重点关注计算复杂度与推理时间的平衡。
...
Method: 提出一个评估管道，在包含真实世界城市景观、校园和室内场景的多样化数据集上，专门研究能够部署到移动机器人边缘设备的小型视觉语言模型的性能。

Result: 实验评估讨论了这些小型模型在边缘设备上的潜力，特别关注挑战、弱点、固有模型偏差以及所获信息的应用。

Conclusion: 研究为小型视觉语言模型在移动机器人边缘设备上的场景解释应用提供了系统评估，揭示了其在资源受限环境中的可行性和局限性。
...
TL;DR: KAO是一个用于卫星图像修复的新框架，利用核自适应优化和扩散模型，专门针对高分辨率卫星数据集设计，通过潜在空间条件化和显式传播实现高效准确的修复。


<details>
  <summary>Details</summary>
Motivation: 卫星图像修复在遥感分析中至关重要，现有方法要么需要大量重新训练，要么计算开销大，因此需要一种平衡效率和灵活性的解决方案。

Method: 提出KAO框架，采用核自适应优化和扩散模型，引入潜在空间条件化方法优化紧凑潜在空间，并加入显式传播实现前向-后向融合。

Result: 实验结果表明KAO在高分辨率卫星图像修复方面设立了新的基准，提供了可扩展的高性能解决方案。

Conclusion: KAO成功平衡了预条件模型的效率和后条件模型的灵活性，为卫星图像修复提供了有效的解决方案。
...
TL;DR: 本文提出了一种名为MVAFormer的多视角动作识别方法，专门针对时空动作识别（STAR）设置，通过基于Transformer的协作模块有效结合多视角信息，在保留空间信息的同时建模视图间关系。


<details>
  <summary>Details</summary>
Motivation: 传统多视角动作识别方法仅适用于从整个视频中识别单一动作的任务设置，无法应用于新兴的时空动作识别（STAR）设置，该设置需要按顺序识别每个人的动作。为解决这一问题，需要开发适用于STAR设置的多视角协作方法。

Method: 提出MVAFormer方法，引入基于Transformer的协作模块，使用保留空间信息的特征图而非丢失空间信息的嵌入向量，并将自注意力分为相同视图和不同视图两部分，以有效建模多视角间关系。

Result: 在新收集的数据集上的实验结果表明，MVAFormer在F-measure指标上比基线方法提升了约4.4个百分点。

Conclusion: MVAFormer成功解决了STAR设置下的多视角动作识别问题，通过保留空间信息的特征图和分视图自注意力机制，实现了有效的多视角协作，显著提升了识别性能。
...
TL;DR: OLATverse是一个大规模真实世界对象数据集，包含约900万张图像，涵盖765个真实物体，在精确控制的光照条件下从多个视角拍摄，为逆渲染和重光照研究提供高质量基准。
...
Method: 使用35台DSLR相机和331个独立控制光源捕获765个真实物体，涵盖多种材料类别，提供相机参数、物体掩码、表面法线和漫反射反照率等辅助资源。

Result: 构建了包含9M图像的大规模数据集，建立了首个全面的真实世界对象中心逆渲染和法线估计基准。
...
TL;DR: 本文介绍了DetectiumFire，一个大规模多模态火灾数据集，包含22.5k高分辨率火灾图像和2.5k真实世界火灾视频，带有详细标注，用于推进火灾相关AI研究。
...
Method: 构建DetectiumFire数据集，包含大量火灾相关图像和视频，并标注传统计算机视觉标签和详细文本提示。

Result: 该数据集在规模、多样性和数据质量方面优于现有基准，显著减少了冗余并增强了真实场景的覆盖。在目标检测、图像生成和视觉语言推理等任务中验证了其有效性。

Conclusion: DetectiumFire数据集有潜力推进火灾相关研究，支持智能安全系统的开发，并已公开发布以促进AI社区对火灾理解的更广泛探索。
...
TL;DR: 提出了UniChange，首个基于多模态大语言模型的统一变化检测模型，通过特殊令牌[T1]、[T2]、[CHANGE]和文本提示统一处理二元变化检测和语义变化检测任务。


<details>
  <summary>Details</summary>
Motivation: 当前变化检测模型通常只能从单一类型标注数据中学习，无法同时利用多种二元变化检测和语义变化检测数据集，导致泛化能力差和功能受限。

Method: 利用多模态大语言模型的语言先验和统一能力，集成生成式语言能力与专门的变化检测功能，通过三个特殊令牌和文本提示引导变化类别识别。

Result: 在四个公开基准测试（WHU-CD、S2Looking、LEVIR-CD+、SECOND）上达到最先进性能，IoU分数分别为90.41、53.04、78.87和57.62。

Conclusion: UniChange成功统一了二元变化检测和语义变化检测任务，能够有效从多源数据集中获取知识，即使类别定义存在冲突，展现了强大的泛化能力和统一框架优势。
...
TL;DR: 本文介绍了PtychoBench基准测试，用于系统比较监督微调(SFT)和上下文学习(ICL)在科学任务中的表现。研究发现最优专业化策略取决于任务模态：视觉任务中SFT和ICL互补，文本任务中ICL表现更优。
...
Method: 引入PtychoBench多模态多任务基准，系统比较监督微调(SFT)和上下文学习(ICL)两种专业化策略，在数据稀缺环境下评估视觉伪影检测和文本参数推荐任务。

Result: 视觉任务中，SFT和ICL高度互补，微调模型在上下文感知示例指导下达到最高性能（Micro-F1 0.728）；文本任务中，ICL在大型基础模型上表现最优（Micro-F1 0.847），优于强大的SFT模型。

Conclusion: 最优专业化路径取决于任务模态，为开发更有效的科学智能系统提供了明确框架，确认了上下文感知提示的优越性，并识别了微调模型中的一致上下文干扰现象。
...
TL;DR: 提出了一种用于移动机器人自动生成自然语言报告的管道，使用本地多模态传感器模型，可在边缘设备上部署，保护隐私且无需外部服务。


<details>
  <summary>Details</summary>
Motivation: 深度学习使机器人系统能在动态非结构化环境中交互，但关键任务应用需要评估大量异构数据。自动报告生成有助于促进这类系统的评估和接受。

Method: 开发了一个自动化报告生成管道，利用各种多模态传感器，完全基于可在边缘计算设备上部署的本地模型。

Result: 在包含室内、室外和城市环境的多样化数据集上评估了实现，提供了定量和定性评估结果。

Conclusion: 该管道能够生成自动报告，保护所有参与者的隐私，无需外部服务，为移动机器人在各领域的应用提供了可行的解决方案。
...
TL;DR: LiteVoxel是一个自调节的稀疏体素光栅化训练管道，通过逆Sobel重加权和深度分位数剪枝等创新方法，解决了传统SV光栅化在低频内容拟合不足、内存占用过高的问题，显著降低了VRAM使用同时保持重建质量。


<details>
  <summary>Details</summary>
Motivation: 传统稀疏体素光栅化方法存在三个主要问题：对低频内容拟合不足、依赖于脆弱的剪枝启发式方法、以及内存过度增长导致VRAM膨胀。这些限制了该方法在实际应用中的稳定性和效率。

Method: 1. 使用逆Sobel重加权和训练中期的gamma斜坡使损失函数对低频内容敏感；2. 采用基于深度分位数的剪枝逻辑替代固定阈值；3. 通过EMA滞后保护和基于射线足迹的优先级驱动细分来稳定结构；4. 在明确的增长预算下进行优化。

Result: 在Mip-NeRF 360和Tanks & Temples数据集上的实验表明，LiteVoxel显著减少了低频区域的误差和边界不稳定性，同时保持了与强基线相当的PSNR/SSIM、训练时间和FPS。关键的是，将峰值VRAM降低了40%-60%，并保留了先前方法遗漏的低频细节。

Conclusion: LiteVoxel实现了更可预测、内存效率更高的训练，无需牺牲感知质量，为稀疏体素光栅化提供了一种更稳定、更轻量的替代方案。
...
Method: 开发了包含可重复缺陷模式的控制数据集，在理想和实际变形条件下系统获取剪切散斑测量数据。定义了两种训练子集：仅包含无缺陷样本和额外包含全局变形但无缺陷的数据。评估了三种无监督学习架构，并与基于标记数据的YOLOv8模型进行基准比较。

Result: 师生特征匹配模型在分类鲁棒性和空间缺陷定位方面表现最佳，相比自编码器模型显示出改进的特征表示可分性。该模型能够精确定位缺陷，并在包含变形引起的条纹图案的实际条件下保持良好性能。

Conclusion: 本研究强调了无监督深度学习在工业环境中实现可扩展、标签高效的剪切散斑检测的潜力，师生方法为自动化检测提供了有前景的解决方案。
...
TL;DR: 该研究使用深度学习模型从基线MRI预测未来数年的大脑MRI图像，在ADNI和AIBL队列上评估了五种架构，实现了高保真度的脑部图像预测，并在独立外部数据集上表现出良好的泛化能力。
...
Method: 在ADNI和AIBL两个纵向队列上实施并评估了五种深度学习架构：UNet、U2-Net、UNETR、时间嵌入UNet和ODE-UNet，通过全局相似性和局部差异指标比较预测结果与实际随访扫描。

Result: 表现最佳的模型实现了高保真度预测，所有模型在独立外部数据集上均表现出良好的泛化能力，展示了稳健的跨队列性能。

Conclusion: 深度学习能够可靠地在体素级别预测参与者特异性的大脑MRI，为个体化预后提供了新的机会。
...
TL;DR: UVH-26是首个印度大规模交通摄像头图像数据集，包含26,646张高分辨率图像，标注了14种印度特有车辆类型，通过众包方式获得180万个边界框标注。在该数据集上训练的检测器相比COCO数据集训练有8.4-31.5%的性能提升。
...
Conclusion: UVH-26填补了现有全球基准在印度交通场景的空白，证明了领域特定训练数据对印度复杂交通条件的重要性，为智能交通系统在新兴国家的部署提供了基础。
...
TL;DR: VCFlow是一个无需受试者特定训练的通用脑解码框架，通过建模人类视觉系统的腹背侧结构来重建连续视觉体验，在仅牺牲7%准确率的情况下实现快速视频重建。


<details>
  <summary>Details</summary>
Motivation: 开发无需受试者特定训练的通用脑解码方法，解决跨受试者泛化挑战和脑信号复杂性，为临床应用提供可扩展解决方案。

Method: 提出分层解码框架VCFlow，明确建模人类视觉系统的腹背侧架构，分离利用早期视觉皮层、腹侧和背侧流特征，并引入特征级对比学习策略增强主体不变语义表示。

Result: 相比传统方法需要12小时以上数据和大量计算，VCFlow仅牺牲7%平均准确率，但能在10秒内生成重建视频且无需重新训练。

Conclusion: VCFlow提供了一种快速且临床可扩展的解决方案，在主体无关脑解码方面具有重要应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型只能输出单一扁平图像，无法满足专业应用中对分层控制的需求。现有解决方案要么需要大量数据集进行微调，要么只能生成孤立的前景元素而无法产生完整连贯的场景。
...
Method: 通过结合Grounding Dino目标检测器和Segment Anything Model 2 (SAM 2)跟踪器，并精心设计启发式方法，开发了一个无需重新训练或超参数调整的跟踪框架。

Result: 在ChimpAct、Bird Flock Tracking、AnimalTrack和GMOT-40子集上的评估显示，该方法在不同物种和环境中表现出强大且一致的性能。

Conclusion: 该方法展示了视觉基础模型在零样本多动物跟踪中的潜力，为动物行为研究提供了有效的工具。
...
TL;DR: 提出了一种轻量级CNN框架来检测面部识别系统中的打印/显示、视频和包裹攻击，确保快速生物特征认证（CPU上1-2秒），并创建了包含60个受试者500多个视频的新型2D欺骗攻击数据集。
...
Result: 提出的鲁棒架构能够快速进行生物特征认证（CPU上1-2秒），并创建了包含60个受试者500多个视频的新型2D欺骗攻击数据集来验证有效性。

Conclusion: 该轻量级CNN框架有效解决了面部识别系统中的呈现攻击问题，提供了快速可靠的活体检测解决方案。
...
TL;DR: UniPruneBench是一个统一的可扩展基准，用于评估多模态大模型中视觉token剪枝方法，涵盖6个能力维度和10个数据集，比较10种压缩算法在3种LMM上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型由于图像编码器引入大量视觉token导致推理效率低下，而现有的token压缩方法评估分散且不一致，需要统一的评估框架。

Method: 构建UniPruneBench基准，采用标准化协议评估10种代表性压缩算法在LLaVA-v1.5、Intern-VL3和Qwen2.5-VL三种LMM上的表现，除任务精度外还包含运行时和预填充延迟等系统级指标。

Result: 实验发现：(1)随机剪枝是强基线；(2)没有单一方法在所有场景中表现最优；(3)剪枝敏感性因任务而异，OCR最脆弱；(4)剪枝比例是性能下降的主导因素。

Conclusion: UniPruneBench为高效多模态建模的未来研究提供了可靠基础，揭示了剪枝方法在不同任务和模型中的复杂表现模式。
...
TL;DR: 提出一种端到端可微分的tokenizer，能够根据图像内容自适应生成token，同时保持与现有架构的向后兼容性，在图像分类和密集预测任务中表现优异。
...
Result: 在图像级分类和密集预测任务中取得竞争性性能，并支持开箱即用的栅格到矢量转换。

Conclusion: 该方法为Vision Transformers提供了更智能的token生成机制，同时保持与现有预训练模型的兼容性。
...
TL;DR: 提出了一种基于情感线索引导的推理框架，通过两阶段调优的视频情感基础模型(VidEmo)来理解视频中的复杂情感状态，并构建了包含210万样本的细粒度情感数据集Emo-CFG。


<details>
  <summary>Details</summary>
Motivation: 视频情感分析面临情感动态性和线索依赖性的挑战，难以理解复杂且不断演化的情感状态及其合理依据。

Method: 提出情感线索引导的推理框架，统一基本属性感知、表情分析和高级情感理解；开发视频情感基础模型(VidEmo)，采用两阶段调优：课程情感学习和情感树强化学习。

Result: 实验结果表明该方法在15个面部感知任务上取得了有竞争力的性能，树立了新的里程碑。

Conclusion: 该框架通过情感线索引导的推理和专门设计的基础模型，有效解决了视频情感分析的挑战，为情感理解任务提供了重要资源。
...
Method: 通过综合研究视频-文本表示对齐，分析不同编码器的能力。提出了参数化测试时缩放定律来捕捉视觉和文本数据丰富度对对齐效果的影响。

Result: 研究发现跨模态对齐高度依赖于测试时提供的视觉和文本数据的丰富程度。强对齐与文本编码器可能与通用视频表示和理解相关。提供了时序推理与跨模态对齐的相关性分析。

Conclusion: 视频-文本对齐是一种信息丰富的零样本方法，可用于探测不同编码器对时空数据的表示能力。这项工作为视频表示学习提供了新的评估框架。
...
TL;DR: PercHead是一种从单张图像进行3D头部重建和语义3D编辑的统一方法，采用双分支编码器和ViT解码器，通过DINOv2和SAM2.1进行感知监督，在重建和编辑任务上均表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决单图像3D头部重建和语义3D编辑面临的严重视角遮挡、弱感知监督和3D编辑模糊性等固有挑战。

Method: 使用双分支编码器和基于ViT的解码器，通过迭代交叉注意力将2D特征提升到3D空间，采用高斯泼溅进行渲染，并基于DINOv2和SAM2.1进行感知监督。

Result: 在新视角合成方面达到最先进性能，对极端视角表现出卓越鲁棒性，可通过轻量级交互GUI实现直观的3D编辑。

Conclusion: PercHead提供了一个统一的3D头部重建和编辑框架，通过感知监督和模块化设计实现了高质量的重建结果和直观的语义编辑能力。
...
TL;DR: MIRA是一个新的基准测试，旨在评估模型在需要生成中间视觉图像来指导推理过程的场景中的表现。该基准包含546个多模态问题，要求模型生成草图、结构图等中间图像来辅助推理，模拟人类"画图思考"的过程。


<details>
  <summary>Details</summary>
Motivation: 传统的思维链方法仅依赖文本，而许多复杂问题需要视觉信息来辅助推理。MIRA旨在评估模型在需要生成和利用中间视觉图像进行推理的任务中的表现，更贴近人类解决复杂问题的方式。

Method: MIRA设计了包含546个多模态问题的基准，每个问题都标注了中间视觉图像和最终答案。提出了统一的评估协议，包括三个层次的输入：仅图像和问题的直接输入、带思维提示的文本思维链输入、以及带注释图像线索和文本思维提示的视觉思维链输入。

Result: 实验结果显示，现有多模态大语言模型在仅依赖文本提示时表现不佳，但当提供中间视觉线索时，所有模型和任务的平均相对增益达到33.7%。扩大搜索空间和设计对齐视觉思维链的文本提示只能带来有限的改进。

Conclusion: MIRA基准证明了想象视觉信息在成功推理中的关键作用，强调了在复杂推理任务中生成和利用中间视觉图像的重要性。
...
TL;DR: 本文提出了一个统一的基准测试框架，用于在受控和可复现条件下系统评估AI生成媒体检测方法，解决了现有方法在基准标准化、训练协议一致性和评估指标方面的关键差距。


<details>
  <summary>Details</summary>
Motivation: AI生成的媒体（特别是深度伪造）对多媒体取证、错误信息检测和生物识别系统构成严重威胁，导致公众对法律系统信任度下降、欺诈增加和社会工程攻击。现有取证方法存在基准不统一、训练协议不一致和评估指标有限等问题。

Method: 引入统一的基准测试框架，对十种最先进的取证方法（从头训练、冻结和微调）和七个公开可用数据集（GAN和扩散生成）进行系统评估，使用多种指标（准确率、平均精度、ROC-AUC、错误率和类敏感度）并分析模型可解释性。

Result: 评估显示不同方法在泛化能力上存在显著差异，某些方法在分布内表现良好但跨模型可迁移性较差。通过置信度曲线和Grad-CAM热图进一步分析模型可解释性。

Conclusion: 本研究旨在引导研究社区更深入理解当前取证方法的优势和局限性，并启发开发更鲁棒、可泛化和可解释的解决方案。
...
TL;DR: PLUTO-4是下一代病理学基础模型，包含PLUTO-4S（紧凑高效）和PLUTO-4G（前沿规模）两个互补的Vision Transformer架构，在55万张WSI上预训练，在多个病理学任务中达到最先进性能。
...
Method: 使用两种Vision Transformer架构：PLUTO-4S采用FlexiViT设置和2D-RoPE嵌入实现多尺度部署优化；PLUTO-4G使用单一补丁尺寸最大化表示能力。基于DINOv2的自监督目标在包含55万张WSI的大规模多机构数据集上预训练。

Result: PLUTO-4在需要不同空间和生物学背景的任务中达到最先进性能，包括补丁级分类、分割和幻灯片级诊断。PLUTO-4S提供高吞吐量和稳健性能，PLUTO-4G在多个病理学基准测试中建立新性能前沿，皮肤病理学诊断提升11%。
...
TL;DR: IG-Pruning是一种新颖的输入感知块级剪枝方法，通过动态选择层掩码来减少大型语言模型的计算成本，无需大量训练即可实现高效动态剪枝。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型计算需求增长，高效推理对实际部署至关重要。现有深度剪枝方法依赖固定块掩码，在不同任务和输入上表现不佳。

Method: 两阶段方法：(1)通过语义聚类和L0优化发现多样化掩码候选；(2)在推理时无需大量训练即可实现高效动态剪枝。

Result: 实验结果表明该方法始终优于最先进的静态深度剪枝方法。
...
TL;DR: 本研究开发了一个自动化基础设施来检测医疗聊天机器人在涉及人口统计信息时的幻觉、遗漏和偏见问题，发现LLM评估者之间存在低一致性，建议使用多个LLM作为评估者以提高结果可靠性。


<details>
  <summary>Details</summary>
Motivation: 医疗聊天机器人在涉及非医疗因素（如人口统计信息）时必须提供一致的建议，但目前存在幻觉、遗漏和偏见问题，需要系统性地检测这些失败情况。

Method: 开发自动化基础设施：1）通过采样患者人口统计、病史、疾病和写作风格自动生成查询；2）使用多个LLM作为评判者设置和提示来评估答案，包括幻觉和遗漏检测以及治疗类别检测。

Result: LLM注释者之间一致性较低（平均Cohen's Kappa κ=0.118），只有特定的（回答、评估）LLM对在不同写作风格、性别和种族上产生统计显著差异。
...
TL;DR: LTD-Bench是一个突破性的基准测试，通过要求模型生成点阵绘图或可执行代码，将LLM评估从抽象分数转变为可直接观察的视觉输出，揭示了当前LLM在空间推理方面的严重缺陷。
...
Conclusion: LTD-Bench的视觉输出能够进行强大的诊断分析，为研究模型相似性提供了潜在方法，揭示了LLM作为真正世界模型的基本局限性。
...
TL;DR: M-Solomon是一种多模态嵌入器，通过自适应查询增强来改进文档检索性能，只在必要时进行查询增强，从而在保持高准确率的同时显著降低嵌入延迟。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的嵌入器对所有查询都进行增强，导致显著的嵌入延迟，且某些查询增强反而会损害性能。同时，之前的方法未在多模态环境中探索。

Method: 首先将训练数据集中的查询分为需要增强和不需要增强两组；利用强大的多模态LLM为需要增强的查询生成合适的增强内容；通过自适应查询增强，M-Solomon只在必要时进行查询增强，学习为需要增强的查询生成带/augment前缀的合成增强，为其他查询生成简单的/embed字符串。

Result: 实验结果显示，M-Solomon不仅大幅超越了无增强的基线方法，也优于总是使用增强的基线方法，同时提供了更快的嵌入延迟。

Conclusion: M-Solomon通过自适应查询增强策略，在多模态环境中实现了更好的检索性能和更低的延迟，证明了选择性查询增强的有效性。
...
### [55] [Beyond Single Embeddings: Capturing Diverse Targets with Multi-Query Retrieval](https://arxiv.org/abs/2511.02770)
*Hung-Ting Chen,Xiang Liu,Shauli Ravfogel,Eunsol Choi*

Main category: cs.CL

TL;DR: 提出了一种新的检索器架构AMER，通过自回归生成多个查询向量来捕捉查询的多模态相关文档分布，相比单向量检索器在目标文档嵌入距离较大时表现显著更好。


<details>
  <summary>Details</summary>
Motivation: 现有检索器通常只生成一个查询向量，但查询的相关文档分布可能是多模态的（如不同的查询解释），单向量难以有效捕捉这种多模态分布。
...
TL;DR: LiveSecBench是一个针对中文LLM应用场景的动态安全基准测试，从六个关键维度评估模型安全性，并持续更新以保持相关性。


<details>
  <summary>Details</summary>
Motivation: 为中文语言模型应用场景建立一个动态更新的安全评估基准，确保评估框架与中国法律和社会规范保持一致。

Method: 构建包含合法性、道德性、事实性、隐私性、对抗鲁棒性和推理安全性六个维度的评估体系，采用动态更新机制纳入新的威胁向量。

Result: 当前版本(v251030)已评估18个LLM，提供了中文语境下AI安全的全景图，并建立了公开的排行榜。

Conclusion: LiveSecBench为中文LLM安全评估提供了标准化框架，其动态更新机制确保了评估的持续相关性。
...
TL;DR: AyurParam-2.9B是一个专门针对阿育吠陀医学领域的双语语言模型，通过专家精心策划的数据集进行微调，在专业医学领域超越了同规模的开源模型，甚至能与更大模型竞争。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在通用任务上表现出色，但在需要深厚文化、语言和专业知识的专业领域（如阿育吠陀医学）表现不佳，无法准确解释和应用这些领域的知识。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM安全评估主要关注单轮交互，而现实攻击往往通过自适应多轮对话展开，需要开发能够评估多轮对话安全性的框架。

Method: AutoAdv结合三种自适应机制：模式管理器从成功攻击中学习增强后续提示；温度管理器基于失败模式动态调整采样参数；两阶段重写策略先伪装有害请求再迭代优化。

Result: 在Llama-3.1-8B上达到95%攻击成功率，相比单轮基线提升24%；在GPT-4o-mini、Qwen3-235B、Mistral-7B等商业和开源模型上均显示多轮攻击持续优于单轮方法。
...
TL;DR: 该论文研究了在金融领域通过合并领域特定的持续预训练专家模型来构建多技能大语言模型的方法，填补了CPT模型合并的研究空白。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用任务上表现出色，但在金融等专业领域表现不佳，需要融合领域知识、数学推理和多语言处理等多种技能。直接进行多技能训练成本高昂且不稳定，而合并领域专家模型提供了一个实用的替代方案。

Method: 创建金融、数学和日语领域的专家模型，提出三阶段评估框架（知识恢复、互补性和涌现性），在包含18个任务、8个数据集的综合金融基准上评估三种合并方法（Task Arithmetic、TIES和DARE-TIES）。

Result: 合并专家模型与其基础模型可以恢复在CPT过程中丢失的通用知识；合并不同专家模型能提升性能并产生跨领域的涌现技能；Task Arithmetic表现强劲但对超参数敏感，TIES更稳健；模型相似性与合并成功相关，但涌现技能取决于更复杂因素。
...
TL;DR: 评估基于角色的提示是否能提高大语言模型在宏观经济预测任务中的表现。研究发现GPT-4o与人类预测者准确度相似，但角色描述对预测准确性没有可衡量的优势。


<details>
  <summary>Details</summary>
Motivation: 探索角色提示是否能增强LLM在宏观经济预测中的表现，比较AI与人类专家在预测任务中的差异。

Method: 使用PersonaHub语料库中的2,368个经济学相关角色提示GPT-4o，复制ECB专业预测者调查，比较角色提示与无角色基线的预测结果，涵盖4个目标变量和4个预测时间范围。

Result: GPT-4o与人类预测者准确度非常相似，统计显著但实际差异不大；角色描述没有带来可衡量的预测优势；在2024-2025样本外数据上GPT-4o保持竞争力但表现与样本内期有差异。

Conclusion: GPT-4o在提供相关背景数据时能在宏观经济预测中达到竞争力，但多样化提示产生的预测结果相比人类面板更为同质化，角色提示组件可省略以节省计算成本而不影响准确性。
...
TL;DR: 该研究分析了Google Translate从英语到罗马尼亚语的机器翻译中的词汇错误，重点关注WHO、Gavi组织和药品说明书等官方COVID-19相关文本的翻译质量。


<details>
  <summary>Details</summary>
Motivation: 提高机器翻译的整体质量，通过改善词汇选择和减少错误来增强Google Translate的翻译准确性，特别是在官方医疗信息的翻译中。

Method: 对230篇从英语翻译成罗马尼亚语的文本进行综合分析，这些文本包含WHO、Gavi组织和药品说明书等官方COVID-19相关信息。

Result: 研究发现机器翻译在词汇选择方面存在错误，特别是在专业医疗术语和官方信息的翻译中。

Conclusion: 通过识别和解决词汇错误，可以显著提高机器翻译的质量，特别是在处理专业和官方文本时。
...
<details>
  <summary>Details</summary>
Motivation: 现有知识追踪模型通常忽略问题文本这一重要的教学洞察来源，这限制了预测性能并错失了机会。

Method: 提出NTKT方法，将知识追踪重构为使用预训练大语言模型的下一词预测任务，将学生历史和问题内容表示为文本序列。

Result: 实验显著优于最先进的神经知识追踪模型，在冷启动问题和用户上表现出更好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统self-consistency方法需要固定调用次数，且当正确答案罕见时会失败，需要更高效的采样策略。

Method: 使用贝叶斯框架，基于token概率或奖励模型得到的置信度信号形成候选答案的后验分布，当某个候选的后验质量超过阈值时自适应停止采样。

Result: 在五个推理基准测试中，CGES将平均模型调用次数减少约69%（如从16.0次降至4.9次），同时保持与self-consistency方法相当的准确性（差异在0.06个百分点内）。

Conclusion: CGES提供了一种理论保证的、高效的采样停止策略，在显著减少计算成本的同时维持推理性能。
...
TL;DR: TRACE框架通过程序化策略应用解决大语言模型对齐问题，实现动态、可扩展的对齐维护，避免大规模重新标注的高成本。
...
Method: TRACE框架通过程序化筛选现有偏好数据、识别高影响冲突的alignment impact score，以及混合优化来反转、丢弃或保留偏好，同时保护模型性能。

Result: 在多种模型家族上的实证结果显示，TRACE在复杂政策变化下实现了稳健的重新对齐，且不降低一般能力。
...
TL;DR: 研究发现，在LLM微调中引入新知识会导致事实幻觉，特别是当特定知识类型完全由新知识组成时。作者提出KnownPatch方法，通过在训练后期添加少量已知知识样本来缓解这种幻觉。
...
Method: 设计了Biography-Reasoning数据集，进行细粒度分析，并提出KnownPatch方法在训练后期添加已知知识样本。

Result: 发现特定知识类型的高陌生度是幻觉的主要驱动因素，新知识学习会降低模型对问题关键实体的注意力，导致过度关注上下文。KnownPatch方法有效缓解了这种注意力破坏。

Conclusion: 新知识微调会导致事实幻觉，KnownPatch方法能有效缓解这一问题，通过恢复模型对关键实体的注意力来提高性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)的巨大规模限制了存储和处理能力，即使存储微调后的模型版本也是一个重大挑战。研究表明微调主要影响一小部分参数，因此需要更高效的微调模型存储方法。

Method: 利用微调更新具有低秩和稀疏特性的观察，提出最优奇异值损伤方法，通过利用奇异向量的交错重要性，有选择地对低秩近似更新进行稀疏化，确保保留最具影响力的组件。

Result: 在相同内存预算下，具有较大秩的稀疏化低秩近似优于具有较小秩的标准低秩近似。通过广泛实验证明，所提方法在相同内存预算下实现了显著的存储效率和优越的准确性。

Conclusion: 结合低秩近似和稀疏化的方法比单独使用任一种技术能更有效地存储微调模型的参数更新，在有限内存资源下保持模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 翻译中经常通过添加背景细节来使隐含的文化含义对新的受众显化，这种现象在翻译理论中广泛讨论但很少进行计算建模。

Method: 通过零对齐识别候选显化案例，并使用主动学习结合人工标注进行精炼，构建多语言语料库和检测框架。

Result: 实体和系统级显化最为频繁，主动学习将分类器准确率提高了7-8个百分点，在跨语言中达到最高0.88的准确率和0.82的F1分数。

Conclusion: PragExTra将语用显化确立为可测量的跨语言现象，并为构建文化感知的机器翻译迈出了一步。
...
<details>
  <summary>Details</summary>
Motivation: 人工智能在全球快速扩散但采用不均衡，前沿大语言模型在低资源语言上表现较差，作者假设这种性能缺陷降低了AI的实用性，从而减缓了低资源语言国家的采用速度。
...
TL;DR: 本文提出了一种集中式多LLM框架CoRL，通过强化学习优化任务性能与推理成本的权衡，实现可控预算下的高效多智能体协作。
...
Method: 采用集中式框架，由控制器LLM选择性地协调专家模型池。将协调问题建模为强化学习，具有双重目标：最大化任务性能同时最小化总体推理成本。提出CoRL框架，在可控多预算设置下优化性能-成本权衡。
...
TL;DR: Oolong是一个长上下文推理基准测试，包含合成任务和真实世界对话任务，要求模型分析文本块并聚合分析结果来回答分布性问题。即使前沿模型在128K上下文长度下准确率也不到50%。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文评估主要依赖检索任务，允许模型忽略大部分上下文作为噪声，这仅代表一种任务类型。需要评估模型在需要分析每个文本块并聚合推理的长上下文任务上的表现。

Method: Oolong分为两个任务集：Oolong-synth（自然合成任务，可轻松消融推理问题组件）和Oolong-real（需要推理真实世界对话数据）。要求模型推理大量示例，在上下文中进行分类和计数，并处理时间和用户关系。

Result: 前沿模型在Oolong上表现不佳，GPT-5、Claude-Sonnet-4和Gemini-2.5-Pro在128K上下文长度下在两个任务集上的准确率都低于50%。

Conclusion: 当前模型在需要分析单个文本块并聚合推理的长上下文任务上仍有困难，Oolong基准测试的发布将促进能够推理大量文本的模型的进一步发展。
...
### [71] [Human-AI Co-Embodied Intelligence for Scientific Experimentation and Manufacturing](https://arxiv.org/abs/2511.02071)
...
TL;DR: 本文提出了一种新的人类-AI共体现智能系统，将人类用户、智能AI和可穿戴硬件集成在一起，用于现实世界的实验和智能制造。通过APEX系统演示，该系统在洁净室柔性电子制造中实现了上下文感知推理，实时纠错，并将专业知识传递给初学者。


<details>
  <summary>Details</summary>
Motivation: 科学实验和制造依赖于复杂、多步骤的程序，需要持续的人类专业知识来精确执行和决策。传统模型局限于虚拟领域，而现实世界的实验和制造仍然依赖人类监督和专业知识，这限制了再现性、可扩展性和可访问性。

Method: 引入人类-AI共体现智能，将人类用户、智能AI和可穿戴硬件集成到一个系统中。人类提供精确执行和控制，智能AI贡献记忆、上下文推理、自适应规划和实时反馈。可穿戴接口持续捕获实验和制造过程，促进人类与AI之间的无缝通信。

Result: APEX系统在洁净室柔性电子制造中实现了上下文感知推理，准确率超过通用多模态大语言模型，实时纠正错误，并将专业知识传递给初学者。
...
### [72] [Automated Reward Design for Gran Turismo](https://arxiv.org/abs/2511.02094)
*Michel Ma,Takuma Seno,Kaushik Subramanian,Peter R. Wurman,Peter Stone,Craig Sherstan*

Main category: cs.AI

TL;DR: 本文展示了如何使用基础模型通过文本指令自动搜索奖励函数，为Gran Turismo 7赛车游戏生成理想的强化学习智能体，结合LLM奖励生成、VLM偏好评估和人类反馈，能够产生与冠军级智能体GT Sophy竞争的赛车智能体及新颖行为。


<details>
  <summary>Details</summary>
Motivation: 在强化学习设计中，通过奖励函数定义期望的智能体行为是一个困难的过程，特别是在复杂环境如自动驾驶赛车中。传统方法难以将期望行为映射到奖励函数。

Method: 采用基础模型搜索奖励函数空间的方法，结合LLM生成奖励函数、VLM进行偏好评估以及人类反馈，仅基于文本指令生成强化学习智能体。

Result: 该系统能够生成与冠军级赛车智能体GT Sophy竞争的赛车智能体，并能产生新颖的行为模式。

Conclusion: 该方法为实现实际应用中自动化奖励设计铺平了道路，展示了基础模型在复杂环境中自动设计奖励函数的潜力。
...
### [73] [Deep Value Benchmark: Measuring Whether Models Generalize Deep values or Shallow Preferences](https://arxiv.org/abs/2511.02109)
*Joshua Ashkinaze,Hua Shen,Sai Avula,Eric Gilbert,Ceren Budak*

Main category: cs.AI

TL;DR: 提出了深度价值基准（DVB）评估框架，用于测试大语言模型是否学习人类基本价值观而非表面偏好。通过实验设计分离深度价值与浅层特征，发现模型平均深度价值泛化率仅为0.30，所有模型表现均低于随机水平。


<details>
  <summary>Details</summary>
Motivation: 区分AI系统是学习人类基本价值观还是仅捕捉表面偏好模式，这对AI对齐至关重要。理解深层价值观的系统能更稳健地泛化人类意图，而仅学习表面模式可能导致行为不匹配。

Method: 使用新颖实验设计，在训练阶段让LLMs接触深度价值与浅层特征故意相关的人类偏好数据，测试阶段打破这些相关性，测量模型基于底层价值而非浅层特征进行泛化的概率（DVGR）。

Result: 测试9个不同模型，平均深度价值泛化率仅为0.30，所有模型泛化深度价值的能力均低于随机水平，且较大模型比小模型DVGR略低。

Conclusion: 当前大语言模型在泛化人类基本价值观方面表现不佳，主要依赖浅层特征而非深层价值原则。DVB提供了对齐核心特征的可解释度量方法。
...
### [74] [InsurAgent: A Large Language Model-Empowered Agent for Simulating Individual Behavior in Purchasing Flood Insurance](https://arxiv.org/abs/2511.02119)
...
Result: LLM对因素有定性理解但定量概率估计不足，InsurAgent能够准确估计边际和双变量概率，并模拟时间决策演化。
...
### [75] [Re-FORC: Adaptive Reward Prediction for Efficient Chain-of-Thought Reasoning](https://arxiv.org/abs/2511.02130)
*Renos Zabounidis,Aditya Golatkar,Michael Kleinman,Alessandro Achille,Wei Xia,Stefano Soatto*

Main category: cs.AI

TL;DR: Re-FORC是一种自适应奖励预测方法，能够根据上下文预测未来奖励期望值，实现推理链的早期停止、模型优化选择和自适应测试时扩展，显著提升计算效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 为了解决推理模型在长推理过程中计算成本高、效率低的问题，需要一种能够预测推理质量并动态调整推理过程的方法。

Method: 训练轻量级适配器在推理模型上，根据思考令牌数量预测未来奖励期望值，实现自适应推理控制。

Result: 计算成本降低26%同时保持准确性；在相同计算量下准确率提高4%，在相同准确率下计算量减少55%；高计算模式下准确率提高11%，低计算模式下提高7%。

Conclusion: Re-FORC通过动态推理长度控制和成本预测，实现了计算效率与模型性能的平衡优化。
...
### [76] [Personalized Decision Modeling: Utility Optimization or Textualized-Symbolic Reasoning](https://arxiv.org/abs/2511.02194)
*Yibo Zhao,Yang Zhao,Hongru Du,Hao Frank Yang*

Main category: cs.AI

TL;DR: 本文提出ATHENA框架，通过结合符号效用发现和语义适应来建模个性化决策，在疫苗选择和出行模式等任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 个体决策模型与群体最优预测存在差距，因为个体决策过程受到数值属性和语言因素的共同影响，需要开发能够整合最优信息的人类中心决策建模方法。

Method: 提出ATHENA框架，包含两个阶段：1）通过LLM增强的符号发现获得群体级符号效用函数；2）基于最优效用指导的个性化语义适应，创建个性化语义模板来建模个人选择。

Result: 在真实世界的出行模式和疫苗选择任务中，ATHENA持续优于基于效用、机器学习和其他LLM的模型，F1分数比最强前沿模型至少提升6.5%。消融研究证实两个阶段都至关重要且互补。

Conclusion: 通过有机整合符号效用建模和语义适应，ATHENA为人类中心决策建模提供了新方案，能够有效捕捉个体决策的独特性。
...
TL;DR: 提出了一个名为\method的框架，用于提升大语言模型在复杂表格数据推理任务中的性能，该框架包含查询分解器、表格清理器和基于程序思维(PoT)的推理器，并在新数据集CalTab151上取得了最先进的性能。
...
Conclusion: 该框架有效提升了大语言模型在复杂表格数值推理任务中的性能，并能与主流大语言模型无缝集成，为复杂表格数值推理提供了稳健的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究仅使用粗粒度的数据集级统计来衡量多模态大语言模型在模态冲突时的行为，忽视了模型在单模态推理中的置信度影响。
...
Method: 首先进行理论分析解释懒惰行为的产生原因，然后引入稳定的因果影响力测量方法，最后提出可验证奖励机制，允许推理智能体丢弃噪声输出、整合指令并在必要时重新开始推理过程。

Result: 大量实验表明，该框架有效缓解了懒惰智能体行为，释放了多智能体框架在复杂推理任务中的全部潜力。

Conclusion: 通过测量因果影响力和实施可验证奖励机制，可以显著改善多智能体推理中的协作效率，避免懒惰行为导致的性能下降。
...
Method: 基于transformer的框架，整合人口统计学、临床和实验室数据，使用量化标记化处理连续实验室值，采用注意力机制提高可解释性，通过掩码语言建模预训练并针对二元分类任务微调。
...
Result: 该系统使用UCI机器学习存储库的数据集进行模型开发和验证，旨在支持医疗专业人员识别高风险患者。

Conclusion: 该专家系统提供了一种非侵入性且易于获取的初步评估方法，有助于确定是否需要进行进一步的诊断程序如活检。
...
TL;DR: 本文提出了一种基于二元分类视角的新框架，用于估计生成模型的完整精确率-召回率(PR)曲线，解决了现有方法只能评估曲线极端值的局限性。
...
Result: 该框架扩展了文献中多个里程碑式的PR指标，这些指标原本只能评估曲线的极端值。实验研究了不同设置下曲线的行为差异。

Conclusion: 提出的框架为生成模型的PR曲线估计提供了新的理论基础和实用工具，能够进行更全面的模型评估分析。
...
TL;DR: 本文提出了一种名为可验证多选重构（VMR）的新训练策略，将开放式任务数据重构为可验证的多选格式，从而在没有明确标准答案的情况下也能有效训练语言模型，在八个开放式基准测试中平均提升了5.99个点。


<details>
  <summary>Details</summary>
Motivation: 现有可验证奖励强化学习（RLVR）方法在数学和编程等有标准答案的领域表现出色，但在缺乏标准答案的开放式任务（如创意写作和指令遵循）中被视为非推理场景，忽略了推理能力的潜在价值。

Method: 提出可验证多选重构（VMR）训练策略，将开放式数据重构为可验证的多选格式，克服了RLVR依赖标准答案的限制，使模型能在开放式任务中有效训练。

Result: 在多个基准测试上的实验验证了该方法的有效性，在八个开放式基准测试中，基于VMR的训练相比基线平均提升了5.99个点。

Conclusion: VMR策略成功地将RLVR范式扩展到开放式领域，证明了强化推理能力可以提升开放式任务的性能，为语言模型在开放域的应用提供了新思路。
...
TL;DR: 本文提出KLPEG框架，通过构建知识图谱来系统建模游戏元素、任务依赖和因果关系，利用LLM解析更新日志并基于知识图谱进行多跳推理，生成针对更新的测试用例，在Overcooked和Minecraft游戏中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现代视频游戏的快速迭代和频繁更新对测试效率和针对性提出了重大挑战。现有的基于大语言模型的自动化游戏测试方法缺乏结构化知识积累机制，难以针对增量游戏更新进行精准高效测试。
...
TL;DR: ORCA基准测试评估大语言模型在多领域真实定量推理任务中的表现，涵盖金融、物理、健康、统计等领域，发现当前最先进模型准确率仅为45-63%，主要错误来自舍入和计算错误。
...
Method: 使用Omni计算器引擎验证的500个自然语言任务，涵盖金融、物理、健康、统计等多个领域，评估五个最先进的大语言模型系统。

Result: 五个最先进系统（ChatGPT-5、Gemini 2.5 Flash、Claude Sonnet 4.5、Grok 4和DeepSeek V3.2）准确率仅为45-63%，主要错误来自舍入（35%）和计算错误（33%）。模型在数学和工程领域表现较好，但在物理和自然科学领域较弱。

Conclusion: 大语言模型在定量推理任务中存在显著局限性，虽然模型之间存在部分互补性而非完全冗余，但在数值精度和跨领域泛化方面仍需改进。
...
<details>
  <summary>Details</summary>
Motivation: 传统静态屏蔽方法假设固定的逻辑规范和手工制作的抽象，在环境假设被违反时无法适应，需要开发能够动态调整的屏蔽框架。

Method: 基于GR(1)规范开发自适应屏蔽框架，运行时检测环境假设违规，使用归纳逻辑编程在线自动修复GR(1)规范，确保屏蔽器演化。

Result: 在Minepump和Atari Seaquest案例研究中，静态符号控制器在优化辅助奖励时通常严重次优，而配备自适应屏蔽的RL智能体保持接近最优的奖励和完美的逻辑合规性。
...
### [87] [A Multi-Agent Psychological Simulation System for Human Behavior Modeling](https://arxiv.org/abs/2511.02606)
...
<details>
  <summary>Details</summary>
Motivation: 人类中心领域的培训和教育需要真实实践，但现有的人类行为模拟方法有限，缺乏透明度和心理学理论基础。

Method: 构建基于心理学理论（如自我效能、思维模式、社会建构主义）的多智能体系统，模拟'内部议会'中代表关键心理因素的智能体之间的审议和互动。
...
### [88] [DecompSR: A dataset for decomposed analyses of compositional multihop spatial reasoning](https://arxiv.org/abs/2511.02627)
*Lachlan McPheat,Navdeep Kaur,Robert Blackwell,Alessandra Russo,Anthony G. Cohn,Pranava Madhyastha*

Main category: cs.AI

TL;DR: DecompSR是一个用于分析组合空间推理能力的大规模基准数据集和生成框架，包含超过500万个数据点，通过程序化构建确保正确性，并系统评估大型语言模型在空间推理任务中的表现。
...
Method: 通过程序化生成方法构建数据集，独立控制组合性的多个维度：生产力（推理深度）、可替换性（实体和语言变异性）、过度泛化（输入顺序、干扰项）和系统性（新语言元素），并使用符号求解器验证数据正确性。

Result: 实验表明，大型语言模型在空间推理任务中难以进行生产性和系统性泛化，但对语言变异性具有更强的鲁棒性。

Conclusion: DecompSR提供了一个可验证正确性的严格基准数据集，能够独立控制组合性的关键维度，为评估LLMs的组合推理能力提供了精细化的测试工具。
...
### [89] [The Collaboration Gap](https://arxiv.org/abs/2511.02687)
*Tim R. Davidson,Adam Fourney,Saleema Amershi,Robert West,Eric Horvitz,Ece Kamar*

Main category: cs.AI

TL;DR: 提出了一个协作迷宫求解基准，评估了32个领先模型在单独、同质和异质配对中的协作能力，发现了显著的'协作差距'，并提出'接力推理'方法改善协作效果。


<details>
  <summary>Details</summary>
Motivation: 随着AI发展，我们将越来越多依赖由独立开发、具有不同信息、权限和工具的智能体组成的系统。这些系统的成功关键在于异构智能体之间的有效协作，但目前缺乏大规模评估此类协作的实证研究。

Method: 设计了一个协作迷宫求解基准框架，该框架能够：(i) 隔离协作能力，(ii) 调节问题复杂度，(iii) 实现可扩展的自动化评分，(iv) 不施加输出格式约束以保持生态合理性。使用该框架评估了32个领先的开源和闭源模型在不同配对配置下的表现。

Result: 发现了显著的'协作差距'：单独表现良好的模型在需要协作时性能大幅下降。协作可能完全崩溃，例如单独能很好解决迷宫的小型蒸馏模型在某些配对中几乎完全失败。发现从较强智能体开始通常能改善结果，这促使了'接力推理'方法的提出。
...
### [90] [Using Span Queries to Optimize for Cache and Attention Locality](https://arxiv.org/abs/2511.02749)
*Paul Castro,Nick Mitchell,Nathan Ordonez,Thomas Parnell,Mudhakar Srivatsa,Antoni Viros i Martin*

Main category: cs.AI

TL;DR: 本文提出span query概念，将推理服务器接口泛化，支持聊天、RAG、推理时缩放和智能体工作负载，通过交换性约束优化KV缓存命中率，在非聊天用例中实现10-20倍的TTFT降低。


<details>
  <summary>Details</summary>
Motivation: 当前推理服务器主要针对聊天完成优化，但客户端已扩展到各种推理时缩放和深度推理技术。现有解决方案通常针对单一用例（如RAG）优化，缺乏通用性。

Method: 引入span query作为推理调用的表达式树，通过交换性约束链接，支持自动优化KV缓存局部性。对vLLM进行少量修改（492行代码）实现高性能span query执行。

Result: span query在两种不同非聊天用例中实现10-20倍的TTFT降低。注意力优化的span query在2b参数模型上表现优于使用8b模型的传统推理服务器。

Conclusion: span query提供了一种通用接口，能够有效支持多种推理工作负载，显著提升性能并解决"迷失在中间"问题。
...
### [91] [LLM-Supported Formal Knowledge Representation for Enhancing Control Engineering Content with an Interactive Semantic Layer](https://arxiv.org/abs/2511.02759)
...
### [92] [Optimizing AI Agent Attacks With Synthetic Data](https://arxiv.org/abs/2511.02823)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI部署变得更加复杂和高风险，准确评估其风险变得至关重要。AI控制是一个评估框架，但良好的控制评估需要强大的攻击策略，这在计算约束导致数据稀缺的复杂智能体环境中具有挑战性。

Method: 将攻击能力分解为五个组成技能（怀疑建模、攻击选择、计划合成、执行和隐蔽性），并分别优化每个组件。为了解决数据稀缺问题，开发了攻击动态的概率模型，在该模拟中优化攻击超参数，然后将结果迁移到SHADE-Arena环境中。

Result: 该方法显著提高了攻击强度，将安全评分从基线0.87降低到使用脚手架后的0.41。

Conclusion: 通过技能分解和概率建模的方法，可以在数据稀缺的复杂环境中有效优化攻击策略，从而更准确地评估AI系统的风险。
...
### [93] [Complexity of counting points on curves and the factor $P_1(T)$ of the zeta function of surfaces](https://arxiv.org/abs/2511.02262)
*Diptajit Roy,Nitin Saxena,Madhavan Venkatesh*

Main category: cs.CC

TL;DR: 本文研究了有限域上曲线和曲面点计数问题的计算复杂性，提出了首个高效的Arthur-Merlin协议来验证曲线点计数、Jacobian群结构和Hasse-Weil zeta函数，并扩展到曲面情况。


<details>
  <summary>Details</summary>
Motivation: 有限域上曲线和曲面点计数问题在计算数论中具有基础重要性，目前尚无亚指数时间算法，且其NP难度尚不明确，需要开发新的计算方法和复杂性分析。

Method: 对于曲线情况，使用Weil和Riemann-Roch界采样哈希函数来验证Jacobian群阶数；对于高维情况，通过Lefschetz铅笔纤维化约化为曲面问题，利用消失循环理论和Deligne的'最大公约数定理'有效版本。

Result: 提出了首个计算曲面zeta函数因子P₁(T)的多项式时间算法（当曲面度固定时），以及一般情况下的量子多项式时间算法，获得了该问题首个非平凡的计算复杂性上界。

Conclusion: 通过将高维问题约化为曲线问题，并利用代数几何工具，本文为有限域上代数簇点计数问题提供了首个有效的计算协议和复杂性分析框架。
...
### [94] [Spectral Certificates and Sum-of-Squares Lower Bounds for Semirandom Hamiltonians](https://arxiv.org/abs/2511.02264)
*Nicholas Kocurek*

Main category: cs.CC

TL;DR: 本文研究了k-XOR问题的量子类比——k-XOR哈密顿量的基态能量计算问题，建立了与经典k-XOR问题的联系，提出了基于Kikuchi矩阵的量子变体谱算法，并证明了在特定条件下的最优性。


<details>
  <summary>Details</summary>
Motivation: 研究经典k-XOR问题的量子类比，探索量子计算在组合优化问题中的应用潜力，建立量子与经典计算复杂性之间的联系。

Method: 开发了Kikuchi矩阵的量子变体用于CSP反驳，设计了时间复杂度为n^O(ℓ)的经典谱算法，通过构造半随机哈密顿量实例和高斯符号k-local泡利算子求和来验证算法性能。

Result: 提出的算法能够在特定条件下（半随机哈密顿量实例或高斯符号k-local泡利算子求和）以n^O(ℓ)时间证明基态能量最多为1/2+ε，达到了所谓的反驳阈值。

Conclusion: 成功建立了k-XOR问题的量子类比模型，开发了有效的经典谱算法，并通过非交换Sum-of-Squares下界证明了在半随机机制下该权衡是紧的。

Abstract: The $k$-$\mathsf{XOR}$ problem is one of the most well-studied problems in
...
### [95] [Relaxed vs. Full Local Decodability with Few Queries: Equivalence and Separations for Linear Codes](https://arxiv.org/abs/2511.02633)
*Elena Grigorescu,Vinayak M. Kumar,Peter Manohar,Geoffrey Mon*

Main category: cs.CC

TL;DR: 本文研究了松弛局部可解码码(RLDC)与标准局部可解码码(LDC)的关系，证明了在3次查询下线性RLDC等价于线性LDC，但在15次查询下两者不等价。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，对于大量查询次数q，RLDC和LDC在码长上存在显著差距，但在q=2时两者等价。本文旨在探索RLDC和LDC开始分离的最小查询次数q。

Method: 通过理论分析证明线性3-查询RLDC必然是3-查询LDC，并构造了线性15-查询RLDC但不是任何常数查询LDC的例子。同时建立了声音错误阈值理论。

Result: 证明了线性RLDC和LDC在3次查询下等价，但在15次查询下不等价。给出了线性15-查询RLDC的具体构造，该构造不是任何常数查询LDC。

Conclusion: 线性RLDC和LDC的等价性在低查询次数下成立，但在较高查询次数下分离。这揭示了RLDC和LDC在结构上的根本差异，并为理解局部可解码码的极限提供了新视角。

Abstract: A locally decodable code (LDC) $C \colon \{0,1\}^k \to \{0,1\}^n$ is an
...
### [96] [Solving cold start in news recommendations: a RippleNet-based system for large scale media outlet](https://arxiv.org/abs/2511.02052)
*Karol Radziszewski,Michał Szpunar,Piotr Ociepka,Mateusz Buczyński*

Main category: cs.IR

TL;DR: 基于RippleNet的可扩展推荐系统在波兰大型在线媒体平台Onet.pl的生产部署，通过集成基于内容的项目嵌入解决冷启动问题，使用Amazon SageMaker进行分布式训练和推理，Apache Airflow编排数据管道。


<details>
  <summary>Details</summary>
Motivation: 解决媒体领域新发布内容的冷启动问题，为大型在线媒体平台提供有效的推荐系统实现。

Method: 将基于内容的项目嵌入集成到RippleNet的知识传播机制中，利用Amazon SageMaker进行分布式训练和推理，Apache Airflow编排工作流，构建包含用户和项目特征的黄金数据集。

Result: 实现了能够有效评分先前未见项目的推荐系统，支持生产环境部署。

Conclusion: 提出的解决方案成功解决了媒体领域的冷启动问题，通过集成内容嵌入和知识传播机制，为大型在线平台提供了可扩展的推荐系统实现。
...
### [97] [Enhancing Multimodal Recommendations with Vision-Language Models and Information-Aware Fusion](https://arxiv.org/abs/2511.02113)
*Hai-Dang Kieu,Min Xu,Thanh Trung Huynh,Dung D. Le*

Main category: cs.IR

TL;DR: VLIF是一个基于视觉语言和信息论的多模态推荐框架，通过VLM视觉增强模块生成细粒度图像描述，并使用信息感知融合模块解耦冗余和协同信号，显著提升推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推荐方法依赖粗糙的视觉特征和不受控制的融合，导致冗余或不对齐的表示，限制了视觉特征的贡献。需要从信息论角度平衡模态间的独特、共享和冗余信息。

Method: 提出VLIF框架：1）VLM视觉增强模块生成细粒度、标题引导的图像描述；2）基于部分信息分解的信息感知融合模块，解耦模态间的冗余和协同信号进行可控融合。

Result: 在三个Amazon数据集上的实验表明，VLIF持续优于最新的多模态基线方法，并显著增强了视觉特征的贡献。

Conclusion: VLIF通过视觉语言增强和信息感知融合，有效提升了多模态推荐的表示质量，证明了细粒度语义对齐和可控信息融合的重要性。
...
### [98] [KGBridge: Knowledge-Guided Prompt Learning for Non-overlapping Cross-Domain Recommendation](https://arxiv.org/abs/2511.02181)
*Yuhan Wang,Qing Xie,Zhifeng Bao,Mengzi Tang,Lin Li,Yongjian Liu*

Main category: cs.IR

TL;DR: 提出了KGBridge框架，一种基于知识图谱的提示学习框架，用于解决非重叠用户场景下的跨域序列推荐问题。该方法通过KG增强的提示编码器和两阶段训练范式，有效解决了KG稀疏性、用户重叠依赖和知识解耦等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识图谱的跨域推荐方法在非重叠用户场景下面临三大挑战：对KG稀疏性和流行度偏差敏感、依赖重叠用户进行域对齐、缺乏对可迁移知识和领域特定知识的显式解耦，这些限制了有效和稳定的知识迁移。

Method: KGBridge框架包含两个核心组件：KG增强的提示编码器（将关系级语义建模为软提示，为用户序列建模提供结构化动态先验）和两阶段训练范式（结合跨域预训练和隐私保护微调，实现无用户重叠的知识迁移）。通过关系感知语义控制和对应驱动的解耦，显式分离和平衡领域共享和领域特定语义。

Result: 在基准数据集上的广泛实验表明，KGBridge始终优于最先进的基线方法，并在不同KG稀疏度下保持鲁棒性，突显了其在缓解KG增强跨域推荐中结构不平衡和语义纠缠方面的有效性。

Conclusion: KGBridge通过知识引导的提示学习框架，成功解决了非重叠用户场景下跨域推荐的关键挑战，实现了更有效和稳定的知识迁移，为KG增强的跨域推荐提供了新的解决方案。
...
### [99] [Average Precision at Cutoff k under Random Rankings: Expectation and Variance](https://arxiv.org/abs/2511.02571)
*Tetiana Manzhos,Tetiana Ianevych,Olga Melnyk*

Main category: cs.IR

TL;DR: 本文推导了AP@k的期望和方差，为MAP@k建立基准线，帮助更可靠地解释推荐系统和信息检索平台的排名质量评估结果。
...
Result: 得到了AP@k的期望和方差表达式，这些结果可以作为MAP@k的基准线，帮助评估排名算法性能是否显著优于随机表现。

Conclusion: AP@k的期望和方差为MAP@k提供了重要的统计基准，使评估结果解释更加可靠，能够区分算法真实性能与随机波动。
...
### [100] [Recursively Enumerably Representable Classes and Computable Versions of the Fundamental Theorem of Statistical Learning](https://arxiv.org/abs/2511.02644)
*David Kattermann,Lothar Sebastian Krapp*

Main category: cs.LG

TL;DR: 该论文研究了可计算概率近似正确学习，探讨了可计算学习器与传统PAC学习理论的关系，特别是有效VC维度与递归可枚举表示类之间的联系。


<details>
  <summary>Details</summary>
Motivation: 传统PAC学习的基本定理在可计算框架下不再成立，需要建立可计算学习理论的新基础，研究有效VC维度与RER类的关系。

Method: 通过分析有效VC维度与递归可枚举表示类的联系，研究各种CPAC学习概念，包括非均匀CPAC学习和不可知学习。

Result: 发现有效VC维度可以取传统维度之上的任意值，但对于满足强CPAC学习概念的类，两个维度重合；证明了CPAC可学习类可以通过包含实现相同样本的RER类来表征。

Conclusion: 建立了可计算学习理论的新框架，证明了RER类在CPAC学习中的核心作用，并为不可知学习提供了保证。
...
### [101] [CudaForge: An Agent Framework with Hardware Feedback for CUDA Kernel Optimization](https://arxiv.org/abs/2511.01884)
...
<details>
  <summary>Details</summary>
Motivation: 手动设计CUDA内核成本高且耗时，现有自动方法生成的核效率低、计算开销大且泛化能力差，需要更高效的自动内核生成方法。

Method: 采用训练免费的多智能体工作流，包含Coder和Judge两个LLM智能体，通过迭代生成、校正和优化CUDA内核，并集成Nsight Compute等硬件性能指标反馈。

Result: 在KernelBench上显著超越OpenAI-o3和Kevin等最先进模型，生成内核正确率达97.6%，平均速度比PyTorch基线提升1.68倍，在多种GPU和基础模型上表现出强泛化能力。

Conclusion: 多智能体、无需训练的工作流能够实现成本效益高、可泛化且高性能的CUDA内核优化，每个优化内核仅需26.5分钟和0.3美元API成本。
...
### [102] [Retrieval-Augmented Multimodal Depression Detection](https://arxiv.org/abs/2511.01892)
*Ruibo Hou,Shiyu Teng,Jiaqing Liu,Shurong Chai,Yinhao Li,Lanfen Lin,Yen-Wei Chen*

Main category: cs.LG

TL;DR: 提出基于检索增强生成(RAG)的多模态抑郁症检测框架，通过从情感数据集中检索相关内容并用大语言模型生成情感提示，提升情绪表征和可解释性，在AVEC 2019数据集上达到SOTA性能。
...
Method: 基于RAG框架，对抑郁症相关文本检索语义相关的情感内容，使用大语言模型生成情感提示作为辅助模态，丰富情绪表征。

Result: 在AVEC 2019数据集上取得最优性能，一致性相关系数(CCC)达到0.593，平均绝对误差(MAE)为3.95，超越了之前的迁移学习和多任务学习基线方法。

Conclusion: 提出的RAG框架有效提升了抑郁症检测的性能和可解释性，通过检索增强的情感提示解决了现有方法的局限性。
...
TL;DR: 本文提出使用特征值熵作为评估分类器性能的新指标，特别适用于类别不平衡的数据集。该指标与常用的评估指标（如灵敏度、特异性、AUC和Gini指数）存在数学关系，并能估计混淆矩阵以解决类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标在类别不平衡数据集上准确性不足，需要开发更准确的评估方法来选择适合的分类方法。

Method: 使用特征值熵作为评估指标，建立了特征值与常用评估指标之间的数学关系，并提出了估计混淆矩阵的方法来处理类别不平衡问题。

Result: 通过多个数据示例验证，所提出的特征值熵评估指标在性能上优于文献中的标准评估指标。

Conclusion: 特征值熵是一个有效的分类器评估指标，特别适用于类别不平衡的情况，能够提供更准确的性能评估。
...
Method: 建立了数学框架，在PyTorch和Qiskit中实现了混合量子经典电路，通过注入量子电路扰动将梯度更新与量子叠加联系起来。

Result: 在合成序列分类和大规模LLM微调任务中，SGD比AdamW收敛更快，获得更低的最终损失。

Conclusion: 尽管结果有前景，但可扩展性和硬件限制阻碍了采用。这项工作为量子计算与深度学习的交叉提供了新见解，提出了利用量子原理控制和增强模型行为的实用途径。
...
### [105] [Neural Green's Functions](https://arxiv.org/abs/2511.01924)
*Seungwoo Yoo,Kyeongmin Yeo,Jisung Hwang,Minhyuk Sung*

Main category: cs.LG

TL;DR: 提出Neural Green's Function，一种用于线性偏微分方程的神经解算子，通过模仿格林函数行为实现跨不规则几何和源/边界函数的优越泛化能力。


<details>
  <summary>Details</summary>
Motivation: 受格林函数启发，旨在解决现有学习型解算子在泛化到未见源或边界函数时的困难，设计对训练中具体函数不敏感的框架以实现鲁棒泛化。

Method: 从表示问题域的体素点云中提取每点特征，预测解算子的分解，随后通过数值积分评估解。

Result: 在MCB数据集机械零件几何的稳态热分析中，优于最先进的神经算子，在五个形状类别上平均误差减少13.9%，比需要计算昂贵网格的数值求解器快350倍。

Conclusion: Neural Green's Function通过模仿格林函数行为，实现了对线性偏微分方程的高效、准确求解，在泛化性和计算效率方面显著优于现有方法。
...
### [106] [DeepContour: A Hybrid Deep Learning Framework for Accelerating Generalized Eigenvalue Problem Solving via Efficient Contour Design](https://arxiv.org/abs/2511.01927)
*Yeqiu Chen,Ziyan Liu,Hong Wang*

Main category: cs.LG

TL;DR: DeepContour是一个混合框架，结合深度学习谱预测器和核密度估计来优化轮廓积分方法中的积分轮廓选择，显著加速大规模广义特征值问题的求解。


<details>
  <summary>Details</summary>
Motivation: 轮廓积分方法在求解大规模广义特征值问题时效率高且可并行化，但其性能严重依赖于积分轮廓的选择。缺乏特征值分布先验知识时，不当的轮廓选择会导致显著的计算开销和数值精度损失。

Method: 提出DeepContour框架：首先使用傅里叶神经算子快速预测给定GEP的谱分布，然后对预测谱应用核密度估计来自动系统地确定合适的积分轮廓，最后用优化后的轮廓指导轮廓积分求解器高效找到所需特征值。
...
### [107] [Deciphering Personalization: Towards Fine-Grained Explainability in Natural Language for Personalized Image Generation Models](https://arxiv.org/abs/2511.01932)
*Haoming Wang,Wei Gao*

Main category: cs.LG

TL;DR: 本文提出了一种名为FineXL的新技术，用于为个性化图像生成模型提供细粒度的自然语言解释，能够识别多个个性化方面及其不同程度。


<details>
  <summary>Details</summary>
Motivation: 现有的个性化模型缺乏可解释性，特别是无法精确识别多个个性化方面及其不同程度，而自然语言解释比视觉特征更易理解。

Method: 开发FineXL技术，通过自然语言描述每个个性化方面，并提供定量分数来指示每个方面的个性化程度。

Result: 实验结果显示，FineXL在不同个性化场景下应用于多种图像生成模型时，可将解释准确性提高56%。

Conclusion: FineXL技术能够有效提升个性化图像生成模型的解释能力，为多个个性化方面提供细粒度的自然语言描述和量化评估。
...
### [108] [Tool Zero: Training Tool-Augmented LLMs via Pure RL from Scratch](https://arxiv.org/abs/2511.01934)
*Yirong Zeng,Xiao Ding,Yutai Hou,Yuxian Wang,Li Du,Juyi Dai,Qiuyang Ding,Duyu Tang,Dandan Tu,Weiwen Liu,Bing Qin,Ting Liu*

Main category: cs.LG

TL;DR: 提出基于纯强化学习的动态泛化引导奖励设计方法，训练Tool-Zero系列模型，在不依赖监督微调的情况下显著提升LLM的工具使用泛化能力


<details>
  <summary>Details</summary>
Motivation: 当前基于监督微调的方法在陌生或复杂工具使用场景中泛化能力不足，需要探索纯强化学习是否能有效激发模型内在推理能力并增强工具无关的泛化性

Method: 设计动态泛化引导的基于规则的强化学习奖励机制，从探索性逐步转向利用性工具使用模式，直接基于基础模型进行强化学习训练

Result: 在相同实验设置下，相比SFT和RL-with-SFT模型获得超过7%的性能提升，在跨数据集和数据集内评估中均表现出一致优势

Conclusion: 纯强化学习方法能有效提升LLM的工具使用泛化能力，Tool-Zero系列模型验证了该方法的有效性和鲁棒性
...
### [109] [Shorter but not Worse: Frugal Reasoning via Easy Samples as Length Regularizers in Math RLVR](https://arxiv.org/abs/2511.01937)
*Abdelaziz Bounhar,Hadi Abdine,Evan Dufraisse,Ahmad Chamma,Amr Mohamed,Dani Bouch,Michalis Vazirgiannis,Guokan Shang*

Main category: cs.LG

TL;DR: 通过保留并适度加权中等难度问题作为隐式长度正则化器，在保持推理准确性的同时显著缩短模型输出长度，实现无显式长度惩罚的简洁性。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR训练流程过滤掉简单问题，导致模型主要在需要长推理链的难题上训练，使模型将"思考更久"与"思考更好"混淆，产生过度冗长的输出。

Method: 保留并适度加权中等难度问题，让模型接触可解决的短链任务，约束输出分布，防止冗长输出。

Result: 在Qwen3-4B-Thinking-2507模型上实现基线AIME25准确率的同时，平均生成解决方案长度缩短近一半。
...
### [110] [Learning a Distance for the Clustering of Patients with Amyotrophic Lateral Sclerosis](https://arxiv.org/abs/2511.01945)
...
<details>
  <summary>Details</summary>
Motivation: ALS是一种严重疾病，患者生存期短且治疗反应差异大，需要个性化护理。但由于队列小、数据稀疏、缺乏明确的临床聚类定义，研究受到限制。现有聚类方法在范围和数量上都有限。

Method: 提出基于疾病进展评分的聚类方法，整合医学专业知识，研究多种距离度量（包括现成距离和弱监督学习方法），并与聚类方法配对，与最先进技术进行基准测试。

Result: 在353名ALS患者数据集上的评估显示，该方法在生存分析方面优于最先进方法，同时获得相当的轮廓分数。学习到的距离增强了结果的相关性和可解释性。

Conclusion: 该方法能够有效识别具有临床意义的ALS患者亚群，为个性化治疗提供支持，同时保持医学专家的可解释性。
...
TL;DR: 提出了一种通用的COFs吸附预测框架COFAP，通过深度学习提取多模态结构化学特征，使用跨模态注意力机制融合特征，无需传统气体相关特征，在hypoCOFs数据集上达到新SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习预测器严重依赖特定气体相关特征，这些特征计算耗时且限制可扩展性，导致效率低下和劳动密集型过程。需要一种更高效的COFs吸附预测方法。

Method: 开发COFAP框架，通过深度学习提取多模态结构和化学特征，使用跨模态注意力机制融合这些互补特征，无需Henry系数或吸附热等传统特征。

Result: COFAP在hypoCOFs数据集上超越了先前方法，达到新的SOTA性能。发现高性能分离COFs集中在狭窄的孔径和表面积范围内。开发了权重可调优先排序方案。

Conclusion: COFAP具有卓越的效率和准确性，可直接应用于晶体多孔材料，为研究人员提供了灵活的应用特定候选COFs排序能力。
...
<details>
  <summary>Details</summary>
Motivation: 脑电图和局部场电位记录中存在大量伪影和噪声，需要自动化预处理方法来解决人工干预带来的可重复性问题和偏见。

Method: 五步预处理流程：包括带通滤波和线噪声滤波、坏通道剔除，以及基于聚类算法的自动成分剔除的高效独立成分分析，使用机器学习分类器确保任务相关信息保留。

Result: NeuroClean成功去除了多种常见伪影，在复杂程度不同的运动任务分类中，优化后的多项逻辑回归模型准确率从原始数据的74%提升至97%以上（随机水平为33.3%）。
...
### [113] [TapOut: A Bandit-Based Approach to Dynamic Speculative Decoding](https://arxiv.org/abs/2511.02017)
*Aditya Sridhar,Nish Sinnadurai,Sean Lie,Vithursan Thangarasa*

Main category: cs.LG

TL;DR: TapOut是一种基于多臂老虎机的动态推测解码算法，无需训练和超参数调优，能智能决定推测生成的最佳token数量，在多种模型和数据集上实现竞争性或更优的加速效果。
...
Method: 提出TapOut算法，使用多臂老虎机在线选择无参数的动态推测策略，基于历史奖励和探索情况，无需训练和超参数调优即可实现即插即用。

Result: 在多样化模型对和数据集上的广泛实验表明，TapOut相比成熟的动态推测基线方法，无需任何超参数调优即可实现竞争性或更优的加速效果。

Conclusion: TapOut提供了一种有效且通用的动态推测解码解决方案，消除了手动阈值调整的需求，在保持性能的同时显著简化了部署过程。
...
### [114] [Shared Parameter Subspaces and Cross-Task Linearity in Emergently Misaligned Behavior](https://arxiv.org/abs/2511.02022)
*Daniel Aarao Reis Arturi,Eric Zhang,Andrew Ansah,Kevin Zhu,Ashwinee Panda,Aishwarya Balwani*

Main category: cs.LG

TL;DR: 研究发现大型语言模型在微调于狭窄有害数据集后会出现广泛的对齐偏差现象，这种现象具有跨任务的线性结构特征，表现为参数更新方向的高度相似性和共享的低维子空间。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型在微调后产生广泛对齐偏差现象的根本机制，特别是这种有害行为在不同领域间泛化的原理。

Method: 采用几何视角研究对齐偏差现象，分析微调权重更新的余弦相似度、主角度、投影重叠等几何特征，并通过线性模式连接性验证功能等价性。

Result: 发现不同狭窄对齐偏差任务的参数更新方向高度收敛，具有较高的余弦相似度和共享的低维子空间，插值模型保持一致的广泛对齐偏差行为。

Conclusion: 对齐偏差现象源于不同狭窄任务发现了相同的共享参数方向，表明有害行为可能被组织到权重景观的特定可预测区域中，揭示了参数几何与行为结果之间的基本联系。
...
### [115] [Quantum-Enhanced Generative Models for Rare Event Prediction](https://arxiv.org/abs/2511.02042)
*M. Z. Haider,M. U. Ghouri,Tayyaba Noreen,M. Salman*

Main category: cs.LG

TL;DR: 提出量子增强生成模型(QEGM)，一种混合经典-量子框架，用于解决罕见事件建模难题，在金融崩溃、极端气候等场景中表现优于传统方法。
...
Method: 结合深度潜变量模型和变分量子电路，采用混合损失函数（重构保真度和尾部感知似然）和量子随机性驱动的噪声注入，通过混合训练循环优化参数。

Result: 在合成高斯混合和真实数据集（金融、气候、蛋白质结构）上测试，QEGM相比GAN、VAE、Diffusion等基线方法，尾部KL散度降低达50%，罕见事件召回率和覆盖校准均有提升。
...
### [116] [Flashlight: PyTorch Compiler Extensions to Accelerate Attention Variants](https://arxiv.org/abs/2511.02043)
...
<details>
  <summary>Details</summary>
Motivation: 现有注意力实现方法通常需要专门的内核或手动调优实现，支持不同注意力变体效率低下。FlexAttention虽然通过静态编程模板支持部分注意力变体，但仍存在局限性。

Method: 利用PyTorch的编译工作流程透明地融合和分块注意力计算，支持任意注意力模式，包括数据依赖的注意力公式化，超越了FlexAttention的能力范围。

Result: Flashlight生成的内核性能与FlexAttention相当或更优，同时提供原生PyTorch代码的灵活性，使开发者能够快速探索新的注意力模型而不牺牲性能。

Conclusion: Flashlight填补了高效支持多样化注意力变体的技术空白，为研究人员和开发者提供了更灵活、高效的注意力实现方案。
...
### [117] [Regularization Through Reasoning: Systematic Improvements in Language Model Classification via Explanation-Enhanced Fine-Tuning](https://arxiv.org/abs/2511.02044)
*Vivswan Shah,Randy Cogill,Hanwei Yue,Gopinath Chennupati,Rinat Khaziev*

Main category: cs.LG

TL;DR: 在LLM分类任务中，为标签附加简短解释进行微调比仅使用标签效果更好。有趣的是，即使使用语法不连贯但词汇对齐的伪解释，也能显著提升性能，这表明收益主要来自结构而非语义。


<details>
  <summary>Details</summary>
Motivation: 研究在LLM微调过程中，为标签附加解释是否能提升模型性能，特别是探索这种提升是否依赖于解释的语义内容。

Method: 使用多个LLM生成的集成数据，对7B参数模型进行微调，在六个对话数据集上评估。比较标签加解释训练与仅标签基线的表现，并测试用随机但词汇对齐的伪解释替代真实解释的效果。

Result: 在18个数据集和任务设置中，标签加解释训练始终优于仅标签基线。伪解释虽然缺乏语义，但仍能提升准确性，缩小与真实解释的差距。模型内部分析显示，解释增强的模型在中间层具有更高的激活熵，输出层预测质量更集中。

Conclusion: 解释增强的微调（无论是真实解释还是伪解释）能提高LLM分类的准确性和可靠性，收益主要来自额外的token预算促进更丰富的中间计算，起到正则化作用，减少过度自信的捷径学习。
...
### [118] [A Dual-Use Framework for Clinical Gait Analysis: Attention-Based Sensor Optimization and Automated Dataset Auditing](https://arxiv.org/abs/2511.02047)
*Hamidreza Sadeghsalehi*

Main category: cs.LG

TL;DR: 提出了一种基于多流注意力机制的深度学习框架，既能优化传感器配置，又能自动审计数据集完整性。该框架在步态分析数据集中发现了严重的侧向性偏差问题。


<details>
  <summary>Details</summary>
Motivation: 可穿戴传感器和AI在客观步态分析中至关重要，但模型容易受到隐藏数据集偏差的影响，且特定任务的传感器优化仍然是一个挑战。需要开发既能优化传感器配置又能审计数据完整性的方法。

Method: 使用多流注意力机制的深度学习框架，应用于Voisard等人的多队列步态数据集，分析四个临床任务（PD、OA、CVA筛查；PD与CVA鉴别诊断）。

Result: 模型注意力机制定量发现了严重的数据集混淆问题。对于OA和CVA筛查任务，模型将超过70%的注意力分配给右脚，而统计上忽略了左脚（注意力小于0.1%），这反映了公共数据集中存在的严重侧向性偏差。

Conclusion: 该工作的主要贡献是方法学上的，证明了可解释框架能够自动审计数据集完整性。作为次要发现，模型提出了新颖的数据驱动传感器协同作用，为未来优化协议提供了假设。
...
TL;DR: 开发一个用于最大化二元变量实值函数的求解器，该求解器基于估计器算法预测目标函数的最优值，训练时不依赖已解决的实例或策略值计算。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要计算策略值或依赖已解决的实例，这在大规模问题中计算成本高昂。本文旨在开发一种不依赖这些要素的高效求解器。

Method: 使用估计器算法预测目标函数的最优值，训练基于一个不等式，允许使用期望总偏差作为损失函数，而非直接使用目标函数本身。

Result: 提出的求解器能够有效估计二元变量实值函数的最大化问题的最优解，无需计算策略值或依赖已解决的实例。

Conclusion: 该方法提供了一种高效且计算成本较低的求解二元变量最大化问题的新途径，避免了传统方法的高计算需求。
...
### [120] [LLM Probing with Contrastive Eigenproblems: Improving Understanding and Applicability of CCS](https://arxiv.org/abs/2511.02089)
*Stefan F. Schouten,Peter Bloem*

Main category: cs.LG

TL;DR: 本文重新审视对比一致性搜索(CCS)方法，将其重新表述为特征值问题，提出了具有可解释特征值的闭式解，并扩展到多变量情况。


<details>
  <summary>Details</summary>
Motivation: CCS是一种无监督探测方法，用于测试大语言模型是否在其内部激活中表示二元特征（如句子真值）。虽然CCS显示出潜力，但其双项目标函数仅被部分理解，需要澄清其机制并扩展其适用性。

Method: 认为应该优化相对对比一致性，基于这一见解将CCS重新表述为特征值问题，得到具有可解释特征值的闭式解，并自然扩展到多变量情况。

Result: 在多个数据集上评估这些方法，发现它们恢复了与CCS相似的性能，同时避免了随机初始化敏感性问题。
...
TL;DR: 提出了一种将物理信息直接融入损失函数的框架，用于分子和自旋系统的预测和生成建模任务，通过基于能量的损失函数实现物理约束。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要在架构层面融入物理洞察，需要更直接地将物理信息整合到机器学习中，特别是在科学领域的应用。

Method: 假设每个数据样本相对于近似能量景观处于热平衡状态，使用反向KL散度和玻尔兹曼分布推导出能量损失函数，将损失表示为数据与模型预测之间的能量差。

Result: 在分子生成和自旋基态预测任务中，相比基线方法取得了显著改进，证明了该方法的有效性。

Conclusion: 提出的能量损失函数框架能够产生物理上有意义的损失函数，其梯度与有效配置更好对齐，同时保持架构无关性和计算效率，并能固有地尊重物理对称性。
...
### [122] [Uncertainty Guided Online Ensemble for Non-stationary Data Streams in Fusion Science](https://arxiv.org/abs/2511.02092)
*Kishansingh Rajput,Malachi Schram,Brian Sammuli,Sen Lin*

Main category: cs.LG

TL;DR: 该论文提出了一种在线学习方法，用于应对聚变装置中非平稳数据流的分布漂移问题，通过不确定性引导的在线集成方法显著提高了托里奥场线圈偏转预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 聚变数据具有非平稳特性，存在分布漂移问题，而传统机器学习模型假设数据分布稳定，在遇到非平稳数据流时性能会下降。在线学习在其他领域已有应用，但在聚变应用领域尚未充分探索。

Method: 采用在线学习技术持续适应漂移数据流，并提出不确定性引导的在线集成方法。利用深度高斯过程近似（DGPA）进行校准的不确定性估计，然后使用不确定性值指导元算法，该算法基于在不同历史数据范围上训练的多个学习器生成预测。

Result: 在线学习相比静态模型将误差降低了80%。在线集成和不确定性引导的在线集成分别比标准单模型在线学习进一步降低了约6%和10%的预测误差。DGPA还为决策者提供了预测的不确定性估计。

Conclusion: 在线学习对于维持机器学习模型在非平稳聚变数据流中的性能至关重要，提出的不确定性引导在线集成方法能够进一步改善性能，为聚变装置的操作提供更可靠的预测和不确定性信息。
...
### [123] [Geometric Data Valuation via Leverage Scores](https://arxiv.org/abs/2511.02100)
...
Method: 采用统计杠杆得分作为几何替代方法，通过衡量数据点在表示空间中的结构影响力来评估其重要性，并将其扩展到岭杠杆得分以获得严格正的边际增益。

Result: 提出的杠杆得分满足Shapley估值的虚拟性、效率和对称性公理，且在主动学习实验中，岭杠杆采样优于标准基线方法。

Conclusion: 杠杆得分提供了一种计算高效的数据估值方法，与下游决策质量建立了严格联系，为数据集管理和模型训练提供了实用工具。
...
### [124] [Measuring the Intrinsic Dimension of Earth Representations](https://arxiv.org/abs/2511.02101)
*Arjun Rao,Marc Rußwurm,Konstantin Klemmer,Esther Rolf*

Main category: cs.LG

TL;DR: 本文首次研究了地理隐式神经表示(INRs)的内在维度，发现其维度在2到10之间，与下游任务性能相关，可作为无监督评估指标。
...
Method: 通过分析内在维度来测量地理INRs的信息含量，研究空间分辨率和输入模态变化对内在维度的影响。

Result: 发现地理INRs的内在维度在2到10之间，与下游任务性能相关，并能捕捉空间伪影，便于模型评估。
...
### [125] [Matrix Sensing with Kernel Optimal Loss: Robustness and Optimization Landscape](https://arxiv.org/abs/2511.02122)
*Xinyuan Song,Jiaye Teng,Ziye Ma*

Main category: cs.LG

TL;DR: 该论文研究了非凸优化问题中损失函数选择对鲁棒性和优化景观的影响，通过噪声矩阵感知任务，提出了一种基于非参数回归的鲁棒损失函数，能在高斯和非高斯噪声下保持稳定。


<details>
  <summary>Details</summary>
Motivation: 传统回归任务中均方误差损失在非高斯或重尾噪声下不可靠，需要开发更鲁棒的损失函数来应对各种噪声分布。

Method: 采用基于非参数回归的鲁棒损失，使用核密度估计残差密度并最大化估计的对数似然，该损失在高斯误差下与MSE一致但在更一般设置下保持稳定。

Result: 理论和实证分析表明，新损失函数在处理大噪声和不同噪声分布时表现优异，通过分析RIP常数上界来研究优化景观的重塑。
...
### [126] [Variance-Aware Feel-Good Thompson Sampling for Contextual Bandits](https://arxiv.org/abs/2511.02123)
*Xuheng Li,Quanquan Gu*

Main category: cs.LG

TL;DR: 本文提出了FGTSVA，一种针对上下文多臂老虎机问题的方差感知Thompson Sampling算法，具有最优的遗憾边界。该算法适用于一般奖励函数，在上下文线性老虎机设置下与基于UCB的算法性能相当。


<details>
  <summary>Details</summary>
Motivation: 当前大多数方差依赖的遗憾边界研究都集中在基于UCB的老虎机算法上，而基于采样的算法如Thompson Sampling研究较少。现有的LinVDTS算法仅限于线性奖励函数且遗憾边界在模型维度上不是最优的。

Method: 提出了FGTSVA算法，核心分析技术是扩展了Feel-good Thompson sampling中常用的解耦系数，用新的解耦系数dc来反映模型空间的复杂性。

Result: FGTSVA实现了遗憾边界为$\tilde{O}(\sqrt{\mathrm{dc}\cdot\log|\mathcal{F}|\sum_{t=1}^T\sigma_t^2}+\mathrm{dc})$，其中$|\mathcal{F}|$是模型空间大小，$T$是总轮数，$\sigma_t^2$是第t轮的噪声方差。

Conclusion: FGTSVA是第一个针对一般奖励函数的方差感知Thompson Sampling算法，具有最优遗憾边界，在上下文线性老虎机设置下与UCB-based算法性能相当。
...
### [127] [Disentangling Causal Substructures for Interpretable and Generalizable Drug Synergy Prediction](https://arxiv.org/abs/2511.02146)
...
Result: 大量实验表明，该方法在冷启动和分布外设置下优于基线模型，并能有效识别药物协同作用的关键子结构。

Conclusion: CausalDDS作为预测药物协同作用和促进药物发现的实用工具具有巨大潜力，为分子水平的药物组合机制提供了清晰见解。
...
### [128] [CFL: On the Use of Characteristic Function Loss for Domain Alignment in Machine Learning](https://arxiv.org/abs/2511.02148)
*Abdullah Almansour,Ozan Tonguz*

Main category: cs.LG

TL;DR: 本文提出使用特征函数作为频域方法来测量高维空间中的分布偏移，作为传统统计方法的替代方案。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在现实世界中部署时经常因分布偏移问题而表现不佳，这在高风险应用中可能导致灾难性后果。传统方法使用统计技术来量化分布偏移，但本文探索了频域方法的替代方案。

Method: 使用特征函数作为频域方法来测量高维空间中的分布偏移，并用于领域自适应。

Result: 特征函数被证明是测量分布偏移的强大替代方法，特别适用于高维空间和领域自适应任务。

Conclusion: 特征函数作为频域方法为分布偏移测量提供了有效的替代方案，能够处理高维空间中的分布变化问题。
...
### [129] [ProtoTSNet: Interpretable Multivariate Time Series Classification With Prototypical Parts](https://arxiv.org/abs/2511.02152)
*Bartłomiej Małkus,Szymon Bobek,Grzegorz J. Nalepa*

Main category: cs.LG

TL;DR: ProtoTSNet是一种用于多变量时间序列数据可解释分类的新方法，通过改进ProtoPNet架构来捕获动态模式和处理不同特征重要性，在30个UEA数据集上实现了可解释方法中的最佳性能。
...
Method: 使用改进的卷积编码器，采用分组卷积，可作为自编码器的一部分进行预训练，旨在保留和量化特征重要性，专门针对时间序列分析的独特挑战。

Result: 在UEA档案的30个多变量时间序列数据集上评估，与现有可解释方法和非可解释基线相比，该方法在可解释方法中表现最佳，同时与非可解释和后处理可解释方法保持竞争力。

Conclusion: ProtoTSNet为领域专家提供了可访问的可解释结果，在保持与非可解释方法竞争力的同时，实现了可解释方法中的最优性能。
...
### [130] [Can LLMs subtract numbers?](https://arxiv.org/abs/2511.02795)
...
TL;DR: 该论文系统研究了大语言模型在减法运算中的表现，发现减法准确率远低于加法，主要问题出现在a<b时模型能计算正确数值但遗漏负号。研究表明指令调优能显著改善这一问题。


<details>
  <summary>Details</summary>
Motivation: 虽然现有基准测试主要关注加法和乘法，但减法作为非交换运算具有结构独特性，却很少受到关注。本研究旨在系统评估LLMs在减法运算中的表现和局限性。

Method: 评估了8个预训练LLMs在加法和减法问题上的表现，通过探测分析研究模型内部表征，并测试了少样本学习和指令调优等技术的改进效果。

Result: 减法准确率显著低于加法，错误主要集中在a<b的情况下，模型能计算正确数值但遗漏负号。指令调优模型在生成负号方面达到近乎完美的准确率。

Conclusion: 研究揭示了LLMs在减法运算中的局限性，特别是负号生成问题，但通过指令调优可以有效恢复模型的算术能力，为理解LLMs数学能力提供了更清晰的表征。
...
### [131] [OmniField: Conditioned Neural Fields for Robust Multimodal Spatiotemporal Learning](https://arxiv.org/abs/2511.02205)
*Kevin Valencia,Thilina Balasooriya,Xihaier Luo,Shinjae Yoo,David Keetae Park*

Main category: cs.LG

TL;DR: OmniField是一个多模态时空学习框架，能够处理稀疏、不规则、有噪声的多模态数据，通过跨模态上下文融合实现统一的重建、插值、预测和跨模态预测。


<details>
  <summary>Details</summary>
Motivation: 解决现实世界实验数据中多模态时空学习的两个挑战：模态内测量稀疏、不规则、有噪声但跨模态相关；可用模态在时空上变化，需要模型能够适应任意子集。

Method: 提出OmniField框架，学习以可用模态为条件的连续神经场，通过多模态串扰块架构和迭代跨模态精炼在解码器前对齐信号。

Result: 广泛评估显示OmniField持续优于八个强大的多模态时空基线方法，在重度模拟传感器噪声下性能仍接近清洁输入水平。

Conclusion: OmniField展示了处理稀疏、噪声多模态数据的鲁棒性，无需网格化或替代预处理即可实现统一的多模态时空学习任务。
...
### [132] [Learning Interactive World Model for Object-Centric Reinforcement Learning](https://arxiv.org/abs/2511.02225)
...
### [133] [Opportunistic Expert Activation: Batch-Aware Expert Routing for Faster Decode Without Retraining](https://arxiv.org/abs/2511.02237)
...
<details>
  <summary>Details</summary>
Motivation: MoE模型在自回归生成时经常进入内存受限状态，因为平均专家负载增长比等效的密集前馈层更慢，导致MoE延迟由激活专家数量决定。
...
### [134] [Probabilistic Graph Cuts](https://arxiv.org/abs/2511.02272)
*Ayoub Ghriss*

Main category: cs.LG

TL;DR: 提出了一个统一的概率框架，用于可微图分割，覆盖了包括归一化割在内的多种割方法，通过积分表示和高斯超几何函数提供紧致的分析上界，支持端到端和在线学习。


<details>
  <summary>Details</summary>
Motivation: 现有的概率图割方法主要集中于RatioCut，缺乏通用保证和原则性梯度，无法覆盖更广泛的割方法如归一化割，限制了在聚类和对比学习中的应用。

Method: 开发了一个统一的概率框架，使用积分表示和高斯超几何函数来推导紧致的期望离散割上界，提供闭式的前向和后向计算。

Result: 该框架为可微图分割提供了严谨、数值稳定的基础，能够处理多种聚类和对比学习目标，无需特征分解。

Conclusion: 提出的概率框架为可扩展、可微的图分割建立了理论基础，支持端到端学习，覆盖了广泛的割方法。
...
### [135] [Gradient-Variation Online Adaptivity for Accelerated Optimization with Hölder Smoothness](https://arxiv.org/abs/2511.02276)
*Yuheng Zhao,Yu-Hu Yan,Kfir Yehuda Levy,Peng Zhao*

Main category: cs.LG

TL;DR: 该论文研究了Hölder光滑函数的在线学习及其对离线优化的影响，设计了自适应梯度变分在线学习算法，并通过在线到批处理转换获得了Hölder光滑性下随机凸优化的通用最优方法。


<details>
  <summary>Details</summary>
Motivation: 光滑性在离线优化加速和在线学习的梯度变分遗憾最小化中至关重要，但这两类问题密切相关。论文旨在研究更广泛的Hölder光滑函数类，探索其对离线优化的意义。

Method: 针对(强)凸在线函数，设计了相应的梯度变分在线学习算法，该算法无需事先知道Hölder光滑参数，具有强自适应性。通过在线到批处理转换获得通用方法，并结合检测式猜测检查程序解决强凸离线优化的通用性问题。

Result: 提出的算法在光滑和非光滑机制之间平滑插值最优保证，实现了随机凸优化在Hölder光滑性下的通用最优方法，并首次获得了在光滑机制中实现加速收敛、在非光滑机制中保持近最优收敛的通用离线方法。

Conclusion: 通过将在线自适应性与检测式猜测检查程序相结合，成功解决了强凸离线优化的通用性问题，为Hölder光滑函数类提供了统一的优化框架。
...
### [136] [Reinforcement learning based data assimilation for unknown state model](https://arxiv.org/abs/2511.02286)
...
<details>
  <summary>Details</summary>
Motivation: 传统数据同化方法在系统动态方程未知时面临挑战，且实际中难以获得无噪声的真实状态序列用于监督学习。
...
### [137] [Federated Quantum Kernel Learning for Anomaly Detection in Multivariate IoT Time-Series](https://arxiv.org/abs/2511.02301)
*Kuan-Cheng Chen,Samuel Yen-Chi Chen,Chen-Yu Liu,Kin K. Leung*

Main category: cs.LG

TL;DR: 提出了一种联邦量子核学习框架，将量子特征映射与联邦聚合相结合，用于分布式、保护隐私的工业物联网异常检测。


<details>
  <summary>Details</summary>
Motivation: 工业物联网系统快速发展带来了高维多变量时间序列异常检测的新挑战，传统联邦学习方法在处理高度非线性决策边界和不平衡异常分布方面存在困难。

Method: 采用联邦量子核学习框架，量子边缘节点使用参数化量子电路本地计算压缩核统计量，仅与中央服务器共享这些摘要信息，服务器构建全局Gram矩阵并训练决策函数。

Result: 在合成工业物联网基准测试中，FQKL在捕捉复杂时间相关性方面表现出比经典联邦基线更好的泛化能力，同时显著降低了通信开销。
...
### [138] [FP8-Flow-MoE: A Casting-Free FP8 Recipe without Double Quantization Error](https://arxiv.org/abs/2511.02302)
*Fengjuan Wang,Zhiyi Su,Xingzhu Hu,Cheng Wang,Mou Sun*

Main category: cs.LG

TL;DR: FP8-Flow-MoE是一种针对大型混合专家模型的FP8训练方案，通过量化一致的数据流和融合算子消除冗余转换，显著提升训练效率和内存使用。


<details>
  <summary>Details</summary>
Motivation: 现有FP8训练方案仍依赖BF16数据流，频繁的量化-反量化转换严重侵蚀FP8的理论效率优势，而完全使用FP8又会导致双重量化误差问题。

Method: 提出量化一致的FP8中心数据流，包含缩放感知转置和融合FP8算子，将显式转换操作从12次减少到2次。

Result: 在671B参数MoE模型上测试，相比BF16和朴素FP8基线，吞吐量提升21%，每GPU内存使用减少16.5GB，同时保持稳定收敛。
...
### [139] [The Sequential Edge: Inverse-Entropy Voting Beats Parallel Self-Consistency at Matched Compute](https://arxiv.org/abs/2511.02309)
*Aman Sharma,Paras Chopra*

Main category: cs.LG

TL;DR: 本文重新审视语言模型推理的测试时扩展策略，发现在同等计算资源下，顺序扩展（链式迭代优化）优于并行自一致性范式，在95.6%的配置中表现更好，准确率提升最高达46.7%。


<details>
  <summary>Details</summary>
Motivation: 挑战当前主流的并行自一致性推理范式，探索在同等计算预算下，顺序扩展策略是否比并行策略更有效。

Method: 使用5个先进开源模型和3个具有挑战性的推理基准进行综合评估，比较并行自一致性与顺序扩展策略，并引入基于逆熵加权投票的新方法。

Result: 顺序扩展在95.6%的配置中优于并行策略，准确率提升最高达46.7%；逆熵加权投票进一步提升了顺序扩展的准确率。

Conclusion: 顺序细化应成为现代LLM推理的默认测试时扩展策略，这挑战了自Wang等人以来主导测试时扩展的并行推理正统观念。
...
### [140] [Large-scale automatic carbon ion treatment planning for head and neck cancers via parallel multi-agent reinforcement learning](https://arxiv.org/abs/2511.02314)
...
TL;DR: 提出了一种可扩展的多智能体强化学习（MARL）框架，用于并行调谐头颈部癌症碳离子治疗中的45个治疗计划参数，显著提高了器官保护效果。


<details>
  <summary>Details</summary>
Motivation: 头颈部癌症治疗计划制定困难，传统碳离子治疗参数调谐过程繁琐且依赖经验，现有深度学习和强化学习方法在数据偏差、计划可行性和高维参数空间探索方面存在局限。

Method: 采用集中训练分散执行（CTDE）QMIX架构，结合Double DQN、Dueling DQN和循环编码（DRQN），使用紧凑历史DVH向量作为状态输入，线性动作-值变换映射离散动作到参数调整，设计临床导向的分段奖励函数，并通过多进程系统与PHOENIX治疗计划系统并行交互。

Result: 在头颈部数据集（10训练，10测试）上同时调谐45个参数，生成的治疗计划与专家手动计划相当或更优（相对计划得分：RL 85.93±7.85% vs 手动 85.02±6.92%），五个器官有显著改善（p值<0.05）。

Conclusion: 该框架能有效探索高维治疗计划参数空间，通过与治疗计划系统直接交互生成具有临床竞争力的碳离子治疗计划，显著改善了器官保护效果。
...
TL;DR: RoME是一个领域鲁棒的专家混合框架，通过动态路由和分布鲁棒优化策略，提升混合整数线性规划求解器在不同领域的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基于学习的MILP求解方法大多在单一领域开发和评估，难以泛化到未见过的领域分布，这限制了构建可扩展通用求解器的能力。

Method: RoME框架使用学习到的任务嵌入动态路由问题实例到专门专家，采用两级分布鲁棒优化策略：域间优化缓解全局分布偏移，域内优化通过对任务嵌入引入扰动增强局部鲁棒性。

Result: 在三个领域训练的单一RoME模型在五个不同领域评估时平均提升67.7%，在MIPLIB的零样本测试中，在现有方法难以泛化的实际挑战性实例上表现出可衡量的性能提升。

Conclusion: 跨领域训练不仅增强了模型对未见领域的泛化能力，还通过鼓励模型捕捉更通用的内在组合模式，提高了每个单独领域的性能。
...
### [142] [Reducing normalizing flow complexity for MCMC preconditioning](https://arxiv.org/abs/2511.02345)
*David Nabergoj,Erik Štrumbelj*

Main category: cs.LG

TL;DR: 提出一种因子化预调节架构，将线性组件与条件归一化流结合，降低NF复杂度并提高对目标几何的适应性。


<details>
  <summary>Details</summary>
Motivation: 现有非线性预调节器使用过参数化的归一化流可能降低采样效率和拟合质量，且现有方法未能根据目标分布自适应调整架构。

Method: 结合线性预调节器和条件归一化流的因子化架构，线性部分处理近似高斯维度，条件NF处理复杂维度。

Result: 在两个复杂合成分布上获得显著更好的尾部样本，在稀疏逻辑回归后验上表现一致更好，在弱似然强漏斗几何的层次贝叶斯模型后验上获得更高有效样本量。
...
### [143] [Human-Machine Ritual: Synergic Performance through Real-Time Motion Recognition](https://arxiv.org/abs/2511.02351)
...
Method: 使用可穿戴IMU传感器收集数据，采用MiniRocket时间序列分类算法，结合多媒体控制实现实时响应。

Result: 系统可靠地支持高精度分类（延迟小于50毫秒），为舞蹈智能机器融入创意、教育和现场表演环境提供了可复制的框架。

Conclusion: 该人本设计方法成功实现了低延迟高精度的人机协同表演，为舞蹈与技术的融合提供了有效解决方案。
...
TL;DR: 本文提出了一种基于约束优化的机器遗忘方法，通过引入可行更新的概念，在保持模型效用的同时实现遗忘目标。该方法基于参数掩码和梯度噪声估计，可作为一个附加组件集成到任何一阶近似遗忘方法中。


<details>
  <summary>Details</summary>
Motivation: 解决机器遗忘问题，统一现有的一阶近似遗忘方法，在实现遗忘的同时不降低初始模型的效用。

Method: 将机器遗忘问题形式化为约束优化问题，引入可行更新的概念，基于参数掩码技术选择值得更新的参数，并考虑梯度估计噪声以提供统计保证。

Result: 在计算机视觉分类器上的实验验证了该方法的有效性。

Conclusion: 提出的可行更新方法可以作为一个可插拔的附加组件，有效提升现有一阶近似遗忘方法的性能。
...
### [145] [Accounting for Underspecification in Statistical Claims of Model Superiority](https://arxiv.org/abs/2511.02453)
*Thomas Sanchez,Pedro M. Gordaliza,Meritxell Bach Cuadra*

Main category: cs.LG

TL;DR: 本研究扩展了统计框架，将模型训练中的不确定性（underspecification）作为额外方差分量纳入考虑，发现即使很小的随机种子变异性也会显著增加支持性能优越性声明所需的证据量。


<details>
  <summary>Details</summary>
Motivation: 机器学习方法在医学影像中应用日益广泛，但许多报道的性能改进缺乏统计稳健性。现有分析未考虑模型训练中的不确定性——即验证分数相似的模型可能因随机初始化或训练动态而在未见数据上表现不同。

Method: 扩展了现有的统计框架，将模型训练不确定性作为额外方差分量建模，通过模拟实验分析随机种子变异性对性能优越性声明的影响。

Result: 模拟实验表明，即使很小的随机种子变异性（约1%）也会显著增加支持性能优越性声明所需的证据量。
...
### [146] [SKGE: Spherical Knowledge Graph Embedding with Geometric Regularization](https://arxiv.org/abs/2511.02460)
*Xuan-Truong Quan,Xuan-Son Quan,Duc Do Minh,Vinh Nguyen Van*

Main category: cs.LG

TL;DR: SKGE是一种在超球面上进行知识图谱嵌入的模型，通过球面几何约束显著优于传统的欧几里得方法，特别是在大规模数据集上表现突出。


<details>
  <summary>Details</summary>
Motivation: 传统的欧几里得知识图谱嵌入方法存在建模复杂关系的固有局限性，可能导致训练效率低下。作者提出使用紧凑流形（超球面）来约束实体表示，以克服这些限制。

Method: SKGE使用可学习的非线性球面化层将实体映射到球面上，并将关系解释为平移-投影混合变换。球面几何约束作为强大的正则化器，创造固有困难负采样环境。

Result: 在FB15k-237、CoDEx-S和CoDEx-M三个基准数据集上的实验表明，SKGE始终显著优于其欧几里得对应模型TransE，特别是在大规模基准测试中表现优异。

Conclusion: 流形的选择不是实现细节，而是基本设计原则。几何先验应成为设计下一代强大稳定KGE模型的基石，球面几何约束能够学习更鲁棒和语义连贯的表示。
...
### [147] [NOWS: Neural Operator Warm Starts for Accelerating Iterative Solvers](https://arxiv.org/abs/2511.02481)
*Mohammad Sadegh Eshaghi,Cosmin Anitescu,Navid Valizadeh,Yizheng Wang,Xiaoying Zhuang,Timon Rabczuk*

Main category: cs.LG

TL;DR: NOWS是一种混合策略，利用学习到的解算子为Krylov方法提供高质量初始猜测，从而加速经典迭代求解器，可将计算时间减少高达90%，同时保持底层数值算法的稳定性和收敛保证。
...
Method: 引入神经算子热启动(NOWS)策略，利用学习到的解算子为共轭梯度和GMRES等Krylov方法生成高质量初始猜测，与现有离散化和求解器基础设施无缝集成。

Result: 在基准测试中，学习到的初始化持续减少迭代次数和端到端运行时间，计算时间减少高达90%。
...
### [148] [BRAINS: A Retrieval-Augmented System for Alzheimer's Detection and Monitoring](https://arxiv.org/abs/2511.02490)
*Rajan Das Gupta,Md Kishor Morol,Nafiz Fahad,Md Tanzib Hosain,Sumaya Binte Zilani Choya,Md Jakir Hossen*

Main category: cs.LG

TL;DR: BRAINS系统利用大型语言模型进行阿尔茨海默病的早期检测和监测，通过认知诊断模块和病例检索模块的双重架构，结合认知评估和类似病例检索来增强诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 随着全球阿尔茨海默病负担持续增长，在医疗资源有限地区实现早期准确检测变得至关重要。

Method: 采用双模块架构：认知诊断模块使用在认知和神经影像数据集上微调的LLM进行结构化风险评估；病例检索模块将患者档案编码为潜在表示并从知识库检索类似病例，通过病例融合层增强上下文理解。

Result: 在真实世界数据集上的评估显示BRAINS在分类疾病严重程度和识别早期认知衰退迹象方面具有有效性。
...
### [149] [An End-to-End Learning Approach for Solving Capacitated Location-Routing Problems](https://arxiv.org/abs/2511.02525)
*Changhao Miao,Yuntian Zhang,Tongyu Wu,Fang Deng,Chen Chen*

Main category: cs.LG

TL;DR: 本文提出了一种基于深度强化学习的异构查询方法（DRLHQ），首次采用端到端学习方式解决容量限制选址路径问题（CLRP）和开放CLRP问题，通过马尔可夫决策过程建模和异构查询注意力机制，在合成和基准数据集上表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 容量限制选址路径问题（CLRP）是组合优化中的经典问题，需要同时做出选址和路径决策。虽然深度强化学习已广泛应用于车辆路径问题，但针对CLRP的研究仍需探索。

Method: 提出DRL with heterogeneous query（DRLHQ）方法，采用编码器-解码器结构，将CLRP重新表述为适应各种决策的马尔可夫决策过程，并引入异构查询注意力机制来动态适应不同决策阶段。

Result: 在合成和基准数据集上的实验结果表明，该方法在解决CLRP和OCLRP时，相比代表性的传统方法和基于DRL的基线方法，具有更优的解决方案质量和更好的泛化性能。

Conclusion: 本研究首次提出了解决CLRP问题的端到端学习方法，通过创新的异构查询注意力机制有效处理选址和路径决策间的相互依赖关系，为基于DRL的方法提供了通用建模框架。
...
### [150] [Rawlsian many-to-one matching with non-linear utility](https://arxiv.org/abs/2511.02533)
*Hortence Nana,Andreas Athanasopoulos,Christos Dimitrakakis*

Main category: cs.LG

TL;DR: 研究具有多样性偏好的多对一匹配问题，提出基于罗尔斯公平的替代解决方案概念，当经典稳定匹配不存在时实现公平分配


<details>
  <summary>Details</summary>
Motivation: 传统稳定匹配模型无法处理大学通过非线性效用函数评估学生集合多样性的情况，导致稳定匹配可能不存在

Method: 设计基于罗尔斯公平的解决方案概念，开发确定性和随机性算法，迭代改进最差大学的结果

Result: 提出了在实际应用中当稳定性无法保证时实现公平分配的实用方法

Conclusion: 在具有多样性偏好的多对一匹配问题中，基于罗尔斯公平的解决方案概念提供了稳定性无法保证时的可行替代方案
...
### [151] [Adaptive Neighborhood-Constrained Q Learning for Offline Reinforcement Learning](https://arxiv.org/abs/2511.02567)
*Yixiu Mao,Yun Qu,Qi Wang,Xiangyang Ji*

Main category: cs.LG

TL;DR: 论文提出了一种新的邻域约束方法来解决离线强化学习中的外推误差问题，通过将贝尔曼目标中的动作选择限制在数据集动作邻域的并集中，避免了现有约束方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习面临由分布外动作引起的外推误差问题。现有约束方法（密度、支撑和样本约束）各有局限性：密度和样本约束过于保守，支撑约束难以准确建模行为策略。

Method: 提出邻域约束方法，将贝尔曼目标中的动作选择限制在数据集动作邻域的并集中。使用数据质量作为适应准则，设计自适应邻域约束，并基于高效的双层优化框架开发ANQ算法。

Result: 理论分析表明该约束能在特定条件下限制外推误差和分布偏移，且无需行为策略建模即可近似支撑约束。实证结果显示ANQ在标准离线RL基准上达到最先进性能，在噪声或有限数据场景下表现出强鲁棒性。

Conclusion: 邻域约束方法有效解决了离线强化学习中的外推误差问题，在理论保证和实践性能上都优于现有约束方法，特别是在处理噪声或有限数据时表现出色。
...
### [152] [Dynamic Priors in Bayesian Optimization for Hyperparameter Optimization](https://arxiv.org/abs/2511.02570)
*Lukas Fehring,Marcel Wever,Maximilian Spliethöver,Leona Hennig,Henning Wachsmuth,Marius Lindauer*

Main category: cs.LG

TL;DR: 本文提出了一种允许用户在贝叶斯优化过程中多次干预的方法，通过用户输入的先验分布来引导超参数优化过程，同时引入了误导性先验检测机制来防止有害输入。


<details>
  <summary>Details</summary>
Motivation: 传统的超参数优化方法由于黑盒性质和有限用户控制而缺乏机器学习专家的接受度，现有方法虽然可以初始化贝叶斯优化，但不支持在优化过程中进行在线引导。

Method: 通过推广现有的πBO方法，允许在HPO过程中运行时以先验分布形式重复输入专家知识和用户偏好，同时引入误导性先验检测方案来保护免受有害用户输入影响。

Result: 实验评估表明，该方法能有效整合多个先验，利用信息性先验，同时可靠地拒绝或克服误导性先验，保持了与未受干扰贝叶斯优化的竞争力。

Conclusion: 该方法成功实现了在贝叶斯优化过程中融入用户交互控制，既利用了专家知识又防止了有害输入的影响，提高了超参数优化的用户接受度。
...
### [153] [Directional-Clamp PPO](https://arxiv.org/abs/2511.02577)
*Gilad Karpel,Ruida Zhou,Shoham Sabach,Mohammad Ghavamzadeh*

Main category: cs.LG

TL;DR: 提出了方向性钳位PPO算法（DClamp-PPO），通过在"错误"方向区域施加更陡峭的损失斜率来惩罚那些优势为正但重要性比率低于1-β，或优势为负但重要性比率高于1+β的动作，从而改进标准PPO算法。


<details>
  <summary>Details</summary>
Motivation: 标准PPO算法在优化过程中，由于随机性和策略优化的随机性，重要性比率经常移动到"错误"方向，这是阻碍PPO改进的关键因素，但一直被忽视。

Method: 提出DClamp-PPO算法，在严格"错误"方向区域（优势为正但重要性比率低于1-β，或优势为负但重要性比率高于1+β）施加钳位惩罚，通过强制执行更陡峭的损失斜率来改进目标函数行为。

Result: 在各种MuJoCo环境中使用不同随机种子的实验表明，DClamp-PPO始终优于PPO及其变体，理论和实证都显示能更好地避免"错误"方向更新，同时保持重要性比率更接近1。

Conclusion: DClamp-PPO通过关注修改目标函数在"错误"方向区域的行为，有效解决了PPO优化过程中的关键问题，实现了更好的性能表现。
...
### [154] [A Large Language Model for Corporate Credit Scoring](https://arxiv.org/abs/2511.02593)
*Chitro Majumdar,Sergio Scandizzo,Ratanlal Mahanta,Avradip Mandal,Swarnendu Bhattacharjee*

Main category: cs.LG

TL;DR: Omega^2是一个基于大语言模型的信用评分框架，结合结构化财务数据和机器学习，在7800个企业信用评级数据上实现了超过0.93的平均测试AUC，提供了可靠且可解释的企业信用风险评估。


<details>
  <summary>Details</summary>
Motivation: 提高企业信用评分的预测可靠性和可解释性，通过结合结构化财务数据和先进机器学习技术来改进传统信用评估方法。

Method: 整合CatBoost、LightGBM和XGBoost模型，通过贝叶斯搜索在时间验证下进行优化，确保前向性和可重复性结果。

Result: 在穆迪、标普、惠誉和Egan-Jones的多机构数据集上，Omega^2实现了平均测试AUC超过0.93，证实了其在不同评级系统中的泛化能力和时间一致性。

Conclusion: 将基于语言的推理与定量学习相结合，为可靠的企业信用风险评估创建了透明且机构级的基础。
...
### [155] [Natural-gas storage modelling by deep reinforcement learning](https://arxiv.org/abs/2511.02646)
*Tiziano Balaconi,Aldo Glielmo,Marco Taboga*

Main category: cs.LG

TL;DR: GasRL是一个模拟器，将天然气市场的校准表示与基于深度强化学习的存储运营商策略模型相结合，用于分析最优库存管理如何影响均衡价格及供需动态。研究发现SAC算法表现最佳，能实现盈利性、稳健市场出清和价格稳定等多重目标，且产生的均衡价格动态与真实价格特征高度匹配。


<details>
  <summary>Details</summary>
Motivation: 研究旨在分析天然气存储运营商的最优库存管理策略如何影响市场均衡价格和供需动态，并评估欧盟强制最低存储阈值对市场韧性的影响。

Method: 开发GasRL模拟器，耦合天然气市场的校准表示与基于深度强化学习的存储运营商策略模型，测试多种RL算法，特别关注SAC算法的表现。

Result: SAC算法在GasRL环境中表现最优，成功实现存储运营商的多重目标，产生的均衡价格动态与真实价格特征高度匹配。欧盟最低存储阈值能增强市场对供应冲击的韧性，在异常大冲击时更有效避免市场中断。

Conclusion: GasRL模拟器能有效分析天然气市场动态，SAC算法是实现最优存储管理的有效方法，欧盟最低存储阈值政策有助于提升市场韧性。
...
TL;DR: 提出一种抗拜占庭攻击的Nesterov加速联邦学习算法(Byrd-NAFL)，在存在恶意工作节点的情况下实现快速且安全的收敛。


<details>
  <summary>Details</summary>
Motivation: 在联邦学习中，当存在能够进行任意恶意行为的拜占庭对手时，需要同时提高通信效率和鲁棒性。

Method: 将Nesterov动量与抗拜占庭聚合规则相结合，在非凸平滑损失函数下实现快速收敛。

Result: 建立了有限时间收敛保证，并通过大量数值实验验证了算法的有效性，在收敛速度、准确性和对多种拜占庭攻击策略的鲁棒性方面优于现有基准方法。

Conclusion: Byrd-NAFL算法成功实现了在拜占庭攻击环境下的高效、鲁棒联邦学习。
...
### [157] [Curriculum Design for Trajectory-Constrained Agent: Compressing Chain-of-Thought Tokens in LLMs](https://arxiv.org/abs/2511.02690)
*Georgios Tzannetos,Parameswaran Kamalaruban,Adish Singla*

Main category: cs.LG

TL;DR: 提出一种课程学习策略，通过在训练过程中逐步收紧约束条件，帮助智能体渐进式掌握部署要求，从而在复杂约束环境下提高训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 在严格约束条件下训练智能体面临重大挑战，特别是当约束使任务变得复杂时。需要一种方法能够平滑过渡到具有挑战性的部署环境。

Method: 受无约束强化学习中自定进度学习技术启发，该方法首先在简化约束版本上训练，然后逐步引入完整部署条件。包括理论分析和在RL和LLM智能体上的实证验证。

Result: 理论分析表明课程策略能加速训练；实证验证在二进制树MDP、多任务导航领域和数学推理任务中均有效，对LLM还能压缩思维链输出，在消费级硬件上实现显著推理加速。

Conclusion: 课程设计在增强复杂轨迹约束下智能体的效率和性能方面具有潜力，特别适用于资源受限的部署场景。
...
### [158] [Does Interpretability of Knowledge Tracing Models Support Teacher Decision Making?](https://arxiv.org/abs/2511.02718)
*Adia Khalid,Alina Deriyeva,Benjamin Paassen*

Main category: cs.LG

TL;DR: 该研究探讨了知识追踪(KT)模型的可解释性是否真正帮助教师做出教学决策。研究发现，虽然模拟研究显示基于可解释KT模型的决策能更快达到掌握水平，但在人类教师实验中，不同KT模型所需的练习任务数量差异不大，表明模型可解释性与教师决策的关系并不直接。
...
Method: 首先进行模拟研究比较基于可解释和非可解释KT模型的决策效果；然后让12名人类教师基于KT模型提供的信息做出教学决策，并评估模型的可用性和可信度。

Result: 模拟研究显示基于可解释KT模型的决策能更快达到掌握水平；人类教师实验表明教师对可解释KT模型的可用性和可信度评价更高，但不同模型所需的练习任务数量差异不大。
...
### [159] [Calibration improves detection of mislabeled examples](https://arxiv.org/abs/2511.02738)
*Ilies Chibane,Thomas George,Pierre Nodet,Vincent Lemaire*

Main category: cs.LG

TL;DR: 本文研究了校准基础模型对误标数据检测的影响，发现使用校准方法能提高误标实例检测的准确性和鲁棒性，为工业应用提供实用解决方案。


<details>
  <summary>Details</summary>
Motivation: 现实应用中误标数据普遍存在，会降低机器学习系统性能。需要有效方法检测误标实例并进行过滤或重新标注处理。

Method: 通过训练基础机器学习模型，然后探测每个实例以获得标签真实性的信任分数，并研究校准该基础模型的影响。

Result: 实证结果表明，使用校准方法能提高误标实例检测的准确性和鲁棒性。

Conclusion: 模型校准是提高误标数据检测性能的有效方法，为工业应用提供了实用解决方案。
...
### [160] [ConMeZO: Adaptive Descent-Direction Sampling for Gradient-Free Finetuning of Large Language Models](https://arxiv.org/abs/2511.02757)
...
<details>
  <summary>Details</summary>
Motivation: 零阶优化（MeZO）在微调大语言模型时具有内存优势，但由于在高维参数空间中搜索下降方向，收敛速度较慢。

Method: 提出ConMeZO优化器，通过将采样限制在以动量估计为中心的锥形区域内，而不是均匀随机采样，从而将搜索集中在真实梯度更可能存在的方向上。

Result: 理论上证明ConMeZO达到与MeZO相同的最坏情况收敛率；实证表明在自然语言任务微调LLMs时，ConMeZO比MeZO快达2倍。
...
TL;DR: SoCo框架通过将单智能体知识迁移到协作学习中，利用单智能体演示数据提升多智能体强化学习的训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体强化学习方法依赖昂贵的多智能体数据，而单智能体经验在许多重要场景中更容易获取，如协作编程、家庭合作和搜救任务。

Method: SoCo首先从单智能体演示中预训练共享单智能体策略，然后通过结合MoE门控选择器和动作编辑器的策略融合机制，在多智能体训练期间将其适应于协作任务。
...
TL;DR: VecComp是ChannelComp框架的扩展，通过集成多天线技术实现向量函数计算，在保持计算复杂度的同时提高了对信道衰落的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统ChannelComp仅支持标量函数计算，而许多数据应用需要向量计算，且易受信道衰落影响。

Method: 将ChannelComp与多天线技术结合，构建VecComp框架，使计算复杂度随向量维度线性增长。

Result: 建立了VecComp在衰落信道条件下的均方误差非渐近上界，数值实验验证了其在向量函数计算和衰落补偿方面的有效性。

Conclusion: VecComp为高维数据应用提供了计算效率高且对信道损伤鲁棒的向量函数计算方法。
...
TL;DR: 提出了STAR-VAE，一种基于Transformer的变分自编码器，使用SELFIES表示法保证分子语法有效性，支持属性引导的条件生成，并通过LoRA实现高效微调。


<details>
  <summary>Details</summary>
Motivation: 药物分子化学空间巨大，需要开发能够学习广泛化学分布、支持条件生成并实现快速分子生成的生成模型。

Method: 使用Transformer编码器和自回归Transformer解码器构建变分自编码器，在7900万个PubChem药物分子上训练，采用SELFIES表示保证语法有效性，通过属性预测器提供条件信号，并利用LoRA进行参数高效微调。

Result: 在GuacaMol和MOSES基准测试中达到或超过基线水平，潜在空间分析显示平滑、语义结构化的表示，支持无条件探索和属性感知生成。在Tartarus基准测试中，条件模型将对接分数分布向更强预测结合方向移动。

Conclusion: 现代化的、规模适当的VAE在结合原则性条件生成和参数高效微调时，在分子生成方面仍然具有竞争力。
...
### [164] [TabTune: A Unified Library for Inference and Fine-Tuning Tabular Foundation Models](https://arxiv.org/abs/2511.02802)
*Aditya Tanna,Pratinav Seth,Mohamed Bouadi,Utsav Avaiya,Vinay Kumar Sankarapu*

Main category: cs.LG

TL;DR: TabTune是一个统一的表格基础模型库，通过单一接口标准化了表格基础模型的完整工作流程，解决了异构预处理、碎片化API、不一致微调程序等限制因素。


<details>
  <summary>Details</summary>
Motivation: 表格基础模型的采用受到异构预处理管道、碎片化API、不一致微调程序以及缺乏部署导向指标标准化评估的限制，需要统一的解决方案。

Method: TabTune提供对7个最先进模型的统一访问，支持多种适应策略（零样本推理、元学习、监督微调、参数高效微调），自动化模型感知预处理，内部管理架构异质性，并集成性能、校准和公平性评估模块。

Result: TabTune实现了表格基础模型适应策略的一致基准测试，支持可扩展性和可重复性研究。

Conclusion: TabTune作为一个开源库，为表格基础模型提供了标准化的完整工作流程，解决了当前采用限制，促进了该领域的发展。
...
TL;DR: 该论文提出了一种基于量子计算启发的QUBO优化方法，通过选择每轮训练中最相关的少量客户端更新来显著降低联邦学习中的隐私暴露风险。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然保护原始数据隐私，但随着训练迭代次数增加，客户端更新被包含在聚合模型中的次数累积，导致敏感数据暴露风险增加。攻击者可以发起成员推断攻击、属性推断攻击和模型反转攻击。

Method: 使用量子计算启发的二次无约束二进制优化(QUBO)公式，在每轮训练中选择一小部分最相关的客户端更新，假设服务器拥有符合全局数据分布的验证/测试集。

Result: 在MNIST数据集300个客户端20轮训练中，实现了95.2%的每轮隐私暴露减少和49%的累积隐私暴露减少，147个客户端的更新从未被使用，同时保持或超过了全聚合的准确率。在CINIC-10数据集30个客户端实验中，实现了82%的每轮隐私改进和33%的累积隐私改进。

Conclusion: 该方法能有效降低联邦学习中的隐私暴露风险，同时保持模型性能，在小规模和复杂模型场景下也表现高效。
...
### [166] [Assessing win strength in MLB win prediction models](https://arxiv.org/abs/2511.02815)
*Morgan Allen,Paul Savala*

Main category: cs.LG

TL;DR: 该研究通过训练一套机器学习模型来预测棒球比赛胜负，并分析预测胜率与比分差异的关系，同时探讨了在跑分线投注中应用这些预测模型的效果。


<details>
  <summary>Details</summary>
Motivation: 在职业棒球大联盟中，策略和规划是决定比赛结果的重要因素。先前的研究通过构建机器学习模型来预测任何给定比赛的获胜队伍，本研究旨在扩展这项工作。

Method: 使用通用数据集训练一套全面的机器学习模型，并将这些模型产生的胜率预测与通过比分差异衡量的获胜强度相关联。

Result: 研究显示最常见的机器学习模型确实在预测胜率与获胜强度之间表现出相关性。在跑分线投注中应用预测胜率作为决策机制时，使用适当的投注策略可获得正回报，但简单使用机器学习模型进行投注会导致显著损失。

Conclusion: 机器学习模型能够有效预测棒球比赛结果并反映获胜强度，但在实际投注应用中需要谨慎的策略设计，避免简单直接使用模型预测导致损失。
...
TL;DR: FPP是一种保护联邦学习训练的新方法，通过参与者评估、训练恢复和信誉机制来防御恶意攻击，同时保持数据隐私。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然保护数据隐私，但难以防御恶意参与者破坏训练结果，需要一种既能保护隐私又能防御攻击的解决方案。

Method: FPP采用安全聚合保护数据隐私，通过参与者评估轮次、训练恢复机制和信誉机制来识别和减轻恶意参与者的影响。

Result: 实验表明FPP在存在模型投毒攻击的情况下仍能快速收敛，性能优于FedAvg、Power-of-Choice和Trimmed Mean/Median等现有方法。

Conclusion: FPP提供了一种有效的联邦学习保护框架，能够在保持隐私的同时防御恶意攻击，确保训练过程的可靠性和安全性。
...
### [168] [GeoCrossBench: Cross-Band Generalization for Remote Sensing](https://arxiv.org/abs/2511.02831)
*Hakob Tamazyan,Ani Vanyan,Alvard Barseghyan,Anna Khosrovyan,Evan Shelhamer,Hrant Khachatrian*

Main category: cs.LG

TL;DR: 本文介绍了GeoCrossBench基准测试，评估遥感模型在不同卫星间的泛化能力，并提出了ChiViT模型来提升跨卫星性能。研究发现现有模型在新卫星上性能显著下降，但通过微调可获得相对一致的性能。


<details>
  <summary>Details</summary>
Motivation: 随着遥感卫星数量和多样性的增加，大多数标注数据来自旧卫星，而基础模型规模扩大导致重新训练成本增加，因此模型对新卫星的泛化能力变得至关重要。

Method: 扩展GeoBench基准测试，引入新的评估协议测试模型在不同卫星间的性能表现；开发自监督的ChannelViT扩展ChiViT来提升跨卫星性能；比较多种基础模型在in-distribution、无波段重叠和额外波段情况下的表现。

Result: 1) 最佳遥感基础模型在in-distribution设置下不优于通用模型DINOv3；2) 无波段重叠时所有模型性能下降2-4倍，ChiViT显著优于DINOv3；3) 测试时提供额外波段，所有模型性能平均下降5-25%；4) 微调最后一层可获得相对一致的跨卫星性能。

Conclusion: 遥感模型在新卫星上的泛化能力仍有很大提升空间，基准测试远未饱和，需要开发更具未来适应性的遥感模型。
