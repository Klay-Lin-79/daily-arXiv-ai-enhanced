...
Method: 使用轻量级卷积分类器"Faster-Than-Lies"与视觉语言模型Qwen2-VL-7B相结合，通过自编码器重构误差图生成伪影定位热图，将70种视觉伪影类型分为8个语义组。

Result: 在增强的CiFAKE数据集上达到96.5%的准确率，在8核CPU上推理时间为175ms，能够生成可解释的文本描述每个检测到的异常。

Conclusion: 这项工作证明了结合视觉和语言推理在低分辨率图像中进行可解释真实性检测的可行性，并概述了在取证、工业检测和社交媒体审核等领域的潜在跨域应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有计数模型在物体具有复杂形状、内部对称性或重叠组件时容易出错，无法像人类那样通过感知视觉重复和结构关系来计数，需要开发更接近人类感知能力的计数系统。

Method: 基于CounTR架构，用自监督基础模型DINOv2替换视觉编码器以获得更丰富和空间一致的特征表示，并加入位置嵌入融合来保持几何关系，最后通过轻量级卷积解码器将特征解码为密度图。

Result: 在FSC-147数据集上达到与当前最先进方法相当的性能，在结构复杂或密集场景中表现出更优越的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 河流中漂浮的人造碎片已成为紧迫的环境问题，对生物多样性、水质以及航运和娱乐等人类活动产生不利影响，需要有效的监测方法。

Method: 使用固定原位相机采集数据，应用深度学习模型进行漂浮碎片检测和量化，测试不同模型在复杂环境条件下的准确性和推理速度，实施几何模型从2D图像估计物体实际尺寸，并考虑数据泄漏偏差。

Result: 研究发现数据集构建协议（特别是负样本整合和时间泄漏考虑）的重要性，证明了使用投影几何结合回归修正进行度量物体估计的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感图像超分辨率方法在分布外条件下（如罕见地貌特征）往往产生视觉合理但物理不准确的结果，需要开发具有物理感知和分布外鲁棒性的方法。

Method: 采用双条件架构：门控ControlNet保持低分辨率输入的几何保真度，文本提示提供复杂特征的语义引导；引入多面损失函数确保光谱和辐射一致性；使用随机前向传播方法量化预测不确定性。

Result: 在地球物理专家的盲评估中，模型输出接近真实图像保真度，显著优于最先进基线；在感知指标上获得定量增益，FID降低近40%。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法局限于单对象生成、需要领域特定训练或缺乏360度全方位视角支持，需要一种更通用的3D场景生成方法。

Method: 将场景生成重新表述为多瓦片去噪问题，独立生成重叠的3D区域并通过加权平均无缝融合，利用对象级先验知识。
...
<details>
  <summary>Details</summary>
Motivation: 教学视频中的场景级字幕可以通过理解视觉线索和时间结构来增强学习效果，但缺乏这种结构捕捉的字幕可能缺乏连贯性和质量，从而削弱视频的教育意图。

Method: 使用YouCookII数据集的场景标注，DynaStride执行自适应帧采样和多模态窗口化，采用多模态思维链过程生成多个动作-对象对，并通过动态步长窗口选择算法进行精炼和融合。

Result: 与VLLaMA3和GPT-4o等强基线相比，在N-gram度量（BLEU、METEOR）和语义相似度度量（BERTScore、CLIPScore）上均显示出持续增益。

Conclusion: DynaStride生成的字幕在时间上更连贯和信息丰富，为改进AI驱动的教学内容生成提供了一个有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人像图像到3D模型存在视觉伪影、细节缺失和身份保持不足的问题，而图像扩散模型虽然能生成高质量图像但缺乏3D一致性且计算昂贵。

Method: 采用前馈图像到虚拟形象生成流程获得初始3D表示和噪声渲染，然后使用基于输入图像的单步扩散模型进行多视角一致的渲染精炼，并采用合成多视图数据预训练和高质量真实图像微调的训练策略。

Result: 该方法在定性和定量评估上都优于当前最先进的人像新视角合成方法，同时保持时间效率。

Conclusion: TurboPortrait3D成功地将图像扩散模型用于增强图像到虚拟形象方法，实现了高质量、多视角一致且低延迟的人像3D合成。
...
<details>
  <summary>Details</summary>
Motivation: 传统3D高斯泼溅在室内场景的大面积低纹理区域中，由于光度损失的模糊性，无法恢复高保真的3D表面几何。

Method: 设计了语言提示平面先验（LP3）管道，使用预训练视觉语言分割模型并通过跨视图融合和几何先验检查来优化区域建议；在3D高斯优化中增加了平面先验监督项和几何先验监督项。

Result: 在标准室内基准测试中，PlanarGS重建出准确且详细的3D表面，显著优于现有最先进方法。

Conclusion: PlanarGS通过结合平面和几何先验，有效解决了室内场景中3D高斯泼溅的几何重建问题，实现了高质量的表面重建。
...
Conclusion: Neural USD框架为精确对象编辑提供了有效解决方案，支持对单个对象的外观、几何和姿态进行独立控制。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像护栏模型受限于预定义类别，缺乏语义推理导致误分类，且难以适应新兴威胁需要昂贵重新训练。需要更高效透明的安全内容防护方案。

Method: 采用有效的数据收集和生成框架、策略遵循训练流程、定制损失函数，以及多样化的QA生成和训练策略来增强学习效果。

Result: SafeVision在VisionHarm-T上比GPT-4o表现提升8.6%，在VisionHarm-C上提升15.5%，同时速度提升超过16倍。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在医学图像分析中缺乏透明性，无法提供医生依赖的逐步推理过程，需要开发能够展示专家推理方式而非仅结论的模型。

Method: 采用高保真视觉编码与两阶段训练方法：推理风格监督微调后接强化学习，使用可验证奖励机制，输出模拟放射科医生系统性思维过程、不确定性和鉴别诊断的推理。

Result: 在分布外评估中实现竞争性多标签分类同时提升可解释性；专家放射科医生研究表明完整推理轨迹增强了信心、支持错误审计并减少报告完成时间。

Conclusion: 该框架支持社区在胸部放射学及其他医学成像任务中开发可信赖、可解释的AI，其中推理质量与预测质量同等重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常孤立评估AI助手的能力，缺乏真实的流式场景或仅支持短期任务，无法满足现实世界自我中心AI助手处理多模态输入、实时响应和保持长期记忆的需求。

Method: 构建包含工作学习、生活方式、社交活动和外出文化四个领域的同步多模态数据集，所有数据在统一时间线上对齐，包含高质量视觉叙述和语音转录，通过人工精炼。定义12个子任务评估记忆、理解和跨记忆推理能力。

Result: TeleEgo包含超过14小时/参与者的数据，3,291个人工验证的QA项目，涵盖多种问题格式，并提出了实时准确率和记忆持续时间两个关键指标来联合评估正确性、时间响应性和长期保持能力。

Conclusion: TeleEgo为实用AI助手的发展提供了现实且全面的评估框架，能够推进自我中心AI助手在真实世界环境中的能力发展。
...
<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力丧失的主要原因，虽然已有许多深度学习模型用于从眼底图像预测DR，但由于采集设备差异、人口统计差异和成像条件等因素导致的分布变化，许多模型在保持鲁棒性方面面临挑战。

Method: 提出AdvBlur方法，集成对抗性模糊图像到数据集中，并采用双损失函数框架来处理领域泛化问题。通过广泛实验探索相机类型、低质量图像和数据集大小等因素的影响，并进行消融研究验证模糊图像和损失函数的有效性。

Result: 实验结果表明该方法在未见外部数据集上取得了与最先进领域泛化DR模型相竞争的性能，有效缓解了分布变化的影响。

Conclusion: AdvBlur方法通过集成对抗性模糊图像和双损失函数框架，成功解决了糖尿病视网膜病变分类中的领域泛化问题，在多个数据集上的综合评估证明了其有效性。
...
Method: 挑战赛在隐藏测试集上对自动化算法进行基准测试，包括可选的计算仿真表面网格生成任务。使用3D U-Net架构和定制化后处理步骤。

Result: 深度学习方法是主流，3D U-Net架构主导顶级提交。排名最高的算法集成模型显著优于单个模型。性能与算法设计（特别是定制化后处理）和训练数据特征密切相关。

Conclusion: 该计划不仅建立了新的性能基准，还提供了持久资源，推动未来创新，开发稳健且可临床转化的工具。
...
<details>
  <summary>Details</summary>
Motivation: 火星科学缺乏标准化的基准测试和评估框架，这限制了开发火星基础模型的进展。

Method: 提供标准化的即用数据集，并使用在自然图像、地球卫星数据和最先进的视觉语言模型上预训练的模型进行基线评估。

Result: 所有分析结果表明，火星特定基础模型可能比通用领域模型具有优势。

Conclusion: Mars-Bench旨在为开发和比较火星科学机器学习模型建立标准化基础，并推动领域自适应预训练的进一步探索。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型的安全机制容易受到对抗性提示的攻击，现有的红队测试方法需要白盒访问、效率低下且生成的提示语义无意义易被过滤器拦截。

Method: 采用交替优化-微调流程，在对抗性后缀优化和利用优化后缀微调LLM之间交替进行；集成双重规避策略，通过辅助LLM困惑度评分和禁止令牌惩罚来绕过基于困惑度的过滤器和黑名单词过滤器。

Result: 广泛实验证明该方法在红队测试中表现优异，生成的对抗性提示具有人类可读性、抗过滤性，并且在零样本迁移性方面表现优越，能够即时适应未见提示并暴露商业API的关键漏洞。

Conclusion: APT框架能够有效评估文本到图像模型的安全漏洞，生成的对抗性提示既人类可读又难以被过滤器检测，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有稀疏调优方法的两阶段范式问题：基于梯度信息定位任务相关权重忽略了微调过程中的参数调整，且仅更新定位权重需要存储所有权重矩阵导致高内存使用。

Method: 提出单阶段方法SNELLA：1）通过两个低秩可学习矩阵合并的稀疏矩阵选择性更新权重矩阵，引入非线性核函数扩展低秩分解；2）提出自适应双层稀疏分配机制，基于重要性分数在层间和层内进行端到端的权重竞争。

Result: 在分类、分割和生成任务上的实验表明，SNELLA在低内存使用下达到SOTA性能：在FGVC基准上比SPT-LoRA提高1.8%的Top-1准确率（91.9% vs 90.1%），在86M到632M参数规模的模型上实现31.1%-39.9%的内存减少。
...
<details>
  <summary>Details</summary>
Motivation: 现有的方法主要关注对抗性微调或提示优化，但忽视了CLIP编码特征中的差距——文本和图像特征相距较远。这种错位在对抗性扰动下被显著放大，导致分类性能严重下降。

Method: COLA首先将对抗性图像嵌入投影到类别文本特征张成的子空间上，有效过滤非语义失真同时保留判别信息；然后通过最优传输建模图像和文本作为多个增强视图上的离散分布，并在成本计算中无缝集成子空间投影。

Result: 在14个零样本分类基准上的广泛评估证明了COLA的有效性，特别是在PGD对抗攻击下，在ImageNet及其变体上平均提升6.7%，同时在干净样本上保持高准确率。

Conclusion: COLA是一个无需训练且兼容现有微调模型的框架，能够在对抗条件下确保稳定的跨模态对齐，显著提升CLIP模型的对抗鲁棒性。
...
Method: 通过研究交叉注意力和自注意力机制的两个关键属性来实现文本移除和控制文本样式与内容。使用自注意力反转来减少文本幻觉，通过交叉注意力重分布来降低文本幻觉。在潜在优化框架中引入新的损失函数：交叉注意力内容损失来提高文本渲染准确性，自注意力样式损失来促进样式定制。

Result: OmniText是第一个能够执行多样化文本图像操作任务的通才方法，在多个任务和指标上实现了最先进的性能，与其他文本修复方法相比表现优异，与专业方法相当。

Conclusion: OmniText框架成功解决了文本图像操作中的关键挑战，通过创新的注意力机制操作和损失函数设计，实现了文本移除、样式控制和内容编辑的多样化任务，为文本图像操作领域提供了首个通才解决方案。
...
Result: 发现可解释性和分类能力呈正相关，高分类能力的表示提供更多可解释语义；通过可解释性最大化微调可进一步提升分类性能。

Conclusion: 实践者可以统一提升预训练视觉模型的可解释性和分类能力，两者可以同时优化。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成质量优秀，但受限于昂贵的迭代采样过程。现有训练自由加速方法忽略了去噪趋势，缺乏对模型特定容忍度的误差控制，导致轨迹偏差和结果不一致。

Method: 1. 一致性趋势预测器：利用扩散轨迹的平滑连续性，将历史去噪模式投影到稳定未来方向，并在多个近似步骤中渐进分布；2. 模型特定误差容忍度搜索机制：通过识别从波动语义规划到稳定质量精炼的过渡点来推导校正阈值。

Result: ETC在FLUX上实现了2.65倍加速，一致性退化可忽略（-0.074 SSIM分数）。

Conclusion: ETC框架通过结合趋势一致性和误差控制，有效解决了扩散模型加速中的轨迹偏差问题，在保持生成质量的同时显著提升采样效率。
...
<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型在组合性方面仍然存在困难，经常无法准确渲染对象数量、属性和空间关系。

Method: 利用大语言模型从输入提示中合成显式布局，将这些布局注入图像生成过程，并使用目标中心视觉语言模型对多个候选结果进行重排序，迭代选择最符合提示的结果。

Result: 通过统一显式布局基础与基于自精炼的推理时扩展，该框架相比最近的文本到图像模型实现了更强的场景与提示对齐。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成领域缺乏针对T2V训练优化的视频字幕策略，高质量的视频-文本对对于生成连贯且符合指令的视频至关重要。

Method: 提出VC4VG框架，从T2V角度分析字幕内容，将视频重建所需要素分解为多个维度，建立原则性的字幕设计方法，并构建VC4VG-Bench基准进行多维度、细粒度的评测。

Result: 广泛的T2V微调实验表明，改进的字幕质量与视频生成性能之间存在强相关性，验证了该方法的有效性。

Conclusion: VC4VG为T2V生成提供了有效的字幕优化框架，通过提升字幕质量可以显著改善视频生成效果，相关工具和代码已开源以支持进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在自动驾驶场景理解任务中面临的多样化问题类型干扰、空间推理能力不足以及输出质量优化等挑战。

Method: 采用四核心组件：混合提示路由分类问题并分发到专家提示；任务特定提示嵌入坐标系统、空间推理规则和思维链；视觉组装模块组合多视角图像和对象裁剪；按任务配置推理参数。

Result: 在Qwen2.5-VL-72B上实现Phase-1（干净数据）70.87%和Phase-2（损坏数据）72.85%的平均准确率。

Conclusion: 结构化提示和空间基础显著提升了视觉语言模型在安全关键自动驾驶任务中的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: SAM2作为SAM的继任者，在视频分割方面具有强大泛化能力，但其鲁棒性尚未被探索。现有对SAM的攻击是否能直接转移到SAM2存在疑问，需要分析性能差距并解决架构差异带来的挑战。

Method: 提出UAP-SAM2方法：1）使用目标扫描策略将每帧分为k个区域并随机分配提示，减少优化过程中的提示依赖性；2）设计双语义偏差框架，通过扭曲当前帧语义和破坏连续帧间语义一致性来优化UAP。

Result: 在六个数据集上的两个分割任务实验表明，UAP-SAM2方法对SAM2非常有效，比较结果显示其显著优于最先进的攻击方法。

Conclusion: UAP-SAM2成功解决了SAM2架构差异带来的挑战，实现了跨提示的通用对抗攻击，为评估SAM2的鲁棒性提供了有效工具。
...
Result: 在四个公开数据集(CVC-ColonDB、CVC-ClinicDB、EtisLaribPolypDB、ACDC)上表现出卓越性能，超越现有SOTA方法，并有效关注解剖结构的相关区域。

Conclusion: CLFSeg在确保计算效率的同时提升了分割性能，是现实世界医疗诊断场景的潜在解决方案。
...
Method: 提出MC-SJD方法，基于耦合的信息论方法，通过最大化连续迭代中采样相同草稿token的概率来加速标准SJD，仅需对现有算法进行单行修改。

Result: 实现了图像生成约4.2倍加速和视频生成约13.3倍加速，且输出质量无任何下降。

Conclusion: MC-SJD是一种高效的无损加速方法，显著提升了自回归视觉生成的推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有扩散模型在推理时干预（如负引导或基于能量的优化）导致分布偏移、身份与非身份属性纠缠、视觉保真度和数据效用下降的问题。

Method: 设计条件扩散模型，包含身份掩码学习方案：身份解耦潜在重组器使用身份变分自编码器建模身份特征，非身份属性从同身份对中提取并通过双向潜在对齐；身份引导潜在协调器通过软门控融合表示；使用基于重组的重建损失训练模型。推理时从学习到的身份空间采样随机身份向量实现匿名化，并引入正交身份映射策略进一步抑制身份泄露。

Result: 实验表明ID²Face在视觉质量、身份抑制和效用保持方面优于现有方法。

Conclusion: ID²Face通过训练中心的解耦方法有效解决了人脸匿名化中的身份与非身份属性纠缠问题，实现了高质量的匿名化效果。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型处理大量视觉token导致计算开销大，现有剪枝方法基于注意力分数选择最显著token，导致语义不完整。

Method: 提出SCOPE方法，结合token显著性和覆盖度，通过计算token-coverage增益和SCOPE分数，迭代选择最优token。

Result: 在LLaVA-1.5和LLaVA-Next模型上的实验表明，该方法在多个视觉语言理解基准测试中持续优于先前方法。

Conclusion: SCOPE通过联合建模显著性和覆盖度，有效解决了视觉token剪枝中的语义完整性问题，提高了多模态大语言模型的效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于注意力的深度阴影去除模型存在固定注意力模式，容易混合无关区域的照明线索，导致结构扭曲和颜色不一致。需要探索更有效的全局上下文建模方法。

Method: 1. 从序列建模角度重新审视阴影去除，引入Mamba选择性状态空间模型；2. 提出CrossGate方向调制机制，在Mamba输入门注入阴影感知相似性；3. 引入ColorShift正则化，通过对比学习目标抑制颜色污染。

Result: 在公共基准测试上的广泛实验表明，DeshadowMamba实现了最先进的视觉质量和强大的定量性能。

Conclusion: 通过将序列建模与阴影去除的结构完整性和色彩一致性要求相结合，提出的方法在阴影去除任务中表现出色，为图像恢复任务提供了新的思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决数据稀缺条件下合成图像来源归因的挑战，特别是在少样本或零样本分类场景下，需要开发无需训练或仅需少量样本的归因方法。

Method: 基于图像重合成的免训练单样本归因方法：首先生成描述待分析图像的提示词，然后用所有候选生成器重合成该图像，在合适的特征空间中比较原始图像与重合成图像的相似度，将图像归因于产生最接近重合成图像的生成器。

Result: 在新建的人脸图像数据集上，所提出的重合成方法在只有少量训练样本的情况下优于现有的少样本方法和其他基线方法，证明了该方法的有效性。

Conclusion: 基于重合成的方法在数据稀缺条件下具有优势，新建的数据集为开发和评估少样本和零样本归因方法提供了有价值的基准测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在细粒度视觉感知方面的瓶颈问题，现有方法如监督微调会损害通用能力，而强化微调则过于侧重文本推理而非视觉感知。

Method: 开发ViPER自举框架，通过自批判和自预测实现迭代进化，整合图像级和实例级重建与两阶段强化学习策略，建立闭环训练范式。

Result: 在Qwen2.5-VL系列上应用ViPER产生Qwen-Viper系列，在7个综合基准测试中平均提升1.7%，在细粒度感知任务上最高提升6.0%，同时保持泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 遥感应用依赖深度学习进行场景分类，但性能受限于标注数据稀缺和跨域标注成本高。虽然CLIP等视觉语言模型通过对齐视觉和文本模态学习可迁移表示，但直接应用于遥感存在显著领域差距和任务特定语义适应需求。

Method: 系统评估了多种代表性提示学习方法：上下文优化、条件上下文优化、多模态提示学习、带自调节约束的提示。这些方法体现了从静态上下文优化到条件提示增强泛化、多模态提示联合适应、语义正则化提示稳定学习的设计理念。

Result: 在多个基准遥感数据集上的广泛实验表明，提示学习在少样本场景中持续优于两个标准基线：手工提示的零样本CLIP和基于冻结CLIP特征的线性探针。带自调节约束的提示方法实现了最稳健的跨域性能。
...
Conclusion: 高性能研究管道可以成功转化为便携、临床可用的工具，证明了轻量化和模块化设计的可行性。
...
TL;DR: 该研究系统探讨了在医学图像分类中如何利用放射学报告，发现在标签与文本关联度高的任务中，预训练阶段使用报告有益；而在关联度低的任务中，显式的图像-文本对齐预训练可能有害。微调阶段使用报告在某些情况下能带来显著改进。


<details>
  <summary>Details</summary>
Motivation: 医学图像通常伴随包含丰富专家注释的放射学报告，但训练时依赖这些报告需要放射科医生的及时手动工作。研究旨在明确在什么情况下可以利用放射学报告来改进仅基于图像的分类。

Method: 系统研究放射学报告在预训练和微调阶段的使用，涵盖诊断和预后任务（如12个月再入院），并在不同训练集规模下进行实验。

Result: 研究发现：(1) 当标签在文本中充分体现时，预训练阶段利用报告对下游分类任务有益；但当标签与文本关联度低时，显式的图像-文本对齐预训练可能有害；(2) 微调阶段使用报告能带来显著改进，在某些情况下甚至比预训练方法影响更大。
...
<details>
  <summary>Details</summary>
Motivation: 脑卒中后MRI不仅显示局灶性病变，还揭示继发性结构变化（如萎缩和脑室扩大）。这些异常作为恢复和预后的影像生物标志物越来越受到重视，但监督分割方法难以有效捕捉这些变化。

Method: 使用REFLECT（基于流的生成模型）进行无监督异常检测。在ATLAS数据上使用双专家中央切片标注，通过Free-Response ROC分析评估异常图的对象级性能。训练了两个模型：一个在脑卒中患者无病灶切片上训练（ATLAS），另一个在健康对照数据上训练（IXI）。

Result: 在ATLAS测试对象上，基于IXI（健康对照）训练的模型实现了更高的病灶分割性能（Dice = 0.37 vs 0.27）和更好的非病灶异常检测敏感性（FROC = 0.62 vs 0.43）。
...
<details>
  <summary>Details</summary>
Motivation: 解决多目标跟踪中目标数量未知和时变、保持目标身份一致性、处理非线性动态以及应对弱噪声检测器等问题。

Method: 采用混合跟踪方法，结合粒子群优化(PSO)和提出的适应度度量，集成社会交互机制，构建基于空间一致性、外观、检测置信度、轨迹惩罚和社会分数的综合状态观测模型。

Result: 在标准基准测试和实际场景中表现出优于最先进跟踪器的性能，减少了ID切换和轨迹丢失，特别是在遮挡情况下。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决非线性动态和时变目标数量下的标识符一致性问题，特别是在目标交互和长时间遮挡情况下保持跟踪稳定性。

Method: 使用随机粒子滤波处理非线性动态和非高斯噪声，结合粒子群优化引导粒子；采用确定性关联机制确保标识符一致性；提出平滑更新方案处理弱跟踪；使用速度回归增强采样。

Result: 实验结果表明该方法相比最先进的跟踪器具有优越性能，适用于预录制视频和实时摄像头流。

Conclusion: 提出的混合随机-确定性跟踪方法在复杂场景下能有效保持目标标识符一致性，并在各种条件下表现出色。
...
Conclusion: 该方法具有鲁棒性和成本效益，为水库存储的连续、独立于传感器的监测提供了实用解决方案，可复制用于其他水体，50年的时间序列数据对气候变化和环境模式研究具有价值。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI模型在安全关键和高风险领域应用的增加，确保透明度和信任至关重要。虽然可解释AI方法在分类任务中的评估已取得显著进展，但针对语义分割的评估策略仍相对不足。

Method: 开发了一个全面的系统评估框架，专门用于评估语义分割中的可解释AI方法。该框架采用像素级评估策略和精心设计的指标，明确考虑了空间和上下文任务复杂性。

Result: 使用最近适应的基于类激活映射的可解释AI方案进行仿真，结果表明所提出方法具有高效性、鲁棒性和可靠性。
...
TL;DR: 提出了一种基于功能分解的深度条件图像压缩框架DCIC-sgp，通过自生成先验来分离图像的结构骨架和纹理细节，有效解决低码率下的几何变形问题。


<details>
  <summary>Details</summary>
Motivation: 当前学习型图像压缩方法在建模自然图像的复杂相关性结构方面存在局限，特别是在单一表示中无法有效分离全局结构和局部纹理，导致低码率下出现严重几何变形。

Method: 采用功能分解框架，首先编码自生成先验来捕获图像结构骨架，然后利用该先验深度调节整个压缩流程，特别是分析变换，使其专注于残差高熵细节。

Result: 视觉分析显示该方法显著减轻了传统编解码器在低码率下的几何变形伪影；定量评估在Kodak、CLIC和Tecnick数据集上分别实现了14.4%、15.7%和15.1%的BD-rate降低。

Conclusion: 所提出的深度条件图像压缩框架通过层次化、依赖驱动的方法实现了信息流的有效分离，在保持高率失真性能的同时显著改善了低码率下的视觉质量。
...
TL;DR: 研究发现视频扩散模型在视觉任务中比大型语言模型具有更高的数据效率，视频预训练为视觉基础模型提供了更好的归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在语言领域表现出色，但在视觉领域仍面临组合理解、样本效率和通用问题解决等挑战，需要探索更好的视觉基础模型方法。

Method: 使用预训练的视频扩散模型和大型语言模型，配备轻量级适配器，在自然模态下进行任务评估，包括ARC-AGI、ConceptARC、视觉游戏、路径规划和元胞自动机等基准测试。

Result: 在多个基准测试中，视频扩散模型比语言模型表现出更高的数据效率，表明视频预训练提供了更好的结构理解能力。

Conclusion: 视频预训练为视觉基础模型的发展提供了有前景的方向，其时空数据预训练赋予模型强大的结构和动态归纳偏置，支持广泛的任务适应性。
...
TL;DR: 该论文提出使用深度学习技术检测茶叶病害，包括红锈病、Helopeltis和红蜘蛛螨三种病害，并评估了SSD MobileNet V2和Faster R-CNN ResNet50 V1两种目标检测模型，其中Faster R-CNN表现更好，mAP达到25%。还使用Mask R-CNN进行实例分割以计算叶片受损区域。


<details>
  <summary>Details</summary>
Motivation: 开发能够自动识别茶叶病害并量化叶片受损区域的深度学习系统，以帮助茶农及时监测和管理病害，提高茶叶产量和质量。

Method: 采用两种目标检测模型（SSD MobileNet V2和Faster R-CNN ResNet50 V1）进行病害检测，并使用Mask R-CNN进行实例分割，同时开发了自定义方法来计算叶片受损区域。

Result: Faster R-CNN ResNet50 V1在IOU 0.50:0.95范围内获得25.2%的精确度和4.4%的召回率，mAP为25%，优于SSD MobileNet V2的20.9% mAP。成功实现了三种茶叶病害的检测和受损区域量化。

Conclusion: Faster R-CNN ResNet50 V1在茶叶病害检测中表现优于SSD MobileNet V2，结合Mask R-CNN的实例分割技术能够有效识别病害并量化叶片受损程度，为茶叶病害管理提供了可行的自动化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的无标定方法虽然减少了对精确相机标定的依赖，但存在计算成本高和重建精度低的问题，限制了非专家用户和在野外环境中的使用。

Method: 利用现成的2D关键点检测器同时标定相机（包括Brown-Conrady畸变系数）并重建3D关键点和密集场景点云；采用置信度驱动的时空关键点采样策略和基于图的全局优化，确保在固定计算成本下实现鲁棒标定。

Result: 在EgoHumans和Human3.6M数据集上的评估显示，相比现有无标定方法，Kineo将相机平移误差降低83-85%，相机角度误差降低86-92%，世界平均关节误差降低83-91%。

Conclusion: Kineo在真实场景中高效运行，处理速度超过视频时长，为无标定运动捕捉提供了实用且可复现的解决方案。
...
TL;DR: 提出Decoupled MeanFlow方法，将流模型转换为流图模型，无需架构修改，实现1-4步高质量生成，在ImageNet上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有去噪生成模型（如扩散模型和流模型）需要大量去噪步骤，存在离散化误差问题。流图模型能缓解此问题并加速采样，但通常需要架构修改，限制了与预训练流模型的兼容性

Method: 提出解耦均值流方法，通过条件化扩散变换器的最后模块于后续时间步，将预训练流模型直接转换为流图模型，无需架构修改，结合增强训练技术

Result: 在ImageNet 256x256和512x256上，1步生成FID分别为2.16和2.12，4步生成FID分别为1.51和1.68，大幅超越现有方法，推理速度提升100倍以上

Conclusion: 训练流模型后转换为流图模型比从头训练流图模型更高效有效，该方法实现了高质量快速生成，在保持性能的同时显著加速推理
...
TL;DR: Latent Sketchpad框架为多模态大语言模型添加内部视觉草稿板，通过视觉生成增强模型的视觉规划和想象力能力，而不损害推理性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在需要视觉规划和想象的复杂场景中表现不佳，而人类使用草图作为视觉思维来开发和交流想法。

Method: 在现有MLLMs基础上集成视觉生成到自回归推理过程中，包含上下文感知视觉头自回归生成视觉表示，以及预训练草图解码器将视觉潜在变量渲染为可解释图像。

Result: 在MazePlanning数据集上的实验表明，Latent Sketchpad在保持骨干模型推理性能的同时，提供了可比较甚至更优的推理表现，并能泛化到不同的前沿MLLMs。
...
Result: 总共记录了171,909个注视点、263,320个扫视和1,867,362个鼠标交互事件，创建了一个全面的行为数据集。

Conclusion: 这些数据可用于改善病理学家和AI系统的训练，所有实验已在https://osf.io/hj9a7预注册，完整数据集和分析代码可在https://go.osu.edu/pathogaze获取。
...
TL;DR: 本文提出了Group Relative Attention Guidance方法，通过重新加权DiT模型中不同token的delta值来调节模型对输入图像相对于编辑指令的关注度，实现无需调参的连续细粒度编辑强度控制。
...
Method: 分析DiT模型中的MM-Attention机制，发现Query和Key token共享仅层依赖的偏置向量，将其解释为模型固有编辑行为，而token与对应偏置的差值编码内容特定编辑信号。提出Group Relative Attention Guidance方法，重新加权不同token的delta值来调节模型关注度。

Result: 在现有图像编辑框架上的大量实验表明，GRAG只需四行代码即可集成，持续提升编辑质量。相比常用的Classifier-Free Guidance，GRAG实现了更平滑、更精确的编辑程度控制。
...
TL;DR: SAGE是一种零样本视频过渡方法，通过结合结构引导（线图和运动流）与生成合成，实现平滑、语义一致的视频过渡，无需微调即可处理具有大时间间隔或显著语义差异的多样化片段。


<details>
  <summary>Details</summary>
Motivation: 传统视频过渡方法（如交叉淡入淡出、变形、帧插值）和最近的生成方法在处理具有大时间间隔或显著语义差异的多样化片段时存在困难，无法产生内容感知和视觉连贯的过渡效果。

Method: 借鉴艺术工作流程，提出SAGE方法，结合结构引导（通过线图和运动流提供）与生成合成，在零样本设置下实现结构保持和感知连续性的视频过渡。

Result: 与现有方法（FILM、TVG、DiffMorpher、VACE、GI）相比，SAGE在定量指标和用户研究中均表现出色，能够为多样化片段生成更好的过渡效果。

Conclusion: SAGE通过结构感知的生成方法成功解决了多样化视频片段之间的过渡挑战，在保持结构一致性和视觉连贯性方面优于现有方法。
...
Result: 在M2I合成数据集和真实世界数据集RoScenes上的广泛实验表明，MIC-BEV在3D目标检测中实现了最先进的性能，并在极端天气和传感器退化等挑战条件下保持鲁棒性。

Conclusion: MIC-BEV具有实际部署的潜力，其数据集和源代码已公开。
...
TL;DR: 研究发现预训练的视觉变换器（ViTs）能够自然涌现物体绑定能力，即识别哪些图像块属于同一物体的能力。这种能力在自监督学习模型中表现突出，但在ImageNet监督学习模型中较弱，表明这是通过特定预训练目标获得的能力而非架构特性。


<details>
  <summary>Details</summary>
Motivation: 探索视觉变换器是否能够自然涌现物体绑定能力，即识别哪些图像块属于同一物体的能力，这对于理解连接主义系统如何获得符号知识具有重要意义。

Method: 使用相似性探针从ViT各层的图像块嵌入中解码IsSameObject属性，比较不同预训练目标（DINO、MAE、CLIP、ImageNet监督）下的表现，并进行消融实验验证该信号的作用。

Result: 自监督ViTs能够以超过90%的准确率解码IsSameObject信号，该信号编码在物体特征之上的低维子空间中，并主动引导注意力机制。消融该信号会降低下游任务性能。

Conclusion: ViTs确实能够自然涌现物体绑定能力，这种能力服务于预训练目标，挑战了ViTs缺乏物体绑定的观点，展示了连接主义系统如何自然获得"哪些部分属于一起"的符号知识。
...
TL;DR: ProMoE是一个用于扩散变换器（DiTs）的混合专家框架，通过两步路由器和显式路由指导来解决视觉MoE中的专家专业化问题，在ImageNet基准测试中超越了最先进的方法。
...
Method: 提出了ProMoE框架，包含两步路由器：1）条件路由根据功能角色将图像标记划分为条件和无条件集；2）原型路由通过可学习原型基于语义内容细化条件图像标记的分配。还提出了路由对比损失来增强原型路由过程。

Result: 在ImageNet基准测试上的广泛实验表明，ProMoE在Rectified Flow和DDPM训练目标下都超越了最先进的方法。

Conclusion: ProMoE通过显式路由指导成功解决了视觉MoE中的专家专业化问题，原型路由提供的基于相似度的专家分配机制为融入显式语义指导提供了自然途径，这对于视觉MoE至关重要。
...
TL;DR: URSA是一个离散视频生成框架，通过迭代全局优化离散时空标记，结合线性化度量路径和分辨率相关时间步偏移机制，实现高效的高分辨率图像合成和长视频生成，性能媲美连续扩散方法。
...
Method: URSA将视频生成任务表述为离散时空标记的迭代全局优化，包含两个关键设计：线性化度量路径和分辨率相关时间步偏移机制，并引入异步时间微调策略统一多种任务。

Result: 在具有挑战性的视频和图像生成基准测试中，URSA始终优于现有离散方法，并实现了与最先进连续扩散方法相当的性能。

Conclusion: URSA是一个简单而强大的离散生成建模框架，能够有效扩展到高分辨率图像合成和长持续时间视频生成，同时需要更少的推理步骤，性能与连续方法相当。
...
TL;DR: BitSkip是一个混合架构框架，研究发现简单的8位量化模型（BitSkip-V1）在质量上优于更复杂的4位和Hadamard增强版本，甚至能与全精度基线竞争，而Hadamard变换会严重降低性能。
...
Method: 引入BitSkip混合架构框架，系统研究不同量化精度（8位、4位）和Hadamard变换的组合效果。

Result: BitSkip-V1（8位量化无Hadamard变换）在困惑度上达到1.13，优于全精度基线的1.19；Hadamard变换即使使用8位精度也会导致性能下降超过37,000%；BitSkip-V1在第18层实现最优早期退出，获得32.5%速度提升，仅损失4%质量。

Conclusion: 简单的8位量化模型比复杂方法表现更好，Hadamard变换存在根本性的训练不稳定问题，BitSkip-V1提供了高效的早期退出策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有手语模型主要基于翻译员或孤立词汇数据，忽略了自然对话中的变异性。人类交流会根据语境和对话者动态调整，这在教育环境中尤其明显。

Method: 收集ASL STEM对话的运动捕捉数据集，使用连续运动学特征分析对话中的协调效应与个体努力减少，比较双人互动手语、单人讲座和翻译文章。

Result: 对话手语比孤立手语持续时间短24.6%-44.6%，在独白语境中未出现显著减少。通过运动学特征成功分离了对话特定协调与个体努力减少。

Conclusion: 研究连接了语言分析和计算建模，理解语用学如何塑造手语发音及其在手语技术中的表示，为开发更自然的手语模型提供了基础。
...
Method: 使用多数投票集成多个语言模型自动标注约4K训练样本，显著优于单模型标注；在基于共识和一致集成协议的子集上微调六个危机检测模型。

Result: 创建了包含临床标准定义的7种危机类型的基准测试，提供临床医生标注的评估和开发样本，以及自动标注的训练语料库。

Conclusion: CRADLE BENCH为心理健康危机检测提供了全面的基准测试，通过集成标注和不同协议标准训练的模型，为这一关键领域提供了重要工具。
...
TL;DR: 提出了一种轻量级框架，无需微调即可让冻结的大型语言模型分析纵向临床数据，在神经心理学评估中实现了准确可靠的性能，对早期阿尔茨海默病监测具有潜力。
...
Method: 开发了一个轻量级框架，将患者病史和上下文信息整合到语言模型空间中，通过集成患者历史数据来生成准确的预测。

Result: 在神经心理学评估中，即使使用最少的训练数据，该框架也能实现准确可靠的性能表现。

Conclusion: 该方法展示了在早期阿尔茨海默病监测方面的应用潜力，为临床数据分析提供了一种高效且资源友好的解决方案。
...
TL;DR: PerFine是一个无需训练的个性化文本生成框架，通过基于用户档案的迭代反馈和精炼来提升生成文本与目标用户风格、语气和主题的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强方法如LaMP和PGraphRAG虽然能利用用户历史生成文本，但在风格、语气和主题一致性方面存在漂移问题，需要更有效的个性化生成方案。

Method: 采用统一的批判-精炼框架：LLM生成器基于检索的用户档案生成初稿，批评者LLM基于相同档案提供结构化反馈（语气、词汇、句子结构、主题性），生成器进行修订，并通过淘汰策略保留更强版本。

Result: 在Yelp、Goodreads和Amazon数据集上，PerFine相比PGraphRAG在个性化方面持续提升，GEval指标增益达7-13%，经过3-5次精炼迭代后效果稳步改善，且随着批评者模型规模增大而扩展性良好。

Conclusion: 基于用户档案的事后反馈为个性化LLM生成提供了强大范式，该方法无需训练且与模型无关，能有效提升个性化文本生成质量。
...
TL;DR: AfriMTEB是一个针对非洲语言的多语言文本嵌入基准测试，覆盖59种语言、14个任务和38个数据集，包括6个新数据集。同时提出了AfriE5模型，通过跨语言对比蒸馏方法在非洲语言上实现了最先进的性能表现。


<details>
  <summary>Details</summary>
Motivation: 现有的多语言文本嵌入基准测试（如MMTEB）中非洲语言代表性不足，现有任务多是从翻译基准（如FLORES聚类或SIB-200）中重新调整用途而来。需要专门针对非洲语言构建更全面的评估基准。

Method: 1. 构建AfriMTEB基准：覆盖59种非洲语言、14个任务和38个数据集，包括6个新数据集，涵盖14-56种语言，并引入仇恨言论检测、意图检测和情感分类等新任务。2. 开发AfriE5模型：通过跨语言对比蒸馏方法，将指令调优的mE5模型适配到非洲语言。

Result: AfriE5模型在评估中实现了最先进的性能，超越了Gemini-Embeddings和mE5等强基线模型。

Conclusion: AfriMTEB为非洲语言提供了更全面的文本嵌入评估基准，而AfriE5模型通过跨语言对比蒸馏方法在非洲语言上取得了优异的性能表现，为非洲语言的NLP应用提供了重要支持。
...
TL;DR: 提出了R3框架，通过强化对比学习优化检索增强生成中的检索器，无需标注数据即可动态探索和优化相关性，在多种任务中显著提升RAG性能。
...
Method: R3框架通过试错反馈的强化对比学习，使检索器能够在RAG环境中动态探索和优化相关性，利用检索结果与环境交互产生的对比信号自动指导检索器自我改进。

Result: 在多样化任务上的实验表明，R3将RAG性能比原始检索器提升5.2%，比最先进检索器高出4.9%，与基于后训练或指令调优LLM的LLM增强检索和RAG系统效果相当。

Conclusion: R3框架高效实用，仅需4个GPU即可在一天内完成训练，为RAG系统提供了一种无需标注数据的有效检索优化方案。
...
TL;DR: 本文提出了一个新颖的增强框架，通过三个即插即用步骤来评估大型语言模型在公平性方面的脆弱性。研究发现LLMs对输入扰动很敏感，更容易表现出刻板偏见，特别是针对研究较少的社群。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型由于训练数据的判别性质而表现出刻板偏见，尽管已有方法试图避免使用刻板信息，但偏见对齐方法存在脆弱性。需要开发更稳健的公平性评估方法。

Method: 提出了一个包含三个即插即用步骤的增强框架，适用于多个公平性评估基准。通过将增强应用于BBQ公平性评估数据集来测试LLMs。

Result: 研究发现包括最先进的开源和闭源模型在内的LLMs对其输入扰动很敏感，更可能表现出刻板行为。模型对研究较少的社群更容易产生偏见行为。

Conclusion: 需要将公平性和安全性研究扩展到更多样化的社群，以解决LLMs对输入扰动的敏感性和对研究较少社群的偏见问题。
...
TL;DR: 提出了一种基于智能体的自动声明匹配方法，使用指令遵循的大语言模型，通过两阶段流程：先用LLM生成提示，再用LLM进行二元分类的声明匹配。


<details>
  <summary>Details</summary>
Motivation: 探索如何利用LLM自动生成提示来改进声明匹配任务，并研究不同规模LLM在提示生成中的表现，以节省计算资源。

Method: 采用两阶段流水线：1) 使用LLM生成提示；2) 使用LLM进行声明匹配的二元分类。支持使用不同LLM分别处理两个步骤。

Result: LLM生成的提示优于人工生成的SOTA方法，较小LLM在提示生成中表现与较大LLM相当，可节省计算资源。使用不同LLM分别处理两个步骤有效。

Conclusion: 该方法展示了LLM在自动提示生成和声明匹配中的有效性，为理解LLM对声明匹配任务的理解提供了见解，并提供了计算资源优化的可能性。
...
TL;DR: 提出了一种无需训练的大语言模型自动提示级联系统，用于电子商务中的产品质量评估，显著提升精度和召回率，同时大幅减少专家工作量。


<details>
  <summary>Details</summary>
Motivation: 解决在复杂工业目录中大规模应用语言模型进行产品质量评估时，传统方法需要大量领域专家标注和模型微调的问题，实现通用语言理解与领域特定知识的有效结合。

Method: 使用无训练级联方法，从人工制作的种子提示开始，自动生成和优化指令，逐步满足目录特定需求，无需训练标签或模型微调。

Result: 相比传统思维链提示，精度和召回率提升8-10%，领域专家工作量从每属性5.1小时减少到3分钟（减少99%），并在五种语言和多个质量评估任务中保持性能增益。

Conclusion: 该自动提示级联系统为大规模电子商务产品质量评估提供了一种高效、可扩展的解决方案，显著降低了人力成本并提升了评估性能。
...
TL;DR: 提出了一种基于连接组学的LLM框架，将动态fMRI连接性编码为时间序列，应用鲁棒归一化，并将这些数据映射到适合冻结预训练LLM的表示形式，用于临床预测。在早期阿尔茨海默病检测中，该方法实现了敏感预测，错误率远低于临床公认阈值。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够利用动态fMRI连接性数据进行早期阿尔茨海默病检测的方法，旨在实现比现有临床方法更敏感的预测，为及时干预提供可能性。

Method: 使用连接组学信息的LLM框架，将动态fMRI连接性编码为时间序列，应用鲁棒归一化技术，并将处理后的数据映射到适合冻结预训练LLM的表示空间。

Result: 在早期阿尔茨海默病检测任务中，该方法实现了高度敏感的预测性能，错误率显著低于临床公认的阈值。

Conclusion: 该连接组学LLM框架在早期阿尔茨海默病检测方面表现出色，错误率远低于临床标准，具有重要的临床干预意义。
...
TL;DR: 本文研究多语言大语言模型在遗忘学习过程中出现的语言混淆问题，发现仅使用英语数据进行知识擦除对多语言模型效果不佳，并提出N-Mix评分来量化语言混淆现象。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注性能指标，而忽略了多语言LLM在完全微调后出现的语言混淆问题，这导致基于参考的评估指标失效。

Method: 通过三个步骤：(1)引入N-Mix评分量化语言混淆现象；(2)证明当N-Mix评分高时，基于参考的指标会产生假阴性；(3)提出需要新的语义评估指标。

Result: 发现语言混淆在多语言LLM中普遍且一致存在，基于参考的评估方法在语言混淆严重时会给出错误的评估结果。

Conclusion: 需要开发能够直接评估生成内容语义的新型遗忘学习评估指标，即基于语义的评估方法。
...
TL;DR: 提出M-Eval方法，基于循证医学的异质性分析来检测RAG医疗问答系统中的事实错误，通过多源证据验证回答准确性，提升LLM可靠性并减少诊断错误。


<details>
  <summary>Details</summary>
Motivation: 当前基于检索增强生成的医疗问答系统存在生成错误信息（如幻觉）和未能正确使用外部知识的问题，需要提高系统的准确性和可靠性。

Method: 从外部知识库提取额外医学文献，检索RAG系统生成的证据文档，使用异质性分析检查证据是否支持回答中的不同观点，同时评估RAG系统提供证据的可靠性。

Result: 该方法在各种大型语言模型上实现了最高23.31%的准确率提升。

Conclusion: M-Eval方法能有效检测当前基于RAG的医疗系统中的错误，使LLM应用更加可靠，减少诊断错误。
...
TL;DR: PICOs-RAG方法通过将用户查询扩展和规范化成专业格式，使用EBM中的PICO格式提取关键信息，显著提升了检索效率和相关性，相比基线方法有8.8%的性能提升。
...
Method: 提出PICOs-RAG方法，将用户查询扩展和规范化成专业格式，并使用EBM中的PICO格式（一种搜索策略工具）提取用于检索的最重要信息。

Result: 该方法显著提升了检索效率和相关性，相比基线方法有8.8%的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 循证医学对证据质量有严格要求，但现有的RAG应用在区分高质量证据方面效率不足，需要更好的证据筛选方法。

Method: 采用多种循证医学方法模拟元分析，包括可靠性分析、异质性分析和外推分析，通过这些过程筛选最佳医学证据供LLMs诊断使用。

Result: 实验结果显示准确率最高提升11.4%，成功从PubMed数据集中提取更高质量和更可靠的证据。

Conclusion: 该方法能够减少错误知识注入到响应中，帮助用户获得更有效的回复，提升了循证医学应用中证据的质量和可靠性。
...
TL;DR: 提出TEXT2DB任务，强调信息抽取输出与目标数据库的集成，并开发OPAL框架来处理用户指令、文档集和数据库的交互。
...
Method: 提出OPAL框架，包含Observer、Planner和Analyzer三个组件，通过生成代码计划和调用IE模型来适应不同的数据库模式。

Result: OPAL能够成功适应不同的数据库模式，生成不同的代码计划并调用所需的信息抽取模型。

Conclusion: TEXT2DB任务和OPAL框架有效解决了信息抽取与数据库集成的挑战，但处理大型数据库和提取幻觉等复杂情况仍需进一步研究。
...
TL;DR: 提出了一种基于细粒度语义置信度奖励的强化学习框架，通过语义聚类引导LLMs在超出知识范围时拒绝回答，显著减少了幻觉问题。
...
Method: 采用强化学习框架，通过采样多个候选答案并进行语义聚类，训练LLM保留高置信度聚类中的答案，丢弃低置信度聚类中的答案，从而实现准确的事后拒绝。

Result: 该方法在领域内和领域外基准测试中显著提高了可靠性。

Conclusion: 提出的细粒度语义置信度奖励方法有效提升了LLMs在知识边界外的拒绝能力，减少了幻觉现象。
...
TL;DR: 提出Speculative Knowledge Distillation (SpecKD)框架，通过动态token级门控机制选择性应用蒸馏损失，避免学生模型学习教师不确定的预测，提升知识蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法对所有token均匀应用蒸馏损失，迫使学生模型学习教师不确定的高熵预测，这会引入噪声并损害学生模型性能，特别是在教师模型远大于学生模型时。

Method: 提出SpecKD框架，采用基于推测解码的"提议-验证"范式，在每个步骤验证学生token提议与教师分布的匹配度，仅对"接受"的token应用蒸馏损失，"拒绝"的token被屏蔽。
...
TL;DR: 该论文提出了一种训练大型多模态模型形成语言约定的方法，通过在模型间进行模拟参考游戏，使模型能够与人类进行高效沟通，减少信息长度达41%同时提高成功率15%。


<details>
  <summary>Details</summary>
Motivation: 人类通过共享对话上下文形成临时语言约定来实现高效沟通，但现有模型缺乏这种能力。研究旨在让大型多模态模型学会形成约定，实现更高效的沟通。

Method: 使用模拟参考游戏在模型之间进行训练，无需额外人工数据。在涉及照片和七巧板图像的重复参考游戏中训练模型形成约定。

Result: 模型能够与人类进行高效沟通：在交互过程中信息长度减少41%，成功率提高15%。人类听众与形成约定的模型交互时响应更快。仅基于成功或成本的训练不足，两者都必要才能引发约定形成。

Conclusion: 通过模拟参考游戏训练大型多模态模型形成语言约定是可行的，能够显著提高沟通效率和成功率，同时证明成功和成本两个因素对约定形成都至关重要。
...
TL;DR: Pie是一个可编程的LLM服务系统，通过将传统生成循环分解为细粒度服务处理器，让用户程序控制生成过程，支持自定义KV缓存策略和生成逻辑，在保持标准任务性能的同时显著提升智能体工作流的效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM服务系统基于单一令牌生成循环构建，无法适应多样化推理策略和智能体工作流的需求，限制了应用的灵活性和优化空间。

Method: 将传统生成循环分解为细粒度服务处理器，通过用户提供的inferlets程序控制生成过程，使用WebAssembly执行这些程序以提供轻量级沙箱环境。

Result: 在标准任务上性能与最先进系统相当（延迟开销3-12%），在智能体工作流上显著提升延迟和吞吐量（1.3x-3.4x更高）。

Conclusion: Pie系统通过可编程架构实现了LLM服务的灵活性和效率，支持应用特定优化，为复杂LLM应用提供了更好的服务支持。
...
TL;DR: 该论文提出了HalloMTBench，一个多语言幻觉检测基准，用于揭示大型语言模型在机器翻译中的幻觉问题，包含11种英语到其他语言的翻译方向，共5,435个高质量实例。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译基准无法充分暴露多语言大型语言模型中的幻觉问题，需要专门的诊断框架来识别和分类翻译失败模式。

Method: 构建了一个诊断框架，将幻觉分为指令脱离和源文本脱离两类，使用4个前沿LLM生成候选翻译，通过LLM评审团和专家验证来筛选高质量实例。

Result: 评估了17个LLM，发现了独特的"幻觉触发器"，包括模型规模、源文本长度敏感性、语言偏见以及强化学习放大的语言混合等失败模式。
...
TL;DR: Global PIQA是一个涵盖100多种语言和文化的常识推理基准，由来自65个国家的335名研究人员手动构建，包含116种语言变体，其中超过50%的示例涉及当地食物、习俗、传统等文化特定元素。
...
Method: 采用参与式方法，由来自65个国家的335名研究人员手动构建包含116种语言变体的常识推理基准，涵盖五大洲、14个语系和23种书写系统。

Result: 最先进的LLM在整体上表现良好，但在低资源语言中表现较弱（准确率差距高达37%，随机机会为50%），开源模型通常表现不如专有模型。
...
TL;DR: 本研究旨在记录和分析孟加拉语方言的语音和形态特征，探索为区域变体构建计算模型（特别是自动语音识别系统）的可行性，以促进方言多样性的保护和孟加拉语社区包容性数字工具的发展。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语虽然在南亚和侨民社区广泛使用，具有丰富的声音多样性，但对其方言的计算处理研究仍然有限。本研究旨在填补这一空白，推动方言多样性的保护和包容性语言技术的发展。

Method: 通过记录和分析孟加拉语方言的语音和形态特性，构建专门用于区域变体的计算模型，特别是自动语音识别系统，并创建了公开可用的数据集。

Result: 研究成功记录了孟加拉语方言的语音和形态特征，构建了针对区域变体的计算模型，并发布了用于公共使用的数据集。

Conclusion: 这项研究为孟加拉语方言的计算处理奠定了基础，展示了构建区域特定自动语音识别系统的可行性，有助于方言多样性的保护和孟加拉语社区包容性数字工具的发展。
...
Conclusion: Squrve为处理复杂真实世界查询开辟了新的有效途径，代码已在GitHub开源。
...
TL;DR: 本文提出了两种模式感知的文本过滤方法：模式感知行级去重（PLD）和模式感知尾随标点过滤（PTF），通过考虑文本行的序列分布来保留结构重要内容，相比传统方法提升了语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统行级过滤技术（如行级去重和尾随标点过滤）有时会丢弃有价值的内容，影响下游任务性能。本文旨在改进这些基础方法，保留更多结构重要的内容。

Method: 提出了PLD和PTF两种方法，不仅考虑行级信号，还考虑它们在文档中的序列分布模式，从而保留可能被传统方法移除的结构重要内容。

Result: 在英语和韩语的小型语言模型（10亿参数）上评估，结果显示这些方法在多项选择题基准上持续提升性能，并在SQuAD v1和KorQuAD v1的生成式问答任务中显著提高准确率。

Conclusion: 模式感知的过滤方法能够有效保留文本结构信息，相比传统过滤技术能更好地提升语言模型在下游任务中的表现。
...
TL;DR: Ko-MuSR是首个全面评估韩语长文本中多步软推理能力的基准，包含完全韩语的叙事、推理链和多项选择题，由人工验证逻辑一致性和可回答性。


<details>
  <summary>Details</summary>
Motivation: 构建一个最小化数据污染的韩语多步软推理基准，以系统评估韩语NLP中的长上下文推理能力。

Method: 遵循MuSR框架构建Ko-MuSR，包含韩语叙事、推理链和多项选择题，并由人工标注者验证逻辑一致性和可回答性。评估了四种大型语言模型（两种多语言和两种韩语专用），并设计了结合少样本示例、推理轨迹和任务特定提示的提示策略。
...
TL;DR: MuSaG是首个德语多模态讽刺检测数据集，包含33分钟德国电视节目片段，提供文本、音频和视频模态。研究比较了9个开源和商业模型的表现，发现人类主要依赖音频线索，而模型在文本上表现最佳，揭示了当前多模态模型与现实场景的差距。


<details>
  <summary>Details</summary>
Motivation: 讽刺作为一种复杂的比喻语言形式，在社交媒体和流行文化中普遍存在，对自然语言理解、情感分析和内容审核构成挑战。随着多模态大语言模型的出现，讽刺检测需要整合音频和视觉线索，但目前缺乏德语多模态数据集。

Method: 创建MuSaG数据集，包含来自德国电视节目的33分钟手动选择和人工标注的语句，每个实例提供对齐的文本、音频和视频模态。评估9个开源和商业模型在单模态和多模态设置下的性能，并与人类标注进行比较。

Result: 结果显示，在对话场景中人类主要依赖音频线索，而模型在文本模态上表现最佳。这突显了当前多模态模型与现实场景之间的差距，模型未能充分利用音频信息。
...
TL;DR: 本文研究了外部知识整合对自然语言生成的影响，通过创建KITGI基准测试，比较了在完整外部知识和过滤知识条件下使用T5-Large模型生成句子的效果。结果显示完整知识条件下的生成正确率高达91%，而过滤知识条件下骤降至6%。


<details>
  <summary>Details</summary>
Motivation: 探索外部知识整合在自然语言生成中的作用，特别是常识生成任务中外部知识的重要性，并设计可解释的知识增强NLG系统。

Method: 采用三阶段方法：(1)识别并移除关键知识，(2)重新生成句子，(3)手动评估输出的常识合理性和概念覆盖度。使用T5-Large模型在完整外部知识和过滤知识两种条件下进行句子生成比较。

Result: 完整知识条件下生成的句子在常识合理性和概念覆盖度方面达到91%的正确率，而过滤知识条件下性能急剧下降至6%。

Conclusion: 相关的外部知识对于保持NLG中的连贯性和概念覆盖度至关重要，需要设计可解释的知识增强NLG系统，并建立超越表面指标的评估框架来捕捉底层推理过程。
...
TL;DR: 本文提出了一种基于潜在空间语义对齐的大语言模型跨规模知识迁移方法，通过激活值而非直接使用层参数来实现有效的知识传递。


<details>
  <summary>Details</summary>
Motivation: 由于不同规模LLM之间存在神经不兼容性（架构和参数差异），现有直接重用层参数的方法受到严重限制，需要找到更有效的跨规模知识迁移方式。

Method: 采用激活值作为层间知识迁移的媒介，利用潜在空间中的语义对齐来实现跨规模知识传递，避免直接使用不兼容的层参数。

Result: 在四个基准测试上的评估证明了该方法的有效性，能够更好地在不同规模模型间对齐模型行为。

Conclusion: 潜在空间语义对齐是实现LLM跨规模知识迁移的关键前提，该方法为理解潜在语义对齐的本质提供了新的见解。
...
TL;DR: 提出基于知识和确定性两个维度的幻觉分类框架，通过模型特定数据集构建和激活操纵验证知识分类，发现不同模型存在不同的幻觉模式，并识别出模型拥有正确知识但仍自信产生幻觉的关键问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常根据幻觉的外部特征进行分类，而忽略了基于LLM内部特性的分类方法。这种外部关注忽视了幻觉可能需要根据其底层机制采用定制化缓解策略。

Method: 提出基于知识和确定性两个维度的幻觉分类框架，包括模型特定数据集构建过程，区分不同类型的幻觉。使用激活操纵验证知识分类，分析不同模型的知识和幻觉模式，并引入新的评估指标来衡量缓解方法在关键案例上的效果。

Result: 验证了知识分类框架的有效性，显示两种幻觉类型之间存在显著差异。发现不同模型尽管共享参数知识，但仍会出现不同的幻觉。识别出模型拥有正确知识但仍自信产生幻觉的关键子集，并发现某些缓解方法在平均表现良好但在这些关键案例上表现不佳。
...
TL;DR: 该研究探讨了如何通过推理和训练时选择来提高大型语言模型在医疗等敏感领域解释的忠实性，分析了少样本示例数量与质量、提示设计以及指令微调对模型忠实性的影响。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生不能忠实反映其预测驱动因素的解释，这在医疗环境中尤其成问题，因为省略重要临床线索或掩盖虚假捷径的解释会削弱临床医生的信任并导致不安全的决策支持。

Method: 评估了三种LLM（GPT-4.1-mini、LLaMA 70B、LLaMA 8B）在两个数据集（BBQ和MedQA）上的表现，操纵了少样本示例的数量和类型、提示策略以及训练过程。

Result: 结果显示：(i) 少样本示例的数量和质量显著影响模型忠实性；(ii) 忠实性对提示设计敏感；(iii) 指令微调阶段提高了在MedQA上测量的忠实性。
...
TL;DR: 该论文提出了一种结合文本和语音信息的多模态方法来解决阿拉伯语方言的变音符号恢复任务，使用CATT预训练模型的编码器处理文本，OpenAI Whisper基础模型的编码器处理语音，通过早期融合和交叉注意力两种策略集成多模态信息。
...
TL;DR: 本研究比较了5个大型语言模型在生成适合挪威5岁和9岁儿童的对话方面的表现，发现大多数模型生成的语言比目标年龄组更复杂，GPT-4和NorBloom-7b表现相对较好。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型主要基于成人对话数据训练，在生成真实、儿童化对话方面面临挑战，特别是在低资源语言环境中。

Method: 通过盲评方式，由11名教育专业人士评估真实儿童访谈数据和LLM生成文本的年龄适宜性和真实性，使用组内相关系数(ICC=0.75)评估评分者间信度。

Result: 评估者对较年轻儿童(5岁)的年龄预测准确率高于较年长儿童(9岁)，大多数模型生成的语言被认为比目标年龄组更复杂。

Conclusion: 研究强调了为儿童开发LLM系统时面临的关键数据相关挑战，特别是在低资源语言环境中缺乏全面年龄适宜词汇资源的问题。
...
TL;DR: 提出了一种自动生成高质量自然语言推理问题变体的方法MERGE，通过替换开放类词汇但保留底层推理逻辑来评估模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 手动创建NLI基准成本高昂，自动生成高质量变体困难，需要一种能保持推理逻辑的自动生成方法。

Method: 通过替换开放类词汇生成原始NLI问题的变体，同时确保保留底层推理逻辑。

Result: NLI模型在变体问题上的性能下降4-20%，表明即使在最小修改的问题上泛化能力也较低。
...
<details>
  <summary>Details</summary>
Motivation: 当前RLVR方法在组rollout过程中采样轨迹多样性有限，同质化轨迹会削弱策略更新的回报信号，阻碍有效策略学习。这种多样性不足主要源于token级随机采样容易导致相似的推理路径。

Method: LATR方法包含三个阶段：(1)在高不确定性生成步骤进行分支；(2)对每个新分支执行前瞻模拟；(3)在模拟过程中剪除持续相似的分支。通过强制分支到可能产生不同延续的候选token来显式促进轨迹级多样性。

Result: 相比随机采样，LATR在GRPO和DAPO算法上平均加速策略学习131%，在多种推理任务上提升最终pass@1性能4.2%。

Conclusion: LATR通过前瞻树结构rollout有效解决了RLVR中轨迹多样性不足的问题，显著提升了策略学习效率和最终性能。
...
TL;DR: Critique-RL是一种无需强监督的在线强化学习方法，通过两阶段优化策略训练批判语言模型，显著提升复杂推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖强监督者标注批判数据，限制了批判语言模型的发展。本文旨在开发无需强监督的批判模型训练方法。

Method: 采用两玩家范式：演员生成响应，批评家提供反馈，演员据此优化响应。使用两阶段优化：第一阶段通过基于规则的直接奖励增强批评家的判别能力；第二阶段引入基于演员优化的间接奖励提升批评家的帮助性，同时通过正则化保持判别能力。

Result: 在多个任务和模型上的广泛实验显示Critique-RL带来显著性能提升，例如Qwen2.5-7B在域内任务上获得9.02%增益，域外任务上获得5.70%增益。

Conclusion: Critique-RL证明了无需强监督即可有效训练批判语言模型的可行性，为复杂推理任务的LLM改进提供了有前景的途径。
...
TL;DR: 该论文提出了一种评估大语言模型在阿拉伯语方言和文化内容上表现的综合方法，通过翻译、转换问题格式、基准测试和思维链微调来评估模型性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在日常问题回答中应用日益广泛，但在文化基础和方言内容上的表现存在语言间差异，需要系统评估模型在阿拉伯语方言上的能力。

Method: 将现代标准阿拉伯语选择题翻译成英语和多种阿拉伯语方言，转换为开放式问题，在选择题和开放式问题设置下基准测试零样本和微调模型，并生成思维链推理进行模型微调。

Result: 模型在阿拉伯语方言上表现不佳，存在文化基础和方言知识的持续差距；阿拉伯语中心模型在选择题上表现良好但在开放式问题上表现不佳；思维链提高了判断正确性但n-gram指标结果不一。

Conclusion: 开发的数据集将公开发布以支持文化和语言包容性评估的进一步研究，揭示了模型在方言和文化内容理解方面的局限性。
...
TL;DR: LongWeave是一个平衡真实世界和可验证评估的长文本生成基准，通过约束验证器评估方法，在7个任务中支持自定义输入/输出长度，评估显示即使最先进的模型在长文本生成中仍面临挑战。


<details>
  <summary>Details</summary>
Motivation: 现有长文本生成基准要么使用难以验证的指标评估真实世界查询，要么使用简化评估但忽略真实世界复杂性的合成设置，需要平衡真实性和可验证性的评估方法。

Method: 提出约束验证器评估方法，首先在真实场景中定义可验证目标，然后系统生成相应查询、文本材料和约束，确保任务既真实又可客观评估。

Result: 在23个大型语言模型上的评估显示，随着真实世界复杂性和输出长度的增加，即使最先进的模型在长文本生成中也面临显著挑战。

Conclusion: LongWeave基准通过平衡真实性和可验证性，为长文本生成提供了更严格的评估框架，揭示了当前模型在复杂长文本生成任务中的局限性。
...
TL;DR: COMPEFFDIST是一个用于情感分析的综合高效蒸馏框架，通过属性自动指令构建和难度数据过滤解决现有方法的局限性，使3B学生模型在大多数任务上达到20倍大教师模型的性能，且仅需10%数据即可达到相同性能水平。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识蒸馏的情感分析方法面临两个关键挑战：手动编写指令多样性不足且数量有限，无法确保蒸馏知识的全面覆盖；大规模用户文本计算成本高，影响方法实用性。

Method: 提出COMPEFFDIST框架，包含两个关键模块：基于属性的自动指令构建和基于难度的数据过滤，分别解决指令多样性和数据效率问题。

Result: 在多个模型系列（Llama-3、Qwen-3和Gemma-3）上应用该方法，3B学生模型在大多数任务上匹配了20倍大教师模型的性能，且仅需10%数据即可达到相同性能水平，显著优于基线方法。
...
Method: 构建平行的真实映射世界和合成映射世界语料库，设计镜像任务（多跳问答和页面导航），在两种设置下评估模型：仅参数知识和知识增强。
...
TL;DR: LuxIT是一个为卢森堡语开发的单语指令调优数据集，旨在解决低资源语言环境下高质量训练数据不足的问题。通过使用DeepSeek-R1-0528从原生卢森堡语文本合成数据，并采用LLM-as-a-judge方法进行质量保证。虽然该数据集对卢森堡语自然语言处理有重要贡献，但在多个小规模LLM上的微调结果表现不一，表明需要进一步研究优化其应用。
...
Method: 1. 从原生卢森堡语语料库合成LuxIT数据集；2. 使用DeepSeek-R1-0528模型进行数据生成；3. 采用LLM-as-a-judge方法进行质量保证；4. 在多个小规模LLM上进行微调实验。

Result: 在卢森堡语语言能力考试上的基准测试结果参差不齐，不同模型的性能差异显著，表明数据集的实际效用存在局限性。

Conclusion: LuxIT对卢森堡语自然语言处理做出了重要贡献，并提供了一个可复制的单语方法学，但研究发现需要进一步研究来优化其应用效果。
...
TL;DR: 本文提出了一种新的对抗性改写任务，通过生成语义等效但语法正确的文本改写来降低多模态大语言模型在推理分割任务中的性能，并开发了SPARTA方法在文本自动编码器的语义潜空间中进行优化。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注图像输入的扰动，而语义等效的文本改写在现实应用中至关重要但研究不足，用户可能以不同方式表达相同意图。

Method: 提出了SPARTA方法——一种黑盒、句子级优化方法，在文本自动编码器的低维语义潜空间中运行，通过强化学习进行指导。

Result: SPARTA在ReasonSeg和LLMSeg-40k数据集上取得了显著更高的成功率，比先前方法高出最多2倍。

Conclusion: 先进的推理分割模型即使受到严格的语义和语法约束，仍然容易受到对抗性改写的攻击。
...
TL;DR: 本文提出了针对多语言或非英语使用场景的基准测试分类法，并为欧洲语言基准测试的协调发展制定了一套最佳实践和质量标准。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力的不断提升，现有基准测试主要关注英语，而在非英语语言中使用和评估LLMs仍然是一个研究较少的领域。

Method: 首先概述LLM基准测试的最新发展，然后提出专门针对多语言或非英语使用场景的基准测试分类法，并制定最佳实践和质量标准。

Result: 提出了新的基准测试分类法，强调提高评估方法的语言和文化敏感性。

Conclusion: 需要更加协调地开发欧洲语言基准测试，并提高评估方法的语言和文化敏感性。
...
<details>
  <summary>Details</summary>
Motivation: 在低资源语言、专业领域和受限部署环境中，大语言模型在有限数据下的微调面临实际挑战，需要高效且有针对性的微调技术。

Method: 系统回顾参数高效微调技术、领域和跨语言适应方法、模型专业化策略以及偏好对齐方法，强调样本和计算效率。

Result: 提供了经验权衡、选择标准和最佳实践，帮助研究人员和从业者根据任务约束选择合适的技术。

Conclusion: 为在数据和资源有限的情况下有效微调大语言模型提供了可操作的见解。
...
Method: 采用应用导向的能力增强视角，提出知识型和逻辑型幻觉分类，系统分析RAG和推理如何分别解决这两类问题，并建立统一框架。

Result: 提供了包含实际应用、评估和基准测试的统一框架，展示了RAG和推理增强在缓解幻觉方面的协同潜力。

Conclusion: RAG和推理增强的整合在Agentic系统中具有显著的协同效应，能够有效平衡创造性和可靠性，为缓解大语言模型幻觉提供了系统化解决方案。
...
Method: 提出了RPT任务，构建了Talk2Ref数据集，评估了零样本检索场景下的文本嵌入模型，设计了双编码器架构，并探索了处理长转录文本和领域适应的策略。
...
TL;DR: 该论文提出使用自然语言批评来增强大语言模型的置信度校准，研究了批评内容（不确定性vs置信度）和方法（自我批评vs批评校准训练），并提出了CritiCal方法，在复杂推理任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在高风险领域使用时需要准确的置信度校准，传统方法难以捕捉置信度评估所需的推理过程，而精确的黄金置信度标签难以获取且需要多次生成。

Method: 提出自然语言批评作为解决方案，研究批评内容（不确定性或置信度）和批评方法（自我批评或批评校准训练）。开发了Self-Critique方法和CritiCal训练方法，利用自然语言批评改进置信度校准。

Result: 实验表明CritiCal显著优于Self-Critique和其他竞争基线，在复杂推理任务中甚至超越了其教师模型GPT-4o。CritiCal在分布外设置中也表现出强大的泛化能力。

Conclusion: 自然语言批评是增强大语言模型置信度校准的有效方法，CritiCal方法显著提升了模型的可靠性，推动了LLM在安全关键应用中的可信度。
...
TL;DR: 该论文提出了一种适应零静默率目标的词性消歧方法，在Silberztein的INTEX系统中实现。该方法强调需要验证转换器路径的交互作用，而不能单独考虑，且当目标为零静默率时，局部语法必须仔细测试。


<details>
  <summary>Details</summary>
Motivation: 许多单词在词性上存在歧义，但在文本中这种歧义通常会减少。词性消歧是词汇标注的主要挑战之一，在自然语言处理中具有广泛应用。现有系统在词汇歧义或无法找到唯一正确解时会产生多个解，目标是保证零静默率（正确标签永远不会被丢弃），这对于唯一标注每个单词的系统来说是不现实的。

Method: 在Silberztein的INTEX系统中实现的词性消歧方法，使用局部消歧语法。该方法强调需要验证转换器路径的交互作用，不能单独考虑转换器路径；当使用多个转换器组合时，结果不能通过单独考虑来预测。

Result: 研究表明，要验证该框架中的局部消歧语法，单独考虑转换器路径是不够的，需要验证它们的交互作用。同样，如果使用多个转换器的组合，结果不能通过单独考虑来预测。
...
TL;DR: 该论文构建了一个包含故意糟糕幽默句子的新语料库，分析了标准幽默检测模型在此类文本上的表现，并研究了文学手法在糟糕幽默中的使用模式。


<details>
  <summary>Details</summary>
Motivation: 文本幽默极其多样化，计算研究需要涵盖各种类型，包括故意糟糕的幽默。通过分析Bulwer-Lytton小说竞赛的句子来更好地理解英语中的"糟糕"幽默。

Method: 收集并分析Bulwer-Lytton小说竞赛句子的新语料库，测试标准幽默检测模型的表现，分析文学手法的使用，并使用LLM生成竞赛风格的句子进行对比。

Result: 标准幽默检测模型在该语料库上表现不佳；这些句子结合了现有幽默数据集中常见的特征（双关、讽刺）与隐喻、元小说和明喻；LLM生成的句子模仿了形式但夸大了效果，过度使用某些文学手法，并包含比人类作家更多的新颖形容词-名词二元组。

Conclusion: 糟糕幽默具有独特的特征组合，需要专门的建模方法；LLM在模仿此类幽默时表现出形式上的相似性但缺乏自然的平衡感。
...
TL;DR: 本文介绍了开放韩语历史语料库，这是一个大规模、开放许可的数据集，涵盖1300年历史、6种语言和多种书写系统，包含1800万文档和50亿标记，用于定量分析韩语的语言变迁。


<details>
  <summary>Details</summary>
Motivation: 韩语历史中存在口语和书面语形式之间的差异以及从汉字到韩文字母的关键转变，但由于缺乏可访问的历史语料库，这一语言演变在自然语言处理领域尚未得到充分探索。

Method: 构建开放韩语历史语料库，该数据集跨越1300年、包含6种语言和多种书写系统，涵盖19个来源，从7世纪到2025年。利用该资源定量分析主要语言变迁。

Result: 定量分析发现：(1)吏读使用在1860年代达到顶峰后急剧下降；(2)从汉字到韩文的转变始于1890年左右，是一个快速转型；(3)朝鲜的词汇分歧导致现代分词器产生高达51倍的词汇外率。

Conclusion: 该工作通过捕捉韩语历史为定量历时分析提供了基础资源，可作为大型语言模型的预训练语料库，可能提高其对现代韩文中的汉字词以及古旧书写系统的理解能力。
...
TL;DR: BEARD框架通过结合BEST-RQ目标和知识蒸馏，使用未标注数据对Whisper编码器进行领域自适应，在航空交通管制语音识别任务中显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言ASR系统经过大规模训练，但在领域外和低资源场景下表现不佳，特别是当标注数据稀缺时。航空交通管制通信领域具有非母语语音、噪声和专业术语等挑战。

Method: 提出BEARD框架，结合BEST-RQ自监督学习目标和冻结教师编码器的知识蒸馏，使用约5000小时未标注语音进行编码器自适应，然后用2小时标注语音进行微调。

Result: 在ATCO2语料库上，BEARD显著优于之前基线和微调模型，相比微调模型实现了12%的相对改进。

Conclusion: 这是首个使用自监督学习目标进行Whisper领域自适应的工作，证明了BEARD在低资源领域自适应任务中的有效性。
...
TL;DR: 本文提出ReForm方法，通过将语义一致性评估集成到自动形式化过程中，使模型能够迭代生成形式化语句、评估语义保真度并通过渐进细化自我纠正错误。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在自动形式化任务中虽然能生成语法正确的形式化语句，但往往无法保持原始问题语义意图的问题。现有方法将自动形式化视为简单翻译任务，缺乏人类专家自然使用的自我反思和迭代优化机制。

Method: 提出ReForm反射式自动形式化方法，结合语义一致性评估；引入前瞻有界序列优化(PBSO)训练方法，在不同序列位置使用不同奖励，确保模型既发展准确的自动形式化能力，又发展正确的语义验证能力。

Result: 在四个自动形式化基准测试中，ReForm相比最强基线平均提升了17.2个百分点；通过ConsistencyCheck基准验证，发现自动形式化本身具有挑战性，人类专家在高达38.5%的情况下也会产生语义错误。

Conclusion: ReForm方法通过集成语义评估和迭代优化，显著提升了自动形式化的性能；同时揭示了自动形式化任务的固有难度，为未来研究提供了重要基准。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散大语言模型存在固定生成长度的问题，需要在解码前确定生成长度作为超参数，这导致效率低下和灵活性不足。
...
Conclusion: 该方法实现了更高准确度和更快推理速度，使扩散大语言模型超越了单纯的学术新颖性，支持其在现实世界应用中的实际使用。
...
<details>
  <summary>Details</summary>
Motivation: 现有静态稀疏方法（如滑动窗口或全局token）由于静态性无法适应注意力中内容依赖的变化，而动态方法依赖预定义模板或启发式机制，降低了通用性并可能剪枝掉上下文重要的token，限制了在不同任务上的准确性。

Method: DHSA动态地将序列分割为变长块，通过长度归一化聚合计算块表示（缩放平均嵌入的平方根），然后将块级相似度上采样到token级相似度来计算重要性分数，决定保留哪些token级交互。

Result: 在Gemma2上的Needle-in-a-Haystack测试和LongBench实验中，DHSA在精度上匹配密集注意力，同时将预填充延迟降低20-60%，峰值内存使用降低35%。与块稀疏注意力等基线相比，DHSA在可比或更低成本下实现6-18%的相对精度提升。
...
TL;DR: 本研究系统评估了三种基于前缀的方法在零样本跨语言迁移中的表现，发现前缀方法在Llama 3.1 8B和Mistral v0.3 7B模型上优于LoRA基线，在Belebele基准上提升达6%，且仅需1.23M学习参数。


<details>
  <summary>Details</summary>
Motivation: 随着Llama和Mistral等大型语言模型的发布，零样本跨语言迁移变得可行，但将这些仅解码器模型适应跨语言任务仍具挑战性。虽然LoRA等参数高效微调技术广泛使用，但基于前缀的技术在仅解码器模型中的零样本迁移潜力尚未充分探索。

Method: 对三种基于前缀的方法（软提示调优、前缀调优和Llama Adapter）进行综合研究，评估从英语到35+种高资源和低资源语言的零样本跨语言迁移，分析跨语言家族和文字的影响，以及模型规模从1B到24B的扩展效应。

Result: 在Llama 3.1 8B上，前缀方法比LoRA基线在Belebele基准上提升达6%。Mistral v0.3 7B也观察到类似改进。尽管前缀调优仅使用1.23M学习参数，但在多样化基准上实现了持续改进。

Conclusion: 基于前缀的技术作为LoRA的有效且可扩展替代方案具有潜力，特别是在低资源多语言环境中。
...
TL;DR: 本文提出了相对缩放定律，通过跟踪不同测试分布之间性能差距随规模的变化，揭示了缩放并非普遍均衡器，不同领域表现出不同的收敛轨迹。


<details>
  <summary>Details</summary>
Motivation: 传统的缩放定律在聚合测试集上测量，平均了异构子群体，掩盖了性能差异。本文旨在研究缩放如何影响不同测试分布之间的相对性能差距。

Method: 使用255个解码器Transformer模型，在匹配计算预算（10^18-10^20 FLOPs）下训练，分析MMLU学术领域、地区英语方言和AI风险行为等不同测试分布的性能轨迹。

Result: 发现多样化的轨迹：学术领域趋于均衡；地区方言表现取决于人口规模；AI风险行为分裂，能力和影响力相关风险在预训练中增加，而对抗性风险不变。

Conclusion: 虽然缩放提高了整体性能，但并非普遍均衡器。作者发布所有模型检查点以支持进一步研究相对缩放定律与传统缩放定律的对比分析。
...
<details>
  <summary>Details</summary>
Motivation: 现有奖励模型在知识密集型和长文本任务上表现不佳，因为评估正确性需要超出模型内部知识的基础，特别是在需要外部证据时难以可靠区分细微质量差异。

Method: 提出OpenRM工具增强长文本奖励模型，使用Group Relative Policy Optimization在27K+合成成对示例上训练，训练目标联合监督中间工具使用和最终结果准确性，学习基于证据的判断策略。

Result: 在三个新收集的数据集和两个广泛使用的基准测试上，OpenRM显著优于现有奖励建模方法，在推理时回答选择和训练时数据选择中集成OpenRM都能在下游LLM对齐任务中获得一致收益。

Conclusion: 工具增强奖励模型具有扩展可靠长文本评估的潜力，能够有效提升大语言模型的对齐效果。
...
TL;DR: 本研究通过大规模自然阅读数据发现，阅读障碍者在词汇长度、频率和可预测性三个特征上都表现出更强的敏感性，其中可预测性的影响最大。通过反事实操作这些特征，阅读障碍与控制组的差距可缩小约三分之一。


<details>
  <summary>Details</summary>
Motivation: 探究阅读障碍者在自然阅读过程中何时以及何种条件下产生额外的阅读成本，识别影响阅读障碍的关键词汇特征。

Method: 使用眼动追踪技术，结合词汇层面的特征（词长、频率和可预测性），建模这些特征对阅读障碍者阅读时间的影响。

Result: 所有三个特征都显著影响典型读者和阅读障碍者的阅读时间，阅读障碍者对每个特征都表现出更强的敏感性，特别是可预测性。反事实操作显示这些特征能显著缩小阅读障碍与控制组的差距。
...
Method: 采用两阶段的MQM评估范式，让标注者审查和编辑现有的MQM标注（可能来自自己、其他人类标注者或自动MQM标注系统）。

Result: 重新标注行为符合预期目标，并且能产生更高质量的标注，主要原因是发现了第一次标注时遗漏的错误。
...
TL;DR: 该研究评估了提示式角色扮演(PBRP)对大型语言模型医学推理能力的影响，发现角色提示主要影响表面语言特征而非核心推理能力。
...
Method: 引入RP-Neuron-Activated评估框架(RPNA)，在三个医学QA数据集上使用神经元消融和表示分析技术来评估推理路径的变化。
...
TL;DR: SPICE是一个强化学习框架，通过单一模型扮演挑战者和推理者两个角色，利用大型语料库中的文档生成多样化推理任务，实现持续的自我改进。
...
Method: SPICE框架中，单一模型扮演两个角色：挑战者从大型语料库中挖掘文档生成多样化推理任务，推理者解决这些任务。通过对抗性动态，挑战者在推理者能力边界创建自动课程。
...
TL;DR: 本文提出了WebLeaper框架，通过构建高覆盖度的信息搜索任务和生成高效解决方案轨迹，解决了当前LLM智能体在信息搜索中效率低下的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的信息搜索智能体存在搜索效率低的问题，这限制了整体性能。主要原因是训练任务中目标实体稀疏，限制了智能体学习和泛化高效搜索行为的机会。

Method: 将信息搜索建模为树结构推理问题，在受限上下文中嵌入更多目标实体；利用维基百科表格提出三种任务合成变体（Basic、Union、Reverse-Union）；通过保留同时准确且高效的训练轨迹来优化模型。

Result: 在五个信息搜索基准测试（BrowserComp、GAIA、xbench-DeepSearch、WideSearch、Seal-0）上的广泛实验表明，该方法在基本和综合设置下均能持续提升效果和效率。

Conclusion: WebLeaper框架通过系统性地提高信息搜索任务的覆盖度和效率，显著提升了LLM智能体在信息搜索任务中的表现，在效果和效率方面均优于强基线方法。
...
TL;DR: ComboBench是一个评估大型语言模型将语义动作转换为VR设备操作序列能力的基准测试，涵盖4个流行VR游戏的262个场景。测试了7个LLM，发现虽然顶级模型如Gemini-1.5-Pro表现出强大的任务分解能力，但在程序推理和空间理解方面仍不如人类。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型是否能够像人类一样，基于常识和具身理解将高级语义动作转换为VR设备的具体操作序列，这一能力在VR游戏交互中尚未得到充分研究。

Method: 创建ComboBench基准测试，包含来自Half-Life: Alyx、Into the Radius、Moss: Book II和Vivecraft四个VR游戏的262个场景，评估7个LLM将语义动作转换为设备操作序列的能力，并与人工标注的真实数据和人类表现进行比较。

Result: 顶级模型如Gemini-1.5-Pro展现出强大的任务分解能力，但在程序推理和空间理解方面仍显著落后于人类。不同游戏间的性能差异明显，表明对交互复杂性敏感。少量示例能显著提升性能。

Conclusion: LLM在VR设备操作序列转换方面展现出潜力，但程序推理和空间理解仍是主要挑战。少量示例学习可有效提升性能，为针对性增强LLM的VR操作能力提供了方向。
...
TL;DR: 本文介绍了WMT25翻译评估共享任务的两个提交系统：MetricX-25用于质量评分预测，GemSpanEval用于错误跨度检测，均基于Gemma 3模型进行微调。
...
Conclusion: 基于Gemma 3的两种方法在翻译质量评估任务中表现优异，证明了生成式方法在错误跨度检测中的有效性。
...
TL;DR: 本文扩展了CDMizer框架，通过模板驱动的方法将非结构化法律合同转换为标准化的CDM格式，使用较小的开源LLM在CSA条款提取任务中取得了与大型专有模型相竞争的性能。
...
Method: 扩展CDMizer框架，采用模板驱动解决方案，确保在合同到CDM转换过程中的语法正确性和CDM模式遵从性，并与显著较小的开源大型语言模型集成。

Result: 在与ISDA开发的CSA条款提取基准比较中，CDMizer在准确性和效率方面取得了具有竞争力的性能，能够与大型专有模型相媲美。
...
<details>
  <summary>Details</summary>
Motivation: 预排序阶段在大规模推荐系统中面临模型表达能力与计算效率的内在权衡。传统的轻量级双塔架构虽然计算效率高但估计能力有限，难以捕捉候选物品间的复杂协同和抑制关系，且会放大样本选择偏差问题。

Method: 提出DUET框架，采用集合级预测方法在单次前向传播中对整个候选子集进行预测，实现候选物品间的信息感知交互并分摊计算成本。同时使用双模型协同训练机制，通过相互伪标签优化将监督扩展到未曝光物品。

Result: 经过广泛的离线实验和在线A/B测试，DUET持续优于最先进的基线方法，并在多个核心业务指标上取得改进。目前已在快手和快手极速版应用中全面部署，为数亿用户的主要流量提供服务。

Conclusion: DUET成功实现了在严格计算预算下的表达性建模，通过集合级预测和双模型协同训练有效解决了预排序阶段的表达能力和样本选择偏差问题。
...
TL;DR: 该论文提出了一种新颖的多阶段RAG架构，利用LLM生成的元数据来处理长结构化财务文件中的稀疏和交叉引用证据问题，通过上下文丰富的文档块和元数据增强技术显著提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 传统RAG在处理长结构化财务文件时表现不佳，因为相关证据稀疏且交叉引用复杂，需要更先进的元数据驱动方法来提升检索精度。

Method: 提出多阶段RAG架构，包括LLM生成的元数据索引管道、预检索过滤、后检索重排序和增强嵌入技术，特别强调将块元数据直接与文本嵌入的"上下文块"方法。

Result: 研究发现强大的重排序器对精度至关重要，但最大的性能提升来自上下文块嵌入。提出的最优架构结合了LLM驱动的预检索优化和上下文嵌入，实现了卓越性能。

Conclusion: 该研究为构建稳健的元数据感知RAG系统提供了蓝图，特别适用于财务文档分析，并在性能峰值和操作效率之间提供了实用的权衡方案。
...
TL;DR: 提出一个使用大语言模型从时间戳和粗略位置生成地理时间嵌入的可扩展框架，捕捉节假日、季节趋势和本地/全球事件，并通过两种方法将地理时间嵌入整合到序列模型中。


<details>
  <summary>Details</summary>
Motivation: 大多数推荐系统将时间戳视为数值或循环值，忽略了现实世界背景如节假日、事件和季节模式，需要更好地捕捉这些上下文信息。

Method: 使用大语言模型生成地理时间嵌入，提出地理时间嵌入信息性测试作为轻量级诊断，通过直接特征融合或辅助损失将地理时间嵌入整合到序列模型中。

Result: 在MovieLens、LastFM和生产数据集上证明这些嵌入提供了与完整模型集成结果一致的预测信号。

Conclusion: 研究结果强调了自适应或混合推荐策略的必要性，并发布了上下文丰富的MovieLens数据集以支持未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够跨异构领域（操作系统、网页、模拟游戏）工作的通用游戏智能体，解决API或GUI方法的局限性，实现大规模持续预训练。

Method: 使用统一、可扩展的动作空间，基于人类键盘鼠标输入；采用衰减持续损失减少因果混淆；使用高效的稀疏思维策略平衡推理深度和推理成本；在500B tokens的多模态数据上进行预训练。

Result: 在开放世界Minecraft任务中成功率比之前最先进模型提高约2倍；在未见过的网页3D游戏中接近人类新手水平；在FPS基准测试中超越GPT-5、Gemini-2.5-Pro和Claude-4-Sonnet；训练时间和测试时间的扩展结果证实统一动作空间在跨游戏和多模态数据扩展时的持续改进。
...
<details>
  <summary>Details</summary>
Motivation: 研究AI在科学问题解决中的角色，特别关注其对学科创造力的潜在影响，探讨AI是否能够真正增强科学创造力。

Method: 基于创造力哲学理论，区分创造性方法和创造性产品，引入学科创造力概念，并通过两个数学案例进行实证分析。

Result: 研究发现，虽然计算可以扩展学科创造力，但某些涉及AI的方法可能会取代学科创造力，从而可能改变科学追求的价值。

Conclusion: AI在科学中的应用可能对学科创造力产生双重影响：既能扩展也可能取代，这种位移可能改变科学追求的价值和意义。
...
TL;DR: 本文提出了多环境POMDPs（ME-POMDPs）和对抗信念POMDPs（AB-POMDPs）的概念，用于处理具有离散模型不确定性的POMDP问题，并开发了精确和近似算法来计算鲁棒策略。


<details>
  <summary>Details</summary>
Motivation: 当多个领域专家对如何建模问题存在分歧时，会产生共享相同状态、动作和观测空间但在转移、观测和奖励模型上可能任意变化的POMDP集合。目标是找到对集合中任何POMDP都鲁棒的单一策略。

Method: 首先将ME-POMDPs推广到具有初始信念集合的AB-POMDPs；然后证明任意ME-POMDP可以简化为仅在转移和奖励函数或仅在观测和奖励函数上变化的ME-POMDP；最后设计精确和近似（基于点）算法来计算鲁棒策略。

Result: 能够为标准POMDP基准扩展到多环境设置计算策略，证明了所提方法的有效性。

Conclusion: ME-POMDPs和AB-POMDPs为处理模型不确定性提供了有效框架，所开发的算法能够计算在多环境设置下的鲁棒策略。
...
### [127] [Test-Time Tuned Language Models Enable End-to-end De Novo Molecular Structure Generation from MS/MS Spectra](https://arxiv.org/abs/2510.23746)
...
<details>
  <summary>Details</summary>
Motivation: 当前串联质谱分析方法依赖数据库匹配或需要中间片段预测的多步骤流程，难以识别参考数据库中不存在的化合物。需要开发能够直接从质谱数据生成分子结构的端到端方法。

Method: 利用测试时调优增强预训练transformer模型，直接从串联质谱和分子式进行端到端从头分子结构生成，绕过手动注释和中间步骤。

Result: 在NPLIB1和MassSpecGym两个基准测试上分别比现有最佳方法DiffMS提升了100%和20%。测试时调优在实验质谱上实现动态适应，比传统微调性能提升62%。

Conclusion: 该框架能够有效生成结构准确的分子候选物，为人类解释提供有价值指导，实现更可靠的化合物识别，特别是在处理未知化合物时表现出色。
...
### [128] [Evaluating In Silico Creativity: An Expert Review of AI Chess Compositions](https://arxiv.org/abs/2510.23772)
...
TL;DR: 该研究探讨生成式AI在象棋谜题创作中的创造力，开发了一个能生成具有美学吸引力、新颖性、反直觉和独特解法的AI系统，并邀请国际象棋专家评估其创意水平。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速发展，人们对其产生创意和新颖输出的能力存在疑问。本研究旨在探索AI在象棋谜题创作领域的创造力表现。

Method: 开发了一个专门生成象棋谜题的AI系统，该系统能产生具有美学吸引力、新颖性、反直觉和独特解法的谜题。通过向三位世界知名象棋专家（国际象棋排局大师Amatzia Avni、特级大师Jonathan Levitt和Matthew Sadler）展示AI生成的谜题选集，让他们评选最喜欢的谜题并解释其吸引力所在。

Result: 三位专家对AI生成的象棋谜题进行了评估，从创意性、挑战水平和美学设计等维度分析了谜题的吸引力。

Conclusion: 该研究表明生成式AI在象棋谜题创作领域展现出一定的创造力，能够产生被专家认可的具有美学价值和创意性的作品。
...
### [129] [Why Foundation Models in Pathology Are Failing](https://arxiv.org/abs/2510.23807)
*Hamid R. Tizhoosh*

Main category: cs.AI

TL;DR: 病理学基础模型在癌症诊断和预后方面表现不佳，存在诊断准确率低、鲁棒性差、计算需求大等根本性弱点，源于通用基础模型假设与人体组织复杂性的概念不匹配。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在计算机视觉和语言处理领域取得了革命性突破，但在计算病理学中的快速应用并未带来预期的癌症诊断和预后突破，反而暴露了系统性弱点。

Method: 通过系统评估分析病理学基础模型的性能缺陷，识别出七个相互关联的根本原因：生物复杂性、无效的自监督学习、过度泛化、过度架构复杂性、缺乏领域特定创新、数据不足以及与组织块大小相关的根本设计缺陷。

Result: 研究发现当前病理学基础模型在诊断准确性、鲁棒性、几何稳定性等方面存在显著不足，计算需求大且存在安全漏洞，无法有效处理人体组织的内在复杂性。

Conclusion: 当前病理学基础模型在概念上与组织形态学性质不匹配，需要对整个范式进行根本性重新思考，开发更适合病理学领域特点的新方法。
...
### [130] [ReCAP: Recursive Context-Aware Reasoning and Planning for Large Language Model Agents](https://arxiv.org/abs/2510.23822)
*Zhenyu Zhang,Tianyi Chen,Weiran Xu,Alex Pentland,Jiaxin Pei*

Main category: cs.AI

TL;DR: ReCAP是一个用于大语言模型的分层推理规划框架，通过计划分解、父计划结构化重注入和内存高效执行机制，解决了长时程任务中的上下文漂移和递归失败问题。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在多步推理和动态重规划任务中的挑战，特别是顺序提示方法容易出现的上下文漂移、目标信息丢失和循环失败问题，以及分层提示方法存在的跨级连续性弱和运行时开销大的问题。

Method: ReCAP框架包含三个关键机制：(1) 计划前瞻分解：生成完整子任务列表，执行第一项并优化剩余任务；(2) 结构化父计划重注入：在递归返回时保持多级上下文一致性；(3) 内存高效执行：限制活动提示，使成本随任务深度线性扩展。

Result: 在多个长时程推理基准测试中，ReCAP显著提高了子目标对齐度和成功率，在严格pass@1协议下，同步Robotouille任务获得32%提升，异步Robotouille任务获得29%改进。

Conclusion: ReCAP通过将高层目标与低层行动对齐、减少冗余提示和保持跨递归的连贯上下文更新，有效提升了长时程推理任务的性能。
...
### [131] [Decentralized Multi-Agent Goal Assignment for Path Planning using Large Language Models](https://arxiv.org/abs/2510.23824)
*Murad Ismayilov,Edwin Meriaux,Shuo Wen,Gregory Dudek*

Main category: cs.AI

TL;DR: 本文研究了多智能体路径规划中的去中心化目标分配问题，比较了贪心启发式、最优分配和基于大语言模型的智能体方法，发现LLM智能体在适当提示下能达到接近最优的性能。
...
Method: 智能体基于环境结构化表示独立生成目标偏好排序，然后交换排序信息，通过固定的确定性冲突解决规则进行目标分配，无需协商或迭代协调。系统比较了贪心启发式、最优分配和LLM智能体方法。
...
### [132] [Generating Creative Chess Puzzles](https://arxiv.org/abs/2510.23881)
...
TL;DR: 本文提出了一种基于强化学习的象棋谜题生成方法，通过设计基于象棋引擎搜索统计的新型奖励机制，显著提升了生成谜题的独特性、反直觉性、多样性和真实性。


<details>
  <summary>Details</summary>
Motivation: 尽管生成式AI在各个领域快速发展，但生成真正具有创造性、审美价值和反直觉性的输出仍然是一个挑战，特别是在象棋谜题领域。

Method: 首先对生成式AI架构进行基准测试，然后引入一个强化学习框架，该框架基于象棋引擎搜索统计设计新颖的奖励机制，旨在增强谜题的独特性、反直觉性、多样性和真实性。

Result: 强化学习方法将反直觉谜题的生成率从0.22%（监督学习）大幅提升至2.5%，超过了现有数据集（2.1%）和最佳Lichess训练模型（0.4%）。生成的谜题符合新颖性和多样性基准，保留了审美主题，并被人类专家评为比传统书籍谜题更具创造性、趣味性和反直觉性，甚至接近经典作品水平。

Conclusion: 最终成果是一本经过精心策划的AI生成谜题集，获得了三位世界知名专家的创造力认可，证明了该方法在生成高质量象棋谜题方面的有效性。
...
### [133] [Hybrid Modeling, Sim-to-Real Reinforcement Learning, and Large Language Model Driven Control for Digital Twins](https://arxiv.org/abs/2510.23882)
*Adil Rasheed,Oscar Ravik,Omer San*

Main category: cs.AI

TL;DR: 本文研究数字孪生在动态系统建模与控制中的应用，比较了物理模型、数据驱动和混合建模方法，以及传统与AI驱动的控制器。在温室测试平台上，评估了四种预测模型和三种控制策略的性能权衡。


<details>
  <summary>Details</summary>
Motivation: 探索数字孪生技术在动态系统建模与控制中的综合应用，旨在比较不同建模方法和控制策略在精度、适应性和实施成本方面的表现。

Method: 使用微型温室作为测试平台，开发了四种预测模型（线性、物理建模、LSTM、混合分析建模）和三种控制策略（MPC、强化学习、LLM控制），在插值和外推场景下进行比较评估。

Result: HAM模型在准确性、泛化能力和计算效率方面表现最均衡；LSTM精度高但资源消耗大；MPC控制器性能稳健可预测；RL控制器适应性强；LLM控制器结合预测工具可实现灵活的人机交互。

Conclusion: 混合分析建模方法在建模中表现最佳，而不同控制器各有优势：MPC稳健、RL适应性强、LLM支持灵活交互，为数字孪生系统设计提供了多样化选择。
...
TL;DR: 本文提出了一种基于变分推理的链式思维训练方法，通过稀疏奖励函数和贝叶斯推理扩展策略，提升大型视觉语言模型在推理任务中的性能、泛化性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有训练方法（如SFT、PPO、GRPO）在未见推理任务上泛化能力不足，且过度依赖有偏的奖励模型，限制了链式思维推理的效果。

Method: 将LVLMs中的推理重新表述为后验推断问题，采用摊销变分推理的扩展训练算法，引入稀疏奖励函数进行token级学习，并使用贝叶斯推理扩展策略替代昂贵的搜索方法。

Result: 在七个推理基准测试中，该方法显著提升了最先进LVLMs的性能，在效果、泛化性和可解释性方面均有改进。

Conclusion: 所提出的基于变分推理的训练算法能够有效提升LVLMs的推理能力，克服了现有方法的局限性，为模型的可解释性和可靠性提供了新思路。
...
TL;DR: 提出了一种基于直觉主义分散化框架的因果发现理论——柔道演算，使用j-稳定因果推断和j-演算在层拓扑中形式化定义，能够处理现实应用中因果效应依赖于不同情境的问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界应用（从生物学到医学和社会科学）中，因果效应通常依赖于特定情境（如年龄、国家、剂量、基因型或实验室协议），需要一种能够形式化这种情境依赖性的因果发现方法。

Method: 使用柔道演算，将情境依赖性形式化为局部真值：因果声明在情境覆盖上被证明为真，而不是全局成立。结合标准的基于分数、约束和梯度的因果发现方法，实现算法框架。

Result: 在从合成到真实世界数据集（生物学和经济学）的实验结果显示，基于层拓扑的分散化因果发现具有计算效率优势，并且性能优于经典因果发现方法。

Conclusion: 柔道演算提供了一种构造性和一致性的因果发现框架，能够有效处理情境依赖性因果问题，在计算效率和性能方面都表现出优势。
...
### [136] [The Sign Estimator: LLM Alignment in the Face of Choice Heterogeneity](https://arxiv.org/abs/2510.23965)
...
<details>
  <summary>Details</summary>
Motivation: 传统LLM对齐方法对人类偏好的异质性很敏感，基于成对比较数据的朴素概率模型会产生不一致的总体平均效用估计，这是社会福利的规范度量。

Method: 提出符号估计器方法，在聚合步骤中用二元分类损失替换交叉熵损失，在温和假设下实现一致的对齐，并获得多项式有限样本误差界。

Result: 在基于数字孪生的LLM对齐现实模拟中，符号估计器显著减少了偏好扭曲，将角度估计误差降低了近35%，与真实总体偏好的不一致性从12%降至8%。
...
### [137] [Learning Individual Movement Shifts After Urban Disruptions with Social Infrastructure Reliance](https://arxiv.org/abs/2510.23989)
*Shangde Gao,Zelin Xu,Zhe Jiang*

Main category: cs.AI

TL;DR: 本研究开发了一个条件深度学习模型，通过整合个体的社会基础设施韧性(SIR)和空间上下文，来预测破坏性事件后个体移动模式的变化。
...
Method: 构建了一个条件深度学习模型，将个体的社会基础设施韧性(SIR)整合到模型中，利用大规模稀疏的个体级数据来捕捉个体移动模式与局部空间上下文之间的复杂关系。

Result: 实验表明，整合个体的SIR和空间上下文能够增强模型预测事件后个体移动模式的能力。条件模型能够捕捉到具有相似事件前模式但SIR不同的个体之间移动模式的差异变化。

Conclusion: 通过将个体的社会基础设施韧性纳入深度学习模型，可以有效预测破坏性事件后个体移动模式的变化，为理解个体在危机中的行为响应提供了新视角。
...
TL;DR: OneCast是一个跨域时间序列预测框架，通过将时间序列分解为季节性和趋势分量，分别使用轻量级投影模块和基于离散扩散的语义感知标记化方法进行建模，实现了在多个领域的有效泛化。


<details>
  <summary>Details</summary>
Motivation: 现有跨域时间序列预测方法在处理异构数据时面临挑战，特别是在面对领域特定趋势变化和不一致周期模式时表现不佳，主要问题在于将时间序列视为未分化的序列而未显式解耦其内在结构组件。

Method: 提出OneCast框架：1）将时间序列分解为季节性和趋势分量；2）季节性分量通过轻量级投影模块使用可解释基函数重建周期模式；3）趋势分量通过语义感知标记器在片段级别编码为离散标记，并通过掩码离散扩散机制进行推断；4）两个分支的输出结合生成最终预测。
...
TL;DR: 本研究比较了经典模型和机器学习模型在电动汽车跟驰行为预测中的表现，发现随机森林模型在所有场景下都优于经典物理模型，特别是在不同间距条件下表现最佳。
...
Result: 随机森林模型在中等、长和超长间距条件下的RMSE分别为0.0046、0.0016和0.0025，显著优于表现最好的经典模型CACC（长间距RMSE为2.67）。

Conclusion: 机器学习模型在预测电动汽车跟驰行为方面表现优越，对于模拟电动汽车行为和分析混合自主交通动态具有重要价值。
...
<details>
  <summary>Details</summary>
Motivation: 为了让医生真正信任人工智能，需要让AI的推理过程透明化，就像咨询同事一样，而不是一个黑盒子。

Method: 创建HistoLens系统，将病理学家的自然语言问题智能翻译为AI引擎的精确查询，提供结构化报告和热图可视化证据，并训练AI专注于患者组织而忽略背景噪声。
...
### [141] [From Observability Data to Diagnosis: An Evolving Multi-agent System for Incident Management in Cloud Systems](https://arxiv.org/abs/2510.24145)
...
TL;DR: OpsAgent是一个轻量级、自进化的多智能体系统，用于云系统的事件管理，通过免训练数据处理将异构可观测数据转换为结构化文本描述，并使用多智能体协作框架实现透明可审计的诊断推理。


<details>
  <summary>Details</summary>
Motivation: 手动事件管理在面临海量异构可观测数据时劳动密集且容易出错，现有自动化方法在跨系统泛化性、可解释性和部署成本方面存在局限，阻碍了实际应用。

Method: 采用免训练数据处理将异构可观测数据转换为结构化文本描述，构建多智能体协作框架实现透明诊断推理，并引入双自进化机制集成内部模型更新和外部经验积累。

Result: 在OPENRCA基准测试中表现出最先进的性能，证明OpsAgent具有泛化性、可解释性、成本效益和自进化能力。
...
### [142] [BLM$_1$: A Boundless Large Model for Cross-Space, Cross-Task, and Cross-Embodiment Learning](https://arxiv.org/abs/2510.24161)
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在数字-物理空间和不同具身之间的泛化能力差的问题，以及现有模型在高级具身推理和跨空间统一操作方面的局限性。

Method: 采用两阶段训练范式：第一阶段通过精选数字语料库向MLLM注入具身知识，同时保持语言能力；第二阶段通过意图桥接接口训练策略模块，从MLLM提取高级语义来指导控制，而不微调MLLM主干。

Result: 在数字和物理基准测试中，单个BLM₁实例优于四个模型家族（MLLMs、ELLMs、VLAs、GMLMs），在数字任务中提升约6%，在物理任务中提升约3%。

Conclusion: BLM₁展示了在数字和物理空间中无缝操作的能力，同时能够泛化到不同的具身和任务，为统一的具身智能模型提供了有效解决方案。
...
### [143] [Investigating Intra-Abstraction Policies For Non-exact Abstraction Algorithms](https://arxiv.org/abs/2510.24297)
*Robin Schmöcker,Alexander Dockhorn,Bodo Rosenhahn*

Main category: cs.AI

TL;DR: 本文针对MCTS中抽象算法的样本效率问题，提出了多种内部抽象策略来改进随机平局决胜规则，并在多数环境中表现更优。


<details>
  <summary>Details</summary>
Motivation: MCTS的样本效率不足，现有抽象算法在多个动作属于同一抽象节点时使用随机平局决胜规则，这可能导致性能下降。

Method: 提出并实证评估了多种内部抽象策略，用于处理同一抽象节点中多个动作的平局情况。

Result: 多个提出的策略在大多数环境和参数设置下表现优于随机策略。

Conclusion: 内部抽象策略的选择对MCTS性能有重要影响，提出的替代策略能有效提升算法表现。
...
### [144] [Verifying Large Language Models' Reasoning Paths via Correlation Matrix Rank](https://arxiv.org/abs/2510.24299)
*Jiayu Liu,Wei Dai,Zhenya Huang,Ning Miao,Enhong Chen*

Main category: cs.AI

TL;DR: 本文提出了一种名为Self-Indicator的方法，通过分析LLM内部行为来评估其推理路径的可信度，无需外部资源即可显著提升推理准确性。
...
Method: 研究发现输入问题与输出推理路径之间的相关性矩阵的秩是推理正确性的可靠指标。基于此设计了Self-Indicator方法，通过重新加权候选推理路径来提升性能。

Result: 实验表明该方法能准确区分正确与错误推理路径（准确率超过75%），并在三个推理基准测试中将准确率提升超过8%。

Conclusion: Self-Indicator是一种简单、即插即用的方法，仅依赖LLM自身即可有效评估推理可信度，显著优于其他投票和验证方法。
...
### [145] [Retrieval and Argumentation Enhanced Multi-Agent LLMs for Judgmental Forecasting](https://arxiv.org/abs/2510.24303)
*Deniz Gorur,Antoni Rago,Francesca Toni*

Main category: cs.AI

TL;DR: 本文提出了一种用于判断性预测的多智能体框架，通过不同智能体对声明真实性进行辩论，生成定量双极论证框架(QBAFs)，结合多种基于大语言模型的智能体方法，实验证明多智能体组合能提高预测准确性并提供可解释的证据组合。


<details>
  <summary>Details</summary>
Motivation: 判断性预测作为未来事件预测任务，可视为声明验证问题。现有方法在证据收集和论证生成方面存在局限，需要更全面、可解释的验证框架来整合不同观点和证据。

Method: 提出多智能体声明验证框架，包含三种基于LLM的智能体：(1) ArgLLM智能体 - 生成和评估QBAFs；(2) RbAM智能体 - 基于关系论证挖掘从外部源生成QBAFs；(3) RAG-ArgLLM智能体 - 扩展ArgLLM并结合检索增强生成从外部源获取论证。使用2-3个智能体组合，基于6种不同基础LLM。

Result: 在标准判断性预测数据集上的实验表明，智能体组合（特别是三个智能体）能提高预测准确性，同时提供可解释的证据组合用于声明验证。

Conclusion: 多智能体框架通过整合不同证据源和观点，有效提升了判断性预测的准确性和可解释性，为声明验证任务提供了新的解决方案。
...
### [146] [Generative Large Language Models (gLLMs) in Content Analysis: A Practical Guide for Communication Research](https://arxiv.org/abs/2510.24337)
*Daria Kravets-Meinke,Hannah Schmid-Petri,Sonja Niemann,Ute Schmid*

Main category: cs.AI

TL;DR: 本文探讨了生成式大语言模型在传播研究内容分析中的应用，指出gLLMs在编码任务上优于众包工作者和训练有素的编码员，但将其整合到传播研究方法工具箱仍不成熟，需要解决七个关键挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管生成式大语言模型在传播研究内容分析中展现出巨大潜力，能够以更低的成本和时间完成编码任务，但其在传播研究方法中的整合仍然不足，需要为研究人员提供实践指南。

Method: 本文综合了关于gLLM辅助定量内容分析的新兴研究，提出了一个全面的最佳实践指南，以应对代码书开发、提示工程、模型选择、参数调优、迭代优化、模型可靠性验证和性能增强等七个关键挑战。

Result: 研究表明gLLMs在传播科学相关的各种编码任务上能够超越众包工作者和训练有素的编码员，能够解码隐含意义和上下文信息，仅需基本编程技能即可部署，且除验证数据集外几乎不需要标注数据。
...
### [147] [VDSAgents: A PCS-Guided Multi-Agent System for Veridical Data Science Automation](https://arxiv.org/abs/2510.24339)
...
TL;DR: 提出一种多智能体生态系统用于N-of-1决策支持，通过协调多个专业智能体来为个体患者提供个性化医疗建议，解决传统AI医疗系统对边缘患者效果不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 传统AI医疗系统服务于平均患者，在罕见变异、多病共存和代表性不足人群等边缘病例上表现不佳，这损害了公平性和信任。需要设计能够真正关注个体患者的系统。

Method: 构建多智能体生态系统，智能体按器官系统、患者群体和分析模式聚类，共享模型库和证据合成工具。通过协调层整合结果，考虑可靠性、不确定性和数据密度，生成包含风险估计、置信区间、异常标志和关联证据的决策支持包。

Result: 验证方法从群体平均转向个体可靠性，测量低密度区域误差、小样本校准和风险-覆盖权衡。系统能够为个体患者提供更准确和透明的决策支持。
...
TL;DR: Orion是一个高效推理框架，通过依赖感知的查询分解和逻辑并行内容扩展来解决LLM在实时Web应用中的延迟和吞吐量瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 将大型语言模型集成到实时Web应用中面临推理质量与低延迟高吞吐量需求之间的矛盾，现有方法难以同时满足这两个要求。

Method: Orion将查询推理分解为两个协同阶段：关键点生成（通过检索增强的少样本提示提取逻辑结构化的关键点）和内容并行扩展（基于依赖图并行扩展内容确保逻辑一致性），并引入流水线调度机制实现跨查询并行。

Result: 实验显示Orion相比基线方法实现了4.33倍的token生成速度提升、3.42倍的答案延迟降低，推理质量提升达18.75%。

Conclusion: Orion通过显式建模关键点间依赖关系，在保持高质量推理的同时显著提升了推理效率，有效解决了Web服务中LLM推理的瓶颈问题。
...
Result: 相比现有通用基准，APTBench能更准确地预测模型在下游作为智能体的表现，同时比后训练的全规模端到端评估更轻量且成本效益更高。
...
TL;DR: 本研究比较了多个大型语言模型（GPT、Claude、DeepSeek等）在逻辑和抽象推理能力方面的表现，并与人类表现进行基准测试，揭示了LLMs在推理方面的显著差异和困难。
...
Method: 使用八道定制设计的推理问题，比较多个LLMs（包括GPT、Claude、DeepSeek、Gemini等）的逻辑和抽象推理技能，并将结果与人类在相同任务上的表现进行基准测试。

Result: 研究揭示了LLMs在推理能力方面的显著差异，表明这些模型在演绎推理方面存在困难。

Conclusion: 大型语言模型在逻辑和抽象推理方面仍存在挑战，与人类表现相比显示出明显的差距，特别是在演绎推理能力上需要进一步改进。
...
TL;DR: 提出Law in Silico框架，使用LLM模拟法律社会，验证了LLM能够复现宏观犯罪趋势，并发现透明、适应性强的法律系统能更好保护弱势群体权利。


<details>
  <summary>Details</summary>
Motivation: 由于现实法律实验成本高或不可行，需要AI系统模拟法律社会来验证和发展法律理论，支持法律管理。LLM具备世界知识和角色扮演能力，适合作为法律社会模拟的基础。

Method: 引入Law in Silico框架，基于LLM的智能体模拟法律场景，包含个体决策和立法、裁决、执法等制度机制。

Result: 实验显示LLM智能体能够很大程度上复现宏观犯罪趋势，提供与现实观察一致的洞见；微观模拟表明功能良好、透明、适应性强的法律系统能更好保护弱势个体权利。

Conclusion: LLM能够有效模拟法律系统，为法律理论验证和发展提供可行途径，同时揭示了良好法律系统设计的重要性。
...
TL;DR: 提出一种成本高效的两阶段流程，通过跨任务示例伪标注和基于图的标签传播方法，减少对LLM数据标注的依赖，构建用于上下文学习的演示示例。


<details>
  <summary>Details</summary>
Motivation: 为新颖或困难任务收集高质量示例成本高昂且劳动密集，需要降低对大型语言模型数据标注的依赖。

Method: 两阶段流程：第一阶段利用跨任务示例提示LLM伪标注少量目标任务实例；第二阶段引入基于图的标签传播方法，无需额外LLM查询即可将标签信息传播到剩余目标示例。

Result: 在五个任务上的实验表明，该方法在降低标注成本的同时实现了强劲性能。

Conclusion: 该流程结合了跨任务监督的灵活性和无LLM传播的可扩展性，为上下文学习提供了有效的解决方案。
...
TL;DR: FunReason-MT是一个用于多轮工具使用的数据合成框架，通过环境-API图交互、高级工具查询合成和引导迭代链来解决真实世界环境中高质量多轮训练数据生成的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有数据合成方法（如随机环境采样或多智能体角色扮演）在真实世界环境中生成高质量数据的能力不足，存在目标模型训练、工具架构隔离和多轮逻辑依赖三个主要挑战。

Method: FunReason-MT采用三种核心方法：1）环境-API图交互收集多样化高质量轨迹；2）高级工具查询合成简化复杂查询构建；3）引导迭代链生成复杂思维链。

Result: 在Berkeley Function-Calling Leaderboard (BFCLv3)上的评估显示，基于FunReason-MT生成数据构建的4B模型在同类尺寸模型中达到最先进性能，优于大多数闭源模型。在BFCLv4上的进一步性能改进证实了其可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习和深度学习在实时计算机视觉中的快速发展，作物定点病害管理从手工特征提取发展到大规模自动特征学习。基础模型以全新的方式处理作物病害数据集，整合视觉和文本数据，解释症状文本，推理症状-管理关系，并为种植者和教育者提供交互式问答支持。

Method: 通过文献综述方法，筛选了约40篇相关论文，重点分析大语言模型和视觉语言模型在作物病害管理中的应用，以及它们在自适应学习、强化学习和数字孪生框架中的集成。

Result: 关键发现包括：基础模型应用在2023-24年激增；视觉语言模型发展快于大语言模型，出版物数量增加5-10倍；强化学习和自适应学习在智能喷洒中仍处于早期阶段；数字孪生与强化学习可虚拟模拟靶向喷洒；解决仿真到现实的差距对实际部署至关重要；人机协作仍然有限；多模态基础模型与实时反馈将推动下一代定点病害管理。

Conclusion: 基础模型正在改变作物定点病害管理的方式，特别是视觉语言模型显示出巨大潜力。未来的发展方向包括解决仿真到现实的差距、加强人机协作、以及开发具有实时反馈能力的多模态基础模型系统。
...
TL;DR: OrchDAG是一个合成数据生成管道，将工具执行建模为具有可控复杂度的有向无环图（DAGs），用于基准测试多轮工具交互性能，并提出基于图的奖励来增强RLVR训练。


<details>
  <summary>Details</summary>
Motivation: 现有工作大多忽视了多轮工具交互的复杂性，需要一种能够建模这种复杂性的方法来提升智能体工具使用能力。

Method: 引入OrchDAG合成数据生成管道，将工具执行建模为DAGs；提出基于图的奖励机制，结合GRPO风格算法进行训练。

Result: 实验表明该数据集提供了一个具有挑战性但可解决的基准，提出的奖励机制在与GRPO风格算法结合时效果显著。

Conclusion: 在多轮工具使用中，利用拓扑结构和数据复杂性具有重要意义，基于图的奖励能够有效提升模型性能。
...
TL;DR: 提出一个通过构建工具知识图谱和文档知识图谱并融合的框架，用于改进示例工件生成，采用深度稀疏集成策略来对齐工具依赖关系和程序知识。


<details>
  <summary>Details</summary>
Motivation: 为了揭示和利用工具与文档之间的依赖关系，以增强示例工件的生成能力。

Method: 从工具模式构建工具知识图谱，从内部文档和标准操作程序构建文档知识图谱，然后将两者融合，采用深度稀疏集成策略对齐工具结构依赖与程序知识。
...
### [158] [How to see the forest for the trees](https://arxiv.org/abs/2510.23614)
*Erika Bérczi-Kovács,András Frank*

Main category: cs.DM

TL;DR: 本文综述了Nash-Williams和Tutte关于图中存在k棵不相交生成树定理及其对偶定理的研究发展，旨在为非专家介绍该领域的重要成果和应用，同时也为专家提供新的视角。


<details>
  <summary>Details</summary>
Motivation: Nash-Williams和Tutte关于图中不相交生成树和森林覆盖的优雅定理引发了广泛研究，产生了深远推广并在看似不相关的领域找到应用。本文旨在阐明这些发展的各个方面和联系。

Method: 采用综述性研究方法，梳理和分析文献中超过千篇相关论文，包括最新研究成果，探讨定理的推广和应用。

Result: 揭示了这些经典定理在离散优化中的基础地位，以及它们在不同领域的广泛应用和深远影响。

Conclusion: 该综述为非专家提供了理解这一重要领域的途径，同时为专家提供了新的研究视角和思路，展示了这些经典结果的持续影响力和研究价值。
...
### [159] [Robust Extensible Bin Packing and Revisiting the Convex Knapsack Problem](https://arxiv.org/abs/2510.23765)
*Noam Goldberg,Michael Poss,Yariv Marmor*

Main category: cs.DM

TL;DR: 本文研究了具有预算不确定性的鲁棒可扩展装箱问题，提出了场景生成算法，分析了分离问题的计算复杂性，开发了动态规划和FPTAS算法，并通过实验验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究鲁棒可扩展装箱问题在预算不确定性下的求解方法，旨在处理实际应用中物品大小不确定的情况，提高装箱方案的鲁棒性。

Method: 提出交替求解主鲁棒装箱问题和分离问题的场景生成算法，针对分离问题开发了伪多项式动态规划和完全多项式时间近似方案。

Result: 证明了分离问题的计算复杂性，开发了高效的动态规划和FPTAS算法，实验表明动态规划比使用SOS约束的MIP求解器更高效，案例研究显示模型优于实际调度和最优名义调度。

Conclusion: 所提出的方法能有效解决鲁棒可扩展装箱问题，动态规划在实际应用中具有显著优势，模型在真实手术调度案例中表现出优越性能。
...
### [160] [Pinwheel Scheduling with Real Periods](https://arxiv.org/abs/2510.24068)
*Hiroshi Fujiwara,Kota Miyagi,Katsuhisa Ouchi*

Main category: cs.DM

TL;DR: 本文研究了扩展的pinwheel调度问题，证明了当任务周期取三个不同实数值且密度不超过5/6时，存在有效调度，并提出了Chan和Chin猜想在实数周期下也成立的猜想。


<details>
  <summary>Details</summary>
Motivation: Chan和Chin在1993年提出的pinwheel调度问题猜想已被Kawamura证实，但该问题仅限于整数周期。本文旨在研究Kawamura提出的实数周期扩展版本，探索在更一般情况下的调度可行性。

Method: 研究扩展的pinwheel调度问题，其中任务周期为实数，有效调度要求每个任务i在任意连续的⌈lai⌉天内至少执行l次（对所有正整数l）。分析周期取三个不同实数值的情况。

Result: 证明了当任务序列的周期取三个不同实数值且密度不超过5/6时，存在有效的调度方案。

Conclusion: 基于研究结果，提出了Chan和Chin猜想在实数周期下也成立的猜想，即对于任何密度不超过5/6的任务序列（周期为实数），都存在有效的调度方案。
...
### [161] [An Enhanced Dual Transformer Contrastive Network for Multimodal Sentiment Analysis](https://arxiv.org/abs/2510.23617)
*Phuong Q. Dao,Mark Roantree,Vuong M. Ngo*

Main category: cs.LG

TL;DR: 本文提出了BERT-ViT-EF模型和其扩展DTCN，用于多模态情感分析。BERT-ViT-EF通过早期融合策略结合BERT和ViT编码器，DTCN在此基础上增加Transformer层和对比学习，在MVSA-Single和TumEmo数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 多模态情感分析通过联合分析文本和图像数据，能比单模态方法提供更丰富准确的情感理解。现有方法在跨模态交互和联合表示学习方面仍有改进空间。
...
Result: 在MVSA-Single数据集上达到76.6%准确率和75.9% F1分数；在TumEmo数据集上达到78.4%准确率和78.3% F1分数，表现最佳。

Conclusion: 早期融合策略和更深层次的上下文建模在基于Transformer的多模态情感分析中具有显著优势，DTCN模型有效提升了多模态情感分析的性能。
...
### [162] [Speeding Up MACE: Low-Precision Tricks for Equivarient Force Fields](https://arxiv.org/abs/2510.23621)
*Alexandre Benoit*

Main category: cs.LG

TL;DR: 该论文研究了如何通过降低精度算术和GPU优化内核来加速SO(3)-等变模型MACE，同时保持物理保真度。研究发现cuEquivariance后端可将推理延迟减少约3倍，仅将线性层转换为BF16/FP16可在FP32模型中获得约4倍额外加速，而对NVT/NPT MD中的能量和热力学观测值影响可忽略。


<details>
  <summary>Details</summary>
Motivation: 机器学习力场可以提供准确的分子动力学模拟，但计算成本高。对于SO(3)-等变模型如MACE，缺乏系统证据表明降低精度算术和GPU优化内核能否在不损害物理保真度的情况下降低计算成本。

Method: 对MACE进行端到端和逐块分析，比较e3nn和NVIDIA cuEquivariance后端，评估FP64/FP32/BF16/FP16设置（使用FP32累加）在推理、短NVT和长NPT水模拟以及玩具训练运行中的性能。

Result: cuEquivariance将推理延迟减少约3倍。仅将线性层转换为BF16/FP16在FP32模型中可获得约4倍额外加速，而NVT/NPT MD中的能量和热力学观测值保持在运行间变异性范围内。训练中的半精度权重会降低力RMSE。
...
### [163] [Adversarially-Aware Architecture Design for Robust Medical AI Systems](https://arxiv.org/abs/2510.23622)
*Alyssa Gerhart,Balaji Iyangar*

Main category: cs.LG

TL;DR: 本研究探讨了对抗性攻击对医疗AI系统的威胁，通过在皮肤病数据集上的实证实验，展示了对抗性方法显著降低分类准确性的问题，并评估了防御措施的效果。
...
Result: 防御措施如对抗性训练和蒸馏能够降低攻击成功率，但需要在防御效果与模型在干净数据上的性能之间取得平衡。
...
### [164] [DiNo and RanBu: Lightweight Predictions from Shallow Random Forests](https://arxiv.org/abs/2510.23624)
*Tiago Mendonça dos Santos,Rafael Izbicki,Luís Gustavo Esteves*

Main category: cs.LG

TL;DR: DiNo和RanBu是两种浅层森林方法，通过将少量深度受限的树转换为高效的基于距离的预测器，显著降低随机森林的推理延迟和内存需求。


<details>
  <summary>Details</summary>
Motivation: 随机森林在表格预测任务中表现优异，但其依赖数百棵深度树导致推理延迟高、内存需求大，限制了在延迟敏感或资源受限环境中的部署。

Method: DiNo通过观测对的最远共同祖先测量共表型距离，RanBu对Breiman的经典邻近度度量应用核平滑。两种方法都在森林训练后操作，无需额外生长树，仅需轻量级矩阵向量操作调整单个带宽参数h。

Result: 在3个合成基准测试和25个公共数据集上，RanBu匹配或超过全深度随机森林的准确性（特别是在高噪声设置中），同时将训练加推理时间减少高达95%。DiNo在低噪声机制中以适度的计算成本实现了最佳偏差-方差权衡。两种方法可直接扩展到分位数回归，在保持准确性的同时获得显著速度提升。

Conclusion: DiNo和RanBu提供了高效的浅层森林替代方案，显著降低了随机森林的计算成本，同时保持了预测性能，特别适合延迟敏感和资源受限的应用场景。
...
### [165] [From Detection to Discovery: A Closed-Loop Approach for Simultaneous and Continuous Medical Knowledge Expansion and Depression Detection on Social Media](https://arxiv.org/abs/2510.23626)
*Shuang Geng,Wenli Zhang,Jiaheng Xie,Rui Wang,Sudha Ram*

Main category: cs.LG

TL;DR: 提出了一个闭环LLM-知识图谱框架，将抑郁症检测预测与医学知识扩展相结合，通过迭代学习循环实现预测准确性和医学理解的共同提升。


<details>
  <summary>Details</summary>
Motivation: 现有研究虽然整合医学知识来提高预测准确性，但忽视了通过预测过程同时扩展医学知识的机会。社交媒体用户生成内容为抑郁症预测提供了实时数据源。
...
### [166] [Chain of Execution Supervision Promotes General Reasoning in Large Language Models](https://arxiv.org/abs/2510.23629)
*Nuo Chen,Zehua Li,Keqin Bao,Junyang Lin,Dayiheng Liu*

Main category: cs.LG

TL;DR: TracePile是一个包含260万样本的大规模语料库，将代码执行转化为显式的逐步推理链（Chain of Execution），通过继续预训练、指令调优和两阶段微调三种方式在多个基准测试中显著提升大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在代码推理方面存在局限性，因为代码中的推理往往是隐式的，并与语法或实现噪声纠缠在一起，直接在原始代码上训练效果不佳。

Method: 构建TracePile语料库，将代码执行转化为显式的逐步推理链，包含变量追踪问题和代码重写，覆盖数学、经典算法和算法竞赛等领域，采用三种训练设置：继续预训练、指令调优和两阶段微调。

Result: 在四个基础模型（LLaMA 3、LLaMA 3.1、Qwen-2.5、Qwen-2.5 Coder）和20个基准测试上的实验显示了一致的改进，特别是LLaMA3.1-8B在九个数学数据集上平均提升7.1%，在两阶段微调下在LiveCodeBench、CRUX和MMLU上也有明显提升。

Conclusion: TracePile通过将隐式代码推理转化为显式执行链，有效提升了大语言模型在数学、代码、逻辑和算法等领域的推理能力。
...
### [167] [NUM2EVENT: Interpretable Event Reasoning from Numerical time-series](https://arxiv.org/abs/2510.23630)
*Ninghui Feng,Yiyan Qi*

Main category: cs.LG

TL;DR: 本文提出数字到事件推理解码任务，通过集成智能体引导事件提取器、标记多变量霍克斯合成生成器和两阶段微调管道，从数值输入推断可解释的结构化事件。
...
Method: 提出推理感知框架，集成智能体引导事件提取器(AGE)、标记多变量霍克斯合成生成器(EveDTS)，以及结合时间序列编码器和结构化解码器的两阶段微调管道。
...
### [168] [Beyond Pairwise: Empowering LLM Alignment With Ranked Choice Modeling](https://arxiv.org/abs/2510.23631)
*Yuxuan Tang,Yifan Feng*

Main category: cs.LG

TL;DR: 提出Ranked Choice Preference Optimization (RCPO)框架，通过最大似然估计将偏好优化与排序选择建模相结合，支持多种反馈格式，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型对齐主要依赖成对偏好优化，这种方法忽略了从更丰富的人类反馈（如多选比较和top-k排序）中学习的机会。

Method: 提出RCPO统一框架，支持基于效用和基于排序的选择模型，包含多种现有成对方法，并为更丰富的反馈格式提供原则性训练目标。具体实例化了多项Logit和Mallows-RMJ两种代表性排序选择模型。

Result: 在Llama-3-8B-Instruct和Gemma-2-9B-it模型上的实证研究显示，RCPO在AlpacaEval 2和Arena-Hard基准测试中持续优于竞争基线方法。
...
### [169] [LLMComp: A Language Modeling Paradigm for Error-Bounded Scientific Data Compression](https://arxiv.org/abs/2510.23632)
*Guozhong Li,Muhannad Alhumaidi,Spiros Skiadopoulos,Panos Kalnis*

Main category: cs.LG

TL;DR: LLMCOMP是一种利用解码器专用大型语言模型进行科学数据压缩的新范式，通过量化、Z-order曲线排列和覆盖引导采样将3D场转换为离散标记，使用自回归变换器建模标记转换，在严格误差边界下实现比现有方法高30%的压缩比。


<details>
  <summary>Details</summary>
Motivation: 高分辨率科学模拟和观测系统产生海量时空数据集，需要高效、有误差边界的压缩方法。同时，解码器专用大型语言模型在建模复杂序列数据方面表现出卓越能力。

Method: 将3D场量化为离散标记，通过Z-order曲线排列以保持局部性，应用覆盖引导采样提高训练效率，使用带时空嵌入的自回归变换器建模标记转换，在压缩时执行top-k预测并存储排名索引和回退校正以确保严格误差边界。

Result: 在多个再分析数据集上的实验表明，LLMCOMP在严格误差边界下始终优于最先进的压缩器，压缩比提高达30%。
...
### [170] [Noise is All You Need: Solving Linear Inverse Problems by Noise Combination Sampling with Diffusion Models](https://arxiv.org/abs/2510.23633)
*Xun Su,Hiroyuki Kasai*

Main category: cs.LG

TL;DR: 提出了一种名为噪声组合采样的新方法，通过合成最优噪声向量来近似测量分数，将条件信息自然嵌入到生成过程中，无需依赖逐步超参数调整，显著提高了逆问题求解的鲁棒性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 预训练扩散模型在零样本逆问题求解中表现出强大能力，但存在一个固有困境：过度整合观测信息会干扰生成过程，而整合不足则无法强调逆问题施加的约束。

Method: 提出噪声组合采样方法，从噪声子空间合成最优噪声向量来近似测量分数，替代标准去噪扩散概率模型过程中的噪声项，使条件信息自然嵌入生成过程。

Result: 该方法可应用于广泛的逆问题求解器，包括图像压缩，特别是在生成步骤数T较小时，以可忽略的计算开销实现卓越性能。

Conclusion: 噪声组合采样方法显著提高了逆问题求解的鲁棒性和稳定性，无需依赖逐步超参数调整，在少量生成步骤下表现优异。
...
### [171] [Monotone and Separable Set Functions: Characterizations and Neural Models](https://arxiv.org/abs/2510.23634)
*Soutrik Sarangi,Yonatan Sverdlov,Nadav Dym,Abir De*

Main category: cs.LG

TL;DR: 该论文研究了单调分离集合函数的设计，旨在保持集合包含关系的偏序结构，并建立了向量维度的上下界。对于无限基础集的情况，提出了弱MAS模型，并展示了其在集合包含任务中的优势。
...
Method: 建立了单调分离集合函数向量维度的上下界，对于无限基础集提出了弱MAS模型，该模型具有Holder连续性稳定性，并构建了能够近似所有单调集合函数的通用模型。

Result: 实验表明，在多种集合包含任务中，所提出的模型相比不包含集合包含归纳偏置的标准集合模型表现更优。

Conclusion: 单调分离集合函数能够有效保持集合包含关系，特别是在无限基础集情况下，弱MAS模型提供了可行的解决方案，并在实际应用中展现出优势。
...
### [172] [Help the machine to help you: an evaluation in the wild of egocentric data cleaning via skeptical learning](https://arxiv.org/abs/2510.23635)
*Andrea Bontempelli,Matteo Busso,Leonardo Javier Malcotti,Fausto Giunchiglia*

Main category: cs.LG

TL;DR: 本研究评估了怀疑学习(SKEL)在真实用户环境下的性能，通过大学生使用iLog移动应用进行四周实验，发现SKEL能减少标注工作量并提高数据质量，但需要在用户努力和数据质量之间找到平衡。


<details>
  <summary>Details</summary>
Motivation: 数字个人助理需要高质量标注来正常工作，但用户标注通常存在错误和噪声。先前怀疑学习研究通过比较离线主动标注与被动数据来评估标注准确性，但缺乏最终用户的确认，而用户才是自己上下文的最佳判断者。

Method: 在真实世界条件下评估SKEL性能，让实际用户基于当前视角和需求精炼输入标签。研究让大学生在设备上使用iLog移动应用，持续四周时间。

Result: 结果突显了在用户努力和数据质量之间找到适当平衡的挑战，以及使用SKEL的潜在好处，包括减少标注工作量和提高收集数据的质量。

Conclusion: 怀疑学习在真实用户环境中能有效减少标注工作量并提升数据质量，但需要仔细平衡用户参与程度与数据质量要求。
...
### [173] [Integrating Genomics into Multimodal EHR Foundation Models](https://arxiv.org/abs/2510.23639)
...
TL;DR: 本文提出了一种创新的电子健康记录基础模型，整合多基因风险评分作为基础数据模态，超越传统仅使用EHR的方法，构建更全面的健康档案。该多模态框架利用All of Us研究项目的广泛数据，学习临床数据与遗传易感性之间的复杂关系，增强预测能力和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统EHR方法存在局限性，无法充分利用遗传信息。为了构建更全面的健康档案，需要整合多基因风险评分，探索临床数据与遗传易感性之间的复杂关系，实现更精准的疾病预测和个性化医疗。

Method: 扩展生成式AI在EHR基础模型领域的应用，开发多模态框架整合PRS和EHR数据。利用All of Us研究项目的多样化数据，学习数据间复杂关系，并探索迁移学习在定制分类任务中的应用。

Result: 在All of Us数据上的评估显示，该模型对多种疾病（特别是2型糖尿病）的发病具有预测价值，并揭示了PRS与EHR数据之间的相互作用。迁移学习实验展示了架构的通用性和效率。

Conclusion: 该方法为疾病预测、主动健康管理、风险分层和个性化治疗策略提供了新的见解，为医疗保健中更个性化、公平和可操作的现实证据生成奠定了基础。
...
### [174] [Spatially Aware Linear Transformer (SAL-T) for Particle Jet Tagging](https://arxiv.org/abs/2510.23641)
...
TL;DR: SAL-T是一种物理启发的线性变换器架构，通过基于运动学特征的空间感知粒子分区和卷积层来降低计算复杂度，在保持高性能的同时显著减少资源消耗和推理延迟。


<details>
  <summary>Details</summary>
Motivation: 传统变换器在高能粒子碰撞等数据吞吐量大的环境中部署困难，其二次复杂度需要大量资源并增加推理延迟。

Method: 结合linformer架构的线性注意力机制，引入基于运动学特征的空间感知粒子分区，并利用卷积层捕捉局部相关性。

Result: 在喷注分类任务中优于标准linformer，性能与全注意力变换器相当，但资源消耗和推理延迟显著降低；在ModelNet10点云分类数据集上验证了相同趋势。

Conclusion: SAL-T通过物理启发的设计实现了高性能与高效率的平衡，为高数据吞吐环境中的变换器部署提供了可行解决方案。
...
Method: 使用logits层干预方法，包括静态和动态两种零样本去偏技术，与隐藏层方法进行对比。

Result: Dynamic方法在减少偏见方面表现最佳，偏见减少高达70%，且流畅性损失最小；logits干预方法优于隐藏层方法。

Conclusion: 语义感知的logits干预方法对于已对齐LLMs的去偏是稳定且有效的。
...
TL;DR: CLP是一个连续的层剪枝框架，通过可微凹门算法自动识别最佳剪枝层段，结合剪断端点调优策略恢复模型性能，显著优于现有方法。
...
Method: 提出CLP框架：1) 可微凹门算法通过梯度优化自动识别最佳连续层段进行剪枝；2) 剪断端点调优策略仅微调剪枝段相邻层以有效恢复模型性能。

Result: 在多个模型架构（LLaMA2、LLaMA3、Qwen）和规模（7B到70B参数）上的实验表明，CLP显著优于现有方法。在20%剪枝率下，LLaMA3-70B平均性能保持率达95.34%，比基线高4.29%-30.52%。

Conclusion: CLP能有效减少大型语言模型的计算开销，且可与量化无缝结合进一步压缩模型，仅带来轻微性能损失。
...
TL;DR: 本文提出了一种名为SAEA的新框架，用于调整交通预测中的时空自相关误差，通过将预测误差建模为时空向量自回归过程来捕捉其内在依赖关系，从而提高深度学习交通预测模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的交通预测模型通常假设预测误差在时间和空间上不相关，但实际交通数据具有时空自相关性，这种假设限制了模型性能。本文旨在填补这一研究空白。

Method: 提出SAEA框架：1) 通过系数矩阵显式捕捉时空误差相关性；2) 引入结构稀疏正则化融入先验空间信息；3) 设计带有测试时误差调整的推理过程来动态优化预测。

Result: 在不同交通数据集上的实验验证了该方法的有效性，结果表明该方法在几乎所有情况下都能提升各种交通预测模型的性能。

Conclusion: SAEA框架能够有效处理交通预测中的时空自相关误差问题，显著提升深度学习交通预测模型的准确性和可靠性。
...
TL;DR: 本研究开发了首个机器学习框架来预测冷等离子体处理下五种作物种子的发芽提升效果，其中Extra Trees模型表现最佳，并揭示了电压和暴露时间的激素效应响应模式。


<details>
  <summary>Details</summary>
Motivation: 冷等离子体作为一种环保方法可以增强种子发芽，但由于复杂的种子-等离子体-环境相互作用，结果难以预测，需要建立预测模型来优化处理参数。

Method: 使用梯度提升(GB)、极端梯度提升(XGB)、Extra Trees(ET)及其混合模型等机器学习方法，在介电阻挡放电等离子体条件下预测大豆、大麦、向日葵、萝卜和番茄的发芽提升。

Result: Extra Trees模型表现最佳(R²=0.919)，特征减少后提升至R²=0.925。工程分析显示激素效应响应：7-15kV电压和200-500s暴露时间可获得最大发芽率，而超过20kV或长时间暴露会降低发芽率。不同物种和品种的预测精度存在差异。
...
### [179] [Quantum Machine Learning for Image Classification: A Hybrid Model of Residual Network with Quantum Support Vector Machine](https://arxiv.org/abs/2510.23659)
*Md. Farhan Shahriyar,Gazi Tanbhir,Abdullah Md Raihan Chy*

Main category: cs.LG

TL;DR: 该研究提出了一种结合ResNet-50特征提取和量子支持向量机(QSVM)分类的混合方法，用于马铃薯病害检测。实验表明基于Z特征映射的QSVM在准确率上达到99.23%，优于传统机器学习模型。
...
Method: 使用ResNet-50从马铃薯病害RGB图像中提取深度特征，通过主成分分析(PCA)降维，然后使用QSVM模型应用ZZ、Z和Pauli-X等量子特征映射将经典数据转换为量子态。

Result: 基于Z特征映射的QSVM模型准确率达到99.23%，超过了传统支持向量机(SVM)和随机森林(RF)模型。
...
### [180] [AI-Driven Carbon Monitoring: Transformer-Based Reconstruction of Atmospheric CO2 in Canadian Poultry Regions](https://arxiv.org/abs/2510.23663)
*Padmanabhan Jagannathan Prajesh,Kaliaperumal Ragunath,Miriam Gordon,Bruce Rathgeber,Suresh Neethirajan*

Main category: cs.LG

TL;DR: 提出了一种结合小波变换和视觉变换器的ST-ViWT框架，用于从稀疏的OCO-2卫星观测中重建连续、不确定性量化的柱平均CO2浓度场，在加拿大南部家禽养殖密集区域实现了高精度XCO2制图。


<details>
  <summary>Details</summary>
Motivation: 准确绘制农业景观上空的柱平均CO2浓度对于指导排放减排策略至关重要，特别是在家禽养殖密集区域需要连续、高精度的CO2监测数据。

Method: 开发了时空视觉变换器与小波变换(ST-ViWT)框架，融合小波时频表示与变换器注意力机制，整合气象、植被指数、地形和土地覆盖等多源数据。
...
Conclusion: 该方法能够生成无缝的0.25度CO2表面并量化不确定性，支持卫星约束与国家清单和精准畜牧平台的整合，为碳核算、热点优先排序和政策相关减排评估提供可扩展、透明的解决方案。
...
### [181] [Transformers from Compressed Representations](https://arxiv.org/abs/2510.23665)
...
Conclusion: TEMPEST展示了压缩数据流在表示学习中的潜力，提供了一种高效且准确的语义分类方法，为压缩格式的直接学习开辟了新途径。
...
### [182] [Optimize Any Topology: A Foundation Model for Shape- and Resolution-Free Structural Topology Optimization](https://arxiv.org/abs/2510.23667)
*Amin Heyrani Nobari,Lyle Regenwetter,Cyril Picard,Ligong Han,Faez Ahmed*

Main category: cs.LG

TL;DR: OAT是一个基于深度学习的拓扑优化框架，能够直接预测任意长宽比、分辨率、体积分数、载荷和边界条件下的最小柔度布局，相比现有方法在多个基准测试中柔度降低高达90%，并在单GPU上实现亚秒级推理。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习拓扑优化方法局限于固定方形网格、少量预定义边界条件和后处理优化，无法实现通用部署。需要开发能够处理任意边界条件、分辨率和长宽比的通用框架。

Method: 结合分辨率无关和形状无关的自编码器、隐式神经场解码器以及条件潜在扩散模型，在包含220万个优化结构和200万种边界条件配置的OpenTO数据集上进行训练。

Result: 在四个公共基准测试和两个未见挑战测试中，OAT相比最佳现有模型平均柔度降低高达90%，在64×64到256×256分辨率及高达10:1长宽比下实现亚秒级推理。

Conclusion: OAT建立了通用、快速且分辨率无关的物理感知拓扑优化框架，并为逆向设计的生成建模研究提供了大规模数据集。
...
### [183] [Traffic flow forecasting, STL decomposition, Hybrid model, LSTM, ARIMA, XGBoost, Intelligent transportation systems](https://arxiv.org/abs/2510.23668)
*Fujiang Yuan,Yangrui Fan,Xiaohuan Bing,Zhen Tian,Chunhong Yuan,Yankang Li*

Main category: cs.LG

TL;DR: 提出了一种基于STL分解的混合预测框架，将LSTM、ARIMA和XGBoost模型结合，分别处理交通流数据的趋势、季节性和非线性残差分量，显著提升了预测精度。


<details>
  <summary>Details</summary>
Motivation: 单一模型难以捕捉交通流数据中复杂的非线性多尺度时间模式，需要开发更有效的混合方法来提高预测准确性。

Method: 使用STL将时间序列分解为趋势、季节性和残差分量，然后分别用LSTM建模长期趋势、ARIMA捕捉季节性周期、XGBoost预测非线性残差波动，最后通过乘法集成获得最终预测。
...
### [184] [DBLoss: Decomposition-based Loss Function for Time Series Forecasting](https://arxiv.org/abs/2510.23672)
...
<details>
  <summary>Details</summary>
Motivation: 现有的均方误差损失函数有时无法准确捕捉预测范围内的季节性或趋势，即使在前向传播中使用分解模块分别建模趋势和季节性也存在局限性。

Method: 使用指数移动平均将时间序列分解为季节性和趋势分量，在预测范围内分别计算这些分量的损失，然后进行加权。DBLoss可作为通用损失函数与任何深度学习预测模型结合。
...
### [185] [On the Societal Impact of Machine Learning](https://arxiv.org/abs/2510.23693)
*Joachim Baumann*
...
<details>
  <summary>Details</summary>
Motivation: 机器学习系统越来越多地影响重要决策，但由于开发时缺乏明确的公平性考虑，存在歧视风险。需要研究如何确保ML的社会影响符合更广泛的社会价值观。

Method: 通过改进公平性测量方法、系统性地分解ML系统以预测偏见动态，以及开发有效的干预措施来减少算法歧视。

Result: 建立了更合适的ML系统公平性测量方法，提出了系统性分解ML系统的方法来预测偏见动态，开发了在保持系统效用的同时减少算法歧视的有效干预措施。

Conclusion: 为机器学习系统日益融入社会提供了确保其社会影响与社会价值观一致的基础，并讨论了生成式人工智能等新兴技术带来的持续挑战和未来研究方向。
...
### [186] [MUStReason: A Benchmark for Diagnosing Pragmatic Reasoning in Video-LMs for Multimodal Sarcasm Detection](https://arxiv.org/abs/2510.23727)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在讽刺检测等复杂任务上表现不佳，因为讽刺理解不仅依赖于字面内容，还需要结合说话者的语调、面部表情和对话上下文等非语言线索进行语用推理。

Method: 1) 引入MUStReason诊断基准，包含模态特定相关线索和推理步骤标注；2) 提出PragCoT框架，引导视频语言模型关注隐含意图而非字面含义；3) 将问题分解为感知和推理两个层面进行评估。

Result: 通过MUStReason基准对视频语言模型的讽刺分类性能进行基准测试，并对生成的推理进行定量和定性评估，验证了PragCoT框架的有效性。

Conclusion: MUStReason基准和PragCoT框架为评估和改进视频语言模型在讽刺检测等复杂语用推理任务上的能力提供了有效工具，强调了关注隐含意图对讽刺检测的重要性。
...
### [187] [Revealing the Potential of Learnable Perturbation Ensemble Forecast Model for Tropical Cyclone Prediction](https://arxiv.org/abs/2510.23794)
*Jun Liu,Tao Zhou,Jiarui Li,Xiaohui Zhong,Peng Zhang,Jie Feng,Lei Chen,Hao Li*

Main category: cs.LG

TL;DR: FuXi-ENS是一种基于AI的台风集合预报系统，通过可学习扰动方案生成集合，相比传统ECMWF-ENS在台风物理变量预测、路径预报和环流捕捉方面表现更优，但强度预报仍存在低估。


<details>
  <summary>Details</summary>
Motivation: 传统台风集合预报系统计算成本高且难以完全表征大气非线性特征，需要开发更高效的AI替代方案来改进台风预报能力。

Method: FuXi-ENS采用可学习扰动方案生成集合预报，并与ECMWF-ENS进行系统性对比分析，评估两者在台风物理变量、路径强度预报以及动力热力场方面的表现。

Result: FuXi-ENS在台风相关物理变量预测和路径预报方面具有明显优势，集合离散度更小，能更好地捕捉大尺度环流特征，但强度预报仍低于观测值。

Conclusion: 可学习扰动方案具有提升台风预报技能的潜力，为改进具有重大社会影响的极端天气事件的AI集合预报提供了宝贵见解。
...
### [188] [Learning Interpretable Features in Audio Latent Spaces via Sparse Autoencoders](https://arxiv.org/abs/2510.23802)
*Nathan Paek,Yongyi Zang,Qihui Yang,Randal Leistikow*

Main category: cs.LG

TL;DR: 该论文提出了一个解释音频生成模型潜在表示的框架，通过将稀疏自编码器特征映射到人类可解释的声学概念（音高、振幅、音色），实现对AI音乐生成过程的可控操作和分析。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器在语言模型中成功提取可解释特征，但在音频生成中面临挑战：音频的密集性使得压缩后语义含义模糊，且自动特征表征有限。需要开发能够解释音频生成模型潜在表示的方法。

Method: 在音频自编码器潜在空间上训练稀疏自编码器，然后学习从SAE特征到离散化声学属性的线性映射。该方法在连续（DiffRhythm-VAE）和离散（EnCodec、WavTokenizer）音频潜在空间上进行验证。

Result: 成功分析了DiffRhythm（最先进的文本到音乐模型），展示了音高、音色和响度在整个生成过程中的演变。框架能够实现可控操作和生成过程分析。

Conclusion: 该框架不仅适用于音频模态，还可以扩展到视觉潜在空间生成模型的可解释分析，为理解生成模型的内部工作机制提供了有效工具。
...
### [189] [How do simple rotations affect the implicit bias of Adam?](https://arxiv.org/abs/2510.23804)
*Adela DePavia,Vasileios Charisopoulos,Rebecca Willett*

Main category: cs.LG

TL;DR: 自适应梯度方法如Adam和Adagrad在机器学习中广泛使用，但其对模型泛化的影响仍不清楚。研究发现Adam存在'丰富性偏差'，但受坐标轴预处理影响，对数据旋转敏感，可能丧失优势。通过重参数化方法可恢复Adam的丰富决策边界偏好。


<details>
  <summary>Details</summary>
Motivation: 理解自适应梯度方法（如Adam）相对于梯度下降等方法的泛化性能差异，特别是Adam表现出的'丰富性偏差'特性及其对数据旋转的敏感性。

Method: 分析Adam在坐标轴预处理下的敏感性，研究数据旋转对其性能的影响，并应用正交变换重参数化方法来增强方法对数据旋转的等变性。

Result: 研究发现即使小的数据旋转也会使Adam丧失其丰富性偏差优势，收敛到远离贝叶斯最优决策边界的线性边界。重参数化方法能够恢复Adam对丰富决策边界的偏好。

Conclusion: 自适应梯度方法对数据旋转敏感，可能影响其泛化优势，但通过适当的重参数化技术可以缓解这一问题，恢复其学习复杂决策边界的能力。
...
### [190] [A Physics-informed Multi-resolution Neural Operator](https://arxiv.org/abs/2510.23810)
*Sumanta Roy,Bahador Bahmani,Ioannis G. Kevrekidis,Michael D. Shields*

Main category: cs.LG

TL;DR: 提出了一种物理信息驱动的算子学习方法，将分辨率无关神经算子框架扩展到完全无数据设置，处理多分辨率数据和数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 传统算子学习需要大量高质量训练数据，但在实际工程应用中难以获得；且数据集可能在不同实现中具有不均匀的离散化，网格分辨率在样本间变化。

Method: 将任意离散化的输入函数投影到潜在嵌入空间，使用预训练基函数；通过多层感知机近似偏微分方程算子，输入潜在编码和时空坐标，在物理空间中通过有限差分求解器强制执行PDE约束。

Result: 在多个具有多分辨率数据的数值示例上验证了方法的性能，输入函数在不同分辨率下采样，包括粗粒度和细粒度离散化。

Conclusion: 该方法能够同时解决数据稀缺和多分辨率数据问题，为实际工程应用提供了有效的算子学习解决方案。
...
### [191] [Combining SHAP and Causal Analysis for Interpretable Fault Detection in Industrial Processes](https://arxiv.org/abs/2510.23817)
*Pedro Cortes dos Santos,Matheus Becali Rocha,Renato A Krohling*

Main category: cs.LG

TL;DR: 该研究针对工业过程复杂数据故障检测的挑战，使用田纳西伊斯曼过程基准，结合SHAP解释性分析和因果分析，开发了创新的故障检测框架，提高了检测准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 工业过程产生的复杂数据给故障检测系统带来挑战，现有机器学习方法往往结果不透明或性能不佳。研究旨在解决这一问题，为复杂制造环境提供更智能、可解释的故障检测方案。

Method: 使用田纳西伊斯曼过程作为基准，首先尝试标准模型发现局限性，然后采用SHAP方法识别关键过程特征，再通过有向无环图进行因果分析，揭示故障传播机制。

Result: SHAP分析成功识别出驱动故障预测的关键过程特征，因果结构与SHAP发现高度一致，明确指出了冷却和分离系统等关键过程元素在故障发展中的核心作用。

Conclusion: 这种双重方法将预测能力与因果理解相结合，不仅提高了检测准确性，还为操作人员提供了清晰、可操作的故障起源洞察，为工业系统更智能、可解释的故障检测开辟了新途径。
...
### [192] [ScaLoRA: Optimally Scaled Low-Rank Adaptation for Efficient High-Rank Fine-Tuning](https://arxiv.org/abs/2510.23818)
*Yilang Zhang,Xiaodong Yang,Yiwei Cai,Georgios B. Giannakis*

Main category: cs.LG

TL;DR: 提出了一种通过连续低秩增量逐步累积高秩权重更新的方法，以克服LoRA在效果和收敛速度上的限制，在保持高效性的同时接近全参数微调的性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模不断扩大，计算开销成为任务特定微调的主要瓶颈。虽然LoRA通过将权重更新限制在低维子空间来有效控制成本，但这种限制会阻碍效果并减慢收敛速度。

Method: 通过连续低秩增量逐步累积高秩权重更新，识别每个更新的最优低秩矩阵以最小化损失函数并接近全参数微调。采用适当的列缩放形成原始低秩矩阵的最优选择，实现高效无缝优化而无需重启。

Result: 在规模达120亿参数的流行LLM上进行广泛数值测试，在自然语言理解、常识推理和数学问题解决等多样化任务上，相对于最先进的LoRA变体实现了持续的性能提升和快速收敛。

Conclusion: 该方法通过逐步累积高秩权重更新，在保持LoRA高效性的同时显著提升了性能表现和收敛速度，为解决大语言模型微调的计算瓶颈提供了有效方案。
...
### [193] [A PDE-Informed Latent Diffusion Model for 2-m Temperature Downscaling](https://arxiv.org/abs/2510.23866)
*Paul Rosu,Muchang Bahng,Erick Jiang,Rico Zhu,Vahid Tarokh*

Main category: cs.LG

TL;DR: 提出一种物理条件化的潜在扩散模型，用于大气数据的动态降尺度，重点重建高分辨率2米温度场。在现有扩散架构基础上，通过残差公式结合PDE损失项来增强物理一致性。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型训练虽然能产生较低的PDE残差，但需要进一步通过物理约束来正则化模型，提高生成场的物理合理性。

Method: 基于预训练的扩散架构，采用残差公式对抗参考UNet，在训练目标中集成PDE损失项。PDE损失在完整分辨率空间中计算，通过有效平流-扩散平衡的有限差分近似来强制物理一致性。

Result: 经验观察表明传统扩散训练已产生低PDE残差，通过额外损失微调进一步正则化模型并增强生成场的物理合理性。

Conclusion: 提出的物理条件化潜在扩散模型能够有效提高大气数据动态降尺度的物理一致性，代码已在GitHub开源供未来发展参考。
...
### [194] [GIFT: Group-relative Implicit Fine Tuning Integrates GRPO with DPO and UNA](https://arxiv.org/abs/2510.23868)
*Zhichao Wang*

Main category: cs.LG

TL;DR: GIFT是一种新颖的强化学习框架，通过最小化隐式和显式奖励模型之间的差异来对齐LLMs，将复杂的奖励最大化目标转化为简单的MSE损失，实现凸优化。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法如PPO、GRPO在直接最大化累积奖励时的复杂性，以及DPO、UNA等离线方法缺乏探索能力的问题，旨在开发更稳定、高效的LLM对齐方法。
...
Conclusion: GIFT通过隐式-显式奖励对齐提供了一种稳定、高效且具有探索能力的LLM对齐方法，在性能和效率方面均优于现有方法。
...
TL;DR: RS-ORT是一种专门的分支定界算法，用于训练最优回归树，通过两阶段优化和结构变量分支，解决了连续特征和大规模数据集上的计算难题。
...
Result: 在包含二元和连续特征的回归基准测试中，RS-ORT在训练和测试性能上优于最先进方法。对于包含200万样本的连续特征数据集，能在4小时内获得保证的训练性能、更简单的树结构和更好的泛化能力。

Conclusion: RS-ORT通过专门的分支定界算法设计，有效解决了大规模连续特征数据集上的最优回归树训练问题，保证了收敛性和计算效率。
...
### [196] [Preference Learning with Response Time: Robust Losses and Guarantees](https://arxiv.org/abs/2505.22820)
...
<details>
  <summary>Details</summary>
Motivation: 虽然二元偏好数据在微调基础模型和生成AI系统中已广泛应用，但用户决策过程中的时间信息尚未被充分利用。响应时间包含了偏好强度的有价值信息，可以显著改进奖励模型学习。

Method: 提出将响应时间信息与二元选择数据结合的新方法，利用证据积累漂移扩散模型，开发了Neyman正交损失函数，实现了oracle收敛速率。

Result: 理论分析表明，对于线性奖励函数，传统偏好学习的错误率随奖励幅度呈指数增长，而响应时间增强方法将其降低为多项式增长。实验验证了在图像偏好学习中的有效性。

Conclusion: 响应时间增强的偏好学习方法显著提高了样本效率，能够为复杂奖励模型建立收敛特性，在理论和实验上都证明了其优越性。
...
TL;DR: 论文证明在简化假设下，注意力机制中的Query权重是冗余的，可以减少超过8%的非嵌入参数，并在GPT-3小架构上验证了减少模型能达到与标准基线相当的验证损失。


<details>
  <summary>Details</summary>
Motivation: 研究当前最先进大语言模型中注意力机制的Query、Key、Value权重三元组是否可以简化，探索参数冗余问题。

Method: 在简化假设下进行理论分析，证明Query权重的冗余性，并在完整复杂度的GPT-3小架构（包含层归一化、跳跃连接和权重衰减）上从头训练进行验证。

Result: 理论证明Query权重是冗余的，可以减少超过8%的非嵌入参数；实验验证减少模型能达到与标准基线相当的验证损失。

Conclusion: 这些发现为在大规模上研究Query权重冗余问题提供了动机。
...
TL;DR: 该论文将折扣奖励MDP的几何解释扩展到平均奖励情况，统一了两种分析框架，证明了在唯一且遍历的最优策略下，值迭代算法具有几何收敛率。


<details>
  <summary>Details</summary>
Motivation: 传统上马尔可夫决策过程的理论分析分为平均奖励和折扣奖励两种情况，通常分开分析。本文旨在统一这两种情况的分析框架。

Method: 将折扣奖励MDP的几何解释扩展到平均奖励情况，建立统一的分析框架。

Result: 证明了在唯一且遍历的最优策略下，值迭代算法在平均奖励情况下也能达到几何收敛率。
...
### [199] [Differential Privacy: Gradient Leakage Attacks in Federated Learning Environments](https://arxiv.org/abs/2510.23931)
*Miguel Fernandez-de-Retana,Unai Zulaika,Rubén Sánchez-Corcuera,Aitor Almeida*

Main category: cs.LG

TL;DR: 本文研究了联邦学习中差分隐私机制（DP-SGD和PDP-SGD）作为防御梯度泄漏攻击的有效性，发现DP-SGD能显著降低攻击风险但牺牲模型性能，而PDP-SGD保持良好性能但防御效果不佳。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然能保护敏感数据，但仍易受梯度泄漏攻击威胁，需要评估差分隐私机制在实际应用中的防御效果。
...
### [200] [ChessQA: Evaluating Large Language Models for Chess Understanding](https://arxiv.org/abs/2510.23948)
*Qianfeng Wen,Zhenwei Tang,Ashton Anderson*

Main category: cs.LG

TL;DR: ChessQA是一个综合性基准测试，用于评估大型语言模型在象棋理解方面的能力，涵盖五个任务类别：结构、模式、短战术、位置判断和语义，对应玩家积累象棋知识时的抽象层次。
...
Method: 开发了ChessQA基准测试，包含五个任务类别，对应象棋知识的不同抽象层次。该基准具有动态性，提示、答案密钥和构建脚本可以随着模型改进而演进。

Result: 评估一系列当代LLM，发现在所有五个类别中都存在持续弱点，并按类别提供了结果和错误分析。

Conclusion: ChessQA提供了比先前简单的走子质量评估更全面的象棋能力理解图景，为诊断和比较提供了受控、一致的设置，并将发布代码、定期更新的数据集和公共排行榜以支持进一步研究。
...
### [201] [A Pragmatic Way to Measure Chain-of-Thought Monitorability](https://arxiv.org/abs/2510.23966)
...
<details>
  <summary>Details</summary>
Motivation: 思维链监控为AI安全提供了独特机会，但训练实践或模型架构的变化可能削弱这种可监控性，需要开发方法来持续跟踪和保持监控能力。

Method: 使用自动评分器提示词，让任何有能力的LLM计算现有CoT的可读性和覆盖度，并在合成CoT退化上进行验证测试。

Result: 在多个前沿模型的挑战性基准测试中，发现这些模型表现出较高的可监控性。

Conclusion: 该方法有助于测量CoT的默认可监控性，应被视为对抗性压力测试的补充工具，而非替代品，用于跟踪设计决策对可监控性的影响。
...
### [202] [An efficient probabilistic hardware architecture for diffusion-like models](https://arxiv.org/abs/2510.23972)
...
Result: 系统级分析显示，该架构在简单图像基准测试中能达到与GPU相当的性能，同时能耗降低约10,000倍

Conclusion: 该全晶体管概率计算机架构解决了现有方案的局限性，有望实现概率计算的高能效实现
...
### [203] [Diffusion Adaptive Text Embedding for Text-to-Image Diffusion Models](https://arxiv.org/abs/2510.23974)
...
<details>
  <summary>Details</summary>
Motivation: 传统文本到图像扩散模型使用固定的文本嵌入，限制了文本条件在生成过程中的适应性。
...
### [204] [Synergistic Neural Forecasting of Air Pollution with Stochastic Sampling](https://arxiv.org/abs/2510.23977)
*Yohan Abeysinghe,Muhammad Akhtar Munir,Sanoojan Baliah,Ron Sarafian,Fahad Shahbaz Khan,Yinon Rudich,Salman Khan*

Main category: cs.LG

TL;DR: SynCast是一个高分辨率神经预测模型，集成了气象和空气成分数据，通过区域适应的transformer架构和基于扩散的随机精炼模块，显著提高了对平均和极端污染水平的预测准确性。


<details>
  <summary>Details</summary>
Motivation: 空气污染是全球主要的健康和环境风险，特别是在易受野火、城市雾霾和沙尘暴影响的地区。现有模型往往低估了罕见但危险的污染事件，需要更准确的PM浓度预测来支持及时的公共卫生预警和干预。

Method: 基于区域适应的transformer架构，集成ERA5和CAMS数据集，采用基于扩散的随机精炼模块，结合领域感知目标和极值理论来捕捉PM激增的非线性动态。

Result: 模型在多个PM变量（PM1、PM2.5、PM10）的预测保真度方面显示出显著提升，特别是在极端条件下。传统损失函数低估了分布尾部（罕见污染事件），而SynCast在高度受影响区域的性能显著增强，同时不损害全局准确性。
...
TL;DR: 本文提出了一种新的组合多臂老虎机消除算法，通过将臂分为确认、活跃和消除三类，并加入显式探索来更新这些集合。该算法在图反馈组合多臂老虎机和线性上下文组合老虎机中均实现了接近最优的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 组合老虎机扩展了经典老虎机框架，学习者在每轮选择多个臂，应用于在线推荐和品类优化等场景。虽然UCB算法在组合老虎机中自然扩展，但臂消除方法的适应更具挑战性。现有UCB方法由于缺乏显式探索可能失败。

Method: 引入新颖的消除方案，将臂分为确认、活跃和消除三类，并整合显式探索来更新这些集合。该方法适用于图反馈组合多臂老虎机和线性上下文组合老虎机两种设置。

Result: 在两种设置下，所提方法都实现了接近最优的遗憾界。相比之下，基于UCB的方法由于缺乏足够的显式探索可能被证明会失败。同时提供了匹配的下界。

Conclusion: 提出的消除算法在组合老虎机中有效解决了UCB方法因缺乏显式探索而失败的问题，在多个设置下实现了接近最优的性能，并通过下界证明了其最优性。
...
### [206] [Predicting Barge Tow Size on Inland Waterways Using Vessel Trajectory Derived Features: Proof of Concept](https://arxiv.org/abs/2510.23994)
*Geoffery Agorku,Sarah Hernandez,Hayley Hames,Cade Wagner*

Main category: cs.LG

TL;DR: 本研究提出了一种使用AIS数据和机器学习预测内河驳船数量的新方法，通过特征工程和模型训练，Poisson回归模型在12个特征上实现了1.92的MAE。
...
Method: 使用AIS船舶跟踪数据，通过时空匹配程序将卫星图像标注的驳船实例与AIS轨迹匹配，创建30个AIS衍生特征，使用递归特征消除选择最具预测性的变量，训练6种回归模型。

Result: Poisson回归模型表现最佳，使用12个特征实现了1.92的MAE。特征重要性分析显示，捕获船舶机动性的指标（如航向熵、速度变异性和行程长度）对驳船数量最具预测性。

Conclusion: 该方法为增强海事领域感知提供了一种可扩展、易于实施的方法，在船闸调度、港口管理和货运规划方面具有强大应用潜力。未来工作将扩展到其他内河以探索模型的可转移性。
...
### [207] [Spatio-temporal Multivariate Time Series Forecast with Chosen Variables](https://arxiv.org/abs/2510.24027)
*Zibo Liu,Zhe Jiang,Zelin Xu,Tingsong Xiao,Yupu Zhang,Zhengkun Xiao,Haibo Wang,Shigang Chen*

Main category: cs.LG

TL;DR: 本文提出了一种新的时空多元时间序列预测问题，研究如何在预算约束下从n个位置中选出m个变量（传感器位置）作为模型输入，以最大化预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有研究假设模型输入的m个变量是预先确定的，但如何选择这m个变量的问题从未被研究过。本文填补了这一空白，研究STMF中变量选择的问题。

Method: 提出了一个统一框架，包含三个技术组件：(1) 掩码变量参数剪枝，通过基于分位数的掩码逐步剪枝信息量较少的变量和注意力参数；(2) 优先变量参数重放，重放低损失的过去样本以保持学习知识的模型稳定性；(3) 动态外推机制，通过可学习的空间嵌入和邻接信息将信息从选定的输入变量传播到所有其他变量。

Result: 在五个真实世界数据集上的实验表明，本文方法在准确性和效率方面显著优于现有最先进的基线方法。

Conclusion: 联合变量选择和模型优化的方法在时空多元时间序列预测中具有有效性，能够同时提高预测精度和模型效率。
...
### [208] [GraphNet: A Large-Scale Computational Graph Dataset for Tensor Compiler Research](https://arxiv.org/abs/2510.24035)
*Xinqi Li,Yiqun Liu,Shan Jiang,Enrong Zheng,Huaijin Zheng,Wenhao Dai,Haodong Deng,Dianhai Yu,Yanjun Ma*

Main category: cs.LG

TL;DR: GraphNet是一个包含2.7K真实深度学习计算图的数据集，提出了Speedup Score S(t)和Error-aware Speedup Score ES(t)两个评估指标，并在CV和NLP任务上对CINN和TorchInductor编译器进行了基准测试。


<details>
  <summary>Details</summary>
Motivation: 为了评估张量编译器在真实深度学习工作负载上的性能，需要包含丰富元数据的计算图数据集和可靠的评估指标。

Method: 构建GraphNet数据集，包含2.7K个跨多个深度学习框架的计算图；提出S(t)和ES(t)评估指标，综合考虑运行速度提升和执行正确性；在CV和NLP任务上对CINN和TorchInductor进行基准测试。

Result: 开发了包含完整构建流程的数据集和评估工具，展示了GraphNet在评估张量编译器性能方面的实用性。

Conclusion: GraphNet为张量编译器性能评估提供了标准化基准，提出的评估指标能够可靠地衡量编译器的通用优化能力，并帮助识别性能瓶颈。
...
### [209] [Localized Kernel Projection Outlyingness: A Two-Stage Approach for Multi-Modal Outlier Detection](https://arxiv.org/abs/2510.24043)
*Akira Tamamori*

Main category: cs.LG

TL;DR: 提出了Two-Stage LKPLO多阶段异常检测框架，通过结合广义损失函数、全局核PCA和局部聚类，解决了传统投影方法依赖固定统计指标和单一数据结构的限制。


<details>
  <summary>Details</summary>
Motivation: 克服传统投影异常检测方法的两大局限：依赖固定统计指标和假设单一数据结构，需要更灵活、适应复杂数据结构的检测方法。

Method: 三阶段方法：1) 广义损失函数PLO替代固定指标；2) 全局核PCA线性化非线性结构；3) 局部聚类处理多模态分布。

Result: 在10个基准数据集上的5折交叉验证显示，该方法在具有挑战性结构的数据集上显著优于基线方法，特别是在多聚类和复杂高维数据上表现突出。
...
### [210] [Mitigating Negative Transfer via Reducing Environmental Disagreement](https://arxiv.org/abs/2510.24044)
*Hui Sun,Zheng Xie,Hao-Yuan He,Ming Li*

Main category: cs.LG

TL;DR: 该论文提出了一种名为RED的方法，通过因果解缠学习来减少环境特征分歧，从而缓解无监督域自适应中的负迁移问题。
...
Conclusion: 通过因果解缠学习减少环境特征分歧是缓解无监督域自适应中负迁移问题的有效方法。
...
### [211] [Causal-Aware Generative Adversarial Networks with Reinforcement Learning](https://arxiv.org/abs/2510.24046)
*Tu Anh Hoang Nguyen,Dang Nguyen,Tri-Nhan Vo,Thuc Duy Le,Sunil Gupta*

Main category: cs.LG

TL;DR: CA-GAN是一个专门为表格数据设计的生成框架，通过因果图提取和条件WGAN-GP结合强化学习目标，在保持数据效用和隐私的同时捕获复杂因果关系。
...
Method: 采用两步法：1）因果图提取学习数据流形中的因果关系；2）基于因果图结构的条件WGAN-GP，并结合强化学习目标对齐真实和生成数据的因果图。

Result: 在14个表格数据集上优于6种最先进方法，在因果保持、效用保持和隐私保持等核心数据工程指标上表现优异。

Conclusion: CA-GAN为数据工程师提供了一个实用、高性能的解决方案，可创建高质量、符合隐私要求的合成数据集，用于数据库系统基准测试、软件开发加速和安全数据驱动研究。
...
### [212] [Low-N Protein Activity Optimization with FolDE](https://arxiv.org/abs/2510.24053)
...
Conclusion: FolDE通过改进的预热启动和批量选择策略，显著提高了蛋白质定向进化的效率，为任何实验室提供了高效蛋白质优化的开源解决方案。
...
### [213] [Information-Theoretic Discrete Diffusion](https://arxiv.org/abs/2510.24088)
*Moongyu Jeon,Sangwoo Shin,Dongjae Jeon,Albert No*

Main category: cs.LG

TL;DR: 提出了一个离散扩散模型的信息论框架，通过得分匹配损失得到对数似然的理论估计器。推导了离散设置下的I-MDSE和I-MDCE关系，将互信息与去噪得分熵损失联系起来，证明常用损失函数是对数似然的紧致估计器。


<details>
  <summary>Details</summary>
Motivation: 受高斯设置中I-MMSE恒等式的启发，旨在为离散扩散模型建立类似的信息论框架，为得分匹配损失提供理论依据。

Method: 引入信息-最小去噪得分熵(I-MDSE)关系，将数据与其扩散版本之间的互信息与最小去噪得分熵损失联系起来；扩展到掩码扩散，建立信息-最小去噪交叉熵(I-MDCE)关系。

Result: 实验在合成和真实世界数据上验证了估计器的准确性、方差稳定性和实用性。

Conclusion: 该框架表明常用的DSE和DCE损失不仅是变分界，而且是紧致且有理论依据的对数似然估计器，支持时间无关公式、条件似然估计和耦合蒙特卡洛估计等实际扩展。
...
### [214] [What do vision-language models see in the context? Investigating multimodal in-context learning](https://arxiv.org/abs/2510.24331)
*Gabriel O. dos Santos,Esther Colombini,Sandra Avila*

Main category: cs.LG

TL;DR: 本文系统研究了视觉语言模型（VLMs）中的上下文学习（ICL），评估了7个模型在3个图像描述基准上的表现，分析了提示设计、架构选择和训练策略对多模态ICL的影响。


<details>
  <summary>Details</summary>
Motivation: 尽管上下文学习在大型语言模型（LLMs）中已被广泛研究，但在视觉语言模型（VLMs）中的有效性仍未被充分探索。本文旨在填补这一研究空白。

Method: 评估了涵盖四种架构的七个VLMs模型，在三个图像描述基准上进行测试；分析了提示设计、架构选择和训练策略的影响；首次分析了VLMs中注意力模式随上下文演示数量增加的变化。

Result: 训练图像-文本交错数据能提升ICL性能，但不意味着能有效整合演示示例中的视觉和文本信息；指令调优改善了指令跟随能力，但可能减少对上下文演示的依赖，表明指令对齐和上下文适应之间存在权衡；注意力分析显示当前VLMs主要关注文本线索，未能充分利用视觉信息。

Conclusion: 当前VLMs在上下文学习能力上存在关键限制，特别是多模态整合能力有限，这为增强从多模态上下文示例中学习的能力提供了重要见解。
...
### [215] [Learning Parameterized Skills from Demonstrations](https://arxiv.org/abs/2510.24095)
*Vedant Gupta,Haotian Fu,Calvin Luo,Yiding Jiang,George Konidaris*

Main category: cs.LG

TL;DR: DEPS是一种从专家演示中学习参数化技能的端到端算法，通过联合学习技能策略和元策略来提升未见任务的泛化能力。
...
Result: 在多任务LIBERO和MetaWorld基准测试中优于多任务和技能学习基线方法，显著提高了对未见任务的泛化能力。
...
### [216] [Graph-Guided Concept Selection for Efficient Retrieval-Augmented Generation](https://arxiv.org/abs/2510.24120)
*Ziyu Liu,Yijing Liu,Jianfei Yuan,Minzhi Yan,Le Yue,Honghui Xiong,Yi Yang*

Main category: cs.LG

TL;DR: G2ConS是一种图引导的概念选择方法，通过选择重要文档块和构建LLM无关的概念图，显著降低了基于图的RAG知识图谱构建成本，同时保持甚至提升了检索和问答质量。


<details>
  <summary>Details</summary>
Motivation: 基于图的RAG方法在构建知识图谱时需要大量LLM调用来提取实体和关系，这在规模应用时成本过高。研究发现某些概念词及其相关文档更重要，因此需要一种方法来降低构建成本而不牺牲性能。

Method: 提出G2ConS方法，包含两个核心组件：1) 块选择方法，选择重要的文档块以减少KG构建成本；2) LLM无关的概念图，在零成本下弥补块选择引入的知识空白。

Result: 在多个真实世界数据集上的评估显示，G2ConS在构建成本、检索效果和问答质量方面均优于所有基线方法。

Conclusion: G2ConS通过智能块选择和概念图构建，有效解决了图基RAG方法的高成本问题，为领域特定文档的多跳推理检索提供了实用的解决方案。
...
TL;DR: 本文研究了联邦学习在异构数据环境下的性能评估问题，提出了一个考虑本地性能和泛化能力的综合评估框架，并引入了带有个性化因子的改进算法FLIU。


<details>
  <summary>Details</summary>
Motivation: 在异构数据联邦学习中，本地模型会收敛到各自的局部最优解，偏离全局数据分布。现有个性化联邦学习方法主要关注本地性能，而忽略了FedAvg带来的泛化能力这一重要鲁棒性优势。

Method: 本文深入评估了联邦学习方法在本地性能和泛化能力两方面的表现，分析了单个通信轮次中的不同阶段。提出了FLIU算法，通过自适应个性化因子扩展了FedAvg，增加了个性化步骤。

Result: 使用MNIST和CIFAR-10在不同分布条件下进行了实证评估，包括基准IID、病理性非IID以及专门设计的Dirichlet分布测试环境，以测试算法在复杂数据异构性下的表现。

Conclusion: 研究强调了在联邦学习评估中同时考虑本地性能和泛化能力的重要性，提出的FLIU算法通过个性化因子有效平衡了这两种性能指标。
...
### [218] [EddyFormer: Accelerated Neural Simulations of Three-Dimensional Turbulence at Scale](https://arxiv.org/abs/2510.24173)
...
Method: 提出EddyFormer，一种基于Transformer的谱元架构，引入SEM标记化将流动分解为网格尺度和亚网格尺度分量，能够捕捉局部和全局特征。创建新的三维各向同性湍流数据集进行训练。
...
Conclusion: EddyFormer成功结合了谱方法的精度和Transformer的可扩展性，为大规模湍流模拟提供了高效准确的解决方案，具有显著的领域泛化能力。
...
### [219] [V-SAT: Video Subtitle Annotation Tool](https://arxiv.org/abs/2510.24180)
...
Method: 结合大型语言模型(LLM)、视觉语言模型(VLM)、图像处理和自动语音识别(ASR)，利用音频和视频的上下文信息来检测和纠正字幕质量问题。

Result: 字幕质量显著改善，SUBER分数从9.6降至3.54，图像模式问题的F1分数达到约0.80。

Conclusion: V-SAT提供了首个全面的字幕标注解决方案，通过人机协作确保高质量结果，为视频字幕质量提升提供了有效工具。
...
### [220] [SPEAR++: Scaling Gradient Inversion via Sparsely-Used Dictionary Learning](https://arxiv.org/abs/2510.24200)
*Alexander Bakarsky,Dimitar I. Dimitrov,Maximilian Baader,Martin Vechev*

Main category: cs.LG

TL;DR: SPEAR++攻击改进了联邦学习中的梯度反演攻击，通过应用稀疏字典学习技术，将原本指数级复杂度的SPEAR攻击变为可处理，能够处理10倍更大的批次大小，同时保持对DP噪声和FedAvg聚合的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然实现了分布式训练而无需显式数据共享，但梯度反演攻击挑战了其隐私保护特性。现有的SPEAR攻击虽然理论突破，但受限于指数级运行时间，无法处理实际场景中的大批次大小。

Method: 应用最先进的稀疏字典学习技术来解决带有ReLU激活的线性层梯度反演问题，使原本指数级复杂度的问题变得可处理。

Result: SPEAR++攻击保留了SPEAR的所有理想特性（对DP噪声和FedAvg聚合的鲁棒性），同时能够处理比SPEAR大10倍的批次大小。

Conclusion: SPEAR++填补了SPEAR攻击在实际应用中的局限性，使梯度反演攻击在更大批次规模下变得可行，对联邦学习的隐私保护构成了更现实的威胁。
...
### [221] [Unlocking Out-of-Distribution Generalization in Dynamics through Physics-Guided Augmentation](https://arxiv.org/abs/2510.24216)
*Fan Xu,Hao Wu,Kun Wang,Nan Wang,Qingsong Wen,Xian Wu,Wei Gong,Xibin Zhao*

Main category: cs.LG

TL;DR: SPARK是一种物理引导的定量增强插件，通过重建自编码器将物理参数整合到物理丰富的离散状态字典中，在潜在空间进行插值生成物理合理的训练样本，并与傅里叶增强图ODE结合进行下游预测，在数据稀缺和分布偏移场景下显著优于现有方法。
...
Method: 1. 使用重建自编码器将物理参数整合到物理丰富的离散状态字典；2. 在潜在空间进行原则性插值生成新的物理合理训练样本；3. 与傅里叶增强图ODE结合进行下游预测，建模增强数据分布并捕获长期时间依赖。

Result: 在多样化基准测试上的广泛实验表明，SPARK显著优于最先进的基线方法，特别是在具有挑战性的分布外场景和数据稀缺情况下。

Conclusion: 物理引导的增强范式被证明是有效的，能够显著提升在数据稀缺和分布偏移情况下的模型性能。
...
### [222] [Closing Gaps: An Imputation Analysis of ICU Vital Signs](https://arxiv.org/abs/2510.24217)
...
<details>
  <summary>Details</summary>
Motivation: ICU数据质量不足阻碍了机器学习在临床预测中的应用，许多生命体征测量数据存在大量缺失段，现有插补技术缺乏全面比较，临床实践中仍使用可能降低预测准确性的临时插补方法。

Method: 建立了一个可扩展和可重用的基准，包含15种插补方法和4种缺失方法，专门用于在主要ICU数据集上进行基准测试。

Result: 通过系统比较确立了不同插补技术的性能表现，为研究人员提供了选择最准确插补方法的指导。

Conclusion: 该工作为ICU生命体征数据插补提供了比较基础，有助于推动更多机器学习模型进入临床实践。
...
### [223] [PRIVET: Privacy Metric Based on Extreme Value Theory](https://arxiv.org/abs/2510.24233)
...
TL;DR: 提出PRIVET方法，通过极值统计和最近邻距离为每个合成样本分配个体隐私泄露分数，可靠检测记忆化和隐私泄露问题。


<details>
  <summary>Details</summary>
Motivation: 深度生成模型在敏感数据上训练引发隐私担忧，现有方法依赖全局标准且缺乏可解释性，缺乏样本级隐私评估方法阻碍合成数据在实际应用中的部署。

Method: 使用极值统计和最近邻距离，提出PRIVET算法，该算法基于样本、模态无关，为每个合成样本分配个体隐私泄露分数。

Result: PRIVET在各种数据模态下可靠检测记忆化和隐私泄露，包括高维数据、小样本遗传数据，甚至在欠拟合情况下也有效。相比现有方法，在提供数据集级和样本级评估方面具有优势。

Conclusion: PRIVET提供了一种有效的样本级隐私评估方法，填补了现有方法的空白，有助于合成数据在实际应用中的安全部署。
...
### [224] [Sparse Optimistic Information Directed Sampling](https://arxiv.org/abs/2510.24234)
*Ludovic Schwartz,Hamish Flynn,Gergely Neu*

Main category: cs.LG

TL;DR: 本文提出了稀疏乐观信息导向采样（SOIDS）算法，能够在数据丰富和数据贫乏两种情况下同时实现最优最坏情况遗憾，无需贝叶斯假设。


<details>
  <summary>Details</summary>
Motivation: 现有算法只能在数据丰富或数据贫乏其中一种情况下达到最优遗憾，无法同时适应两种情况。稀疏IDS算法虽然能在贝叶斯设置下同时适应两种情况，但需要贝叶斯假设。

Method: 使用稀疏乐观信息导向采样（SOIDS）算法，通过引入时间依赖的学习率来平衡信息和遗憾。

Result: SOIDS算法在理论上能够同时实现数据丰富和数据贫乏情况下的最优最坏情况遗憾，并通过实验验证了其良好性能。

Conclusion: SOIDS算法扩展了IDS的理论保证，是第一个在两种情况下同时实现最优最坏情况遗憾的算法，无需贝叶斯假设。
...
### [225] [PaTaRM: Bridging Pairwise and Pointwise Signals via Preference-Aware Task-Adaptive Reward Modeling](https://arxiv.org/abs/2510.24235)
...
Method: 结合偏好感知奖励机制和任务自适应标准系统，利用成对数据中的相对偏好信息构建点对训练信号，同时灵活生成评估标准。
...
### [226] [SALS: Sparse Attention in Latent Space for KV cache Compression](https://arxiv.org/abs/2510.24273)
*Junlin Mu,Hantao Huang,Jihang Zhang,Minghui Yu,Tao Wang,Yidong Li*

Main category: cs.LG

TL;DR: SALS是一种在潜在空间中执行稀疏注意力的框架，通过低秩投影将KV缓存压缩到紧凑的潜在空间，并在该空间中使用无RoPE的查询-键交互进行稀疏令牌选择，从而避免完整KV缓存重建的开销。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理长上下文时面临KV缓存大小和内存带宽的挑战，而现有的低秩压缩方法由于RoPE机制导致精度严重下降或产生新的速度瓶颈。

Method: 提出SALS框架：1）将KV缓存通过低秩投影到紧凑潜在空间；2）在该空间中使用无RoPE的查询-键交互进行稀疏令牌选择；3）仅重建重要令牌子集。

Result: 在多个任务上评估显示，SALS在保持竞争性精度的同时实现SOTA性能：在4K序列上实现6.4倍KV缓存压缩和5.7倍注意力算子加速；在端到端吞吐量上，相比GPT-fast在4K和32K序列上分别实现1.4倍和4.5倍提升。

Conclusion: SALS通过将注意力计算转移到潜在空间并避免完整KV缓存重建，有效解决了长上下文LLM推理中的KV缓存压缩和速度瓶颈问题。
...
### [227] [EDC: Equation Discovery for Classification](https://arxiv.org/abs/2510.24310)
...
Method: 使用中等复杂度的语法，包含线性、二次、指数项以及特征乘积项，构建可配置的模型结构，特别适合捕获XOR类依赖关系。

Result: EDC能够同时发现目标方程的结构和参数值，在二元分类中性能优于现有ED方法，并与最先进的二元分类方法性能相当。
...
### [228] [Perception Learning: A Formal Separation of Sensory Representation Learning from Decision Learning](https://arxiv.org/abs/2510.24356)
*Suman Sanyal*
...
<details>
  <summary>Details</summary>
Motivation: 传统方法将感知和决策耦合在一起优化，这可能导致感知模块无法独立发展出鲁棒的感知能力。PeL旨在分离感知和决策学习，让感知模块能够专注于发展稳定的感知特性。

Method: PeL通过定义与目标函数或参数化无关的感知属性（如对干扰的稳定性、信息性而不崩溃、可控几何结构），并使用表示不变性指标来评估这些属性，从而独立优化感官接口。

Result: 证明了PeL更新在保持足够不变性的情况下与贝叶斯任务风险梯度正交，并提供了一套任务无关的评估指标来验证感知质量。

Conclusion: PeL提供了一种新的学习范式，能够独立优化感知模块，发展出更鲁棒和通用的感知能力，为构建更可靠的智能系统奠定了基础。
...
### [229] [Filtering instances and rejecting predictions to obtain reliable models in healthcare](https://arxiv.org/abs/2510.24368)
...
TL;DR: 提出一种两阶段数据驱动方法，通过实例难度过滤和置信度拒绝机制提升医疗领域机器学习模型的可靠性。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在高风险医疗领域应用时，经常忽略不确定性，即使置信度低也提供预测，这会影响模型可靠性。

Method: 1. 训练阶段使用实例难度(IH)过滤问题实例；2. 推理阶段引入置信度拒绝机制，只保留可靠预测。

Result: 在三个真实医疗数据集上验证，该方法能有效提升模型可靠性，同时平衡预测性能和拒绝率。

Conclusion: 该方法为安全关键应用中的机器学习系统部署提供了实用解决方案，能有效增强模型性能并保留大部分实例。
...
### [230] [A Comprehensive Evaluation Framework for Synthetic Trip Data Generation in Public Transport](https://arxiv.org/abs/2510.24375)
*Yuanyuan Wu,Zhenlin Qin,Zhenliang Ma*

Main category: cs.LG

TL;DR: 提出了一个代表性-隐私-效用（RPU）框架，系统评估公共交通合成出行数据在三个维度和三个层级上的质量，发现合成数据不能自动保证隐私，需要在隐私与代表性/效用之间权衡。


<details>
  <summary>Details</summary>
Motivation: 解决合成数据在公共交通研究中缺乏全面评估的问题，现有评估通常只关注人口层面的代表性或记录层面的隐私，而忽略了群体层面的变化和任务特定的效用。

Method: 提出了RPU框架，在三个互补维度（代表性、隐私、效用）和三个层级（记录、群体、人口）上系统评估合成出行数据，整合了一致的指标来量化相似性、披露风险和实践有用性。

Result: 对12种代表性生成方法进行基准测试，结果表明合成数据不能固有地保证隐私，不存在"一刀切"的模型，隐私与代表性/效用之间的权衡很明显。CTGAN提供了最平衡的权衡，建议用于实际应用。

Conclusion: RPU框架为研究人员和从业者提供了系统和可重复的基础，用于比较合成数据生成技术并在公共交通应用中选择适当的方法。
...
TL;DR: 提出一种使用少量成功演示来初始化强化学习智能体价值函数的简单有效方法，通过离线演示预计算价值估计作为早期学习目标，在稀疏奖励环境中显著提高样本效率。


<details>
  <summary>Details</summary>
Motivation: 稀疏奖励环境中的强化学习面临重大挑战，因为缺乏信息反馈。需要解决探索负担重和样本效率低的问题。

Method: 使用少量成功演示预计算价值估计，将其作为强化学习智能体早期学习的目标，提供有前景动作的先验知识，然后通过标准在线交互精炼这些估计。

Result: 在基准任务上的实验表明，该方法加速了收敛速度，即使使用最少或次优的演示数据，也优于标准基线方法。

Conclusion: 这种混合离线到在线范式显著减少了稀疏奖励设置中的探索负担，提高了样本效率，为稀疏奖励环境中的强化学习提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统生存分析方法在处理复杂数据时存在局限性，需要评估机器学习模型在生存分析中的表现，以改进生存预测和决策支持。

Method: 使用巴西圣保罗医院癌症登记处近45,000名结直肠癌患者数据，评估了六种机器学习生存分析模型（RSF、GBSA、SSVM、XGB-Cox、XGB-AFT、LGBM），采用不同采样器进行超参数优化，使用C-Index、C-Index IPCW、时间相关AUC和IBS评估性能，并与分类算法预测的生存曲线进行比较，使用SHAP和排列重要性进行预测因子解释。

Result: XGB-AFT模型表现最佳（C-Index = 0.7618；IPCW = 0.7532），其次是GBSA和RSF模型。

Conclusion: 机器学习生存分析方法在改善生存预测和支持决策制定方面具有显著潜力和应用价值。
...
### [233] [MIMIC-Sepsis: A Curated Benchmark for Modeling and Learning from Sepsis Trajectories in the ICU](https://arxiv.org/abs/2510.24500)
...
<details>
  <summary>Details</summary>
Motivation: 现有脓毒症研究存在数据集过时、预处理流程不可重复、临床干预覆盖有限等问题，需要构建一个标准化的基准框架来支持可重复的脓毒症建模研究。

Method: 基于Sepsis-3标准构建透明的预处理流程，包括结构化插补策略和治疗纳入方法，提供早期死亡率预测、住院时间估计和休克发作分类等基准任务。
...
### [234] [LoRA-DA: Data-Aware Initialization for Low-Rank Adaptation via Asymptotic Analysis](https://arxiv.org/abs/2510.24561)
...
Method: 建立基于渐近分析的理论框架，从最小化微调模型与目标模型参数差异的优化目标出发，推导包含偏置项和方差项的优化问题，开发LoRA-DA算法从小规模目标域样本估计优化项并获取最优LoRA初始化。

Result: 在多个基准测试中，LoRA-DA相比现有初始化方法持续提升最终精度，表现出更快更稳定的收敛性、跨秩鲁棒性，且初始化开销很小。

Conclusion: LoRA-DA通过数据感知的初始化策略有效解决了现有方法的局限性，为LoRA初始化提供了理论基础和实用算法，显著提升了微调性能。
...
### [235] [DistDF: Time-Series Forecasting Needs Joint-Distribution Wasserstein Alignment](https://arxiv.org/abs/2510.24574)
...
<details>
  <summary>Details</summary>
Motivation: 传统直接预测方法在存在标签自相关时，条件负对数似然的估计存在偏差，需要新的方法来更好地对齐预测分布与标签分布。

Method: 提出DistDF方法，通过最小化条件预测分布与标签分布之间的差异来实现对齐。引入联合分布Wasserstein差异来上界条件差异，该差异可从经验样本中进行可微估计，并与基于梯度的训练无缝集成。

Result: 大量实验表明，DistDF提高了多种预测模型的性能，并达到了最先进的预测性能。

Conclusion: DistDF通过最小化分布差异而非直接优化条件负对数似然，有效解决了标签自相关导致的估计偏差问题，显著提升了时间序列预测的准确性。
...
### [236] [Physics-Informed Extreme Learning Machine (PIELM): Opportunities and Challenges](https://arxiv.org/abs/2510.24577)
*He Yang,Fei Ren,Hai-Sui Yu,Xiaohui Chen,Pei-Zhi Zhuang*

Main category: cs.LG

TL;DR: 本文是对物理信息极端学习机（PIELM）发展的综述，分析了其在求解偏微分方程方面的优势、当前进展和未来挑战。


<details>
  <summary>Details</summary>
Motivation: 由于目前缺乏对物理信息极端学习机（PIELM）的总结性文献，作者希望分享对这一有前景研究方向的观点和经验。

Method: 通过综述分析PIELM在求解具有尖锐梯度、非线性、高频行为、硬约束、不确定性和多物理场耦合的偏微分方程方面的应用方法。

Result: PIELM在计算效率和精度方面取得了显著进展，成功应用于多种复杂物理问题的求解。

Conclusion: 尽管PIELM已取得成功，但仍面临许多紧迫挑战，这为开发更鲁棒、可解释和可泛化的PIELM框架提供了机会，有望在科学和工程领域得到更广泛应用。
...
### [237] [Symbolic Snapshot Ensembles](https://arxiv.org/abs/2510.24633)
*Mingyue Liu,Andrew Cropper*

Main category: cs.LG

TL;DR: 本文提出了一种新的归纳逻辑编程（ILP）集成学习方法，通过单次训练保存中间假设，并使用最小描述长度加权方案组合这些假设，在多个基准测试中实现了4%的预测准确率提升，且计算开销不到1%。
...
Result: 在包括游戏玩法和视觉推理在内的多个基准测试中，该方法将预测准确率提高了4%，同时计算开销不到1%。

Conclusion: 该方法提供了一种计算效率高的ILP集成学习方案，能够在几乎不增加计算成本的情况下显著提升模型性能。
...
### [238] [Causal Ordering for Structure Learning From Time Series](https://arxiv.org/abs/2510.24639)
...
<details>
  <summary>Details</summary>
Motivation: 时序数据中的因果发现对于理解复杂现象至关重要，但传统基于排序的方法限制了模型的表示能力，需要解决这个问题。

Method: 提出DOTS方法，利用扩散过程进行时序因果发现，整合多个有效因果排序，使用得分匹配和扩散过程进行高效的Hessian估计。
...
### [239] [Pearl: A Foundation Model for Placing Every Atom in the Right Location](https://arxiv.org/abs/2510.24670)
...
TL;DR: Pearl是一种用于蛋白质-配体共折叠的基础模型，通过大规模合成数据训练、SO(3)等变扩散架构和可控推理等创新，在蛋白质-配体复合物结构预测方面实现了最先进的性能。
...
Method: Pearl采用三个关键创新：1）包含大规模合成数据的训练方案；2）集成SO(3)等变扩散模块的架构，尊重3D旋转对称性；3）可控推理系统，支持多链模板和双无条件/条件模式。

Result: 在生成准确（RMSD < 2Å）且物理有效的姿态方面，Pearl在Runs N' Poses和PoseBusters基准测试中分别比AlphaFold 3和其他开源基线提升了14.5%和14.2%。在口袋条件共折叠机制下，在更严格的RMSD < 1Å阈值下，对具有挑战性的真实药物靶点实现了3.6倍的改进。
...
### [240] [Eigenfunction Extraction for Ordered Representation Learning](https://arxiv.org/abs/2510.24672)
*Burak Varıcı,Che-Ping Tsai,Ritabrata Ray,Nicholas M. Boffi,Pradeep Ravikumar*

Main category: cs.LG

TL;DR: 本文提出了一个通用框架来提取有序且可识别的特征函数，通过模块化构建块满足关键需求，包括与上下文核的兼容性和现代设置的可扩展性。验证表明提取的特征值可作为有效的特征重要性评分，实现自适应维度表示。
...
Method: 提出了基于模块化构建块的通用框架来提取有序可识别的特征函数，包括低秩近似和瑞利商优化两种主要方法范式。

Result: 在合成核和真实世界图像数据集上验证了方法的有效性，提取的特征值可作为特征选择的重要性评分，实现效率-准确性的权衡。

Conclusion: 该框架能够实现精确的谱分解，提取有序可识别的特征函数，为特征重要性评估和自适应维度表示提供了理论基础。
...
### [241] [Learning to Drive Safely with Hybrid Options](https://arxiv.org/abs/2510.24674)
*Bram De Cooman,Johan Suykens*

Main category: cs.LG

TL;DR: 该论文将选项框架应用于高速公路自动驾驶任务，定义了纵向和横向操作的专用选项，并提出了几种分层控制设置，其中基于混合选项的灵活策略在不同交通条件下表现最佳。


<details>
  <summary>Details</summary>
Motivation: 虽然深度强化学习在自动驾驶中有许多应用，但很少使用选项框架，而该框架天然适合分层控制应用，特别是自动驾驶任务。

Method: 为纵向和横向操作定义专用选项，嵌入安全和舒适约束；提出几种分层控制设置，遵循最先进的强化学习技术推导实用算法；通过分别选择纵向和横向控制动作，引入基于组合和混合选项的策略。

Result: 在所有研究方法中，基于混合选项的灵活策略在不同交通条件下表现最佳，优于基于动作的基线策略。

Conclusion: 选项框架可以有效地将先验领域知识融入学习过程，更容易约束学习到的驾驶行为，同时保持与人类驾驶员相同的表达能力和灵活性，且比基于连续动作的经典策略更容易解释。
...
### [242] [Near Optimal Hardness of Approximating $k$-CSP](https://arxiv.org/abs/2510.23991)
*Dor Minzer,Kai Zhe Zheng*

Main category: cs.CC

TL;DR: 该论文证明了对于任意k和ε>0，在大字母表R下，区分k-CSP至少满足1-ε约束和最多满足1/R^{k-1-ε}约束是NP困难的，改进了先前的结果。


<details>
  <summary>Details</summary>
Motivation: 改进Chan在2016年提出的较弱结果，该结果只能区分满足1-ε约束和O(k/R^{k-2})约束的情况，目标是获得更紧密的近似下界。

Method: 采用作者近期工作的方法，主要新贡献是格拉斯曼图中伪随机集合间超边的计数引理。

Result: 证明了在大字母表R下，区分k-CSP满足至少1-ε约束和最多1/R^{k-1-ε}约束是NP困难的，这几乎匹配了平凡近似算法的性能。

Conclusion: 该结果显著改进了先前工作，为k-CSP问题提供了更紧密的近似下界，其中超边计数引理可能具有独立的研究价值。

Abstract: We show that for every $k\in\mathbb{N}$ and $\varepsilon>0$, for large enough
...
### [243] [Reachability of Independent Sets and Vertex Covers Under Extended Reconfiguration Rules](https://arxiv.org/abs/2510.24226)
*Shuichi Hirahara,Naoto Ohsaka,Tatsuhiro Suga,Akira Suzuki,Yuma Tamura,Xiao Zhou*

Main category: cs.CC

TL;DR: 本文研究了独立集重构(ISR)和顶点覆盖重构(VCR)问题在k-Token Jumping(k-TJ)和k-Token Sliding(k-TS)模型下的计算复杂度。证明了ISR在k = |I| - μ时对固定μ值保持NP难，而VCR在参数μ = |S| - k时属于XP类。同时建立了多个图类上ISR和VCR的PSPACE完全性。


<details>
  <summary>Details</summary>
Motivation: 重构问题研究两个可行解之间通过一系列可行中间解的转换可能性。近期研究从单元素修改扩展到不同重构规则对计算复杂度的影响，本文旨在进一步探索ISR和VCR问题在不同k值下的计算复杂度梯度。

Method: 采用k-TJ和k-TS两种重构模型：k-TJ允许同时替换最多k个顶点，k-TS额外要求被移除和添加顶点之间存在完美匹配。通过参数化分析和复杂度归约技术，在不同图类上建立复杂度边界。

Result: 1) ISR在k = |I| - μ时对固定μ值在最大度3图或最大度4平面图上保持NP难；2) ISR对μ = O(log|I|)属于NP类；3) VCR在参数μ = |S| - k时属于XP类；4) 多个图类上ISR和VCR对固定k和超常数k都是PSPACE完全的。

Conclusion: 重构问题的计算复杂度对参数k高度敏感，呈现从多项式时间可解到NP难再到PSPACE完全的复杂梯度。不同重构模型和参数设置导致显著不同的复杂度特征，为理解重构问题的计算边界提供了系统框架。

Abstract: In reconfiguration problems, we are given two feasible solutions to a graph
